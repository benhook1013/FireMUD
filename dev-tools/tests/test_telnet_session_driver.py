...
class TelnetSessionDriverTest(unittest.TestCase):
    ...
    def test_allow_insecure_accepts_only_localhost_equivalents(self):
    ...
    def test_allow_insecure_rejects_remote_and_private_hosts(self):
        parser = telnet_session.build_parser()
        for host in (
            "preview.example",
            "192.168.1.25",
            ...
            with self.subTest(host=host):
                ...
                self.assertIn(telnet_session.INSECURE_HOST_ERROR, stderr.getvalue())

    def test_direct_insecure_session_rejects_non_loopback_before_connect(self):
        with tempfile.TemporaryDirectory() as directory:
            session = telnet_session.TelnetSession(
                "192.168.1.25",
                32000,
            ...
            with (
                patch.object(telnet_session.socket, "create_connection") as create_connection,
                self.assertRaisesRegex(
                    ValueError, f"^{telnet_session.INSECURE_HOST_ERROR}$"
                ),
            ):
                session.connect()
    ...
    def test_connect_port_is_validated_before_connection_dispatch(self):
        ...
        for invalid in ("not-an-integer", "-1", "0", "65536"):
            with self.subTest(invalid=invalid):
                ...
                with (
                    ...
                    telnet_session.main([*base_args, "--port", invalid])
                self.assertEqual(raised.exception.code, 2)
                self.assertIn(
        ...
        for valid in ("1", "65535"):
            with self.subTest(valid=valid):
                with patch.object(
                    telnet_session, "run_connect", return_value=0
                ) as run_connect:
                    self.assertEqual(
                        telnet_session.main([*base_args, "--port", valid]), 0
                    )
                run_connect.assert_called_once()
                self.assertEqual(run_connect.call_args.args[0].port, int(valid))
    ...
    def test_run_connect_rejects_non_loopback_insecure_before_connect(self):
        with tempfile.TemporaryDirectory() as directory:
            args = argparse.Namespace(
                host="10.0.0.8",
                port=32000,
            ...
            with (
                ...
                self.assertEqual(telnet_session.run_connect(args), 2)
            self.assertIn(telnet_session.INSECURE_HOST_ERROR, stderr.getvalue())
            create_connection.assert_not_called()
    ...
    def test_logon_alias_redacts_secret_with_exact_transmitted_bytes(self):
        command = "LOGON demo@example.com secret-7"

        safe, raw, replacement = telnet_session._login_redaction(command)

        self.assertEqual(safe, "LOGON demo@example.com [REDACTED]")
        self.assertEqual(raw, command.encode("iso-8859-1"))
        self.assertEqual(replacement, safe.encode("iso-8859-1"))
        self.assertNotIn(b"secret-7", replacement)
