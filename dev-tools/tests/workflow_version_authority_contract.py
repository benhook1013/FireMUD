...
def main() -> int:
    ...
    for name, (entries, includes) in locks.items():
        if name == "ci" and includes != ["yaml-requirements.txt"]:
            fail("ci requirements must include its canonical YAML lock exactly once")
        if name != "ci" and includes:
            fail(f"{name} requirements must not include another profile")
    if "pyyaml" in locks["ci"][0]:
        fail("PyYAML must have one canonical requirements authority")
