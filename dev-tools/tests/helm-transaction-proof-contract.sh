...
if "docker exec firemud-helm-proof k3s kubectl get --raw='/livez' --request-timeout=2s" not in job_text:
    raise SystemExit("Helm transaction proof must wait on the disposable API server liveness condition")
