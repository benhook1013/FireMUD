...
- `validation/report-pr-review-checkpoints.py` – read-only, paginated extraction of existing Hosted and CLI checkpoint comments for PR reports. For example: `python3 dev-tools/validation/report-pr-review-checkpoints.py --repo owner/name --pr 123`; use `--json` for machine-readable output, `--details <comment-id>` to validate a linked CLI capture, or `--rejections N` to inspect recorded rejections from the latest N CLI checkpoints. The overview interleaves standardized scope-change timeline comments; see [PR lifecycle](../design/developer-workflows/pr-lifecycle.md) for checkpoint markers and rejection-record rules.
