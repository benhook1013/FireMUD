...
- `validation/report-worktree-pr-topology.sh` – read-only local worktree, branch, and open-PR inventory. Use `--pr N` for a bounded exact branch/SHA selected-stack report and `--json` for machine-readable output; Renovate heads are excluded unless `--include-renovate` is supplied. Ambiguous branch or PR identities fail closed. Its focused proof is [worktree-topology-contract.sh](tests/worktree-topology-contract.sh).
