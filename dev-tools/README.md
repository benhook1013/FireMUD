...
- `validation/report-pr-review-checkpoints.py` – read-only, paginated extraction of existing Hosted and CLI checkpoint comments for PR reports. For example: `python3 dev-tools/validation/report-pr-review-checkpoints.py --repo owner/name --pr 123`; use `--json` for machine-readable output, `--details <comment-id>` to validate linked local evidence, `--hosted [REVIEW_ID]` to save or inspect an actual completed Hosted review, or `--rounds N --disposition all|accepted|rejected` to inspect the latest N combined Hosted and CLI rounds in chronological order. Add `--source cli|hosted` for explicit filtering (`--rejections N` is the concise CLI rejected-only form). The overview interleaves standardized scope-change timeline comments; see [PR lifecycle](../design/developer-workflows/pr-lifecycle.md) for checkpoint markers, saved decisions, and evidence limitations.
- `validation/report-pr-status.py` – normal complete, read-only single-PR report that composes the checkpoint reporter, CodeRabbit checker, and compact GitHub PR metadata read. For example: `python3 dev-tools/validation/report-pr-status.py --repo owner/name --pr 123`; use `--json` for machine-readable evidence. Use the lower-level reporters above for deeper inspection.
...
- `maintenance/update-workflow-tool.py` – updates a downloaded workflow tool's canonical version and Linux checksum together. Renovate uses the `github-release-attachments` datasource for kubectl, Helm, GitHub CLI, Buf, Kubeconform, Trivy, and Lychee, replacing each version, checksum-version, and SHA as one authority block; CI still rejects any unpaired bump. Actionlint remains version-only because it has no archive checksum authority. Velero retains Renovate image updates as a release signal, but its image digest and CLI archive checksum are separate authorities; every Velero Renovate PR must run `python3 dev-tools/maintenance/update-workflow-tool.py velero <version> --image-evidence-file <path>` for its transaction before merge. An authority-only update leaves the production CronJob at its known-good tag until a later PR supplies the canonical promotion attestation; the updater's production-manifest projection is accepted only with that evidence. An offline `--checksum-file` must begin with `version=<version>` on its own line, followed by the publisher manifest; the version is checked before any checksum is extracted. Velero evidence must contain one exact immutable `velero/velero:v<version>@sha256:<digest>` reference, which the updater verifies against Docker Hub before preparing the authority and production manifest as one rollback-protected transaction; committing the production projection requires the canonical promotion attestation.
