...
- `pr-review` – the sole review controller. Use `dev-tools/pr-review stack set <pr>...` and `stack show` for the one repository review stack, `status [--pr <pr>]` for fail-closed live state, `run hosted|cli [--expect-pr <pr>]` for a complete checkpoint, `evidence [<pr>]` for anchored evidence, and `decide ...` for explicit policy or head judgments. It stores only configuration and judgments in a locked private common-directory state file; live PR, CI, checkpoint, and review evidence is derived when requested. The implementation and focused contracts live in `pr_review/` and `tests/pr-review-controller-contract.sh`.
