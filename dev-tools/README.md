...
- `maintenance/update-workflow-tool.py` – updates a downloaded workflow tool's canonical version and Linux checksum together. Renovate can propose version bumps but cannot derive GitHub release-asset checksums; CI therefore rejects an unpaired bump. Run `python3 dev-tools/maintenance/update-workflow-tool.py <tool> <version>` to complete it from the publisher's checksum manifest. An offline `--checksum-file` must begin with `version=<version>` on its own line, followed by the publisher manifest; the version is checked before any checksum is extracted. Velero updates also resolve its image digest and prepare the authority and production manifest as one rollback-protected transaction; committing the production projection requires the canonical promotion attestation.
