...
RECOVERY_EXTERNAL_CREDENTIAL_CLASSES = (
    "backup-storage",
    "asset-storage",
    "outbound-comms",
    "operator-credentials",
...
def parse_evidence_timestamp(value: Any, label: str) -> tuple[dt.datetime | None, str | None]:
    if not value:
...
def load_production_recovery_record(
    path_ref: Any,
    label: str,
    root_dir: Path,
    ...
    *,
    ...
    if recovery.get("recoveryStatus") != expected_status:
        return (
            "fail",
            f"Production {label} recoveryStatus must be {expected_status}",
            None,
        )
    if recovery.get("recoveryPurpose") != expected_purpose:
        return (
            "fail",
            f"Production {label} recoveryPurpose must be {expected_purpose}",
            None,
        )
    if recovery.get("trafficExposure") != expected_exposure:
        return (
            "fail",
            f"Production {label} trafficExposure must be {expected_exposure}",
            None,
        )
    if recovery.get("coordinationRecoveryMode") != "cold_start_restore":
        return (
            "fail",
            f"Production {label} must prove coordinationRecoveryMode=cold_start_restore",
            None,
        )
    if recovery.get("backupArtifactRef") != traffic.get("backupArtifactRef"):
        return ("fail", f"Production {label} backupArtifactRef mismatch", None)
    for key in ("backupToolDigest", "recoveryToolDigest", "recoveryContractFingerprint"):
        if recovery.get(key) != traffic.get(key):
            return ("fail", f"Production {label} {key} mismatch", None)
    if recovery.get("sourceEnvironmentBinding") != traffic.get("sourceEnvironmentBinding"):
        return ("fail", f"Production {label} sourceEnvironmentBinding mismatch", None)
    if recovery.get("targetBoundary") != expected_target_boundary:
        return ("fail", f"Production {label} targetBoundary mismatch", None)
    if recovery.get("expectedBindingsRef") != "design/operations/environments/production/expected-bindings.yaml":
        return ("fail", f"Production {label} expectedBindingsRef mismatch", None)

    for key in RECOVERY_CONTROL_GROUP_FIELDS:
        value = recovery.get(key)
        if not isinstance(value, dict) or not value:
            return ("fail", f"Production {label} missing {key}", None)
        if value.get("status") != "pass":
            return ("fail", f"Production {label} {key} status must be pass", None)

    if not isinstance(recovery.get("backupArtifactLineage"), dict):
        return ("fail", f"Production {label} backupArtifactLineage must be an object", None)
    if not isinstance(recovery.get("restoreSafeMode"), dict):
        return ("fail", f"Production {label} restoreSafeMode must be an object", None)
    if recovery.get("smokeStatus") != "pass":
        return ("fail", f"Production {label} smokeStatus must be pass", None)
    session_recovery = recovery.get("sessionRecovery")
    if (
        not isinstance(session_recovery, dict)
        or session_recovery.get("gameSessionHandling") != "invalidated"
        or session_recovery.get("authSessionHandling") != "invalidated"
    ):
        return ("fail", f"Production {label} sessionRecovery must invalidate gameplay and Account sessions", None)

    external_validation = recovery.get("externalCredentialValidation")
    credential_records = external_validation.get("records")
    if not isinstance(credential_records, dict):
        return ("fail", f"Production {label} externalCredentialValidation missing records", None)
    for credential_class in RECOVERY_EXTERNAL_CREDENTIAL_CLASSES:
        record = credential_records.get(credential_class)
        if not isinstance(record, dict):
            return (
                "fail",
                f"Production {label} externalCredentialValidation missing {credential_class}",
                None,
            )
        for key in (
            "status",
            ...
            if not record.get(key):
                return (
                    "fail",
                    f"Production {label} externalCredentialValidation.{credential_class} missing {key}",
                    None,
                )
        if record.get("status") != "pass":
            return (
                "fail",
                f"Production {label} externalCredentialValidation.{credential_class} status must be pass",
                None,
            )

    lifecycle_timestamp_fields = ["quarantineStartedAt", "restoredAt"]
    if expected_status == "ready_to_reopen":
        lifecycle_timestamp_fields.append("readyToReopenAt")
        if not recovery.get("reopenApprovedBy"):
            return ("fail", f"Production {label} missing reopenApprovedBy", None)
        preflight_ref = recovery.get("preflightReportPath")
        if not preflight_ref:
            return ("fail", f"Production {label} missing preflightReportPath", None)
        if str(preflight_ref) != str(traffic.get("preflightReportPath")):
            return ("fail", f"Production {label} preflightReportPath mismatch", None)
    else:
        lifecycle_timestamp_fields.extend(["readyToReopenAt", "quarantineReleasedAt", "finalizedAt"])
        if not recovery.get("reopenApprovedBy"):
            return ("fail", f"Production {label} missing reopenApprovedBy", None)

    lifecycle_timestamps: dict[str, dt.datetime] = {}
    for key in lifecycle_timestamp_fields:
        parsed, error = parse_evidence_timestamp(recovery.get(key), f"Production {label} {key}")
        if error:
            return ("fail", error, None)
        assert parsed is not None
        lifecycle_timestamps[key] = parsed
    if lifecycle_timestamps["restoredAt"] < lifecycle_timestamps["quarantineStartedAt"]:
        return ("fail", f"Production {label} restoredAt precedes quarantineStartedAt", None)
    if "readyToReopenAt" in lifecycle_timestamps and lifecycle_timestamps["readyToReopenAt"] < lifecycle_timestamps["restoredAt"]:
        return ("fail", f"Production {label} readyToReopenAt precedes restoredAt", None)
    if expected_status == "finalized":
        if lifecycle_timestamps["quarantineReleasedAt"] < lifecycle_timestamps["readyToReopenAt"]:
            return ("fail", f"Production {label} quarantineReleasedAt precedes readyToReopenAt", None)
        if lifecycle_timestamps["finalizedAt"] < lifecycle_timestamps["quarantineReleasedAt"]:
            return ("fail", f"Production {label} finalizedAt precedes quarantineReleasedAt", None)
...
def production_traffic_check(
    ...
    traffic_open_status = data.get("trafficOpenStatus")
    allowed_statuses = {"ready_to_reopen", "finalized"} if event == "reopen" else {"finalized"}
    if traffic_open_status not in allowed_statuses:
        return (
            "fail",
            "Production traffic-open evidence trafficOpenStatus must be "
            + " or ".join(sorted(allowed_statuses)),
        )
    if data.get("trafficExposure") != "isolated-drill":
        return ("fail", "Production traffic-open evidence trafficExposure must be isolated-drill")
    if traffic_open_status == "finalized":
        if not data.get("trafficOpenedAt"):
            return ("fail", "Production traffic-open evidence missing trafficOpenedAt")
        _, traffic_opened_error = parse_evidence_timestamp(
            data.get("trafficOpenedAt"), "Production traffic-open evidence trafficOpenedAt"
        )
        if traffic_opened_error:
            return ("fail", traffic_opened_error)
    now = dt.datetime.now(dt.timezone.utc)
    for key, max_age, label in (
        ("backupLastSuccessAt", 90 * 60, "backup"),
        ("backupVerifyLastSuccessAt", 36 * 60 * 60, "backup verification"),
        ("restoreDrillLastSuccessAt", 30 * 24 * 60 * 60, "restore drill"),
    ):
        parsed, error = parse_evidence_timestamp(data.get(key), f"Production traffic-open evidence {key}")
        if error:
            return ("fail", error)
        assert parsed is not None
        age = (now - parsed).total_seconds()
        if age < 0 or age > max_age:
            return ("fail", f"Production {label} evidence is stale or from the future: {key}")
    try:
    ...
    if backup_readiness.get("deploymentRef") != deployment_ref:
        return ("fail", "Production backup-readiness evidence deploymentRef mismatch")
    for key in ("backupLastSuccessAt", "backupVerifyLastSuccessAt", "restoreDrillLastSuccessAt"):
        if backup_readiness.get(key) != data.get(key):
            return ("fail", f"Production backup-readiness evidence {key} mismatch")
    readiness_status, readiness_message, _ = load_production_recovery_record(
        backup_readiness.get("restoreRecoveryRecordRef"),
        "backup-readiness restore recovery record",
        root_dir,
        data,
        expected_status="finalized",
        expected_purpose="production-equivalent-drill",
        expected_exposure="isolated-drill",
        expected_target_boundary=data.get("drillTargetBoundary"),
    )
    if readiness_status != "pass":
        return ("fail", readiness_message)
    baseline_status, baseline_message, _ = load_production_recovery_record(
        data.get("baselineRecoveryRecordRef"),
    ...
    if event == "reopen":
        if not data.get("actualRecoveryRecordRef"):
            return ("fail", "Production traffic-open evidence missing actualRecoveryRecordRef")
        if not data.get("playerFacingTargetBoundary"):
            return ("fail", "Production traffic-open evidence missing playerFacingTargetBoundary")
        actual_status, actual_message, _ = load_production_recovery_record(
            data.get("actualRecoveryRecordRef"),
            "actual recovery controller",
            root_dir,
            data,
            expected_status=str(traffic_open_status),
            expected_purpose="actual-recovery",
            expected_exposure="player-facing-reopen",
            expected_target_boundary=data.get("playerFacingTargetBoundary"),
        )
        if actual_status != "pass":
            return ("fail", actual_message)
