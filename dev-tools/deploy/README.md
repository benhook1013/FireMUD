...
- `bootstrap-account-response-envelope-ring.py`
  - Unactivated initial-source bootstrap candidate for a trusted operator, not a normal deploy or PR job. It requires an absolute output file outside this checkout in a pre-existing operator-owned `0700` custody directory, an exact environment and namespace, and an explicit approved `--source-ttl-seconds`; it creates one `0600` source record with distinct random purpose keys, a random key ID and generation, and an immutable source-created freshness anchor. It refuses an existing output rather than regenerating on retry and prints no key bytes. The custody operator must separately establish durable protected storage, backup, access control, and audit; this helper alone does not prove them. Rotation and proven safe retirement of decrypt-only keys need separate owner evidence.
