...
- `materialize-account-response-envelope-ring.py`
  - Unactivated trusted-writer candidate for the Account-only `account-response-envelope-key-ring` Secret. It reads one already-created, owner-only, environment-specific source record (`--source-record`) that binds version 1, canonical Base64 for the exact `manifest.v1` bytes, opaque `sourceGeneration`, and `sourceExpiresAt`; the trusted controller supplies the credential-class `--class-max-age-seconds` policy. It never generates key material. An exact same-generation retry is read-only, while a replacement uses Kubernetes resource-version compare-and-swap, introduces a new active key ID, preserves all prior decrypt-only key material, and verifies the exact Secret bytes and ADR 0032 freshness annotations after the write.
  - This tool is not wired into preview, dev-demo, or player-facing deployment and does not itself establish durable custody, source-generation ordering, sole-writer RBAC, a freshness-enforcing workload readiness actor, or live rotation/rollout proof. Do not publish the Account Secret mount or enable either credential-envelope producer until those gates are complete; a missing source or Secret is an activation failure, not a reason to generate fresh keys during deployment.
