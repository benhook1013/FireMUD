...
The client requires a POSIX environment because it uses `fcntl` file locking
and `select` on stdin. Linux and WSL are supported; native Windows Python is
not supported.
...
Use `--ca-file /path/to/ca.pem` to replace the system trust roots with a custom
CA bundle, or `--server-hostname name` to override the TLS SNI and hostname-check
