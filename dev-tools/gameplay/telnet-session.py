...
def _is_loopback_host(host: str) -> bool:
    """Allow plaintext only for literal localhost-equivalent destinations."""
...
class _TransportArgumentParser(argparse.ArgumentParser):
    ...
    def parse_args(self, args=None, namespace=None):
        ...
        if not _is_loopback_host(parsed.host):
            self.error(INSECURE_HOST_ERROR)
        return parsed
...
def _login_redaction(command: str) -> tuple[str, bytes, bytes] | None:
    ...
    if not match or match.group(3) is None:
        return None
    safe = f"{match.group(1).upper()} {match.group(2)} [REDACTED]"
...
class TelnetSession:
    ...
    def connect(self) -> None:
        if not self.tls_enabled and not _is_loopback_host(self.host):
            raise ValueError(INSECURE_HOST_ERROR)
        try:
...
def run_connect(args: argparse.Namespace) -> int:
    ...
    if allow_insecure and not _is_loopback_host(args.host):
        print(f"Unable to use insecure raw TCP: {INSECURE_HOST_ERROR}", file=sys.stderr)
        return 2
    if allow_insecure:
        print("WARNING: using insecure raw TCP; localhost/loopback use only.")
