...
- Never manually resolve CodeRabbit threads. Unresolved current or outdated threads block merge; once every finding is verified fixed in `HEAD`, rerun `@coderabbitai review` so CodeRabbit self-resolves them. Do not rerun while a review is active or CodeRabbit is rate limited.
