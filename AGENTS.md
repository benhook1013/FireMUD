...
In an explicitly assigned Overseer/Worker arrangement, the Overseer owns project direction, priorities, consequential scope and architecture decisions, evidence adjudication, coordination, and user communication. The Overseer delegates operational investigation, edits, and validation to workers or subagents; the detailed boundaries are in [AI delegation and review](design/developer-workflows/ai-delegation-and-review.md). Workers own assigned-lane planning, execution, integration, validation, and PR/review operations and continue within the authorized scope without step-by-step approval. Without that arrangement, the main worker retains those responsibilities and human design discussion; it need not create an Overseer task. Roles are assigned explicitly, not inferred from model choice. Product and architecture decisions remain with the assigned main decision-maker and are not delegated to execution subagents.

Subagent approval is model-specific: `gpt-5.6-sol` may run at medium autonomously, but using it at high requires explicit human approval. Luna reasoning levels are not approval-gated.
