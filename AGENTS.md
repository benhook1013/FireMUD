...
- Merge a PR autonomously once it has had at least one substantive CodeRabbit review, every current and outdated thread is resolved, required CI is green, and every later commit directly addresses that review. A fresh CodeRabbit run is required before merge if any later commit adds functionality or changes scope beyond review fixes.
- Keep open PRs meaningful: after that merge threshold, merge rather than leaving completed PRs parked. Open PRs should normally be awaiting CI, CodeRabbit, or actionable review fixes so the review queue is visible to the operator.
