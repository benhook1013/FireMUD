...
Helm treats same-name `Secret/jwt-jwks` and `ConfigMap/jwt-jwks` objects as distinct resource identities, so an upgrade creates the target ConfigMap and removes the obsolete Secret without a manual pre-delete. The player-facing Kustomize Secret contract remains separate.
