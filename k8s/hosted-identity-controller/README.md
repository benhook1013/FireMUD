...
### Live egress evidence before activation

From the approved operator context, retain a non-secret matrix of network reachability results from the actual controller Pod after the exact-address policy is installed in `observe` mode. The loopback-only paused policy cannot establish positive egress proof. Record the Pod UID, policy resource version, observed source and destination addresses/ports, DNS answer, command exit status, timestamp, and the exact controller image digest. Do not record a kubeconfig, token, Secret, TLS key, request body, or full Pod manifest. Network success means the expected destination was reached; it does not substitute for the controller's separate TLS/SNI/SAN/issuer checks.

The positive matrix covers `kubernetes.default:443` and the observed API endpoint `:6443`, kube-dns TCP/UDP 53, the selected Gateway ClusterIP and post-DNAT Pod target `:8443`, Account `:6565`, public Traefik HTTPS `:8443`, and, only in public mode, the allocated TCP Proxy NodePort and post-DNAT Pod target `:2323`. The negative matrix covers the same ports on a namespace without the canonical preview/dev-demo label and on Pods without the exact workload labels, an unallocated NodePort, an unrelated external address on 443, and a known-open local-node port outside the API allowance. For every denied target, first prove it is reachable from a separately isolated control Pod with no controller policy label; a refusal from a closed destination is not denial evidence. Preserve the control Pod's identity and cleanup result. Test the local-node case explicitly: if the CNI bypasses NetworkPolicy for local-node traffic, the operator must establish and prove a separate host-level restriction before activation rather than reporting the NetworkPolicy as sufficient. Run the matrix again after any CNI, Service, label, NodePort, or policy change.

The operator records `allowed`, `denied`, or `unproved` per row with the expected and observed result. Any unexpected reachability or `unproved` negative row blocks `active` activation and PO-3.2 completion. This is an execution checklist, not a claim that the current cluster has passed it; no live matrix has been run for this revision.
