...
- `ListScriptScheduleInstances` – durable instance-scoped scheduler materialization read for one `(tenantId, gameInstanceId)` scope, exposing the currently pinned patch's projected schedule rows, cadence, materialization status, due-point fields, and observed pin/runtime metadata without forcing operators to infer scheduler substrate from Redis.
