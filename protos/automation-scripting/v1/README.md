...
- `ListScriptHandoffEvents` – per-command handoff history for durable work items, including `automationDispatchId`, target entity, resolved `playableStateScope`, rendered emitted command text, Game Session command id, and handoff outcome/reason so richer multi-command output remains debuggable after handoff.
