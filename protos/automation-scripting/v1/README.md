...
- `ListScriptScheduleInstances` – durable instance-scoped scheduler materialization read for one `(tenantId, gameInstanceId)` scope, exposing the currently pinned patch's projected schedule rows, plugin owner metadata, linked Game Design script-patch publication metadata, resolved `playableStateScope`, binding target identity, cadence, materialization status, due-point fields, observed pin/runtime metadata, and explicit `isPinStale` / `isRuntimeProgressStale` flags without forcing operators to infer scheduler substrate from Redis.
- `ListScriptTimerAuditEvents` – append-only scheduler timer audit history, including linked Game Design script-patch publication metadata so operators can inspect catch-up truncation and runtime-scope fence outcomes without separately joining the design-time patch state.
...
- `ListScriptHandoffEvents` – per-command handoff history for durable work items, including `automationDispatchId`, target entity, resolved `playableStateScope`, rendered emitted command text, Game Session command id, handoff outcome/reason, and linked Game Design script-patch publication metadata so richer multi-command output remains debuggable after handoff without reconstructing design-time patch truth separately.
