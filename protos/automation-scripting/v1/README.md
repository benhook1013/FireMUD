...
- `GetAutomationDrainStatus` – current rollback-drain visibility for one `(tenantId, gameInstanceId[, regionId])` scope, reporting `admissionMode`, `admissionEpoch`, active executions, cancelable pending work, and `observedAtMs` from durable admission/work-item state.
...
- `ListScriptScheduleInstances` – durable instance-scoped scheduler materialization read for one `(tenantId, gameInstanceId)` scope, exposing the currently pinned patch's projected schedule rows, plugin owner metadata, resolved `playableStateScope`, binding target identity, cadence, materialization status, due-point fields, and observed pin/runtime metadata without forcing operators to infer scheduler substrate from Redis.
