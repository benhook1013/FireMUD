...
- `scriptEventId` is generated by the caller (typically the Game Session Service) and uniquely identifies a trigger within the Trigger Identity scope (at least including `<tenantId, regionId, entityId, eventType, scriptPatchVersion>` for entity-scoped events).
...
- Event ingress RPCs (for example, `TriggerScriptEvent`) – **idempotent with respect to `scriptEventId`**:
  ...
  - For entity-scoped external events, the Trigger Identity is at least `<tenantId, regionId, entityId, scriptId, eventType, scriptPatchVersion, scriptEventId>`.
