...
- `ListScriptScheduleInstances` – durable instance-scoped scheduler materialization read for one `(tenantId, gameInstanceId)` scope, exposing the currently pinned patch's projected schedule rows, plugin owner metadata, resolved `playableStateScope`, binding target identity, cadence, materialization status, due-point fields, observed pin/runtime metadata, and explicit `isPinStale` / `isRuntimeProgressStale` flags without forcing operators to infer scheduler substrate from Redis.
