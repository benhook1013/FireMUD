...
- `GetPluginPolicyConvergence` reports enabled plugin runtime states whose current Game Design publication, signer, or component-policy metadata would now fail closed, and now carries the linked publication metadata on each violation so operators can see the failing publication truth without a second join.
