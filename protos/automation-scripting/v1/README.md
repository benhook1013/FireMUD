...
- `GetAutomationPinConvergence` – current pin-observation visibility for one `(tenantId, gameInstanceId)` scope, reporting the observed pinned patch, persisted `lastObservedControlPlaneRequestId`, observation time, and projection freshness flags (`projectionAsOfMs`, `projectionLagMs`, `isProjectionStale`) from Automation’s durable pin projection.
