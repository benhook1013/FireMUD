...
- `ListScriptScheduleInstances` – durable instance-scoped scheduler materialization read for one `(tenantId, gameInstanceId)` scope, exposing the currently pinned patch's projected schedule rows, plugin owner metadata, linked Game Design script-patch publication metadata, linked Game Design plugin publication metadata when a plugin version owns the row, resolved `playableStateScope`, binding target identity, cadence, materialization status, due-point fields, observed pin/runtime metadata, and explicit `isPinStale` / `isRuntimeProgressStale` flags without forcing operators to infer scheduler substrate from Redis.
- `ListScriptTimerAuditEvents` – append-only scheduler timer audit history, including linked Game Design script-patch publication metadata and linked Game Design plugin publication metadata when present so operators can inspect catch-up truncation and runtime-scope fence outcomes without separately joining the design-time patch or plugin state.
