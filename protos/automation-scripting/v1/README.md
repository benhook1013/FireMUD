...
- `ListScriptHandoffEvents` – per-command handoff history for durable work items, including `automationDispatchId`, target entity, resolved `playableStateScope`, rendered emitted command text, Game Session command id, handoff outcome/reason, linked Game Design script-patch publication metadata, and linked Game Design plugin publication metadata when a plugin-owned row is involved so richer multi-command output remains debuggable after handoff without reconstructing design-time patch or plugin truth separately.
