...
- For focused shell-script checks, from the repository root run `shellcheck -x <scripts>` so sourced repository helpers are followed; do not suppress `SC1091` or change global ShellCheck configuration or CI.
