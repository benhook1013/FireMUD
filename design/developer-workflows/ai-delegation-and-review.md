...
- Subagents may not run Gradle, Docker, smoke, or repository-wide validation unless the main thread delegates one named command. They must not replace a prohibited canonical command with ad hoc flags or a hand-built equivalent; run a repository-configured narrow command when one exists, otherwise report the check as deferred. The main thread runs consolidated validation after integration.
