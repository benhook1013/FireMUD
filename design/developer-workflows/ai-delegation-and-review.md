...
- Subagents may not run Gradle, Docker, smoke, or repository-wide validation unless the main thread delegates one named command. A repository-configured narrow validation command may run only when the main thread delegates that named command; otherwise report the check as deferred. Subagents must not invent substitute commands or ad hoc flags, and must not replace a delegated canonical command with a hand-built equivalent. The main thread runs consolidated validation after integration.
