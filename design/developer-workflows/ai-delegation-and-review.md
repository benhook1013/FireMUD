...
- Subagents must never invoke CodeRabbit CLI or hosted review, post review commands, start or stop CodeRabbit, trigger or rerun CI, resolve review threads, or otherwise alter review state. They must not commit, push, retarget branches, open or merge PRs, or close PRs unless the main thread explicitly delegates that exact action and any human authorization required by `AGENTS.md` is present. Delegation never authorizes merging or closing a PR without that explicit human authorization, and every delegation prompt must repeat this prohibition. They must never revert, delete, overwrite, stage, or commit concurrent edits outside their assigned file set. Preserve unrelated dirty edits and continue; stop only for overlapping assigned-file edits or unclear or risky intent, and report the overlap or ambiguity.
