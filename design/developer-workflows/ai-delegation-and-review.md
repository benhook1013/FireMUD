...
- Subagents must not commit, push, retarget branches, open or merge PRs, close PRs, trigger or rerun CI, post review commands, invoke CodeRabbit CLI, start or stop hosted CodeRabbit, or mutate review threads or review comments (including comment, edit, dismiss, resolve, or reopen) unless the main thread explicitly delegates that exact action and the action has any human authorization required by `AGENTS.md`. Delegation never authorizes merging or closing a PR without that explicit human authorization. Every delegation prompt must repeat this prohibition; omitting it never grants permission. They must never revert, delete, overwrite, stage, or commit concurrent edits outside their assigned file set; report an overlap and stop instead.
