...
- Use `gpt-5.6-luna` at `xhigh` reasoning as the minimum subagent invocation. Use Luna at `max` when the task is known to require more intelligence; the model may stop reasoning early when the allowance is unnecessary.
- Escalate beyond Luna to `gpt-5.6-sol` at `medium`. Sol at `high` is the absolute subagent ceiling and still requires explicit human approval under root guidance. Do not use Terra as a normal routing tier or delegate product or architecture design decisions.
