...
- Canonical file-count guidance is to prefer natural 50-70-file review boundaries for broad alignment or other file-heavy work, remain under the active 100-file ceiling, and leave substantial headroom for valid review fixes. Do not open a tiny artificial extraction PR merely to cross the ceiling. When two coherent domains can be separated, use a balanced split rather than preserving one majority PR; if no natural split exists, keep the work together and use the available CLI or scoped-review path. The ceiling changes review topology, never which valid findings are fixed.
