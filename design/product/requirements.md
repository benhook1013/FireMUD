...
- Account must support a protective `account_security_lock` for a compromise assessment supported by verified or high-confidence evidence, an explicit Account security policy, or an audited operator action. The Account-owned lock revokes ordinary account/bootstrap authority and is cleared only through Account security recovery; it is not a punitive moderation category.
