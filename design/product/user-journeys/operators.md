...
The service also exposes moderation tools such as reports, bans, and runtime feature toggles. Administrators review logs and configure these options through the [Admin UI](../../architecture/microservices/logging-admin-service/admin-ui.md). Policies are summarized in the [Moderation Policies](../../architecture/microservices/logging-admin-service/moderation-policies.md) document. Short synchronous moderation orchestration uses the shared saga helpers, while durable control-plane workflows use Temporal as described in [Transaction Strategies](../../architecture/system-architecture-transactions.md). Review these guides alongside the [Security Architecture](../../architecture/system-architecture-security.md) to ensure moderation actions follow platform rules.
...
Operator tooling should also support revoking player visibility/admission for a fork without deleting the fork outright, so support, moderation, or incident-response workflows can hide a fork temporarily while preserving its fork-local state and audit history. The canonical mechanism is the same explicit Account-owned realm-access grant contract used by creator flows and authentication/discovery: revoke only the tenant-qualified grant record for the affected account(s), while leaving the fork realm and its fork-local state intact. Current grant revocation exists as a partial implementation capability. Expiry/`expiresAt`-driven grant management and lifecycle automation remain target/future behavior, not current implementation proof; see the [Player Access and Session implementation tracker](../../project-management/implementation-tracking/player-access-and-session.md#active-gaps).
**Target behavior, not yet fully implemented:** revoking or expiring a `{tenantId, worldSlug, realmSlug, accountId}` grant removes only that account's authority for the affected fork, terminates its connected fork sessions, and blocks subsequent discovery, reconnect, and `PLAY` for that realm. Account-wide bootstrap and access to unrelated tenants, worlds, and realms remain available. When a creator or operator wants a friendly scheduled playtest ending, they close and drain the realm first and revoke grants afterward; access revocation itself is not overloaded as a drain request.
