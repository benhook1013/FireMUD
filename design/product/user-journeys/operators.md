...
- **Moderation ingress:** Current report creation uses the physically reachable internal service-to-service `CreateReport` seam, which remains unsupported and unproved. Public/player report ingress is currently unavailable until its caller-bound contract exists. The target report and operator-write contracts are defined in the [Logging & Admin API status](../../architecture/microservices/logging-admin-service/api-contracts.md#implementation-status) and the [Platform Operations tracker](../../project-management/implementation-tracking/platform-operations-and-delivery.md#capability-status).
- **Safety and appeals:** The target fixed categories, owner-local enforcement, and bounded appeal case are defined in [Moderation Policies](../../architecture/microservices/logging-admin-service/moderation-policies.md). Current category persistence, owner commands, player notices, and appeal workflow are not implemented or proved. Account is the target enforcement owner for `platform_access_ban` and separately owns the current `security_locked` foundation plus target `account_security_lock` security/recovery; the separate punitive platform-ban enforcement is not current proof. Logging & Admin owns moderation appeals, while Game Session and Social & Groups remain target enforcement owners for their categories.
...
- **Grant revocation:** Current Account grant persistence and internal lookup can support a grant-backed admission read, but target expiry, revision/tombstone mutation, tenant-admin and break-glass commands, and active-binding ejection are not implemented or end-to-end proved. The target grant contract is canonical in [Account API Contracts](../../architecture/microservices/account-service/api-contracts.md#account-owned-playtest-grant-contract). Current text `PLAY` still consumes the existing membership/admission fields; non-public missing or non-admitting membership remains the current `WORLD_ACCESS_DENIED` path. Target denial of discovery, reconnect, and `PLAY`, together with termination of affected fork bindings, applies after grant expiry/revocation only once that target enforcement is implemented; unrelated account access remains available. Expiry and lifecycle automation remain gaps.
...
Operators monitor the game and enforce rules using the [Logging & Admin Service](../../architecture/microservices/logging-admin-service/README.md) together with the observability paths declared by the selected deployment profile. The default indexed profile uses **Elasticsearch** for log search; profiles that install metrics add **Prometheus**, and profiles that install tracing add **Jaeger**. Compatible profiles document equivalent mappings, while reduced profiles may use console/journal retrieval or omit indexed search. The current Logging & Admin service has no observability-backend clients or separate Admin UI and its live log query reads only PostgreSQL `log_events`; these integrations remain target/profile-specific. For target-state usage examples see the [Analytics Dashboards](../../architecture/microservices/logging-admin-service/analytics-dashboards.md).

The current investigation surface is backend-only and includes read/investigation tooling. The physically reachable internal `CreateReport` report-persistence seam remains unsupported and unproved, while public report persistence is unavailable. Runtime feature-toggle ingress is fail-closed and externally unavailable; there is no separate live Admin UI or Logging & Admin forwarding path. The target Admin UI and fixed-category owner-local restriction actions and bounded appeals remain target/partial, not current operator controls. Policies are summarized in the [Moderation Policies](../../architecture/microservices/logging-admin-service/moderation-policies.md) document. Review these guides alongside the [Security Architecture](../../architecture/system-architecture-security.md) to ensure moderation actions follow platform rules.
...
Operator tooling should expose the Account-owned grant state, revision, effective expiry, tombstone/revocation outcome, exact fork lifecycle, and active-binding termination status without maintaining a shadow grant store. The operator UI forwards only an authenticated, tenant-scoped, digest-bound grant-revocation request to Account's authorization boundary; mutation is allowed only for that tenant's `tenantAdmin` or an audited `platformAdmin` break-glass action. Account retains grant state, mutation, and replay authority; generic tools must not mutate grants directly. It should support revoking player visibility/admission for a fork without deleting the fork, preserving fork-local state and audit history. The local operator outcome is targeted termination, while unrelated account access remains available. When a creator or operator wants a friendly scheduled playtest ending, they close and drain the realm first and revoke grants afterward; access revocation itself is not overloaded as a drain request. Gate failures block start/expansion but leave closure, cleanup, repair, and audit reachable under the lifecycle owner contract.
...
The target operator workflow troubleshoots issues and tunes performance using the [Logging & Admin Service](../../architecture/microservices/logging-admin-service/README.md) and the observability capabilities advertised by the selected profile. The current service has no Elasticsearch, Prometheus, Jaeger, Grafana, Kibana, or Alertmanager clients and no separate Admin UI; it only exposes its narrower PostgreSQL log-query surface. The following integrations are therefore target/profile-specific:

1. **Log Aggregation** – In the default indexed profile, Fluent Bit forwards service logs to **Elasticsearch**, and operators explore them in **Kibana**. Compatible profiles document equivalent mappings; reduced profiles use their declared console/journal path or mark indexed search unavailable. See [Logging & Monitoring](../../architecture/system-architecture-logging-monitoring.md).
2. **Metrics & Alerts** – In profiles that install them, **Prometheus** scrapes metrics and evaluates alert rules, **Alertmanager** routes alerts, and **Grafana** visualizes dashboards such as the [Service Overview](../../observability/grafana/service-overview.json).
3. **Tracing** – In profiles that install tracing, distributed traces are sent to **Jaeger** via the OpenTelemetry Collector as described in [Tracing](../../architecture/system-architecture-tracing.md).
4. **Kibana Dashboards** – In the default indexed profile, pre-built views like the [Log Volume dashboard](../../observability/kibana/log-volume.json) help monitor logging rates.
5. **Operator Dashboards** – Target-state Grafana and Kibana views are described in the [Analytics Dashboards](../../architecture/microservices/logging-admin-service/analytics-dashboards.md) document and are available only where the selected profile provides those backends.

```plaintext
Default indexed profile: Service Logs → Elasticsearch → Kibana
Profiles that install metrics: Metrics → Prometheus → Grafana / Alertmanager
Profiles that install tracing: Traces → Jaeger
```
