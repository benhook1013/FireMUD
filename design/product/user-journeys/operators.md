...
- **Safety and appeals:** The target fixed categories, owner-local enforcement, and bounded appeal case are defined in [Moderation Policies](../../architecture/microservices/logging-admin-service/moderation-policies.md). Current category persistence, owner commands, player notices, and appeal workflow are not implemented or proved. Account authenticates the appellant and owns security recovery; Logging & Admin owns moderation appeals; Game Session and Social & Groups remain enforcement owners.
...
3. **Operator Chooses Enforcement Type** – The moderator selects exactly one fixed category and explicit scope: `account_security_lock`, `platform_access_ban`, `gameplay_ban`, `chat_mute`, or `chat_ban`. An `account_security_lock` remains subject to Account security-policy/recovery authority; the other punitive actions use the applicable moderation jurisdiction. Unqualified legacy values such as `ban` are rejected.
4. **Owning Service Enforces** – Account, Game Session, or Social & Groups commits its own monotonic revision and enforces locally; Logging & Admin records policy intent, cases, appeal evidence, and audit and does not mutate foreign state.
