...
For script-transition incidents, operator tooling shows the authoritative Game Session pin/epoch and rollout history alongside Automation readiness, projection freshness, plugin state, timer reconciliation, and stage-aware dead-letter outcomes. A missing or stale local pin view blocks new scripted admission; it does not authorize a stale-pin override, and ordinary gameplay may continue. Routine rollback is therefore visible as scoped Automation convergence and asynchronous cleanup rather than an assumed whole-game pause.
