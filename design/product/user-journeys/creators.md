...
6. **Rollback** – A `tenantAdmin` may explicitly choose the applicable rollback path, subject to authorization and the predicates defined for that rollback path:
   ...
   - **Target-state script-patch rollback** uses a Game Session repin to a previously published, tenant-`READY`, base-compatible script patch with the exact current pin/epoch fence and advances `scriptPinEpoch`; Automation must converge the exact tuple before script admission resumes.
   - **Linked-plugin-only rollback** follows the separate script/plugin runtime path, selecting exact `(pluginId, pluginVersionId, bindingId)` and requiring current `PUBLISHED` publication, policy eligibility, and the matching Automation runtime-state fence; it is not a full-version replacement-instance cutover.
