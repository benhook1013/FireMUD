...
1. **Fork a Source Realm** – A `tenantAdmin` selects a source realm, usually the live production realm, and requests a fork. The platform snapshots the source realm using the canonical v1 fork-snapshot boundary from [Versioning & Runtime Configuration](../../architecture/system-architecture-versioning-runtime.md) and creates a temporary isolated playtest realm with its own stable playable-state namespace and runtime `gameInstanceId`. Each new playtest lifecycle receives a fresh namespace; replacing the runtime instance within that lifecycle retains the namespace.
