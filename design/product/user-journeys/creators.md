...
4. **Choose the Rollout Path**
   ...
   - **Non-script change** – A `tenantAdmin` creates a replacement-instance cutover to a new published version. The replacement must pass `READY`, compatibility, and exact pin gates before the realm route changes. A successful cutover makes the new instance the target for new admissions; an incompatible or otherwise failed preparation leaves the replacement unavailable and reports the reason to the creator. See [Game Session Service API Contracts](../../architecture/microservices/game-session-service/api-contracts.md) and [Versioning & Runtime Configuration](../../architecture/system-architecture-versioning-runtime.md#replacement-instance-upgrade-contract).
