...
- **Hosted content terms and commerce** – Official-hosted creator content requires target Account-owned acceptance of the current terms for the identified operator before the first persisted Draft and before later content-bearing authoring or publication writes; Game Design is the target fail-closed gate. Creators retain content ownership, hosting grants no marketplace or model-training rights, and no marketplace is currently available. After a material terms change, decline or missing reacceptance blocks new covered writes after the effective date, while supported reads/downloads and narrowly reducing lifecycle actions remain available within limited prior-terms continuity. The acceptance gate and changed-terms behavior remain target-only and unimplemented under [ADR 0180](../../architecture/decisions/adr-0180-account-owned-hosted-terms-acceptance-gate.md) and [ADR 0181](../../architecture/decisions/adr-0181-changed-hosted-terms-decline-and-existing-content-continuity.md); the hosted terms file is a pre-launch baseline with Benjamin James Hook as the identified planned operator and New Zealand legal review pending. Community self-hosting is outside this official-hosted gate.
...
Before Creator Content is persisted or hosted on the official service, the creator party must complete explicit Account-owned acceptance of the displayed hosted-terms version for the identified operator. That evidence is consumed by Game Design at its authoritative mutation boundary; a tenant role, billing owner, payment, or UI check cannot substitute for it. If material terms change, no new covered content write or publication proceeds after the effective date without reacceptance, while supported reads/downloads and bounded lifecycle-reducing actions remain available under limited continuity. This is target behavior, not current implementation, and does not apply to community self-hosting.
...
Account creation and bootstrap establish identity only; they do not provision a default actor. Before a realm admits actors, the creator publishes the versioned, generic realm-authored entry policy and only its policy-applicable artifact: `PLAYER_CREATED` uses an exact descriptor, `AUTO_PROVISIONED` uses an exact template, and `PRESEEDED_ONLY` uses neither. Entity Management consumes that policy after explicit realm entry. This handoff is defined by [ADR 0140](../../architecture/decisions/adr-0140-realm-authored-controllable-actor-entry.md); its fields remain game-authored rather than an RPG-specific platform schema.
...
5. **Reset or expire the fork** – A reset within the same playtest lifecycle starts a new playtest-state generation with a fresh namespace and explicit fresh/seeded/snapshot input; failed preparation leaves the prior generation authoritative. Fork expiry closes admission and follows the lifecycle cleanup contract. Runtime writes remain isolated and never merge automatically into production or another playtest.
