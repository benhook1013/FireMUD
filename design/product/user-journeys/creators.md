...
4. **Choose the Rollout Path**
   - **Script-only patch** – After the patch is published and runtime-ready, Game Session explicitly repins the target `scriptPatchVersion` and advances `scriptPinEpoch`. The runtime records the exact transition and fences older script work; ordinary gameplay continues while the script transition converges.
...
6. **Rollback** – A `tenantAdmin` may explicitly choose the applicable rollback path, subject to authorization, publication/readiness, compatibility, and exact-pin gates:
   ...
   - **Script-patch rollback** uses a Game Session repin to a previously published compatible script patch and advances `scriptPinEpoch`; the script runtime then converges under its exact fence.
