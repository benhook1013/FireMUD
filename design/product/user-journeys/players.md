...
2. **Friends and Guilds** – In-game friend links and guild memberships are tenant-scoped (`tenantId`). Account-global contacts are a separate cross-game relationship; when the feature is enabled, Social & Groups may project those contacts into this game's UI only through an explicit `tenantId`-filtered projection. That projection does not create tenant friend records and must not expose unrelated-tenant relationship, roster, or location data, as outlined in [Multi-Tenancy](../../architecture/system-architecture-multi-tenancy.md).
...
5. **Profanity & Friends** – The Social & Groups Service performs profanity checks, logs communication, and delivers messages. When moderation applies, the required Logging & Admin records are policy intent, case/evidence references, and audit; retaining a message as evidence is a separate optional signal. Feature-enabled account-global contacts may appear only in the explicitly selected tenant's filtered in-game projection; they remain account-global contacts, do not become tenant friend links, and cannot reveal another tenant's relationship or location.
