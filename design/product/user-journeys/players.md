...
5. **Bind to Gameplay** – `PLAY <world> [realm] [character]` resolves the selected realm to its stable playable-state namespace and currently active runtime, then binds the session to that target. Replacing the runtime does not move the player's durable state into a new realm; after this step, normal gameplay commands become available. See [ADR 0122](../../architecture/decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md).
