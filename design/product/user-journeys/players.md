...
3. **Deferred Commerce** – Player purchases, paid game subscriptions, creator donations/tips, platform fees on creator transactions, revenue sharing, payouts, and settlement are deferred and are not supported player journeys. The future direction in [ADR 0179](../../architecture/decisions/adr-0179-firemud-managed-creator-commerce-boundary.md) would require separate creator opt-in and launch proof; external creator-commerce payments or receipts, and creator assertions or claims from an unintegrated provider, cannot create FireMUD-managed entitlements or gameplay value. Supported FireMUD/Stripe hosting-plan billing remains the sole v1 billing boundary. Existing generic payment/refund code does not widen this scope.
