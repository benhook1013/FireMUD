...
1. Connect to the Game Session WebSocket at `/ws/game`. Advanced clients or test harnesses that resume an existing session created via REST (`POST /sessions`) may first send `SESSION <sessionId>` (and include `X-Session-Id` on the connection); typical browser clients never need to send `SESSION` and can rely on the server to create or bind a session on `LOGIN`, matching the Telnet flow.
