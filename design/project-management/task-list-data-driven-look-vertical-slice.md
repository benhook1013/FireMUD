...
- [ ] Add a dedicated Spring client/component that encapsulates the World and Entity service calls, captures Micrometer timers for the remote invocations, and can fall back to the existing dev-isolated stub when `game-session.dev-isolated=true` so developers can run without dependencies.
