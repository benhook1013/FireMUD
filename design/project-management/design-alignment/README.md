...
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 8 productive; report boundary reached without taper | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed manifest adds the Account service owner README and current AA-1 ADRs to the recovered 17-source prompt, with named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, and billing handoffs. Cycles 1–8 were productive (`16/1`, `24/1`, `18/1`, `33/1`, `13/1`, `17/1`, `47/3`, `28/3` raw/useful). In addition to the prior corrections, Cycle 8 bounds Account's Game Session RPCs and returns retryable `AUTH_UNAVAILABLE` / HTTP 503 only for unavailable or deadline transport failures, rejects malformed admission-pointer state scopes instead of coercing them to shared state, and requires existing gameplay-admitting membership before character discovery. Explicit JOIN and the known authority, lifecycle, replay, provider, abuse-control, and hosted-terms gaps remain routed. All eight cycles were productive, so taper is not established and no Cycle 9 was started at this report boundary. |
