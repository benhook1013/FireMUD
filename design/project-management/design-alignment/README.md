...
Completed phases: capability allocation, implementation/proof reconciliation, cross-domain convergence, structural enforcement, independent validation of prior parcel states, human-led adversarial review of all `183` queue/navigation rows in the remotely backed review archive, and selective import of all `182` distinct decision keys with family-local contract consolidation. Packets 3-4 apply ADRs 0051-0130 plus the no-ADR TICK-05 outcome. Packet 5 applies all `21` reviewed outcomes through ADRs 0131-0150, including the superseded `SOCIAL-01` provenance. Packet 6 P0 applies ten operations/delivery outcomes through ADRs 0151-0159, with the coupled preflight pair sharing ADR 0152. The final Packet 6 P1-P3 and Packet 7 import applies the previously remaining `32` reviewed outcomes through ADRs 0160-0178, including thirteen outcomes that strengthen existing ADR or canonical-design authority without a new ADR. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. The canonical registry contains `176` ADR records; the current validator classifies `168` records as reviewed and `11` as pre-formal, with those categories overlapping where accepted legacy pre-formal records now carry exact review metadata. Source-archive identifiers remain archive-local evidence and may collide numerically with canonical ADRs, so each selective parcel preserves canonical repository numbering rather than importing source numbers.
...
This table describes merged repository state, not merely completed human review. The completed review archive remains source evidence; all `182` distinct reviewed outcomes are now selectively applied to `develop`. Selective import is complete; the remaining work is the planned whole-corpus authority review.
...
The prior combined Packet 5 tail and Packet 6 P0 validation completed with `332` discovered sources (`329` allocated, `3` explicit exemptions), ADR review status `146` reviewed / `11` pre-formal, `linkCheck` at `6,039` total links (`6,002` OK, `37` excluded, `0` errors), `lintMarkdown` at `509` files (`0` issues), allocation regression `28` tests, ADR review regression `141` tests, and authorization-route regression `216` tests passing within the architecture suite. The merged Packet 6 P1-P3 parcel checks report `340` discovered sources (`337` allocated, `3` explicit exemptions), `165` ADR records (`154` reviewed and `11` pre-formal), `6,257` total links (`6,220` OK, `37` excluded, `0` errors), and `517` Markdown files (`0` issues). The allocation regression (`28`), ADR review regression (`141`), and authorization-route regression (`216`) checks pass; focused observability checks, the full Logging & Admin service check, project-wide test-source compilation, and Testcontainers 2.0.5 dependency-resolution proof also pass. Repository-wide `./gradlew check` was partial: Docker-backed Testcontainers initialization for `common-saga` stopped because no valid Docker environment was available (`/var/run/docker.sock` was absent). These are current parcel status and proof results, not normative design. The final reviewed import brings the structural source state to `351` discovered sources (`348` allocated, `3` explicit exemptions) and `176` ADR records; the current ADR validator reports `168` reviewed and `11` pre-formal records, with overlapping classifications for accepted legacy pre-formal records carrying exact review metadata. Final parcel checks must validate these current totals rather than treating the earlier Packet 6 P0 or P1-P3 run as proof. No hosted environment-backed deployment proof was run. Canonical fresh-image rebuild-and-boot proof was attempted but remains unavailable locally because Docker Desktop WSL integration is disabled, the `docker` command is unavailable, and `/var/run/docker.sock` is absent; repository-wide `./gradlew check` also stopped at Docker-dependent Testcontainers initialization. Static and focused checks do not replace any environment-backed proof required by implementation-changing work.

- `python3 dev-tools/validation/check-design-capability-allocation.py` derives the product and architecture source sets, parses each allocation ledger, and reconciles the declared current coverage summary (`351` discovered sources, `348` allocated sources, and the canonical `2` governance/template exemptions plus `1` registry exemption).
