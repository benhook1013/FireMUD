...
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 11 productive; dry streak zero | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed manifest includes Account authority and named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, and billing handoffs. Cycles 1–8 were productive; Cycle 9 was dry (`15/0`); Cycle 10 was productive (`23/2` deduplicated). Paired Cycle 11 found cross-account privileged access to the current profile GET/PUT route and stale Gateway status text about forwarding the connect-token cookie. Profile access now requires the exact account subject and existing tenant membership; Gateway status matches the implemented cookie stripping. Tenant-qualified profile routing and membership-authority-generation proof remain open. Cycle 11 is productive, so Unit 1A has not tapered. |
