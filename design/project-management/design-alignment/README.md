...
Existing 5B implementation work remains preserved in open PRs #2808, #2678, #2804, and #2679. Draft #2826 is the first Schema replacement candidate, stacked on non-5B #2824; the old PRs are not closure candidates until all unique changes and findings are verified in their replacements. They and older merged reviews remain historical evidence, not completed restart coverage or a 1:1 map to the three subunits below. Overall 5B is complete only after all three subunits taper and a cross-subunit synthesis is recorded; there is no separate overall 5B status row.
...
| 5B-Schema | SQL/Flyway/jOOQ, schema and identifier migration | Draft candidate; fresh broad review pending | [#2826](https://github.com/benhook1013/FireMUD/pull/2826) | V2 readiness and stable script identity are published for review; retained migration/identifier and PostgreSQL proof obligations remain open. |
