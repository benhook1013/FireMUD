...
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 23 productive; dry streak zero | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed Unit 1A manifest covers Account authority and named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, billing handoffs, and the Account Service README primary source; the starting source count is 29. Cycles 1–8 were productive; Cycle 9 was dry; Cycles 10–23 were productive. Cycle 13 corrected fail-open empty/ambiguous subscription-row entitlement projection. Cycle 14 confirmed the full authority/consolidation lens and routed public `PLAY` admission-outcome ordering to Unit 1B. Cycle 15 corrected billing status/registration schema and classified retryable entitlement outages. Cycle 16 corrected known connect-token billing denial to `TENANT_BILLING_BLOCKED` and routed exact-binding reconnect token/lease to Unit 1B without an LKG fallback. Cycle 17 exposed public-registration membership bypass; #2847 converged registration to global-only identity and explicit policy-bound JOIN, while #2848 supplies the durable audit receiver. Cycle 18 corrected current/target status and routed non-public realm-discovery/CHARS disclosure to Unit 1B. Cycle 19 fixed Account's canonical UUID realm binding and stale membership-response status. Cycle 20 corrected local JOIN availability claims, `past_due` status precision, and Account producer audit-payload compaction after verified receiver minimization. Cycle 21 terminalized deterministic changed-policy JOIN conflicts in #2847 and corrected further stale local-JOIN status in #2848 consumer docs and historical decision inventories. Cycle 22 contained the previously exposed Game Design creator writes at Gateway with explicit read-only routes; the Account-owned hosted-content gate remains unimplemented and unproved. Cycle 23 guarded Account credential-granting gRPC methods by exact Game Session peer, contained deferred currency RPCs, and corrected stale route, membership-evidence, and registration status; other legacy gRPC subject binding remains open. Drafts #2846–#2848 are correction evidence, not taper or merge readiness. PostgreSQL execution, retained-data deployment disposition, unknown-commit recovery, direct text-to-Logging proof, lifecycle/generation snapshot completion, finite `COMMITTED` audit retention, and full billing evidence remain open. The initial billing-owner source before the first subscription row remains undecided, so creation stays disabled and AA-1.4 tracks the decision; explicit free/trial state, current-row authority, and strict new-binding controls remain open. Unit 1A has not tapered. |
