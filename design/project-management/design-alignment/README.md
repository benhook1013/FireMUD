...
| 1B | Admission, session continuity, and reconnect | Three earlier productive passes; fresh two-cycle section-exit review in progress, not merge-ready | [#2857](https://github.com/benhook1013/FireMUD/pull/2857), [#2861](https://github.com/benhook1013/FireMUD/pull/2861) | #2857 corrects bounded Game Session admission/reconnect behavior; #2861 prepares ADR 0144's non-mutating email-link GET landing and explicit POST, not a deployed mail journey. Cycle 1 on the corrected cumulative #2861 tree found misleading current `PLAY`/`CHARS` and post-logout claims plus routed issuer, Entity roster, and Gateway signed-context gaps; accepted local corrections and focused proof are being integrated before Cycle 2. Account runtime reads still deny new `PLAY` before data access. Exact-current-head CI, PostgreSQL/cross-service proof, Account and Entity target authority, signed-context handoff, HTTPS/mail/abuse controls, and live browser proof remain open. The prior code-head CI run `36006632988` cleared 27 WebSocket and 39 Game Session cross-service cases but was red overall; local Docker skips are not runtime proof. |
