...
Completed phases: capability allocation, implementation/proof reconciliation, cross-domain convergence, structural enforcement, independent validation, human-led adversarial review of all `183` queue/navigation rows in the remotely backed review archive, and the completed baseline authority pass for ADRs 0001-0050 plus its major adjacent non-ADR contract families. Packet 3 is complete: ADRs 0051-0092 and the no-ADR TICK-05 outcome are selectively applied with family-local contract consolidation; they do not redefine the completed baseline. Packet 4 is complete: the prior publication/assets/promotion/procedural-generation, patch-pin/script-transition/timer and unified DSL/plugin-lifecycle, runtime-controls, and final lifecycle/authoring parcels apply all `36` reviewed outcomes. Packet 5 tail moves applied provenance `134→140` through six reviewed outcomes, bringing Packet 5 to all `21` applied rows through ADRs 0131-0150: six connection/output outcomes, four playtest/lifecycle/player-entry outcomes, five moderation/commerce/frontend/protocol outcomes, and six communication/moderation/social outcomes including the superseded `SOCIAL-01` provenance. Packet 6 P0 moves `140→150` through ten reviewed operations/delivery outcomes in active ADRs 0151-0159, with the coupled preflight pair sharing ADR 0152; the combined import creates `14` new active ADR records, while `SOCIAL-01` creates no ADR. Both parcels have family-local contract consolidation. Packet 6 P1-P3 and Packet 7 remain pending. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. The canonical registry now contains `157` ADR records (`146` reviewed and `11` pre-formal). `150` of the `182` distinct decision keys have checked applied provenance; the other `32` reviewed active decision keys remain pending selective import and are not yet canonical repository state. Canonical ADRs 0093-0103, 0106-0159 are repository state here; unimported source-archive identifiers remain archive-local evidence and may collide numerically with canonical ADRs, so this parcel preserves its canonical numbering rather than importing source numbers.
...
| Accepted-decision application | In progress | `150` distinct decision keys have checked applied provenance; `32` reviewed active decision keys remain pending selective import |
| Contract-authority consolidation | Baseline pass complete for ADRs 0001-0050 and major adjacent non-ADR families; Packets 3-4 (ADRs 0051-0103 and 0106-0130 plus TICK-05), Packet 5 (ADRs 0131-0150), and Packet 6 P0 (ADRs 0151-0159) are selectively applied with family-local consolidation; continue through Packet 6 P1-P3 and Packet 7, then perform a whole-corpus authority review | [Architecture contract authority map](../../architecture/README.md#contract-authority-map) and owner-link-plus-local-consequence conversions |
...
flowchart LR
    ...
    APPLY -->|merged to develop| APPLIED_DECISIONS[150 applied decision keys, including complete Packets 3-4, Packet 5 through ADR 0150, and Packet 6 P0 through ADR 0159, plus owning design changes]:::status
    APPLIED_DECISIONS -. explains; does not replace .-> ARCH
    APPLY -->|reviewed; pending selective import| PENDING[32 active decision keys pending selective import, including Packet 6 P1-P3 and Packet 7]:::pending
...
The combined Packet 5 tail and Packet 6 P0 validation completed with `332` discovered sources (`329` allocated, `3` explicit exemptions), ADR review status `146` reviewed / `11` pre-formal, `linkCheck` at `6,038` total links (`6,001` OK, `37` excluded, `0` errors), `lintMarkdown` at `509` files (`0` issues), allocation regression `28` tests, ADR review regression `141` tests, and authorization-route regression `216` tests passing within the architecture suite. Focused preflight, observability, player-experience smoke-evidence, and player-experience smoke-runner contract proof also passed, with clean Ruff, ShellCheck, Python syntax, and `git diff --check` results. No hosted environment-backed deployment proof was run. Canonical fresh-image rebuild-and-boot proof was attempted but remains unavailable locally because Docker Desktop WSL integration is disabled, the `docker` command is unavailable, and `/var/run/docker.sock` is absent; repository-wide `./gradlew check` also stopped at Docker-dependent Testcontainers initialization. Because this packet changes operational tooling and Kubernetes wiring, static and focused checks alone do not make it release-ready; the validation workflow still requires that fresh-image rebuild-and-boot evidence.

- `python3 dev-tools/validation/check-design-capability-allocation.py` derives the product and architecture source sets, parses each allocation ledger, and reconciles the declared current coverage summary (`332` discovered sources, `329` allocated sources, and the canonical `2` governance/template exemptions plus `1` registry exemption).
