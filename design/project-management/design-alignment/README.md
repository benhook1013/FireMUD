...
| 1B | Admission, session continuity, and reconnect | Implementation correction draft; whole-unit review not started | [#2857](https://github.com/benhook1013/FireMUD/pull/2857) | Stacked on #2856, this draft removes synthetic/unowned `PLAY` actor selection and pre-admission Entity lookup, disables unqualified `CHARS`, and separates fresh first-party `LOOK` from retained-context replay. Local Game Session checks pass, but most Docker-backed integration/cross-service cases skipped. Account's typed runtime readers, Entity namespace/policy roster, durable logout/resume episode, and namespace-scoped controller remain activation successors; neither admission readiness nor corpus closure is claimed. |
