...
This is the sole live corpus section-status index for a fresh whole-corpus review restart after the completed ADR work. The [whole-corpus authority review plan](./whole-corpus-authority-review-plan.md) defines the section boundaries and method; this subsection records the current status, one main PR slot, and evidence or routed work per section. Prior merged reviews and pull requests are historical evidence only and do not establish completion of this restart. A published implementation draft does not by itself complete its semantic-unit review.
...
5B synthesis: Corpus discovery accepted closed by Overseer at final cumulative #2829 head `dce1d8246e3ff2bf9c3bfcb9649ae0e5c4181e6b`; no fourth closure cycle is required. Paired whole-5B closure Cycle 1 was dry, Cycle 2 was productive, and corrected-state Cycle 3 was dry. Cycle 2's two independent reviewers produced three accepted obligation families after deduplication. The corrected stack enforces coordinated `Recreate`, roll-forward-only deployment for the incompatible Automation V3 and Game Design V26 migration boundaries; quarantines only V13-marked unattested legacy full versions while preserving bundled and script-only rows; and explicitly classifies the two Automation plugin-history retention families without inventing a cleanup TTL. Cycle 3's two independent reviewers returned 18 and 19 raw observations with zero new accepted obligations. Remaining implementation and proof gaps stay with their named owners; discovery closure does not claim those capabilities implemented or merged.

| Section | Original scope | Status | Main PR | Evidence / routed work |
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 17 productive; dry streak zero | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed Unit 1A manifest covers Account authority and named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, and billing handoffs. Cycles 1–8 were productive; Cycle 9 was dry; Cycles 10–17 were productive. Cycle 13 found a fail-open empty or ambiguous subscription-row entitlement projection: Account now returns retryable unavailable authority rather than permission, and connect-token issuance does not store that transient failure as a terminal replay receipt. Cycle 14 confirmed the full authority/consolidation lens and routed a new public `PLAY` admission-outcome defect to Unit 1B: `JOIN_REQUIRED` may precede entitlement availability and public-join-policy checks. Cycle 15 refreshed the 28-source manifest, corrected stale billing status and the live registration schema, and separated retryable entitlement outages from other authority outages through Account and Game Session. Cycle 16 corrected known connect-token billing denial to `TENANT_BILLING_BLOCKED` and recorded the unresolved exact-binding reconnect token/lease handoff for Overseer without authorizing an LKG fallback. Cycle 17 exposed the concrete public-registration membership bypass of disabled public joining; AA-1.1/AA-1.2 now state the live gap, and a containment/deployment decision is pending because coherent global registration also needs platform-scoped durable audit. The optional external-provider product summary now matches ADR 0049. The initial billing-owner source before the first subscription row is undefined; creation remains disabled and AA-1.4 carries this target-only Overseer decision. Explicit free/trial state, current-row authority, strict new-binding controls, and full billing evidence remain open. Unit 1A has not tapered. |
...
Focused validation on 2026-09-09 Pacific/Auckland passed for the current `355` discovered sources (`352` allocated, `3` explicit exemptions) and `180` ADR records, with `172` carrying completed review metadata and `11` pre-formal records; those categories overlap for accepted legacy pre-formal records carrying exact provenance. Prior parcel validation remains historical evidence only. On 2026-08-28 Pacific/Auckland, the integrated local gates passed against the then-current `354` discovered sources (`351` allocated, `3` explicit exemptions) and `179` ADR records, with `171` carrying completed review metadata and `11` pre-formal records: implementation tracking covered `79` leaves across `10` trackers, the authorization-route matrix passed, the full architecture-document contract suite passed, `linkCheck` checked `6,537` links (`6,489` OK, `0` errors, `48` excluded), `lintMarkdown` checked `535` files (`0` issues), and `git diff --check` was clean. These listed gates are local documentation/structure checks only. The current port also includes controller, Kubernetes, workflow, and deployment-tooling implementation with focused local proof; its owning domain tracker remains authoritative for implementation and proof status. Live hosted-environment proof remains unavailable, and neither the target contracts nor local checks should be read as commit, PR, CI, or live-hosted proof.

- `python3 dev-tools/validation/check-design-capability-allocation.py` derives the product and architecture source sets, parses each allocation ledger, and reconciles the declared current coverage summary (`355` discovered sources, `352` allocated sources, and the canonical `2` governance/template exemptions plus `1` registry exemption).
