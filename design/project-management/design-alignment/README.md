...
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 18 productive; dry streak zero | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed Unit 1A manifest covers Account authority and named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, billing handoffs, and the Account Service README primary source; the starting source count is 29. Cycles 1–8 were productive; Cycle 9 was dry; Cycles 10–18 were productive. Cycle 13 found a fail-open empty or ambiguous subscription-row entitlement projection: Account now returns retryable unavailable authority rather than permission, and connect-token issuance does not store that transient failure as a terminal replay receipt. Cycle 14 confirmed the full authority/consolidation lens and routed a new public `PLAY` admission-outcome defect to Unit 1B: `JOIN_REQUIRED` may precede entitlement availability and public-join-policy checks. Cycle 15 corrected stale billing status and the live registration schema, and separated retryable entitlement outages from other authority outages through Account and Game Session. Cycle 16 corrected known connect-token billing denial to `TENANT_BILLING_BLOCKED` and recorded the unresolved exact-binding reconnect token/lease handoff for Overseer without authorizing an LKG fallback. Cycle 17 exposed the concrete public-registration membership bypass of disabled public joining. Cycle 18 corrected stale corpus status: global-only registration, the local explicit `JOIN` operation, and the Account audit-ingress seam are implemented locally, while PostgreSQL execution, retained-data deployment disposition, pending-operation recovery/reconciliation, direct text-to-Logging end-to-end proof, lifecycle/authority-generation snapshot completion, and full billing evidence remain open. Drafts #2846, #2847, and #2848 are correction evidence only and do not establish corpus taper. The optional external-provider product summary now matches ADR 0049. Cycle 18 routed non-public realm-discovery leakage and stale join guidance to Unit 1B; those are not discarded. The initial billing-owner source before the first subscription row remains undecided, so creation stays disabled and AA-1.4 retains the target decision. Explicit free/trial state, current-row authority, strict new-binding controls, and full billing evidence remain open. Unit 1A has not tapered. |
