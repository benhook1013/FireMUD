...
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 7 productive; taper not established | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed manifest adds the Account service owner README and current AA-1 ADRs to the recovered 17-source prompt, with named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, and billing handoffs. Cycles 1–7 were productive (`16/1`, `24/1`, `18/1`, `33/1`, `13/1`, `17/1`, `47/3` raw/useful). In addition to the prior corrections, Cycle 7 prevents cross-account reauthentication from retaining another account's character/runtime binding, makes password-reset and username-recovery initiation neutral for unknown email, and replaces the public email-verification route's numeric account selector with normalized email plus the same neutral behavior. Same-account reconnect remains distinct, and tenant admission remains a later `JOIN`/`PLAY` concern. Known authority, lifecycle, replay, provider, abuse-control, and hosted-terms gaps remain routed. |
