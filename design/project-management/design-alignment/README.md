...
Completed phases: capability allocation, implementation/proof reconciliation, cross-domain convergence, structural enforcement, independent validation, human-led adversarial review of all `183` queue/navigation rows in the remotely backed review archive, and the completed baseline authority pass for ADRs 0001-0050 plus its major adjacent non-ADR contract families. Packet 3 is complete: ADRs 0051-0092 and the no-ADR TICK-05 outcome are selectively applied with family-local contract consolidation; they do not redefine the completed baseline. The prior ten-outcome publication/assets/promotion/procedural-generation parcel, the seven-outcome patch-pin/script-transition/timer and unified DSL/plugin-lifecycle parcel, and this ten-outcome runtime-controls parcel together apply 27 of 36 reviewed Packet 4 outcomes. This parcel contains nine accepted/revised target decisions plus one formal superseded historical decision recorded in ADR 0121; that historical outcome is applied provenance, not an excluded lightweight alias. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. The canonical registry now contains `119` ADR records (`116` accepted, `2` superseded, `1` withdrawn). On this parcel, `110` of the `182` active decision keys have checked applied provenance; the other `72` reviewed active decision keys remain pending selective import and are not yet canonical repository state. Nine Packet 4 outcomes remain pending, with packets 5-7 still pending. Canonical ADRs 0093-0121 are repository state here; unimported source-archive identifiers remain archive-local evidence and may collide numerically with canonical ADRs, so this parcel preserves its canonical numbering rather than importing source numbers.
...
| Accepted-decision application | In progress | `110` active decision keys have checked applied provenance; `72` reviewed active decision keys remain pending selective import |
...
flowchart LR
    ...
    APPLY -->|merged to develop| APPLIED_DECISIONS[110 applied decision keys, including complete Packet 3 ADRs 0051-0092 plus TICK-05 and the applied Packet 4 parcels, plus owning design changes]:::status
    APPLIED_DECISIONS -. explains; does not replace .-> ARCH
    APPLY -->|reviewed; pending selective import| PENDING[72 active decision keys pending selective import, including 9 remaining Packet 4 outcomes and packets 5-7]:::pending
...
| Selective Packet 4, prior 10-outcome publication/assets/promotion/procedural-generation parcel, 7-outcome patch-pin/script-transition/timer parcel, and ten-outcome runtime-controls parcel (27 of 36 reviewed outcomes) | Complete in the review archive | Prior parcels' 10 plus 7 plus ten runtime-controls outcomes (9 accepted/revised target decisions and one formal superseded historical decision, recorded as ADR 0121) applied with checked provenance; 9 Packet 4 outcomes and packets 5-7 remain pending; the separately tracked service-scan `MS-AA-TOKEN-REVOCATION` alias remains the only excluded navigation alias; canonical ADRs 0093-0121 are applied here, while source-archive numbering remains archive-local evidence | Family-local owner-link and local-consequence consolidation for this parcel; continue through remaining Packet 4 and packets 5-7, then perform a whole-corpus authority review | Reconcile owning trackers only where accepted target state changes implementation/proof gaps |
