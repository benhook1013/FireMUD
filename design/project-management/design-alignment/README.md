...
| --- | --- | --- | --- | --- |
| 1A | Identity, entitlement, and hosted terms | Cycle 12 productive; dry streak zero; report boundary | [#2840](https://github.com/benhook1013/FireMUD/pull/2840) | The refreshed manifest includes Account authority and named 5A, 1B, 5B/5D, 3A, 6A/6B/6C, platform, and billing handoffs. Cycles 1–8 were productive; Cycle 9 was dry; Cycles 10–12 were productive. Cycle 12 corrected AA-1.2's cached connect-token replay status and contained a live login-factor mutation that lacked ADR 0045 recent-reauthentication proof: GET remains available, while PUT fails before mutation pending Account-owned step-up evidence. Registration-created membership, explicit JOIN, entitlement/provider, token, hosted-terms, and proof gaps remain routed. Two consecutive dry corrected-state cycles have not occurred, so Unit 1A has not tapered. |
