...
Completed phases: capability allocation, implementation/proof reconciliation, cross-domain convergence, structural enforcement, independent validation, human-led adversarial review of all `183` queue/navigation rows in the remotely backed review archive, and the completed baseline authority pass for ADRs 0001-0050 plus its major adjacent non-ADR contract families. Packet 3 is complete: ADRs 0051-0092 and the no-ADR TICK-05 outcome are selectively applied with family-local contract consolidation; they do not redefine the completed baseline. Packet 4 is now complete: the prior publication/assets/promotion/procedural-generation, patch-pin/script-transition/timer and unified DSL/plugin-lifecycle, runtime-controls, and final lifecycle/authoring parcels apply all `36` reviewed outcomes. The final parcel contains eight Accepted-status target ADRs (six with Revised disposition, one with Accepted disposition, and one with Deferred disposition), plus one Superseded historical ADR (0130); the historical outcome is applied provenance, not an excluded lightweight alias. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. The canonical registry now contains `128` ADR records (`124` accepted, `3` superseded, `1` withdrawn). `119` of the `182` distinct decision keys have checked applied provenance; the other `63` reviewed active decision keys remain pending selective import and are not yet canonical repository state. Packets 5-7 remain pending. Canonical ADRs 0093-0103 and 0106-0130 are repository state here; unimported source-archive identifiers remain archive-local evidence and may collide numerically with canonical ADRs, so this parcel preserves its canonical numbering rather than importing source numbers.
...
flowchart LR
    ...
    APPLY -->|merged to develop| APPLIED_DECISIONS[119 applied decision keys, including complete Packet 3 ADRs 0051-0092 plus TICK-05 and complete Packet 4, plus owning design changes]:::status
    APPLIED_DECISIONS -. explains; does not replace .-> ARCH
    APPLY -->|reviewed; pending selective import| PENDING[63 active decision keys pending selective import, including packets 5-7]:::pending
...
- `python3 dev-tools/validation/check-design-capability-allocation.py` derives the product and architecture source sets, parses each allocation ledger, and reconciles the declared `303`-source coverage summary (`300` capability allocations plus the canonical `2` governance/template exemptions and `1` registry exemption).
