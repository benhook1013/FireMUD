...
- The pending AI-authored ADR 0183 proposal and the later direct human `EDGE-WITHDRAWAL-01` decision in accepted ADR 0184 add two allocated ADR sources to the four post-archive decisions above. The current canonical source set is `357` discovered sources (`354` allocated, `3` explicit exemptions), including `182` ADR records plus the registry/index exemption. ADR 0183 remains `Proposed - Pending Human Review` with `AI-AUTHORED-PENDING` provenance; its mechanical renumbering preserves the proposal but does not accept it. ADR 0184's emergency withdrawal remains unproved in a live hosted environment; this allocation is not runtime proof.
