...
- The later direct human `EDGE-WITHDRAWAL-01` decision adds ADR 0184 to the current source set without changing the historical Packet 1-7 archive or the four earlier direct-decision outcomes. The current allocation is `356` discovered sources (`353` allocated, `3` explicit exemptions) and `181` ADR records. Emergency withdrawal remains unproved in a live hosted environment; this source allocation is not runtime proof. ADR 0183 is reserved by a separate stacked proposal and is not imported into this direct-to-`develop` decision record.
...
| [Architecture decision registry](../../architecture/decisions/README.md) | Registry plus 181 ADRs | Per-record allocation | Per-record affected capabilities | Decision record | The registry is an index; reviewed, accepted, superseded, and withdrawn records remain distinguishable |
...
| `design/architecture/decisions/adr-0184-emergency-tcp-proxy-identity-withdrawal.md` | `SF-1` | `PO-2`, `PO-3`, `PO-4` | Accepted |
