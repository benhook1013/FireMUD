...
- Pre-licensing-integration allocation and ADR review-status baseline on 2026-08-28 Pacific/Auckland: allocation validation passed with `351` discovered sources (`348` allocated, `3` explicit exemptions), and ADR review-status validation passed with `168` reviewed ADRs and `11` pre-formal records. These are historical baseline/provenance facts, separate from implementation or runtime proof.
- Licensing-integration validation on 2026-08-28 Pacific/Auckland: allocation validation passed with `354` discovered sources (`351` allocated, `3` explicit exemptions); ADR review-status validation passed with `171` reviewed ADRs and `11` pre-formal records; implementation tracking passed with `79` leaves across `10` trackers; the authorization-route matrix and full architecture-document contract suite passed; `linkCheck` checked `6,537` links (`6,489` OK, `0` errors, `48` excluded); `lintMarkdown` checked `535` files (`0` issues); and `git diff --check` was clean. These checks establish documentation and structural consistency for the integration only; they do not claim commit, PR, CI, or runtime proof.
