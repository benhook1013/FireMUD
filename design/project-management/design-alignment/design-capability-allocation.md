...
- Current validation record on 2026-08-22 Pacific/Auckland (2026-08-21 UTC): `linkCheck` checked `5,487` links (`5,449` OK, `38` excluded, `0` errors); `lintMarkdown` checked `486` files with `0` issues; architecture contracts passed `184` tests; ADR review is `123` reviewed / `11` pre-formal; the allocation validator confirms `309` sources (`306` allocated, `3` explicit exemptions); `verifyPlatformSettingsDocs` passed; and `git diff --check` is clean. The six Packet 5 connection/output ADR records are allocated below.
