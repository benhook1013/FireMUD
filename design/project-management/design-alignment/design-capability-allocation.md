...
This file intentionally owns the detailed allocation-validation history; the design-alignment workstream may summarize parcel-level validation without duplicating those records.
...
- Historical allocation-validation record on 2026-08-22 Pacific/Auckland (2026-08-21 UTC): the allocation validator confirmed `309` sources (`306` allocated, `3` explicit exemptions), and `git diff --check` was clean. The six Packet 5 connection/output ADR records were allocated below. This records allocation coverage and file-format validation only; it does not claim implementation or runtime proof.
...
- Packet 6 P1-P3 allocation state: ADRs 0160-0167 add 8 reviewed records, bringing the canonical source set to `340` discovered sources (`337` allocated, `3` explicit exemptions) and the ADR registry to `165` records. The final reviewed import through Packet 7 ADRs 0168-0178 adds 11 more ADR records (19 reviewed records across both parcels), yielding `351` discovered sources (`348` allocated, `3` explicit exemptions) and `176` ADR records. Those review categories overlap where accepted legacy pre-formal records now carry exact review metadata. This structural state does not upgrade implementation or runtime proof for the allocated decisions; final parcel validation is reported by the design-alignment workstream rather than inferred from allocation arithmetic.
- The four post-archive direct human decisions `COMMERCE-02`, `HOSTED-TERMS-01`, `HOSTED-TERMS-02`, and `OPS-07` add ADRs 0179-0182 without changing the completed historical Packet 1-7 archive. They bring the canonical source set to `355` discovered sources (`352` allocated, `3` explicit exemptions) and the ADR registry to `180` records. The marketplace, hosted-terms, signer, and billing-lifecycle boundaries remain future or pre-launch target state. This allocation entry records allocation authority and target-state boundaries only; for ADR 0182, only local/static prerequisite evidence exists, while live activation, certificate issuance, and consumer convergence remain unproved. It does not establish broader implementation status, focused proof, or environment-backed runtime proof.
- The pending AI-authored ADR 0183 proposal is an additional allocated ADR source, not a reviewed decision: it brings the current canonical source set to `356` discovered sources (`353` allocated, `3` explicit exemptions), including `181` ADR records plus the registry/index exemption. Its pending status and `AI-AUTHORED-PENDING` provenance remain unchanged until human review; mechanical renumbering from the conflicting source-branch ADR 0182 is preservation only and does not accept the proposal.
- Historical baseline allocation-validator run on 2026-08-27 Pacific/Auckland: `python3 dev-tools/validation/check-design-capability-allocation.py` passed with `340` discovered sources (`337` allocated, `3` explicit exemptions). This records the Packet 6 baseline allocation coverage only and does not claim implementation or runtime proof.
- Pre-licensing-integration allocation and ADR review-status baseline on 2026-08-28 Pacific/Auckland: allocation validation passed with `351` discovered sources (`348` allocated, `3` explicit exemptions), and ADR review-status validation passed with `168` reviewed ADRs and `11` pre-formal records. ADRs 0002, 0007, and 0008 are overlapping records present in both the reviewed count and the 11 pre-formal records, so the deduplicated registry total is `168 + 11 - 3 = 176` before licensing integration. These are historical baseline/provenance facts, separate from implementation or runtime proof.
- Licensing-integration validation on 2026-08-28 Pacific/Auckland: allocation validation passed with `354` discovered sources (`351` allocated, `3` explicit exemptions); ADR review-status validation passed with `171` reviewed ADRs and `11` pre-formal records; implementation tracking passed with `79` leaves across `10` trackers; the authorization-route matrix and full architecture-document contract suite passed; `linkCheck` checked `6,537` links (`6,489` OK, `0` errors, `48` excluded); `lintMarkdown` checked `535` files (`0` issues); and `git diff --check` was clean. ADRs 0002, 0007, and 0008 are overlapping records present in both the reviewed count and the 11 pre-formal records, so the deduplicated registry total is `171 + 11 - 3 = 179` after licensing integration. These checks establish documentation and structural consistency for the integration only; they do not claim commit, PR, CI, or runtime proof.
...
| Architecture decisions | 182 | 181 | 0; 1 registry exemption | 100% classified |
| Product documentation | 7 | 7 | 0 | 100% classified |
| **Total** | **356** | **353** | **0; 3 explicit exemptions** | **100% classified** |
...
| [Architecture decision registry](../../architecture/decisions/README.md) | Registry plus 181 ADRs | Per-record allocation | Per-record affected capabilities | Decision record | The registry is an index; reviewed, accepted, superseded, withdrawn, and pending records remain distinguishable |
...
| `design/architecture/decisions/adr-0137-isolated-playtest-state-modes-and-reset.md` | `AR-3` | `AA-3`, `GR-2`, `GR-3`, `AA-1` | Accepted |
| `design/architecture/decisions/adr-0138-expiring-playtest-grants-with-bounded-active-revocation.md` | `AR-3` | `AA-3`, `AA-2`, `PO-1`, `EA-3` | Accepted |
| `design/architecture/decisions/adr-0139-tenant-owned-runtime-lifecycle-with-audited-break-glass.md` | `AR-3` | `AA-1`, `PO-1` | Accepted |
| `design/architecture/decisions/adr-0140-realm-authored-controllable-actor-entry.md` | `AA-2` | `AA-3`, `GR-3`, `EA-3` | Accepted |
| `design/architecture/decisions/adr-0141-fixed-safety-restriction-categories-and-independent-lifecycles.md` | `EA-2` | `PO-1`, `AA-1` | Accepted |
| `design/architecture/decisions/adr-0142-bounded-moderation-appeal-cases.md` | `PO-1` | `EA-2`, `AA-1` | Accepted |
...
| `design/architecture/decisions/adr-0146-owner-local-moderation-enforcement.md` | `PO-1` | `EA-2`, `AA-1`, `PO-4`, `SF-2` | Accepted |
...
| `design/architecture/decisions/adr-0153-measured-online-backup-rpo-and-future-pitr-trigger.md` | `PO-3` | `GR-1`, `PO-4`, `SF-2` | Accepted |
| `design/architecture/decisions/adr-0154-automated-recovery-proof-and-differentiated-traffic-open-gates.md` | `PO-3` | `GR-1`, `PO-4`, `SF-2` | Accepted |
| `design/architecture/decisions/adr-0155-automated-event-classified-post-restore-trust-reset.md` | `PO-3` | `GR-1`, `PO-2`, `PO-4`, `SF-1` | Accepted |
| `design/architecture/decisions/adr-0156-risk-tiered-progressive-rollout-with-compatibility-bounded-rollback.md` | `PO-3` | `AR-3`, `PO-1`, `PO-4` | Accepted |
| `design/architecture/decisions/adr-0157-dependency-classified-liveness-readiness-and-route-admission.md` | `PO-4` | `GR-1`, `PO-2`, `PO-3` | Accepted |
| `design/architecture/decisions/adr-0158-simplified-observability-degradation-without-fallback-alert-authority.md` | `PO-4` | `PO-1` | Accepted |
| `design/architecture/decisions/adr-0159-profile-dependent-independent-deadman-and-public-path-monitoring.md` | `PO-4` | `PO-2` | Accepted |
| `design/architecture/decisions/adr-0160-staged-profile-aware-player-experience-slo-contract.md` | `PO-4` | `PO-2`, `GR-1`, `EA-2` | Accepted |
| `design/architecture/decisions/adr-0161-profile-aware-isolated-synthetic-player-flow-canaries.md` | `PO-4` | `EA-3`, `PO-2`, `AA-1` | Accepted |
| `design/architecture/decisions/adr-0162-profile-aware-asynchronous-end-to-end-log-queryability-evidence.md` | `PO-4` | — | Accepted |
...
| `design/architecture/decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md` | `GR-1` | `GR-4`, `AS-1`, `SF-1`, `SF-2`, `PO-4` | Proposed - Pending Human Review |
