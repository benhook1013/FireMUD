...
- The three post-archive direct human decisions `COMMERCE-02`, `HOSTED-TERMS-01`, and `HOSTED-TERMS-02` add ADRs 0179-0181 without changing the completed historical Packet 1-7 archive. They bring the canonical source set to `354` discovered sources (`351` allocated, `3` explicit exemptions) and the ADR registry to `179` records. These records define future or pre-launch target boundaries only and do not claim marketplace, hosted-terms, signer, billing-lifecycle, or runtime implementation proof.
- The pending AI-authored ADR 0182 proposal is an additional allocated ADR source, not a reviewed decision: it brings the current canonical source set to `355` discovered sources (`352` allocated, `3` explicit exemptions), including `180` ADR records plus the registry/index exemption. Its pending status and `AI-AUTHORED-PENDING` provenance remain unchanged until human review.
...
| Architecture decisions | 181 | 180 | 0; 1 registry exemption | 100% classified |
| Product documentation | 7 | 7 | 0 | 100% classified |
| **Total** | **355** | **352** | **0; 3 explicit exemptions** | **100% classified** |
...
| [Architecture decision registry](../../architecture/decisions/README.md) | Registry plus 180 ADRs | Per-record allocation | Per-record affected capabilities | Decision record | The registry is an index; reviewed, accepted, superseded, withdrawn, and pending records remain distinguishable |
...
| `design/architecture/decisions/adr-0182-deterministic-effect-id-allocation-and-replay-binding.md` | `GR-1` | `GR-4`, `AS-1`, `SF-1`, `SF-2`, `PO-4` | Proposed - Pending Human Review |
