...
- Current canonical validation evidence on 2026-08-20 Pacific/Auckland: `design capability allocation passed: 294 sources (291 allocated, 3 explicit exemptions)`; `linkCheck` checked 5,106 links (5,068 OK, 0 errors, 38 excluded); `lintMarkdown` checked 471 files with 0 issues; ADR review status validation passed for 108 reviewed ADRs and 11 pre-formal records; the architecture contract suite passed 184 tests; and `git diff --check` passed with no errors.
