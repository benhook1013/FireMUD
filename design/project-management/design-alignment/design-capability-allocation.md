...
- Historical baseline allocation-validator run on 2026-08-27 Pacific/Auckland: `python3 dev-tools/validation/check-design-capability-allocation.py` passed with `340` discovered sources (`337` allocated, `3` explicit exemptions). This confirms the Packet 6 baseline allocation coverage only and does not claim implementation or runtime proof; it is superseded by the current PR-head validation below.
- Current PR-head allocation-validator run on 2026-08-28 Pacific/Auckland: `python3 dev-tools/validation/check-design-capability-allocation.py` passed with `351` discovered sources (`348` allocated, `3` explicit exemptions). This confirms allocation coverage only and does not claim implementation or runtime proof.
- Current PR-head ADR review-status validator run on 2026-08-28 Pacific/Auckland: `python3 dev-tools/validation/check-adr-review-status.py` passed with `168` reviewed ADRs and `11` pre-formal records. This is review-provenance status, separate from allocation coverage, and does not claim implementation or runtime proof.
