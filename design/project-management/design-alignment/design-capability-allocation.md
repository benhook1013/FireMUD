...
- The four post-archive direct human decisions `COMMERCE-02`, `HOSTED-TERMS-01`, `HOSTED-TERMS-02`, and `OPS-07` add ADRs 0179-0182 without changing the completed historical Packet 1-7 archive. They bring the canonical source set to `355` discovered sources (`352` allocated, `3` explicit exemptions) and the ADR registry to `180` records. These records define future or pre-launch target boundaries only and do not claim marketplace, hosted-terms, signer, billing-lifecycle, hosted controller, or runtime implementation proof.
