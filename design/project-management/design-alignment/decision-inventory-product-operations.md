...
- **Decision / status / importance:** Gameplay admission establishes an immutable logical binding expiry derived from the configured JWT lifetime plus safety margin; it is a logical binding-validity ceiling, not an extension of any token's `exp`. `firemud.reconnection.policy.resume-window-ms` is the shorter disconnected-resume admission window, measured from disconnect/suspension and bounded by the remaining logical binding lifetime. Physical Redis key presence or retention is non-authoritative, and neither it nor transcript retention can determine or extend resume eligibility. Status `accepted-explicit`; `H/hard`.
- **Sources / headings:** Assigned sources: [environment-and-secrets-catalog.md](../../architecture/infrastructure/environment-and-secrets-catalog.md) `§ Authentication & JWT` and `§ Additional Notes`; [platform-settings-reference.md](../../architecture/generated/platform-settings-reference.md) `§ firemud.reconnection`; [system-architecture-frontend.md](../../architecture/system-architecture-frontend.md) `§ Authentication and Session Handling`; [system-architecture-mud-client-protocol.md](../../architecture/system-architecture-mud-client-protocol.md) `§ Reconnection & Session Recovery`. Cross-partition reconciliation references, excluded from the 35-path scan and assigned-source count: [system-architecture-reconnection.md](../../architecture/system-architecture-reconnection.md) `§ Requirements`; [system-architecture-redis.md](../../architecture/system-architecture-redis.md) `§ Session Keys and Gameplay Binding`; [system-architecture-session-behavior.md](../../architecture/system-architecture-session-behavior.md) `§ Session Types and Lifetimes`.
- **Strongest alternative:** Use one lifetime for credentials, binding retention, disconnected resume, and transcript retention, accepting tighter coupling between security, continuity, and storage policy.
- **ADR recommendation:** Yes if later adversarial review changes this separation or its precedence; the canonical sources now agree on the baseline.
- **Human consultation:** Later adversarial review should include security, session-runtime, frontend, and player-experience owners.
...
`SESSION-08` was the only direct contradiction found in the assigned sources. It is now resolved: the three-minute setting is the disconnected-resume admission cap under the remaining absolute session lifetime, stale bindings follow the configured fresh-entry fallback, and transcript retention is independent. Later adversarial review may revisit that policy without treating the sources as currently contradictory.
...
1. **P0 - Verify resolved `SESSION-08`.** Prove fresh-token reconnect, the 180-second resume cap under the remaining absolute session lifetime, stale-session fall-through, independent transcript retention, and cross-device behavior against `AUTH-02`, `SESSION-03`, `SESSION-04`, `CMD-04`, and `SET-01`.
