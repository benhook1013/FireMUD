...
- **Decision / status / importance:** Do not open or reopen player-facing traffic, including first live production traffic after restore, until the deployment has a fresh backup, successful verification, and a successful restore drill within the stated windows, all bound to the canonical `tenant_id + region_id` scope. Keep the evidence and attestation chain in the deployment evidence path. Status `proposed/deferred` because the target contract is stronger than the current implementation evidence; `H/hard`.
- **Sources / headings:** [system-architecture-backup-recovery-evidence-and-compliance.md](../../architecture/system-architecture-backup-recovery-evidence-and-compliance.md) `§ Production Backup Readiness Evidence`, `§ Production Traffic-Open Backup Evidence`, `§ Hobby Backup Compliance Evidence`, `§ Hobby Traffic-Open Evidence`, and `§ Canonical Recovery Record`; [system-architecture-deployment-runbook.md](../../architecture/system-architecture-deployment-runbook.md) `§ Production Traffic-Open Backup Gate` and `§ Fresh-Boundary Restore Bootstrap`.
...
- **ADR recommendation:** Yes. Consolidate the traffic-open gate, evidence freshness windows, scope binding, and fail-closed behavior with existing `OPS-03` and `OPS-04`.
...
- **Decision / status / importance:** A restored environment remains quarantined until one canonical recovery record is complete and exactly one recovery mode, `cold_start_restore` or `scoped_reset_restore`, is selected. The record must cover secret and certificate hardening, external credential validation, coordination recovery, smoke checks, and approval; staging restores from production-origin data require sanitization. Reopen is a separate, ordered gate after the record passes. Status `proposed/deferred`; `H/hard`.
...
- **ADR recommendation:** Yes. Make the quarantine boundary, recovery mode, credential rotation, external-validation matrix, and reopen authority one recoverability contract; reconcile the current gap recorded by `OPS-04`.
...
#### `SESSION-08` - Reconnect lifetime is currently contradictory
...
- **Decision / status / importance:** The environment catalog derives `session_expiration_ms` as JWT expiration plus a session safety margin, with defaults of 3,600,000 ms plus 300,000 ms (65 minutes), and says existing keys retain their logical expiry. The generated settings reference separately sets `firemud.reconnection.policy.resume-window-ms` to 180,000 ms (3 minutes). It is unresolved whether 180 seconds is a stricter resume-admission cap, a replacement for the derived session lifetime, or an obsolete generated value. Status `conflicting`; `H/hard`.
- **Sources / headings:** [environment-and-secrets-catalog.md](../../architecture/infrastructure/environment-and-secrets-catalog.md) `§ Authentication & JWT` and `§ Additional Notes`; [platform-settings-reference.md](../../architecture/generated/platform-settings-reference.md) `§ firemud.reconnection`; [system-architecture-frontend.md](../../architecture/system-architecture-frontend.md) `§ Authentication and Session Handling`; [system-architecture-mud-client-protocol.md](../../architecture/system-architecture-mud-client-protocol.md) `§ Reconnection & Session Recovery`.
- **Strongest alternative:** Keep both values but define the 3-minute value as a bounded resume window while retaining the 65-minute JWT/session lifetime; alternatively choose one canonical lifetime and remove the other.
- **ADR recommendation:** Yes, urgently. Define the distinction between transport/session validity, resume eligibility, transcript retention, and fresh-entry behavior, then reconcile `SESSION-03`, `SESSION-04`, `CMD-04`, and `SET-01`.
- **Human consultation:** Yes; security, session-runtime, frontend, and player-experience owners must approve the reconnect promise.
...
| `OPS-03`, `OPS-04` | Backup freshness, verification, restore-drill, quarantine, and reopen evidence are explicit; the sources still identify current player-facing restore and region-pause gaps. | [system-architecture-backup-recovery-evidence-and-compliance.md](../../architecture/system-architecture-backup-recovery-evidence-and-compliance.md) `§ Production Backup Readiness Evidence`, `§ Production Traffic-Open Backup Evidence`, `§ Canonical Recovery Record`; [schedule.md](../../architecture/infrastructure/schedule.md) `§ Kubernetes Cluster (Production)` and `§ Hobby / Self-Hosted Environments`; [system-architecture-deployment-runbook.md](../../architecture/system-architecture-deployment-runbook.md) `§ Fresh-Boundary Restore Bootstrap`. | `RECOVERY-01` and `RECOVERY-02` record the new evidence/quarantine decisions; existing operational gaps remain open rather than being marked accepted. |
| `AUTH-02`, `AUTH-06`, `AUTH-07`, `ADMIT-01`, `SESSION-03`, `SESSION-07`, `EDGE-04`, `SESSION-09` | Browser bootstrap uses an HttpOnly connect-token cookie, reconnects with a fresh token and LOGIN/PLAY, public production join is discoverable/auto-membership, and billing-safe management remains reachable. | [system-architecture-frontend.md](../../architecture/system-architecture-frontend.md) `§ Authentication and Session Handling`, `§ API Usage Patterns`; [user-journeys-players.md](../../architecture/user-journeys-players.md) `§ 2. Join a Game for the First Time`, `§ 4. Player Login and Gameplay`, `§ 6. Purchases and Subscriptions`; [user-journeys-creators.md](../../architecture/user-journeys-creators.md) `§ 4. Publish and Start a Game Instance`. | Stronger implementation/product evidence only. `PLAYER-01` and `SESSION-08` capture the additional character and lifetime boundaries; cross-cutting `EDGE-04` and `SESSION-09` carry the distinct carrier/replay and refresh/logout contracts. |
...
`SESSION-08` is the only direct contradiction found in the assigned sources. It must be resolved before reconnect behavior, generated settings, or session-security evidence can be considered aligned. The 3-minute generated resume window and 65-minute derived JWT/session lifetime may coexist only if the former is explicitly an admission cap and the latter remains token/key lifetime; otherwise one source must be changed. The resolution must also state whether stale sessions fall through to fresh entry, how transcript retention relates to resume eligibility, and which setting has precedence.

The following are stronger evidence or implementation gaps, not additional contradictions:

- The recovery documents require traffic-open evidence and post-restore quarantine, while `OPS-04` records that player-facing restore and region pause are not fully implemented.
...
- [platform-settings-reference.md](../../architecture/generated/platform-settings-reference.md) is generated from metadata and explicitly says not to edit it manually. Its values are evidence for `SET-01`, `SET-02`, `CMD-04`, `CMD-05`, `CMD-06`, and `SOCIAL-01`; it has no independent authority except where its reconnect value conflicts in `SESSION-08`.
...
| 6 | [system-architecture-deployment-runbook.md](../../architecture/system-architecture-deployment-runbook.md) | Runbook | `§ Prerequisites`; `§ Implementation Notes`; `§ Environment Bootstrap (First Deployment Only)`; `§ Fresh-Boundary Restore Bootstrap`; `§ Production Traffic-Open Backup Gate`; `§ Overlay Deployment Flow (Staging and Production)`; `§ Hobby Manifest/Chart Deployment Flow (Hobby / Self-Hosted)`; `§ Canary or Phased Rollouts`; `§ Rollback`; `§ Per-Environment Deployment & Rollback Summary` | Strengthens `PREFLIGHT-01`, `RECOVERY-01`, `OPS-01`, `OPS-02`, `OPS-04`, `OPS-05`, `OPS-06`; no additional rollout key. |
...
| 31 | [environment-and-secrets-catalog.md](../../architecture/infrastructure/environment-and-secrets-catalog.md) | Reference | `§ Common Application Settings`; `§ PostgreSQL Credentials`; `§ Redis Coordination & Cache`; `§ TLS & Certificates`; `§ Authentication & JWT`; `§ Service Discovery`; `§ Observability`; `§ Asset Storage`; `§ Backup & Restore Variables`; `§ Additional Notes` | `SESSION-08` conflict; strengthens `REDIS-02`, `SEC-01`, `SEC-02`, `EDGE-03`, and `PREFLIGHT-02`. |
...
1. **P0 - Resolve `SESSION-08`.** Decide whether the 180-second generated resume window is an admission cap under the 65-minute JWT/session lifetime or whether one value replaces the other. Test fresh-token reconnect, stale-session fall-through, transcript retention, and cross-device behavior against `AUTH-02`, `SESSION-03`, `SESSION-04`, `CMD-04`, and `SET-01`.
2. **P0 - Prove or reject `RECOVERY-01`, `RECOVERY-02`, `PREFLIGHT-01`, `PREFLIGHT-02`, and `COMPLIANCE-01` as traffic gates.** Attempt a fresh-boundary restore with missing backup evidence, stale credentials, cross-environment bindings, incomplete quarantine, and an expired waiver. The expected result is fail closed before player admission; current gaps must be explicitly accepted or fixed.
