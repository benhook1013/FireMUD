...
- **Decision / status / importance:** The derived JWT lifetime plus safety margin is the absolute storage and authentication-validity ceiling for a gameplay binding. `firemud.reconnection.policy.resume-window-ms` is the shorter disconnected-resume admission window, measured from disconnect/suspension and bounded by the remaining absolute lifetime. Transcript retention is independent and cannot extend either boundary. Status `accepted-explicit`; `H/hard`.
- **Sources / headings:** [environment-and-secrets-catalog.md](../../architecture/infrastructure/environment-and-secrets-catalog.md) `§ Authentication & JWT` and `§ Additional Notes`; [system-architecture-reconnection.md](../../architecture/system-architecture-reconnection.md) `§ Requirements`; [system-architecture-redis.md](../../architecture/system-architecture-redis.md) `§ Session Keys and Gameplay Binding`; [system-architecture-session-behavior.md](../../architecture/system-architecture-session-behavior.md) `§ Session Types, Lifetimes, and Redis Boundaries`; [platform-settings-reference.md](../../architecture/generated/platform-settings-reference.md) `§ firemud.reconnection`; [system-architecture-frontend.md](../../architecture/system-architecture-frontend.md) `§ Authentication and Session Handling`; [system-architecture-mud-client-protocol.md](../../architecture/system-architecture-mud-client-protocol.md) `§ Reconnection & Session Recovery`.
