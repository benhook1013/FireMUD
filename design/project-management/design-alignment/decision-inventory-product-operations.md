...
- **Decision / status / importance:** Gameplay admission establishes an immutable logical binding expiry derived from the configured JWT lifetime plus safety margin; it is a server-side binding/storage ceiling, not an extension of any token's `exp`. `firemud.reconnection.policy.resume-window-ms` is the shorter disconnected-resume admission window, measured from disconnect/suspension and bounded by the remaining logical binding lifetime. Transcript retention and physical Redis deletion are independent and cannot extend resume eligibility. Status `accepted-explicit`; `H/hard`.
