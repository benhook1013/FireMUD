...
Status: Inventory and human-led review are complete and independently coverage/fidelity-audited. Packet 6 is complete at 25 applied outcomes through ADRs 0151-0167. `165` of `182` distinct decision keys have applied review provenance; the 17 Packet 7 outcomes remain pending selective import. This parcel creates 8 new active ADR records (ADRs 0160-0167). The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. ADR numbering records how applied outcomes were materialized but does not define the application boundary; the planned whole-corpus authority review follows completion of Packet 7.
...
| **Decision keys / navigation rows** | **All 340 design sources: 337 allocated decision-bearing sources (172 allocated non-ADR documents and 165 ADR records), 1 exempt architecture decision registry/index, and 2 excluded governance/template exemptions; 337 allocated and 3 total exemptions** | **182 decision keys; 183 navigation/key rows including one historical service-scan alias** | **All 183 queue/navigation rows reviewed in the source archive** | **165 decision keys have reviewed outcomes applied cumulatively; Packet 6 is complete through ADRs 0151-0167, and 17 Packet 7 outcomes remain pending** |
...
The remotely backed `design/adversarial-decision-review` archive records the completed human-led review of all `183` queue/navigation rows. This section is the authoritative checked provenance for the `165` applicable outcomes currently applied and merged to `develop`: `applied` means the reviewed result is present in merged canonical design, not merely recorded in the archive. Every applied row is checked and must link that merged ADR or canonical design; the 17 Packet 7 rows remain unchecked pending import, and the one excluded navigation alias remains historical archive evidence only.
...
| **Total** | | **183** | **165** | `review-complete`; partial application |

The source-archive total counts 183 navigation rows. Packet 2 contains 31 distinct decision keys plus the `MS-AA-TOKEN-REVOCATION` historical alias, producing 32 archive rows but only 31 applicable decisions. Packet 6 is complete at 25 applied outcomes through ADRs 0151-0167, including the 15-key P1-P3 import in this parcel. Packet 7 contains 17 reviewed outcomes through ADRs 0168-0178 and remains pending. This parcel creates 8 new active ADR records; the legacy ADR labels above are mappings to decision keys rather than additional rows. The later whole-corpus authority review remains required after Packet 7.
