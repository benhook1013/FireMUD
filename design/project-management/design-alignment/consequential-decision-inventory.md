...
| Microservice architecture | 76 sources | 23 rows = 22 active decisions + superseded historical `MS-AA-TOKEN-REVOCATION` alias; stronger evidence for 40 existing keys | Prioritized in the source ledger | Complete and independently audited |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `AS-INGRESS-IDEMPOTENCY` | `AS-1` | What identity makes script-event ingress idempotent across retries and failover? | The endpoint-specific Trigger Identity matrix, including the explicit dry-run discriminator `isDryRun` (execution mode); scheduler/timer entries additionally carry schedule identity and exactly one tagged persisted due point, either `dueTickId` or `dueAt`, never both or neither | `accepted-explicit` | `P1` | [ADR 0001](../../architecture/decisions/adr-0001-scripting-event-ingress-idempotency-identity.md); [Trigger Identity matrix](../../architecture/system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields) | Event ID alone, or an infrastructure-generated opaque dedupe key | Accepted record exists; human-led adversarial review pending |
...
- `DATA-01`: [ADR 0050](../../architecture/decisions/adr-0050-versioned-export-retention-and-erasure-policy.md), the Account runtime model, and the player journey keep durable asynchronous full export separate from deletion, require every owning-service contribution to fan in, and permit manifest retrieval only after that fan-in.
...
| 2 | Identity, authority, and security | 31 | 32 | `completed` |
...
| **Total** | | **40** | **183** | `in-progress` |

The progress total counts reviewed distinct decisions against checklist/navigation rows. Packet 2 contains 31 distinct decision keys represented by 32 navigation rows; one is the historical alias and is excluded from checklist disposition. Its 31 checked dispositions, covering ADRs 0020-0050 with the accepted/revised gaps represented by the branch's reviewed records, therefore represent exactly 31 reviewed decisions.
