...
Status: Inventory and human-led review are complete and independently coverage/fidelity-audited. Cumulative reviewed outcomes with applied provenance now cover 134 of 182 distinct decision keys, up from 129 before this parcel; this parcel adds five applied reviewed decision outcomes in the Packet 5 moderation/commerce/frontend/protocol lane (five Accepted-status target ADRs with Revised disposition, ADRs 0141-0145). The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias, and 48 reviewed decisions remain pending selective import. ADR numbering records how applied outcomes were materialized but does not define the application boundary.
...
| **Decision keys / navigation rows** | **All 318 design sources: 311 allocated decision-bearing sources (172 allocated non-ADR documents and 143 ADR records), 1 exempt architecture decision registry/index, and 2 excluded governance/template exemptions; 315 allocated and 3 total exemptions** | **182 decision keys; 183 navigation/key rows including one historical service-scan alias** | **All 183 queue/navigation rows reviewed in the source archive** | **134 of 182 decision keys have reviewed outcomes applied cumulatively, up from 129 before this parcel; this parcel added five applied reviewed outcomes in the Packet 5 moderation/commerce/frontend/protocol lane (five Accepted-status target ADRs with Revised disposition, ADRs 0141-0145); 48 remain pending selective import** |
...
| 5 | Gameplay and player experience | 21 | 15 | `partially-applied`; remaining lanes `pending-import` |
| 6 | Operations and delivery | 25 | 0 | `pending-import` |
| 7 | Existing ADR-backed and lower-risk remainder | 17 | 0 | `pending-import` |
| **Total** | | **183** | **134** | `review-complete`; `import-in-progress` |

The source-archive total counts 183 navigation rows. Packet 2 contains 31 distinct decision keys plus the `MS-AA-TOKEN-REVOCATION` historical alias, producing 32 archive rows but only 31 applicable decisions. The applied-provenance checklist excludes that service-scan alias and therefore has 182 rows: 134 reviewed outcomes applied cumulatively, up from 129 before this parcel because this parcel added five applied reviewed outcomes, and 48 unchecked decision keys pending selective import. The five outcomes in this parcel are Accepted-status target ADRs with Revised disposition for the Packet 5 moderation/commerce/frontend/protocol lane, ADRs 0141-0145. The nine legacy ADR labels above are mappings to decision keys rather than additional rows.
