...
Status: Inventory and human-led review are complete and independently coverage/fidelity-audited. All 182 distinct decision keys now have applied review provenance. Packet 6 is complete at 25 applied outcomes through ADRs 0151-0167. Packet 7 is complete at 17 applied outcomes through ADRs 0168-0178 plus clarifications to existing authority. The combined Packet 6 P1-P3 and Packet 7 imports create 19 active ADR records, of which Packet 7 adds 11. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. ADR numbering records how applied outcomes were materialized but does not define the application boundary; the planned whole-corpus authority review follows this completed selective-import program.

## Implementation Status

`Complete` applies to inventory coverage, the human-owned review run, and selective import of all `182` distinct decision keys; it does not claim runtime implementation or proof. The checked provenance below records reviewed outcomes already applied and merged to `develop`. Canonical design owns the merged target state; accepted ADRs provide consequential rationale within that boundary, while the domain trackers define implementation and proof status. Selective import is complete; remaining work is the planned whole-corpus authority review.
...
| **Decision keys / navigation rows** | **All 351 design sources: 348 allocated decision-bearing sources (172 allocated non-ADR documents and 176 ADR records), 1 exempt architecture decision registry/index, and 2 excluded governance/template exemptions; 348 allocated and 3 total exemptions** | **182 decision keys; 183 navigation/key rows including one historical service-scan alias** | **All 183 queue/navigation rows reviewed in the source archive** | **All 182 decision keys have reviewed outcomes applied cumulatively; Packet 6 is complete through ADRs 0151-0167, and Packet 7 is complete through ADRs 0168-0178 plus existing-authority clarifications** |
...
| `AS-RELOAD-BACKPRESSURE` | `AUTO-03` | `revised`; applied | [ADR 0173](../../architecture/decisions/adr-0173-registry-classified-reload-admission-policy.md), superseding [ADR 0003](../../architecture/decisions/adr-0003-reload-backpressure-and-retry-contract.md) |
| `AA-WORLD-SELECTOR-IDENTITY` | `AUTH-01` | `revised`; applied | [ADR 0168](../../architecture/decisions/adr-0168-snapshot-bound-lobby-selectors-and-stable-realm-identity.md), superseding [ADR 0005](../../architecture/decisions/adr-0005-tenant-identifiers-in-gameplay-protocol.md) |
...
| `SF-TCP-PROXY-IDENTITY` | `EDGE-03` | `revised`; applied | [ADR 0169](../../architecture/decisions/adr-0169-exclusive-environment-bound-tcp-proxy-trust.md), superseding [ADR 0010](../../architecture/decisions/adr-0010-tcp-proxy-identity-canonicalization.md), and [ADR 0038](../../architecture/decisions/adr-0038-explicit-jwt-profiles-and-mtls-workload-identity.md) |
| `GR-SESSION-FRONTEND-EXECUTION` | `SESSION-01` | `revised`; applied | [ADR 0170](../../architecture/decisions/adr-0170-fenced-command-forwarding-and-authoritative-region-transition.md), superseding [ADR 0011](../../architecture/decisions/adr-0011-gameplay-session-front-end-and-region-execution.md) |
