...
Status: Inventory and human-led review are complete and independently coverage/fidelity-audited. Packet 6 has 21 of 25 outcomes applied through `MS-PO-OWNER-REMEDIATION` and ADRs 0151-0165. `161` of `182` distinct decision keys have applied review provenance; the four-decision Packet 6 tail and 17 Packet 7 outcomes remain pending selective import. This parcel creates 6 new active ADR records (ADRs 0160-0165). The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias. ADR numbering records how applied outcomes were materialized but does not define the application boundary; the planned whole-corpus authority review follows completion of Packet 7.

## Implementation Status

`Complete` applies to inventory coverage and the human-owned review run, not to decision import or runtime implementation/proof. The checked provenance below records reviewed outcomes already applied and merged to `develop`; the unchecked Packet 6 tail (`SCRIPT-14`, `SCRIPT-15`, `TRACE-02`, and `TRACE-03`) and unchecked Packet 7 rows remain pending selective import. Canonical design and accepted ADRs define merged target state, while the domain trackers define implementation and proof status.
...
| **Decision keys / navigation rows** | **All 338 design sources: 335 allocated decision-bearing sources (172 allocated non-ADR documents and 163 ADR records), 1 exempt architecture decision registry/index, and 2 excluded governance/template exemptions; 335 allocated and 3 total exemptions** | **182 decision keys; 183 navigation/key rows including one historical service-scan alias** | **All 183 queue/navigation rows reviewed in the source archive** | **161 decision keys have reviewed outcomes applied cumulatively; the four-decision Packet 6 tail and 17 Packet 7 outcomes remain pending** |
...
The remotely backed `design/adversarial-decision-review` archive records the completed human-led review of all `183` queue/navigation rows. This section is the authoritative checked provenance for the `161` applicable outcomes currently applied and merged to `develop`: `applied` means the reviewed result is present in merged canonical design, not merely recorded in the archive. Every applied row is checked and must link that merged ADR or canonical design; the four Packet 6 tail rows and 17 Packet 7 rows remain unchecked pending import, and the one excluded navigation alias remains historical archive evidence only.
...
| 6 | Operations and delivery | 25 | 21 | `review-complete`; four-decision tail pending selective import |
| 7 | Existing ADR-backed and lower-risk remainder | 17 | 0 | `review-complete`; pending selective import |
| **Total** | | **183** | **161** | `review-complete`; partial application |

The source-archive total counts 183 navigation rows. Packet 2 contains 31 distinct decision keys plus the `MS-AA-TOKEN-REVOCATION` historical alias, producing 32 archive rows but only 31 applicable decisions. Packet 6 has 21 applied outcomes through `MS-PO-OWNER-REMEDIATION` and ADRs 0151-0165; `SCRIPT-14`, `SCRIPT-15`, `TRACE-02`, and `TRACE-03` remain as its four-decision tail. Packet 7 contains 17 reviewed outcomes through ADRs 0168-0178 and remains pending. This parcel creates 6 new active ADR records; the legacy ADR labels above are mappings to decision keys rather than additional rows. The later whole-corpus authority review remains required after Packet 7.
