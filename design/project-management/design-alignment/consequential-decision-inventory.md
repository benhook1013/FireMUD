...
The archive total counts navigation rows. Packet 2 contains 31 distinct decision keys represented by 32 rows; `MS-AA-TOKEN-REVOCATION` is a historical alias and does not add a decision. The 40 applied decisions on `develop` are Packet 1's nine decisions plus Packet 2's 31 distinct decisions.
