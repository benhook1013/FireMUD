...
Status: Inventory and human-led review are complete and independently coverage/fidelity-audited. Cumulative reviewed outcomes with applied provenance now cover 150 of 182 distinct decision keys. The Packet 5 tail moves `134→140` through six applied reviewed outcomes, bringing Packet 5 to 21 applied rows (active ADRs 0146-0150 plus superseded `SOCIAL-01` provenance); Packet 6 P0 then moves `140→150` through ten applied reviewed outcomes in active ADRs 0151-0159. The combined import creates 14 new active ADR records; `SOCIAL-01` is superseded provenance and creates no ADR. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias, and 32 reviewed decisions remain pending selective import. ADR numbering records how applied outcomes were materialized but does not define the application boundary.
...
| **Decision keys / navigation rows** | **All 332 design sources: 329 allocated decision-bearing sources (172 allocated non-ADR documents and 157 ADR records), 1 exempt architecture decision registry/index, and 2 excluded governance/template exemptions; 329 allocated and 3 total exemptions** | **182 decision keys; 183 navigation/key rows including one historical service-scan alias** | **All 183 queue/navigation rows reviewed in the source archive** | **150 of 182 decision keys have reviewed outcomes applied cumulatively, up from 134 before this combined import; Packet 5 increased from 15 to 21 through six applied outcomes, and Packet 6 P0 added ten through ADRs 0151-0159; 32 remain pending selective import** |
...
| 6 | Operations and delivery | 25 | 10 | P0: 10 applied, 15 `pending-import`; P1-P3 remain `pending-import` |
| 7 | Existing ADR-backed and lower-risk remainder | 17 | 0 | `pending-import` |
| **Total** | | **183** | **150** | `review-complete`; `import-in-progress` |

The source-archive total counts 183 navigation rows. Packet 2 contains 31 distinct decision keys plus the `MS-AA-TOKEN-REVOCATION` historical alias, producing 32 archive rows but only 31 applicable decisions. The applied-provenance checklist excludes that service-scan alias and therefore has 182 rows: 150 reviewed outcomes applied cumulatively, with the Packet 5 tail moving `134→140` and Packet 6 P0 moving `140→150`; 32 unchecked decision keys remain pending selective import. Packet 5 reaches 21 applied outcomes through six reviewed outcomes: five active ADR records 0146-0150 plus the superseded `SOCIAL-01` provenance linked to ADRs 0147, 0149, and 0150. Packet 6 P0 adds ten reviewed outcomes through nine active ADR records 0151-0159, with the coupled preflight pair sharing ADR 0152. The combined import creates 14 new active ADR records; the legacy ADR labels above are mappings to decision keys rather than additional rows.
