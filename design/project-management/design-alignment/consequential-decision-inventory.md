...
Status: Inventory and human-led review are complete and independently coverage/fidelity-audited. Cumulative reviewed outcomes with applied provenance now cover 143 of 182 distinct decision keys. The Packet 5 tail moves `134→140` through six applied reviewed outcomes, bringing Packet 5 to 21 applied rows (active ADRs 0146-0150 plus superseded `SOCIAL-01` provenance); the front of Packet 6 P0 then moves `140→143` through the compliance/preflight outcomes in ADRs 0151-0152. The separately tracked service-scan `MS-AA-TOKEN-REVOCATION` row remains the only excluded navigation alias, and 39 reviewed decisions remain pending selective import. ADR numbering records how applied outcomes were materialized but does not define the application boundary.

## Implementation Status

`Complete` applies to inventory coverage and the human-owned review run, not to decision import or implementation. The checked provenance below records only reviewed outcomes already applied and merged to `develop`; unchecked rows are reviewed in the source archive but remain pending selective import. Canonical design and accepted ADRs define merged target state, while the domain trackers define implementation and proof status.
...
- [Microservice decisions](./decision-inventory-microservices.md) contains 22 service-source rows = 21 current distinct decisions (20 active plus the formal superseded `MS-GR-EQUIPMENT-BODY-LAYOUT`) plus the navigation-only `MS-AA-TOKEN-REVOCATION` alias, and stronger evidence for 40 cross-cutting decisions from 76 covered microservice architecture paths (74 allocated evidence paths and 2 explicit governance/template exemptions). `MS-PO-OWNER-REMEDIATION` remains historical review evidence in this control ledger, not a current service-source row.
...
- [Product and operations decisions](./decision-inventory-product-operations.md) contains 38 decisions and stronger evidence for 11 existing keys from the remaining 38 product, frontend, authoring, protocol, infrastructure, deployment, recovery, observability, and generated-settings sources.

The canonical design allocation covers all 325 discovered product and architecture sources: 322 are allocated decision-bearing sources and 3 are explicit exemptions (the ADR registry/index plus two microservice governance/template documents). The four disjoint source-scoped partitions now account for all 174 non-ADR paths (`22 + 76 + 38 + 38`); 172 are allocated (`22 + 74 + 38 + 38`) and the two microservice governance/template paths are explicit exemptions. The 151-path ADR partition contributes 150 allocated ADR records plus the exempt ADR registry/index, yielding `174 + 151 = 325` discovered, `172 + 150 = 322` allocated, and 3 exemptions. The decision evidence set is 323 sources: the 322 allocated sources plus the exempt ADR registry/index; the two exempt microservice governance/template documents are discovered but excluded from decision evidence. A source may provide evidence without producing a distinct decision row. The source-scoped ledgers preserve unique primary ownership for the 182 distinct decision keys; cross-cutting evidence/reference rows may repeat existing keys across ledgers and do not add distinct keys. The current microservice service-source ledger has 22 rows: 21 non-alias current service decisions plus the superseded historical service-scan alias `MS-AA-TOKEN-REVOCATION`. The removed `MS-PO-OWNER-REMEDIATION` key remains historical review evidence below and is not a current service-source row; preserving that archive evidence leaves the source-scoped universe at 182 distinct keys and 183 key/navigation rows. The completed human-review archive likewise has one row for each of the 182 decision keys plus the historical service-scan alias. The nine legacy ADR labels in [Legacy ADR Alias Navigation](#legacy-adr-alias-navigation) map to nine of those decision keys; they substitute as historical navigation labels and do not add archive or provenance rows. The historical service-scan alias is the only extra navigation row and is excluded from applied-review disposition. Collectively, the inventories reference all 79 leaf capabilities in the taxonomy.

| Capability | Sources reviewed | Decisions inventoried | Human-review candidates | Coverage state |
| --- | ---: | ---: | ---: | --- |
| Existing ADR set | 150 ADR records (139 reviewed; 11 pre-formal) plus the architecture decision registry/index | 9 original aliases within the 68 cross-cutting decisions; later reviewed records are allocated directly | Complete in the source archive | Complete and allocated through ADR 0152 |
...
| Microservice architecture | 76 covered paths (74 allocated evidence paths; 2 explicit governance/template exemptions) | 22 rows = 21 current decisions (20 active + formal superseded `MS-GR-EQUIPMENT-BODY-LAYOUT`) + excluded historical `MS-AA-TOKEN-REVOCATION` alias; `MS-PO-OWNER-REMEDIATION` remains historical review evidence; stronger evidence for 40 existing keys | Complete in the source archive | Complete and independently audited |
...
| **Decision keys / navigation rows** | **All 325 design sources: 322 allocated decision-bearing sources (172 allocated non-ADR documents and 150 ADR records), 1 exempt architecture decision registry/index, and 2 excluded governance/template exemptions; 322 allocated and 3 total exemptions** | **182 decision keys; 183 navigation/key rows including one historical service-scan alias** | **All 183 queue/navigation rows reviewed in the source archive** | **143 of 182 decision keys have reviewed outcomes applied cumulatively, up from 134 before this front import; Packet 5 increased from 15 to 21 through six applied outcomes, and the Packet 6 P0 front added three through ADRs 0151-0152; 39 remain pending selective import** |
...
The remotely backed `design/adversarial-decision-review` archive records the completed human-led review of all `183` queue/navigation rows. This section is the authoritative checked provenance only for reviewed outcomes already applied and merged to `develop`: `applied` means the reviewed result is present in merged canonical design, not merely recorded in the archive. A checked row must link that merged ADR or canonical design. An unchecked row here means `reviewed, pending import`, not `unreviewed`.
...
The source-archive total counts 183 navigation rows. Packet 2 contains 31 distinct decision keys plus the `MS-AA-TOKEN-REVOCATION` historical alias, producing 32 archive rows but only 31 applicable decisions. The applied-provenance checklist excludes that service-scan alias and therefore has 182 rows: 143 reviewed outcomes applied cumulatively, with the Packet 5 tail moving `134→140` and the Packet 6 P0 front moving `140→143`; 39 unchecked decision keys remain pending selective import. Packet 5 reaches 21 applied outcomes through six reviewed outcomes: five active ADR records 0146-0150 plus the superseded `SOCIAL-01` provenance linked to ADRs 0147, 0149, and 0150. The Packet 6 P0 front adds three reviewed outcomes through ADRs 0151-0152, with the coupled preflight pair sharing ADR 0152. The legacy ADR labels above are mappings to decision keys rather than additional rows.
...
- [ ] `OPS-03` — `revised` on 2026-07-20; reviewed, pending selective import
- [ ] `RECOVERY-01` — `revised` on 2026-07-20; reviewed, pending selective import
- [ ] `RECOVERY-02` — `revised` on 2026-07-20; reviewed, pending selective import
- [ ] `OPS-02` — `revised` on 2026-07-20; reviewed, pending selective import
- [ ] `HEALTH-01` — `revised` on 2026-07-20; reviewed, pending selective import
- [ ] `OBS-04` — `revised` on 2026-07-20; reviewed, pending selective import
- [ ] `OBS-05` — `revised` on 2026-07-20; reviewed, pending selective import
