...
Current Unit 1B execution first checks design authority and corrects exposed Game Session behavior on the cumulative Account tail; the separate typed Account reader, Entity namespace/entry-policy, and durable reconnect-controller successors remain explicit activation dependencies rather than inferred from a green unit suite. After a coherent corrected checkpoint, independent reviewers assess the whole 16-source unit, unchanged sources, material code/proof, and cross-owner handoffs for missing or conflicting contracts, duplicated normative owners, broken handoffs, and misleading completion claims. Adjudicate and integrate useful roots between corrected-state passes; if three complete passes remain materially productive, report for Overseer judgment before more. A minor wording correction alone does not require two additional perfectly dry rounds. Section exit classifies every surviving root as an exposed correction, ordered pre-v1 successor with containment, deliberate safe deferral with activation gate, or exact missing proof.
