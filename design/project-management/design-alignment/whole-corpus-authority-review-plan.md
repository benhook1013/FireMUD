...
This is the maintained scope and method for the post-ADR corpus programme. The [alignment index](./README.md#corpus-section-programme-status) is the **only live section-status table**. This plan defines section boundaries and starting source manifests; neither file gives technical authority. Canonical product and architecture documents own target behavior, and implementation trackers own implementation/proof state.

## Purpose and sequence

The capability allocation, human-led adversarial ADR review, selective accepted-decision application, and point-in-time implementation reconciliation preceded this effort. The corpus programme checks the *whole* product and architecture corpus for missing or conflicting contracts, duplicate normative owners, broken owner/consumer handoffs, and misleading implementation or proof claims. It preserves useful local consequences while keeping one canonical owner. It is not another ADR decision round. Document also implements accepted pre-v1 requirements and their prerequisites before claiming a section complete; deliberately later target features remain outside that gate only with a documented reason and activation boundary.

The first stage catches up known required work; implementation and review then proceed section by section. Previous reviews, fixes, and evidence remain valid history, but an earlier review stop or taper does not establish the new completion gate. Reconcile live trackers as work changes target, implementation, proof, or gap status. After section completion, run a corpus-wide owner/secondary/tracker consistency check and the documentation validation gates before claiming the post-ADR programme complete.

## Section boundaries

The 23 original family sections remain the top-level corpus map, with 5B split into three semantic review subunits: 5B-Schema, 5B-Publication, and 5B-Retention. This produces 25 review units while preserving one Shared Runtime family and one 5B source union. The split responds to implementation expansion across migrations, services, runtime code, and proof; it is a semantic boundary, not a map of implementation PRs. The 21 recovered exact manifests from the pre-split plan each contained 10–18 existing source paths before heading-scoped overlap was counted. The earlier restart and its review coverage remain historical evidence; this method does not undo merged code or accepted ADRs. The two missing manifests, 7A and 7B, are supplied below. These numbered lists are starting anchors, not an exhaustive inventory of allocated sources. At section start, use the current [design allocation](./design-capability-allocation.md) to identify all primary sources and separately record material secondary handoffs. A source can be read in more than one section for a specific heading-scoped handoff; that does not create two normative owners.
...
**5B historical boundary:** the 18-source 5B corpus union was split into **5B-Schema** for SQL/schema/identifier evolution, **5B-Publication** for durable version/publication/readiness state, and **5B-Retention** for retention, cleanup, and recovery-safe durable state. Their independent reviews and paired whole-5B synthesis remain evidence of earlier alignment work, not final implementation-first section completion. A future completion attempt retains the three semantic boundaries and checks shared identities, migration gates, publication terminality, retention horizons, and owner handoffs across the union. Findings cross back to their owner; 5C, 5D, 6D, 7B, 7C, and other domain work remain explicit dependencies or independent handoffs. Implementation PRs are not section boundaries.

## Implementation and review method

### Phase 1: catch up known work

1. Preserve published PRs, corrected code, earlier review evidence, and finding dispositions. Reclassify an earlier “complete” section as **provisional** when accepted required pre-v1 implementation or essential proof remains. Do not replay historical review cycles merely because the completion rule changed.
2. Reconcile known findings and implementation-tracker rows against current code. Decide what is already resolved, required for pre-v1, or deliberately later. Record later work with its reason, containment or activation boundary, and revisit trigger. Document makes routine scope calls; bring consequential or genuinely ambiguous decisions to Overseer in small batches with a recommendation.
3. Order accepted pre-v1 work by dependency and implement it in coherent owner PRs, including shared prerequisites and cross-service consumers where the invariant requires them. Document owns this work unless another active worker owns the slice. Before substantial cross-service work, briefly verify the canonical design and producer/consumer contracts. The entire repository need not be feature-complete before a substantially implemented section resumes broad review.

### Phase 2: work through each section

1. Refresh the starting manifest against the current allocation, canonical owner links, ADR rationale, material secondary handoffs, live code, proof, and tracker gaps. Expand each ordinary section's numbered source list to cover its allocated primary sources; keep the 5B union and heading-scoped subunit boundaries. Check the target design briefly and resolve consequential ambiguity before implementation. Reuse earlier findings as inputs without replaying their review cycles.
2. Implement required pre-v1 behavior and the prerequisites needed for that section's current paths in coherent owner PRs, with focused proof and honest tracker status. Deliberately later features need an explicit reason and activation boundary. If a real dependency prevents progress, mark the section provisional and do another useful section or prerequisite slice rather than repeatedly scanning the blocked state.
3. Focused review sub-lanes may investigate difficult areas. For a section-closure attempt, assign **two fresh independent unguided reviewers in parallel**, each covering the complete changed and unchanged semantic section, material code and tests, proof claims, normative authority, and owner/consumer handoffs. Adjudicate their results as one paired round, deduplicated by underlying issue. Do not seed either reviewer with a list of prior findings. A focused check or PR-wide CodeRabbit review cannot substitute for this whole-section assessment.
4. Fix a material behavior or contract finding in its owner boundary and make another paired whole-section assessment of the corrected state. An isolated wording, fixture, or test correction needs focused validation and an explicit closeout judgment rather than an automatic broad restart. Numerical zero-finding streaks and arbitrary cycle counts do not decide corpus completion. If repeated independent side findings make the review unconvincing, leave the section provisional; do not create detailed partial-coverage records or narrow the return prompt to avoid those findings. Return later to a fresh unguided ground-up whole-section assessment after relevant implementation advances.
5. A finding that affects this section's current behavior or prerequisite blocks completion even when its code owner is an adjacent service. A genuinely independent side finding is recorded in its owning tracker/section and does not force this section's PR to absorb it. Split an unusually broad section into coherent semantic parts when needed, then perform a final whole/cumulative synthesis; PR topology alone never defines corpus scope.
6. Mark a section **corpus complete** only when accepted required pre-v1 behavior and prerequisites are implemented, the target design and implementation/proof claims agree, the final whole-section assessment is convincing, and no material section finding remains. Essential implementation proof that is absent keeps the section provisional; proof that necessarily requires post-merge activation stays a separate named gate. Update only the [alignment index](./README.md#corpus-section-programme-status) for current section status and concise evidence; keep detailed findings in owning trackers and PR/review artifacts. Merging a PR, completing its CodeRabbit review, or passing CI alone does not complete a corpus section.
7. After the first traversal, revisit **every provisional section** in dependency order as implementation and proof advance. Repeat that return sweep as often as needed; it is not a one-time second pass. Each return reassesses the section from current code and contracts with fresh unguided whole-section reviewers when closure is attempted. At final programme closeout, compare all refreshed source lists with the [allocation](./design-capability-allocation.md): each allocated source must have an owning section, an explicit heading-scoped handoff, or a reasoned exemption. Then run the corpus-wide owner/secondary/tracker consistency and documentation checks.
...
When the three subunits reach their implementation-first closure gate, make a paired unrestricted whole-5B synthesis over the complete 18-source union and every declared overlap. The synthesis must reconcile the durable identity tuple across schema and publication records; migration compatibility against published and retained versions; readiness terminality against retention and cleanup; owner-local foreign-key and mapping consequences; and the boundary from 5B durable state into 5C Redis, 5D transactions/replay/workflows, 6D deployment/reset/backup, and 7B/7C proof. A material synthesis finding returns to the owning subunit and requires corrected-state whole-5B assessment after the fix. Earlier tapered reviews remain historical alignment evidence; neither their counts nor child-PR closure establishes this final completion gate.
