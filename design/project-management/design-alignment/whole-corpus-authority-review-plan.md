...
| Shared runtime | 5A–5D | Shared primitives; SQL/durable state; Redis; transaction, replay and workflow patterns |
...
**5B decision before restart:** keep 5B as one 18-source *corpus review section*. SQL/schema evolution, owner-local durable records, retention, and publication/readiness must be checked together for cross-service authority conflicts. Its implementation expanded into several PRs, but PR packaging is not evidence that the 18-source review boundary is too large. Review it through explicit SQL/migration, durable-publication, and retention lenses, then synthesize one whole-5B owner/handoff result. If an actual fresh pass cannot keep those relationships in view, split the section in this plan and the index **before** another pass; do not silently redefine it through PR topology. Existing merged and open 5B PRs remain preserved implementation evidence. Reconcile and assign their remaining work when 5B resumes after 5A; do not count their historical reviews as new section coverage.

## Review and tracking method

1. Start with 5A. Before each section, refresh its starting manifest against current allocation, canonical owner links, ADR rationale, consumer handoffs, and live trackers. Expand that section's numbered list to include every allocated primary source within its boundary; identify heading-scoped handoffs and any explicit exemptions separately. Every numbered source path below existed when this plan was rebuilt, but inclusion is a review input, not an assertion that its wording remains correct.
2. Run a fresh broad pass over the **entire section**, including unchanged sources and material code/proof consequences. A focused check can answer a finding but does not replace broad discovery. Normalize findings by underlying issue; apply coherent fixes, route substantial implementation work to its owner, and retain honest tracker gaps.
3. Repeat broad passes on the corrected state while they find useful section work. Completion needs a coverage account for the manifest and handoffs, disposition of accepted findings, and an evidence-based taper judgment. No fixed pass count or demand for a review of every fix SHA overrides that judgment. PR-wide CodeRabbit and CI assess changed code and merge risk; they do not prove unchanged corpus coverage.
4. Update **only the alignment index** for section status, main PR, completion evidence, and routed obligations. A split implementation PR is linked from its main PR rather than creating another section row. Keep detailed finding evidence with the relevant review/PR artifacts, not a second live section-status ledger. The old local status JSON, dated handoffs, and per-PR working ledgers are historical evidence, never current programme state. When a real semantic section split is necessary, update both maintained documents together before assigning new rows or PRs.
5. A section is complete only when its refreshed source list and material handoffs have been assessed, valid findings are fixed or explicitly owned elsewhere, and the current section review has tapered. Record the reviewed/allocated source count and main evidence link in its index row. At final closeout, compare the union of the refreshed section lists with the [allocation](./design-capability-allocation.md): every allocated source must be reviewed in an owning section, explicitly handed off by heading, or listed as an exemption with a reason. Merging one PR alone does not establish section completion.
