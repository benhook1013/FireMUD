# LOOK and SAY Cross-Service Regression Suites

This document centralizes the cross-service regression plans for the `LOOK` and `SAY` text commands so WebSocket and Telnet experiences stay aligned.
...
- Issue `SAY` (and aliases like `YELL`/`WHISPER`) from one client and verify that:
  - All participants receive the canonical `OK SAY` payload with deterministic speaker and `Delivered-To` ordering.
  - Narrative rendering (for example, `Emberline says, "Hello travelers"`) is derived from the structured payload.
  - `gamesession.command.say.*` metrics are emitted as described in `design/project-management/vertical-slices/04-task-list-chat-and-social-vertical-slice.md`.
