...
- Replay the canonical `LOGIN` + `LOOK` sequence over both WebSocket and Telnet, using the transcript defined in [LOOK cross-service tests](./look-cross-service-tests.md).
- Assert that:
  - Response payloads match the canonical transcript for both transports (modulo framing).
  - `gamesession.command.look.*` metrics/logs are emitted as described in [LOOK instrumentation](./look-instrumentation.md).
