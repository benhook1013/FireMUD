...
Both smoke paths are manual convenience flows that fail closed on readiness. For Telnet, target proof follows the complete owner-defined [direct-text discovery and admission flow](../../architecture/system-architecture-authentication.md#login-and-session-flow) before `LOOK`; first-party WebSocket target proof follows its separate owner-defined [bootstrap and connect-token flow](../../architecture/system-architecture-authentication.md#websocket-connect-token-contract-wsgame). The current helpers perform the documented compatibility subset—required `WORLDS` discovery when applicable, then `LOGIN` → `PLAY` → `LOOK`—and compare the player-visible result with the canonical transcript. WebSocket coverage includes controlled `ROOM_NOT_FOUND` continuity; Telnet coverage preserves transcript parity and adds the isolated failure case when implemented. Readiness requires exact 2xx and top-level `status=UP`; the TCP Proxy path additionally requires ready listener and downstream traffic-admission fields. These flows supplement cross-service proof; optional diagnostics never replace the client-visible assertions.

## Implementation Status

The WebSocket and Telnet helper executables are the canonical service-local convenience flows and reuse the shared `dev-tools/smoke` library. They are not canonical CI/runtime proof until the exact-2xx/top-level-JSON readiness predicate is implemented. Current implementation caveat: `dev-tools/smoke/smoke_common.py` accepts readiness when HTTP status is below 500 and a whitespace-normalized `"status":"UP"` substring appears anywhere in the body, including nested JSON; it retries readiness only, not the complete `WORLDS` → `LOGIN` → `PLAY` → `LOOK` flow. Therefore these helpers are not proof of the canonical exact-2xx/top-level-JSON readiness predicate or of the full flow until that implementation gap is fixed. The preselected-local-target `WORLDS` bypass described below is current compatibility behavior only; it does not validate the target discovery prerequisite, which remains `WORLDS` before `LOGIN`. Each helper must wait for the canonical readiness endpoints for the path under test and fail if readiness does not converge; it must not mask startup races by timing out and continuing or by rerunning the full smoke until the stack eventually stabilizes. The shared fail-closed readiness semantics are documented in [Deployment Environments](../../architecture/infrastructure/deployment-environments.md#docker-health-checks) and [TCP Proxy readiness](../../architecture/microservices/tcp-proxy-service/README.md#readiness-and-liveness).
...
6. (Optional) Record the relevant counters before `LOOK`, then read the same endpoint afterward and require `after - before == 1` for the exercised outcome. `/actuator/prometheus` exposes `gamesession_command_look_invocations_total` and `gamesession_command_look_failures_total{error="..."}`; the Micrometer registry uses `gamesession.command.look.invocations` and `gamesession.command.look.failures{error="..."}`. Do not use an absolute counter value because prior traffic may already have incremented it.
...
2. For a fresh direct-text flow, issue `WORLDS` and wait for its successful discovery response before `LOGIN`; local runs with a preselected target may intentionally skip discovery as a current compatibility exception only, not as proof of the target discovery prerequisite. The target flow remains `WORLDS` before `LOGIN`. Send `LOGIN demo@example.com swordfish` and expect the same `OK LOGIN` line as the WebSocket script; this is the baseline flow and does not require a `SESSION` envelope.
...
- When replaying the scripts, capture the endpoint-specific LOOK counters and before/after delta described above, plus optional Game Session/Game Logic diagnostics for the `ERROR <CODE>` mappings, `PlayerOutput`, and deterministic text projection. These diagnostics supplement the player-visible transcript and are not automated cross-service proof.
...
- Run `./gradlew crossServiceTest` to replay the automated LOOK flows. Current proof is limited to the WebSocket controlled `ROOM_NOT_FOUND` error, connection continuity, and successful transcript, plus successful Telnet transcript parity; the Telnet missing-room/error/continuity case is not implemented. Instrumentation counters/logs remain supplementary diagnostics.
