...
- `LookWebSocketCrossServiceTest` boots Game Session, Game Logic (pointed at stubbed World/Entity servers), Redis, Postgres, and the Gateway stub. `GameplayCrossServiceStack` creates an `AccountRuntimeStubServer` on an ephemeral port before starting Game Session, then wires that endpoint as `firemud.services.accountService=localhost:<port>`; there is no separate Account health probe, while `GameplayWebSocketScenarios.openReady` waits through the login/play/readiness transcript before `LOOK`. Its fixture provisions a running game instance directly and retains the end-to-end assertion of `LookTestFixtures.canonicalLookText()`. Structured `LookResult`, typed-failure, `PlayerOutput`, and text-projection records are optional owner-local diagnostics; the cross-service assertion is the player-visible transcript and, for the controlled `ROOM_NOT_FOUND` case, connection continuity with a follow-up command.
- `TelnetGatewayGameSessionAccountCrossServiceIntegrationTest` uses the same `GameplayCrossServiceStack` AccountRuntimeStubServer mechanism, including the ephemeral Account endpoint wired into Game Session before the stack starts. Its `GameplayTelnetScenarios` helper waits for initial connection guidance and the successful login/play readiness transcript; the test's Spring `@DynamicPropertySource` supplies the separately started Gateway WebSocket URL and TCP Proxy defaults, not a second Account fixture. It provisions its running instance directly and drives `LOGIN` → `PLAY` → `LOOK` (with `WORLDS` only where the scenario requires discovery). It currently proves successful Telnet transcript parity with the WebSocket output; an isolated missing-room Telnet case using a controlled World stub remains future coverage.
