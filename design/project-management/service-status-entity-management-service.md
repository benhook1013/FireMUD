...
- Character creation and gameplay-scoped character lookup now use the canonical resolved playable-state scope, and character read surfaces echo that scope back to callers instead of hiding realm policy behind an internal state-key derivation.
