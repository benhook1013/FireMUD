...
- Exercise stage-aware dead-letter recovery: evaluation-stage retry uses the original frozen manifest and graph; post-evaluation recovery replays stored output and unfinished child dispatches without DSL re-entry; both paths require exact current matches for the admitted patch/epoch, plugin identity/activation, component policy, capability grants, signer evidence, runtime region/regionEpoch, and routing bundle; missing or contradictory evidence remains dead-lettered; purge is separate and audited. Target/capability-gated API-shape proof rejects empty-ID, filter-only, and mixed explicit-ID-plus-filter mutation requests, and permits only bounded nonempty explicit dead-letter work-item IDs with persisted per-row outcomes. Duplicate `controlPlaneRequestId` requests return the stored result without re-evaluation or redispatch.
