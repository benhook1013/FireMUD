...
- Test Game Session's atomic exact-pin tuple and append-only rollout-history boundary, including failure rollback, concurrent first pin, tagged precondition conflicts, exact-request idempotency, and new same-version repin. An exact retry must not advance `scriptPinEpoch` or append another logical history row. When the `ScriptPatchPinChanged` advisory family is enabled, a same-version repin advances `scriptPinEpoch` and the producer emits exactly one occurrence only after the owner pin and history commit; exact retries and no-ops emit none. Its downstream delivery remains a lossy advisory reread wake-up, so loss, duplication, delay, or reordering must recover through owner reads and cannot become pin or history authority. Test producer-owned version/epoch/cursor projection application separately: strictly newer producer tuples advance the projection, while lower, reordered, or contradictory equal updates are rejected without replacing owner truth; replay after a gap rereads the owner and does not infer ordering from notification arrival. The advisory wake-up may be observed before or after a consumer applies its projection, so it proves neither projection freshness nor lifecycle completion. Separately test the named `ScriptPinConvergenceTimedOut` producer consequence: exactly one event is captured in the same transaction as the terminal timeout state for each `convergenceAttemptGeneration`; that generation is distinct from `scriptPinEpoch`, while downstream delivery remains advisory and consumers reread the owner. See [ADR 0120](../architecture/decisions/adr-0120-owner-read-first-control-plane-notifications.md), the [notification and recovery contract](../architecture/system-architecture-scripting-control-plane-events.md#notification-and-recovery-contract), and [targeted durable delivery](../architecture/system-architecture-scripting-control-plane-events.md#targeted-durable-delivery).

- Test instance-bound admission, scheduling, retry, handoff, and final effects with the authoritative `(scriptPatchVersion, scriptPinEpoch)` fence; reject missing, stale, or mismatched evidence without local fallback. Keep tenant-readiness `onLoad` as the explicit pre-pin exception. For fan-out and replay, assert that child Command-Handoff Identity contains complete source scope, optional target scope only when the runtime target is distinct, persisted `automationDispatchId`, and deterministic `commandOrdinal`; same-instance handoffs omit every optional target field, while the parent Trigger Identity and `scriptEventId` remain correlation-only for the child and `outboxWorkItemId` remains correlation-only for child identity but authoritative for locating/replaying the parent work item. See the [Normative Contract Tables](../architecture/system-architecture-scripting-normative-contract-tables.md#command-handoff-identity-target-state).
...
- Test promotion and rollback pin-transition workflows against the [pin-transition state machine](../architecture/system-architecture-scripting-rollout-and-rollback.md#pin-transition-orchestration-state-machine-required), including atomic initial `convergenceAttemptGeneration=1` workflow creation, pin-commit failure, repeated/concurrent repins, one workflow/request identity carried through every step via `controlPlaneRequestId`, explicit `RecoverScriptPinConvergence` CAS against the expected timed-out generation, exactly-once generation allocation and recovery mapping, exact recovery retries returning the stored resulting generation/deadline, changed recovery fingerprints conflicting, and retries of the original promotion/rollback not recovering, generation-bound Game Session/Automation acknowledgments, immutable `operationKind` exactly `SET`, `ROLLBACK`, or `REPIN`, exact retries returning the stored workflow/terminal result without restarting or mutating history, promotion and rollback convergence timeout, same-workflow timeout recovery idempotency, exactly one durable terminal timeout outcome and one `ScriptPinConvergenceTimedOut` event per `convergenceAttemptGeneration` for each distinct terminal convergence attempt, with retries/recovery unable to duplicate the timeout event for an existing generation; `convergenceAttemptGeneration` is distinct from `scriptPinEpoch`. Also prove a separate `automation_pin_convergence_timeout_total` metric increment with labels `scope`, `operation`, and `reason` at the terminal transition without requiring restart/replay exactly-once metric semantics, and terminal `pin_convergence_timeout` admission. For rollback specifically, test admission barriers, cancellation and cleanup ambiguity, and preserve its pause/backpressure assertions. Preserve ordinary gameplay while scoped Automation work drains on the routine fenced path. Separately prove that a full gameplay pause is accepted only for a declared unfenced effect family, migration, or compatibility transition that cannot enforce the final `scriptPinEpoch` fence, with the smallest complete scope and quiescence evidence.

- Test stage-aware dead-letter recovery and claim reclamation with stable identities, exact `(scriptPatchVersion, scriptPinEpoch)`, plugin `(pluginId, pluginVersionId, bindingId)` plus activation/lifecycle, component-policy, capability-grant, and signer/publication evidence, captured `pluginActivationEpoch`, runtime `regionEpoch`, and admitted routing bundle; revalidate immediately before evaluation and immediately before each post-evaluation child dispatch; prove stale-owner CAS rejection, bounded selectors, and no DSL re-entry after evaluation. For `ReplayDeadLetteredWorkItems`, prove bounded non-empty explicit parent `workItemIds[]`; each public ID resolves atomically to its internal `(tenantId, outboxWorkItemId, failureGeneration)` parent identity; an exact duplicate `controlPlaneRequestId` plus canonical request fingerprint returns the stored per-item results without new work, while a changed fingerprint conflicts; and immutable evidence binds `(tenantId, outboxWorkItemId, failureGeneration)`. Prove recovery lease reclaim preserves recovery-attempt identity and committed evidence while advancing the canonical owner fence through reclaim CAS. Purge remains separate and audited; tenant-readiness stale `ONLOAD_RUNNING` is terminalized by its recovery owner with exact `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and `finalReason=stale_execution_fenced`, without DSL re-entry or any new `scriptEventId`, Trigger Identity, or readiness identity; retry requires a newly republished immutable `scriptPatchVersion`/readiness identity. See the [operator replay contract](../architecture/system-architecture-scripting-control-plane-operations.md#replaydeadletteredworkitems). This is target-state proof focus only and does not claim current implementation proof.
