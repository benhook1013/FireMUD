...
- Exercise stage-aware dead-letter recovery: evaluation-stage retry uses the original frozen manifest and graph; post-evaluation recovery replays stored output and unfinished child dispatches without DSL re-entry; both paths require exact current matches for the admitted patch/epoch, plugin identity/activation, component policy, capability grants, signer evidence, runtime region/regionEpoch, and routing bundle; missing or contradictory evidence remains dead-lettered; purge is separate and audited. For tenant readiness, target proof must also show stale `ONLOAD_RUNNING` is terminalized by the recovery owner as audited `finalReason=stale_execution_fenced`, without DSL re-entry or minting a new `scriptEventId` or step identity; the current executor gap leaves stale `ONLOAD_RUNNING` unreclaimed, so this remains target-only proof. Target/capability-gated API-shape proof rejects empty-ID, filter-only, and mixed explicit-ID-plus-filter mutation requests, and permits only bounded nonempty explicit dead-letter work-item IDs with persisted per-row outcomes. An exact retry with the same `controlPlaneRequestId` and canonical request fingerprint returns the stored per-row outcomes without re-evaluation or redispatch; reusing that request identity with a conflicting fingerprint deterministically conflicts and performs no mutation, evaluation, or redispatch.
