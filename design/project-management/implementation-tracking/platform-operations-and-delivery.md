...
| [Basic Multiplayer Load Proof Vertical Slice Task List](../vertical-slices/05.1-task-list-basic-multiplayer-load-proof-vertical-slice.md) - Multiplayer load proof | implementation-complete for the bounded first-pass proof, including deterministic ten-client WebSocket entry, movement/LOOK churn, SAY acceptance, and mixed WebSocket/Telnet movement parity; higher-volume `load-testing` remains future work | 1-137 | [source evidence](#source-05-1-task-list-basic-multiplayer-load-proof-vertical-slice-1-137) |
...
`check-metrics-cardinality.py` scans service metric-building code, canonical observability documentation, metrics catalogs, and shipped Prometheus rules. It rejects forbidden raw identifier labels in metric examples as well as PromQL `sum by (...)` and `on (...)` clauses. Canonical Redis and scripting examples use bounded `scope`/`scope_type` and the documented bounded vocabulary (`script_category`, `plugin_family`, `plugin_version_family`, `component_class`, and `tier`); tick/tail-loss recording rules aggregate and join on bounded `scope`.

Gateway gameplay-handshake rejection metrics use only the canonical route, HTTP status, and handshake error class. Gateway-originated client closes are normalized to the bounded close/subreason taxonomy and counted through `gateway.websocket.closes`; outbound bridge-buffer pressure closes use `policy_violation/edge_backpressure` and increment the dedicated slow-client subset meter. Arbitrary upstream close text is not exposed as a metric label.
...
Preflight can process `FIREMUD_TRAFFIC_OPEN_EVENT=first-live|reopen` and optional evidence, and production/hobby backup gates consume canonical evidence records. `write-traffic-open-evidence.py` writes those records; the gates require schema version, operator identity, non-empty evidence references, and a canonical referenced preflight report. This is executable report/evidence validation, but actual first-live/reopen evidence still has to be produced by an operator or automation. The staging/production Kustomize path is canonical, renders no placeholder player-facing credentials/TLS resources, supports immutable image/digest control, and uses the same binding contract. Hosted preview/dev-demo render checked-in baseline internal-service `NetworkPolicy` resources through Helm.
...
The canonical Redis coordination reset/recovery sequence is the sole normative command sequence in `system-architecture-redis-operations.md#canonical-coordination-reset-sequence`. Incident, reset/recovery, backup/restore, and related runbooks link to it and retain only scenario-specific scope, session-policy, evidence, and abort details. The documented `coordination-maintenance` CLI/control-plane surface itself is not implemented by this tooling slice.
...
Game Session is the persisted runtime feature-flag authority. Logging & Admin retains privileged HTTP/gRPC operator ingress and audit, forwards mutations to Game Session, and no longer has a duplicate feature-flag table/entity/repository/mapper/migration/seeding path. Moderation records are policy/control-plane input, not the destructive enforcement action; account-security and other consequences remain with their owning executors. Logging & Admin also exposes the bounded tenant-qualified compatibility readback `GET /admission-pointers/version-upgrades/{tenantId}/{sourceGameInstanceId}/compatibility/{targetVersionId}`, returning the canonical Game Session result, reasons, checked participants, optional remap set, and participant details after tenant authorization, without creating a prepared-upgrade row. Its scoped tick-remediation pause/resume forwarding is live, but quota-override ingress and the broader tick-remediation `remediate` route remain design-level contracts rather than owner-backed routes. Richer embedded observability and real-time moderation tooling likewise remain future application work.
...
- Production and hobby traffic-open gates validate evidence records and referenced preflight reports, but operators or automation still need to produce and retain actual first-live/reopen evidence before traffic is opened.
- JWT/JWKS file mounting and preview-unique rendering are live; automated credential rotation and its operational proof are not.
...
| Deploy preflight tooling | Binding identity, secret, certificate, external-isolation, discovery, traffic-open, and policy validation | `dev-tools/deploy/preflight.py`, evidence writer, and manifest contracts |
