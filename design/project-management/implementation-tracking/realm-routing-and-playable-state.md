...
The shared `GameplayAdmissionPointerSnapshots` rule defines a complete pointer as one with positive `tenantId`, positive `gameInstanceId`, positive `pointerVersion`, nonblank `worldSlug`, `realmSlug`, and `stateScope`. Singular runtime-target authority exists only when exactly one complete pointer survives. A lone partial row, missing runtime identity, missing scope, or multiple current pointers is no authority for singular routing. Raw pointer lists still expose such rows for operator inspection; only singular projections fail closed.
