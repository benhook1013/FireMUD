...
- Blank-scope login/bootstrap command staging currently retains the normalized shell selector bundle with `playableStateScope=UNSPECIFIED` when runtime-target authority is absent, ambiguous, or incomplete. This exception still needs convergence with the all-or-none admitted-routing rule and focused no-bundle proof.
