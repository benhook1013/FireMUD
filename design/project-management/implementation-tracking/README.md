...
These files are the permanent domain-oriented implementation tracking surface. They do not define product requirements, observable product behavior, or technical contracts. Product authority remains under [`design/product`](../../product/README.md), technical-contract authority remains under [`design/architecture`](../../architecture/README.md), and the [Capability Allocation](./capability-allocation.md) assigns every leaf in the [Product Capability Taxonomy](../../product/capability-taxonomy.md) to exactly one primary tracker and records secondary handoffs.
...
Use the relevant tracker as the reader-facing account of implementation status. Keep its status, implementation record, validation, gaps, decisions, and service-map sections aligned when a domain boundary changes, while keeping product requirements and observable behavior in the canonical product documents and technical contracts in the canonical architecture documents.
