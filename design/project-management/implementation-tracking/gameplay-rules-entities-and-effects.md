...
The live implementation is strongest in item containment and transfer, durable effect replay, and the first evaluated actor-state/effect seams. The broader authored-action, generic actor, targeting, resource-cost, and combat models remain incomplete. The source appendix is preserved as an audit backstop; the sections above it are the reader-facing implementation record.
...
| [Data-Driven LOOK Vertical Slice Task List](../vertical-slices/03-task-list-data-driven-look-vertical-slice.md) - Visible-entity and containment-model facts | visible-entity listing, deterministic fixtures, and LOOK-oriented tests/docs are implemented; presentation remains intentionally bounded | 41-51 | [source evidence](#source-03-task-list-data-driven-look-vertical-slice-41-51) |
...
| [Entity Stats and Conditions Vertical Slice](../vertical-slices/07-task-list-entity-stats-and-conditions-vertical-slice.md) - Entity stats and conditions | first evaluated actor-state substrate, shared effect evaluation, equipment contributions, and transient action-state execution are live; generic authored state and combat remain future work | 1-343 | [source evidence](#source-07-task-list-entity-stats-and-conditions-vertical-slice-1-343) |
| [Shared Effect Engine Vertical Slice](../vertical-slices/07.1-task-list-shared-effect-engine-vertical-slice.md) - Shared typed effect evaluation | first typed Entity Management seam is live for conditions, equipped items, replay-guarded action states, and one release-admitted authored action effect; broader effects and combat consumers remain future work | 1-143 | [source evidence](#source-07-1-task-list-shared-effect-engine-vertical-slice-1-143) |
| [Equipment and Action-State Contributions Vertical Slice](../vertical-slices/07.2-task-list-equipment-and-action-state-contributions-vertical-slice.md) - Equipment and action-state effect contributions | first equipment contribution path is live, and the first transient action-state command path applies `blocking` through the shared actor-state/effect seam; richer combat-facing consumption remains future work | 1-67 | [source evidence](#source-07-2-task-list-equipment-and-action-state-contributions-vertical-slice-1-67) |
...
Transfer failures emit canonical warning context including item instance/item identity, expected and current holder, destination, verb/cause, and actor character when available. Successful item-instance and stack-backed transfers write `item_transfer_audits` in the same local transaction boundary as the mutation. Audit records carry verb, actor character, item identity, quantity or stack-family delta, source/destination holder context, and available room/game/session/effect correlation. Durable item/equipment/container mutation requests carry attested `sessionId` and durable `effectId`; when present, `effectId` is the explicit audit correlation key. A deterministic fallback correlation key is derived from verb, actor, item, quantity/family, and holder endpoints, but this is not broader replay semantics.
...
Game Session persists a stable same-batch `effectKey` and deterministically derives `effectId` from `tickBatchId` plus that key. The current key vocabulary is `command:<commandId>` when a durable command id exists, or `command-text:<hash>:slot:<index>` for a queue entry without one. Same-batch duplicate keys are rejected. Durable drained effects resume after restart and, after an ownership-fence check, converge to `APPLIED`, `REPLAY_NOOP`, or `REJECTED`; stale executors abandon and requeue rather than applying under the wrong fence.
...
Equipped item payloads and active condition payloads contribute through this evaluator. `ApplyActorCondition` is gameplay-attested, validates nonblank source/effect identity and canonical effect JSON before replay lookup, and is routed by Game Logic. `BLOCK`/`GUARD` is the first player-facing transient action-state path: durable Game Session execution applies a short-lived `blocking` condition with a `block_mitigation` payload, using the durable effect id as source provenance. Entity Management removes elapsed transient states through its scheduled expiry job. Game Session no longer infers state mutation from broad tags such as `COMBAT`; only explicitly declared behavior routes to a concrete mutation.
...
- The broad authored target-resolution and structured-outcome model is not live: cross-actor target sets, canonical candidate/fact snapshots, target-resolution evidence, `ResolvedEffectPlan` application, remote legs, semantic presentation events, and general required/optional outcomes remain design work beyond the self-targeted v1 path. The locked boundary is Game Logic-owned resolution from frozen policies and owner fact snapshots/tokens, followed by idempotent Entity Management application; Game Session must not choose final actor identities. Candidate selection is explicitly bounded and deterministic, observation failures must not disclose hidden facts, same-region required legs commit atomically or reject, and cross-region legs use durable follow-up coordination rather than claimed distributed atomicity. The future `GameplayActionOutcome` separates source `commitState` from `completionState`, carries ordered target-leg outcomes, and drives idempotent semantic presentation events rather than treating free-form text as gameplay truth.
...
The exact allocated source records and historical checklist material remain below solely for audit. They are not required to discover the current implementation state summarized above.
