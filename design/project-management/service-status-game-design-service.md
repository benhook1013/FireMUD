...
- The repo already reflects substantial design-time workflow modeling for templates, publishing, runtime flags, script patches, and plugin lifecycle, and now includes a live plugin publication metadata path via `PublishPluginVersion` and `GetPublishedPluginVersion`, including signer key, signer revocation, and component-policy decision metadata consumed by Automation activation gates.
- Version-scoped asset publication now has a real control-plane lifecycle: publish proof, exact-bytes repair, tombstoning, and CAS-guarded purge all exist in the live service path, and purge eligibility already fails closed on current version-state, release-bundle, launch-descriptor, and approved remap-set references.
...
- Signed bundle upload/extraction, richer signer-validation history, and broader creator-tooling surfaces remain future implementation work.
