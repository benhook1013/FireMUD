...
- [x] Ensure the log-only profile keeps the LOGIN path runnable by providing a lightweight stubbed `SessionContextService` that replaces the Redis-backed implementation whenever `game-session.log-only=true`, and document the TODO so future work knows to drop the stub when Redis is deployed (`services/game-session-service/src/main/java/net/firedevops/firemud/service/impl/RedisSessionContextService.java`).
- [x] Teach the log-only `GameInstanceService` (and any supporting components) to persist or expose a fake `GameInstance` record so `LoginCommandHandler` can resolve the session row instead of returning `SESSION_NOT_FOUND`, and leave a clear migration note that this logic should be removed once Postgres-backed persistence is available (`services/game-session-service/src/main/java/net/firedevops/firemud/service/logonly/`).
- [ ] Audit tests that currently rely on these log-only stubs or unstarted services; wrap them with the proper Spring profiles or `@Disabled("depends on TODO-service")` annotations and add corresponding TODO reminders, so the suite can be flipped to real-service mode later without silent failures.
- [ ] Update the developer documentation (smoke tests section, setup guides, etc.) with notes about the temporary stubbed services and the steps required to replace them with the production-grade Account/Redis/GameInstance implementations when those services land.
