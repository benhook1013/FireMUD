...
What remains open is wider cutover consumers beyond the now-live stop and replacement-session termination paths, the remaining launch failure vocabulary around remap requirements and later patch/readiness cases, and fuller instance-topology materialization beyond the now-live `world_instance` / starter `region_instance` substrate.
