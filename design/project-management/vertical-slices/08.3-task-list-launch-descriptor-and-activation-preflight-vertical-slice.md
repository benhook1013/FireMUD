...
What remains open is wider cutover consumers beyond the now-live stop and replacement-session termination paths, the later multi-domain cutover-compatibility orchestration that will consume `remapSetId`, and later runtime world-state families beyond the now-live topology and runtime-event substrate.
...
- extend the now-live approved-remap launch-descriptor substrate into later cutover-preflight/orchestration consumers instead of leaving `remapSetId` proof isolated to replacement launch resolution.
