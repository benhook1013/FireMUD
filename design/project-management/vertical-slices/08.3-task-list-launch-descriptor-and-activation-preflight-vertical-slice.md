...
What remains open is wider cutover consumers beyond the now-live stop, replacement-session termination, compatibility-preflight path, persisted preparation record, and canonical execution path; richer Entity `S2` target-template validation beyond the current approved-remap identity fence; and later runtime world-state families beyond the now-live topology and runtime-event substrate.
