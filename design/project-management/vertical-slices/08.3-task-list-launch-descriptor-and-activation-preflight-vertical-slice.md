...
- World Management now persists `world_instance`, `region_instance`, `zone_instance`, `room_instance`, and `room_instance_exit` rows keyed by `(tenantId, gameInstanceId)` and revalidates release-bundle, published-asset artifact proof, and `versionStateEpoch` proof before activation opens.
...
What remains open is wider cutover consumers beyond the now-live stop and replacement-session termination paths, the later approved-remap control-plane model behind the first `LAUNCH_REMAP_REQUIRED` fail-closed seam, and later runtime world-state families beyond the now-live topology and runtime-event substrate.
...
- extend the now-live `ResolveLaunchDescriptor` path from the first typed missing-attestation/remap-required outcomes into the later approved-remap control-plane model instead of leaving replacement launches permanently fail-closed.
...
- [ ] Implement the slice end to end.
  The first canonical launch-descriptor persistence, typed missing-attestation/remap-required launch outcomes, Game Session and World Management asset-proof preflight, World Management activation lifecycle, instance-backed world/region/zone/room topology, and runtime-scoped world-event cleanup are now live, but broader cutover consumers, the later approved-remap model, and later runtime world-state families are still open.
