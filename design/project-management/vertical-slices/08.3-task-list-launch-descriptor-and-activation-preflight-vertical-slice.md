...
What remains open is completion of activation-side consumers and wider version-state/cutover adoption rather than launch-descriptor existence itself.
...
- move World Management activation/create flows onto the resolved launch-descriptor seam so the same preflight proof is used end to end instead of stopping at Game Session instance creation.
- extend the now-live Game Design `GetVersionState` / `CompareAndSetVersionState` seam into activation/cutover consumers so launch, replacement, and retirement-safe transitions share one control-plane fence.
- prove that failed launch preflight leaves no committed runtime rows and no World `PREPARING` state across the full activation path rather than only the current Game Session start-session seam.

## Checklist

- [x] Define target-state behavior and scope.
- [ ] Implement the slice end to end.
  The first canonical launch-descriptor persistence and Game Session preflight seam are now live, but World Management activation/cutover consumers and the full failure vocabulary are still open.
