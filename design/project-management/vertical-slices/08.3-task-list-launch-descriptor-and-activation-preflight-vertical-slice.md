...
- Game Session `StartSession` now resolves launch descriptors and verifies the attested `published_release_bundle` before any `game_instances` row is created;
...
What remains open is completion of activation-side consumers rather than launch-descriptor existence itself.
...
- strengthen version-state epoch handling from the current first stored fence to full CAS-safe activation/cutover proof once the world-activation side is on the same contract.
