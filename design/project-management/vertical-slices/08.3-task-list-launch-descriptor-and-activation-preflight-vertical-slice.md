...
What remains open is wider cutover consumers beyond the now-live stop, replacement-session termination, and first compatibility-preflight path; deeper `PrepareVersionUpgrade` workflow state; richer Entity/World `S1` / `S2` domain checks; and later runtime world-state families beyond the now-live topology and runtime-event substrate.
...
- extend the now-live approved-remap launch-descriptor substrate and first cutover-preflight seam into later `PrepareVersionUpgrade` / admission-pointer-cutover orchestration consumers instead of stopping at compatibility reporting.
...
- [ ] Implement the slice end to end.
  The first canonical launch-descriptor persistence, typed missing-attestation/remap-required launch outcomes, approved remap-set control-plane APIs, replacement-launch `remapSetId` resolution/persistence, Game Session and World Management asset-proof preflight, World Management activation lifecycle, instance-backed world/region/zone/room topology, runtime-scoped world-event cleanup, and the first cross-service `ValidateInstanceCutoverCompatibility` seam are now live, but broader cutover workflow state and richer `S1` / `S2` participant checks are still open.
