...
- World Management now persists `world_instance`, `region_instance`, `zone_instance`, `room_instance`, and `room_instance_exit` rows keyed by `(tenantId, gameInstanceId)` and revalidates release-bundle / `versionStateEpoch` proof before activation opens.
...
What remains open is wider cutover consumers beyond the now-live stop and replacement-session termination paths, the remaining launch failure vocabulary around remap requirements and later patch/readiness cases, and the later runtime world-state tables beyond the now-live `world_instance` / `region_instance` / `zone_instance` / `room_instance` topology substrate.
...
- [ ] Implement the slice end to end.
  The first canonical launch-descriptor persistence, Game Session preflight seam, World Management activation lifecycle, and instance-backed world/region/zone/room topology are now live, but broader cutover consumers, later runtime world-state families, and the full failure vocabulary are still open.
