...
Goal: stop holding local database transactions open while workflow services perform remote side effects, and move those side effects to explicit coherent transaction boundaries. Status: implemented for the currently audited workflow outliers.
...
That closes the originally audited workflow outliers for this slice.
...
This slice is complete for the known outliers. Reopen it only if a new workflow path is introduced that:
