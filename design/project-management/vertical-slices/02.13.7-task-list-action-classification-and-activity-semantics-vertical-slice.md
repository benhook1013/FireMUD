...
- direct `onCommand` script-event publications that synthesize `GameplayCommand` records from parsed text commands now also preserve the original raw command text through one shared helper, so built-in and authored alias usage continues through the scripting seam instead of disappearing on non-durable command paths before canonical alias/category resolution runs;
