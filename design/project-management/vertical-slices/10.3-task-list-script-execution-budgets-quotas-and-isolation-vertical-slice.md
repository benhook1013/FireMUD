...
- keep dry-run/test isolation on the same canonical budget model rather than a sidecar shortcut; live quota bypass and first per-minute dry-run rate limits are now enforced, but dedicated dry-run concurrency/capacity reservation remains follow-through.
