...
Implementation is underway. The first concrete cuts wire the documented terminal outbox retention knobs into Automation & Scripting configuration and a scheduled cleanup path for `HANDED_OFF`, `CANCELED`, and `DEAD_LETTERED` `script_work_items`, enforce the serialized work-item payload-size ceiling at event ingress before persistence, enforce command-count output ceilings when claimed work items are evaluated, charge live per-script quota at handler admission before durable work-item materialization, enforce dedicated dry-run per-minute tenant/principal quotas before handler resolution, skip live `ScriptQuotaService` charging for dry-run executions, and make Automation pin/rollout projection freshness use the supported `SCRIPT_PIN_PROJECTION_STALE_THRESHOLD_MS` runtime knob instead of a hardcoded service threshold. The heavier tenant-budget charging, dry-run concurrency/capacity reservation, signer-policy freshness, and broader sandbox execution policy paths still remain.
...
- extend the now-live admission-time per-script quota charging into broader tenant-budget accounting.
...
- [ ] Implement the slice end to end.
  ...
  - [x] Dry-run execution skips live per-script quota acquisition and never hands commands to Game Session.
