...
Goal: make per-script quotas, tenant automation budgets, sandbox isolation, and admission-time fairness one bounded execution slice so runtime behavior under load is governed by one canonical policy model instead of scattered knobs. Status: planned.
