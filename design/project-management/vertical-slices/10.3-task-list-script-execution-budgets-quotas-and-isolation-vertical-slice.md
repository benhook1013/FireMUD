...
- implement and wire the remaining signer-policy-freshness knobs into the owning runtime paths; outbox-retention cleanup, serialized work-item payload-size rejection, first command-count output ceilings, and pin-projection freshness are now live.
...
- [ ] Implement the slice end to end.
  ...
  - [x] Claimed work-item evaluation rejects command fan-out that exceeds documented per-run and per-entity command ceilings.
