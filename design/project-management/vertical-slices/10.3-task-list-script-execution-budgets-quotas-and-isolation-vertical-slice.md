...
Implementation is underway. The first concrete cuts wire the documented terminal outbox retention knobs into Automation & Scripting configuration and a scheduled cleanup path for `HANDED_OFF`, `CANCELED`, and `DEAD_LETTERED` `script_work_items`, enforce the serialized work-item payload-size ceiling at event ingress before persistence, enforce command-count output ceilings when claimed work items are evaluated, and make Automation pin/rollout projection freshness use the supported `SCRIPT_PIN_PROJECTION_STALE_THRESHOLD_MS` runtime knob instead of a hardcoded service threshold. The heavier budget-charging, dry-run isolation, signer-policy freshness, and broader sandbox execution policy paths still remain.
