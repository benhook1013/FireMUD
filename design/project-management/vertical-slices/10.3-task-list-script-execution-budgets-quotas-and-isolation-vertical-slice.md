...
- extend the now-live per-script and tenant-budget charging into richer tier/source policy once priority tags are persisted on durable work items.
...
- [ ] Implement the slice end to end.
  ...
  - [x] Live work-item execution reserves tenant runtime budget before script definition evaluation and cancels budget-denied work with `tenant_budget_exceeded` audit outcomes.
