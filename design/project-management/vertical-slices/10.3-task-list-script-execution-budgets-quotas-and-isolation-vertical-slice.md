...
- [ ] Extend the now-live per-script and priority-tagged tenant-budget charging into richer source policy as more durable work-item producers land. The next bounded child slice is [`10.3.1`](./10.3.1-task-list-source-aware-budget-charging-follow-through-vertical-slice.md).
...
- [ ] Extend operator metrics beyond the current durable work-item outcome counter, which now already breaks outcomes down by `sourceKind`, `sourceService`, `eventType`, `priorityTag`, and `dryRun`, into broader dashboards and alerting as more producers land. The next bounded child slice is [`10.3.2`](./10.3.2-task-list-script-execution-operator-metrics-and-alerting-convergence-vertical-slice.md).
