...
- keep dry-run/test isolation on the same canonical budget model rather than a sidecar shortcut; live quota/budget bypass, first per-minute dry-run rate limits, tenant-local dry-run capacity reservation, and cluster-wide dry-run capacity reservation are now enforced.
- extend operator metrics from the current durable work-item outcome counter into broader source-specific dashboards as more producers land.
