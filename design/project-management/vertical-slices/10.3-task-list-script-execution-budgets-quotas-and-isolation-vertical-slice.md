...
Implementation is underway. The first concrete cuts wire the documented terminal outbox retention knobs into Automation & Scripting configuration and a scheduled cleanup path for `HANDED_OFF`, `CANCELED`, and `DEAD_LETTERED` `script_work_items`, and enforce the serialized work-item payload-size ceiling at event ingress before persistence. The heavier budget-charging, dry-run isolation, command-count output limits, and sandbox execution policy paths still remain.
...
- implement and wire the remaining now-documented output-budget, pin-freshness, and signer-policy-freshness knobs into the owning runtime paths; outbox-retention cleanup and serialized work-item payload-size rejection are now live.
