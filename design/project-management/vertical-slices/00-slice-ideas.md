...
6. **Non-Edge Failover Coordination Prerequisites** - Externalize reconnect-critical state that still lives only inside one Game Session instance so same-type instances can later take over safely from shared state (planned in `02.5-task-list-non-edge-failover-invisibility-vertical-slice.md`).
7. **Live Backend Rebind Invisibility** - Remove the remaining implementation debt that still makes Game Session or other non-edge restarts visible to players by rebinding backend handling behind the edge connection instead of forcing reconnect (planned in `02.6-task-list-live-backend-rebind-invisibility-vertical-slice.md`).
8. **Shared Communication Infrastructure** - Refactor the initial room-speech path into a reusable communication system that all later communication actions can use, while keeping `say` as the first fully live mode (planned in `04.1-task-list-shared-communication-infrastructure-vertical-slice.md`).
9. **Whisper Communication** - The first true target-directed in-room communication mode is now baseline live, with canonical sender/target prose and default metadata-only observer wording recorded in `04.2-task-list-whisper-vertical-slice.md`.
10. **Tell Communication** - The first direct non-room-scoped live gameplay tell is now baseline live, with canonical sender/target prose recorded in `04.3-task-list-tell-vertical-slice.md`.
11. **Communication Observers & Interceptors** - The baseline observer/interceptor slice is now live for structured metadata-only `whisper` observer resolution in `04.4-task-list-communication-observers-and-interceptors-vertical-slice.md`.
12. **Communication Recipient Delivery** - Target-side and metadata-only observer-side communication delivery are now baseline live for generic WebSocket and Telnet in `04.5-task-list-communication-recipient-delivery-vertical-slice.md`; first-party/MCP-aware delivery remains future work.
