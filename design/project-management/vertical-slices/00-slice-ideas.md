...
4. **Reconnect and Session Recovery Semantics** - Bring the implemented reconnect/takeover behavior into line with the richer explicit recovery model in the design docs, including edge restarts, fresh connect tokens, and clearer resume outcomes. Core reconnect, screen-buffer restore, fresh-entry fallback, and edge classification are now live; non-edge invisibility and first-party parity remain in progress in `02.3-task-list-reconnect-and-session-recovery-vertical-slice.md`.
...
6. **Whisper Communication** - The first true target-directed in-room communication mode is now baseline live, with canonical sender/target prose and default metadata-only observer wording recorded in `04.2-task-list-whisper-vertical-slice.md`.
7. **Tell Communication** - The first direct non-room-scoped live gameplay tell is now baseline live, with canonical sender/target prose recorded in `04.3-task-list-tell-vertical-slice.md`.
