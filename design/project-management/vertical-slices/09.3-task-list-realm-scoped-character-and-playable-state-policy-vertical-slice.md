...
- inventory and equipment now follow that same realm policy transitively because they are anchored to the scoped `characterId` namespace rather than a separate tenant-wide playable-state seam, and the REST surfaces for those character-owned state families now also require the resolved `{gameInstanceId, playableStateScope}` target before serving or mutating tenant-scoped character inventory/equipment views;
...
- downstream gameplay-state namespaces beyond the character/inventory/equipment/progression-resource namespace still need the same realm-policy rollout;
- the next honest follow-through is later loadout/ability/resource-table families as they become real rather than reworking inventory/equipment again.
...
- keep later loadout/ability/resource-table and gameplay-state families on the same resolved playable-state namespace rather than reintroducing tenant-wide shortcuts.
