...
- define the first narrow realm-state policy that is concrete enough for current gameplay systems.
- identify which runtime subsystems must treat isolated-state realms as separate durable state namespaces first.
- define how character discovery and selection surfaces expose the realm-state distinction without requiring raw implementation terminology.
