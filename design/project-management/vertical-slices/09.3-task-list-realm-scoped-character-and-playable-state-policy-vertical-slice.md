...
- [ ] Keep later loadout, ability, authored-effect, resource-table, and other future gameplay-state families on the same resolved playable-state namespace rather than reintroducing tenant-wide shortcuts as those families land. The next bounded child slice is [`09.3.1`](./09.3.1-task-list-playable-state-family-namespace-follow-through-vertical-slice.md).
