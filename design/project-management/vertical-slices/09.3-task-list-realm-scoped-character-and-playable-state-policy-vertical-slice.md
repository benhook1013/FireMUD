...
- shared-state realms continue to use the tenant-wide roster seam for now;
- isolated-state realms now fail closed with `REALM_STATE_POLICY_UNSUPPORTED` instead of silently browsing or entering against the shared tenant roster.

What remains open is the final roster/state substrate:

- character discovery is still `listCharactersByAccount(tenantId, accountId)` rather than a realm-/instance-aware roster contract;
- downstream gameplay-state namespaces beyond lobby/admission still need the real isolated-state rollout.
...
- define the first concrete discovery/API shape for realm-local roster plus creation-policy exposure without forcing clients to understand internal storage terminology.
