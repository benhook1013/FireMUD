...
- The preflight expected-binding validation now checks exact rendered Secret and image-pull binding identity, cross-environment external-binding isolation, and referenced preflight-report consumption, but it should continue to tighten against richer Kubernetes live-state evidence as that becomes available.
