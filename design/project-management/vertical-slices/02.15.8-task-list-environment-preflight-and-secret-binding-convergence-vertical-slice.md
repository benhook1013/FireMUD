...
- The traffic-open backup gates now validate both the evidence record shape and the referenced preflight report contract, but real environment evidence files still need to be produced by operators or automation before first live traffic.
- The preflight expected-binding validation now checks rendered secret mounts for JWT/JWKS wiring plus referenced preflight-report consumption, but it should continue to tighten against richer Kubernetes live-state evidence as that becomes available.
