...
Goal: make the live built-in script event producers and ingress/runtime readers converge on one explicit payload contract so registry-declared payload identity, required fields, and optional metadata are enforced consistently instead of drifting by producer family. Status: planned.

## Current Snapshot (2026-06-29)

- This slice is still `planned`.
- No complete end-to-end contract-convergence pass has been applied under this slice yet.
- Keep this doc as planned until payload audit, producer/readers convergence edits, and parity tests are all finished.
- Accuracy note (2026-06-29): still not started as a bounded slice; registry and producers show partial parity already but no complete sweep has been done for all built-in event families.
...
- [ ] Enumerate already-live built-in event families and their current producer paths.
- [ ] Record the registry-declared payload contract and any producer/reader drift for each touched family.
- [ ] Keep the batch limited to already-live built-in events; do not invent new families.

### 2. Contract Convergence

- [ ] Update touched producers and ingress/runtime readers so required fields, optional fields, and first-class payload identity align with the registry contract.
- [ ] Reject missing or contradictory required built-in payload fields where the contract already says they are mandatory.
- [ ] Keep changes bounded to the event families covered by the audit.

### 3. Focused Proof

- [ ] Add or refresh focused tests for missing required payload fields, malformed built-in payload shape, and parity between registry declarations and live producer output.
- [ ] Prove the touched event families remain durable/audit-compatible after convergence.
