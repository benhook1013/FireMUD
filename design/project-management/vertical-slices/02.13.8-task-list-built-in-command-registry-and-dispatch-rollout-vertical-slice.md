...
- `TextCommandInterpreter` dispatches through registry-owned command groups instead of remaining the primary owner of per-command stage branching;
...
This slice is not finished yet. The current command registry is still built-in only, and follow-up work still needs to:

- keep shrinking interpreter-owned command knowledge;
