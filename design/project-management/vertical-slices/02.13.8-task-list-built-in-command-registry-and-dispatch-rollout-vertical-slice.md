...
This slice is not finished yet. The runtime now has the first non-built-in provider registered too, and follow-up work still needs to:

- keep later game-authored command growth on the same provider-backed seam instead of growing special interpreter-local fallback logic;
- keep `commandId` ownership canonical as authored command coverage expands.
