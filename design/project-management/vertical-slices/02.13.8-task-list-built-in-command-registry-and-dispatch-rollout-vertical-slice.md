...
Goal: replace the growing hardcoded per-command branching in `TextCommandInterpreter` with one registry-driven built-in command dispatch model so command growth stays manageable and future game-authored commands can plug into the same architecture. Status: planned.
...
- Each registered built-in command definition owns at least:
  ...
  - future ownership metadata for authored-command compatibility.
...
- [ ] Use one registry-driven dispatch model for built-ins now rather than waiting for game-authored commands.
- [ ] Move stage eligibility into command definitions rather than leaving it implicit in interpreter control flow.
- [ ] Keep the first dispatcher grouped by command family rather than forcing one class per command.
- [ ] Preserve the current parser and typed payloads in the first pass unless a payload shape is actively blocking the registry rollout.
- [ ] Treat future game-authored commands as later users of the same registry concept, not as a second parallel dispatch path.
...
4. Collapse the item-command families behind one shared item-manipulation handler seam.
5. Remove the now-redundant per-command branch tree from `TextCommandInterpreter`.
6. Only after that, continue expanding built-ins or game-authored command work.

## Validation

- [ ] Prove the interpreter no longer owns the main per-command branch tree for built-ins.
- [ ] Prove adding a new built-in command requires a registry definition plus family-handler extension rather than a new central branch.
- [ ] Keep the active command suite green with parser, interpreter, unit, and cross-service tests.
- [ ] Confirm the resulting structure is compatible with later game-authored command registration.
