...
- duplicate command-definition ownership is now rejected at registry construction time rather than silently allowing one provider to override another.
...
- carry the same provider-backed model forward into later game-authored command registration;
- prove the first non-built-in provider uses the same seam without reintroducing interpreter-local fallback logic.
