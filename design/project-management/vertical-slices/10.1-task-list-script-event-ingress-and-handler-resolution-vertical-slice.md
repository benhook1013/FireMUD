...
The canonical event-registry ownership and entry model now lives in `design/architecture/system-architecture-scripting-event-registry.md`, and the live registry read APIs now expose `payloadSchemaRef` alongside producer/identity rules, so this slice should treat registry semantics as settled design and focus on runtime ingress enforcement rather than reopening ownership shape.
