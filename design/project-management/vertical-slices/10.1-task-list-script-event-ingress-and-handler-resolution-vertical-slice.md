...
- [ ] Extend real producer coverage beyond the now-live Game Session `onCommand` coverage for queued gameplay, communication, direct gameplay reads, gameplay-scoped `HELP`, gameplay-scoped world browse, fresh-PLAY `onSpawn`, direct `LOGOUT` room exit, movement lifecycle, and scheduler/timer paths into additional gameplay lifecycle events.
