...
- complete handler-scoped `script_event_audit` rows and work-item/outbox creation after binding resolution.
- connect real Game Session or scheduler callers to the ingress API so retry behavior is proven across service boundaries.
- add the canonical event-registry read APIs once the static built-in registry needs to become externally consumable by Game Design and operator tooling.
...
- [ ] Implement the slice end to end.
  ...
  - [ ] Handler-scoped audit/work-item creation and downstream execution remain to be implemented.
