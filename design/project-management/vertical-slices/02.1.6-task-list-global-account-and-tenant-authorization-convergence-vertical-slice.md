...
- [~] Make account export/delete/recovery flows account-owned rather than tenant-owned. Target design is now explicit, but REST/proto/service implementation still needs follow-through to remove tenant-keyed export/delete behavior and add the separate tenant-scoped billing-safe export.
