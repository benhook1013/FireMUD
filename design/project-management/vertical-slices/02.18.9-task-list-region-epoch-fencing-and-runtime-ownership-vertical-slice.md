...
Goal: introduce durable region ownership, epoch, and fencing semantics so stale executors cannot safely continue acting after failover/reset and runtime coordination has one canonical owner-of-record model. Status: in progress.
...
First current-boundary ownership substrate now live:
...
- carry fencing beyond the current batch-drain seam into later downstream effect-application boundaries once `02.18.10` starts.
...
5. Finish the slice by rejecting stale batch finalization attempts and exposing a bounded ownership-status surface.
...
- [ ] Prove the same stale-fence guarantees still hold once later effect-application/idempotency work lands.
