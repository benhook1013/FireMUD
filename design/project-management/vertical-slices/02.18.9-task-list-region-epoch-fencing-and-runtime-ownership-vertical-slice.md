...
- reject stale in-flight batch finalization when durable epoch/fence changes mid-tick rather than only recording ownership snapshots;
- expose one bounded operator/runtime query surface for current ownership status;
