...
- the first durable cross-region substrate is now executable on the origin side: `remote_followup`, `remote_command_coordinator`, and origin-addressed `remote_followup_result` rows exist as Game Session-owned records; the origin-side runtime can now schedule follow-ups, persist target results, reconcile deadline timeouts, and preserve an explicit coordinator `followupId` link instead of relying on ambiguous tuple matching, even though no gameplay producer or target-region drain path is writing/claiming them yet.
...
- Carry the newly landed remote follow-up / coordinator / result substrate from origin-side schedule/result/timeout execution into live gameplay producers plus target-region claim/drain paths so later cross-region work is not forced back into ad hoc tables or payload-only state.
