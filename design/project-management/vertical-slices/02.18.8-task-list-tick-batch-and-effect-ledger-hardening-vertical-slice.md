...
- the first durable cross-region substrate is now present in schema and control-plane reads only: `remote_followup`, `remote_command_coordinator`, and origin-addressed `remote_followup_result` rows exist as Game Session-owned records, but no gameplay producer or target-region drain path is writing/claiming them yet.
...
- Carry the newly landed remote follow-up / coordinator / result substrate from read-only schema into live producer, claim, drain, and reconciliation paths so later cross-region work is not forced back into ad hoc tables or payload-only state.
