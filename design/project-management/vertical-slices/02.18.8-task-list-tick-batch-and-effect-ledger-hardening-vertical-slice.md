...
- remote coordinator control-plane reads are now no longer point-lookup only: `ListRemoteCommandCoordinators` provides a bounded tenant-scoped list over persisted origin/target game-instance and region scope, state, linked `followupId`, and script/plugin/dispatch/command provenance so operator diagnostics can pivot into the same durable remote substrate without already knowing one exact coordinator id.
