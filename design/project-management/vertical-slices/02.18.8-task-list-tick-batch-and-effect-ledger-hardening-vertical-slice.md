...
- the standalone remote followup/result list surfaces now also carry the coordinator's origin deadline and late-result policy, so operator reads can inspect the current remote leg plus its timeout/late-result contract without a second coordinator join just to recover those policy fields, and those list reads now support bounded filtering over persisted origin/target scope plus script/plugin/dispatch/command provenance instead of forcing one exact coordinator id or one full target-region scan.
...
- remote coordinator control-plane reads are now no longer point-lookup only: `ListRemoteCommandCoordinators` provides a bounded tenant-scoped list over persisted origin/target region scope, state, linked `followupId`, and script/plugin/dispatch/command provenance so operator diagnostics can pivot into the same durable remote substrate without already knowing one exact coordinator id.
