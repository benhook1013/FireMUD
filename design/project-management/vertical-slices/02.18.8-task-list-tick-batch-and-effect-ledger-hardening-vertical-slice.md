...
- Extend the current gameplay-command selected-work manifest into source-claim truth for timer, retry, and remote-follow-up candidates so all work sources are ordered and replayed from one persisted batch contract rather than inferred from current narrow drain behavior.
