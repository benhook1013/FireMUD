...
Goal: add the first durable tick-batch and effect-ledger substrate so Redis tick staging becomes hot-path coordination over durable execution truth instead of the only real record of in-flight gameplay work. Status: in progress.
...
- The current batch/effect lifecycle vocabulary is concrete for the present runtime seam:
  - batch states: `STAGED`, `DRAINED`, `ABANDONED`
  - effect states: `STAGED`, `DRAINED`, `ABANDONED`
...
- This is still intentionally narrower than the long-term architecture:
  - the current tick path does not yet persist region ownership, region epoch, or `tickId`;
  - `tick_effect` rows currently represent command-stage execution entries in the present runtime, not final downstream domain mutations;
  - replay/idempotency/fencing semantics beyond the current stage/drain lifecycle remain later slices.
