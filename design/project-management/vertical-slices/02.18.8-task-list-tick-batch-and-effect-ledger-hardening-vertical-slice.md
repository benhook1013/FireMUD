...
- the first live target-side payload handler (`enqueue_automation_command`) now also treats that persisted remote-row metadata as canonical fallback input instead of requiring duplicate script/dispatch/routing fields inside payload JSON just to rehydrate an ordinary automation admission.
