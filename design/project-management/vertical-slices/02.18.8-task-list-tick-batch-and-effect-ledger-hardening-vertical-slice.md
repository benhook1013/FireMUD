...
- Carry the newly landed remote follow-up / coordinator / result substrate from origin-side schedule/result/timeout execution plus first hint/backlog observability and target-side claim/release truth, including shared batch-failure release behavior, into live gameplay producers and full target-region tick execution/drain paths so later cross-region work is not forced back into ad hoc tables or payload-only state.
