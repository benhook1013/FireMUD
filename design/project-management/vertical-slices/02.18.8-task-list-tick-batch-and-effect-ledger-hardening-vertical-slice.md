...
- those result reads now also prefer the durable `followupId` -> target-command link as authority for that target-leg status instead of trusting payload JSON first, keeping the remote row authority pattern consistent even when payload summaries drift.
