...
- Generalize the first current-boundary comparable `enqueueSeq` from gameplay commands into the later timer/retry/remote-follow-up work sources so every candidate persists one comparable ordering key at claim time instead of relying on timestamps or implied source-local iteration order.
