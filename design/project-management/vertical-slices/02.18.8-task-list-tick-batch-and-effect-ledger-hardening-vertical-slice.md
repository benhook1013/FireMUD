...
- the first durable cross-region substrate is now executable on the origin side: `remote_followup`, `remote_command_coordinator`, and origin-addressed `remote_followup_result` rows exist as Game Session-owned records; the origin-side runtime can now schedule follow-ups, write the bounded best-effort `remote:<tenantId>:<entityId>` wake-up hint, persist target results, reconcile deadline timeouts, preserve an explicit coordinator `followupId` link instead of relying on ambiguous tuple matching, and surface durable due/backlog pressure from target-region follow-up rows during ticks even though no gameplay producer or target-region claim/drain path is writing/claiming them yet.
...
- Carry the newly landed remote follow-up / coordinator / result substrate from origin-side schedule/result/timeout execution plus first hint/backlog observability into live gameplay producers and target-region claim/drain paths so later cross-region work is not forced back into ad hoc tables or payload-only state.
