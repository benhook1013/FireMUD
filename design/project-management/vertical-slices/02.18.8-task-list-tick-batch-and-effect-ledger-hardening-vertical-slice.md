...
- That current queue-claim truth no longer lives only inside sealed batch JSON:
  - `gameplay_command` now persists the last selected queue-source kind/state/ordinal (`queueSourceKind`, `queueSourceState`, `queueSourceOrdinal`) whenever the tick runtime claims work for a batch;
