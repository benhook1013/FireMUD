...
- the first durable cross-region substrate is now executable on both sides of the ownership seam: `remote_followup`, `remote_command_coordinator`, and origin-addressed `remote_followup_result` rows exist as Game Session-owned records; the origin-side runtime can now schedule follow-ups, write the bounded best-effort `remote:<tenantId>:<entityId>` wake-up hint, persist target results, reconcile deadline timeouts from committed tick progress, preserve an explicit coordinator `followupId` link instead of relying on ambiguous tuple matching, and surface durable due/backlog pressure from target-region follow-up rows during ticks; the target-side runtime now also has a dedicated claim/release service that durably marks bounded due follow-ups `CLAIMED` against one `tickBatchId` before later execution wiring lands, and the shared tick rollback/stale-fence paths now explicitly release those claimed follow-ups again instead of leaving abandoned batch ownership implicit.
...
- Carry the newly landed remote follow-up / coordinator / result substrate from origin-side schedule/result/timeout execution plus committed-tick timeout reconciliation, first hint/backlog observability, and target-side claim/release truth, including shared batch-failure release behavior, into live gameplay producers and full target-region tick execution/drain paths so later cross-region work is not forced back into ad hoc tables or payload-only state.
