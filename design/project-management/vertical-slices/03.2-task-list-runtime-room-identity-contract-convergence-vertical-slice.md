...
- Converged current-state Game Session unit-test room fixtures onto the canonical runtime room family:
  - command, communication, presence, auth, and session-persistence proofs that are not explicitly testing legacy-room rejection now use `R-...` room ids instead of continuing to teach `room-...` or generic `room` placeholders as ordinary gameplay state;
