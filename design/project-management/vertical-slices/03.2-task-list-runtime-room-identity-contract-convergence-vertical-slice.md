...
Goal: converge FireMUD onto one canonical cross-service runtime room identity contract so `roomInstanceId` means exactly one thing at service boundaries, while any World Management numeric storage surrogate remains an explicitly internal implementation detail with a different name. Status: planned; repository audit and contract-teaching batch complete, code migration still pending.
...
- [ ] Rename World Management internal numeric room-instance storage keys away from shared contract naming.
