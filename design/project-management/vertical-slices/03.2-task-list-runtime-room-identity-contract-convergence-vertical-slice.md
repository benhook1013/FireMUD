...
Goal: converge FireMUD onto one canonical cross-service runtime room identity contract so `roomInstanceId` means exactly one thing at service boundaries, while any World Management numeric storage surrogate remains an explicitly internal implementation detail with a different name. Status: planned.
...
- [ ] Audit every live `roomInstanceId` / `room_instance_id` / `RoomInstanceRef` surface across proto, service DTOs, repository/entity models, read fences, attestation claims, and gameplay/session helpers.
...
- [ ] Update the identifier glossary and nearby architecture docs so the canonical distinction is explicit and repo-wide.
