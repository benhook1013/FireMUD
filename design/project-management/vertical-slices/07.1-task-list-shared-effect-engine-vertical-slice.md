...
2. It loads the declared release-pinned DML `TargetingPolicy`, resolves its typed candidate selector against canonical actor identity and World Management occupancy, then evaluates its referenced `ObservationPolicy.observableWhen` before `eligibleWhen` for observable candidates.
