...
Each action or standard-path binding owns a frozen DML `TargetSelection` over the eligible candidates. The initial modes are `EXACTLY_ONE`, bounded `UP_TO_N`, and `ALL_ELIGIBLE` within an operator ceiling; optional-target behavior stays on that action/binding. The policy therefore remains reusable across a direct strike, a multi-target ability, and a room-wide effect. Candidate order is canonical and stable rather than query-order dependent, and any later random selection must derive from the idempotent effect id.
