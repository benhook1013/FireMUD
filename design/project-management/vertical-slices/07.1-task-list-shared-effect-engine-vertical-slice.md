...
- `INSTANT` declarations execute once under a `ResolvedEffectPlan` and its idempotent effect id. The initial runtime mutation operations are `ADJUST_RESOURCE`, `APPLY_CONDITION`, and `REMOVE_CONDITION`. They persist their result or active source and are never re-applied by later state reads.
