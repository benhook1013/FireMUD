...
- the same status read now also exposes the last selected queue-source kind/state/ordinal from the durable command ledger, so current attempt provenance no longer depends on parsing the tick batch manifest or guessing from `RETRY_QUEUED`.
