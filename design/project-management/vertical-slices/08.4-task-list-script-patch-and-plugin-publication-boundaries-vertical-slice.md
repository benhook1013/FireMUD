...
- implement the remaining signer-policy and publication-lifecycle follow-through so `SUPERSEDED`, `REVOKED_DESIGN`, and durable `PluginVersionStatusChanged` publication events are enforced by real control-plane state transitions rather than only documented target-state rules.
