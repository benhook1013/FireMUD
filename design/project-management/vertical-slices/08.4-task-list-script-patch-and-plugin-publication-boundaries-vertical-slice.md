...
- runtime activation now already fails closed on non-eligible publication state, base-version mismatches, digest mismatches, revoked signer metadata, and blocked or missing component-policy decisions; keep that proof current as the remaining lifecycle work lands.
- enforce the remaining documented activation-time validation around command-alias scope, instance-scoped binding conflicts, and policy/report-only behavior in the owning services.

## Recommended Stopping Points

1. Land explicit design-time revoke transitions plus focused tests and stop.
2. Land durable `PluginVersionStatusChanged` publication events plus focused tests and stop.
3. Land activation-time command-alias / instance-binding conflict validation plus focused tests and stop.
