...
- define the non-recursive signing envelope and signed-manifest schema so `bundleDigest`, `plugin-manifest.json`, `signatures.json`, and `assetRefs[]` can be verified deterministically across independent packers.
- settle the initial plugin authoring/signing workflow, signer-revocation model, and environment promotion semantics so immutable plugin-version identity is not left to per-environment invention.
- tighten publish-time versus activation-time validation around command-alias scope, instance-scoped binding conflicts, and policy/report-only behavior.
