...
Goal: define one canonical actor model for players, NPCs, gods, and later summons/pets so gameplay, presence, targeting, stats, and permissions build on one shared runtime concept rather than diverging entity families. Status: direction locked; implementation is future work.
...
Player actors may additionally link to account and character identity. NPC actors may additionally link to an NPC definition id. Elevated staff/god state attaches as capability/visibility over a `PLAYER` actor rather than creating a separate actor kind.
