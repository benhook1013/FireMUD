...
The persisted disposition is the actor's one main gameplay state, authored in the frozen game version. A normal state, unconsciousness, defeat, death, ghost state, or exhaustion are examples, not platform enums. It defines base action admission, targetability, visibility, and feedback policy. Conditions and equipment are overlays that can narrow or modify that policy, but they never become competing lifecycle owners. Transport/session presence remains separate: disconnecting does not defeat an actor, and an NPC can be defeated without a player session.
