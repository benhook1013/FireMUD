...
- at the time, the startup bootstrap initializer still read `GameplayCatalogProperties` to seed pointer authority on an empty store; later `09.1.37` moved that bootstrap seam onto dedicated pointer-seed config
...
The production world/realm catalog now has one authority story only: pointer-backed runtime projection. Config-backed world/realm shaping still exists where it is genuinely needed, but only in explicit test support; the later startup pointer bootstrap follow-through moved that production seam onto dedicated pointer-seed config.
