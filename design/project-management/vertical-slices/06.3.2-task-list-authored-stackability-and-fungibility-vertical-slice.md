...
- `DEFINITION_AND_VARIANT` is only a real implementation target once the runtime has a canonical per-source or per-instance variant discriminator that can differ within the same item definition;
- until that substrate exists, the only honest live compatibility mode is the current definition-level baseline plus an eventual explicit non-merging escape hatch;
- do not land a fake `variant` mode that just reuses definition-level sameness under a second name.
...
That follow-up should only start once there is a canonical source for non-definition variant state. If that source does not exist yet, the correct next move is to design or land that substrate first rather than forcing `06.3.2` to fake it locally.
