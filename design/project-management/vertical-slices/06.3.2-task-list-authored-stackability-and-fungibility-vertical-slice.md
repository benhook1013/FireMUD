...
- item definitions now also expose an explicit `stackCompatibilityMode`, and stack rows carry a canonical `stackFamilyKey` where the compatibility mode requires more than plain definition-level sameness;
...
- the family key must live on the transferable stack substrate itself, not only on the item definition, so two quantities of the same item definition can remain distinct when they came from different authored stack families;
...
5. family-preserving stack transfers across inventory, room-ground, and container holders;
6. ambiguity rejection when one item definition now exists in multiple stack families within the same holder;
7. merged stack query surfaces for those holders;
8. room-ground prose/entity reads that stay aligned with the new stack storage truth.

The remaining follow-up is narrower:

1. add richer authored sources for `stackFamilyKey` beyond the current definition-default seam;
2. carry the model forward into stack-selector UX so players can act on a specific family when one item definition appears in several stack families within the same holder;
3. decide whether later authored modes need a non-merging escape hatch once there is a real consumer for that behavior.
