...
- The runtime service configs now use Spring Boot SSL bundles under `spring.ssl.bundle.pem.firemud-grpc`, while the remaining legacy top-level `grpc.server.*` blocks are temporarily allowlisted by the transport sanity check until the migration lane removes them.
