...
What remains open is purge workflow coverage and the richer derived-artifact / artifact-status surfaces, not the basic lifecycle direction.
...
- extend the now-live `version_asset_artifact` API/read layer with purge workflow status and later richer artifact-detail surfaces.
