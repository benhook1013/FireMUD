...
What remains open is the later normalized reference-table truth (`version_asset`, `revision_asset`, broader template/history refs beyond the current launch-descriptor and approved-remap checks) plus the richer derived-artifact / artifact-status surfaces, not the basic lifecycle direction.
