...
What remains open is broader deletion-eligibility truth outside Game Design's current local artifact state, plus the richer derived-artifact / artifact-status surfaces, not the basic lifecycle direction.
...
- widen deletion-eligibility checks beyond the current local lifecycle state once draft/revision/version reference tables are implemented in canonical form.
