...
- `PublishVersion` keeps the synchronous caller contract, but the durable work now runs through Temporal-backed reconciliation instead of one inline JVM-only path;
