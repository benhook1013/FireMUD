...
- downstream consumers still need broader proof coverage for all failure modes, especially missing-attestation reads and later repair-orchestration consumers beyond the first canonical launch/repair paths.
...
- [ ] Implement the slice end to end.
  The canonical publish-attempt framework, Game Design control-plane digest RPC, full-version and script-patch participant coverage, release-bundle participant-digest attestation, recorded-digest comparison, and typed publish-gate failures are now live, but fully version-scoped digest inputs and the remaining downstream proof coverage are still open.
