...
The biggest current implementation gap is now explicit rather than hidden:

- the canonical framework exists, but full-version publish still fails closed because World Management, Entity Management, and Game Logic do not yet expose the required digest participants behind the full-version matrix;
- Automation & Scripting now participates honestly for script-patch publish, but it still does not provide a full-version digest surface;
...
- implement the missing required full-version digest participants in World Management, Entity Management, Game Logic, and Automation & Scripting so the now-live full-version matrix can converge instead of failing closed immediately.
...
- [ ] Implement the slice end to end.
  The canonical publish-attempt framework, Game Design control-plane digest RPC, script-patch digest gating, and release-bundle participant-digest attestation are now live, but full-version participant coverage and recorded-digest comparison are still missing.
