...
- successful logout/stop now also drives the canonical World Management termination seam instead of relying on a ping-only session stop path, so gameplay-bound shutdown now closes admission before World/Entity runtime cleanup converges;
