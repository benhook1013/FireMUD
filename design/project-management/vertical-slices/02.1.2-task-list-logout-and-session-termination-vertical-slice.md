...
- focused unit and websocket integration coverage exists for the basic logout path and replay-buffer cleanup.

Still missing in this slice:

- broader proof that fresh `LOGIN` after `LOGOUT` does not resume stale reconnect state across every transport variant, beyond the current websocket coverage;
- fuller proof that deliberate logout remains distinct from takeover/replacement and transport-loss recovery across the wider reconnect slices;
