...
- [ ] Ensure `LOGOUT` is available only when authenticated or gameplay-bound state exists, with bounded graceful behavior when used pre-login.
- [ ] Delete Redis-backed session context for the current session on successful logout.
- [ ] Stop the active gameplay session on successful logout instead of leaving it resumable.
...
- [ ] Close the transport after the canonical logout success response instead of returning the client to an in-band pre-login front door state.
