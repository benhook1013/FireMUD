...
Each action or standard-path binding separately declares a frozen DML `TargetSelection` over the eligible candidates: `EXACTLY_ONE`, bounded `UP_TO_N`, or `ALL_ELIGIBLE` within an operator ceiling. This keeps target policy reusable while preserving action-specific cardinality and optional-target outcomes. Candidate order is canonical and stable; a later random mode must be deterministically seeded from the action effect id.
