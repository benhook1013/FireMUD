...
Each action or standard-path binding references a frozen reusable DML `TargetSelectionPolicy` over the eligible candidates. It declares `EXACTLY_ONE`, bounded `UP_TO_N`, or `ALL_ELIGIBLE` within an operator ceiling and chooses by `PLAYER_SELECTED`, `CANONICAL_ORDER`, typed `RANKED`, or effect-id-seeded `RANDOM_SEEDED` strategy. This keeps target and selection policy reusable while optional-target outcomes remain action-specific. Candidate order is canonical and stable; the policy must declare ceiling truncation and records its snapshot, selected targets, ranking values or seed in target-resolution evidence.
