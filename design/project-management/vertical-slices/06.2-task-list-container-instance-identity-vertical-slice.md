...
Current live safety rails intentionally keep the bad persistence shape from leaking further:

- `DROP` rejects non-empty carried containers.
- `WEAR` rejects non-empty carried containers.

Those guards are correct for the current branch state, but they are only temporary protection. They should remain in place until container instances become the authoritative identity and location carrier for container-capable items.
...
- keep the current non-empty `DROP` / `WEAR` safeguards in place until later location-transfer semantics land.
...
- [ ] Surface `container_instance_id` through inventory and equipment query results for container-capable rows.
...
- [ ] Follow the identity replacement with a second bounded pass that moves container-instance location on drop, pickup, wear, and remove while preserving contents unchanged.
