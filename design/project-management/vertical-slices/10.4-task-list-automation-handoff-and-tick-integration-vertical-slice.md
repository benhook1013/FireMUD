...
- [x] Implement the slice end to end.
  ...
  - [x] Extended `ListScriptHandoffEvents` to expose current owned target runtime scope plus stale-scope signaling from Game Session runtime-state truth, so remote handoff diagnostics no longer require a second manual ownership lookup.
