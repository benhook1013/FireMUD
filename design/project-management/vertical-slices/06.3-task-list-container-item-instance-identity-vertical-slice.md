...
- stable lifetime-visible refs are not yet allocated through the intended monotonic per-type sequence system tracked in `06.3.1`;
...
- [ ] Allocate stable lifetime-visible refs for item instances using a monotonic per-type sequence in the game/world namespace (tracked in `06.3.1`).
