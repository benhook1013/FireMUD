...
The remaining hard limitation is that physical items themselves are still aggregated by `item_id` in inventory and room-ground state. That means the branch still treats "two identical backpacks" or "two identical swords" as one stack in places where the game should treat them as two separate things.
...
- [ ] Introduce a first-class persisted identity for each physical item instance.
- [ ] Replace aggregated inventory, equipment, room-ground, and container-content storage truth with item-instance identity.
- [ ] Keep `container_instance_id` canonically linked to the corresponding item instance rather than letting container identity float separately from item identity.
- [ ] Surface concrete item-instance identity through the entity-management API where Game Session must disambiguate one item from another.
...
- [ ] Surface one canonical compact explicit item-reference form for output/help/tab completion while keeping the interpreter free to accept bounded equivalent readability aliases if they remain cheap and unambiguous.
...
- [ ] Keep ordinary prose views natural, while inventory/management views surface per-instance refs for non-stackable items.
...
- [ ] Prove that two identical carried non-container items can exist and be targeted independently.
- [ ] Prove that two identical room-ground items can be dropped, listed, and picked up independently.
...
- [ ] Prove that management views such as inventory-style listings show stable refs for non-stackable instances.
- [ ] Prove that `CONTAINER`, `PUT`, `TAKE`, `DROP`, `GET`, `WEAR`, and `REMOVE` continue to act on the correct concrete item instance when duplicates exist.
