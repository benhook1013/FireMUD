...
- item definitions now also expose an explicit `stackable` capability flag, but holder/query merge behavior is still a later follow-up rather than live runtime behavior.
...
- [ ] Define authored stackability as an explicit capability rather than a derived consequence of item sameness (tracked in `06.3.2`).
- [ ] Define stack compatibility so only fungible interchangeable items merge into quantity-bearing stack state (tracked in `06.3.2`).
