...
- Automation -> Game Session gameplay-command handoff now preserves that same bundle too: `EnqueueAutomationCommandIfAbsent` carries `playableStateScope`, `worldSlug`, `realmSlug`, and `pointerVersion`, and staged `GameplayCommand` rows keep those fields instead of dropping back to an automation-dispatch-only ledger record before later execution/replay seams inspect them.
