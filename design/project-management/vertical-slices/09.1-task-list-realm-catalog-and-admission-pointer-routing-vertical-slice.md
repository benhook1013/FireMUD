...
- Logging & Admin now exposes operator-facing admission-pointer list, audit, and cutover mutation routes on top of the Game Session control-plane APIs instead of leaving pointer management as an internal-only gRPC seam;
...
- the current pointer audit seam is authoritative and now has first operator-facing tooling plus first-party reconnect/cutover proof; the remaining gap is broader runtime-change verification for future reconnect/cutover consumers beyond login/bootstrap/PLAY.
