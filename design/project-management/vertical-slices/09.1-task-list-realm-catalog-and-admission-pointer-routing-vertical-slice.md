...
- `account-service` bootstrap world/realm discovery and connect-token validation now consume that Game Session routing surface instead of reading local gameplay catalog config directly.

What remains open is the final control-plane authority:

- routing is now served through `GetAdmissionPointer(...)`, but the authority behind it is still config-backed inside Game Session rather than mutable control-plane state;
...
- replace the current config-backed Game Session routing source with mutable control-plane-backed admission-pointer state.
- make the routing/audit story explicit for realm cutover, retirement, and pointer changes.
- keep in-band admission and first-party bootstrap on the same Game Session routing surface as the authority evolves.
