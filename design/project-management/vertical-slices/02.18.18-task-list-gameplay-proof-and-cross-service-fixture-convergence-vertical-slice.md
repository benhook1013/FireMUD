...
What remains is no longer “we need a driver.” The remaining inconsistency is one level up:
...
The next honest gap is shared proof orchestration above the transport layer:

- one reusable gameplay cross-service stack fixture should own repeated nested Game Logic / Game Session / backing-store bootstrapping, migration-dir resolution, and common test-state initialization;
- reconnect/takeover helpers should exist as explicit scenario fixtures rather than each suite rebuilding disconnect/reconnect choreography locally;
- prompt-tolerant transcript matchers and async eventual-assert helpers should be shared instead of separately reimplemented in websocket, telnet, and smoke-adjacent code;
- gateway websocket integration should keep its lower-level intent, but a tiny shared probe helper should own the repeated `StandardWebSocketClient`/latch plumbing.
...
- extract one gameplay cross-service stack fixture and migrate the larger telnet account/gameplay suite onto it instead of keeping its local nested app/bootstrap copy;
- converge `EntityManagementStubServer` and `ChatEntityManagementStubServer` on one configurable stub or one shared base so communication proof no longer carries a parallel entity fake;
- extract reconnect/takeover scenario helpers for websocket and telnet where current suites still repeat login/disconnect/reconnect choreography;
- extract shared async assertion helpers for metric counters, buffered screen/prompt visibility, and similar eventual proof;
- move prompt-tolerant LOOK / movement-refresh matchers into shared test support instead of keeping separate websocket and telnet copies;
- add a small gateway websocket probe helper so the bridge/look integration suites stop open-coding `StandardWebSocketClient` + latch/session wiring;
- decide how much smoke-script transport helper logic can be unified without weakening the scripts’ current black-box/operator role.
