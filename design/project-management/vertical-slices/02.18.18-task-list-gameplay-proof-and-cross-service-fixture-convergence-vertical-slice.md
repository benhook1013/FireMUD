...
The narrower residual gameplay-proof debt is now tracked explicitly in [`02.18.18.1`](./02.18.18.1-task-list-gameplay-transcript-and-baseline-proof-hardening-vertical-slice.md):

- websocket transcript assertions that still flap on live ordering under suite load;
- suite-specific baseline reset versus global-default reset drift;
- remaining inline fresh-baseline choreography that should converge on one shared helper.
