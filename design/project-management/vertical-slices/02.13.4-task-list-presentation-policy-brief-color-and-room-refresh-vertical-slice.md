...
- Structured room views now carry an explicit refresh reason (`EXPLICIT_LOOK`, `QUICKLOOK`, `MOVE_REFRESH`, `RECONNECT_REFRESH`), so brief-style room-refresh policy no longer depends only on command-name checks.
