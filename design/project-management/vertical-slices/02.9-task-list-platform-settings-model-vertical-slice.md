...
- Current effective-config behavior is now explicit for the surfaced pre-`06` domains:
  - Game Session now has a first bounded effective-settings resolver for `presentation`, `movement`, and `world-topology`, merging operator defaults with optional tenant-scoped and game-instance-scoped file/env overrides under `firemud.settings-overrides.*`;
