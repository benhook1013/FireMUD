...
- Current effective-config behavior is now explicit for the surfaced pre-`06` domains:
  - Game Session now has the first bounded effective-settings read surface for `presentation`, `movement`, and `world-topology`, merging operator defaults with optional tenant-scoped and game-instance-scoped file/env overrides under `firemud.settings-overrides.*`;
  - Game Session exposes that bounded result at `/actuator/settings/effective` for operator/debug inspection of the current effective values;
  - `communication` still resolves directly from `CommunicationProperties` in Game Logic, but Game Logic now exposes the current effective communication defaults at `/actuator/settings/effective/communication`;
  - reconnect transcript/buffer policy still resolves directly from `FiremudReconnectionProperties` in `common-data-runtime`;
  - there is still not yet a shared DB-backed tenant/game override read model or cross-service settings authority in code.
...
- [~] Define the first-pass caching and invalidation expectations for the first Game Session effective-settings read surface so services can use it safely without pretending every change is instantly hot-reloaded everywhere.
