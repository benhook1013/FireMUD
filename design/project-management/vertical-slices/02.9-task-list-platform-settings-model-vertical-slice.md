...
- Current effective-config behavior is now explicit for the surfaced pre-`06` domains:
  - each owning service resolves surfaced `firemud.*` settings directly from typed Spring properties;
  - those settings currently act as operator/file-env-backed defaults;
  - there is not yet a shared tenant/game override read model or merged runtime resolver in code.
