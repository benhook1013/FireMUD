...
- Continue the Logging & Admin operator-ingress follow-through so documented `/api/admin/**` runtime-control routes exist only when backed by real owner-side forwarding contracts; scoped tick pause/resume now qualify, while broader remediation still remains deferred until Game Session exposes the canonical RPC.
- Keep test/profile fixtures honest with the canonical public allowlist so `routes-test.yml` and similar lightweight route fixtures do not silently under-cover the documented external route-family contract.
