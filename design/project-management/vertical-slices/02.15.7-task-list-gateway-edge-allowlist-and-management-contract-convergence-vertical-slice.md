...
- Converge live Game Session and Gateway enforcement so the documented `/api/session/**` read/internal-only/operator-ingress split is reflected in route inventory and handler enforcement rather than only in coarse family routing.
