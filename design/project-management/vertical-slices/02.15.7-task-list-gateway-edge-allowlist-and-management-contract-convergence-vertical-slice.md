...
- Audit follow-through still belongs here even after the coarse allowlist cleanup:
  ...
  - `/api/session/**` still needs runtime convergence from documented classification into actual gateway/service enforcement;
...
- Converge live Game Session and Gateway enforcement so the documented `/api/session/**` read/internal-only/operator-ingress split is reflected in runtime route handling rather than only in docs.
