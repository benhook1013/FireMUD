...
- Audit follow-through still belongs here even after the coarse allowlist cleanup:
  ...
  - `/api/session/**` still needs one explicit HTTP admin/control versus `/ws/game/**` gameplay WebSocket route/protocol split;
  - the external contract still needs one route matrix that classifies `/api/session/**`, `/api/admin/**`, and `/assets/**` by external read, external bypass-safe write, Logging & Admin ingress only, or internal-only;
...
- Import the resolved `/api/{service}/**` namespace decision back into the high-level overview, context, and responsibility-matrix docs.
- Publish one canonical route inventory for `/api/session/**`, including auth class, audit requirements, and lease-owner forwarding expectations for mutable session control paths.
- Classify `/assets/**` in the edge model with one canonical public/authenticated and gateway/CDN story.
- Converge live gateway route/filter behavior so undocumented service-local internal subtrees under public prefixes are denied rather than forwarded by coarse family match alone.
