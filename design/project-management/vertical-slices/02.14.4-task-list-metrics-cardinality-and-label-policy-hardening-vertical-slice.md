...
- CI now runs a static cardinality-policy check that rejects obvious forbidden label keys such as raw `tenantId`, `sessionId`, `characterId`, and `script_patch_version` in metric-building code.

Still outstanding in this slice:

- finishing the repo-wide audit for more subtle high-cardinality patterns that a literal-label static check will not catch by itself;
- tightening the canonical allowlist/denylist wording across services now that the first guardrail exists;
- deciding whether any bounded tenant bucketing should exist, or whether tenant identity belongs only in logs/traces.
