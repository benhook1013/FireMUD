...
- `ApplyActorCondition` is wired to the production actor-condition mutation service and validates canonical effect payload JSON before idempotent replay lookup, so malformed payloads cannot be acknowledged by a prior replay response.
