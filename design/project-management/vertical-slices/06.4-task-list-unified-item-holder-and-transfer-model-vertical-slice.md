...
- the design still needs to decide whether that direct-field holder model is the canonical target-state runtime contract or whether a more explicit holder object / transfer contract should eventually replace it at the service boundary;
- until that is settled, future `06` work should not casually drift into separate command-family semantics.
...
- The docs must state clearly whether that abstraction is represented directly as holder fields on `item_instances` or as a later explicit holder object/contract. One canonical answer is required.
...
- [ ] Decide and document whether direct holder fields on `item_instances` are the canonical runtime representation or only an interim implementation shape.
