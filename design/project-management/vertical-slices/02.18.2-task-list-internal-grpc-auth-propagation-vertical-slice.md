...
Goal: make secured internal blocking gRPC calls attach the required auth metadata so downstream JWT-protected RPCs work consistently and safely. Status: mostly implemented.
...
- document the remaining intentional raw blocking clients such as `TcpProxyEventClient` explicitly as policy exceptions, or migrate them if they no longer deserve to stay distinct.
...
- [x] Validate the first secured flows end to end, especially:
  ...
  - Logging/Admin -> Game Session
  - Game Design -> Automation Scripting
- [ ] Document or clean up the remaining intentional outliers now that the canonical pattern is locked.
