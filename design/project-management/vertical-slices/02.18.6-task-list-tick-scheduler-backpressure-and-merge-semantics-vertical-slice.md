...
- the canonical tick alert snippets now include scheduler-pressure alert examples for sustained rejection, high executor queue depth, and elevated merge rate.

Still open:

- decide whether merge/skip pressure should ever feed back into gameplay timing policy rather than the current observability-only stance;
- refine the initial scheduler alert thresholds once real preview/prod-like load data exists;
...
- current scheduler pressure handling is observability-first:
  ...
  - gameplay timing policy does not automatically adapt yet in response to pressure;
  - any future timing feedback policy should be a deliberate follow-up backed by preview/prod-like data rather than an incidental side effect of scheduler saturation.
