...
- Live recipient push and reconnect screen-buffer fanout now exist for the baseline metadata-only `whisper` observer path. Richer first-party/MCP-aware presentation remains part of the follow-on delivery work.
...
- live recipient delivery reuses those views directly and appends them to the recipient reconnect screen buffer;
