...
7. [10.3 Script Execution Budgets, Quotas, and Isolation](./10.3-task-list-script-execution-budgets-quotas-and-isolation-vertical-slice.md)
   Remaining: terminal outbox retention cleanup, serialized work-item payload-size rejection, and first command-count output-budget enforcement are now live from the documented knobs; the next gaps are pin/signer freshness enforcement, admission-time versus execution-time budget charging, and dry-run/test isolation.
...
9. [10.5 Scripting Operator Visibility and Runtime Convergence](./10.5-task-list-scripting-operator-visibility-and-runtime-convergence-vertical-slice.md)
   Remaining: durable work/audit/cancel/claim state, terminal cleanup, current script-patch status summaries, bounded dead-letter listing plus replay, first scope-local drain reads, first Automation/Game Session pin-convergence reads with persisted pin request ids, first per-instance script-patch rollout reads with explicit freshness flags, Automation-owned plugin runtime state transitions, and the first Game Design-backed plugin publication plus immutable runtime compatibility checks (`baseVersionId` and release-bundle-backed `abilitySchemaDigest`) now exist; the next gaps are admission-epoch-aware drain semantics, richer event-projected rollout history, and remaining signer/component-policy gates.
