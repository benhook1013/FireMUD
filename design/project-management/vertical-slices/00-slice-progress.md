...
8. [10.4 Automation Handoff and Tick Integration](./10.4-task-list-automation-handoff-and-tick-integration-vertical-slice.md)
   Remaining: Automation-owned Redis queue and quota helpers now use canonical instance-aware key families; the stale standalone `automation:tick:*` staging path has been retired from the live/runtime design; `automation:queue:*` now stores the documented durable outbox-pointer envelope; queue drains dedupe by `outboxWorkItemId`; bounded scheduled rebuild now republishes missing queue pointers from durable pending work; bounded queue-health inspection now emits orphan/oldest-age projection metrics; Game Session now has an idempotent `EnqueueAutomationCommandIfAbsent` API backed by dispatch identity on the gameplay command ledger and fenced by durable runtime ownership so stale-epoch, paused, or missing ownership requests cannot enter the tick queue; Game Session command-status reads can now resolve by the same automation dispatch tuple used by Automation handoff history; Automation now has the matching handoff service that maps emitted work-item commands to Game Session outcomes; durable per-command `ListScriptHandoffEvents` now keeps dispatch identity, rendered emitted command text, and Game Session admission outcomes queryable; and a first production executor now drains queue pointers for discovery, still claims durable work rows before execution, evaluates the current target-aware command-emission format including `commandAlias` plus templated `arguments`, bounded `when` / `unless` conditional emission, and first multi-target `targetEntityIds[]` fan-out, then hands off resulting gameplay commands. The next gaps are richer graph/runtime output semantics above this first structured command shape and broader execution-time fairness proof.
...
10. [10.2 Script Timer, Scheduler, and Runtime Ownership](./10.2-task-list-script-timer-scheduler-and-runtime-ownership-vertical-slice.md)
   Remaining: durable patch-scoped `script_schedule_definitions` plus the first instance-scoped `script_schedule_instances` materialization are now live, including `scheduleDefinitionId` duplicate detection, compiled plugin owner metadata, binding-target-aware schedule rows, observed pin request ids, wall-clock `nextDueAt` derivation, explicit `PENDING_RUNTIME_PROGRESS` rows for tick-aligned schedules, and first plugin-version-aware schedule filtering against Automation's runtime registry; the remaining gaps are the real scheduler runtime pieces on top of that substrate: Game Session heartbeat ingestion, region/epoch-aware due-point advancement, bounded catch-up, and richer runtime progression/reconciliation proof.
11. [02.1.3 Session Activity and WHO Presence](./02.1.3-task-list-session-activity-and-who-presence-vertical-slice.md)
...
12. [02.1.4 Cross-Game Social Presence and Friend Activity](./02.1.4-task-list-cross-game-social-presence-and-friend-activity-vertical-slice.md)
...
13. [06 Task List Inventory, Containers, Equipment](./06-task-list-inventory-containers-equipment-vertical-slice.md)
...
14. [06.3.1 Stable Item Instance Visible Ref Allocation](./06.3.1-task-list-item-instance-visible-ref-allocation-vertical-slice.md)
   Remaining: decide whether and where ordinary prose views ever expose refs beyond management surfaces.
15. [06.3.2 Authored Stackability and Fungibility](./06.3.2-task-list-authored-stackability-and-fungibility-vertical-slice.md)
