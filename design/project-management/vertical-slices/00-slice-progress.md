...
7. [10.3 Script Execution Budgets, Quotas, and Isolation](./10.3-task-list-script-execution-budgets-quotas-and-isolation-vertical-slice.md)
   Remaining: terminal outbox retention cleanup, serialized work-item payload-size rejection, first command-count output-budget enforcement, configurable pin/rollout projection freshness, admission-time live per-script quota charging, live tenant-budget reservation, live-budget bypass for dry-runs, and first per-minute dry-run tenant/principal quotas are now live from documented knobs; the next gaps are richer priority-tier/source policy, dedicated dry-run concurrency/capacity reservation, signer-policy freshness enforcement, and proving operator metrics/outcomes match actual charging decisions.
