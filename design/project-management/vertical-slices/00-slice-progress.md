...
9. [10.5 Scripting Operator Visibility and Runtime Convergence](./10.5-task-list-scripting-operator-visibility-and-runtime-convergence-vertical-slice.md)
   Remaining: durable work/audit/cancel/claim state, terminal cleanup, current script-patch status summaries, bounded dead-letter listing, Automation-owned plugin runtime state transitions, and the first Game Design-backed plugin publication plus `baseVersionId` activation checks now exist; the next gaps are richer drain/convergence reads, dead-letter replay, remaining signer/policy/`abilitySchemaDigest` compatibility gates, and freshness/staleness signaling.
