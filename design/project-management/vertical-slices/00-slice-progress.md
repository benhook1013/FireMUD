...
7. [10.3 Script Execution Budgets, Quotas, and Isolation](./10.3-task-list-script-execution-budgets-quotas-and-isolation-vertical-slice.md)
   Remaining: terminal outbox retention cleanup, serialized work-item payload-size rejection, target-aware command-count output-budget enforcement, configurable pin/rollout projection freshness, admission-time live per-script quota charging, priority-tagged live tenant-budget reservation, live-budget bypass for dry-runs, first per-minute dry-run tenant/principal quotas, and tenant-local dry-run execution capacity are now live from documented knobs; the next gaps are richer source policy, cluster-level dry-run capacity policy, signer-policy freshness enforcement, and proving operator metrics/outcomes match actual charging decisions.
