...
2. [02.18.8 Tick Batch and Effect Ledger Hardening](./02.18.8-task-list-tick-batch-and-effect-ledger-hardening-vertical-slice.md)
   Remaining: build on the now-live durable batch/effect substrate, gameplay-command selected-work manifest with current-boundary `enqueueSeq`, first retry-claim provenance now mirrored onto durable command status rows, source-aware requeue metrics, automation command/source identity plus admitted routing-bundle carry-through, sealed-manifest-authoritative replay recovery, stale-fence batch drain, and post-drain execution seam by broadening durable execution truth into timer and remote-follow-up source-claim manifests plus the later cross-region return-path/result contract.
...
9. [10.2 Script Timer, Scheduler, and Runtime Ownership](./10.2-task-list-script-timer-scheduler-and-runtime-ownership-vertical-slice.md)
   Remaining: durable patch-scoped `script_schedule_definitions` plus the first instance-scoped `script_schedule_instances` materialization are now live, including `scheduleDefinitionId` duplicate detection, compiled plugin owner metadata, binding-target-aware schedule rows, observed pin request ids, admitted routing-bundle persistence, wall-clock `nextDueAt` derivation, explicit `PENDING_RUNTIME_PROGRESS` rows for tick-aligned schedules, first plugin-version-aware schedule filtering against Automation's runtime registry, Game Session `ObserveRuntimeTickProgress` publication from the durable runtime ownership tick counter, deterministic timer-derived work-item emission, and bounded round-robin catch-up with response-level truncation counts plus aggregate firing/truncation metrics; the remaining gaps are audit-level truncation detail and richer runtime progression/reconciliation proof.
