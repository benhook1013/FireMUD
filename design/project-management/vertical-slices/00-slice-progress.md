...
10. [10.2 Script Timer, Scheduler, and Runtime Ownership](./10.2-task-list-script-timer-scheduler-and-runtime-ownership-vertical-slice.md)
   Remaining: the first durable patch-scoped `script_schedule_definitions` catalog is now live, including `scheduleDefinitionId` duplicate detection plus normalized cadence/unit metadata captured during patch reload; the remaining gaps are still the real scheduler runtime pieces: Game Session heartbeat ingestion, schedule instantiation into instance/region timer rows, bounded catch-up, and plugin-timer reconciliation.
