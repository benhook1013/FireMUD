...
1. [09.1 Realm Catalog and Admission-Pointer Routing](./09.1-task-list-realm-catalog-and-admission-pointer-routing-vertical-slice.md)
   Remaining: keep broader reconnect/cutover consumers on the now-live persisted Game Session admission-pointer authority and preserve the full world/realm/resolved-target/pointer-freshness routing bundle in later routing-sensitive seams now that operator-facing prepare/read/cutover/audit tooling, retry-safe cutover execution, gateway-routed operator access, explicit public-production pointer metadata, active session/presence routing-bundle carry-through, first-party stale-pointer proof, gameplay-originated scripting ingress/work-item/operator-read carry-through, the current scheduler-owned timer/work-item/audit routing carry-through, Automation->Game Session staged-command plus command-status carry-through, and fail-closed `playableStateScope` plus command-ledger routing-metadata persistence are in place.
2. [02.18.8 Tick Batch and Effect Ledger Hardening](./02.18.8-task-list-tick-batch-and-effect-ledger-hardening-vertical-slice.md)
   Remaining: build on the now-live durable batch/effect substrate, explicit current-boundary `regionId` on ownership and `tick_batch`, gameplay-command selected-work manifest with current-boundary `enqueueSeq`, queue-claim kind/state/ordinal plus due-point metadata now mirrored onto durable command status rows, source-aware requeue metrics, automation command/source identity plus admitted routing-bundle carry-through, fail-closed Automation -> Game Session region-scope ownership checks that now reject stale same-instance handoffs before local queue staging, sealed-manifest-authoritative replay recovery, stale-fence batch drain, origin-side executable `remote_followup` / `remote_command_coordinator` / `remote_followup_result` substrate with explicit coordinator `followupId` linking plus first `remote:*` hint, direct request-supplied routing/provenance carry-through at remote scheduling time, target-side payload fallback onto persisted remote-row provenance for the live automation-command and gameplay-command handlers, target-side gameplay-command rows now persisting `remoteCoordinatorId` / `remoteFollowupId` with durable `remoteFollowupId` idempotence and status lookup from those ids, remote coordinator/followup/result reads plus origin command-status reads now exposing target-command id/status from durable followup linkage rather than payload-only inference, pending origin reconciliation now waiting for linked target-command terminal convergence, failing closed when the target command evidence is missing, mirroring concrete target-command failure codes back onto the origin command instead of flattening everything to generic remote-abandoned status, and now also preferring concrete durable remote-result failure code/message when the target command row itself is unavailable, mirroring that same concrete remote failure code/message onto the terminal follow-up row instead of flattening it there too, rejecting retry-time remote metadata rewrites instead of allowing due/deadline/payload/provenance drift under one durable coordinator/followup identity, persisting remote payload kind/requested command plus result command/error/message identity as first-class row authority instead of reparsing JSON first on every read, carrying that same command/provenance/routing/payload-summary truth into the sealed `REMOTE_FOLLOWUP_QUEUE` manifest instead of leaving remote replay/inspection dependent on opaque payload blobs, and clearing stale release failure metadata when remote follow-ups are fairly re-claimed for a new drain pass, structured payload/result summaries on remote control-plane reads and gameplay command status, due/backlog observation now exposed on runtime ownership reads plus `remoteFollowupDrainLagMs` and on the documented per-scope `remote_followups_due_total` / `remote_followups_drain_lag_ms` gauges, coordinator/followup/result control-plane reads now driven by current followup/result state plus persisted remote-row routing/provenance and direct script-dispatch identity truth, committed-tick timeout reconciliation, origin-tick result-inbox reconciliation ahead of timeout evaluation with stale-origin-epoch guardrails plus fail-closed result-scope validation, immutable retry identity guards, immutable `resultId` replay guards plus deterministic target-side `remote-result:<followupId>` writes that no longer rewrite original inbox timing on identical replay, stable mirrored timeout failure taxonomy across ignored late-result bookkeeping, paused-origin result reconciliation that stays live without advancing timeout progression, linked gameplay-command convergence onto `PENDING_REMOTE` / `TIMEOUT` / terminal remote outcomes, and target-side claim/release work now staged as `REMOTE_FOLLOWUP_DRAIN` batches with one-claim-per-entity fairness plus explicit persisted `claimOrdinal` ordering, explicit missing-coordinator abandonment, and the first live remote payload families before broader producers and the later full cross-region return-path/result contract land.
...
3. [02.18.9 Region Epoch, Fencing, and Runtime Ownership](./02.18.9-task-list-region-epoch-fencing-and-runtime-ownership-vertical-slice.md)
   Remaining: the durable owner row, explicit current-boundary `regionId`, pause/resume epoch bumps, stale-fence batch rejection, stale post-drain effect requeue, last-committed-batch pointer, bounded ownership-status query, and Automation tick-progress publication from that stored scope are now live at the current game-instance boundary; the next gap is carrying that model into true region ownership and later downstream/domain-specific effect families.
4. [10.1 Script Event Ingress and Handler Resolution](./10.1-task-list-script-event-ingress-and-handler-resolution-vertical-slice.md)
   Remaining: build on the now-live event-scope admission, ingress audit, durable bindings, pending-work materialization, tenant-readiness `onLoad`, current Game Session `onCommand`, fresh-PLAY `onSpawn`, and movement lifecycle producers, gameplay plus current scheduler and staged-command/status routing-bundle carry-through, fail-closed runtime region-scope fencing on gameplay-originated ingress, non-session-backed `onCommand` carry-through for Automation/remote durable execution, and first durable evaluator by broadening producer coverage and moving beyond the current command-template execution boundary.
...
8. [10.5 Scripting Operator Visibility and Runtime Convergence](./10.5-task-list-scripting-operator-visibility-and-runtime-convergence-vertical-slice.md)
   Remaining: durable work/audit/cancel/claim state, terminal cleanup, current script-patch readiness projections plus supersession semantics, bounded dead-letter listing plus replay including readiness-scoped `onLoad` replay behind canonical readiness projection checks, operator-canceled readiness now surfacing as `ROLLED_BACK`, patch-status listing preserving legacy/runtime-only rows alongside readiness-projected rows and now cross-linking current Game Design script-patch publication metadata, Automation pin-convergence and its durable pin projection now carrying current owned runtime `regionId` / `regionEpoch`, Game Session pin-convergence, pinned patch version/runtime-state rows now also exposing current owned runtime `regionId` / `regionEpoch`, gameplay command status, schedule-instance rows, dead-letter rows, handoff history including pre-enqueue stale-scope cancellation, and scheduler timer audit now also carrying script-patch publication cross-links, first scope-local drain reads, first Automation/Game Session pin-convergence reads with persisted pin request ids, first per-instance script-patch rollout reads with explicit freshness flags and now publication cross-links on current rollout status, rollout listings, and rollout event history, Automation-owned plugin runtime state transitions plus append-only runtime history with publication cross-links on current status, runtime event, and policy-convergence violation surfaces, Game Design-backed plugin publication plus immutable runtime compatibility checks (`baseVersionId` and release-bundle-backed `abilitySchemaDigest`), signer/component-policy activation gates, scheduled policy reconciliation for already-enabled plugins, and plugin-version pending-work cancellation now exist; the next gaps are later freshness/read-model separation and richer activation/publication event handling.
...
1. [10 Scripting, Automation, and Runtime Orchestration](./10-task-list-scripting-automation-and-runtime-orchestration-vertical-slice.md)
   Remaining: the scripting domain now has a canonical family instead of fragmented indirect coverage; `10.1` through `10.5` are the bounded runtime/control-plane cuts, with explicit freshness now live across rollout, plugin-policy status, plugin-policy convergence, Automation pin-convergence, Game Session pin-convergence, drain-status, and scheduler-materialization reads, while design-time publication boundaries remain intentionally anchored in `08.4`.
