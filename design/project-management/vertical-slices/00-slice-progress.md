...
8. [10.4 Automation Handoff and Tick Integration](./10.4-task-list-automation-handoff-and-tick-integration-vertical-slice.md)
   Remaining: Automation-owned Redis queue, tick-staging, and quota helpers now use canonical instance-aware key families; the next gaps are per-command `automationDispatchId` mapping, bounded Game Session admission outcome mapping, and real outbox-to-tick handoff.
