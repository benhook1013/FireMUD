...
8. [10.4 Automation Handoff and Tick Integration](./10.4-task-list-automation-handoff-and-tick-integration-vertical-slice.md)
   Remaining: Automation-owned Redis queue, tick-staging, and quota helpers now use canonical instance-aware key families, and Game Session now has an idempotent `EnqueueAutomationCommandIfAbsent` API backed by dispatch identity on the gameplay command ledger; the next gap is wiring Automation's durable outbox drain to that API and mapping returned outcomes back to work-item/audit state.
