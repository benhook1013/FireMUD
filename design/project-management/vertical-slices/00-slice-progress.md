...
2. `08` game-design publish/version/activation family
   Best next control-plane implementation family after `09`; `08.1` now has the canonical publish-attempt framework and script-patch gating, and `08.2` now has artifact-state proof / repair, so the next honest work is full-version participant coverage, purge workflow coverage, and then `08.3`.
