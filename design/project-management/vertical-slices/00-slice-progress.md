...
9. [10.5 Scripting Operator Visibility and Runtime Convergence](./10.5-task-list-scripting-operator-visibility-and-runtime-convergence-vertical-slice.md)
   Remaining: durable work/audit/cancel/claim state, terminal cleanup, current script-patch readiness projections plus supersession semantics, bounded dead-letter listing plus replay including readiness-scoped `onLoad` replay behind canonical readiness projection checks, operator-canceled readiness now surfacing as `ROLLED_BACK`, patch-status listing preserving legacy/runtime-only rows alongside readiness-projected rows and now cross-linking current Game Design script-patch publication metadata, Automation pin-convergence, Game Session pin-convergence, schedule-instance rows, dead-letter rows, handoff history, and scheduler timer audit now also carrying script-patch publication cross-links, first scope-local drain reads, first Automation/Game Session pin-convergence reads with persisted pin request ids, first per-instance script-patch rollout reads with explicit freshness flags and now publication cross-links on current rollout status, rollout listings, and rollout event history, Automation-owned plugin runtime state transitions plus append-only runtime history with publication cross-links on current status, runtime event, and policy-convergence violation surfaces, Game Design-backed plugin publication plus immutable runtime compatibility checks (`baseVersionId` and release-bundle-backed `abilitySchemaDigest`), signer/component-policy activation gates, scheduled policy reconciliation for already-enabled plugins, and plugin-version pending-work cancellation now exist; the next gaps are later freshness/read-model separation and richer activation/publication event handling.
