...
6. [10.1 Script Event Ingress and Handler Resolution](./10.1-task-list-script-event-ingress-and-handler-resolution-vertical-slice.md)
   Remaining: build on the now-live event-scope `TriggerScriptEvent` admission, ingress audit, built-in registry enforcement/read APIs, snapshot-token validation, durable script event bindings, and first `script_work_items` outbox materialization by adding handler-scoped `script_event_audit` progression, runtime consumption, and first real Game Session or scheduler caller proof.
