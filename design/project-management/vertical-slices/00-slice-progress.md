...
1. [09.1 Realm Catalog and Admission-Pointer Routing](./09.1-task-list-realm-catalog-and-admission-pointer-routing-vertical-slice.md)
   Remaining: keep broader reconnect/cutover consumers on the now-live persisted Game Session admission-pointer authority and preserve the full world/realm/resolved-target/pointer-freshness routing bundle in later routing-sensitive seams now that operator-facing prepare/read/cutover/audit tooling, retry-safe cutover execution, gateway-routed operator access, explicit public-production pointer metadata, active session/presence routing-bundle carry-through, first-party stale-pointer proof, gameplay-originated scripting ingress/work-item/operator-read carry-through, the current scheduler-owned timer/work-item/audit routing carry-through, and Automation->Game Session staged-command plus command-status carry-through are in place.
2. [02.18.18.1 Gameplay Transcript and Baseline Proof Hardening](./02.18.18.1-task-list-gameplay-transcript-and-baseline-proof-hardening-vertical-slice.md)
   Remaining: finish the narrower gameplay-proof follow-through by hardening websocket transcript assertions against suite-load ordering flake, preserving suite-specific baseline fixtures across shared reset, and converging the remaining inline Redis/runtime/stub cleanup on one canonical fresh-gameplay-baseline helper.
3. [02.18.8 Tick Batch and Effect Ledger Hardening](./02.18.8-task-list-tick-batch-and-effect-ledger-hardening-vertical-slice.md)
   Remaining: build on the now-live durable batch/effect substrate, explicit current-boundary `regionId` on ownership and `tick_batch`, gameplay-command selected-work manifest with current-boundary `enqueueSeq`, queue-claim kind/state/ordinal plus due-point metadata now mirrored onto durable command status rows, source-aware requeue metrics, automation command/source identity plus admitted routing-bundle carry-through, sealed-manifest-authoritative replay recovery, stale-fence batch drain, origin-side executable `remote_followup` / `remote_command_coordinator` / `remote_followup_result` substrate with explicit coordinator `followupId` linking plus first `remote:*` hint, due/backlog observation now exposed on runtime ownership reads, coordinator reads now joined with current followup state plus latest result summary, committed-tick timeout reconciliation, origin-tick result-inbox reconciliation ahead of timeout evaluation with stale-origin-epoch guardrails plus fail-closed result-scope validation, immutable retry identity guards, immutable `resultId` replay guards plus deterministic target-side `remote-result:<followupId>` writes that no longer rewrite original inbox timing on identical replay, stable mirrored timeout failure taxonomy across ignored late-result bookkeeping, paused-origin result reconciliation that stays live without advancing timeout progression, linked gameplay-command convergence onto `PENDING_REMOTE` / `TIMEOUT` / terminal remote outcomes, and target-side claim/release work now staged as `REMOTE_FOLLOWUP_DRAIN` batches with one-claim-per-entity fairness plus explicit persisted `claimOrdinal` ordering, explicit missing-coordinator abandonment, and the first `enqueue_automation_command` payload handler before broader payload families and the later full cross-region return-path/result contract land.
4. [02.18.9 Region Epoch, Fencing, and Runtime Ownership](./02.18.9-task-list-region-epoch-fencing-and-runtime-ownership-vertical-slice.md)
...
5. [10.1 Script Event Ingress and Handler Resolution](./10.1-task-list-script-event-ingress-and-handler-resolution-vertical-slice.md)
   Remaining: build on the now-live event-scope admission, ingress audit, durable bindings, pending-work materialization, tenant-readiness `onLoad`, current Game Session `onCommand`, fresh-PLAY `onSpawn`, and movement lifecycle producers, gameplay plus current scheduler and staged-command/status routing-bundle carry-through, and first durable evaluator by broadening producer coverage and moving beyond the current command-template execution boundary.
6. [02.15.8 Environment Preflight and Secret-Binding Convergence](./02.15.8-task-list-environment-preflight-and-secret-binding-convergence-vertical-slice.md)
...
3. [09.1.1 Routing Bundle and Catalog Authority Follow-Through](./09.1.1-task-list-routing-bundle-and-catalog-authority-follow-through-vertical-slice.md)
   Remaining: eliminate the remaining selector-shape and local-catalog shortcuts by keeping `PLAY`-adjacent and reconnect/cutover-sensitive consumers on the full canonical routing bundle and fail-closed Game Session authority surface.
...
5. [02.13.7 Action Classification and Activity Semantics](./02.13.7-task-list-action-classification-and-activity-semantics-vertical-slice.md)
...
6. [02.13.9 Authored Action Definition and Execution Model](./02.13.9-task-list-authored-action-definition-and-execution-model-vertical-slice.md)
...
7. [10.1 Script Event Ingress and Handler Resolution](./10.1-task-list-script-event-ingress-and-handler-resolution-vertical-slice.md)
...
8. [10.3 Script Execution Budgets, Quotas, and Isolation](./10.3-task-list-script-execution-budgets-quotas-and-isolation-vertical-slice.md)
...
9. [10.5 Scripting Operator Visibility and Runtime Convergence](./10.5-task-list-scripting-operator-visibility-and-runtime-convergence-vertical-slice.md)
   Remaining: durable work/audit/cancel/claim state, terminal cleanup, current script-patch readiness projections plus supersession semantics, bounded dead-letter listing plus replay including readiness-scoped `onLoad` replay behind canonical readiness projection checks, operator-canceled readiness now surfacing as `ROLLED_BACK`, patch-status listing preserving legacy/runtime-only rows alongside readiness-projected rows and now cross-linking current Game Design script-patch publication metadata, Automation pin-convergence, Game Session pin-convergence, pinned patch version/runtime-state rows, schedule-instance rows, dead-letter rows, handoff history, and scheduler timer audit now also carrying script-patch publication cross-links, first scope-local drain reads, first Automation/Game Session pin-convergence reads with persisted pin request ids, first per-instance script-patch rollout reads with explicit freshness flags and now publication cross-links on current rollout status, rollout listings, and rollout event history, Automation-owned plugin runtime state transitions plus append-only runtime history with publication cross-links on current status, runtime event, and policy-convergence violation surfaces, Game Design-backed plugin publication plus immutable runtime compatibility checks (`baseVersionId` and release-bundle-backed `abilitySchemaDigest`), signer/component-policy activation gates, scheduled policy reconciliation for already-enabled plugins, and plugin-version pending-work cancellation now exist; the next gaps are later freshness/read-model separation and richer activation/publication event handling.
