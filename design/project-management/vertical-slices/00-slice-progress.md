...
2. [02.18.8 Tick Batch and Effect Ledger Hardening](./02.18.8-task-list-tick-batch-and-effect-ledger-hardening-vertical-slice.md)
   Remaining: build on the now-live durable batch/effect substrate, gameplay-command selected-work manifest with current-boundary `enqueueSeq`, queue-claim kind/state/ordinal plus due-point metadata now mirrored onto durable command status rows, source-aware requeue metrics, automation command/source identity plus admitted routing-bundle carry-through, sealed-manifest-authoritative replay recovery, stale-fence batch drain, and post-drain execution seam by broadening durable execution truth into timer and remote-follow-up source-claim manifests plus the later cross-region return-path/result contract.
