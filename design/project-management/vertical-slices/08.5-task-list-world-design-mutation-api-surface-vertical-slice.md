...
- World Management now exposes `ApplyWorldDesignMutation` for typed Draft region, zone, room, room-exit, and generation-rule mutations with revision-ledger idempotency plus aggregate/scope epoch enforcement.
...
- The remaining gap is breadth, not the foundational seam: spawn/population bindings, richer generation scope semantics, and the remaining focused error-behavior cases are still to be added on top of the live contract.
...
- Add declarative spawn/population binding mutations and their unresolved-entity validation path.
