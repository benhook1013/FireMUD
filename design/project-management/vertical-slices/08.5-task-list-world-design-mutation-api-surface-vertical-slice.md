...
- The remaining gap is breadth, not the foundational seam: broader composite generation payloads and broader editor caller rollout still remain on top of the live contract.
...
- Extend generation writes beyond the first scoped generation-rule/spawn-binding seams into broader composite payloads such as multi-room subtree replace/append flows.
- Broaden the mutation surface beyond scope-validated topology, generation-rule, and spawn-binding enforcement into richer generation payload breadth and broader caller adoption.
