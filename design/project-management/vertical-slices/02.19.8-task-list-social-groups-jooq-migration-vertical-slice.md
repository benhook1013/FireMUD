...
Goal: migrate `social-groups-service` onto `jOOQ + Flyway` so friend, guild, and social-presence persistence no longer sits on a separate ORM stack after the rest of the SQL services have converged. Status: implemented at the service-repository boundary.
