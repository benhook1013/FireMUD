...
Goal: add durable effect-identity and idempotency guards so replay, retry, and failover converge safely without double-applying gameplay consequences. Status: in progress with movement replay/no-op, first Entity Management item mutation response replay, first Game Session-owned communication/activity replay guard, and first Social Groups communication message replay guard now live.
...
- This is still not the full target state:
  ...
  - the current movement-backed guard, local communication/activity replay guard, and first Entity Management mutation guard are proving grounds that later slices can carry into broader domain guards and migrated command families.
