...
- [ ] Carry the same `effectId` replay/no-op contract into later owning domains as new authoritative mutation families land, especially future authored/game-defined gameplay mutations that gain real downstream side effects, richer actor/resource seams beyond the current actor-condition path, and later automation-owned durable side effects. The next bounded child slice is [`02.18.10.1`](./02.18.10.1-task-list-authored-action-and-resource-effect-replay-guards-vertical-slice.md).
