...
- treat the current built-in mutation surface as sufficiently proved; do not force another replay-guard batch only to keep this slice busy.
...
- keep later queue-source and ownership work in `02.18.8` and `02.18.9`; this slice is now about owning-domain duplicate-application convergence, not broader tick-substrate breadth.
