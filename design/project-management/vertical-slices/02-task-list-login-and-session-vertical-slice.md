...
- [x] Update `design/project-management/task-list-game-session-service.md`, `design/project-management/task-list-account-service.md`, `design/project-management/task-list-tcp-proxy-service.md`, and `design/project-management/task-list-spring-cloud-gateway.md` to reconcile tasks related to `LOGIN`, session resumption, and reconnection so those lists reference this vertical slice where appropriate rather than duplicating details.

---

## 7. Dev Mode Stubs and Real-Service Rollout

- [x] Ensure the dev-isolated profile keeps the LOGIN path runnable by providing a lightweight stubbed `SessionContextService` that replaces the Redis-backed implementation whenever `game-session.dev-isolated=true`, and document the TODO so future work knows to drop the stub when Redis is deployed (`services/game-session-service/src/main/java/net/firedevops/firemud/service/impl/RedisSessionContextService.java`).
- [x] Teach the dev-isolated `GameInstanceService` (and any supporting components) to persist or expose a fake `GameInstance` record so `LoginCommandHandler` can resolve the session row instead of returning `SESSION_NOT_FOUND`, and leave a clear migration note that this logic should be removed once Postgres-backed persistence is available (`services/game-session-service/src/main/java/net/firedevops/firemud/service/devisolated/`).
- [x] Audit tests that currently rely on these dev-isolated stubs or unstarted services; added `@Disabled` notes on `DevIsolatedGameSessionSmokeTest`, `GameSessionLoginIntegrationTest`, `GameSessionWebSocketHandlerIntegrationTest`, and `SessionResumptionFlowTest` so they clearly document the TODO and can be flipped when the real Account/Redis/GameInstance wiring is ready.
- [x] Update the developer documentation (smoke tests section, setup guides, etc.) with notes about the temporary stubbed services and the steps required to replace them with the production-grade Account/Redis/GameInstance implementations when those services land; specifically the `DEVELOPER_SETUP.md` dev-isolated section now mentions the disabled tests and the Game Session Service design doc describes the TODO.
