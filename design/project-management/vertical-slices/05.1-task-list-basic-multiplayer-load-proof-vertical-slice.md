...
- [x] The first next increment is now live as a bounded concurrent movement plus immediate destination `LOOK` churn proof. Later increments remain mixed WebSocket/Telnet clients, a communication fanout proof once the stack includes the required settings authority path, and later inventory/equipment concurrency after `06`.
