...
- The canonical local entry point is `./gradlew checkFlywayVersions`, which runs `dev-tools/check-flyway-versions.sh`.
...
- [x] Add a `dev-tools/check-flyway-versions.sh` style script that scans service migration directories.
