...
- WebSocket cross-service coverage also proves a successful carried-item equipment loop with `EQUIPMENT` / `WEAR` / `EQUIPMENT` / `REMOVE` / `EQUIPMENT`, including Entity Management equipment request context and nonblank durable effect ids. The remaining equipment cross-service gap is a representative failure case, not the successful bind/unbind path.
