...
- [x] Rework docs and examples so `SESSION` is no longer presented as part of the normal human-facing Telnet flow.
- [x] Keep `SESSION` only as advanced/smart-client metadata if it remains justified.
...
- [x] Update Telnet examples and tests so normal parity coverage does not rely on `SESSION` unless the specific test is intentionally exercising advanced-client attach behavior.
