...
Goal: replace the remaining coarse public-family forwarding for `/api/session/**` and `/api/admin/**` with one explicit external route inventory and matching owner-side enforcement so the gateway edge is deny-by-default beneath the documented public families. Status: complete at the current bounded boundary.
