...
Goal: close the remaining malformed JWT, malformed signed-token, and inconsistent claim-shape entry gaps so equivalent bad auth payloads fail the same way across live HTTP, websocket, gRPC, and bootstrap seams. Status: planned.

## Current Snapshot (2026-06-29)

- This slice remains `planned` and is not yet completed.
- Canonical claim parsing is present in several seams, but the malformed claim-shape parity batch in this file has not been executed end-to-end yet.
- Use this slice for the bounded parity work once the target seams are taken in one batch.
- Accuracy note (2026-06-29): this slice intentionally stays in planned state because a bounded parity batch has not yet been scheduled.
...
- [ ] Enumerate the still-live JWT and signed-token parsing seams after `02.1.7.1`, including HTTP interceptors, websocket/bootstrap filters, connect-context/signed-token helpers, and auth-adjacent service entry points.
- [ ] Record which required claims each seam currently trusts and where equivalent malformed claim shapes still drift in behavior.
- [ ] Skip already-converged or stale suggestions.

### 2. Canonical Helper Convergence

- [ ] Move repeated malformed-claim handling onto the smallest owning helper when the invariant is truly shared.
- [ ] Remove caller-local fallback identity reconstruction when canonical parsed claims are absent, blank, or contradictory.
- [ ] Converge on stable application error semantics rather than surface-local exception behavior.

### 3. Focused Negative-Path Proof

- [ ] Add or refresh focused tests for malformed token structure, wrong-type claims, blank claim strings, malformed role maps, contradictory account/tenant identity, and equivalent invalid signed-token cases where live.
- [ ] Prove touched seams return canonical application-level errors rather than partial success or transport leakage.
...
- This slice is still open and should be executed as a bounded batch before marking complete.
- Existing docs and tests already show partial adoption of `JwtClaims`, but no authoritative one-pass parity pass is complete in this slice.
