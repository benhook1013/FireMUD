...
Define one canonical policy for how player-entered spoken text is normalized for presentation so speech reads like deliberate dialogue without silently mutating meaning or creating transport-specific inconsistencies.
...
- Implementing the runtime normalization path beyond the initial `say` / `whisper` / `tell` adoption set.
...
- Runtime normalization implementation for `say`, `whisper`, and `tell`
- Transcript tests proving exact normalized dialogue output
