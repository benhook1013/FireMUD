...
- [ ] Migrate `game-session-service`.
  The first bounded Game Session control-plane cluster is now on `jOOQ`, and the first hot-path runtime/tick batch is also migrated: admission-pointer authority and audit, prepared version-upgrade persistence, feature flags, game manifests, game instances, gameplay command rows, runtime region status, and tick batch/effect persistence now use explicit `jOOQ` repositories. The remaining JPA surface is concentrated in the remote followup/result/coordinator cluster plus the later service-wide Hibernate/JPA runtime removal.
- [ ] Migrate `game-design-service`.
