...
- [ ] Migrate `game-session-service`.
  The first bounded Game Session control-plane cluster is now on `jOOQ`: admission-pointer authority and audit, prepared version-upgrade persistence, feature flags, and game manifests. The remaining JPA surface is concentrated in the heavier runtime/tick/remote/game-instance cluster.
