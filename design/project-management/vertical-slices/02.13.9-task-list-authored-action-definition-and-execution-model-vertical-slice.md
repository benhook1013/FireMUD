...
- the first non-built-in provider now exists:
  ...
  - fail-fast validation that rejects unsupported first-pass targeting, cooldown, and cost metadata instead of silently accepting dead config before the later shared timing/effect substrate exists.
