...
- A reusable `TargetingPolicy` returns eligible candidates; each action owns its own release-pinned `TargetSelection` cardinality and optional-target behavior so one policy can safely serve both single-target and multi-target effects.
...
- Authored actions snapshot a DML `TargetSelection` alongside their targeting-policy key. `EXACTLY_ONE`, bounded `UP_TO_N`, and operator-capped `ALL_ELIGIBLE` are the initial modes; later randomized selection must use a deterministic effect-id-derived seed.
