...
- A reusable `TargetingPolicy` returns eligible candidates; each action references a release-pinned reusable `TargetSelectionPolicy` for cardinality and typed selection strategy, while retaining action-specific optional-target behavior so one policy can safely serve both single-target and multi-target effects.
...
- Authored actions snapshot a DML `TargetSelectionPolicy` alongside their targeting-policy key. `EXACTLY_ONE`, bounded `UP_TO_N`, and operator-capped `ALL_ELIGIBLE` combine with `PLAYER_SELECTED`, `CANONICAL_ORDER`, typed `RANKED`, or deterministic effect-id-seeded `RANDOM_SEEDED` selection; optional-target behavior remains on the action definition.
