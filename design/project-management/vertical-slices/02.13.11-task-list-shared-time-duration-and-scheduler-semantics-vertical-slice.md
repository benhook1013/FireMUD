...
Goal: define one canonical timing and duration model for gameplay and platform workflows so cooldowns, buffs, reconnect grace, OTP expiry, AFK timers, and future scheduled effects do not each invent separate local semantics. Status: in progress with the first cross-service proto naming guard live.
...
- `dev-tools/check-proto-time-fields.py` scans all checked-in proto contracts for ambiguous time-related field names.
