...
Goal: decide whether future friendships, cross-game presence, and recent activity are account-scoped, character-scoped, or intentionally mixed by surface. Status: partially implemented.
...
- the first player-facing friend-presence payload is deliberately conservative and policy-light.

The current implementation is still narrower than the full target state:

- there is no canonical `lastSeenAt` source yet;
- visibility policy is currently enforced as a conservative default rather than a full explicit policy enum;
- hidden/privacy/staff suppression is not fully propagated across all future social surfaces yet.
...
- Replace the implicit conservative-default behavior with one explicit presence-visibility policy seam shared across social consumers.
- Add canonical account-scoped recent-presence / last-seen facts.
- Extend the mixed-scope model into later friend/activity consumers without leaking exact in-world location or hidden identity.
