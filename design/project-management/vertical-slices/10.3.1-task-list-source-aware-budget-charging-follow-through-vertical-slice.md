...
Goal: carry the now-live per-script and priority-tagged tenant-budget charging onto every current durable `script_work_items` producer family so budget policy is derived from one persisted source-truth contract instead of drifting by ingress path. Status: planned.
...
- [ ] Enumerate every current durable `script_work_items` producer family that can reach live budget charging.
- [ ] Record which source fields each producer persists today (`sourceKind`, `sourceService`, `eventType`, `priorityTag`, and any source-specific policy inputs).
- [ ] Skip producer families already fully converged or not yet live.

### 2. Charging-Contract Convergence

- [ ] Repair any touched producer path that still relies on weaker request-local or fallback source semantics before budget reservation.
- [ ] Keep budget reservation and denial outcomes derived from durable row authority for each touched source family.
- [ ] Preserve canonical denial vocabulary instead of creating producer-specific budget failure semantics.

### 3. Focused Proof and Docs

- [ ] Add or refresh focused tests proving source-aware budget charging across the touched producer families.
- [ ] Update `10.3` docs/status notes so the surviving open work is explicit after this cut.
- [ ] Re-run touched-service validation and Markdown/link proof.
