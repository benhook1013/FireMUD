...
Goal: carry the canonical input model from architecture into code by introducing a normalized command envelope with typed payload shapes through the active Game Session command path, without attempting a one-shot rewrite of every parser and handler. Status: partially implemented.
...
- The first bounded envelope step is now live in the Game Session text parser:
  ...
  - and the active handlers for `LOGIN`, `PLAY`, movement, and communication can consume those typed payloads while preserving `args()` compatibility.
- This slice is still incomplete:
  ...
  - and the active command handlers still mix typed-payload reads with legacy `args()` fallback instead of being fully migrated.
