...
- The first bounded envelope step is now live in the Game Session text parser:
  ...
  - and the active handlers for `LOGIN`, `PLAY`, movement, and built-in communication now consume those typed payloads directly instead of relying on legacy `args()` fallback.
