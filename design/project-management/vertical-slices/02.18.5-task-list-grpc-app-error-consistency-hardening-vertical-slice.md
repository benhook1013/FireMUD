...
One remaining concrete example is the public Game Session lifecycle API:
...
- but those failures can still escape as uncaught `IllegalStateException` transport failures instead of normal response `ErrorDetail` payloads unless the gRPC layer maps them explicitly.
...
- [ ] Convert Game Session lifecycle RPC failures caused by runtime-state/dependency validation into normal response `ErrorDetail` payloads instead of transport errors.
