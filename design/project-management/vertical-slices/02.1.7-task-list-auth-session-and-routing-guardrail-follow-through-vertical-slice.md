...
- Uncompleted seam pending main-thread review before slice completion: scan adjacent websocket/connection-read paths for remaining duplicated `SessionContext` bootstrap-route identity checks (tenant/runtime/world/realm/pointer-version) and converge any still-live instances into shared helpers if they are in-scope.
