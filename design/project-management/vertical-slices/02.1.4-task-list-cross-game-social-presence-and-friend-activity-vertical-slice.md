...
- no fabricated offline `lastSeenAt` values when the system does not yet have a canonical source.

Still future work under this slice:

- canonical bounded `lastSeenAt` / recent-presence storage and policy;
- explicit presence-visibility policy controls beyond the current conservative default;
...
5. defer `lastSeenAt`, richer privacy modes, and friend-facing character/world presentation until a canonical source exists.

This keeps the first slice structurally correct without pretending later policy work is already solved.

## Current Remaining Work

- Define and implement canonical recent-presence / `lastSeenAt` storage rather than returning only live presence.
- Introduce the shared presence-visibility policy seam across social surfaces.
