...
- the same recent-presence substrate now carries disconnect disposition (`TRANSPORT_LOSS`, `LOGOUT`, `TAKEOVER`) for later social-policy consumers, even though the first friend-presence payload still exposes only conservative `lastSeenAt`;
