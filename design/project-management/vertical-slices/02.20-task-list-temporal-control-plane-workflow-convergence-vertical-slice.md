...
Goal: introduce a real durable workflow engine for FireMUD’s long-running control-plane orchestration by adopting Temporal narrowly for workflow classes that need crash-proof execution, durable waits/timers, resumability, and operator-visible state, while explicitly keeping gameplay runtime, ticks, and Redis coordination out of scope. Status: planned.
...
- [ ] Land the shared Temporal workflow foundation and common contracts.
