...
Goal: make `automation-scripting-service` the first full `jOOQ + Flyway` service migration so the repo’s new SQL house style is proven on a queue/projection-heavy control-plane service rather than on a low-signal CRUD seam. Status: implemented at the service-migration boundary.
...
- [ ] Remove JPA/Hibernate runtime/config dependencies from the service.
  This is now a family-wide cleanup concern shared with the later `02.19.10` runtime removal slice rather than remaining service-local product-code migration debt.
