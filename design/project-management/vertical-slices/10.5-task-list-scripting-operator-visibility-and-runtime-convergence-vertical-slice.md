...
- [ ] Implement the slice end to end.
  ...
  - [x] `GetPluginPolicyConvergence` now reports enabled plugin runtime states whose current publication, signer, or component-policy metadata would fail closed, plus an explicit stale flag and violation-level publication cross-links for the underlying convergence evaluation.
