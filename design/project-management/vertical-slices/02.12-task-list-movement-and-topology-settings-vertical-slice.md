...
- Current effective-config behavior is now explicit:
  - movement and topology settings resolve directly from `MovementProperties` and `WorldTopologyProperties` in Game Session;
  - they currently act as operator/file-env-backed defaults;
  - topology values are a typed runtime contract for later consumers, not yet a broadly consumed world-state authority.
