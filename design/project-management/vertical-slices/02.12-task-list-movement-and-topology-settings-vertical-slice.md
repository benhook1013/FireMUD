...
- Current effective-config behavior is now explicit:
  - movement and topology settings now resolve through Game Session's first effective-settings read surface, merging `MovementProperties` and `WorldTopologyProperties` defaults with optional `firemud.settings-overrides.*` tenant and game-instance overrides;
  ...
  - those overrides are still operator/file-env-backed rather than DB-backed tenant/game settings;
  - topology values are now part of the same effective-settings contract even though broader runtime consumers remain future work.
