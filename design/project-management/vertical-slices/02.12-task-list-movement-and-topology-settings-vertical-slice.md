...
- Current effective-config behavior is now explicit:
  - movement and topology settings now resolve through Game Session's bounded effective-settings resolver, merging `MovementProperties` and `WorldTopologyProperties` defaults with optional `firemud.settings-overrides.*` tenant and game-instance overrides;
