...
Goal: define the canonical text-client authentication model around account-selected login modes, a shared account-level OTP secret, and a text-first `LOGIN <email> [secret]` flow that supports password, emailed OTP, and later authenticator-app TOTP without fragmenting the auth system. Status: Account Service challenge lifecycle and password/email-OTP policy, plus Game Session `LOGIN <email>` challenge initiation, are live; account-mode selection and authenticator-app TOTP enrollment remain pending.
