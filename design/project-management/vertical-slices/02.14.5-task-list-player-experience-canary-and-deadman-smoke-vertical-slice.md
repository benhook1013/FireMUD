...
- retained evidence no longer synthesizes a fake “incident opened” boolean; it now records the authoritative deadman check status plus an explicit `evidenceRef` so operators can tie the smoke record back to the external pager/deadman product.
