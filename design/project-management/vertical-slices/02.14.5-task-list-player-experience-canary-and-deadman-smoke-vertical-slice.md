...
Goal: make the prod-like observability contract executable by adding canonical player-flow canaries, independent entry-path blackbox/deadman signals, alert rules, and retained smoke evidence for the player-facing paths documented in the logging, monitoring, and testing architecture. Status: in progress.
...
What remains open is the runtime/evidence side: actual canary/blackbox/deadman signal producers, controlled non-production failure injection, and retained traffic-open evidence validation still need to land before this slice is end-to-end executable.
