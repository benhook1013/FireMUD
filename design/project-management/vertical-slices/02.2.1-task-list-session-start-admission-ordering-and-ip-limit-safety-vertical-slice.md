...
- exact behavior if the new session is admitted successfully but old-session teardown fails afterward;
- whether non-gRPC session-start entry points should adopt the same replacement-safe ordering.
...
- What should be the canonical behavior if the new replacement session is admitted successfully but the old session fails to stop afterward?
- Should every session-start entry point adopt the same replacement-safe ordering, or is this gRPC path intentionally the canonical admission seam?
