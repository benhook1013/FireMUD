...
The main risk is not bad architecture. It is excessive surface area. At this snapshot, 11 of 79 leaf capabilities are `implemented`, 66 are `partial`, and 2 are `not-implemented`; verification includes 5 `drift-found` capabilities. The system therefore has many credible foundations but relatively few completely converged product capabilities. It can remain nearly complete everywhere without becoming complete in the player-visible paths that matter most.
