...
- **Internal microservices communicate over gRPC**, with **mTLS** as the canonical target-state transport for Kubernetes-backed environments. Hosted preview may temporarily run plaintext internal gRPC as an explicitly documented implementation exception while Spring gRPC server TLS configuration is migrated to SSL bundles.
