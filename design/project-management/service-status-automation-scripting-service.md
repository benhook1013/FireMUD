...
- The current runtime implementation includes event-scope ingress admission with serialized payload-size enforcement, durable script event bindings, durable `script_work_items`, handler-scoped audit rows, pending-work cancellation, pending-work claiming, bounded dead-letter listing plus controlled dead-letter replay, a first Game Session `onCommand` producer, canonical instance-aware Automation Redis key helpers for queue/tick staging plus quota counters, the Game Session command handoff client/service boundary for emitted work-item commands, script-patch status reads derived from durable work, a durable plugin runtime registry/control-plane state transition surface with last-request and actor metadata, live activation-time reads from Game Design/Game Session for plugin publication plus base-version validation, and scheduled terminal outbox cleanup governed by the documented retention knobs.
...
- DSL execution and its command-output loop, scheduler/timer producers, richer plugin compatibility enforcement beyond publication plus base-version validation, and richer operator convergence reads beyond current patch/plugin status summaries remain incomplete.
