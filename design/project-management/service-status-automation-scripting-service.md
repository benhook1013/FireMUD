...
- DSL execution and its command-output loop, scheduler/timer producers, richer plugin compatibility enforcement beyond publication plus base-version validation, and richer operator convergence reads beyond current patch/plugin status summaries remain incomplete. The queue/index seam is no longer ingress-only, but the first real evaluator/consumer path from claimed work items through DSL execution and command handoff still remains.
