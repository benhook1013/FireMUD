...
- [x] Convert admin gRPC authorization failures from transport `PERMISSION_DENIED` exceptions into normal response `ErrorDetail` payloads with the usual metrics/logging/span path.
...
- Current admin gRPC services now use an explicit shared admin-role guard and return `PERMISSION_DENIED` through normal response `ErrorDetail` payloads instead of relying on transport-failure AOP interception.
