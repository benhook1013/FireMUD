...
- [ ] Tighten `game-session-service` readiness beyond downstream reachability by adding a bounded internal check for the session-state / command-enqueue path that first `LOOK` actually depends on.
