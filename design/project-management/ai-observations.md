...
- `2026-06-22`: Source-built Docker smoke can reuse stale service images even after local boot jars change
  - Context: while validating `09.1.33`, `bash dev-tools/verify-fresh-bootstrap.sh` kept booting a `tcp-proxy-service` container that still showed the old missing-`BlockingGrpcStubCustomizer` failure even after local code, tests, and the packaged boot jar had been updated. Comparing the local boot jar run with container logs and image metadata showed the local jar was current but `docker-tcp-proxy-service` was still on an older image timestamp.
  - Observation: in this repo's jar-context compose build path, a normal `docker compose build <service>` inside the smoke loop can appear green while still reusing an older service image, so runtime logs may disagree with the actual current boot jar on disk.
  - Expected pattern: when smoke is supposed to prove a freshly changed service image, force a no-cache rebuild of the touched compose services before the bootstrap script and then tear the stack back down once proof is complete.

- `2026-06-22`: Singular routing contracts can stay internally inconsistent if one selector dimension is removed before multi-world fixtures are revalidated
  - Context: after `09.1.24` and `09.1.39` converged public admission-pointer reads away from `worldSlug + realmSlug`, broader validation exposed that one tenant can legitimately carry both `demo/production` and `sandbox/production` at the same time in real proof fixtures.
  - Observation: collapsing a singular routing contract to a narrower selector can look cleaner locally, but if duplicate realm slugs remain valid across worlds the narrower key silently becomes under-specified and the drift only shows up later in bootstrap readers, idempotent replay keys, or cross-service admission checks.
  - Expected pattern: before declaring one selector dimension dead in routing contracts, revalidate the contract against multi-world duplicate-selector fixtures and ensure singular reads, replay/idempotency storage, and cross-service consumers all still have enough identity to remain unambiguous.
  - Expected pattern: when source-built smoke logs disagree with fresh local service proof, verify the running image timestamp against the current boot jar and force a targeted `docker compose ... build --no-cache <service>` before trusting the runtime result; treat jar-vs-container timestamp drift as a real validation smell, not just Docker noise.

- `2026-06-22`: Session indexes may discover tenant identity, but they should not outlive tenant-scoped session authority
  - Context: extending the `09.1` session-normalization follow-through showed that `SessionRoutingNormalizationService.resolveProjectedSessionContext(...)` could still fall back to `findBySessionId(...)` after the raw session index had already identified the tenant, and `SessionAuthenticationService` still kept a duplicate raw tenant lookup beside that shared helper.
  - Observation: once a session reader already knows which `{tenantId, sessionId}` row should be authoritative, reviving the raw session shell when that tenant-scoped row is gone quietly preserves a second authority path for login, projection, or reconnect helpers.
  - Expected pattern: raw `sessionId` indexes may be used to discover tenant identity, but the actual session shell should then come only from the canonical tenant-scoped row or fail closed; higher-level readers should converge on one shared helper for that sequence instead of duplicating raw storage lookups.

- `2026-06-22`: Lifecycle policies should not keep selector-shaped proof once runtime-target authority is already known
...
- `2026-06-22`: Public authority interfaces should shed dead selector-shaped reads even when proof still needs selector identity
  - Context: after `09.1.24` moved live Game Session and Account admission-pointer reads onto the canonical `{tenantId, realmSlug}` contract, `GameplayAdmissionPointerAuthorityService` still exposed a dead `findPointer(worldSlug, realmSlug)` overload and one integration proof still used it directly to find the `demo/production` pointer inside a fixture where tenant `22` legitimately has multiple `production` realms across worlds.
  - Observation: even when proof still needs selector identity, that does not justify preserving a dead singular selector-shaped public read helper, especially when the fixture itself proves that selector identity and tenant-scoped singular lookup solve different problems.
  - Expected pattern: once one live singular lookup shape is canonical, remove superseded public overloads and let selector-sensitive proof use broader read surfaces such as `listPointers()` plus explicit filtering instead of keeping a convenience singular helper alive.

- `2026-06-22`: Overlapping local Gradle test runs can corrupt service test-result state and fake follow-on failures
  - Context: while repairing PR `2324`, a second `:tcp-proxy-service:check -PfullCheck` rerun was started before an earlier Gradle run against the same service had fully drained, and the later pass reported `NoSuchFileException` / `EOFException` under `services/tcp-proxy-service/build/test-results/crossServiceTest/binary` even though the test XML suites themselves were already green.
  - Observation: when two local Gradle test runs write to the same service `build/test-results` tree at once, Gradle can surface bookkeeping failures that look like new test breakage even though the underlying assertions and generated XML reports passed.
  - Expected pattern: before trusting a late-stage `EOFException` or missing `in-progress-results-generic.bin` during local verification, confirm there is only one active Gradle/test worker touching that service's build directory; rerun the affected validation cleanly after the earlier process exits instead of patching code against the corrupted result.
