...
- 2026-04-20: Running overlapping Gradle invocations against proto-heavy services can surface transient generated-source/classpath failures (for example missing `Errors.class` or proto output directories) that are not real code regressions. For AI workflows, keep heavy Gradle verification serial rather than parallel when proto generation is in play.

- 2026-04-20: Static gateway route docs/tests need to prove external-prefix rewriting, not just route allowlisting. Adding operator endpoints exposed that `/api/<family>/**` Gateway routes were documented as canonical external surfaces, while the YAML route definitions forwarded the raw `/api/<family>` prefix to services whose controllers are rooted at service-local paths such as `/auth/login`, `/logs/query`, and `/admission-pointers`. The expected pattern is to test both the public path predicate and the forwarding rewrite/strip filter so a route is not merely allowlisted but actually reaches the documented internal API.
