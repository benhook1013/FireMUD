...
- `2026-04-29`: Shared hosted-environment helpers should not live under one environment's folder with environment-specific names
  - Context: reviewing `dev-tools/hosted/preview/` against `dev-tools/hosted/dev-demo/` showed that kubeconfig setup, namespace deletion, GHCR pull-secret setup, TLS secret setup, runtime-image waiting, and hosted TCP smoke were all used by both lanes even though the files still lived under `preview/` with preview-specific names.
  - Observation: once a second hosted environment starts reusing helpers from a sibling folder, leaving them under the first environment's namespace makes the ownership model harder to trust and encourages misleading names like `delete-preview-namespace.sh` or `wait-for-preview-images.sh` for shared behavior.
  - Expected pattern: move genuinely shared hosted-environment helpers into a neutral `hosted/shared/` lane and reserve `preview/` and `dev-demo/` for logic that is truly specific to those environment classes.
