...
- `2026-09-07`: Relative `apply_patch` paths can escape a delegated worktree
  - Context: a delegated pruning task read ten files with `git show HEAD:<path>` in `/home/ben/src/FireMUD-corpus-review-5b-admission-authority-integration`, then passed delete/add patches with repository-relative paths to `apply_patch`; the patch tool resolved those paths against the session root `/home/ben/src/FireMUD-wsl-copy` instead of the command workdir. Nine Java files remained overwritten there after one file was manually corrected. That checkout already contained two conflicted architecture documents and two unrelated modified workflow guides; those pre-existing files were not part of the overwrite.
  - Observation: an `exec_command` workdir does not establish `apply_patch` path resolution. The missing guard was an absolute-path preflight for every patch target plus a post-edit status check in both the intended worktree and the session-root checkout. Recovery replaced only the nine still-modified Java files from the affected checkout's existing index blobs through absolute-path patches; every recovered working-tree hash matched its index blob, a scoped `git diff --quiet` passed, and the checkout's two pre-existing conflicts plus two unrelated workflow edits remained unchanged.
  - Expected pattern: delegated edits in a non-root worktree must use absolute `apply_patch` paths, verify the path prefix before the first write, and compare scoped status in both the target worktree and session root immediately after editing.
