...
- `2026-09-07`: Relative `apply_patch` paths can escape a delegated worktree
  - Context: a delegated pruning task read ten files with `git show HEAD:<path>` in `[delegated worktree]`, then passed delete/add patches with repository-relative paths to `apply_patch`; the patch tool resolved those paths against the session root `[session-root checkout]` instead of the command workdir. Nine Java files remained overwritten there after one file was manually corrected. That checkout already contained two conflicted architecture documents and two unrelated modified workflow guides; those pre-existing files were not part of the overwrite.
