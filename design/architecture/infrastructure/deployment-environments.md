...
- **Edge termination plus internal PROXY mode** – Expose a small Telnet edge proxy (for example, HAProxy) as the public TLS-terminating `LoadBalancer`. Forward the decrypted stream with PROXY protocol to the dedicated internal-only `TCP_PROXY_PROXY_PROTOCOL_PORT`. A separate `Service` plus source allowlist and `NetworkPolicy` must make that listener reachable only from the network-restricted edge path; network restriction is not authentication unless the edge-to-proxy channel also provides cryptographic authentication such as mTLS. PROXY framing alone does not authenticate its sender. TCP Proxy parses the header, forwards the recovered address as `X-Proxy-Client-IP`, and Gateway standardizes it as `X-Client-IP`.
