...
- Hosted `pr-preview` and `dev-demo-cluster` currently reuse the preview Helm values path. The target requires one environment-unique shared-HMAC signing key and non-secret diagnostic `jwt-jwks` content per deployment event, reused unchanged through render, dry-run, Helm apply, and retries. Current renderers regenerate material per invocation, so retry reuse is not proved. The preview workflow runs `preflight.py hosted-bridge` in `ci-static` context after Helm render and before Kubernetes server dry-run or Helm apply, then runs the operator context only after the rollout can consume the controller-projected TLS Secrets and before bootstrap, smoke, or reconciled-head publication. Missing or delayed projection therefore remains fail-closed without making a pre-projection Secret read circular. Dev-demo retains its existing hosted-bridge ordering pending convergence with the separate hosted identity lifecycle. This narrow gate validates the rendered internal Gateway bridge contract and, in operator context, the controller-projected TLS Secret keys. It does not prove live JWKS acceptance or asymmetric non-exportable signer custody. Helm materializes the signing key as the namespace-scoped `jwt-signing-keys` `Secret` and the diagnostic content as the namespace-scoped `jwt-jwks` `ConfigMap`, with the Account workload mounting that ConfigMap. Current validators use the Secret/path contract; the ConfigMap content is not the validation key. This preview diagnostic wiring is not the player-facing target: the sole target is Account-published public JWKS with non-exportable signer custody and validator consumption proven separately. The separate `helm-jwks-contract.sh` proof covers only configured preview resource/path/mount wiring, not live JWKS acceptance or target asymmetric, non-exportable signer custody.
