...
| --- | --- | --- |
| `LEGACY_SECRET_DIAGNOSTIC` | Checked-in player-facing Kustomize requires environment-owned `jwt-signing-keys` and `jwt-jwks` Secret bindings. | Hosted preview and `dev-demo-cluster` diagnostic Helm may list its `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap in the candidate inventory; each requires post-apply identity, public-state, reference, and mount proof. |
