...
The trusted consumer derives one immutable exposure mode from the validated sanitized artifact before allocation or runtime mutation: `Service/tcp-proxy-service.spec.type: ClusterIP` selects `private`, and `NodePort` selects `public`; any other type, missing service, or later mode mismatch fails closed. The trusted workflow records that mode on the runtime Namespace as `firemud.dev/preview-exposure-mode`; PR-controlled rendering cannot choose a different mode after validation. Requester credentials remain confined to the protected `trusted-hosted-cluster` workflow boundary and are used only for the canonical identity request, never exposed to the PR-controlled source or render jobs.
