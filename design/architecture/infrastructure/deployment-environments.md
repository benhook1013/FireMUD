...
- Redis is deployed as **two logical roles** in both Kubernetes and Docker Compose (see [Redis Architecture](../system-architecture-redis.md)):
  - A **Coordination Redis** deployment runs as a clustered StatefulSet with automatic failover in production and smaller clusters in non-production. It uses AOF, persistent volumes, and `noeviction` in player-facing environments so it behaves as a long-lived coordination log across normal rollouts and pod restarts. Security-critical writes such as connect-token consumption additionally require their documented replication/persistence acknowledgement and fail-closed continuity barrier.
...
- **Edge termination plus internal PROXY mode** – Expose a small Telnet edge proxy (for example, HAProxy) as the public TLS-terminating `LoadBalancer`. Forward the decrypted stream with PROXY protocol to the dedicated internal-only `TCP_PROXY_PROXY_PROTOCOL_PORT`. A separate `Service` plus source allowlist and `NetworkPolicy` must make that listener reachable only from the network-restricted edge path; network restriction is not authentication unless the edge-to-proxy channel also provides cryptographic authentication such as mTLS. PROXY framing alone does not authenticate its sender. Until that edge-to-TCP Proxy channel is authenticated and cryptographically protected, the recovered address is advisory only and must not drive per-IP throttles, abuse controls, or admission decisions. TCP Proxy parses the header, forwards the recovered address as `X-Proxy-Client-IP`, and Gateway standardizes it as `X-Client-IP`.
