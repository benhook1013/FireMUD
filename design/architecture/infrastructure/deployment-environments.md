...
Local Docker smoke is a disposable test-deployment boundary, not a Coordination Redis reset mechanism. The fresh-bootstrap and image-tag entrypoints require an explicit run-owned `COMPOSE_PROJECT_NAME` in the local `firemud-smoke-<unique-run-id>` form or existing CI `smoke-full-<run-id>-<attempt>` form before whole-stack teardown. Their default two-transport flow is the read-only `LOGIN` -> `PLAY` -> `LOOK` baseline; mutating item/container/equipment checks require the standalone client mutation opt-in and validated run-owned boundary. Shared environments remain ineligible for that mutation path until the restricted-synthetic identity/namespace/fence verifier exists.
