...
- The trusted preview workflow now has an explicit, fail-closed exposure mode. Its static/local proof is present for the #2713 private Gateway-to-TCP Proxy bridge path, but no live controller rollout or private bridge proof is claimed; public transport-complete work remains the #2795 path.
...
- `hobby-self-hosted`: the project's recognized and supported target-state ordinary community no-money lane. It is player-facing, but player traffic remains blocked until the accepted custody proof and other traffic-open checks are implemented and pass; the current `LEGACY_SECRET_DIAGNOSTIC` mode is non-authorizing. Architecture support for this topology does not grant or narrow legal permission under [LICENSE.md](../../../LICENSE.md), including its separate institutional permissions, or grant unconditional brand permission; descriptive references remain conditional on [TRADEMARKS.md](../../../TRADEMARKS.md). See [LICENSING.md](../../../LICENSING.md).
...
| `pr-preview` | Hosted single-node Kubernetes preview cluster with one runtime namespace per PR and a retained identity namespace | Kubernetes Secrets/ConfigMaps plus registry pull credentials | Production-like for HTTPS, auth, and session flows; simplified operator controls acceptable | Runtime namespace is reset on redeploy; retained identity has explicit retirement; no backup/restore guarantee | Trusted default-branch workflow consumes validated PR render artifacts and prepares runtime; `standalone` remains the default, while only explicit `hosted-controller` mode requests controller reconciliation via Helm-backed runtime delivery |
| `dev-demo-cluster` | Shared but non-player-facing Kubernetes cluster with disposable `dev` runtime and retained `dev-demo` identity | Kubernetes Secrets/ConfigMaps | Basic hardening; can prioritize iteration speed | Runtime is reproducible; identity reconciles while active; ad hoc backup/restore unless explicitly scheduled | Fixed `develop` deploy workflow prepares runtime; explicit `hosted-controller` mode then requests controller reconciliation |
...
The Kubernetes target classes above are separate from the GitHub Actions credential boundary. Every secret-consuming hosted preview and dev-demo job uses the protected GitHub Environment `trusted-hosted-cluster`, whose deployment branch policy must be restricted to `develop`. The PR-controlled preview source and render jobs select no GitHub Environment and receive no cluster, registry-write, or requester credentials. Legacy GitHub Environments named `pr-preview` and `dev-demo-cluster` are not selected by the trusted workflows and must not contain requester credentials. The protected Environment uses distinct secret names: `TRUSTED_HOSTED_PREVIEW_KUBECONFIG`, `TRUSTED_HOSTED_PREVIEW_RUNTIME_KUBECONFIG`, `TRUSTED_HOSTED_PREVIEW_GHCR_USERNAME`, `TRUSTED_HOSTED_PREVIEW_GHCR_TOKEN`, and `TRUSTED_HOSTED_IDENTITY_REQUESTER_KUBECONFIG`. Old repository-scoped runtime and GHCR credentials must be migrated into the protected Environment or removed and rotated before live activation; environment creation and static workflow checks alone do not establish that proof.

Local Docker smoke is a disposable test-deployment boundary, not a Coordination Redis reset mechanism. Fresh-bootstrap and image-tag entrypoints establish a new claim when the expected project is empty, or reuse the exact matching claim/capability. Restart-state requires/reuses an already-existing matching claim/capability. All retain the exact per-run ID/project binding and protected Compose access; their default two-transport flow is the read-only `LOGIN` -> `PLAY` -> `LOOK` baseline, while mutating item/container/equipment checks require the standalone client mutation opt-in and validated run-owned boundary. Project-name shape alone is not authorization. Shared environments remain ineligible for that mutation path until the restricted-synthetic identity/namespace/fence verifier exists. See [Testing: player-flow smoke and reset boundaries](../system-architecture-testing.md#player-flow-smoke-and-reset-boundaries) for the owner contract.
...
| Custody mode or diagnostic state | Baseline prerequisites before apply | Candidate-owned resources allowed absent before create |
| --- | --- | --- |
| `LEGACY_SECRET_DIAGNOSTIC` (diagnostic only; rejected before readiness/traffic-open) | Checked-in player-facing Kustomize requires environment-owned `jwt-signing-keys` and `jwt-jwks` Secret bindings. | No player-facing candidate-owned JWT/JWKS resource may be absent. Hosted preview and `dev-demo-cluster` diagnostic Helm are outside the ADR 0152 candidate-inventory contract; their rendered Secret/ConfigMap details are checked only by the hosted Helm render/server dry-run and the separate `helm-jwks-contract.sh` diagnostic proof. |
...
- `pr-preview` and `dev-demo-cluster` target the same Spring gRPC SSL-bundle mTLS contract as the rest of the Kubernetes-backed environments; they do not define a separate long-term internal gRPC transport mode. The temporary plaintext internal-gRPC exception defined by [gRPC TLS Requirements](../system-architecture-grpc.md#tls-requirements) is reserved policy for the bounded Spring gRPC `1.0.x` SSL-bundle migration and preview re-proof, not the current preview transport mode: current preview uses mTLS. That preview exception is non-player-facing and is neither traffic-open nor promotion evidence; `dev-demo-cluster` has no such exception. Their shared `firemud-grpc-tls` Secret otherwise proves encrypted transport only, not target per-workload identity or player-facing equivalence; distinct workload leaf identities with a shared CA and generic identity proof remain target work.
...
- Kubernetes-backed environments use mTLS for internal gRPC via Spring Boot SSL bundles and Spring gRPC server SSL bundle binding, including the hosted preview/dev-demo Helm path. The temporary plaintext `pr-preview` migration exception defined by [gRPC TLS Requirements](../system-architecture-grpc.md#tls-requirements) is reserved policy, not the current preview transport mode: current preview uses mTLS. That exception is non-player-facing, is not promotion or traffic-open evidence, and does not apply to `dev-demo-cluster`.
...
- On the explicit `hosted-controller` target path for hosted preview/dev-demo, **cert-manager** issues the public ingress and Telnet certificates plus the distinct Gateway-internal WebSocket server and TCP Proxy bridge client leaves from controller-owned `Certificate` resources. The in-cluster controller separately generates the shared gRPC mTLS bundle directly from the fixed `firemud-system/firemud-grpc-ca` Secret, then materializes and reconciles all five runtime projections; its non-secret Ready evidence does not replace served-certificate or player-facing preflight proof. This remains local/static target behavior: `standalone` is the default/current path until live controller activation and consumer-convergence proof exists.
...
- **Hosted dev/demo Kubernetes environments** use [`dev-demo.yml`](../../../.github/workflows/dev-demo.yml) for the fixed environment and [`preview.yml`](../../../.github/workflows/preview.yml) for credential-free per-PR rendering. The trusted default-branch [`hosted-identity-request.yml`](../../../.github/workflows/hosted-identity-request.yml) validates the render artifact and deploys the per-PR runtime. These workflows use the full-stack chart under [`k8s/helm/firemud`](../../../k8s/helm/firemud) with environment-specific example values and deployment metadata. These environments are intentionally excluded from the staging → production promotion chain and must not emit attestation artifacts.
...
A dedicated staging cluster mirrors production using smaller node sizes. Pull requests may also obtain a hosted [`pr-preview`](#canonical-environment-classes) environment through credential-free [preview.yml](../../../.github/workflows/preview.yml) rendering and trusted [hosted-identity-request.yml](../../../.github/workflows/hosted-identity-request.yml) deployment, but those previews remain isolated per PR and are not promotion candidates; staging is the intended environment for prod-like playtests and routing/TLS validation.
...
Hosted Helm values select certificate identity explicitly at `previewStack.certificateIdentity.mode`. A missing mode means `standalone`; the only accepted explicit values are `standalone` and `hosted-controller`. Malformed, duplicate, non-string, or unknown mode configuration is fatal, and selecting `hosted-controller` never falls back to workflow-created certificate identity. This keeps the controller and its authority dormant before activation while preserving existing standalone deployments.

For a standalone PR preview, the credential-free render artifact excludes PR-authored `Certificate` resources. After creating the exact runtime namespace, the trusted default-branch consumer creates fixed cert-manager `Certificate` resources for the public Telnet leaf (`pr-N-telnet-tls`, `letsencrypt-prod`), Gateway-internal WebSocket server (`pr-N-gateway-internal-ws`, `firemud-ca-issuer`), and TCP Proxy bridge client (`pr-N-tcp-proxy-bridge`, `firemud-ca-issuer`). It waits for issuance and key-complete Secrets before applying the sanitized runtime artifact. Names, issuers, DNS and URI identities are derived from the validated PR number and fixed trust contract, never from PR-controlled Certificate manifests. The trusted dev-demo Helm path retains its own standalone certificate rendering; hosted-controller mode uses only controller projections.

The canonical hosted identity API is `platform.firemud.dev/v1alpha1` `HostedEnvironmentIdentity` in `firemud-system`, named `pr-<PR_NUMBER>` or `dev-demo`, with only `spec.desiredState: Active|Retired`. The in-cluster controller owns identity namespaces, derived hosts, certificate resources, Secret projections, and non-secret status. A trusted workflow first validates the PR-produced sanitized render artifact and waits for its fixed-head runtime images without holding the shared preview-allocation lifecycle lock. Under that non-cancelling lock it revalidates the target, allocates capacity and, only for `public` mode, the Telnet port, creates and annotates the disposable runtime namespace with the requested preview head, requests `Active`, waits for all five labeled, key-complete Secret projections (`pr-<PR_NUMBER>-tls`, `pr-<PR_NUMBER>-telnet-tls`, `pr-<PR_NUMBER>-gateway-internal-ws`, `pr-<PR_NUMBER>-tcp-proxy-bridge`, and `firemud-grpc-tls`), applies the validated runtime, and records the deployed-preview-head annotation. The controller validates and consumes the trusted lifecycle's public Telnet allocation, or the private profile's zero sentinel; it does not derive or overwrite either value. Post-apply identity, rollout, and mode-specific verification runs after the mutation lock is released, revalidating the exact PR head around its waits so stale verification cannot publish success. The fixed dev-demo path also requires all five labeled, key-complete projections before Helm. Only a successful runtime apply may write the distinct deployed-preview-head annotation, and controller readiness requires the normalized requested and deployed SHA values to match. Identity provisioning failure therefore blocks deployment without giving PR-controlled rendering any cluster, runtime, or identity credential, while a failed deployment cannot make the requested preview appear deployed.

#### Preview exposure modes and proof boundaries

The trusted consumer derives one immutable exposure mode from the validated sanitized artifact before allocation or runtime mutation: `Service/tcp-proxy-service.spec.type: ClusterIP` selects `private`, and `NodePort` selects `public`; any other type, missing service, or later mode mismatch fails closed. The trusted workflow records that mode as the `firemud.dev/preview-exposure-mode` label on the runtime Namespace, not as an annotation; PR-controlled rendering cannot choose a different mode after validation. Requester credentials remain confined to the protected `trusted-hosted-cluster` workflow boundary and are used only for the canonical identity request, never exposed to the PR-controlled source or render jobs.

The bounded #2713 private proof is a parent bridge proof, not transport-complete preview acceptance. A `private` runtime must keep `tcp-proxy-service` as `ClusterIP`, have no public NodePort, and have no `firemud.dev/last-preview-telnet-port` annotation; the internal `telnetPort=0` value is a controller sentinel and is never written as a Service port. The current-head-bound controller may become `Ready` only after all five labeled, key-complete projections and required Gateway/TCP Proxy rollouts converge, and after served HTTPS, the private Gateway-to-TCP Proxy mutual-TLS bridge, and gRPC probes succeed. Private success therefore does not claim a public Telnet endpoint or the public gameplay smoke.

The `public` mode owned by #2795 retains the allocated preview NodePort and its Telnet-port annotation. It requires the same head-bound projections, rollout, HTTPS, bridge, and gRPC readiness plus the served public Telnet check and `LOGIN -> PLAY -> LOOK` smoke; a failed public check is a terminal failure for that mode. The workflow never converts a failed public deployment or smoke into `private`, and private success cannot satisfy the public mode's Telnet or ADR 0178 two-public-path obligations. The private exception is limited to this explicit parent bridge proof and does not amend the accepted public transport-complete target.

The fixed dev-demo lifecycle uses the same authority split only when `hosted-controller` is selected. It recreates and annotates the exact `dev` runtime with the requested head, image, and fixed NodePort `32016`, switches to requester credentials only for the fixed-shape `dev-demo` `Active` request, restores runtime credentials, and requires all five labeled, key-complete runtime projections before Helm. A distinct deployed-head annotation is written only after Helm succeeds; controller readiness requires it to match the requested head, so a failed Helm attempt cannot make the runtime look deployed. Success is recorded only after runtime rollout, generation/head-bound controller `Ready`, operator preflight, and the public TCP smoke. Its non-cancelling reconciler lets any in-progress same-head deploy candidate converge, allows at most three completed non-success deploy attempts for that exact head, reports retry exhaustion as a terminal workflow error, and does not reset a runtime already aligned by a successful deploy candidate. It checks nonterminal runs independently before walking completed history only until the latest exact candidate and its bounded retry result are known, so an older-record re-run is not hidden by a newer completed attempt. A workflow-owned run name binds each run to its action and effective target head, keeping explicit destroy and other-target deploy runs outside that deploy retry budget even when a run ends before its jobs materialize or more than one API page behind newer history. A new `develop` head receives a fresh retry budget. Explicit dev-demo destroy observes the runtime namespace as `NotFound` before requesting `Retired`, waiting for terminal retirement, and deleting the request. This does not activate the separate PR-close mutation path.

Privileged preview janitor, preview reconciler, dev-demo deploy/destroy, and dev-demo reconciler jobs accept scheduled or typed `repository_dispatch` requests rather than branch-selectable `workflow_dispatch` requests. A repository maintainer can trigger those trusted default-branch operations through `POST /repos/<owner>/<repo>/dispatches` with `event_type` set to `preview-janitor`, `preview-reconcile`, `dev-demo`, or `dev-demo-reconcile`; `dev-demo` accepts a validated `client_payload` with optional `action` (`deploy` or `destroy`), `head_sha`, `image_tag`, fixed `hostname`, and fixed `telnet_port`. The reconciler uses the same `dev-demo` event for exact-head retry. Manual per-PR deploy/release requests use the default-branch preview source workflow's typed `preview-deploy` or `preview-destroy` repository dispatch with a validated `client_payload` containing matching `action`, canonical `pr_number`, current `head_sha`, and fixed `preview_domain`; the source resolves the effective head-or-base image tag independently and produces only a sanitized render artifact or destroy intent for the trusted lifecycle. Priority labeling, capacity allocation, and cleanup remain trusted lifecycle decisions.

Before a trusted operator activates the hosted identity controller, that operator must provision the fixed `firemud-system/firemud-grpc-ca` `Opaque` Secret with exactly `ca.crt` and `ca.key`. The configured trust-anchor value is the lowercase 64-hex SHA-256 fingerprint of the DER-encoded CA certificate. Bootstrap verifies the key shape and certificate fingerprint without printing either value; the controller independently verifies that the CA certificate and private key correspond before it issues any transport-only leaf. The controller only consumes this prerequisite and is not a CA provisioning authority.

Runtime reclaim, redeploy, release, and lease expiry delete only the runtime namespace. Close/merge retirement is runtime-first: the trusted lifecycle deletes the exact runtime, requires a successful empty `--ignore-not-found` lookup proving Namespace `NotFound`, and only then requests `Retired`; lookup or deletion uncertainty retains identity and fails closed. Explicit identity removal of a live runtime is retirement-first: the trusted requester sets `Retired`, the controller scales Gateway and TCP Proxy to zero and waits for their observed shutdown, and the trusted runtime lifecycle then deletes the runtime and proves exact Namespace `NotFound`. In both paths, retained identity deletion remains blocked until the runtime is absent. Before deleting retained identity, the controller accounts for all five declared consumer projection roles: (1) [public ingress TLS](../system-architecture-security.md#tls-termination-for-gateway), (2) [public Telnet TLS](../system-architecture-protocol-bridging.md#public-telnet-tls-modes), (3) the [Gateway internal WebSocket server](../system-architecture-security.md#tls-config-matrix-tcp-proxy-spring-cloud-gateway-websocket), (4) the [TCP Proxy bridge client](../system-architecture-security.md#tls-config-matrix-tcp-proxy-spring-cloud-gateway-websocket), and (5) the [shared gRPC transport bundle](../decisions/adr-0038-explicit-jwt-profiles-and-mtls-workload-identity.md#mtls-identity-and-certificate-lifecycle). Controller readiness binds the CR generation, live runtime namespace UID, exact requested and deployed head SHA pair, rollout and mode-specific served proof, and non-empty ingress, Telnet projection, Gateway-internal WebSocket, TCP Proxy bridge, and gRPC revisions; public mode additionally requires the served Telnet probe. Canonical 40-hex SHA values are normalized before equality comparison. This controller contract is local/static proof only until a live hosted rollout; it does not alter stage or production deployment paths.

The hosted identity controller's egress boundary is exact-destination and operator-rendered. Its checked-in NetworkPolicy contains no `0.0.0.0/0` or `::/0`; trusted bootstrap substitutes validated `/32` IPv4 CIDRs for the observed `kubernetes.default` Service ClusterIP on TCP/443, the single API endpoint on TCP/6443, and the fixed public preview probe on TCP/443 plus NodePorts 32000-32016. Gateway access is separately limited to `spring-cloud-gateway` pods in the canonical preview and dev-demo namespaces on TCP/443 and TCP/8443. The observed k3s LoadBalancer path additionally requires the exact kube-system Traefik pod selector on TCP/8443 after public HTTPS DNAT, and the public TCP Proxy NodePort path requires the exact `tcp-proxy-service` selectors in those canonical namespaces on TCP/2323 after NodePort DNAT. DNS and Account TCP/6565 selectors remain explicit. `observe` and `active` bootstrap require all three addresses, require exactly one matching fixed public DNS A record, and verify the API Service and single-endpoint readback before any write; paused mode without addresses renders loopback-only deny markers. Standard NetworkPolicy may exempt traffic to the local node, and CNI post-DNAT behavior must be proven live for the public Traefik HTTPS and NodePort paths; the private proof separately needs live allow/deny evidence for its ClusterIP bridge path. This static boundary does not complete PO-3.2: live allow/deny and served Gateway↔TCP Proxy bridge proof remain required.
...
- Eligible same-repository pull requests request preview allocation automatically. Ordinary allocation is first-come at the serialized allocation boundary while capacity remains available; GitHub's raw job-scheduling order is not itself an allocation guarantee. The `preview:priority` label lets a later priority request reclaim the oldest ordinary preview, but never another currently priority-labelled preview. Allocation, deployment, hosted proof, reclaim, and cleanup share one durable bounded non-cancelling lifecycle queue so a running proof is not deleted underneath itself and normal waiting lifecycle work uses GitHub's expanded queue instead of its replaceable single pending slot. Both target and victim label state are re-read immediately before reclaim. Removing the label returns an existing environment to ordinary status without destroying it. A displaced pull request keeps its ordinary CI and historical preview result, while its current hosted status reports that no environment is allocated.
- Each successfully allocated eligible preview receives its own namespace and holds a bounded renewable visible lease. An ordinary active lease is protected except for the explicit priority rule above; a priority-labelled active lease is not silently evicted, while an expired lease may be reclaimed even if the pull request remains open. A request that is eligible but not allocated, including because capacity is exhausted or every slot is priority-protected, reports `preview_unavailable`, not passed.
- Any review claiming hosted proof requires successful evidence for its current head SHA.
- Preview runtime namespaces are removed when the pull request closes or merges, is explicitly released, or its lease expires; the retained identity namespace remains until the trusted lifecycle explicitly retires it under the exact runtime-`NotFound` rule.
