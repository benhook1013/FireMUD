...
- Kubernetes-backed environments use mTLS for internal gRPC via Spring Boot SSL bundles and Spring gRPC server SSL bundle binding, including the hosted preview/dev-demo Helm path, except for the bounded non-player-facing `pr-preview` migration state defined by [gRPC TLS Requirements](../system-architecture-grpc.md#tls-requirements). That exception is not promotion or traffic-open evidence and does not apply to `dev-demo-cluster`.
