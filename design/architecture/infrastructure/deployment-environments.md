...
- The independently versioned frontend artifact, unprivileged static container, preview/hobby `Deployment` and `Service`, reserved-path site routing, frontend health/security configuration, and deployed browser journey required by [ADR 0144](../decisions/adr-0144-stateless-first-party-frontend-application-boundary.md) are not yet implemented.
...
- Preview and hobby/self-hosted environments must include the independently released frontend as a small unprivileged static container with its own `Deployment` and `Service`. It requires no database, Redis, secret, or writable application filesystem.
- One public site-routing layer sends frontend documents and compiled files to that frontend Service, `/auth/**`, `/api/**`, and `/ws/game/**` to Spring Cloud Gateway, and `/assets/**` to the approved published-asset delivery origin. Gateway remains the sole public API and gameplay ingress.
- Production may use the same frontend container directly, place a CDN in front of it, or later publish the identical immutable artifact contract to object storage/CDN. Release identity, rollback, cache/CSP behavior, runtime-configuration freshness, and the reserved-path topology must remain unchanged.
