...
- Preview Telnet proof exists, but the deployed browser path, shared semantic assertions, head-bound lease handling, isolation proof, and transport-complete acceptance under [ADR 0178](../decisions/adr-0178-disposable-transport-complete-pr-preview-proof.md) remain implementation gaps. Hosted preview and dev-demo Helm select hosted-controller mode for the retained identity and direct TCP Proxy TLS on the existing NodePort, with cert-manager remaining the public issuer and a dedicated `<release>-telnet-tls` certificate Secret. Live preview TLS issuance and end-to-end proof are evidenced by run `34013593910` against runtime commit `069db222158059ef0badf2178dd44906aa04564b`; controller-backed issuance, rotation, served-certificate, and consumer-convergence proof, along with staging/production certificate lifecycle and deployment evidence, remain gaps.
...
The canonical hosted identity API is `platform.firemud.dev/v1alpha1` `HostedEnvironmentIdentity` in `firemud-system`, named `pr-<PR_NUMBER>` or `dev-demo`, with only `spec.desiredState: Active|Retired`. The in-cluster controller owns identity namespaces, derived hosts and ports, certificate resources, Secret consumers, and non-secret status. A trusted workflow first validates the PR-produced sanitized render artifact, allocates capacity and the Telnet port, creates and annotates the disposable runtime namespace, and applies the runtime; only then does the requester-only credential apply `Active`. PR-controlled rendering receives no cluster, runtime, or identity credentials.
...
Runtime reclaim, redeploy, release, and lease expiry delete only the runtime namespace. Close/merge and explicit retirement request `Retired` only after runtime deletion has returned exact `NotFound`; uncertainty retains identity and fails closed. Controller readiness binds the CR generation, live runtime namespace UID, exact deployed head SHA, rollout and served proof, and non-empty ingress, Telnet, and gRPC revisions. This controller contract is local/static proof only until a live hosted rollout; it does not alter stage or production deployment paths.
