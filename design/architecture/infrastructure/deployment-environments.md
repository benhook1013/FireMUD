...
The canonical hosted identity API is `platform.firemud.dev/v1alpha1` `HostedEnvironmentIdentity` in `firemud-system`, named `pr-<PR_NUMBER>` or `dev-demo`, with only `spec.desiredState: Active|Retired`. The in-cluster controller owns identity namespaces, derived hosts, certificate resources, Secret projections, and non-secret status. A trusted workflow first validates the PR-produced sanitized render artifact, allocates capacity and the Telnet port, and creates and annotates the disposable runtime namespace with the requested preview head. The controller validates and consumes that trusted lifecycle's allocated and annotated Telnet port; it does not derive or overwrite the allocation. The requester-only credential then applies `Active`; on the hosted PR bridge path, the workflow waits with its runtime credential for all five labeled, key-complete Secret projections (`pr-<PR_NUMBER>-tls`, `pr-<PR_NUMBER>-telnet-tls`, `pr-<PR_NUMBER>-gateway-internal-ws`, `pr-<PR_NUMBER>-tcp-proxy-bridge`, and `firemud-grpc-tls`) before application preflight, server dry-run, and deploy. The fixed dev-demo path also requires all five labeled, key-complete projections before Helm. Only a successful runtime apply may write the distinct deployed-preview-head annotation, and controller readiness requires the normalized requested and deployed SHA values to match. Identity provisioning failure therefore blocks deployment without giving PR-controlled rendering any cluster, runtime, or identity credential, while a failed deployment cannot make the requested preview appear deployed.
