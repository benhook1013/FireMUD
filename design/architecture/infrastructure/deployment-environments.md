...
- `pr-preview` currently allows a temporary preview-only exception for **plaintext internal gRPC** while the repository completes the Spring gRPC SSL-bundle migration. This exception must be documented in preview/operator docs and removed once `spring.grpc.server.ssl.*` plus `spring.ssl.bundle.*` are the canonical server-TLS path repo-wide.
...
- In the canonical target state, Kubernetes-backed environments use mTLS for internal gRPC. The current hosted preview environment may temporarily override that to plaintext internal gRPC as an explicitly documented implementation exception while server TLS configuration is migrated to Spring gRPC SSL bundles.
