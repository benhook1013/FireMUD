...
- Preview Telnet proof exists, but the deployed browser path, shared semantic assertions, head-bound lease handling, isolation proof, and transport-complete acceptance under [ADR 0178](../decisions/adr-0178-disposable-transport-complete-pr-preview-proof.md) remain implementation gaps. Hosted preview and dev-demo Helm now select direct TCP Proxy TLS on the existing NodePort with a dedicated `<release>-telnet-tls` certificate Secret. Live preview TLS issuance and end-to-end proof are evidenced by run `34013593910` against runtime commit `069db222158059ef0badf2178dd44906aa04564b`; staging/production certificate lifecycle, rotation, and deployment evidence remain gaps.
