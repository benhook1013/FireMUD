...
- Hosted `pr-preview` and `dev-demo-cluster` currently reuse the preview Helm values path, but the live preview value renderer now injects per-render signing-key and JWKS content rather than relying on one static inline shared JWT secret. The remaining gap is lifecycle/rotation proof and broader deployment-contract convergence, not total absence of generated preview JWT/JWKS material.
- The staged player-facing Kustomize path (`k8s/overlays/*` over `k8s/base`) is still under deployment-contract convergence. It remains the intended staging/production Git-tracked path, but it should not yet be treated as a finished doc-first deployment surface because `base/` still includes bootstrap placeholder resources and the overlays are still thin includes rather than environment-complete manifests.
...
- Canonical baseline `NetworkPolicy` manifests for the staged player-facing Kustomize path live in
  ...
  directory documents that baseline policy set. Hosted preview/dev-demo do not
  yet render matching checked-in `NetworkPolicy` resources from the Helm chart;
  that remains an explicit convergence gap rather than an implicit shared
  contract.
...
- **Staging and production** deployments are intended to be applied from a secure operator environment using **Kustomize overlays** (for example `kubectl apply -k k8s/overlays/stage` and `kubectl apply -k k8s/overlays/prod`). Immutable image digest changes for these overlays are the target promotion model, but the checked-in overlay path is still under deployment-contract convergence and should not yet be treated as a finished doc-first deployment surface. See [Deployment Runbook](../system-architecture-deployment-runbook.md) and [CI/CD Pipeline](../system-architecture-cicd.md#promotion--rollback-model).
