...
- Preview Telnet proof exists, but the deployed browser path, shared semantic assertions, head-bound lease handling, isolation proof, and transport-complete acceptance under [ADR 0178](../decisions/adr-0178-disposable-transport-complete-pr-preview-proof.md) remain implementation gaps. Hosted preview and dev-demo Helm now select direct TCP Proxy TLS on the existing NodePort with a dedicated `<release>-telnet-tls` certificate Secret. Run `34013593910` against runtime commit `069db222158059ef0badf2178dd44906aa04564b` authenticated the certificate served by the deployed Telnet endpoint with hostname/SNI validation; it did not prove certificate issuance, controller projection, rotation or migration, or withdrawal. Those certificate-lifecycle concerns and staging/production deployment evidence remain gaps.
