...
- Certificates for TLS termination and mTLS are issued by **cert-manager** and mounted from Kubernetes Secrets. Hosted preview/dev-demo retained identity is materialized and reconciled by the in-cluster controller; cert-manager remains the public issuer and the controller's non-secret Ready evidence does not replace served certificate or player-facing preflight proof.
