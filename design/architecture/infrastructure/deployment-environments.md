...
Local Docker smoke is a disposable test-deployment boundary, not a Coordination Redis reset mechanism. The fresh-bootstrap, restart-state, and image-tag entrypoints require the explicit per-run ID/project binding defined by [Testing: player-flow smoke and reset boundaries](../system-architecture-testing.md#player-flow-smoke-and-reset-boundaries) before Compose access; project-name shape alone is not authorization. Destructive entrypoints additionally require that binding before whole-stack teardown. Their default two-transport flow is the read-only `LOGIN` -> `PLAY` -> `LOOK` baseline; mutating item/container/equipment checks require the standalone client mutation opt-in and validated run-owned boundary. Shared environments remain ineligible for that mutation path until the restricted-synthetic identity/namespace/fence verifier exists.
