...
- Bootstrap and preflight resource ownership is custody-mode-aware. Only `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK` requires the fixed pre-created `jwt-signing-keys` Secret, while `TARGET_NON_EXPORTABLE_SIGNER` requires a signer binding plus post-apply signer health/generation and challenge-signature correspondence evidence and has no private `jwt-signing-keys` Secret or application private-material mount. The checked-in `LEGACY_SECRET_DIAGNOSTIC` hosted Helm path remains diagnostic implementation drift: its candidate-owned `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap may be absent at live pre-apply only when ADR 0152's candidate inventory records `absent-before-create`; candidate inventory plus post-apply validation must exact-match each resource's stable identity, kind, namespace, non-secret/public state (including an authenticated non-secret digest or authenticated owner/controller equality), workload reference, and mount before readiness or traffic-open. Raw Secret data never enters preflight evidence; custody-specific private/public correspondence remains Account-owned evidence under the canonical [JWT and Token Contracts](../system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative), rather than being established by preflight Secret inspection. The current executable does not implement this candidate-resource lifecycle or either accepted custody proof, so the lifecycle gap remains deployment-blocking.
- Player-facing classes (`hobby-self-hosted`, `staging`, `production`) must complete environment bootstrap before first deploy: baseline secrets, certificate issuer bindings, registry pull credentials, and environment-specific external integration credentials must exist and pass preflight before workloads are applied. A JWT/JWKS resource declared as candidate-owned is not a baseline prerequisite: ADR 0152 permits `absent-before-create` only when that exact candidate is listed in the candidate inventory, and post-apply identity/data/reference/mount proof must pass before readiness or traffic-open; baseline JWT/JWKS bindings still must pre-exist.
