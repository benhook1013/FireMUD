...
- Hosted `pr-preview` and `dev-demo-cluster` currently reuse the preview Helm values path. The target requires one environment-unique shared-HMAC signing key and non-secret diagnostic `jwt-jwks` content per deployment event, reused unchanged through render, dry-run, Helm apply, and retries. Current renderers regenerate material per invocation, so retry reuse is not proved. The trusted preview workflow runs `preflight.py hosted-bridge` in `ci-static` context after render and before server dry-run/apply, then runs its `operator` form only after the controller and runtime rollouts have had an opportunity to project and consume the TLS Secrets and before smoke or success publication. The fixed dev-demo workflow runs the `operator` form after render and before server dry-run/apply; its deploy target rejects an image tag that does not equal the reconciled head SHA, and its namespace annotation is written only after deploy and TCP smoke succeed. These checks do not prove live JWKS acceptance or asymmetric non-exportable signer custody. Helm materializes the signing key as the namespace-scoped `jwt-signing-keys` `Secret` and the diagnostic content as the namespace-scoped `jwt-jwks` `ConfigMap`, with the Account workload mounting that ConfigMap. Current validators use the Secret/path contract; the ConfigMap content is not the validation key. This preview diagnostic wiring is not the player-facing target: the sole target is Account-published public JWKS with non-exportable signer custody and validator consumption proven separately. The separate `helm-jwks-contract.sh` proof covers only configured preview resource/path/mount wiring, not live JWKS acceptance or target asymmetric, non-exportable signer custody.
- Preview Telnet proof exists, but the deployed browser path, shared semantic assertions, head-bound lease handling, isolation proof, and transport-complete acceptance under [ADR 0178](../decisions/adr-0178-disposable-transport-complete-pr-preview-proof.md) remain implementation gaps. Hosted preview and dev-demo Helm select hosted-controller mode for retained identity and direct TCP Proxy TLS on the existing NodePort, with cert-manager remaining the public issuer. Run `34013593910` against runtime commit `069db222158059ef0badf2178dd44906aa04564b` authenticated the certificate served by the deployed Telnet endpoint with hostname/SNI validation; it did not prove certificate issuance, controller projection, rotation or migration, or withdrawal. Controller-backed issuance, rotation, served-certificate and consumer-convergence proof, and staging/production deployment evidence remain gaps.
...
Runtime reclaim, redeploy, release, and lease expiry delete only the runtime namespace. Close/merge requests `Retired` only after runtime deletion has returned exact `NotFound`; uncertainty retains identity and fails closed. An explicit `Retired` request while the runtime still exists is an identity-removal command: the controller first scales Gateway and TCP Proxy to zero and waits for observed shutdown so stale in-memory bridge identities cannot survive missing replacements. Controller readiness binds the CR generation, live runtime namespace UID, exact deployed head SHA, rollout and served proof, and non-empty ingress, Telnet, Gateway-internal WebSocket, TCP Proxy bridge, and gRPC revisions. This controller contract is local/static proof only until a live hosted rollout; it does not alter stage or production deployment paths.
