...
- Hosted `pr-preview` and `dev-demo-cluster` currently reuse the preview Helm values path. The target requires one environment-unique shared-HMAC signing key and non-secret diagnostic `jwt-jwks` content per deployment event, reused unchanged through render, dry-run, Helm apply, and retries. Current renderers regenerate material per invocation, so retry reuse is not proved. `preflight.py hosted-bridge` provides separate `ci-static` rendered-contract validation and `operator` validation of controller-projected TLS Secret keys; the trusted preview workflow now invokes `ci-static` before runtime apply and `operator` after identity readiness and rollout, while live hosted acceptance remains unproved. The operator form can therefore fail closed for missing or delayed projection without making a pre-projection Secret read circular. It does not prove live JWKS acceptance or asymmetric non-exportable signer custody. Standalone Helm materializes the namespace-scoped `jwt-signing-keys` `Secret` and diagnostic `jwt-jwks` `ConfigMap`; in hosted-controller preview, the trusted runtime lifecycle provisions both and excludes their PR-rendered copies from the sanitized artifact. Account mounts the diagnostic ConfigMap in either mode. Current validators use the Secret/path contract; the ConfigMap content is not the validation key. This preview diagnostic wiring is not the player-facing target: the sole target is Account-published public JWKS with non-exportable signer custody and validator consumption proven separately. The separate `helm-jwks-contract.sh` proof covers only configured preview resource/path/mount wiring, not live JWKS acceptance or target asymmetric, non-exportable signer custody.
- Hosted preview and dev-demo Helm currently support direct TCP Proxy TLS on the existing NodePort with a dedicated `<release>-telnet-tls` certificate Secret. Current validation covers hostname/SNI authentication of the certificate served by the Telnet endpoint, while certificate issuance, controller projection, rotation or migration, withdrawal, and staging/production deployment evidence remain gaps. The deployed browser path, shared semantic assertions, head-bound lease handling, isolation proof, and transport-complete acceptance under [ADR 0178](../decisions/adr-0178-disposable-transport-complete-pr-preview-proof.md) also remain implementation gaps.
...
- On the explicit `hosted-controller` target path for hosted preview/dev-demo, the in-cluster controller owns the `Certificate` resources for the public ingress, public Telnet, Gateway-internal WebSocket server, and TCP Proxy bridge client leaves, which **cert-manager** issues. The controller separately generates the shared gRPC mTLS bundle from the fixed `firemud-system/firemud-grpc-ca` Secret, then materializes and reconciles all five runtime projections. In the default/current `standalone` path, Helm renders chart-owned `Certificate` resources for the two internal bridge leaves, **cert-manager** issues and reconciles those leaves, and the hosted identity controller remains dormant. Controller non-secret Ready evidence does not replace served-certificate or player-facing preflight proof. This remains local/static target behavior until live controller activation and consumer-convergence proof exists.
...
- TLS watcher health is intentionally split from route readiness. The aggregate `tlsCertificateReloadHealthIndicator` remains on the default health endpoint for alerting and diagnosis: partial or final loss of a registered watcher reports `OUT_OF_SERVICE`. It is not imported wholesale into an explicit readiness group because a service can host watchers for unrelated, background, or feature-degradable surfaces. Each traffic-admission-critical TLS surface feeds its route-specific readiness gate; for TCP Proxy, `trafficAdmissionReadiness` includes the Gateway bridge/readiness path and fails closed for new Telnet admission when that path’s watcher or active client material is unusable. Existing credentials and established sessions may remain usable while their negotiated path remains safe; explicit identity withdrawal remains a controller-owned disruptive termination boundary.
