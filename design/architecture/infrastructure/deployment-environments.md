...
Local Docker smoke is a disposable test-deployment boundary, not a Coordination Redis reset mechanism. The fresh-bootstrap, restart-state, and image-tag entrypoints require a proven run-owned claim/capability for the exact ID-to-project binding before Compose access; project-name shape alone is not authorization. Their default two-transport flow is the read-only `LOGIN` -> `PLAY` -> `LOOK` baseline; mutating item/container/equipment checks require the standalone client mutation opt-in and validated run-owned boundary. Shared environments remain ineligible for that mutation path until the restricted-synthetic identity/namespace/fence verifier exists. See [Testing: player-flow smoke and reset boundaries](../system-architecture-testing.md#player-flow-smoke-and-reset-boundaries) for the owner contract.
