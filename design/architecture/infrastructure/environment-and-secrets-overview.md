...
The trusted publisher for this Git metadata is the authenticated environment/secrets provisioning, rotation, or rebinding controller, using a dedicated publisher identity scoped to one environment’s compliance path and its content-addressed evidence. Git authorship, branch protection, commit timestamps, file paths, and process exit codes are not publisher authority. Before a recovery or release transition, the owner supplies one complete immutable handoff containing the recovery operation identity, operation/event kind and identity, release or promotion identity when applicable, target environment, exact credential-class keys, evidence references and digests, selected freshness field/value and previous field/value, retained material-lineage identity when applicable, exact target binding when applicable, and canonical parent-record digest. A rebinding handoff also carries the exact new target binding. The publisher validates and atomically publishes that exact handoff tuple; a retry is allowed only when every field and digest matches, while a partial, conflicting, or ambiguous owner, publisher, or dereferenced-evidence result fails closed. The owner handoff and dereferenced evidence are the sole pre-release authority; the Git record is exported only after finalization and is never read back to authorize the same recovery or release.
