...
The P0 owner contracts for this overview are [event-scoped Tier A credential compliance](../decisions/adr-0151-event-scoped-automated-tier-a-credential-compliance.md), [phased environment-bound preflight and expected bindings](../decisions/adr-0152-phased-environment-bound-deployment-preflight-and-expected-bindings.md), and [event-classified post-restore trust reset](../decisions/adr-0155-automated-event-classified-post-restore-trust-reset.md). This document owns environment-specific resource, binding, and compliance consequences; the linked decisions own the cross-cutting gates.
...
- The current hosted `pr-preview` diagnostic contract binds one preview-unique shared-HMAC signing key and its non-secret diagnostic `jwt-jwks` content to each deployment event, reusing the exact rendered values/artifact unchanged through render, dry-run, Helm apply, and retries; new signing material is permitted only through an explicit rotation or an explicitly new deployment event, including an intentional clean namespace reset. The current preview and dev-demo renderers regenerate material per invocation, so the current workflow does not yet provide that retry-stable reuse. Helm materializes the signing key as the namespace-scoped `jwt-signing-keys` Secret and the diagnostic content as the namespace-scoped `jwt-jwks` ConfigMap, with Account mounting that ConfigMap at `FIREMUD_AUTH_JWKS_PATH`. The ConfigMap content is not the shared-HMAC validation key and cannot validate preview tokens; current preview validators use the Secret/path contract. This diagnostic wiring is current implementation drift, not the target hosted-preview posture. The target is Account-published public JWKS with non-exportable signer custody, consumed by Account and every validator without private signing material reaching application workloads. The preview workflow's Helm render and Kubernetes server dry-run, plus the separate `helm-jwks-contract.sh` resource/mount check, do not prove runtime JWKS acceptance.
- `dev-tools/deploy/preflight.py` consumes player-facing expected-binding manifests under `design/operations/environments/`, emits `expectedBindingsRef`, and validates the first required binding fields and policy IDs. It is not a `pr-preview` workflow gate.
...
The current hosted `pr-preview` path uses the renderer and Helm resource contract described above: `jwt-signing-keys` is a namespace-scoped `Secret`, public `jwt-jwks` is a namespace-scoped `ConfigMap`, and Account mounts the ConfigMap. This shared-HMAC signing and diagnostic ConfigMap wiring is current implementation drift, not the target hosted-preview posture. The separate `helm-jwks-contract.sh` proof checks that preview resource and Account mount; it is concrete diagnostic wiring evidence only, not proof of live JWKS acceptance by every validator or player-facing JWT readiness. The target JWT authority, validator convergence, and signer-custody contract is Account-published public JWKS with non-exportable signer custody, consumed by Account and every validator without private signing material reaching application workloads, as defined canonically in [JWT and Token Contracts](../system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative); this document retains only environment resource and mount consequences. Checked-in player-facing Kustomize fixtures remain on legacy Secret-backed signing with a public `jwt-jwks` Secret resource and advisory `PREFLIGHT-JWKS-001` Secret/path/mount diagnostics; environment-owned resources still supply the external bindings.

The expected-binding selector is separate from the hosted preview resource shape. Player-facing manifests currently record exactly one of `LEGACY_SECRET_DIAGNOSTIC`, `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK`, or `TARGET_NON_EXPORTABLE_SIGNER`; `LEGACY_SECRET_DIAGNOSTIC` is only the current diagnostic/drift classification and never authorizes player-facing traffic. The current `preflight.py` constant `IMPLEMENTED_JWT_CUSTODY_MODE` is `LEGACY_SECRET_DIAGNOSTIC`; its required `FIREMUD_AUTH_JWT_SECRET_PATH` and shared-HMAC Secret checks are diagnostic only and never satisfy player-facing readiness. The interim mode requires Account-only private-bundle proof plus public-JWKS consumption by Account and every validator, while target mode requires the private path to be absent and no application private material. Only the separately authenticated interim or target custody proof can authorize player-facing traffic; neither current legacy checks nor the preview ConfigMap/mount check claims that authority.
...
| -------- | ------- | ----- |
| `SPRING_PROFILES_ACTIVE` | Spring profile selector | Docker Compose local development sets `dev`; direct local execution may leave it unset when supported; Kubernetes deployments set `prod`; automated tests set `test`. Do not use ad hoc profiles. |
...
- Player‑facing environments must use **distinct Redis processes and endpoints** for coordination and cache/rate-limit roles.
- Only an explicitly labelled one-shot test/CI stack may collapse roles into a single Redis instance, and it must not be used to validate coordination isolation, replay, or SLOs. Hosted `pr-preview` is not part of this exception and keeps the normal role split; see [Redis usage and profiles](../system-architecture-redis-usage-and-profiles.md#environment-mappings).
- `pr-preview` keeps the normal Redis role split and preview-unique trust material, but it uses a preview-scoped rendered-Helm validation posture rather than the full player-facing preflight and backup/admission binding contract used for staging and production traffic-open decisions.

Operational notes:

- Coordination Redis hosts gameplay session bindings (`session:game:*`). The target contract assigns Account-owned issued-token registry and revocation/version projections (`session:auth:token:*` and `session:auth:generation:*`) to Coordination Redis, but the current runtime still uses legacy Account session records (`session:auth:account:*` and `session:auth:tenant:*`). Redis configuration alone neither proves target registry/revocation enforcement nor authorizes protected traffic; the Account-owned contract and its runtime proof remain authoritative.

### Authentication & JWT

The resource and startup requirements in this subsection are the accepted target-mode contract. Current hosted preview wiring is described under [Current and Target JWT/JWKS Resource Modes](#current-and-target-jwtjwks-resource-modes); its hosted Helm/resource validation is limited to concrete ConfigMap/resource/mount wiring and does not prove these target requirements.

| Variable | Purpose | Rotation / Safety Notes |
| -------- | ------- | ----------------------- |
| `FIREMUD_AUTH_JWT_SECRET_PATH` | Legacy runtime shared-HMAC Secret/path input (current diagnostic/drift mode); reserved interim Account-only active-key path | In the current legacy runtime, a configured workload reads this file and uses its shared-HMAC bytes for JWT signing/validation; the corresponding preflight result is diagnostic only and cannot authorize player-facing traffic. In `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK`, only Account Service mounts `jwt-signing-keys` read-only at `/var/run/secrets/firemud/jwt` and reads the active bundle at `/var/run/secrets/firemud/jwt/current.key` through this variable; validators use only the public `FIREMUD_AUTH_JWKS_PATH`. Target non-exportable signer custody leaves this variable unset. |
...
| `FIREMUD_AUTH_JWT_EXPIRATION_MS` | Lifetime of newly issued `control-ui` and receiver-specific private player-delegation JWTs (target profile scope) | It does not control `player-bootstrap` or `gameplay-connect`; changing it changes only the `exp` claim on newly issued tokens. |
| `FIREMUD_ACCOUNT_TOKENS_SESSION_EXPIRATION_MS` | Current legacy Account authentication-session record TTL | Production default is `3600000` ms; it is separate from target gameplay continuity and issued-token-registry retention. |
| `FIREMUD_ACCOUNT_TOKENS_PLAYER_BOOTSTRAP_EXPIRATION_MS` | `player-bootstrap` JWT lifetime | Target maximum is `300000` ms (five minutes); values at or below zero or above the ceiling must fail startup/preflight. |
| `FIREMUD_ACCOUNT_TOKENS_CONNECT_SCOPE_EXPIRATION_MS` | Current Account discovery `connectScopeId` selector lifetime | Production default is `120000` ms and current Account surfaces it as `connectScopeExpiresAt`; target scope proof remains short-lived and expiry-bound. |
| `FIREMUD_ACCOUNT_TOKENS_CONNECT_TOKEN_EXPIRATION_MS` | `gameplay-connect` JWT lifetime | Target maximum is `30000` ms; values at or below zero or above the ceiling must fail startup/preflight. |
...
- Normal local Compose development seeds and uses JWT secret material from the ignored repository-root `.env` (normally copied from `.env.sample`) and passes it to services; random JWT key generation is available only under the `test` profile.
...
- TLS certificates are issued by **cert-manager** and rotated automatically; the canonical target gives each workload a distinct leaf certificate/private-key Secret. Current hosted manifests give five publication services distinct leaf Secrets and project only `ca.crt` from the shared `firemud-grpc-tls` trust bundle into those Pods. The other gRPC workloads still use the shared leaf identity, so hosted rendering remains incomplete evidence for the full target identity boundary and is not player-facing equivalence. Services reload updated certificates using `TlsCertificateWatcher` / `GrpcServerTlsReloader`.
...
| Environment class | JWT resource posture | Redis role posture | Local readiness consequence |
| --- | --- | --- | --- |
| `local-dev` | Normal Compose uses JWT material from the ignored repository-root `.env`; random JWT key generation is restricted to the explicit `test` profile, and an explicit local/test profile may use the packaged classpath fixture. | Uses distinct Coordination and Cache/Rate-Limit Redis processes and endpoints; local development is non-ephemeral for role separation. | Cross-service token validity need not survive restarts unless operators configure persistent material. |
| `dev-demo-cluster` | Use environment-unique test material; this non-player-facing class may use convenience provisioning and must not share trust material across environments. | Uses distinct Redis roles unless the stack is explicitly labelled as a one-shot ephemeral test/CI topology; only that labelled exception may collapse roles and it forfeits isolation, replay, and SLO evidence. | It is not promotion, rollback, or DR-readiness evidence. |
| `pr-preview` | **Target:** Account-published public `jwt-jwks` with non-exportable signer custody; Account and every validator consume the read-only public projection, and no application workload receives private signing material. **Current diagnostic drift:** one preview-unique shared-HMAC signing key and non-secret diagnostic `jwt-jwks` content per deployment event, reused unchanged across render, dry-run, Helm apply, and retries; explicit rotation or an explicitly new deployment event may replace it. Renderers currently regenerate per invocation, so retry-stable behavior remains unproved. Helm materializes `jwt-signing-keys` as a namespace-scoped Secret and the diagnostic `jwt-jwks` as a namespace-scoped ConfigMap; Account mounts the ConfigMap at `FIREMUD_AUTH_JWKS_PATH`, but current validators use the Secret/path rather than that ConfigMap. | Uses the normal distinct Redis role processes and endpoints; hosted `pr-preview` is not part of the one-shot ephemeral exception. | Use the preview-scoped Helm render/server-dry-run validation; `helm-jwks-contract.sh` proves only the diagnostic ConfigMap and Account mount wiring, not live validator acceptance. Player-facing backup/admission evidence is not required. |
| `hobby-self-hosted` | Target: non-exportable signer custody; no application workload mounts or receives private signing material, and Account-owned public `jwt-jwks` is delivered to Account and every validator. Interim drift: Account-only `jwt-signing-keys` private mount plus the mounted public projection. | Player-facing: Coordination and Cache/Rate-Limit Redis must use distinct processes and endpoints. | Target requires signer health, no-private-mount proof, and public-JWKS convergence; interim drift additionally requires private/public projection proof. Common binding, credential-age, and applicable backup evidence are required. |
| `staging` | Target: non-exportable signer custody; no application workload mounts or receives private signing material, and Account-owned public `jwt-jwks` is delivered to Account and every validator. Interim drift: Account-only `jwt-signing-keys` private mount plus the mounted public projection. | Player-facing: Coordination and Cache/Rate-Limit Redis must use distinct processes and endpoints. | Target requires signer health, no-private-mount proof, and public-JWKS convergence; interim drift additionally requires private/public projection proof. Player-facing preflight and promotion evidence apply. |
| `production` | Target: non-exportable signer custody; no application workload mounts or receives private signing material, and Account-owned public `jwt-jwks` is delivered to Account and every validator. Interim drift: Account-only `jwt-signing-keys` private mount plus the mounted public projection. | Player-facing: Coordination and Cache/Rate-Limit Redis must use distinct processes and endpoints. | Target requires signer health, no-private-mount proof, and public-JWKS convergence; interim drift additionally requires private/public projection proof. Strict player-facing preflight, promotion, and backup evidence apply. |
...
The pre-release authority is a dedicated authenticated handoff store/API owned by the environment/secrets provisioning, rotation, or rebinding controller. Its publisher uses an identity scoped to one environment’s compliance path and content-addressed evidence. Before a recovery or release transition, the owner supplies one complete immutable handoff containing the target environment, applicable event kind and identity, exact credential-class keys, evidence references and digests, selected freshness field/value and previous field/value, retained material-lineage identity when applicable, exact target binding when applicable, and canonical parent-record digest. Recovery handoffs use the recovery controller's existing canonical operation identity: `operationId` for the general recovery workflow, or the branch-specific `restoreOperationId` for the ADR 0155 handoff below. That identity binds only the matching recovery event and is absent from ordinary release, promotion, and other transition handoffs. Ordinary release and transition identity uses the existing event-scoped tuple `{deploymentRef, deploymentEventId, trafficOpenEvent}` from the preflight contract. A production promotion additionally retains the existing promotion lineage `{stagingOverlayCommitSha, stagingDeploymentEventId, productionOverlayRef}`; these canonical identifiers are not replaced by a fabricated generic release or promotion operation ID. A rebinding handoff also carries the exact new target binding. The handoff publisher validates and atomically publishes that exact tuple; a retry is allowed only when every field and digest matches, while a partial, conflicting, or ambiguous owner, publisher, or dereferenced-evidence result fails closed. The owner handoff and dereferenced evidence are the sole pre-release authority. A separate authenticated Git exporter projects the finalized record for retention and reporting; Git authorship, branch protection, commit timestamps, file paths, and process exit codes are not publisher authority, and the Git projection is never read back to authorize the same recovery or release.

For the ADR 0155 same-boundary PostgreSQL-only rewind branch, the credential event kind for `verified_not_restored` is exactly `recovery-trust-verification`. It is a `secretComplianceRefresh` handoff event, not a second recovery owner or release state; the recovery controller and the detailed [post-restore hardening handoff](../system-architecture-post-restore-hardening.md#5-secret-compliance-evidence-refresh) remain authoritative. Its immutable per-class tuple is exactly `{restoreOperationId, credentialEventKind, credentialOperationId, targetEnvironment, credentialClass, disposition, freshness, materialLineageIdentity, targetBinding, restoreArtifact, evidenceRef, evidenceKey, immutableArtifactId, parentRecordDigest, jwtEvidence}`; `jwtEvidence` is present only for `jwt-signing-keys-jwks`. Here `freshness` is exactly `{lineage, field, value, previousField, previousValue}` and, for this disposition, retains the existing `lineage=existing` and exact `lastProvisionedAt` or `lastRotationAt` field/value pair; `materialLineageIdentity` is the retained current-material lineage; `targetBinding` is the exact environment-owned binding identity declared by expected bindings (`bindingRef` or `fingerprint`, using that manifest’s canonical form); `restoreArtifact` is exactly `{artifactRef, artifactDigest, artifactLineage}` for the selected PostgreSQL restore artifact; and `evidenceRef` plus `evidenceKey` dereference the evidence object whose `immutableArtifactId` is the complete `sha256:<64 lowercase hex>` digest of its UTF-8 RFC 8785 canonical bytes with only that `immutableArtifactId` member omitted from the preimage. `parentRecordDigest` is the same canonical digest relationship applied to the parent compliance record. The evidence must prove the current material and binding were outside the restore artifact, with the artifact identity, digest, and lineage exact-matching the recovery controller’s selected restore artifact; no path, timestamp, mutable reference, or Git projection substitutes for this tuple.

For `jwt-signing-keys-jwks`, `jwtEvidence` is exactly `{accountSignerGeneration, accountJwksGeneration, currentKeyset, validatorAcceptance}`: it records the current Account signer generation, Account-published JWKS generation, intact current keyset/`kid` set, and one validator-convergence result per authoritative validator proving acceptance of that unchanged current set. The outer immutable per-class tuple carries the exact environment-owned `targetBinding`; `targetBinding` is not a member of `jwtEvidence` and must not be duplicated inside that exact schema. The Account authority/delegation and Game Session invalidations remain universal rewind controls. The recovery controller accepts `verified_not_restored` only when it atomically compare-and-set exact-matches the complete tuple, dereferenced evidence digest, restore-artifact lineage, retained freshness pair, and JWT evidence where applicable; missing, extra, stale, conflicting, or ambiguous fields fail closed. This is target-state validation: the current static compliance validator remains a post-publication retention check and cannot authorize the current recovery from a Git record alone.

The live recovery controller consumes the owner handoff and dereferenced evidence as pre-release input. The Git record and supporting evidence under `design/operations/secret-compliance/` are exported only as a post-finalization projection, are never read back as authority for the release that produced them, and cannot be used circularly. The existing post-publication validator checks retained projection age, references, and shape for future readiness events; target-state validation additionally binds the owner handoff and canonical digests. Git drift blocks the next applicable promotion, first-live/reopen, or disaster-recovery-readiness claim but cannot turn a failed or incomplete current handoff into a pass.
...
- **Secrets**:
  ...
  - Normal local Compose JWT material comes from the ignored repository-root `.env`; random JWT key generation is restricted to the `test` profile. Other non-JWT throwaway credentials may be generated or randomized automatically for local development.
...
Internal service-to-service **gRPC** traffic uses mutual TLS in shared and player-facing Kubernetes environments. Gateway-to-backend HTTP/WebSocket hops remain explicit in-cluster exceptions with their existing Gateway trust, header-canonicalization, and NetworkPolicy controls; see [Security Architecture](../system-architecture-security.md#cross-service-trust). Explicit local-development and throwaway-test profiles may use the documented plaintext internal transports and do not provide player-facing or promotion evidence. The bounded Spring gRPC `1.0.x` plaintext `pr-preview` migration exception documented in [gRPC TLS requirements](../system-architecture-grpc.md#tls-requirements) is reserved policy, not the current hosted configuration. In hosted `pr-preview` and `dev-demo-cluster`, trusted certificate workflows request distinct cert-manager-issued leaves for the five publication services. The checked-in `k8s/base/` Pod manifests only bind and mount the corresponding `firemud-grpc-<workload>` Secrets; they do not issue those certificates. Live leaf issuance and consumer readiness must be proven before hosted identity activation. Staging and production operators must provision the five namespace-scoped publication Secrets and the shared `firemud-grpc-tls` trust bundle through the environment bootstrap process in the [deployment runbook](../system-architecture-deployment-runbook.md#environment-bootstrap). The remaining gRPC workloads continue to use the legacy shared leaf. Each publication Pod reads peer trust from only the public `ca.crt` key projected from `firemud-grpc-tls`; its private identity remains the service-specific `tls.crt` and `tls.key`. The shared CA bundle contains the legacy and current issuer trust anchors so these publication workloads can verify peers using either leaf generation. Hosted rendering remains non-player-facing/non-promotion evidence. The canonical target provisions distinct per-workload leaf certificate/private-key Secrets so each service has a concrete private identity. These certificates secure:
