...
- TLS certificates are issued by **cert-manager** and rotated automatically; the canonical target gives each workload a distinct leaf certificate/private-key Secret. Current hosted Helm manifests for `pr-preview` and `dev-demo-cluster` instead use the shared `firemud-grpc-tls` Secret across gRPC workloads, which is evidence of encrypted transport only and not distinct workload leaf identity or player-facing equivalence. Services reload updated certificates using `TlsCertificateWatcher` / `GrpcServerTlsReloader`.
...
Internal service-to-service **gRPC** traffic uses mutual TLS in shared and player-facing Kubernetes environments. Gateway-to-backend HTTP/WebSocket hops remain explicit in-cluster exceptions with their existing Gateway trust, header-canonicalization, and NetworkPolicy controls; see [Security Architecture](../system-architecture-security.md#cross-service-trust). Explicit local-development and throwaway-test profiles may use the documented plaintext internal transports and do not provide player-facing or promotion evidence. The bounded Spring gRPC `1.0.x` plaintext `pr-preview` migration exception documented in [gRPC TLS requirements](../system-architecture-grpc.md#tls-requirements) is reserved policy, not the current hosted configuration. Hosted `pr-preview` currently uses the shared `firemud-grpc-tls` Secret for mTLS and remains non-player-facing/non-promotion evidence; that shared identity proves encrypted transport, not distinct workload identity. The canonical target provisions certificates through **cert-manager** and mounts distinct per-workload leaf certificate/private-key Secrets so each service has a concrete private identity. Current `dev-demo-cluster` manifests also use the shared `firemud-grpc-tls` Secret across gRPC workloads and therefore do not prove the target identity boundary. These certificates secure:
