...
| Variable | Purpose | Rotation / Safety Notes |
| -------- | ------- | ----------------------- |
| `FIREMUD_AUTH_JWT_SECRET_PATH` | Interim asymmetric-fallback path to the versioned private signing bundle | In player-facing environments, mount `jwt-signing-keys` read-only only into Account Service; the canonical mount is `/var/run/secrets/firemud/jwt` and the active bundle is `/var/run/secrets/firemud/jwt/current.key`. The final target delegates private-key operations to a non-exportable signer. |
...
- Sensitive values (database passwords, TLS private keys, and interim JWT signing bundles) are delivered through Kubernetes `Secret` objects. The Secret-mounted JWT bundle and Secret/ConfigMap public projection are an interim asymmetric fallback; the final target uses a non-exportable delegated signer for private-key operations.
