...
The trusted publisher for this Git metadata is the authenticated environment/secrets provisioning, rotation, or rebinding controller, using a dedicated publisher identity scoped to one environment’s compliance path and its content-addressed evidence. Git authorship, branch protection, commit timestamps, file paths, and process exit codes are not publisher authority. Before a recovery or release transition, the owner supplies one complete immutable handoff containing the operation/event kind and identity, target environment, credential-class keys, evidence references and digests, selected freshness field/value, and canonical parent-record digest. A rebinding handoff also carries the retained material-lineage identity and exact new target binding. The publisher validates and atomically publishes that exact operation/digest tuple; a retry is allowed only when the complete tuple matches, while a partial, conflicting, or ambiguous handoff fails closed.
