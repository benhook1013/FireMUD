...
| --- | --- | --- | --- |
| `local-dev` | `.env` and throwaway generated keys are allowed; an explicit local/test profile may use the packaged classpath fixture. | Uses distinct Coordination and Cache/Rate-Limit Redis processes and endpoints; local development is non-ephemeral for role separation. | Cross-service token validity need not survive restarts unless operators configure persistent material. |
