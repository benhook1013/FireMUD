...
- JWT signing keys are stored in a fixed Secret and public JWKS in a fixed ConfigMap. Account Service owns lifecycle requests, validation, promotion, JWKS publication, and public/private pruning. A non-exportable signer performs only delegated private-key operations; in the controlled Secret fallback, the materialization controller is the sole signing-Secret writer through name-scoped `resourceVersion` CAS while Account reads that Secret and alone updates public JWKS. Dedicated Kubernetes Jobs request Account-owned transitions, prove validator visibility, observe the Account-published key set, and record evidence; they never read or update `jwt-signing-keys` or write `jwt-jwks`. See `system-architecture-security.md#jwt-key--jwks-rotation-workflow` for details.
...
- **Staging / Non-production clusters**
  - Player-facing staging must use fixed, pre-created `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap resources. The materialization controller alone CAS-updates the signing Secret after an Account-authorized lifecycle request; Account CAS-publishes the public JWKS and consumes both read-only projected mounts. `FIREMUD_AUTH_JWT_SECRET_PATH` selects the private bundle and `FIREMUD_AUTH_JWKS_PATH` selects `/var/run/secrets/firemud/jwks/jwks.json`.
...
- **Production**
  - Required to use fixed, pre-created `jwt-signing-keys` Secret and public `jwt-jwks` ConfigMap resources. The materialization controller alone CAS-updates the signing Secret after an Account-authorized lifecycle request; Account CAS-publishes `jwt-jwks`, mounts both resources read-only, and sets `FIREMUD_AUTH_JWKS_PATH` to `/var/run/secrets/firemud/jwks/jwks.json`. JWKS is the canonical trust source for all validating services.
