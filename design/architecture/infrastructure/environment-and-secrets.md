...
The packaged classpath JWKS fallback is permitted only in local/test environments. Hosted `pr-preview` environments require preview-unique, pre-created JWT signing-key and JWKS resources in each preview namespace; shared preview trust material is not allowed. For player-facing environments (`hobby-self-hosted`, staging, production), the materialization controller alone updates the fixed `jwt-signing-keys` Secret after an Account-authorized lifecycle request, while Account alone publishes the fixed `jwt-jwks` ConfigMap; both use name-scoped Kubernetes `resourceVersion` CAS. Account consumes both through read-only projected mounts selected by `FIREMUD_AUTH_JWT_SECRET_PATH` and `FIREMUD_AUTH_JWKS_PATH=/var/run/secrets/firemud/jwks/jwks.json`; inline-only JWT secrets and classpath JWKS fallback are not allowed. Account startup must fail when either path or file is missing or unreadable, the JWKS is malformed, or its public JWK does not match the Account signing key and `kid`.
