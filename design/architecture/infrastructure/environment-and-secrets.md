...
The checked-in player-facing Kustomize fixtures and runtime remain on the legacy Secret-backed signing path, with shared-HMAC and classpath-fallback drift still documented as implementation gaps; their public JWKS resource is the fixed-name `jwt-jwks` ConfigMap. The executable `PREFLIGHT-JWKS-001` check requires the explicit public `configmap://firemud/jwt-jwks` binding, non-empty `data.jwks.json` content, and an Account mount at the configured JWKS path. This is resource/path wiring evidence only; it does not establish runtime JWKS acceptance, validator convergence, signer custody, or player-facing JWT readiness.
