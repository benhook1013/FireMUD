...
This value defines the absolute storage and authentication-validity ceiling for a gameplay session binding. The Game Session Service uses the derived value both for its in-memory/session bookkeeping and as the TTL for `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` keys in Redis (see `../system-architecture-redis.md#session-keys-and-gameplay-binding`). It is not the player-facing disconnected-resume eligibility window.
