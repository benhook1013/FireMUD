...
In player-facing environments (`hobby-self-hosted`, staging, production), the Secret-mounted private bundle and Secret/ConfigMap public projection are the interim Account-only mounted fallback, not target non-exportable signer mode. The fixed, pre-created `jwt-signing-keys` Secret is exposed only to Account Service through the read-only `FIREMUD_AUTH_JWT_SECRET_PATH` mount (canonical bundle `/var/run/secrets/firemud/jwt/current.key`). The current checked-in public resource is a fixed `jwt-jwks` Secret mounted into Account at `/var/run/secrets/firemud/jwks` and selected by `FIREMUD_AUTH_JWKS_PATH`; target environments use a fixed `jwt-jwks` ConfigMap delivered read-only at `/var/run/secrets/firemud/jwks/jwks.json` through `FIREMUD_AUTH_JWKS_PATH` to Account Service and every JWT validator. No checked-in automation currently proves the target JWT lifecycle or validator convergence.
...
In the target player-facing contract (`hobby-self-hosted`, staging, production), `FIREMUD_AUTH_JWT_SECRET_PATH` is required for Account Service only in the interim mounted fallback, while `FIREMUD_AUTH_JWKS_PATH` is required for Account Service and every JWT validator. Account startup must fail closed when either Account path is unset, missing, or unreadable; when the private signing bundle is incomplete, malformed, or non-asymmetric; or when the JWKS is malformed or does not contain a public JWK matching the Account signing key and `kid`. Every validator must fail closed when its public JWKS path is unset, missing, unreadable, malformed, or unavailable for asymmetric verification; private signing material must not be mounted or delivered to an application workload outside the approved signer boundary. There is no classpath fallback in player-facing profiles, and Account must also fail on inline-only or HMAC-only signing configuration. The current preflight does not yet enforce this validator-specific JWKS path boundary.
