...
| `FIREMUD_AUTH_JWT_SECRET_PATH` | Path for the interim Account-only mounted asymmetric fallback bundle in player-facing environments; it is not used by target non-exportable signer mode. Current code still treats this as one HMAC secret. | *(none)* |
