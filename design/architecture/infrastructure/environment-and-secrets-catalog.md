...
The canonical JWT profile, registry, authority-generation, signing, publication, convergence, and rotation contract is [JWT and Token Contracts](../system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative). This catalog owns environment variables, filesystem mounts, Kubernetes resource modes, and their current implementation drift. Certificate and secret watching is described in [Security](../system-architecture-security.md#key-and-certificate-rotation).

Target state: a non-exportable delegated signer performs private-key operations for Account Service, Account publishes the matching JWKS, and Account plus every JWT validator receive the public JWKS through their own read-only `FIREMUD_AUTH_JWKS_PATH` delivery. The interim asymmetric fallback uses a file-mounted signing bundle in Account. The packaged classpath JWKS fallback is permitted only in explicit local/test profiles. Player-facing Account and validator startup/readiness fails closed when the path or file is unset, missing, unreadable, malformed, or cannot support the required asymmetric `kid`; validators never receive private signing material. The current checked-in environment manifests and preflight still use the Secret-backed JWKS resource contract; the target ConfigMap publication mode and validator-specific delivery are distinct and must be explicitly selected and proved by target preflight.
...
| `FIREMUD_AUTH_JWT_SECRET_PATH` | Interim asymmetric-fallback path to a versioned JWT signing bundle for Account Service in player-facing environments; a watcher may detect changes, but Account promotes only a complete bundle whose key and `kid` match a converged published JWK. The final target delegates private-key operations to a non-exportable signer. Current code still treats this as one HMAC secret. | *(none)* |
...
In player-facing environments (`hobby-self-hosted`, staging, production), the Secret-mounted private bundle and Secret/ConfigMap public projection are an interim asymmetric fallback; the final target delegates private-key operations to a non-exportable signer. The fixed, pre-created `jwt-signing-keys` Secret is exposed only to Account Service through the read-only `FIREMUD_AUTH_JWT_SECRET_PATH` mount (canonical bundle `/var/run/secrets/firemud/jwt/current.key`). The materialization controller is the Secret's sole API reader/writer and uses name-scoped RBAC plus `resourceVersion` CAS. Because Kubernetes Secret `get` returns Secret data, this is interim private-material custody: the controller may use the data only for the authenticated materialization/CAS/pruning operation and must not sign, select or promote a signer, publish JWKS, log, expose, or export private material. Account has no signing-Secret API authority. The current checked-in public resource is a fixed `jwt-jwks` Secret mounted into Account at `/var/run/secrets/firemud/jwks` and selected by `FIREMUD_AUTH_JWKS_PATH`; target publication may replace it with a fixed Account-published `jwt-jwks` ConfigMap delivered read-only to every validator. Neither actor receives list/create/delete authority for these pre-created resources. The materialization controller and observation-only rotation Job remain confined to their owner-defined custody and evidence roles; the controller's Secret-data read is custody-only, and neither may become an alternate issuer, signer, or exporter of private material. No checked-in rotation Job currently implements the target protocol.

Initial target-mode publication preserves a quarantined Account bootstrap exception: Account alone may wait for the controller-materialized private bundle, derive or receive the operation-bound public JWK, CAS-populate the pre-created `jwt-jwks` ConfigMap, and verify the resulting private/public projections. Account readiness, token issuance, validation-dependent protected traffic, and validator readiness remain closed until that proof succeeds. No controller, Job, or external bootstrap process receives a one-time JWKS publication exception.
