...
In player-facing environments (`hobby-self-hosted`, staging, production), JWT signing keys are stored in a fixed, pre-created `jwt-signing-keys` Secret and exposed only to Account Service via the read-only mount path referenced by `FIREMUD_AUTH_JWT_SECRET_PATH` (the canonical mount is `/var/run/secrets/firemud/jwt`, with the active bundle at `/var/run/secrets/firemud/jwt/current.key`). The materialization controller is the Secret's sole API reader/writer and uses name-scoped Kubernetes RBAC plus `resourceVersion` CAS after an Account-authorized lifecycle request; Account has no signing-Secret API authority and consumes only the projected files. The current checked-in JWKS contract uses a fixed, pre-created `jwt-jwks` Secret, mounted read-only into Account Service at `/var/run/secrets/firemud/jwks`, and selected by `FIREMUD_AUTH_JWKS_PATH` (normally `/var/run/secrets/firemud/jwks/jwks.json`). The target publication mode may replace that public resource with a fixed `jwt-jwks` ConfigMap, which Account alone publishes through name-scoped RBAC and `resourceVersion` CAS; that target mode is not an alternate current signer authority. Neither actor has list/create/delete authority for the pre-created resources. The JWKS endpoint is the only Account-key material validators consume. Account Service owns signing-key lifecycle requests, issuer transitions, validation, promotion, session invalidation, JWKS publication, and public/private pruning; the materialization controller performs only private-signing-Secret generation and CAS writes, and does not issue, promote, publish, or revoke. The separate non-exportable signer custody option accepted in ADR 0014 is a future private-key backend, not a second Secret writer or issuer authority in this catalog. Rotation automation, including the `jwt-rotation` Job or CronJob, only observes Account-owned transitions, publication, and validator convergence and records retained evidence; it must never initiate a lifecycle transition, read or update `jwt-signing-keys`, or write `jwt-jwks`. The phased protocol, private-key mount enforcement, and retained rotation proof described in `../system-architecture-security.md#jwt-key--jwks-rotation-workflow` remain target-state follow-through; no checked-in rotation Job currently implements them.

Initial target-mode publication uses the same ownership: Account runs only in a quarantined bootstrap state, waits for the controller-materialized private bundle, derives or receives the operation-bound public JWK, CAS-populates the pre-created `jwt-jwks` ConfigMap, and verifies the resulting private/public projections before becoming ready. Issuance, validation-dependent protected traffic, and validator readiness remain closed until that proof succeeds. No controller, Job, or external bootstrap process receives a one-time JWKS publication exception.
