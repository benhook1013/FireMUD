...
| `FIREMUD_AUTH_JWT_SECRET_PATH` | Path for the interim Account-only mounted asymmetric fallback bundle in player-facing environments; private-file validation applies only in this mode, and target non-exportable signer mode leaves it unset. Current code still treats this as one HMAC secret. | *(none)* |
...
The target retention bound is absolute for the complete scope-bound persisted context envelope, including scope identity, ordering, output, structured payload, rendering/replay metadata, and derived compatibility text—not only rendered text. Message/line floors and the soft ceiling are subordinate best-effort preferences. An entry that cannot fit must be omitted or represented by a complete bounded marker; no partial or silently truncated entry is permitted. Current Game Session runtime behavior retains a single entry above `hardMaxBytes`, and its current key/envelope accounting uses `gameInstanceId` rather than the canonical `playableStateNamespaceId` scope; complete schema-envelope accounting, namespace migration, and oversized-entry omission/marker enforcement remain implementation/proof gaps.
...
Current checked-in hosted `pr-preview` manifests and `dev-tools/deploy/preflight.py` still use a preview-unique, pre-created signing-key `Secret` and `jwt-jwks` `Secret`, and the preflight rejects the required ConfigMap. This is legacy wiring drift, not an accepted JWKS mode or proof that every validator receives or can fail closed on Account JWKS.
