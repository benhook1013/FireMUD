...
This value defines the maximum window during which a disconnected gameplay session can be resumed. The Game Session Service uses the derived value both for its in-memory/session bookkeeping and as the TTL for `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` keys in Redis (see `../system-architecture-redis.md#session-keys-and-gameplay-binding`). After this TTL elapses, reconnect attempts for that session are treated as expired and require a fresh `LOGIN`.
