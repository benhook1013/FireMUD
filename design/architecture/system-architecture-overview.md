...
- **Bounded durable semantic recent context** is owned by Game Session. Its persisted semantic context identity is `{tenantId, playableStateNamespaceId, characterId}` and is rendered after reconnect under the current session and policy; ordered PostgreSQL rows are the source of truth and Redis is only a hot cache. It is separate from the derived rendered room cache, whose identity is the exact viewer/session plus an immutable viewer-policy/presentation binding and which remains presentation-only. See [ADR 0134](./decisions/adr-0134-bounded-durable-semantic-reconnect-context.md).
