...
- **Coordination and session keys (Coordination Redis)**  
  ...
  - Canonical prefixes include (non-exhaustive):  
    ...
    - `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>` – Account-owned exact-input connect-token issuance-result/idempotency records; current values are short-lived result caches, while target recovery is durable/fenced exact-result recovery or fail-closed refusal. This family is distinct from the four Account auth-session families and Gateway replay/deny keys.
...
| Account/session token control metadata | Account Service | Account PostgreSQL plus Account-owned Coordination Redis prefixes `session:auth:account:*`, `session:auth:tenant:*`, `session:auth:token:*`, and `session:auth:generation:*`; the separate exact-input connect-token issuance-result family `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>` is also Account-owned |
