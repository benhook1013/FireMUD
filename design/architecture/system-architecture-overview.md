...
- `POST /api/admin/tick-remediation/pause` is not bypass-safe and must enter through Logging & Admin.
...
| `POST /api/admin/admission-pointers/{worldSlug}/{realmSlug}:set` | External operator write | Not allowed as a direct Game Session route | Admission changes enter through Logging & Admin; Game Session validates and commits the fenced pointer mutation |
