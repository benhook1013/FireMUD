...
- `POST /api/design/assets` and `POST /api/design/templates` are bypass-safe creator writes when implemented by Game Design as tenant-scoped domain-local asset/template workflows with Game Design-owned validation and audit behavior. They bypass Logging & Admin only: for covered official-hosted operations, Game Design's fail-closed mutation gate must obtain current Account hosted-terms/currentness authority and evidence before any side effect and record the local tenant/creator-party/evidence/generation binding.
...
| Game Design tenant-scoped creator writes for assets and templates | Yes, with Game Design's fail-closed mutation gate consuming Account hosted-terms/currentness authority and evidence; bypass-safe bypasses neither | Game Design service contract plus Account hosted-terms authority |
...
| `POST /api/design/templates` | Game Design creator write | Allowed when Game Design's fail-closed mutation gate obtains Account hosted-terms/currentness authority and evidence before side effects | Domain-local creator workflow owned by Game Design; bypasses Logging & Admin only, not the Account authority check or Game Design enforcement |
...
| Game Design creator writes for assets and templates | Gateway allowlisted Game Design APIs | Allowed when the owning Game Design contract documents tenant access, validation, audit, and its fail-closed mutation gate's pre-side-effect Account hosted-terms/currentness authority/evidence check with local binding; bypass-safe bypasses neither |
...
| `/api/design/**` | Game Design external creator/admin family | Reads plus the explicitly allowlisted bypass-safe creator-write classes for assets/templates and other architecture-approved domain-local workflows | Game Design's fail-closed mutation gate owns tenant checks, validation, and audit while consuming Account hosted-terms/currentness authority and evidence; bypass-safe bypasses Logging & Admin only, not the Account authority check or Game Design enforcement. |
