...
- **Canonical room-read interoperability:** The current live proto seam carries World `worldSnapshotId` / `world_snapshot_id` and Entity `entitySnapshotId` / `entity_snapshot_id` as deterministic scope markers only. Target requests carry a `CausalReadFence` for the same room scope and epoch, including the committed tick floor; each participant returns its actual component version. Bounded component skew newer than the requested floor is allowed, while a component behind the floor or mixed scope/epoch is rejected or retried. The composed identity exposes the requested floor plus World and Entity component versions; it does not claim an exact cross-database historical snapshot. The target causal-floor request/response and composite-version protocol is not present in the current adapter/proto path.
...
- World Management emits the room-read correlation value on `GetRoomSnapshot`, currently as the deterministic scope marker `worldSnapshotId` / `world_snapshot_id`; Game Logic owns `ResolveLook` composition. The target request carries a `CausalReadFence` containing at least `(tenantId, gameInstanceId, roomInstanceId, regionEpoch, committedTickId)`.
- World Management and Entity Management serve the requested scope and epoch at or beyond that floor and return their actual component versions. Bounded versions newer than the requested floor are valid for presentation; a participant behind the floor or a mixed scope/epoch is rejected or retried. Game Logic exposes the requested floor plus component versions in the composite room-view identity.
...
2. Game Logic carries the requested `CausalReadFence` to World Management; the current path returns only its scope marker, while the target path returns the World component version after serving the floor.
3. Game Logic calls Entity Management `ListRoomEntities` with the same scope/epoch floor; the current path returns only its scope marker, while the target path returns the Entity component version.
4. Under the current implementation, the markers remain scope/read correlation values. Under the target protocol, Game Logic accepts same-scope/epoch responses at or beyond the floor, including bounded newer skew, or rejects/retries a behind-floor or mixed response.
5. Game Logic composes one `LookResult` with a composite identity containing the requested floor and both actual component versions, then Game Session renders and caches the transcript.
...
- The current scope markers and target causal floor are valid only within one `(tenantId, gameInstanceId, roomInstanceId)` scope; the target floor also carries `regionEpoch` and `committedTickId`.
- World Management and Entity Management return actual local component versions after serving the requested floor. They do not echo or mint one shared temporal token.
- Bounded component versions newer than the requested floor may compose for presentation. A component behind the floor, or any mixed tenant, game instance, room, or epoch, is rejected or retried; `STALE_READ_FENCE` and `READ_FENCE_UNAVAILABLE` remain the service rejection shapes for an unsatisfied request.
- The composite identity contains the requested floor plus the World and Entity component versions. The current scope-derived markers remain neither freshness proof nor cache-invalidation authority; the causal-floor/component-version protocol and focused failure/retry proof remain implementation gaps.
...
2. Game Logic carries a causal floor such as `(tenantId=42, gameInstanceId=7, roomInstanceId=R-44, regionEpoch=17, committedTickId=42)` to World Management and receives a World component version.
3. Game Logic asks Entity Management to satisfy the same scope/epoch floor and receives an Entity component version.
4. Success path: both downstream reads are at or beyond the floor, with any bounded newer skew allowed, and Game Logic composes the requested floor plus both versions into the room-view identity.
...
- **Canonical room state** – A room view assembled only when World Management and Entity Management satisfy the requested same-scope/epoch causal floor; actual component versions appear in the composite identity and bounded newer skew may be accepted.
