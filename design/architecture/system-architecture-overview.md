...
- **Non-edge failover contract:** Meaningful live gameplay state must be externalized into durable or shared coordination systems rather than hidden in single-process memory. Same-type Game Session and Game Logic instances must be able to take over from shared state after non-edge restarts, with visible client reconnect treated as implementation debt unless the edge transport itself dropped. In-flight bytes or the last not-yet-durable command may still be lost, but non-edge restart must not require fresh `LOGIN`/`PLAY` as the normal design path.
...
- **SQL persistence stack:** For SQL-backed services, FireMUD’s target persistence stack is `jOOQ + Flyway`. Flyway remains the canonical schema authority, and explicit SQL generation/execution is the target runtime model. Existing JPA/Hibernate usage is migration debt during the `02.19` convergence family, not a co-equal long-term platform direction.
