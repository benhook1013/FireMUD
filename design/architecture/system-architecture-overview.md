...
- The current Logging & Admin `POST /reports` surface is administrative persistence: its controller accepts caller-supplied `tenantId`, `reporterAccountId`, and optional `targetAccountId` as report data, persists the report, and is reachable through the current `/api/admin/reports/**` Gateway route. Those references do not authorize the operator or tenant; the authenticated operator is the audit authority. It is not player ingestion. The separate target player-bootstrap submission route, which would derive reporter and tenant identity from authenticated player authority, is unavailable; this does not disable the current administrative route.
- `POST /moderation/actions` currently persists moderation policy input and audit; the separate `EvaluateModerationPolicy` read is consumed at Game Session's `GAMEPLAY_ADMISSION` boundary and Social & Groups' `CHAT_SEND` boundary, with applicable high-risk decisions failing closed when fresh policy evidence is unavailable. Versioned snapshot/event propagation and broader enforcement are missing; the route remains persistence/audit, not an enforcement mutation.
...
- Game Session's current `/sessions*` lifecycle routes are owner-local hooks, not a complete external Logging & Admin ingress family. The target operator path is Logging & Admin ingress to Game Session owner RPCs; do not treat the current owner-local routes as current external coverage.
...
Current forwarding is narrower than that ADR 0048 target: runtime feature flags, admission-pointer reads/audit/version-upgrade preparation/read, and scoped tick pause/resume are the live Logging & Admin families. Admission-pointer CAS open/close and prepared cutover remain target-state; current forwarding does not prove ADR 0048 durable intent, full authority-bound idempotency, owner-result recovery, or lease-fenced reconciliation. Game Session's current `/sessions*` lifecycle hooks remain owner-local and are not a current external Logging & Admin family. Moderation uses a different live read path: Logging & Admin persists policy input/audit and evaluates policy, while Game Session and Social & Groups synchronously consume those decisions at `GAMEPLAY_ADMISSION` and `CHAT_SEND`. The operator action is not a forwarded enforcement mutation; versioned propagation and broader policy coverage remain target-only. Quota and broader coordination remediation remain target-only.
...
| --- | --- | --- | --- | --- | --- |
| Moderation action (`gameplay_ban`, `chat_mute`, `chat_ban`) | Logging & Admin HTTP(S) APIs via Gateway | Logging & Admin owns policy and audit; Game Session and Social & Groups own runtime enforcement | Current: persist policy/audit, then runtime owners synchronously call `EvaluateModerationPolicy` at `GAMEPLAY_ADMISSION` or `CHAT_SEND`. Target: add versioned snapshot/event propagation and broader coverage without turning the operator action into a direct enforcement mutation. | Logging & Admin PostgreSQL policy/audit state; target versioned projections in enforcement owners | No |
...
| Admission-pointer preparation (current); open, close, or retarget CAS (target-state) | Logging & Admin HTTP(S) APIs via Gateway | Game Session | Current: Logging & Admin records intent and calls the implemented preparation/read surfaces. Target: Game Session validates current catalog/runtime authority and compare-and-sets the pointer under the current version/fence, including explicit `CLOSED` handling. | Current: Logging & Admin intent/audit state plus implemented Game Session preparation state. Target: version-fenced Game Session admission-pointer state as well. | No |
...
- **Core admin actions** – Feature flag toggles, live admission-pointer reads/audit/preparation, scoped tick-remediation controls, and target-state admission CAS/cutover and session-lifecycle controls that primarily talk to domain microservices (for example, Account, Game Session, Social & Groups) via the Gateway. These are designed to remain available even if Elasticsearch, Prometheus, Jaeger, or Alertmanager are temporarily unavailable. Current moderation actions persist policy input and audit; Game Session and Social & Groups independently enforce synchronous policy evaluations. Versioned propagation is target coverage rather than a shipped mutation path.
...
The current administrative `/reports` persistence route is not caller-bound: its OpenAPI/controller accepts caller-supplied `tenantId` and `reporterAccountId` as data fields, but those values do not authorize the operator or tenant. The authenticated operator is the audit authority, and the route is not player ingestion. The separate target player-bootstrap route derives both identities from the validated player session and remains unavailable until that contract is implemented.
