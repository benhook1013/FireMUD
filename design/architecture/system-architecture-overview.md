...
Broader scoped remediation beyond the live pause/resume endpoints is coverage drift: no current OpenAPI route or Game Session owner RPC exists, so it is not part of the implemented availability contract and must not be represented as an executable route.
