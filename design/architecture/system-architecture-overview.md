...
| **[Social & Groups Service](./microservices/social-groups-service/README.md)** | Owns relationships, groups, audiences, social-channel communication, mail envelopes, applicable history, and social delivery state; enforces owner-local `chat_mute`/`chat_ban` state at sending, participation, and history boundaries. It does not own Account identity/presence, Game Session transports, or Entity items, currency, containers, or attachments. |
