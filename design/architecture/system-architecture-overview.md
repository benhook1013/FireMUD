...
On initial login, Game Session delegates full credential verification (including lockout and MFA rules) to the **Account Service**. Every fresh successful `LOGIN` obtains a new Account-issued backend token, and Game Session stores its `authTokenHash`, `authTokenIssuedAt`, and `authTokenExpiresAt` atomically with the gameplay binding.
