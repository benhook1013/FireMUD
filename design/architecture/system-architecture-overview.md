...
- Moderation action persistence and audit are currently gated/unavailable: `POST /moderation/actions` and `ApplyModerationAction` are present legacy transport stubs that hard fail closed after narrow tenant/admin checks, without dispatch or persistence; separate fixed-category audit evidence and owner-side enforcement are absent, and no Gateway moderation route exposes this local REST/gRPC path. The separate `EvaluateModerationPolicy` read remains live at the Game Session and Social & Groups owner boundaries (`GAMEPLAY_ADMISSION` and `CHAT_SEND`), with applicable high-risk decisions failing closed when fresh policy evidence is unavailable. That compatibility seam is a legacy, deny-only lookup over generic action rows ordered by recency; it is nonconformant with ADR 0141's broadest-effect response precedence (`platform_access_ban` over `account_security_lock`, `chat_ban` over `chat_mute`) and does not implement or prove all five fixed categories or their independent lifecycles. For chat, that current fail-closed seam covers only new sends: the current effect replay can return before the policy read and lacks authenticated-caller and request-digest binding, so chat retry/replay remains an explicit implementation gap. Target state uses the fixed categories and owner-local durable enforcement commands defined in [Moderation Policies](./microservices/logging-admin-service/moderation-policies.md) and active [ADR 0146](./decisions/adr-0146-owner-local-moderation-enforcement.md) (mapped from reviewed archive record `archive-ADR-0133`), with lifecycle rules from [ADR 0141](./decisions/adr-0141-fixed-safety-restriction-categories-and-independent-lifecycles.md) and appeals from [ADR 0142](./decisions/adr-0142-bounded-moderation-appeal-cases.md); complete persistence, owner command delivery, enforcement, and appeal handling remain missing.
...
- **Gameplay session routing inside Game Session:** Connected gameplay sockets attach to a stable Game Session session front-end pod, while region-scoped tick execution remains fenced to the current lease owner for `<tenantId, gameInstanceId, regionId>`. Owner directories and execution-region pointers are routing projections; the receiving executor validates authority. See [ADR 0170](./decisions/adr-0170-fenced-command-forwarding-and-authoritative-region-transition.md).
- **Non-edge failover contract:** Per [ADR 0013](./decisions/adr-0013-bounded-invisible-non-edge-restart-recovery.md), meaningful live gameplay state must be externalized into durable or shared coordination systems rather than hidden in single-process memory. Invisible recovery is limited to failures where the established client-facing edge socket remains owned and open, healthy replacement capacity and shared authority remain available, and an explicit owner/rebind path can preserve the session (for example, a Game Session lease-owner move or a Game Logic worker replacement behind a surviving Game Session front end). A Game Session front-end/process loss qualifies only where an external socket owner such as Gateway explicitly retains that socket and rebinds the upstream; without that protocol it is a visible reconnect boundary. The ordinary functional target is 10 seconds and the hard hidden-recovery cutoff is 30 seconds, after which affected sessions close with `1013/backend_unavailable`. In-flight bytes or one ambiguously delivered command may still be lost under the at-most-once edge contract.
- **Multi-cluster gameplay sharding scope:** FireMUD target state permits exactly one active gameplay cluster per deployment, with scale via lease-based in-cluster Game Session rebalancing. Any recovery cluster remains fenced and quarantined from gameplay execution until an explicit cutover makes it the sole active cluster. Cross-cluster gameplay sharding is out of scope until a dedicated end-to-end design package is accepted. See `design/architecture/decisions/adr-0008-multi-cluster-gameplay-sharding-scope.md`.
...
- **Bypass-safe workflow allowlist policy:** Service-level docs may not invent new classes of externally writable bypass-safe workflows on their own. A workflow is bypass-safe only when the overview or responsibility matrix explicitly names it as a bypass-safe class or explicitly delegates that class to a service-owned contract. Game Design creator writes for tenant-scoped assets and templates are delegated to the Game Design service contract as domain-local creator workflows, but every covered official-hosted write requires the exact Account Creator Party, acceptance evidence, and material-acceptance-generation authorization to remain valid through Game Design's commit linearization point; a preflight read alone is insufficient, and failure is fail-closed before authoritative mutation. Otherwise, external writes are denied by default and require an architecture-doc update in the same change.
...
- **Redis topology policy:** In all non-ephemeral environments, Coordination Redis and Cache/Rate-Limit Redis are separate deployments. Local development is treated as non-ephemeral and should run two Redis deployments to exercise role separation. Only an explicitly labelled one-shot test/CI stack may collapse roles into a single Redis instance, and it must be guarded as an ephemeral topology; hosted `pr-preview` is not exempt.
  - **Coordination Redis ownership boundary:** Coordination Redis prefixes and fences are owner-governed (Game Session for gameplay coordination prefixes such as `session:game:*`, `tick:*`, `timer:*`, `retry:*`, and `tick-executor-lease:*`; Account Service for the registered auth-session and exact-input issuance-result families; Automation & Scripting for its documented scheduling families; Spring Cloud Gateway only for the narrow edge replay/denial contract), and no wildcard grant over future families is implied. The Account connect-token family is distinct from Gateway replay markers and follows its Account-owned fail-closed reset/recovery consequence. Gateway's exception does not grant it general Coordination Redis ownership. Non-owner participation is allowed only through documented shared-helper contracts. See [ADR 0009](./decisions/adr-0009-coordination-redis-ownership-boundary.md), [Redis Coordination Invariants](./system-architecture-redis.md#coordination-vs-cache-prefixes), the [reset policy matrix](./system-architecture-redis-reset-and-recovery.md#reset-policy-matrix-prefix-summary), and the Gateway replay/denial contract in [Gateway Architecture](./system-architecture-gateway.md).
- **TCP Proxy identity canonicalization:** Gateway selects exactly one environment-bound TCP Proxy trust profile. Steady-state player-facing deployments require the exact environment-specific URI SAN identity; named DNS migration and fingerprint break-glass profiles are explicit and expiring, while CIDR trust is development/test-only. See [ADR 0169](./decisions/adr-0169-exclusive-environment-bound-tcp-proxy-trust.md).
- **Canonical room-read composition:** [Transaction Strategies](./system-architecture-transactions.md) owns split spatial authority and operation-bound effect behavior. The [Identifier Glossary](./system-architecture-identifier-glossary.md#cross-service-effect-identity) owns root `EffectId`/participant identity and enclosing-root lineage, and its [causal-read fence contract](./system-architecture-identifier-glossary.md#cross-service-causal-read-fence-identity) owns the requested floor and composite identity. Game Logic composes World occupancy with Entity presentation under those owner contracts; see [Canonical Room Runtime Contract](#canonical-room-runtime-contract). Exact generated-child allocation remains non-authoritative in [ADR 0183](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md).
...
- **Game Session region-transition contract:** The session front-end owns connection-local FIFO and a derived execution-region pointer, while the fenced lease owner owns region-scoped admission. Cross-region actions converge through the authoritative durable spatial/effect result; only afterward does the front-end update its routing pointer. `sessionSequence` is not retry identity or mutation authority. See [Session Sharding & Routing](#session-sharding--routing).
...
- **All application-level gameplay and admin traffic is routed through the Spring Cloud Gateway**, ensuring centralized **traffic routing, monitoring, and observability**. External admin and creator APIs are HTTP(S) surfaces routed through the Gateway allowlist; external domain gRPC is not an edge contract unless a dedicated design update explicitly adds it. Raw Telnet TCP terminates at the Telnet edge proxy and TCP Proxy Service before being bridged to the Gateway over WebSocket. See [Gateway Architecture](./system-architecture-gateway.md) for deployment details and stateless behavior.
  ...
  - Backpressure and slow-client behavior across the TCP Proxy and WebSocket layers are described in [Protocol Bridging](./system-architecture-protocol-bridging.md#backpressure--slow-clients).
   > 🛑 **Gameplay login is fronted by the Game Session Service**, which handles the `LOGIN` command and binds sessions in Redis. Credential-bearing text clients send credentials through `LOGIN`, and Game Session delegates their verification to Account Service. First-party WebSocket clients have credentials verified by Account during `player-bootstrap`, then use a short-lived edge connect token on `/ws/game/**`; Game Session validates the resulting signed connect context for bare `LOGIN`/`PLAY` rather than performing a second credential check. The Gateway forwards ordinary admin/creator `Authorization` JWTs unchanged, and each consuming service validates them locally through the shared library and Account JWKS; gameplay protocol clients do not present JWTs. See [Authentication & Authorization](./system-architecture-authentication.md#login-and-session-flow) for the full login flow.
...
The architecture uses three top-level traffic planes and five named surfaces/subsurfaces:
...
- **First-party frontend delivery subsurface** – public site router/static frontend host delivery of frontend documents and compiled `/frontend-assets/**`; this is not a Gateway route family or an additional top-level plane.
- **Published asset delivery subsurface** – Gateway- or CDN-fronted `/assets/**` reads for published assets and exported content; this is a read-only external surface, not a creator/control-plane write family or an additional top-level plane.

These three planes and two delivery subsurfaces are normative and should be used consistently in service docs and diagrams to avoid conflating infrastructure control, external admin APIs, public delivery, and player gameplay traffic.
...
- **First-party frontend delivery subsurface:** Frontend documents and compiled assets are read from the static frontend host through the public site router. `/frontend-assets/**` is reserved for compiled first-party files, never SPA-fallbacks, and is absent from the Gateway Java route catalog; the target public `/auth/**` family rewrites to Gateway's existing `/api/account/auth/**` route before API ingress. The router/rewrite and browser proof remain unimplemented, and the frontend host and published-asset origin do not serve each other's families.
- **Published asset delivery subsurface:** Published game assets are read through the canonical `/assets/**` surface. This surface is edge-routable for release artifact delivery and caching, but it is not an admin/creator ingress path and does not authorize design-time mutation.
...
- `POST /api/design/assets` and `POST /api/design/templates` are bypass-safe creator writes when implemented by Game Design as tenant-scoped domain-local asset/template workflows with Game Design-owned validation and audit behavior. They bypass Logging & Admin only: for covered official-hosted operations, Game Design's fail-closed mutation gate must keep the exact Account Creator Party, acceptance evidence, and material-acceptance-generation authorization valid through its commit linearization point; a preflight read alone is insufficient, and failure is fail-closed before authoritative mutation or local tenant/creator-party/evidence/generation binding.
...
| Game Design tenant-scoped creator writes for assets and templates | Yes, with Game Design's fail-closed mutation gate keeping the exact Account Creator Party, acceptance evidence, and material-acceptance-generation authorization valid through commit linearization; a preflight read alone is insufficient and failure is fail-closed before authoritative mutation; bypass-safe bypasses neither | Game Design service contract plus Account hosted-terms authority |
...
| `POST /api/design/templates` | Game Design creator write | Allowed when Game Design's fail-closed mutation gate keeps the exact Account Creator Party, acceptance evidence, and material-acceptance-generation authorization valid through commit linearization; a preflight read alone is insufficient and failure is fail-closed before authoritative mutation | Domain-local creator workflow owned by Game Design; bypasses Logging & Admin only, not the Account authority check or Game Design enforcement |
...
| Game Design creator writes for assets and templates | Gateway allowlisted Game Design APIs | Allowed when the owning Game Design contract documents tenant access, validation, audit, and its fail-closed mutation gate's exact Account Creator Party, acceptance evidence, and material-acceptance-generation authorization through commit linearization; a preflight read alone is insufficient and failure is fail-closed before authoritative mutation; bypass-safe bypasses neither |
...
- Among application microservices, only Gateway and TCP Proxy Service are reachable from external networks. Separately, the target public site/static host serves frontend documents and `/frontend-assets/**`, and the published `/assets/**` family is a separate read-only asset-delivery surface (Gateway- or CDN-fronted); these delivery surfaces are not additional application-microservice ingress paths and remain governed by their own target delivery contracts.
...
| `/api/design/**` | Game Design external creator/admin family | Reads plus the explicitly allowlisted bypass-safe creator-write classes for assets/templates and other architecture-approved domain-local workflows | Game Design's fail-closed mutation gate owns tenant checks, validation, and audit while keeping the exact Account Creator Party, acceptance evidence, and material-acceptance-generation authorization valid through commit linearization; a preflight read alone is insufficient and failure is fail-closed before authoritative mutation. Bypass-safe bypasses Logging & Admin only, not the Account authority check or Game Design enforcement. |
...
Before an external mutation family is enabled, it requires the minimum schema/authorization gate: a versioned action-family schema, shared cross-language `mutationDigest/v1` golden vectors consumed by every participant, Account issuance of the bounded canonical `authorization-reference`, and redemption of that reference with Account at the contract-declared receiving boundary before mutation. That minimum gate is necessary but not sufficient for a forwarded owner mutation; the complete durable, idempotent, fenced owner-execution contract in [ADR 0048](./decisions/adr-0048-durable-idempotent-operator-write-execution.md) must also be implemented and proved. For ordinary forwarded mutations that boundary is the selected domain owner, which also validates current domain facts independently. Moderation policy-intent persistence instead redeems its policy-intent reference at Logging & Admin and requires its local durable intent/audit contract; only the later distinct owner-enforcement reference, scoped by its independent `ownerEnforcementRequestId` and digest, is redeemed by the selected enforcement owner. A family missing any applicable prerequisite remains gated or target-only.
...
| Runtime feature-flag override | Target: Logging & Admin HTTP(S) APIs via Gateway; current fail-closed ingress stub is externally disabled pending the minimum schema/authorization gate and complete ADR 0048 owner-execution contract and has no callable forwarding method | Game Session | Target: Logging & Admin records audit and calls Game Session `ToggleFeatureFlag`/equivalent control API once the complete owner-forwarding gate exists | Game Session PostgreSQL plus Logging & Admin PostgreSQL audit state | No |
...
| Admission-pointer reads/audit and prepared-upgrade proof reads (implemented but nonconformant); open, close, or prepared-cutover CAS (target-state) | Current: admission-pointer GET reads/audit and prepared-upgrade proof reads are live; externally callable mutation/preparation paths are disabled pending the minimum schema/authorization gate and complete ADR 0048 owner-execution contract for target mutations. Target: Logging & Admin HTTP(S) APIs via Gateway | Game Session | Current: admission-pointer reads/audit and prepared-upgrade proof reads are live; internal pointer-write/prepared-cutover methods are nonconformant seams and do not record durable operator intent before forwarding. Target: Logging & Admin durably records intent, then Game Session validates current catalog/runtime authority and compare-and-sets the pointer under the current version/fence, including explicit `CLOSED` handling. See [Versioning & Runtime Configuration](./system-architecture-versioning-runtime.md) for the canonical preparation/proof contract. | Current: Game Session proof/read and preparation state only; no current Logging & Admin durable intent/audit store for these mutations. Target: version-fenced Game Session admission-pointer state plus the durable operator intent/audit record. | No |
| Tick remediation (`PauseTicks`, `ResumeTicks`) | Target: Logging & Admin HTTP(S) APIs via Gateway; current fail-closed ingress stubs are externally disabled pending the minimum schema/authorization gate and complete ADR 0048 owner-execution contract and have no callable forwarding method | Game Session | Target: Logging & Admin records audit and calls Game Session control APIs once the complete owner-forwarding gate exists; direct Redis mutation is reserved for documented runbooks, not UI/API request handlers | Game Session PostgreSQL/control-plane state plus Logging & Admin PostgreSQL audit state | No |
...
- **Gameplay sessions (players)**  
  - Credential-bearing text clients authenticate using a credential-bearing `LOGIN` handled by the **Game Session Service**, with Account verifying the configured `PASSWORD` or verified `EMAIL_OTP` mode. First-party browser/mobile clients have credentials verified during Account's `player-bootstrap` flow, then obtain a connect token and use bare `LOGIN` for the WebSocket session.
  - Account owns all credential and account-security decisions. For text `LOGIN`, Game Session delegates credential verification to Account; for first-party WebSocket bare `LOGIN`, Game Session validates the gateway-signed connect context and its bootstrap subject instead of performing full credential verification again.
...
- **Coordination Redis** holds volatile, gameplay-critical structures (session bindings, tick queues, locks, timers) owned primarily by the Game Session Service and a small number of cooperating services using shared helpers, plus Gateway's narrow connect-token replay/denial state and readiness fence.
- **Cache/Rate-Limit Redis** is used for best-effort caches and rate limiting by Spring Cloud Gateway, the Game Session Service's session front end, and selected backend services; TCP Proxy participation is target/optional and the current TCP Proxy implementation uses no Redis keys (only generic Redis bean wiring). Game Session's current presentation and limiter projections are legacy and disposable, while target presentation-cache behavior remains owned by the canonical Redis cache contract. The lease-owner/tick engine does not directly access Cache/Rate-Limit Redis. These keys use dedicated prefixes and must not share a deployment with coordination keys in player-facing environments.
...
- **Coordination and session keys (Coordination Redis)**  
  - Examples: gameplay sessions, tick-region leases, command queues, timers, and automation timer/scheduler coordination.
  - Canonical prefixes include (non-exhaustive):  
    - `session:game:*` – gameplay session bindings and takeover metadata (Game Session-owned).
    - `session:auth:token:*` (for example, `session:auth:token:<tokenHash>`) – target-only exact issued-token registry records projected by Account; separate Account-owned authority generations provide bulk revocation/version authority. Current live auth projections remain the Account-owned `session:auth:account:*` and `session:auth:tenant:*` families unless live implementation evidence establishes that the target token family is deployed and proved.
    - `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<sha256(connectScopeId)>:request:<sha256(requestId)>` – Account-owned current legacy exact-input result-map records; they are short-lived, reset-sensitive, use only the server-issued opaque `connectScopeId` and request identity, do not accept or use client-supplied target-bearing fields, and leave target recovery to the server-side versioned target path rather than serving as a target-input or target-recovery path.
    - `session:connect-token:v1:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>` – Account-owned target exact-input issuance-result/idempotency records; values require `schemaVersion: "connect-token-issuance-result-v1"`, while target recovery is durable/fenced exact-result recovery or fail-closed refusal. Target readers/writers are isolated from the legacy family. Both families are distinct from the four Account auth-session families and Gateway replay/deny keys.
    ...
    - Automation scheduling and timer coordination state – owned and named by the canonical Redis ownership contract; other services interact via gRPC APIs rather than writing undocumented automation keys directly.
    - Gateway connect-token replay, browser-revocation denial, and readiness/fence state – the narrow Gateway Coordination Redis exception; the canonical key representation is owned by the Redis contract and does not grant general coordination ownership.
...
Coordination Redis and Cache/Rate-Limit Redis are **separate Redis deployments in all non-ephemeral environments** so cache or rate-limit spikes cannot degrade tick execution or session coordination. Local development runs both roles as separate deployments to exercise role separation. Only an explicitly labelled one-shot test/CI stack may collapse roles into a single Redis instance; hosted `pr-preview` retains the normal split.
...
- **DMZ services (TCP Proxy Service and Spring Cloud Gateway)** remain stateless with respect to PostgreSQL; Spring Cloud Gateway uses **Cache/Rate-Limit Redis**, while TCP Proxy's Cache/Rate-Limit Redis participation is target/optional and absent from the current implementation (which has only generic Redis bean wiring). Both always emit logs/metrics and do not own persistent domain tables.
...
| Account/session token control metadata | Account Service | Account PostgreSQL plus Account-owned Coordination Redis prefixes: current `session:auth:account:*` and `session:auth:tenant:*`; target-only `session:auth:token:*` and `session:auth:generation:*`. The separate current legacy `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<sha256(connectScopeId)>:request:<sha256(requestId)>` and target `session:connect-token:v1:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>` issuance-result families are also Account-owned |
...
- A cache shared across viewers may contain only viewer-independent structured facts and proof and must reapply caller authorization, visibility/character filtering, and locale/rendering on every request. Any derived rendered room/UI cache entry must be bound to the exact viewer/session and complete immutable viewer-policy/presentation context and invalidated when that context changes. The complete causal-read fence alone is insufficient; Game Session remains the owner of derived player-facing rendering.
...
Per [ADR 0170](./decisions/adr-0170-fenced-command-forwarding-and-authoritative-region-transition.md), this stable surface is implemented as a **session front-end + lease-owner execution** model:
...
- The front-end keeps a derived execution-region pointer and advances it only after the authoritative durable spatial/effect result establishes the new location.

Forwarded internal gameplay requests carry complete tenant/game/region scope, stable `commandId`, expected executor fence, session identity, active `frontEndGeneration`, and `sessionSequence`. Owner-directory data is advisory: the receiving executor validates current authority. Stale-fence rejection refreshes routing and retries only the same logical identity when it remains safe.

Cross-region commands follow these canonical rules:

- The session front-end serializes forwarded work from one live connection generation with `sessionSequence`. Stable command/effect/request identities, not the sequence, govern dedupe and ambiguous-result reconciliation.
- A region transition is established by the normal authoritative spatial/effect contract. It is not a separate source-release/destination-accept/front-end-pointer transaction.
...
- If lease ownership changes before admission, the session front-end refreshes routing and may retry the same identity. An ambiguous admitted command is resolved through authoritative command status; already committed effects are never reissued blindly.
...
Gameplay authentication is transport-specific. Credential-bearing Telnet and other non-WebSocket text clients use `LOGIN`; Game Session forwards their credentials to Account for verification. First-party WebSocket clients complete Account credential verification during `POST /auth/player-bootstrap`, obtain the short-lived connect token, and then use bare `LOGIN` only after Game Session validates the gateway-signed connect context and binds it to the bootstrap subject. Account completes issuance of the new backend token for the credential-bearing text flow within its own authority boundary under one stable, high-entropy `requestId` bound to a pre-binding login tuple containing the normalized login, the exact calling Game Session workload, and the applicable credential/authentication context. This Account issuance identity is established before realm, character, and gameplay-binding selection and is distinct from the later gameplay-binding CAS identity. Account's idempotent issuance and issued-token registry contract, defined by [ADR 0031](./decisions/adr-0031-revocation-safe-session-token-rotation-and-logout.md) and [ADR 0035](./decisions/adr-0035-single-record-issued-token-registry.md), must return the same committed issuance result for a matching retry and reject reuse of that request ID with a different digest; it must not create multiple valid tokens for one logical attempt. Durable operation evidence contains no raw JWT; if response recovery requires the credential itself, Account uses a bounded Account-encrypted response envelope bound to the request, workload, binding, lineage, and token identity, with the same expiry and fail-closed rules as the accepted token-rotation contract. After `PLAY` selects the exact gameplay target, Game Session stores the token's `authTokenHash`, `authTokenIssuedAt`, and `authTokenExpiresAt` through a distinct gameplay-binding CAS using the expected old binding generation and a reference to the Account issuance operation and token identity. These are ordered cross-service steps, not one atomic transaction spanning Account and Game Session.
...
If the Account response is lost during credential-bearing text login, Game Session retries with the same `requestId` and exact request digest. Account resumes matching pending work or returns the stored committed result/terminal outcome; it must not mint a second token merely because the first response was not observed. A confirmed binding-CAS conflict or superseded binding causes Game Session to keep the candidate unusable and issue Account's idempotent retire request for that exact operation and token identity. An ambiguous timeout instead remains fail-closed and provisional while Game Session reconciles the expected binding generation plus operation identity against the authoritative binding: it retires the candidate only after proving that the CAS did not commit, and completes installation or the canonical superseded outcome when the committed operation is found. Account performs bounded orphan cleanup after the operation's lease/expiry horizon; an orphan registry record is never sufficient to make a gameplay binding admissible. Proof must cover Account success followed by response loss, confirmed binding-CAS conflict, ambiguous CAS reconciliation for both committed and uncommitted outcomes, retry with the same request ID, retry with a conflicting digest, and cleanup/reconciliation after each cross-service failure window. The protocol uses ordered durable evidence, idempotent owner-side operations, and fail-closed provisional state; it does not claim a cross-store transaction.
After an actual client-visible edge disconnect, clients repeat their transport-specific authentication path before re-binding gameplay scope with `PLAY <world> [realm] [character]`. Credential-bearing text clients issue the applicable `LOGIN` form (`PASSWORD` or verified-email `EMAIL_OTP`) defined by [Authentication](./system-architecture-authentication.md#login-and-session-flow); first-party WebSocket clients complete a fresh bootstrap/connect-token handshake and then use bare `LOGIN` backed by the verified connect context. This requirement does not apply while the edge connection is retained during an internal Game Session bridge or lease-owner rebind; the bounded non-edge recovery path keeps the existing `LOGIN`/`PLAY` state in that case. For either transport, target `JOIN` is required when first-time public-production membership is needed, followed by conditional realm-scoped `CHARS` or character creation unless fresh transport-local resolution supplies exactly one valid current character, and finally `PLAY`; returning members skip `JOIN` only after current `ACTIVE` membership and authority checks pass. Current eligible public missing or existing non-admitting membership still returns non-actionable `JOIN_REQUIRED` from `PLAY`. The Account JOIN RPC, browser bootstrap join facade, and direct-text REALMS-scope/JOIN dispatch now exist locally, but this does not establish frontend or production readiness: PostgreSQL execution, direct text-to-Logging end-to-end proof, unknown-outcome recovery, and membership lifecycle/generation rereads remain unproved. Session resumption does not bypass the fresh WebSocket bootstrap/connect-token handshake. Game Session uses Coordination Redis to resolve the existing gameplay session by controller key `{tenantId, playableStateNamespaceId, characterId}`, separately validates server-derived `playableStateScope` as binding/routing evidence, and resumes it only when gameplay state permits; otherwise it starts a fresh session. A same-character target takeover fences further source commands, reconciles already-admitted source effects, and switches the binding by CAS with the next `bindingGeneration` before admitting target commands; other source sessions remain under the bounded drain in [ADR 0027](./decisions/adr-0027-single-realm-admission-target.md). The current per-instance uniqueness implementation is a gap, and this owner rule is defined in [Session Behavior](./system-architecture-session-behavior.md#session-and-identity-management).
Session recovery/takeover is authorized by the gameplay binding plus current Account identity, membership, entitlement, and revocation authority; it does not require the previous backend token registry record to remain valid. When transport-specific reauthentication is required, the client must complete the applicable fresh credential/bootstrap and connect-context steps before backend calls resume. A missing or expired prior token therefore causes replacement or reauthentication, not acceptance of stale token state.
...
- **Core admin actions** – Live admission-pointer reads/audit and prepared-upgrade proof reads use the Gateway-to-Logging & Admin domain path and remain available independently of observability backends. Separate mutation workflows—feature-flag toggles and scoped tick-remediation controls (implemented ingress but gated), version-upgrade preparation (internal implementation-participating seam; public operator mutation target-only absent), admission-pointer CAS/cutover (internal nonconformant seams; target atomic CAS/cutover unimplemented and public mutation target-only absent), and session-lifecycle controls (target-only/non-callable)—are not currently callable and must remain independent of observability backends when implemented. Moderation action persistence/audit is currently gated/unavailable: `/moderation/actions` and `ApplyModerationAction` are present legacy transport stubs that hard fail closed without dispatch or persistence; separate fixed-category audit evidence and owner-side enforcement are absent, and no Gateway moderation route exposes this local REST/gRPC path. Game Session and Social & Groups independently consume the separate live `EvaluateModerationPolicy` read at their owner boundaries. Target-state moderation persistence/audit and digest-bound owner-command delivery are not a shipped mutation path.
...
5. Each actual mutation owner uses its persisted effect identity, binds its operation/aggregate-specific guard to the immutable request digest, and returns a durable result; evidence-only services are not artificial mutation participants. Retries/replays use the same identity and converge through owner guards and Game Session reconciliation; no service may treat local admission or partial writes as authoritative command completion.

`DROP` and `PICKUP` are command-root Entity-local containment commits against the admitted `RoomInstanceRef`; World location/version evidence is carried in an attestation bound to the command-root `EffectId`, `actorEntityId`, `RoomInstanceRef`, operational `regionId`, `regionEpoch`, `executorFence`, and `requestDigest`, and Entity verifies that binding with its participant guard at local commit. Game Session's durable barrier and actor-lock/fence gate prevents a conflicting `MOVE` across lock expiry or handoff until terminal evidence; stale re-resolution is requested by Game Session through Game Logic under the same command-root `EffectId`, preserving the `requestDigest`. A later valid `MOVE` is allowed after Entity commit and barrier terminalization. The current proto/request and focused proof do not yet carry or demonstrate this attestation/barrier path.
