...
- **Bounded durable semantic recent context** is owned by Game Session and scoped by `{tenantId, playableStateNamespaceId, characterId}`; ordered PostgreSQL rows are the source of truth and Redis is only a hot cache. It is separate from the rendered room cache, which remains presentation-only. See [ADR 0134](./decisions/adr-0134-bounded-durable-semantic-reconnect-context.md).
