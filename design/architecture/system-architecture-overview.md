...
- **Durable async contract:** Best-effort edge hints may use internal gRPC event sinks, but durable cross-service business events and saga updates use the transactional outbox/background-worker boundary classified in [Transaction Strategies](./system-architecture-transactions.md#tick-adjacent-workflows-outbox-boundary). High-level docs must not imply an unspecified shared event bus.
