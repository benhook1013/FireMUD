...
- Moderation action persistence and audit are currently gated/unavailable: `POST /moderation/actions` and `ApplyModerationAction` currently persist neither the `moderation_actions` record nor audit evidence and do not perform owner-side enforcement. The separate `EvaluateModerationPolicy` read remains live at the Game Session and Social & Groups owner boundaries (`GAMEPLAY_ADMISSION` and `CHAT_SEND`), with applicable high-risk decisions failing closed when fresh policy evidence is unavailable. Target state uses the fixed categories and owner-local durable enforcement commands defined in [Moderation Policies](./microservices/logging-admin-service/moderation-policies.md), with lifecycle rules from [ADR 0141](./decisions/adr-0141-fixed-safety-restriction-categories-and-independent-lifecycles.md) and appeals from [ADR 0142](./decisions/adr-0142-bounded-moderation-appeal-cases.md); complete persistence, owner command delivery, enforcement, and appeal handling remain missing.
...
| **[Account Service](./microservices/account-service/README.md)** | Manages player accounts, credentials, authentication, JWT/JWKS issuance, separate protective security-lock and punitive platform-access-ban state, Stripe v1 hosting billing, and canonical tenant entitlement/quota outcomes consumed by enforcement points |
...
| **[Social & Groups Service](./microservices/social-groups-service/README.md)** | Manages chat, mail, guilds, and social features, and enforces owner-local `chat_mute`/`chat_ban` state at sending, participation, and history boundaries. The long-term communication model distinguishes communication type, target/scope, recipient resolution, and per-recipient presentation rather than permanently treating all speech as one room-broadcast abstraction. |
| **[Logging & Admin Service](./microservices/logging-admin-service/README.md)** | Provides admin tools, metrics dashboards, and audit logs; owns fixed safety policy intent, moderation cases, bounded appeal cases/evidence, and audit while existing owner services enforce restrictions. It provides operator UX and auditing for hypothetical Account entitlement overlays without owning entitlement state. |
...
The complete fixed-category, independent-lifecycle, owner-local enforcement, and bounded-appeal mechanics are defined in [Moderation Policies](./microservices/logging-admin-service/moderation-policies.md), [ADR 0141](./decisions/adr-0141-fixed-safety-restriction-categories-and-independent-lifecycles.md), and [ADR 0142](./decisions/adr-0142-bounded-moderation-appeal-cases.md). At overview level:

- Logging & Admin owns policy intent, cases, appeal eligibility/jurisdiction/evidence/review, and append-only audit; it is not a runtime enforcement owner.
- Account owns global `account_security_lock` and `platform_access_ban`; Game Session owns exact tenant or tenant-and-realm `gameplay_ban`; Social & Groups owns exact tenant/realm/channel `chat_mute` and `chat_ban`.
