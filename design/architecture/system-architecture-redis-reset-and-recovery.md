...
7. **Run the post-reset smoke check**
   - Before smoke, recovery requests Account Service's issuer/account/tenant/membership-generation and issued-token projection rebuild or re-projection, awaits the durable result, and verifies the returned freshness/generation evidence. Recovery is not an Account projection writer.
...
3. Call public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=AWAITING_RESUME`; only the internal release phase may then reopen the scope after `finalized`.
4. If the operator aborts, call `coordination-maintenance release-lock --operation-id <operationId> --scope region --tenant 7b3b074e-d597-4e9b-b96f-4f5946d26120 --game-instance 9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78 --region R7 --maintenance-lock-token <token> --reason <reason> --evidence-ref <evidenceRef>`. This records audited abandonment, retains the paused/fenced state, and does not reopen traffic.
...
3. Call public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=AWAITING_RESUME`; only the internal release phase may then reopen the scope after `finalized`.
4. If the operator aborts, call `coordination-maintenance release-lock --operation-id <operationId> --scope tenant --tenant 7b3b074e-d597-4e9b-b96f-4f5946d26120 --maintenance-lock-token <token> --reason <reason> --evidence-ref <evidenceRef>`. The scope remains fenced and does not reopen.
