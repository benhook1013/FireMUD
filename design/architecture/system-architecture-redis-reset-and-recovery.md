...
7. **Apply the session-policy phase for the affected regions**
   ...
   - For each inventory row, recovery validates the corresponding `session:game:*` record and rebuilds the region binding and all applicable bounded indexes through their owned CAS/lease protocols. Issuer coverage uses the durable active-layout pointer and finite partition range; a reset never changes `issuerIndexPartitionCount` in place. Each partition is read back and receives a durable acknowledgement only after all expected inventory rows, reservations, repairs, and removals for the same snapshot, layout, and fence are accounted for. Redis `SCAN`, an empty partition, or a partial session list is never an active-binding inventory or recovery proof.
   - The complete [canonical preserved-session rebind predicate](./system-architecture-redis.md#session-and-region-binding-contract) requires all of the following before the region bridge may recreate a binding:
     ...
     - Current Account authority for the exact account and tenant, including `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `authorityTuple.membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable, plus current `membershipVersion`, entitlement, revocation, and the complete exact `outboxCheckpoints` set for all applicable authority streams. New admission, target changes, and capacity-creating operations require a fresh Account-authenticated `account-auth-evidence-bundle/v1`; if it is unavailable, stale, contradictory, or gapped, they fail closed. Only resume of the same unchanged binding may use the protected previously accepted positive bundle for at most five minutes, and that bounded fallback never supplies current authority or validates itself.
   ...
   - During the gap between reset and successful rebind, each attempted command must converge to command-record status `TERMINAL`, `executionOutcome = REGION_REBIND_REQUIRED`, and a non-applied gameplay result rather than treating `session:game:*` or pre-auth transport context advisory fields as region-local authority. `REGION_REBIND_REQUIRED` terminalizes that command/admission attempt, not the connected socket or gameplay-session record; socket closure or session invalidation requires a separate explicit policy or auth/expiry transition.
...
- Any reset scope that preserves gameplay sessions but clears region-local `tick:{tenantRegionTag}:session-binding:*` keys must complete the Account authority/token projection rebuild and verified durable freshness gate before applying the preserve policy or starting rebind. Rebind must consume only the exact Game Session-owned `GameplayBindingInventory` snapshot and `inventorySnapshotRevision` captured behind the operation's admission/creation fence; every later transition must be covered by the bound `coverageGeneration`, and every partition acknowledgement must match the operation fence, snapshot, active layout, exact issuer cutoff/checkpoint, and capacity proof. Stale, partial, unavailable, conflicting, or Redis-derived evidence keeps the scope fenced. Rebind must complete before normal command intake resumes.
