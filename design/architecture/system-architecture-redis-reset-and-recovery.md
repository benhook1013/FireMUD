...
7. **Apply the session-policy phase for the affected regions**
   ...
   - The complete [canonical preserved-session rebind predicate](./system-architecture-redis.md#session-and-region-binding-contract) requires all of the following before the region bridge may recreate a binding:
     ...
     - Current Account authority for the exact account and tenant, including `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `authorityTuple.membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable, plus current `membershipVersion`, entitlement, revocation, and the complete exact `outboxCheckpoints` set for all applicable authority streams. Fresh Account-authenticated evidence remains mandatory for every current authority predicate, the committed activation lease, and the fresh delegation JWT. If the Account entitlement read alone is unavailable while Account can still make the rest of the current authority and lease decision, the existing protected same-binding continuity fallback may supply only the entitlement input for its bounded five-minute window; it never supplies authority, revocation, checkpoint, lease, or token evidence. Every other missing, stale, contradictory, regressed, or gapped value fails closed. A previously accepted bundle may describe the episode being reconciled, but it never substitutes for current Account validation, the committed activation lease, or the fresh delegation JWT.
