...
7. **Apply the session-policy phase for the affected regions**
   ...
   - During the gap between reset and successful rebind, each actually attempted command or admission command with valid identity and an established command record must converge to command-record status `TERMINAL`, `executionOutcome = REGION_REBIND_REQUIRED`, and a non-applied gameplay result rather than treating `session:game:*` or pre-auth transport context advisory fields as region-local authority. Missing, invalid, or unauthenticated requests record only recovery/session evidence and do not synthesize a command record or identity. `REGION_REBIND_REQUIRED` terminalizes that command/admission attempt, not the connected socket or gameplay-session record; socket closure or session invalidation requires a separate explicit policy or auth/expiry transition.
