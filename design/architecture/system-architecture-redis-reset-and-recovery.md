...
7. **Apply the session-policy phase for the affected regions**
   ...
   - The rebind phase enumerates the Game Session-owned durable `GameplayBindingInventory`, not Redis keys. After the pause/creation fence, recovery records an immutable `inventorySnapshotRevision` and reads every `ACTIVE`, `PROVISIONAL`, and unresolved account/issuer repair row in that repeatable snapshot, including the exact `bindingRef`, `bindingGeneration`, issuer/layout identity, lifecycle state, reservation entries, and repair obligations. A narrow region- or tenant-scoped snapshot may acknowledge binding-local recovery only. It cannot acknowledge issuer-wide coverage; that requires a separate issuer-wide inventory and admission fence under the canonical ops-access contract. Until that issuer-wide proof exists, affected issuer admission remains fenced. Every post-fence transition must carry both the operation fence and a later `coverageGeneration`; a missing row, unavailable snapshot, duplicate identity, or unresolved binding/reservation obligation keeps the scope fenced.
   - For each inventory row, recovery validates the corresponding `session:game:*` record and rebuilds the region binding and all applicable bounded indexes through their owned CAS/lease protocols. Issuer coverage uses the durable active-layout pointer and finite partition range only when the separate issuer-wide inventory and admission fence are in force; a reset never changes `issuerIndexPartitionCount` in place. Each partition is read back and receives a durable acknowledgement only after all expected inventory rows, exact reservation identities and lifecycle/fence/revision evidence, repairs, and removals are accounted for under one exact tuple carrying both `operationId`/maintenance operation fence and a later `coverageGeneration`, plus the applicable issuer-wide `inventorySnapshotRevision`, coverage fence, active layout version/partition identity/count, exact `issuer/<issuerId>` cutoff checkpoint including issuer generation, `issuanceFence`, and complete applicable `outboxCheckpoints`, plus `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. The per-partition tuple must prove both local and aggregate capacity; aggregate-only proof cannot authorize recovery. A narrow-scope operation cannot publish an issuer-partition acknowledgement. Redis `SCAN`, an empty partition, or a partial session list is never an active-binding or reservation inventory or recovery proof.
