...
6. **Rebuild Account authority and token projections**
   ...
   - Recovery is not an Account projection writer. Region- and tenant-scoped resets preserve Account-owned records but still require idempotent re-projection and exact-generation validation; a cluster reset verifies the Account repair/reset cutover that preceded physical cleanup, then registers replacement issued-token projections and proves exact-token validation before representative-region smoke.
   - Missing, stale, malformed, mismatched, or ambiguous authority, token, generation, or durable freshness evidence keeps the operation fenced and prevents session-policy application.
