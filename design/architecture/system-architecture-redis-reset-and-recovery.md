...
| `tick-events:{tenantRegionTag}` and `tick-events-offset:{tenantRegionTag}` | Coordination | **Reset-tolerant** | Tick event streams and consumer offsets are dropped; observers re-establish their baselines from the gRPC heartbeat and domain state. | Tick event streams are best-effort coordination structures that power “every N ticks” schedulers and reconnection hints; correctness derives from the committed tick timeline `(region_epoch, tickId)` and PostgreSQL-ledgered effects, not from retaining all past events. |
