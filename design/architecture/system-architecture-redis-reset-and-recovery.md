...
- **Cluster‑scoped reset** – affects an entire Coordination Redis deployment:
  ...
  - Establishes a protected-traffic quarantine and invokes the Account-owned repair/reset cutover. That durable cutover advances the issuer authority generation before reset cleanup may reopen traffic; the Redis reset may then drop all Account-owned issued-token registry records, regardless of any gameplay-session preservation option, so protected calls require reauthentication and new token issuance. Physical deletion is cleanup, not the authorization boundary.
...
The `--preserve-sessions` option has one narrow meaning: it controls whether tenant-scoped gameplay session and bootstrap-context records are retained. It never changes issued-token registry retention. Region and tenant resets preserve Account-owned `session:auth:token:<tokenHash>` records. A cluster reset first closes protected admission and completes the Account repair/reset cutover, then drops those records as cleanup; this ordering is independent of whether gameplay sessions are preserved.

For any cluster reset or cold start that can drop `session:auth:*`, the required ordering is: close protected admission; complete the Account restore/reset cutover and advance the issuer authority generation; clear or quarantine the Redis token records and rebuild the current generation projection; then register replacement issued-token records and prove them before reopening traffic. A missing issued-token registry record is a denial condition for every protected route that requires that record. Validators must not recreate a record from a still-valid JWT, and a valid signature, `exp`, or gameplay/session key is not sufficient authority; unresolved cutover or projection state keeps admission closed.
...
6. `coordination-maintenance rebind-sessions --scope tenant --tenant 7b3b074e-d597-4e9b-b96f-4f5946d26120 --region-epoch-map ... --maintenance-lock-token <token>` recreates region-authoritative binding keys for preserved, still-valid sessions while retaining their `session:game:*` and current `sessionctx:*` records/indexes; Account-owned `session:auth:token:<tokenHash>` records and authority generations remain retained, and rebind validates current identity, membership, and revocation before gameplay admission.
...
4. The maintenance workflow wipes Coordination Redis for the cluster, including old Account-issued token registry records, replay markers, remaining leases, queues, timers, retries, remote follow-up hints, and observer streams; this is physical cleanup after the Account cutover, not the authorization boundary. Immediately after replay-marker continuity is discarded, the replay controller persists a new `QUARANTINED` readiness record with an advanced `replayAdmissionFence` and the reset detection cutoff. Account issues no replacement gameplay-connect token and Gateway admits no first-party gameplay handshake against absent or pre-reset replay readiness.
...
6. `RebuildAndProveAccountIssuerGenerationProjection(--scope cluster --maintenance-lock-token <token>)` rebuilds the current issuer-generation projection from the durable Account cutover and proves it, then `InitializeRegionMeta(--scope cluster --region-epoch-map ... --current-tick-id -1 --current-tick-state APPLIED --current-tick-terminal-at-ms <resetTimeMs>)` re-establishes the full per-region Redis meta baseline. `ProveReplayDomainQuarantine` then waits through at least the maximum gameplay-connect lifetime (`30 seconds`) plus two configured clock-skew intervals and proves a disposable marker write with the threshold-satisfying `WAITAOF` acknowledgement `DURABLE_REPLAY_CONSUME_ACK`; any failure preserves `QUARANTINED`.
7. Only after the replay-domain quarantine proof succeeds may Account register replacement issued-token records under the rebuilt current issuer generation and prove exact-token validation against the complete applicable authority snapshot. If replacement issuance is intentionally deferred, only public reauthentication and token-issuance endpoints may reopen; every registry-gated protected route remains closed until a newly issued replacement record passes that proof. If an explicitly documented cluster-preserve session policy is used, `RebindRegionSessions(--scope cluster --region-epoch-map ...)` runs before smoke checks; the default cluster reset invalidates gameplay sessions and skips this step. `RunPostResetSmokeCheck(--scope cluster)` samples at least one representative region per executor/shard group and must pass before protected admission can reopen.
8. `ResumeTicks(--scope cluster)` resumes normal scheduling on the new epochs. Registry-gated protected admission reopens only after the Account projection proof, replacement-token registration/proof, and post-reset smoke check succeed; reauthentication-only mode is not protected-admission reopen.
...
- A cluster reset that clears replay markers must also advance `replayAdmissionFence`, complete the full lifetime-plus-skew quarantine, and prove durable replay consumption before replacement-token registration or smoke checks. None of the Account projection gates substitutes for this replay-domain proof.
