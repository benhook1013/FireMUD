...
- **Tenant‑scoped reset** – affects a single `tenantId`:
  ...
  - Preserves only `session:game:{tenantInstanceTag}:<sessionId>` records when operators explicitly choose `--preserve-sessions`; affected tenant-owned `session:game:index:*` projections are derived and may be dropped, but must reconcile before release. Untagged global account and issuer indexes are not narrow reset targets; replacement requires their complete owner-defined durable inventory, coverage proof, and exact readback before release. Pre-auth transport context is not covered by session preservation and is invalidated or rebuilt.
...
- Every region- or tenant-scoped reset records exactly one session policy: `--preserve-sessions` or `--invalidate-sessions`; the choice is never inferred from scope. Cluster scope accepts only explicit `--invalidate-sessions`. The policy applies only to affected `session:game:*` records; pre-auth transport context is always invalidated or rebuilt, while clearing `tick:{tenantRegionTag}:*` also clears region-authoritative `tick:{tenantRegionTag}:session-binding:<entityId>` keys.
...
| `session:game:index:character:{tenantInstanceTag}:<characterId>` | Coordination | **Reset-sensitive with mandatory binding reconciliation** | The character-to-session uniqueness entry is dropped; takeover, resume, and new admission using that character remain fenced until Game Session rebuilds the exact entry from the durable binding inventory and validates it against the session record. | Instance-scoped and shard-local with the session CAS. Rebuild removes stale entries, recreates missing entries, and rejects conflicting identities; an empty key is not proof that the character is available. |
...
| `session:game:index:account-tenant:{tenantInstanceTag}:<accountId>` | Coordination | **Reset-sensitive with mandatory binding reconciliation** | The instance-qualified account lookup is dropped; account-in-instance resume, takeover, and bounded revocation remain fenced until Game Session rebuilds it from the durable inventory and the matching session/index obligations are acknowledged. | Instance-scoped and part of the shard-local session CAS. Reconciliation is limited to the instance tag, but every entry must still match the complete `{tenantId, gameInstanceId, sessionId}` binding identity and current lifecycle. |
| `session:game:index:tenant:{tenantInstanceTag}` | Coordination | **Reset-sensitive with mandatory binding reconciliation** | Instance-wide active-session lookup is dropped; instance revocation, inspection, and repair remain fenced until Game Session rebuilds and reads back the instance index from durable inventory. | Instance-scoped and shard-local. Reconciliation must not use the global account index as a substitute or admit a binding from instance-local key presence alone. |
| `session:game:index:realm-grant:{tenantInstanceTag}:<worldSlug>:<realmSlug>:<accountId>` | Coordination | **Reset-sensitive with mandatory binding reconciliation** | Grant-gated realm lookup is dropped; realm admission and grant-driven revocation remain fenced until Game Session rebuilds the exact instance-qualified entries and validates current Account grant authority. | Instance-scoped and shard-local. Redis entries are only bounded lookup evidence; Account-owned grant state remains authoritative, and stale or cross-tenant entries are removed during reconciliation. |
...
- Target state only: once the bounded recovery controller and cluster scope are implemented and proved, it plans a **cluster-scoped reset** as part of a controlled maintenance window.
  - The future controller executes the canonical reset and [complete canonical pre-wipe gates and physical-dedication proof](./system-architecture-redis-ops-access.md#canonical-pre-wipe-gates) at cluster scope with explicit `--invalidate-sessions`; this scenario does not restate those gates or the recovery controller's order. Replacement startup verification remains separate post-reset evidence.
