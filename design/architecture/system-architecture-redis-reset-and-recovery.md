...
7. **Apply the session-policy phase for the affected regions**
   ...
   - The rebind phase enumerates the Game Session-owned durable `GameplayBindingInventory`, not Redis keys. After the pause/creation fence, recovery records an immutable `inventorySnapshotRevision` and reads every `ACTIVE`, `PROVISIONAL`, and unresolved account/issuer repair row in that repeatable snapshot, including the exact `bindingRef`, `bindingGeneration`, issuer/layout identity, lifecycle state, reservation entries, and repair obligations. A narrow region- or tenant-scoped recovery is the durable parent and may acknowledge binding-local recovery only. It cannot map the issuer layout, perform issuer Redis or capacity repair, or acknowledge issuer-wide coverage from its local snapshot; that requires a separate issuer-wide inventory and admission fence under the canonical ops-access contract. When issuer coverage is required, the parent creates or consumes exactly one linked child per affected issuer, bound by `parentRecoveryOperationId`, `issuerCoverageOperationId`, the parent `operationFence`, the child `issuerCoverageOperationFence`, and the child's distinct `coverageFence`. Until that child proves complete coverage, affected issuer admission remains fenced. Every post-fence transition must carry the complete parent/coverage evidence carrier, including linked operation lifecycles, `inventorySnapshotRevision`, and a later `coverageGeneration`; a missing row, unavailable snapshot, duplicate identity, or unresolved binding/reservation obligation keeps the scope fenced.
   - For `session:game:index:account:<accountId>`, the parent links one canonical Game Session-owned account-wide recovery child operation for that account. The durable link records `parentRecoveryOperationId` and its current lifecycle, `accountCoverageOperationId` and its current lifecycle, the parent `operationFence`, the child's `operationFence`, and the child's distinct `coverageFence`; the child also records the immutable complete-account `inventorySnapshotRevision` and later `coverageGeneration`. The child enumerates every tenant's complete `GameplayBindingInventory`, including all active/provisional and unresolved account-index obligations, rebuilds the one full account key, reads back the exact expected generation-qualified full-key member set, and publishes one acknowledgement only after that exact readback and complete inventory coverage. The parent validates the linked identities, lifecycles, both fences, snapshot revision, and later coverage generation, then consumes only the child acknowledgement after the child reaches its coverage-proven lifecycle. A region- or tenant-local snapshot, affected-member readback, empty key, `SCAN`, aggregate count, or narrow repair acknowledgement cannot prove account-wide coverage; a failed, paused, superseded, aborted, stale, or ambiguous child leaves the parent fenced.
   - For each inventory row, recovery validates the corresponding `session:game:*` record and rebuilds the region binding and all applicable bounded indexes through their owned CAS/lease protocols. Only a standalone issuer-wide operation or the linked issuer-wide child may use the durable active-layout pointer and finite partition range, map every binding through the canonical issuer mapping, repair issuer Redis fields or reservation capacity, read back partitions, and publish issuer-partition acknowledgements; a reset never changes `issuerIndexPartitionCount` in place. Before every child layout mapping, repair, readback, or acknowledgement, it validates the exact `parentRecoveryOperationId`, `issuerCoverageOperationId`, parent and child lifecycles, parent `operationFence`, child `issuerCoverageOperationFence`, and child `coverageFence`. Each partition is read back and receives a durable acknowledgement only after all expected inventory rows, exact reservation identities and lifecycle/fence/revision evidence, repairs, and removals are accounted for under the linked child tuple carrying both operation identities and both maintenance fences, later `coverageGeneration`, applicable issuer-wide `inventorySnapshotRevision`, coverage fence, active layout version/partition identity/count, exact `issuer/<issuerId>` cutoff checkpoint including issuer generation, `issuanceFence`, and complete applicable `outboxCheckpoints`, plus `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. The parent consumes only the complete finite set after the child reaches its coverage-proven lifecycle; it never publishes an issuer-partition acknowledgement. The per-partition tuple must prove both local and aggregate capacity; aggregate-only proof cannot authorize recovery. Redis `SCAN`, an empty partition, or a partial session list is never an active-binding or reservation inventory or recovery proof.
   ...
   - The complete [canonical preserved-session rebind predicate](./system-architecture-redis.md#session-and-region-binding-contract) requires all of the following before the region bridge may recreate a binding:
     - A complete target `schemaVersion=2` authenticated gameplay session payload with the exact `issuanceFence`, `rebindHandle`, `continuityBindingExpiresAt`, `membershipVersion`, `authorityTuple.membershipAuthorityGeneration`, and complete `outboxCheckpoints` set; a `schemaVersion=1` or incomplete record is storage-only and cannot be rebound.
     ...
     - Current Account authority for the exact account and tenant, including `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `authorityTuple.membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable, plus current `membershipVersion`, revocation, and the complete exact `outboxCheckpoints` set for all applicable authority streams. Fresh Account-authenticated evidence remains mandatory for every current authority predicate, the committed activation lease, and the fresh delegation JWT. The public-production predicate requires current caller-bound membership and current entitlement: unavailable membership authority is `AUTH_UNAVAILABLE`, missing membership is `JOIN_REQUIRED`, entitlement authority unavailable is `ENTITLEMENT_UNAVAILABLE`, and authoritative billing denial is `TENANT_BILLING_BLOCKED` (a nonbilling public-admission denial remains distinct). The private/playtest predicate requires existing caller-bound membership and the current Account-owned grant for the exact `{tenantId, worldSlug, realmSlug}`; it never creates membership, returns `JOIN_REQUIRED`, or treats entitlement-only/public-production outcomes as grant proof. Unavailable membership or grant authority remains an availability failure; missing/invalid membership and missing/invalid grant remain separate private/playtest admission denials. The entitlement predicate is therefore used only for public production. Its bounded same-binding fallback is permitted only for an outage of that entitlement-read-only call, after every other current Account predicate, the committed activation lease, and the fresh Account-authenticated delegation-JWT check have succeeded; it supplies only the public-production entitlement input. At `fallbackAt`, its deadline is `min(continuityBindingExpiresAt, resumeDeadline, fallbackAt + 5 minutes, fallbackAt + session_expiration_ms)`, where `session_expiration_ms = min(FIREMUD_AUTH_SESSION_EXPIRATION_MS, 300000)`. It never supplies authority, revocation, checkpoint, lease, token, membership, or private-realm grant evidence and cannot be used after that deadline. Every other missing, stale, contradictory, regressed, or gapped value fails closed.
...
- Any reset scope that preserves gameplay sessions but clears region-local `tick:{tenantRegionTag}:session-binding:*` keys must complete the Account authority/token projection rebuild and verified durable freshness gate before applying the preserve policy or starting rebind. Rebind must consume only the exact Game Session-owned `GameplayBindingInventory` snapshot and `inventorySnapshotRevision` captured behind the operation's admission/creation fence; every post-fence transition and every partition acknowledgement must carry the complete parent/coverage evidence carrier, including the parent operation fence, distinct coverage fence, linked operation IDs/lifecycles, and later `coverageGeneration`, and must match the full operation/snapshot/coverage tuple, active layout, exact issuer cutoff/checkpoint, reservation evidence, and per-partition capacity tuple defined by the [canonical recovery evidence contract](./system-architecture-redis-ops-access.md#canonical-active-binding-recovery-evidence-contract). Stale, partial, unavailable, conflicting, or Redis-derived evidence keeps the scope fenced. The applicable rebind admission predicate is realm-specific: public production requires current membership plus entitlement and preserves `AUTH_UNAVAILABLE`, `JOIN_REQUIRED`, `ENTITLEMENT_UNAVAILABLE`, `TENANT_BILLING_BLOCKED`, and nonbilling public-admission denial; private/playtest requires existing membership plus the current exact realm grant and preserves separate membership-denied and grant-denied outcomes without `JOIN_REQUIRED`. If the applicable predicate fails, recovery must not emit `REGION_REBIND_REQUIRED`; only after it passes may a failed rebind record `TERMINAL` / `REGION_REBIND_REQUIRED` for an actually attempted command with valid identity and an established command record, never for a missing or invalid identity. Rebind must complete before normal command intake resumes.
...
The six gameplay index families below are Game Session-owned derived coordination state, not authorization state. A reset may drop them only after the affected scope is paused and fenced. For `--preserve-sessions`, Game Session rebuilds and reconciles every applicable index from the durable `GameplayBindingInventory` before preserved-session rebind or normal admission; for `--invalidate-sessions`, the index rebuild still completes before new bindings are admitted. Every post-fence transition and acknowledgement must carry and validate the parent `operationFence`, distinct `coverageFence`, linked operation IDs/lifecycles, exact `inventorySnapshotRevision`, and later `coverageGeneration`; the original snapshot fence/generation cannot substitute for any member of that tuple. Missing, partial, stale, conflicting, or ambiguous index, reservation, or coverage evidence keeps the scope fenced.

### Global Index-Family Recovery Rule

A region- or tenant-scoped recovery parent cannot acknowledge coverage for either global index family from its local snapshot. For `session:game:index:account:<accountId>`, the parent may repair only affected tenant-qualified references and must leave unrelated global members untouched; account-wide coverage requires the linked canonical Game Session-owned account-wide recovery child, its complete all-tenant `GameplayBindingInventory` snapshot, parent/child lifecycle and linkage, parent and child operation fences, distinct `coverageFence`, later `coverageGeneration`, and exact full-key readback consumed by the parent only after the child reaches its coverage-proven lifecycle. For `session:game:index:issuer:*`, the parent may consume only the complete result of the linked issuer-wide child; that child alone maps the complete issuer inventory, performs issuer Redis and reservation-capacity repair, and publishes the finite partition acknowledgements under its own `issuerCoverageOperationFence`, distinct `coverageFence`, and later `coverageGeneration`. A cluster reset rebuilds complete account and issuer families from their complete authoritative inventories. A local empty key, partial partition set, aggregate count, or narrow snapshot is never global coverage proof.
