...
| `inventory:<tenantId>:<playableStateNamespaceId>:*` | Cache | **Reset-tolerant** | Namespace-backed cached inventory/container aggregates are flushed; subsequent reads recompute views from PostgreSQL and repopulate Redis. | The cache scope is `(tenantId, playableStateNamespaceId, containerId)`; `playableStateScope` is separately validated evidence and is not inferred from the key. Inventories remain authoritative in PostgreSQL; cache loss affects latency only. |
| `inventory:room-ground:<tenantId>:<gameInstanceId>:<roomInstanceId>:*` | Cache | **Reset-tolerant** | Cached explicitly instance-scoped S3 synthetic room-ground holders are flushed; Entity Management rebuilds them under the owner-defined instance lifecycle when needed. | Each key's complete cache scope is `(tenantId, gameInstanceId, roomInstanceId/containerId)`; the reset wildcard covers every holder in the selected room and is not represented by a fabricated namespace key. Redis is only a cache and does not become durable authority. |
| `character-cache:<tenantId>:<playableStateNamespaceId>:*` | Cache | **Reset-tolerant** | Namespace-backed cached character graphs are dropped; hot paths fall back to Entity Management and repopulate caches on demand. | The cache scope is `(tenantId, playableStateNamespaceId, characterId)`; character state remains authoritative in PostgreSQL and `playableStateScope` remains separately validated evidence. |
