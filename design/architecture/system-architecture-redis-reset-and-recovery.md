...
| `gateway:connect-token:jti:<jti>` and replay-readiness generation | Coordination | **Reset-sensitive, fail-closed** | Gateway rejects all new first-party gameplay handshakes after reset or uncertain failover until a shared quarantine of at least the maximum connect-token lifetime plus clock skew has completed. | Consumed markers are security-critical and non-evicting. The quarantine ensures all tokens issued before replay-state continuity was lost expire before a new readiness generation opens admission. Existing admitted WebSockets are unaffected. |
