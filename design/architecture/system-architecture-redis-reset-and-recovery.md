...
- **Region‑scoped reset** – affects a single `<tenantId, gameInstanceId, regionId>`:
  ...
  - Does not clear Account-owned issued-token registry records, authority-generation projections, or exact-input `session:connect-token:*` issuance-result projections because these are not region-scoped coordination state; all three families are preserved and remain subject to Account-owned recovery/readback if a projection is unavailable.
...
- **Tenant‑scoped reset** – affects a single `tenantId`:
  ...
  - Does not clear Account-owned `session:auth:token:<tokenHash>` records, authority-generation projections, or exact-input `session:connect-token:*` issuance-result projections. Region- and tenant-scoped resets preserve all three families because they are not region-scoped coordination state; if any preserved projection is missing or untrusted, Account must repair and read it back before the affected operation can release. A tenant reset may terminate affected gameplay bindings, but it must not turn a tenant-region coordination reset into account-wide token revocation.
...
- **Cluster-scoped reset** – affects an entire Coordination Redis deployment:
  ...
  - Establishes protected-traffic quarantine and pauses and epoch-fences all affected regions. For a reset that can delete the Account-owned `session:auth:token:*`, `session:auth:generation:*`, or `session:connect-token:*` prefixes, Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence complete before the destructive reset; after reset, Account repairs each affected projection family from its durable operation/authority records and durably reads back the exact token, generation, and connect-token issuance-result records before release. Replay admission remains quarantined while `replayAdmissionFence` advances, at least the maximum gameplay-connect lifetime plus two configured clock-skew intervals elapse from the recorded detection cutoff, and durable replay-consume proof completes before replacement issued-token projections are registered. Only after registration are replacement records validated against the exact-token registry and exercised by representative smoke; Gateway remains blocked throughout and until the complete recovery release gate succeeds. Physical deletion is cleanup, not the authorization boundary.
...
- Account authority, token-projection, connect-token projection, replay, session-policy, and immutable evidence gates remain fail-closed. Reset recovery invokes the Account-owned repair/replacement workflow and verifies its durable result; it never writes or reconstructs Account-owned projections itself. When a destructive wipe can remove the Account-owned `session:auth:token:*`, `session:auth:generation:*`, or `session:connect-token:*` prefixes, Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence complete before the wipe, while Account repairs each affected projection family and durably reads back the exact records after reset. Physical deletion is cleanup, not authorization: cleanup removes only old or stale projections, replay quarantine and durable replay-consumption proof complete before replacement registry records are registered, and those replacement records remain after registration. A poisoned or ambiguous token, generation, or connect-token projection cannot authorize protected traffic.
