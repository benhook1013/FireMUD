...
- **Cluster‑scoped reset** – affects an entire Coordination Redis deployment:
  ...
  - Drops all Account-owned issued-token registry records, regardless of any gameplay-session preservation option, so protected calls require reauthentication and new token issuance.
...
The `--preserve-sessions` option has one narrow meaning: it controls whether tenant-scoped gameplay session and bootstrap-context records are retained. It never changes issued-token registry retention. Region and tenant resets preserve Account-owned `session:auth:token:<tokenHash>` records, while a cluster reset drops them; this is independent of whether gameplay sessions are preserved.

For any cluster reset or cold start that can drop `session:auth:*`, a missing issued-token registry record is a denial condition for every protected route that requires that record. Validators must not recreate a record from a still-valid JWT, and a valid signature, `exp`, or gameplay/session key is not sufficient authority. Before player-facing protected traffic can reopen, Account must complete its restore/reset cutover and advance the issuer authority generation, rebuild the current generation projection, and prove that new issued-token records are registered; unresolved cutover or projection state keeps admission closed.
...
- All coordination state is reset; ticks restart from a clean slate. Tenant-scoped resets preserve Account-owned `session:auth:token:*` records while invalidating tenant gameplay/session-context state unless the command explicitly records `--preserve-sessions`; cluster resets drop issued-token records, deny missing-record token use, and invalidate gameplay sessions by default until Account completes the required issuer-generation cutover.
