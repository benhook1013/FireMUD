...
7. **Apply the session-policy phase for the affected regions**
   ...
   - The rebind phase enumerates the Game Session-owned durable `GameplayBindingInventory`, not Redis keys. After the pause/creation fence, recovery records an immutable `inventorySnapshotRevision` and reads every `ACTIVE`, `PROVISIONAL`, and unresolved account/issuer repair row in that repeatable snapshot, including the exact `bindingRef`, `bindingGeneration`, issuer/layout identity, lifecycle state, and repair obligations. Later transitions are captured by the operation fence or a later coverage generation; a missing row, unavailable snapshot, duplicate identity, or unresolved obligation keeps the scope fenced.
   - For each inventory row, recovery validates the corresponding `session:game:*` record and rebuilds the region binding and all applicable bounded indexes through their owned CAS/lease protocols. Issuer coverage uses the durable active-layout pointer and finite partition range; a reset never changes `issuerIndexPartitionCount` in place. Each partition is read back and receives a durable acknowledgement only after all expected inventory rows, reservations, repairs, and removals are accounted for under one exact tuple: `operationId` and maintenance operation fence, `inventorySnapshotRevision`, coverage fence and `coverageGeneration`, active layout version/partition identity/count, exact `issuer/<issuerId>` cutoff checkpoint including issuer generation, `issuanceFence`, and complete applicable `outboxCheckpoints`, plus `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. The per-partition tuple must prove both local and aggregate capacity; aggregate-only proof cannot authorize recovery. Redis `SCAN`, an empty partition, or a partial session list is never an active-binding inventory or recovery proof.
...
| `session:game:index:issuer:{issuerIndexLayoutTag}:<issuerId>:<partitionId>` | Coordination | **Reset-sensitive with mandatory issuer-coverage rebuild** | Issuer cutoff and issuer-wide active-binding lookup remain fenced until Game Session rebuilds every finite active-layout partition, proves capacity and exact readback, and acknowledges coverage for the operation fence or a later `coverageGeneration`. | Global per issuer across tenants and partitioned by the immutable issuer layout, not tenant-tagged. Region- and tenant-scoped resets reconcile only affected binding references and never flush a whole issuer partition; cluster reset rebuilds all partitions before protected admission reopens. |
