...
- **Region‑scoped reset** – affects a single `<tenantId, gameInstanceId, regionId>`:
  ...
  - Does not clear Account-owned issued-token registry records, live legacy account/tenant auth projections, authority-generation projections, or exact-input `session:connect-token:*` issuance-result projections because these are not region-scoped coordination state; these families are preserved and remain subject to Account-owned recovery/readback if a projection is unavailable.
...
- **Tenant‑scoped reset** – affects a single `tenantId`:
  ...
  - Does not clear Account-owned `session:auth:token:<tokenHash>` records, live legacy account/tenant auth projections, authority-generation projections, or exact-input `session:connect-token:*` issuance-result projections. Region- and tenant-scoped resets preserve these families because they are not region-scoped coordination state; if any preserved projection is missing or untrusted, Account must repair and read it back before the affected operation can release. A tenant reset may terminate affected gameplay bindings, but it must not turn a tenant-region coordination reset into account-wide token revocation.
...
- **Cluster-scoped reset** – affects an entire Coordination Redis deployment:
  ...
  - Establishes protected-traffic quarantine and pauses and epoch-fences all affected regions. For a reset that can delete the Account-owned live legacy `session:auth:account:*` and `session:auth:tenant:*` projections, target `session:auth:token:*` and `session:auth:generation:*` projections, or `session:connect-token:*` prefixes, Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence complete before the destructive reset; after reset, Account repairs each affected projection family from its durable operation/authority records and durably reads back the exact legacy auth, token, generation, and connect-token issuance-result records before release. Replay admission remains quarantined while `replayAdmissionFence` advances, at least the maximum gameplay-connect lifetime plus two configured clock-skew intervals elapse from the recorded detection cutoff, and durable replay-consume proof completes before replacement issued-token projections are registered. Only after registration are replacement records validated against the exact-token registry and exercised by representative smoke; Gateway remains blocked throughout and until the complete recovery release gate succeeds. Physical deletion is cleanup, not the authorization boundary.
...
Every region- or tenant-scoped reset requires exactly one explicit gameplay-session policy: `--preserve-sessions` or `--invalidate-sessions`. That choice applies only to the scope-selected `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` records. Derived gameplay indexes are independent of that choice: affected tenant-owned `session:game:index:*` projections may be dropped under either policy, then must be reconciled and read back before release. Untagged global account and issuer indexes are not narrow reset targets; replacement requires their complete owner-defined durable inventory, coverage proof, and exact readback before the maintenance lock can release. Pre-auth transport context is never preserved by `--preserve-sessions` and is always invalidated or rebuilt. Live legacy `session:auth:account:*` and `session:auth:tenant:*` projections, target `session:auth:token:*` and `session:auth:generation:*` authority projections, and the separate `session:connect-token:*` issuance-result projection family remain outside session policy. Region- and tenant-scoped resets preserve those records and the connect-token projections, and no session-policy option advances, deletes, or recreates them. If a preserved Account projection is missing or untrusted, Account must perform the exact durable repair and readback before release; recovery must not reconstruct a connect-token issuance result from a JWT or request fields alone. Every cluster-scoped reset requires explicit `--invalidate-sessions` and rejects `--preserve-sessions`: it first closes protected admission and pauses and epoch-fences the affected regions; when the reset can delete any Account-owned auth or connect-token prefix, Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence complete before the destructive reset, Account repairs each affected projection family and durably reads back the exact records after reset, and replay quarantine plus durable replay-consumption proof complete before replacement issued-token registration, so old gameplay bindings cannot remain authoritative through that cutover.

For any cluster reset or cold start that can drop the Account-owned live legacy `session:auth:account:*` and `session:auth:tenant:*` projections, target `session:auth:token:*` and `session:auth:generation:*` projections, or `session:connect-token:*` prefixes, the required ordering is: close Gateway protected admission and command intake; complete the internal pause/maintenance-lock and epoch-fencing prerequisites and the owner-defined pre-wipe gates, including Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence; perform the destructive reset; after reset, request Account-owned repair or replacement for each affected projection family and verify its durable result and exact readback; advance `replayAdmissionFence`, keep replay quarantine closed for at least the maximum gameplay-connect lifetime plus two configured clock-skew intervals from the recorded detection cutoff, and prove durable replay consumption; only then register replacement issued-token records. If a Redis projection mutation is unavailable, the Account-owned idempotent durable generation-mutation/repair operation is the fallback for auth generations, while the exact-input durable issuance-result operation is the fallback for connect-token projections; each may be retried by its same operation identity and its projection remains fail-closed until Account reprojects and verifies the committed durable value. Reset tooling and downstream services must never synthesize or advance Redis generations or reconstruct connect-token results locally. An unavailable or ambiguous Account durable mutation keeps the operation fenced and aborts destructive handoff or release. Validate each replacement against the exact-token registry and run representative smoke after registration, while Gateway remains blocked throughout. A missing issued-token registry record is a denial condition for every protected route that requires that record, and a missing or ambiguous connect-token result is a denial/fail-closed condition for the corresponding handshake. Validators must not recreate a record from a still-valid JWT, request fields, or a gameplay/session key; unresolved cutover, replay proof, exact-token validation, connect-token projection, or smoke state keeps admission closed.
...
- **Replay-marker cleanup gate:** the shared replay domain must prove continuity for the readiness/fence record and both `jti` replay and browser-revocation `deny:jti` marker families, advance `replayAdmissionFence` when continuity is lost, and remain quarantined for at least the maximum gameplay-connect lifetime (30 seconds) plus two configured clock-skew intervals from the recorded detection cutoff. It must retain and read back surviving or legacy replay and deny markers exactly through their original acceptance windows, then record guarded cleanup/readback and durable consume proof before replacement replay admission can proceed. This replay evidence authorizes only the replay/deny-marker cleanup boundary; it does not authorize Account token-projection deletion.
...
- Account authority, token-projection, connect-token projection, replay, session-policy, and immutable evidence gates remain fail-closed. Reset recovery invokes the Account-owned repair/replacement workflow and verifies its durable result; it never writes or reconstructs Account-owned projections itself. When a destructive wipe can remove the Account-owned live legacy `session:auth:account:*` and `session:auth:tenant:*` projections, target `session:auth:token:*` and `session:auth:generation:*` projections, or `session:connect-token:*` prefixes, Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence complete before the wipe, while Account repairs each affected projection family and durably reads back the exact records after reset. Physical deletion is cleanup, not authorization: cleanup removes only old or stale projections, replay quarantine and durable replay-consumption proof complete before replacement registry records are registered, and those replacement records remain after registration. A poisoned or ambiguous token, generation, or connect-token projection cannot authorize protected traffic.
...
- **Cold start** (empty Coordination Redis because the data directory/PVC is missing, wiped, or corrupted)
  ...
  - For a Redis-only cold start with PostgreSQL still authoritative, empty-start recovery is not a separate operator path:
    ...
    - The Account-owned cold-start consequence includes the live legacy `session:auth:account:*` and `session:auth:tenant:*` projections as well as the target `session:auth:token:*`, `session:auth:generation:*`, and `session:connect-token:*` families; none are restored from Redis, and all remain fail-closed until Account cutover, repair, and exact readback qualify.
...
Entity cache rows below intentionally distinguish namespace-backed aggregates from explicitly instance-scoped S3 holders. Reset tooling may enumerate the complete affected namespace or instance/room scope and drop both families, but Redis remains only a disposable cache: PostgreSQL or the owning Entity lifecycle contract remains authoritative, and `playableStateScope` is separately validated evidence rather than a cache-key or reset-authority dimension.
...
| `session:game:index:realm-grant:{tenantGameplayTag}:<grantScopeId>` | Coordination | **Reset-sensitive with mandatory binding reconciliation** | Lifecycle-bound private/playtest grant lookup is dropped; reset tooling enumerates the complete expected tuple set from the exact Account v1 inventory, obtains every key through the shared `grant/v1` lifecycle-qualified realm-grant index builder, and keeps realm admission and grant-driven revocation fenced until Game Session rebuilds and reads back those exact entries under current Account grant authority. | `grantScopeId` is the exact canonical scope ID for the complete Account-owned tuple. Reset and reconciliation never concatenate world/realm selectors or other tuple fields independently. The entries are tenant-scoped, shard-local bounded lookup evidence; Account state remains authoritative, and stale, wrong-lifecycle, or cross-tenant entries are removed. Public-production and unscoped bindings have no entry. |
...
| `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>` | Coordination | **Reset-sensitive, Account-owned issuance-result projection; approved untagged exception** | Region- and tenant-scoped coordination resets preserve this exact Account-owned issuance-result record; a cluster reset or cold start may discard it only after Account-owned authority/token cutover, immutable pre-wipe evidence, protected-admission quarantine, projection repair, and durable readback qualify. If the record is missing or untrusted, Account must recover the exact result or fail closed until the applicable connect-scope/request identity expires; it must not reconstruct the result from a JWT or silently mint a replacement. | This family is the bounded Redis issuance-result projection for Account's connect-token issuance result. Its complete identity is tenant, account, connect scope, and request; it is intentionally untagged because it is single-key Account issuance state rather than gameplay/session aggregate state or a multi-key Lua participant. It is outside gameplay session policy and is not a substitute for Account's durable issuance operation/response-envelope authority. |
...
| `gateway:connect-token:{gateway-connect-token-replay-v1}:jti:<jti>`, `gateway:connect-token:{gateway-connect-token-replay-v1}:deny:jti:<jti>`, and `gateway:connect-token:{gateway-connect-token-replay-v1}:readiness` | Coordination | **Reset-sensitive, fail-closed** | These shared Gateway replay-domain keys are an explicit exception to tenant/region key tagging and are untouched by region- or tenant-scoped resets. The readiness value carries `replayAdmissionFence` and all three keys share the literal hash tag `{gateway-connect-token-replay-v1}` for one Redis Cluster slot. If replay markers or browser-revocation deny markers are dropped or become untrusted, or fence/readiness continuity is uncertain, the reset advances the fence and rejects all new first-party gameplay handshakes for at least the maximum gameplay-connect lifetime (`30 seconds`) plus two configured clock-skew intervals. Reopen requires the same quarantine and disposable-marker proof: one fresh invocation of the canonical consume script must return `1` and create a disposable replay marker, followed by one same-connection `WAITAOF` local/replica durability acknowledgement meeting the configured thresholds. A `0` or `-1` result is non-mutating and cannot supply a second write acknowledgement; any cleanup is a separate guarded cleanup write/readback and is not part of `DURABLE_REPLAY_CONSUME_ACK`. This gate covers loss of either replay or deny markers and is in addition to, and cannot bypass, the Account cutover, issuer-generation projection rebuild/proof, and post-reset smoke ordering. The current generic Cache-bound writer emits only the untagged replay marker; it has no current deny-marker, readiness, or replay-fence writer. Migration requires quarantine, exact old replay-prefix inventory/readback, named Coordination client/ACL cutover, target-key readback, and retention or explicit cleanup of old markers through their original acceptance window. Resetting this replay state alone does not close already admitted WebSockets. A cluster reset closes affected gameplay sockets under its required explicit invalidation policy. | Consumed replay and deny markers are security-critical and non-evicting. The fence is advanced only after old marker state is dropped or rejected; quarantine and the single durability acknowledgement must complete before the readiness record reopens admission. Existing admitted WebSockets are unaffected by replay-state reset alone. |
...
| `inventory:<tenantId>:<playableStateNamespaceId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Namespace-backed cached inventory/container aggregates are flushed; subsequent reads recompute views from PostgreSQL and repopulate Redis. | The cache scope is `(tenantId, playableStateNamespaceId, containerId)`; `playableStateScope` is separately validated evidence and is not inferred from the key. Inventories remain authoritative in PostgreSQL; cache loss affects latency only. |
| `inventory:room-ground:<tenantId>:<gameInstanceId>:<roomInstanceId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Cached explicitly instance-scoped S3 synthetic room-ground holders are flushed; Entity Management rebuilds them under the owner-defined instance lifecycle when needed. | Each key's complete cache scope is `(tenantId, gameInstanceId, roomInstanceId/containerId)`; the reset wildcard covers every holder in the selected room and is not represented by a fabricated namespace key. Redis is only a cache and does not become durable authority. |
| `character-cache:<tenantId>:<playableStateNamespaceId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Namespace-backed cached character graphs are dropped; hot paths fall back to Entity Management and repopulate caches on demand. | The cache scope is `(tenantId, playableStateNamespaceId, characterId)`; character state remains authoritative in PostgreSQL and `playableStateScope` remains separately validated evidence. |
| `world-dynamic:<tenantId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Cached dynamic world aggregates are flushed; subsequent reads recompute views from PostgreSQL. | World topology/dynamic state remains authoritative in PostgreSQL; resets may increase load temporarily. |
| `room:<tenantId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Cached room topology snapshots are dropped; callers reload rooms from PostgreSQL and repopulate caches. | Used for LOOK/navigation snapshots; resets never affect canonical topology in PostgreSQL. |
| `view:room-look:<tenantId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Cached rendered room views are dropped; Game Session recomputes views on demand for affected rooms. | Strictly Class B, TTL-only caches; correctness-critical flows (combat, visibility, movement) do not read from this prefix, and Game Session is the sole writer/reader of these keys. |
| `chat:say:<tenantId>:*`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:*`, legacy `chat:whisper:<tenantId>:<accountId>`, `chat:tell:<tenantId>:*`, `chat:guild:<tenantId>:*`, `chat:city:<tenantId>:*`, `chat:account:<tenantId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Target and live legacy chat projections are cleared; reads use or fall back to authoritative Social/PostgreSQL history where the communication type promises durable player history, while non-durable projections may rebuild or remain absent. Reset/TTL loss may create gaps or non-contiguous windows only in transient Redis projections and cannot create a durable-history entitlement gap. `chat:city` is a current legacy TTL projection, not durable history and not `SHOUT`. | `WHISPER` reset scope is the complete gameplay recipient namespace `(tenantId, playableStateNamespaceId, characterId)`, and reset cleanup includes the live legacy `chat:whisper:<tenantId>:<accountId>` prefix alongside the target prefix until migration completes. Resets drop recent in-memory history but do not lose persisted moderation logs where required; the legacy prefix must not be used for history/refill across same-account characters or namespaces. No `SHOUT` Redis/reset family exists until a selected profile publishes its type/storage/topology contract. |
| `script-scheduler:{tenantRegionTag}:lastTickId` (target-only, once implemented) | Coordination | **Reset-tolerant (target-only, once implemented)** | Once implemented, Automation scheduler treats the next heartbeat as its baseline and may re-scan due interval boundaries, but durable trigger-instance uniqueness prevents duplicate logical trigger creation. Until then, no current reset operation is defined for this unimplemented target projection. | Target-state Automation scheduler checkpoint for “every N ticks” triggers; once implemented, losing it causes the scheduler to re-establish its baseline from the heartbeat stream while PostgreSQL-backed trigger-instance rows remain the de-duplication boundary. |
| `automation:timer:{tenantRegionTag}` (target-only, once implemented) | Coordination | **Reset-tolerant (target-only, once implemented)** | Once implemented, Automation timer indexes for affected regions are discarded and rebuilt from durable schedules, trigger-instance rows, and heartbeat progress. Until then, no current reset operation is defined for this unimplemented target projection. | Target-state region-scoped coordination index for script timers/intervals. Once implemented, entries must remain instance-aware in payload and rebuild logic (`gameInstanceId`, and plugin identifiers when applicable) even though the Redis key is region-scoped for slotting/locality. |
| `automation:queue:{tenantInstanceTag}:*`, `automation:quota:<tenantId>:*`, `automation:tenant-budget:<tenantId>:tier:<tier>`, `automation:test:quota:<tenantId>:script:<scriptId>:tenant`, `automation:test:quota:<tenantId>:script:<scriptId>:principal:<principalKey>`, `automation:test:capacity:<tenantId>:*`, `automation:test:capacity:cluster*`, `automation:readiness:capacity:<tenantId>:*`, `automation:readiness:capacity:cluster*` | Cache/Rate-Limit | **Reset-tolerant** | Queued work, quota windows, and capacity reservations restart from an empty state; durable automation work/readiness state remains authoritative. The service does not rebuild the one-minute test quota windows or ten-minute readiness leases; new requests acquire fresh disposable reservations. | Best-effort buffers, counters, and owner-token leases; resets clear them but do not affect authoritative state. Repeated resets or lease expiry may temporarily relax test fairness or readiness concurrency, but must not change which durable work eventually runs. Only the Automation service's Cache/Rate-Limit ACL may access these families; they are not Coordination Redis scheduler/tick keys. |
...
- An operator used `redis-cli` or a raw script to mutate `tick:*`, `timer:*`, `retry:*`, `remote:*`, `session:game:*`, `session:auth:account:*`, `session:auth:tenant:*`, `session:auth:token:*`, `session:auth:generation:*`, `session:connect-token:*`, or `tick-executor-lease:*`.

Recommended actions:

- Treat the affected scope as “coordination state may be inconsistent”.
- Select the smallest valid domain-specific recovery workflow for every mutated prefix rather than assuming a region or tenant reset can repair all keys. Region-local coordination keys use the [Canonical Coordination Reset Sequence](./system-architecture-redis-operations.md#canonical-coordination-reset-sequence) with an explicit session-policy choice. Account-owned `session:auth:account:*`, `session:auth:tenant:*`, `session:auth:token:*`, `session:auth:generation:*`, or `session:connect-token:*` edits require Account-owned exact projection repair/readback or cluster reset, and replay-marker edits require replay quarantine, fencing, and durable consume acknowledgement. For `session:connect-token:*`, Account SQL `connect_token_issuance_operation` plus its encrypted response envelope is the exact-result authority; Redis is only the reset-sensitive projection/cache. A missing, changed, or ambiguous connect-token issuance-result projection must deny the corresponding handshake and cannot be reconstructed from a JWT or request fields. Because those target workflows are not yet shipped end to end, current operators must preserve evidence, keep all affected scopes fenced, and use the [Current Operator Fallback](#current-operator-fallback) instead of attempting a reset.
...
- All coordination state is reset; ticks restart from the new epoch baseline. The required explicit `--invalidate-sessions` policy keeps protected admission fenced; Account-owned authority/token/connect-projection cutover and immutable pre-wipe handoff evidence complete before the wipe, Account repairs each affected live legacy `session:auth:account:*` and `session:auth:tenant:*`, target `session:auth:token:*` and `session:auth:generation:*`, and `session:connect-token:*` projection family and durably reads back exact records after reset, and replay quarantine plus durable replay-consumption proof complete before replacement registration. Physical deletion is cleanup only for old or stale projections; replacement registry records remain after registration, and fresh authentication/play is required after owner-defined release. A missing or ambiguous connect-token result remains fail-closed. Cluster scope rejects `--preserve-sessions`. By contrast, a tenant-scoped reset preserves Account-owned auth and connect-token projection families while its `session:game:*` policy is determined by the explicitly recorded session policy; pre-auth transport context is always invalidated or rebuilt.
