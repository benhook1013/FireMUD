...
7. **Apply the session-policy phase for the affected regions**
   ...
   - The complete [canonical preserved-session rebind predicate](./system-architecture-redis.md#session-and-region-binding-contract) requires all of the following before the region bridge may recreate a binding:
     ...
     - Current Account authority for the exact account and tenant, including `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `authorityTuple.membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable, plus current `membershipVersion`, revocation, and the complete exact `outboxCheckpoints` set for all applicable authority streams. Fresh Account-authenticated evidence remains mandatory for every current authority predicate, the committed activation lease, and the fresh delegation JWT. The entitlement predicate uses the fresh Account entitlement read when available. A fallback is permitted only for an outage of that entitlement-read-only call, after every other current Account predicate, the committed activation lease, and the fresh Account-authenticated delegation-JWT check have succeeded; it supplies only the entitlement input. At `fallbackAt`, its deadline is `min(continuityBindingExpiresAt, resumeDeadline, fallbackAt + 5 minutes, fallbackAt + session_expiration_ms)`, where `session_expiration_ms = min(FIREMUD_AUTH_SESSION_EXPIRATION_MS, 300000)`. It never supplies authority, revocation, checkpoint, lease, or token evidence and cannot be used after that deadline. Every other missing, stale, contradictory, regressed, or gapped value fails closed. For public-production recovery, the current membership/entitlement admission predicate is evaluated before any rebind outcome: unavailable membership authority is `AUTH_UNAVAILABLE`, missing membership is `JOIN_REQUIRED`, entitlement authority unavailable is `ENTITLEMENT_UNAVAILABLE`, and an authoritative entitlement denial is `TENANT_BILLING_BLOCKED`. For private/playtest recovery, both existing membership and the current grant are required; recovery never creates membership.
   ...
   - Only when all Account identity, token, authority, membership, lease, and delegation-JWT predicates have succeeded, with entitlement satisfied by either a fresh read or the qualifying bounded fallback above, may a solely non-security region rebind gap, such as a missing or stale region binding, epoch, lease, or bridge result, leave the socket connected but not gameplay-admitted to that region until fresh `LOGIN` / `PLAY` succeeds. During that gap, recovery first evaluates the current public-production membership/entitlement admission predicate. An unavailable membership authority produces `AUTH_UNAVAILABLE`; missing membership produces `JOIN_REQUIRED`; unavailable fresh entitlement without a qualifying fallback produces `ENTITLEMENT_UNAVAILABLE`; authoritative entitlement denial produces `TENANT_BILLING_BLOCKED`. If that predicate fails, recovery must not emit `REGION_REBIND_REQUIRED`. Only after it passes, and only for an actually attempted command or admission command with valid identity and an established command record, may recovery converge to command-record status `TERMINAL`, `executionOutcome = REGION_REBIND_REQUIRED`, with a non-applied gameplay result. Missing, invalid, or unauthenticated requests record only recovery/session evidence and do not synthesize a command record or identity. `REGION_REBIND_REQUIRED` terminalizes that command/admission attempt, not the connected socket or gameplay-session record.
...
- Any reset scope that preserves gameplay sessions but clears region-local `tick:{tenantRegionTag}:session-binding:*` keys must complete the Account authority/token projection rebuild and verified durable freshness gate before applying the preserve policy or starting rebind. Rebind must consume only the exact Game Session-owned `GameplayBindingInventory` snapshot and `inventorySnapshotRevision` captured behind the operation's admission/creation fence; every post-fence transition and every partition acknowledgement must carry both that operation fence and a later `coverageGeneration`, and must match the full operation/snapshot/coverage tuple, active layout, exact issuer cutoff/checkpoint, reservation evidence, and per-partition capacity tuple defined by the [canonical recovery evidence contract](./system-architecture-redis-ops-access.md#canonical-active-binding-recovery-evidence-contract). Stale, partial, unavailable, conflicting, or Redis-derived evidence keeps the scope fenced. Recovery first evaluates the current public-production membership/entitlement predicate: unavailable membership authority is `AUTH_UNAVAILABLE`, missing membership is `JOIN_REQUIRED`, entitlement unavailability is `ENTITLEMENT_UNAVAILABLE`, and authoritative entitlement denial is `TENANT_BILLING_BLOCKED`; if the predicate fails, recovery must not emit `REGION_REBIND_REQUIRED`; only after it passes may a failed rebind record `TERMINAL` / `REGION_REBIND_REQUIRED` for an actually attempted command with valid identity and an established command record, never for a missing or invalid identity. Rebind must complete before normal command intake resumes.
