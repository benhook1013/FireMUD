...
- The full `coordination-maintenance recover --mode reset` operation and its internal pause, epoch/reset, ledger-reconciliation, command-convergence, metadata, smoke-check, and release phases remain the target-state operator model; it should not be read as fully implemented tooling in this repository today. Those internal phases are not separate public commands; continuation and abort use the supported controls for the same durable operation.
...
- **Cold start** (empty Coordination Redis because the data directory/PVC is missing, wiped, or corrupted)
  ...
  - For a Redis-only cold start with PostgreSQL still authoritative, empty-start recovery is not a separate operator path:
    - Operators must run the same bounded `coordination-maintenance recover --mode reset --scope ...` operation defined above. Its internal phases include pause/fencing, epoch bump, Redis clearing, ledger and command convergence, metadata initialization, preserved-session rebind where applicable, smoke verification, and success release.
