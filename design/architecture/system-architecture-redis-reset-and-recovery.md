...
1. **Internal pause-and-lock phase for the chosen scope**
   ...
   - This pause step is complete only once the scope reaches the control-plane `PAUSED` state defined in `system-architecture-redis-ops-access.md`: no executor in the target scope is allowed to create new durable tick batches or new Redis coordination state under the old epoch. The resulting `scope_paused_and_locked` evidence must identify the same durable `operationId` and server-issued `maintenanceLockToken` that authorize every later reset phase; a paused scope under another operation or lock is not sufficient.
