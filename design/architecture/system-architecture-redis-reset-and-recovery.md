...
7. **Apply the session-policy phase for the affected regions**
   ...
   - The complete canonical preserved-session rebind predicate requires all of the following before the region bridge may recreate a binding:
     ...
     - An exact `session:auth:token:<tokenHash>` registry record that is present, active, unrevoked, unexpired, and matches the session's exact `jti`, `tokenGeneration`, account identity, canonical profile, audience, and registry `iat`/`exp` values, with the verified JWT's required `nbf` also valid.
