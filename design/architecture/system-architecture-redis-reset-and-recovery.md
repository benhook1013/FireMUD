...
| `chat:say:<tenantId>:*`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:*`, legacy `chat:whisper:<tenantId>:<accountId>`, `chat:tell:<tenantId>:*`, `chat:guild:<tenantId>:*`, `chat:city:<tenantId>:*`, `chat:account:<tenantId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Target and live legacy chat projections are cleared; subsequent reads fall back to PostgreSQL or rebuild windows from persisted history where the communication type promises durable player history. CITY durable history may fall back to PostgreSQL after reset, and clients must tolerate gaps and non-contiguous windows. | `WHISPER` reset scope is the complete gameplay recipient namespace `(tenantId, playableStateNamespaceId, characterId)`, and reset cleanup includes the live legacy `chat:whisper:<tenantId>:<accountId>` prefix alongside the target prefix until migration completes. Resets drop recent in-memory history but do not lose persisted moderation logs where required; the legacy prefix must not be used for history/refill across same-account characters or namespaces. |
