...
| `chat:say:<tenantId>:*`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:*`, `chat:tell:<tenantId>:*`, `chat:guild:<tenantId>:*`, `chat:account:<tenantId>:*` | Cache/Rate-Limit | **Reset-tolerant** | Target short-lived chat projections are cleared; subsequent reads fall back to PostgreSQL or rebuild windows from persisted history. | `WHISPER` reset scope is the complete gameplay recipient namespace `(tenantId, playableStateNamespaceId, characterId)`; resets drop recent in-memory history but do not lose persisted moderation logs where required. The live account-keyed whisper prefix remains an implementation migration gap and must not be used for history/refill across same-account characters or namespaces. Clients must tolerate gaps and non-contiguous windows after resets. |
