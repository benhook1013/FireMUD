...
| Pre-auth transport context | Coordination | **Reset-sensitive** | Every scope records `--preserve-sessions` or `--invalidate-sessions` explicitly. Preserved pre-auth transport context is retained only under the former and invalidated under the latter. | Canonical pre-auth transport context is not region-local gameplay authority; preserved context must still pass authenticated rebind validation before gameplay admission resumes. The current implementation mapping is recorded in Implementation Notes. |
