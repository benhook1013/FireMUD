...
7. **Run the post-reset smoke check**
   ...
   - Any smoke tick exercised by the recovery or `continueRecovery` path is synthetic maintenance traffic only. It is not the first real tick, must not be exposed to players, and must not authorize player ingress or real `tickId=0` admission.
...
8. **Resume ticks on the new epoch**
   ...
   - Through `AWAITING_RESUME`, `RESUME_AUTHORIZED`, and the internal release phase, the traffic fence remains active: player traffic and real `tickId=0` remain blocked. Only successful internal release may clear the fence and reopen normal admission.
...
2. If the operation pauses for an external infrastructure step or controller restart, run `coordination-maintenance continue-recovery --operation-id <operationId> --expected-phase ready_to_reopen --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`. It reconciles only into `AWAITING_RESUME` and cannot select or skip phases.
3. Run public `coordination-maintenance resume --operation-id <operationId> --expected-phase awaiting_resume --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`; only the internal release phase may then reopen the scope after `finalized`.
...
3. Run public `coordination-maintenance resume --operation-id <operationId> --expected-phase awaiting_resume --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`; only the internal release phase may then reopen the scope after `finalized`.
4. If the operator aborts, call `coordination-maintenance release-lock --operation-id <operationId> --scope tenant --tenant 7b3b074e-d597-4e9b-b96f-4f5946d26120 --maintenance-lock-token-file <permissioned-token-file> --reason <reason> --evidence-ref <evidenceRef>`. The scope remains fenced and does not reopen.
...
3. If the operation pauses for an external infrastructure step or controller restart, run `coordination-maintenance continue-recovery --operation-id <operationId> --expected-phase ready_to_reopen --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`. It reconciles only into `AWAITING_RESUME`; run public `coordination-maintenance resume --operation-id <operationId> --expected-phase awaiting_resume --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`, which must record `RESUME_AUTHORIZED` before the internal release phase can reopen the cluster. If the operator aborts, run `coordination-maintenance release-lock --operation-id <operationId> --scope cluster --maintenance-lock-token-file <permissioned-token-file> --reason <reason> --evidence-ref <evidenceRef>`; it retains quarantine and the fence, and neither control can bypass the Account, replay, or smoke proofs.
...
3. If it pauses for an external step, operators run `coordination-maintenance continue-recovery --operation-id <operationId> --expected-phase ready_to_reopen --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`; it reconciles into `AWAITING_RESUME` and does not release the fence. Operators then run public `coordination-maintenance resume --operation-id <operationId> --expected-phase awaiting_resume --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>`; the first committable tick in epoch `14` begins only after the recover operation's internal release reaches `finalized`. If they abort, they run the audited `coordination-maintenance release-lock --operation-id <operationId> --scope region --tenant 7b3b074e-d597-4e9b-b96f-4f5946d26120 --game-instance 9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78 --region R7 --maintenance-lock-token-file <permissioned-token-file> --reason <reason> --evidence-ref <evidenceRef>`.
