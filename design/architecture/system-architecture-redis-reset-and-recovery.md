...
7. **Apply the session-policy phase for the affected regions**
   ...
   - During the gap between reset and successful rebind, recovery first evaluates the current public-production membership/entitlement admission predicate. An unavailable membership authority produces `AUTH_UNAVAILABLE`; missing membership produces `JOIN_REQUIRED`; unavailable fresh entitlement produces `ENTITLEMENT_UNAVAILABLE`; authoritative entitlement denial produces `TENANT_BILLING_BLOCKED`. If that predicate fails, recovery must not emit `REGION_REBIND_REQUIRED`. Only after it passes, and only for an actually attempted command or admission command with valid identity and an established command record, may recovery converge to command-record status `TERMINAL`, `executionOutcome = REGION_REBIND_REQUIRED`, with a non-applied gameplay result. Missing, invalid, or unauthenticated requests record only recovery/session evidence and do not synthesize a command record or identity. `REGION_REBIND_REQUIRED` terminalizes that command/admission attempt, not the connected socket or gameplay-session record; socket closure or session invalidation requires a separate explicit policy or auth/expiry transition.
