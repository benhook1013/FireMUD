...
| `session:game:index:character:{tenantGameplayTag}:<playableStateNamespaceId>:<playableStateScope>:<characterId>` | Coordination | **Reset-sensitive with mandatory binding reconciliation** | The target `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` character-to-session uniqueness entry is dropped; takeover, resume, and new admission using that character remain fenced until Game Session rebuilds the exact entry from the durable binding inventory and validates the value/evidence `{sessionId, gameInstanceId, bindingGeneration}` against the session record. | Tenant-scoped and namespace/scope-qualified, with the shard-local session CAS. Rebuild removes stale entries, recreates missing entries, and rejects conflicting identities; an empty key is not proof that the character is available. The existing `<gameInstanceId>:<characterId>` index key is current implementation drift, not the target key. |
