...
- **Tenant‑scoped reset** – affects a single `tenantId`:
  ...
  - Preserves `session:game:*` only when operators explicitly choose `--preserve-sessions`.
  - Does not clear Account-owned issued-token registry records or authority-generation projections. A tenant reset may terminate affected gameplay bindings, but it must not turn a tenant-region coordination reset into account-wide token revocation.
...
Worked example: region-scoped reset for `<tenantId=T1, gameInstanceId=G1, regionId=R7>`

1. `PauseTicks(--scope region --tenant T1 --game-instance G1 --region R7)` rejects new command intake and stops new batch creation for `R7`.
2. Control plane bumps `region_epoch` for `(T1, G1, R7)` from `12` to `13` in PostgreSQL.
3. `RunScopedCoordinationReset(--scope region --tenant T1 --game-instance G1 --region R7)` clears `tick:{tenantRegionTag}:*`, `timer:{tenantRegionTag}`, `retry:{tenantRegionTag}`, and `tick-executor-lease:{tenantRegionTag}` for `R7`.
4. `ReconcileTickLedger(--scope region --tenant T1 --game-instance G1 --region R7 --old-region-epoch 12)` drives old-epoch `SCHEDULED` rows to `APPLIED` or `ABANDONED`, and `ConvergeCommandRecords(...)` moves accepted-but-unbound commands to `executionOutcome = LOST_BEFORE_STAGING` with default `gameplayResult = NOT_APPLIED`.
5. `InitializeRegionMeta(--scope region --tenant T1 --game-instance G1 --region R7 --region-epoch 13 --current-tick-id -1 --current-tick-state APPLIED --current-tick-terminal-at-ms <resetTimeMs>)` re-establishes the full Redis-side meta baseline.
6. `RebindRegionSessions(--scope region --tenant T1 --game-instance G1 --region R7 --region-epoch 13)` recreates region-authoritative `tick:{tenantRegionTag}:session-binding:<entityId>` entries for preserved sessions that still validate.
7. `RunPostResetSmokeCheck(--scope region --tenant T1 --game-instance G1 --region R7)` proves a fresh lease can be acquired and a sample tick can stage/clear.
8. `ResumeTicks(--scope region --tenant T1 --game-instance G1 --region R7)` allows `tickId=0` in epoch `13` to begin.
...
2. Control plane bumps `region_epoch` for every `(T1, gameInstanceId, regionId)` in PostgreSQL.
...
Worked example: normal failover for `<tenantId=T1, gameInstanceId=G1, regionId=R7>`

1. Redis leader fails, but the replacement node replays intact AOF state and restores `tick:{tenantRegionTag}:pending`, `tick:{tenantRegionTag}:meta`, and the region lease key for `(T1, G1, R7)`.
...
Worked example: cold start for `<tenantId=T1, gameInstanceId=G1, regionId=R7>`

1. Coordination Redis starts empty after loss of its data directory, while PostgreSQL still shows `RegionStatus(regionEpoch=13, lastCommittedTickId=41)` for `(T1, G1, R7)`.
...
3. Control plane bumps `region_epoch` for the affected scope, for example from `13` to `14` for `(T1, G1, R7)`.
...
| `gateway:connect-token:jti:<jti>` and `replayAdmissionFence` | Coordination | **Reset-sensitive, fail-closed** | These shared Gateway replay-domain keys are an explicit exception to tenant/region key tagging and are untouched by region- or tenant-scoped resets. Gateway rejects all new first-party gameplay handshakes after a shared-domain reset or uncertain failover until a shared quarantine of at least the maximum gameplay-connect lifetime (`30 seconds`) plus two configured clock-skew intervals has completed. Resetting this replay state alone does not close already admitted WebSockets. A broader cluster reset separately invalidates gameplay sessions by default and closes their sockets through the gameplay-session reset flow. | Consumed markers are security-critical and non-evicting. The quarantine ensures all tokens issued before replay-state continuity was lost expire before `replayAdmissionFence` opens admission again. Existing admitted WebSockets are unaffected by replay-state reset alone. |
