...
7. After steps 1-6 have validated the immutable evidence and established the exact durable controller at `ready_to_reopen`, require `PREFLIGHT-BACKUP-002=pass`. Only then call `continueRecovery(operationId, expectedPhase, evidenceRef)` against that controller with the exact operation identity, target boundary, and immutable evidence reference. The controller idempotently reconciles its internal `ready_to_reopen -> releasing -> finalized` phases, applies and observes quarantine release, and only afterward permits player traffic; checked-in actual-recovery and traffic-open projections are exported after the finalized transition.
