...
- `PREFLIGHT-JWT-INTERIM-001`, `PREFLIGHT-JWT-002`, and `PREFLIGHT-JWT-ROTATION-001` are not yet implemented and remain fail-closed, so the checked-in legacy Secret-backed signing plus public `jwt-jwks` ConfigMap deployment mode, shared-HMAC drift, and private-key distribution topology cannot satisfy player-facing JWT readiness.
