...
7. Require `PREFLIGHT-BACKUP-002=pass`, then call `continueRecovery(operationId, expectedPhase, evidenceRef)` against that controller with the exact `ready_to_reopen` operation identity, target boundary, and immutable evidence reference. The controller idempotently reconciles its internal `ready_to_reopen -> releasing -> finalized` phases, applies and observes quarantine release, and only afterward permits player traffic; checked-in actual-recovery and traffic-open projections are exported after the finalized transition.
