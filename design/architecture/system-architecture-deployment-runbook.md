...
5. Invoke the canonical target-state `coordination-maintenance recover --mode reset --scope cluster --invalidate-sessions` operation defined in [Canonical Coordination Reset Sequence](./system-architecture-redis-operations.md#canonical-coordination-reset-sequence). The cluster selector is the complete Coordination Redis deployment/gameplay-environment boundary; do not add a restore-specific operation flag. No recovery scope is currently implemented and proven, so tooling must reject this invocation until cluster recovery has the required end-to-end proof rather than silently approximating it with a narrower scope. Coordination-maintenance owns pause/fencing, epoch advancement, Coordination Redis reset, and durable tick/command reconciliation. Account Service attests invalidation of restored Account authority and `game-session-account-delegation` lineages; Game Session attests invalidation of restored gameplay bindings and sessions. Recovery only records and verifies those owner attestations; it is not a second Account or Game Session invalidation writer. Record an empty Redis result only as observed post-reset evidence; key absence is not direct destructive proof and cannot authorize recovery or resume.
...
7. Request that Account Service rebuild and verify the issuer, account, tenant, and membership generation projections plus issued-token projections from Account durable authority; recovery must await the durable cutover result and verify its returned freshness/generation evidence before restore-safe smoke or recovery continuation. Refresh the environment secret-compliance record, complete every pre-release recovery and confidentiality control, capture immutable `initialCatchupHighWater` and final-cutover `restoreHighWater`, and replay the erasure intervals in order: first `(artifactErasureHighWater, initialCatchupHighWater]`, then `(initialCatchupHighWater, restoreHighWater]`. Persist `erasureOverlayReconciliation.sequenceDispositions[]` with exactly one canonical owner disposition for every sequence in the final interval before installing `restoreHighWater` as the consumer cursor; aggregate counts are insufficient. Complete replay-domain quarantine/fence and durable consume proof where applicable. Complete and record the immutable pre-release evidence while quarantine remains active, then establish the durable recovery controller at `ready_to_reopen`. Require the applicable traffic-open preflight gate to pass before calling `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with the exact operation identity, `expectedPhase=ready_to_reopen`, and evidence reference; it must reconcile only into `AWAITING_RESUME`. Then call public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=awaiting_resume` to record `RESUME_AUTHORIZED`; only the internal release phase may reconcile `releasing -> finalized`, apply and observe quarantine release, open traffic, and export the checked-in evidence projection.
