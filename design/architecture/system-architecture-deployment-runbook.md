...
3. Select and record the custody mode before applying workloads. The current legacy Secret-backed `jwt-signing-keys`/`jwt-jwks` wiring may be observed as non-authorizing wiring evidence only; it does not establish interim or target custody. For `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK`, provision the per-environment asymmetric Account signing bundle (`jwt-signing-keys`) mounted only into Account Service and the public validation resource (`jwt-jwks`) consumed by every validator. For target non-exportable signer mode, provision the approved signer reference and signer-health evidence, the Account-owned public `jwt-jwks` resource consumed by Account and every validator, and proof that no application workload mounts or receives private signing material.
