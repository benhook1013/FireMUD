...
5. Invoke the canonical `coordination-maintenance recover --mode reset --scope <scope> --operation restore --invalidate-sessions` operation, with `<scope>` resolving to the exact environment-wide restore boundary. Coordination-maintenance owns pause/fencing, epoch advancement, Coordination Redis reset, and durable tick/command reconciliation. Account Service attests invalidation of restored Account authority and `game-session-account-delegation` lineages; Game Session attests invalidation of restored gameplay bindings and sessions. Recovery only records and verifies those owner attestations; it is not a second Account or Game Session invalidation writer. Record an empty Redis result only as observed post-reset evidence; key absence is not direct destructive proof and cannot authorize recovery or resume.
