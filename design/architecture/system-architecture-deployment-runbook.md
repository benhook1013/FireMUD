...
8. **Post-Deployment Checks**
   ...
   - Only after the durable recovery/deployment controller reaches `finalized` and its quarantine-release postconditions are observed may operators run the login/session checks described in `../developer-workflows/login-session-smoke-tests.md` or confirm Web client/Telnet player connectivity. `finalized` is the sole player-traffic release boundary; smoke success, readiness, `ready_to_reopen`, `AWAITING_RESUME`, `RESUME_AUTHORIZED`, or deployment status cannot authorize exposure.
...
4. **Monitor Rollout and Run Smoke Checks**
   ...
   - For `first-live` and `reopen`, keep generic smoke limited to recovery-safe service and tick checks while traffic is quarantined; exclude login, session/reconnect, and player-connectivity checks until the controller reaches `finalized`. Run those player-facing checks only as post-finalization release verification.
