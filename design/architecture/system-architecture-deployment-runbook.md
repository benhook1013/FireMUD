...
6. For both `first-live` and `reopen`, confirm the durable actual-recovery controller is `ready_to_reopen`, has `recoveryPurpose=actual-recovery`, uses the event-matching `trafficExposure` (`player-facing-first-live` or `player-facing-reopen`), and names the exact target boundary; the isolated drill projections alone are insufficient.
7. After steps 1-6 have validated the immutable evidence and established the exact durable controller at `ready_to_reopen`, require `PREFLIGHT-BACKUP-002=pass`. The controller derives `targetBoundary` from the persisted operation scope and compares that value with the immutable evidence before continuation; `targetBoundary` is not a `continueRecovery` argument and a caller-supplied or mismatched boundary cannot narrow or widen the operation. Only then call `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` against that controller with the exact operation identity, `expectedPhase=ready_to_reopen`, and immutable evidence reference. The controller idempotently reconciles into `AWAITING_RESUME` without releasing quarantine. Call public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=awaiting_resume`; only the internal release phase may then apply and observe quarantine release, reach `finalized`, and permit player traffic. Checked-in actual-recovery and traffic-open projections are exported after the finalized transition.
...
8. **Post-Deployment Checks**
   ...
   - Only after the durable recovery/deployment controller reaches `finalized` and its quarantine-release postconditions are observed may operators run the login/session checks described in `design/developer-workflows/login-session-smoke-tests.md` or confirm Web client/Telnet player connectivity. `finalized` is the sole player-traffic release boundary; smoke success, readiness, `ready_to_reopen`, `AWAITING_RESUME`, `RESUME_AUTHORIZED`, or deployment status cannot authorize exposure.
