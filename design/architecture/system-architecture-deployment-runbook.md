...
3. Select and record the custody state before applying workloads. The one actual checked-in deployment mode is legacy Secret-backed JWT/JWKS wiring (`jwt-signing-keys` and `jwt-jwks` Secrets); it may appear in pre-apply diagnostics but is non-authorizing and is not an accepted player-facing custody state. For `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK`, establish trusted pre-apply evidence for the pre-created interim `jwt-signing-keys` Secret, the separate interim Account-owned public `jwt-jwks` ConfigMap, and the materialization controller's name-scoped RBAC. The controller may materialize or generate private slots, and later prune them, only after an Account-authorized operation; environment setup and operators never become a continuing writer. For target non-exportable signer mode, establish trusted pre-apply evidence for the approved signer reference, target Account-owned public `jwt-jwks` ConfigMap, Account publication/CAS authority, and no private-material mount or distribution. These bootstrap records prove intended bindings and custody boundaries only; post-apply live signer, public-JWKS, and validator convergence is a separate owner-produced gate.
...
4. Re-run `./dev-tools/deploy/preflight.py <environment>` and require the bootstrap, secret, bridge, Redis, external-binding, and service-discovery checks to pass, plus exactly one accepted custody proof: `PREFLIGHT-JWT-INTERIM-001` for `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK` or `PREFLIGHT-JWT-002` for target non-exportable signer custody. `PREFLIGHT-JWT-001` and `PREFLIGHT-JWKS-001` may be emitted only as legacy diagnostic wiring results; neither is accepted proof or a substitute for the selected proof. An unknown, unsupported, or not-yet-implemented state fails closed before progressing.
...
7. **Target-only recovery/reopen sequence (currently blocked):** Once the target controller exists, it would request that Account Service rebuild and verify the issuer, account, tenant, and membership generation projections plus issued-token projections from Account durable authority; recovery would await the durable cutover result and verify its returned freshness/generation evidence before restore-safe smoke or recovery continuation. The target sequence would refresh the environment secret-compliance record, complete every pre-release recovery and confidentiality control, capture immutable `initialCatchupHighWater` and final-cutover `restoreHighWater`, and replay the erasure intervals in order: first `(artifactErasureHighWater, initialCatchupHighWater]`, then `(initialCatchupHighWater, restoreHighWater]`. It would persist `erasureOverlayReconciliation.sequenceDispositions[]` with exactly one canonical owner disposition for every sequence in the final interval before installing `restoreHighWater` as the consumer cursor; aggregate counts are insufficient. It would complete replay-domain quarantine/fence and durable consume proof where applicable, record immutable pre-release evidence under the [restore-cutover evidence boundary](./system-architecture-backup-recovery-evidence-and-compliance.md#restore-cutover-evidence-boundary) while quarantine remains active, and establish the durable recovery controller at `ready_to_reopen`. The target controller would require the applicable traffic-open preflight gate before `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` reconciles only into `AWAITING_RESUME`, then public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=awaiting_resume` records `RESUME_AUTHORIZED`. Only the internal release phase may apply and observe the operation-owned release effects while traffic remains fenced, reconcile `releasing -> finalized`, and then record `status=SUCCEEDED`; only that resulting `phase=finalized` and `status=SUCCEEDED` state may expose traffic. Only after `phase=finalized` and `status=SUCCEEDED` may the exporter write the checked-in evidence projection; that post-success-finalization export cannot participate in release authorization. This step is documentation of the blocked target contract, not a current executable procedure.
...
7. After steps 1-6 have validated the immutable evidence and established the exact durable controller at `phase=ready_to_reopen` and `status=RUNNING`, require `PREFLIGHT-BACKUP-002=pass`. The controller derives `targetBoundary` from the persisted operation scope and compares that value with the immutable evidence before continuation; `targetBoundary` is not a `continueRecovery` argument and a caller-supplied or mismatched boundary cannot narrow or widen the operation. The following continuation and release sequence is target state and is not currently executable because the complete durable controller is not shipped. Until it is implemented and proved, operators must use the fail-closed [Current Operator Fallback](./system-architecture-redis-reset-and-recovery.md#current-operator-fallback) and must not attempt these controls. Once available, call `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` against that controller with the exact operation identity, `expectedPhase=ready_to_reopen`, and immutable evidence reference. The controller idempotently reconciles into `AWAITING_RESUME` without releasing quarantine. Call public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=awaiting_resume`; only the internal release phase may then apply and observe quarantine release, reach `finalized`, record `SUCCEEDED`, and permit player traffic. Checked-in actual-recovery and traffic-open files are post-release audit projections exported from the durable controller/outbox only after both `phase=finalized` and `status=SUCCEEDED`. Their repository-write acknowledgement is deliberately not part of the live release transaction and cannot authorize, block, or roll back traffic; a failed export remains a durable retryable audit backlog rather than changing the controller result.
...
2. Have Account alone reconcile and advance the production rotation operation through the mode-specific evidence required by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative): target signer-side retirement and hard-cutover evidence, or interim Account-authorized materialization-controller private-slot pruning and separate interim private/public projection evidence. The recovery controller only persists the operation and evidence, invokes the Account-owned rotation operation, and observes its returned convergence; it cannot select, promote, prune, reconcile, or advance rotation. The rotation-evidence workload is observation-only and may record public-JWKS and bounded validator results. Both modes retain Account-owned JWKS convergence, validator visibility, continuity, and rejection proof.
