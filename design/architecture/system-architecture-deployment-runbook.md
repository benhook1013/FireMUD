...
7. After steps 1-6 have validated the immutable evidence and established the exact durable controller at `ready_to_reopen`, require `PREFLIGHT-BACKUP-002=pass`. Only then call `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` against that controller with the exact operation identity, `expectedPhase=ready_to_reopen`, target boundary, and immutable evidence reference. The controller idempotently reconciles into `AWAITING_RESUME` without releasing quarantine. Call public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=awaiting_resume`; only the internal release phase may then apply and observe quarantine release, reach `finalized`, and permit player traffic. Checked-in actual-recovery and traffic-open projections are exported after the finalized transition.
