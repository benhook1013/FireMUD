...
3. **Run Preflight Policy Checks**
   ...
   - Store the preflight report artifact under `design/operations/deployments/<environment>/preflight/<deployment-ref>.json` with optional waiver record `.../<deployment-ref>.waiver.json` as defined in `design/architecture/system-architecture-deploy-preflight-policy.md`. Generate a new `deploymentEventId` for each concrete preflight/apply event; when a waiver is used, supply the same UUID through `FIREMUD_DEPLOYMENT_EVENT_ID` and the waiver record.
