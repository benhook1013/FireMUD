...
7. Refresh the environment secret-compliance record, complete every pre-release recovery and confidentiality control, record approval in the durable recovery controller, and advance it to `ready_to_reopen`. Reconcile `ready_to_reopen -> releasing -> finalized`; the controller must apply and observe the quarantine release before traffic opens, and the checked-in evidence projection is exported only afterward.
