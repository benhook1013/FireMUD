...
5. Invoke the canonical `coordination-maintenance recover --mode reset --scope <scope> --operation restore --invalidate-sessions` operation for the restored boundary. It owns pause/fencing, epoch advancement, Coordination Redis reset, session invalidation, and recovery ordering. Record an empty Redis result only as observed post-reset evidence; key absence is not direct destructive proof and cannot authorize recovery or resume.
