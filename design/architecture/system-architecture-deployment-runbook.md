...
3. Select and record the custody mode before applying workloads. Both accepted player-facing modes require the Account-owned public `jwt-jwks` ConfigMap consumed by Account Service and every validator. For `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK`, also provision the per-environment asymmetric Account signing bundle (`jwt-signing-keys`) mounted only into Account Service. For target non-exportable signer mode, provision the approved signer reference and signer-health evidence, and prove that no application workload mounts or receives private signing material. Checked-in Secret-backed JWT/JWKS wiring is legacy drift and non-authorizing evidence; it does not establish either accepted custody mode.
...
2. Run the production rotation artifact through the mode-specific evidence required by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative): target signer-side retirement and hard-cutover evidence, or interim Account-authorized private-slot pruning and private/public projection evidence. Both modes retain Account-owned JWKS convergence, validator visibility, continuity, and rejection proof.
