...
- `PREFLIGHT-JWT-INTERIM-001`, `PREFLIGHT-JWT-002`, and `PREFLIGHT-JWT-ROTATION-001` are not yet implemented and remain fail-closed, so the checked-in player-facing Kustomize legacy Secret-backed signing plus public `jwt-jwks` Secret deployment mode, shared-HMAC drift, and private-key distribution topology cannot satisfy player-facing JWT readiness. Hosted preview Helm remains a separate diagnostic ConfigMap-backed path.
...
4. **Run Preflight Policy Checks**
   ...
   - Before apply, every applicable `apply-blocking` policy check must pass. Legacy diagnostic or advisory results may be recorded without blocking only a non-player or quarantined operational apply; they are never authorization. The target player-facing contract requires operators to present exactly one authenticated, accepted `jwtCustodyProof`, but the current executable preflight does not yet validate that proof or provide player-facing authorization. Until that proof is implemented and accepted, player-facing deployment, promotion, first-live, reopen, and fresh-boundary restore must remain closed. Applicable `non-waivable-promotion-traffic-open` checks gate promotion or player-facing traffic opening.
   - Use the canonical entrypoint: `./dev-tools/deploy/preflight.py <staging|production|hobby-self-hosted>`.
   - Store the preflight report artifact under `design/operations/deployments/<environment>/preflight/<deployment-ref>/<deploymentEventId>.json`. Preflight generates a new `deploymentEventId` for each concrete run; copy that UUID into the immutable deployment record, apply within 30 minutes of report completion, and generate a new event path for every retry or re-apply. The target-state waiver record is the adjacent `<deploymentEventId>.waiver.json`; any hobby first-live waiver follows the canonical [`recovery-unverified` traffic-open schema](./system-architecture-backup-recovery-evidence-and-compliance.md#hobby-traffic-open-evidence), including its backup-compliance, fingerprint, preflight, and finalized-controller gates, and is never valid for reopen. Executable waiver use remains blocked until trusted one-time consumption authority exists.
...
2. **Run Operator Preflight**
   ...
   - For the target controller-backed first-live and reopen flow, require `PREFLIGHT-BACKUP-003=pass` before requesting controller continuation. A hobby first-live waiver may omit only the rehearsal-only restore, disposition, and rehearsal-cutover evidence described by ADR 0154; consume the canonical [`recovery-unverified` traffic-open schema](./system-architecture-backup-recovery-evidence-and-compliance.md#hobby-traffic-open-evidence), which retains its backup-compliance and fingerprint requirements, omits verified-only recovery references, and cannot authorize reopen. The preflight result is evidence for that request; it does not authorize release or open player traffic.
   ...
   - Store the preflight report at `design/operations/deployments/hobby-self-hosted/preflight/<deployment-ref>/<deploymentEventId>.json`. Waiver artifacts use the adjacent target-state path `<deploymentEventId>.waiver.json`; validation and consumption follow the canonical [`recovery-unverified` traffic-open schema](./system-architecture-backup-recovery-evidence-and-compliance.md#hobby-traffic-open-evidence), and the waiver is never valid for reopen. Executable waiver use remains blocked until trusted one-time consumption authority exists.
...
## Target Progressive Rollout and Rollback Contract

The rollout and rollback contract is [ADR 0156: risk-tiered progressive rollout with compatibility-bounded rollback](./decisions/adr-0156-risk-tiered-progressive-rollout-with-compatibility-bounded-rollback.md).

The following is target-state behavior, not a current executable deployment flow or proof claim. The current runbook uses the ordinary rolling or recreate steps above and operator-directed rollback below; it does not implement or prove canary cohorts, progressive promotion, or automatic rollback.

For ordinary multi-replica changes:

- Target: resolve the exact candidate and known-good digest sets and verify the rollback classification before apply.
- Target: use an explicit conservative rolling strategy, automated smoke/SLO observation, and pause on hard failure; readiness alone does not authorize progression.
- Target: bind each rollout attempt to one durable `rolloutOperationId`, the exact candidate digest set, target environment, and observed Deployment UID/generation, including the server-issued `resourceVersion` or equivalent owner compare-and-swap token. Every progression, pause, abort, or restoration mutation submits that token as a server-enforced precondition; a failed precondition records a stale-operation conflict and performs no mutation. Automatically restore the known-good digest set only when this precondition succeeds, the current release remains rollback-compatible, and the failure matches policy. A roll-forward-only release stops for forward remediation and never reapplies old binaries automatically.

Target: for higher-risk changes, use a bounded candidate cohort or observation window when replica count and representative traffic make that evidence meaningful; a failure stops broader progression, and success does not bypass preflight, promotion, recovery, or post-apply gates. One-replica and small deployments use a simple rolling or recreate strategy and do not claim nominal canary proof.

The manual rollback guidance below is the current operator path; it is not evidence that the target automatic rollback or canary contract is implemented.
