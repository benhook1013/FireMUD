...
2. Confirm at least one successful online PostgreSQL backup upload exists and verification proved artifact integrity and restore readability.
...
9. **Record Rollback Classification**
   ...
   - For `rollback-compatible` releases, prove reuse compatibility with the current recovery drill without copying the full record. For `drill_required` or `roll-forward-only` releases, attach `design/operations/deployments/production/backup-readiness/<deployment-ref>.json`; the `roll-forward-only` drill must start from current production database lineage and prove the exact candidate recovery tooling, service digests, migration path, config, and bindings.
   - Current implementation note: player-facing recovery readiness lacks the required environment-wide cold-start proof and executable validation, so production `roll-forward-only` releases are not approvable. Record `rollback-compatible` only when old-binary rollback is genuinely safe; otherwise stop the promotion.
