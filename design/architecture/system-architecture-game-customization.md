...
**Target-state customization model:** Customization scripts use one DSL and one sandbox, with two artifact/lifecycle roles: embedded game-owned patches follow the Game Design/Game Session pin workflow, while linked plugins remain separately versioned immutable bundles with activation metadata scoped to `(tenantId, gameInstanceId, pluginId)`. Linked plugins are not folded into the Game Session script-patch pin tuple; every activation or update resolves a `PUBLISHED`, compatibility- and policy-admitted exact `(pluginId, pluginVersionId, bindingId)` set plus applicable Automation-owned lifecycle, component/capability, and signer evidence, and instance-bound gameplay/runtime work carries the current Game Session `(scriptPatchVersion, scriptPinEpoch)`, that exact plugin tuple, and captured `pluginActivationGeneration`; stale reactivation fails closed. Tenant-readiness `onLoad` remains the pre-instance-pin exception and retains its declared readiness identity without fabricated instance fields or `scriptPinEpoch`. The [DSL lifecycle reference](./system-architecture-scripting-dsl-reference-and-lifecycle.md) owns that distinction; this document retains only the local creator/customization consequence.
...
- **Target-state script-patch lifecycle:** Scripts are versioned alongside other game data. Designers may publish a `scriptPatchVersion` like `v42-script.3` to update automation without republishing all assets; changing a script patch creates a new recorded immutable runtime tuple and requires explicit `READY`, compatibility, and Game Session pin rollout. A linked-plugin member change uses an independent per-instance plugin activation/update rollout, does not advance `scriptPinEpoch`, and requires the exact `PUBLISHED`, compatibility, lifecycle, component/capability, and signer-policy predicates plus the Automation-owned `pluginActivationGeneration` fence; stale reactivation fails closed. Hot reload must not mutate a running descriptor or follow a latest patch/plugin alias.
