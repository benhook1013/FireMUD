...
- **Target delivery:** Runtime clients fetch this manifest using its delivery URL or location recorded in the published version metadata and load published game assets directly from the CDN or through the gateway's `/assets/**` route when a local MinIO instance is used. The `/assets/**` family is exclusively published game assets; compiled first-party frontend files use the static host's separate `/frontend-assets/**` family, and neither origin serves the other's family. The Game Design Service is never queried during gameplay, and URL availability or object paths do not replace the attested release metadata.
