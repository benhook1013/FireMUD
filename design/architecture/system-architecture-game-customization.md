...
- A default world is available, but creators can define custom worlds entirely through the **Game Design Service**. They add rooms, items, and NPCs through the [world editing tools](./microservices/game-design-service/world-editing-tools.md) and canonical service-owned typed design APIs. Whole-game package import/export, round-trip JSON authoring, filesystem projects, and external Git synchronization are not current target capabilities. Stable authored identities and normalized references preserve future options without promising a portable snapshot; a later first-party copy may remain server-side orchestration. See [ADR 0110](./decisions/adr-0110-defer-whole-game-portability-and-external-authoring-formats.md).
