...
- Current live instance-bound admission carries and validates `scriptPinEpoch`, but does not yet carry the complete applicable plugin fence `(pluginId, pluginVersionId, bindingId, pluginActivationEpoch, lifecycleRevision)` or prove final-effect enforcement across every path; old-activation-epoch/lifecycle-revision rejection remains target-state behavior rather than implementation proof.
