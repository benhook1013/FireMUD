...
- Flags are defined in the Game Design Service (see [Feature Flags](./microservices/game-design-service/feature-flags.md)); **target state:** the [Logging & Admin Service](./microservices/logging-admin-service/README.md) operator surface forwards a gated toggle request to the Game Session owner. **Current implementation:** Logging & Admin has no separate live admin UI or forwarding path; its toggle ingress fails closed as unavailable.
