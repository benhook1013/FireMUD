...
`pr-preview` and `dev-demo-cluster` are non-player-facing hosted Helm environments and are outside the ordinary environment-report applicability table. The preview path validates the final trusted candidate after its allocator-selected Telnet NodePort has been injected, passing that exact value through `--expected-hosted-telnet-node-port`; omitting the option preserves the raw-render rule that hosted-controller mode cannot contain any explicit NodePort. Preview runs the `ci-static` result before Kubernetes server dry-run/apply, then runs the `operator` result only after identity readiness and runtime rollout and before smoke or success publication. The fixed dev-demo path runs the `operator` result after render and before server dry-run/apply, rejects a deploy target whose image tag does not equal its reconciled head SHA, and publishes its reconciled namespace target only after deploy and TCP smoke succeed. `PREFLIGHT-BRIDGE-001` is apply- and acceptance-blocking for the hosted preview and covers both the internal Gateway bridge and the selected direct Telnet TLS candidate; success is reported as `Gateway bridge and direct Telnet TLS alignment is valid`. The separate `helm-jwks-contract.sh` test covers preview JWKS resource and Account-mount wiring. These checks are diagnostic hosted-environment proof and do not produce player-facing preflight evidence, expected-bindings reports, promotion authority, traffic-open authorization, or the public-listener exposure-exclusivity proof reserved for target-state `PREFLIGHT-TELNET-001`.
