...
- the public `jwt-jwks` resource and public-JWKS evidence required by every player-facing custody mode, plus exactly one accepted player-facing custody proof:
  ...
  - `PREFLIGHT-JWT-001` and `PREFLIGHT-JWKS-001` are legacy diagnostic-only wiring checks, never a selectable player-facing custody mode or accepted proof,
...
- `PREFLIGHT-JWT-001` – current executable legacy Secret/path diagnostic check only. In legacy `Secret` mode, it rejects inline JWT secret configuration and checks the shared path/mount contract where every primary workload declares `FIREMUD_AUTH_JWT_SECRET_PATH` and a workload whose resolved path is under `/var/run/secrets/firemud/jwt/` mounts `jwt-signing-keys` at that root. A custom resolved private path does not imply the canonical mount. This legacy result is not selectable player-facing custody, is not the interim mounted-fallback proof, and cannot satisfy any target custody or player-facing readiness gate.
