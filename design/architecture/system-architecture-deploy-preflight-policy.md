...
`pr-preview` and `dev-demo-cluster` are non-player-facing hosted Helm environments and are outside the ordinary environment-report applicability table. Their trusted workflows use the `preflight.py hosted-bridge` form after Helm render. Preview runs its `ci-static` render result before Kubernetes server dry-run or Helm apply, then runs the operator result after the rollout has had the opportunity to consume controller-projected identity and before bootstrap, smoke, or reconciled-head publication; missing or delayed projection remains apply-blocking for acceptance. Dev-demo retains its existing ordering pending convergence with the separate hosted identity lifecycle. `PREFLIGHT-BRIDGE-001` covers both the internal Gateway bridge and the selected direct Telnet TLS render; success is reported as `Gateway bridge and direct Telnet TLS alignment is valid`. The separate `helm-jwks-contract.sh` test covers preview JWKS resource and Account-mount wiring. These checks are diagnostic hosted-environment proof and do not produce player-facing preflight evidence, expected-bindings reports, promotion authority, traffic-open authorization, or the public-listener exposure-exclusivity proof reserved for target-state `PREFLIGHT-TELNET-001`.
