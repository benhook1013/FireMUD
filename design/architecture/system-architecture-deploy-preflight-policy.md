...
- `PREFLIGHT-JWKS-001` – player-facing environments use fixed, pre-created `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap resources; Account alone has name-scoped get/update/patch authority, rotation automation has no resource write authority, projected mounts are read-only, and classpath fallback is limited to explicit local/test use.
