...
The recovery controller's public continuation contract is `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)`. `maintenanceLockToken` must be issued by the recovery controller for the durable operation and bind the operation identity, environment, affected scope, and authorized actor or workload; a caller-supplied or scope-only token is invalid. Its internal pause/lock phase is not a standalone public recovery verb. No recovery-controller RPC currently exists in the checked-in `protos/` source, so this remains a target-state contract rather than an implemented gRPC surface.

Public phase inputs use a deliberately narrower lowercase wire vocabulary than the durable controller state. `continueRecovery` accepts only `expectedPhase=ready_to_reopen`, compare-and-sets durable `ready_to_reopen`, and records `AWAITING_RESUME`; `resume` accepts only `expectedPhase=awaiting_resume`, compare-and-sets durable `AWAITING_RESUME`, and records `RESUME_AUTHORIZED`. The controller may also persist `PAUSED`, `collecting`, `releasing`, `PARTIAL_RELEASE_RECONCILING`, and `finalized`, but none of those names is a caller-selectable `expectedPhase`. API adapters and CLIs must perform this exact mapping and reject alternate capitalization, internal phases, operation statuses, and unknown values before mutation.

Preflight is a prerequisite gate, not the release authority. It validates the applicable environment, recovery, security, binding, and evidence prerequisites and may produce passing prerequisite evidence that permits an authorized operator to invoke `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` only when the live recovery controller is at `ready_to_reopen` with matching evidence. The controller derives `targetBoundary` from the persisted operation scope and compares that boundary with the target-boundary identity in preflight evidence; callers do not provide `targetBoundary`, and it is not an argument to `continueRecovery`. That continuation uses `expectedPhase=ready_to_reopen`, validates the server-issued token against the durable operation scope and identity, and may reconcile only into `AWAITING_RESUME`; the public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` then uses `expectedPhase=awaiting_resume` to record `RESUME_AUTHORIZED`. Resume must validate the same server-issued token and `evidenceRef` against the durable `AWAITING_RESUME` continuation record before any mutation; a token, scope, identity, phase, or evidence mismatch fails closed without mutation. The durable recovery controller alone owns the guarded internal `releasing -> finalized` transition and the actual quarantine/traffic release; preflight must not infer release permission from a checked-in projection or report.
...
- `PREFLIGHT-BACKUP-002` – the target production first-live or traffic-reopen gate verifies a readable environment-wide PostgreSQL backup, a successful production-equivalent `cold_start_restore` drill within the required freshness window, backup-confidentiality evidence, and the current environment-specific actual-recovery controller with `phase=ready_to_reopen` and `status=RUNNING` before traffic is opened. A `FAILED` status, any other status, or any phase other than `ready_to_reopen` fails closed. The evidence record must retain the environment-wide artifact identity/readability result and the confidentiality evidence reference together; a tenant-, game-instance-, region-, or cluster-scoped substitute must not overwrite or satisfy either environment-wide fact. The current executable has no durable controller read and therefore fails this gate closed for player-facing authorization; checked-in recovery/traffic-open JSON is emitted only after `phase=finalized` and `status=SUCCEEDED`, and is not pre-release authority.
- `PREFLIGHT-BACKUP-003` – the target hobby/self-hosted first-live or traffic-reopen gate verifies current backup-baseline compliance evidence and the current environment-specific actual-recovery controller with `phase=ready_to_reopen` and `status=RUNNING` before traffic is opened. A `FAILED` status, any other status, or any phase other than `ready_to_reopen` fails closed. The current executable has no durable controller read and therefore fails this gate closed for player-facing authorization; any static baseline result, checked-in recovery/traffic-open JSON, or caller-supplied scope/timestamp evidence is supporting evidence only and cannot authorize traffic. The checked-in recovery/traffic-open projection is exported only after `phase=finalized` and `status=SUCCEEDED` and is never a preflight input.
...
- `PREFLIGHT-BACKUP-002` is required for `production` on first-live opens and reopen-after-restore events, and `not_applicable` for routine steady-state rollouts that do not change traffic-open status. Its target implementation reads the current environment-specific actual-recovery controller at `phase=ready_to_reopen` and `status=RUNNING`, derives `targetBoundary` from the controller's persisted operation scope, compares it with the target boundary in preflight evidence, verifies event-matching traffic exposure plus a fresh controller state and a drill completed within 30 days, then emits passing prerequisite evidence that permits an authorized operator to invoke `continueRecovery(... expectedPhase=ready_to_reopen ...)`; `targetBoundary` is not added to that continuation signature. A `FAILED` status or any other phase/status combination fails closed. The continuation must reach `AWAITING_RESUME`, after which public `resume(... expectedPhase=awaiting_resume ...)` records `RESUME_AUTHORIZED`. `releasing` is never a passing preflight state. The controller remains the sole release authority and must idempotently reconcile the authorized internal release through `releasing -> finalized`, apply and observe quarantine release, and keep traffic closed on failure. The current executable fails this check closed because that controller read is not implemented. Tenant/game-instance/region/cluster pause or reset evidence may be retained as supporting maintenance evidence, but it cannot replace the readable environment-wide PostgreSQL artifact or its backup-confidentiality evidence, and checked-in projections are neither required nor sufficient authority.
- `PREFLIGHT-BACKUP-003` is required for `hobby-self-hosted` on first-live opens and reopen-after-restore events, and `not_applicable` otherwise. Its target pre-release authorization consumes current backup-baseline compliance, the general preflight report, immutable pre-release evidence, and a fresh live actual-recovery controller at `phase=ready_to_reopen` and `status=RUNNING`; `continueRecovery(... expectedPhase=ready_to_reopen ...)` must reach `AWAITING_RESUME`, and public `resume(... expectedPhase=awaiting_resume ...)` must record `RESUME_AUTHORIZED` before the internal release reaches `finalized`. A `FAILED` status or any other phase/status combination fails closed. `releasing` is never a passing preflight state. The checked-in traffic-open projection under `design/operations/deployments/hobby-self-hosted/traffic-open/<deployment-ref>/<deploymentEventId>.json` is exported only after `phase=finalized` and `status=SUCCEEDED` and is not a preflight input. The current executable fails closed for the same missing controller-backed authorization and does not implement post-finalization projection export.
