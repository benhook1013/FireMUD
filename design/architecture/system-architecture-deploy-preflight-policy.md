...
- Operator pre-apply execution (`preflight.py`) currently enforces resolved-manifest and target-environment checks for required secret/key bindings, the legacy Secret-backed JWKS resource type and Account mount, Redis role split, bridge alignment, bootstrap completeness, and external integration isolation. Its legacy JWT check requires every primary workload to declare `FIREMUD_AUTH_JWT_SECRET_PATH`; it additionally requires a `jwt-signing-keys` mount only when that resolved path is under `/var/run/secrets/firemud/jwt/`. Passing it is not evidence of Account-only private-key distribution. Convergence to the accepted ConfigMap authority, Account-only private-key distribution, validator `kid`/JWKS behavior, and traffic-open JWT rotation evidence remain target-state gates; `PREFLIGHT-JWT-002` and `PREFLIGHT-JWT-ROTATION-001` are not yet emitted by the executable.
...
- `PREFLIGHT-JWT-001` – the current executable rejects inline JWT secret configuration but otherwise proves only the legacy shared path/mount contract: every primary workload declares `FIREMUD_AUTH_JWT_SECRET_PATH`, and a workload whose resolved path is under `/var/run/secrets/firemud/jwt/` mounts `jwt-signing-keys` at that root. Passing this check does not prove asymmetric signing, Account-only custody, or validator separation.
- `PREFLIGHT-JWKS-001` – the current executable verifies that player-facing rendered workloads supply the legacy `jwt-jwks` Secret/reference and that Account mounts it at the configured JWKS path. Passing this implemented check does not prove the accepted ConfigMap authority, RBAC, read-only projection, or fallback restrictions.
- `PREFLIGHT-JWT-002` (target-state-only; not currently emitted) – player-facing resolved manifests use fixed, pre-created `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap resources. The materialization controller alone has name-scoped get, update, and patch authority for the signing Secret so it can read the current `resourceVersion` and perform CAS; Account has name-scoped read access to that Secret and get/update/patch authority for public JWKS; rotation automation has no resource write authority. Neither actor may list, create, or delete the pre-created resources. Projected mounts are read-only and classpath fallback is limited to explicit local/test use. The Account JWT private signing bundle is mounted only into Account Service; every validator uses asymmetric Account `kid`/JWKS verification with HMAC fallback disabled and receives no Account private key.
