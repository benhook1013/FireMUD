...
- `PREFLIGHT-JWT-002` (target-state-only; not currently emitted) – player-facing resolved manifests use fixed, pre-created `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap resources; Account alone has name-scoped get/update/patch authority, rotation automation has no resource write authority, projected mounts are read-only, and classpath fallback is limited to explicit local/test use. The Account JWT private signing bundle is mounted only into Account Service; every validator uses asymmetric Account `kid`/JWKS verification with HMAC fallback disabled and receives no Account private key.
