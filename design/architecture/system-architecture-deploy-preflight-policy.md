...
The accepted traffic-open target below is not yet enforced by the executable. Current production checking still validates the legacy `traffic-open-record/v1` shape, tenant/region coordinated-backup scope, timestamps, and referenced preflight report; hobby checking trusts the bounded projected compliance record. It does not dereference and validate backup readiness, `restoreRecoveryRecordRef`, a compatible finalized baseline, complete validator/participant/external-effect inventories, or the durable actual-recovery controller. Those omissions are blocking implementation debt: a current `PREFLIGHT-BACKUP-002` pass is not sufficient authority to open or reopen player-facing traffic. The target gate must fail closed on missing or unreadable records, incomplete or non-authoritative inventories, unknown or unsafe dispositions, unreachable participants, mismatched environment-wide artifact lineage, stale baselines, or invalidating recovery-contract changes. A production-equivalent drill may use current production lineage in an isolated production-equivalent boundary with compatible contracts/tooling, but it cannot authorize production traffic; reopen still requires the exact `actualRecoveryRecordRef`.
