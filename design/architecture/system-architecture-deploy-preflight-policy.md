...
- `jwtCustodyProof` for authorizing player-facing deployment, traffic-open, or fresh-boundary-restore reports, containing the exact accepted `proofId`, `custodyMode`, and `contractVersion`; `ci-static` reports may omit it because their JWT custody results are non-authorizing static evidence
...
For a player-facing deployment or fresh-boundary restore, the deployment/recovery record must preserve the exact `jwtCustodyProof` tuple for retries and replay. A retry may consume only a proof with the same `proofId`, `custodyMode`, and `contractVersion`; it must not reselect a mode or treat a legacy diagnostic result as the selected proof.
