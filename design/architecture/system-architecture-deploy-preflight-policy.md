...
- `PREFLIGHT-BACKUP-002` – production first-live or traffic-reopen events verify at least one successful logical backup upload, one successful backup verification result, one successful restore drill within the required restore-proof freshness window, and canonical coordinated-backup scope for the environment before traffic is opened.
