...
Target Gateway replay-domain preflight must resolve the non-secret `FIREMUD_GATEWAY_REPLAY_DOMAIN` value and require the exact versioned literal `gateway-connect-token-replay-v1`. It must also prove that every Gateway instance, the target Coordination Redis ACL/key helper, and migration/reset tooling resolve that same literal `{gateway-connect-token-replay-v1}` tag, then read back the readiness record plus bounded disposable replay/deny probe keys from the shared Coordination deployment and verify their common Cluster slot plus the readiness `replayAdmissionFence`. This is a cross-instance/continuity proof, not a host/port or configuration-presence check. The current preflight and Gateway helper do not implement this value, tagged-key readback, or replay-domain result; existing preflight passes remain partial non-authorizing evidence and player-facing admission stays quarantined.
...
`pr-preview` and `dev-demo-cluster` are non-player-facing hosted Helm environments and are outside the ordinary environment-report applicability table. The hosted-render contract validates the current chart output in a no-flag form: hosted-controller mode must carry the allocator-selected Telnet port annotation and one matching explicit Telnet `nodePort`. A trusted caller may additionally pass `--expected-hosted-telnet-node-port` to require that exact listener port and reject any other explicit NodePorts; this option is additive and does not relax the annotation, certificate ownership, namespace, or other hosted checks. The hosted-controller branch of the dev-demo workflow now invokes `ci-static` before server dry-run/apply and `operator` after identity readiness and runtime rollout, before smoke or success publication; this provides static contract proof but no live hosted acceptance proof. `PREFLIGHT-BRIDGE-001` defines the apply- and acceptance-blocking proof required for hosted preview integration and covers both the internal Gateway bridge and the selected direct Telnet TLS candidate; successful output uses the context-aware base wording `Private Gateway bridge TLS and certificate identity alignment is valid` (or `Private Gateway bridge and public direct Telnet TLS alignment is valid` when public Telnet is included), followed by `; controller-projected Secret readiness is confirmed` for `operator` or `; ci-static did not check controller-projected Secret readiness` for `ci-static`. The separate `helm-jwks-contract.sh` test covers preview JWKS resource and Account-mount wiring. These checks are diagnostic hosted-environment integration proof only: they do not establish standalone or player-facing preflight evidence, expected-bindings reports, promotion authority, traffic-open authorization, or the public-listener exposure-exclusivity proof reserved for target-state `PREFLIGHT-TELNET-001`.
...
- `PREFLIGHT-BRIDGE-001` – `GATEWAY_WS_URL` is canonical `wss://` for the environment's dedicated internal Gateway Service and DNS SAN; Gateway renders a distinct TLS server-identity mount and exactly one approved, mutually exclusive certificate-bound trust profile (`production_uri`, `migration_dns`, or `breakglass_fingerprint`) with only its required identity and metadata settings, no inactive-profile settings, and no hosted/player-facing `development_cidr`; migration DNS and break-glass profiles require their owner/incident metadata and future expiry; TCP Proxy renders a distinct WebSocket client-identity mount rather than reusing gRPC or Telnet credentials; the listener is ClusterIP-only on service TCP 443 to target TCP 8443; NetworkPolicy allows that listener only from TCP Proxy and allows the proxy bridge egress only to Gateway on TCP 8443; and identity-bearing Deployments declare fail-closed replacement configuration. Gateway retains RollingUpdate or the Kubernetes default for ordinary availability-preserving replacement, while TCP Proxy uses Recreate for fail-closed identity replacement. Those rendered strategies do not prove emergency certificate withdrawal terminates existing pods or bridges, interrupts existing connections, or prevents stale instances from serving indefinitely; emergency Gateway withdrawal requires controller termination of the old pods and their bridges. For direct Telnet TLS, the same result validates the NodePort Service, `DIRECT_TLS` environment, read-only dedicated Secret mount, and separation from HTTP Ingress and gRPC credentials. Standalone mode requires exactly one chart-owned dedicated `-telnet-tls` Certificate; hosted-controller mode requires matching explicit ownership labels on the TCP Proxy Service and Deployment, forbids a chart-owned Telnet Certificate, and consumes the release-scoped controller-projected Telnet Secret. The current chart emits the allocator-selected Telnet port in both the Service annotation and Telnet listener `nodePort`; no-flag hosted-controller validation requires that pair to be present and matching. A trusted caller may additionally supply `--expected-hosted-telnet-node-port`, which requires exactly one correct Telnet listener with that exact `nodePort` and no other explicit NodePorts; this additive option does not relax certificate ownership or any other hosted-controller check. The hosted-render `ci-static` result is defined to validate this contract without live Secret reads before server dry-run/apply. The `operator` result is defined to additionally verify the controller-projected Gateway server and bridge client Secrets contain `tls.crt`, `tls.key`, and `ca.crt` and the controller-projected Telnet Secret contains `tls.crt` and `tls.key`; the trusted preview workflow invokes it after identity readiness and rollout, before smoke or success publication, with live execution still unproved. Its successful output uses the same context-aware wording: the private base is `Private Gateway bridge TLS and certificate identity alignment is valid`, or the public base is `Private Gateway bridge and public direct Telnet TLS alignment is valid`; `operator` appends `; controller-projected Secret readiness is confirmed`, while `ci-static` appends `; ci-static did not check controller-projected Secret readiness`. This is rendered configuration, ownership, exact trusted NodePort binding, and Secret-presence evidence, not runtime withdrawal, pod or bridge termination, bridge draining, certificate issuance, rotation/readiness, peer-handshake, end-to-end bridge proof, or public Telnet exposure-exclusivity proof.
...
### Canonical target operator credential binding record

The selected operator binding resolves to one immutable per-generation target `operator-credential-binding/v1` record owned by the environment binding/preflight authority. The record is selected by the environment's `operatorCredentials.bindingRef`, or by its selected `fingerprint` when no platform-native binding reference exists; the selector does not create a second endpoint-local authority. The record contains:

- the exact target environment and selected binding reference or fingerprint;
- one immutable `bindingType`, exactly one of `SECRET_BACKED`, `CERT_MANAGER`, or `WORKLOAD_IDENTITY`. The binding authority resolves this discriminator from the selected `bindingRef`; when selection uses a fingerprint, the fingerprint must resolve to exactly one current binding record and its recorded `bindingType`. Unknown, missing, ambiguous, or selector/type-mismatched resolution fails closed. Consumers must use this recorded type and may not infer or override it from a local path, Secret name, certificate issuer, or endpoint configuration;
- a positive credential `generation`;
- one immutable absolute UTC `publishedAt` publication/effective timestamp for this record (the sole timestamp used for overlap ordering);
- the exact `activeLeafFingerprints` set, which is non-empty for every non-`REVOCATION` record and empty for a terminal `REVOCATION`, disjoint from `overlapLeafFingerprints`, and, only during a routine bounded rotation with a non-empty `overlapLeafFingerprints` set, that overlap set plus an absolute UTC `overlapExpiresAt` deadline; when `overlapLeafFingerprints` is empty, `overlapExpiresAt` is omitted;
- the exact required client-certificate `uriSan` and `profile`, plus the endpoint and workload audience binding;
- a `recordDigest`, computed as the SHA-256 digest of the canonical serialized record excluding the digest field itself, so every consumer can verify that the resolved record and its generation are unchanged;
- source-specific resolution evidence: `SECRET_BACKED` names the dedicated Secret and its file-backed client material; `CERT_MANAGER` names the Certificate/issuer binding and its declared output Secret; `WORKLOAD_IDENTITY` names the provider-projected client identity and trust bundle and does not require a substitute Secret. A source-specific reference or projected identity that cannot be resolved exactly under the recorded `bindingType` is invalid;
- the one durable `operationId` and its typed operation kind (`INITIAL_ISSUANCE`, `ROTATION`, or `REVOCATION`) that created or advanced this generation; generation 1 must carry `INITIAL_ISSUANCE` and cannot claim a rotation or revocation; and
- immutable readback evidence bound to this record and generation proving the applicable leaf transition: initial issuance proves the new leaf is accepted and no previous leaf exists; a routine overlap proves both old and new leaves are accepted; the later generation-advancing cutover proves the new leaf remains accepted while every old leaf is rejected; and `REVOCATION` proves the terminal empty active/overlap sets and rejection of every leaf accepted under prior generations.

Target operator endpoint validators, rotation/revocation workflows, preflight, and recovery must resolve and exact-match this same record, generation, `recordDigest`, `publishedAt`, leaf sets, URI SAN/profile, audiences, operation identity, and readback evidence. The `activeLeafFingerprints` and `overlapLeafFingerprints` sets must always be disjoint. When `operationKind=ROTATION` has a non-empty overlap set, `overlapExpiresAt` is required and must satisfy `overlapExpiresAt > publishedAt`; it bounds acceptance of every overlap leaf, and endpoints reject each overlap leaf at or after the deadline. When any operation has an empty `overlapLeafFingerprints` set, `overlapExpiresAt` is omitted (never serialized as `null`, an empty value, or a stale prior deadline). The generation-advancing cutover must remove every old leaf from `activeLeafFingerprints` and clear `overlapLeafFingerprints` no later than the prior overlap deadline; a late or missing cutover cannot extend acceptance. `INITIAL_ISSUANCE` requires an empty overlap set and no `overlapExpiresAt`; `REVOCATION` requires both empty active and overlap sets, no `overlapExpiresAt`, and readback proof that every previously accepted leaf is rejected. A routine overlap is bounded and is cleared by the generation-advancing transition; an incident/revocation transition permits no overlap. The richer record and its live endpoint/readback enforcement are target-state only: the checked-in `expected-bindings.yaml` schema and current validators remain the existing binding-reference/fingerprint forms and must not be presented as this proof.
...
- Restore validation tooling may derive shell environment variables such as `EXPECTED_PG_DUMP_BUCKET`, `EXPECTED_ASSET_STORE_BUCKET`, `EXPECTED_ASSET_STORE_ENDPOINT`, and `EXPECTED_SMTP_HOST` from this manifest rather than maintaining a second source of truth. It may derive an operator-binding fingerprint directly only when the manifest declares `operatorCredentials.fingerprint`; when the manifest is bindingRef-only, preflight must resolve the canonical `operator-credential-binding/v1` record and exact credential generation to obtain and validate its fingerprint, without deriving one from the reference or requiring a second manifest field. Target recovery evidence uses that same canonical record and exact generation; the current fingerprint-only validator remains a diagnostic gap.
- When validating operator-only credentials, preflight should compare like-for-like against the expected binding form: compare `bindingRef` values when the manifest declares `bindingRef`, and compare fingerprints when the manifest declares `fingerprint`; a bindingRef-only declaration additionally requires the resolved canonical record's exact generation and fingerprint. Implementations should not invent a second canonical representation during validation.
...
- **Target-state field (not currently emitted):** `operatorCredentialBinding` for operator-context reports that resolve operator credentials, containing the exact resolved canonical binding identity (`bindingRef` or fingerprint), positive `generation`, and `recordDigest`; when implemented, preflight must resolve this tuple from the manifest selected by `expectedBindingsRef`/`expectedBindingsDigest`, and consumers must exact-match the selector identity, generation, and digest against that canonical record. A binding reference or fingerprint alone is not sufficient evidence, and a later rotation generation must not be substituted for the generation recorded by the report.
