...
The restore mode, continuation, replay, and reopen lifecycle is canonical in [Backup & Disaster Recovery](./system-architecture-backup-recovery.md). Preflight retains only its local consequence: it validates the live owner-defined recovery state and immutable evidence and emits prerequisite evidence. It is never release authority and does not perform continuation, authorization, or release. No recovery-controller RPC currently exists in the checked-in `protos/` source, so this remains a target-state contract rather than an implemented gRPC surface.
