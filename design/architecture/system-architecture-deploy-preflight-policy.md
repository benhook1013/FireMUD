...
- `PREFLIGHT-JWKS-001` – JWKS resource type matches environment policy (an immutable `jwt-jwks` ConfigMap or equivalent integrity-controlled public artifact in every environment; classpath fallback only for explicit local/test use).
