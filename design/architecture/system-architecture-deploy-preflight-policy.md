...
- `PREFLIGHT-BACKUP-002` is required for `production` on first-live opens and reopen-after-restore events, and `not_applicable` for routine steady-state rollouts that do not change traffic-open status. Its target implementation reads the current environment-specific controller and emits prerequisite evidence only when the owner-defined target boundary and evidence lineage match. The current executable fails this check closed because that controller read is not implemented; maintenance-scope pause/reset evidence and checked-in projections cannot replace the environment-wide artifact or confidentiality evidence.
- `PREFLIGHT-BACKUP-003` is required for `hobby-self-hosted` on first-live opens and reopen-after-restore events, and `not_applicable` otherwise. Its target authorization consumes current backup-baseline compliance, immutable pre-release evidence, and the live actual-recovery controller under the owner-defined continuation/release boundary. The checked-in projection is exported only after owner-defined finalization and is not a preflight input. The current executable fails closed for the same missing controller-backed authorization.
