...
- `operatorCredentialBinding` for operator-context reports that resolve operator credentials, containing the exact resolved canonical binding identity (`bindingRef` or fingerprint), positive `generation`, and `recordDigest`; preflight must resolve this tuple from the manifest selected by `expectedBindingsRef`/`expectedBindingsDigest`, and consumers must exact-match the selector identity, generation, and digest against that canonical record. A binding reference or fingerprint alone is not sufficient evidence, and a later rotation generation must not be substituted for the generation recorded by the report.
