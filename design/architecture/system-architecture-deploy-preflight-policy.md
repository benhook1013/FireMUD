...
- `deploymentEventId`, a canonical UUID unique to the current preflight/apply event; retries or later re-applies must use a new value
...
- Waiver records must include: `deploymentEventId` matching the current report, approver identity, incident/change ticket, scope (policy IDs waived), expiration (deployment event only), and timestamp. `FIREMUD_DEPLOYMENT_EVENT_ID` must be supplied when running preflight with a waiver.
