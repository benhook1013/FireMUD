...
| --- | --- | --- |
| `PREFLIGHT-DIGEST-001` | `apply-blocking` | emitted (selected application images only) |
...
- `PREFLIGHT-DIGEST-001` – every image in every rendered staging/production workload, including application, database, backup, telemetry, sidecar, and other supporting workloads, is an immutable digest (`image@sha256:...`). A later accepted owner-bound exemption is the only way to exclude a rendered workload image; application-service coverage alone is insufficient. The current executable enumerates only selected application images, so mutable-tag examples such as `postgres:16`, `velero/velero:v1.12.3`, and `fluent/fluent-bit:2.2.2` are not currently covered by this result.
...
- `PREFLIGHT-PROMOTION-001` – production promotions reference a valid staging attestation with matching application-service digests and complete rendered-workload image lineage; `serviceDigests` alone does not cover supporting images.
  ...
  "checkResults": [
    {
      ...
      "required": true,
      "status": "pass",
      "message": "all enumerated application-service images are digest pinned; supporting workload images are not checked by the current executable"
    }
  ],
