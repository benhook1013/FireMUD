...
- For Telnet traffic, the TCP Proxy Service supplies `X-Proxy-Client-IP`, `X-Proxy-Game-Instance-Id`, `X-Proxy-Tenant-Id`, and `X-Proxy-Connection-Id` on the mTLS-authenticated WebSocket hop. After verifying the TCP Proxy client certificate on the internal WebSocket mTLS listener, the gateway treats those names as bridge-only inputs, strips the raw `X-Proxy-*` values before downstream forwarding, and:
  ...
  - Re-issues the authenticated `X-Proxy-Connection-Id` only as a server-owned opaque correlation identifier for disconnect events and observability; a public or untrusted raw value is never forwarded.
...
- `X-Proxy-Connection-Id` – re-issued only as a gateway-authenticated, server-owned opaque correlation field so downstream services can correlate lifecycle signals; the raw inbound `X-Proxy-Connection-Id` is stripped.
