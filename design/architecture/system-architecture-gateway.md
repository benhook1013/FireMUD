...
If the bridge transport fails before that close can be sent, producers must not substitute an ad hoc alternate signal. The outcome is treated as an unattributed bridge failure. For already-established Telnet sessions, the proxy closes immediately with `backend_unavailable`; only external WebSocket gameplay sessions continue to use the normal Gateway `unreachable` / recovery-window rules. Both sides must emit a bounded structured field such as `bridge_shutdown_class=planned_drain|unattributed_failure` in logs/metrics so operators can distinguish deliberate drain from outage and can detect when a planned drain degraded into an unattributed transport loss.

For clarity, this unattributed bridge-failure rule applies to the internal authenticated TCP Proxy bridge. The proxy maps that failure immediately to Telnet `backend_unavailable` for already-established Telnet sessions, while Gateway’s own `firemud.gateway.backendUnavailableGraceMs` still governs how long external WebSocket gameplay sessions may remain open before Gateway emits `1013/backend_unavailable`.
