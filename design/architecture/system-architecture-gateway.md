...
The close-reason parser splits the reason at the first `;`: the first segment must be one exact top-level reason token, and the remaining text may contain at most one exact `subreason=<bounded-value>` suffix. A missing, unknown, or omitted subreason is normalized to `none` and never changes the recognized top-level class. Duplicate, malformed, or conflicting suffixes are invalid diagnostic metadata and likewise cannot override the top-level class. The parser must validate the complete canonical `(close code, top-level reason)` pair from the matrix below; a recognized reason paired with the wrong code is invalid metadata. An unrecognized top-level token, missing code/reason, otherwise unparseable close, or invalid code/reason pair is `unknown_intent` and fails closed to `1011/internal_error` for the external WebSocket. On the authenticated TCP Proxy bridge, invalid close metadata instead uses the observation-specific `backend_unavailable` outcome with `bridge_shutdown_class=unattributed_failure`. After parsing, apply failure precedence `policy_violation` > `backend_unavailable` > `internal_error` > `idle_timeout`, then lifecycle precedence `logout` > `session_replaced` > `service_restart`; suffixes are never inputs to either precedence rule.
...
When observations overlap, first apply failure precedence: `policy_violation` > `backend_unavailable` > `internal_error` > `idle_timeout`. Only when no higher-priority failure supersedes a valid explicit lifecycle outcome, apply lifecycle precedence `logout` > `session_replaced` > `service_restart`: a committed terminal logout stays terminal; otherwise a successful controller takeover beats a concurrent planned drain; otherwise the result is planned `service_restart`. None of these outcomes is inferred from a subreason.
...
- **Trusted TCP Proxy exception**
  - Connect-token checks are bypassed only for the TCP Proxy Service's `/ws/game/**` bridge handshake on the internal mTLS listener when it satisfies the header-trust allowlists; this is the trusted TCP branch, not a public WebSocket mode.
...
- **Verified context handoff (Gateway -> Game Session)**
  ...
  - Context payload must carry the registered schema: `accountId`, `tenantId`, `worldSlug`, `realmSlug`, `playableStateNamespaceId`, `playableStateScope`, `gameInstanceId`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, `audience: game-session`, `recipient: game-session-service`, the bounded selected-target `authorityTuple` including its applicable caller-membership generation, exact `membershipVersion: {tenantId: version}` map containing only the selected tenant, `replayAdmissionFence`, `connectTokenJti`, `issuedAt`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`. Gateway is the producer and must set the registered audience and recipient itself; it carries the Account-issued selected-target snapshot's namespace/scope claims and authority evidence unchanged, with exact equality through the signed context, and does not derive an independent namespace or scope authority. Gateway accepts those namespace/scope and runtime-target fields only as the server-resolved target already bound to the exact `catalogRevision`/`pointerVersion` pair; it must not select a newer pointer, substitute carrier fields, or resolve a separate namespace/scope. The evidence provenance and Game Session continuation check are defined by the [canonical JWT and token contract](./system-architecture-jwt-and-token-contracts.md#canonical-authority-tuple); Game Session is the consumer and must accept only those exact values.
