...
- **Transport location**
  - Only explicitly classified non-first-party/public native-mobile, server-side, or other non-browser clients may send the connect token in the dedicated handshake header (`X-Firemud-Connect-Token`) on `/ws/game/**`; native clients store it in OS secure storage where applicable.
