...
The close-reason parser splits the reason at the first `;`: the first segment must be one exact top-level reason token, and the remaining text may contain at most one exact `subreason=<bounded-value>` suffix. A missing, unknown, or omitted subreason is normalized to `none` and never changes the recognized top-level class. Duplicate, malformed, or conflicting suffixes are invalid diagnostic metadata and likewise cannot override the top-level class; an unrecognized top-level token, missing code/reason, or otherwise unparseable close is `unknown_intent` and fails closed to `1011/internal_error` for the external WebSocket. After parsing, apply failure precedence `policy_violation` > `backend_unavailable` > `internal_error` > `idle_timeout`, then lifecycle precedence `logout` > `session_replaced` > `service_restart`; suffixes are never inputs to either precedence rule.
