...
- **Stateless and horizontally scalable** – no cookie-based session affinity is required. The gateway does not own gameplay lease state or shard-mapping state (those are owned by Game Session and stored in Coordination Redis), but it may consume the mapping view to route `/ws/game/**` connections to the correct Game Session shard.
...
Gateway is the gameplay admission surface for `/ws/game/**`, but it is not the owner of shard leases or shard mapping. When gameplay execution is sharded across multiple Game Session instances by `<tenantId, regionId>` lease ownership:
...
- `1013` with reason `reroute` – the gameplay shard mapping for the connection’s `<tenantId, regionId>` has moved (lease transfer, planned drain, or shard handoff). Clients should reconnect promptly so the new connection lands on the new shard owner. This signal is not an outage and should not trigger long exponential backoff; clients should use a small randomized delay (for example 0–250ms) to avoid thundering herds during coordinated lease moves.
...
- `reroute` (`1013`) – Gateway owns emitting the client-visible `1013/reroute` close when the shard mapping for a gameplay connection has moved. Game Session indicates reroute intent by closing its upstream WebSocket with `1013/reroute` (or by returning an explicit reroute error/metadata on a mapping-sensitive request); Gateway must translate those upstream signals into the canonical external close category and log/metric-tag it distinctly from `backend_unavailable`.
