...
- **Verified context handoff (Gateway -> Game Session)**
  ...
  - If the failure occurs before HTTP 101, Gateway exposes the existing handshake class and reason surface: source-token failures use `CONNECT_TOKEN_REJECTED` with `reason=invalid_token_content` when that is the classified cause. If the signed context is rejected after HTTP 101, Game Session uses the existing `ERROR <CODE> <message>` protocol surface above and closes the socket. Browser clients therefore observe the same bounded class/reason through an accepted HTTP handshake response or the established gameplay protocol, without a browser-specific failure taxonomy.
