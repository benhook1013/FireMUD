...
- Connect-token carrier parsing, required routing claims, expiry, and Redis replay checks are implemented for `/ws/game/**`. Remaining gaps are the target 30-second `iat`-to-`exp` hard maximum and the full durability/quarantine contract described below; those gaps do not mean that the existing signature, claim, expiry, carrier, or replay checks are absent.
