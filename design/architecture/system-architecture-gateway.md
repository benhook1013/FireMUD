...
- **Verified context handoff (Gateway -> Game Session)**
  ...
  - Gateway must also emit `X-Firemud-Connection-Mode: first_party_web` for these browser-cookie connect-token-validated handshakes. This current marker does not establish support for `X-Firemud-Connect-Token`; a future dedicated non-browser route may use the same marker only after its route and proof are complete.
