...
- Connect-token carrier parsing, required routing claims, expiry, and Redis replay checks are implemented for `/ws/game/**`, but the current carrier path still does not reject duplicate header values or duplicate token cookies and does not strip the cookie carrier after successful cookie authentication. The current `/ws/game/**` path is therefore non-promotable in player-facing environments until those carrier requirements, replay-marker durability, replay-quarantine continuity, and Account planned-rotation/compromise-cutover evidence are implemented and proved. The target contract also requires the 30-second `iat`-to-`exp` hard maximum; these readiness gaps do not mean that the existing signature, claim, expiry, carrier, or replay checks are absent.
...
- `X-Firemud-Connect-Context` – for non-proxy `/ws/game/**` handshakes that pass connect-token validation, gateway emits a short-lived signed context payload containing the complete verified connect scope: `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, the bounded selected-target `authorityTuple`, exact `membershipVersion: {tenantId: version}` map, `replayAdmissionFence`, `connectTokenJti`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`. The `membershipVersion` map is Account-produced: its sole key is the exact selected binding tenant ID and its value is the exact membership version from the same authoritative Account membership snapshot; it is never a scalar, aggregate, partial map, or differently scoped key. Gateway carries the map without reinterpretation, and Game Session exact-consumes and compares the complete map against its fresh Account membership snapshot. See the [canonical JWT and token contract](./system-architecture-jwt-and-token-contracts.md#canonical-authority-tuple). Game Session must validate signature, expiry, exact claim/context equality, and the later exact-binding admission predicates before using this context; replay protection for `connectTokenJti` remains Gateway-owned and is not re-implemented as a second authority in Game Session. `connectTokenJti` is internal replay/audit evidence and is never returned in client metadata or protocol output.
...
- **Verified context handoff (Gateway -> Game Session)**
  ...
  - Context payload must include at least: `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, the bounded selected-target `authorityTuple`, separate exact `membershipVersion: {tenantId: version}` map containing only the selected tenant, `replayAdmissionFence`, `connectTokenJti`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`. A scalar, aggregate, partial, or differently scoped membership version does not satisfy this context contract.
...
- **Gameplay WebSocket handshake errors**
  ...
  - Gateway must emit a machine-readable handshake error class for all non-101 upgrades (for example response header `X-Firemud-Handshake-Error-Class` and mirrored structured log field) using the bounded set:
    ...
    - `PROTOCOL_MISMATCH`
    - `INTERNAL_ERROR`
  - The specific connect-token classes above should be used when the gateway can classify the failure. `CONNECT_TOKEN_REJECTED` covers an unsupported or unregistered carrier/route and malformed, signature-invalid, missing-claim, wrong-audience, or otherwise rejected token content outside the narrower classes above. `CONNECT_SCOPE_MISMATCH` may also appear as a post-handshake Game Session admission error when the verified connect context no longer matches the selected gameplay target before `PLAY` completes. `POLICY_DENY` is reserved for other trust-boundary and route-policy denials after carrier/token classification is no longer the deciding factor.
