...
- While in `degraded_but_reachable`, sessions stay open and command outcomes are explicit backend errors. This state must not be used for stalled sessions that cannot produce a bounded explicit gameplay/protocol response; those sessions must transition to `unreachable`. While in `unreachable`, the backend unavailable timer runs and handshake/sending behavior follows the rules below.
