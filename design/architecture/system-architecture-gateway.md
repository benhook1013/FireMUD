...
- `X-Firemud-Connect-Context` – for non-proxy `/ws/game/**` handshakes that pass connect-token validation, gateway emits a short-lived signed context payload containing the complete verified connect scope: `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, `audience: game-session`, `recipient: game-session-service`, the bounded selected-target `authorityTuple`, exact `membershipVersion: {tenantId: version}` map, `replayAdmissionFence`, `connectTokenJti`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`. The `audience` and `recipient` values are registered context bindings, not client input. The `membershipVersion` map is Account-produced: its sole key is the exact selected binding tenant ID and its value is the exact membership version from the same authoritative Account membership snapshot; it is never a scalar, aggregate, partial map, or differently scoped key. Gateway carries the map without reinterpretation, and Game Session exact-consumes and compares the complete map against its fresh Account membership snapshot. See the [canonical JWT and token contract](./system-architecture-jwt-and-token-contracts.md#canonical-authority-tuple). Game Session must validate signature, expiry, exact audience/recipient and claim/context equality, and the later exact-binding admission predicates before using this context; replay protection for `connectTokenJti` remains Gateway-owned and is not re-implemented as a second authority in Game Session. `connectTokenJti` is internal replay/audit evidence and is never returned in client metadata or protocol output.
...
- **Verified context handoff (Gateway -> Game Session)**
  ...
  - Context payload must include at least: `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, `audience: game-session`, `recipient: game-session-service`, the bounded selected-target `authorityTuple`, separate exact `membershipVersion: {tenantId: version}` map containing only the selected tenant, `replayAdmissionFence`, `connectTokenJti`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`. A scalar, aggregate, partial, or differently scoped membership version does not satisfy this context contract. Gateway is the producer and must set the registered audience and recipient itself; Game Session is the consumer and must accept only those exact values.
  ...
  - Game Session validates signature (`kid` aware), exact audience/recipient, and expiry bounds before using scope for `CONNECT_SCOPE_MISMATCH` enforcement. Replay protection for `connectTokenJti` is performed only at Gateway handshake time.
