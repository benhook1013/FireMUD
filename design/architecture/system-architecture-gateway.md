...
- `gateway.websocket.closes{reason="<reason>",subreason="<subreason>",bridge_shutdown_class="<value>"}` – counter incremented whenever the gateway closes a client WebSocket, with `reason` drawn from the bounded close taxonomy (`logout`, `session_replaced`, `service_restart`, `idle_timeout`, `policy_violation`, `internal_error`, `backend_unavailable`), `subreason` drawn from the bounded subreason taxonomy (`user_logout`, `takeover`, `gateway_restart`, `admin_termination`, `edge_backpressure`, `none`), and `bridge_shutdown_class` restricted to `planned_drain`, `upstream_logout`, or `unattributed_failure`. `planned_drain` is a valid authenticated `1012/service_restart` bridge close and takes precedence over the general class; `upstream_logout` covers every other valid authenticated upstream top-level close; `unattributed_failure` covers bridge termination without any valid top-level lifecycle close.
