...
- For Telnet traffic, the TCP Proxy Service supplies `X-Proxy-Client-IP`, `X-Proxy-Game-Instance-Id`, `X-Proxy-Tenant-Id`, and `X-Proxy-Connection-Id` on the mTLS-authenticated WebSocket hop. After verifying the TCP Proxy client certificate on the internal WebSocket mTLS listener, the gateway treats those names as bridge-only inputs, strips the raw `X-Proxy-*` values before downstream forwarding, and:
  ...
  - Preserves the authenticated TCP Proxy's `X-Proxy-Connection-Id` value unchanged as a server-owned opaque Telnet-socket identity for disconnect correlation and observability; Gateway does not generate a replacement identifier or treat it as authorization material. The TCP Proxy's `NotifyDisconnect` event carries this same `proxyConnectionId` with its per-connection `disconnectSequence`; a public or untrusted raw value is never forwarded.
...
- `X-Proxy-Connection-Id` – the TCP Proxy's server-generated stable identifier for the Telnet socket. Gateway preserves this authenticated value unchanged so it remains the `proxyConnectionId` used with the TCP Proxy's `disconnectSequence` to correlate `NotifyDisconnect` events; it is not a Gateway-generated session or authorization identifier.
...
- `X-Proxy-Connection-Id` – preserved unchanged only after TCP Proxy authentication as a gateway-authenticated, server-owned opaque correlation field so downstream services can correlate lifecycle signals with the TCP Proxy's `NotifyDisconnect {proxyConnectionId, disconnectSequence}`; the raw inbound header is stripped before this authenticated value is re-added.
- `X-Game-Instance-Id` / `X-Tenant-Id` – the only downstream game/tenant context headers generated from TCP Proxy metadata. They are emitted only when the upstream hop is authenticated as the TCP Proxy Service and the corresponding `X-Proxy-Game-Instance-Id` / `X-Proxy-Tenant-Id` inputs were provided. Raw `X-Proxy-Game-Instance-Id` and `X-Proxy-Tenant-Id` are stripped and never forwarded. These remain advisory admission hints; the Game Session Service validates any game-instance/tenant claims against Redis, entitlements, and authenticated session state.
...
- **Required headers** – Spring Cloud Gateway preserves or sets:
  ...
  - `X-Proxy-Game-Instance-Id`, `X-Proxy-Tenant-Id`, and `X-Proxy-Connection-Id` are bridge-only inputs on the TCP Proxy → Gateway hop when the proxy supplies server-owned bootstrap metadata or future explicitly selected transport hints, or when the proxy needs disconnect correlation. The gateway strips raw `X-Proxy-*` names before downstream forwarding and, after authenticating the TCP Proxy identity, emits only canonical `X-Game-Instance-Id` / `X-Tenant-Id` plus the same preserved `X-Proxy-Connection-Id` value for correlation (see [Header Trust Model](#header-trust-model)).
