...
- **Lifetime and replay**
  ...
  - Replay checks must be backed by the player-facing Coordination Redis deployment (not Cache/Rate-Limit Redis or per-pod memory) so `jti` replay decisions are consistent across horizontally scaled gateway pods and cannot be selectively evicted. The deployment uses `noeviction`, AOF plus the required replica acknowledgement, replay-key ACL isolation, and admission-blocking capacity thresholds.
