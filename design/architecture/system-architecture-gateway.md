...
- The current replay implementation is narrower than that readiness statement: `GameplayHandshakeFilter` uses a `ReactiveStringRedisTemplate` bound by `spring.data.redis` to `FIREMUD_REDIS_CACHE_HOST` and records `gateway:connect-token:jti:*` with a plain `SETNX`-equivalent operation. It does not yet use the dedicated Coordination client, atomic readiness/fence/deny-marker consume, same-connection `WAITAOF`, or the reset/quarantine gate required by ADR 0029. This is an implementation/proof gap; the Cache/Rate-Limit endpoint remains correct for ordinary Gateway request limiting, but must not serve the security-critical replay continuity domain.
