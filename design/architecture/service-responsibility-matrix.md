...
The current moderation policy-input row assigns `POST /moderation/actions` persistence and audit to Logging & Admin; its separate `EvaluateModerationPolicy` read supplies runtime decisions. The gated enforcement-mutation row does not make `POST /moderation/actions` an enforcement mutation or current operator support: the route remains unavailable/gated pending the action schema, shared digest vectors, and Account authorization-reference issuance/redemption flow, while runtime decisions are consumed at `GAMEPLAY_ADMISSION` and `CHAT_SEND`.
