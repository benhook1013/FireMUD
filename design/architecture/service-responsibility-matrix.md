...
| External operator read/preparation ingress (live admission-pointer reads, audit, and prepared-upgrade proof reads; current Game Session session-lifecycle routes are owner-local rather than external ingress) | | | | | | | | | ✔ | | ✔ |
...
- The exact current matrix rows are **External operator read/preparation ingress (live admission-pointer reads, audit, and prepared-upgrade proof reads; current Game Session session-lifecycle routes are owner-local rather than external ingress)** and **External operator mutating ingress (feature-flag overrides, version-upgrade preparation, and scoped `PauseTicksForScope`/`ResumeTicksForScope` have implementation participation but remain externally gated pending action-family schema and `mutationDigest/v1` vector conformance; moderation policy persistence is live; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, versioned moderation propagation, quota overrides, and broader remediation remain unavailable)**.
