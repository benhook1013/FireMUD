...
| Authoritative owner: admission-pointer reads and prepared-upgrade reads (live); version-upgrade preparation mutation, open/close/retarget `pointerVersion` CAS, and cutover (externally gated or target-only) | | | | ✔ | | | | | | | |
| Admission-pointer reads and prepared-upgrade reads (live); version-upgrade preparation mutation, open/close/retarget CAS, and cutover invocation (externally gated or target-only) | | | | ✔ | | | | | ✔ | | |
| Runtime feature-flag truth and override participation (Game Session owns runtime truth; external toggle mutation is gated pending action-family schema and `mutationDigest/v1` vector conformance) | | | | ✔ | | | | | ✔ | | |
...
| Admin panel UX and runtime feature-flag override workflow (implementation path gated pending action-family schema and `mutationDigest/v1` vector conformance) | | | | ✔ | | | | | ✔ | | |
...
| Automated tick/coordination remediation (per-instance `PauseTicksForScope`/`ResumeTicksForScope` participation exists but external mutation is gated pending action-family schema and `mutationDigest/v1` vector conformance; guarded automation enumerates affected instances; regional mutation and broader reset/remediate are hypothetical target coverage) | | | | ✔ | | | | | ✔ | | |
...
| External operator read/preparation ingress (live admission-pointer reads/audit/prepared-upgrade reads; version-upgrade preparation mutation is externally gated pending action-family schema and `mutationDigest/v1` vector conformance; current Game Session session-lifecycle routes are owner-local rather than external ingress) | | | | | | | | | ✔ | | ✔ |
| External operator mutating ingress (feature-flag overrides, version-upgrade preparation, and scoped `PauseTicksForScope`/`ResumeTicksForScope` have implementation participation but remain externally gated pending action-family schema and `mutationDigest/v1` vector conformance; moderation policy persistence is live; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, versioned moderation propagation, quota overrides, and broader remediation remain unavailable) | | | | | | | | | ✔ | | ✔ |
...
- The exact current matrix rows are **External operator read/preparation ingress (live admission-pointer reads/audit/prepared-upgrade reads; version-upgrade preparation mutation is externally gated pending action-family schema and `mutationDigest/v1` vector conformance; current Game Session session-lifecycle routes are owner-local rather than external ingress)** and **External operator mutating ingress (feature-flag overrides, version-upgrade preparation, and scoped `PauseTicksForScope`/`ResumeTicksForScope` have implementation participation but remain externally gated pending action-family schema and `mutationDigest/v1` vector conformance; moderation policy persistence is live; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, versioned moderation propagation, quota overrides, and broader remediation remain unavailable)**.
- Proposed route: `POST /api/session/game-sessions/{id}/feature-flags/{flagKey}:toggle`. Matrix check: `Game Session` participates in `Admin/creator API participation`, but the **External operator mutating ingress (feature-flag overrides, version-upgrade preparation, and scoped `PauseTicksForScope`/`ResumeTicksForScope` have implementation participation but remain externally gated pending action-family schema and `mutationDigest/v1` vector conformance; moderation policy persistence is live; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, versioned moderation propagation, quota overrides, and broader remediation remain unavailable)** row routes this workflow through `Logging & Admin`, so the direct external Game Session route is not allowed without a design update.
...
- Proposed route: `GET /api/account/accounts/{id}`. Matrix check: `Account Service` participates in `Admin/creator API participation`, and the request is an external admin read rather than an operator mutation covered by the **External operator mutating ingress (feature-flag overrides, version-upgrade preparation, and scoped `PauseTicksForScope`/`ResumeTicksForScope` have implementation participation but remain externally gated pending action-family schema and `mutationDigest/v1` vector conformance; moderation policy persistence is live; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, versioned moderation propagation, quota overrides, and broader remediation remain unavailable)** row, so the route may be edge-routable when the owning service documents it as a bypass-safe read contract.

Moderation policy-definition and policy-action rows include Logging & Admin evaluation and audit persistence. They do not make `POST /moderation/actions` an enforcement mutation; runtime decisions remain at `GAMEPLAY_ADMISSION` and `CHAT_SEND`.
...
- **Operator-action coverage split** – Admission-pointer reads/audit/prepared-upgrade reads are the live external operator read/preparation ingress. Version-upgrade preparation is an implementation-participating mutation but remains externally gated pending its action-family schema and `mutationDigest/v1` vector conformance. Runtime feature-flag overrides and scoped tick pause/resume are implementation-participating mutation families with the same external gate; moderation policy persistence is live through Logging & Admin. Game Session's `/sessions*` lifecycle routes are current owner-local hooks, not a complete external ingress family; target lifecycle forwarding enters through Logging & Admin. Admission-pointer open/close/retarget CAS and cutover are target-only. Current gameplay/chat enforcement synchronously consumes Logging & Admin policy evaluation; versioned propagation, quota override, and broader reset/remediate actions remain target families.
- **Tick remediation split** – Logging & Admin owns the implementation-participating per-instance pause/resume ingress, target-state remediation APIs, automation policy, and audit trail; external pause/resume mutation enablement remains gated pending the action-family schema and `mutationDigest/v1` vector conformance. Game Session owns all tick/coordination state mutation. Broader reset/remediate actions remain hypothetical until the owner APIs exist.
...
- **Moderation policy propagation** – Logging & Admin owns live gameplay/chat moderation policy persistence, evaluation, definition, and audit; Game Session consumes `GAMEPLAY_ADMISSION` and Social & Groups consumes `CHAT_SEND` at their owner boundaries. Versioned policy snapshot/event propagation, pull-on-miss refresh, and broader enforcement remain missing; the applicable high-risk decisions fail closed when fresh evidence is unavailable. See the canonical moderation propagation contract in `design/architecture/system-architecture-overview.md`.
- **Ban taxonomy** – Account owns account-security bans and auth authority-generation advances; Logging & Admin owns gameplay/chat moderation policy definitions, evaluation, and audit; Game Session and Social & Groups currently enforce gameplay and chat decisions respectively. The current moderation mutation endpoint persists policy input and audit; it does not directly perform runtime enforcement.
...
- **External operator read/preparation ingress** – Logging & Admin participates in the live external ingress for admission-pointer reads/audit/prepared-upgrade reads, while version-upgrade preparation is an implementation-participating mutation that remains externally gated pending its action-family schema and `mutationDigest/v1` vector conformance. Game Session remains the authority for the underlying read/preparation contract. Game Session's current `/sessions*` lifecycle routes remain owner-local rather than current external ingress.
- **External operator mutating ingress** – Logging & Admin is the mandatory external ingress for implementation-participating runtime feature flags, version-upgrade preparation, and scoped tick pause/resume, but those mutations remain externally gated pending their action-family schemas and shared `mutationDigest/v1` vectors; moderation policy persistence is live. Target lifecycle forwarding, admission-pointer open/close/retarget CAS and cutover, versioned moderation propagation, hypothetical quota overrides, and broader tick remediation also enter through Logging & Admin; Gateway participates only as the edge routing and coarse protection layer for those writes.
