...
- **Movement hot-path shape** – Movement uses the ordinary bounded model: Game Session dispatches the fenced operation, Game Logic is the one fan-out owner, and World Management plus Entity Management are its two authoritative participants. Neither participant may fan out further, and a third authority requires a new architecture decision.
...
- **Gameplay hot path policy** – Service APIs follow [ADR 0056](./decisions/adr-0056-one-hot-path-fan-out-owner.md): one ingress/dispatch step, one synchronous fan-out owner, and at most two authoritative participants across the transitive critical path. Every RPC stage/retry remains inside the measured latency contract; a third participant requires an architecture decision.
