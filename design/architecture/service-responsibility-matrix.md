...
The gated moderation policy-input row assigns target `POST /moderation/actions` persistence and audit to Logging & Admin; its separate live `EvaluateModerationPolicy` read supplies runtime decisions. The unavailable route does not itself perform owner-side enforcement. Target owner-side enforcement, quota, runtime, admission-control/pointer-write, tick, and lifecycle mutations use the canonical Logging & Admin ingress and remain individually gated by their owning action, digest, and authorization contracts.
