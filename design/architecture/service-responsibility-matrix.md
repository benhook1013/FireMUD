...
| Authoritative owner: admission-pointer reads/preparation (live); open/close/retarget `pointerVersion` CAS and cutover (target-only) | | | | ✔ | | | | | | | |
| Admission-pointer read/preparation invocation (live); open/close/retarget CAS and cutover invocation (target-only) | | | | ✔ | | | | | ✔ | | |
...
| Automated tick/coordination remediation (live per-instance `PauseTicksForScope`/`ResumeTicksForScope`; guarded automation enumerates affected instances; regional mutation and broader reset/remediate are hypothetical target coverage) | | | | ✔ | | | | | ✔ | | |
...
| External operator read/preparation ingress (live admission-pointer reads/audit/version-upgrade preparation/read; current Game Session session-lifecycle routes are owner-local rather than external ingress) | | | | | | | | | ✔ | | ✔ |
| External operator mutating ingress (live feature-flag overrides, moderation policy persistence, and scoped `PauseTicksForScope`/`ResumeTicksForScope`; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, versioned moderation propagation, quota overrides, and broader remediation) | | | | | | | | | ✔ | | ✔ |
...
- **Operator-action coverage split** – Admission-pointer reads/audit/version-upgrade preparation/read are the live external operator read/preparation ingress and are not mutations. Runtime feature-flag overrides, moderation policy persistence, and scoped tick pause/resume are live external operator mutating ingress families through Logging & Admin. Game Session's `/sessions*` lifecycle routes are current owner-local hooks, not a complete external ingress family; target lifecycle forwarding enters through Logging & Admin. Admission-pointer open/close/retarget CAS and cutover are target-only. Current gameplay/chat enforcement synchronously consumes Logging & Admin policy evaluation; versioned propagation, quota override, and broader reset/remediate actions remain target families.
...
- **External operator read/preparation ingress** – Logging & Admin participates in the live external ingress for admission-pointer reads/audit/version-upgrade preparation/read, while Game Session remains the authority for the underlying read/preparation contract. Game Session's current `/sessions*` lifecycle routes remain owner-local rather than current external ingress.
- **External operator mutating ingress** – Logging & Admin is the mandatory external ingress for live runtime feature flags, moderation policy persistence, and scoped tick pause/resume. Target lifecycle forwarding, admission-pointer open/close/retarget CAS and cutover, versioned moderation propagation, hypothetical quota overrides, and broader tick remediation also enter through Logging & Admin; Gateway participates only as the edge routing and coarse protection layer for those writes.
