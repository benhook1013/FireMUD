...
| External operator write ingress (live feature flags/admission reads/preparation/`PauseTicksForScope`/`ResumeTicksForScope`; current Game Session session-lifecycle routes are owner-local rather than external ingress; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, moderation enforcement/quota/broader remediation) | | | | | | | | | ✔ | | ✔ |
...
- The exact current matrix row is **External operator write ingress (live feature flags/admission reads/preparation/`PauseTicksForScope`/`ResumeTicksForScope`; current Game Session session-lifecycle routes are owner-local rather than external ingress; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, moderation enforcement/quota/broader remediation)**.
- Proposed route: `POST /api/session/game-sessions/{id}/feature-flags/{flagKey}:toggle`. Matrix check: `Game Session` participates in `Admin/creator API participation`, but the **External operator write ingress (live feature flags/admission reads/preparation/`PauseTicksForScope`/`ResumeTicksForScope`; current Game Session session-lifecycle routes are owner-local rather than external ingress; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, moderation enforcement/quota/broader remediation)** row routes this workflow through `Logging & Admin`, so the direct external Game Session route is not allowed without a design update.
...
- Proposed route: `GET /api/account/accounts/{id}`. Matrix check: `Account Service` participates in `Admin/creator API participation`, and the request is an external admin read rather than an operator write covered by the **External operator write ingress (live feature flags/admission reads/preparation/`PauseTicksForScope`/`ResumeTicksForScope`; current Game Session session-lifecycle routes are owner-local rather than external ingress; target Logging & Admin session-lifecycle forwarding plus admission open/close/retarget CAS/cutover, moderation enforcement/quota/broader remediation)** row, so the route may be edge-routable when the owning service documents it as a bypass-safe read contract.
...
- **Operator-action coverage split** – Runtime feature-flag overrides, admission-pointer reads/preparation, and scoped tick pause/resume are live Logging & Admin ingress families. Game Session's `/sessions*` lifecycle routes are current owner-local hooks, not a complete external ingress family; target lifecycle forwarding enters through Logging & Admin. Admission-pointer open/close/retarget CAS and cutover are target-only. The current moderation route persists policy input and audit only; enforcement propagation, quota override, and broader reset/remediate actions are hypothetical target families with no current owner mutation route.
...
- **Ban taxonomy (target-state enforcement classification)** – Account owns account-security bans and auth authority-generation advances; Logging & Admin owns gameplay/chat moderation ban policy definitions; Game Session and Social & Groups are target-state enforcement owners for gameplay and chat scopes respectively. The current moderation endpoint persists policy input and audit only.
...
- **External operator write ingress** – Logging & Admin is the mandatory external ingress for live runtime feature flags, admission-pointer reads/preparation, and scoped tick pause/resume. Game Session's current `/sessions*` lifecycle routes remain owner-local rather than current external ingress; target lifecycle forwarding enters through Logging & Admin. Admission-pointer open/close/retarget CAS and cutover are target-only, as are moderation enforcement, hypothetical quota overrides, and broader tick remediation; Gateway participates only as the edge routing and coarse protection layer for those writes.
