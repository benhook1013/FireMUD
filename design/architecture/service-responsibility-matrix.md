...
- Proposed route: `GET /api/account/accounts/{id}`. Matrix check: `Account Service` participates in `Admin/creator API participation`, and the request is an external admin read rather than an operator write covered by the **External operator write ingress (live feature flags/admission/`PauseTicksForScope`/`ResumeTicksForScope`; target moderation enforcement/quota/broader remediation)** row, so the route may be edge-routable when the owning service documents it as a bypass-safe read contract.
