...
- **Tick remediation split** – Logging & Admin owns the gated target-state per-instance pause/resume ingress, target-state remediation APIs, automation policy, and audit trail; no live admin UI or automation is claimed until the action-family schema, shared cross-language `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption exist. Game Session owns mutation only within its gameplay coordination keyspaces, including `tick:*`, `timer:*`, `retry:*`, `session:game:*`, and `tick-executor-lease:*`; Account exclusively owns `session:auth:*`, and this explicitly excludes both the Account auth family and Automation & Scripting-owned `automation:tick:*`, `automation:timer:*`, and `script-scheduler:*` families. Broader reset/remediate actions remain hypothetical until the owner APIs exist.
...
- **Moderation policy propagation** – Logging & Admin owns gameplay/chat moderation policy definitions and the target action-input/audit record, while the moderation action mutation is currently unavailable/gated. `POST /moderation/actions` and `ApplyModerationAction` do not currently persist or perform owner-side enforcement; they remain unavailable pending the action-family schema, shared cross-language `mutationDigest/v1` vectors, Account authorization-reference issuance, and Logging & Admin receiving-service validation/redemption. The separate live `EvaluateModerationPolicy` read supplies Account, Game Session, and Social & Groups at their owner boundaries. Versioned policy snapshot/event propagation, pull-on-miss refresh, and broader enforcement remain missing; the applicable high-risk decisions fail closed when fresh policy evidence is unavailable. See the canonical moderation propagation contract in `design/architecture/system-architecture-overview.md`.
