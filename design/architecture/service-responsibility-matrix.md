...
- **Tick remediation split** – Logging & Admin owns the gated target-state per-instance pause/resume ingress, target-state remediation APIs, automation policy, and audit trail; no live admin UI or automation is claimed until the action-family schema, shared cross-language `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption exist. Game Session owns all tick/coordination state mutation. Broader reset/remediate actions remain hypothetical until the owner APIs exist.
