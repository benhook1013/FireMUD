...
- Live external operator availability is limited to Logging & Admin reads, investigation, admission-pointer reads/audit, and prepared-upgrade reads. The target mutation rows below are normative contracts, not claims that every listed mutation is currently routable.
- `/moderation/actions` is unavailable/gated until its action-family schema, shared cross-language `mutationDigest/v1` golden vectors, and Account-issued authorization-reference flow exist. Synchronous `EvaluateModerationPolicy` consumption at `GAMEPLAY_ADMISSION` and `CHAT_SEND` remains separate owner enforcement and does not make the action route live.
- Feature-flag overrides, version-upgrade preparation, scoped tick pause/resume, admission-pointer CAS/cutover, and target session-lifecycle forwarding have implementation or owner contract seams but remain gated or target-only pending the same action/digest/reference contract and, where applicable, the target-only `privileged_control` issuance flow.
...
| Authoritative owner: admission-pointer reads, prepared-upgrade reads, version-upgrade preparation mutation, open/close/retarget `pointerVersion` CAS, and cutover | | | | ✔ | | | | | | | |
| Admission-pointer reads, prepared-upgrade reads, version-upgrade preparation mutation, open/close/retarget CAS, and cutover invocation | | | | ✔ | | | | | ✔ | | |
...
| Game moderation policy definition, synchronous evaluation, persistence, and audit investigation | | | | | | | | | ✔ | | |
| Moderation enforcement (`GAMEPLAY_ADMISSION` / `CHAT_SEND`) | | | | ✔ | | | | ✔ | ✔ | | |
...
Moderation policy-definition and policy-action rows include Logging & Admin evaluation and the target audit/persistence boundary. They do not make `POST /moderation/actions` current operator support or an enforcement mutation; the route is unavailable/gated pending the action schema, shared digest vectors, and Account authorization-reference flow, while runtime decisions remain at `GAMEPLAY_ADMISSION` and `CHAT_SEND`.
...
- **Moderation policy propagation** – Logging & Admin owns gameplay/chat moderation policy evaluation, definition, and the target persistence/audit boundary; `POST /moderation/actions` is unavailable/gated until its action-family schema, shared `mutationDigest/v1` vectors, and Account authorization-reference flow exist. Game Session consumes `GAMEPLAY_ADMISSION` and Social & Groups consumes `CHAT_SEND` at their owner boundaries. Versioned policy snapshot/event propagation, pull-on-miss refresh, and broader enforcement remain missing; the applicable high-risk decisions fail closed when fresh evidence is unavailable. See the canonical moderation propagation contract in `design/architecture/system-architecture-overview.md`.
- **Ban taxonomy** – Account owns account-security bans and auth authority-generation advances; Logging & Admin owns gameplay/chat moderation policy definitions and evaluation plus the target audit boundary; Game Session and Social & Groups enforce gameplay and chat decisions respectively. The moderation mutation endpoint is unavailable/gated and does not directly perform runtime enforcement.
