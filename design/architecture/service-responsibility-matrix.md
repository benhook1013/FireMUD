...
- **Movement/location write contract orchestration** – Game Session orchestrates movement under tick/effect identity and owns per-session sequencing plus the current execution-region pointer; Game Logic computes deterministic movement outcomes and orchestrates causal-floor room-read composition, World Management commits authoritative room occupancy/location and supplies the causal floor, and Entity Management applies entity-side consequences without owning occupancy indexes.
