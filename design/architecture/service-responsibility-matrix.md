...
| External `POST /moderation/actions` policy-input/audit persistence ingress (gated/unavailable; not owner enforcement) | | | | | | | | | ✔ | | |
...
- **Tick remediation split** – Logging & Admin owns the implementation-participating per-instance pause/resume ingress, target-state remediation APIs, automation policy, and audit trail; external pause/resume mutation enablement remains gated pending the action-family schema, shared cross-language `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption. Game Session owns all tick/coordination state mutation. Broader reset/remediate actions remain hypothetical until the owner APIs exist.
