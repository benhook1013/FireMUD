...
| External session-lifecycle operator forwarding through Gateway and Logging & Admin (target; not current route) | | | | ✔ | | | | | `target only` | | ✔ |
...
| Authoritative owner: admission-pointer reads, prepared-upgrade reads, gated version-upgrade preparation mutation (action-family schema, shared cross-language `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption required), target-only open/close/retarget `pointerVersion` CAS, and target-only cutover | | | | ✔ | | | | | | | |
| Admission-pointer reads, prepared-upgrade reads, gated version-upgrade preparation mutation (action-family schema, shared cross-language `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption required), target-only open/close/retarget CAS, and target-only cutover invocation | | | | ✔ | | | | | ✔ | | |
...
- The exact current matrix rows are **External operator read/preparation ingress** and **External operator write ingress**; their current availability and drift are recorded in **Implementation Status**, while the rows remain the canonical target participation contract.
...
- **External operator read/preparation ingress** – Gateway is the external edge ingress and Logging & Admin participates in the live external ingress for admission-pointer reads/audit/prepared-upgrade proof reads, while version-upgrade preparation is an implemented operator-facing mutation that remains externally gated and unavailable pending its action-family schema, shared cross-language `mutationDigest/v1` vector conformance, and Account authorization-reference issuance/redemption. Game Session remains the authority for the underlying read/preparation contract. Game Session's current `/sessions*` lifecycle routes remain owner-local rather than current external ingress.
