...
- Feature-flag overrides, version-upgrade preparation, scoped per-instance tick pause/resume, admission-pointer CAS/cutover, and target `Gateway -> Logging & Admin -> Game Session` session-lifecycle forwarding have implementation or owner contract seams but remain gated or target-only pending their action-family schemas, shared cross-language `mutationDigest/v1` golden vectors, and Account-issued authorization-reference issuance/redemption flow and, where applicable, the target-only `privileged_control` issuance flow.
...
| Authoritative owner: admission-pointer reads, prepared-upgrade proof reads, gated version-upgrade preparation mutation (action-family schema, shared cross-language `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption required), target-only open/close/retarget `pointerVersion` CAS, and target-only cutover | | | | ✔ | | | | | | | |
| Admission-pointer reads, prepared-upgrade proof reads, gated version-upgrade preparation mutation (action-family schema, shared cross-language `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption required), target-only open/close/retarget CAS, and target-only cutover invocation | | | | ✔ | | | | | ✔ | | |
...
- The exact current matrix rows are **External operator read/proof-read ingress** and **External operator write ingress**; their current availability and drift are recorded in **Implementation Status**, while the rows remain the canonical target participation contract. Version-upgrade preparation is a mutation and belongs only to the gated write-ingress row; the read row covers proof reads, not preparation writes.
