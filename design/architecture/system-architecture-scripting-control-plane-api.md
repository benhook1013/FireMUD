...
- **Current output:** newest-first durable `script_event_audit` rows expose tenant, game-instance, region/epoch, entity, `playableStateScope`, admitted routing bundle, script/plugin/event identity, `scriptPatchVersion`, `scriptEventId`, trigger mode, scheduler source state/ordinal/due fields, optional `workItemId`, final stage/outcome/reason, row timestamps, the current owned runtime game-instance/region/epoch and routing bundle, and stale scope/routing flags. The live mapper does not expose `playableStateNamespaceId`, `scriptPinEpoch`, or a current-runtime namespace, so those fields cannot yet drive live exact validation.
- **Target-state additions:** each row also carries the applicable Trigger Identity's `playableStateNamespaceId` and exact `scriptPinEpoch`, plus `currentRuntimePlayableStateNamespaceId` and stale-namespace signaling. Those additions retain `playableStateScope` as separately exact-validated evidence rather than changing Trigger Identity.
