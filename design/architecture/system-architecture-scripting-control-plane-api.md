...
Implementation note: the current Automation & Scripting implementation exposes the scheduler-owned subset of `script_event_audit` directly for timer troubleshooting. This read is bounded to `sourceKind=SCHEDULE_TIMER` and currently returns both timer-handler rows for persisted work and scheduler candidate rows persisted as `SCHEDULE_DROPPED`, including candidate outcomes such as `catch_up_truncated` and `runtime_scope_changed`. The live proto, record, and converter expose patch-level output only; `scriptPinEpoch`, `playableStateNamespaceId`, and target namespace/staleness fields are target-only. Target state keeps append-only candidate history on the separate scheduler candidate-audit surface; these current `script_event_audit` rows are not that target authority.
