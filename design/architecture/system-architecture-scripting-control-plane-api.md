...
- `statePresent`
- `admissionMode`
- `admissionEpoch` (current mutable admission-state epoch; not a separately identified acknowledgement epoch)
...
- explicit target `mode`, request-result fingerprint, and acknowledgement timestamp when a matching successful history row exists; the live response does not expose that row's resulting epoch as a separate acknowledgement field
...
- The live response is backed by durable Automation-owned admission mode/epoch state plus durable work-item truth already owned by Automation & Scripting. The immutable matching history row can supply `controlPlaneRequestId`, explicit target mode, outcome, request fingerprint, resulting `admissionEpoch`, and acknowledgement timestamp, but the live read response does not return that row's epoch as a separately identified acknowledgement field; its `admissionEpoch` describes current state. Do not treat the current-state epoch plus history-derived acknowledgement fields as a complete acknowledgement tuple in readback.
