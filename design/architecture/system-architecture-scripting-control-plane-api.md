...
- Current ingress enforces `SCRIPT_OUTPUT_MAX_SERIALIZED_WORK_ITEM_BYTES` on the request `payloadJson` input envelope before durable work-item persistence. An oversized envelope is a transport/pre-domain validation failure and must use canonical non-OK gRPC `INVALID_ARGUMENT`; it must not use `TRIGGER_ADMISSION_OUTCOME_OUTPUT_BUDGET_EXCEEDED`, create an event-scope admission result, or produce an ingress-audit/ingress-drop metric record. The current implementation still maps this check to the output-budget enum, which is implementation drift. The legacy runtime evaluator separately enforces command-count and per-entity ceilings incrementally while expanding emitted commands. Artifact-pinned cost metadata, generated-output serialized-byte/data-dependent metering, and atomic all-or-none output persistence remain target-state gaps; the current evaluator does not provide those proofs or claim that a handler-local generated-output byte violation leaves no generated output or handoff.
