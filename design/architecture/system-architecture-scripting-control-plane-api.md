...
Implementation note: the separate Game Session pin-authority remainder exposes this convergence read directly from the persisted game-instance pin record; #2682 consumes that prerequisite rather than implementing it. The live proto and service return the observed pinned patch, `observedScriptPinEpoch`, observational `lastObservedControlPlaneRequestId`, and observed timestamp, making the observed exact pin tuple public instead of leaving convergence identity implicit in actor/reason text. The current proto/implementation still does not expose `currentConvergenceAttemptGeneration`, and current proof does not yet establish attempt-generation binding for convergence or the ADR 0103 commit-boundary attestation and revalidation required by the target contract below. Semantic `UNPINNED` is represented separately by the patch, epoch, and associated request identity being absent together only before the first pin, while a partial tuple is invalid owner state, cannot satisfy `EXPECT_UNPINNED` or `EXPECT_EPOCH`, and is rejected/fails closed even for an `UNCONDITIONAL` request.
...
- The normalized exact scope is `(tenantId, gameInstanceId, regionId)`, where an omitted or blank `regionId` selects the exact empty/unscoped row and never a wildcard for regional rows. The explicit step/target mode (`PAUSED_FOR_ROLLBACK` or `NORMAL`) is part of the request-result key. A workflow-scoped `controlPlaneRequestId` may deliberately span distinct steps or exact scopes, including the pause and normal steps, but each idempotency key is the tuple `(controlPlaneRequestId, normalized exact scope, explicit step/target mode)`.
- For both this mutation and `GetAutomationDrainStatus`, `tenantId` and `gameInstanceId` are required nonblank after normalization. Each API trims them once, rejects a blank value before request fingerprinting, durable-result lookup, owner-state read, or mutation-target selection, and uses only the normalized values thereafter.
...
Implementation note: the current Automation & Scripting implementation persists a scope-local `automation_admission_states` record keyed by `(tenantId, gameInstanceId, regionId)`, exposes `SetAutomationAdmissionMode`, stamps admitted `script_work_items` with the current `admissionEpoch`, and serves this read from an exact read-only admission-state lookup plus durable work-item truth. A missing state is not materialized by the read. The response exposes `statePresent` and, when matching durable request history exists, the request ID, target mode, outcome, fingerprint, and acknowledgement timestamp; a present state without that history is diagnostic with `ACKNOWLEDGEMENT_UNAVAILABLE`, while a missing state returns the explicit `NOT_FOUND` diagnostic/default. The counted set is mode-dependent: `NORMAL` includes only rows whose `workItem.admissionEpoch == current admissionEpoch`, while `PAUSED_FOR_ROLLBACK` includes rows whose `workItem.admissionEpoch <= 0` or `workItem.admissionEpoch < current admissionEpoch`, excluding positive current-epoch rows; `activeExecutionCount` and `pendingCancelableWorkItemCount` are then derived from that set. This current projection is distinct from the target mapping's current-epoch pre-DSL and evaluated-descriptor counts.
Inputs:
...
- The [shared normalized required-scope validation](#setautomationadmissionmode) applies before any exact-scope lookup.
...
- A present admission-state row without a durable successful `SetAutomationAdmissionMode` acknowledgement is an inconsistent or legacy state: return its mode and epoch as diagnostic state only, leave the request ID, target mode, and fingerprint absent, and mark the result with an explicit bounded non-success diagnostic such as `ACKNOWLEDGEMENT_UNAVAILABLE`. A present row alone must never be treated as a successful Set acknowledgement or recovery evidence.
...
- Optional `gameInstanceId`
- Optional `scriptPatchVersion`
- Optional source runtime identity filters (`playableStateNamespaceId`, `regionId`, `regionEpoch`); `playableStateScope` is a separately validated evidence filter
- Optional `workItemId`
- Optional `handoffOutcome`
- Optional target runtime identity filters (`targetGameInstanceId`, `targetPlayableStateNamespaceId`, `targetRegionId`, `targetRegionEpoch`); `targetPlayableStateScope` is separately validated target evidence
...
- **Target-state output:** ordered event rows containing `eventId`, `tenantId`, source Command-Handoff runtime identity (`gameInstanceId`, `playableStateNamespaceId`, `regionId`, `regionEpoch`), separately validated source `playableStateScope` evidence, exact source owner tuple `scriptPatchVersion`/`scriptPinEpoch`/`scriptPinControlPlaneRequestId`, `scriptId`, optional plugin identity tuple `(pluginId, pluginVersionId, bindingId)` and target-state `pluginActivationEpoch`/captured `lifecycleRevision`; absent tuple fields are explicit and must not be inferred from epoch/revision, `workItemId`, `commandOrdinal`, `automationDispatchId`, target-state `handoffRequirement`, optional `gameSessionCommandId`, distinct target Command-Handoff runtime identity when applicable (`targetGameInstanceId`, `targetPlayableStateNamespaceId`, `targetRegionId`, `targetRegionEpoch`), separately validated target `targetPlayableStateScope` evidence, optional remote follow-up ids (`remoteCoordinatorId`, `remoteFollowupId`), current owned target runtime identity (`currentTargetRuntimePlayableStateNamespaceId`, `currentTargetRuntimeGameInstanceId`, `currentTargetRuntimeRegionId`, `currentTargetRuntimeRegionEpoch`) plus the current owned routing bundle (`currentTargetRuntimePlayableStateScope`, `currentTargetRuntimeWorldSlug`, `currentTargetRuntimeRealmSlug`, `currentTargetRuntimePointerVersion`) and stale-scope/routing comparison against the persisted distinct target namespace/scope and admitted target routing bundle, later Game Session gameplay-command execution truth (`gameplayCommandExecutionOutcome`, `gameplayCommandGameplayResult`, failure details, and remote-state tail), `targetEntityId`, rendered `emittedCommandText`, `handoffOutcome`, `handoffReason`, and `observedAt`
...
- **Target-state rule:** Automation must persist one durable handoff event per attempted emitted command, including pre-handoff rollback fencing and Game Session acceptance/rejection outcomes.
- `automationDispatchId` is the canonical low-cardinality correlation key between Automation handoff history and the Game Session gameplay-command ledger; metrics still must not label by it. Operator/debug reads can resolve the Game Session side either from the returned `gameSessionCommandId` or from the full automation identity tuple `(tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch, automationDispatchId, commandOrdinal)`, plus the applicable distinct target identity `(targetGameInstanceId, targetPlayableStateNamespaceId, targetRegionId, targetRegionEpoch)`, when the command id is not yet known to the caller; `playableStateScope` remains separately validated evidence.
...
- The persisted source and optional target fields follow the [canonical Command-Handoff Identity](./system-architecture-scripting-normative-contract-tables.md#command-handoff-identity-target-state): source `playableStateNamespaceId`, `regionId`, and `regionEpoch`, plus `targetGameInstanceId`, `targetPlayableStateNamespaceId`, `targetRegionId`, and `targetRegionEpoch` only for a distinct target, together with the dispatch and ordinal. Source and target `playableStateScope` values are separately persisted, exact-validated policy/routing/authorization/fence evidence and are excluded from identity. These complete fields are target-state only: the current live handoff proto, storage, and read remain narrower and do not establish live support for this contract.
- The read must also expose the current owned target runtime identity and routing bundle from Game Session when the target instance still exists, so operators can compare `currentTargetRuntimePlayableStateNamespaceId` and the current scope/routing bundle against the persisted distinct target identity and admitted target bundle, preserving same-target semantics without a separate ownership-status lookup.
...
Implementation note: the current Automation & Scripting implementation now persists a durable `script_patch_pin_projections` view keyed by `(tenantId, gameInstanceId)`. Automation refreshes that projection opportunistically from the shared Game Session runtime-state surface and serves `GetAutomationPinConvergence` from the persisted projection for operator visibility, so freshness and temporary Game Session read failures do not force raw pass-through reads. The current proto/implementation exposes `observedScriptPinEpoch` and `lastObservedControlPlaneRequestId`, but not `observedConvergenceAttemptGeneration`; the observed exact owner tuple is available for exact-tuple admission/replay, while attempt-bound convergence proof remains unavailable. Missing `observedScriptPinEpoch` or its associated request identity keeps exact-tuple admission/replay fail-closed. The target output below additionally requires attempt generation for the distinct attempt-bound convergence contract. Projection stale flags use the `SCRIPT_PIN_PROJECTION_STALE_THRESHOLD_MS` runtime knob; see the [cross-service version-fencing contract](./system-architecture-scripting-contracts.md#3-version-fencing-rollback-safety).
...
Implementation note: the current Automation & Scripting implementation now exposes the first durable instance-scoped timer materialization read from `script_schedule_instances`. Those rows are refreshed from the same observed Game Session pin state used by admission and rollout reads, and they project the currently pinned patch's durable schedule definitions into one `(tenantId, gameInstanceId)` scope, retaining the observed exact `scriptPatchVersion`/`scriptPinEpoch` pair and owner request identity. The rows and wire responses remain narrower than the target because namespace binding, full scheduler identity, and richer recovery proof are incomplete. Materialization is now per matching event binding rather than per raw script definition only, so each row carries target-scope identity and binding priority alongside schedule definition identity. Wall-clock timers currently compute `nextDueAt`, which maps to target durable `dueAt` plus projected `nextRunAt`, not to independent deadlines; tick-aligned schedules are persisted explicitly as `PENDING_RUNTIME_PROGRESS` until heartbeat-driven `nextTick` materialization lands.
...
- Reconciliation across repins uses the canonical stable continuity key `{stableOwnerKind, stableOwnerId, scheduleDefinitionId, playableStateNamespaceId, targetScopeType, targetScopeId}`. Plugin ownership and binding metadata must resolve to and exact-match that complete immutable schedule identity; `scheduleDefinitionId` alone, a semantic similarity inference, or a binding without the namespace cannot select continuity.
...
- **Target-state contract:** `playableStateNamespaceId` is the immutable playable-state identity/allocation dimension for applicable schedule, trigger, and command identities; `playableStateScope` remains separately persisted, exact-validated policy/routing/authorization/migration-fence evidence. Missing or mismatched namespace or scope evidence fails closed during materialization, due-candidate claim, retry, and reconciliation and remains visible on this read rather than being inferred or refreshed. See the [canonical Trigger Identity and Command-Handoff Identity tables](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields).

#### `ListScriptTimerAuditEvents`

Implementation note: the current Automation & Scripting implementation exposes the scheduler-owned subset of `script_event_audit` directly for timer troubleshooting. This read is bounded to `sourceKind=SCHEDULE_TIMER` and currently returns both timer-handler rows for persisted work and scheduler candidate rows persisted as `SCHEDULE_DROPPED`, including candidate outcomes such as `catch_up_truncated` and `runtime_scope_changed`. The live proto, record, and converter now expose the exact `scriptPinEpoch`; `playableStateNamespaceId` and target namespace/staleness fields remain target-only. Target state keeps append-only candidate history on the separate scheduler candidate-audit surface; these current `script_event_audit` rows are not that target authority.

The current read returns mutable `script_event_audit` snapshots, not append-only history: retries and later lifecycle progress update handler rows in place, while current scheduler candidate drops are persisted as `SCHEDULE_DROPPED` rows in the same table. `changedAfter` and `changedBefore` therefore filter `updatedAt`, and newest-first results order by `updatedAt` descending with `id` descending as the stable tie-breaker; an older `createdAt` does not hide a row changed after the cursor. Target-state append-only candidate history remains a separate scheduler surface.
...
- Target-state newest-first timer audit rows contain the Trigger Identity fields, including the exact `scriptPatchVersion`, `scriptPinEpoch`, and paired owner `scriptPinControlPlaneRequestId`, plus persisted non-identity `playableStateNamespaceId` evidence, resolved `playableStateScope`, admitted routing bundle, plugin owner metadata, trigger mode, scheduler source state/ordinal/due-point fields, optional `workItemId`, final stage/outcome/reason, row timestamps, and the current owned runtime scope (`currentRuntimePlayableStateNamespaceId`, `currentRuntimeGameInstanceId`, `currentRuntimeRegionId`, `currentRuntimeRegionEpoch`) plus the current owned routing bundle (`currentRuntimePlayableStateScope`, `currentRuntimeWorldSlug`, `currentRuntimeRealmSlug`, `currentRuntimePointerVersion`) and stale-namespace/scope/routing signaling beside the persisted timer row evidence. Current live output now includes `scriptPinEpoch` and its paired owner request ID; `playableStateNamespaceId`, namespace/staleness fields, and append-only candidate-history authority remain target-only.
...
- The current implementation exposes both timer-fired work that reached durable work-item persistence and scheduler candidate decisions persisted as `SCHEDULE_DROPPED` through this read. In target state, the separate scheduler candidate-audit surface is authoritative for append-only candidate history; current `script_event_audit` rows must not be interpreted as that target surface.
...
- **Target-state contract:** Timer audit retains `playableStateNamespaceId` as the identity field and `playableStateScope` as separately validated evidence from claim/admission time. Missing or mismatched namespace or scope evidence fails closed before firing and is never repaired by a current-owner read; the current output remains the narrower projection described above.
...
- The current Automation & Scripting implementation persists an Automation-owned observation keyed by `(tenantId, gameInstanceId, scriptPatchVersion)` with the observed `scriptPinEpoch` and associated owner request identity. Its refresh implementation also derives non-authoritative local projection statuses/events from observed Game Session pin state plus durable work-item/current-pin state. These local rows/events are implementation drift and diagnostics, not rollout-history authority; the target projection/read must remain exact-tuple aware and must not synthesize history from work-item transitions.
- These Automation reads are non-authoritative projection reads, not Game Session rollout-history reads. Target reads return only observed projection rows for the requested exact tuple and must not synthesize historical `PINNED`, `ROLLED_BACK`, or `REPINNED` status; current instance/patch rows and locally derived events are implementation drift and must not be treated as committed history. When an exact-tuple target projection row is absent, the read returns a deterministic not-found result. Use the authoritative Game Session-owned `ListScriptPatchInstanceRolloutEvents` target read described above—or Logging & Admin's owner-read composition once implemented—for committed rollout history; the current Automation RPC of the same name exposes only non-authoritative local projection events and is not a substitute.
...
- If the requested active version and resulting runtime state already equal the committed state, returns the existing committed state without updating `lastChangedAt`, appending lifecycle event history, emitting an advisory notification, triggering projections, or reconciling schedules/timers. The fresh request still stores one immutable digest-bound success receipt so an exact retry returns the same result and changed-input reuse conflicts after later lifecycle changes.
...
- If the plugin is already `DRAINING`, returns the existing committed state without updating `lastChangedAt`, appending lifecycle event history, or emitting a notification. The fresh no-op request still stores one immutable digest-bound success receipt. Otherwise, records owner history and must publish `PluginVersionRuntimeStateChanged(transitionKind=LIFECYCLE, newState=DRAINING)` as an advisory notification after the committed transition.
...
`TriggerScriptEvent` and equivalent ingress RPCs must classify transport/pre-domain failures before attempting the event-scope claim. Only a successfully produced, post-claim event-scope/domain decision uses the structured admission result; callers must not infer that result from a transport error.

#### Transport and admission ordering (Normative)

The following ordering is the contract boundary. The detailed event identity, enum vocabulary, and metric labels remain owned by the [normative scripting tables](./system-architecture-scripting-normative-contract-tables.md) and [observability contract](./system-architecture-scripting-observability-contract.md); this table records only their transport-versus-admission projection.

| Boundary | Examples | Result channel | Claim, audit, and metric consequence |
| --- | --- | --- | --- |
| Before event-scope claim: transport authentication or authorization | Missing/invalid workload or caller credentials; caller is not authorized for this RPC or tenant scope | Non-OK gRPC (`UNAUTHENTICATED` or `PERMISSION_DENIED`) | Do not acquire an event-scope claim, populate admission fields, write `script_event_ingress_audit`, or increment `automation_script_triggers_dropped_total`. |
| Before event-scope claim: request or schema validation | Missing/invalid required request fields, unsupported request/schema shape, malformed payload envelope, or the [ADR 0088](./decisions/adr-0088-static-and-incremental-script-output-bounds.md) ingress-payload byte bound | Non-OK gRPC `INVALID_ARGUMENT` | Do not acquire an event-scope claim, populate admission fields, write `script_event_ingress_audit`, or increment `automation_script_triggers_dropped_total`. |
| Event-scope claim | Request has passed the preceding gates; Automation atomically inserts or claims the complete event-scope identity | No final admission result yet; a concurrent retry may wait or receive a retryable transport/application signal | Create or reuse the single fenced claim and its ingress-audit record as required by [Table 1](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields); do not emit an ingress-drop metric until a typed domain decision is finalized. |
| After claim: event-scope/domain admission rejection | Reload/rollback backpressure, pin/version visibility, event-registry rejection, or a dry-run/test budget decision that the domain can classify | gRPC `OK` with `admitted=false`, typed `admissionOutcome`/`admissionReason`, and `retryAfterMs` when applicable | Finalize the claim and matching `script_event_ingress_audit` fields, and increment the Table 4 ingress-drop family once with the bounded reason. No handler audit row is created. |
| After claim: inability to produce an admission decision | Dependency/owner-read unavailability, resource exhaustion, deadline/cancellation, or internal failure before a domain decision exists | Canonical non-OK gRPC (`UNAVAILABLE`, `RESOURCE_EXHAUSTED`, deadline/cancellation status, or the applicable internal status) | Do not fabricate admission fields or an ingress-drop metric. If a claim already exists, it remains subject to its fenced retry/recovery lifecycle rather than becoming a typed rejection. |
| After handler resolution | Per-handler quota, plugin, sandbox, persistence, or handoff outcome | The ingress result remains `admitted=true`; handler-scoped result is durable in `script_event_audit` and applicable handler metrics | Do not rewrite the event-scope admission result or use an ingress-drop metric for a handler-only outcome. |

Transport authentication/authorization and request/schema validation are deliberately distinct from event-registry producer authorization and other event-scope/domain policy decisions. The former cannot establish an event claim; the latter may be classified only after the claim exists and then use the typed event-scope result.

Required fields for structured post-claim admission responses:

Once transport/authentication and request/schema gates have passed, an event-scope claim exists, and the service returns a structured admission result, the response includes:
...
- `admissionOutcome` and `admissionReason` describe the **event-scope/domain ingress decision** only. They must not be interpreted as a summary of all handler-scoped outcomes created after binding resolution.
- `TRIGGER_ADMISSION_OUTCOME_OUTPUT_BUDGET_EXCEEDED` is reserved for an event-scope emitted-work/artifact budget decision; it is not a request-envelope or schema-validation result.
...
- Only genuine post-claim event-scope/domain admission decisions use the typed `admitted`/`admissionOutcome`/`admissionReason` fields; typed `admitted=false` rejections retain the matching ingress-audit and bounded metric behavior. Transport authentication/authorization, schema or request validation, deadlines/cancellation, resource exhaustion, dependency unavailability, and internal failures that prevent producing an admission decision use canonical non-OK gRPC status and do not require admission fields, a finalized ingress-audit result, or an ingress-drop metric; the [gRPC outcome classification](./system-architecture-grpc.md#outcome-and-transport-classification) owns that split.
- Current ingress enforces `SCRIPT_OUTPUT_MAX_SERIALIZED_WORK_ITEM_BYTES` on the request `payloadJson` input envelope before durable work-item persistence. An oversized envelope raises `INVALID_ARGUMENT` before any event-scope claim, and the gRPC adapter propagates that as a non-OK transport status; it must not use `TRIGGER_ADMISSION_OUTCOME_OUTPUT_BUDGET_EXCEEDED`, create an event-scope admission result, or produce an ingress-audit/ingress-drop metric record. The legacy runtime evaluator separately enforces command-count and per-entity ceilings incrementally while expanding emitted commands. Artifact-pinned cost metadata, generated-output serialized-byte/data-dependent metering, and atomic all-or-none output persistence remain target-state gaps; the current evaluator does not provide those proofs or claim that a handler-local generated-output byte violation leaves no generated output or handoff.
...
- `resolvedHandlerCount` is a finalized ingress result, not an estimate or a summary of handler outcomes; it must not replace per-handler audit records as the source of truth. The current gRPC exception/unavailable fallback leaves the protobuf scalar at its default `0` because no finalized admission result exists; some current exception paths also serialize `admitted=false` and an error in an otherwise successful response. These fallbacks are implementation behavior, not target proof of a resolved handler count or typed admission result.
