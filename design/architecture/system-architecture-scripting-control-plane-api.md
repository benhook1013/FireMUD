...
- Every committed transition that invalidates current plugin work records its resulting `pluginActivationGeneration` exactly once. Failed operations, no-ops, and exact retries of the same committed control-plane request reuse the stored generation and do not advance it; a later same-version enable or activation has a newer generation.
...
- A committed materially changed activation or binding advances `pluginActivationGeneration`; a same-version enable or activation therefore publishes a newer generation. Exact idempotent retries and no-op requests do not mint another generation.
