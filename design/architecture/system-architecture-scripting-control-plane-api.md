...
- Current ingress enforces `SCRIPT_OUTPUT_MAX_SERIALIZED_WORK_ITEM_BYTES` before durable work-item persistence and rejects oversized payloads with `TRIGGER_ADMISSION_OUTCOME_OUTPUT_BUDGET_EXCEEDED` / `work_item_size_exceeded`. Target runtime evaluation additionally verifies the artifact-pinned cost metadata/cap digests and incrementally meters command count, per-entity count, serialized bytes, and data-dependent bounds before constructing output; those handler-local violations remain `DSL_EVAL` outcomes and cannot be reported as ingress success.
