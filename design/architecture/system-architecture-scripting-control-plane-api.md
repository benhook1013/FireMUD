...
- Plugin generation advancement follows [ADR 0111's target-state transition matrix](./decisions/adr-0111-unified-dsl-with-distinct-embedded-script-and-plugin-lifecycles.md#canonical-design): every successful committed `activePluginVersionId` change increments `pluginActivationGeneration` exactly once; binding-set changes occur only through such a version change because bindings are immutable within a published version; direct or final disable, revocation, or policy-driven disable increments once when an active generation exists; `DRAINING` and non-invalidating pending/reload bookkeeping do not; same-version re-enable after invalidation increments; and failed operations, no-ops, and exact retries do not advance it.
...
- On success, when `targetPluginVersionId` changes the active version or a same-version activation/enable re-admits work after invalidation, the generation-advancement rule above applies; the operation updates the registry for `(tenantId, gameInstanceId, pluginId)`, reconciles durable plugin-owned schedules/timers against the new version/generation fence before new trigger admission, and emits `PluginVersionRuntimeStateChanged` with the committed runtime state. Old candidates retain their displaced evidence and are not reused under the new fence.
...
- The generation-advancement rule above applies to this operation: `DRAINING` and other non-invalidating changes retain the current generation, same-version enable or activation after invalidation publishes a newer generation, and exact idempotent retries, failed operations, and no-op requests do not mint another generation.
...
- For a never-activated plugin at `pluginActivationGeneration` `0`, disabling it does not invalidate admitted work or advance the generation; the first successful activation/enable emits generation `1`. Thereafter, the generation-advancement rule above applies: an invalidating direct or final disable, revocation, or policy-driven disable advances it exactly once when an active generation exists, while non-invalidating state changes and exact retries retain the current value.
