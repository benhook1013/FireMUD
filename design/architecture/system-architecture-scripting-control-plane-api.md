...
- **Auditable and observable.** Every accepted mutating request must produce its applicable durable audit/history record. Named durable event families emit only their contracted status events; other consumers recover committed state and chronology from the owning service's durable state/history and advisory wake-ups. Consumers do not infer deterministic failed attempts from success-only change events.
...
Target state additionally exposes the Automation-owned monotonic `pluginActivationEpoch` for the scoped runtime row and resulting lifecycle history. A newly created runtime row starts at epoch `0` while no activation is admitted; the first successful activation/enable atomically establishes epoch `1`. A version switch, completed disable of an active/current lifecycle, final drain or forced drain, same-version reactivation after invalidation, or revocation of an active/current lifecycle advances the epoch exactly once. Entering `DRAINING`, never-active disable, failed operations, no-ops, and exact retries do not advance it. The current runtime/status implementation does not yet persist or expose that epoch; same-version re-enable fencing remains an implementation and proof gap.
...
- repeated `events[]` with `eventId`, `tenantId`, `gameInstanceId`, `pluginId`, `previousPluginVersionId`, `activePluginVersionId`, `pluginActivationEpoch`, `pluginState`, `statusReason`, `controlPlaneRequestId`, `actor`, and `observedAtMs`
- `error`
...
- An already-applied idempotent request (the requested active version and resulting runtime state already equal the committed state) returns the existing committed state without updating `lastChangedAt`, appending history, emitting an advisory notification, or triggering projections.
- [ADR 0119](./decisions/adr-0119-epoch-fenced-per-instance-plugin-activation.md) owns epoch advancement: a successful committed active-version change, completed disable of an active/current lifecycle, final drain or forced drain, same-version reactivation after invalidation, or revocation of an active/current lifecycle advances `pluginActivationEpoch` exactly once. Entering `DRAINING`, never-active disable, failed operations, exact idempotent retries, and no-op requests do not advance it. A notification may refresh projections after the owner history commit but cannot replace the owner history or fence acknowledgement.
...
- On success, when `targetPluginVersionId` changes the active version or a same-version activation/enable re-admits work after invalidation, the epoch-advancement rule above applies. The operation updates the Automation registry, installs the exact new version/epoch/state at Game Session through an idempotent control-plane command, and waits for durable fence acknowledgement before admitting work. Durable plugin schedules, pending work, and follow-ups reconcile asynchronously; the version-and-epoch fence, not cleanup completion, prevents displaced work from mutating gameplay. The committed owner history may publish `PluginVersionRuntimeStateChanged` as an advisory notification.
...
- If the target version is already active but the runtime state changes without invalidating or re-admitting current work, including entry into `DRAINING`, updates the registry and records the settled `PluginVersionRuntimeStateChanged` history without reconciling schedules/timers. A same-version activation/enable after an invalidating state is instead the epoch-advancing reconciliation case above.
- The epoch-advancement rule above applies to this operation: every successful activation or version switch advances the epoch, while entering `DRAINING`, exact idempotent retries, failed operations, and no-op requests do not mint another epoch.
...
- Digest-bound idempotent.
- When the plugin is not already `DRAINING`, transitions it to `DRAINING` so no new triggers are admitted while previously admitted work may complete within bounded limits.
- `DRAINING` permits already-admitted work only while its exact version, activation epoch, policy, and runtime fences pass. Bounded completion or forced timeout then advances the epoch into a non-executable state and installs the final Game Session fence; cleanup remains asynchronous.
