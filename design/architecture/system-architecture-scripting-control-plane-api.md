...
- `breakerState` (`CLOSED` or `DISABLED_DUE_TO_ERRORS`, target state; keyed to the exact active plugin version and runtime scope)
- `breakerReason` (nullable unless `breakerState=DISABLED_DUE_TO_ERRORS`; bounded value `failure_rate_breaker`)
- `breakerRevision` (Automation-owned monotonic cursor for the exact active-version breaker aggregate; target state)
- `breakerSampleGeneration` (Automation-owned monotonic sample-accounting generation for that exact aggregate; initialized to `0` for a new active version and advanced once by each reset; target state)
