...
6. **Removing published versions**
   ...
   - Do not run deletion as a separate check + manual delete sequence. `CanDeleteVersionAssets(tenantId, versionId)` is read-only and never tombstones an artifact. After eligibility is confirmed and an operator explicitly abandons retry for a Failed artifact, call `TombstoneVersionAssets(tenantId, versionId, expectedArtifactStateEpoch, tombstoneWorkflowId)` to CAS-guard the `FAILED -> TOMBSTONED` transition; reload the proof row and use its resulting epoch for `BeginPurgeVersionAssets(tenantId, versionId, expectedArtifactStateEpoch)`.
