...
flowchart TD
    ...
    Account -- auth owner: target projections unimplemented, legacy projections current --> CoordRedis
    Entity -- shared-helper participation --> CoordRedis
    Script -- target-only, unimplemented Automation ownership/writes (automation:timer:*, script-scheduler:*) --> CoordRedis
...
Coordination Redis arrows in this diagram follow ADR 0009 for the service-owned families: Game Session owns gameplay coordination state, including the generic `timer:*` family; Account owns the target registered auth-session and exact-input issuance-result projection families; and Automation & Scripting owns its documented scheduling families `automation:timer:*` and `script-scheduler:*` once implemented. Account's target `session:auth:token:*` and `session:auth:generation:*` projections are target-only/unimplemented. Current Account code writes only the legacy `session:auth:account:*` and `session:auth:tenant:*` projections and the unversioned legacy `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<sha256(connectScopeId)>:request:<sha256(requestId)>` result map. The target connect-token projection is physically distinct at `session:connect-token:v1:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>`, requires `schemaVersion: "connect-token-issuance-result-v1"`, and is not currently implemented or proven. Both Account connect-token families are reset-sensitive projection/cache state, not exact-result authority: Account SQL `connect_token_issuance_operation` and its bounded encrypted response envelope own the exact issuance result. Target readers never use the legacy map; cutover requires full Account-cohort quiescence, in-flight drain, and the configured legacy TTL/retry horizon because the legacy writer has no fence. Those Automation families are target-only/unimplemented and have no current writes. The Account connect-token families are distinct from Gateway's replay/denial state. Spring Cloud Gateway's separate narrow edge replay/denial/readiness contract is defined by the Redis ownership documents and [Gateway Architecture](./system-architecture-gateway.md); current code has the legacy replay marker, while target convergence and complete profile, ACL, and proof requirements remain incomplete. Gateway does not own general coordination state. Non-owner services (for example Entity) participate only through approved shared-helper contracts rather than ad hoc key ownership.
