...
The current static recovery validator enforces the closed credential universe and generic applicability/disposition consistency, but it does not yet enforce production's mandatory `operator-credentials` applicability or the target projector handoff. Until those owner checks exist, a validator pass cannot authorize production recovery.
...
- `sessionRecovery`
- `credentialApplicability` is an object keyed by exactly the closed nine-class credential universe: `jwt-signing-keys-jwks`, `postgres-application-credentials`, `workload-leaf`, `bridge-leaf`, `operator-leaf`, `backup-storage`, `asset-storage`, `outbound-comms`, and `operator-credentials`. The five internal classes (`jwt-signing-keys-jwks`, `postgres-application-credentials`, `workload-leaf`, `bridge-leaf`, and `operator-leaf`) are always `applicable`; production `backup-storage` and `operator-credentials` are also always `applicable` because production requires the backup binding and operator control-plane binding. `operator-leaf` is the internal operator certificate class, and `operator-credentials` is its one canonical external environment-binding/compliance mapping; they remain distinct recovery keys and each requires its own disposition and evidence. Only `asset-storage` and `outbound-comms` may be `not_applicable` in a production recovery record. Missing, unknown, or malformed class or value fails closed
