...
The current static recovery validator enforces the closed credential universe and generic applicability/disposition consistency, but it does not yet enforce the target projector handoff or enumerate the authoritative verified-point set to select and tie-check the newest eligible point. Production's mandatory `operator-credentials` applicability is now part of the static production profile, but a validator pass still cannot authorize production recovery until the owner checks exist.
...
- `newestVerifiedRestorablePointRef` resolves to one immutable `verified-restorable-point/v1` record under `design/operations/deployments/production/verified-restorable-points/`. `newestVerifiedRestorablePointDigest` must exactly equal that record's `recordDigest`, which is encoded as `sha256:` plus 64 lowercase hexadecimal characters and computed over the record's canonical UTF-8 bytes with the top-level `recordDigest` member omitted from the preimage. A digest-looking value or mutable reference is not evidence.
...
Verified-point namespaces are target-bound: production points are under `design/operations/deployments/production/verified-restorable-points/`, staging points under `design/operations/deployments/staging/verified-restorable-points/`, and hobby points under `design/operations/deployments/hobby-self-hosted/verified-restorable-points/`. The point's `environment` must exact-match both its namespace and the recovery target; it never identifies the source environment. For staging, the selected point's immutable artifact identity, digest, and lineage must exact-match the outer recovery record's `sourceEnvironmentBinding` and, when the staging recovery path supplies it, `sourceBackup`. A production-origin source still requires the separate `sanitizationEvidenceRef`; staging-origin or synthetic source does not manufacture sanitization evidence, as required by [ADR 0155](./decisions/adr-0155-automated-event-classified-post-restore-trust-reset.md#staging-sanitization-depends-on-source-data). `sourceEnvironmentBinding` remains an outer recovery-record binding because adding source fields to this exact inner point schema is prohibited.
...
The consumer dereferences the immutable record, rejects duplicate members and any schema deviation, recomputes the same RFC 8785/UTF-8 preimage and digest, and exact-matches `recordDigest` to `newestVerifiedRestorablePointDigest`. It also exact-matches `environment`, `backupArtifact.snapshotAt` to `newestVerifiedRestorablePointAt`, and `backupArtifact.artifactRef` to `backupArtifactRef`; `verification.verifiedAt` is the selected record's own per-point verification timestamp and must be no later than the assessment time, no earlier than the captured artifact, and within the separate 36-hour `backupVerifyLastSuccessAt` freshness gate. The record itself binds the target database identity, artifact identity/digest/lineage and capture time, verification operation/time, and supported restore-tool identity. The consumer must verify the record's target-environment binding and database/artifact/tool identities against the authoritative environment and backup lineage, not merely trust the fields' presence. No timestamp, object-store key, Git path, or mutable pointer substitutes for this record.
...
- `recordVersion` (positive monotonic publisher version required for a qualifying `verified` or `recovery-unverified` record)
...
- `verifiedRestorablePointRef` when `status=verified`
- `verifiedRestorablePointDigest` when `status=verified`
- `verifiedRestorablePointSnapshotAt` when `status=verified`
- `verifiedRestorablePointNamespace` when `status=verified`
- `verifiedRestorablePointEligibility` when `status=verified`; its exact shape is `{status: eligible, assessedAt: <RFC3339-UTC-timestamp>}`
...
Every qualifying `verified` record must identify a successful logical backup whose immutable verified-restorable point is no more than 24 hours old and must have `retentionDailyPoints >= 7`; a schedule declaration or pipeline timestamp without the current point evidence is insufficient. A qualifying record has a positive `recordVersion` that increases monotonically for this logical compliance record; a reused version with changed content is invalid. For `status=verified`, `verifiedRestorablePointRef` must resolve to exactly one immutable `verified-restorable-point/v1` record under `verifiedRestorablePointNamespace`, which must be exactly `design/operations/deployments/hobby-self-hosted/verified-restorable-points/`; that record's `environment` must be `hobby-self-hosted`, `verifiedRestorablePointDigest` must exact-match its `recordDigest`, and `verifiedRestorablePointSnapshotAt` must exact-match its `backupArtifact.snapshotAt`. `verifiedRestorablePointEligibility.status=eligible` and its `assessedAt` are the publisher's recorded selection observation, not authority: the consumer must recompute eligibility at the current event's trusted `assessedAt`, requiring a canonical, digest-valid record with valid artifact, database, lineage, and restore-tool bindings, `backupArtifact.snapshotAt <= assessedAt`, `verification.verifiedAt <= assessedAt`, `verification.verifiedAt >= backupArtifact.snapshotAt`, and the greatest eligible snapshot timestamp among the complete immutable hobby point set. A future, stale, zero/multiple, or tied eligible point fails closed. `lastSuccessfulBackupAt` records pipeline execution separately and cannot substitute for the point snapshot or its 24-hour gate. `recovery-unverified` records include the operator acknowledgement, timestamp, reason, and exact declared `recoveryContractFingerprint`; they must omit the verified-point fields, may omit or set `lastSuccessfulRestoreDrillAt` to `null`, make no restore-readiness claim, and must not include or claim `lastSuccessfulRecoveryRecordRef`. Verified status additionally requires a non-null successful restore-drill timestamp no more than 30 days old, the supported automated local rehearsal, current evidence, and `lastSuccessfulRecoveryRecordRef`. The Git-tracked `backup-compliance.yaml` remains a versioned audit/discovery index, not proof or release authority. For every qualifying event, the compliance owner must retain one immutable snapshot of the exact UTF-8 bytes of the existing `hobby-backup-compliance/v1` record and expose it through the logical `backupComplianceRef` and positive `recordVersion`. The canonical dereference is `(environment=hobby-self-hosted, backupComplianceRef, backupComplianceRecordVersion)`: it must resolve to exactly one retained byte sequence, from which the consumer recomputes SHA-256 and exact-matches `backupComplianceRecordDigest`; zero, multiple, mutable, changed, unavailable, or digest-mismatched bytes fail closed. The digest is event-binding evidence and is not an additional field in the hobby YAML schema. A traffic-open event therefore snapshots the qualifying record by this exact reference/version/digest tuple; supporting event evidence remains separately content-addressed under ADR 0151. The current executable intentionally fails `PREFLIGHT-BACKUP-003` closed before consuming this target schema because the durable controller read and hobby compliance validator are not implemented; checked-in shape alone is not validation. Missing or unavailable automation cannot be presented as verified proof. Restore hardening for hobby/self-hosted always fails closed for post-restore player-traffic reopen if verified actual-recovery evidence is missing, stale, or below baseline.

## Hobby Traffic-Open Evidence

The `hobby-self-hosted` operation-bound pre-release record reuses the complete production `traffic-open-pre-release-evidence/v1` schema defined above, including `schemaVersion=traffic-open-pre-release-evidence/v1`, `projectionSchemaVersion=traffic-open-record/v1`, `operationId`, `eventType`, `deploymentEventId`, `preflightReportPath`, `backupComplianceRef`, `backupComplianceRecordVersion`, `backupComplianceRecordDigest`, `actualRecoveryRecordRef`, `playerFacingTargetBoundary`, and `trafficExposure`; the consumed `evidenceRef` must appear in the finalized projection's `evidenceRefs[]`. After the durable controller finalizes a `hobby-self-hosted` first-live or reopen event, the exporter records the retained traffic-open projection at:
...
- `backupComplianceRef` identifies the logical backup-compliance record, while `backupComplianceRecordVersion` and `backupComplianceRecordDigest` bind the exact immutable record snapshot consumed for this event. The owner must not dereference a mutable current `backup-compliance.yaml` alias; it must resolve `(environment=hobby-self-hosted, backupComplianceRef, backupComplianceRecordVersion)` to one retained record, return its exact retained UTF-8 bytes, recompute the digest from those bytes, and exact-match both the version and digest binding. Accepting a changed record under the same version, a zero/multiple dereference, or a caller-supplied digest without the retained bytes fails closed
- a verified first-live event requires that immutable backup-compliance snapshot to have `status=verified`; its `lastSuccessfulRecoveryRecordRef`, the traffic-open `baselineRecoveryRecordRef`, and the traffic-open `lastSuccessfulRecoveryRecordRef` must all resolve to the same current verified finalized recovery record and its automated local rehearsal proving the environment-wide `cold_start_restore` contract
- the traffic-open `recoveryContractFingerprint` and the compliance record's fingerprint must exact-match the `recoveryContractFingerprint` value in that same verified recovery record; stale, split, or mismatched references, versions, digests, or fingerprints fail closed
- a first-live event may instead use `recoveryPosture=recovery-unverified` with explicit acknowledgement and a clear no-recovery-promise diagnostic; the same immutable backup-compliance snapshot must have `status=recovery-unverified` and its `recoveryContractFingerprint` must exact-match the traffic-open fingerprint, the event must omit `baselineRecoveryRecordRef` and `lastSuccessfulRecoveryRecordRef`, and it must not claim verified status
...
- `recoveryToolDigest`
- `recoveryContractFingerprint`
- `recoveryPointDecisionAt`, `recoveryPointId`, and `effectiveDataLossWindow` for an actual rewind. `recoveryPointDecisionAt` is the controller-owned UTC decision timestamp established only after the old-authority fence and write cutoff are active and final candidate selection is durably fixed. `effectiveDataLossWindow` is the non-negative integer ceiling of elapsed seconds from the selected point's authoritative `backupArtifact.snapshotAt` to `recoveryPointDecisionAt`, not a display string, fractional value, or alternate unit; a point captured after the decision timestamp fails closed. The controller records both values with the point binding and, immediately before destructive apply, rereads the point and recomputes the same timestamp binding and ceiling, exact-matching both to the durable controller tuple. Later no-write apply delay does not change the authorized state-loss window. For an actual rewind, exactly one recovery-point authorization branch is valid: the operator branch requires `recoveryPointApprovedBy`, `recoveryPointApprovalRef`, and `recoveryPointApprovalDigest` (`sha256:<64 lowercase hex>` over the exact retained UTF-8 bytes of one immutable closed `recovery-point-approval/v1` record). That approval record must contain exactly `approvedBy`, `approvedAt`, `recoveryPointId`, `recoveryPointDecisionAt`, and `effectiveDataLossWindow`; its values must exact-match the top-level `recoveryPointApprovedBy`, `recoveryPointId`, `recoveryPointDecisionAt`, and `effectiveDataLossWindow`, and its `approvedAt` must be a UTC timestamp. The operator branch omits `automaticDrPolicyRef`. The automatic-DR branch requires `automaticDrPolicyRef` as the exact immutable reference to a separately accepted policy/proof that explicitly enables automatic recovery for the target environment, exact-binds the same selected `recoveryPointId` and `recoveryPointDecisionAt`, authorizes a maximum loss window in the same integer-second unit that is greater than or equal to the recomputed `effectiveDataLossWindow`, and proves strict old-authority fencing and validated candidate selection for this recovery event and target boundary; it omits all operator approval fields (`recoveryPointApprovedBy`, `recoveryPointApprovalRef`, and `recoveryPointApprovalDigest`). The branches are mutually exclusive, and the automatic-DR branch is unavailable under the current baseline policy. Missing, negative, fractional, stale, unit-mismatched, non-exact, or digest-mismatched approval/policy values fail closed
...
- `sessionRecovery`
- `credentialApplicability` is an object keyed by exactly the closed nine-class credential universe: `jwt-signing-keys-jwks`, `postgres-application-credentials`, `workload-leaf`, `bridge-leaf`, `operator-leaf`, `backup-storage`, `asset-storage`, `outbound-comms`, and `operator-credentials`. The five internal classes (`jwt-signing-keys-jwks`, `postgres-application-credentials`, `workload-leaf`, `bridge-leaf`, and `operator-leaf`) are always `applicable`. The required-applicable set is environment-specific: production additionally requires `backup-storage` and `operator-credentials` because production requires the backup binding and operator control-plane binding, while non-production profiles may mark those absent external bindings `not_applicable`. `operator-leaf` is the internal operator certificate class, and `operator-credentials` is its one canonical external environment-binding/compliance mapping; they remain distinct recovery keys and each requires its own disposition and evidence. Thus only `asset-storage` and `outbound-comms` may be `not_applicable` in a production recovery record, while the non-production profile may additionally allow `backup-storage` and `operator-credentials`. Missing, unknown, or malformed class or value fails closed
...
- `smokeStatus`
- `smokeEvidence` is a non-empty list of closed `{ref, contentDigest}` entries. Each `ref` is a unique repository-relative retained JSON evidence file, and `contentDigest` is lowercase `sha256:<64 hex>` over that file's exact UTF-8 bytes. Every referenced file must pass the canonical player-experience smoke evidence validator with `executionMode=live` and `externalAuthorityProvenance=retained-external`; recovery-baseline evidence uses this content-addressed shape without promotion-only deployment-lineage binding, and the `independent-omitted` external-authority profile remains valid under the recovery contract.
...
- `credentialApplicability` is the complete, exact-key classification for the closed internal and external credential universes. The five internal classes are always `applicable`; the environment-specific profile additionally requires production `backup-storage` and `operator-credentials`, while non-production profiles may mark those absent external bindings `not_applicable`. `operator-credentials` is the canonical external mapping of the production `operator-leaf` certificate binding. `applicable` requires one valid `credentialDispositions` value and the corresponding hardening or validation evidence; `not_applicable` requires no disposition and must not carry applicable/pass evidence. Applicability is an environment binding, not a claim that absent credentials were safely restored
...
- `databaseCredentialRotation` includes the event-classified disposition, rotation job reference when applicable, affected Secret refs, durable operation identity, phase persisted before password mutation, crash-recovery or rollback outcome across database, Secret, and every authoritative consumer, and rollout/reload completion evidence proving all consumers are healthy and using the expected credential lineage before `rotated` or `rebound` is recorded. For `postgres-application-credentials`, `verified_not_restored` additionally requires a read-only target-bound PostgreSQL role-verifier check proving that the restored role's current verifier accepts the current trusted application credential value and matches its non-secret credential lineage; external Secret/provider evidence alone is insufficient, and raw password values must never be retained. If that verifier/value/lineage proof is unavailable or mismatched, the disposition is `rotated` or a fully evidenced safe `rebound` before reopen. When `postgres-admin-credentials` is used, its fresh target-cluster authority is operation-scoped evidence rather than another `credentialApplicability`, `credentialDispositions`, or `secretComplianceRefresh.freshness` key: the target cluster/database, exact allowlisted role, authority generation, binding, identities, immutable issuance artifact, and validation timestamp must match the durable operation and precede `finalizedAt`; snapshot-derived, stale, or ambiguous authority fails closed.
- `certificateReissuance` includes the event-classified disposition, workload, bridge, and operator leaf identity evidence plus peer-convergence evidence. `operator-leaf` is the internal certificate class; its one canonical external environment-binding/compliance mapping is `operator-credentials`, and the two evidence paths must refer to the same target binding without substituting one for the other
- `externalCredentialValidation.records` retains exactly one record for each class in the closed external universe. An `applicable` class uses exactly `status`, `evidenceRef`, `isolationAssertion`, `validationMethod`, `validatedAt`, `validatedBy`, and non-secret `observedValue`, with `status=pass`; a `not_applicable` class uses exactly `status=not_applicable`, `reason=credential-class-not-present`, and a non-empty immutable `evidenceRef`, with no validation fields or observed credential detail. In production, `operator-credentials` must be `applicable` and use the applicable/pass shape; its `not_applicable` shape is prohibited. A record status must match `credentialApplicability`, and any disposition or pass evidence for a non-applicable class fails closed. `observedValue` is explicitly non-secret and is limited to a resource ID, certificate/key fingerprint, or redacted presence indicator; it must never contain a password, token, private key, raw secret material, or an unredacted credential-bearing connection string
- `secretComplianceRefresh` references the refreshed `design/operations/secret-compliance/<environment>.yaml` record, the immutable evidence payload updated by restore hardening, and an exact `credentialClasses`/`freshness` key set formed from the applicable entries in the closed six-class secret-compliance projection universe: `jwt-signing-keys-jwks`, `postgres-application-credentials`, `backup-storage`, `asset-storage`, `outbound-comms`, and `operator-credentials`. The recovery `credentialApplicability` and `credentialDispositions` objects remain the exact nine-class recovery universe; the applicable `workload-leaf`, `bridge-leaf`, and `operator-leaf` classes are intentionally excluded from secret-compliance projection and are proved only by `certificateReissuance`, without duplicate secret-compliance rows. The recovery-to-compliance projector and owner handoff apply the fixed, non-caller-selectable namespace mapping `jwt-signing-keys-jwks` -> secret-compliance `jwt-signing-keys-jwks`; `postgres-application-credentials` -> secret-compliance `postgres-application-credentials`; `backup-storage` -> secret-compliance `backup-object-store-credentials`; when external asset storage is applicable, `asset-storage` -> secret-compliance `asset-store-credentials`; when outbound communications are enabled, `outbound-comms` -> secret-compliance `outbound-communications-credentials`; and `operator-credentials` -> secret-compliance `operator-credentials` for the external binding mapped from `operator-leaf`. Recovery `credentialClasses`/`freshness` retain the recovery keys, while the compliance record and evidence payload retain their compliance keys. Disabled asset and outbound integrations have no projected compliance class or freshness entry. A missing or extra projected key, an unmapped class, or a recovery/compliance cross-namespace key fails closed; missing certificate disposition or `certificateReissuance` evidence fails closed independently. New lineage/first issuance uses `lineage=new` with either `rotated` or `reissued`, `lastProvisionedAt`, and no previous pair; existing-lineage rotated or reissued replacement uses an advanced `lastRotationAt`; rebound and `verified_not_restored` preserve the exact existing field/value pair. Every selected and previous freshness timestamp is no later than `finalizedAt`. Missing or mismatched disposition-to-freshness semantics, including a reissued existing lineage using `lastProvisionedAt` or a rebound changing its timestamp, fails closed. Rebound also records the distinct rebind operation identity, retained material-lineage identity, and exact new target binding. This mapping is target-state owner/projector behavior; the current static validator now enforces the exact applicable-class coverage while the projection or handoff check remains target-state.
...
- The operation-bound scope, server-issued lock identity (`maintenanceLockTokenDigest` or the canonical opaque lock reference derived from the controller-issued `maintenanceLockToken`), immutable evidence lineage, Account projection evidence, replay-domain proof, session-policy result, and post-restore control-group evidence must match the durable controller. The plaintext token is never retained in finalized evidence. Missing, stale, mismatched, or ambiguous evidence keeps this record non-qualifying and player traffic closed.
