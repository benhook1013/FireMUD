...
The `state` label on `recovery_participant_convergence_state` is a closed enum: `blocked`, `converged`, `terminalized`, `invalidated`, or `fenced_disabled_backlog_retained`. The derived `recovery_participant_convergence_blocked` series is produced from the current source state when `state="blocked"` or `state="fenced_disabled_backlog_retained"`; it is not written as independent controller state. The `source_family` label on `recovery_participant_convergence_source_missing` is also closed; the current values are `inventory_complete` and `participant_inventory`. These labels must not carry arbitrary participant, service, workflow, or inventory-source identities.

The controller owns only the source projection: it publishes each environment's required participant-family inventory and current participant state as one all-or-nothing projection. It sets the completeness marker to `0` before changing source series, reconciles the full authoritative family set by removing or explicitly tombstoning every family absent from that inventory, including stale `recovery_required_participant_inventory` and `recovery_participant_convergence_state` series, and exposes the complete current set before setting the marker to `1`; a partial projection must never carry `complete=1`. It does not independently write the derived coverage, blocked, or source-missing recordings. The `recovery_participant_convergence_coverage_missing` recording intentionally has only `environment` and `participant` labels: it identifies missing coverage and has no `state` label. The blocked recording preserves `environment`, `participant`, and its bounded `state` while the corresponding current source state is `blocked` or `fenced_disabled_backlog_retained`; if a required source series partially disappears while the authoritative inventory remains complete, the last known blocked state is retained and readiness remains blocked until a complete projection explicitly reports a non-blocked state or tombstones that family. The global source-missing recording is derived separately from bounded `absent(...)` conditions only when the complete required source family disappears and carries only `source_family`; partial source loss uses coverage-missing state and cannot be treated as total family absence. Because a missing source family has no remaining environment label, it cannot replace environment-specific source health or readiness proof. The cumulative `recovery_participant_convergence_total` event counter is audit history only and must not drive an active blocked alert.
...
Phase names have a deliberate representation boundary. Control-plane/API transition labels may use uppercase enum names such as `AWAITING_RESUME` and `RESUME_AUTHORIZED`; persisted recovery projections and every public `expectedPhase` input use lower-snake-case values such as `ready_to_reopen` and `awaiting_resume`. Consumers map control-plane enum labels to persisted values before comparing them and never compare the two representations as raw strings.
...
- only the internal release phase may consume `RESUME_AUTHORIZED`, apply and observe the guarded `releasing` work, and advance the controller to `finalized`; only then may actual player traffic flow and only then may the checked-in recovery and traffic-open projections be exported. Any failed or ambiguous release remains fail-closed and resumable or requires audited abandonment.
