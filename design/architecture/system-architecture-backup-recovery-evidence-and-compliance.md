...
Evidence records the Account-owned authority/delegation result separately from the Game Session-owned gameplay-binding/session result. Pre-apply trusted bootstrap evidence is separate from post-apply live signer, public-JWKS, and validator convergence evidence. The JWT rotation-evidence workload is observation-only: it observes the operation and writes operation-bound evidence, but does not reconcile it. Account owns JWT signing/publication/reconciliation, the recovery controller owns recovery reconciliation, and the evidence workload cannot create a second cutover, access private material, mutate Account signing state, invalidate sessions, or request validator changes.
...
Each production and `hobby-self-hosted` `traffic-open-record/v1` projection computes its own `contentDigest` from its own complete schema-defined canonical JSON payload, excluding only `contentDigest`. The projections share only the canonicalization, encoding, and comparison rules: the payload and resulting digest are not shared between production and hobby projections or between separate `first-live` and `reopen` events. The digest is computed from the UTF-8 bytes of that projection's payload serialized with RFC 8785 JSON Canonicalization Scheme, using SHA-256; it is encoded as `sha256:` followed by 64 lowercase hexadecimal characters. An exporter recomputes the applicable projection digest for a retry and may reuse an existing event path only when the exact finalized event tuple and `contentDigest` match.
...
- `jwtHardening` includes the observation-only rotation-evidence workload reference, the stable `restoreCutoverOperationId`, the immutable restore-cutover request digest, resulting key IDs, the recorded compromise/reset `issuerAuthorityGeneration` advance, explicitly distinguished from per-token `tokenGeneration` and any other authority generation, and proof that issued-token registry snapshots whose `issuerAuthorityGeneration` is older than that recorded compromise/reset generation are rejected. Account owns JWT reconciliation and the recovery controller owns recovery reconciliation; the evidence workload only records owner results. It also includes bounded physical registry/session cleanup status; cleanup may delete only old or stale projections after the Account authority cutover and before replacement registration, replacement registry records remain after registration, and cleanup is required retained evidence but is not a wildcard-scan revocation authority
...
- Post-restore JWT rotation preserves Account Service signing/publication authority in both accepted custody states. In target non-exportable signer custody, no application workload receives private material. In the interim mounted fallback, the materialization controller may materialize/generate or prune private slots only under an Account-authorized operation and its name-scoped RBAC; it is excluded from the general observation-only rule only for that bounded operation. The rotation-evidence workload remains observation-only and never receives private material. Account owns the JWT transition and the recovery controller owns recovery reconciliation; the evidence workload does not request the transition, write either mode's separate public `jwt-jwks` projection, read or export private keys, or mutate Account signing state. Recovery evidence must never contain private signer material
