...
- `recoveryRef`
- `operationId`
- `recoveryStatus` (`finalized` in the checked-in projection; the runtime controller also uses `collecting`, `ready_to_reopen`, `AWAITING_RESUME`, `RESUME_AUTHORIZED`, `releasing`, and `finalized`)
...
- the public `resume(operationId, expectedPhase, scope, maintenanceLockToken, evidenceRef)` call must use `expectedPhase=AWAITING_RESUME`, validate the exact operation, scope, server-issued lock, immutable evidence, Account projection evidence, and replay-domain proof, then atomically record `RESUME_AUTHORIZED` without releasing the lock or traffic fence
