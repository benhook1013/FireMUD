...
- `erasureOverlayReconciliation` is bound exactly to the canonical half-open sequence interval `(artifactErasureHighWater, restoreHighWater]`. It records both immutable boundaries, sequence and integrity verification, and `sequenceDispositions[]` with exactly one canonical owner disposition for every erasure sequence in that interval, using `converged`, `terminalized`, `invalidated`, or `fenced_disabled_backlog_retained` as applicable. Aggregate counts or a replay-through high-water without per-sequence dispositions are insufficient; any gap, duplicate, unverifiable record, unavailable owner, or incomplete replay fails the gate
