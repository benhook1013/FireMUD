...
Phase names have a deliberate representation boundary. The canonical control-plane enumeration is `PAUSED`, `collecting`, `ready_to_reopen`, `AWAITING_RESUME`, `RESUME_AUTHORIZED`, `releasing`, `PARTIAL_RELEASE_RECONCILING`, and `finalized`. Control-plane/API transition labels may use enum names such as `AWAITING_RESUME`, `RESUME_AUTHORIZED`, and `PARTIAL_RELEASE_RECONCILING`; persisted recovery projections and every public `expectedPhase` input use lower-snake-case values such as `ready_to_reopen`, `awaiting_resume`, and `partial_release_reconciling`. Consumers map control-plane enum labels to persisted values before comparing them and never compare the two representations as raw strings. `partial_release_reconciling` is a persisted representation of the `PARTIAL_RELEASE_RECONCILING` phase, not a caller-selectable alias.
