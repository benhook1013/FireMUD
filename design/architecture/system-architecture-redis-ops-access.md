...
- Recovery maps every snapshot row to the immutable active issuer layout and performs the expected-value repair/removal and reservation-capacity CAS, then reads back every expected finite partition and exact reservation entry. Each durable per-partition acknowledgement must bind one exact tuple: `operationId`, maintenance operation fence, `inventorySnapshotRevision`, coverage fence and later `coverageGeneration`, active layout version/partition identity/count, the exact `issuer/<issuerId>` cutoff checkpoint including issuer generation, `issuanceFence`, complete applicable `outboxCheckpoints`, and the exact reservation identities with owner, transition, lifecycle, fence, and revision, plus the per-partition capacity tuple `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. The capacity tuple must prove both the partition-local and aggregate bounds; aggregate capacity alone is insufficient. Every post-fence transition and acknowledgement must carry both the operation fence and later `coverageGeneration`. An acknowledgement from an older layout, cutoff, snapshot, coverage generation, operation fence, partition identity, reservation set, or capacity tuple is stale and cannot authorize recovery. No acknowledgement is published for an unavailable read, malformed/conflicting value, missing partition or reservation, partial partition set, excess capacity, or unresolved obligation.
...
- Internal recovery phase contract:
  ...
  - Internal session-rebind phase
    ...
    - Before recreating a binding, it validates the complete [canonical preserved-session rebind predicate](./system-architecture-redis.md#session-and-region-binding-contract), including the target `schemaVersion=2` payload's exact `issuanceFence`, `authorityTuple.membershipAuthorityGeneration`, complete `outboxCheckpoints` set, exact equality of `authTokenIssuedAt`/`authTokenExpiresAt`/`tokenGeneration` to the registry `iat`/`exp`/`tokenGeneration`, and every token-registry, identity, authority, expiry, generation, epoch, and lease-fence predicate defined there. `session:game:*` and pre-auth transport context are not authority substitutes. Any missing, stale, ambiguous, mismatched, expired, partial, Redis-derived, or otherwise non-exact snapshot or predicate evidence blocks preserved-session rebind and reopen and records durable recovery/session evidence for the binding and predicate result. A failed predicate leaves the session connected but not gameplay-admitted; it produces command-record status `TERMINAL` with non-applied `executionOutcome = REGION_REBIND_REQUIRED` only when a gameplay command or admission command was actually attempted after valid identity and command-record establishment. No command record or identity is synthesized for a missing, invalid, or unauthenticated request, and the command outcome does not close or terminalize the connected session.
