...
- Target-state integration rule, not a current invocation instruction: after this tooling is implemented and proven, runbooks and Helm hooks may call it for any write operation on coordination prefixes. Current runbooks and Helm hooks must not invoke the unavailable CLI. Raw Redis access is limited to:
  ...
  - The external infrastructure step of a bounded `recover` operation for node-level destructive actions such as `FLUSHALL` or AOF reset. Before the external reset, the controller must own the durable `operationId` and maintenance lock, have reached `scope_paused_and_locked`, completed the protected Account authority/token cutover and replay-domain quarantine/fence, and retained immutable pre-wipe evidence bound to the exact deployment and scope. After replacement starts, the controller creates a fresh challenge bound to that `operationId`, resolved scope, and intended replacement deployment identity; a trusted deployment attestor returns the immutable `post_reset_replacement_verification`. An operator assertion or caller-supplied observation cannot satisfy it. The controller validates the attestation and independently verifies the replacement endpoint identity, ACL/configuration, empty keyspace, and deployment health before internal recovery continuation, `continueRecovery`, or protected admission can reopen.
