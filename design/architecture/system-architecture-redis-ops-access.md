...
- After the internal pause and region-creation fence, Game Session captures one immutable `GameplayBindingInventory` snapshot identified by `inventorySnapshotRevision`. The snapshot is the complete durable inventory of every `ACTIVE`, `PROVISIONAL`, and unresolved account/issuer-repair binding in the selected scope, read under repeatable-snapshot semantics at or before that revision. The operation's admission/creation fence is the immutable coverage fence for the snapshot: every later activation, replacement, terminalization, removal, or repair transition must be represented by that same operation fence and a later `coverageGeneration` before coverage can be acknowledged.
- Preserved-session recovery may consume only that exact Game Session-owned snapshot and its bound coverage evidence. It rejects a stale, partial, unavailable, ambiguous, duplicate, or Redis-derived snapshot; a current `session:game:*` query, region binding key, Redis `SCAN`, key presence/absence, or partial index is never inventory or coverage proof. Any post-fence transition not represented by the operation's later coverage generation, or any unresolved repair obligation, keeps the scope fenced and blocks preserved-session rebind and reopen.
- Recovery maps every snapshot row to the immutable active issuer layout and performs the expected-value repair/removal CAS, then reads back every expected finite partition. Each durable per-partition acknowledgement must bind one exact tuple: `operationId`, maintenance operation fence, `inventorySnapshotRevision`, coverage fence and `coverageGeneration`, active layout version/partition identity/count, the exact `issuer/<issuerId>` cutoff checkpoint including issuer generation, `issuanceFence`, and complete applicable `outboxCheckpoints`, plus the per-partition capacity tuple `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. The capacity tuple must prove both the partition-local and aggregate bounds; aggregate capacity alone is insufficient. An acknowledgement from an older layout, cutoff, snapshot, coverage generation, operation fence, partition identity, or capacity tuple is stale and cannot authorize recovery. No acknowledgement is published for an unavailable read, malformed/conflicting value, missing partition, partial partition set, excess capacity, or unresolved obligation.
