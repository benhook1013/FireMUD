...
- Active-binding inventory source and recovery contract:
  - The authoritative inventory for active-binding completeness is the Game Session-owned PostgreSQL `GameplayBindingInventory` (or an equivalent durable control-store record with the same transaction and snapshot guarantees), not `session:game:*`, a Redis index, socket memory, or Redis `SCAN`.
  - The inventory has one exact `bindingRef` row for every `ACTIVE`, `PROVISIONAL`, or unresolved account/issuer repair binding and records the tenant/game/session identity, `bindingGeneration`, issuer and `issuerIndexLayoutVersion`, lifecycle/index-obligation state, reservation state, and durable inventory revision. It is authoritative for recovery enumeration and coverage completeness, while Account remains the authority for token/account/entitlement state, `session:game:*` remains the reconnect record, and the region binding remains command-admission authority.
  - Before a tenant-local binding CAS publishes an active or provisional record, Game Session durably commits the inventory row and applicable repair obligations in one transaction. Activation, replacement, terminalization, and removal use fenced compare-and-set; ambiguous cleanup leaves the row and reservation unresolved.
  - After pause and the region-creation fence, recovery records one immutable `inventorySnapshotRevision` and reads all inventory rows and unresolved obligations at or before that revision under a repeatable snapshot. Later transitions are captured by the operation fence or a later coverage generation. The recovery owner maps every row to the active issuer layout, performs expected-value Redis/index CAS operations, reads back every finite partition, and records per-partition acknowledgements tied to the same inventory snapshot, layout, and operation fence. Missing rows, duplicate identities, unavailable snapshots, post-fence transitions without coverage, or unresolved obligations fail closed; empty Redis partitions cannot be treated as empty inventory.
- Issuer layout migration contract:
  - `issuerIndexPartitionCount` is immutable within an `issuerIndexLayoutVersion`. A change creates a distinct versioned key family and durable migration record, fences new binding/reconnect/resume admission and issuer cutoffs, rebuilds from the durable inventory, proves capacity and exact partition readback, then compare-and-sets the durable active-layout pointer. Before the pointer CAS only the old layout is authoritative; after it only the new layout is authoritative.
  - A crash leaves the old pointer active and resumes or abandons the recorded migration. The old layout is retained read-only until the post-cutover drain/reconciliation window proves no in-flight old-layout operation, then is removed by separate fenced cleanup. Missing, mixed, or ambiguous layout-pointer evidence keeps the issuer scope closed.
