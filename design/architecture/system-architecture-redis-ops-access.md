...
- Linked child result normalization: the issuer-wide and account-wide child operation/result is parent-independent and carries only child-owned lifecycle, scope, snapshot, fences, acknowledgements, and `coverageGeneration`; the parent tuple remains on the independent edge and the parent's exact consumption record. Every pre-coverage carrier uses `coverageGeneration=NOT_YET_ISSUED`, and only the atomic coverage-proven transition assigns a concrete value. `accountAdmissionFence` is concrete only for an active account-wide child or standalone account-wide flow, remains held through exact readback and coverage proof, and is atomically released with that transition; issuer-wide, local, and other non-account-wide flows use `accountAdmissionFence=NOT_APPLICABLE` plus `NO_ACTIVE_ACCOUNT_WIDE_FLOW`.
