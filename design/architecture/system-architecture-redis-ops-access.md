...
- Target-state integration rule, not a current invocation instruction: after this tooling is implemented and proven, runbooks and Helm hooks may call it for any write operation on coordination prefixes. Current runbooks and Helm hooks must not invoke the unavailable CLI. Raw Redis access is limited to:
  ...
  - The external infrastructure step of a bounded `recover` operation for node-level destructive actions such as `FLUSHALL` or AOF reset. Before the external reset, the controller must own the durable `operationId` and maintenance lock, have reached `scope_paused_and_locked`, completed the protected Account authority/token cutover and replay-domain quarantine/fence, and retained immutable pre-wipe evidence bound to the exact deployment and scope. After replacement starts, the operator must record immutable `post_reset_replacement_verification`, which the controller validates before internal recovery continuation, `continueRecovery`, or protected admission can reopen.
...
- Recovery maps every snapshot row to the immutable active issuer layout and performs the expected-value repair/removal and reservation-capacity CAS, then reads back every expected finite partition and exact reservation entry. An issuer-partition acknowledgement is valid only under the issuer-wide inventory and admission fence; a narrow-scope operation cannot publish one. Each durable per-partition acknowledgement must bind one exact tuple: `operationId`, maintenance operation fence, issuer-wide `inventorySnapshotRevision`, coverage fence and later `coverageGeneration`, active layout version/partition identity/count, the exact `issuer/<issuerId>` cutoff checkpoint including issuer generation, `issuanceFence`, complete applicable `outboxCheckpoints`, and the exact reservation identities with owner, transition, lifecycle, fence, and revision, plus the per-partition capacity tuple `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. The capacity tuple must prove both the partition-local and issuer-wide aggregate bounds; aggregate capacity alone is insufficient. Every post-fence transition and acknowledgement must carry both the operation fence and later `coverageGeneration`. An acknowledgement from an older layout, cutoff, snapshot, coverage generation, operation fence, partition identity, reservation set, or capacity tuple is stale and cannot authorize recovery. No acknowledgement is published for an unavailable read, malformed/conflicting value, missing partition or reservation, partial partition set, excess capacity, unresolved obligation, or narrow-scope inventory.
...
`post_reset_replacement_verification` is a separate post-startup continuation/resume gate, not a canonical pre-wipe gate. After the replacement starts, an immutable record verifies its endpoint identity, ACL/configuration, empty-keyspace state, and deployment health. The controller binds that record to the same `operationId`, resolved scope, and server-issued `maintenanceLockToken`, and must validate it together with the pre-wipe evidence before recovery continuation or public resume can authorize the release path. Replacement verification never authorizes the pre-wipe handoff, and pre-wipe evidence cannot substitute for it.
