...
- Every active-binding recovery inventory proof, post-fence transition, repair/readback result, acknowledgement, and preserved-session rebind decision must persist and validate one complete recovery evidence carrier. For a preserved region/tenant parent and linked global child, that carrier contains `parentRecoveryOperationId` and the current parent lifecycle, the linked child operation ID and current child lifecycle, the parent `operationFence`, the child operation fence where applicable, a distinct `coverageFence`, the exact `inventorySnapshotRevision`, and a later `coverageGeneration`. A standalone coverage operation uses its own operation ID, lifecycle, and operation fence in the coverage-operation fields but still carries a distinct `coverageFence`. All fields must match the same operation attempt, immutable snapshot, resolved scope, binding/transition identity, and readback; a local operation fence, narrow snapshot, high-water mark, or Redis-derived result never substitutes for this carrier.
