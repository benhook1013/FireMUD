...
- Active-binding inventory source and recovery contract:
  ...
  - After the internal pause and region-creation fence, the recovery owner captures the immutable inventory revision required by that contract, performs its exact expected-value repairs and finite-partition readback, and records acknowledgements bound to the same operation fence, inventory snapshot, and active layout. Missing or conflicting inventory, unavailable snapshots or reads, uncovered later transitions, duplicate identities, or unresolved obligations keep recovery blocked; Redis presence, absence, or `SCAN` never proves active-binding coverage.
