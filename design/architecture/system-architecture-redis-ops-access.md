...
- Recovery maps every snapshot row to the immutable active issuer layout and performs the expected-value repair/removal CAS, then reads back every expected finite partition. Each durable per-partition acknowledgement must be bound to the exact `operationId` and maintenance operation fence, `inventorySnapshotRevision`, coverage fence and `coverageGeneration`, active layout version with its immutable partition count/capacity and exact partition identity, the exact `issuer/<issuerId>` cutoff checkpoint (including issuer generation, `issuanceFence`, and complete applicable `outboxCheckpoints`), and an immutable capacity proof. The proof must establish the per-partition reservation count does not exceed that layout's capacity and the aggregate bound; aggregate capacity alone is insufficient. Acknowledgements from an older layout, cutoff, snapshot, coverage generation, operation fence, or capacity proof are stale and cannot authorize recovery. No acknowledgement is published for an unavailable read, malformed/conflicting value, missing partition, partial partition set, excess capacity, or unresolved obligation.
