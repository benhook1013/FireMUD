...
- Target-state integration rule, not a current invocation instruction: after this tooling is implemented and proven, runbooks and Helm hooks may call it for any write operation on coordination prefixes. Current runbooks and Helm hooks must not invoke the unavailable CLI. Raw Redis access is limited to:
  ...
  - The external infrastructure step of a bounded `recover` operation for node-level destructive actions such as `FLUSHALL` or an AOF reset. A node-level AOF reset must resolve its actual cluster blast radius as `cluster`, or have a fresh signed record from a trusted deployment attestor proving physical dedication to the exact resolved region or tenant scope, or an equivalent proven node-isolation protocol that independently proves no other scope can be affected. Whole-deployment destructive actions, including `FLUSHALL` and AOF replacement, are allowed only for resolved `cluster` scope or under that fresh dedication or equivalent proven node protocol. Region- or tenant-scoped requests must be rejected before the external handoff when the required proof is absent; a logical scope label or caller assertion never proves physical dedication. Before the external reset, the controller must own the durable `operationId` and maintenance lock, have reached `scope_paused_and_locked`, completed the protected Account authority/token cutover and replay-domain quarantine/fence, and retained immutable pre-wipe evidence bound to the exact deployment and scope. After replacement starts, the controller creates a fresh challenge bound to that `operationId`, resolved scope, and intended replacement deployment identity; a trusted deployment attestor returns the immutable signed `post_reset_replacement_verification`. An operator assertion or caller-supplied observation cannot satisfy it. The controller validates the attestation and independently verifies the replacement endpoint identity, ACL/configuration, empty keyspace, and deployment health before internal recovery continuation, `continueRecovery`, or protected admission can reopen.
...
- Every active-binding recovery inventory proof, post-fence transition, repair/readback result, acknowledgement, and preserved-session rebind decision must persist and validate one complete recovery evidence carrier. For a preserved region/tenant parent and linked global child, that carrier contains `parentRecoveryOperationId` and the current parent lifecycle, the linked child operation ID and current child lifecycle, the parent `operationFence`, the parent's distinct binding-local `coverageFence`, the child operation fence, the child's separate `coverageFence`, the exact `inventorySnapshotRevision`, and a later `coverageGeneration`. Binding-local character, account-tenant, tenant, and realm-grant recovery marks every child-only field explicitly `NOT_APPLICABLE`; it never fabricates a child identity or fence. A standalone global coverage operation uses its own operation ID, lifecycle, and operation fence in the coverage-operation fields but still carries a distinct `coverageFence`. Every applicable field must match the same operation attempt, immutable snapshot, resolved scope, binding/transition identity, and readback; a local operation fence, narrow snapshot, high-water mark, or Redis-derived result never substitutes for this carrier.
...
The external AOF/deployment reset handoff may execute only after the same durable recover operation and maintenance lock have passed these named internal evidence gates. A node-level AOF reset must resolve its actual cluster blast radius as `cluster`, or provide a fresh signed attestation of physical dedication to the exact resolved region/tenant scope, or use an equivalent proven node-isolation protocol that independently proves no other scope can be affected. A whole-deployment `FLUSHALL` or AOF replacement is permitted only for `cluster` scope or under that fresh dedication or equivalent proven node protocol; otherwise region/tenant scope is rejected before handoff.
