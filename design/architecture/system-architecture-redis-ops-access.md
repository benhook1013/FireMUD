...
- Read-only ops user (for example `coord_ops_ro`):
  ...
  - The policy identifiers and digests (`coord_ops_ro/v2` plus the SHA-256 of its canonical sorted allow/deny command and key-pattern lists, and `coord_redis_config/v1` plus the SHA-256 of its canonical normalized configuration allowlist/denylist) are the expected deployment values. A verifier must prove positive probes for every command in the `coord_ops_ro/v2` allowlist and negative probes for database-wide `SCAN`, writes, mutating expiry, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`; a caller assertion, a `@read` category check, or a configuration dump is insufficient.
...
- Preserved-session recovery may consume only that exact Game Session-owned snapshot and its bound coverage evidence. The preserved-session rebind decision, region-bridge transition, and exact readback must persist and validate the complete recovery evidence carrier, including the parent operation fence, distinct `parentCoverageFence`, applicable linked operation IDs/lifecycles and child fences or their explicit `NOT_APPLICABLE` markers, `inventorySnapshotRevision`, and later `coverageGeneration`. It rejects a stale, partial, unavailable, ambiguous, duplicate, or Redis-derived snapshot; a current `session:game:*` query, region binding key, Redis `SCAN`, key presence/absence, or partial index is never inventory or coverage proof. Any post-fence transition not represented by the complete carrier, or any unresolved binding, reservation, or repair obligation, keeps the scope fenced and blocks preserved-session rebind and reopen.
  ...
  - Linked child result normalization: the issuer-wide and account-wide child operation/result is parent-independent and carries only child-owned lifecycle, scope, snapshot, fences, acknowledgements, and `coverageGeneration`; the parent tuple remains on the independent edge and the parent's exact consumption record. Every pre-coverage carrier uses `coverageGeneration=NOT_YET_ISSUED`, and only the atomic coverage-proven transition assigns a concrete value. `accountAdmissionFence` is concrete only for an active account-wide child or standalone account-wide flow, remains held through exact readback and coverage proof, and is atomically released with that transition; the exact released value is retained as the distinct `historicalAccountAdmissionFence` and is exact-compared in the child result, linked edge, edge retry, child readback, and parent-consumption CAS. A flow that acquired account-wide fencing never changes to the `NOT_APPLICABLE` arm; issuer-wide, local, and other flows that never acquired account-wide fencing use `accountAdmissionFence=NOT_APPLICABLE`, `historicalAccountAdmissionFence=NOT_APPLICABLE`, and `NO_ACTIVE_ACCOUNT_WIDE_FLOW`.
...
| --- | --- |
| Deployment boundary | The accepted `post_reset_replacement_verification/v1` binds the exact operation, scope, source/target deployment identities, mode, startup/deployment generation, challenge consumption, and operation fence; `SAME_DEPLOYMENT` includes both new boot/startup and storage/AOF generations plus independent `postWipeEmptyStartupAttestation/v1`, while `REPLACEMENT_DEPLOYMENT` includes a distinct signed `replacementEmptyStartupAttestation/v1` bound to the same challenge, operation/fence, exact target identity, target deployment generation, and resolved scope. |
| ACL/config contract | The target matches the expected `coord_ops_ro/v2` and `coord_redis_config/v1` digests and proves successful positive probes for every allowlisted command (`GET`, `MGET`, `HGET`, `HMGET`, `HGETALL`, `SMEMBERS`, `SCARD`, `ZRANGE`, `ZRANGEBYSCORE`, `ZCARD`, `XRANGE`, `XLEN`, `SSCAN`, `HSCAN`, `ZSCAN`, `TYPE`, `TTL`, `PTTL`, `EXISTS`, `SLOWLOG\|LEN`, `CLIENT\|ID`, `MEMORY\|USAGE`, and `CLUSTER\|INFO`). Its negative probes prove no database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, or `CLUSTER NODES` access; needed server-wide telemetry comes only from the authenticated filtered control-plane endpoint. |
