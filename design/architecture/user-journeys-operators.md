...
Operator tooling should also support revoking player visibility/admission for a fork without deleting the fork outright, so support, moderation, or incident-response workflows can hide a fork temporarily while preserving its fork-local state and audit history. The canonical mechanism is the same explicit realm-access grant contract used by creator flows and authentication/discovery: revoke the tenant-scoped grant record for the affected account(s), while leaving the fork realm and its fork-local state intact. Current grant revocation exists as a partial implementation capability. Expiry/`expiresAt`-driven grant management and lifecycle automation remain target/future behavior, not current implementation proof; see the [Player Access and Session implementation tracker](../project-management/implementation-tracking/player-access-and-session.md#active-gaps).
**Target behavior, not yet fully implemented:** grant revocation or expiry removes current authority, terminates affected connected fork sessions, and blocks subsequent discovery, reconnect, and `PLAY`. When a creator or operator wants a friendly scheduled playtest ending, they close and drain the realm first and revoke grants afterward; access revocation itself is not overloaded as a drain request.
