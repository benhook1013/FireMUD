...
The service also exposes moderation tools such as reports, bans, and runtime feature toggles. Administrators review logs and configure these options through the [Admin UI](./microservices/logging-admin-service/admin-ui.md). Policies are summarized in the [Moderation Policies](./microservices/logging-admin-service/moderation-policies.md) document. Short synchronous moderation orchestration uses the shared saga helpers, while durable control-plane workflows use Temporal as described in [Transaction Strategies](./system-architecture-transactions.md). Review these guides alongside the [Security Architecture](./system-architecture-security.md) to ensure moderation actions follow platform rules.
...
For tenant-scoped runtime lifecycle, operators are not the routine owners of creator realms. `tenantAdmin` is accountable for launch, admission open/close, drain, stop/restart, playtest fork/reset/expiry, script-patch pinning, cutover, rollback, retirement, and failed-launch cleanup. Publishing as `designer` grants no lifecycle authority. Fine-grained delegated roles are deferred; future playtest delegation must keep fresh or seeded operation separate from production-derived snapshot access.

`platformAdmin` uses a distinct reasoned and audited break-glass action when an incident, abuse, safety, or billing event requires intervention. Break-glass never impersonates a tenant administrator, creates tenant membership, or bypasses data-integrity, compatibility, fencing, or readiness invariants. Platform safety or billing authority may force a realm closed or suspended; reopening must pass the ordinary publication, entitlement, quota, compatibility, readiness, and current safety gates.

Production start and capacity-increasing operations fail closed when those gates cannot be established. Stop, close, drain, failed-launch cleanup, billing-safe repair, and audit remain reachable under their proper authority so denial cannot trap unsafe or incomplete runtime state. Marketplace and public-discovery eligibility remain separate from lifecycle and recovery authority.
