...
- **Gameplay-equivalent, fairness-critical automation**
  ...
  - Canonical handoff contract:
    ...
    - Game Session first records the admission attempt in its durable command/admission ledger with a uniqueness constraint on `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, automationDispatchId, commandOrdinal)`, storing the immutable Trigger Identity, canonical due-point state, entityId, and command payload hash alongside it. For a schedule-derived handoff, the due point is required and validated as exactly one of `dueTickId:<value>` or `dueAt:<epochMillis>`; immediate event-driven handoffs omit the scheduler-only fields and both physical due fields, and are identified by their full event Trigger Identity.
  ...
  - Worked example:
    ...
    - Automation & Scripting creates the canonical trigger claim for that due point and derives a per-command `automationDispatchId=AD1`, `commandOrdinal=0`; the durable row is unique on `(tenantId=7b3b074e-d597-4e9b-b96f-4f5946d26120, gameInstanceId=9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, playableStateScope=shared, regionId=R1, regionEpoch=7, entityId=E1, scriptId=NPC_MOVE, eventType=onInterval, eventSchemaVersion=v1, scriptPatchVersion=P3, isDryRun=false, scheduleDefinitionId=S1, duePoint=dueTickId:420, triggerMode=CATCH_UP)`, plus the command-level dispatch identity and any plugin fields. It retains the complete schedule projection, including `cadence`, `unit`, `priorityTag`, `targetScopeType`, `targetScopeId`, binding priority/exclusivity, `scheduleSemanticsHash`, the pin operation's `controlPlaneRequestId`, owner/version metadata, due-point state, and `runtimeRegionId`/`runtimeRegionEpoch`.
    ...
    - Game Session inserts or reads the durable admission row identified by `(7b3b074e-d597-4e9b-b96f-4f5946d26120, 9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, shared, R1, 7, AD1, 0)`, verifies its stored immutable Trigger Identity and `duePoint=dueTickId:420`, validates the active lease, epoch, and due point for `R1`, maps the request to `tick:{tenantRegionTag}:queue:E1`, and returns `"ENQUEUED"` on the first successful materialization.
