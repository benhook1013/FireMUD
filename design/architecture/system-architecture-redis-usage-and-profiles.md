...
- **Gameplay-equivalent, fairness-critical automation**
  ...
  - Canonical handoff contract (target state):
    ...
    - Automation & Scripting then calls a Game Session gRPC/API contract such as `EnqueueAutomationCommandIfAbsent`, carrying the complete immutable Trigger Identity and child handoff identity:
      ...
      - Scheduled rows also carry `scheduleDefinitionId`, `triggerMode`, and exactly one tagged `duePoint`: `dueTickId:<value>` or `dueAt:<epochMillis>`. Immediate event-driven rows use the separate event branch, omit scheduler-only fields and `duePoint`, and carry a stable `scriptEventId`; their physical `dueTickId` and `dueAt` fields are both `NULL`.
      - Child handoff identity: `automationDispatchId` and `commandOrdinal`. `commandOrdinal` identifies the emitted command within the dispatch and is not replaced by `commandKind` or by `scriptEventId`.
    ...
    - Game Session first records the admission attempt in its durable command/admission ledger. Its uniqueness projection is the complete applicable Trigger Identity, the target runtime timeline `(targetGameInstanceId, targetRegionId, targetRegionEpoch)`, the branch-specific due point, and the child handoff identity `(automationDispatchId, commandOrdinal)`; it must not collapse admission uniqueness to a narrow target scope plus child identifiers. The row stores the immutable Trigger Identity, canonical due-point state, entityId, target timeline, and command payload hash alongside it. For a schedule-derived handoff, the due point is required and validated as exactly one of `dueTickId:<value>` or `dueAt:<epochMillis>`; immediate event-driven handoffs omit the scheduler-only fields and both physical due fields, and are identified by their full event Trigger Identity with no scheduler due point.
    - Game Session deduplicates on that durable admission record, not on Redis queue contents. Retries, duplicate gRPC delivery, or leader changes must present the same full Trigger Identity, `automationDispatchId`, `commandOrdinal`, due-point branch, `lane`, `cost_class`, and payload for the same admission; any absent, malformed, or different immutable field is a conflicting request, not a replay/no-op, and must fail closed rather than enqueueing a second command. Immediate event-driven retries must reuse the same event Trigger Identity and consistently omit a physical due point; supplying a schedule-derived due point or schedule definition is a conflicting request.
  ...
  - Worked example:
    ...
    - Automation & Scripting creates the canonical trigger claim for that due point and derives a per-command `automationDispatchId=AD1`, `commandOrdinal=0`; the source trigger-claim row is unique on `(tenantId=7b3b074e-d597-4e9b-b96f-4f5946d26120, gameInstanceId=9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, playableStateScope=shared, regionId=R1, regionEpoch=7, entityId=E1, scriptId=NPC_MOVE, eventType=onInterval, eventSchemaVersion=v1, scriptPatchVersion=P3, isDryRun=false, scheduleDefinitionId=S1, duePoint=dueTickId:420, triggerMode=CATCH_UP)`, plus the command-level dispatch identity and any plugin fields. This source trigger-claim projection is distinct from the Game Session durable admission projection, which additionally includes the complete applicable Trigger Identity, target timeline, branch-specific due point, and child identity. It retains the complete schedule projection, including `cadence`, `unit`, `priorityTag`, `targetScopeType`, `targetScopeId`, binding priority/exclusivity, `scheduleSemanticsHash`, the pin operation's `controlPlaneRequestId`, owner/version metadata, due-point state, and `runtimeRegionId`/`runtimeRegionEpoch`.
    - It calls `EnqueueAutomationCommandIfAbsent` with the complete Trigger Identity, `automationDispatchId=AD1`, `commandOrdinal=0`, `duePoint=dueTickId:420`, the target region timeline fields, and the deterministic command payload.
    - Game Session inserts or reads the durable admission row identified by the complete applicable Trigger Identity, target timeline `(targetGameInstanceId=9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, targetRegionId=R1, targetRegionEpoch=7)`, branch-specific `duePoint=dueTickId:420`, and child identity `(automationDispatchId=AD1, commandOrdinal=0)`. It verifies the stored identity and due point, validates the active lease, epoch, and due point for `R1`, maps the request to `tick:{tenantRegionTag}:queue:E1`, and returns `"ENQUEUED"` on the first successful materialization.
