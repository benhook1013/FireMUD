...
The target automation handoff also requires the complete Trigger Identity, including `scriptPinEpoch`, plus `automationDispatchId` and `commandOrdinal`. The current Game Session request carries and validates `scriptPinEpoch` (and the optional owner request identity) but remains narrower than the complete contract; `commandOrdinal`, complete identity, and uniqueness/final-fence proof below are not implementation proof.
