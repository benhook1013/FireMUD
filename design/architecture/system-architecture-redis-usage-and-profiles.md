...
- `binding_generation`
- The full automation Trigger Identity and per-command `automationDispatchId` plus `commandOrdinal` child identity. The current proto/client contract uses `commandOrdinal` as the per-work-item discriminator; it does not define a `commandKind` field, so command identity must not invent or depend on one.
...
- `script-scheduler:{tenantRegionTag}:lastTickId`
  ...
  - Purpose: per-region checkpoint for “every N ticks” schedulers tied to the canonical `(regionEpoch, tickId)` timeline. Its physical value is `{regionEpoch, latestTickId, streamOffset}`; consumers reject and rebuild it when the stored epoch differs from the authoritative epoch.
  - Reset behavior: classified as reset-tolerant in the reset policy matrix; region/tenant/cluster resets may drop this key. After resets or data loss, Automation & Scripting recomputes due work from PostgreSQL schedules and the tick heartbeat as described in the tick and scripting docs. Duplicate trigger prevention comes from a durable PostgreSQL trigger-instance or outbox row with a uniqueness projection over the applicable identity branch. For schedule-derived triggers, that branch includes `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, entityId, scriptId, eventType, eventSchemaVersion, scriptPatchVersion, scriptEventId, isDryRun, scheduleDefinitionId, duePoint, triggerMode)`, plus plugin identity when applicable. `duePoint` is exactly `dueTickId:<value>` or `dueAt:<epochMillis>`; if physical storage uses nullable `dueTickId`/`dueAt` columns, the alternate field is explicitly `NULL`, never an empty/zero substitute, and both fields may not be null or populated together. For an immediate event-driven trigger, the scheduler-only fields `scheduleDefinitionId`, `duePoint`, and `triggerMode` are absent, both physical due columns are `NULL`, and the event-driven branch includes the stable `scriptEventId` as the occurrence discriminator. Storage must represent the event-driven versus scheduler-derived branch explicitly, for example with a discriminator or branch-specific unique constraints, rather than allowing nullable scheduler fields to alias distinct event triggers. The per-command `automationDispatchId` and `commandOrdinal` are separately unique under the same runtime timeline and link the trigger claim to each emitted command. The row retains the full schedule projection when applicable (`cadence`, `unit`, `priorityTag`, `targetScopeType`, `targetScopeId`, binding priority/exclusivity, `scheduleSemanticsHash`, observed pin request, owner/version metadata, due-point state, and `runtimeRegionId`/`runtimeRegionEpoch`); it is not reduced to this Redis checkpoint or a schedule ID alone.
...
- **Gameplay-equivalent, fairness-critical automation**
  ...
  - Canonical handoff contract:
    - Automation & Scripting creates or reuses a durable PostgreSQL trigger-instance / outbox row keyed by the applicable Trigger Identity branch. A schedule-derived row uses `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, entityId, scriptId, eventType, eventSchemaVersion, scriptPatchVersion, scriptEventId, isDryRun, scheduleDefinitionId, duePoint, triggerMode)`, plus plugin identity when applicable. An immediate event-driven row omits the scheduler-only fields and uses its stable `scriptEventId` to distinguish one event occurrence from another; its storage uniqueness branch must remain distinct from scheduled rows. The immutable source Trigger Identity, including source `regionId` and `regionEpoch`, is retained on the row; current enqueue routing fields are separate. The per-command `automationDispatchId` plus `commandOrdinal` is stored as the child handoff identity, not as a replacement for the trigger identity. The row also retains `cadence`, `unit`, `priorityTag`, `targetScopeType`, `targetScopeId`, binding priority/exclusivity, `scheduleSemanticsHash`, observed pin request, owner/version metadata, due-point state, and `runtimeRegionId`/`runtimeRegionEpoch` when those fields apply.
    - Automation & Scripting then calls a Game Session gRPC/API contract such as `EnqueueAutomationCommandIfAbsent`, carrying:
      ...
      - The canonical `duePoint` for a schedule-derived handoff: exactly one tagged value, `dueTickId:<value>` or `dueAt:<epochMillis>`. The two forms are mutually exclusive; neither an empty/zero placeholder nor both forms are valid. Immediate event-driven handoffs omit `duePoint` because they have no scheduler due boundary and use the separate event-driven identity branch described above.
      - `entityId`
    ...
    - Game Session first records the admission attempt in its durable command/admission ledger with a uniqueness constraint on `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, automationDispatchId)`, storing the immutable canonical due-point state, entityId, and command payload hash alongside it. For a schedule-derived handoff, the due point is required and validated as exactly one of `dueTickId:<value>` or `dueAt:<epochMillis>`; immediate event-driven handoffs omit the scheduler-only fields and are identified by their full handler Trigger Identity.
    - Game Session deduplicates on that durable admission record, not on Redis queue contents. Retries, duplicate gRPC delivery, or leader changes for a schedule-derived handoff must present the same canonical `duePoint` and payload for the same dispatch; an absent, malformed, or different due point is a conflicting request, not a replay/no-op, and must fail closed rather than enqueueing a second command. Immediate event-driven retries must reuse the same handler Trigger Identity and consistently omit a physical due point; supplying a schedule-derived due point or schedule definition is a conflicting request.
  ...
  - Worked example:
    - A schedule for NPC `entityId=E1` becomes due at `(tenantId=7b3b074e-d597-4e9b-b96f-4f5946d26120, gameInstanceId=9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, playableStateScope=shared, regionId=R1, regionEpoch=7, duePoint=dueTickId:420)`.
    - Automation & Scripting creates the canonical trigger claim for that due point and derives a per-command `automationDispatchId=AD1`; the durable row is unique on `(tenantId=7b3b074e-d597-4e9b-b96f-4f5946d26120, gameInstanceId=9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, playableStateScope=shared, regionId=R1, regionEpoch=7, entityId=E1, scriptId=NPC_MOVE, eventType=onInterval, eventSchemaVersion=v1, scriptPatchVersion=P3, isDryRun=false, scheduleDefinitionId=S1, duePoint=dueTickId:420, triggerMode=CATCH_UP)`, plus the command-level dispatch identity and any plugin fields. It retains the complete schedule projection, including `cadence`, `unit`, `priorityTag`, `targetScopeType`, `targetScopeId`, binding priority/exclusivity, `scheduleSemanticsHash`, observed pin request, owner/version metadata, due-point state, and `runtimeRegionId`/`runtimeRegionEpoch`.
    - It calls `EnqueueAutomationCommandIfAbsent` with `automationDispatchId=AD1`, `duePoint=dueTickId:420`, the target region timeline fields, and the deterministic command payload.
    - Game Session inserts or reads the durable admission row identified by `(7b3b074e-d597-4e9b-b96f-4f5946d26120, 9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, shared, R1, 7, automationDispatchId)`, verifies its stored immutable `duePoint=dueTickId:420`, validates the active lease, epoch, and due point for `R1`, maps the request to `tick:{tenantRegionTag}:queue:E1`, and returns `"ENQUEUED"` on the first successful materialization.
    - If the same trigger retries due to gRPC timeout with the same `duePoint`, Game Session sees the same durable admission row and returns a replay/no-op outcome instead of enqueuing a second command; a retry with a different due point is rejected as conflicting identity.
