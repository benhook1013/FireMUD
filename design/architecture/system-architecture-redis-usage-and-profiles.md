...
- **Gameplay-equivalent, fairness-critical automation**
  ...
  - Canonical handoff contract:
    ...
    - Automation & Scripting then calls a Game Session gRPC/API contract such as `EnqueueAutomationCommandIfAbsent`, carrying:
      ...
      - `tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `dueTickId`
      - `entityId`
    ...
    - Game Session first records the admission attempt in its durable command/admission ledger with a uniqueness constraint on `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, automationDispatchId)` and the requested tagged due point, `entityId`, and command payload hash.
  ...
  - Worked example:
    ...
    - Game Session inserts or reads the durable admission row for `(7b3b074e-d597-4e9b-b96f-4f5946d26120, 9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78, shared, R1, 7, automationDispatchId)`, validates the active region lease and epoch for `R1`, maps the request to `tick:{tenantRegionTag}:queue:E1`, and returns `"ENQUEUED"` on the first successful materialization.
