...
- **Gameplay-equivalent, fairness-critical automation**
  ...
  - Canonical handoff contract:
    - Automation & Scripting creates or reuses a durable PostgreSQL trigger-instance / outbox row keyed by the canonical scheduler Trigger Identity: `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, entityId, scriptId, eventType, eventSchemaVersion, scriptPatchVersion, isDryRun, scheduleDefinitionId, duePoint, triggerMode)`, plus plugin identity when applicable. The per-command `automationDispatchId` is stored as the child handoff identity, not as a replacement for the trigger identity. The row also retains `cadence`, `unit`, `priorityTag`, `targetScopeType`, `targetScopeId`, binding priority/exclusivity, `scheduleSemanticsHash`, observed pin request, owner/version metadata, due-point state, and `runtimeRegionId`/`runtimeRegionEpoch`.
    ...
    - Game Session first records the admission attempt in its durable command/admission ledger with a uniqueness constraint on `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, automationDispatchId)`, storing the immutable canonical `duePoint`, `entityId`, and command payload hash alongside it. For a schedule-derived handoff, the due point is required and validated as exactly one of `dueTickId:<value>` or `dueAt:<epochMillis>`; immediate event-driven handoffs omit it.
    - Game Session deduplicates on that durable admission record, not on Redis queue contents. Retries, duplicate gRPC delivery, or leader changes for a schedule-derived handoff must present the same canonical `duePoint` and payload for the same dispatch; an absent, malformed, or different due point is a conflicting request, not a replay/no-op, and must fail closed rather than enqueueing a second command. Immediate event-driven retries must consistently omit `duePoint`; supplying one is a conflicting request.
