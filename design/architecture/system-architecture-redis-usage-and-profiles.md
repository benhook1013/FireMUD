...
The current hosted application wiring still exposes one generic `firemud.redis` client, with the shared hosted profile setting `FIREMUD_REDIS_HOST=redis-cache`; the role-specific Redis variables are present in the profile but are not consumed by the shared client configuration. Consequently, Game Session and Account coordination/auth paths cannot claim Coordination endpoint isolation in the hosted profile and may currently land on Cache/Rate-Limit Redis. This is implementation drift against ADR 0171, not a new role contract: hosted `pr-preview` and every other non-ephemeral/player-facing environment require named role clients and fail-closed endpoint-collision handling. The only exception is an explicitly labelled one-shot ephemeral CI/test stack, which may remain live after a collision only while surfacing the shared endpoint and forfeiting role-isolation, replay, and SLO evidence.

The shared `ConflictTracker` is another concrete consumer of this generic wiring: current Entity and Game Session callers emit `conflict:entity:<tenantId>:<entityId>` and `conflict:session:<tenantId>:<gameInstanceId>` through `RedisConflictTracker`, which performs separate `INCR` and `EXPIRE` calls and only warns when expiry fails. These counters are reset-tolerant diagnostics, never authority, but their current endpoint placement and mutation sequence remain implementation drift. The target role, owner, finite cardinality, TTL, ACL, and atomic-helper convergence are defined in [Redis Architecture](./system-architecture-redis.md#coordination-vs-cache-prefixes) and its [reset-policy entry](./system-architecture-redis-reset-and-recovery.md#reset-policy-matrix-prefix-summary); no current runtime proof is implied.
...
- **Coordination Redis**
  ...
  - Characteristics:
    ...
    - Game Session owns its tick/session coordination prefixes and registered scripts, as well as the live and recent presence projection responsibility: gameplay coordination families such as `tick:*`, `timer:*`, `retry:*`, `tick-executor-lease:*`, and the target-only, once-implemented `tick-events:*` stream, plus the canonical `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` records and their derived `session:game:index:*` projections. The separately approved `session:game:auth:issuer-generation:v1:*` consumer projection is a distinct family with its own ownership and recovery rules. Automation & Scripting is the target registered consumer owner for the corresponding `tick-events-offset:*` and `tick-events-lease:*` records; live integration uses `ObserveRuntimeTickProgress`. See the [tick event contract](./system-architecture-ticks.md#tick-events--heartbeat-stream). Account Service retains ownership of the current legacy `session:auth:account:*` and `session:auth:tenant:*` projections, the explicit `session:auth:token:*` and `session:auth:generation:*` families, legacy unversioned `session:connect-token:tenant:*`, and target `session:connect-token:v1:tenant:*` prefixes, and Automation & Scripting Service retains ownership of its other automation-specific prefixes and scripts as documented below; Game Session invokes those contracts but does not write them.
  ...
  - Example prefixes:
    ...
    - `session:game:auth:issuer-generation:v1:*` separately approved consumer projection
...
- **Cache/Rate‑Limit Redis**
  ...
  - Example prefixes:
    ...
    - Current Game Session `ratelimit:<sessionId>` is a separately registered legacy heuristic written through the generic client; it is not target subject authority and remains inadmissible for target shared helpers until explicit Cache admission and maximum-TTL drain/versioned migration.
...
The current `gameplaypresence:*` and `accountrecentpresence:*` physical shapes are unregistered legacy implementation drift, not Coordination or Cache/Rate-Limit role-assigned prefixes and not target authority. Their role, exact scope, atomicity, and owner readback remain unproved. The reset matrix classifies them as reset-sensitive and private-by-failure: Game Session must fence/suppress affected presence, rebuild the exact target owner projection, and complete exact readback before exposing presence again.
...
Capacity dimensions must not be inferred from a key's apparent shape. The exact tenantless deployment-wide counters are `automation:test:capacity:{automation-test-capacity}:cluster` and `automation:readiness:capacity:{automation-readiness-capacity}:cluster`; their scoped lease variants, `automation:test:capacity:{automation-test-capacity}:cluster:lease:<tenantId>:<workItemId>` and `automation:readiness:capacity:{automation-readiness-capacity}:cluster:lease:<tenantId>:<workItemId>`, remain bound to the named tenant and work item. Tenant-scoped quota keys retain their tenant identity, and work/coordination leases such as `tick-executor-lease:{tenantRegionTag}`, `tick-events-lease:{tenantRegionTag}`, and any `automation:queue:{tenantInstanceTag}:*` entries remain bound to their tenant/game/region or work scope. A tenantless capacity counter is never a substitute for a scoped lease, and a scoped lease must not be aggregated or granted an ACL pattern as though it were a deployment-wide capacity record.

The capacity-key families are a direct-convergence migration, not a dual-read authority. During a rolling deployment, readers and writers use only the canonical builders above; any legacy capacity keys may remain visible until their existing TTLs expire, but are never summed with or preferred over canonical values. A deployment must not delete or rewrite legacy keys as part of normal admission. After all old workers are gone and the legacy TTL window has elapsed, the owner may retire the legacy compatibility note; capacity counters remain bounded TTL windows and capacity leases retain their owner-defined renewal/reacquisition TTL (currently ten minutes), so expiry or reset safely releases only disposable capacity state.
...
- **Gameplay-equivalent, fairness-critical automation**
  ...
  - Prefixes: use the same current coordination families as player commands (for example `tick:{tenantRegionTag}:queue:<entityId>`, `tick:{tenantRegionTag}:pending`, and Game Session-owned `timer:{tenantRegionTag}`) only through Game Session-owned enqueue contracts. The Automation-owned scheduler checkpoint `script-scheduler:{tenantRegionTag}:lastTickId` (and `automation:timer:{tenantRegionTag}` where applicable) is target-only and unimplemented; it must not be read as a current Automation prefix or access grant.
  ...
  - Canonical handoff contract (target state):
    ...
    - Automation & Scripting then calls a Game Session gRPC/API contract such as `EnqueueAutomationCommandIfAbsent`, carrying the complete immutable Trigger Identity and child handoff identity:
      ...
      - Identity clarification: the source and target runtime scopes in the child handoff identity use `playableStateNamespaceId` and `targetPlayableStateNamespaceId` where applicable. `playableStateScope` and `targetPlayableStateScope` remain separately persisted, exact-validated policy/routing/authorization/migration-fence evidence and are excluded from child uniqueness.
...
- **Best-effort, non-critical automation**
  ...
  - Prefixes: `automation:queue:{tenantInstanceTag}:*`, `automation:quota:<tenantId>:*`, `automation:tenant-budget:<tenantId>:tier:<tier>`, `automation:test:quota:<tenantId>:script:<scriptId>:tenant`, `automation:test:quota:<tenantId>:script:<scriptId>:principal:<principalKey>`, `automation:test:capacity:{<tenant-id>}:*` where `<tenant-id>` excludes the reserved tag `automation-test-capacity`, the exact tenantless counter `automation:test:capacity:{automation-test-capacity}:cluster`, its scoped lease variant `automation:test:capacity:{automation-test-capacity}:cluster:lease:<tenantId>:<workItemId>`, `automation:readiness:capacity:{<tenant-id>}:*` where `<tenant-id>` excludes the reserved tag `automation-readiness-capacity`, the exact tenantless counter `automation:readiness:capacity:{automation-readiness-capacity}:cluster`, its scoped lease variant `automation:readiness:capacity:{automation-readiness-capacity}:cluster:lease:<tenantId>:<workItemId>`, and similar TTL-only queues, counters, and leases documented in the Redis Cache & Rate Limiting design. The two exact `:cluster` counters are deployment-wide bounded-capacity records; their `:lease:<tenantId>:<workItemId>` variants are tenant/work-scoped leases. Quota and capacity counters/leases remain TTL-only and reset-tolerant, while the current `automation:queue` reset/rebuild/resume path is unavailable and must fail closed; the target owner-reconciled queue rebuild is reset-tolerant once implemented. The `automation:queue:{tenantInstanceTag}:*` family is reserved for disposable pointers to durably admitted work and is not a silently droppable work queue; a genuinely droppable analytics or notification queue requires its own registered prefix and owner contract. None becomes durable Coordination Redis state or hard correctness authority.
  - Guarantees:
    - Cache counters and genuinely best-effort queue hints are **TTL-only, reset-tolerant**: they may reset, be dropped, duplicated, or processed late, and quota/fairness heuristics may temporarily relax or tighten.
    - Loss of an `automation:queue:*` pointer may delay or duplicate projection delivery, but Automation must rebuild/re-drive it from the durable trigger/work-item row; pointer loss is never durable-work loss.
    - Correctness (for example, “was this workflow triggered at least once?”) must come from durable trigger tables and idempotent domain logic, not from Redis contents.
  - Design rule: features that require strong ordering, fairness relative to player actions, or at-least-once guarantees **must not** rely solely on Cache/Rate-Limit queues or pointers; they either belong in the coordination category above or should use tick-adjacent outbox/saga patterns.
...
| Service | Redis Usage |
| --- | --- |
| **Account Service** | Uses **Coordination Redis** for the current legacy Account auth/session projections (`session:auth:account:*` and `session:auth:tenant:*`) plus the unversioned legacy `session:connect-token:tenant:<tenantId>:account:<accountId>:scope:<sha256(connectScopeId)>:request:<sha256(requestId)>` result map. The target-only `session:auth:token:*`, `session:auth:generation:*`, and versioned exact-result `session:connect-token:v1:tenant:<tenantId>:account:<accountId>:scope:<scopeHash>:request:<requestHash>` projection/envelope contract are not currently implemented and proven end-to-end. Target values require `schemaVersion: "connect-token-issuance-result-v1"`; target readers/writers never use the legacy family. The target named application principal is `account_coord_app`, an instance of the logical `coord_app` profile, restricted to these registered Account families; the durable Account operation/response envelope remains authoritative, and Redis is reset-sensitive projection state only. Account owns no Cache/Rate-Limit Redis prefixes. The current generic `firemud.redis.*` binding may still route a connection to the cache deployment in existing implementations; that is implementation drift, not an intentional Account cache dependency or proof of target role separation. |
| **Game Session Service** | Owns only its **Coordination Redis** tick/session families: tick queues, locks, timers, retry metadata, region leases, and Redis-backed session liveness, binding, and rebind coordination, together with their registered scripts. It also has sole read/write ownership of the **Cache/Rate‑Limit Redis** `view:room-look:*` disposable presentation/redraw helper; other services do not read or write that prefix directly. Game Session owns that prefix's presentation semantics, while the canonical [cache catalog](./system-architecture-redis-cache-reference.md) records its TTL, size, invalidation, metrics, and reset contract; shared libraries enforce defaults and guardrails without taking ownership. Durable semantic reconnect context remains Game Session-owned persistence rather than Redis session state. It does not write Account-owned current legacy `session:auth:account:*` / `session:auth:tenant:*` projections or target-only `session:auth:token:*` / `session:auth:generation:*` / `session:connect-token:v1:*` families, and it does not write Automation-owned prefixes/scripts. |
| **Automation & Scripting Service** | Owns current automation-specific prefixes such as `automation:queue:{tenantInstanceTag}:*`; the `automation:timer:{tenantRegionTag}` and `script-scheduler:{tenantRegionTag}:lastTickId` projections are target-only and unimplemented, but reserved to Automation when delivered. It does **not** own gameplay `tick:*` queues or locks. It is the target registered consumer owner for the currently unavailable `tick-events-offset:{tenantRegionTag}:{consumerId}` and `tick-events-lease:{tenantRegionTag}` records associated with the target-only, once-implemented Game Session-produced `tick-events:{tenantRegionTag}` stream; live integration uses `ObserveRuntimeTickProgress`. See the [tick event contract](./system-architecture-ticks.md#tick-events--heartbeat-stream). It reads tick heartbeats via gRPC in target-state deployments, uses PostgreSQL as the durable work source of truth, and uses **Cache/Rate‑Limit Redis** for script quotas and best-effort queue projection where documented. |
| **Spring Cloud Gateway** | **Target dual-role wiring:** Cache/Rate‑Limit Redis serves token‑bucket rate limiting and best-effort caches through a separate `gateway_cache_app` principal (the Gateway instance of logical `cache_app`) and separate deployment, while a separate `gateway_coord_app` principal (the Gateway instance of logical `coord_app`) and Coordination Redis deployment serves the narrow one-use connect-token replay marker `gateway:connect-token:{gateway-connect-token-replay-v1}:jti:<jti>`, browser-revocation deny marker `gateway:connect-token:{gateway-connect-token-replay-v1}:deny:jti:<jti>`, and readiness/fence record `gateway:connect-token:{gateway-connect-token-replay-v1}:readiness` (whose value carries `replayAdmissionFence`). Replay consumption and browser-deny writes use pinned Coordination connections and the configured threshold-satisfying `WAITAOF` acknowledgement; an ambiguous deny write is unconfirmed. The target wiring never touches tick, session, or other gameplay-coordination prefixes. The current generic Cache-bound writer emits only the untagged replay marker; it has no current deny/readiness/fence writer. Those untagged replay keys are migration-only drift and are not target ACL state. |
...
| **Other microservices (Game Logic, Entity Management, World Management, Social & Groups, etc.)** | Do not define or own gameplay coordination prefixes; they participate in Coordination Redis **only** through shared helpers and Lua descriptors owned by Game Session (for example, `tick:{tenantRegionTag}:lock:<entityId>` for tick locks). Entity Management is the narrow exception for the registered `conflict:entity:<tenantId>:<entityId>` ConflictTracker producer: target `entity_coord_app` may access only `~conflict:entity:*`, and the reset inventory must treat this reset-tolerant, non-authoritative diagnostic family as a complete family; it does not grant tick-lock or other owner-family access. The live Entity `TickLockServiceImpl` `tick:lock:<tenantId>:<entityId>` bean is an explicitly registered legacy exception/drift family, not a second lock authority, and remains fenced from target admission until removed or routed through the canonical helper. Where they cache read‑heavy aggregates, they use **Cache/Rate‑Limit Redis** and the key patterns from the Redis Cache & Rate Limiting design. |
