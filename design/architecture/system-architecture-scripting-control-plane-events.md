...
`LIFECYCLE` identifies an active-version or lifecycle transition. When no active plugin version exists, `activePluginVersionId`, `pluginActivationEpoch`, and all breaker fields (`breakerState`, `breakerRevision`, `breakerSampleGeneration`, and `breakerReason`) are `null`/`not_applicable`; lifecycle events must not fabricate breaker identity or evidence. When an active plugin version exists, `pluginActivationEpoch` and those breaker fields are populated from that exact active-version breaker aggregate, including for lifecycle events, and are never copied from another version. This nullable event projection matches `GetPluginStatus`: the status read retains the owner lifecycle epoch for a bound runtime row, while an event without an active version does not invent an epoch value. `BREAKER_TRIPPED` and `BREAKER_RESET` reuse the breaker history transition taxonomy, require the previous/new breaker states and exact `activePluginVersionId`, and leave that active version, `newState`, `pluginActivationEpoch`, and `lifecycleRevision` unchanged. Because `breakerRevision` is scoped to one exact active-version aggregate, consumers scope that cursor with `(tenantId, gameInstanceId, pluginId, activePluginVersionId)` and never apply it to another version. Consumers do not infer the transition kind from unchanged counters or row timestamps; this notification remains only a reread wake-up.
