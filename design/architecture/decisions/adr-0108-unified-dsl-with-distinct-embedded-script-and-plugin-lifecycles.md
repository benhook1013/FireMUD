...
Automation owns a monotonic `pluginActivationGeneration` for each `(tenantId, gameInstanceId, pluginId)`. The generation is a plugin runtime-state fence, independent of Game Session's script pin and `scriptPinEpoch`. Automation captures it as evidence on admitted plugin work and preserves and revalidates it through evaluation, durable persistence, handoff, staged and final effect execution, retry, replay, and recovery; a mismatch fails closed. Each committed transition that invalidates current plugin work advances the generation exactly once, including disable, revocation or policy-driven disable, and materially changed activation or binding. A later same-version enable or activation therefore uses a newer generation. Failed operations, no-ops, and exact retries of the same committed control-plane request reuse the stored result and do not advance the generation. Automation remains authoritative for this generation; Game Session consumes the applicable current generation evidence for final enforcement and does not own or mint a plugin epoch.
