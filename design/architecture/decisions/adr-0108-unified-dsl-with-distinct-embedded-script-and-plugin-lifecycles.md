# ADR 0108: Unified DSL with Distinct Embedded Script and Plugin Lifecycles
...
Automation owns a monotonic `pluginActivationGeneration` for each `(tenantId, gameInstanceId, pluginId)`. A newly created runtime row starts at generation `0` while no plugin activation is admitted; the first successful activation/enable atomically establishes generation `1` and emits that value. The generation is a plugin runtime-state fence, independent of Game Session's script pin and `scriptPinEpoch`. Automation captures it as evidence on admitted plugin work and preserves and revalidates it through evaluation, durable persistence, handoff, staged and final effect execution, retry, replay, and recovery; a mismatch fails closed. Each later committed transition that invalidates current plugin work advances the generation exactly once, including disable, revocation or policy-driven disable, and materially changed activation or binding. A later same-version enable or activation therefore uses a newer generation. Failed operations, no-ops, and exact retries of the same committed control-plane request reuse the stored result and do not advance the generation. `DRAINING` blocks new trigger admission but does not invalidate already-admitted work; exact-fenced recovery of that work remains permitted while draining. Automation remains authoritative for this generation; Game Session consumes the applicable current generation evidence for final enforcement and does not own or mint a plugin epoch.
