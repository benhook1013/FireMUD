...
Failure handling is likewise exact. A retryable pre-release failure leaves `status=RUNNING` and the operation in `PAUSED`, `collecting`, or its current pre-release phase with a durable attempt outcome; retry resumes the same operation and effect identities. A definitive pre-release failure records `status=FAILED`, preserves the observed phase, traffic fence, and operation evidence, and rejects `continueRecovery`, `resume`, and release-worker execution. Audited abandonment may release only the maintenance lock under ADR 0039 while retaining the non-gameplay-safe fence; later recovery uses a new operation after the failed scope is safely adopted. A failure after any release effect may have applied does not record terminal `FAILED`: it records phase `PARTIAL_RELEASE_RECONCILING` with `status=RUNNING`, durably inventories every effect, and contains already-released workloads before returning to `releasing`. Only `phase=finalized` with all release postconditions observed permits `status=SUCCEEDED`; neither `FAILED` nor `SUCCEEDED` is a public `expectedPhase`.
