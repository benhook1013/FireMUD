...
- Capture an immutable `restoreHighWater` from the authoritative erasure ledger, then replay the complete interval `(artifactErasureHighWater, restoreHighWater]` into the restored environment without gaps before reopen. If the authoritative high-water advances during recovery, capture the new value immutably and extend the replay interval before reopening.
...
- Reopen requires one explicit operator-authorized `continueRecovery(operationId, expectedPhase, evidenceRef)` call against the durable recovery controller in `ready_to_reopen`. The controller's durable state and linked immutable evidence are the pre-release authority; checked-in recovery or traffic-open JSON is not required for, or consulted to authorize, that same release. The controller idempotently reconciles `ready_to_reopen -> releasing -> finalized`: it applies the quarantine release, observes that the release is active, advances to `finalized`, and only then permits player traffic. A failed or ambiguous apply remains fail-closed. After `finalized`, the workflow exports checked-in recovery and traffic-open evidence as immutable projections including the later release timestamp; repository evidence is not part of the release transaction.
...
6. Replays the complete erasure interval through the immutable `restoreHighWater` and proves that the replay is gap-free before the controller may reach `ready_to_reopen`.
