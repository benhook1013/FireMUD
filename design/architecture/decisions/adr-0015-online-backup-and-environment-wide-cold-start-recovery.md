...
- Every artifact records its environment binding, database identity, snapshot time, schema/migration lineage, deployed service digests, backup-tool digest, object-storage identity, and immutable `artifactErasureHighWater`. The high-water is the greatest authoritative erasure-ledger sequence visible inside the same PostgreSQL snapshot and is recorded in the artifact manifest before that snapshot closes.
...
The public recovery-control surface has one continuation verb: `continueRecovery(operationId, expectedPhase, evidenceRef)`. It is idempotent and phase-checked: the durable controller validates the immutable evidence identified by `evidenceRef`, then serializes one atomic compare-and-set or transactional transition from `expectedPhase` for `operationId`. Exactly one durable continuation result is recorded per operation; concurrent or retried calls return that result and cannot apply quarantine release twice. The internal durable `pause/lock` phase protects the quarantined operation; `pause`, `resume`, `lock`, and `release-lock` are not standalone public recovery verbs.
