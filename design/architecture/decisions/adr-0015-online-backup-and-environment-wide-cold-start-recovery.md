...
The durable recovery controller preserves the exact phase identifiers used by the broader recovery contract: `PAUSED`, `collecting`, `ready_to_reopen`, `AWAITING_RESUME`, `RESUME_AUTHORIZED`, `releasing`, `PARTIAL_RELEASE_RECONCILING`, and `finalized`. Durable operation status is separate and uses `RUNNING`, `FAILED`, or `SUCCEEDED`. Public `expectedPhase` is a lower-snake-case wire precondition, not a second controller state; the controller maps it through this fixed table and rejects case variants or aliases:

| Public wire `expectedPhase` | Durable controller phase checked or recorded |
| --- | --- |
| `ready_to_reopen` | `ready_to_reopen` |
| `awaiting_resume` | `AWAITING_RESUME` |

`PAUSED`, `collecting`, `RESUME_AUTHORIZED`, `releasing`, `PARTIAL_RELEASE_RECONCILING`, and `finalized` are internal durable phases and have no public `expectedPhase` value. `continueRecovery` accepts only the first mapping and records `AWAITING_RESUME`; `resume` accepts only the second mapping and records `RESUME_AUTHORIZED`. Durable phase state and audit use these exact spellings. External JSON evidence may render a phase through a separately documented lower-snake-case projection field, but that representation is not another controller phase and cannot be sent as an alias. No case normalization is permitted.

Failure handling is likewise exact. A retryable pre-release failure leaves `status=RUNNING` and the operation in `PAUSED`, `collecting`, or its current pre-release phase with a durable attempt outcome; retry resumes the same operation and effect identities. A definitive pre-release failure records `status=FAILED`, preserves the observed phase, traffic fence, and operation evidence, and rejects `continueRecovery`, `resume`, and release-worker execution. Audited abandonment may release only the maintenance lock under ADR 0039 while retaining the non-gameplay-safe fence; later recovery uses a new operation after the failed scope is safely adopted. A failure after any release effect may have applied does not record terminal `FAILED`: it records phase `PARTIAL_RELEASE_RECONCILING` with `status=RUNNING`, durably inventories every effect, and contains already-released workloads before returning to `releasing`. That phase is not eligible for audited abandonment or `release-lock`; remediation resumes the same operation until durable successful observation evidence exists for every applicable release postcondition. Only `phase=finalized` permits `status=SUCCEEDED`, and neither `FAILED` nor `SUCCEEDED` is a public `expectedPhase`. The only direct-finalization exception to the continuation/release lifecycle is a strictly observational dry run that acquires no gameplay fence or maintenance lock and performs no mutation or release effect. After recording those no-effect conditions, it transitions to `finalized` before recording `SUCCEEDED`; because it has no release effects, it has no release postconditions to satisfy. Any run that acquires a fence or lock or changes runtime state follows the normal continuation and release lifecycle.
...
- The durable controller transactions `ready_to_reopen -> AWAITING_RESUME` and `AWAITING_RESUME -> RESUME_AUTHORIZED` are the only public authorization transitions. They do not make Game Session, Coordination Redis, ingress, affected-scope, or maintenance-lock effects one distributed transaction. Only the internal release worker may consume `RESUME_AUTHORIZED`, transition through `releasing`, and finalize the operation. While `releasing`, the controller must idempotently apply and read back the required external postconditions: Game Session admission and every affected region's scope gate are correct, Redis has the expected post-recovery epochs and metadata with no usable stale lock/session state, ingress quarantine is released only through an idempotent apply and its open/readiness is observed, and the maintenance lock is released and observed as released.
- The controller may record terminal `SUCCEEDED` only after `phase=finalized` and every required external effect has a successful, current observation whose evidence is durably recorded. Retryable pre-release and release failures follow the exact `RUNNING` rules above; conclusive pre-release failure records `FAILED`, while ambiguous partial release enters `PARTIAL_RELEASE_RECONCILING` rather than pretending rollback or terminal failure. Retries reconcile from durable state and must not repeat a successful external effect. After `SUCCEEDED`, the workflow exports checked-in actual-recovery and traffic-open evidence as immutable projections including the later release timestamp; repository evidence is not part of the release transaction.

### Recovery Continuation Contract

The public recovery-control surface has two gated verbs: `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` and `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)`. Both verbs use one canonical authorization contract: the controller authenticates the current caller as the actor or workload bound to the server-issued `maintenanceLockToken`, derives the environment and affected scope from that durable token binding, and rejects any caller, environment, scope, operation, or evidence reference that does not match it. Callers cannot mint or narrow that authority, and a previously authenticated response does not authorize a different actor or workload after the binding changes. The authorized continuation uses `expectedPhase=ready_to_reopen`, validates the token against the durable operation scope and identity plus the immutable evidence, and atomically moves the operation to `AWAITING_RESUME`; it does not authorize release. That transition records a durable `AWAITING_RESUME` continuation record containing the accepted token binding, operation identity, scope, and `evidenceRef`. The authorized `resume` call uses `expectedPhase=awaiting_resume` and must apply the same current-actor/workload, environment, scope, operation, token, and evidence checks against that continuation record before atomically moving the operation to `RESUME_AUTHORIZED`; any mismatch fails without mutation. Callers retry the exact tuple for the phase they own and do not submit a later public `expectedPhase=releasing` call. Only the internal release worker may consume `RESUME_AUTHORIZED`, reconcile external effects through `releasing` or `PARTIAL_RELEASE_RECONCILING`, and finalize the operation. The compare-and-set/transaction makes only each durable controller transition atomic; it does not commit Game Session, Coordination Redis, ingress, affected-scope, or maintenance-lock effects atomically with that row change. A retryable or ambiguous external effect leaves the current internal phase durable and returns a retryable attempt outcome; a retry resumes observation or an idempotent effect step without applying a confirmed effect twice. An expected-phase, token, scope, identity, or evidence mismatch fails without mutation and is not cached as the operation result. Concurrent calls with the same tuple observe the same durable attempt or final result, conflicting tuples fail closed, and each phase transition has one durable attempt record; retryable and ambiguous attempts are not terminal results. The internal durable `pause/lock` phase protects the quarantined operation; `pause`, `lock`, and `release-lock` are not standalone public recovery verbs.
