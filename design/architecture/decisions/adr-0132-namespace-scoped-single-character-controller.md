# ADR 0132: Namespace-Scoped Single Character Controller
...
FireMUD permits only one active gameplay controller for one durable character state. The previous uniqueness key included `gameInstanceId`. That is too narrow after ADR 0122 separated replaceable runtime identity from stable `playableStateNamespaceId`.
