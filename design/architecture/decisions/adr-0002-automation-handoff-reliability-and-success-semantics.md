...
Scripting produces commands that must flow into the tick system. The historical design included Redis-based staging (the former `automation:tick:*` path), which is retired and not live; current automation stages work through `automation:queue:*` discovery pointers and hands gameplay enqueue to Game Session. The target design records script work items in a durable PostgreSQL outbox and publishes instance-aware queue pointers in Cache/Rate-Limit Redis. Queue pointers are disposable discovery indexes, not work truth; durable outbox rows and Game Session handoff custody determine recovery and success. The current implementation has narrower work-item persistence and queue-pointer paths, but does not yet prove this accepted durable publication and custody boundary. Without a clear contract, “success” can mean either “DSL ran” or “effects will happen”, which undermines rollback semantics and operator trust.
