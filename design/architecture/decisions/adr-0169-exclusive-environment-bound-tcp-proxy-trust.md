# ADR 0169: Exclusive Environment-Bound TCP Proxy Trust
...
Trust-profile expiry and emergency identity withdrawal are separate boundaries. Gateway enforces migration and break-glass expiry at runtime by stopping the dedicated listener and terminating its established bridges. For emergency withdrawal of an otherwise unexpired identity, the environment controller must first invalidate the withdrawn identity in Gateway's active trust profile, then disruptively terminate the old Gateway pods and their established bridges even when replacement credentials or replacement pods are unavailable; admission through replacements is allowed only after the new allowed identity and trust are established. Rollout strategy or rendered preflight alone is not withdrawal proof.
