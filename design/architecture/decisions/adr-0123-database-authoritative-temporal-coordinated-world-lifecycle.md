...
Activation may commit only from `PREPARING` under the current lifecycle epoch after every required preparation result is durable and admission checks still pass. A termination request received during `PREPARING` uses the same fence to transition the instance to `TERMINATING`; this makes any stale activation compare-and-set fail. Every transition into `TERMINATING`, including direct `PREPARING -> TERMINATING`, creates or binds the stable cleanup request identity and durably freezes the required-owner set with its ownership-registry revision before invoking cleanup. All cleanup progress, retries, repair, and finalization must use and exact-compare that frozen snapshot. A preparation workflow must not reopen or activate an instance after that transition.

When preparation cannot converge, the lifecycle row enters `FAILED_PRE_ACTIVATION`, which is terminal for admission and activation of that `gameInstanceId` but not terminal for cleanup. The coordinator uses the same cleanup-request-bound transition rule to move `FAILED_PRE_ACTIVATION -> TERMINATING` before invoking owner cleanup. Cleanup progress is recorded separately in durable owner-scoped state, including the frozen snapshot, the owners' acknowledgements, failures, and last progress time. Retries and repair remain in `TERMINATING`; a failed finalization stays there until every required owner acknowledgement is durable, so cleanup never changes the failed instance into an admissible state.
...
Replacement cutover adds a separate World-owned one-shot hold record; it does not add a lifecycle state and does not create a distributed transaction. After World commits the target `ACTIVE` transition, it atomically locks source and target lifecycle rows in stable order, requires their exact `ACTIVE` epochs, rejects a conflicting nonterminal hold, and commits one hold bound to the prepared-upgrade identity, exact control-plane request/normalized digest, tenant and realm selector, namespace, source/target instance and version pairs, exact lifecycle proofs, expected Game Session pointer version, opaque equality-only hold fence, and authoritative World-DB `expiresAt`. Every source or target termination CAS includes absence of a nonterminal hold. Game Session binds that hold and both lifecycle proofs in its local pointer/audit/prepared-execution/source-cleanup/drain-fence transaction; World finalizes only after authoritative post-swap readback proves that transaction committed. Exact retries reuse the identity and fence. Lost or contradictory responses reconcile by exact hold identity and owner-local reads; abort requires proof that the pointer transaction did not commit and the prior pointer remains authoritative, otherwise the hold stays `RECONCILIATION_REQUIRED` and blocks termination. Expiry is diagnostic/repair input only and never auto-releases the hold.
