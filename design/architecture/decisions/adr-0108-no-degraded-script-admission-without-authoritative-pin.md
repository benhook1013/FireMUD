...
This failure closes new instance-bound gameplay/runtime script and plugin-trigger admission for the affected scope, not ordinary gameplay by default. Game Session may continue player commands and other non-script runtime work when its normal gameplay dependencies and exact execution fences remain healthy. Already admitted Automation work retains its captured script version and epoch; plugin work additionally retains the exact `(pluginId, pluginVersionId, bindingId)`, captured `(pluginActivationEpoch, lifecycleRevision)` pair, and applicable activation/lifecycle, component-policy, capability-grant, and signer-policy evidence. At each applicable Automation persistence and handoff boundary and final Game Session execution boundary, the script tuple and independent plugin fence are revalidated against current authoritative Game Session and Automation state respectively, subject only to the bounded same-version, same-epoch `DRAINING` predecessor-revision exception defined above; Automation remains authoritative for plugin activation/lifecycle/epoch/policy/capability/signer evidence, while Game Session remains authoritative for the script-pin and final command-execution fences.
