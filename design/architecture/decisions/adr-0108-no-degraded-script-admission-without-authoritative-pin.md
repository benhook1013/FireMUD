...
Instance-bound plugin work carries two independent fences: script provenance must match the exact Game Session `(scriptPatchVersion, scriptPinEpoch)`, while plugin provenance must match the current authoritative activation/lifecycle tuple `(pluginId, pluginVersionId, bindingId)` and applicable component-policy, capability-grant, and signer-policy evidence. A plugin-only activation or update changes only the separate plugin fence and does not advance `scriptPinEpoch`; subsequent instance-bound plugin work still validates both its captured script tuple and the current plugin fence.
...
This failure closes new instance-bound gameplay/runtime script and plugin-trigger admission for the affected scope, not ordinary gameplay by default. Game Session may continue player commands and other non-script runtime work when its normal gameplay dependencies and exact execution fences remain healthy. Already admitted Automation work retains its captured script version and epoch; plugin work additionally retains the exact `(pluginId, pluginVersionId, bindingId)` and applicable activation/lifecycle, component-policy, capability-grant, and signer-policy evidence. At each applicable Automation persistence and handoff boundary and final Game Session execution boundary, the script tuple and independent plugin fence are revalidated against current authoritative Game Session and Automation state respectively; Automation remains authoritative for plugin activation/lifecycle/policy/capability/signer evidence, while Game Session remains authoritative for the script-pin and final command-execution fences.
