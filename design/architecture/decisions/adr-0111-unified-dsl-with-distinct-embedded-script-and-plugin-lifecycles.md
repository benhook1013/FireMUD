...
One DSL runtime with distinct embedded-script and linked-plugin lifecycles is target state. FireMUD has signed ZIP intake, immutable plugin metadata and asset storage, publication rows, runtime plugin state, activation preflight, policy reconciliation, durable script work items, and shared sandbox seams. Current intake requires an allowlisted Ed25519 signature, parses only part of the documented plugin manifest, and does not yet demonstrate one complete bundle-to-compiled-runtime path for bindings, component requirements, capability grants, or unsigned platform-attested intake; the current evaluator is also narrower than the complete graph runtime described by the target architecture.
...
Plugin intake must persist an immutable digest, complete manifest and content identity, provenance channel, optional author-signature evidence, explicit approver and approval scope, platform acceptance attestation, requested components and capabilities, granted exact-version capabilities, compatibility evidence, and publication status. Activation must reject digest or attestation mismatches, missing approval, incompatible target contracts, absent grants, prohibited unsigned origin, and stale or revoked provenance without partially changing runtime state.
