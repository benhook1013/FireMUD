...
Automation owns a monotonic `pluginActivationGeneration` for each `(tenantId, gameInstanceId, pluginId)`. A newly created runtime row starts at generation `0` while no plugin activation is admitted; the first successful activation/enable atomically establishes generation `1` and emits that value. The generation is a plugin runtime-state fence, independent of Game Session's script pin and `scriptPinEpoch`. Automation captures it as evidence on admitted plugin work and preserves and revalidates it through evaluation, durable persistence, handoff, staged and final effect execution, retry, replay, and recovery; a mismatch fails closed. Each later committed transition that invalidates current plugin work advances the generation exactly once: a successful `activePluginVersionId` change advances it, and any binding-set change is represented only by such a version change because bindings are immutable within a published plugin version; no in-place `bindingId` or binding mutation exists. A direct `ENABLED -> DISABLED` transition, the final `DRAINING -> DISABLED` transition, revocation, or policy-driven disable advances it once when an active generation exists. Entering `DRAINING` does not advance the generation: it blocks new trigger admission while exact-fenced completion and recovery of already-admitted work remain permitted. Once an invalidating transition commits, the newer generation rejects that work; a same-version re-enable after that state advances once to another newer generation. Pending-version or reload bookkeeping that does not change the active version or invalidate or re-admit current work does not advance the generation. Failed operations, no-ops, and exact retries of the same committed control-plane request reuse the stored result and do not advance the generation. Automation remains authoritative for this generation; Game Session consumes the applicable current generation evidence for final enforcement and does not own or mint a plugin epoch.

The target-state transition matrix is:

| Committed lifecycle mutation | Generation result | Work/admission consequence |
| --- | --- | --- |
| New runtime row with no activation | `0` | No plugin trigger admission. |
| First successful activation/enable | Atomically establish `1` | Emit generation `1`; admit only work carrying that generation. |
| Enter `DRAINING` | Unchanged | Block new trigger admission; exact-fenced admitted work may complete/recover. |
| Successful `activePluginVersionId` change, including a binding-set change represented by that version change | Atomically increment exactly once | Reject work carrying the displaced generation after commit; no in-place `bindingId` or binding mutation exists. |
| Direct `ENABLED -> DISABLED`, final `DRAINING -> DISABLED`, revocation, or policy-driven disable with an active generation | Atomically increment exactly once | Reject work carrying the displaced generation after commit. |
| Same-version re-enable after an invalidating state | Atomically increment to a newer generation | Admit only work carrying the newer generation. |
| Pending-version or reload bookkeeping without an active-version change or current-work invalidation/re-admission | Unchanged | Preserve the current generation and work fence. |
| Failed operation, no-op, or exact retry of a committed request | Unchanged; reuse stored result | No generation advance. |

Each row is one Automation-owned atomic runtime-state mutation; no row advances for an uncommitted or duplicate attempt.
...
Plugin runtime proof must cover entry into `DRAINING` without a generation advance; in-flight completion and restart recovery under the unchanged exact fence while draining; one generation advance for each successful `activePluginVersionId` change, with any binding-set change represented by that version change because bindings are immutable within a published plugin version; one advance for direct `ENABLED -> DISABLED`, final `DRAINING -> DISABLED`, revocation, or policy-driven disable when an active generation exists; rejection of the displaced generation after each such transition; same-version re-enable after an invalidating state under a newer generation; unchanged generation for pending-version or reload bookkeeping that does not change the active version or invalidate or re-admit current work; exact-request idempotency without an additional advance; and generation mismatch rejection at every evaluation, persistence, handoff, staged/final effect, retry, replay, and recovery boundary. The proof must keep the Automation-owned plugin generation independent from Game Session's script pin epoch.
