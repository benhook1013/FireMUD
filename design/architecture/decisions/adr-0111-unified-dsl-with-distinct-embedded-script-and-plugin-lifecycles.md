...
Plugin runtime proof must cover entry into `DRAINING` without a generation advance; in-flight completion and restart recovery under the unchanged exact fence while draining; one generation advance at the final disable, revocation, or policy-driven invalidation; rejection of the displaced generation after that transition; same-version re-enable under a newer generation; exact-request idempotency without an additional advance; and generation mismatch rejection at every evaluation, persistence, handoff, staged/final effect, retry, replay, and recovery boundary. The proof must keep the Automation-owned plugin generation independent from Game Session's script pin epoch.
