...
Automation owns a monotonic `pluginActivationGeneration` for each `(tenantId, gameInstanceId, pluginId)`. A newly created runtime row starts at generation `0` while no plugin activation is admitted; the first successful activation/enable atomically establishes generation `1` and emits that value. The generation is a plugin runtime-state fence, independent of Game Session's script pin and `scriptPinEpoch`. Automation captures it as evidence on admitted plugin work and preserves and revalidates it through evaluation, durable persistence, handoff, staged and final effect execution, retry, replay, and recovery; a mismatch fails closed. Each later committed transition that invalidates current plugin work advances the generation exactly once, including the final transition from `DRAINING` to `DISABLED`, revocation or policy-driven disable, and materially changed activation or binding. Entering `DRAINING` does not advance the generation: it blocks new trigger admission while exact-fenced completion and recovery of already-admitted work remain permitted. Once the final invalidating transition commits, the newer generation rejects that work; a later same-version enable or activation uses another newer generation. Failed operations, no-ops, and exact retries of the same committed control-plane request reuse the stored result and do not advance the generation. Automation remains authoritative for this generation; Game Session consumes the applicable current generation evidence for final enforcement and does not own or mint a plugin epoch.
...
Plugin runtime proof must cover entry into `DRAINING` without a generation advance; in-flight completion and restart recovery under the unchanged exact fence while draining; one generation advance for materially changed activation or binding that invalidates current plugin work and at the final disable, revocation, or policy-driven invalidation; rejection of the displaced generation after each such transition; same-version re-enable under a newer generation; exact-request idempotency without an additional advance; and generation mismatch rejection at every evaluation, persistence, handoff, staged/final effect, retry, replay, and recovery boundary. The proof must keep the Automation-owned plugin generation independent from Game Session's script pin epoch.
