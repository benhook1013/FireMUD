...
This decision is not implemented. Lifecycle routes remain primarily internal or operator-oriented; tenant-scoped authority, action-class gates, recovery availability, complete entitlement composition, and audited break-glass proof (including privileged-control and target-generation freshness) remain absent or partial.
...
`platformAdmin` intervention is a distinct control-plane action, not impersonation of a `tenantAdmin` and not an implicit tenant membership. Every break-glass mutation carries exactly one explicit target scope from the union `TENANT_WIDE(tenantId)`, `REALM(tenantId, worldSlug, realmSlug)`, or `INSTANCE(tenantId, gameInstanceId)`. For a playtest `REALM` or `INSTANCE`, the exact target additionally carries `playtestLifecycleId`; its owner workflow must read and exact-match the current lifecycle generation and admission-pointer fence/version before force-close, rollback, cleanup, or any equivalent mutation, with missing, stale, or mismatched evidence failing closed into reconciliation under [ADR 0137](./adr-0137-isolated-playtest-state-modes-and-reset.md). Cleanup of a preparation that failed before any admission pointer existed instead exact-matches the current durable preparation's `playtestLifecycleId`, playtest-state generation, and stable preparation/request identity; consumes owner-defined authoritative proof that no pointer existed, or exact-matches a `CLOSED` pointer for that generation; and limits deletion to the owner-manifest resources recorded for that preparation. Absence alone is not cleanup authority, and once a pointer has existed, cleanup requires its exact current fence/version. Non-playtest `REALM(tenantId, worldSlug, realmSlug)` and `INSTANCE(tenantId, gameInstanceId)` targets retain their exact shapes. A tenant-wide action covers current and later tenant-owned resources through the owner workflow and reconciliation; realm and instance actions remain exact. Each mutation requires an authenticated platform actor, bounded action type, reason, audit record, and the same stable idempotency and owner-workflow contracts as the corresponding routine mutation.

Break-glass can force closure, suspension, drain, stop, cleanup, rollback, or another explicitly supported recovery action when platform safety, abuse, billing, or incident response requires it. Tenant-wide actions fan out through the owning workflow/reconciliation path to current and later tenant resources; realm and instance actions affect only their exact target. It may initiate a launch or cutover only through the canonical preflight and lifecycle path and cannot override integrity or readiness failures. Reopening any target is a new routine lifecycle action under the ordinary gates. Break-glass grants no gameplay actor, hidden player presence, content authorship, or continuing tenant role.

Every tenant-bearing `TENANT_WIDE`, `REALM`, and `INSTANCE` break-glass mutation requires an active server-side `privileged_control` window established by recent reauthentication plus independent TOTP. The authorization and any opaque authorization reference must bind the exact current `target_tenant_generation`; the owner mutation tuple must retain and bind that same generation to the exact scope, target, action, digest, reason, and reference. Immediately before the owner mutation, the owner workflow rereads the authoritative current `target_tenant_generation` and exact-matches it against the bound authorization/reference and mutation tuple. Missing, expired, revoked, stale, unverifiable, or mismatched assurance/window or generation evidence fails closed; a selected tenant, global role, ordinary reauthentication, or other account-level generation cannot substitute for either predicate.
...
Gate proof must cover unpublished or unattested releases, incompatible tuples, missing remaps, stale readiness, entitlement outage, hard billing denial, quota excess, safety suspension, and successful cleanup after failed preparation. It must show stop, close, cleanup, billing repair, and audit remain reachable under the correct authority while start or expansion is blocked. Break-glass proof must cover mandatory reason and actor attribution, no tenant impersonation, no gameplay or design grant, no integrity bypass, forced suspension, and ordinary-gated reopen. For each tenant-bearing target scope, it must reject absent, expired, revoked, stale, unverifiable, or independently-TOTP/reauth-mismatched `privileged_control` assurance, and reject absent or stale `target_tenant_generation`, any authorization/reference-to-tuple mismatch, and a target-generation advance before the immediate owner-mutation reread. Playtest realm/instance break-glass proof must include the exact `playtestLifecycleId` plus current lifecycle-generation and admission-pointer-fence readback, including stale, missing, and mismatched rejection; non-playtest exact targets remain separately covered. Future delegated playtest proof must distinguish fresh or seeded operation from production-derived snapshot access.
