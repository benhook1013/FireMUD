...
`platformAdmin` intervention is a distinct control-plane action, not impersonation of a `tenantAdmin` and not an implicit tenant membership. Every break-glass mutation carries exactly one explicit target scope from the union `TENANT_WIDE(tenantId)`, `REALM(tenantId, worldSlug, realmSlug)`, or `INSTANCE(tenantId, gameInstanceId)`. A tenant-wide action covers current and later tenant-owned resources through the owner workflow and reconciliation; realm and instance actions remain exact. Each mutation requires an authenticated platform actor, bounded action type, reason, audit record, and the same stable idempotency and owner-workflow contracts as the corresponding routine mutation.
