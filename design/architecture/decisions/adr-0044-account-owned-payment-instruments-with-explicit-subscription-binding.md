...
`CreateSubscription` resolves the authoritative billing owner before its subscription row exists and requires either an explicit saved instrument ID or deterministic selection input that resolves to exactly one saved instrument owned by that owner. Account first commits one durable `pending` subscription intent containing the owner, confirmed Stripe customer, explicit instrument binding, stable provider-idempotency identity, and outbox work item. A worker claims that intent as `provisioning`; retries and recovery reconcile the same provider-idempotency identity rather than creating a replacement subscription. The subscription cannot become `trialing` or `active`, authorize hosting entitlement, or be billed until Account has recorded a confirmed provider subscription ID. Stripe customer and provider defaults are prohibited.
...
The transfer is one durable, idempotent state machine keyed by an immutable transfer request ID. Account first validates both owners, the new owner's confirmed customer and selected instrument, the current subscription row version, and the absence of another nonterminal transfer. It then commits a `pending` transfer intent with the old binding, proposed binding, expected row version, provider-operation identity, and outbox work before any provider mutation. Provider retries reuse that identity. Account records the local owner/customer/instrument change only after the provider confirms the subscription rebind and a row-version compare-and-set still succeeds. A timeout or ambiguous provider result remains nonterminal and is reconciled; it does not authorize a second provider operation. A definitive provider rejection leaves the old binding authoritative. If provider success is confirmed but the local commit fails, reconciliation completes the same local compare-and-set or raises an operator-visible conflict for manual resolution; it never creates another subscription or silently restores the old provider binding. The transfer reports complete only after the provider and Account binding agree and the durable outcome and audit record are committed.
...
- billing-owner transfer is an explicit audited, row-version-guarded state machine through the dedicated `cross_tenant_billing_safe` route, uses the new owner's Stripe customer and instrument, reconciles partial or ambiguous provider outcomes without duplicate subscriptions, and never copies or transfers instruments;
