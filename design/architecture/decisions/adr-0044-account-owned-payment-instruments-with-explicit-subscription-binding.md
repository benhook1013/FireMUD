...
This decision is partially implemented. Account has payment and subscription persistence plus Stripe/payment-operation foundations, but the complete target path is not yet converged: explicit owner-authorized instrument binding, the single-use transfer proof and database transfer fence, audited billing-owner handoff, provider creation without customer or provider defaults, replacement-before-detach enforcement, immutable provider-history resolution, verified webhook entitlement completion, and their end-to-end proof remain incomplete. This status describes implementation coverage only and does not change the human-reviewed decision metadata below.
...
3. The account-keyed `billing_customer` record and customer-provisioning intent are durably unique by `accountId` and advanced under Account row-version/CAS fencing. Concurrent first subscriptions for one account therefore serialize onto the same customer-provisioning intent; losing subscription intents reuse the one existing `customer_provisioning_request_id` and its shared provider idempotency identity rather than creating another customer operation or Stripe customer. Confirm or reconcile that shared operation and attach the confirmed Stripe customer reference to the subscription intent. A worker may then claim the intent as `provisioning` in a separate local CAS transaction only after revalidating the instrument owner, exact reserved version, persisted Stripe customer reference, and reservation state.
...
The transfer is one durable, idempotent state machine keyed by an immutable `transferRequestId`. After the new owner independently authenticates and selects an instrument, Account issues a high-entropy, opaque, single-use, expiring transfer proof bound to exactly `transferRequestId`, immutable `subscriptionId`, new-owner `accountId`, selected opaque `paymentInstrumentId`, and selected instrument version. Only the Account transfer operation may consume that proof; a mismatch, expiry, prior consumption, or recovery without the proof fails closed. Account atomically consumes the proof with the transfer intent, so a retry after a lost response replays the durable transfer result and cannot spend the proof on another subscription or owner. The orchestrating `billingAdmin` receives and forwards only the proof or its bounded result and never gains wallet access.

Account first validates both owners, the new owner's confirmed customer, the selected instrument's ownership and customer binding, the current subscription row version, the existing paid-through boundary, and the absence of another nonterminal transfer. It then commits a `pending` transfer intent with the old binding, proposed binding, expected row version, effective boundary, outbox work, and three durable, distinct provider-operation identities: replacement creation, predecessor end/cancellation, and cleanup cancellation for an uncommitted replacement or other partial provider outcome. Provider retries reuse only the identity for their operation; cleanup retries always reuse the cleanup-cancellation identity rather than either transfer or predecessor-end identity.

The single-nonterminal-transfer rule is a database invariant, not only an application check. A forward migration reconciles any historical duplicate transfer group before creating a partial unique index on `subscription_transfer(subscription_id)` for `pending`, `provider_transition`, `cutover_pending`, and `cutover_committed`. Transfer creation locks the canonical subscription row, verifies the expected version and absence of a nonterminal transfer, and inserts the intent in that same transaction; a concurrent loser replays the existing transfer or returns a deterministic conflict without calling Stripe.
...
Timeouts or ambiguous provider results remain nonterminal and are reconciled using the same three provider-operation identities; they never authorize another replacement. A definitive replacement failure leaves the old binding authoritative and cancels any uncommitted transfer intent. If replacement creation succeeds but old cancellation scheduling or the local cutover cannot be confirmed, the workflow remains blocked and operator-visible while reconciliation converges or performs idempotent cleanup of the not-yet-authoritative replacement using the distinct cleanup-cancellation identity. It must not expose overlapping entitlement, silently strand two billable subscriptions, or claim transfer completion. Immediate mid-period cross-customer transfer, cross-customer credits, and proration are unsupported; an owner may separately choose terminal cancellation and a new subscription when an immediate boundary is required. Account deletion remains blocked until the scheduled transfer has cut over or the old subscription is terminally canceled.

Card management and every new real-money charge complete through HTTPS and the payment-provider flow. Adopting [ADR 0045](./adr-0045-ordinary-login-factors-and-https-sensitive-action-step-up.md)'s gameplay-to-HTTPS handoff, Telnet or gameplay may initiate the operation and return a short-lived, single-use, opaque checkout URL whose server-side handoff state is bound to the exact initiating account, gameplay session, action, product, amount, currency, and `requestId`, plus the originating effective security-policy version and digest resolved by Account. Before disclosing checkout details, accepting the required step-up, accepting verified provider completion, or atomically consuming the handoff, Account must re-resolve the exact current policy and require exact version-and-digest equality; a policy advance invalidates the unconsumed handoff. A tenant-scoped purchase binds exactly one validated target `tenantId`; its handoff, authenticated checkout request, and completion proof must carry that same tenant. An account-scoped purchase, grant, or donation persists and transmits no `tenantId` at all; its handoff, checkout request, and completion proof must require tenant absence and reject any supplied tenant. The URL contains no authority or mutable commercial parameters; replay, expiry, policy advance, or parameter substitution cannot change that tuple. FireMUD recognizes payment completion only from a verified provider webhook after the current-policy check, then applies the resulting durable entitlement or account-scoped grant idempotently. Fulfillment uniqueness is the immutable purchase-operation ID plus product grant key; the Stripe event ID is audit and evidence only.
...
- `CreateSubscription` resolves the authoritative billing owner before creating its subscription row, commits one durable `pending` intent and outbox item with an explicit instrument/version reservation, a stable creation-operation provider idempotency identity, and a distinct cleanup/cancellation-operation identity before provider creation, atomically revalidates ownership, version, and attachability at worker claim, defines the detach-vs-provision race, retains the creation identity for provider-object lookup, and uses the cleanup identity for idempotent cancellation/end retries before local terminal cancellation and reservation release. Its first-customer path must enforce durable uniqueness of the account-keyed `billing_customer`/customer-provisioning intent by `accountId`, serialize concurrent first subscriptions with the Account row-version/CAS fence, and make every losing subscription reuse the one shared `customer_provisioning_request_id` and provider idempotency identity. It performs idempotent terminal cleanup for permanent failure, expiry, or abandonment, blocks detach and terminal responses until that cleanup commits, reconciles ambiguous provider outcomes without duplicate creation, records a confirmed provider subscription ID before activation or billing, and does not use provider or customer defaults;
...
- billing-owner transfer requires an Account-issued single-use, expiring proof bound to `transferRequestId`, immutable `subscriptionId`, new-owner account, and selected instrument/version; the proof is consumed atomically with the transfer intent and cannot be reused or replaced by unrestricted support or `billingAdmin` access;
- billing-owner transfer is an explicit audited, row-version-guarded replacement state machine through the dedicated `cross_tenant_billing_safe` route, has a database-enforced single-nonterminal-transfer invariant, persists distinct replacement-create, predecessor-end, and cleanup-cancellation idempotency identities, preserves the immutable tenant binding, schedules old termination and new-owner billing at one paid-through boundary, reconciles partial or ambiguous provider outcomes without duplicate billing or entitlement, and never copies or transfers instruments;
- predecessor and replacement Stripe provider IDs resolve through immutable transfer/history mappings to the canonical subscription; predecessor events update transfer evidence or reach terminal ignore and never heuristically overwrite the canonical provider binding;
- FireMUD persists provider identifiers and safe display metadata only, never raw card data;
- gameplay-issued checkout URLs use the ADR 0045 HTTPS handoff, expire, are single-use, and prove the exact account, session, action, product, amount, currency, `requestId`, and originating effective policy version/digest; Account requires exact current-policy equality at disclosure, step-up, provider completion, and atomic consume, and a policy advance invalidates the unconsumed handoff; tenant presence is exact and required only for tenant-scoped purchases, while account-scoped actions prove `tenantId` is absent in handoff state, checkout disclosure/request, and completion binding; and
