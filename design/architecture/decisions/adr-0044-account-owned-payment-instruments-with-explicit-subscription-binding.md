...
`CreateSubscription` resolves the authoritative billing owner before its subscription row exists and requires either an explicit saved instrument ID or deterministic selection input that resolves to exactly one saved instrument owned by that owner. Account first commits one durable `pending` subscription intent containing the owner, a separate customer-provisioning operation reference, explicit instrument binding, the observed instrument version reservation, a stable creation-operation provider idempotency identity, a distinct cleanup/cancellation-operation provider idempotency identity, and outbox work item in a local database transaction. That transaction performs only local reservation/CAS work; it ends before any Stripe call and never spans remote Stripe validation or mutation. The confirmed Stripe customer is attached only after that separate customer operation is confirmed or reconciled. A worker claims the intent as `provisioning` in a separate local CAS transaction that revalidates the instrument's owner, exact reserved version, persisted Stripe customer reference, and reservation state. After that claim commits, the worker separately asks Stripe to validate the persisted customer/payment-method relationship and attachable state before making the provider call; the remote validation and provider mutation are never held inside or treated as part of the database transaction. Creation retries and recovery reconcile the same creation-operation identity rather than creating a replacement subscription. Detach and provisioning use the same instrument reservation/version boundary: a pending or provisioning reservation blocks detach; if detach commits before the reservation or claim, it advances the instrument version and the stale reservation or claim fails before any Stripe call; once a reservation or provisioning claim owns the boundary, detach waits. A permanent provider or local failure, intent expiry, or explicit abandonment enters one idempotent terminal cleanup for that immutable subscription operation. Before local cancellation or reservation release, cleanup uses the distinct cleanup/cancellation-operation identity for its idempotent cancellation/end attempt while retaining the creation-operation identity for lookup and reconciliation of any provider subscription object attributable to the create; cleanup retries reuse the same cleanup/cancellation identity. An ambiguous provider result leaves cleanup nonterminal for retry. Only after provider absence or terminal cancellation is durably recorded does cleanup lock the subscription and instrument, validate the exact owner, instrument binding, reserved instrument version, reservation identity, and row version, mark the local subscription terminal as `canceled` with the durable failure, expiry, or abandonment outcome, advance the instrument version exactly once, and release the reservation in the same transaction and outbox boundary. An exact retry returns the committed terminal cleanup result; a stale or competing detach/claim cannot clear a different reservation and must fail closed or reconcile under the same lock. No terminal failure, expiry, or abandonment response, and no detach that relies on the released instrument, is returned before cleanup commits. The subscription cannot become `trialing` or `active`, authorize hosting entitlement, or be billed until Account has recorded a confirmed provider subscription ID. Stripe customer and provider defaults are prohibited.
...
Card management and every new real-money charge complete through HTTPS and the payment-provider flow. Adopting [ADR 0045](./adr-0045-ordinary-login-factors-and-https-sensitive-action-step-up.md)'s gameplay-to-HTTPS handoff, Telnet or gameplay may initiate the operation and return a short-lived, single-use, opaque checkout URL whose server-side handoff state is bound to the exact initiating account, gameplay session, action, product, amount, currency, and `requestId`. A tenant-scoped purchase additionally binds the exact target `tenantId`; an account-scoped purchase, grant, or donation remains account-scoped and carries no fabricated tenant. The URL contains no authority or mutable commercial parameters; replay, expiry, or parameter substitution cannot change that tuple. FireMUD recognizes payment completion only from a verified provider webhook, then applies the resulting durable entitlement or account-scoped grant idempotently. Fulfillment uniqueness is the immutable purchase-operation ID plus product grant key; the Stripe event ID is audit and evidence only.
