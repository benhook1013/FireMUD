...
The repository now contains the selected in-cluster materializer/reconciler API, manifests, and a strict mode-gated dev-demo lifecycle. A missing certificate-identity mode continues to select the existing standalone path; only literal `hosted-controller` activates the requester/projection/readiness sequence, so the controller remains dormant until a reviewed activation successor changes the hosted Helm values and consumers together. Local/static checks cover the trust split, lifecycle ordering, status gate, and projected Secret shape; no live controller bootstrap, hosted rollout, certificate issuance, renewal, or consumer-convergence proof is claimed yet. The trusted PR-preview activation and PR-close retirement consumer remain successor work. Stage and production continue to use their existing externally managed certificate and identity lifecycle; no new stage or production external integration is claimed or proven here.
