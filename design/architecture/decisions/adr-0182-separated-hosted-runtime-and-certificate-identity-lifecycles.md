...
The repository now contains the selected in-cluster materializer/reconciler API, manifests, and a strict mode-gated dev-demo lifecycle. A missing certificate-identity mode continues to select the existing standalone path; only literal `hosted-controller` activates the requester/projection/readiness sequence, so the controller remains dormant until a reviewed activation successor changes the hosted Helm values and consumers together. Local/static checks cover the trust split, lifecycle ordering, status gate, and projected Secret shape; no live controller bootstrap, hosted rollout, certificate issuance, renewal, or consumer-convergence proof is claimed yet. The mode-gated `pull_request_target` PR-close retirement consumer is implemented and locally/static tested, while trusted PR-preview activation remains successor work. Stage and production continue to use their existing externally managed certificate and identity lifecycle; no new stage or production external integration is claimed or proven here.
...
Runtime namespaces remain clean and disposable. A trusted lifecycle may reclaim, redeploy, release, or let a lease expire on the runtime namespace while retaining the active identity namespace. Runtime preparation creates or reuses the exact `pr-<PR_NUMBER>` or `dev` namespace, records the requested reviewed head and image, and allocates the bounded Telnet port. The trusted requester then applies `Active`, and the runtime lane waits for all five labeled, key-complete Secret projections defined by the [deployment-environment contract](../infrastructure/deployment-environments.md) before application preflight, server dry-run, and deploy. Only successful deployment may record the separate deployed-head evidence consumed by readiness. A failed or incomplete identity projection blocks runtime deployment, while a failed deployment cannot make the request appear deployed.

A PR close or merge follows a runtime-first path: it requests `Retired` only after runtime cleanup has observed the exact runtime Namespace `NotFound` result, so that path gives the controller no live runtime to shut down. A trusted explicit identity-removal request for a live runtime follows a retirement-first path: it may set `Retired` while the runtime still exists, after which the controller must terminate Gateway and TCP Proxy and observe both bridge workloads at zero before the trusted runtime lifecycle deletes the namespace and proves exact `NotFound`. Retained identity deletion remains blocked until that runtime absence proof. Before deleting retained material, the controller accounts for all five declared projection roles: (1) [public ingress TLS](../system-architecture-security.md#tls-termination-for-gateway), (2) [public Telnet TLS](../system-architecture-protocol-bridging.md#public-telnet-tls-modes), (3) the [Gateway internal WebSocket server](../system-architecture-security.md#tls-config-matrix-tcp-proxy-spring-cloud-gateway-websocket), (4) the [TCP Proxy bridge client](../system-architecture-security.md#tls-config-matrix-tcp-proxy-spring-cloud-gateway-websocket), and (5) the [shared gRPC transport bundle](./adr-0038-explicit-jwt-profiles-and-mtls-workload-identity.md#mtls-identity-and-certificate-lifecycle). Runtime cleanup and identity retirement remain separate operations. If the provider or lifecycle API cannot reliably establish the requested lifecycle state, the safe result is to retain identity, report the lifecycle as incomplete or uncertain, and fail closed; an uncertain response never triggers broader deletion.

If retirement observes a live runtime profile that differs from the profile already recorded in controller status, it reports `RuntimeIdentityChanged`, preserves the previously observed profile, and never refreshes retirement authority from the mismatched live values. Recovery belongs to the trusted lifecycle: it removes the whole disposable runtime namespace and obtains the runtime-first exact `NotFound` proof. A later retirement reconcile can then continue deleting the retained identity through the normal ownership-checked path. This recovery does not permit manual deletion of retained identity material and does not weaken the live-runtime bridge-shutdown or runtime-absence gates.

### Independent public keys and retained internal identity

Ingress and public Telnet certificates use independent key material and certificate identities. Renewal or replacement of one does not silently replace the other. The internal gRPC identity and trust material are retained across ordinary runtime namespace replacement, reclaim, release, and lease expiry, subject to the identity lifecycle and explicit retirement rules. Gateway's internal WebSocket server identity and TCP Proxy's WebSocket client identity are distinct leaves under the same fixed internal trust anchor: the server leaf has the exact Gateway Service DNS SAN and `serverAuth`, while the client leaf has the exact TCP Proxy SPIFFE URI SAN and `clientAuth`. Ordinary renewal may update their retained and projected material without forced bridge termination; served readiness remains false until consumers accept the current material. This retention avoids needless internal trust churn while preserving independent leaf compromise and renewal boundaries.
...
All controller-owned certificate leaves use RSA-2048 keys. The cert-manager resource factory and the in-process gRPC bundle generator consume the same canonical key-size constant so that these issuance paths cannot silently diverge.

### Generation-bound readiness and certificate convergence

The controller publishes non-secret status only. A `Ready=True` result is valid only when `status.observedGeneration` equals `metadata.generation`, the live runtime namespace UID and requested head SHA match the status profile, a separately recorded post-deployment head equals that request after canonical 40-hex normalization, rollout and served proofs have converged, and non-empty ingress, Telnet, Gateway-internal WebSocket, TCP Proxy bridge, and gRPC revisions are present. PR preview and dev-demo preparation write only their distinct requested-head annotations; only a successful runtime deployment writes the corresponding deployed-head annotation. The workflow treats `Pending`, `Provisioning`, `WaitingForCertificate`, `RuntimeAbsent`, `Syncing`, `Verifying`, `Degraded`, `Blocked`, `Retiring`, and `Retired` as non-ready and uses the Ready condition's reason/message for failure detail.
