...
A PR close or merge requests `Retired` only after runtime cleanup has observed the exact runtime `NotFound` result. Runtime cleanup and identity retirement are separate operations. A trusted explicit identity-removal request may set `Retired` while the runtime still exists; the controller must then terminate Gateway and TCP Proxy and observe both bridge workloads at zero before retained material can be removed. If the provider or lifecycle API cannot reliably establish the requested lifecycle state, the safe result is to retain identity, report the lifecycle as incomplete or uncertain, and fail closed; an uncertain response never triggers broader deletion.
