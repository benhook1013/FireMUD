...
The internal Gateway-to-Game-Session hop must distinguish rebindable backend lifecycle or transport loss from terminal session outcomes such as logout, takeover, policy rejection, and revocation. Public close code alone is insufficient for that internal distinction. A backend lifecycle classification is not a new client-visible close category: successful recovery remains hidden, while exhausted recovery maps to `1013/backend_unavailable`.

A replacement Game Session continues the established edge session from current server-side session authority and the stable edge transport identity. It must not require an originally admitted first-party connect token to remain valid as though rebind were a fresh public handshake. Current membership, entitlement, revocation, tenant/game scope, active-session authorization, and fencing still apply. The continuity and disconnected-resume deadlines in ADR 0019 apply after loss of the edge transport; they do not turn an internal upstream rebind into a fresh public admission. If continuation authority cannot be established safely, recovery fails closed through the applicable terminal-session result or `backend_unavailable` fallback.
