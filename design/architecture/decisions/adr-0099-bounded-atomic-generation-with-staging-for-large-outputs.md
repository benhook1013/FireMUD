...
Outputs that cannot satisfy the bounded transaction limits use private staging keyed by the generation run. Chunked staging is never reader-visible authoritative topology. Each chunk contributes its exact row count and canonical serialized-byte count to the persisted staged-run totals, as well as to the canonical output digest. A short owner-local finalize transaction checks the stable request identity, uniqueness, current scope fence, terminal staging state, canonical output digest, exact expected row count, exact expected canonical serialized-byte count, and declared scope, then atomically installs or selects the staged graph and advances generation state. Missing, duplicate, truncated, or inconsistent staged content fails closed and remains non-visible.
...
Enforce tested row and serialized-byte admission limits for the single-transaction path. Persist the stable request identity, target scope, exact output row count, exact canonical serialized-byte count, output digest, immutable generation inputs, lifecycle state, generation ownership, and scope fence. The database must enforce request uniqueness, and every commit or finalize predicate must reject a stale fence atomically. Finalize must compare both persisted exact counts and the digest with the staged content before making it visible.
