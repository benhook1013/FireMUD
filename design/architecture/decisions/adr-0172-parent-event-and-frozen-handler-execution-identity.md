...
This ADR defines how those identities interact and how their immutable fencing is preserved; it does not replace the canonical owner contracts. The parent event identity and its claim/retry lifecycle are owned by the [event-scope claim contract](../system-architecture-scripting-normative-contract-tables.md#event-scope-claim-and-retry-semantics-normative); durable handler execution-child persistence and recovery consequences are owned by [Scripting Runtime Execution](../system-architecture-scripting-runtime-execution.md#scripteventid-lifecycle-and-deduplication); the handler-scoped Trigger Identity is owned by [Normative Table 1](../system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields); and emitted command-child identity is owned by [Normative Table 2's Command-Handoff Identity](../system-architecture-scripting-normative-contract-tables.md#command-handoff-identity-target-state). Those owner contracts remain authoritative for field applicability, storage, API shape, and proof.
