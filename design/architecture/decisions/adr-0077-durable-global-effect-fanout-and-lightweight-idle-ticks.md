...
Durable parent/child fan-out and lightweight idle advancement are target state; frozen participant sets, reconciliation ownership, fair admission, idle watermarking, and focused proof remain incomplete.
...
Every global or multi-region effect first creates one durable parent identity. At acceptance, the parent freezes both the affected region set and the topology generation from which that set was resolved. The parent and every regional child carry or immutably reference the canonical Game Session root binding from [ADR 0069](./adr-0069-at-least-once-effect-execution-with-one-logical-terminal-outcome.md): root `EffectId`, typed operation, immutable request digest, required-participant context, and sealed manifest. Each child is one member of that sealed expected participant set and cannot replace, add, or omit participants during retry or reconciliation. Retry or reconciliation uses that recorded set; it does not expand the effect again from current topology.

Fan-out creates an idempotent durable child or injection row for every affected region before publishing any wake signal for that region. The parent and children expose pending, injected, terminal, and feature-specific outcome information sufficient to show partial injection. Game Session owns fan-out reconciliation. A feature may not casually assign another durable coordinator: any transfer must follow [ADR 0057](./adr-0057-game-session-owned-reconciliation-with-isolated-workers.md)'s complete authority-transfer model and a new accepted ADR defining the new owner's ledger/backlog, frozen participant set, retry/reconciliation, outcome, audit, retention, and worker controls. The owner retries missing injections, reconciles duplicate attempts to the same child identity, and derives the visible parent outcome.

The exact child-identity allocation, manifest schema, and replay binding are not selected by this accepted fan-out decision. They are described only as a non-authoritative pending proposal in [ADR 0183](./adr-0183-deterministic-effect-id-allocation-and-replay-binding.md); no implementation may treat that proposal as current target state.
