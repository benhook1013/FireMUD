...
For the public-production join predicate, `JOIN_REQUIRED` is emitted only when the caller-bound membership is missing or `INACTIVE`. `ACTIVE` is the only admissible state; every other lifecycle state is a join denial and is not restorable or transitioned by `JOIN`. Those states must be resolved by their owning lifecycle or remediation contract rather than by public enrollment.
