...
Bootstrap, connect-token, and visible `LOGIN`/`PLAY` pieces exist, but the target sequence is not runtime-complete. `/auth/player-bootstrap` now authenticates account identity before tenant selection and accepts the account's configured password or verified-email OTP mode. Browser and mobile-browser connect-token responses are metadata-only and carry the raw token only in the HttpOnly cookie; explicitly classified non-first-party/public native-mobile and other non-browser clients use protected secure storage plus the dedicated header carrier. Explicit `POST /auth/bootstrap/join` before bootstrap character creation/connect-token work, the required current-membership reread, and the membership-generation live reread at connect-token issuance remain gaps. `IssueConnectToken` now returns `JOIN_REQUIRED` when public-production membership is absent and no longer creates membership; explicit `JOIN` remains unimplemented. The explicit `JOIN`/`Join & Play` action and the `membershipAuthorityGeneration` reread are both unimplemented/proof gaps; affected first-party admission remains fail-closed and unavailable until both are implemented and proven. The target sequence is `LOGIN` -> explicit `JOIN`/`POST /auth/bootstrap/join` for a first public-production entry -> `CHARS`/bootstrap character creation as needed -> `POST /auth/connect-token` with fresh membership and authority-generation rereads -> WebSocket `LOGIN` -> `PLAY`. Returning members use their existing membership. Grant-backed private/playtest players require both a current grant and existing membership, skip `JOIN`, and never create membership through this flow. The route matrix records the unimplemented explicit-membership `JOIN` path; its `known_drift` must also explicitly name the missing membership-generation reread.
...
- Prove `PLAY` rereads both current membership and the current `membershipAuthorityGeneration` after connect-token issuance, rejects either authority changing before gameplay binding, and never treats token-carried membership evidence as the final admission decision.
