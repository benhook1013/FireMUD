...
3. A first public-production entry explicitly invokes `JOIN` or `Join & Play`. The public-production join may create durable `player` membership only when that membership is missing; otherwise it returns the existing membership without creating another row. If the caller lacks the required public-production membership, connect-token issuance and `PLAY` return `JOIN_REQUIRED` until this operation succeeds. Grant-backed private/playtest realms require both the applicable current grant and existing membership, skip `JOIN`, and never create membership through this flow.
...
9. `PLAY` remains mandatory and receives the selected stable world, realm, and character context. After connect-token issuance, Game Session captures one current authoritative membership snapshot together with its `membershipAuthorityGeneration` and independent `membershipVersion`. Account issues the exact admission lease from that snapshot, and Game Session's gameplay-binding CAS atomically carries and fences the complete snapshot, generation, and version at the binding commit; Account finalizes only that same fenced lease. Any change to any captured membership or authority value after token issuance or before commit rejects the binding, with no partially admissible record. Game Session resolves the current admissible `gameInstanceId` server-side; only the internal gameplay binding carries `{tenantId, resolvedGameInstanceId, characterId}` after these current admission checks.
...
- Prove `JOIN_REQUIRED` for a public-production caller without membership, prove explicit `JOIN` is the only action that may create a missing public-production membership and resolves its user selector to a verified `connectScopeId` before the Account write, and prove grant-backed private/playtest admission requires both its grant and existing membership without creating membership.
- Prove `PLAY` captures one current membership snapshot plus the independent `membershipAuthorityGeneration` and `membershipVersion` after connect-token issuance, atomically fences all of them at gameplay-binding commit, rejects any change before commit, and never treats token-carried membership evidence as the final admission decision.
