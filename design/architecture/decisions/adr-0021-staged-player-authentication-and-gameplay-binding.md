...
Bootstrap, connect-token, and visible `LOGIN`/`PLAY` pieces exist, but the target sequence is not runtime-complete. `/auth/player-bootstrap` now authenticates account identity before tenant selection and accepts the account's configured password or verified-email OTP mode. Browser and mobile-browser connect-token responses are metadata-only and carry the raw token only in the HttpOnly cookie; explicitly classified non-first-party/public native-mobile and other non-browser clients use protected secure storage plus the dedicated header carrier. Explicit `POST /auth/bootstrap/join` before bootstrap character creation/connect-token work remains a gap. The current caller-bound membership existence check is implemented: `IssueConnectToken` and text `PLAY` return `JOIN_REQUIRED` when public-production membership is absent and do not create membership. The exact `membershipLifecycleState=ACTIVE` admission check, complete `membershipAuthorityGeneration` live reread, and independent `membershipVersion` issuance baseline remain unimplemented/unproven, and explicit `JOIN` remains unimplemented. Affected first-party admission remains fail-closed and unavailable until explicit `JOIN`, the active-state check, the generation reread, and the unchanged issuance baseline are implemented and proven. The target sequence is `LOGIN` -> explicit `JOIN`/`POST /auth/bootstrap/join` for a first public-production entry -> `CHARS`/bootstrap character creation as needed -> `POST /auth/connect-token` with fresh membership lifecycle, generation, and version rereads -> WebSocket `LOGIN` -> `PLAY`. Returning members use their existing membership. Grant-backed private/playtest players require both a current grant and existing membership, skip `JOIN`, and never create membership through this flow. The route matrix records the unimplemented explicit-membership `JOIN` path; its `known_drift` must also explicitly name the missing active-state, membership-generation, and membership-version baseline checks.
...
3. A first public-production entry explicitly invokes `JOIN` or `Join & Play`. The public-production join may create durable `player` membership only when that membership is missing; otherwise it returns the existing membership without creating another row. A retained inactive relationship is restored only through the membership lifecycle transition defined by [ADR 0042](./adr-0042-global-account-and-tenant-scoped-game-relationships.md), not by an implicit upsert or duplicate row. A successful join returns the exact Account snapshot with `membershipLifecycleState=ACTIVE`, `membershipAuthorityGeneration`, and `membershipVersion`. If the caller lacks the required public-production membership, connect-token issuance and `PLAY` return `JOIN_REQUIRED` until this operation succeeds. Grant-backed private/playtest realms require both the applicable current grant and existing membership, with lifecycle state exactly `ACTIVE`, skip `JOIN`, and never create membership through this flow.
...
5. `POST /auth/connect-token` revalidates the selected target, rereads current caller-bound membership, requires exact `membershipLifecycleState=ACTIVE`, and captures an immutable issuance baseline `{membershipLifecycleState, membershipAuthorityGeneration, membershipVersion}` from that one Account snapshot. It issues a single-use connect token with a lifetime no longer than 30 seconds and never creates membership. A missing public-production membership is a durable terminal `JOIN_REQUIRED` outcome for that request digest, not a retryable token-issuance side effect.
...
- Prove `JOIN_REQUIRED` for a public-production caller without membership, prove explicit `JOIN` is the only action that may create a missing public-production membership and resolves its user selector to a verified `connectScopeId` before the Account write, restores a retained relationship only through the ADR 0042 lifecycle, and returns exact `membershipLifecycleState=ACTIVE`, `membershipAuthorityGeneration`, and `membershipVersion`. Prove grant-backed private/playtest admission requires both its grant and existing membership with exact lifecycle state `ACTIVE` without creating membership.
- Prove `IssueConnectToken` persists the immutable `{membershipLifecycleState=ACTIVE, membershipAuthorityGeneration, membershipVersion}` issuance baseline and durable terminal outcome by request digest, and prove `PLAY` compares one current membership snapshot to that baseline, atomically carries the unchanged values through the Account lease, gameplay-binding CAS, and Account finalization, rejects any change before commit, and never treats token-carried membership evidence as the final admission decision.
