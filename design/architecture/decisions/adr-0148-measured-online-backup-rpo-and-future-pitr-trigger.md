# ADR 0148: Measured Online-Backup RPO and Future PITR Trigger
...
A restore point counts only after the supported verification path establishes that its artifact is complete, readable, bound to the correct environment and database lineage, and usable by the supported recovery tooling. Backup cadence and verification timing are implementation parameters that must be set tightly enough for the measured newest verified restorable point to remain within the objective.
