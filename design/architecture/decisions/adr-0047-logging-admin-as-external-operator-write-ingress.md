...
The canonical encoding is versioned as `mutationDigest/v1`. It serializes fields in the fixed order `scope`, `target`, `expectedVersion`, `mutation`, and `auditReason` using a typed length-prefixed representation: each field has an explicit name, type, presence marker, byte length, and UTF-8 value; absent and explicit `null` are distinct and are encoded explicitly. Numeric values use canonical base-10 decimal without a sign on zero, leading zeros, or exponent notation; strings are UTF-8 after Unicode NFC normalization; object fields use the documented key order and arrays retain request order. The SHA-256 input is the UTF-8 bytes of this representation, including the version tag and field names. Any future encoding changes require a new version and compatibility rule; implementations must not silently reinterpret an existing version. The contract must publish test vectors covering null versus absent values, numeric/string distinctions, Unicode normalization, field ordering, and representative scope/target mutations, with the canonical bytes and expected digest retained as review or contract-test evidence.
