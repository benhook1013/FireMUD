...
Each published action-family schema must define finite maxima for normalized string byte length, canonical numeric byte length and scale, object member count, array element count, composite nesting depth, and the total canonical preimage byte length. Account, Logging and Admin, and the domain owner enforce the same schema limits before canonicalization, hashing, authorization issuance, persistence, or forwarding. Missing limits, an over-limit value, or disagreement between participants makes the family unsupported or the request invalid; implementations must not hash an unbounded payload and reject it only afterward.
