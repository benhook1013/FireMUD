...
This ADR is partially implemented. Logging & Admin has current operator-facing read, investigation, and coordination surfaces, but no operator mutation family is currently supported as an executable external route until its action schema, `mutationDigest/v1` vectors, and Account-issued authorization-reference flow exist. In particular, `/moderation/actions` is unavailable: its implementation path is not current live operator support, does not invoke an owner-side enforcement RPC, and must not be treated as a successful policy mutation. The versioned policy-propagation and owner-enforcement path below remains target-state work.
...
This protocol defines the target ingress for the following action families. It does not provide executable forwarding for `/moderation/actions`, which is unavailable/gated pending the action schema, shared `mutationDigest/v1` vectors, Account authorization-reference flow, and an owning enforcement contract:
...
| Moderation enforcement | `/moderation/actions` is unavailable/gated pending action-family schema, cross-language `mutationDigest/v1` vectors, and Account authorization-reference flow; no current supported moderation mutation is claimed | Versioned policy propagation to Game Session or Social & Groups enforcement owner |
...
- prove that no feature-flag, admission-pointer/version-upgrade, tick, or moderation mutation route is classified executable before its action-family schema, cross-language `mutationDigest/v1` golden vectors, and Account authorization-reference flow are published and consumed by every participating implementation; `/moderation/actions` is unavailable/gated and executable moderation routes are rejected until those artifacts and the owning enforcement contract exist;
