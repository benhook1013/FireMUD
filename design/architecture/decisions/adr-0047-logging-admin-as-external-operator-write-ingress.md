...
`canonical_decimal_count(n)` is the ASCII base-10 encoding of a non-negative integer count: zero is `0`, positive values have no leading zero, and the encoding has no sign, decimal point, exponent, or whitespace. It is used for byte lengths, declared object-member counts, and array-element counts; counts are integers and never pass through numeric-value canonicalization.

Scalar and composite payloads are canonicalized normatively: every input must first be decoded as valid Unicode scalar values; malformed UTF-8, unpaired UTF-16 surrogates, non-scalar code points, and replacement-character recovery are rejected before normalization. `string` is Unicode NFC-normalized before its normalized value is UTF-8 encoded. The accepted numeric source grammar is ASCII `-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?`, with no leading or trailing whitespace, no leading zero in a nonzero integer, no `+` sign, and no exponent; a source using any other exponent, plus-sign, leading-zero, whitespace, or non-decimal form is rejected before decimal conversion. A source with arbitrary precision is interpreted as a base-10 decimal rather than a binary floating-point value, but its raw coefficient/scale and normalized bytes remain bounded by the action-family limits below. When the fractional part is all zeroes, fractional trailing zeroes and the decimal point are removed, so `10.000 -> 10`; fractional zeroes are not removed from a nonzero fractional value such as `10.0100 -> 10.01`. `-0`, `-0.00`, and every other signed-zero spelling accepted by the grammar normalize to canonical `0`, and non-finite values such as `NaN`, `Infinity`, and `-Infinity` are rejected. The resulting canonical `number` must match `0` or `-?(?:[1-9][0-9]*(\.[0-9]*[1-9])?|0\.[0-9]*[1-9])`; the canonical grammar has no negative-zero form, plus sign, leading zero, exponent, or trailing fractional zero. Required golden-vector outcomes therefore include `1 -> 1`, `10.000 -> 10`, `-12.3400 -> -12.34`, `-0.00 -> 0`, and rejection of `+1`, `1e3`, `01`, `1.`, `.5`, an ASCII space before or after `1`, `NaN`, and `Infinity`; a mathematically valid arbitrary-precision value is rejected when its declared raw/normalized scale or byte limit is exceeded rather than rounded or converted through a finite-precision float. `boolean` is exactly `true` or `false`.

For an `object`, the action-family schema declares one fixed member list and order. Each key is valid Unicode, NFC-normalized before UTF-8 encoding, and compared only after normalization; two raw keys that normalize to the same key are a collision and are rejected, as are duplicate raw keys, unknown keys, missing required keys, or any input order other than the declared schema order. The object is `segment(canonical_decimal_count(declared_member_count))` followed by every declared member, including an absent optional member, as `segment(utf8(normalized_key)) + value`. The member count is the number of declared members, not the number supplied by the caller. An absent optional member is therefore always serialized in its declared position as `segment(utf8(normalized_key)) + segment(utf8("absent")) + ascii("0") + segment(empty)`; it is never omitted or reordered. `array` is `segment(canonical_decimal_count(element_count))` followed by each element `value` in request order. Nested object and array values use the same rules. A value whose wire type does not match its action-family schema is rejected before hashing. Transport credentials and the opaque authorization reference are excluded; every scope, target, expected-version, mutation, and audit-reason field declared by the action-family schema is included, including explicit absence or null where allowed.
...
Account creates a durable bounded issuance record keyed by `controlPlaneRequestId` and the immutable typed request tuple and must durably retain the original opaque reference for replay. The retention stores that reference only inside Account's protected authority record or its bounded encrypted response envelope and covers the reference lifetime plus the complete retry and read-only reconciliation window. Logging and Admin does not retain a second recovery envelope or competing authority record.

An exact retry of the typed issuance RPC returns the original opaque reference, fingerprint, expiry, and immutable authority evidence, or the original terminal outcome; it is a replay of Account's durable response, not replacement issuance. For a lost or ambiguous issuance response, Account also exposes a read-only `RecoverOperatorAuthorizationReference` operation to the exact authenticated Logging and Admin mTLS workload. The recovery request carries the same `controlPlaneRequestId`, complete immutable typed issuance tuple, actor or automation-policy evidence, and current Logging and Admin reservation-claim identity and fence. Account validates every field against its issuance record and returns only the original response. A changed tuple or claim mismatch returns canonical `IDEMPOTENCY_CONFLICT` or `FENCE_REJECTED`; an absent record returns a durable `NOT_FOUND` lookup result and never creates one. Recovery does not mint, extend, rotate, redeem, or semantically recreate a reference. After the original response is recovered, the caller follows the ordinary `RedeemOperatorAuthorization` path with that exact opaque reference. A replacement Logging and Admin recovery owner may perform this read only after a fenced recovery CAS has superseded the expired reservation claim; it cannot issue a new reference or mutate Account's issuance record.

They must never mint a replacement reference, reissue a semantically equivalent reference, or silently create a new request identity for the same mutation. Redemption and owner reconciliation are retryable and non-destructive while the exact reference remains valid; after expiry or revocation, the unresolved operation remains non-replayable and requires a new explicit operator request.

A stale owner or fence returns the canonical `FENCE_REJECTED` wire/status outcome with reason `STALE_OWNER` or `STALE_FENCE`. This outcome is reconciliation evidence, not direct retry permission. It must contain the same `controlPlaneRequestId`, exact mutation digest and authorization fingerprint, the rejecting owner identity and observed fence, and an owner-side operation/effect lookup proving that no mutation transaction accepted or committed that request. The rejecting owner alone may perform the canonical transition from `FENCE_REJECTED` to ADR 0048's terminal `NOT_EXECUTED`: it records immutable no-accept/no-commit proof under the original `ownerMutationId`, then returns that terminal evidence through read-only result lookup. Logging and Admin records the returned owner outcome but cannot synthesize or advance it. Only owner-recorded terminal `NOT_EXECUTED` permits the same external request tuple to be rearmed for the current owner under a new `ownerMutationId` and current fence; `FENCE_REJECTED` itself is non-rearmable. Audit records, contract tests, and metrics use `FENCE_REJECTED` as the initial outcome and the stale-owner or stale-fence value as its reason, then preserve the linked terminal owner evidence if reconciliation proves `NOT_EXECUTED`. A timeout, transport failure, missing or inconsistent no-commit evidence, an owner result after mutation acceptance, or any other rejection is ambiguous; callers must stop replay and use read-only reconciliation of the original request and exact tuple before taking any further mutation action.
...
- prove Account issues and owns the bounded operator authorization reference, exact retries and `RecoverOperatorAuthorizationReference` return only the original response under the complete tuple and current reservation-claim fence, absent or mismatched recovery never creates a reference, and the owner redeems that exact reference rather than trusting Logging and Admin assertions;
