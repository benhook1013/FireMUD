...
Unattended operator automation uses `IssueAutomationOperatorAuthorizationReference` with `exact_mtls_workload_plus_versioned_automation_policy`. The supported automation branch is explicitly restricted to tenant-scoped `tenant_regular` action families that do not require `privileged_control`; Account authenticates the exact Logging and Admin workload mTLS identity, validates the current versioned automation policy, and binds the exact target `tenant_generation` to the automation reference for the requested tenant/scope, action family, action-family schema identifier/version, and canonical mutation digest. Automation therefore cannot issue `platformAdmin` target-tenant or cross-tenant `billingAdmin` references. If a future policy introduces an automated global-role branch, Account must first define an automation-specific privileged predicate and bind that predicate plus the exact `target_tenant_generation` through issuance, forwarding, redemption, and owner commit; no current automation reference may imply that authority. The automation reference is also bound to `workload_identity`, `automation_policy_id`, `automation_policy_version`, `tenant_generation`, tenant/scope, action family, `actionFamilySchemaId`, `actionFamilySchemaVersion`, `controlPlaneRequestId`, mutation digest, issue time, and expiry; it has no user account, end-user token, or `control-ui` identity. The owner must require the same exact Logging and Admin mTLS identity and redeem the reference with `RedeemOperatorAuthorization`. Logging and Admin records whether the actor is human or workload automation and retains the applicable human or policy evidence.
...
- require exact `control-ui`, current role, role-appropriate assurance, and the applicable `tenant_generation` or `target_tenant_generation` plus `privileged_control` predicate for human ingress, while proving the separate unattended path is restricted to supported tenant-scoped policy branches, carries the exact `tenant_generation` through Account issuance, Logging and Admin forwarding, redemption, and owner commit, and cannot impersonate a user;
