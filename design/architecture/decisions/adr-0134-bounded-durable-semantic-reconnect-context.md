...
The current implementation substantially persists replay-eligible structured entries in ordered `resume_transcript_entry` rows and uses Redis only as a best-effort cache. Existing proof covers significant durable retention and replay behavior, including focused local proof that fresh first-party `PLAY` after explicit logout does not replay retained entries, but it does not prove oversized-entry omission or omission-marker behavior; the current runtime may retain an oversized entry. It does not yet implement this decision completely: durable context is keyed by `gameInstanceId` rather than `playableStateNamespaceId`, and the distinct durable `resumeContextGeneration` episode fence is not persisted or enforced, leaving concurrent and delayed replay unproved. Previous documentation contradicted the implemented oversized-entry behavior; this decision resolves the target in favor of a strict ceiling without claiming complete envelope-accounting and marker proof.
...
`bindingGeneration` remains the controller-ownership and concurrency fence. Game Session separately owns a positive, monotonic durable `resumeContextGeneration` scoped to the stable `<tenantId, playableStateNamespaceId, characterId>` context authority; it is not a fourth durable scope key. Every `resume_transcript_entry` and cache envelope carries the exact `resumeContextGeneration`, and append, read, restore, and Redis cache fill/rebuild/mutation exact-compare the current generation and recheck durable proof that the episode is not terminated. Authorized reconnect, controller takeover, and runtime replacement carry `resumeContextGeneration` unchanged even when `bindingGeneration` advances; they do not copy rows or authorize reads from a prior context generation. Explicit `LOGOUT`, character ownership transfer, or loss of replay authorization durably terminates the current context episode before acknowledgement. A later independently authorized episode receives a newer `resumeContextGeneration`; PostgreSQL append is conditional on that still-current, non-terminated episode, and cache work must refuse or invalidate old-generation state so stale in-flight work cannot repopulate replayable context. Restore uses only the exact current `resumeContextGeneration` in canonical order and then obtains a fresh `LOOK`; physical deletion may remain asynchronous. This is target behavior, and current generation/termination proof is incomplete.

FireMUD replays private context only after current authentication and authorization establish access to that exact tenant, playable-state namespace, character, and current `resumeContextGeneration`. Retention alone never grants access.
...
- Authorized reconnect, controller takeover, and runtime replacement retain the same private-context episode even when controller `bindingGeneration` advances; terminal privacy events rotate the separate `resumeContextGeneration` before any later episode can retain or restore output.
...
- Private replay fails closed after logout, character ownership transfer, or lost authorization.
...
Proof must cover runtime instance replacement within one playable-state namespace, isolation between production and playtest namespaces, Redis loss, database restart, ordering, eviction at soft and hard bounds, a single oversized entry, and a bounded omission marker. It must prove monotonic `resumeContextGeneration` allocation; exact-generation append, read, restore, and cache behavior; unchanged context generation across authorized reconnect, controller takeover, and runtime replacement even when `bindingGeneration` advances; durable episode termination before acknowledgement; rejection of stale-generation work; and newer generation allocation for a later authorized episode without copying or reading prior-generation rows. It must also cover repeated output, unavailable output, context followed by fresh `LOOK` and exactly one current prompt when both effective reconnect-prompt settings are enabled, zero reconnect prompts when either is disabled, explicit logout, character ownership transfer, authorization loss, cross-tenant and cross-namespace denial, expiry, and rendering for Telnet and structured clients.
