...
The fencing unit is the owner-local tuple `(owner, aggregateId, scopeId, epoch)`. A scope identifier names the narrowest independently editable unit; an aggregate-wide invariant is represented by the canonical aggregate scope and therefore expands the affected set when required. Game Design coordination retains the complete cross-owner affected set; each owner CAS checks every expected tuple in the complete subset owned by that service while validating the complete affected set required by its typed mutation. A mutation touching multiple owned scopes advances them atomically in that owner's transaction. Disjoint scope tuples may advance independently, while an operation that declares an incomplete or under-scoped set is rejected.

### Owner-Local Atomic Compare-and-Swap

Each authoritative owner validates the complete affected aggregate and scope set required by its typed mutation. In one owner-local storage transaction, each owner must:

1. compare every expected epoch required by the typed mutation for every fencing tuple in the complete subset owned by that service, as a storage-level write predicate or equivalent lock-protected condition;
2. apply all of that owner's local mutations for the commit;
3. advance the affected aggregate and scope epochs owned by that service; and
...
An owner-local apply does not by itself make the commit accepted shared Draft truth. Normal authoring reads and subsequent edits bind to the Game Design-owned synchronized visibility fence, which records the exact commit, digest, and complete `(owner, aggregateId, scopeId, epoch)` set visible to ordinary Draft readers. Game Design advances that fence only after every required owner reports an `APPLIED` or equivalent successful terminal result matching the exact commit and digest at all of its owned expected fencing units. Rejected, failed, and incomplete results do not advance the fence. Owner storage must preserve the ability to serve the synchronized fence while later partial work exists; the physical staging, visibility, or cleanup representation may vary by owner.
...
Storage proof must demonstrate compare-and-swap in the mutation statement or equivalent owner-local locking transaction for every tuple in each owner's complete owned `(owner, aggregateId, scopeId, epoch)` subset, not only a service-layer pre-read; typed-mutation validation must prove the complete affected set required by that owner. Cross-owner proof must demonstrate restart-safe coordination and exact commit/digest convergence without claiming distributed atomicity. Read-fence proof must show that ordinary Draft visibility advances only through the Game Design-owned synchronized fence carrying that same complete set.
