...
For the current single-admin/operator phase, Git-reviewed, CI-validated in-repository evidence is the production promotion trust root. The production overlay, staging deployment record, and production promotion attestation form one machine-verifiable evidence chain; for official releases, the release digest manifest additionally completes that chain. A production overlay change without the required attestation fails validation; absence of evidence is not a reason to skip the gate.

Promotion tooling machine-generates the candidate attestation and related evidence from observed staging state. It records the applied staging overlay commit, exact running service image digests, successful deployment and smoke evidence, required compliance references, production overlay target, and rollback classification; for an official release, it also records the release manifest reference. The operator reviews and approves that generated evidence and the resulting Git change. Routine digest and lineage fields are not manually reconstructed from memory or copied from intended configuration when observed state is available.
...
CI validates the evidence schema and required fields, confirms that the referenced staging commit exists in Git history, resolves the corresponding successful staging deployment record, verifies its live-state and required compliance evidence, compares every staged, attested, and production-overlay digest, and verifies all cross-references. For official releases, it additionally validates the release digest manifest's presence, schema, exact digest set, and bidirectional cross-references binding the release or deployment reference, source commit, promotion attestation, and staging deployment record.
