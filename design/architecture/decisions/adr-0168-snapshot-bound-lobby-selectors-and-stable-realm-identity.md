...
A tenant represents one hosted game. The tenant-facing `tenantSlug` remains a stable player-facing selector, owned by Game Design and resolved to opaque UUID `tenantId` from the exact `WORLDS` snapshot. An authored `worldSlug` is a stable tenant-scoped selector resolved only as `{tenantId, worldSlug}` from that same snapshot; neither selector is a globally unique identity, and `tenantSlug` is not retired from the player-facing lobby contract. A future rename is an explicit audited alias/redirect lifecycle; it never changes tenant identity silently.
...
Lobby menu ordinals are convenience selectors bound to the exact connection/list snapshot that produced them, including catalog revision and a bounded expiry. The selection chain is explicit: `WORLDS` resolves the tenant-facing selector (and any tenant-scoped `{tenantId, worldSlug}` authored-world selector); `REALMS` then accepts a snapshot-bound realm ordinal or stable tenant-local `realmSlug` and resolves it to `realmId` plus the applicable `gameInstanceId`; `CHARS` carries that resolved realm target; and `PLAY` completes the binding with the selected character. `PLAY 2`, `REALMS 2`, or a character ordinal never indexes a newly sorted catalog. An expired or changed snapshot, including a stale `REALMS` ordinal, asks the player to list/select again rather than risking a wrong target. Direct slugs remain convenient stable selectors and are re-resolved and reauthorized at use time.
