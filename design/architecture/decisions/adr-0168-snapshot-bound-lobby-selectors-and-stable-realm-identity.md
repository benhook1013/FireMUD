# ADR 0168: Snapshot-Bound Lobby Selectors and Stable Realm Identity
...
A tenant represents one hosted game. The tenant-facing `tenantSlug` remains a stable player-facing selector, owned by Game Design and resolved to opaque UUID `tenantId`; a client may supply it directly without a prior `WORLDS` snapshot, but the server re-resolves and reauthorizes it against current authority. An authored `worldSlug` is a stable tenant-scoped selector resolved only as `{tenantId, worldSlug}`; it may likewise be supplied directly once the tenant scope is known, with current-authority re-resolution and reauthorization. A list-derived ordinal is instead bound to the exact originating list snapshot that produced it (`WORLDS`, `REALMS`, or `CHARS`, as applicable). Neither selector is a globally unique identity, and `tenantSlug` is not retired from the player-facing lobby contract. A future rename is an explicit audited alias/redirect lifecycle; it never changes tenant identity silently.
...
Lobby menu ordinals are convenience selectors bound to the exact connection/list snapshot that produced them, including catalog revision and a bounded expiry. An ordinal is meaningful only against that originating list snapshot: `PLAY 2`, `REALMS 2`, or a character ordinal never indexes a newly sorted catalog. Direct accepted slugs are different: a client may use a stable slug without first obtaining a menu snapshot, but Game Session re-resolves and reauthorizes it against current authority at use time. The selection chain is explicit: `WORLDS` resolves the tenant-facing selector (and any tenant-scoped `{tenantId, worldSlug}` authored-world selector); `REALMS` then accepts a snapshot-bound realm ordinal or stable tenant-local `realmSlug` and resolves it to `realmId` plus the applicable `gameInstanceId`; `CHARS` carries that resolved realm target; and `PLAY` completes the binding with the selected character. An expired or changed snapshot, including a stale `REALMS` ordinal, asks the player to list/select again rather than risking a wrong target.
...
- Tenant-facing and tenant-scoped world selectors remain distinct, so tenant-scoped `{tenantId, worldSlug}` resolution avoids a global world namespace while preserving hand-authored client convenience.
...
Current public/cross-service identifiers remain largely numeric. The Game Session catalog can merge duplicate tenant-qualified slugs, realm/character ordinal parity is incomplete, and world ordinals are not snapshot-bound. Implementation must migrate canonical identities, introduce `realmId`, preserve tenant-facing `tenantSlug` while resolving tenant-scoped `{tenantId, worldSlug}`, bind ordinals to catalog snapshots, carry the resolved realm/runtime target through `CHARS` into `PLAY`, and align public discovery authorization.
...
Selector syntax and handle use can evolve without changing durable identities. Revisit selector syntax or alias policy if namespace pressure or federation requires owner-qualified names; preserve `tenantId` and `realmId` across any change.
