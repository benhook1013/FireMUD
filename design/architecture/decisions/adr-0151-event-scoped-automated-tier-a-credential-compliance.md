# ADR 0151: Event-Scoped Automated Tier A Credential Compliance
...
This decision is not implemented. Automated event-scoped Tier A evidence, readiness-only enforcement, unattended repair playbooks, and focused compliance proof remain gaps.
...
The evidence records the actual result of provisioning, rotation, or a safe binding-only rebind through content-addressed output. The immutable artifact identifier is a complete collision-resistant digest of the canonical evidence bytes; a truncated prefix or a manually supplied digest-looking value is not evidence. The dereferenced payload must remain bound to the parent record and target environment, credential class, and durable provisioning, rotation, or rebinding event/operation identity. A rebind records the retained material lineage and exact new target binding but preserves the material's actual `lastProvisionedAt` or `lastRotationAt`; the rebind event does not reset credential age. A Git-tracked compliance record is a versioned, post-finalization audit index to this evidence. The supporting event output is the immutable content-addressed artifact, and a finalized projection must not mutate a prior event's projection; the Git record's existence, timestamp, or commit history is not self-proof that the underlying action occurred.
...
A `not-provisioned` result is valid only when the environment inventory independently corroborates that the relevant environment does not exist. Bootstrap uses one durable operation identity and a monotonic provisioning generation to bind inventory confirmation, expected credential bindings, namespace/resource creation, and bootstrap evidence. Partial or crashed work remains pending, quarantined, or noncompliant and resumes under that same operation; only a completed exact generation may project `provisioned`. `not-provisioned` cannot be used to bypass evidence for an existing or partially provisioned environment.
...
Implementation must define the Tier A inventory and measurable rules; capture and retain canonical-byte, complete-digest provisioning and rotation output; verify that Git audit records resolve to output whose environment, class, target, and durable event/operation identity exact-match the parent record; corroborate `not-provisioned` against environment inventory; bind bootstrap resources and evidence to one durable operation and monotonic generation; and enforce the limited promotion, first-live, reopen, staging-evidence, and disaster-recovery-readiness gates.
