...
- A token-authority-only outage is recognized only when the binding also carries a current positive `coordinationHealthLease` issued by the coordination authority. That signal must identify the coordination generation and attest that the gameplay queues, locks, leases, session state, and tick coordination required by the binding are currently healthy. A missing, expired, conflicting, or ambiguous health signal fails closed and is treated as inability to establish the complete coordination plane; local process state or a successful token signature is not a positive health signal.
