...
After authorization and policy evaluation, Logging & Admin durably records the actor, reason, exact subject and scope, typed action, local policy-intent payload digest, local authorization reference, and one `policyIntentRequestId`. That identity scopes only Logging & Admin's policy-input and audit mutation. Under [ADR 0048](./adr-0048-durable-idempotent-operator-write-execution.md), it creates a separate owner-enforcement command with its own `ownerEnforcementRequestId`, payload digest, and owner-command authorization reference. The owner command may reference the policy intent for audit correlation, but it must not forward or reuse the local mutation identity, digest, or authorization reference:
...
The owner validates current authority and scope, then atomically commits the subject-scoped enforcement record and its idempotent command result. Owner-command idempotency and reconciliation use only the `ownerEnforcementRequestId`, owner payload digest, and owner authorization reference: reuse with the same tuple returns the prior result, while conflicting reuse is rejected. Logging & Admin reports success only after the owner acknowledges that durable commit. Its local policy-intent/audit persistence remains independently idempotent under `policyIntentRequestId`; a missing final audit-outcome update reconciles the local mutation against the recorded owner result and does not roll back or replay committed owner state.
...
- Policy intent and enforcement state can temporarily disagree after a failed delivery or acknowledgement. Each mutation reconciles under its own request identity, digest, and authorization reference without weakening runtime checks or reusing the other mutation's identity.
