...
- A canonical cross-service reference carries its kind and complete scope. Template references use `(tenantId, versionId, templateId)`; runtime references use `(tenantId, gameInstanceId, instanceId)`.
- APIs use typed reference messages when confusing identifier families could change authority or mutate the wrong state. A bare generic `id` is not a sufficient consequential cross-service contract.
