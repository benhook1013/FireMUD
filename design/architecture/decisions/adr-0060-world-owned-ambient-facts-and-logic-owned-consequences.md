...
World has ambient snapshot and event seams, but the typed `ApplyRoomAmbientStatePatch` handler, durable Game Session admission, operation/digest-bound participant guards, and focused replay/failure proof are not complete. The Weather aggregate selector remains explicitly pending; no Weather write path is admitted while that selector is unresolved.
...
- Review source: `MS-GR-AMBIENT-STATE-AUTHORITY`
...
- Before World evaluates any owner-local scope, epoch, version, or participant-guard predicate, it authenticates the complete canonical Game Session root binding: `EffectId`, typed operation, exact target aggregate and runtime scope, immutable request digest, and the sealed required-participant/manifest context (plus the applicable executor fence). A gameplay-session attestation or caller-provided `effectId` alone authenticates routing or correlation, not mutation authority. A missing, conflicting, or digest-mismatched binding fails closed before local validation or mutation.
