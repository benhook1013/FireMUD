...
1. Acquire the region's Redis lease without an expected active lease token: atomically claim only an absent or expired key using a unique opaque ownership token and a caller-supplied `lease_ttl_ms` validated as a positive integer before inspecting or mutating lease state, returning bounded `ACQUIRED` or `BUSY`. Missing, malformed, zero, or negative values return non-mutating `INVALID_ARGS`. This proves Redis liveness possession only; it does not establish the semantic `regionEpoch`.
...
5. Every post-acquisition region lease renewal and tick/lock Lua mutation compares the expected Redis token and region epoch. Renewal validates the same positive-integer `lease_ttl_ms` contract before inspecting or mutating lease state and never advances `executorFence`. No shared numeric minimum or maximum is selected here; that bound remains a pending settings-owner decision.
