...
For a same-boundary restore that rewinds only PostgreSQL, a credential class may use `verified_not_restored` when automation proves that its current Kubernetes or provider trust material was outside the selected restore artifact, was not replaced by snapshot-era state, and still matches the live expected binding for that environment. For JWT, that proof must include the current Account signing authority and JWKS remaining outside the restore, exact target binding, current issuer generation and keyset integrity, and validator acceptance of that unchanged current set. The universal Account authority/delegation and Game Session gameplay-binding/session invalidation still applies.
