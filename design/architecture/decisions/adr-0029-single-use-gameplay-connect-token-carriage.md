...
- Gateway atomically consumes `jti` in the player-facing Coordination Redis deployment before attempting the upstream WebSocket connection. That deployment uses `noeviction`, AOF persistence, the configured minimum replica-AOF acknowledgement, ACLs that prevent non-replay callers from mutating replay and fence keys, and reserved capacity above the bounded maximum live marker set. Replay markers never share a cache/rate-limit deployment or an eviction policy. Capacity alerts and an admission-readiness threshold fail new handshakes before memory exhaustion can make marker writes unreliable. The security-critical marker remains until `exp` plus the bounded clock-skew allowance. The canonical `DURABLE_REPLAY_CONSUME_ACK` is an atomic marker commit plus local AOF persistence and the configured minimum replica-AOF acknowledgement; a successful `SET`, `WAIT` replica-delivery acknowledgement, read-back, or client write completion alone is not sufficient. A player-facing profile that cannot prove the required configuration, capacity headroom, and acknowledgement is not eligible to admit new handshakes.
...
- Prove the replay deployment's `noeviction`, persistence, replica acknowledgement, ACL, capacity, and readiness controls; prove replay decisions are atomic across Gateway instances and remain unavailable through the full reset/failover/eviction quarantine whenever marker continuity is uncertain.
