...
- Every token carries the exact shared `replayAdmissionFence` observed at issuance. Account may issue only after reading the shared replay-readiness record as `OPEN`; an absent, unreadable, `QUARANTINED`, or changing record fails issuance with `CONNECT_REPLAY_PROTECTION_UNAVAILABLE`. Gateway validates signature and key identity, issuer and audience, lifetime and expiry, required claims, request routing scope, `state=OPEN`, and exact equality between the signed fence and the current shared fence before consuming the token. The fenced consume operation repeats the state and equality checks atomically with marker creation, so a quarantine transition racing issuance or admission rejects the stale token.
