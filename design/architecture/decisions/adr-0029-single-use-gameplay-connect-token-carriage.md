...
- `firemud.gateway.replayConsumeAckTimeoutMs` (environment variable `FIREMUD_GATEWAY_REPLAY_CONSUME_ACK_TIMEOUT_MS`) is the one replay-durability acknowledgement timeout. It is a positive finite integer number of milliseconds with a deployment hard upper bound; the same value is passed as the `WAITAOF` timeout and bounds the acknowledgement wait. There is no separate replay, AOF, or marker-write timeout. This setting does not replace the token lifetime or the quarantine deadline.
