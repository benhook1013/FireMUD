# ADR 0149: Automated Recovery Proof and Differentiated Traffic-Open Gates
...
The controlled-reopen boundary also cannot be implemented as a literal atomic transaction across PostgreSQL, Kubernetes traffic controls, workloads, and retained evidence. It requires an idempotent, crash-recoverable state machine whose durable states make partial progress safe and diagnosable.
...
### Scheduled Drills and Evidence Are Automated

Hosted production automatically schedules isolated restore drills and refreshes the retained recovery evidence when they succeed. The automation performs the restore rehearsal, convergence and validation path, records its exact inputs and results, and exposes actionable failure diagnostics. A human operator does not manually manufacture timestamps or proof records for routine drill success.
...
4. restores PostgreSQL with empty Coordination Redis;
...
Every step is idempotent and resumable. A crash, retry, or temporarily absent operator resumes from durable recovery state without replaying an unsafe destructive step, losing the reason for a failure, or silently opening traffic.
...
A hobby operator may explicitly open first-live traffic without that successful rehearsal. The tooling must present a clear warning, record the explicit recovery-unverified choice, and make no verified-recovery promise for that deployment. This exception applies only to hobby first-live; it does not redefine a failed or absent drill as success.
...
- Implement an idempotent, resumable actual-rewind workflow covering quarantine, old-authority fencing, candidate verification, restore, empty Redis, convergence, hardening, validation, smoke, and reopen preparation.
