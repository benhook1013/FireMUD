...
Account orchestrates an asynchronous, versioned JSON export manifest across every owning service for the authenticated global subject. A successful complete export contains all required owner contributions and their schema versions; partial failure remains visibly incomplete and retryable.
...
Tenant-admin export is a separate tenant-controlled recovery and billing contract, not a proxy full-subject request. It remains available through the billing-safe surface when that tenant is billing-blocked. It returns tenant-owned exportable records and only the minimum stable subject references needed to interpret them. It never exposes global email, credentials, external identities, security state, unrelated account attributes, or the existence of other tenant relationships.
...
Terminal deletion must not claim that immutable backup copies were edited in place. Backups expire under their existing schedule and are not ordinary searchable data. A restored environment must replay durable erasure and tombstone state before traffic reopens so deleted identity and authority cannot be resurrected.
