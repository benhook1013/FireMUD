...
- Prove missing, expired, malformed, unsupported-version, state-specific `active`/`retiring`/`revoked`, wrong-profile, wrong-account, wrong-generation, deleted, and unavailable registry state fails closed with stable errors.
