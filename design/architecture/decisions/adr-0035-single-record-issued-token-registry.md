...
| --- | --- | --- | --- | --- |
| `control-ui` | At most the catalog's finite positive `maxControlUiTenantScopes`; keys exactly equal non-empty `scopedRoles` keys | Same exact cap and key set as tenant authority | At most the same configured tenant-scope cap and only for applicable scoped tenants | `0` |
