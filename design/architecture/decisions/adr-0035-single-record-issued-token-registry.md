...
- Every supported version of the bounded record contains `schemaVersion`, a non-empty string JWT `kid`, the positive `signerGeneration` selected by the Account signer-promotion record, `accountId`, exact token profile/audience, `jti`, `iat`, `exp`, the profile-defined positive `tokenGeneration`, the complete applicable `authorityTuple`, the JWT's separate `membershipVersion` map (empty `{}` when non-applicable), positive `issuanceFence`, the matching Account evidence-bundle version/source/freshness evidence, and the explicit `state` defined below. The stored `kid` must be the exact verified JWT header key identifier, and `signerGeneration` must match the active Account signer-promotion record and its `kid`; signer generation is a signer-lifecycle value, not an alias or numeric substitute for `authorityTuple.issuerAuthGeneration`. The record does not duplicate tenant-role maps or global-role grants from the signed token.
...
- `signerGeneration` is linked canonically to the JWT header `kid` and the Account signer-promotion record, and the registry, issuance intent, reconciliation evidence, and response envelope must carry the same immutable linkage. It is compared only within that signer-promotion/`kid` linkage and is never compared numerically with any `authorityTuple` generation.
...
- Define one versioned bounded record schema with a non-empty string JWT `kid`, exact `signerGeneration`/`kid` linkage, immutable `operationId`/`requestId` and request-digest semantics, token `tokenHash` semantics, profile-specific `tokenGeneration` semantics, and mandatory conditional rotation fields on every supported version, and validate every field against the cryptographically verified token and matching Account-owned evidence.
