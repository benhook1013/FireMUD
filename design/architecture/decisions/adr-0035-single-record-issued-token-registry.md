...
Account exposes one versioned `account-auth-evidence-bundle/v1` for each authorization operation. At one Account linearization point, the bundle contains the complete applicable registry tuple, both cutoff objects, `issuanceFence`, the exact token-identity fence, `membershipVersion` (the exact tenant-ID map, or `{}` when non-applicable), and, when applicable, the replacement lease and exact gameplay binding identity/version/fence. It also contains the exact Account scope, a positive `bundleVersion`, the durable source transaction or outbox version, projection freshness/source evidence, and `outboxCheckpoints`: one exact `{outboxStreamKey, outboxSequence}` checkpoint for every applicable authority stream defined by the scope. Checkpoints cover the exact `account:auth-authority:v1:<scopeId>` stream keys involved in the bundle, with no aggregate maximum or cross-stream sequence substituted for an individual stream checkpoint. Non-applicable lease or binding fields are explicitly marked not applicable by the profile and operation; they are never silently omitted from an ambiguous partial response.

Account issuance and refresh use one Account transaction or composite CAS/fence to create only the durable Account evidence and the versioned `account-auth-evidence-bundle/v1` reference, including the exact authority snapshot, operation identity, source-version/freshness evidence, applicable `outboxCheckpoints`, and applicable downstream postcondition evidence. That transaction/CAS does not create the Coordination Redis registry record or any Redis lease or Game Session binding/installation. Each registry, lease, binding, and installation creation is a separately fenced, idempotent postcondition outside the Account transaction, using the same operation, request, digest, and `issuanceFence`; retries and reconciliation apply only missing postconditions, preserving recovery from partial failure. The registry record stores the matching `bundleVersion` and source-version/freshness evidence with its tuple. Validation performs exactly one issued-token registry lookup, captures its version/conditional-read fence, and reads one complete Account bundle. It then compares the JWT, registry tuple, every applicable cutoff as its complete generation-plus-`outboxStreamKey`-plus-`outboxSequence` tuple, every applicable `outboxCheckpoint`, fences, leases, binding, separate `membershipVersion` claim, and source evidence as one equality decision. The registry lookup is bound to the Account evidence linearization by that versioned/conditional fence or an equivalent cross-store consistency fence: if the registry record changes between lookup and comparison, the operation denies or retries from a fresh evidence read and never authorizes from the earlier registry snapshot. The `membershipVersion` claim, registry field, and Account evidence value must all use the same exact map, including `{}` for non-applicable membership; no omitted/null/sentinel form is equal. If a bundle version, source version, projection version, checkpoint, or related registry/binding version changes during the read or comparison, the consumer discards the partial result and retries the complete evidence read from a fresh linearization point; it never combines independently changing fields or authorizes from the earlier version.
