...
Account issuance and refresh use one Account transaction or composite CAS/fence to create only the durable Account evidence and the versioned `account-auth-evidence-bundle/v1` reference, including the exact authority snapshot, operation identity, source-version/freshness evidence, and applicable downstream postcondition evidence. That transaction/CAS does not create the Coordination Redis registry record or any Redis lease or Game Session binding/installation. Each registry, lease, binding, and installation creation is a separately fenced, idempotent postcondition outside the Account transaction, using the same operation, request, digest, and `issuanceFence`; retries and reconciliation apply only missing postconditions, preserving recovery from partial failure. The registry record stores the matching `bundleVersion` and source-version/freshness evidence with its tuple. Validation reads one registry record and one complete Account bundle, then compares the JWT, registry tuple, cutoffs, fences, leases, binding, separate `membershipVersion` claim, and source evidence as one equality decision. The `membershipVersion` claim, registry field, and Account evidence value must all use the same exact map, including `{}` for non-applicable membership; no omitted/null/sentinel form is equal. If a bundle version, source version, projection version, or related registry/binding version changes during the read or comparison, the consumer discards the partial result and retries the complete evidence read from a fresh linearization point; it never combines independently changing fields or authorizes from the earlier version.
