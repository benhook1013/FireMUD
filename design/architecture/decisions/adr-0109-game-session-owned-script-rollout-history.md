...
Game Session-owned exact pin and append-only rollout history with Automation-only convergence projection are target state. The current Game Session implementation persists a pin and exposes convergence reads, while Automation persists synthetic rollout projections and local rollout events derived from observed pin state and work-item transitions. Append-only request-idempotent Game Session history, exact epoch coverage, direct history reads, monotonic `scriptPinEpoch` projection application, bounded reconciliation, mandatory admission-time rereads, and removal or demotion of the synthetic rollout lifecycle remain implementation and proof gaps.
...
Automation & Scripting owns tenant patch readiness and, for rollout-history state, only the instance-local observed exact-pin, convergence, and freshness state needed for admission, scheduling, handoff, and diagnostics. Those records are explicitly projections. Automation does not author `PINNED`, `ROLLED_BACK`, or `REPINNED` history from work-item presence, projection refresh, or local guesses, and its reads do not become rollout authority. A projection reread applies an exact `(scriptPatchVersion, scriptPinEpoch)` only when its authoritative `scriptPinEpoch` is newer than the currently applied `scriptPinEpoch`; lower or reordered results cannot overwrite newer state, and an equal `scriptPinEpoch` is accepted only when the exact tuple and canonical digest/content agree. A contradictory equal-epoch result is rejected without changing effective projection state or its evidence. Lost advisory wake-ups recover through bounded periodic reconciliation. At every instance-bound admission, Automation performs a mandatory bounded reread from Game Session when the projection is missing, stale, or ambiguous; without exact authoritative evidence, admission fails closed.
...
`ScriptPatchPinChanged` is an advisory cache-refresh wake-up from Game Session that may accelerate Automation refresh and operator updates. Consumers recover current truth by rereading Game Session's authoritative current-pin and rollout-history APIs after a lost, duplicated, delayed, or reordered wake-up; loss or duplication affects freshness latency only and cannot erase rollout history. The ordinary family has no transactional-outbox or guaranteed durable-delivery contract. A future outbox is permitted only for a separately named guaranteed asynchronous consequence or durable consumer that adopts [ADR 0083](./adr-0083-no-general-event-broker-until-measured-adoption-gates.md). The separately named same-transaction `ScriptPinConvergenceTimedOut` producer-capture exception remains governed by its rollout contract and does not upgrade `ScriptPatchPinChanged`. A mandatory event-sourced Automation rollout-history projection, a duplicate `ScriptPatchInstanceRolloutChanged` family distinct from the committed pin-change record, and independent projection retention, replay, and SLO machinery are deferred until measured query load, availability, or consumer needs justify them.

## Consequences

- Pin mutation, exact epoch, and rollout history cannot disagree across two authorities.
- Operator history remains correct through advisory `ScriptPatchPinChanged` loss or duplication and Automation projection rebuild; recovery rereads Game Session.
...
Game Session must enforce uniqueness for `controlPlaneRequestId`, durably bind each request identity to its normalized request digest, atomically commit pin/epoch and append-only history, and expose bounded current/history reads with stable ordering and pagination. Reusing an identity with a different digest must conflict before any pin, epoch, or history mutation. History records are immutable except for explicitly modeled correction metadata that cannot rewrite the committed pin result. `ScriptPatchPinChanged` remains an advisory wake-up with no ordinary transactional-outbox obligation; consumers recover by rereading the owner, and loss or duplication affects freshness latency only. Any future transactional outbox must be tied to a separately named guaranteed asynchronous consequence or durable consumer and satisfy ADR 0083. This does not alter the separately named same-transaction `ScriptPinConvergenceTimedOut` producer-capture exception.

Proof must cover same-request retry; changed-digest pin requests rejected without pin, epoch, or history mutation; concurrent pin and rollback attempts; repin to the same version with a new epoch; failure before and after database commit; lost, duplicate, delayed, and reordered advisory wake-ups with owner reread recovery; lower and contradictory equal projection rereads rejected without state or evidence changes; strictly newer `scriptPinEpoch` rereads applied atomically; bounded periodic reconciliation after lost wake-ups; missing/stale/ambiguous projection admission forcing one bounded owner reread and failing closed without exact evidence; Automation restart and projection rebuild; direct history reads while Automation is unavailable; disagreement surfaced as convergence lag; pagination and retention; and Logging & Admin composition without a competing state write.
