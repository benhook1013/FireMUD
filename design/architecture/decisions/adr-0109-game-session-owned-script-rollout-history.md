...
Proof must cover same-request retry; changed-digest pin requests rejected without pin, epoch, or history mutation; concurrent pin and rollback attempts; repin to the same version with a new epoch; failure before and after database commit; lost, duplicate, delayed, and reordered advisory wake-ups with owner reread recovery; Automation restart and projection rebuild; direct history reads while Automation is unavailable; disagreement surfaced as convergence lag; pagination and retention; and Logging & Admin composition without a competing state write.
