# ADR 0161: Profile-Aware Isolated Synthetic Player-Flow Canaries
...
[ADR 0159](./adr-0159-profile-dependent-independent-deadman-and-public-path-monitoring.md) makes independent public-path monitoring profile dependent. Synthetic player-flow requirements follow the same honest assurance boundary instead of imposing hosted monitoring infrastructure on every hobby deployment.
...
The current implementation is partial. `dev-tools/observability/run-player-experience-smoke.py` performs real WebSocket or Telnet `LOGIN -> PLAY -> LOOK` through `run_websocket_canary` and `run_telnet_canary`; `--canary-path` selects the preferred first transport and one invocation exercises every declared exposed path. Its defaults use the ordinary seeded demo credentials, Account has no authoritative synthetic classification, no checked-in schedule proves continuous execution for every exposed path, and the current rules do not prove a stopped runner before its first expected emission. Existing contract tests prove simulated evidence shape and failure injection, not a live independently scheduled hosted deployment.
...
- [design/architecture/decisions/adr-0159-profile-dependent-independent-deadman-and-public-path-monitoring.md](./adr-0159-profile-dependent-independent-deadman-and-public-path-monitoring.md)
