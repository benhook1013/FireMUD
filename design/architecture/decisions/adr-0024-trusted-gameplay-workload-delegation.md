...
Game Session is itself the attestation signer and gameplay-session authority, so attestations cannot protect against a compromised Game Session signer. They also cannot protect against a compromised destination that can misuse an attestation it accepts. Their incremental protection is against a compromised or seriously buggy allowlisted intermediary fabricating or misbinding player context: the intermediary cannot create a valid Game Session-origin attestation for an arbitrary context or use one outside its destination, method, and one-time-use bounds. FireMUD currently runs first-party gameplay workloads inside one operated trust domain and does not admit third-party workloads to internal gameplay RPCs.
