...
A retry retains the original command or effect identity, immutable request digest, semantic lane, `entity_enqueue_seq`, scheduler priority, and durable owner record. Among scheduling fields, it changes only the persisted future `nextEligibleTick`; attempt count, failure class, last outcome, and timing evidence are recorded on that same owner record. When eligible, the work re-enters the deterministic fair scheduler defined by ADR 0065. It does not enter a retry-private FIFO or gain priority by failing.
