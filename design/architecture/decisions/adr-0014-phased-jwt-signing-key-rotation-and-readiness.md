...
The Kubernetes baseline uses fixed, pre-created `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap resources. The Account Service account has narrowly name-scoped `get`, `update`, and `patch` authority over those two resources, but no list, create, or delete authority. Account writes both through Kubernetes `resourceVersion` compare-and-set and then consumes their read-only projected mounts. The rotation Job has no write access to either resource; it requests transitions through Account's control interface and observes the resulting generation.

After each CAS update, Account must observe the pending generation in both read-only projected mounts, including the mounted generation markers, rather than treating Kubernetes API write success or a served JWKS response as proof that its process has consumed the update. It must cryptographically derive or challenge-sign the public key from the mounted private key and prove that it matches the published JWK for the same `kid` and generation before that generation can be promoted.
...
4. Atomically promote the Account-owned active signer reference to the validated generation and `kid` only after Account has observed the matching Secret and ConfigMap generation in its mounted files, cryptographically proved that the mounted private key matches the published JWK, and proved that both the non-authorizing canary and representative production-family probes have converged at every required validator. The private key remains in non-exportable signer custody, or in the interim Account-only bundle. A malformed bundle, missing or stale mount, generation mismatch, key mismatch, unobservable pending key, incomplete validator inventory, or failed probe aborts promotion and leaves the old signer active while readiness fails and operators are alerted. The pending key is never used for issuance; existing traffic may continue under the old signer only while its existing readiness remains valid, otherwise the gate fails closed.
...
The rotation resources carry a common generation identifier and phase so separately updated Kubernetes objects cannot be mistaken for one atomic transaction. Every resource update uses its observed Kubernetes `resourceVersion`; a CAS conflict aborts the phase and requires reread/reconciliation. Signer promotion is the commit point. Publishing an additional verification key is safe before that point; publishing a signer whose key is not converged is not.
...
4. Advance the environment-wide issuer authority generation and perform required session/allowlist cleanup. The generation is defense in depth: an attacker holding the old private key can choose fresh claims, so key removal and validator convergence remain mandatory.
...
- Account observes the mounted Secret and ConfigMap generations and cryptographically proves the mounted private key matches the published JWK before signer promotion;
...
- Provision fixed Account-owned JWT/JWKS resources and name-scoped Account RBAC; prove `resourceVersion` CAS, mounted Secret/ConfigMap generation observation, cryptographic private-key/JWK correspondence, and absence of rotation-Job write authority.
