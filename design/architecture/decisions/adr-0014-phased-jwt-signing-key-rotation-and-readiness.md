...
Startup and recovery always reconcile from the durable Account issuer record first. Account must not infer the current generation or active authority from signer/JWKS agreement, a Redis key that lacks freshness evidence, token timestamps, or registry presence. A readable but missing, malformed, regressed, or conflicting durable record, projection evidence, token claim, or signer/JWKS generation invalidates the affected authority and keeps issuance, new admission, reconnect/resume, and authority-dependent protected operations quarantined until Account completes the required reconciliation and correspondence proof. An unavailable authority dependency is a distinct retryable outage: it blocks issuance, new admission, reconnect/resume, and authority-dependent protected operations, but an already-admitted gameplay binding may continue only within its still-valid bounded lease and cannot be renewed after that lease expires.

### Crash-Safe Issuance And Registry Projection

Issuance is a durable Account operation, not a process-local sequence. Before signing or exposing a JWT, Account creates a compare-and-set protected issuance-intent record containing an operation ID, a pre-sign request identity/fingerprint, exact profile and subject, the expected durable issuer-authority record version, and the exact positive `issuerAuthGeneration`. The intent must not claim the final compact-JWT hash because that value does not exist until signing succeeds. A Redis projection, signer response, or unsigned request fingerprint never creates issuance authority by inference.

After the signer returns the compact JWT, Account computes its exact token hash and CAS-binds that hash to the still-owned issuance intent. Account then CAS-writes the issued-token registry projection with the same compact-JWT hash, exact profile and subject, issuer generation, exact JWT-header `kid`, signer generation, and authority source-version/freshness evidence. It must read back and verify the complete postcondition: the durable intent still owns the expected authority version and bound hash; the registry record has the same exact hash, identity, `kid`, and signer generation; its state is authorizing as required; its issuer generation and source-version evidence match the durable Account record; and the JWT-header `kid`, registry `kid`, active signer-promotion record, and signer generation correspond exactly. Only after that postcondition is verified may Account CAS-transition the issuance intent to `COMMITTED` and return the JWT. Any failed, stale, ambiguous, or unavailable hash binding, key/generation correspondence, projection, or readback leaves issuance quarantined and the token unexposed; it is not repaired by treating Redis presence as success.

After a crash or ambiguous response, Account reconciles unresolved issuance intents from durable state. Reconciliation may commit only when the exact durable authority version, post-sign compact-JWT hash binding, token identity, JWT-header and registry `kid`, signer generation, active signer-promotion record, registry projection, source-version/freshness evidence, and required postcondition all match the intent; otherwise it revokes or removes the candidate projection and records terminal failure without returning or authorizing the token. Startup and recovery therefore quarantine issuance until incomplete intents are reconciled, and neither Redis, a JWT claim, nor a matching signer/JWKS pair can bypass that durable decision.
...
- Define and prove the Account-owned durable `issuerAuthGeneration` record, its set-if-greater projection freshness evidence, and exact equality with the JWT claim and issued-token registry field without introducing a second issuer-generation authority.
