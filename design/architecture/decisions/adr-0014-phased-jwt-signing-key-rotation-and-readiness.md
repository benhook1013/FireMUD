...
After the signer returns the compact JWT, Account computes its exact token hash and CAS-binds that hash to the still-owned issuance intent. Account then CAS-writes the issued-token registry projection with the same compact-JWT hash, exact profile and subject, issuer generation, and authority source-version/freshness evidence. It must read back and verify the complete postcondition: the durable intent still owns the expected authority version and bound hash, the registry record has the same exact hash and identity, its state is authorizing as required, and its issuer generation and source-version evidence match the durable Account record. Only after that postcondition is verified may Account CAS-transition the issuance intent to `COMMITTED` and return the JWT. Any failed, stale, ambiguous, or unavailable hash binding, projection, or readback leaves issuance quarantined and the token unexposed; it is not repaired by treating Redis presence as success.
