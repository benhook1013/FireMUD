...
3. Wait at least the configured validator JWKS maximum cache age and prove every required validator can resolve the pending `kid`. An unknown `kid` causes one forced JWKS refresh and one validation retry, then fails closed.
