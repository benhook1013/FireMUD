...
The canonical issuer-generation source is the Account-owned durable authority record for `session:auth:generation:issuer:<issuerId>`. Its canonical field is the positive monotonic `issuerAuthGeneration`; no parallel `issuerGeneration` authority or alias is introduced. Account owns every durable write and advance, and records the Account transaction or outbox version that established the value plus the current lifecycle status. The Coordination Redis key is only an idempotent projection with freshness evidence; a Redis value, JWT claim, token `iat`, JWKS document, signer response, or registry record is not an independent authority source.
...
Issuance is a durable Account operation, not a process-local sequence. Before signing or exposing a registry-backed Account JWT, Account creates a compare-and-set protected issuance-intent record containing an operation ID, an immutable pre-sign request identity/fingerprint, exact profile and subject, the exact applicable authority scope, the complete applicable `authorityTuple` defined by the [canonical JWT authority-tuple contract](../system-architecture-jwt-and-token-contracts.md#canonical-authority-tuple) (with explicit absence where a member does not apply), independent `membershipVersion` when applicable, a positive `issuanceFence`, the expected durable issuer-authority record version, and the exact positive `issuerAuthGeneration`. The intent must not claim the final compact-JWT hash because that value does not exist until signing succeeds. A Redis projection, signer response, or unsigned request fingerprint never creates issuance authority by inference.

After the signer returns the compact JWT, Account computes its exact token hash and CAS-binds that hash to the still-owned issuance intent. Account then CAS-writes the issued-token registry projection with the same compact-JWT hash, exact profile and subject, complete applicable `authorityTuple`, independent applicable `membershipVersion`, positive `issuanceFence`, issuer generation, exact JWT-header `kid`, signer generation, and an immutable authority-evidence bundle containing the authority scope, source transaction/outbox or event version, projection status/freshness, and issuance-operation identity. It must read back and verify the complete postcondition: the durable intent still owns the expected authority version, tuple, membership version, issuance fence, and bound hash; the registry record has the same exact hash, identity, tuple, membership version, issuance fence, `kid`, and signer generation; its state is `active`; for the applicable private `game-session-account-delegation` profile, the ADR 0035 installation lease and Game Session binding postcondition is durably `INSTALLED` before the replacement is exposed for protected use; its issuer generation and evidence bundle match the durable Account record; and the JWT-header `kid`, registry `kid`, active signer-promotion record, and signer generation correspond exactly. Only after that postcondition is verified may Account CAS-transition the issuance intent to `COMMITTED` and return the JWT. Any failed, stale, ambiguous, or unavailable hash binding, authority tuple, fence, key/generation correspondence, evidence bundle, projection, installation postcondition, or readback leaves issuance quarantined and the token unexposed; it is not repaired by treating Redis presence as success. The Account durable transaction/evidence and the Redis registry projection are ordered, fenced postconditions, not one global transaction; Account does not include Redis or a Game Session binding in its transaction.

After a crash or ambiguous response, Account reconciles unresolved issuance intents from durable state. Reconciliation may commit only when the exact durable authority version, complete applicable `authorityTuple`, independent applicable `membershipVersion`, positive `issuanceFence`, post-sign compact-JWT hash binding, token identity, JWT-header and registry `kid`, signer generation, active signer-promotion record, immutable authority-evidence bundle, registry projection, source-version/freshness evidence, and required postcondition all match the intent; otherwise it revokes or removes the candidate projection and records terminal failure without returning or authorizing the token. Startup and recovery therefore quarantine issuance until incomplete intents are reconciled, and neither Redis, a JWT claim, nor a matching signer/JWKS pair can bypass that durable decision.

### Committed Response-Loss Recovery

A `COMMITTED` issuance intent is never signed or issued again. To recover a response lost after the registry postcondition committed, Account may retain a separate bounded response envelope under Account-owned application encryption, using the same exception and binding discipline as ADR 0031. The envelope contains the compact JWT and is bound to the exact request identity, pre-sign request fingerprint, token hash, profile, subject, complete applicable `authorityTuple`, independent applicable `membershipVersion`, positive `issuanceFence`, authority-record version, and issuer generation; it is not issuance evidence, registry state, or authorization authority. A retry with the exact request identity returns that same JWT only while the envelope is present and within its bounded lifetime, after revalidating the durable intent, registry record, authority tuple, membership version, issuance fence, authority version, profile, subject, and current revocation state. Account never re-signs or creates a second registry record for that request.
...
- Define and prove the Account-owned durable `issuerAuthGeneration` record, its set-if-greater projection freshness evidence, and exact equality with the JWT claim and issued-token registry field without introducing a second issuer-generation authority. For registry-backed Account JWT profiles, the issuance intent, registry postcondition, reconciliation record, and bounded response envelope must also carry and compare the complete applicable `authorityTuple`, independent applicable `membershipVersion`, positive `issuanceFence`, and immutable authority-evidence bundle. Gameplay-connect token replay remains governed by [ADR 0029](./adr-0029-single-use-gameplay-connect-token-carriage.md); signed connect-context signing, carriage, and proof use the trusted workload delegation contract in [ADR 0024](./adr-0024-trusted-gameplay-workload-delegation.md).
