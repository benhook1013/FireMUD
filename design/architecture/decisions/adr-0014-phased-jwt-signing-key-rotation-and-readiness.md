...
- Define and prove the Account-owned durable `issuerAuthGeneration` record, its set-if-greater projection freshness evidence, and exact equality with the JWT claim and issued-token registry field without introducing a second issuer-generation authority. The issuance intent, registry postcondition, reconciliation record, and bounded response envelope must also carry and compare the complete applicable `authorityTuple`, independent applicable `membershipVersion`, positive `issuanceFence`, and immutable authority-evidence bundle.
