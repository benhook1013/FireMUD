...
Each emitted command has a durable child dispatch row beneath its parent work item. At the first durable evaluated-dispatch commit, Automation allocates and persists one opaque `automationDispatchId` that is never reused within the complete source runtime scope; retries of that evaluation reuse the same ID. Each child uses the complete [Command-Handoff Identity](../system-architecture-scripting-normative-contract-tables.md#command-handoff-identity-target-state): stable source/target scope, that persisted dispatch ID, and deterministic `commandOrdinal`/canonical output position. The complete source/target runtime scope, dispatch ID, and ordinal prevent unrelated evaluations from converging. `automationDispatchId` is a dispatch-group suffix and is not globally unique; the parent Trigger Identity and `outboxWorkItemId` are retained for correlation only and are not part of command-child uniqueness. The durable child stores the immutable command/request digest and links to downstream outcome evidence; the same complete identity with the same digest converges on the existing child, while a conflicting digest fails closed.
