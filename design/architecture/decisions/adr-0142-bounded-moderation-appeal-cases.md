...
An upheld decision creates no enforcement mutation or owner command. A modified or overturned decision sends a new command referencing the appeal case and exact appealed restriction. That owner command carries the exact appealed owner revision and payload digest as an expected-state precondition; if the current owner state no longer matches, the owner returns a stable conflict/no-op without mutating newer state. The owner applies a matching command as a newer ordered transition and must not erase a later unrelated restriction. The same owner-command identity and digest replay the stored result. Original action, restriction, evidence, appeal transition, and review decision histories remain append-only.
