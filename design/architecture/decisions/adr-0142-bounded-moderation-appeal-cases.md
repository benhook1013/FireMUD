...
Active-case admission uses a separate key over the caller-authorized appellant and exact appealed restriction identity/revision. Logging & Admin owns this appeal-case key. Transactional uniqueness or an equivalent compare-and-set keeps that key active across `SUBMITTED` and `UNDER_REVIEW`; for a `DECIDED` `MODIFIED` or `OVERTURNED` outcome, Logging & Admin retains the key or an equivalent outcome-in-flight lock until it durably reconciles the linked enforcement owner's terminal result, including the durable terminal `IDEMPOTENCY_CONFLICT` outcome, and only then releases it. The enforcement owner persists and exposes its terminal outcome but does not release the Logging & Admin-owned key. Distinct request identities racing on the same key have one winner; the loser or a repeat while the outcome command is unresolved receives a stable caller-safe existing-case result and creates no second case, evidence, audit, or owner-command side effects. This admission key is separate from the per-request idempotency identity.
