# ADR 0162: Profile-Aware Asynchronous End-to-End Log Queryability Evidence
...
Each retained `logPipelineQueryability` result also records the profile-resolved `evidenceFreshnessBudgetSeconds`. This budget is a positive, finite value selected by the exercised deployment profile; it is not a universal constant. The producer derives `evidenceExpiresAt` from the trusted `evidenceObservedAt` timestamp as `evidenceObservedAt + evidenceFreshnessBudgetSeconds`, and the validator rejects missing, non-positive, non-finite, or inconsistent values. For a non-omitted result, the trusted evaluation chronology is `emittedAt <= retrievedAt <= evidenceObservedAt <= verifiedAt`; source timestamps may not be future-dated relative to `verifiedAt`, and an evidence result is unusable at or after `evidenceExpiresAt`. `verifiedAt` is the evaluator's trusted completion time, not an ingestion or dashboard timestamp.

The serialized `logPipelineQueryability.capability` vocabulary is closed: `indexed-log-observability` proves the indexed backend path, `console-journal-log-observability` proves a reduced console or journal retrieval path, and `log-queryability-omitted` explicitly records that indexed queryability is not claimed. Non-omitted capabilities use only `passed` or `failed`; `not_applicable` is reserved for explicit omission. A `failed` result is non-authorizing for readiness, promotion, release, or an observability claim, and may be retained only under the explicit incident/failure-evidence mode. An explicit omission retains `evidenceObservedAt`, `evidenceFreshnessBudgetSeconds`, `evidenceExpiresAt`, and `evidenceRef`; the enclosing evidence still requires the trusted evaluator `verifiedAt`, but the omitted queryability object does not contain `verifiedAt`. Omitted evidence must not fabricate indexed or retrieval fields such as backend, storage target, query path, emission/retrieval timestamps, delay measurements, record identity, source identity, or verified fields.
...
There is no implemented runtime check that emits a known synthetic record, retains its `traceId`, polls the configured backend, and proves retrieval through the supported query path within the configured target. Existing player-experience evidence can record a supplied log-query status but does not itself perform that emit-and-query proof. The freshness-budget and trusted-chronology rules above are a target evidence contract and validator boundary; they do not claim that runtime emit-and-query automation is complete.
...
- [design/observability/kibana/](../../observability/kibana/)
- [Logging & Admin Service architecture documentation](../microservices/logging-admin-service/README.md)
