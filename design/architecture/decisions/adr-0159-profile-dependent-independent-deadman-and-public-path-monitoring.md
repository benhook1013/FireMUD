...
Each `independent-required` profile records its configured heartbeat interval and timeout, stale threshold, expected probe cadence and timeout, ingestion/observation delay, evaluation cadence, and resulting maximum detection budget, together with a reproducible derivation. A deployment may choose tighter or looser values for its environment, but it must make the resulting detection promise explicit and validate freshness against those configured values. Changing the cadence without updating the corresponding threshold, evidence, and operational claim is non-compliant.

Whenever `playerFlowCanary` is advertised, the profile timing declaration also records every derivation input and the reproducible formula for its positive `detectionBudgetSeconds`: canary cadence, execution timeout, observation/ingestion delay, and evaluation cadence are required for `independent-omitted`. That profile's budget is local canary timing only; it does not create deadman, public-path, or other external-monitoring authority.
...
Current implementation is partial. FireMUD has player-flow smoke tooling, mirrored metric vocabulary, profile-bound Prometheus deadman rules, retained-evidence validation, and documented external deadman behavior. The authoritative off-cluster monitor, deployment-owned expected-series inventory, path-specific Prometheus blackbox alert gate, and paging deployment remain environment-specific and are not checked in or proved as a complete hosted production path. Current repository evidence does not establish ongoing freshness, real external failure-domain separation, or successful off-cluster page delivery for a live production environment.
