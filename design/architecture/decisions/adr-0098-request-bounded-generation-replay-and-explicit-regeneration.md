...
The request-bounded replay and committed-topology authority contract is target state. Current admission identity, immutable implementation selection, staged-output retention, rolling-deployment retry handling, explicit regeneration, and persistent recovery proof are incomplete; existing generation metadata must not be treated as proof of this contract.
