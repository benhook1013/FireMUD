...
- The event is the fast path. From the authoritative Account commit at `T0` to completed socket/admission termination, the end-to-end active-revocation budget is at most 60 seconds. For the configured worst-case issuer scope, let `N_issuer` be the maximum active or provisional bindings covered by one issuer cutoff, counting each binding once across all `issuerIndexPartitionCount` partitions and including bindings covered by unresolved issuer repair obligations. The deployment proof must establish the positive `issuerIndexPartitionCount` and `issuerIndexPartitionCapacity` and derive `N_issuer <= issuerIndexPartitionCount * issuerIndexPartitionCapacity`; admission or index repair must be rejected or backpressured rather than exceed that bound. The proof must not use average occupancy or a later wildcard scan. Let `C_terminate` be the minimum fenced termination throughput in bindings per second under the worst permitted partial-outage conditions. A capacity measured in another unit must be explicitly converted to bindings per second before this calculation. Before evaluating the equation, deployment validation must establish that `N_issuer` is a finite non-negative integer, `C_terminate` is a finite strictly positive bindings-per-second value, `B_termination_setup` is finite and non-negative, and every other budget term is finite and non-negative. A missing, non-finite, zero, negative, otherwise invalid, or unit-ambiguous prerequisite rejects the deployment; it must not be coerced or passed into the equation. Define `B_termination = B_termination_setup + ceil(N_issuer / C_terminate) seconds`, where `B_termination_setup` is only the non-throughput termination setup time. The single canonical deployment-validated equation is `B_event_delivery + B_gap_detection + B_scheduler_jitter + B_retries + B_datastore_timeouts + B_clock_skew + B_termination <= 60 seconds`; the termination setup and capacity terms are included only through `B_termination`. No retry or backoff may extend beyond its assigned budget, and the configured sum must be validated before deployment.
- The deploy-time capacity proof must establish the worst-case `N_issuer`, `C_terminate`, and `B_termination_setup` used by that equation, not only per-binding latency. If the invariant cannot be proven for a rollout, the deployment is rejected or backpressured; if current occupancy would exceed the proven bound, new `PLAY`/admission is rejected or backpressured until capacity is restored. The proof must cover all Game Session partitions and Gateway termination work and must not assume an unbounded queue or average-case throughput.
