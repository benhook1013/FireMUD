...
The previous target distinguished soft and hard billing states and required event-driven revocation, but it left new-admission behavior during `grace`, private-realm grant revocation, and the meaning of “immediate” propagation unclear. It also had no implemented producer/consumer, monotonic membership version, auth-generation workflow, bounded active-session indexes, or hard-cutoff proof. Current code principally rechecks membership and a limited entitlement response at `PLAY`; already-connected sessions are not revoked by these changes.
...
- Account is the sole authority and auth-generation writer. It commits the durable state/version change, applicable monotonic generation advance, and outbox event atomically in its database, then idempotently projects the generation set-if-greater. [ADR 0036](./adr-0036-monotonic-authority-generations-for-bulk-token-revocation.md) supersedes timestamp watermark ordering. A cutoff workflow does not report enforcement complete until generation projection succeeds.
...
- The event is the fast path. Batched generation/version reconciliation must ensure a missed event cannot preserve revoked gameplay authority for more than 60 seconds.
...
- Routine gameplay commands do not call Account or read auth generations. The bounded reconciliation is periodic and batched by active authority scope.
...
- The contract adds durable event, generation-projection, versioning, active-index, reconciliation, and end-to-end cutoff proof obligations.
...
- Implement Account-owned account, tenant, and membership generation projection with set-if-greater retry and cutoff-completion semantics.
...
- Prove routine gameplay commands perform no Account or auth-generation lookup.
