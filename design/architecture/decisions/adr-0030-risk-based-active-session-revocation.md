...
`issuerAuthGeneration` and `accountAuthorityGeneration` are positive Account-owned generations. `tenantAuthorityGeneration` and `membershipAuthorityGeneration` are independent maps keyed by exact tenant IDs; each map's applicable keys are determined separately by the token profile and route classification. The closed `billing_safe_tenant` exception can therefore require a membership entry while deliberately omitting the target-tenant generation. Explicitly unscoped artifacts use empty maps. `privateRealmGrantVersions` contains exact `{tenantId, worldSlug, realmSlug, grantVersion}` entries and is empty for public production. `accountSecurityCutoff` and `tenantBillingCutoff` are optional object-shaped cutoff evidence, not new authority sources; each is omitted only when it is not applicable, and a present `tenantBillingCutoff` retains only its exact applicable tenant-ID entry or entries. Optional objects are never covered by list/map omission-as-empty rules: omission does not mean `{}` or `[]`, and an applicable object must be present with its exact shape. `membershipVersion` is separate membership projection/version data and must be compared independently from `membershipAuthorityGeneration`; neither field substitutes for the other. A missing applicable field, extra scope, malformed value, or mismatch fails closed.
...
- `tenantBillingSequence` has one canonical billing-cutoff stream mapping: for tenant `<tenantId>`, every billing cutoff advances the Account-owned sequence and is emitted on `account:auth-authority:v1:tenant/<tenantId>`. The `tenantBillingCutoff[tenantId]` evidence carries that exact `tenantBillingSequence`, `outboxStreamKey`, and `outboxSequence`; no second billing stream or tenant-local sequence source exists. The billing cutoff consumer stores `lastAppliedOutboxSequence`, `lastAppliedTenantBillingSequence`, and the event ID/digest for that exact stream. It validates the ordinary outbox sequence as contiguous for every event, and for each billing-cutoff event validates the exact tenant and stream mapping plus `tenantBillingSequence == lastAppliedTenantBillingSequence + 1`; a non-billing event must not advance or regress the billing sequence. Matching duplicates are no-ops only when their event ID/digest and sequence evidence match; a same-key gap, conflicting duplicate, missing sequence evidence, regression, or mismatch stops the affected scope, reconciles from Account, and advances either watermark only after the Account checkpoint proves coverage. A first event initializes both watermarks from the Account checkpoint rather than assuming a gap or inferring a billing sequence.
...
- Game Session consumes revocation events durably and idempotently. The canonical Account outbox payload is:
  ...
  `sourceScope` must decode to the scope encoded by `outboxStreamKey`; a mismatch is malformed evidence. A consumer stores one `lastAppliedOutboxSequence` and a bounded per-sequence evidence window containing the applied event ID and digest for each exact `outboxStreamKey`, never one global watermark. If `outboxSequence <= lastAppliedOutboxSequence`, the consumer may treat the event as an older or duplicate no-op only when retained per-sequence evidence contains the same `outboxStreamKey`, `outboxSequence`, event ID, payload digest, full applicable `authorityTuple`, and `issuanceFence`, or an authoritative Account checkpoint returns that exact committed event evidence; an evicted local entry is not proof of a match. The same sequence with different evidence is a reachable stream conflict, so the scope is quarantined and reconciled fail-closed. If the retained evidence has been evicted and the Account checkpoint is unavailable or timed out, the affected scope remains quarantined and the operation returns retryable `AUTH_UNAVAILABLE`. If `outboxSequence == lastAppliedOutboxSequence + 1`, the consumer validates the full `authorityTuple` and `issuanceFence` against the matching Account evidence. An equal authority value is an idempotent accepted checkpoint and a higher value applies through set-if-greater before the watermark advances; a lower value is contradictory evidence that quarantines the scope and requires Account reconciliation before either value advances. If `outboxSequence > lastAppliedOutboxSequence + 1`, it is a gap only for that same `outboxStreamKey`: the consumer stops admitting or reconnecting affected bindings, reconciles the exact scope from Account, and advances the watermark only after the Account checkpoint proves the missing range is covered. A first event for a scope is not a false gap; the consumer first obtains the Account checkpoint and initializes that exact stream watermark. An event for an unrelated scope uses another stream key and another watermark and must not create a gap in this scope. Event delivery, projection application, and consumer convergence are part of the cutoff proof.
...
- Game Session maintains six physical active-binding index families: the tenant-scoped character uniqueness index, plus five cutoff/revocation families for issuer, account, tenant, caller-bound membership `{accountId, tenantId}`, and private-realm grant scope. The canonical issuer family is `session:game:index:issuer:{issuerIndexPartitionTag}:<issuerId>:<partitionId>`, a bounded Redis hash whose `<bindingRef>` field maps to `{schemaVersion: 1, issuerId, accountId, tenantId, gameInstanceId, sessionId, bindingGeneration, issuerAuthGeneration}`. `<bindingRef>` is the versioned length-prefixed encoding of the complete `{tenantId, gameInstanceId, sessionId}` binding identity, avoiding delimiter or textual-normalization ambiguity. `partitionId` is the deterministic hash of that exact encoded identity modulo the positive deployment `issuerIndexPartitionCount`, and `{issuerIndexPartitionTag}` is derived opaquely from `(issuerId, partitionId)`. A membership cutoff sweeps only the exact membership index rather than scanning every binding for the tenant. Issuer and other scope cutoffs perform the corresponding bounded index sweep, partitioned as needed within the existing revocation budget, and terminate every binding whose captured authority generation or grant version is below the committed cutoff. Revocation must not rely on Redis wildcard scans.
- Issuer-index writes and removals are single-key compare-and-sets against the complete expected binding reference, binding generation, and issuer generation. An identical value is an idempotent success; a stale or conflicting value creates a repair conflict. A tenant-local binding CAS records `issuerIndexState=REPAIR_REQUIRED` and keeps the binding non-admissible until the deterministic issuer repair obligation is durably acknowledged and a follow-up CAS records `issuerIndexState=ACKNOWLEDGED`. Cutoff reads exactly the configured issuer partitions, validates every returned entry, and fences or terminates each referenced binding through its ordinary binding CAS. Missing, unreadable, conflicting, or unresolved-repair partitions are unavailable or ambiguous coverage, never empty coverage. Recovery rebuilds partitions from the durable active-binding inventory with the same compare-and-set and clears the obligation only after all required partition acknowledgements are proven.
...
- Prove bounded per-sequence event ID/digest retention, duplicate verification after local evidence eviction, and fail-closed reconciliation when retained or authoritative evidence is unavailable.
