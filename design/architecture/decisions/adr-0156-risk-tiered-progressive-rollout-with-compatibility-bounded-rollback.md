# ADR 0156: Risk-Tiered Progressive Rollout With Compatibility-Bounded Rollback
...
This decision is not implemented. Risk-tier classification, automated observation and pause, bounded canaries, compatibility checks, and one-operator rollout playbooks remain gaps.
...
### Narrow Emergency Binary-Only Rollback Exception

When a live candidate is failing during an incident, an operator may use a narrowly bounded emergency restoration without waiting for a fresh verified backup point only when all of the ordinary `rollback-compatible` requirements above still hold and the operator has the exact predeclared known-good compatible digest set selected before the candidate was applied. The exception is limited to reapplying those exact binary digests. It must not rewind PostgreSQL, downgrade a schema, restore durable database state, rebuild or retag images, or change the release's compatibility classification.

This is a binary-only incident restoration, not an ordinary promotion, a new release authorization, or a first-live/reopen authorization. Player-facing traffic must remain closed or at its existing explicitly controlled posture. The operator must retain the incident record and complete restoration evidence, including the failing candidate digest set, the exact known-good digest set, compatibility evidence, actor, trigger, timestamps, action, and resulting health. Backup freshness must be repaired and re-verified before the next ordinary promotion or any first-live/reopen event.

This ADR owns the exception's rollout and rollback boundary. ADR 0153 owns the backup freshness objective and evidence that the exception temporarily does not wait for; the deployment runbook operationalizes this same exception and does not create a separate rollback authority.

Every rollout attempt has one durable `rolloutOperationId` bound to the exact candidate digest set, target environment, and observed Kubernetes Deployment identity. The operation records the Deployment UID and generation, with `resourceVersion` retained as an observed compare token where the adapter exposes it. Progression, pause, abort, and restoration requests must carry that exact tuple and re-read the active operation and Deployment immediately before mutation. Each mutation must submit the server-side `resourceVersion`, or an owner-equivalent server-side compare-and-set token when that is the adapter's concurrency primitive, as a mutation precondition; an observed token without that server-side precondition is insufficient. A candidate-digest, Deployment-identity/generation, operation-identity, or CAS-precondition mismatch is a stale watcher or executor: record the conflict and perform no mutation. An exact retry reuses the same operation identity; a new apply or re-apply starts a new operation and cannot consume the prior operation's restoration authority.
...
Proof must cover an unready candidate; a ready candidate that fails smoke; a latency or error-budget hard failure; a successful ordinary rollout; a compatible automatic restoration; refusal to restore when compatibility evidence is missing or stale; a stale watcher or executor whose candidate digest, Deployment identity/generation, operation identity, or server-side CAS precondition no longer matches; a `roll-forward-only` failure that pauses for forward remediation; operator-selected manual handling; high-risk canary failure and success; one-replica hobby deployment; and preservation of exact digest and deployment-evidence lineage throughout. The interleaving proof must run two progression, pause, abort, or restoration attempts against one prior server-side version: one succeeds and advances the resource version, while the other receives the stale conflict and performs no mutation; an exact retry then re-reads the new tuple and either succeeds under a fresh precondition or remains a no-op according to the durable outcome.
...
- [k8s/base/](../../../k8s/base/)
- [k8s/overlays/stage/](../../../k8s/overlays/stage/)
