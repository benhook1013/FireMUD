# ADR 0135: Social Relationship Authority and Entity-Owned Value
...
- its side of moderation integration under [ADR 0133](./adr-0133-owner-local-moderation-enforcement.md).
...
- `CHARACTER`, identified by `{playableStateNamespaceId, characterId}` within the group's tenant.
...
Attaching an item or currency to mail uses an owner-controlled transfer or escrow protocol. Entity atomically reserves or transfers item ownership and containment; the currency owner does the equivalent for currency. Social records only stable attachment or escrow references and mail lifecycle state. Sending, claiming, returning, expiring, retrying, or deleting mail must not duplicate, lose, or recreate value after an ambiguous handoff.

Ordinary account or social mail enters Social directly. A game may define world-specific delivery requirements, costs, interception, or other gameplay effects; those rules enter Game Logic under [ADR 0134](./adr-0134-explicit-communication-classes-and-owner-delivery.md), then hand an authorized semantic result to the applicable Social and value owners. Tenant-authored gameplay rules do not gain access to ordinary private platform mail.
...
- `design/architecture/microservices/social-groups-service/runtime-and-data.md`
