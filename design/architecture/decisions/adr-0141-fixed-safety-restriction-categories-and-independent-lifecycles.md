...
The player-facing response names the category that denies the attempted boundary and supplies its safe notice or next-step information. It does not expose internal evidence, moderator notes, hidden policy, or restrictions unrelated to the attempted action.
...
Implementation must replace generic action strings with one closed category enum and typed scope union at every ingress and owner contract. Existing development or test rows using legacy aliases must converge explicitly before the new path is claimed; runtime code must not retain an alias fallback. Each owner must persist its revision and current-state projection transactionally, implement owner-local indexed reads, and expose bounded operator reconciliation without giving Logging & Admin direct owner-table mutation.
...
Storage schemas, safe reason vocabularies, indexes, cache implementation, and operator UI may evolve while preserving the five fixed categories, explicit scopes, independent lifecycles, monotonic revisions, and ADR 0133 owner-local enforcement. Adding another category or scope class requires human review of its owner, enforcement boundary, stacking, notice, appeal, and failure semantics; implementations must not introduce one through a free-form action string.
