...
- Account Service owns authentication allowlist/session-auth prefixes (`session:auth:*`) and their lifecycle semantics, plus the already accepted `session:connect-token:*` issuance-result projection/cache defined by [ADR 0029](./adr-0029-single-use-gameplay-connect-token-carriage.md).
