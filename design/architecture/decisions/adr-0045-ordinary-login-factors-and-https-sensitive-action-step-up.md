...
Telnet or gameplay may explicitly initiate a sensitive action. FireMUD may then return a short-lived HTTPS URL containing a high-entropy, unguessable, single-use opaque handoff handle. The URL contains no independently authorizing credential; the handle is only a server-side lookup key for state bound to the initiating account, gameplay session, exact action scope, action, product, amount, currency, and `requestId`. Account-scoped actions persist `tenantId` as explicitly absent; tenant-scoped actions persist exactly one validated `tenantId`. At creation, Account also records the exact originating `effectivePolicyVersion` and policy digest resolved for that handoff's scope.

The HTTPS client authenticates the user over HTTPS before FireMUD discloses the bound action, product, amount, or currency details, and discloses the exact tenant only when the action is tenant-scoped. Before disclosure, before accepting recent reauthentication or step-up, before accepting provider-backed completion, and in the transaction that atomically consumes the handoff, Account re-resolves the exact current effective policy for the handoff scope and requires exact equality of both `effectivePolicyVersion` and policy digest with the values recorded at handoff creation. A policy advance therefore invalidates every unconsumed handoff issued under the prior policy, even if its ordinary lifetime has not elapsed. Callers must not satisfy this check from a cached or unversioned policy result. FireMUD then performs any required recent reauthentication, elevation, or payment-provider flow while preserving those bindings. It verifies the authenticated account and gameplay-session bindings, verifies the exact `tenantId` for tenant-scoped actions, and requires `tenantId` to remain absent for account-scoped actions, together with the `requestId` binding. For payment or other provider-backed actions, it completes the action asynchronously only after Account receives and verifies the corresponding provider webhook and the current-policy equality check succeeds; an authenticated HTTPS result or generic provider result alone is not completion authority. It atomically consumes the handoff only after the same equality check succeeds. Any later reuse, alteration, policy advance, or opening of an expired handoff is rejected; retries and duplicate callbacks may converge through the bound `requestId` idempotency record but cannot reopen the consumed handoff. The gameplay session may observe the eventual success, failure, expiry, or cancellation, but it cannot approve the action itself.
...
- a handoff handle is high-entropy, unguessable, server-side, opaque, non-authorizing, short-lived, single-use, bound to every declared field plus the originating `effectivePolicyVersion` and policy digest, and rejected if the exact current policy differs at disclosure, step-up, provider completion, or atomic consume; a policy advance invalidates the unconsumed handoff; account-scoped handoffs prove `tenantId` is absent in state, disclosure, and completion binding, while tenant-scoped handoffs prove the exact `tenantId` is present and consistent across all of them;
