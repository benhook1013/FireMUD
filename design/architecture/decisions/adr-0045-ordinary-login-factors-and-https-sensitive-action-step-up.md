...
Every listed HTTPS-sensitive action requires recent ordinary reauthentication using the account's enabled primary-login mechanism policy, including account deletion and global administration. If both `PASSWORD` and `EMAIL_OTP` are enabled, either one valid supplied secret satisfies this ordinary reauthentication; the request is not a sequential multi-factor exchange. Initial elevation to `platformAdmin`, or to `billingAdmin` authority that crosses tenant boundaries, additionally requires an independent TOTP. That TOTP step occurs once per bounded elevated window, not once per action, and never appears in gameplay. An allowlisted `platformAdmin_global` branch in `tenant_regular` requires this same ADR 0045 bounded elevated proof: recent ordinary reauthentication plus independent TOTP elevation, with the current control-plane token, account/issuer authority, explicit target tenant, live global role, and all target-tenant freshness checks still valid. Tenant-scoped `tenantAdmin` and `moderator` sensitive actions use the complete `tenant_regular` contract: the exact token/account/issuer checks, explicit target tenant, live target-tenant membership and role, caller-bound membership-generation freshness, and target-tenant-generation freshness. The receiver selects the global branch only from conclusive fresh global-role evidence; otherwise it selects the tenant-role branch only from conclusive global-role absence, and unavailable or inconclusive evidence denies without fallback. These tenant-role actions do not acquire or claim the global `privileged_control` window merely to make the shared operator-delegation protocol work.

Every `privileged_control` proof is an Account-persisted, single-purpose record bound to the exact `{accountId, issuerId, tokenJti, audience, role, targetTenantId, actionClass, privilegedControlVersion, issuedAt, expiresAt}` tuple. `targetTenantId` is exactly the requested tenant for a tenant-scoped or cross-tenant action and is explicitly absent for an action class with no target tenant; no UI selection, default, global role, or account-wide value supplies it. Account validates every persisted field against the current token, live role, exact request target, and route action class both when the proof is issued and on every use. A changed role, audience, target tenant, or action class requires a new proof and cannot broaden or reuse the existing one.
...
- every `privileged_control` proof persists and revalidates the exact token `jti`, issuer, audience, role, target tenant presence/value, action class, version, and bounded timestamps; focused proof rejects redemption of a proof under another role, audience, target tenant, or action class, including cross-role and cross-tenant reuse;
- a handoff handle is high-entropy, unguessable, server-side, opaque, non-authorizing, short-lived, single-use, bound to every declared field plus either the originating gameplay token `jti`/`tokenGeneration`/`issuanceFence` or, for Telnet/non-JWT, the server-issued exact `gameplayBindingId` and Account-issued `issuanceFence`, complete applicable authority tuple, security and policy versions, and applicable TOTP/elevation versions; JWT-only fields are explicitly absent on the Telnet branch. The selected identity and issuance fence are revalidated at disclosure, step-up, provider completion, atomic consume, cancellation, and replay/idempotency checks, so an authority or policy advance invalidates the unconsumed handoff; account-scoped handoffs prove `tenantId` is absent in state, disclosure, and completion binding, while tenant-scoped handoffs prove the exact `tenantId` is present and consistent across all of them;
