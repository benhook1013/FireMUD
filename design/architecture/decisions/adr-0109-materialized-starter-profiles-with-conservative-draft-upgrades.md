# ADR 0109: Materialized Starter Profiles with Conservative Draft Upgrades
...
The creator previews and resolves the complete plan before mutation. Game Design then applies it as one normal Draft commit guarded by the current Draft aggregate or scope epochs. Concurrent edits make the preview stale and require replanning; they are not silently overwritten. Normal cross-service Draft reconciliation, validation, digest gating, and publication apply. After publication, moving gameplay to the new game version uses the ordinary version migration, launch, playtest, and cutover contracts rather than any profile-specific runtime path.
