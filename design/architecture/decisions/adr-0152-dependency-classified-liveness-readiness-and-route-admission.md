# ADR 0152: Dependency-Classified Liveness, Readiness, and Route Admission
...
Each service evaluates the direct capabilities needed for the admission contract it owns. It does not recursively import every downstream service's complete readiness state or require every dependency to have all optional features healthy. An application-level response that proves the required downstream path is reachable may satisfy a side-effect-free canary even when the synthetic request is deliberately rejected.
...
- affected service health and admission documentation under `design/architecture/microservices/`
