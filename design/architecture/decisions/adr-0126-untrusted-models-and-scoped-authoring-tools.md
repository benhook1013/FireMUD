...
A mutation or compound application carries a stable request or proposal identity bound to its proposed input, target `tenantId`/`versionId`, exact `baseCommitId`, canonical revision order, the complete affected `(owner, aggregateId, scopeId, epoch)` set, and canonical digest; replay compares every binding field and rejects any changed or omitted field as changed-request reuse. Preview and acceptance refer to the same proposal identity and evaluate freshness over that complete set: a synchronized Draft change confined to disjoint scopes does not invalidate the proposal, while a change overlapping an affected scope or a required owner-derived containing scope advances the relevant fence and requires a fresh proposal and review. The immutable base commit remains provenance for the reviewed diff; no stale proposal is silently merged or applied.
