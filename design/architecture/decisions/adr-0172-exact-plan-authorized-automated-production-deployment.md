# ADR 0172: Exact-Plan-Authorized Automated Production Deployment
...
FireMUD describes production apply as operator-controlled, but current runbooks interpret that as an operator manually running `kubectl`, observing rollout, executing smoke checks, transcribing evidence, and deciding rollback. ADR 0151 already expects bounded automated observation and compatibility-approved restoration. Manual supervision makes the sole operator's workstation, command accuracy, and continued availability part of the production deployment critical path.
