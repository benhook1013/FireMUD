...
1. For the exact `(account_id, tenant_id)` pair, first classify the canonical rows. Exactly one valid canonical membership row is the sole relationship candidate and takes precedence over the legacy `accounts.tenant_id` value. Zero canonical rows proceeds to the legacy backfill step; duplicate or conflicting canonical rows cannot establish membership until reconciliation or quarantine completes, and the legacy value must never resolve that ambiguity. The legacy value is migration input only; other tenant-scoped rows must not synthesize a membership or select a default tenant.
2. A valid legacy pair with no canonical membership row is backfilled as exactly one canonical membership and receives durable source-to-target preservation evidence.
