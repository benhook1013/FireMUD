...
- Make `JOIN` the only open-enrollment creator of the durable tenant membership and prove the `(account_id, tenant_id)` uniqueness constraint, serialized membership-key protocol, canonical-row reuse for concurrent and exact retried joins, one logical join event, tenant binding, and audit behavior.
