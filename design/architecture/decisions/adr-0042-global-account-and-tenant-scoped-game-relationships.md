...
### Membership Reconciliation And Drop Gate
...
3. Exact duplicate canonical rows with identical relationship payload and lifecycle state retain the lowest canonical membership row ID as the survivor. Losing row IDs and the equality proof are recorded in immutable audit evidence; they are not silently deleted. A duplicate with any differing relationship, role, admission, lifecycle, or authority/version payload is a conflict, not a candidate for merging.
