...
- The canonical membership relation enforces exactly one row per `(account_id, tenant_id)` with a database unique constraint. Account serializes `JOIN` on that membership key; a concurrent or exact retried join reuses the canonical row and stored operation outcome rather than inserting another row or logical join event.
- Every `JOIN` carries a caller-stable operation ID (`requestId`) and a canonical digest over the operation kind, authenticated account, verified target, and other authority-bearing inputs. Account persists the operation ID, digest, lifecycle status, and terminal outcome in durable storage before returning a terminal result. An exact retry with the same ID and digest returns the stored outcome; reuse with a different digest is rejected as `IDEMPOTENCY_CONFLICT`; an unresolved or ambiguous operation remains pending and never becomes an implicit success.
- The membership transition and its durable audit/outbox event are committed atomically with the same persisted `JOIN` operation ID and digest. A retry therefore replays the one stored outcome and cannot create a second membership transition or logical event. The operation record, membership row, transition evidence, and event must be sufficient to reconcile a lost response without re-executing the join.
