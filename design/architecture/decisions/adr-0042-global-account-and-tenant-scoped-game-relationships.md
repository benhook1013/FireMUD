...
- The canonical membership relation enforces exactly one row per `(account_id, tenant_id)` with a database unique constraint. Account serializes `JOIN` on that membership key. An exact retry with the same request ID and digest replays that operation's stored outcome. A concurrent request with a different request ID or digest waits for the first membership-key transition, then records its own durable no-op outcome referencing the canonical membership; it does not replay another operation's stored result, insert another row, or emit another logical membership event.
- Every `JOIN` carries a caller-stable operation ID (`requestId`) and a canonical digest whose preimage includes `operationKind=JOIN`, the authenticated account, verified caller binding, verified target, and other authority-bearing inputs. Account persists those inputs with the operation ID, digest, lifecycle status, and terminal outcome in durable storage before returning a terminal result. An exact retry with the same ID and digest returns that operation's stored outcome; reuse with a different digest is rejected as `IDEMPOTENCY_CONFLICT`; an unresolved or ambiguous operation remains pending and never becomes an implicit success.
- The membership transition and its durable audit/outbox event are committed atomically with the same persisted `JOIN` operation ID and digest. An exact retry therefore replays that one stored outcome and cannot create a second membership transition or logical event; a concurrent distinct operation records its own no-op outcome after the canonical row is visible and also emits no transition event. The operation record, membership row, transition evidence, and event must be sufficient to reconcile a lost response without re-executing the join.
- Leaving is a distinct operation kind with its own stable operation ID, canonical digest, stored outcome, membership transition, and audit/outbox event. A later rejoin is a new `JOIN` operation and a distinct `INACTIVE` -> `ACTIVE` transition; it must not replay or mutate the earlier leave outcome. `JOIN` is not a general lifecycle repair and cannot restore any state other than `INACTIVE`.
...
- Immutable provenance/source identifiers are retained only under a documented audit/provenance exception. The exception must name the minimum identifier set, purpose, non-authoritative access controls, finite retention/deletion rule, and retention/deletion evidence; provenance is never an authority-bearing reference and tenant operators cannot use it to infer unrelated identity. Global account-deletion proof must reconcile every exception by proving source-identifier deletion or a still-valid, access-restricted exception with its eventual deletion evidence. Immutability is not permission to retain personal identifiers indefinitely.
...
3. Exact duplicate canonical rows with identical relationship payload and lifecycle state retain the lowest canonical membership row ID as the survivor. Before a losing row can be retired, one deterministic reconciliation operation acquires and holds a write fence, or equivalent locking protocol, over the duplicate group and every authority-bearing reference path so new authority-bearing references to the group are blocked. It locks existing dependent mutable authority-bearing rows, remaps their identities and foreign keys to the survivor, and records the durable old-to-survivor mapping, affected-row digest, and equality proof in immutable audit evidence. Immediately before atomic loser retirement, it rechecks the write fence, duplicate group, dependent-reference set, source digest, and equality proof; any change causes a retry or abort and never permits retirement on stale evidence. Immutable provenance rows remain non-authoritative and retain their original source identity only under the documented audit/provenance exception; the mapping is the durable bridge to the survivor and must not make provenance an authority-bearing reference. The remap and loser retirement commit in one transaction; retrying the same operation is idempotent and replays its committed result. A duplicate with any differing relationship, role, admission, lifecycle, or authority/version payload is a conflict, not a candidate for merging.
...
Dropping `accounts.tenant_id` is a separate gated step after code convergence. It is blocked while any source row lacks a preservation disposition, any valid legacy pair lacks exactly one canonical membership, any duplicate/conflict quarantine remains unresolved, any mutable authority-bearing dependent identity or foreign key still points at a retiring loser, or the migration cannot prove the complete source-to-target audit set. Immutable provenance and source-history rows may retain the losing source identity only under the documented audit/provenance exception, with non-authoritative access controls, finite retention/deletion evidence, and reconciliation to global account-deletion proof; otherwise the source identifier must be erased or irreversibly redacted under the applicable deletion contract. Such rows are not authority and are not drop blockers. The deployed-code scan must persist a release-versioned no-reader attestation naming the exact release/artifact identity and schema target, together with the scan/tool version, covered entity/DTO/repository/authentication/fixture/test surfaces, evidence digest, and completion time. The drop gate is also a live-version-convergence gate: that attestation must prove that the exact attested release/schema target is live and that every older reader version is drained or blocked from serving the pre-drop schema. Deployment preflight validates the no-reader and live-version-convergence attestation together with the complete preservation evidence and rejects a missing, stale, mismatched, incomplete, or differently versioned result. Only those durable gates permit the separate forward Flyway migration to drop the column; the migration must atomically revalidate the preservation, no-reader, and live-version-convergence gates for the exact same release/schema target in the same release operation and fail closed rather than remove `accounts.tenant_id` when any gate is absent, stale, mismatched, or unproven.

- Add a forward preservation migration or equivalent durable migration phase, without editing any applied migration, that first snapshots, classifies, and validates every legacy `accounts.tenant_id` relationship, backfills only rows classified as valid and non-conflicting into exactly one canonical membership row, and records row-level preservation evidence without removing the legacy column. Matching canonical rows remain the sole relationship; legacy tenant mismatches, duplicate canonical rows, and invalid or conflicting source data follow the reconciliation, quarantine, and drop-gate rules above. No relationship may be silently omitted, duplicated, or assigned a default tenant. Deploy the converged schema readers, entities, DTOs, repositories, authentication flows, fixtures, and tests, remove every fallback that derives tenant authority from the account row, and persist the release-versioned no-reader/live-version-convergence attestation naming the exact release/schema target and proving older readers are drained or blocked. Finally, run a separate forward Flyway drop migration only after deployment preflight validates that attestation and a complete preservation proof pass; that migration must atomically verify all durable gates against the same release/schema target in the same release operation before dropping `accounts.tenant_id`, and fail closed if any gate is absent or mismatched.
...
- Make `JOIN` the only open-enrollment creator of the durable tenant membership and prove the `(account_id, tenant_id)` uniqueness constraint, serialized membership-key protocol, durable operation ID/digest/status/outcome persistence, digest-mismatch rejection, atomic binding of the membership transition and one logical audit/outbox event to that same operation, canonical-row reuse for concurrent and exact retried joins, tenant binding, and audit behavior. Keep `JOIN_REQUIRED` on pre-join admission surfaces rather than treating it as a `JOIN` writer failure.
