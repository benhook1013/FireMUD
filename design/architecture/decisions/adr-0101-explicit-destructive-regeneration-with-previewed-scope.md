# ADR 0101: Explicit Destructive Regeneration with Previewed Scope
...
Historical replay preservation and preview-bound destructive regeneration are target state. Current World Management and Game Design paths prove scope epochs and bounded replacement policies but do not yet implement or prove exact destructive previews, plan-digest binding, reference analysis, or identity mappings.

## Canonical Design

- [Procedural Generation](../system-architecture-procedural-generation.md#request-bounded-replay-and-explicit-regeneration)
- [World Management Procedural Generation Control](../microservices/world-management-service/procedural-generation-control.md#destructive-regeneration-plans)
- [Game Design World Editing Tools](../microservices/game-design-service/world-editing-tools.md)
...
Before accepting `REPLACE_SCOPE`, FireMUD produces an exact destructive plan for the declared scope. The preview identifies at least the rows and logical objects to create, retain, replace, and delete; affected references; identifier mappings; and blocking validation failures. The approved request carries the `generationRequestId`, immutable generator or model implementation identity, exact canonical generation inputs, and a canonical generated-result/plan digest that binds the resolved output to the previewed destructive plan, current Draft scope epoch, and reference facts. Apply rejects the operation if the request identity, implementation identity, inputs, digest, resolved output, scope epoch, or reference facts differ from the approved preview; a changed plan must be regenerated and approved again.
