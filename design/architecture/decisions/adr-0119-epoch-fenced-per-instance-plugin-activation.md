...
The current V3 implementation persists `pluginActivationEpoch` and `lifecycleRevision` on runtime state and plugin-backed ingress, work, audit, schedule, and handoff rows. The local execution path validates complete captured/current plugin state, version, epoch, and revision before quota/DSL evaluation and every gameplay handoff, and persists tuple-wide lifecycle request history including the inactive `DRAIN` `FAILED_PRECONDITION` receipt. Pending-transition/admission-barrier ownership, Game Session fence-install acknowledgement/projection, final Game Session effect enforcement, replay/recovery, remote handoff, policy coordination, and complete cleanup remain implementation and proof gaps, not relaxed runtime-fencing behavior.
