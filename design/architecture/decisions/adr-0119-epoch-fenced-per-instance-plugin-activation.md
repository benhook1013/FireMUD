...
Current implementation has instance-scoped plugin state, publication and compatibility preflight, policy reconciliation, ingress version checks, plugin identity and the captured `(pluginActivationEpoch, lifecycleRevision)` on plugin-backed work, audit, schedule, and handoff rows, schedule filtering, tuple-wide lifecycle request history, and cleanup surfaces. The local execution path enforces complete captured/current plugin state, version, epoch, and revision before quota/DSL evaluation and every gameplay handoff. Pending-transition/admission-barrier ownership, Game Session fence-install acknowledgement/projection, exact lifecycle enforcement at final Game Session execution, replay/recovery, remote handoff, policy coordination, and complete lifecycle cleanup remain implementation and proof gaps.
