...
The current V3 implementation persists `pluginActivationEpoch` and `lifecycleRevision` on runtime state and plugin-backed ingress, work, audit, schedule, and handoff rows. The local execution path validates captured/current plugin state, version, epoch, and revision before quota/DSL evaluation and gameplay handoff, and persists tuple-wide lifecycle request history including the inactive `DRAIN` `FAILED_PRECONDITION` receipt. A timer due candidate snapshots `pluginId`, `pluginVersionId`, applicable `bindingId`, `pluginActivationEpoch`, and `lifecycleRevision`, then rechecks that snapshot against the schedule row and local plugin-owner state before emission. This candidate check does not yet establish durable winning-admission CAS/barrier ordering, a retained firing claim, or the final Game Session effect fence, so it is not complete timer lifecycle-tuple validation. Pending-transition/admission-barrier ownership, Game Session fence-install acknowledgement/projection, final Game Session effect enforcement, replay/recovery, remote handoff, policy coordination, and complete cleanup remain implementation and proof gaps, not relaxed runtime-fencing behavior.
...
Current implementation has instance-scoped plugin state, publication and compatibility preflight, policy reconciliation, ingress version and executable-state checks, plugin identity and the captured `(pluginActivationEpoch, lifecycleRevision)` on plugin-backed work, audit, schedule, and handoff rows, schedule filtering, and tuple-wide lifecycle request history for state changes, successful no-ops, and deterministic inactive-drain failure. V3 gives retained runtime rows with an active version one positive founding pair, propagates it only to schedules with an exact tenant/instance/plugin/version/runtime-region match, rejects malformed current pairs at the schema and repository boundaries, and leaves historical work at `0/0` because its winning admission fence cannot be reconstructed. Timer due candidates snapshot `pluginId`, `pluginVersionId`, applicable `bindingId`, `pluginActivationEpoch`, and `lifecycleRevision`, then recheck the snapshot against the schedule row and local plugin-owner state before emission. Durable winning-admission CAS/barrier ordering, retained firing claims, and the final Game Session effect fence remain gaps, so this does not establish complete timer lifecycle-tuple validation. The local execution path enforces captured/current plugin state, version, epoch, and revision before quota/DSL evaluation and each gameplay handoff. Pending-transition/admission-barrier ownership, Game Session fence-install acknowledgement/projection, exact lifecycle enforcement at final Game Session execution, replay/recovery, remote handoff, policy coordination, and complete lifecycle cleanup remain implementation and proof gaps.
