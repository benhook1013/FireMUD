...
For target-state `COALESCE_ONE` recovery, Automation owns one durable timer-recovery resume-window record per playable-state namespace/runtime scope, epoch, and `isDryRun` mode. Its `resumeWindowId` is the exact tuple `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch, isDryRun, resumeGeneration>`. A durable compare-and-set creates or returns the one `OPEN` window for that namespace/runtime scope, epoch, and mode; the server-derived `playableStateScope` is retained and exact-validated as immutable policy/routing/authorization/fence evidence, not as a uniqueness input. An authoritative scope transition starts a new playable-state lifecycle and namespace; stale presented scope with unchanged authority may reuse the existing identity. Leader, worker, takeover, and attempt IDs never participate in the identity. At creation, the durable record binds the exact `(scriptPatchVersion, scriptPinEpoch)` tuple as immutable execution-fence evidence without adding it to `resumeWindowId`, freezes the effective numeric tenant-local cap and the selection-policy revision, and persists a frozen pre-window `recoveryStart` watermark plus a monotonic `lastObservedThrough` watermark for each applicable clock domain. Each observation supplies an `observedThrough` watermark, which is durably recorded as the new `lastObservedThrough` only with settlement under that `resumeWindowId`. `recoveryStart` is the exclusive lower bound for the first scan, the prior `lastObservedThrough` is exclusive for every later scan, and the current `observedThrough` is inclusive, so the ranges are `(recoveryStart, observedThrough]` initially and `(lastObservedThrough, observedThrough]` thereafter. A candidate exactly at the inclusive through point is eligible once; a candidate at an exclusive lower bound is not rediscovered. Same-window retries, observations, and takeovers re-read and compare those persisted watermarks and reuse the record, its mode-specific `resumeWindowId`, and its frozen cap and selection-policy revision unchanged; each `CATCH_UP` firing claim and candidate audit includes that ID in uniqueness and comparison. A cap or selection-policy change applies only to a later window allocated after this one reaches `COMPLETE`. The current window becomes `COMPLETE` only after the selected candidates' firing or shared-admission-denial outcomes and all cap-excluded candidate outcomes governed by its frozen values are durable. Only a later recovery episode after completion allocates the next `resumeGeneration`, producing a distinct mode-specific `resumeWindowId`.
...
Proof must cover mandatory clock-unit and recovery-class declarations; wall-clock stability across tick-cadence changes; tick/game-time cadence under changed real-world tick duration; explicit time-scale behavior; durable one-shot reconstruction with at-least-once replay-safe physical execution, one logical terminal outcome, and no duplicate logical effect; old-scope/epoch one-shot reconciliation without rebinding or unproven new lineage; duplicate recovery; `SKIP_MISSED`; one `COALESCE_ONE` firing per complete stable schedule-instance identity and mode-specific durable resume-window identity; exact script-pin selection changes, including same-patch/new-`scriptPinEpoch` repin, moving prior mode-specific `OPEN` windows out of `OPEN` before target admission, allocating the next generation, and preserving old-window fencing and settlement; persisted `recoveryStart`, `lastObservedThrough`, and `observedThrough` watermark progression anchored to the same durable `resumeWindowId`, including exclusive lower and inclusive upper boundaries and repeated exact-boundary observations; the tenant-local cap within each tenant/mode resume window; deterministic fair selection across complete stable schedule-instance identities within that window; audited exclusions; shared scheduler/cluster admission under [ADR 0166](./adr-0166-attributable-script-breakers-and-tenant-first-fairness.md) tenant-first fairness and per-tenant or cluster ceilings; a tenant/cluster candidate-level capacity denial consuming the selected catch-up slot and terminalizing without a firing claim, retry, deferral, backfill, or remint while leaving future ordinary cadence eligible; later handler-scoped per-script and materialized sandbox/dry-run capacity denials following Table 3 under their established identity without catch-up reminting; no burst or deferred missed-firing backlog; deterministic elapsed-time aggregation; corrected exact-boundary resume mathematics; distinction between reload preservation and downtime recovery; Redis loss and index rebuild; and player/operator-visible skipped and terminal outcomes.
