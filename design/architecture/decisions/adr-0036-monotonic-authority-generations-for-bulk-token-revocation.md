...
`AuthLogout` and `AuthLogoutAll` have a narrow retry branch that returns a lifecycle result and never creates authorization context. On the initial authorization attempt, or when no exact durable retry evidence matches, local signature/profile/time/subject validation is followed by exactly one bounded lookup of the presented token's exact issued-token registry record and exactly one bounded read of the `account-auth-evidence-bundle/v1` before Account authorizes and records the logout operation; the initial path does not assemble authority from separate reads, cached values, Redis, or the token alone. `AuthLogout` may replay only a stored lifecycle result from a durable Account pending/committed intent or tombstone whose original `requestId`, `requestDigest`, operation kind, and exact token identity all match. `AuthLogoutAll` may replay only a stored lifecycle result from durable evidence whose original `requestId`, `requestDigest`, operation kind, and presented token identity match the prior logout-all that superseded the authority.

After that local validation, only an exact stored-result/tombstone retry whose request identity, request digest, operation kind, and exact token identity match reads the durable Account operation result or tombstone needed to prove that exact lifecycle outcome instead of performing either initial lookup. It is exempt from both the token-registry lookup and `account-auth-evidence-bundle/v1` read, and an unavailable or non-matching stored result does not become authorization. The retry returns only the stored lifecycle result: no JWT, role, tenant, generation, or other auth context; its result cannot be reused by another route or authorize any operation. A request without an exact matching durable result follows the initial path and therefore performs exactly one token-registry lookup and exactly one bundle read rather than being treated as a retry.
...
`cross_tenant_data_bearing` is not in the omission allowlist. A `platformAdmin` request in that class must bind and validate the current target-tenant generation just like a target-specific `tenant_regular` operation. The separate no-target classification set is closed: `public`, `pre_tenant_discovery`, `account_scoped`, `pending_deletion_scoped`, and targetless `internal_workload` have no target tenant; `public_production_onboarding`, `caller_membership_scoped`, and `gameplay_admission` are separately named target-context classifications that use their own membership, grant, admission, routing, pointer, or binding fences. None of these classifications is a member of the tenant-bearing omission allowlist, and no tenant-bearing route may omit target generation because a route declaration, role name, service name, internal caller, operator reference, or “cross-tenant” label says it has no target-tenant operational authority. A newly introduced classification requires tenant generation unless a future architecture decision explicitly names it as a separate no-target classification.

Every other tenant-bearing classification, including a newly introduced classification, requires tenant generation according to its route declaration. The allowlist is not inherited by route variants, internal callers, or operator references. Negative proof is performed separately for each route classification, not only for a shared middleware path: it must show that a tenant-generation advance denies `tenant_regular` and `cross_tenant_data_bearing`, while each allowlisted class is denied when its own issuer/account, membership, live role, exact target-tenant binding, global scope, or required assurance predicate is absent, stale, mismatched, or unavailable. `gameplay_admission` proof instead advances or invalidates each branch-specific membership, grant, admission-lease, binding, routing, or pointer authority and proves that the affected admission or active binding is denied or terminated within its declared bound; it is a named target-context classification, not a generic tenant-generation omission. The proof must respect intentionally omitted predicates: `billing_safe_tenant` omits only target-tenant generation while requiring exact `membershipVersion` alongside caller membership generation and live `current_operator_roles`, and rejects missing, stale, or unavailable version evidence; `cross_tenant_support_safe` omits membership, target-tenant generation, and `privileged_control` on its support branch; `cross_tenant_billing_safe` omits membership and target-tenant generation but requires `privileged_control`; and `pending_deletion_scoped` uses its separate no-target credential contract rather than inheriting this allowlist. An intentionally omitted predicate is not converted into a required check or a hidden authority source, while any predicate declared applicable by the classification remains mandatory and no new classification may create an omission.
...
- Prove the closed route-class omission set independently per classification: tenant-generation advances deny every ordinary target-tenant-authority route outside the exact three-class allowlist; the explicit `tenant_regular` `platformAdmin` branch requires the route-declared `control-ui` profile, current issuer/account/global role, exact target scope, target-tenant generation, and active `privileged_control` while requiring no membership, and tenant-role branches require live membership, membership generation, tenant-role, and fresh target-tenant-generation checks; `billing_safe_tenant` still requires issuer, account, exact membership generation and `membershipVersion`, exact tenant binding, and the canonical live `current_operator_roles` check proving `tenantAdmin`, rejecting missing, stale, or unavailable version evidence while intentionally omitting only target-tenant generation; support-safe routes reject missing issuer/account/current global role and reject support's use of billing or data-bearing routes without adding `privileged_control` to the support branch; billing-safe cross-tenant routes reject missing issuer/account/global billing role, wrong target scope, and missing `privileged_control` while intentionally omitting membership and target-tenant generation; the named no-target and target-context classifications are proven separately and cannot be inherited by a tenant-bearing route; `pending_deletion_scoped` is separately proven as a no-target classification and is not treated as an allowlist member; and `gameplay_admission` is proven target-tenant-generation-free while each selected admission branch rejects stale membership, grant, admission, routing, pointer, or binding authority. Add negative tests proving a newly named class or route cannot inherit an allowlist entry, proving that intentionally omitted predicates do not become implicit requirements, and proving that initial `AuthLogout`/`AuthLogoutAll` authorization performs exactly one token-registry lookup and exactly one evidence-bundle read while only locally validated stored-result/tombstone retries with exact request identity, digest, operation kind, and token identity perform no new registry or bundle read and create no authorization context.
