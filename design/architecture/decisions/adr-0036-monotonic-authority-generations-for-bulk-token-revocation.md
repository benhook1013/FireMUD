...
The route entry must also declare `membership_authority_generation_applies: conditional_by_operator_role` with `membership_authority_generation_condition: {tenant_role: true, platformAdmin_global: false}`, `global_platform_admin_membership_required: false`, and a role set that explicitly includes `platformAdmin`. Its required live checks must include `current_operator_roles`, `current_global_role`, `role_appropriate_assurance`, and `target_tenant_generation` (plus any route-specific domain checks). The tenant-role branch uses the same route entry only when its `tenant_role` predicate is true and must instead satisfy live membership and membership-generation checks.
