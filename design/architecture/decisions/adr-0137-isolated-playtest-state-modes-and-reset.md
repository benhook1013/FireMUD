...
Snapshot preparation records the exact source tenant, realm, `gameInstanceId`, `playableStateNamespaceId`, runtime version, optional script patch, and participant snapshot evidence. Every required owning service must return a mode-specific manifest. For `selected-roster`, each data-family entry must state whether the family is necessary for the represented behavior, whether its inclusion is authorized by the selected roster and scope, and its redaction classification (`unredacted`, `redacted`, or `excluded`); a necessary family that is not authorized, or an unauthorized inclusion, rejects preparation. Owner manifests also record included families, scope, counts or equivalent completeness evidence, source/build identity, and digest or immutable snapshot identity.
...
Reset is generation replacement, not an in-place destructive rewrite. It retains the `playtestLifecycleId`, prepares a fresh `playableStateNamespaceId` from the selected `fresh`, `seeded`, or `snapshot` input, advances to a fresh playtest-state generation, validates the same completeness and isolation rules, and then moves the playtest realm's admission pointer to a runtime bound to that new namespace. Before that pointer CAS, the existing owner-consistent cutover/reset boundary must durably acknowledge each owner's flush/drain and classify and read back the outcome as `APPLIED`, `NOT_APPLIED`, or unknown, using the existing [cutover hold and readback vocabulary](./adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md). Only an aggregate of `APPLIED` for every required owner permits admission-pointer advancement; any mix of `APPLIED` and `NOT_APPLIED`, unknown outcome, or contradictory lifecycle/generation evidence produces `RECONCILIATION_REQUIRED` and keeps admission closed. If the pointer CAS fails after any owner has reported `APPLIED`, the old generation cannot reopen until [ADR 0122](./adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md) authoritative reconciliation proves the pointer, fence, and owner outcomes; no generic compensation is inferred. This is coordinated owner evidence and readback, not a cross-owner atomic transaction. The admission-pointer CAS and the old-generation write fence remain one atomic owner transaction/critical section: every incoming write must validate the current generation and pointer fence, so no old-generation write can commit in the interval between the pointer move and fencing. The old generation is fenced from new admission and retired through the bounded lifecycle and retention contract. Failure before the pointer move leaves the old generation authoritative and admissible according to its prior state unless the aggregate outcome is unknown or contradictory; it does not expose a partially reset namespace.
