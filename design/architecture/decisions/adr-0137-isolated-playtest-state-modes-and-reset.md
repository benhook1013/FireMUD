...
Every new playtest lifecycle receives a fresh immutable `playtestLifecycleId`, a fresh `playableStateNamespaceId` under [ADR 0122](./adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md), and an initial monotonic playtest-state generation. The lifecycle ID identifies one lifecycle for the exact tenant/world/realm target and is never reused for another lifecycle. Replacing a runtime instance within that lifecycle retains the lifecycle ID and namespace; creating another playtest lifecycle does not reuse either. The selected runtime `gameInstanceId` remains the execution and routing identity, not the durable-state key.
...
Every preparation records the exact target tenant/world/realm, fresh target `playableStateNamespaceId`, initialization mode, target runtime version, optional script patch, and participant evidence. Snapshot preparation additionally records the exact source tenant/world/realm, source `gameInstanceId`, source `playableStateNamespaceId`, and shared `sourceSnapshotFence`. Every required owning service must return a mode-specific manifest. For `selected-roster`, each data-family entry must state whether the family is necessary for the represented behavior, whether its inclusion is authorized by the selected roster and scope, and its redaction classification (`unredacted`, `redacted`, or `excluded`); a necessary family that is not authorized, or an unauthorized inclusion, rejects preparation. Owner manifests also record included families, scope, counts or equivalent completeness evidence, target namespace/build identity, and the mode-appropriate provenance and digest evidence defined below.
...
Reset is generation replacement, not an in-place destructive rewrite. It retains the `playtestLifecycleId`, prepares a fresh `playableStateNamespaceId` from the selected `fresh`, `seeded`, or `snapshot` input, advances to a fresh playtest-state generation, validates the same completeness and isolation rules, and then moves the playtest realm's admission pointer to a runtime bound to that new namespace. Before that pointer CAS, each owner performs the owner-local fence and flush/drain, durably acknowledges the result, and classifies and reads back the outcome as `APPLIED`, `NOT_APPLIED`, or unknown, using the existing [cutover hold and readback vocabulary](./adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md). The reset pointer operation binds the exact current `playtestLifecycleId`, generation, namespace, pointer fence/version, and ADR 0122 hold identity/fence into the authoritative pointer/CAS/readback record; rollback, abort, or reopen must reread and exact-match that binding before any state transition. Missing, stale, mismatched, contradictory, or unavailable pointer or hold evidence keeps admission closed in `RECONCILIATION_REQUIRED`. The coordinator aggregates those outcomes before Game Session performs the admission-pointer CAS: only an aggregate of `APPLIED` for every required owner permits advancement; any mix of `APPLIED` and `NOT_APPLIED`, unknown outcome, or contradictory lifecycle/generation evidence produces `RECONCILIATION_REQUIRED` and keeps admission closed. Every incoming write must validate the current generation and pointer fence at its owner boundary, so stale generation or pointer-fence evidence is rejected throughout the transition. This is coordinated owner-local evidence and readback followed by a Game Session pointer CAS, not a cross-owner atomic transaction. If the pointer CAS fails after any owner has reported `APPLIED`, the old generation cannot reopen until [ADR 0122](./adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md) authoritative reconciliation proves the current pointer, hold, fence, and owner outcomes; no generic compensation is inferred. The old generation is fenced from new admission and retired through the bounded lifecycle and retention contract. Failure before the pointer move leaves the old generation authoritative and admissible according to its prior state only after every required owner has durably cleared its reset fence and returned a known, non-contradictory `NOT_APPLIED` unfence readback; missing, unknown, mixed, or contradictory evidence keeps admission closed pending [ADR 0122](./adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md) authoritative reconciliation. It does not expose a partially reset namespace.
