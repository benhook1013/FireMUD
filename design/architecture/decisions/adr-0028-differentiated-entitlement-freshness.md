...
- Define `freshUntil = evaluatedAt + 15 seconds` and `continuityUntil = evaluatedAt + 5 minutes`. A snapshot is fresh only while the skew-adjusted age from the predicate above is strictly below 15 seconds; at the exact boundary it is stale. An eligible last-known-good snapshot may authorize continuity only while that skew-adjusted age is strictly below five minutes; at the exact boundary it expires. A future `evaluatedAt` beyond `maxClockSkew` is invalid and fails closed; one within the bound is accepted only through the same conservative skew-adjusted predicate and never widens either window.
