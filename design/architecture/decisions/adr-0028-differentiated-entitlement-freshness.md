...
- `evaluatedAt` represents when Account evaluated authoritative committed inputs, stamped from its deployment-synchronized UTC clock. Runtime receipt time, Redis time, and caller restamping never make an older snapshot fresh. Consumers use one common predicate for every finite limit `L`: `maxClockSkew < L`, `evaluatedAt <= localNow + maxClockSkew`, and `localNow < evaluatedAt + L - maxClockSkew`. This is equivalent to `conservativeAge = max(0, localNow - evaluatedAt) + maxClockSkew < L`, but the deadline form is the canonical `evaluatedAt + L - maxClockSkew` boundary. A limit is unusable when `maxClockSkew >= L`; a future timestamp beyond the skew bound is invalid, and a timestamp within the bound never widens the window. Exact boundaries remain expired; no caller may widen a freshness window to compensate for clock uncertainty.
...
- Define `freshUntil = evaluatedAt + 15 seconds - maxClockSkew` and `continuityUntil = evaluatedAt + 5 minutes - maxClockSkew`. A snapshot is fresh only when the common consumer predicate with `L=15 seconds` holds, and an eligible last-known-good snapshot may authorize continuity only when it holds with `L=5 minutes`; both limits are unusable when `maxClockSkew` is greater than or equal to the applicable limit. Exact deadlines are stale/expired. A future `evaluatedAt` beyond `maxClockSkew` is invalid and fails closed; one within the bound is accepted only through the common predicate and never widens either window.
