...
The target-state architecture requires Gateway to route `/ws/game/**` WebSocket connections to the Game Session shard that owns the `<tenantId, gameInstanceId, regionId>` lease. Multiple documents describe:
