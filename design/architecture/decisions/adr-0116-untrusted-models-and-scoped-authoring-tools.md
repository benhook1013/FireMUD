# ADR 0116: Untrusted Models and Scoped Authoring Tools
...
A mutation or compound application carries a stable request identity bound to its proposed input, an exact Draft or scope epoch, and sufficient digest identity to detect changed-request reuse. Preview and acceptance refer to the same proposal identity. If the Draft advances after preview, application rejects the stale base and requires an updated proposal or review rather than merging silently.
