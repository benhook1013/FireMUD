...
- A closure or replacement cutover that leaves source sessions draining also creates a durable drain-enforcement work item in that transaction. The work item records the source drain identity, source instance, absolute deadline, and desired lifecycle actions. The same commit publishes an inactive source-instance command-fence identity carrying that drain identity and deadline; existing source commands remain usable before `sourceDrainDeadlineAt`. At the deadline, or immediately for zero duration, the retryable drain reconciler activates and confirms the lifecycle fence, after which command handling rejects source commands without consulting the admission-pointer database. The retryable claim fence, per-effect replay identities, readback, crash recovery, and deadline ordering are canonical in [Bounded Source-Drain Reconciliation](../system-architecture-versioning-runtime.md#bounded-source-drain-reconciliation). A crash after the pointer commit and before the first durable work-item claim therefore leaves the persisted deadline and inactive fence identity available to startup or retry reconciliation; a committed pointer or drain record is not evidence that deadline enforcement or any runtime effect already occurred.
- A replacement instance remains non-admissible until preparation and compatibility checks pass. Cutover atomically moves `OPEN(source)` to `OPEN(target)`; failure leaves the source as the sole target. Stopping a realm without a replacement first moves it to `CLOSED` and persists the same source drain identity, source instance, absolute deadline, and lifecycle state before the old runtime drains; it does not use an untracked shutdown path. A zero-duration policy makes the source command fence effective immediately at that closure/cutover commit and makes the same durable work item immediately due; the retryable reconciler retains ownership of notice delivery, socket closure, authoritative readback, and idempotent `InstanceTermination` rather than bypassing that work item.
...
- Existing connected sessions may continue on the source only before the persisted deadline. The source may terminate early when no sessions remain. At `sourceDrainDeadlineAt`, the retryable reconciler activates the persisted source-instance fence and confirms its readback before command handling rejects subsequent source commands. It then performs one bounded update-notice attempt, closes remaining source sockets after that attempt completes or times out, and hands off idempotent `InstanceTermination`; notice failure never holds sockets open indefinitely. The reconciler owns those effects and their retries, while the command path performs no drain side effects beyond enforcing the activated fence.
...
- Atomically persist pointer state, audit, request replay, the source command fence, and the applicable drain-enforcement work item in one transaction, plus prepared-cutover execution for a replacement cutover that changes `gameInstanceId`, including crash/retry proof from pointer commit through the first durable work-item claim. Prove that command rejection is immediate at a zero-duration deadline without a pointer-database lookup, while reconciler and startup recovery remain idempotent for notice delivery, socket closure, authoritative readback, and terminal `InstanceTermination`. If those records cannot share a transaction in a future topology, stop and obtain a new accepted decision before implementing or claiming the split-store cutover contract.
...
- Persist and prove the resolved drain policy, unique drain identity, absolute deadline, committed source command fence, early-empty completion, deadline enforcement, socket closure, authoritative readback, command rejection, and idempotent source termination.
