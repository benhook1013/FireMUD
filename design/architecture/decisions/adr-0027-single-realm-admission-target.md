...
- A closure or replacement cutover that leaves source sessions draining also creates a durable drain-enforcement work item in that transaction. The work item records the source drain identity, source instance, absolute deadline, and desired lifecycle actions. The same commit publishes a deadline-armed source-instance command-fence identity carrying that drain identity, `sourceDrainDeadlineAt`, and `effectiveAt=sourceDrainDeadlineAt`; existing source commands remain usable before that time. The command path evaluates the persisted fence against the authoritative clock and rejects source commands at or after `effectiveAt` without consulting the admission-pointer database, even if reconciliation lags. For zero duration, `effectiveAt` is the cutover commit, so rejection is effective at commit. The retryable drain reconciler records and reads back the fence-activation and lifecycle effects, then performs the notice, socket-close, and termination work. The retryable claim fence, per-effect replay identities, readback, crash recovery, and deadline ordering are canonical in [Bounded Source-Drain Reconciliation](../system-architecture-versioning-runtime.md#bounded-source-drain-reconciliation). A crash after the pointer commit and before the first durable work-item claim therefore leaves the persisted deadline-armed fence available to startup or retry reconciliation; a committed pointer or drain record is not evidence that lifecycle or other runtime effects already occurred.
- A replacement instance remains non-admissible until preparation and compatibility checks pass. Cutover atomically moves `OPEN(source)` to `OPEN(target)`; failure leaves the source as the sole target. Stopping a realm without a replacement first moves it to `CLOSED` and persists the same source drain identity, source instance, absolute deadline, and lifecycle state before the old runtime drains; it does not use an untracked shutdown path. A zero-duration policy sets the source command fence's `effectiveAt` to the closure/cutover commit and makes the same durable work item immediately due; the retryable reconciler retains ownership of recording/read-back, notice delivery, socket closure, and idempotent `InstanceTermination` rather than bypassing that work item.
...
- Existing connected sessions may continue on the source only before the persisted deadline. The source may terminate early when no sessions remain. At or after `sourceDrainDeadlineAt`, the command path rejects source commands from the deadline-armed fence using the authoritative clock, even if reconciliation has not yet recorded or read back its activation. The reconciler records and reads back the fence-activation and lifecycle effects, then performs one bounded update-notice attempt, closes remaining source sockets after that attempt completes or times out, and hands off idempotent `InstanceTermination`; notice failure never holds sockets open indefinitely. The reconciler owns those effects and their retries, while command rejection does not wait for reconciler progress.
...
- Atomically persist pointer state, audit, request replay, the deadline-armed source command fence with its `effectiveAt`, and the applicable drain-enforcement work item in one transaction, plus prepared-cutover execution for a replacement cutover that changes `gameInstanceId`, including crash/retry proof from pointer commit through the first durable work-item claim. Prove that command rejection occurs at or after the persisted deadline from the authoritative clock without a pointer-database lookup or timely reconciliation, including effective-at-commit rejection for zero duration, while reconciler and startup recovery remain idempotent for fence/lifecycle readback, notice delivery, socket closure, and terminal `InstanceTermination`. If those records cannot share a transaction in a future topology, stop and obtain a new accepted decision before implementing or claiming the split-store cutover contract.
...
- Persist and prove the resolved drain policy, unique drain identity, absolute deadline, deadline-armed source command fence with `effectiveAt`, early-empty completion, authoritative-clock command rejection, fence/lifecycle readback, socket closure, and idempotent source termination.
