...
- A closure or replacement cutover that leaves source sessions draining also creates a durable drain-enforcement work item in that transaction. The work item records the source drain identity, source instance, absolute deadline, and desired lifecycle actions. The same commit publishes the source-instance command fence, which carries that identity and deadline and rejects source commands at or after `sourceDrainDeadlineAt` without a pointer-database lookup. The retryable claim fence, per-effect replay identities, readback, crash recovery, and deadline ordering are canonical in [Bounded Source-Drain Reconciliation](../system-architecture-versioning-runtime.md#bounded-source-drain-reconciliation). A crash after the pointer commit and before the first durable work-item claim therefore leaves command enforcement authoritative while startup or retry reconciliation discovers and claims the work item; a committed pointer or drain record is not evidence that the reconciler's runtime effects already occurred.
...
- Existing connected sessions may continue on the source only before the persisted deadline. The source may terminate early when no sessions remain. The command path uses the committed source-instance fence as its authoritative source fence and rejects at or after `sourceDrainDeadlineAt` without consulting the admission-pointer database. The retryable reconciler applies the canonical bounded source-drain sequence around that fence: optional bounded notice, confirmed lifecycle-fence readback, socket closure, and idempotent `InstanceTermination`; it owns those effects and their retries, while the command path performs no drain side effects beyond rejection.
