...
- Tenant isolation is logical and mandatory. Tenant-scoped APIs, persistence, Redis keys, object-storage paths, authorization, audit context, tenant-scoped hosting/game entitlements, and quotas carry and validate the authoritative `tenantId` at every applicable boundary.
- Global tables are an explicit exception to tenant-column rules. Platform-wide identity, explicit account-scoped purchase grants and donations, and other deliberately global authorities do not acquire a synthetic `tenantId`; relationships from global records to tenant-owned state use explicit tenant-scoped membership, ownership, or grant-consumption records.
- Account Service owns the canonical tenant-scoped hosting/game entitlement and plan-limit records, and returns them with the target identity and authority/version context. Explicit account-scoped purchase grants and donations remain global account records; applying one to a tenant-scoped feature requires a separate explicit tenant binding or consumption record, and no caller, default, or migration may fabricate a `tenantId`. Gateway and TCP Proxy enforce edge-safety limits, while Game Session and other domain services enforce tenant-aware capacity, storage, and workload budgets at their owning boundaries. An entitlement for one tenant, a global role, or a cross-tenant read never supplies quota or capacity for another tenant; there is no cross-tenant entitlement inheritance or account-wide fallback.
- Entitlement and quota decisions fail closed when the Account result is unavailable, stale, future-dated beyond the allowed skew, malformed, incomplete, or bound to the wrong account or tenant unless the owning contract explicitly permits a still-fresh last-known-good value for that operation class. Account stamps entitlement evidence with `evaluatedAt` from its synchronized UTC clock. An eligible last-known-good (LKG) record must satisfy `evaluatedAt <= localNow + maxClockSkew`; consumers then define `conservativeAge = max(0, localNow - evaluatedAt) + maxClockSkew` and require it to be strictly below the applicable finite freshness bound. Clamping the observed age before adding skew ensures that a future timestamp within `maxClockSkew` never widens that bound, and the exact boundary is expired. A future timestamp beyond `maxClockSkew` is invalid and fails closed. An entitlement-only unavailable or timed-out read maps to `ENTITLEMENT_UNAVAILABLE` when no eligible LKG value supplies that entitlement input; an unavailable or timed-out non-entitlement auth, lease, binding, or token-issuance dependency maps to `AUTH_UNAVAILABLE`. This exception requires an exact account/tenant-bound, present, valid, contract-permitted, versioned value; it does not require a numeric value greater than zero, so a valid zero quota remains usable. Expiry, a newer denial, or a sequence/authority conflict returns the closed outcome. No fallback may become unlimited or account-wide, or source entitlement or quota from a global role, account-scoped grant, or another tenant.
- LKG ownership and eligibility remain in accepted [ADR 0028](./adr-0028-differentiated-entitlement-freshness.md#fresh-and-last-known-good-windows), which permits only its documented exact-same-binding/non-expanding continuity and already-entitled non-expanding recovery cases. This ADR creates no broader operation-class allowlist. Any future expansion requires a separate accepted owning contract that defines a closed operation-class allowlist, finite freshness, revocation behavior, and owning service; until then, all other shared-infrastructure entitlement and quota decisions fail closed without fresh Account evidence.
...
- Account-scoped purchase grants and donations remain usable at account scope without being represented as tenant entitlements; any tenant use is auditable through an explicit binding or consumption record.
...
- Complete Account-owned, exact-`tenantId` entitlement and quota response fields, authoritative freshness/versioning, event invalidation, and owning-service enforcement for sessions, ticks, commands, automation, storage, and other bounded resources. Prove that unavailable, stale, future-dated beyond `maxClockSkew`, malformed, incomplete, wrong-account, and wrong-tenant Account data fails closed, that LKG evidence first rejects out-of-bound future timestamps and then uses `max(0, localNow - evaluatedAt) + maxClockSkew < freshnessBound` without widening the bound, that entitlement-only and non-entitlement outages map to `ENTITLEMENT_UNAVAILABLE` and `AUTH_UNAVAILABLE` respectively, and that no fallback grants unlimited, account-wide, global-role, or cross-tenant entitlement or quota.
