...
- Complete Account-owned, exact-`tenantId` entitlement and quota response fields, authoritative freshness/versioning, event invalidation, and owning-service enforcement for sessions, ticks, commands, automation, storage, and other bounded resources. Prove that unavailable, stale, future-dated beyond `maxClockSkew`, malformed, incomplete, wrong-account, and wrong-tenant Account data fails closed, that LKG evidence uses the skew-adjusted freshness predicate without widening its bound, that entitlement-only and non-entitlement outages map to `ENTITLEMENT_UNAVAILABLE` and `AUTH_UNAVAILABLE` respectively, and that no fallback grants unlimited, account-wide, global-role, or cross-tenant entitlement or quota.
