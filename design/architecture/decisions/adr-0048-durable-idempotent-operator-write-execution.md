...
An exact duplicate of an existing reservation observes the same durable record and waits for the reservation owner to complete it or for the bounded reservation attempt to reach its terminal no-mutation outcome; it must not issue another authorization reference or forward another request. A request with the same `controlPlaneRequestId` but any difference in the reserved tuple returns canonical `IDEMPOTENCY_CONFLICT` without mutation. Only the reservation owner may complete the record with the post-authorization tuple. If that owner cannot complete within the bounded attempt, recovery may record terminal `NOT_EXECUTED` only after proving that no mutation was forwarded and that any authorization reference which may already have been issued is revoked or expired unused. Recovery may not transfer completion ownership or issue another reference. Forwarding under that request identifier is allowed only through the owner-side terminal-`NOT_EXECUTED` rearm rule below; every other later mutation requires a new request identifier.
...
Logging and Admin retains the original durable intent and reconciles through that result lookup until the owner result can be recorded. Same-identifier mutation redelivery is allowed only after a durable terminal owner result `NOT_EXECUTED` proves that forwarding or owner execution could not have occurred, the complete authority-bound tuple still matches, and the original authorization remains valid. The owner must atomically rearm that exact result record with a fresh `ownerMutationId` and fence as one new attempt before execution while retaining the completed attempt history. Any other terminal result is replay-only through the read-only lookup. An expired `PENDING` claim, missing owner record after dispatch, or any other indeterminate state remains read-only and non-replayable and retains its existing `ownerMutationId`; it never allocates a successor attempt. Durable absence cannot prove that an already dispatched mutation will not commit later, and authority revocation never creates a fresh reference merely to replay an old write.
