...
- returns the previously committed result only when a duplicate `controlPlaneRequestId` carries an exact match for the full authority-bound tuple; a same-identifier request with any difference in digest, authorization-reference fingerprint, workload identity, applicable automation-policy identity, or bound absence fails with canonical `IDEMPOTENCY_CONFLICT` and never applies either payload again.
