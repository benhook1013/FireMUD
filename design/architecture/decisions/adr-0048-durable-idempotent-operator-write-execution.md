...
An exact duplicate of an existing reservation observes the same durable record and waits for the reservation owner to complete it or for the bounded reservation attempt to reach its terminal no-mutation outcome; it must not issue another authorization reference or forward another request. A request with the same `controlPlaneRequestId` but any difference in the reserved tuple returns canonical `IDEMPOTENCY_CONFLICT` without mutation. Only the reservation owner may complete the record with the post-authorization tuple. Reservation state transitions are explicit: `RESERVED -> AUTHORIZED` is the owner-token-protected completion after Account issues the reference; `RESERVED` or `AUTHORIZED -> NOT_EXECUTED` is allowed only when durable evidence proves that no owner request was forwarded and no in-flight owner call can still be made; `AUTHORIZED -> PENDING` occurs only when the exact reference and completed tuple are forwarded; and `PENDING -> NOT_EXECUTED` is allowed only when the owner commits an explicit terminal no-mutation result before invoking the business mutator, with no applicable `FENCE_REJECTED` or `IDEMPOTENCY_CONFLICT` outcome. A stale owner/fence is `FENCE_REJECTED`, and a tuple mismatch is `IDEMPOTENCY_CONFLICT`; neither is rearmable. An expired claim, lost response, missing record after dispatch, or any other ambiguous state remains `PENDING`/indeterminate and is never converted to `NOT_EXECUTED`. Recovery may not transfer reservation ownership or issue another reference.
