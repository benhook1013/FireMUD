...
- an allowlisted read-only owner result lookup that locates by `controlPlaneRequestId` but validates the complete authority tuple, including digest, authorization-reference fingerprint, exact workload/policy identity, and bound presence/absence values, returning `IDEMPOTENCY_CONFLICT` for any mismatch and recovering the durable outcome after authorization expiry or revocation without executing the mutation;
