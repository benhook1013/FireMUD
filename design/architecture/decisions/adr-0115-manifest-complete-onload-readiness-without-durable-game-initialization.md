...
The current path admits bounded tenant-readiness `onLoad` work and exposes a readiness projection, but it does not yet seal or prove the immutable complete handler manifest required by this ADR. Game Design currently notifies Automation with an empty `affectedScripts` list; Automation returns before creating readiness state, and its Temporal tracking hook also skips empty lists. Normal script-patch publication therefore does not currently establish a readiness candidate or workflow. For non-empty notifications, Automation resolves only definitions found by name and seeds readiness from that found set; an incomplete expected-handler comparison can therefore mark a patch `READY` while required definitions are missing. Notification transport and response failures are swallowed without a durable redrive, so publication can remain `PUBLISHED` without a readiness candidate or workflow. An explicit empty handler set is valid only when the immutable patch manifest proves that zero handlers are required. For an identifiable tenant/patch notification, absent or unverifiable zero-handler proof has one canonical outcome: the readiness owner durably records `FAILED` with a bounded reason such as `zero_handler_manifest_unverifiable`, publishes the normal readiness status/audit evidence, and does not silently reject, bypass `onLoad`, start synthetic handler work, or retry the DSL. An unidentifiable malformed notification may still fail at transport/validation without fabricating a patch identity. Monotonic publication acceptance and late-completion fencing, along with readiness-owner recovery and terminalization of stale `ONLOAD_RUNNING` work, remain target convergence and proof gaps; the generic replay behavior described in the automation tracker is implementation drift rather than supported readiness recovery.
...
- An explicitly handler-free patch can become ready without synthetic work only with verifiable manifest proof; missing or unverifiable proof for an identifiable patch becomes an observable durable `FAILED` readiness record rather than a silent rejection, while missing work for a non-empty manifest fails closed.
...
The immutable patch artifact and readiness projection must expose enough identity to compare declared, admitted, and terminal handler sets. Proof must cover complete and empty manifests, missing or unverifiable zero-handler proof producing one durable `FAILED` record with status/audit evidence, capacity failure, logical/sandbox failure, bounded retry under one identity, duplicate completion, reordered publication, monotonic supersession, cancellation of not-yet-started work, late old-candidate success, cache loss after `READY`, and rejection of durable/shared side effects.
