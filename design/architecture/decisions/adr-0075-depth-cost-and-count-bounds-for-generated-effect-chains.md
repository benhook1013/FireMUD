...
Every generated child carries an immutable parent identity, immutable root identity, depth equal to `parent.depth + 1`, and an owner-defined deterministic child ordinal allocated and persisted by the owning contract for replay-stable reuse. For generated work under an admitted command, the immutable root identity is the opaque Game Session root `EffectId` allocated for the enclosing command plan and its `planOrdinal`; this decision introduces no second root authority. Generated work under a passive, inbound/remote, timer, retry, or other non-command-root contract retains that owner's root identity and does not acquire a `planOrdinal` unless it materializes as an admitted command. A generated child never allocates a new command plan root, and its child ordinal is lineage/accounting metadata rather than an input to the root allocation binding. The sealed root manifest also persists the resolved platform, operator, and feature depth/count/cost/per-target limits, cost weights, and cost-model version. Retry, replay, and reconciliation reuse those persisted resolved values and never re-resolve them. Replay reuses the recorded child identity, lineage, ordinal, and budget accounting; it does not increment them again or mint a replacement identity.
...
Implement one canonical generated-chain admission and accounting path with the platform hard depth ceiling, shared depth bootstrap default of `8`, deterministic total count and cost budgets, per-target caps, operator bounds, and feature-lowering rules. When the enclosing work is command-root work, persist its `planOrdinal`/opaque root allocation binding; otherwise retain the owner/source-specific root identity. Persist immutable parent and root identities, `parent.depth + 1`, owner-defined deterministic child ordinals allocated and persisted for replay-stable reuse, required or optional classification, and the complete suppression evidence; child ordinals must not allocate or replace a command plan root.

Prove hard-ceiling and lower-bound resolution; depth, count, cost, and per-target suppression at each boundary; deterministic admission order; cost accounting; immutable lineage and ordinals; request-digest validation; crash and replay without re-incrementing accounting or reminting identity; suppression of only the new child; preservation of committed parents and earlier children; truthful `SUCCESS`, `PARTIAL`, and `FAILED` derivation; complete durable evidence; bounded metric labels and bands; raw-identity exclusion from Prometheus; audit-query retrieval; one-off optional audit behavior; repeated optional designer warnings; prompt required-suppression alerts; and operational paging for sustained aggregate suppression or tick degradation. The pending [ADR 0182 proposal](./adr-0182-deterministic-effect-id-allocation-and-replay-binding.md) is not proved or accepted by this ADR.
