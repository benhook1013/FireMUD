...
Stage-aware dead-letter recovery with frozen-input retry and stored-dispatch continuation is target state. The current `ReplayDeadLetteredWorkItems` path requeues eligible rows as `PENDING_EVALUATION`, returns aggregate counts, and does not prove stage-specific recovery, `failureGeneration` fencing, stored-output continuation, or idempotent per-request outcomes.
...
For recoverable instance-bound gameplay/runtime work, both paths require an exact current match for the admitted `scriptPatchVersion`, `scriptPinEpoch`, the exact `(pluginId, pluginVersionId, bindingId)` tuple when applicable, and the captured `pluginActivationEpoch` when applicable, plus the captured-lifecycle-revision rule below, fresh binding and lifecycle evidence, runtime region and `regionEpoch`, and admitted routing bundle including playable-state scope, world/realm identity, and pointer version. Plugin work persists the captured activation epoch and lifecycle revision and revalidates them at every applicable boundary: durable persistence, handoff, staged effect execution, final effect execution, retry, replay, recovery, and final execution. It also performs immediate checks before evaluation, each post-evaluation dispatch, and each applicable staged/final effect. Current plugin activation/lifecycle, component-policy, capability-grant, and signer/publication evidence must be fresh and valid at those boundaries. `ENABLED` recovery requires the exact current lifecycle revision. `DRAINING` blocks new trigger admission; a new trigger admission during `DRAINING` is rejected with `admissionOutcome=TRIGGER_ADMISSION_OUTCOME_VERSION_UNAVAILABLE` and `admissionReason=plugin_not_active`, while dead-letter recovery may proceed only for work whose winning admission CAS durably committed the immediately preceding `ENABLED` lifecycle revision before the durable Automation-owned `DRAINING` admission barrier was created, with the same exact plugin version and activation epoch while every other fence passes. Merely observing or capturing the old revision without committed admission/fence evidence is rejected with the applicable recovery fence-mismatch reason, `plugin_binding_mismatch` for the lifecycle-evidence mismatch. Reserve `plugin_disabled` for a committed lifecycle invalidation of already-admitted work, such as a `DISABLED` transition, revocation, or policy-driven disablement; a proven publication mismatch uses `plugin_version_not_published`, a blocked component uses `plugin_component_policy_blocked`, and signer revocation uses `signer_revoked`. A present plugin tuple, binding, or lifecycle-revision mismatch uses `plugin_binding_mismatch`, while a present-but-different activation epoch uses `plugin_activation_epoch_mismatch`. Missing, stale, contradictory, or unavailable runtime/fence, component-policy, or signer-policy evidence fails closed with `authority_unavailable`, `component_policy_unavailable`, or `signer_policy_unavailable` as applicable. An arbitrary lower revision or a non-executable or epoch-advancing transition rejects displaced work. Matching patch text under a different pin epoch is ineligible. These bounded dispositions never allow recovery from a projection guess.
