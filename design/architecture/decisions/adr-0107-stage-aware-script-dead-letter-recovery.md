...
Stage-aware dead-letter recovery with frozen-input retry and stored-dispatch continuation is target state. The current `ReplayDeadLetteredWorkItems` path requeues eligible rows as `PENDING_EVALUATION`, returns aggregate counts, and does not prove stage-specific recovery, same-generation fencing, stored-output continuation, or idempotent per-request outcomes.
