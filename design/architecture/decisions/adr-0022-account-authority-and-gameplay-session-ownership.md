...
| --- | --- | --- | --- |
| Protected control-plane, bootstrap, and JWT-presenting admission operations | A registry-backed JWT (`control-ui`, `player-bootstrap`, or named private delegation) | Local signature/profile validation, one matching registry record, and one current Account evidence comparison for the route. A named private delegation that uses replacement rotation additionally requires the exact lease to be `INSTALLED`, with matching `installationFence`, token lineage, and gameplay binding identity | Required; profiles without an installation lease use registry-plus-current-Account validation only. A replacement private delegation cannot authorize from registry state alone; reachable missing or mismatched installation evidence denies, while an unreachable or timed-out dependency is `AUTH_UNAVAILABLE` |
