...
Later clarification: [ADR 0132](./adr-0132-namespace-scoped-single-character-controller.md) supersedes the controller-uniqueness portion of that earlier accepted wording with the exact identity `{tenantId, playableStateNamespaceId, characterId}`. `playableStateScope` remains server-derived binding/routing evidence, not an additional controller key; [Session Behavior](../system-architecture-session-behavior.md#namespace-scoped-controller-transfer-session-02) is the canonical runtime contract. The historical authority split and rationale above remain unchanged.
