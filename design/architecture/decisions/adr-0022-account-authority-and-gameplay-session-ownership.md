...
| Reconnect, resume, or rebind without a presented JWT | The exact existing gameplay binding context/record and its resume/rebind proof | Exact controller key; an independent exact match between the currently authenticated `accountId` and the binding's stored owner `accountId`; separately validated binding scope evidence and fence; current Account lifecycle/security/membership, applicable entitlement, realm-grant, billing, and revocation authority; and the Account-owned applicable resume lease. An eligible exact-binding entitlement snapshot under [ADR 0028](./adr-0028-differentiated-entitlement-freshness.md) may supply only the entitlement input within the Account-owned `resumeActivationLease` contract in [ADR 0030](./adr-0030-risk-based-active-session-revocation.md); that lease remains required and does not itself supply or replace the snapshot; no target or scope expansion | Not used; stale, missing, or conflicting binding evidence denies the operation |
...
- Prove `PLAY`, reconnect/resume, role changes, membership removal, and billing cutoff consume current Account authority and fail closed when it is unavailable, with only the ADR 0028/0030 exact-binding entitlement-snapshot exception for grace resume. Reconnect/resume proof must independently exact-match the currently authenticated `accountId` to the binding's stored owner `accountId` in addition to validating the controller key, binding scope, and fence evidence.
