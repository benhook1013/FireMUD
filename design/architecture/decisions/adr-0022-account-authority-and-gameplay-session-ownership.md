...
| Reconnect, resume, or rebind without a presented JWT | The exact existing gameplay binding context/record and its resume/rebind proof | Exact controller key, separately validated binding scope evidence and fence, current Account lifecycle/security/membership, applicable entitlement, realm-grant, billing, and revocation authority, and the Account-owned applicable resume lease; an eligible exact-binding entitlement snapshot under [ADR 0028](./adr-0028-differentiated-entitlement-freshness.md) may supply only the entitlement input within the Account-owned `resumeActivationLease` contract in [ADR 0030](./adr-0030-risk-based-active-session-revocation.md); that lease remains required and does not itself supply or replace the snapshot; no target or scope expansion | Not used; stale, missing, or conflicting binding evidence denies the operation |
