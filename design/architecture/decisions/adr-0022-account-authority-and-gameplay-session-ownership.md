...
The strict issued-token-registry and authority-generation middleware boundary applies to protected control-plane and JWT-bearing bootstrap/admission operations. Fresh gameplay admission, in-band `PLAY`, reconnect, and resume use their bound-session admission contracts and only the current-authority checks those contracts require; they do not invent a JWT registry lookup where no issued JWT is presented. Once a gameplay binding has been admitted, routine gameplay commands use the validated bound Game Session context, binding fences, and domain authorization rather than repeating registry or authority-generation lookups for every command. Bounded reconciliation still consumes subsequent revocation or authority changes, and any invalid or conflicting evidence invalidates the binding rather than being treated as routine continuity.
