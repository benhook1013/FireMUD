...
The canonical Game Session root binding is authenticated and exact-compared before any World or Entity owner-local guard or expected-state precondition runs. That binding includes the root `EffectId`, typed operation, exact target aggregate and runtime scope, immutable request digest, sealed participant/manifest context, and the applicable actor/executor fence. A gameplay-session attestation, target snapshot, or effect identifier by itself is not a substitute for this admission binding; a missing or conflicting binding fails closed.
...
Every owner derives a participant guard identity from the persisted mutation `EffectId`, typed operation, and target aggregate within the owner-declared runtime-family partition: S1/S2 use `(tenantId, playableStateNamespaceId)`, explicit S3 uses `(tenantId, gameInstanceId)`, and an unknown or unclassified family fails closed. In this ADR’s root-mutation shorthand, the persisted mutation identity is the root `EffectId`; a generated child uses its own persisted child `EffectId`, with the enclosing root retained only as lineage. Same guard and same request replays the prior result; the same guard with a different operation, target, partition, or digest fails closed. Generated-child allocation and root/child lineage beyond this accepted guard boundary remain subject to the non-authoritative [ADR 0182 proposal](./adr-0182-deterministic-effect-id-allocation-and-replay-binding.md).
