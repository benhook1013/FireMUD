...
The target bounded typed Game Session → Gateway lifecycle-intent contract is not yet versioned or implemented. Current behavior infers lifecycle intent from upstream close/error behavior; this ADR does not invent a new internal Game Session → Gateway wire protocol or claim that the target intent contract is available. The client-visible WebSocket close values and equivalent Telnet reason tokens defined below are external transport outcomes, not that internal contract.
