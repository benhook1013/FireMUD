...
`OUTCOME` must be non-empty, use either the semicolon or `by` form shown in the queue, and contain one or more Markdown links. Provenance is label-based, not disposition-based: an ADR provenance reference is specifically a link labeled `[ADR NNNN]` whose target filename is `adr-NNNN-*.md`, and every such ADR link in any checked row receives that row's source key, date, and disposition. Every checked non-alias `accepted` or `revised` row with no exact `[ADR NNNN]` link may use the ordinary strict no-ADR form shown above: exactly one link labeled `[canonical contract]`, targeting an existing Markdown file inside `design/architecture` but outside the canonical ADR directory, with the outcome ending exactly `; no ADR required`. A checked `deferred` row without exact `[ADR NNNN]` provenance uses that same strict no-ADR form: exactly one link labeled `[canonical contract]`, targeting an existing Markdown file inside `design/architecture` but outside the canonical ADR directory, and the outcome must end exactly `; no ADR required`. A formal checked non-alias `superseded` row requires exact `[ADR NNNN]` provenance. A superseded replacement-map row is the narrow exception for a reviewed decision that is superseded by other decisions without creating its own ADR: it must use the documented `by` replacement-key form with backtick-delimited `DECISION-KEY` values, one or more non-provenance links labeled `[replacement ADR NNNN]`, each targeting the matching existing ADR in the canonical ADR directory, and the exact final suffix `; no ADR required`. These replacement links do not contribute ADR review metadata, and exact `[ADR NNNN]` or arbitrary labels are invalid in that form. A valid no-ADR row contributes no ADR review metadata. Checked non-alias rows with a `withdrawn` disposition always require exact `[ADR NNNN]` provenance. The deliberate deferred exception is an `Accepted` ADR that records the deferred non-support boundary and revisit trigger, in which case the row may carry one exact `[ADR NNNN]` link and the ADR's completed disposition must be `Deferred`. A superseded scan alias is a separate historical replacement mapping, not a superseded decision: every Markdown link in its outcome must be a replacement decision with either a non-provenance `[replacement ADR NNNN]` label targeting that exact canonical ADR, or a decision-key label such as `[JWT-01]` targeting an existing Markdown decision document. Exact `[ADR NNNN]` labels, arbitrary labels, external targets, and non-Markdown replacement references are invalid. Alias links do not contribute provenance to the historical row, and a replacement ADR receives provenance only from its own checked row. Other outcome prose may follow outcome links, but in a strict no-ADR row it must precede the final `; no ADR required` suffix. Distinct coupled queue rows may reference the same ADR only when their date and disposition agree. Duplicate source keys, conflicting duplicate ADR provenance, duplicate ADR links in one row, malformed checked rows, and checked rows without an outcome link are invalid. Unchecked rows are not parsed as completed review evidence.
...
| [ADR 0137](./adr-0137-isolated-playtest-state-modes-and-reset.md) | Accepted | `AR-3.4` | `AA-3.2`, `GR-2.1`, `GR-3.1`, `GR-3.2`, `AA-1.2`, `AR-3.3` | Explicit fresh, seeded, and snapshot playtest initialization with namespace-replacement reset and no merge-back |
...
| [ADR 0146](./adr-0146-owner-local-moderation-enforcement.md) | Accepted | `PO-1.2` | `EA-2.4`, `AA-1.3`, `PO-1.3`, `PO-4.1`, `SF-2.3` | Owner-local moderation enforcement through acknowledged typed commands |
| [ADR 0147](./adr-0147-explicit-communication-classes-and-owner-delivery.md) | Accepted | `EA-2.1` | `EA-2.2`, `EA-2.3`, `GR-4.1`, `PO-1.2`, `SF-2.3` | Explicit gameplay, social-channel, and operator communication classes |
| [ADR 0148](./adr-0148-social-relationship-authority-and-entity-owned-value.md) | Accepted | `EA-2.2` | `EA-2.1`, `AA-1.3`, `PO-1.2`, `SF-2.1` | Social relationship authority with Entity-owned transferable value |
| [ADR 0149](./adr-0149-communication-type-specific-history-and-retention.md) | Accepted | `EA-2.1` | `SF-2.1`, `SF-2.2`, `SF-2.3`, `PO-1.3` | Communication-type-specific history, evidence, retention, and acknowledgement |
...
| [ADR 0182](./adr-0182-separated-hosted-runtime-and-certificate-identity-lifecycles.md) | Accepted | `PO-3.2` | `PO-3.1`, `PO-4.4`, `SF-1.3` | Separated hosted runtime and certificate-identity lifecycles |
