...
- Normal operator mutations use version-matched, owner-supported tooling. The initial public recovery surface is explicitly bound to the Coordination Redis role and is bounded to `status`, one `recover --mode <replay-first|reset|session-schema-cleanup>` operation, `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)`, `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)`, and the canonical `releaseMaintenanceLock(operationId, scope, maintenanceLockToken, reason, evidenceRef)` operation. It advertises only Coordination scopes implemented and proved by the runtime. Reset requests require exactly one explicit session-policy option, `--preserve-sessions` or `--invalidate-sessions`, bound to the durable operation and maintenance lock; the policy is never inferred from scope. Pause-and-lock is an internal fenced phase of the durable recovery operation, not a standalone public verb. Cache/Rate-Limit Redis clearing remains a separate deployment-scoped owner-tooling concern and cannot invoke this gameplay recovery surface; any future mutating cache reset must carry its own exact deployment/prefix scope, authorization, immutable evidence, maintenance exclusion, and release-postcondition proof.
...
- A strictly observational dry run is the only direct-finalization exception. It may perform discovery and validation without acquiring a gameplay fence or maintenance lock and without writing, deleting, rebuilding, or releasing any runtime state; its `recover` response records `maintenanceLockApplicability=NOT_APPLICABLE` and omits `maintenanceLockToken`. After those no-effect conditions are durably recorded, it may transition directly to `finalized` and `SUCCEEDED`. Every mutating `recover` response records `maintenanceLockApplicability=REQUIRED` and returns the non-empty controller-issued token through the protected issuance channel. Null, blank, caller-chosen, or sentinel token values are invalid. A run that acquires a fence or lock, mutates state, or could require a release effect is not observational and must follow the ordinary continuation, authorization, release, readback, and finalization lifecycle.
