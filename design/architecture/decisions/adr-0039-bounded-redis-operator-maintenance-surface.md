...
- Normal operator mutations use version-matched, owner-supported tooling. The initial public maintenance surface is bounded to `pause`, `status`, `recover` or `reset`, `resume`, and a maintenance-lock release operation.
- The high-level recovery operation owns the required ordered phases, including durable epoch handling, ledger and command convergence, metadata initialization, session policy, and the post-reset smoke gate. Those phases may have internal APIs and focused tests without becoming separately supported public verbs.
