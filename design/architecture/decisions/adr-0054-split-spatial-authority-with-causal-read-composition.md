...
For a root mutation, Game Session assigns one stable root `EffectId` to the logical effect and persists intended pre/post state and required participants. Each participant derives a deterministic guard identity from the persisted mutation `EffectId`, typed operation, and target aggregate within the owner-declared runtime-family partition: S1/S2 use `(tenantId, playableStateNamespaceId)`, explicit S3 uses `(tenantId, gameInstanceId)`, and an unknown or unclassified family fails closed. For a root mutation the persisted mutation identity is the root `EffectId`; where a generated child is in scope, it is the persisted child `EffectId`, with the enclosing root retained only as lineage. Its durable guard binds that identity to an immutable request digest and stored outcome. The separate [ADR 0182 proposal](./adr-0182-deterministic-effect-id-allocation-and-replay-binding.md) is not accepted target state and may not be used as an implementation requirement.
