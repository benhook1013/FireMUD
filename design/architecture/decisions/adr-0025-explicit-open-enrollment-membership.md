...
The accepted explicit-join decision remains the target contract, with a local Account implementation now present. Global-only registration creates global account state and a platform-scoped audit outbox record without membership or profile authority; `JoinPublicProductionMembership` is the sole local public-production membership writer, with a policy-independent durable intent followed by a policy-bound digest, membership transition, operation outcome, and tenant-scoped audit/outbox evidence committed by the Account transaction. Current first-party connect-token issuance and text `PLAY` require existing membership and do not invoke an implicit membership writer; fresh authoritative missing or non-admitting public-production membership fails closed with `JOIN_REQUIRED` where the current adapter can establish that predicate. The obsolete differently named writer RPC and service implementation remain removed rather than retained as compatibility behavior. This local seam has focused unit and repository proof, but PostgreSQL execution is unavailable locally, no pending-operation recovery/reconciliation worker or direct text-to-Logging end-to-end proof is claimed, and the complete lifecycle snapshot plus independent `membershipVersion` and `membershipAuthorityGeneration` reread at every connect-token issuance remain incomplete. ADR 0025 therefore does not claim complete runtime or production proof.
