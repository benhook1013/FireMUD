...
The accepted explicit-join decision is target state. Current first-party connect-token issuance and text `PLAY` now require existing membership and do not invoke `EnsurePublicProductionPlayerMembership`; missing public-production membership fails closed with `JOIN_REQUIRED`. The differently named writer RPC and service implementation remain as unused implementation drift and must be replaced or removed rather than retained as compatibility behavior. Explicit `JOIN` / `Join & Play`, the membership transaction, monotonic versioning, durable audit/outbox boundary, and the exact current membership snapshot plus independent `membershipVersion` and `membershipAuthorityGeneration` reread at every connect-token issuance remain incomplete. No complete runtime implementation is claimed by this ADR.
...
The earlier admission flow created membership implicitly during first-party connect-token issuance or text-client `PLAY` through `EnsurePublicProductionPlayerMembership`. Current admission callers have been corrected to require existing membership and return `JOIN_REQUIRED`, but the writer RPC and service implementation still exist without the accepted explicit-join boundary. The remaining implementation also lacks the target transaction, versioning, and durable audit/outbox contract.
