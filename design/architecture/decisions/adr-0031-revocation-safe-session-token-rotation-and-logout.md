...
- `POST /auth/logout-all` atomically advances the durable account auth generation with its account-wide logout event, distinct `account_logout_all` audit record, and outbox record, then idempotently projects `session:auth:revoked_after:account:<accountId>` to that generation.
