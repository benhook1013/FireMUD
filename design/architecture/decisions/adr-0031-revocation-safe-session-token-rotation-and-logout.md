...
The current target already requires periodic refresh, atomic binding replacement, idempotent per-token logout, and generation-based logout-all. The review retains those boundaries and makes refresh generation, expiry scheduling, overlap, and logout scope explicit. Current implementation has no complete periodic rotation or refresh-generation path; the role-refresh service is a placeholder, gameplay session state lacks the complete target token metadata, and Account logout/logout-all generation workflows are absent.
...
- The Account-owned refresh request presents the current token identity and captured authority generations, the gameplay binding/account/tenant identity, and an idempotent request ID. Account validates the current issued-token registry generation, account state, membership/version, and applicable issuer, account, tenant, and membership auth generations. [ADR 0035](./adr-0035-single-record-issued-token-registry.md) defines the one-record registry shape and [ADR 0036](./adr-0036-monotonic-authority-generations-for-bulk-token-revocation.md) replaces timestamp cutoff ordering.
...
- An auth-expired response may trigger immediate refresh while the current generations remain refresh-eligible. An auth-revoked response triggers authoritative reconciliation; logout-all, password reset, security lock, membership loss, or another generation advance cannot be bypassed by giving the replacement a newer `iat`.
...
- `POST /auth/logout-all` commits a durable account-generation advance, account-wide logout event, and distinct `account_logout_all` audit record, then idempotently projects `session:auth:generation:account:<accountId>` set-if-greater.
- The account generation, not deletion of every token record, is immediate correctness authority. Bounded background cleanup may remove older token records.
...
- Password reset, security lock, and other account-wide revocations use distinct audit/event action types even when they share the same account-generation mechanism.
...
- The model requires a generation-bound refresh API, expiry-aware scheduler, token-entry lifecycle, durable account-wide events, auth-generation projection, and cross-device/gameplay proof.
...
Allowing Game Session to mint a fresh player token from mTLS plus an account ID is simple, but can cross logout-all and security generation advances. The current token and authority generations must remain part of refresh authority.

### Delete Every Token For Logout-All

Enumerating and deleting all token keys creates race, indexing, and partial-failure problems. A monotonic account generation gives one correctness decision while cleanup remains bounded background work.
...
- Define and implement `RefreshGameplayServiceToken` with current token/authority-generation subject binding, idempotency, and live membership/account validation.
...
- Implement logout-all durable event/audit, Account-owned generation projection, active-gameplay termination, idempotency, and background token cleanup.
...
- Prove gameplay-domain routine actions acquire no Service JWT refresh or auth-generation lookup.
