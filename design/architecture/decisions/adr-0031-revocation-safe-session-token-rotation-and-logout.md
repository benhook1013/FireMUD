...
- Durable operation evidence never becomes a replay store for the raw replacement JWT. Concurrent callers in the live issuing process may share the single-flight result, but after response uncertainty Account does not reconstruct or reissue the same credential from hashes or operation metadata. A matching retry returns the durable operation state; if the original replacement cannot be proved installed, the lease reconciler revokes that candidate and records a stable fail-closed outcome. Game Session may begin a new rotation with a new request ID only while the predecessor and current authority tuple remain valid; otherwise it requires fresh login.
...
- Implement half-life scheduling, jitter, single-flight, expiry safety margin, bounded retry, the durable pre-mutation `TOKEN_ROTATION` `PENDING`/`COMMITTED`/`FAILED` operation state machine, idempotent replacement/predecessor registry mutations and reconciliation, the Account-owned replacement lease/CAS reconciler, idempotent old-entry retirement acknowledgement, and failure-to-fresh-login behavior. Do not implement rotation as a cross-store transaction.
