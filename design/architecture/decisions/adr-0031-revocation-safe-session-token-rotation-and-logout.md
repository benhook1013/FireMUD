...
- Account atomically creates the replacement token registry record, moves the predecessor record to `retiring`, and records an Account-owned replacement lease before returning the replacement. The lease identifies the immutable refresh request and lineage and sets a bounded `installBy` deadline. That Account-owned commit records `predecessorUsableUntil = min(originalExp, replacementIssuedAt + maxInFlightRpcDeadline)`, so predecessor authority is independently bounded without pretending the later Game Session binding compare-and-set shares an Account transaction.
- Game Session must atomically replace `authTokenHash`, `authTokenIssuedAt`, `authTokenExpiresAt`, and refreshed membership metadata before new calls use the replacement. After that binding swap, no new call may use the predecessor. Account rejects the predecessor after `predecessorUsableUntil` even if cleanup has not run or a retirement acknowledgement is lost; RPCs already authorized before the swap remain bounded by their own deadlines and cannot extend the cutoff. If Game Session cannot install the replacement, it invokes the Account-owned idempotent replacement-abort/reconciliation operation. Account revokes and removes the orphan replacement, and the old binding may be preserved only while the predecessor remains unexpired and active, no competing replacement has been installed, and the complete authority tuple still matches; abort never reactivates authority invalidated by a concurrent cutoff.
- Game Session then sends Account an idempotent retirement acknowledgement carrying the old token identity, refresh lineage, replacement identity, and retirement request ID. Account marks the replacement lease installed, owns the registry cleanup, rejects mismatched identity, and returns the stored success for a replay of the same request; the acknowledgement is cleanup/replay evidence, not the predecessor's safety cutoff. An Account reconciler claims expired unacknowledged leases and idempotently revokes the orphan replacement, completes predecessor retirement, and records the terminal result. If Game Session installed the replacement but crashed before acknowledgement, the reconciler still revokes it and the gameplay binding fails closed to fresh login; it never reactivates the predecessor. Lease recovery is bounded independently of arbitrary JWT expiry and serializes with abort, acknowledgement, logout, and authority-generation cutoff through the same lineage state. An absent record without matching durable operation evidence is not sufficient authority.
