...
- Persist `authTokenHash`, `authTokenIssuedAt`, `authTokenExpiresAt`, the secret-valued single-use rebind handle, refreshed membership metadata, and rotation CAS generation in the protected authenticated gameplay binding; never persist the raw delegation JWT outside the sole accepted bounded Account-encrypted response-envelope exception described above.
- Implement half-life scheduling, jitter, single-flight, expiry safety margin, bounded retry, the durable pre-mutation `TOKEN_ROTATION` `PENDING`/`COMMITTED`/`FAILED` operation state machine, the bounded Account-encrypted response envelope, idempotent replacement/predecessor registry mutations and reconciliation, the Account-owned replacement lease/CAS reconciler, idempotent old-entry retirement acknowledgement, and failure-to-fresh-login behavior. Prove `COMMITTED` cannot replay success until the active-replacement/retiring-predecessor and lease/fence postconditions are revalidated. The envelope may be used only for same-request recovery after current-state validation; it must never become registry, operation, authorization, or Game Session storage. Do not implement rotation as a cross-store transaction.
