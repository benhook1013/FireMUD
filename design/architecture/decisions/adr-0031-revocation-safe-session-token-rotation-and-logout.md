...
- Account atomically creates the replacement token registry record and moves the predecessor record to `retiring` before returning the replacement. That Account-owned commit records `predecessorUsableUntil = min(originalExp, replacementIssuedAt + maxInFlightRpcDeadline)`, so predecessor authority is independently bounded without pretending the later Game Session binding compare-and-set shares an Account transaction.
...
- Game Session then sends Account an idempotent retirement acknowledgement carrying the old token identity, refresh lineage, and retirement request ID. Account owns the registry cleanup, rejects mismatched identity, and returns the stored success for a replay of the same request; the acknowledgement is cleanup/replay evidence, not the predecessor's safety cutoff. An absent record without matching completed-request evidence is not sufficient authority.
