...
- The exact-token tombstone is also an Account-owned monotonic token-identity fence. The fence records the token identity and lineage metadata (`tokenHash`, `jti`, profile, subject/account, and fence version) without persisting the raw JWT. Every refresh, rebind, or installation that could recreate an issued-token registry record must present and validate the current fence through the Account-owned CAS before changing Coordination Redis or making the replacement binding admissible. A logout fence advance therefore rejects stale refresh/rebind work rather than allowing it to recreate revoked registry state. A pending-operation reconciler may commit only when its operation fence still dominates every competing installation fence for that lineage and remains the authoritative fence; otherwise it records a stale/no-op outcome and cannot recreate the registry record. These are ordered idempotent operations across Account and Coordination Redis, not a cross-store transaction.
...
- Implement idempotent `/auth/logout` for `control-ui` and `player-bootstrap` profiles with bounded scoped-key lookup, durable `COMMITTED` exact-token tombstones, an Account-owned monotonic token-identity fence that refresh/rebind/install CAS operations must validate, and distinct audit events.
