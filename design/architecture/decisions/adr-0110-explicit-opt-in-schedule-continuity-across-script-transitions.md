...
`pluginActivationEpoch` and `lifecycleRevision` are runtime execution-fence evidence, not stable schedule-row or continuity identity. An invalidating `pluginActivationEpoch` change fences displaced candidates and admitted work under their original complete candidate/firing-claim identities, which include `pluginActivationEpoch` as a tagged composite field, and never reuses their claims; the captured `lifecycleRevision` is carried and revalidated separately as lifecycle-fence evidence. Neither fence by itself denies an otherwise explicitly declared, typed-compatible cross-plugin-version schedule-row and due-state continuity transition, except that a disablement or revocation transition always takes the explicit reset path. These lifecycle fences are distinct from Automation's dead-letter recovery `failureGeneration`. Disablement or revocation fences and tombstones the displaced schedule before any reactivation, even when the target uses the same version and matching definitions; reactivation creates fresh schedule state and never reuses an old candidate or firing identity.
...
For a preserved interval, the source plugin's captured `(pluginActivationEpoch, lifecycleRevision)` pair remains retained as historical execution-fence provenance on the displaced schedule/transition evidence and continues to fence old candidates and admitted work. An invalidated source lifecycle alone does not reject an explicitly compatible ordinary continuity transition, but disablement or revocation has already forced the reset/tombstone path above and cannot preserve the row through reactivation. Before rewriting ownership or resuming admission on a valid continuity path, reconciliation must acquire and validate the complete applicable current target plugin evidence set defined by [Scripting Contracts §8](../system-architecture-scripting-contracts.md#8-plugin-version-fencing-and-control-plane-scope), including the target plugin version/binding, current target activation/lifecycle, fresh component-policy, capability-grant, signer/publication evidence, and the target `(pluginActivationEpoch, lifecycleRevision)` pair. Missing, stale, revoked, contradictory, or unavailable target evidence fails closed before the ownership rewrite or resume. The lifecycle pair remains execution-fence evidence, not continuity identity. Reconciliation then rewrites the durable patch/plugin ownership metadata to the exact target `(scriptPatchVersion, scriptPinEpoch)` tuple, including applicable `pluginId`, `pluginVersionId`, schedule-owned `bindingId`, and target `(pluginActivationEpoch, lifecycleRevision)` pair, before admission resumes. The same current-target evidence gate applies before any candidate or firing-claim derivation; candidate and firing-claim identities include the captured `pluginActivationEpoch` as a tagged composite field, while `lifecycleRevision` is carried and revalidated separately. It calculates the next due point under the normative [Timer Resume Rule](../system-architecture-scripting-scheduler-and-timers.md#timer-resume-rule-normative): tick schedules use committed tick identifiers, while compatible wall-clock schedules retain persisted absolute `dueAt` and regenerate `nextRunAt` strictly as its projection; target resume time does not rewrite `dueAt`. It does not blindly retain an overdue point, restart from deployment time, or replay every missed interval.
...
Persist and validate explicit continuity policy, stable owner identity, `scheduleDefinitionId`, target scope, interval kind and cadence, source and target exact script pin tuples, applicable plugin versions, schedule-owned bindings, captured `(pluginActivationEpoch, lifecycleRevision)` pairs, previous and recalculated due points, transition identity, tombstone/fresh/preserved outcome, and the exact execution fences. Compilation and publication must reject missing required identity, duplicate continuity keys, and incompatible declarations deterministically.

Proof must cover default reset even when `scheduleDefinitionId` matches; explicit typed-compatible preservation across patch and plugin-version changes without using `pluginActivationEpoch` or `lifecycleRevision` as continuity identity; owner metadata rewrite without changing the preserved row's logical identity; rollback; source-lifecycle invalidation that fences old source candidates, firing claims, and work under their captured identities without rewriting or reusing them while preserving an explicitly declared, typed-compatible schedule-row/due-state continuity transition; absent target; changed owner, identifier, target, kind, cadence, playable scope, binding, or incompatible target evidence; same-version plugin disable/re-enable with fresh state; disablement or revocation without a replacement target resetting and tombstoning; explicit reset; duplicate definitions; idempotent transition retry without duplicate row or firing; and failure or restart during reconciliation. For plugin-owned schedules, proof must also cover acquisition and validation of the complete applicable current-target plugin evidence set, including the target `(pluginActivationEpoch, lifecycleRevision)` pair, before ownership rewrite/resume and before candidate/firing-claim derivation, with missing, stale, revoked, contradictory, or unavailable fail-closed outcomes, following [Scripting Contracts §8](../system-architecture-scripting-contracts.md#8-plugin-version-fencing-and-control-plane-scope).
