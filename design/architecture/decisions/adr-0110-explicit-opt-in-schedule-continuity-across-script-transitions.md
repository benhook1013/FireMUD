...
Proof must cover default reset even when `scheduleDefinitionId` matches; explicit preservation across patch and plugin-version changes; owner metadata rewrite without changing the preserved row's logical identity; rollback; absent target; changed owner, identifier, target, kind, cadence, playable scope, binding, or `pluginActivationGeneration`; same-version plugin disable/re-enable; missing or stale applicable plugin-generation evidence before candidate/firing-claim derivation; explicit reset; plugin disablement/revocation; duplicate definitions; idempotent transition retry without duplicate row or firing; and failure or restart during reconciliation.
