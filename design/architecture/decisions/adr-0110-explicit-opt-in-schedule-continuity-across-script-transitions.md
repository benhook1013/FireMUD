...
`pluginActivationEpoch` is runtime execution-fence evidence, not continuity identity. An invalidating `pluginActivationEpoch` change fences displaced candidates and admitted work under their original identities and never reuses their claims, but it does not by itself deny an otherwise explicitly declared, typed-compatible cross-plugin-version schedule-row and due-state continuity transition. This activation fence is distinct from Automation's dead-letter recovery `failureGeneration`. Disablement or revocation without a replacement target resets and tombstones the displaced schedule; a same-version disable followed by re-enable starts with fresh schedule state even when the definitions match.
