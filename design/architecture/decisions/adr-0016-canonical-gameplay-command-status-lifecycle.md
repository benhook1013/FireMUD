...
- `GetGameplayCommandStatus` reads the authoritative durable record only by the point-lookup key `{tenantId, gameInstanceId, commandId}`. The automation tuple `(tenantId, gameInstanceId, regionId, regionEpoch, automationDispatchId)` is a collection/filter correlation key only, not a point-lookup or uniqueness key: one dispatch may contain multiple emitted commands distinguished by `commandOrdinal` in the complete Command-Handoff Identity defined by the scripting normative contract table. If an automation-correlated status view is exposed, it returns an explicitly bounded collection rather than selecting one authoritative record from that tuple; `regionId` and `regionEpoch` apply only as routing/filter fields. Redis coordination state is not part of the lookup authority.
