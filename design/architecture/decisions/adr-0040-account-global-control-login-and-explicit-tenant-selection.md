...
- `tenant_regular` authorization is the disjunction of two explicit, mutually exclusive predicates. The tenant-role predicate requires the route-declared tenant role/capability, live membership in the explicit target tenant, the caller-bound membership authority generation, and fresh target-tenant authority-generation evidence. The allowlisted `platformAdmin` predicate is available only when the route-matrix entry explicitly declares the `platformAdmin` override; it requires the live global role, explicit target tenant, current issuer/account checks, role-appropriate assurance, and fresh `target_tenant_generation`, but does not require or imply target-tenant membership or membership-generation authority. Neither branch may use the UI's selected tenant as evidence, and no other global role inherits this branch.
- The receiver selects the `tenant_regular` branch from live Account authority before evaluating predicates: when the route explicitly allows the global `platformAdmin` override and the caller has that live global role, only the `platformAdmin` branch is eligible, even if the caller also has a tenant role. A missing assurance, target-tenant generation, or any other `platformAdmin` predicate fails closed and must not fall back to the tenant-role branch. Otherwise only the tenant-role branch is eligible, and it must retain the live membership and caller-bound membership-generation checks; if neither branch is eligible, the request is denied. This makes branch selection deterministic and prevents a stale or unavailable privileged check from changing the required membership-generation behavior.
