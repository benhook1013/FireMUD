...
- Membership omission is not implied by a global role. A route-matrix-declared `tenant_regular` `platformAdmin_global` branch may omit caller-bound membership only with its required target-tenant generation and elevated-assurance predicates. The explicitly allowed `cross_tenant_support_safe` and `cross_tenant_billing_safe` classes have separate declared omission rules and must enforce their own global role, assurance, scope, and current Account-owned issuer/account authority checks. Support-safe access requires the matrix-approved `support` or `platformAdmin` path with its required assurance; billing-safe access requires the matrix-approved `billingAdmin` or `platformAdmin` path with its required `privileged_control` assurance. The `tenant_regular` tenant-role branch, `billing_safe_tenant`, `cross_tenant_data_bearing`, and newly introduced classes retain the exact membership, membership-generation/version, and target-generation predicates declared by the matrix. Any class or branch not explicitly admitted there fails closed.
