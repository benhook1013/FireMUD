...
The durable usage-charge and fenced-capacity-lease boundary is target state. Current durable charge coverage, exact execution-start accounting, duplicate/recovery reuse, lease fencing and reclamation, and isolated readiness accounting are partial and are not claimed as complete proof. See the [automation and scheduler runtime tracker](../../project-management/implementation-tracking/automation-and-scheduler-runtime.md#capability-status).

## Canonical Design

- [Scripting runtime execution: Budget Charge Points](../system-architecture-scripting-runtime-execution.md#budget-charge-points)
- [Scripting quotas and operations: Budget Accounting Rules](../system-architecture-scripting-quotas-and-operations.md#budget-accounting-rules)
- [Scripting event registry: Registry Entry Contract](../system-architecture-scripting-event-registry.md#registry-entry-contract)
...
Immediately before evaluation, the worker acquires the fenced lease and then uses one Automation-owned durable executor-acceptance transaction to revalidate the current lease fence, durably accept/claim the run for the executor, persist the exactly-once execution-start marker, and transition the work item to `EXECUTING`. Evaluation begins only after that transaction commits. If the executor cannot accept the run, the transition does not commit, the lease is released or reclaimed, and no execution charge is recorded. A crash after commit recovers from the durable executor claim and may reacquire a lease, but never creates a second marker.
