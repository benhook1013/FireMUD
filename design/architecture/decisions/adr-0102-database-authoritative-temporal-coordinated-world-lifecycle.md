# ADR 0102: Database-Authoritative, Temporal-Coordinated World Lifecycle
...
- Consultation: human-led review of lifecycle authority, activation and termination races, pre-activation failure, cross-service cleanup convergence, workflow substrate, and gameplay-path cost
...
When preparation cannot converge, `FAILED_PRE_ACTIVATION` is terminal for admission and activation of that `gameInstanceId`. It is not evidence that cleanup completed. Cleanup progress is recorded separately in durable owner-scoped state, including the cleanup request identity, required durable instance-data owners, their acknowledgements, failures, and last progress time. Cleanup continues or is repaired without changing the failed instance into an admissible state.

Termination reaches `TERMINATED` only after every registered owner of durable data keyed by that `gameInstanceId` has acknowledged its idempotent cleanup obligation. World Management then performs the final fenced transition and retains the terminal lifecycle evidence. Any new durable `gameInstanceId`-owned table or data family must join the common ownership inventory and define its termination cleanup, replacement-state classification, stable operation identity, acknowledgement, retry, and retention behavior before launch paths may write that family. An unregistered or missing owner acknowledgement fails closed rather than being ignored.
