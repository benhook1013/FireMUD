# ADR 0152: Phased Environment-Bound Deployment Preflight and Expected Bindings
...
- Review source: `PREFLIGHT-02`, `PREFLIGHT-01`
- Decision date: 2026-07-20
- Decision keys: `PREFLIGHT-01`, `PREFLIGHT-02`
- Primary capability: `PO-3.1` packaging, CI/CD, deployment, and infrastructure topology
- Affected capabilities: `PO-3.2`, `PO-3.4`, `PO-4.4`, `SF-1.1`, `SF-1.3`
...
Implement modular static, live pre-apply, and post-apply validators. Live validation must compare declared, rendered, and observed bindings using binding-type-appropriate identity rather than raw names alone. It must also bind a complete candidate-resource inventory to the reviewed render and record each resource's pre-apply expectation and observation, including the expected-absent/create case. Post-apply evidence must account for every candidate resource, exact-match its stable observed identity to the reviewed candidate, and enforce the applicable promotion or traffic-open gates. Missing candidate resources, undeclared extras within the candidate-owned set, and expected-absent resources that were not created fail closed.
...
- [`design/operations/environments/README.md`](../../operations/environments/README.md)
