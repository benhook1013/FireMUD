...
- A normalized URI SAN is the canonical ASCII form `scheme://authority/path`: the URI must be absolute with a non-empty scheme and authority, and must have no userinfo, query, fragment, or opaque component; its canonical path component is `/` when the raw path is empty. The scheme and the case-insensitive host portion of the authority are lowercased; the path remains case-sensitive. The authority's port, when present, is decimal and has no leading zeroes, and default ports are not removed. The path is absolute, and a trailing slash is significant (`/service` and `/service/` are different identities); dot-segment resolution is not performed. Every percent escape must be exactly `%` followed by two hexadecimal digits; hex digits are uppercased, percent-encoded unreserved bytes are decoded to their literal form, and reserved bytes such as `/` are never decoded. Percent-encoded URI delimiter octets are invalid identity syntax rather than alternate spellings: `%2F`/`%2f`, `%3A`/`%3a`, `%40`, `%3F`/`%3f`, and `%23` are rejected wherever they occur, and neither the escape nor its decoded delimiter is accepted by extraction, allowlists, renewal, audit, or trusted forwarded context. Non-ASCII, control, invalid UTF-8, malformed URI syntax, and any other form with more than one possible parse are rejected. Certificate extraction, route allowlists, renewal overlap, audit identity, and trusted forwarded mTLS context all use this normalized value and never compare raw SAN spellings.
