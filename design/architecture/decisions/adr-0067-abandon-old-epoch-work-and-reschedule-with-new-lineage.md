...
- Post-review consolidation: the later accepted old-epoch evidence policy merged with ADRs 0051-0054 rules out blanket abandonment; reset scope or epoch age alone never proves that an effect was unapplied.
...
The complete old-epoch nonterminal set is evaluated under each durable record type's canonical lifecycle. An effect-ledger projection is nonterminal unless it is `APPLIED` or `ABANDONED`; cross-region follow-up, result, and coordinator records use [ADR 0066](./adr-0066-durable-asynchronous-cross-region-result-arbitration.md)'s type-specific terminal semantics, while separately linked reconciliation or compensation uses its owning workflow lifecycle. Append-only late-result evidence is not itself outstanding work. The set includes every row that remains nonterminal under its owning predicate, including an effect-ledger `SCHEDULED` row and any durably claimed or staged substate. Every item in that set is completely enumerated and reconciled under an authority-fenced evidence policy. Durable evidence that an effect was applied produces `APPLIED`; durable evidence that it was unapplied and cannot safely remain valid produces `ABANDONED` with an explicit reset or topology reason. Other record types reach only a terminal outcome permitted by their owning lifecycle. Inconclusive work remains explicitly non-terminal and reconciliation-required under its original identity, keeps the affected scope fenced, and blocks reopen until its type-specific terminal decision is supported by authoritative evidence. Reset scope, epoch age, missing responses, and retry exhaustion do not prove `ABANDONED` or any equivalent terminal non-application outcome.
...
Before the affected scope reopens, recovery completely enumerates the old-epoch rows that remain nonterminal under those same type-specific predicates and obtains the authority-fenced terminal evidence required for each row. Redis hints may accelerate discovery but are irrelevant to the completeness of that enumeration.
...
Proof must cover complete durable enumeration under every included record type's canonical terminal predicate—including effect-ledger `SCHEDULED` rows and durably claimed or staged substates—before reopen; effect-ledger `APPLIED` only with durable application evidence; effect-ledger `ABANDONED` only with durable proof that the effect was unapplied and cannot safely remain valid; the owning lifecycle's equivalent evidence requirement for other record types; explicit non-terminal reconciliation state for inconclusive rows; fenced reopen while any required type-specific terminal decision remains unresolved; immutability of old identities, epochs, requests, and terminal outcomes; rejection of rebinding, rewriting, or current-epoch replay; independence from missing or stale Redis hints; authorized post-abandon reconstruction with a new current-epoch identity; lineage linking old and new requests; revalidation of scope, ownership, location, versions, and feature policy; audit visibility for both identities; and idempotent recovery reruns.
