...
Before the affected scope reopens, recovery completely enumerates the old-epoch rows that remain nonterminal under those same type-specific predicates and obtains the authority-fenced terminal evidence or exception required for each row. Redis hints may accelerate discovery but are irrelevant to the completeness of that enumeration.
