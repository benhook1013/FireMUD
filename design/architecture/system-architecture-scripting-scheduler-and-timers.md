...
- Schedules may be preserved across patch or plugin changes only when the old and new definitions share both the same `scheduleDefinitionId` and the same canonical `scheduleSemanticsHash`. Matching by cadence, node shape, entity binding, or other inferred semantic similarity is not sufficient; a changed digest is a definition change and requires a new schedule identity.
