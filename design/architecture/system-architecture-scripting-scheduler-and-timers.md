...
- The same replacement-before-retirement rule applies to plugin activation, disable, rollback, and signer-revocation flows: reconciliation must create or confirm the replacement schedule owned by the new `(pluginVersionId, bindingId)` before retiring the displaced row under the same `controlPlaneRequestId`, and normal scheduling must not resume until that atomic or resumable operation completes.
