...
- Recovery classes are:
  ...
  - **Durable recurring**: the schedule declares exactly one missed-occurrence policy, `SKIP_MISSED` or `COALESCE_ONE`. The latter may create at most one synthetic firing for the logical schedule in one durable resume-window identity.
...
- Missed firings are handled in a bounded, deterministic way:
  - `COALESCE_ONE` schedules may enqueue at most one synthetic firing for that logical schedule in the durable resume window; `SKIP_MISSED` schedules enqueue none. One global fair cap applies across schedules, and excluded candidates are dropped rather than deferred as backlog.
...
- Consequences:
  ...
  - reload and rollback preservation and downtime recovery remain distinct behaviors. The resume rule governs preserved timers after a version transition; the declared recovery class and durable resume-window policy govern missed firings after scheduler downtime within the same logical schedule and version.
