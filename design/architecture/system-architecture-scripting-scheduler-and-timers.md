...
- When leadership changes, the new leader:
  ...
  - for each timer entry in the region index `automation:timer:{tenantRegionTag}`, determines which current-epoch "every N ticks" boundaries were crossed during the gap. Before enqueueing any `onInterval` trigger, it first claims or inserts a durable firing claim keyed by the stable schedule-instance identity, current runtime scope, and tagged due point.
