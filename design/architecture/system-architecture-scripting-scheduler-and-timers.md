...
- During reload, leaders set `reloadState=RELOADING` for the affected runtime scope after observing that Game Session has committed a tenant-`READY` patch and new `scriptPinEpoch` for that instance. This is projection and reconciliation state, not an Automation-owned active selection. `onLoad` is not part of this instance reload path; it has already completed without instance or gameplay side effects as part of tenant patch readiness. Existing timer entries in the region index `automation:timer:{tenantRegionTag}` and any derived per-script projections remain in Redis but are treated as pending until reconciliation completes.
- Once reconciliation succeeds, the leader:
  ...
  - tombstones displaced interval rows and creates fresh target rows by default; only intervals with compatible explicit continuity declarations preserve a row and recalculate `nextTick` or `nextRunAt` using the normative resume rule; and
  - resumes normal scheduling for `onInterval` using the exact committed `(scriptPatchVersion, scriptPinEpoch)`. No interval runs against a partially loaded or substituted script definition.
- If candidate preparation fails before pin commit, Game Session leaves the old pin and epoch unchanged. If loading or reconciliation fails after pin commit, the leader records `reloadState=FAILED` and keeps new timer admission fail-closed; it does not resume the prior schedule set or graph as a local fallback. Recovery requires repair or explicit Game Session repin to a still-`READY` patch with a new epoch.

Timer reconciliation on patch or plugin change is a required part of reload and rollback safety:

- Every compiled interval definition must carry a `scheduleDefinitionId` generated by the Game Design compilation pipeline and persisted with the schedule metadata. Compilation and publish reject duplicate stable-owner/schedule/scope keys deterministically.
- The default transition behavior is cancel-and-recreate: tombstone the displaced interval before it can mint more triggers, then create a fresh target row and due point when the target version still defines the interval.
- Continuity is opt-in on both source and target definitions. Its stable logical key is `{stableOwnerKind, stableOwnerId, scheduleDefinitionId, targetScopeType, targetScopeId}` within tenant and runtime instance. Core scripts use their durable script identity; plugins use `pluginId`, not replaceable `pluginVersionId`.
- Preservation additionally requires compatible interval kind, cadence unit/value, target binding, runtime playable scope, and binding ownership. An absent declaration or target, explicit reset, changed or ambiguous identity, incompatible cadence/target, or displaced owner without a compatible target resets the schedule.
- `scriptPatchVersion` and `pluginVersionId` remain exact provenance and execution-fence metadata. Reconciliation rewrites them to the target version before admission resumes, but they do not define cross-version logical continuity.
- `scheduleSemanticsHash` is diagnostic evidence only. It neither grants continuity nor replaces typed identity and compatibility checks.
- When an instance observes a newly pinned `(scriptPatchVersion, scriptPinEpoch)`, the scheduler must compare the durable schedules for the previously observed pin with those for the newly pinned patch before resuming timer admission.
- An explicitly preserved schedule retains its logical row, rewrites exact target-version ownership, and recalculates its next due point from the prior durable due point and the normative resume rule. It does not retain an overdue point blindly, restart from deployment time, or replay every missed interval.
...
- One-shot timers are not transferred or recreated by this interval rule. They retain their separate correctness-bearing due identity, payload, cancellation, version/epoch fences, and terminal-outcome contract.
...
- If plugin versions `combat-helper@v4` and `combat-helper@v5` both explicitly declare continuity for `pluginId=combat-helper`, `scheduleDefinitionId=combat-helper.guard-aura.v1`, the same target scope, and the compatible 20-tick cadence, reconciliation preserves that durable timer row, rewrites provenance from `pluginVersionId=v4` to `pluginVersionId=v5`, and recalculates the next due point from the resume rule.
...
- Scripts are versioned and published via the Game Design Service; the Game Session Service is the sole authority for the active `(scriptPatchVersion, scriptPinEpoch)` per game.
- When a new script patch is published, the Automation & Scripting Service:
  ...
  - runs tenant-scoped `onLoad` handlers without instance context or gameplay side effects while the patch is still in `PENDING_VALIDATION` / `ONLOAD_RUNNING`;
  - marks the patch `READY` or `FAILED` for the tenant; and only then
  - optionally prepares the exact candidate before pin commit, then uses instance-scoped reload state (for example, `reloadState=RELOADING`) to reconcile the exact version and epoch committed by Game Session.
- During reloads, triggers are paused or skipped while in-flight runs drain under existing concurrency settings:
  ...
  - Pending timer-based triggers remain non-admissible until version reconciliation completes. Target intervals start with fresh due state by default; only compatible explicitly continuous intervals retain cadence through the normative due-point calculation.
- On successful reconciliation, future triggers carry and execute the exact Game Session pin tuple. On failure:
  - candidate failure before commit leaves the current pin and epoch unchanged;
  - failure after commit leaves `reloadState=FAILED` and new scheduling fail-closed until repair or explicit repin, rather than resuming a prior local graph; and
  - triggers referencing a failed, unknown, mismatched, or stale version and epoch are rejected explicitly with `finalOutcome=version_unavailable` (with specific cause in `finalReason`) and metrics like `automation_script_triggers_dropped_total{reason="version_unavailable"}` rather than silently falling back.

- Timer-based triggers such as `onInterval` and `onTimerExpire` always carry the exact `scriptPatchVersion` and `scriptPinEpoch` current at admission and resolve that immutable compiled graph. Work under a displaced epoch is fenced before later handoff or gameplay effect.
- Older immutable script versions remain in the Automation & Scripting Service database for auditing, in-flight exact-version evaluation, and potential explicit rollback, but they are never selected as fallback.
