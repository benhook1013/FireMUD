...
Current implementation note: player-facing production `roll-forward-only` promotion remains non-compliant because the backup path does not emit complete lineage or confidentiality proof and the restore path does not enforce quarantine and environment-wide cold-start convergence. CI/preflight validates the compact compatibility result, its finalized baseline, the expanded backup-readiness top-level envelope, and available bindings, then rejects these promotions until complete nested candidate recovery-controller, participant, confidentiality, hardening, and controlled-reopen validation is implemented.
