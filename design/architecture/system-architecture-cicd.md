...
The `hosted-environment-identity-controller` image follows the same exact-head publication convention but is a trusted infrastructure artifact: it is built and published only from the trusted default-branch workflow, then deployed by digest. PR-controlled source can produce reviewed runtime render artifacts but can never build or deploy the controller image.
