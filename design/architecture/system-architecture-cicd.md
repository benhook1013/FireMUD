...
- Preflight artifacts, secret-compliance snapshots, smoke evidence, and live-state verification are supporting evidence linked from the deployment record rather than parallel sources of truth. Promotion/staging `smokeEvidence` entries are closed `{ref, contentDigest}` objects; the digest covers the exact retained JSON bytes. Each retained smoke artifact preserves `deploymentRef` as the environment-agnostic deployment lineage (for Git-managed staging, the selected `stagingOverlayCommitSha`) and carries a distinct `deploymentEventId` equal to the selected staging deployment event. Recovery-baseline smoke evidence retains its separate list-of-string-refs shape.
  ...
  "smokeEvidence": [
    {
      ...
      "contentDigest": "sha256:<exact-retained-json-bytes>"
    }
  ]
}
```
