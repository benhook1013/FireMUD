...
- The standalone `k8s/velero` manifests are pre-release assets while FireMUD has no player-facing production deployment, so maintenance updates there do not require production promotion evidence. Once a live deployment exists, changing the production overlay and its attestation becomes the authority for the deployed backup projection.
