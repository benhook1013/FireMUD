...
Workflow runtime versions have repository-owned authorities. Node setup reads [`.node-version`](../../.node-version), and the shared Python setup action reads the exact Python 3.14 patch in [`.python-version`](../../.python-version) and installs an explicit repository-owned dependency profile. Workflow Python packages and documentation packages use exact pins in [`config/python`](../../config/python) and [`config/docs/requirements.txt`](../../config/docs/requirements.txt). Operational setup and install steps load kubectl, Helm, GitHub CLI, Buf, Kubeconform, Velero, Actionlint, Trivy, Lychee, ORT, and ZAP versions from [`config/workflow-tool-versions.env`](../../config/workflow-tool-versions.env). Downloaded Linux tool artifacts retain publisher checksums, while the Velero authority and the consumed ORT and ZAP images retain registry digests. Every consumer checks out the repository before reading these files. Renovate uses the `github-release-attachments` datasource to update the version, checksum-version, and archive SHA together for kubectl, Helm, GitHub CLI, Buf, Kubeconform, Trivy, and Lychee; Actionlint remains version-only because it has no archive checksum authority. Velero retains an autonomous image manager so new releases remain visible as Renovate PRs, but its image digest and CLI archive checksum are separate authorities. Each Velero Renovate PR carries a note requiring `dev-tools/maintenance/update-workflow-tool.py velero` with exact image evidence before merge; the fail-closed authority contract rejects the image-only intermediate state until the CLI checksum is updated transactionally. Authority-only updates leave the production Velero CronJob at its known-good tag until a later evidence-backed promotion PR updates that projection with canonical staging, deployment, smoke, recovery, custody, and human-approval evidence. The updater serializes authority transactions through a durable recovery journal, reconciles prepared or committed state before continuing, verifies the Velero digest against Docker Hub, and rolls back the authority and production manifest if either replacement fails; follow the [validation and runtime proof](../developer-workflows/validation-and-runtime-proof.md) guidance for the promotion evidence procedure.
