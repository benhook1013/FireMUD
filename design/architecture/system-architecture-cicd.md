...
Current implementation note: the executable blocks every player-facing production promotion class, including `rollback-compatible`, until complete recovery inventory membership and linked immutable evidence are dereferenced. CI/preflight validates the compact compatibility result, its finalized baseline, the expanded backup-readiness top-level envelope, and available bindings, but no rollback classification becomes current promotion authority until complete nested candidate recovery-controller, participant, confidentiality, hardening, and controlled-reopen validation is implemented.
