...
| --- | --- | --- | --- |
| [`runtime-images.yml`](../../.github/workflows/runtime-images.yml) (`Build Runtime Images`) | Untrusted PR merge commit with read-only repository permissions | Build all fixed-head-SHA runtime images locally, run the one full-stack smoke proof, and upload the successful image artifact | The PR code built and passed full-stack smoke without receiving registry-write credentials. |
...
| [`publish-pr-runtime-images.yml`](../../.github/workflows/publish-pr-runtime-images.yml) (`Publish PR Runtime Images`) | Trusted default-branch `workflow_run` definition | Download only the successful same-repository PR artifact and publish its fixed head-SHA tags to GHCR | GitHub displays the default branch because the trusted publisher runs there; its summary identifies the source PR run and head SHA. |
...
The trust split is deliberate: PR-controlled source can build and execute without package-write authority, while the trusted publisher never checks out or executes PR source. The PR head SHA is the immutable handoff identity across build, publication, and preview.
