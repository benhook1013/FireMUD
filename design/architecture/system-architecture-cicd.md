...
Hosted verification waits for `status.observedGeneration == metadata.generation`, `Ready=True`, the live runtime namespace UID and requested head SHA, rollout and served proof, and non-empty ingress, Telnet, and gRPC revisions before running the existing gameplay smoke. Missing TLS Secrets may leave runtime pods pending during preparation; they must not cause identity issuance or a pre-controller rollout deadlock. This target is local/static contract evidence until a live controller rollout and does not change stage or production deployment.
