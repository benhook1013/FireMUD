...
The trusted hosted lifecycle consumes the sanitized render artifact produced by the PR workflow only after validating its source, run conclusion, head/base binding, checksum, closed schema, exact image tags, and allowlisted Secret consumers. It then prepares the runtime namespace, capacity allocation, and fixed Telnet NodePort with the runtime credential before a requester-only credential applies `HostedEnvironmentIdentity` `Active`. The controller materializes the retained identity namespace and cert-manager Certificate resources; it does not create disposable runtime namespaces. Close/merge cleanup deletes runtime and waits for exact `NotFound` before requesting `Retired`. All uncertain lifecycle/API results fail closed and retain identity.

Hosted verification waits for `status.observedGeneration == metadata.generation`, `Ready=True`, the live runtime namespace UID and requested head SHA, rollout and served proof, and non-empty ingress, Telnet, Gateway-internal WebSocket, TCP Proxy bridge, and gRPC revisions before running the existing gameplay smoke, as defined by the owning [deployment-environments contract](./infrastructure/deployment-environments.md). Missing TLS Secrets may leave runtime pods pending during preparation; they must not cause identity issuance or a pre-controller rollout deadlock. This target is local/static contract evidence until a live controller rollout and does not change stage or production deployment.
