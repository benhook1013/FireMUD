...
Workflow runtime versions have repository-owned authorities. Node setup reads [`.node-version`](../../.node-version), Python setup reads [`.python-version`](../../.python-version), and documentation jobs install the exact MkDocs set in [`config/docs/requirements.txt`](../../config/docs/requirements.txt). Operational setup actions load kubectl, Helm, GitHub CLI, and ORT versions from [`config/workflow-tool-versions.env`](../../config/workflow-tool-versions.env); the Helm and GitHub CLI installers also verify the authority's Linux AMD64 checksums before extraction. Every consumer checks out the repository before reading these files. Renovate owns updates to the language files, documentation requirements, and annotated operational versions; checksum-bearing operational updates remain fail closed until the matching checksum is updated with the version.
