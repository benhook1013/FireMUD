...
- **Deploy hosted Kubernetes development/demo environments** through the dedicated preview and dev-demo workflows. [`preview.yml`](../../.github/workflows/preview.yml) builds credential-free per-PR render artifacts, [`hosted-identity-request.yml`](../../.github/workflows/hosted-identity-request.yml) validates and deploys them from the trusted default branch, and [`dev-demo.yml`](../../.github/workflows/dev-demo.yml) manages the fixed `develop` dev-demo environment. Staging and production clusters use Kustomize overlays as described in the Deployment Runbook; the current production implementation applies them via `kubectl` from a secure admin environment, while the accepted steady-state target is an exact-plan-authorized protected executor.
...
| --- | --- | --- | --- |
| [`runtime-images.yml`](../../.github/workflows/runtime-images.yml) (`Build Runtime Images`) | Untrusted PR merge commit with read-only repository permissions | Build the PR merge commit locally, run applicable full-stack smoke proof, and upload the successful image artifact under an immutable tag keyed to the tested merge SHA | The recorded base/head/merge input was built without receiving registry-write credentials. |
| [`smoke.yml`](../../.github/workflows/smoke.yml) (`PR Smoke Gate`) | PR metadata and Actions API | Decide whether runtime smoke is required and expose the protected `Smoke Gate` by tracking the matching `Build Runtime Images` result | This is a controller and required gate; it does not run a second copy of full-stack smoke. Metadata-only edits use the distinct non-required `PR Metadata Edit (Smoke Gate)` job context, do not dispatch or cancel runtime construction, and may finish while the substantive required gate remains pending; they cannot replace its result. |
| [`publish-pr-runtime-images.yml`](../../.github/workflows/publish-pr-runtime-images.yml) (`Publish PR Runtime Images`) | Trusted default-branch `workflow_run` definition | Verify the current PR and exact source artifact identity, then publish immutable `pr-merge-<merge SHA>` tags in GHCR | GitHub displays the default branch because the trusted publisher runs there; its summary identifies the source PR run and base/head/merge SHAs. |
| [`preview.yml`](../../.github/workflows/preview.yml) (`PR Preview Environment`) | PR-scoped untrusted source run | Render and upload a sanitized full-stack chart artifact without deployment credentials | The artifact is bound to the PR head and recorded base/merge SHAs; PR-controlled code never receives cluster or registry credentials. |
| [`hosted-identity-request.yml`](../../.github/workflows/hosted-identity-request.yml) (`Hosted Identity Request`) | Trusted default-branch workflow | Validate the exact source run, current PR base/head/merge, and artifact, wait for the matching merge-scoped images or proven base images, then deploy or retire the PR namespace and report preview access | Only the trusted job receives protected deployment credentials; it never executes PR-controlled code. |

The trust split is deliberate: PR-controlled source can build and execute without package-write authority, while the trusted publisher never checks out or executes PR source. The current base branch ref and the test merge's ordered base/head parents establish the exact tuple for image publication and preview selection; a pull request event or API `base.sha` can lag the branch ref and is not current-base authority for those actions. The required Smoke Gate accepts a successful credential-free smoke run for its verified PR-event base/head/merge tuple. If GitHub's event base SHA already lags the event merge's parents, the gate instead requires a successful trusted, typed refresh for the verified current base/head/merge tuple. An unrelated base advance after valid event-tuple smoke does not invalidate that tested merge or require the PR head to absorb new dependencies; a changed PR head or base branch requires new proof. The source run records the verified base, head, and merge SHAs, and the merge SHA keys the immutable PR image tag. Ordinary previews remain limited to same-repository, non-dependency-bot pull requests targeting `main` or `develop`. A stacked pull request may enter the same chain only while it is open, mergeable, based on another current branch in the same repository, and carries the maintainer-controlled `preview:priority` label. After a long-lived base branch's CI push run completes, trusted orchestration dispatches a new credential-free merge-image run and preview reconciliation for affected open PRs without changing their heads; scheduled reconciliation provides the equivalent exact-tuple detection for an eligible stacked parent advance. A failed branch CI run cannot publish branch images; the separately dispatched PR merge images can be published only after their own full-stack smoke succeeds and the trusted publisher validates their current eligibility and provenance. Preview selection fails closed until a successful image run for the new merge is published. An ordinary `main`/`develop` PR with no runtime-image change may instead reuse the proven base-commit service images. A stacked PR always uses its exact merge image because its feature-branch base SHA is not a trusted branch-image identity.
...
Workflow runtime versions have repository-owned authorities. Node setup reads [`.node-version`](../../.node-version), and the shared Python setup action reads the exact Python 3.14 patch in [`.python-version`](../../.python-version) and installs an explicit repository-owned dependency profile. Workflow Python packages and documentation packages use exact pins in [`config/python`](../../config/python) and [`config/docs/requirements.txt`](../../config/docs/requirements.txt). Operational setup and install steps load kubectl, Helm, GitHub CLI, Buf, Kubeconform, Velero, Actionlint, Trivy, Lychee, ORT, ZAP, ShellCheck, cloc, and matching Chrome for Testing/ChromeDriver versions from [`config/workflow-tool-versions.env`](../../config/workflow-tool-versions.env). Chrome for Testing and ChromeDriver archive SHA values are repository-recorded pins; upstream metadata supplies their URLs only. Those pins are distinct from publisher-supplied checksums retained for downloaded Linux tool artifacts. The Velero authority and the consumed ORT and ZAP images retain registry digests. Every consumer checks out the repository before reading these files. Renovate uses version-only `github-releases` managers for kubectl and Helm because their releases do not expose the required checksum assets; their checksum fields remain fail-closed until the updater refreshes them. The `github-release-attachments` datasource updates GitHub CLI, Buf, Kubeconform, Trivy, and Lychee version, checksum-version, and archive SHA together; Actionlint remains version-only because it has no archive checksum authority. Velero retains an autonomous image manager so new releases remain visible as Renovate PRs, but its image digest and CLI archive checksum are separate authorities. Each Velero Renovate PR carries a note requiring `dev-tools/maintenance/update-workflow-tool.py velero` with exact image evidence before merge; the fail-closed authority contract rejects the image-only intermediate state until the CLI checksum is updated transactionally. The updater atomically updates the Velero authority, the verifier Dockerfile's pinned Velero stage, and the Terraform Helm projection, which are canonical pre-release plans rather than a live production deployment; the independently promoted backup-verifier CronJob digest is not one of those updater targets. It serializes authority transactions through a durable recovery journal, reconciles prepared or committed state before continuing, verifies the Velero digest against Docker Hub, and rolls back all three updater targets if any replacement fails; follow the [validation and runtime proof](../developer-workflows/validation-and-runtime-proof.md) guidance for future deployment evidence.
...
- [`.coderabbit.yaml`](../../.coderabbit.yaml) supplies repository-local path instructions and summary guidance for manually requested CodeRabbit reviews. Automatic reviews are disabled; the unified `dev-tools/pr-review` controller owns target selection and full-review requests.
...
- Hosted preview and dev-demo deploy, reconciliation, janitor, and cleanup flows run [`check-kubectl-version-skew.sh`](../../dev-tools/hosted/shared/check-kubectl-version-skew.sh) after cluster access succeeds and fail closed when client/server minors differ by more than one. CI also runs [`helm-transaction-proof.sh`](../../dev-tools/validation/helm-transaction-proof.sh) against a pinned disposable k3s server to prove Helm install, upgrade, history, rollback, and status independently of preview capacity.
...
- [`validate-secret-compliance.yml`](../../.github/workflows/validate-secret-compliance.yml) validates secret-compliance records; applicable production-promotion/readiness gates require passing event-scoped evidence, while generic advisory workflow invocations may remain advisory.
...
- [`runtime-images.yml`](../../.github/workflows/runtime-images.yml) builds pull-request images locally and runs applicable full-stack smoke without a registry-write token. After that workflow succeeds, [`publish-pr-runtime-images.yml`](../../.github/workflows/publish-pr-runtime-images.yml) runs from the trusted default-branch workflow definition, verifies the exact source and artifact, and publishes only the immutable `pr-merge-<merge SHA>` tags needed by hosted previews. It never checks out or executes PR source and never writes shared cache or branch tags. Trusted branch images use a candidate tag through full-stack smoke before the tested digests receive the canonical commit-SHA and branch aliases.

---

## Building and Pushing Images

Trusted branch images retain commit-SHA tags. Pull-request images are built locally without registry credentials; only the successful artifact for an exact base/head/merge crosses into the trusted publisher, which pushes immutable `pr-merge-<merge SHA>` tags to **GitHub Container Registry (GHCR)**. Trusted `develop` and `main` builds first publish unique candidate tags, run full-stack smoke against those candidates, then assign commit-SHA and branch aliases to the tested digests. Shared build-cache tags are not deployment evidence.

The `hosted-environment-identity-controller` image is a trusted infrastructure artifact published only by its dedicated job during trusted-source `develop` and `main` runs. That job builds the controller service from the checked-out branch commit against the exact registry-returned `firemud-base` digest produced by its prerequisite job, smokes the exact local controller image, pushes that same image to the dedicated `ghcr.io/benhook1013/firemud-hosted-identity-controller` package, and records SLSA provenance for the registry-returned controller digest. A pre-existing tag is overwritten by the exact smoke-tested build rather than accepted as trusted. Pull-request runs may build and smoke a clearly local controller image without registry credentials, but they exclude it from the exported PR runtime-image artifact, and the trusted PR artifact publisher never publishes it. Runtime deployment accepts only a digest-pinned reference from the dedicated controller package whose GitHub attestation verifies the `runtime-images.yml` signer workflow and a `develop` or `main` source ref.
...
- Each production release must have one canonical release digest manifest under `design/operations/deployments/production/release-manifests/<deployment-ref>.json`, where `<deployment-ref>` is the stable production deployment reference.
- The manifest stores the final `releaseTag` as a field and binds it with the source commit, production deployment reference, production promotion attestation, staging deployment record, release-note/compliance asset refs, and exact service image digest set required by the [Promotion Attestation Contract](./system-architecture-promotion-attestation.md#artifact-format).
...
- If a release tag is created before production promotion completes, the manifest remains at the stable deployment-ref path and is updated with the final `releaseTag` before the release is treated as official.
...
- [`preview.yml`](../../.github/workflows/preview.yml) renders and publishes a sanitized full-stack chart artifact without deployment credentials; the trusted default-branch [`hosted-identity-request.yml`](../../.github/workflows/hosted-identity-request.yml) validates the exact source run, pull-request head, and artifact before deploying per-PR preview namespaces in either certificate-identity mode.
...
- Tear the preview down when the pull request closes or merges, is explicitly released, or its visible bounded renewable lease expires. Eligible same-repository pull requests allocate automatically; ordinary requests remain first-come at the serialized allocation boundary rather than relying on GitHub's raw job-scheduling order. A currently `preview:priority`-labelled request may reclaim the oldest ordinary allocation when the pool is full, but never another priority-labelled allocation; both labels are re-read at the destructive boundary, and the full allocation/deploy/proof/cleanup lifecycle uses one durable bounded non-cancelling queue. The displaced pull request retains ordinary CI and its historical hosted result while reporting no current preview.

For both `standalone` and explicit `hosted-controller` preview modes, the preview source workflow definition runs from the repository default branch on `pull_request_target` or typed `repository_dispatch`; only its GitHub-hosted, read-only render job checks out the PR merge candidate. PR-controlled build and render work has no cluster, registry-write, or hosted-identity requester credentials. The trusted default-branch lifecycle consumes its sanitized render artifact only after validating source-run provenance, current head/base/merge binding, checksum, closed schema, exact image tags, and allowlisted Secret consumers. In `standalone` mode, the trusted lifecycle restores only the canonical cert-manager ingress issuer annotation after artifact validation; the PR render cannot choose it. A manual release publishes an unprivileged destroy intent for the trusted lifecycle; PR close is handled directly by the trusted close event. Privileged preview maintenance and dev-demo operations run only from default-branch schedule, push, workflow-run, pull-request-target, or typed repository-dispatch events, never a branch-selectable privileged manual dispatch. Every secret-consuming hosted job selects the separately protected `trusted-hosted-cluster` GitHub Environment, whose deployment branch policy must allow only `develop`; the legacy GitHub Environments named `pr-preview` and `dev-demo-cluster` must not hold requester credentials. Before live activation, repository-scoped runtime, GHCR, and any remaining hosted-identity credentials must be migrated to the protected Environment or removed and rotated as appropriate. Cluster kubeconfigs are written only to job-temporary files and removed at job end; operators must clear any historical runner-home copy and rotate exposed credentials as needed.

On the explicit `hosted-controller` target path, that lifecycle prepares the runtime namespace, capacity allocation, bounded allocated preview Telnet NodePort (or fixed dev-demo NodePort `32016`), and requested base/head/merge/image tuple annotations with the runtime credential before a requester-only credential applies `HostedEnvironmentIdentity` `Active`. The controller materializes the retained identity namespace and cert-manager Certificate resources; it does not create disposable runtime namespaces. Only successful runtime deployment writes the distinct deployed-head annotation consumed by controller readiness; the trusted workflow writes the separate proof-complete tuple and Namespace UID only after all verification checks and the exact Namespace UID fence succeed. Close/merge cleanup deletes runtime and waits for exact `NotFound` before requesting `Retired`. All uncertain lifecycle/API results fail closed and retain identity.

Hosted verification waits for the complete readiness tuple defined by the owning [deployment-environments contract](./infrastructure/deployment-environments.md) before running the existing gameplay smoke; this CI/CD overview does not duplicate that tuple. In `hosted-controller` mode, preparation creates the runtime Namespace, records its requested base/head/merge/image tuple and port annotations, submits the active identity request, and waits for all five raw controller-projected Secrets before Helm installs or upgrades the runtime workloads. A deployed tuple alone is not reconciled health: after the verification checks and exact Namespace UID fence, the workflow records proof annotations containing the exact base/head/merge/image tuple and live Namespace UID, and the reconciler requires those annotations to match before it treats the preview as aligned. This ordering rule does not relax the separate bootstrap prerequisite: the fixed `firemud-system/firemud-grpc-ca` Secret must already exist before controller activation so the gRPC projection can be issued. `standalone` remains the default/current certificate-identity path until live controller activation and consumer-convergence proof exists; the `hosted-controller` target does not change stage or production deployment.

Main CI remains responsible for stack startup, smoke, and cross-service verification. Preview deployment is intentionally a separate concern focused on reviewer-accessible environments. Capacity exhaustion or infrastructure failure reports preview proof as unavailable; it never records a pass, and any claimed hosted proof is bound to the current pull-request base/head/merge/image tuple.
...
- The standalone `k8s/velero` manifests are pre-release assets while FireMUD has no player-facing production deployment, so maintenance updates there do not require production promotion evidence today. Their checked-in production consumer is `k8s/terraform-production/main.tf`, through `kubernetes_manifest.velero_schedule` and `kubernetes_manifest.velero_verify`; no live player-facing production apply is proven. If that Terraform production stack is later applied live, the exact applied stack plan and its promotion evidence govern the deployment; this does not imply a currently implemented Terraform gate. The production Kustomize overlay and its attestation remain the authority for actual overlay resources.
...
6. Production release publication references one release digest manifest at the stable path `design/operations/deployments/production/release-manifests/<deployment-ref>.json`; the final `releaseTag` is stored as a manifest field.
...
The `jwt-signing-keys-jwks` age record is necessary but not sufficient for JWT readiness. Every production first-live or reopen traffic-open event must itself carry exactly one authenticated custody proof selected by that event (`PREFLIGHT-JWT-INTERIM-001=pass` for `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK` or `PREFLIGHT-JWT-002=pass` for `TARGET_NON_EXPORTABLE_SIGNER`) and current `PREFLIGHT-JWT-ROTATION-001=pass` evidence for that selected custody mode. Separately, any staging deployment record used for production attestation or promotion must carry the matching selected custody and rotation evidence as an additional lineage check; staging lineage never substitutes for event-bound production proof. Interim evidence must prove Account-only asymmetric signing, Account authorization of the materialization controller's bounded private-slot generate/materialize/resource-version-CAS/prune operations, private/public projection correspondence, and validator convergence. Target signer evidence must prove Account-owned signing authority with signer-bound non-exportable custody, authenticated signer generation and health, challenge-signature correspondence with the Account-published JWK, validator convergence, Account-authorized signer retirement for planned rotation and hard cutover for compromise, and that no application workload mounts or receives private material. The two custody proofs are alternatives, not cumulative requirements. Recovery disposition remains event-scoped: `verified_not_restored` may retain the unchanged current key set when current binding, generation, keyset integrity, and validator acceptance are proved; it does not require a replacement `kid`, rejection of the still-current `kid`, or an issuer-generation advance solely for credential evidence. Replacement dispositions instead require the applicable replacement-key acceptance and old-key rejection evidence. The current executable preflight does not yet implement these checks, so mounted JWT/JWKS resources and a fresh credential-age record cannot be used to claim the player-facing JWT gate is satisfied.
...
- **Hosted preview and dev-demo credential boundary**
  - All secret-consuming hosted deploy, verify, cleanup, janitor, reconciler, and dev-demo jobs select the protected GitHub Environment `trusted-hosted-cluster`. Its required deployment branch policy is `develop`, so a pull-request branch cannot authorize these credentials by changing workflow code.
  - `pr-preview` and `dev-demo-cluster` remain Kubernetes target classes; the legacy GitHub Environments of those names are not credential boundaries. The PR source/render path selects no GitHub Environment, and no requester kubeconfig may be stored in either legacy environment.
  - The protected Environment uses distinct secret names: `TRUSTED_HOSTED_PREVIEW_NAMESPACE_MANAGER_KUBECONFIG`, `TRUSTED_HOSTED_PREVIEW_RUNTIME_KUBECONFIG`, `TRUSTED_HOSTED_STANDALONE_CERTIFICATE_KUBECONFIG`, `TRUSTED_HOSTED_IDENTITY_REQUESTER_KUBECONFIG`, `TRUSTED_HOSTED_PREVIEW_GHCR_USERNAME`, and `TRUSTED_HOSTED_PREVIEW_GHCR_TOKEN`. Existing repository-scoped runtime and GHCR credentials must be migrated into those protected names, or removed and rotated, before live activation. Environment creation and static workflow checks do not themselves prove credential cleanup or a live hosted deployment.
