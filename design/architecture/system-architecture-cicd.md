...
- [`.coderabbit.yaml`](../../.coderabbit.yaml) configures CodeRabbit pull request review behavior for the repository. CodeRabbit inherits organization-level defaults, uses the repository-local path instructions and summary guidance, auto-reviews non-draft pull requests targeting `develop` and `main`, auto-pauses incremental reviews after one reviewed commit until a manual resume or review request, and is currently configured as advisory rather than a merge-blocking "request changes" reviewer.
