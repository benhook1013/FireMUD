...
Workflow runtime versions have repository-owned authorities. Node setup reads [`.node-version`](../../.node-version), and the shared Python setup action reads the exact Python 3.14 patch in [`.python-version`](../../.python-version). Workflow Python packages and documentation packages use exact pins in [`config/python`](../../config/python) and [`config/docs/requirements.txt`](../../config/docs/requirements.txt). Operational setup and install steps load kubectl, Helm, GitHub CLI, Buf, Kubeconform, Velero, Actionlint, Trivy, Lychee, ORT, and ZAP versions from [`config/workflow-tool-versions.env`](../../config/workflow-tool-versions.env). Downloaded Linux archives retain publisher checksums, while ORT and ZAP images retain registry digests. Every consumer checks out the repository before reading these files. Renovate owns authority updates, but it cannot derive GitHub release-asset checksums: those version-only proposals fail closed until `dev-tools/maintenance/update-workflow-tool.py` updates the version and checksum together. The static Velero CronJob image is a validated projection of the same Velero authority.
