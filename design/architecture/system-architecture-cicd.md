...
The `hosted-environment-identity-controller` image follows the same exact-head publication convention but is a trusted infrastructure artifact: the PR source workflow may build it during credential-free validation and include the reviewed fixed PR head-SHA artifact/tag in the successful source run; the trusted default-branch publisher may publish that fixed artifact/tag, and runtime deployment consumes only a digest-pinned controller reference. PR-controlled source cannot publish to GHCR or deploy the controller image.
