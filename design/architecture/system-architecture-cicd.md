...
The trust split is deliberate: PR-controlled source can build and execute without package-write authority, while the trusted publisher never checks out or executes PR source. The source run's recorded base, head, and merge SHAs identify the tested input; its merge SHA keys the immutable PR image tag. When a base advances, preview selection fails closed until a successful image run for the new merge is published. A PR with no runtime-image change may instead reuse the proven base-commit service images.
