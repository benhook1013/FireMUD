...
- [`manual-backup-restore.yml`](../../.github/workflows/manual-backup-restore.yml) is wired for an explicitly isolated, namespaced Velero manifest/resource drill using a required, caller-bound control-plane namespace and fixed `firemud` -> `restore-test` mapping. It currently fails closed before cluster operations because no safe checked-in kubeconfig/context credential is bound to the isolated target. Even when externally enabled, it does not produce the canonical environment-wide recovery record, PostgreSQL/full-environment recovery proof, quarantine/controller evidence, or traffic-open authority; it must not mutate live staging or production namespaces.
...
- `manual-backup-restore.yml` is limited to the named isolated `firemud` -> `restore-test` Velero namespace-mapping contract on a pre-provisioned namespace labeled `firemud.io/recovery-drill=isolated`, with a required caller-bound Velero control-plane namespace and external protected kubeconfig/context binding. It must not restore into live staging or production namespaces, create a target namespace, restore Pods or built-in workload controllers, restart ordinary workloads as recovery proof, or hold credentials capable of modifying the currently player-facing cluster boundary. The repository can require the label and read-only workflow permissions, but cannot provision or attest the protected GitHub Environment, dedicated credential custody, isolated cluster, quarantine controller, or recovery-controller evidence; those external configuration and implementation gaps remain blockers to any protected full-recovery claim.
