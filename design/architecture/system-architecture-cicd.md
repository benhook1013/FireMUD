...
- **Hosted preview and dev-demo credential boundary**
  ...
  - The protected Environment uses distinct secret names: `TRUSTED_HOSTED_PREVIEW_KUBECONFIG`, `TRUSTED_HOSTED_PREVIEW_RUNTIME_KUBECONFIG`, `TRUSTED_HOSTED_PREVIEW_GHCR_USERNAME`, `TRUSTED_HOSTED_PREVIEW_GHCR_TOKEN`, and `TRUSTED_HOSTED_IDENTITY_REQUESTER_KUBECONFIG`. Existing repository-scoped runtime and GHCR credentials must be migrated into those protected names, or removed and rotated, before live activation. Environment creation and static workflow checks do not themselves prove credential cleanup or a live hosted deployment.
