...
The replay-cache miss/hit qualifier applies only to the separate Account REST connect-token issuance path. Text `PLAY` does not consult the connect-token replay cache. Its implementation attempts fresh Account membership and, for private/playtest targets, realm-grant reads, but those Game Session gRPC methods currently return `FAILED_PRECONDITION` before data access.

The admission rules and canonical outcomes are owned by [Authentication](../../system-architecture-authentication.md#implementation-status). A connect-token replay-cache miss/hit describes the separate Account REST issuance path: issuance can consume Account membership evidence, while a cached `session:connect-token:*` success can currently bypass fresh entitlement, membership, and pointer rereads. This cache-hit bypass is implementation drift, not fail-closed admission proof. Text `PLAY` is independent of that replay cache, but its current Game Session membership, grant, and entitlement gRPC calls fail with `FAILED_PRECONDITION` before data access. A valid-route text `PLAY` therefore returns `AUTH_UNAVAILABLE` before membership-derived `JOIN_REQUIRED` or `WORLD_ACCESS_DENIED`; direct-text `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before calling Entity. Any downstream membership outcomes described below are conditional implementation branches exercised only with successful mocked Account responses, not currently reachable end-to-end behavior. The exact replay/persistence gap and target revalidation contract remain owned by [Account Runtime and Data](../account-service/runtime-and-data.md#connect-token-issuance-persistence-and-retention).
The compact summary above's replay-cache qualifier is limited to connect-token issuance: text `PLAY` does not depend on connect-token replay-cache state, but its current Account gRPC reads fail before membership or grant checks can complete. The standalone `session_replaced` and `service_restart` preservation and bridge classification described in the target flow are target-only. Current close recognition, fallback, and shutdown classification remain the handler behavior recorded in [Operations Implementation Status](./operations.md#implementation-status).
...
The text `PLAY` implementation checks `membershipExists` and `gameplayAdmissionAllowed` and checks the realm grant where applicable if it receives successful Account responses; its current Game Session Account gRPC calls instead fail before data access. The separate connect-token REST path has its own current route behavior, and its private/playtest grant check remains an implementation and proof gap. These implementation branches do not change the Authentication-owned current outcomes.
The discovery-snapshot shortcut is restricted to the first-party token-backed WebSocket path. Direct text/Telnet currently reaches the abbreviated `LOGIN` -> `PLAY` command path, but valid-route `PLAY` returns `AUTH_UNAVAILABLE` at the Account gRPC boundary before membership results can be evaluated; `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity. The documented `JOIN_REQUIRED` and `WORLD_ACCESS_DENIED` branches are not currently reachable end-to-end and are exercised only with successful mocked Account responses. The target `WORLDS` -> `LOGIN` -> `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY` flow is documented separately and is not current behavior. The current proxy still bootstraps hidden default routing and does not enforce `WORLDS` as a prerequisite. See [Authentication Implementation Status](../../system-architecture-authentication.md#implementation-status) for the canonical admission outcome.

## Cross-Path Connectivity Contract

The following are canonical contracts across Telnet and WebSocket paths:

- Target Telnet discovery-first without typed attach hints follows optional non-discovery `HELP`, mandatory fresh `WORLDS` discovery -> credential-bearing `LOGIN` -> authenticated `REALMS` -> the Authentication-owned conditional `JOIN` when required -> conditional `CHARS`/character creation when no valid selected character exists -> `PLAY`. The local Account JOIN route/RPC and Game Session text dispatch exist, but the full discovery-first text sequence and end-to-end proof remain incomplete; hidden default routing is still bootstrapped. The target `PLAY` path does not create membership; current valid-route text `PLAY` fails at the Account gRPC boundary with `AUTH_UNAVAILABLE` before the conditional membership outcomes. Direct text does not use a first-party WebSocket discovery snapshot.
...
- **Target canonical player flow**
  ...
  - For a public-production target, follow [Authentication](../../system-architecture-authentication.md#direct-text-realms-to-join-scope-normative) for the conditional `JOIN`; its local Account route/RPC and Game Session text dispatch exist, while the full discovery-first sequence and end-to-end proof remain incomplete. Current valid-route text reconnect returns `AUTH_UNAVAILABLE` before membership evaluation. With successful mocked Account responses, downstream code has public missing/non-admitting membership and private/playtest missing/non-admitting membership branches; these are not currently reachable end-to-end. Target behavior returns `JOIN_REQUIRED` for `INACTIVE` public-production membership; only an existing `ACTIVE` public-production member skips `JOIN`. Private/playtest targets do not use public `JOIN`; target admission requires existing `ACTIVE` membership plus the exact current realm grant rather than `allowPublicJoin`.
...
- **Two Telnet windows**
  - Target Window A connects, performs fresh `WORLDS` discovery, issues `LOGIN`, performs authenticated `REALMS` discovery, follows the conditional `JOIN` step if it is first public-production entry, conditionally completes `CHARS`/character creation when no valid selected character exists, and enters gameplay with `PLAY`. Local JOIN dispatch exists; the complete discovery-first sequence remains target-only.
...
- Gateway WebSocket text fragments are reassembled before writing one Telnet line. The proxy bounds each completed Gateway message at 64 KiB of UTF-8 text and closes with `policy_violation` if it exceeds that limit; `TCP_PROXY_MAX_LINE_BYTES` remains the separate inbound Telnet-line limit.
...
- current Telnet runtime uses hidden default routing and accepts the abbreviated `LOGIN` -> `PLAY` -> `LOOK` command path. Valid-route text `PLAY` returns `AUTH_UNAVAILABLE` before membership or grant evaluation, and `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity. The downstream public `JOIN_REQUIRED` and private/playtest `WORLD_ACCESS_DENIED` code branches are exercised only with successful mocked Account responses. Target behavior requires `JOIN_REQUIRED` for `INACTIVE` and skips `JOIN` only for an existing `ACTIVE` public-production member; the full `WORLDS` -> `LOGIN` -> `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY` flow remains target-only;
...
- any future adapter-carried attach hint must remain advisory and must not bypass the current abbreviated `LOGIN` -> `PLAY` -> `LOOK` command path or the target `WORLDS` -> `LOGIN` -> `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY` flow; current text admission fails at the Account gRPC boundary, and only existing `ACTIVE` public-production members skip target `JOIN`.
...
- Public Telnet must use either edge TLS termination with the authenticated internal PROXY listener or direct TCP Proxy TLS without a PROXY header; the two modes are mutually exclusive. Both modes continue through the same Proxy -> Gateway WebSocket bridge. Current Telnet runtime does not implement the target full sequence because hidden default routing is bootstrapped instead and `WORLDS` is not enforced as a prerequisite; the local Account JOIN route/RPC and Game Session text dispatch exist, but valid-route text `PLAY` currently returns `AUTH_UNAVAILABLE` at the Account gRPC boundary. The target sequence is owned by [Authentication](../../system-architecture-authentication.md#login-and-session-flow).
