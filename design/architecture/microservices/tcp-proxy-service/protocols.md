...
- `close_due_to_valid_upstream` – target behavior is to preserve the same Telnet token when the authenticated bridge supplies any valid Gateway top-level close. Clean authenticated `1012/service_restart` uses `planned_drain`; clean authenticated `1000` logout-class closes, including `logout` and `session_replaced` (except planned drain), use `upstream_logout`; other valid or non-clean bridge outcomes use `unattributed_failure` while preserving their Telnet token. Standalone `session_replaced` and `service_restart` preservation/classification remain target-only; current handler mapping is recorded in [Operations Implementation Status](./operations.md#implementation-status).
