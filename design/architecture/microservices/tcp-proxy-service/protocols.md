...
The admission rules are owned by [Authentication](../../system-architecture-authentication.md#login-and-session-flow). Current proxy-routed admission is fail-closed: connect-token issuance and text `PLAY` accept only fresh `membershipLifecycleState=ACTIVE` membership. Eligible missing or `INACTIVE` public-production membership returns `JOIN_REQUIRED`; missing or non-`ACTIVE` membership for a non-public target remains `WORLD_ACCESS_DENIED`. For private/playtest targets, current text `PLAY` also requires the current realm grant, while connect-token issuance does not yet validate that grant. Target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` and `REALM_ACCESS_DENIED` are not current behavior. The target flow below must not be read as proof that explicit join is complete across all clients.
...
The current text `PLAY` path checks gameplay membership admission and the realm grant where applicable, but the Account response currently exposes no lifecycle-state field beyond `membershipExists` and `gameplayAdmissionAllowed`; the connect-token private/playtest grant check remains an implementation and proof gap. These local gaps do not change the Authentication-owned admission outcomes.
