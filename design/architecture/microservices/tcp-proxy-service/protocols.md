...
The replay-cache miss/hit qualifier applies only to connect-token issuance. Text `PLAY` does not consult the connect-token replay cache and always performs its fresh membership check and, for private/playtest targets, its fresh current realm-grant check.

The admission rules are owned by [Authentication](../../system-architecture-authentication.md#login-and-session-flow). Current proxy-routed admission is fail-closed on a connect-token replay-cache miss, where issuance requires fresh Account membership evidence (`membershipExists=true` and `gameplayAdmissionAllowed=true`); a cached `session:connect-token:*` success can currently bypass fresh entitlement, membership, and pointer rereads. This cache-hit bypass is implementation drift, not fail-closed admission proof. Text `PLAY` is independent of connect-token replay-cache state and always performs its fresh membership check and, for private/playtest targets, its applicable current realm-grant check. The exact replay/persistence gap and target revalidation contract remain owned by [Account Runtime and Data](../account-service/runtime-and-data.md#connect-token-issuance-persistence-and-retention). The remaining current and target admission details below must not be read as proof that the cache-hit path is revalidated.
The compact summary above's replay-cache qualifier is limited to connect-token issuance: text `PLAY` always performs fresh membership and applicable private/playtest grant checks, regardless of any connect-token replay-cache state. The standalone `session_replaced` and `service_restart` preservation and bridge classification described in the target flow are target-only. Current close recognition, fallback, and shutdown classification remain the handler behavior recorded in [Operations Implementation Status](./operations.md#implementation-status).
...
- **Target canonical player flow**
  ...
  - For a public-production target, follow [Authentication](../../system-architecture-authentication.md#direct-text-realms-to-join-scope-normative) for the conditional `JOIN`; its local Account route/RPC and Game Session text dispatch exist, while the full discovery-first sequence and end-to-end proof remain incomplete. Current text reconnect may stop with non-actionable `JOIN_REQUIRED` when the current response reports `membershipExists=false` or existing `gameplayAdmissionAllowed=false` for an eligible public-production request; the latter cannot be classified as `INACTIVE`. Target behavior also returns `JOIN_REQUIRED` for `INACTIVE` membership; only an existing `ACTIVE` public-production member skips `JOIN`. Private/playtest targets do not use public `JOIN`; current missing or non-admitting membership returns `WORLD_ACCESS_DENIED`, while target admission requires existing `ACTIVE` membership plus the exact current realm grant rather than `allowPublicJoin`.
...
- current Telnet runtime uses hidden default routing and the abbreviated `LOGIN` -> `PLAY` -> `LOOK` path for existing members. It fails closed with non-actionable `JOIN_REQUIRED` when an otherwise eligible public-production response reports `membershipExists=false` or existing `gameplayAdmissionAllowed=false`; the latter cannot be classified as `INACTIVE`. Target behavior requires `JOIN_REQUIRED` for `INACTIVE` and skips `JOIN` only for an existing `ACTIVE` public-production member. Current private/playtest text `PLAY` maps missing or non-admitting membership to `WORLD_ACCESS_DENIED` and checks the current realm grant; the full `WORLDS` -> `LOGIN` -> `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY` flow remains target-only;
...
- Public Telnet must use either edge TLS termination with the authenticated internal PROXY listener or direct TCP Proxy TLS without a PROXY header; the two modes are mutually exclusive. Both modes continue through the same Proxy -> Gateway WebSocket bridge. Current Telnet runtime does not implement the target full sequence because hidden default routing is bootstrapped instead and `WORLDS` is not enforced as a prerequisite; the local Account JOIN route/RPC and Game Session text dispatch exist, while otherwise admitting existing members may use the abbreviated current `LOGIN` -> `PLAY` -> `LOOK` path. The target sequence is owned by [Authentication](../../system-architecture-authentication.md#login-and-session-flow).
