...
- normal Telnet players perform fresh `WORLDS` discovery, then use `LOGIN`, fresh authenticated `REALMS` discovery as needed, and the Authentication-owned target-only `JOIN` procedure when the target membership snapshot says public-production membership is missing or `INACTIVE`; `CHARS`/character creation is conditional before `PLAY` when no valid selected character exists. Current public-production admission may return non-actionable `JOIN_REQUIRED` only when an otherwise eligible response reports `membershipExists=false`; the current adapter cannot distinguish `INACTIVE` from another existing non-admitting result, and an existing `gameplayAdmissionAllowed=false` response retains its established denial. Current private/playtest text `PLAY` maps missing or non-admitting membership to `WORLD_ACCESS_DENIED`; target private/playtest admission requires existing `ACTIVE` membership and the exact current grant and never uses `allowPublicJoin`;
...
- Public Telnet must use either edge TLS termination with the authenticated internal PROXY listener or direct TCP Proxy TLS without a PROXY header; the two modes are mutually exclusive. Both modes continue through the same Proxy -> Gateway WebSocket bridge. The target Authentication-owned sequence is `WORLDS` -> `LOGIN` -> `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY`, with returning members skipping `JOIN`; current Telnet runtime does not implement that full sequence because explicit `JOIN` is unavailable, hidden default routing is bootstrapped instead, and existing members may use the abbreviated current `LOGIN` -> `PLAY` -> `LOOK` path.
