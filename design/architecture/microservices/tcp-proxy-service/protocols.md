...
- The proxy preserves every valid authenticated Gateway top-level close as the same Telnet token: `logout`, `session_replaced`, `service_restart`, `idle_timeout`, `policy_violation`, `internal_error`, or `backend_unavailable`. For bridge-only operational attribution, clean authenticated `1012/service_restart` uses `bridge_shutdown_class=planned_drain`; clean authenticated `1000` logout-class closes, including `logout` and `session_replaced` (except planned drain), use `upstream_logout`; other valid top-level or non-clean bridge outcomes use `unattributed_failure` while preserving the corresponding Telnet reason. Only absent or invalid top-level close metadata maps to observation-specific `backend_unavailable`. Optional subreasons never redefine the lifecycle class.
...
- `close_due_to_valid_upstream` – if the authenticated bridge supplies any valid Gateway top-level close, preserve the same Telnet token under the translation above. Clean authenticated `1012/service_restart` uses `planned_drain`; clean authenticated `1000` logout-class closes, including `logout` and `session_replaced` (except planned drain), use `upstream_logout`; other valid or non-clean bridge outcomes use `unattributed_failure` while preserving their Telnet token.
