...
| --- | --- | --- | --- |
| Telnet login-first flow | All Telnet clients may optionally browse `WORLDS` before login, then issue `LOGIN`; first public-production entry must issue `JOIN` before `PLAY`, while returning members skip `JOIN`. Telnet shares the same admission pipeline as WebSocket clients. Hidden smart-client attach hints may return later through MCP metadata only; they are not player-facing commands. `LOGIN` / `LOGON` semantics remain canonical in the Authentication & Authorization doc; this row only describes how Telnet traffic is forwarded into that flow. | `LOGIN` and `PLAY` are implemented with focused proof, but explicit `JOIN` is target behavior: current `PLAY` returns `JOIN_REQUIRED` for missing public-production membership, while connect-token issuance remains an implicit membership writer. Manual end-to-end QA also remains. | [Player Access and Session](../../../project-management/implementation-tracking/player-access-and-session.md) |
...
- `readiness` is traffic-admission safety for new Telnet sessions. The service is ready only when:
  ...
  - the current downstream gameplay admission path is not a complete safe first-entry path: `PLAY` now returns `JOIN_REQUIRED` for missing public-production membership, but connect-token issuance may still create it implicitly. The target path is `connect -> LOGIN -> conditional JOIN -> PLAY -> first LOOK`, with returning members taking the `LOGIN -> PLAY` branch; explicit `JOIN` remains unimplemented.
