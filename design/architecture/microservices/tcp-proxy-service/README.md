...
| Proxy -> Gateway WebSocket mTLS | TCP Proxy is the WebSocket mTLS client: it presents the dedicated `FIREMUD_GATEWAY_WS_CLIENT_CERT_CHAIN_PATH` and `FIREMUD_GATEWAY_WS_CLIENT_PRIVATE_KEY_PATH` identity to the Gateway server, with `FIREMUD_GATEWAY_WS_CA_CERT_PATH` for trust, while Gateway verifies the exclusive environment-bound identity required by [ADR 0169](../../decisions/adr-0169-exclusive-environment-bound-tcp-proxy-trust.md). This client identity is distinct from TCP Proxy's internal gRPC server mTLS identity and from the separate Telnet server-TLS identity. | Client-side TLS support exists, but hosted wiring remains plaintext/CIDR-trusted and end-to-end player-facing listener identity proof is incomplete. | [Platform Operations and Delivery](../../../project-management/implementation-tracking/platform-operations-and-delivery.md) |
