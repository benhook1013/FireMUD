...
| Optional classic-client semantic extensions | Plain text remains universal. MCP/GMCP/Telnet semantic negotiation and package support are deferred, disabled, unadvertised, and unsupported; if a future adapter is selected, Game Session owns semantics while TCP Proxy remains a generic bounded bridge. | The current `TCP_PROXY_MCP_ENABLED` marker/greeting seam is implementation drift, not support; it must remain disabled, unadvertised, and fail closed until a Game Session-owned adapter is accepted and proven end to end. No interoperability or end-to-end extension proof exists. | [MCP-01](../../decisions/adr-0145-plain-text-gameplay-and-deferred-classic-client-extensions.md) |
