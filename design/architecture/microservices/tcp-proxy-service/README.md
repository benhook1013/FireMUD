...
- `readiness` is traffic-admission safety for new Telnet sessions. The service is ready only when:
  ...
  - the downstream gameplay admission path supports the canonical first-entry path `connect -> LOGIN <email> [secret] -> conditional JOIN -> PLAY -> first LOOK`, with returning members taking the `LOGIN <email> [secret] -> PLAY` branch.
