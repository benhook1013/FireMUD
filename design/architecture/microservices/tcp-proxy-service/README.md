...
| Proxy -> Gateway WebSocket mTLS | TCP Proxy is the WebSocket mTLS client: it presents the dedicated `FIREMUD_GATEWAY_WS_CLIENT_CERT_CHAIN_PATH` and `FIREMUD_GATEWAY_WS_CLIENT_PRIVATE_KEY_PATH` identity to the Gateway server, with `FIREMUD_GATEWAY_WS_CA_CERT_PATH` for trust, while Gateway verifies the exclusive environment-bound identity required by [ADR 0169](../../decisions/adr-0169-exclusive-environment-bound-tcp-proxy-trust.md). This client identity is distinct from TCP Proxy's internal gRPC server mTLS identity and from the separate Telnet server-TLS identity. | TCP Proxy now loads and rotates the dedicated PEM client identity and Gateway CA into one hostname-verifying client shared by gameplay bridges and Gateway readiness, rejects plaintext unless an explicit local/dev/test profile is active, and fails new admission closed after an invalid reload. The dedicated authenticated Gateway listener and hosted Helm/render-preflight wiring now exist. Controller-owned certificate lifecycle and real hosted peer-handshake, rotation, and withdrawal proof remain incomplete. | [Platform Operations and Delivery](../../../project-management/implementation-tracking/platform-operations-and-delivery.md) |
