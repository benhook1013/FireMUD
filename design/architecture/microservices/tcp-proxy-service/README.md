...
- `readiness` is traffic-admission safety for new Telnet sessions. The service is ready only when:
  ...
  - the current downstream gameplay admission path is safe for `connect -> LOGIN -> conditional JOIN -> PLAY -> first LOOK`; returning members take the `LOGIN -> PLAY` branch, while first public-production entry takes `LOGIN -> JOIN -> PLAY`.
