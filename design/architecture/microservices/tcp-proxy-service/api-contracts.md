...
The proxy may still forward `X-Proxy-Game-Instance-Id` and `X-Proxy-Tenant-Id` when those values come from server-owned defaults or an explicitly selected future transport adapter. It also forwards the coherent server-owned bootstrap routing bundle as `X-World-Slug`, `X-Realm-Slug`, and positive `X-Pointer-Version`; the three routing headers are all present or all absent. This generic metadata is advisory context only and cannot alter authentication, canonical routing, or gameplay-admission authority. Spring Cloud Gateway strips proxy-owned metadata from public ingress, accepts it only on the authenticated TCP Proxy hop, rejects a partial or malformed routing bundle, emits `X-Firemud-Connection-Mode: trusted_tcp_proxy`, and forwards the validated advisory values to Game Session. These headers must never bypass direct credential `LOGIN` + `PLAY`.
