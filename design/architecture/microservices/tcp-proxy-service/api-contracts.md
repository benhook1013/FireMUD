...
After actual edge loss, the Telnet transport carries the owner-defined `WORLDS` -> credential-bearing `LOGIN` -> authenticated `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation when required -> `PLAY` sequence. The proxy may carry the owner-defined conditional `JOIN`, which can create missing or `INACTIVE` public membership only after the canonical fresh checks; `PLAY` requires existing `ACTIVE` membership. The proxy neither decides nor mutates membership. Membership and admission authority remain owned by [Authentication](../../system-architecture-authentication.md#direct-text-realms-to-join-scope-normative) and [Game Session](../game-session-service/protocols.md); this document records only the transport consequence. The proxy does not replay input, frames, bytes, or MCP traffic onto the new transport. Gateway owns the top-level close taxonomy, and a close reason never proves whether a command committed.
