...
The proxy may still forward `X-Proxy-Game-Instance-Id` and `X-Proxy-Tenant-Id` when those values come from server-owned defaults or an explicitly selected future transport adapter. This generic metadata is advisory context only and cannot alter authentication, canonical routing, or gameplay-admission authority. Spring Cloud Gateway strips these from public ingress, emits `X-Firemud-Connection-Mode: trusted_tcp_proxy` on authenticated TCP Proxy bridge hops, and may forward canonical `X-Game-Instance-Id` / `X-Tenant-Id` headers only after authenticating the TCP Proxy identity. These headers remain advisory only and must never bypass `LOGIN` + `PLAY`.
