...
Gateway route mutation APIs are explicitly enabled dev/test-only control-plane surfaces. The current implementation is disabled by default, permits enablement only with exclusively active `dev`/`test` profiles, fails startup for enabled mutation under any other profile set, and guards both REST and gRPC writes. Complete ingress isolation and route-input allowlist safeguards remain target-state gaps; see the [Gateway implementation status](./README.md#implementation-status) and [operational guardrails](./operations.md#dynamic-route-operational-guardrails).
