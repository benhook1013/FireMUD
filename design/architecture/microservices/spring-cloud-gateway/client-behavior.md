...
- Close classes describe transport/session lifecycle only. They never establish the result of an in-flight command; callers with a `commandId` use Game Session's canonical durable `GetGameplayCommandStatus` read scoped by `{tenantId, gameInstanceId, commandId}` (see [ADR 0016](../../decisions/adr-0016-canonical-gameplay-command-status-lifecycle.md)). Close or rebind does not imply completion, and durable status may be `LOST_BEFORE_STAGING`.
