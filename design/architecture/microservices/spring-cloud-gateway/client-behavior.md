...
- First-party gameplay connect-token replay protection is gateway-owned. The gateway atomically stores each consumed connect-token `jti` in non-evicting Coordination Redis under a bounded replay keyspace and retains it through that token's full validity window plus the allowed clock skew; it must not evict a live marker early. If capacity would be exhausted, the gateway fails closed with `CONNECT_REPLAY_PROTECTION_UNAVAILABLE` rather than evicting markers or admitting a handshake. It rejects reuse across gateway instances. Player-facing environments fail closed without that shared coordination authority; only an explicitly classified local test profile may use isolated in-memory replay state with visible drift telemetry.
