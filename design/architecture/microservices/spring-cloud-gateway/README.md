...
- Gateway restarts disconnect gameplay WebSocket clients; clients reconnect by opening a new `/ws/game/**` WebSocket and issuing `LOGIN` again as described in [Reconnection Strategy](../../system-architecture-reconnection.md). Telnet clients typically remain connected to the TCP Proxy Service during brief Gateway restarts, but gameplay pauses while the proxy re-establishes its WebSocket bridge within its bounded reconnect/buffer window; prolonged gateway unavailability results in proxy-side `backend_unavailable` disconnects.
