...
- When the dedicated TCP Proxy TLS listener is enabled, `tcpProxyTlsListenerReadiness` additionally requires that the listener has bound successfully. Invalid listener, environment, trust-profile, expiry, or TLS-material configuration fails startup before readiness can pass.
