...
| WebSocket close and handshake observability | Gateway emits bounded close, handshake-rejection, and slow-client metrics/log classifications so clients and operators can distinguish retry classes and planned vs unattributed loss. | Implemented at the current bridge and first-party handshake boundary. |
