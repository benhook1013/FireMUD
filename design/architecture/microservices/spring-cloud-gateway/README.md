...
- Public Gateway readiness remains scoped to public/gameplay route safety. The dedicated TCP Proxy TLS listener has route-specific admission and is proved separately by reaching readiness through that listener; its `tcpProxyTlsListenerReadiness` indicator reports the listener state without making the public readiness group depend on it. The Gate 2 candidate returns unavailable for new internal bridge requests and internal-listener readiness when watched TLS material is unhealthy, while liveness and already-established bridges remain separate. Invalid listener, environment, trust-profile, expiry, or startup TLS-material configuration still fails startup before either listener can serve traffic.
...
| TCP Proxy bridge admission | Traffic from the TCP Proxy Service always targets `/ws/game/**`; player-facing bridge authority requires the exclusive environment-bound mTLS identity contract in [ADR 0169](../../decisions/adr-0169-exclusive-environment-bound-tcp-proxy-trust.md). | Gateway runtime now provides a separately bound TLS listener, requires CA-validated client certificates for certificate-bound profiles, enforces exact URI SAN, expiring DNS migration, expiring leaf fingerprint, or development-only TLS/CIDR selection exclusively, and promotes bridge headers only after the selected trust check. The #2713 Helm candidate wires the private Gateway ↔ TCP Proxy bridge with its dedicated Service, separate Gateway-server/TCP-Proxy-client Secret mounts and environment, and exact-port NetworkPolicy. The Gate 2 candidate adds watched listener certificate/key and client-CA reload with local tests, but private hosted peer-handshake, served-material convergence/readiness, emergency withdrawal, and player-facing traffic-authorization proof remain incomplete. Public Telnet TLS and transport-complete/playable hosted proof remain the #2795 delivery slice. |
