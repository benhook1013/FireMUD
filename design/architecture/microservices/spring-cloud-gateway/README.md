...
- Gateway restarts disconnect gameplay WebSocket clients; clients reconnect by opening a new `/ws/game/**` WebSocket and issuing `LOGIN` again as described in [Reconnection Strategy](../../system-architecture-reconnection.md). Telnet clients are also disconnected because the TCP Proxy Service fail-closes Telnet sockets when its WebSocket bridge to the gateway drops; clients reconnect and re-`LOGIN`.
