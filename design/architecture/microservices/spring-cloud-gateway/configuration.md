...
The dedicated TCP Proxy TLS listener and exclusive ADR 0169 trust-profile selection are implemented in Gateway runtime. The #2713 Helm candidate wires the private Gateway ↔ TCP Proxy bridge with the listener Service, credential mounts and environment, TCP Proxy client credential consumption, and exact-port NetworkPolicy. The Gate 2 candidate adds live projected listener certificate/key and client-CA reload for new handshakes with local tests; private bridge live hosted-preview proof remains pending. Controller-owned identity issuance/projection, served rotation/readiness, emergency withdrawal evidence, and exact-head hosted peer-handshake proof remain separate incomplete work. Public Telnet TLS and transport-complete/playable hosted proof remain the #2795 delivery slice. Emergency withdrawal still requires deployment-controller proof that every old pod and established bridge was forcibly terminated.
...
### Hosted preview trusted configuration

The trusted default-branch workflow treats the hosted preview Gateway configuration as an exact contract. The rendered `firemud-config` must match the canonical `previewStack.sharedConfig` values from the trusted Helm values file; only the two source-only MinIO credential entries are removed before the artifact is applied. Unknown, missing, or changed shared configuration is rejected. In particular, Gateway bridge and route URI variables, `SPRING_APPLICATION_JSON`, and `JAVA_TOOL_OPTIONS` are not accepted through the shared ConfigMap.

The Gateway Deployment has a matching exact contract for its per-container `env` and `envFrom` fields. PR-controlled `extraEnv` or additional configuration references cannot replace the managed listener/trust settings or introduce an equivalent route or JVM override. This contract is enforced by the hosted artifact validator and must be updated together with the trusted Helm defaults when a legitimate Gateway setting is added.

The hosted Gateway pod also has default-deny egress. Its only permitted destinations are cluster DNS, the five canonical HTTP route services on TCP/8080 (`account-service`, `game-design-service`, `game-session-service`, `logging-admin-service`, and `social-groups-service`), Cache/Rate-Limit Redis on TCP/6379, and the OpenTelemetry collector on TCP/4317. This egress boundary prevents a surviving configuration or runtime override from turning the Gateway into an arbitrary network proxy.
...
An explicit certificate-instance revocation, identity removal, or emergency trust withdrawal of otherwise-unexpired material is a disruptive boundary even with the Gate 2 candidate's live projected-material reload: an old process may retain an established bridge or previously accepted trust. Unlike runtime profile expiry, the deployment controller or authorized control path must terminate every old Gateway pod and its established TCP Proxy bridges even when replacement pods remain unready, so stale in-memory trust cannot continue serving. A rollout request by itself is not proof of withdrawal; completion evidence must show that no old listener process or bridge remains. Ordinary renewal reload has local candidate tests, but hosted served-material convergence is not yet proved.
