...
| `FIREMUD_GATEWAY_TCP_PROXY_TRUST_MIGRATION_OWNER` / `FIREMUD_GATEWAY_TCP_PROXY_TRUST_MIGRATION_REASON` / `FIREMUD_GATEWAY_TCP_PROXY_TRUST_MIGRATION_EXPIRES_AT` | Required ownership, reason, and RFC 3339 expiry for `migration_dns`. Expiry is enforced at startup and on every new admission, then terminates the listener and established bridges. |
| `FIREMUD_GATEWAY_TCP_PROXY_TRUST_FINGERPRINT_SHA256` | One exact leaf SHA-256 fingerprint for `breakglass_fingerprint`. |
| `FIREMUD_GATEWAY_TCP_PROXY_TRUST_INCIDENT_REFERENCE` / `FIREMUD_GATEWAY_TCP_PROXY_TRUST_BREAKGLASS_EXPIRES_AT` | Required incident reference and RFC 3339 expiry for `breakglass_fingerprint`. Expiry is enforced at startup and on every new admission, then terminates the listener and established bridges. |
...
An explicit certificate-instance revocation, identity removal, or emergency trust withdrawal is a disruptive boundary with the current startup-loaded listener. The deployment controller or authorized control path must terminate every old Gateway pod and its established TCP Proxy bridges even when replacement pods remain unready, so stale in-memory trust cannot continue serving. A rollout request by itself is not proof of withdrawal; completion evidence must show that no old listener process or bridge remains. Ordinary certificate renewal and live trust reload remain unimplemented for this listener.
