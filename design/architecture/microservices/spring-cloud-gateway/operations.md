...
Non-`101` `/ws/game/**` handshake failures must emit the canonical bounded handshake error class in the gateway response and structured logs so clients and operators can distinguish `CONNECT_TOKEN_REJECTED`, `POLICY_DENY`, `BACKEND_UNAVAILABLE`, `REPLAY_CHECK_UNAVAILABLE`, and other documented retry classes.
...
- Player-facing environments must fail startup when dynamic route mutation is enabled without those controls.
