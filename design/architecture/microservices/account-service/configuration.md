...
Implementation drift: the current runtime still permits the classpath fallback when the configured file is absent, and the current preflight/manifests still treat signing paths and `jwt-signing-keys` mounts as shared workload configuration rather than enforcing Account-only access. These target-state requirements are not proof of current startup or mount enforcement; runtime, preflight, and manifest alignment is outside this documentation slice.
