...
The fixed `jwt-jwks` ConfigMap has a non-circular Account-owned publication path before Account admission. Account authenticates the initial private-material operation to the materialization controller and remains unready while the controller CAS-materializes the private bundle. Account then derives or receives the operation-bound public JWK, populates the pre-created ConfigMap through its own name-scoped service-account authority, and validates the mounted private/public correspondence before becoming ready. Account uses the same `resourceVersion` CAS authority for every subsequent JWKS change; there is no separate bootstrap writer.
