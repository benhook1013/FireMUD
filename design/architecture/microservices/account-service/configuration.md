...
The local Account response-envelope crypto primitive parses the separate mounted ring format. An unactivated trusted-operator helper can create an initial protected source record exactly once with distinct random purpose keys in an operator-selected owner-only custody directory. Its explicit rotation action reads the protected prior record, retains every old decrypt key, adds distinct active purpose keys, and writes a new predecessor-bound source record without overwriting either file. Ordinary deployment must never run either key-generation action, and owner-only files alone are not durable backed-up custody proof. An unactivated materializer candidate reads the source's exact manifest bytes, environment, namespace, generation/predecessor, immutable source-created freshness anchor, and expiry, then writes the canonical Secret with bounded freshness metadata, exact readback, retry-stable generation handling, and retained decrypt-only keys. No actual protected environment source, trusted writer identity/RBAC, deployment readback, freshness-enforcing readiness actor, or mounted Secret is established yet; the proposed mount remains unpublished. V35 stores an unwired connect-token issuance operation and encrypted envelope, but neither credential producer consumes it. This is local format, storage, bootstrap, rotation-source, and materializer-contract proof only, not response-loss recovery or an enabled credential path.
