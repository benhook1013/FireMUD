...
The local Account response-envelope crypto primitive parses the separate mounted ring format. An unactivated deployment-tool candidate reads one protected source record binding the exact manifest bytes, environment, namespace, generation/predecessor, immutable source-created freshness anchor, and expiry, then materializes the canonical Secret with bounded freshness metadata, exact readback, retry-stable generation handling, and retained decrypt-only keys. No durable protected environment source, trusted writer identity/RBAC, deployment readback, freshness-enforcing readiness actor, or mounted Secret is established yet; the proposed mount remains unpublished. V35 stores an unwired connect-token issuance operation and encrypted envelope, but neither credential producer consumes it. This is format, storage, and local materializer-contract proof only, not response-loss recovery or an enabled credential path.
