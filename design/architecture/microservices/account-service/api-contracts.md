...
- `IssueConnectToken` – issue short-lived gameplay connect token for `/ws/game/**` handshake policy after resolving discovery `connectScopeId`, validating live membership and the current applicable membership authority generation, public admission, runtime entitlements, and the current admission pointer for the target `{tenantId, worldSlug, realmSlug, gameInstanceId}`. At the Account issuance commit gate, it must reread the exact caller-bound runtime membership snapshot from `GetTenantMembershipForRuntime` and compare its opaque `membershipAuthorityGeneration` with the generation used during validation before issuing. The commit-gate snapshot must also retain exact `membershipLifecycleState` and independent `membershipVersion`; any stale, unavailable, regressed, or mismatched read fails closed and no token or `COMMITTED` issuance record is created.
...
- `JoinPublicProductionMembership` – canonical target Account-owned explicit open-enrollment operation selected by verified `connectScopeId`, idempotently creating or returning the durable `player` membership and returning its monotonic `membershipVersion`. Its lifecycle is exact: missing creates `ACTIVE`; `INACTIVE` restores `ACTIVE` while advancing both `membershipAuthorityGeneration` and `membershipVersion`; `ACTIVE` returns the exact current snapshot without an unnecessary advance; every other state rejects and never admits gameplay. The current proto RPC and internal compatibility seams are `EnsurePublicProductionPlayerMembership` and `POST /internal/runtime/public-production-membership`; they accept direct `accountId`, `tenantId`, `worldSlug`, `realmSlug`, and `requestId` fields, and `IssueConnectToken` may still invoke the RPC implicitly when membership is absent. These compatibility seams are not the target operation and remain current-state drift until explicit join converges.
...
- `GetTenantMembershipForRuntime` returns only a fresh Account-authoritative snapshot. A stale, unavailable, regressed, or otherwise unproven read is rejected as `MEMBERSHIP_AUTH_UNAVAILABLE` or represented only as unavailable/denied, never as positive admission authority. It is an internal-only gameplay/runtime authority surface and must not be reused as a caller-facing tenant membership endpoint or as a substitute for `GetCallerTenantMembership`.
