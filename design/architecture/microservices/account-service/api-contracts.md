...
| Pending-deletion access | `/accounts/{accountId}/deletion`, `/accounts/{accountId}/deletion/cancel`, pending-account `/accounts/{accountId}/exports/**`, `/accounts/{accountId}/deletion/billing-settlement` | Opaque `pending-deletion-access` credential through the Account-owned workflow registry | Target-only exact `pending_deletion_scoped` route classification for deletion-specific routes and pending-account export operations. Once the account enters pending deletion, this row takes precedence over ordinary control-plane classification: `control-ui` authority is revoked and cannot access export or any other pending-deletion route. |
...
- The portable-data `/accounts/{accountId}/exports/**` lifecycle is state-dependent, while `DELETE /accounts/{accountId}` is an ordinary subject-bound account route:
  ...
  - Once the account is `deactivated_pending_delete`, export status/content and any pending-deletion export operation use only the dedicated pending-deletion credential and `pending_deletion_scoped` classification; ordinary JWT authority is revoked.
