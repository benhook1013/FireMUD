...
- `JoinPublicProductionMembership` – canonical target Account-owned explicit open-enrollment operation selected by verified `connectScopeId`, idempotently creating or returning the durable `player` membership and returning its monotonic `membershipVersion`. Its lifecycle is exact: missing creates `ACTIVE`; `INACTIVE` restores `ACTIVE` while advancing both `membershipAuthorityGeneration` and `membershipVersion`; each create or restore commits exactly one logical transition audit/outbox event; `ACTIVE` returns the exact current snapshot without an unnecessary advance or event; every other state rejects and never admits gameplay. The current proto RPC and internal compatibility seams are `EnsurePublicProductionPlayerMembership` and `POST /internal/runtime/public-production-membership`; they accept direct `accountId`, `tenantId`, `worldSlug`, `realmSlug`, and `requestId` fields, and `IssueConnectToken` may still invoke the RPC implicitly when membership is absent. These compatibility seams are not the target operation and remain current-state drift until explicit join converges.
  - Required behavior: consume the verified caller-bound `connectScopeId` plus `requestId`, resolve and revalidate the tenant/world/realm/game-instance and exact `{catalogRevision, pointerVersion}` pair at the Account commit gate, permit only the tenant's current default public production realm, fail closed for non-production realms, bind `requestId` to the resolved operation/account/target identity and exact catalog/pointer pair, validate fresh entitlement atomically at the membership commit boundary, and emit exactly one durable transition audit/outbox event for a missing create or `INACTIVE` restore. An `ACTIVE` retry returns the stored exact snapshot and remains event-free. Independently supplied account, tenant, world, or realm values are not player-path authority.
  - Required failure codes at minimum: `JOIN_REQUIRED`, `AUTH_UNAVAILABLE`, `ENTITLEMENT_UNAVAILABLE`, `PUBLIC_PRODUCTION_ADMISSION_DENIED`, `ADMISSION_POINTER_UNAVAILABLE`, `TENANT_BILLING_BLOCKED`.
...
- `GetTenantMembershipForRuntime` returns only a fresh Account-authoritative snapshot. A stale, unavailable, regressed, or otherwise unproven read sets `authorityAvailability=UNAVAILABLE` and is rejected as `MEMBERSHIP_AUTH_UNAVAILABLE` for billing-safe callers or mapped to `AUTH_UNAVAILABLE` for gameplay admission; it is never positive admission authority and must not be interpreted as missing/`INACTIVE` membership. It is an internal-only gameplay/runtime authority surface and must not be reused as a caller-facing tenant membership endpoint or as a substitute for `GetCallerTenantMembership`.
...
- `AUTH_UNAVAILABLE` - required current non-routing, non-entitlement authority, including membership, registry, or grant authority, is unavailable; callers must fail closed and must not reinterpret the result as missing/`INACTIVE` membership.
- `MEMBERSHIP_AUTH_UNAVAILABLE` - authoritative membership/role lookup is unavailable for a billing-safe mutation; gameplay admission maps the same authority outage to `AUTH_UNAVAILABLE`.
