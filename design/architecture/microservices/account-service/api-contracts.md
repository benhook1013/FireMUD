...
- `GetProfile` – retrieves the caller's profile for one explicit tenant after current-membership validation.
- `UpdateProfile` – modifies the caller's profile for one explicit tenant after current-membership validation and triggers notification emails. Account holders may select `PUBLIC`, `FRIENDS_ONLY`, or `PRIVATE` presence visibility; `HIDDEN_STAFF` remains reserved for the staff-visibility owner and cannot be set through ordinary profile writes.
...
| `POST` | `/auth/logout-all` | Idempotently revoke all control-plane, player-bootstrap, and active gameplay authority for the authenticated account by advancing the Account authority generation, projecting `session:auth:generation:account:<accountId>` set-if-greater, and emitting the account-security event; older token records may be cleaned up asynchronously |
...
| `GET` | `/accounts/{accountId}/export` | Start or read the asynchronous versioned full-subject export manifest across all owning services |
...
| `GET` | `/tenants/{tenantId}/profiles/{accountId}` | Retrieve the caller's profile relationship for one tenant/game |
| `PUT` | `/tenants/{tenantId}/profiles/{accountId}` | Update the caller's profile relationship for one tenant/game |
...
| Authenticated account control-plane APIs | `/auth/logout`, `/auth/logout-all`, `/profiles/*`, `/accounts/*/export`, `/accounts/*/tenant-export`, `/accounts/*`, `/tenants/{tenantId}/memberships/me`, `/tenants/{tenantId}/memberships/{accountId}`, `/tenant-admin/tenants/{tenantId}/export` | Exact `control-ui` profile through JWT middleware (`AuthTokenInterceptor` + route classification) | Must enforce route class, subject-binding rules, current authority generations, and explicit tenant/global role checks; UI tenant selection grants no authority. |
| Pending-deletion access | `/accounts/{accountId}/deletion`, `/accounts/{accountId}/deletion/cancel`, `/accounts/{accountId}/deletion/export`, `/accounts/{accountId}/deletion/billing-settlement` | Opaque `pending-deletion-access` credential through the Account-owned workflow registry | Exact `pending_deletion_scoped` route classification; no normal account generation, JWT profile, login, bootstrap, tenant, purchase, connect-token, or gameplay authority. |
...
- `GET /accounts/{accountId}/export` and `DELETE /accounts/{accountId}` are subject-bound account routes:
...
- `GET /tenant-admin/tenants/{tenantId}/export` is the tenant-wide billing-safe export route. It requires the exact `control-ui` profile plus the caller's live `tenantAdmin` membership for the path `tenantId`, remains reachable when that tenant is `suspended` or `canceled`, and returns only tenant-owned exportable records with minimum stable subject references. It has no `accountId` selector and is not subject-bound to one account. The older `GET /accounts/{accountId}/tenant-export?tenantId={tenantId}` shape is implementation drift, not a second canonical export contract.
