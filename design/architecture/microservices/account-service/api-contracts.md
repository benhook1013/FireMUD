...
The full-account versus tenant-scoped export split and the nonterminal billing-owner deletion precondition are implemented at the current Account Service boundary. `ExportAccount` and `DeleteAccount` are account-scoped and no longer accept a caller-selected `tenantId`; `ExportTenantData` is the separate tenant-scoped recovery/export surface. The lifecycle state transitions, recent-auth confirmation, cross-service erasure workflow, retained-record handling, and complete portable export remain target-state gaps; the current successful deletion path is an immediate Account-owned-row hard delete and must not be treated as canonical behavior. Password reset, username reminder, and email-verification tokens are account-scoped rather than tenant-keyed.
...
- `UpdateProfile` – modifies profile fields and triggers notification emails. Account holders may select `PUBLIC`, `FRIENDS_ONLY`, or `PRIVATE` presence visibility. `HIDDEN_STAFF` is not a canonical player policy; staff who enter gameplay use ordinary player presence rules.
