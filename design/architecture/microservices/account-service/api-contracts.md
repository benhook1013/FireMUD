...
| Authenticated account control-plane APIs | `/auth/logout`, `/auth/logout-all`, `/tenants/{tenantId}/profiles/{accountId}`, `/accounts/{accountId}/export`, `/accounts/{accountId}`, `/tenants/{tenantId}/memberships/me`, `/tenants/{tenantId}/memberships/{accountId}`, `/tenant-admin/tenants/{tenantId}/export` | Exact `control-ui` profile through JWT middleware (`AuthTokenInterceptor` + route classification) | Must enforce route class, subject-binding rules, current authority generations, and explicit tenant/global role checks; UI tenant selection grants no authority. |
