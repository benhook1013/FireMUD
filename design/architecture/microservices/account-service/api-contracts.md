...
- `ExportAccount` – starts or reads an asynchronous versioned portable-data export manifest within the defined account scope across all owning services. Complete, partial, omitted, redacted, retryable, and separately retained owner contributions remain explicit in the manifest; this is not a promise of every account-related record or secret.
