...
`RedeemOperatorAuthorizationRequest` contains the opaque reference, the Account-returned `authorizationReferenceFingerprint` transported unchanged through Logging & Admin and the owner, and the owner-recomputed target owner/scope, action-family schema pair, typed target, typed expected version, typed mutation, typed audit reason, `controlPlaneRequestId`, `mutationDigest`, `reservationOwnerId`, and positive `reservationClaimFence`. Account authenticates the exact mTLS caller and requires it to equal the recorded redeemer, then freshly recomputes the fingerprint from the exact reference in the authenticated redemption result. Before the owner treats redemption as successful or authorizes the mutation, it compares the unchanged forwarded fingerprint with Account's fresh recomputation and fails closed on mismatch. A matching retry before expiry returns the same bounded authority projection, fingerprint, and immutable evidence bundle; redemption is not destructively consumed. A different caller, fingerprint, action, schema pair, scope, typed field, request identifier, digest, reservation owner, or claim fence cannot reuse the reference.
