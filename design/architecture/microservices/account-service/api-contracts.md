...
| Authenticated account control-plane APIs | Target `/auth/logout`, `/auth/logout-all`, `/tenants/{tenantId}/profiles/{accountId}`, `/accounts/{accountId}/exports/**`, `/accounts/{accountId}`, `/tenants/{tenantId}/memberships/me`, `/tenants/{tenantId}/memberships/{accountId}`, `/tenant-admin/tenants/{tenantId}/export` | Exact `control-ui` profile through JWT middleware (`AuthTokenInterceptor` + route classification) | This is the target authentication class. The REST inventory identifies which routes are currently implemented and which remain target-only. |
| Pending-deletion access | `/accounts/{accountId}/deletion`, `/accounts/{accountId}/deletion/cancel`, `/accounts/{accountId}/exports/**`, `/accounts/{accountId}/deletion/billing-settlement` | Opaque `pending-deletion-access` credential through the Account-owned workflow registry | Target-only exact `pending_deletion_scoped` route classification; no normal account generation, JWT profile, login, bootstrap, tenant, purchase, connect-token, or gameplay authority. |
