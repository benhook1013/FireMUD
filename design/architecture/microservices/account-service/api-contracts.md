...
- `IssueHumanOperatorAuthorizationReference` – target Account-owned issuance RPC used only by the exact Logging & Admin workload while forwarding the authenticated operator's `control-ui` evidence. The request contains the target owner service, typed target scope, action family, action-family schema identifier/version, `controlPlaneRequestId`, and canonical `mutationDigest`; Account derives the actor account from the forwarded token rather than trusting request-supplied identity, validates current role, assurance, account generation, and the applicable `tenant_generation` or `target_tenant_generation`, and requires the `privileged_control` predicate for the allowed global-role branches before issuing one opaque bounded human reference.
- `IssueAutomationOperatorAuthorizationReference` – target Account-owned issuance RPC used only by the exact Logging & Admin workload with no end-user token or human identity. Account derives `workloadIdentity` solely from the authenticated mTLS transport peer; it is not serialized in the request and caller input cannot supply, override, or impersonate it. The request carries the current `automation_policy_id` and `automation_policy_version`, target owner service, typed target scope, action family, action-family schema identifier/version, `controlPlaneRequestId`, and canonical `mutationDigest`; Account validates the versioned policy and binds the exact target `tenant_generation` for the supported tenant-scoped branch. That branch does not authorize `platformAdmin`, cross-tenant `billingAdmin`, or any operation requiring `privileged_control`; a future automated global branch would require an explicitly defined automation privileged predicate and `target_tenant_generation` binding.
