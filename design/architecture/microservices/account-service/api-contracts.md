...
- `IssueHumanOperatorAuthorizationReference` – target Account-owned human issuance RPC used only by the exact Logging & Admin workload while forwarding the authenticated operator's `control-ui` JWT. It validates the current actor, required role and assurance, derives the actor account and authority generation rather than trusting request-supplied identity, and issues one opaque bounded reference for the requested owner, action family, target scope, `controlPlaneRequestId`, and `mutationDigest`.
- `IssueAutomationOperatorAuthorizationReference` – separate target Account-owned automation issuance RPC used only by the exact Logging & Admin workload under a versioned automation policy. It accepts no end-user JWT or caller-asserted actor, prohibits current global-role branches, and binds the exact workload, policy identity/version, tenant generation, owner, action family/schema, scope, `controlPlaneRequestId`, and `mutationDigest`.
...
`IssueHumanOperatorAuthorizationReferenceRequest` contains the target owner service, typed target scope, action family and schema identity, `controlPlaneRequestId`, and canonical `mutationDigest`. Actor identity is not a request field: Account derives it from the validated `control-ui` token forwarded by the exact Logging & Admin mTLS workload. `IssueAutomationOperatorAuthorizationReferenceRequest` carries the same mutation identity plus the exact automation policy identity/version; Account derives and validates the caller workload and accepts only policy-listed tenant-scoped action families. Account chooses a short expiry and stores only a hash of the high-entropy opaque reference, bound to the applicable human actor/role/assurance or automation workload/policy evidence, issuer and redeemer workload identities, target owner/scope, action family/schema, request identity, digest, issue time, expiry, and revocation state. Each typed response contains only `operatorAuthorizationReference` and `expiresAt`.
