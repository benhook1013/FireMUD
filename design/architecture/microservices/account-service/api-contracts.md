...
- Pending-deletion access is a separate opaque Account credential, not a JWT or normal account session. The deletion-specific routes are `GET /accounts/{accountId}/deletion`, `POST /accounts/{accountId}/deletion/cancel`, `GET /accounts/{accountId}/deletion/export`, and `POST /accounts/{accountId}/deletion/billing-settlement`; each is classified `pending_deletion_scoped` for its exact action and bound to the Account-owned deletion workflow registry. The account export lifecycle is state-dependent: while the account is active and eligible for ordinary authority, it uses the ordinary authenticated account subject; once the account is pending deletion, its export operation uses only `GET /accounts/{accountId}/deletion/export` with the pending-deletion credential and route classification. These target routes are not currently routable.
...
| `POST` | `/accounts/{accountId}/exports` | **Target-state only.** Initiate or idempotently replay one asynchronous portable-data export within the defined account scope and return its stable `exportId` and status resource. |
| `GET` | `/accounts/{accountId}/exports/{exportId}` | **Target-state only.** Read durable portable-data export status, owner contribution state, and the versioned manifest. |
...
| `GET` | `/accounts/{accountId}/deletion/export` | **Target-state only.** Return the asynchronous versioned cross-owner export manifest using only the Account-owned pending-deletion credential. |
...
| Authenticated account control-plane APIs | Target `/auth/logout`, `/auth/logout-all`, `/tenants/{tenantId}/profiles/{accountId}`, active-account `/accounts/{accountId}/exports/**`, `/accounts/{accountId}`, `/tenants/{tenantId}/memberships/me`, `/tenants/{tenantId}/memberships/{accountId}`, `/tenant-admin/tenants/{tenantId}/export` | Exact ordinary account subject through the declared `control-ui` JWT route class | This is the target authentication class only while the account remains eligible for ordinary authority. The export lifecycle uses the authenticated account subject while active; the REST inventory identifies which routes are currently implemented and which remain target-only. |
| Pending-deletion access | `/accounts/{accountId}/deletion`, `/accounts/{accountId}/deletion/cancel`, `/accounts/{accountId}/deletion/export`, `/accounts/{accountId}/deletion/billing-settlement` | Opaque `pending-deletion-access` credential through the Account-owned workflow registry | Target-only exact `pending_deletion_scoped` route classification for deletion-specific routes and the pending-deletion export operation. Once the account enters pending deletion, this row takes precedence over ordinary control-plane classification: `control-ui` authority is revoked and cannot access export or any other pending-deletion route. |
...
- The portable-data `/accounts/{accountId}/exports/**` lifecycle is state-dependent, while `DELETE /accounts/{accountId}` is an ordinary subject-bound account route. Pending-deletion export is the separate `GET /accounts/{accountId}/deletion/export` action:
  ...
  - Once the account is `deactivated_pending_delete`, active export status/content is no longer an ordinary JWT operation; the pending-deletion export operation is only `GET /accounts/{accountId}/deletion/export` with the dedicated pending-deletion credential and `pending_deletion_scoped` classification.
