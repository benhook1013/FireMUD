...
- A customer-wide provider default may be used only to bootstrap initial customer or payment-method setup. It must never activate or charge a FireMUD subscription. Provider webhook processing and reconciliation verify the persisted per-subscription binding for activation and every charge, including renewals; a missing or mismatched binding fails closed.
