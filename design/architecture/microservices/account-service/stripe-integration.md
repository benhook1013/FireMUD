...
3. Account atomically persists the subscription’s owner, provider customer, and non-null subscription-specific instrument binding in one database transaction before creating the provider subscription. The provider call must use those persisted identifiers; Stripe customer defaults and other provider defaults are prohibited. A missing, stale, or mismatched binding fails closed and cannot be repaired by a provider default.
4. After the binding transaction commits, Account creates or updates the provider subscription and records its Stripe `subscription` ID and local `status`. Stripe webhook and reconciliation processing rechecks the persisted binding before accepting activation or any invoice charge, including renewals.
