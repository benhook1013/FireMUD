...
- **Subscription**  
  - Keyed by internal ID plus `accountId`, `tenantId`, and `plan_code`. The current `accountId` is the subscription's billing-owner subject binding; it is changed only by an explicit audited billing-owner handoff and is not an ad-hoc tenant role.
