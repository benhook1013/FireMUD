...
The target `CreateSubscription` caller boundary uses a high-entropy stable `requestId` as the sole logical create-operation key. Account computes a versioned `requestDigest` from the normalized create tuple, including the resolved billing owner, immutable `tenantId`, initial `plan_code`, and exact instrument-selection input; clients do not supply that digest. Account persists the pair before any provider call. A retry with the same `requestId` and normalized create fields recomputes the same digest and replays the same `subscriptionId`, durable operation, and outcome; changed fields that produce a different digest return `IDEMPOTENCY_CONFLICT`. The provider `subscription_provisioning_request_id` and cleanup identity remain separate provider-operation keys and must not replace the caller identity.
...
- Freshness and sequencing metadata:
  ...
  - positive `bundleVersion` plus opaque `snapshotIdentity` and `evaluationIdentity`, and
  - the complete applicable `authorityTuple`, exact applicable `tenantBillingCutoff`, and complete `outboxCheckpoints[]`.
...
- A strict commitment captures one canonical Account-authenticated entitlement-admission evidence bundle from the fresh evaluation: `bundleVersion`, `snapshotIdentity`, `evaluationIdentity`, every entitlement field, the complete applicable `authorityTuple` including the exact `tenantAuthorityGeneration` value for this tenant, the separate applicable `tenantBillingCutoff`, the separate `entitlementVersion`, and the complete exact billing checkpoint evidence. The billing cutoff carries `tenantBillingSequence`, `outboxStreamKey`, and `outboxSequence`, and its stream/sequence must equal the matching entry in `outboxCheckpoints[]`; none of these values may be assembled from separate entitlement, authority, billing, or projection reads. An authority, entitlement, billing-sequence, or checkpoint advance between evaluation and commit causes a retry or fail-closed rejection; cache invalidation alone is not a commitment fence.
- Strict capacity admission has an Account-owned final commit fence rather than relying on a caller's conditional read. The target internal `CommitTenantCapacityAdmission` RPC is called only by Game Session or World Management after their exact owner-specific context has been authenticated; it accepts stable outer `(tenantId, requestId, admissionId)`, fresh per-attempt `admissionAttemptId`, current `capacityRearmFence`, the exact Account-authenticated entitlement-admission evidence bundle (`authorityTuple`, applicable `tenantBillingCutoff`, `outboxCheckpoints[]`, `entitlementVersion`, positive `bundleVersion`, and its snapshot/evaluation identity), a bounded capacity delta, and the required versioned `mutationDigest` over that attempt identity, fence, and complete payload. Account rejects missing, contradictory, separately sourced, stale, regressed, or checkpoint-incomplete evidence before quota mutation. In one Account database transaction it locks the tenant entitlement/usage authority, authenticates the supplied evidence against the current tenant lifecycle, entitlement, cutoff, and checkpoint state, and creates or replays an idempotent capacity reservation only if the evidence and quota remain valid. It returns a stable commitment ID and the exact committed evidence; stale evidence returns `CAPACITY_ADMISSION_STALE`, an exhausted quota returns `CAPACITY_QUOTA_EXCEEDED`, and neither creates a reservation. The initial attempt needs no rearm CAS. A new attempt requires an Account-owned atomic CAS over the same outer tuple, terminal `OWNER_EXECUTION/NOT_EXECUTED_AFTER_AUTHORIZATION` plus durable no-commit evidence, current fence, and prior digest; the CAS advances the fence and binds the fresh attempt without allocating a second reservation or capacity claim. Exact retries of the same outer tuple, attempt ID, fence, complete evidence/payload, and digest replay the stored result; changed fields without a matching rearm return `IDEMPOTENCY_CONFLICT`, while stale or losing fences return `FENCE_REJECTED` only with durable no-accept/no-runtime-mutation proof. The reservation counts against capacity until the owner finalizes or Account releases it through the same fenced idempotent operation. Game Session or World Management must present that commitment to its own fenced resource commit and may not claim strict capacity safety from a snapshot or local recheck alone. The current runtime has no durable authority/usage ledger, so this remains target behavior rather than a live implementation claim. See the canonical [capacity-attempt proof obligations](../../system-architecture-versioning-runtime.md#focused-capacity-attempt-proof-obligations).
