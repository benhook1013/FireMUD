...
Hosted-terms acceptance and the tenant's Creator Party are separate from subscription and billing ownership. Current V1 subscription behavior remains the hosting-billing boundary in [ADR 0143](../../decisions/adr-0143-stripe-v1-hosting-billing-and-deferred-creator-monetization.md). Under [ADR 0181](../../decisions/adr-0181-changed-hosted-terms-decline-and-existing-content-continuity.md), decline or missing reacceptance blocks new covered creator-content mutations but does not itself cancel a subscription, delete content, or decide a refund; those mutations still require the Account-owned currentness outcome. The exact renewal, paid-through continuity, exit, refund, and service-credit choreography across a material terms change is an ADR 0181 candidate pre-launch policy pending complete terms and New Zealand legal review, not a settled subscription rule. No current route or implementation proves these changed-term behaviors.
...
- When a subscription is `suspended` or `canceled`:
  ...
  - The route-matrix-declared billing-safe tenant, cross-tenant support-safe, and cross-tenant billing-safe control-plane routes remain accessible so authorized callers can resolve billing issues, troubleshoot, or export tenant-scoped data. They remain subject to their own live membership, role, and assurance checks; this exception does not preserve ordinary tenant or gameplay authority. The surface includes actions such as updating payment methods, viewing invoices, and initiating tenant-bounded exports, but does not include full account export, starting game instances, or editing live gameplay configuration.
...
These behaviors tie directly into the revocation rules in [Session Behavior](../../system-architecture-session-behavior.md#session-and-identity-management): `TenantBillingStateChanged` events for `suspended` or `canceled` must trigger revocation of gameplay sessions and ordinary tenant-scoped authority, while preserving the route-matrix-declared billing-safe and support-safe control-plane paths for callers that still pass those paths' own checks. Softer billing states (`trialing`, `active`, `past_due`, `grace`) do not trigger automatic revocation and instead rely on quota and availability rules.
