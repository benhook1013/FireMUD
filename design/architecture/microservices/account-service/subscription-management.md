...
Saved payment instruments are account-owned and managed only by the authenticated billing-owner subject or an explicitly audited cross-tenant billing administrator. A tenant role alone cannot inspect or mutate another account’s instruments. Each subscription binds its chosen instrument explicitly; changing one subscription does not alter another. Detachment is blocked until every referencing subscription has a successfully installed replacement, and billing-owner transfer uses the new owner’s Stripe customer without transferring saved instruments.
...
- When a subscription is `suspended` or `canceled`:
  ...
  - As part of the transition into `suspended` or `canceled`, the Account Service must:
    ...
    - Advance the durable tenant auth generation and idempotently project `session:auth:generation:tenant:<tenantId>` set-if-greater; the cutoff workflow does not report enforcement complete until this projection succeeds.
  ...
  - Downstream services must not write `session:auth:generation:*` keys directly. Implementations must not scan `session:auth:token:*` in hot paths; opportunistic cleanup of older token records is allowed only via purpose-built, bounded Account-owned indexes and background work.
...
- Existing uninterrupted sessions do not check entitlements per action. Hard billing events still revoke them through the canonical event/generation flow.
