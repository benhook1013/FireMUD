...
Full-account export and tenant-scoped export are separate contracts. `ExportAccount` durably initiates an asynchronous operation, gathers every required owning-service contribution of portable subject data across tenants that the caller is entitled to receive, and permits manifest retrieval only after owner fan-in completes. Missing, omitted, failed, or separately retained categories remain explicit. Tenant billing-safe export is a separate tenant-scoped route for `tenantAdmin` recovery while a tenant is billing-blocked; it returns only tenant-owned exportable records and minimum subject references and must not expose global email, credentials, external identities, security state, or unrelated account data from other tenants.
