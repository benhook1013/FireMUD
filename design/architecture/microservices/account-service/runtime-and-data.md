...
- Provides a JWKS endpoint for other services to validate tokens. Account Service is the one canonical JWT rotation authority: it owns signing-key lifecycle, token issuance/rotation, JWKS publication, and private-key custody. Kubernetes Jobs or CronJobs may request Account-owned rotation, observe convergence, and retain evidence, but they do not own rotation, read or write private keys, or publish JWKS, as described in the [Security Architecture](../../system-architecture-security.md).
