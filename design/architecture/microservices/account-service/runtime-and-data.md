...
If Game Session loses ownership, its replacement-binding compare-and-set fails, or the refresh response is lost after Account created the replacement, Game Session must call an Account-owned idempotent abort/retire operation with the replacement token identity, lineage, rotation request ID, and request digest. Account marks that replacement aborted/retired and removes or revokes its registry record; a repeated matching request returns the stored result and a mismatched digest is rejected. Recovery reconciles the stable operation ID, so an orphaned replacement cannot remain accepted beyond the bounded rotation-overlap and cleanup horizon.

Per-token logout deletes only the presented token's one registry record and is idempotent. Logout-all commits a distinct durable account-wide logout/audit event and then idempotently advances the account authority generation; it does not depend on scanning token keys. Raw token values are excluded from both audit forms. The account-wide event terminates active gameplay under ADR 0030, whereas per-token logout leaves other devices and unrelated gameplay bindings intact.
