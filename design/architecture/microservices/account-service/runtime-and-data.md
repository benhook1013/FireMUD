...
- The producer proof must show that the event's `membershipAuthorityGeneration` is the same committed generation advanced and projected by Account for the membership change; it must not be reconstructed from `membershipVersion` or a projection read after commit. Consumers treat an event as a duplicate only when its opaque generation exactly equals the already-authorized generation. They must not compare generations as older/newer, infer ordering from their representation, or accept a mismatched generation as authoritative. Any mismatch, including a value that appears older or newer, requires reconciliation through Account or an Account-owned comparison API before the consumer admits or retains authority.
