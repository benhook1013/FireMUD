...
- Telnet and other non-WebSocket text transports authenticate through the credential-bearing `LOGIN` flow described in [Authentication & Authorization](../../system-architecture-authentication.md) and do not carry public JWTs or gameplay connect tokens. Explicitly classified public non-browser WebSocket clients establish identity through the `player-bootstrap` flow, present the one-use gameplay connect token only in the `X-Firemud-Connect-Token` handshake header, and then use bare `LOGIN` followed by `PLAY`. First-party browser/mobile gameplay clients use the short-lived `player-bootstrap` profile for HTTPS bootstrap and an HttpOnly-cookie connect token for the WebSocket handshake. The Game Session Service calls the internal `Authenticate` gRPC method (mTLS-protected) to verify credential-bearing login and obtain `game-session-account-delegation` JWTs for Account, while `/auth/login` remains a browser/control-plane endpoint.
