...
- Bootstrap discovery surfaces must return the target snapshot `connectScopeId` together with `tenantId`, `worldSlug`, `realmSlug`, `gameInstanceId`, the exact `catalogRevision` and `pointerVersion` pair, `evaluatedAt`, and `connectScopeExpiresAt`.
- The discovery snapshot is short-lived proof of the evaluated target, not a reservation. `JoinPublicProductionMembership` must re-resolve the selected target and require the exact discovery `catalogRevision`/`pointerVersion` pair at the membership commit gate. Missing proto or caller support for carrying and proving this complete snapshot through that gate is implementation drift.
...
- `JoinPublicProductionMembership` is the sole target canonical name for the authoritative explicit open-enrollment membership operation. Its target request accepts only a verified `connectScopeId` and `requestId`; Account derives `accountId` and the target `{tenantId, worldSlug, realmSlug}` from the signed authority bound to that scope. Connect-token issuance and `PLAY` must require its result but never invoke it implicitly. Current implementation drift only: the live proto RPC is named `EnsurePublicProductionPlayerMembership`, with the separate direct-field seam described in the API contract; it is not a target alias.
