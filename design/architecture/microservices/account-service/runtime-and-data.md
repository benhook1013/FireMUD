...
Deletion must not blindly hard-delete billing, payment, refund, Stripe customer, tax, fraud, or audit records that must survive for settlement and compliance; those records retain the minimum account reference and access needed for reconciliation. Exact record categories, legal bases, and durations are owned by the canonical `DATA-01` retention decision rather than the deletion endpoint.
