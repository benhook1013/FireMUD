...
Account-scoped portable-data export and tenant-scoped export are separate contracts. `ExportAccount` durably initiates an asynchronous operation, gathers every required owning-service contribution of portable subject data within the defined account scope that the caller is entitled to receive, and permits manifest retrieval only after owner fan-in completes. Missing, omitted, failed, or separately retained categories remain explicit; the manifest is not a promise to return every account-related record, secret, retained evidence, or provider object. Tenant billing-safe export is a separate tenant-scoped route for `tenantAdmin` recovery while a tenant is billing-blocked; it returns only tenant-owned exportable records and minimum subject references and must not expose global email, credentials, external identities, security state, or unrelated account data from other tenants.
...
- Membership responses include `evaluatedAt` and `membershipVersion` so callers can verify freshness and detect stale reads. The target `GetTenantMembershipForRuntime` response additionally carries an opaque `membershipAuthorityGeneration`; callers do not interpret Account-owned generation values.
...
- The focused producer proof must show that the event's `membershipAuthorityGeneration` is the same committed generation advanced and projected by Account for the membership change; it must not be reconstructed from `membershipVersion` or a projection read after commit. Consumers deduplicate by `eventId` and order or gap-check by `membershipVersion`. The opaque `membershipAuthorityGeneration` is only a freshness fence: consumers compare it for exact equality with the Account-authorized value and must not compare generations as older/newer, infer ordering from their representation, or use it as the event dedupe key. Any mismatch requires reconciliation through Account or an Account-owned comparison API before the consumer admits or retains authority.
