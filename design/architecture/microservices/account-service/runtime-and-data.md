...
- `GetTenantMembershipForRuntime(accountId, tenantId)` is the authoritative internal membership surface for gameplay/runtime flows.
  ...
  - Target minimum response fields: `accountId`, `tenantId`, `roles[]`, `membershipLifecycleState`, `authorityAvailability`, `gameplayAdmissionAllowed`, `membershipVersion`, `membershipAuthorityGeneration`, and `evaluatedAt`.
...
- `GetTenantEntitlementsForRuntime(tenantId)` is the authoritative internal entitlement surface for gameplay/runtime flows. Runtime callers may cache its positive result only under the strict-new-commitment and bounded-continuity rules in [ADR 0028](../../decisions/adr-0028-differentiated-entitlement-freshness.md); the cache never becomes a second writer.
  ...
  - The target authority response includes both `authorityAvailability` and `tenantAuthorityGeneration`. The former says whether fresh entitlement authority was established; the latter is an opaque tenant-authority fence that callers compare exactly and never interpret.
  - Current implementation/proto/caller status: support for this paired target authority contract is not yet fully aligned across the live response, generated proto, and runtime callers. That is implementation drift, not a change to the Account-owned entitlement authority.
...
- `IssueConnectToken` / `POST /auth/connect-token` is the target gameplay bootstrap token-issuance surface.
  ...
  - For a selected public-production target, after required current routing and authority checks, `PUBLIC_PRODUCTION_ADMISSION_DENIED` takes precedence when the public-production joining policy disallows joining. `JOIN_REQUIRED` is returned only when that policy permits joining but the fresh membership snapshot is missing or `INACTIVE`; unavailable authority remains a distinct failure.
  ...
  - Response fields for all clients are non-secret metadata only: `expiresAt`, `accountId`, `tenantId`, `realmSlug`, `gameInstanceId`, and `issuedAt`. The token `jti` remains only in the dedicated issuance/replay record and access-controlled security audit evidence; it is not returned to clients.
  - Account delivers the current connect token only as `Set-Cookie: Firemud-Connect-Token=...` with the gateway-required security attributes; the token is never returned in the response body or a response-header fallback. Browser/mobile clients and first-party non-browser clients with a protected cookie jar retain and use the cookie normally. A future explicitly classified public/non-first-party WebSocket integration may use a dedicated one-time secure-storage response and handshake header only through the separately registered target route defined by ADR 0029; it does not convert this cookie endpoint into a header fallback.
- Bootstrap discovery surfaces must return the target snapshot `connectScopeId`, which cryptographically binds the exact `tenantId`, `worldSlug`, `realmSlug`, `gameInstanceId`, `catalogRevision`, `pointerVersion`, and `connectScopeExpiresAt`, together with `evaluatedAt`.
  - `connectScopeId` is short-lived cryptographic proof of the exact evaluated target, not a reservation. `JoinPublicProductionMembership` accepts the unchanged `connectScopeId` and `requestId`; `connectScopeId` exclusively binds the target and exact `catalogRevision`/`pointerVersion` pair, so the request has no duplicate catalog or pointer fields. At the membership commit gate, Account verifies the scope's signature, expiry, and exact bound pair against authoritative current state. A changed pair is a stale-scope failure, and Account must not silently re-resolve or select a newer target. Missing proto or caller support for carrying and proving this complete bound snapshot through that gate is implementation drift.
...
- `JoinPublicProductionMembership` is the sole target canonical name for the authoritative explicit open-enrollment membership operation. Its target request preserves exactly `connectScopeId` and `requestId`; Account derives `accountId` and the target `{tenantId, worldSlug, realmSlug}` from the cryptographic authority bound to that scope and verifies the exact bound catalog/pointer pair at commit. Connect-token issuance and `PLAY` must require its result but never invoke it implicitly. Current implementation drift only: the live proto RPC is named `EnsurePublicProductionPlayerMembership`, with the separate direct-field seam described in the API contract; it is not a target alias.
  ...
  - `requestId` is the attempt idempotency key and `connectScopeId` remains a required request field. The canonical join idempotency digest includes the exact `connectScopeId`, including its cryptographically bound `catalogRevision`/`pointerVersion` pair. Retrying the same `{connectScopeId, requestId}` must return the same resulting membership identity/version or the same deterministic application failure.
  - The resulting membership must be immediately visible to `GetTenantMembershipForRuntime`.
  - Minimum preconditions: the selected realm is still the tenant's default public production realm, it remains publicly visible to the caller, `authorityAvailability=AVAILABLE`, `allowPublicJoin=true`, current runtime entitlements still allow gameplay admission, and authoritative admission-pointer state exactly matches the `catalogRevision`/`pointerVersion` pair cryptographically bound in `connectScopeId`; a newer target is not silently selected. The membership freshness baseline (`membershipAuthorityGeneration`, independent `membershipVersion`, and lifecycle state) is bound to the same `connectScopeId` and `requestId` from issuance through this commit gate.
  ...
  - Required failure codes at minimum: `PUBLIC_PRODUCTION_ADMISSION_DENIED`, `ADMISSION_POINTER_UNAVAILABLE`, `TENANT_BILLING_BLOCKED`, `AUTH_UNAVAILABLE`, and `ENTITLEMENT_UNAVAILABLE`.
...
- `GetTenantEntitlementsForRuntime(tenantId)` is the authoritative producer for admission-critical entitlement snapshots. Responses must include:
  - explicit `authorityAvailability`, opaque `tenantAuthorityGeneration`, `subscriptionStatus`, `gameplayAvailable`, `allowPublicJoin`, `allowNewGameplayBindings`, `allowNewInstanceStarts`, and applicable quota fields; absence of a subscription row is not implicit permission, and free/trial hosting is explicit;
