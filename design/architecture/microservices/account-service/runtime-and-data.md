...
- Current implementation note:
  ...
  - Explicit `JOIN`/`Join & Play` and membership-write removal from connect-token issuance remain target follow-through; text `PLAY` now rejects missing public-production membership with `JOIN_REQUIRED`. Broader reconnect/cutover consumption of admission-pointer truth and operator-facing pointer tooling are also incomplete.
