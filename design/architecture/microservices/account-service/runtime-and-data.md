...
The account lifecycle state machine, global deletion preconditions, and full-account versus tenant-scoped export split are canonical target design. Current v1 commerce scope is Stripe-backed FireMUD hosting billing; creator monetization and `purchase_entitlement` are deferred behind the separate marketplace and settlement decision required by [ADR 0143](../../decisions/adr-0143-stripe-v1-hosting-billing-and-deferred-creator-monetization.md). The current service has live identity/authentication, profile, external-link persistence, recovery/verification, payment, subscription, notification, and virtual-currency foundations, but lifecycle state transitions, genuine provider authentication, cross-service export, retained-record handling, verified Stripe webhook and reconciliation processing, and complete hosting billing/subscription follow-through remain partial. The current external-link route accepts caller-asserted provider identity and stores tenant scope; it is not a supported provider login and directly conflicts with [ADR 0049](../../decisions/adr-0049-optional-provider-specific-external-identity-linking.md). The current full export returns only Account and profile rows, the tenant export includes excessive global Account fields, and successful deletion immediately removes Account-owned rows including payment transactions. Those behaviors directly conflict with [ADR 0050](../../decisions/adr-0050-versioned-export-retention-and-erasure-policy.md) and the bounded erasure workflow below.
...
- Creator monetization and player purchase entitlements are not part of v1. If a later marketplace decision accepts them, durable product grants must be separate from raw payment attempts: payment rows remain financial/provider evidence, while an explicit entitlement authority is fulfilled only from verified idempotent provider webhook or reconciliation completion. Off-platform receipts never create that authority.
...
The Account Service owns Stripe-backed billing records for FireMUD's own hosting plans and platform subscriptions as outlined in the [Core Requirements](../../../project-management/core-requirements.md#2.8-moderation-administration--monetization). Stripe is the sole supported v1 processor; a future provider requires a reviewed integration rather than a speculative common abstraction. Entities include `payment_transaction` and `subscription` tables with Flyway migrations, and the proto definitions live in [`payment_service.proto`](../../../../protos/account/v1/payment_service.proto).

Current implementation is broader and less complete than the accepted product boundary. It can create Stripe PaymentIntents, invoke refunds, mark donation transactions, compute the configured 5% application fee and remaining creator share, and create local subscription rows. It does not demonstrate verified webhook-driven completion, durable idempotent purchase entitlements, provider reconciliation, creator settlement or payouts, reserves, chargeback/negative-balance handling, or end-to-end hosting-subscription enforcement. `CreateDonation`, generic purchase, creator-share, and related refund surfaces are therefore partial substrate, not supported creator-monetization capabilities.

Creator monetization is deferred behind the marketplace and settlement decision defined by [ADR 0143](../../decisions/adr-0143-stripe-v1-hosting-billing-and-deferred-creator-monetization.md). Off-platform payment evidence cannot create FireMUD entitlements. Any future entitlement must be separate from raw payment records and activated only after verified, idempotent provider webhook or reconciliation completion. Refund behavior must distinguish revocable access, unconsumed conserved value, and consumed or transferred value instead of claiming that every effect can be deleted. More detailed hosting-billing behavior lives in the [Stripe Integration Design](./stripe-integration.md) and [Subscription Management Design](./subscription-management.md).

### Virtual Currency

Each tenant may define game-specific currencies. Balances are stored in the `currency_balance` table keyed by account, and the `VirtualCurrencyService` gRPC API allows authorized services to add or spend currency for an account. These foundations do not make virtual currency purchasable, cash-convertible, or creator-settleable in v1. The existing platform-fee and `creator_share_cents` fields are partial implementation substrate for the deferred marketplace, not an active revenue-sharing contract.

### Premium Hosting

Premium hosting tiers are modeled as subscription plans with higher resource limits. Game creators upgrade through the supported Stripe hosting-billing flow.
