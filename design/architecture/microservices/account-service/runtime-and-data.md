...
- `EnsurePublicProductionPlayerMembership(accountId, tenantId, worldSlug, realmSlug, requestId)` is the current proto RPC name for the membership seam. Its target behavior is the authoritative explicit open-enrollment membership operation for a tenant's default public production realm; connect-token issuance and `PLAY` must require its result but never invoke it implicitly.
