...
- Membership responses include authoritative `membershipExists`, `evaluatedAt`, and `membershipVersion` fields so callers can distinguish absence from denied admission, verify freshness, and detect stale reads. The target `GetTenantMembershipForRuntime` response additionally carries an opaque `membershipAuthorityGeneration`; callers do not interpret Account-owned generation values.
...
- Canonical public-production join operation and seams:
  - Account owns one operation: proto `EnsurePublicProductionPlayerMembership`, with target semantic name `JoinPublicProductionMembership`. The player-facing REST seam is `POST /auth/bootstrap/join`; the text seam is Game Session's `JOIN <world>` translation to that same Account operation. No second REST, text, or proto membership writer is defined.
  - Game Session resolves `<world>` from the exact `WORLDS` snapshot to `worldSlug` and the configured default public-production `realmSlug`. It passes the opaque discovery `connectScopeId` plus `requestId`; Account derives and revalidates the caller and the `{worldSlug, realmSlug}` target from that scope. These slugs are bound target metadata, not an independently authoritative selector.
  - Ambiguous or stale `worldSlug`/`realmSlug` selection is rejected before the join call and requires fresh discovery. A stale or mismatched `connectScopeId` fails closed with the applicable scope/admission error; it is never silently translated to a newer realm target.
  - `requestId` is bound to the caller, `connectScopeId`, resolved `worldSlug`, and `realmSlug`. An exact retry returns the same membership identity/version or deterministic failure; a changed selector or target is an idempotency conflict and creates no second membership.
  - Explicit `JOIN`/`Join & Play` remains unimplemented. The current proto seam exists, text `PLAY` returns `JOIN_REQUIRED` for absent public-production membership, and connect-token issuance has the same required-membership behavior without invoking the seam; these are implementation facts, not evidence that the canonical join operation is live.
- `GetTenantMembershipForRuntime(accountId, tenantId)` is the authoritative internal membership surface for gameplay/runtime flows.
  ...
  - Target minimum response fields: `accountId`, `tenantId`, `membershipExists`, `roles[]`, `gameplayAdmissionAllowed`, `membershipVersion`, `membershipAuthorityGeneration`, and `evaluatedAt`.
  ...
  - Current implementation/proto gap: the live response currently exposes `membershipVersion` and `evaluatedAt` but not `membershipAuthorityGeneration`; the generated proto and Account implementation must add the target field before those runtime consumers can enforce this generation contract.
- `GetTenantEntitlementsForRuntime(tenantId)` is the authoritative internal entitlement surface for gameplay/runtime flows. Runtime callers may cache its positive result only under the strict-new-commitment and bounded-continuity rules in [ADR 0028](../../decisions/adr-0028-differentiated-entitlement-freshness.md); the cache never becomes a second writer.
  ...
  - Minimum response fields: `tenantId`, `subscriptionStatus`, `gameplayAvailable`, `allowPublicJoin`, `allowNewGameplayBindings`, `allowNewInstanceStarts`, `quotas { ... }`, `evaluatedAt`, `entitlementVersion`, `tenantBillingSequence`, and opaque `tenantAuthorityGeneration`.
...
- Account Service must use the authoritative realm-routing contract when issuing `/auth/connect-token` so connect-token scope is pinned to the tenant's current admissible instance for the selected realm instead of a caller-supplied guess.
  ...
  - Existing caller-bound membership is mandatory for every target. A missing public-production membership returns `JOIN_REQUIRED`, commits no membership row, successful gameplay-connect issuance record, token registry record, token, or cookie, and records the same deterministic failure under `(accountId, connectScopeId, requestId)`. An exact retry replays `JOIN_REQUIRED` even if membership is created later; the caller must use a new request ID after the explicit join. The target replay outcome is durable for the complete issuance-idempotency window. The current implementation provides only the bounded Session Service replay record, so durable database-backed failure replay remains an implementation gap. A private or playtest target additionally requires the current Account-owned realm-access grant; a grant never substitutes for membership.
