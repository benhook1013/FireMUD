...
- Stateless authentication uses exact short-lived JWT profiles for meta/control and gameplay bootstrap:
  ...
  Raw Telnet and generic gameplay command streams never see or transmit these tokens. First-party browser/mobile gameplay clients use only the bounded `player-bootstrap` profile for HTTPS bootstrap; the one-use WebSocket connect token is a separate edge credential.
...
- Raw Telnet and generic gameplay command clients authenticate through the `LOGIN` flow described in [Authentication & Authorization](../../system-architecture-authentication.md) and never carry JWTs. First-party browser/mobile gameplay clients use the short-lived `player-bootstrap` profile for HTTPS bootstrap and an HttpOnly-cookie connect token for the WebSocket handshake. The Game Session Service calls the internal `Authenticate` gRPC method (mTLS-protected) to verify in-band credentials and obtain `game-session-account-delegation` JWTs for Account, while `/auth/login` remains a browser/control-plane endpoint.
...
- `IssueConnectToken` / `POST /auth/connect-token` is the target gameplay bootstrap token-issuance surface.
  - Target issuance requires a current caller-bound membership authority generation in addition to the live membership, entitlement, and admission-pointer checks; discovery output and a stale bootstrap decision are not sufficient authority.
  ...
  - All first-party clients receive the connect token only as `Set-Cookie: Firemud-Connect-Token=...` with the gateway-required security attributes; the token is never returned in the response body or a header fallback.
