...
- Owns the target **Stripe** hosting-plan billing and recurring hosting-subscription boundary; current provider lifecycle and entitlement proof remain partial. Incomplete `CreateSubscription` fails closed before mutation until caller-bound billing ownership, durable request identity, instrument selection, and provider-confirmed activation exist. The target containment contract also requires generic PaymentIntent, refund, donation, and creator-share surfaces to reject before any provider mutation, and the current generic gRPC handlers now return `FAILED_PRECONDITION` before service/provider calls; the underlying provider-mutating service and non-exposed purchase-workflow substrate remain unsupported; see [Operations](./operations.md#saga-participation) and [Configuration](./configuration.md#implementation-status) for the current status. These surfaces are not advertised product paths. One-time and creator/player commerce remain future-gated under [ADR 0179](../../decisions/adr-0179-firemud-managed-creator-commerce-boundary.md).
