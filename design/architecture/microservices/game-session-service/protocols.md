...
The direct Game Session WebSocket remains useful only as an internal/test seam, but it is not a separate public authentication path. The Gateway-routed first-party WebSocket route is `/ws/game/**`: `POST /auth/player-bootstrap` issues the tenant-free bootstrap credential, `/auth/bootstrap/*` performs authenticated discovery/actions, and `POST /auth/connect-token` sets the one-use token cookie. The exact accepted Gateway connection modes are `first_party_web` and `trusted_tcp_proxy`. `first_party_web` includes first-party browser, mobile-browser, native-mobile, and other first-party non-browser clients using a protected cookie jar for the `Firemud-Connect-Token` cookie. The target-only `non_first_party_public` header-carrier variant is unavailable until its dedicated route and proof are complete; an unclassified generic WebSocket header is rejected. `trusted_tcp_proxy` remains the separately authenticated trusted TCP Proxy exception. Real end-to-end client-path verification should prefer Gateway or TCP Proxy rather than relying only on direct Game Session WebSocket coverage.
