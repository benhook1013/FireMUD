...
The same resolution rules apply to `PLAY demo production 2`, where `demo` is the one-world tenant shorthand, `PLAY demo/main production 2`, or `PLAY 1 1 Sora`: response-local menu indices and stable qualified slug selectors identify the same player-facing choices. Game Session resolves the current admissible runtime target and stable namespace, then binds the internal session to the complete `{tenantId, playableStateNamespaceId, playableStateScope, gameInstanceId, characterId}` tuple; the player never selects `gameInstanceId`, `playableStateScope`, or a storage namespace directly.
