...
For bootstrap-backed direct-text `LOGIN` and first-party `/ws/game/**`, Gateway validates the source `gameplay-connect` JWT where that carrier exists and emits the signed context. Before using any context field for `LOGIN`, `PLAY`, or scope comparison, Game Session must validate the complete signed context: signature, declared type, alteration/integrity, expiry, audience, recipient, and binding to the selected target and source artifact. Missing, wrong-typed, altered, expired, incorrectly audience-bound, recipient-mismatched, or otherwise unbound context fails closed. The trusted TCP Proxy exception remains the only positive non-context exception.
