...
Unless explicitly described as current behavior, this document defines the target protocol. The target direct-text flow is public `WORLDS` discovery -> credential-bearing `LOGIN` -> authenticated `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY`. Current credential and email-code `LOGIN` authenticate global Account identity without sending a tenant or requiring membership; the bootstrap instance tenant remains local transport context only and is not Account authentication authority. The flow requires explicit `JOIN`/`Join & Play` for first public-production entry, while an existing durable `membershipLifecycleState=ACTIVE` membership permits direct `PLAY` and a grant-backed non-public path may proceed only when that same `ACTIVE` membership already exists. A grant never creates or substitutes membership. The local Account `JoinPublicProductionMembership` operation and Game Session translation/scope seam now exist, but complete direct-text and first-party end-to-end proof, PostgreSQL execution, pending-operation recovery/reconciliation, and the Account lifecycle/authority-generation snapshot needed by final admission remain incomplete. Current text `PLAY` may return non-actionable `JOIN_REQUIRED` for an eligible public-production target when `membershipExists=false` or existing `gameplayAdmissionAllowed=false`; the latter cannot be classified specifically as `INACTIVE`, without creating or restoring membership; current text `PLAY` enforces the applicable non-public realm grant, while current connect-token issuance does not validate that grant and retains its existing Account rejection mapping. Current consumers decide from `membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt` plus the text `PLAY` grant check; the Account proto/producer now also emits `membershipLifecycleState` and `membershipAuthorityGeneration`, but current Game Session and connect-token consumers do not enforce lifecycle or generation. Current non-public membership/admission denial remains `WORLD_ACCESS_DENIED`; target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` is defined separately. The missing membership-authority-generation reread and target non-public grant check at connect-token issuance remain gaps.
...
The current Unit 1B safety correction removes `PLAY`'s account/name/hash-derived actor fallback and performs account-scoped persisted Entity roster selection only after Account admission checks. That narrower roster is not yet namespace- and policy-qualified, while Account's current runtime reads deny even the exact Game Session peer until typed caller/target authority exists; it is not a live successful admission claim. Direct-text `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity reads rather than exposing an unqualified roster. Fresh first-party `PLAY` no longer treats a connect-context token alone as retained-context replay authority, but still requests a fresh `LOOK`; a resumed `PLAY` requests that redraw even if no retained buffer exists. Durable logout episode/generation fencing, effective resume-window enforcement, and namespace-scoped controller takeover remain pre-v1 successors.
...
- **Logged in, not yet playing** – target behavior allows existing members with confirmed `membershipLifecycleState=ACTIVE` durable membership to issue `PLAY` directly and requires a first-time public-production player to issue `JOIN` first. The local Account `JoinPublicProductionMembership` operation and Game Session `JOIN` translation/scope path now exist, but the Account proto/producer emits `membershipLifecycleState` and `membershipAuthorityGeneration` alongside the four admission fields; current `PLAY` applies only `membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt` plus existing grant checks, so it cannot enforce or classify `ACTIVE` or `INACTIVE`. An eligible public-production denial may return non-actionable `JOIN_REQUIRED` when `membershipExists=false` or existing `gameplayAdmissionAllowed=false`; the latter remains non-admitting without a lifecycle classification. Direct-text and first-party end-to-end proof, PostgreSQL execution, and pending-operation recovery remain incomplete; non-public membership/admission denial returns `WORLD_ACCESS_DENIED`. Players may use lobby helper commands such as `REALMS` and `CHARS` to disambiguate selection.
...
Current runtime still uses the per-instance controller lookup and does not prove the namespace-key binding CAS. The Account proto/producer emits `membershipLifecycleState` and `membershipAuthorityGeneration` alongside the four admission fields (`membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt`), but the current Game Session consumer does not enforce lifecycle or generation and therefore cannot classify `ACTIVE` or `INACTIVE`. Current text `PLAY` may return non-actionable `JOIN_REQUIRED` for an otherwise eligible public-production request when `membershipExists=false` or an existing membership has `gameplayAdmissionAllowed=false`; the latter cannot currently be classified specifically as `INACTIVE` because lifecycle state is not consumed by the admission path. Current non-public missing or non-admitting membership remains `WORLD_ACCESS_DENIED`. A fresh Telnet admission requires current entitlement, and current text `PLAY` checks the applicable private/playtest grant; current connect-token grant validation remains a gap.
...
- `JOIN_REQUIRED` – the selected public-production target either has missing membership (`membershipExists=false`) or an existing but non-admitting membership (`gameplayAdmissionAllowed=false`) for the account, so the client must complete explicit `JOIN`/`Join & Play`; a grant or cached discovery result is not a substitute. The local Account route/RPC and Game Session text dispatch now exist, but the current result remains fail-closed and non-actionable to consumers until lifecycle-aware admission and end-to-end/runtime proof are complete. Account now emits lifecycle state and authority generation, but the existing non-admitting case cannot be classified as `INACTIVE` because those fields are not yet consumed by the admission path.
