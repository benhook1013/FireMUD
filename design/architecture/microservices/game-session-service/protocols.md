...
| `CHARS <world> [realm]` | Requires successful `LOGIN` and lists characters for a world and optional realm from the authoritative character store, filtered by the complete `{accountId, tenantId, playableStateNamespaceId, gameInstanceId, playableStateScope}` query tuple. Game Session resolves the scope and stable playable-state namespace from the exact realm catalog/pointer snapshot and carries them to Entity Management; neither is caller-selected or accepted as a text, query, or join input, and the player-facing text projection exposes roster choices, not storage keys. | `CHARS demo production` |
