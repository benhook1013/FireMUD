...
For `PLAYER-01`, `CHARS` is a realm-local persisted-actor discovery operation, not an Account roster read or a session-binding shortcut. Entity Management owns the detailed actor/policy contract in [Entity API Contracts](../entity-management-service/api-contracts.md); Game Session supplies the authenticated account and server-resolved target, presents Entity's policy-specific result, and keeps `CHARS` as discovery evidence. `PLAY` independently revalidates one persisted actor and the active runtime fence; no account/name/hash/runtime-derived fallback is permitted. The current protocol remains partial as recorded in [Implementation Status](#implementation-status).
...
Unless explicitly described as current behavior, this document defines the target protocol. The target direct-text flow is public `WORLDS` discovery -> credential-bearing `LOGIN` -> authenticated `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY`. Current credential and email-code `LOGIN` authenticate global Account identity without sending a tenant or requiring membership; the bootstrap instance tenant remains local transport context only and is not Account authentication authority. The target flow requires explicit `JOIN`/`Join & Play` for first public-production entry, while an existing durable `membershipLifecycleState=ACTIVE` membership permits direct `PLAY` and a grant-backed non-public path may proceed only when that same `ACTIVE` membership already exists. A grant never creates or substitutes membership. The local Account `JoinPublicProductionMembership` operation and Game Session translation/scope seam now exist, but complete direct-text and first-party end-to-end proof, PostgreSQL execution, pending-operation recovery/reconciliation, and the Account lifecycle/authority-generation snapshot needed by final admission remain incomplete. Current Account gRPC membership, grant, and entitlement reads used by Game Session fail with `FAILED_PRECONDITION` before data access, including for the Game Session workload. Accordingly, a valid-route text `PLAY` reaching runtime admission returns `AUTH_UNAVAILABLE` before membership, grant, or roster evaluation, and direct-text `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity is called; neither is currently an actionable JOIN, roster, or PLAY path. The status and target admission contract are canonical in [Authentication](../../system-architecture-authentication.md#implementation-status). The local JOIN operation does not alter the target `JOIN` -> `CHARS`/character creation -> `PLAY` sequence.
...
The current Unit 1B safety correction removes `PLAY`'s account/name/hash-derived actor fallback and performs account-scoped persisted Entity roster selection only after Account admission checks. That narrower roster is not yet namespace- and policy-qualified, and Account's current gRPC membership, grant, and entitlement reads fail with `FAILED_PRECONDITION` before data access even for the Game Session workload. Current text `PLAY` therefore fails closed with `AUTH_UNAVAILABLE` before Entity roster selection; direct-text `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity is called. These statuses do not establish whether a membership, grant, or character exists. Fresh first-party `PLAY` no longer treats a connect-context token alone as retained-context replay authority, and focused post-`LOGOUT` replay suppression is present; it still requests a fresh `LOOK`. Durable logout episode/generation fencing, delayed-race proof, effective resume-window enforcement, and namespace-scoped controller takeover remain open.

Current text `WORLDS` and `REALMS` expose only visible public-production realms. Caller-bound non-public discovery still needs Account's typed current membership/grant authority before those realms can be shown; a visible catalog pointer alone does not prove access. This containment does not remove internal non-public routing or make private `PLAY` a supported admission path while Account's runtime reads remain disabled.
...
The direct Game Session WebSocket remains useful only as an internal/test seam, but it is not a separate public authentication path. The Gateway-routed first-party WebSocket route is `/ws/game/**`: `POST /auth/player-bootstrap` issues the tenant-free bootstrap credential, `/auth/bootstrap/*` performs authenticated discovery/actions, and `POST /auth/connect-token` sets the one-use token cookie. `first_party_web` is the public WebSocket mode and includes first-party browser, mobile-browser, native-mobile, and other first-party non-browser clients using a protected cookie jar for the `Firemud-Connect-Token` cookie. `trusted_tcp_proxy` is not a public WebSocket mode; it is the separately authenticated internal mTLS TCP Proxy bridge branch. The target-only `non_first_party_public` header-carrier variant is unavailable until its dedicated route and proof are complete; an unclassified generic WebSocket header is rejected. Real end-to-end client-path verification should prefer Gateway or TCP Proxy rather than relying only on direct Game Session WebSocket coverage.
...
- **Logged in, not yet playing** – target behavior allows existing members with confirmed `membershipLifecycleState=ACTIVE` durable membership to issue `PLAY` directly and requires a first-time public-production player to issue `JOIN` first. The local Account `JoinPublicProductionMembership` operation and Game Session `JOIN` translation/scope path now exist, but complete direct-text and first-party end-to-end proof, PostgreSQL execution, and pending-operation recovery remain incomplete. Current Game Session Account reads fail before data access, so text `PLAY` returns `AUTH_UNAVAILABLE`; it does not currently classify membership as `ACTIVE`/`INACTIVE` or return an actionable `JOIN_REQUIRED` or `WORLD_ACCESS_DENIED`. Current `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity. Players may use lobby helper commands such as `REALMS` and `CHARS` to disambiguate selection in the target flow; see [Implementation Status](#implementation-status) for the current command outcomes.
...
| `JOIN <world>` | **Implemented locally; end-to-end/runtime proof remains incomplete.** Explicitly joins the selected world's public production realm through the Account-owned idempotent membership writer. The resulting membership is durable and powers later return discovery. PostgreSQL execution, pending-operation recovery, and direct text/first-party path proof remain open. `<world>` follows the same tenant-scoped selector rules as `REALMS`. | `JOIN demo/demo` |
| `CHARS <world> [realm]` | Target behavior requires successful `LOGIN` and presents Entity's persisted roster/policy result for the resolved realm. It is discovery evidence, not admission authority; `PLAY` revalidates the selected actor and current runtime fence. Current direct-text `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before calling Entity. | `CHARS demo/demo production` |
| `PLAY <world> [realm] [character]` | Target behavior binds the authenticated connection to a world, optional realm, and optional character after discovery, `LOGIN`, and the conditional membership/character gates, enforcing tenant authorization, realm routing, and entitlements. Players may omit `[realm]` or `[character]` only when fresh target resolution is unambiguous; direct text never reuses the first-party WebSocket discovery snapshot. A first-time public player must complete `JOIN <world>` first; a grant-backed non-public path requires existing durable `membershipLifecycleState=ACTIVE` membership. Current Game Session Account reads fail before data access, so text `PLAY` returns `AUTH_UNAVAILABLE` and does not currently produce an actionable `JOIN_REQUIRED` result. | `PLAY demo/demo production Sora` |
...
The target Account membership operation is `JoinPublicProductionMembership`; the local Account operation and Game Session translation/scope seam now exist, and there is no current implicit membership writer. The local path does not claim complete direct-text/first-party transport proof, PostgreSQL execution, or pending-operation recovery. Game Session translates `JOIN <world>` to the sole operation; it does not create membership locally or use another join writer.
...
Current local `JOIN` availability and the membership-authority-generation gap are recorded in [Implementation Status](#implementation-status); this section defines Game Session's transport consequence and must not be read as complete end-to-end or production proof.
...
- For sessions on the Gateway-routed first-party WebSocket route `/ws/game/**` that already carry a validated Gateway connect context, bare `LOGIN` completes gameplay authentication from the pre-established bootstrap identity instead of prompting for credentials. First-party browser, mobile-browser, native-mobile, and other first-party non-browser clients use the protected connect-token cookie; only the target-only, explicitly classified `non_first_party_public` route variant may use the dedicated connect-token handshake header after its proof is complete. This bootstrap identity must not quietly reintroduce gameplay binding into `LOGIN`; `PLAY` remains the sole gameplay-admission and gameplay-scope binding step. Reauthentication as a different account clears every retained character, runtime-route, playable-scope, and connect-scope field before `PLAY`; only an exact same-account reconnect may retain its candidate gameplay context for fresh admission validation.
...
Current runtime still uses the per-instance controller lookup and does not prove the namespace-key binding CAS. The current Account gRPC membership, grant, and entitlement reads used by Game Session fail with `FAILED_PRECONDITION` before data access, including for the Game Session workload. A valid-route text `PLAY` reaching runtime admission therefore returns `AUTH_UNAVAILABLE` before it can evaluate membership, current entitlement-dependent admission, private/playtest grants, or Entity roster state; `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before Entity. The lifecycle and authority-generation predicates, target roster policy qualification, and namespace-key binding CAS remain unproved. The target public-production membership result is `JOIN_REQUIRED` only after its prescribed earlier checks, while the target non-public requirements remain distinct; neither target outcome is evidence of the current runtime result. See [Authentication Implementation Status](../../system-architecture-authentication.md#implementation-status) for the canonical current authority-read status.
...
- `JOIN_REQUIRED` – target `PLAY` outcome for a public-production target whose membership is missing or `INACTIVE` after the canonical preceding checks; clients then complete explicit `JOIN`/`Join & Play`. Current text `PLAY` instead fails with `AUTH_UNAVAILABLE` before membership evaluation because the Account gRPC read fails before data access.
- `WORLD_ACCESS_DENIED` – target policy-denial outcome for a non-public or other target for a reason other than the target non-public membership/grant outcomes. Current Game Session Account reads fail before these predicates can be evaluated; see [Authentication Implementation Status](../../system-architecture-authentication.md#implementation-status).
...
World/gameplay communication enters Game Logic when topology, perception, abilities, effects, authored interception, or other gameplay state determines its meaning. Game Logic emits a bounded authorized plan; Social & Groups applies social authorization, moderation, applicable history, and delivery-state rules; Game Session owns final delivery to connected gameplay transports. The shared [ADR 0147 audience contract](../../decisions/adr-0147-explicit-communication-classes-and-owner-delivery.md#world-and-gameplay-communication) governs the audience ceiling. Game Session validates that the resolved plan remains within the shared effective cap before sending any recipient view and rejects malformed or over-limit results without partial transport delivery; Social persistence remains a separate outcome and must perform its own pre-persistence validation. For an over-limit resolved plan, the sender receives exactly one stable client-visible `ERROR COMMUNICATION_NOT_DELIVERED Audience limit exceeded` outcome, with no partial transport delivery or additional per-recipient outcome. Game Session never truncates or recomputes the authorized audience, creates independently meaningful plans or chunks, or treats transport chunking as semantic commitment. For one atomically resolved plan, it may use identity-preserving transport-level chunks only under the [ADR 0150 transport contract](../../decisions/adr-0150-closed-observer-views-and-profile-scoped-shout.md#shout-is-a-profile-defined-topology-contract), with backpressure and explicit completion/failure reporting. Game Logic returns `AUDIENCE_LIMIT_EXCEEDED` before handoff when authoritative resolution exceeds the effective cap. The current delivery implementation does not yet enforce or prove this limit. Account messaging, ordinary guild/group channels, browser social actions, and ordinary account/social mail enter Social & Groups directly after authentication and applicable membership/privacy/moderation checks. An in-game adapter does not reclassify private platform communication as gameplay or expose it to tenant-authored scripts. Active ADR 0147 records this owner split, mapped from reviewed archive record `archive-ADR-0134`.
...
- over-limit resolved plan -> `ERROR COMMUNICATION_NOT_DELIVERED Audience limit exceeded`
...
4. Social & Groups applies its local moderation, social-audience, history, retention, and persistence/history acceptance acknowledgement contract to the accepted plan; that acknowledgement covers only the Social-owned semantic or history outcome, never connected-player transport delivery. Game Session owns connected gameplay transport delivery and reports transport outcome separately. Account messaging, ordinary group/channel communication, and ordinary account/social mail use direct Social ingress and do not require Game Logic or a running world. Active [ADR 0149](../../decisions/adr-0149-communication-type-specific-history-and-retention.md) records the type-specific history/retention boundary, mapped from reviewed archive record `archive-ADR-0136`.
...
The target handoff preserves one stable identity for the resolved communication plan and distinct stable delivery identities for each Social/history or connected-transport outcome, all bound to the plan's canonical digest, type/version, scope, and authorized recipient views. Retries and recovery reuse the same applicable identity and digest; conflicting reuse fails closed. Plan acceptance, Social persistence/history acknowledgement, and connected transport delivery are separate outcome boundaries: a success at one boundary is not success at another, and a lost or ambiguous response is reconciled by the owning service through read-only status using the same identity rather than a replacement plan or delivery. For every partial-success combination, reconciliation must avoid a second history append, semantic effect, or recipient delivery. A malformed, stale, contradictory, or over-limit resolved plan is rejected atomically before Social persistence or Game Session transport; no owner may broaden, truncate, recompute, or independently chunk the authorized plan. After a communication attempt has been admitted, any unresolved owner or transport outcome is surfaced as exactly one stable `ERROR COMMUNICATION_NOT_DELIVERED <message>` result for the sender, not as success inferred from history or an attempted send.

Cross-service proof for this target boundary must cover same-plan/same-digest replay and conflicting reuse, duplicate and reordered Social and Game Session deliveries, lost acknowledgements after plan, history, or transport progress, restart/read-only reconciliation, cross-pod recipient delivery, audience-cap rejection with zero partial effects, and recipient-view/history disclosure preservation. Current `SendCommunication` and Social delivery stubs remain narrower and do not prove these identities, outcome boundaries, or no-duplicate guarantees.
