...
Prompt outputs now also carry a first minimal structured status field list alongside their rendered classic text. Text clients still receive the traditional prompt line, and the first-party WebSocket edge can project the structured payload. A supported browser consumer remains future work and requires an explicitly versioned client envelope rather than treating the current internal shape as a public contract.
...
4. Reconnecting Telnet or WebSocket clients do not receive buffered input, command, frame, or byte replay. A command already durably admitted before edge loss may still complete once under its recorded server identity; that is not resubmission from the old transport. After successful `LOGIN` and `PLAY`, Game Session may replay the bounded per-player semantic transcript and then emits a fresh authoritative `LOOK` so current room state wins over stale context. New transcript entries preserve structured `PlayerOutput` replay metadata alongside rendered protocol text; first-party web clients receive typed `transcript_entry` replay events when that metadata exists, while older text-only entries and classic clients continue to use transcript chunks/plain text.
...
- Every text-protocol response is plain text. The first line is either `OK <COMMAND>` or `ERROR <CODE> <message>`. A supported structured first-party response requires its own explicitly versioned envelope while retaining a deterministic text projection.
