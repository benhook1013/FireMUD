...
2. Authenticated `LOOK` commands call Game Logic's `ResolveLook`, passing the resolved tenant/namespace/scope, `gameInstanceId`, session/character context, room, region/epoch, and a causal read floor allocated from the current region commit authority.
3. Game Logic returns a structured `LookResult` only when World and Entity prove the requested scope and epoch, each has `servedThroughTickId >= requestedFloor`, and each returns its own opaque component version. Game Logic rejects or retries missing, below-floor, mixed-scope/epoch, or unavailable evidence; it never compares component versions numerically or treats equality as temporal proof. Game Session renders the accepted result into the `OK LOOK` text projection and emits `gamesession.command.look.*` metrics/logs.
4. Reconnecting Telnet or WebSocket clients do not receive buffered command/input/frame replay. After successful `LOGIN` and `PLAY`, Game Session may render bounded semantic recent context from durable structured entries, then obtains a fresh authoritative `LOOK` and emits exactly one current prompt. New context entries preserve versioned `PlayerOutput` metadata alongside derived rendered text; first-party web clients receive typed entries when supported, while classic clients consume the mandatory deterministic text projection.
