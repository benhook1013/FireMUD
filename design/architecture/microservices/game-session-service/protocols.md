...
- Authenticated caller identity and subject authority come only from the validated caller-bound `PlayerExecutionContext`; Account separately validates that context and never treats `connectScopeId` as caller authority. The resolved target's opaque `connectScopeId` is the selector passed to Account with the caller-generated `requestId`; it selects and binds the target world/realm and routing snapshot. Account validates the target scope at its commit boundary; a client-supplied runtime ID or independently supplied slug cannot replace the scope.
