...
| `CHARS <world> [realm]` | Requires successful `LOGIN` and lists characters for a world and optional realm from the authoritative character store. The durable roster lookup is keyed by `{accountId, tenantId, playableStateNamespaceId, playableStateScope}`; `gameInstanceId` and, where applicable, `regionEpoch` are fresh runtime authorization fences, not durable character identity or lookup fallbacks. Game Session resolves the namespace/scope and active runtime fence from the exact realm catalog/pointer snapshot and carries them to Entity Management; neither is caller-selected or accepted as a text, query, or join input, and the player-facing text projection exposes roster choices, not storage keys. Character creation and `PLAY` must independently revalidate that same fresh active namespace/instance/epoch fence; a `CHARS` result is roster evidence, not admission authority. | `CHARS demo production` |
...
The same resolution rules apply to `PLAY demo production 2`, where `demo` is the one-world tenant shorthand, `PLAY demo/main production 2`, or `PLAY 1 1 Sora`: response-local menu indices and stable qualified slug selectors identify the same player-facing choices. Game Session resolves the current admissible runtime target and stable namespace, then binds the internal session to the durable controller identity `{tenantId, playableStateNamespaceId, characterId}` while retaining the authenticated `accountId` as binding-owner evidence, `playableStateScope` as server-derived scope context, and `{gameInstanceId, regionId, regionEpoch}` as the current runtime fence where applicable; the player never selects `gameInstanceId`, `playableStateScope`, or a storage namespace directly.
...
2. **Target-only causal-floor flow:** Authenticated `LOOK` commands call Game Logic's `ResolveLook`, passing the broader validated tenant/namespace/scope and `gameInstanceId` session/character context alongside a `CausalReadFence` whose identity is at least `{tenantId, gameInstanceId, regionId, roomInstanceId, regionEpoch, committedTickId}`. `playableStateNamespaceId` and `playableStateScope` may remain broader admission context, but are not runtime fence identity. The target fence is allocated from the current region commit authority and propagated to World and Entity. The current `ResolveLook` request/proto remains floor-free, so this propagation is not current behavior.
3. **Target-only causal-floor rejection:** Game Logic returns a structured `LookResult` only when World and Entity prove the requested fence scope and epoch, each has `servedThroughTickId >= requestedFloor` (the committed-tick floor), and each returns its own opaque component version. Game Logic rejects or retries missing, below-floor, mixed-scope/epoch, or unavailable evidence; it never compares component versions numerically or treats equality as temporal proof. Current adapters still return only deterministic scope markers and do not provide this causal-floor/component-version evidence, so the target rejection behavior remains an implementation gap. Game Session renders the accepted result into the `OK LOOK` text projection and emits `gamesession.command.look.*` metrics/logs.
4. Reconnecting Telnet or WebSocket clients do not receive buffered command/input/frame replay. After successful `LOGIN` and `PLAY`, Game Session may render bounded semantic recent context from durable structured entries, then obtains a fresh authoritative `LOOK` and emits exactly one reconnect prompt only when both effective `firemud.presentation.prompt.enabled` and `firemud.presentation.prompt.emit-after-reconnect-restore` are enabled; if either is disabled, it emits zero reconnect prompts. When enabled, duplicate-prevention still ensures one prompt. New context entries preserve versioned `PlayerOutput` metadata alongside derived rendered text; first-party web clients receive typed entries when supported, while classic clients consume the mandatory deterministic text projection. See [Input, Output, and Presentation](../../system-architecture-input-output-and-presentation.md#prompt-behavior) for prompt-setting precedence.
...
- Every supported `PlayerOutput` envelope has an explicit schema version and a deterministic plain-text projection. Telnet and generic text WebSocket use that projection; structured clients may consume typed payloads under the documented version compatibility rules.
