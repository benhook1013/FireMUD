...
- Game Session resolves `<world>` from the exact `WORLDS` snapshot to the tenant-scoped `{tenantId, worldSlug}` pair, then resolves the world's configured default public-production `realmSlug` to durable `realmId`. The resolved slugs identify the operation target for people but are not independent client authority.
