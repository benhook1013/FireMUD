...
- **Current authorization gap:** the endpoint is admitted by the shared HTTP `PRIVILEGED` setting, which accepts a privileged role scoped to any tenant, but its query selectors and persisted `sessionId` are not bound to that role's tenant. This permits a tenant-scoped operator to inspect another tenant's effective settings; existing malformed-input tests do not prove cross-tenant denial. Target configuration keeps global operator access explicit and requires exact scoped-tenant/session binding, target-tenant generation, and `privileged_control` where the global branch applies, with fail-closed negative proof.
...
- Reconnection sections retain bounded diagnostics for the current tenant/game-instance resolver and envelope when a persisted override is disregarded because its merged byte bounds are invalid; the next valid layer remains effective. In the target state, `reconnection.buffer` precedence and runtime-replacement inheritance use stable `{tenantId, playableStateNamespaceId}`, not `gameInstanceId`. That stable namespace scope is target-only until the current `gameInstanceId` resolver/envelope and actuator/query diagnostics migrate; current diagnostic inputs remain tenant/game-instance based.
...
`firemud.*` operator defaults, then tenant-scoped persisted overrides from Game Design, then the applicable more-specific persisted override from Game Design. Current ordinary domains and `reconnection.buffer` resolution use game-instance scope. Target `reconnection.buffer` precedence and replacement inheritance use stable `{tenantId, playableStateNamespaceId}` scope so runtime replacement inherits its bounds without copying; that target scope is not current resolver, envelope, or actuator/query behavior until the migration is implemented.
