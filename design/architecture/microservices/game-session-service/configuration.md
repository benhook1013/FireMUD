...
| `GAME_TICK_BUDGET_MS` | Tick work budget (`tick_budget_ms`): soft max work/lock-hold target; the shared bootstrap derives about `0.8 x GAME_TICK_DURATION_MS`, while production values are evidence-calibrated under [ADR 0073](../../decisions/adr-0073-evidence-calibrated-tick-budgets-and-lock-ttls.md) | Derived |
...
- Reconnection sections also expose bounded diagnostics when a persisted tenant or game-instance override is disregarded because its merged byte bounds are invalid; the next valid layer remains effective.
