...
- Replacement does not make the runtime mapping a durable playable-state identity: Game Session carries the resolved `playableStateNamespaceId` with the active `{tenantId, gameInstanceId}` binding, fences new admissions during cutover, and accepts the pointer move only after owner-local S1/S2/S3 evidence and registered cleanup acknowledgements. Temporal lifecycle status is coordination evidence; the World Management database row/epoch remains authoritative.
