...
- **Deferred:** Dynamic lighting, line-of-sight filtering, and script-driven room prose remain future work once instrumentation, metrics, and cross-service regression coverage stabilize. Reconnect does not replay cached room snapshots; after authorized reconstruction it obtains a fresh authoritative `LOOK` and emits exactly one reconnect prompt if and only if both effective `firemud.presentation.prompt.enabled` and `firemud.presentation.prompt.emit-after-reconnect-restore` are true, otherwise zero, with any bounded semantic recent context governed by the owner documents above.
