...
- Replacement does not make the runtime mapping a durable playable-state identity: Game Session carries the resolved `playableStateNamespaceId` with the active `{tenantId, gameInstanceId}` binding, fences new admissions during cutover, and accepts the pointer move only after owner-local S1/S2/S3 compatibility evidence and idempotent source-cleanup work registration are in the same pre-CAS commit. The post-swap reconciler obtains authoritative pointer/readback confirmation of that cleanup-work registration and source binding before requesting World’s `TERMINATING` transition; this is not the all-owner terminal cleanup acknowledgement, which follows in World’s `TERMINATING` workflow before `TERMINATED`. A cleanup acknowledgement is not a pre-CAS substitute. Temporal lifecycle status is coordination evidence; the World Management database row/epoch remains authoritative.
