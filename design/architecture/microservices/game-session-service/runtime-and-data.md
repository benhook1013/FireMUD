...
Game Session schedules private `game-session-account-delegation` JWT rotation at approximately half-life with jitter and an expiry safety margin, and single-flights refresh for each binding. The profile audience is `account-service`. Refresh presents the current per-lineage `tokenGeneration` to Account; workload identity by itself cannot mint a replacement after a blocking authority change. Account linearizes replacement with a compare-and-set against the current issued-token record plus the applicable issuer, account, tenant, and membership authority generations and private-realm `grantVersion`. Game Session then compare-and-sets the same expected lineage and authority tuple when atomically installing the new token identity and refreshed membership metadata. A concurrent logout-all, password reset, security lock, membership loss, or realm-grant revocation that advances the applicable authority state before either CAS completes wins; no replacement may be installed afterward. The old entry survives only for the bounded already-started RPC overlap and is then deleted idempotently. Rotation never changes gameplay continuity or resume deadlines. If the current token expires before authority can be refreshed, backend-authenticated actions fail closed and the player must authenticate again. Gameplay-domain RPCs continue to use concrete workload identity and typed `PlayerExecutionContext`; they do not acquire a per-call player JWT requirement from this rotation contract.
