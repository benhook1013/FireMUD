...
Game Session is the sole writer for these active-binding indexes. Gameplay session creation/update plus the tenant-scoped character, account-tenant, tenant, and realm-grant index mutations are one shard-local Redis CAS/update flow under `{tenantGameplayTag}`. Before that CAS publishes a new binding, Game Session commits a deterministic, account-keyed repair obligation in PostgreSQL, idempotently keyed to the binding transition. The CAS may then publish only a provisional binding that references the obligation and carries `accountIndexState=REPAIR_REQUIRED`; it must not publish an admissible binding without prior durable repair evidence. The cross-tenant `session:game:index:account:<accountId>` index remains a separate idempotent secondary write because it cannot share every tenant slot. After that write is acknowledged, Game Session durably marks the obligation acknowledged and uses a follow-up shard-local CAS to set `accountIndexState=ACKNOWLEDGED` and make the binding admissible. This protocol does not claim a cross-store atomic transaction: ordering, idempotency, durable reconciliation, and fail-closed provisional state close each crash window. Until acknowledgement the binding is non-admissible for resume, takeover, or new gameplay admission. Existing bounded in-flight work may finish, but no new admission state may rely on tenant-local presence alone. The repair deadline is no later than the account-wide revocation enforcement bound. The logout-all/revocation consumer checks both the account index and durable repair obligations, so an index outage cannot hide a binding from a current cutoff. If repair does not complete before the deadline, Game Session terminates the provisional binding and records the failed-closed outcome; it does not extend the deadline or treat tenant-local presence as complete account-wide coverage. Bounded reconciliation repairs missing entries and removes stale ones after checking authoritative session records. Indexes never grant authority and no wildcard keyspace scan is part of admission or revocation. Region-local gameplay admission still uses the separate `tick:{tenantRegionTag}:session-binding:<entityId>` bridge contract and is not folded into the same Lua invocation.
