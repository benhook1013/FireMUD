...
- Owner-directory state and the front-end's execution-region pointer are routing projections, not mutation authority. The receiving executor revalidates its fence; movement updates the pointer only after the authoritative durable spatial/effect result.
- Connection-local FIFO uses `sessionSequence`, while retry and duplicate protection use stable command/effect/request identities. Asynchronous results address one active `{sessionId, frontEndGeneration}` so a replaced front end cannot receive them.
