...
Session state needed for reconnect recovery is stored under `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>`. These per-session keys, including session-scoped command queues or metadata, are removed when the corresponding session stops or expires. Every gameplay-binding create/update/heartbeat/refresh must cap the physical Redis TTL at the remaining `continuityBindingExpiresAt` lifetime, for example `min(requestedTtlMs, max(0, continuityBindingExpiresAt - now))`; when the remaining lifetime is `<= 0`, the operation must delete or retire the binding and fail closed rather than issue a zero-TTL success. The TTL must not become a sliding authority or resume deadline.
...
Game Session must also maintain the bounded secondary indexes defined in the authentication architecture as an optimization so takeover, reconnect, and revocation do not require scans; Account authority and the durable Game Session revocation consumer remain the hard logout-all mechanism:
...
Game Session is the sole writer for these active-binding indexes. Every binding transition has a monotonic `binding_generation`, a unique durable transition identity, and an `accountIndexFence` ordered by that generation. Before the session-only CAS publishes a new binding, Game Session commits a PostgreSQL repair obligation containing the expected prior generation, candidate generation, transition identity, desired account-index add/remove, and `accountIndexFence`. The shard-local CAS may publish the candidate only when the expected prior generation and current session owner/fence still match; it records the obligation and `accountIndexState=REPAIR_REQUIRED`, so the candidate is provisional and non-admissible.

The cross-tenant `session:game:index:account:<accountId>` update is a separate idempotent operation because it cannot share every tenant slot. It must carry the transition identity and `accountIndexFence`, add or retain a candidate member before removing an old member, and refuse a remove when the authoritative session binding or durable obligation has a newer generation/fence. A stale add may be repaired as an extra projection, but a stale remove must never hide a newer binding. Only after the exact fenced add/remove outcome is acknowledged does Game Session durably mark the matching obligation acknowledged; a follow-up shard-local CAS promotes the binding to `accountIndexState=ACKNOWLEDGED` only if the same candidate generation/fence is still current. A superseded or conflicting candidate remains non-admissible and cannot downgrade or overwrite the newer binding.

The admission predicate also requires an Account-owned exact-binding admission lease with the applicable authority tuple, committed outbox cutoff checkpoint, and monotonic `leaseFence`. The shard-local binding CAS includes that lease identity and fence and may publish only a provisional candidate until Account finalizes the lease as `COMMITTED`. If a membership, grant, billing, or security cutoff fences the lease first, local admission and late finalization fail closed; if finalization wins first, the binding was admitted under the then-current authority and later revocation follows the bounded event path. Missing, expired, ambiguous, or uncertain lease state never admits a binding.

The reconciler reads authoritative session bindings and durable obligations, compares generations/fences rather than Redis write order, repairs missing or stale cross-tenant index entries idempotently, and rechecks the current generation before recording acknowledgement or removing an index member. This protocol does not claim a cross-store atomic transaction: generation/fence ordering, durable acknowledgement, idempotency, and fail-closed provisional state close each crash window. Until acknowledgement the binding is non-admissible for resume, takeover, or new gameplay admission. Existing bounded in-flight work may finish, but no new admission state may rely on tenant-local presence alone. The repair deadline is no later than the account-wide revocation enforcement bound. The logout-all/revocation consumer checks both the account index and durable repair obligations, so an index outage cannot hide a binding from a current cutoff. If repair does not complete before the deadline, Game Session terminates the provisional binding and records the failed-closed outcome; it does not extend the deadline or treat tenant-local presence as complete account-wide coverage. Indexes never grant authority and no wildcard keyspace scan is part of admission or revocation. Region-local gameplay admission still uses the separate `tick:{tenantRegionTag}:session-binding:<entityId>` bridge contract and is not folded into the same Lua invocation.

Gameplay session bindings must include the server-side auth token identity used for Account and other control-plane calls on behalf of the session, including `authTokenHash`, `authTokenIssuedAt`, `authTokenExpiresAt`, monotonic `authTokenGeneration`, and the secret-valued Account-issued single-use rebind handle, plus authoritative membership and applicable realm-grant freshness metadata so resume, rotation, and active revocation can validate current identity, current authority, and current revocation state before rebinding to a fresh backend token. The raw `game-session-account-delegation` JWT remains process-local and is never persisted in the binding. The rebind handle is stored only in the protected Game Session binding, excluded from logs/metrics/traces, and usable solely by the exact Game Session workload against Account's rebind API:
...
- applicable issuer, account, tenant, and membership authority generations and private-realm `grantVersion` do not block the account, tenant, or selected realm.
- the handle is bound to this binding, token hash/generation, account/tenant scope, authority tuple, and continuity deadline and has not already been consumed.
...
Game Session consumes Account-owned membership, grant, security, and hard-billing events through an idempotent durable consumer. Stable event IDs and monotonic authority versions make duplicates and older events no-ops; a gap triggers authoritative reconciliation. A delivered and accepted authority event drives immediate targeted termination through the bounded indexes, including socket closure and removal of admission state. The periodic batched authority-generation/version reconciliation is the missed/delayed-event safety net: it renews a revocation-authority freshness lease for active bindings at least once every 60 seconds, and active bindings whose authority cannot be re-established terminate within that bound. New admission fails closed while that lease is stale. Routine gameplay commands do not read Account or authority-generation state.

Game Session schedules private `game-session-account-delegation` JWT rotation at approximately half-life with jitter and an expiry safety margin, and single-flights refresh for each binding. The profile audience is `account-service`. Normal refresh presents the current raw token and per-lineage `tokenGeneration` to Account. After restart or takeover, where the raw token is unavailable, the new owner exchanges the durable single-use rebind handle; Account consumes it and revalidates the bound registry lineage, gameplay binding, complete authority tuple, and continuity deadline before returning a fresh token and replacement handle. Workload identity, token hash, or binding identifiers alone cannot mint a replacement. Account linearizes either replacement with a compare-and-set against the current issued-token record plus the applicable issuer, account, tenant, and membership authority generations and private-realm `grantVersion`, and returns an Account-owned installation lease with a monotonic lease fence. Before changing the protected binding, Game Session must atomically claim that unexpired lease; the binding CAS includes the expected old binding generation and lease fence, and the candidate replacement is not admissible for new calls until Account durably finalizes the claim as installed. A lease expiry, abort, reconciliation, logout-all, password reset, security lock, membership loss, or realm-grant revocation wins by fencing the claim before finalization; a stale Game Session CAS is then not a successful installation and the candidate binding is terminated or removed. The old entry survives only for the bounded already-started RPC overlap and is then deleted idempotently. Rotation and rebind never change gameplay continuity or resume deadlines. A receiver-token expiry or transient refresh failure permits fresh `LOGIN`/`PLAY` to consume a still-valid continuity episode when current Account authority passes; Account or tenant authority revocation is terminal for the old binding and cannot be resumed. A missing, replayed, cross-binding, expired, or ambiguous rebind handle fails closed; if authority cannot be refreshed before expiry, backend-authenticated actions fail and the player must authenticate again. Gameplay-domain RPCs continue to use concrete workload identity and typed `PlayerExecutionContext`; they do not acquire a per-call player JWT requirement from this rotation contract.

Rebind-handle exchange uses a stable exchange request ID and an immutable request digest for the binding, consumed-handle identity, token lineage, and authority tuple. Account durably binds that operation and the consumed handle/replacement outcome to the Account-owned installation lease before exposing a replacement, following ADR 0031's `TOKEN_ROTATION` operation and ADR 0035's durable idempotency evidence. Account may retain the issuance result only in a bounded Account-encrypted response envelope bound to that exact request, caller workload, binding, lineage, replacement, and lease fence. If the response is lost, retrying the same request ID can recover the same JWT and replacement handle from that envelope after current-state revalidation; it does not consume the handle again, reconstruct a credential from hashes, or invent a Game Session-local signing path. The envelope is not authorization or registry evidence and expires no later than the applicable installation, token, or idempotency deadline. Missing, unreadable, expired, or mismatched envelope state causes Account to revoke the orphan candidate and fail closed. A different request ID, digest, handle, binding, authority tuple, or lease fence is an idempotency conflict and fails closed rather than replaying or widening the exchange.
