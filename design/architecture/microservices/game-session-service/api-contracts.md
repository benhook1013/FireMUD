...
- `ScheduleRemoteFollowup` – now accepts first-class trigger-script-event identity too (`eventType`, `eventSchemaVersion`, `scriptEventId`, `triggerMode`, `readSnapshotToken`, `eventPayloadJson`) so that the `trigger_script_event` family persists and replays from durable columns instead of payload-blob-only identity. The current `readSnapshotToken` is an owner-specific manifest input or correlation value, not universal evaluation authority. The target handler-scoped input manifest is owned by the [DSL lifecycle contract](../../system-architecture-scripting-dsl-reference-and-lifecycle.md#read-consistency-contract); current producers and storage do not yet seal the complete manifest.
