...
The current proto remains incomplete against this target. `StartSessionRequest`, `PauseTicksForScopeRequest`, and `ResumeTicksForScopeRequest` carry `controlPlaneRequestId` but not `mutationDigest`; those RPCs must not be reported as ADR-0048-complete until the missing request fields, authorization binding, owner-side digest verification, replay persistence, and focused proof converge together.
