...
- `GetPinnedScriptPatchVersion` returns the authoritative exact `scriptPatchVersion` and monotonic `scriptPinEpoch` for `(tenantId, gameInstanceId)`, together with the persisted `controlPlaneRequestId`, timestamp, and actor, at the time of the read. It does not promise same-request retry snapshot semantics unless a future explicit snapshot contract is added.
- `GetGameSessionPinConvergence` – reports the persisted Game Session-side pin observation for `(tenantId, gameInstanceId)`: `observedPinnedScriptPatchVersion`, `observedScriptPinEpoch`, `lastObservedControlPlaneRequestId`, and `observedAt`. Automation treats its own convergence read as a projection and must not use it as a substitute for this owner read.
- The Game Session control-plane read surface also exposes a bounded, stable-order, paginated rollout history for `(tenantId, gameInstanceId)`. Each immutable entry records the request identity, operation kind, previous and resulting exact pin tuples, actor/reason where applicable, outcome, and commit time. Once a syntactically valid request is accepted and its `controlPlaneRequestId` is bound to the normalized digest, a deterministic validation or preparation failure persists one immutable unsuccessful history attempt with previous and resulting tuples equal and no epoch advance; an exact retry returns it, while a different digest conflicts. A repeated request identity does not append another logical history entry. This history is authoritative even when Automation projections or notifications are unavailable.
