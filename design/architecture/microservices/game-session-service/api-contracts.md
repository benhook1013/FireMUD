...
For script transitions, the exact `scriptPinEpoch`, atomic pin/history commit, and bounded authoritative history read are target-state consequences. The current implementation's patch/request convergence read and existing version-fence seams do not prove complete epoch propagation or Game-Session-owned append-only rollout history; the tracker retains those gaps.
...
- The Game Session control-plane read surface also exposes a bounded, stable-order, paginated rollout history for `(tenantId, gameInstanceId)`. Each immutable entry records the request identity, operation kind, previous and resulting exact pin tuples, actor/reason where applicable, outcome, and commit time. A repeated request identity does not append another logical history entry. This history is authoritative even when Automation projections or notifications are unavailable.
