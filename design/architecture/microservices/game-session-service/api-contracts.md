...
- `GetPinnedScriptPatchVersion` returns the authoritative exact `scriptPatchVersion` and monotonic `scriptPinEpoch` for `(tenantId, gameInstanceId)`, together with the persisted `controlPlaneRequestId`, timestamp, and actor. Same-request retries return the same tuple.
