...
- `ListRemoteCommandCoordinators` – returns bounded remote coordinator rows for a tenant, filterable by persisted origin/target region scope, state, linked `followupId`, and script/plugin/dispatch/command provenance instead of requiring point lookup by one coordinator id.
