...
- `ExecutePreparedVersionCutover` – executes the canonical prepared cutover workflow for one realm pointer. The request identifies the realm, replacement `gameInstanceId`, durable `preparedVersionUpgradeId`, and the expected pointer version; Game Session resolves the source from the current pointer and exact preparation, then revalidates that `sourceGameInstanceId` and target instance match before performing a real database CAS. The pointer, audit event, request replay outcome, preparation execution state, source instance, unique `sourceDrainId`, resolved drain-policy version/duration, absolute `sourceDrainDeadlineAt`, an initially inactive source command-fence identity, and drain-enforcement work item commit atomically. Before any database CAS, Game Session resolves and validates the effective `firemud.game-session.cutover-drain.duration-ms` in the inclusive range `0..300000`; an invalid platform or tenant/game value rejects the request without changing the pointer. The duration defaults to five minutes and may be shortened or set to zero by tenant/game policy. The drain reconciler activates the persisted fence at `sourceDrainDeadlineAt`, immediately for zero duration; command handling rejects source commands only after that activation, and the durable work item is immediately due for zero duration. `sourceDrainId` and execution evidence remain available for replay.
- `GetPreparedVersionUpgrade` returns execution state once a prepared cutover has run: the replacement and exact source `gameInstanceId` values, resulting pointer version, execution timestamp/request id, `sourceDrainId`, and absolute `sourceDrainDeadlineAt` are frozen onto the durable preparation artifact in the same logical commit as the route swap. Readback is valid only when the source value still matches the prepared artifact and the cutover's source pointer identity. Game Session exposes drain state until early-empty completion or deadline-enforced transition to the idempotent `InstanceTermination` workflow; a zero-duration drain reports its immediate deadline and reconciliation state rather than a null exemption.
- The prepared-version cutover commit must also persist one idempotent drain-enforcement outbox/work item, keyed by `{tenantId, sourceGameInstanceId, sourceDrainId}`, in the same atomic database outcome as the pointer swap and initially inactive source command-fence identity. The work item, every acknowledgement, and every authoritative readback must carry and exactly match that same `sourceGameInstanceId`; a missing, changed, or ambiguous source binding is not a successful reconciliation. A zero-duration cutover makes that work item immediately due; a non-zero drain leaves it pending until early-empty completion or its deadline. At the deadline the reconciler activates the fence, then command handling rejects source commands. It unconditionally attempts one bounded update notice before closing sockets; the bounded attempt cannot hold sockets open indefinitely. The reconciler owns notice delivery, socket closure, readback, and the idempotent `InstanceTermination` transition.
- A dispatcher reconciles the durable work item after commit and retries delivery until the source instance's termination state is acknowledged by authoritative readback. Lost responses, dispatcher restarts, and duplicate deliveries are recovered by the stable work-item identity, source instance binding, and acknowledgement state; the cutover result is not reported as fully completed merely because an in-memory termination request was sent.
