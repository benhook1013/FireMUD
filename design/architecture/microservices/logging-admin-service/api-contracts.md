...
| Player-bootstrap report (HTTP) | `/reports` | Target only: dedicated player ingress/delegation route with exact `player-bootstrap` profile, caller-bound tenant membership, and `membership_authority_generation_applies` plus its conditional `membership_authority_generation_condition` when the route requires it | Until the target binding exists, exposed report submission is disabled or fails closed. The current OpenAPI/controller accepts caller-supplied `tenantId` and `reporterAccountId`, so it is not an operator authority path or an accepted player contract. |
