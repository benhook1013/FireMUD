...
- `POST /reports` – current administrative report-persistence route. The current OpenAPI/controller accepts caller-supplied `tenantId` and `reporterAccountId`, so it is not a caller-bound player route and remains implementation drift. The fail-closed target derives reporter and tenant identity from the validated `player-bootstrap` session and evaluates routing and authorization under the dedicated player ingress/delegation policy.
