...
- `POST /admission-pointers/version-upgrades` – operator-facing preparation call that persists the Game Session `PrepareVersionUpgrade` compatibility proof for a source instance and target version under the caller-supplied `controlPlaneRequestId`. That identifier is the canonical durable idempotency identity for the preparation and is reused for retries; no separate idempotency key exists.
