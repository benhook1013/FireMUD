...
- Feature-flag toggles and scoped tick-remediation pause/resume forwarding are live executable operator mutations in the current service. Admission-pointer CAS open/close and prepared cutover are target-state mutations, not current implementation claims.
- The current live operator mutation surface is limited to feature-flag and tick pause/resume families; moderation is currently persistence-only. Admission-pointer reads, audit, and version-upgrade preparation/read are live control-plane surfaces, while admission-pointer CAS and cutover remain target-state. These distinctions do not mean that read, audit, log-search, saga, metrics, or dashboard/observability endpoints are absent; those are separate live read and investigation surfaces.
...
| Owner mutation calls (gRPC internal) | Current feature-flag and tick owner RPCs; target admission, moderation, quota, and broader-remediation owner RPCs | Exact Logging & Admin mTLS identity plus Account redemption of the matching human or versioned automation authorization reference | The owner validates domain facts, fencing, and idempotency. Logging & Admin assertions alone are not authority. |
