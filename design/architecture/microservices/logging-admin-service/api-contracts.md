...
- The current `POST /reports` controller accepts caller-supplied `tenantId` and `reporterAccountId`, persists the report, and returns `200 OK` with `ApiResponse<ReportDto>` through the current `/api/admin/reports/**` Gateway route. Those supplied references are report data only and do not authorize the operator or tenant; the authenticated operator is the audit authority for the administrative action. This is an administrative persistence seam, not an accepted player submission contract. The target player route instead binds the `player-bootstrap` subject and live tenant membership and is not implemented.
...
- `POST /reports` – current administrative report persistence accepts caller-supplied `tenantId` and `reporterAccountId` as data fields and returns `200 OK` with `ApiResponse<ReportDto>`. Those fields do not authorize the operator or tenant; the authenticated operator is the audit authority. It is not the target player route, which derives reporter and tenant identity from a validated `player-bootstrap` session and applies the dedicated player ingress/delegation policy.
...
- `CreateReport(CreateReportRequest) returns (CreateReportResponse)` – administrative report persistence using caller-supplied tenant and reporter identities as data fields; those fields do not authorize the operator or tenant, and player submission is target-only and unavailable.
