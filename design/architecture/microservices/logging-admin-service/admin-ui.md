...
- Admission-pointer reads, audit, and prepared-upgrade proof reads remain API-only to this UI. `POST /admission-pointers/version-upgrades` is an unavailable/gated preparation mutation pending the owner-side variant of the Operator Mutation Support Gate; it is not a read surface. Same-target CAS open/close and prepared cutover mutations are implemented but externally gated and have no current UI control; session-lifecycle controls, broader remediation, versioned moderation propagation, and richer dashboards remain target-only. Current runtime enforcement occurs in Game Session and Social & Groups from Logging & Admin policy evaluations; the target inventory below must not be read as proof that broader surfaces are live.
