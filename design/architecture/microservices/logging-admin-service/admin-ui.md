...
- Admission-pointer reads, audit, and prepared-upgrade proof reads remain API-only to this UI. `POST /admission-pointers/version-upgrades` is an unavailable/gated preparation mutation pending the owner-side variant of the Operator Mutation Support Gate; it is not a read surface. Admission-pointer open/close/retarget CAS and prepared cutover mutations are implemented but externally gated and have no current UI control; session-lifecycle controls, broader remediation, versioned moderation propagation, and richer dashboards remain target-only. Current runtime enforcement occurs in Game Session and Social & Groups from Logging & Admin policy evaluations; the target inventory below must not be read as proof that broader surfaces are live.
...
- Unavailable/gated mutation: `POST /admission-pointers/version-upgrades` remains unavailable pending the owner-side variant of the Operator Mutation Support Gate; `GET /admission-pointers/version-upgrades/{tenantId}/{preparationId}` is the read-only prepared-upgrade proof surface.
...
- Implemented-but-externally-gated mutations not presented by this UI: admission-pointer open/close/retarget CAS and prepared cutover.
...
The live admission-pointer reads, audit, and prepared-upgrade proof GET are intentionally excluded from this UI inventory: no corresponding React/admin surface is implemented in this module. Operators must use the documented `/admission-pointers*` API family for those live reads. The preparation POST is an unavailable/gated mutation pending the owner-side variant of the Operator Mutation Support Gate; it is not an investigation read. Admission-pointer open/close/retarget CAS and prepared cutover mutations are implemented but externally gated and are not current operator controls.
...
Backend endpoints for the live UI-listed features are available as described in the [API contracts](./api-contracts.md). The React interface consumes only those documented UI routes directly. Other live backend surfaces, including admission-pointer reads, audit, and prepared-upgrade proof reads, remain API-only until a UI is implemented; feature-flag, version-upgrade preparation, tick-forwarding, and the implemented-but-externally-gated admission-pointer open/close/retarget CAS and prepared cutover mutations remain unavailable pending the owner-side variant of the Operator Mutation Support Gate. Public administrative `/admin/reports` persistence and reserved player-bootstrap `POST /reports` remain unavailable for their separate documented reasons.
