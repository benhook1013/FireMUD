...
- Player report review is target coverage; record moderation policy input and audit only once a caller-bound report-submission surface exists. The current `/reports` submission route is disabled or fails closed until player-bootstrap subject and tenant-membership binding exist. The UI does not issue bans or warnings and does not enforce moderation outcomes; owner-side enforcement is a target-state path.
