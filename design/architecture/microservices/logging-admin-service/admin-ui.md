...
- The admin UI is a lightweight operator surface. Its live scope is log search, saga inspection, and analytics. `/moderation/actions` is unavailable/gated, as are feature-flag toggles and per-instance tick-remediation pause/resume, until each action family has its schema, shared cross-language `mutationDigest/v1` golden vectors, and Account-issued authorization-reference flow. The separate backend `EvaluateModerationPolicy` contract is consumed by Game Session for `GAMEPLAY_ADMISSION` and Social & Groups for `CHAT_SEND`; it is not an admin UI capability. Versioned propagation and broader enforcement remain unavailable.
...
- Admission-pointer reads, audit, and prepared-upgrade proof reads remain API-only to this UI. `POST /admission-pointers/version-upgrades` is an unavailable/gated preparation mutation, not a read surface. Same-target CAS open/close and prepared cutover mutations are target-only, as are session-lifecycle controls, broader remediation, versioned moderation propagation, and richer dashboards. Current runtime enforcement occurs in Game Session and Social & Groups from Logging & Admin policy evaluations; the target inventory below must not be read as proof that broader surfaces are live.
...
- Target capability: toggle runtime feature flags for a specific tenant after its conformance gate passes.
...
- Unavailable/gated mutation: `POST /admission-pointers/version-upgrades` remains unavailable until the same three-part mutation gate passes; `GET /admission-pointers/version-upgrades/{tenantId}/{preparationId}` is the read-only prepared-upgrade proof surface.
- Gated implemented owner-forwarding paths not presented as usable UI controls: `POST /feature-flags/toggle`, `POST /tick-remediation/pause`, and `POST /tick-remediation/resume` remain unavailable pending their action schemas and shared `mutationDigest/v1` vectors.
...
The live admission-pointer reads, audit, and prepared-upgrade proof GET are intentionally excluded from this UI inventory: no corresponding React/admin surface is implemented in this module. Operators must use the documented `/admission-pointers*` API family for those live reads. The preparation POST is an unavailable/gated mutation, not an investigation read. Same-target CAS open/close and prepared cutover mutations are target-only and are not current operator controls.

Current-state note: `POST /tick-remediation/pause` and `POST /tick-remediation/resume` have Logging & Admin forwarding implementations for a specific `<tenantId, gameInstanceId>` scope, but the admin APIs and UI remain target/unavailable until the three-part conformance gate passes. Game Session `/sessions*` lifecycle routes are current owner-local hooks, not current UI or Logging & Admin ingress. Quota override and broader remediation are coverage drift: no current routes or owner-side contracts exist, so the UI must not present placeholder controls.

`POST /moderation/actions` is an unavailable/gated policy-input persistence/audit route for a selected target player. It is independent of unavailable public administrative `/reports` persistence and the unavailable target player-submission route. The UI does not directly perform runtime enforcement or policy evaluation; Game Session and Social & Groups consume synchronous `EvaluateModerationPolicy` decisions, while versioned propagation remains unavailable. Log search, saga inspection, admission-pointer reads, audit, prepared-upgrade proof GET, and analytics dashboards remain read or investigation surfaces; the version-upgrade preparation POST remains an unavailable/gated mutation. This UI does not expose `/reports` as an investigation or player-submission surface.
