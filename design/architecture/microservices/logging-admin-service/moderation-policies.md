...
Target behavior, once the mutation gate is complete, is for Logging & Admin to durably record policy intent/case/audit evidence and then issue a typed digest-bound owner command; the receiving owner redeems/authorizes that command and commits local enforcement. Current behavior is the gated/unavailable action path: it performs no moderation persistence, owner forwarding, redemption, or enforcement. The live `EvaluateModerationPolicy` read is the only current moderation decision seam.
...
Logging & Admin rejects unknown or legacy values such as `ban` and `account_ban`; the protective `account_security_lock` is the Account-owned fixed category and is not a punitive moderation alias. Unknown values are never heuristically translated. The owner durably commits the accepted revision, current effective projection, and idempotent result together. Same request identity with the same digest replays the stored result; a different digest conflicts. Delayed, duplicate, or reordered revisions cannot erase newer state or resurrect older state.

In the target owner workflow, one `controlPlaneRequestId` and digest spans intent, forwarding, owner result, and audit. Logging & Admin durably records actor, reason, scope, mutation, request identity, and normalized digest before forwarding. The receiving owner independently authorizes the current actor/scope and validates current domain facts, then atomically commits its state and result. Success is reported only after that commit. Lost responses or final audit failures are reconciled by the same identity and digest; a timeout or ambiguous result never creates a new mutation identity. This forwarding description is target behavior only; the current action path is gated/unavailable as stated above.
...
3. In the target workflow, Logging & Admin validates the closed vocabulary, policy, actor, scope, and safe notice, records the durable intent/case/audit row, and forwards the typed command when the owner contract is available; the receiving owner redeems/authorizes and commits enforcement. The current mutation is gated/unavailable and performs none of that persistence or forwarding.
