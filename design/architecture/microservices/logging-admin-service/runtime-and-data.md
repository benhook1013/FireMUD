...
- The current moderation action path persists policy input and audit only; no owner-side enforcement RPC is exposed, so versioned propagation and downstream enforcement remain target coverage rather than a claimed implementation.
...
- Core control-plane endpoints include moderation policy-input persistence actions, live feature-flag controls, reports, saga inspection, live admission-pointer reads/audit/preparation/cutover/same-target mutations, and live tick-remediation pause/resume APIs. These paths must not block on Elasticsearch, Prometheus, Jaeger, Grafana, Kibana, or Alertmanager for request success. Quota override, moderation enforcement, and broader remediation are hypothetical target families, not current endpoints.
