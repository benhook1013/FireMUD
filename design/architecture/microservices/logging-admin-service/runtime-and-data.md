...
- Moderation policy-input and audit persistence is supported through authenticated `POST /moderation/actions` and the internal `ApplyModerationAction` gRPC ingress. These paths persist `moderation_actions` records only; they do not apply Account, Game Session, or Social & Groups enforcement state. Public administrative report persistence is unavailable: the HTTP controller and Gateway route were removed because canonical authorization and live reference validation are not implemented. The internal `CreateReport` gRPC ingress remains behind the existing mTLS/workload boundary; the caller-bound external/player submission route is also unavailable.
