...
- The current moderation action path persists policy input and audit only; it does not forward or enforce a mutation. Runtime owners call the live `EvaluateModerationPolicy` read at their enforcement boundaries. Versioned snapshot/event propagation and broader enforcement remain target work.
...
- Exposes admin APIs and UI controls to:
  ...
  - request the implemented scoped tick pause/resume operation through Game Session control APIs after its action-schema and digest-vector conformance gate is complete; broader reset/remediate actions are hypothetical target routes and remain unavailable until the owner API exists.
- Implements guarded automation that:
  - enumerates affected game instances from coordination-health signals and, once the conformance gate is met, calls Game Session `PauseTicksForScope`/`ResumeTicksForScope` for each exact `<tenantId, gameInstanceId>` scope; regional pause/resume and other regional mutations remain target-state only; and
...
- All admin APIs are secured via role-based access control integrated with the Account Service. Human operator writes require the current `control-ui` identity and Account's human authorization reference; unattended automation uses the exact Logging and Admin mTLS identity and Account's typed, versioned automation-policy authorization reference rather than a human token or identity.
...
- Currently live core control-plane endpoints include moderation policy-input persistence and synchronous evaluation, saga inspection, and admission-pointer reads, audit, and prepared-upgrade proof reads. Feature-flag mutation, admission open/close/retarget and cutover, version-upgrade preparation, and per-instance tick pause/resume have implementation participation but remain externally disabled until their action-family schemas and shared `mutationDigest/v1` vectors converge; regional or broader remediation remains target-only. When those gated paths become executable, they belong to the core control-plane partition and must not block on Elasticsearch, Prometheus, Jaeger, Grafana, Kibana, or Alertmanager for request success. Public administrative `/reports` persistence is unavailable pending canonical checks; the internal gRPC report ingress is separate, and the target caller-bound player-submission surface is unavailable and is not a current player-facing data path. Quota override and versioned moderation propagation remain target families.
