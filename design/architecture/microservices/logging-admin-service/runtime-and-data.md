...
- Admission-pointer reads, audit, and prepared-upgrade proof reads are implemented and live. Version-upgrade preparation is a mutation and remains externally disabled pending the action schema, shared `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption; open/close/retarget CAS and cutover remain target-only and are not current endpoint capabilities.
- Per-instance tick pause/resume forwarding is implemented at the `<tenantId, gameInstanceId>` boundary but remains externally disabled pending the action schema, shared `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption. Regional pause/resume and regional remediation remain target-only behavior, with no claim that the implemented instance control implicitly expands to a region or aggregate scope; they are tracked in [Game Session runtime and tick coordination](../../../project-management/implementation-tracking/game-session-runtime-and-tick-coordination.md#capability-status).
...
- The current moderation action path is an unavailable/gated implementation path and is not supported operator mutation. `/moderation/actions` and `ApplyModerationAction` remain unavailable until the action schema, shared `mutationDigest/v1` golden vectors, and Account authorization-reference issuance/redemption exist. Runtime owners call the live `EvaluateModerationPolicy` read at their enforcement boundaries. Versioned snapshot/event propagation and broader enforcement remain target work.
...
Operator write requests use one canonical logical `controlPlaneRequestId` across HTTP, Java/domain contracts, ADRs, and audit records. Protobuf contracts may use the wire spelling `control_plane_request_id`, which maps directly to that same identifier. Human requests use the typed Account `IssueHumanOperatorAuthorizationReference` path and require the current `control-ui` identity; unattended automation uses the typed Account `IssueAutomationOperatorAuthorizationReference` path with `exact_mtls_workload_plus_versioned_automation_policy`, bound to the exact Logging and Admin workload mTLS identity and its `automation_policy_id`/`automation_policy_version`. Unattended authorization contains no user account, end-user token, or human identity, and neither path may substitute the other's identity.
...
- Exposes admin APIs and UI controls to:
  ...
  - request the implemented scoped tick pause/resume operation through Game Session control APIs after the action schema, shared digest-vector, and Account authorization-reference issuance/redemption conformance gate is complete; broader reset/remediate actions are hypothetical target routes and remain unavailable until the owner API exists.
- Implements guarded automation that:
  - enumerates affected game instances from coordination-health signals and, once the three-part conformance gate is met, calls Game Session `PauseTicksForScope`/`ResumeTicksForScope` for each exact `<tenantId, gameInstanceId>` scope; regional pause/resume and other regional mutations remain target-state only; and
...
- Currently live core control-plane endpoints include synchronous `EvaluateModerationPolicy`, saga inspection, and admission-pointer reads, audit, and prepared-upgrade proof reads. Feature-flag mutation, admission open/close/retarget and cutover, version-upgrade preparation, per-instance tick pause/resume, and `ApplyModerationAction` have implementation participation but remain unavailable until their action-family schemas, shared `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption converge; regional or broader remediation remains target-only. When those gated paths become executable, they belong to the core control-plane partition and must not block on Elasticsearch, Prometheus, Jaeger, Grafana, Kibana, or Alertmanager for request success. Public administrative `/reports` persistence is unavailable pending canonical checks; the internal gRPC report ingress is separate, and the target caller-bound player-submission surface is unavailable and is not a current player-facing data path. Quota override and versioned moderation propagation remain target families.
...
- `moderation_actions` is the target durable record for moderation policy input and audit evidence with timestamps and includes a `tenant_id` column; the unavailable/gated `ApplyModerationAction` path does not represent owner-side enforcement state.
...
- The current `POST /moderation/actions` path is unavailable/gated and does not invoke an owner-side enforcement RPC. It cannot be treated as supported moderation mutation until the action schema, shared `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption exist.
