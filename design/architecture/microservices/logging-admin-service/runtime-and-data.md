...
For script transitions, the complete owner-read composition is target-state and currently unavailable; current operator routes expose only bounded Game Session current-pin and convergence reads, as described by the [API/current-status boundary](./api-contracts.md#script-transition-read-and-audit-consequences-target-state-complete-composition-unavailable) and linked implementation trackers. Target-state operator responses compose owner reads at response time: Game Session supplies authoritative pin/history evidence and Automation supplies readiness, convergence, plugin policy/runtime, schedule, and work/dead-letter diagnostics. The canonical shared tuple, plugin, projection, and audit-evidence schema is defined by the [scripting contracts](../../system-architecture-scripting-contracts.md), [scripting control-plane API](../../system-architecture-scripting-control-plane-api.md), and [scripting observability contract](../../system-architecture-scripting-observability-contract.md). A successful Game Session result with both pin fields absent is semantic `UNPINNED`, regardless of whether bounded history is empty or contains unsuccessful attempts; empty history is a valid `UNPINNED` result, not `UNAVAILABLE`. `UNAVAILABLE` is reserved for owner/dependency failures, missing reads, or ambiguous/incomplete evidence, while successful Automation responses with stale or missing observations are reported as projection lag/convergence. Logging & Admin does not persist a separate composed state/read model, pin/history copy, or rollback state machine, and never resolves disagreement by writing a projection or by choosing Automation history over Game Session history.
