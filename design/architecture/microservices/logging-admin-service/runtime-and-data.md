...
- The current moderation policy-input and audit persistence path is live and authenticated through `POST /moderation/actions` and the internal `ApplyModerationAction` gRPC ingress, but it is not currently a supported owner-side enforcement mutation. These paths persist `moderation_actions` records only; they do not apply Account, Game Session, or Social & Groups enforcement state. Public administrative report persistence is unavailable: the HTTP controller and Gateway route were removed because canonical authorization and live reference validation are not implemented. The internal `CreateReport` gRPC ingress remains behind the existing mTLS/workload boundary; the caller-bound external/player submission route is also unavailable.
...
- The current moderation action path is supported for policy-input and audit persistence only. `/moderation/actions` and `ApplyModerationAction` do not represent owner-side enforcement mutation. The target owner-side action schema, shared `mutationDigest/v1` golden vectors, Account authorization-reference issuance/redemption, versioned snapshot/event propagation, and broader enforcement remain gated or target work. Runtime owners call the live `EvaluateModerationPolicy` read at their enforcement boundaries.
...
- Exposes admin APIs and UI controls to:
  ...
  - request the scoped tick pause/resume operation through Game Session control APIs only after that three-part conformance gate is complete; broader reset/remediate actions are hypothetical target routes and remain unavailable until the owner API exists.
...
- Currently live core control-plane endpoints include moderation policy-input/audit persistence, synchronous `EvaluateModerationPolicy`, saga inspection, and admission-pointer reads, audit, and prepared-upgrade proof reads. Feature-flag mutation, admission open/close/retarget and cutover, version-upgrade preparation, per-instance tick pause/resume, and owner-side moderation enforcement remain gated or target-only pending their action-family schemas, shared `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption; regional or broader remediation remains target-only. When those gated paths become executable, they belong to the core control-plane partition and must not block on Elasticsearch, Prometheus, Jaeger, Grafana, Kibana, or Alertmanager for request success. Public administrative `/reports` persistence is unavailable pending canonical checks; the internal gRPC report ingress is separate, and the target caller-bound player-submission surface is unavailable and is not a current player-facing data path. Quota override and versioned moderation propagation remain target families.
...
- `moderation_actions` is the current durable record for accepted moderation policy input and audit evidence, with timestamps and a `tenant_id` column. The current `POST /moderation/actions` and `ApplyModerationAction` paths persist this record only; owner-side enforcement records, snapshots, and propagation remain target work.
...
- The current `POST /moderation/actions` path persists policy input and audit evidence but does not invoke an owner-side enforcement RPC. It must not be described as the target ban/mute mutation until the action schema, shared `mutationDigest/v1` vectors, and Account authorization-reference issuance/redemption exist.
