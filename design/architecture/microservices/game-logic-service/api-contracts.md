...
This is the local consequence of [ADR 0112](../../decisions/adr-0112-typed-bounded-gameplay-effect-extension.md): Game Logic resolves bounded, release-pinned target plans and typed effect outcomes from owner-scoped fact snapshots. It does not evaluate arbitrary script predicates, accept caller-selected target ids as authority, or expose an unbounded extension path; Entity Management remains the mutation owner.
...
- In the target causal-read path, Game Session allocates the complete `CausalReadFence`, including the operational `regionId`, from durable region commit authority before invoking `ResolveLook`. Game Logic propagates that fence unchanged to World and Entity, accepts only participant proofs for the same region/scope/epoch with `servedThroughTickId >= committedTickId`, and rejects or retries behind-floor or mixed-region/scope/epoch responses. The floor and composite-identity details remain owned by the [Identifier Glossary](../../system-architecture-identifier-glossary.md#cross-service-causal-read-fence-identity) and [ADR 0059](../../decisions/adr-0059-causal-floor-cross-service-presentation-reads.md); the current request/proto path remains floor-free.
