...
- Game Logic resolves target selectors at durable execution time against the source actor, frozen release declaration, Entity Management actor state, and World Management occupancy state. It first uses the typed platform candidate selector, then evaluates the release-pinned DML `TargetingPolicy` over the candidate facts: `discoverableWhen` before `eligibleWhen`.
