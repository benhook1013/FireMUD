...
- Every player-delegated gameplay RPC carries the complete validated typed `PlayerExecutionContext`, including `tenantId`, `playableStateNamespaceId`, server-derived `playableStateScope`, active `gameInstanceId`, and applicable region/epoch or executor fences. Game Logic validates that context against the request and owner responses, never treats caller-supplied IDs as authority, remains stateless, and owns semantic gameplay outcomes; Game Session owns session, reconnect, and presentation state. See the [PlayerExecutionContext contract](../../system-architecture-authentication.md#gameplay-player-execution-context-contract-normative).
