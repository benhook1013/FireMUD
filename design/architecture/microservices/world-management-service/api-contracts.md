...
Every Draft `ApplyWorldDesignMutation` must first lock and check the same owner-freeze row for its `(tenantId, versionId)` target, including an ordinary or otherwise unbound mutation that does not carry a publication request identity. A mutation that belongs to a publication request must additionally carry `publicationRequestId`, the exact `requestDigest`, the exact observed Game Design `versionStateEpoch`, and the exact WMS `publicationFence` from that acknowledgement; the row must be open for that request and exact digest. An ordinary mutation may proceed only while the owner row is in its writable `OPEN` state with no active publication freeze. Both forms reject or drain stale or mismatched evidence, `PUBLISHED`, `RECONCILIATION_REQUIRED`, and any active `FROZEN` phase; an `ABORTED` attempt remains closed to ordinary writes until its failed or ambiguous outcome is reconciled and the owner row is reopened without accepting the old fence. A pre-freeze writer that already holds the row lock may finish before the acknowledgement; no writer reaching the lock afterward may commit. The owner mutation result and updated digest checkpoint remain bound to the applicable request, digest, and fence.
