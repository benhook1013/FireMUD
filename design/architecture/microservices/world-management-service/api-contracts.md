...
WMS authenticates the caller, resolves the target `(tenantId, versionId)`, verifies that Game Design still reports the expected Draft state/epoch, and in one owner-local transaction locks or creates the durable owner-freeze row for that pair, persists `publicationRequestId`, `requestDigest`, the observed Game Design `versionStateEpoch`, one equality-only opaque `publicationFence`, and the WMS digest checkpoint `{appliedCommitId, contentDigest, digestSchemaVersion}`, then changes the phase to `FROZEN`. The successful acknowledgement is emitted only after that transaction commits. The phase vocabulary is `OPEN`, `FROZEN`, `PUBLISHED`, `ABORTED`, and `RECONCILIATION_REQUIRED`; `FROZEN` is entered only after the row, opaque fence, and checkpoint commit, while terminal attempt evidence remains available for exact replay before a later attempt may reopen an `ABORTED` row. Every Draft mutation takes the same row lock, so a writer that reaches the lock after the freeze cannot commit an unbound `OPEN` mutation. A missing/changed epoch, missing Draft authority, unauthorized caller, conflicting nonterminal request, or changed request digest fails closed.
...
Every Draft `ApplyWorldDesignMutation` must first lock and check the same owner-freeze row for its `(tenantId, versionId)` target, including an ordinary or otherwise unbound mutation that does not carry a publication request identity. An ordinary mutation may proceed only while the owner row is in its writable `OPEN` state with no active publication freeze. A publication-bound mutation must additionally carry `publicationRequestId`, the exact `requestDigest`, the exact observed Game Design `versionStateEpoch`, and the exact WMS `publicationFence` from that acknowledgement, so it cannot be admitted before the fence exists. It may execute in `FROZEN` only when it is one of the explicitly pre-authorized final publication mutations covered by that request digest; `FROZEN` is not reopened for arbitrary Draft writes, and an unbound or mismatched mutation remains rejected. The final publication digest checkpoint includes the result of any such authorized mutation, and the owner must not accept another mutation after that bounded publication step. Both forms reject or drain stale or mismatched evidence, `PUBLISHED`, `RECONCILIATION_REQUIRED`, and any unauthorized `FROZEN` phase; an `ABORTED` attempt remains closed to ordinary writes until its failed or ambiguous outcome is reconciled and the owner row is reopened without accepting the old fence. A pre-freeze writer that already holds the row lock may finish before the acknowledgement; no writer reaching the lock afterward may commit unless it is the exact pre-authorized publication-bound mutation. The owner mutation result and updated digest checkpoint remain bound to the applicable request, digest, and fence.

Game Design completes the handoff through one outcome-typed terminal operation, `CompleteVersionPublication`, carrying exactly `tenantId`, `versionId`, `publicationRequestId`, `publishWorkflowId`, `requestDigest`, `publicationFence`, `outcome`, and the final owner evidence. The cross-service field bindings are canonical: Game Design `publishRequestId` is WMS `publicationRequestId`; Game Design `publishWorkflowId` is the durable publication-workflow identity for the attempt and is required as explicit input and evidence; Game Design `publishedReleaseBundleRef` is WMS `publishedReleaseBundleId`; and each Game Design participant `commitId` is WMS `appliedCommitId`. `outcome=PUBLISHED` requires the immutable release evidence `{publishedReleaseBundleId, publishedReleaseBundleDigest, publishWorkflowId, versionStateEpoch}` plus the final WMS digest checkpoint `{appliedCommitId, contentDigest, digestSchemaVersion}`. Every participant digest and artifact evidence item is bound under that same outer `publishWorkflowId`, and WMS exact-compares the workflow identity across the request, release bundle, participant attestations, and stored publication attempt; any mismatch, omission, or incomplete binding fails closed as an idempotency or reconciliation conflict. WMS records that evidence and atomically advances the row to `PUBLISHED`, after which all later Draft mutations with that fence are rejected. `outcome=ABORTED` requires the same request/fence/workflow identity and reconciled no-publication evidence, records the terminal abort, and permits Draft writes only after the failed/ambiguous Game Design attempt has been reconciled and the old fence is no longer accepted. The operation authenticates the Game Design caller and never treats a Temporal completion as owner proof.
...
- The implementation currently proves the source `world_instance` exists for `(tenantId, sourceGameInstanceId)`, requires a cutover-eligible world lifecycle state, and verifies retained instance topology rows for `region_instance`, `zone_instance`, and `room_instance` while still reporting only the initial World-owned `S3` families. The `world_instance` check is lifecycle/source evidence, not replacement payload; target-only/absent `population_schedule_instance` and runtime-instance `generation_run` families are not current checked-family evidence, with `generation_run` diagnostic/provenance only if later recorded. The live wire/check list omits the legacy zone-child `instance` family and emits no `EXCLUDED` reason for it; this omission is part of the partial, non-admissible response and cannot be used as exhaustive replacement evidence. It does not yet validate the exact namespace, source/target version pair, target instance binding, report capability/completeness, exact registry revision or owner-scoped catalog epochs, or persisted remap evidence required above; those are explicit implementation/proof gaps.
