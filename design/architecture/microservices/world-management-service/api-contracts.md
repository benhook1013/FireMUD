...
Game Design completes the handoff through one outcome-typed terminal operation, `CompleteVersionPublication`, carrying exactly `tenantId`, `versionId`, `publicationRequestId`, `requestDigest`, `publicationFence`, `outcome`, and the final owner evidence. The cross-service field bindings are canonical: Game Design `publishRequestId` is WMS `publicationRequestId`; Game Design `publishWorkflowId` is the durable publication-workflow identity for the attempt; Game Design `publishedReleaseBundleRef` is WMS `publishedReleaseBundleId`; and each Game Design participant `commitId` is WMS `appliedCommitId`. `outcome=PUBLISHED` requires the immutable release evidence `{publishedReleaseBundleId, publishedReleaseBundleDigest, versionStateEpoch}` plus the final WMS digest checkpoint `{appliedCommitId, contentDigest, digestSchemaVersion}`. The artifact digests, published-bundle digest, and participant digests collectively identify one immutable release for the same tenant/version/commit/workflow; any mismatch or incomplete binding fails closed as an idempotency or reconciliation conflict. WMS records that evidence and atomically advances the row to `PUBLISHED`, after which all later Draft mutations with that fence are rejected. `outcome=ABORTED` requires the same request/fence and reconciled no-publication evidence, records the terminal abort, and permits Draft writes only after the failed/ambiguous Game Design attempt has been reconciled and the old fence is no longer accepted. The operation authenticates the Game Design caller and never treats a Temporal completion as owner proof.
