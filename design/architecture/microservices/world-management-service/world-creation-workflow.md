...
Target state: World creation is a long-running process that prepares the initial world state for a new **game instance** using already-published world data for a given `tenantId`. The target workflow runs on the shared **Temporal** substrate so prepare/activate/fail/terminate orchestration survives service restarts, keeps deterministic workflow identity, and exposes operator-visible execution status without pulling gameplay runtime onto the workflow engine. The current implementation/proof is partial: extracted prepare/activate/status and a synchronous termination seam exist, but complete restart-safe lifecycle, durable step guards, frozen all-owner cleanup, and termination proof remain incomplete. [ADR 0123](../../decisions/adr-0123-database-authoritative-temporal-coordinated-world-lifecycle.md) owns lifecycle authority and all-owner cleanup convergence; this document records the World workflow consequence. [ADR 0122](../../decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md) owns stable playable identity and replacement classification. World Management's activation lifecycle surface is invoked when the platform provisions a new game instance for an existing tenant, typically from the Game Session Service. The identifiers involved are:
...
The cross-platform workflow placement and adopter-proof requirements are owned by [Transaction Strategies](../../system-architecture-transactions.md#mandatory-workflow-adopter-classification) and [Temporal Control-Plane Workflows](../../system-architecture-temporal-workflows.md). This document records the World Management consequences: the target `world-lifecycle` workflow coordinates the local lifecycle commands and read surface, while World Management's durable lifecycle row and epoch remain authoritative. Its canonical Temporal workflow ID is stable per world instance, and retries use that workflow identity plus durable launch/workflow identity and the shared digest-bound step identity (stable step name, deterministic occurrence, execution role, and immutable request digest; see [ADR 0078](../../decisions/adr-0078-digest-bound-workflow-and-step-retry-identities.md)). Gameplay ticks and live in-world mutations remain outside this workflow. Current lifecycle proof covers the extracted command path and Temporal-backed status surface, while full durable step-guard and restart/failure proof remains an implementation gap noted below. The current `TemporalWorldLifecycleOrchestrator` also treats `WorkflowExecutionAlreadyStarted` as reuse and returns the existing snapshot without exact-comparing its complete immutable prepared-launch identity with the incoming request. Until that race path validates at least the exact `controlPlaneRequestId` and `launchDescriptorId` plus every other immutable `PreparedWorldInstanceRequest` field, a conflicting request can observe the existing workflow as a successful retry; this is an implementation and race-proof gap, not target authorization to reuse a mismatched workflow.

## Implementation Status

The current first implementation cut exposes four synchronous lifecycle commands: `PrepareWorldInstance`, `ActivatePreparedWorldInstance`, `FailPreparedWorldInstance`, and `TerminateWorldInstance`. The first three create, admit, or fail a prepared instance; `TerminateWorldInstance(tenantId, gameInstanceId, expectedLifecycleEpoch, terminationRequestId)` is the only live cleanup seam. Game Session reads fresh lifecycle state through `GetWorldInstanceLifecycle` immediately before termination. Termination accepts `ACTIVE -> TERMINATING`, invokes the current Entity cleanup seam, deletes the runtime `world_event`, `room_instance_exit`, `room_instance`, `zone_instance`, and `region_instance` subset, and then saves `TERMINATED` while retaining terminal lifecycle evidence. An exact terminal retry returns the stored snapshot; a changed request identity or absent retained identity fails with `IDEMPOTENCY_CONFLICT` before cleanup or mutation, with focused unit proof. It does not yet implement the target frozen required-owner snapshot and ownership-registry revision, per-owner durable acknowledgements, complete registered World-owned cleanup, or final all-owner lifecycle CAS. The current lifecycle tests prove the synchronous cleanup and terminal-identity guard only; they do not prove `TERMINATING` retention across owner failure/retry, frozen-snapshot acknowledgement, final fenced CAS, or Temporal terminal reconciliation. These live commands are not proof of the target cross-service termination workflow.
...
Game Design owns the immutable mapping from `<tenantId, gameTemplateId, controlPlaneRequestId>` to `launchDescriptorId`. After validating that descriptor, the Game Session instance-creation orchestrator owns the durable mapping from the same launch attempt and `launchDescriptorId` to one `gameInstanceId`; an exact retry reuses both mappings, while a conflicting descriptor, template, or instance fails closed. The canonical Temporal workflow ID is `world-lifecycle:<tenantId>:game-instance:<gameInstanceId>`, stable across workflow run replacement. `controlPlaneRequestId` and `launchDescriptorId` remain immutable launch/request and digest/step evidence bound to that workflow; they are not workflow-ID components. Each lifecycle step uses the shared digest-bound workflow/step guards and its deterministic occurrence and role.
...
```json
{
  ...
  "workflowId": "world-lifecycle:7b3b074e-d597-4e9b-b96f-4f5946d26120:game-instance:9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78",
...
- Stable business scope is the resolved world instance for `<tenantId, gameInstanceId>`. The immutable `controlPlaneRequestId` and `launchDescriptorId` remain bound launch/request and digest/step evidence for that workflow identity.
...
### Lifecycle signal identity and serialization

The target workflow must represent each activate, fail, or terminate request as a durable queued signal/update envelope carrying a stable operation identity, normalized request digest, expected lifecycle epoch/status, and owner CAS evidence. Exact retries read back one logical result; changed identity, digest, status, or epoch fails closed, and conflicting lifecycle operations follow explicit precedence/conflict rules rather than last-writer-wins behavior. A signal may not be combined with or overwrite another logical operation whose outcome is still unresolved.

**Current implementation gap:** `TemporalWorldLifecycleWorkflowImpl` stores only one pending epoch/reason per signal type and has no operation identity or digest. Repeated same-type signals overwrite one another, while an activate and terminate received together are both processed sequentially; termination then reuses the pre-activation epoch after activation has advanced it and can retry until the workflow fails. The current `TemporalWorldLifecycleOrchestrator`/workflow path therefore does not provide one durable lifecycle result for concurrent or lost signal requests. This is in addition to the existing `AlreadyStarted` identity-comparison gap above and remains unproved until the queued envelope, owner lifecycle CAS, conflict handling, and readback result are implemented.
