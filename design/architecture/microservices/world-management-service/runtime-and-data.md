...
- `DROP` and `PICKUP` commit their holder changes in an Entity-local transaction only after the World attestation has passed the shared Game Session actor-lock/executor-fence critical section; Entity verifies the attestation binding before its local commit. Every `MOVE` for the actor uses the same lock, so it cannot interleave; stale evidence or a fence change is re-resolved by Game Session through Game Logic under the same root `EffectId`, preserving the `requestDigest`. The current proto/request and focused proof do not yet demonstrate this validation path.
