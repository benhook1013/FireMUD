...
- World V22 directly renames `room_instance` and `room_instance_exit` columns and a related index, but rolling- or rollback-compatibility with older World Management binaries is not proven. Before readiness, World Management must record service- and environment-specific no-overlap and retained-data evidence or a forward-compatible migration/rollout path, with old-era and mixed-binary proof; this status does not claim whether the migration has run in any environment.
