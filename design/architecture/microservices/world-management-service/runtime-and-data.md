...
- The non-Weather due-event query now requires a referenced `region_instance` to match the event's tenant/game-instance scope, and the processor defensively leaves a returned mismatch unprocessed. Focused service tests cover wrong-scope and valid rows; PostgreSQL query proof is not yet executed. `V16` still has only a scalar region FK, so retained-data/composite-ownership migration and readback remain separate open work.
