...
- `generation_run`, or equivalent, persists the stable `generationRequestId`, immutable generator selection and inputs, provenance, canonical `outputDigest`, and recorded or staged output identity required to retry one admitted generation request safely. A retry of the same request reuses its recorded or staged output and fails closed if that output is unavailable rather than silently selecting a different implementation. Committed topology is authoritative after finalize; the row does not require indefinite retention or executability of the historical generator.
