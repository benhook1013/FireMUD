...
- World V22 directly renames `room_instance` and `room_instance_exit` columns and renames `idx_room_instance_exit_from` to `idx_room_instance_exit_from_record`; older-binary compatibility and retained-data rollout proof remain open in [Shared Runtime Contracts and Persistence](../../../project-management/implementation-tracking/shared-runtime-contracts-and-persistence.md#unit-5b-persistence-review-consequences).
...
- The synchronous termination command now compares the supplied `terminationRequestId` with retained terminal identity before returning a `TERMINATED` snapshot; an exact retry returns the snapshot and a changed or missing stored identity fails with `IDEMPOTENCY_CONFLICT` without cleanup or mutation. Focused unit proof covers this narrow guard. It does not prove the target frozen all-owner termination snapshot, durable acknowledgements, final fenced CAS, or Temporal lost-response and closed-workflow reconciliation.
- The current Weather path fails closed while its aggregate selector and typed effect fence remain unresolved: `WorldEventServiceImpl.scheduleEvent` rejects `WEATHER_CHANGE` before persistence, and the due-event sweep leaves retained Weather rows unprocessed without changing `region_instance.weather`. Region-bearing non-Weather scheduling also rejects a region outside the event's exact tenant/game-instance scope. This is containment, not Weather implementation or a disposition for retained Weather events; the owner still needs the accepted selector, effect identity/fence, safe reconciliation, and production-shaped proof before admitting Weather.
- The non-Weather due-event query now requires a referenced `region_instance` to match the event's tenant/game-instance scope, and the processor defensively leaves a returned mismatch unprocessed. Focused service tests cover wrong-scope and valid rows, and Docker-backed PostgreSQL integration proof covers Weather exclusion, exact tenant/game-instance scope, and single-claim `SKIP LOCKED` behavior; local execution requires Docker/Testcontainers. `V16` still has only a scalar region FK, so retained-data/composite-ownership migration and readback remain separate open work.
...
- While the Weather selector is unresolved, `WEATHER_CHANGE` scheduling and application are explicitly deferred/non-mutating: no new `world_event` row is inserted, retained Weather rows remain unprocessed, and no `region_instance.weather` mutation is permitted. The live service now enforces that containment before region lookup or due-event application; it does not turn an old Weather row into an admitted effect or prove its eventual safe disposition. Region-bearing non-Weather events require an exact tenant/game-instance match to the selected runtime region.
