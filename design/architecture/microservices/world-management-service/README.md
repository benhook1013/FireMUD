...
Template/topology data is keyed by `(tenantId, versionId)`, durable playable state by `(tenantId, playableStateNamespaceId)` plus the applicable domain object identity, and runtime world instances by `(tenantId, gameInstanceId)` with references back to the active `versionId`. `playableStateScope` is immutable server-derived policy/routing/authorization evidence that travels with and is validated against the namespace and active instance; it is not a durable-key or uniqueness dimension. The physical runtime-instance key `{tenantId, gameInstanceId}` is replaceable and is not the persisted or validated playable-state binding; the namespace/scope proof boundary is defined by [runtime-and-data.md](./runtime-and-data.md#replacement-instance-state-classification) and [ADR 0122](../../decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md).
