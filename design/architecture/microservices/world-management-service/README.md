...
Template/topology data is keyed by `(tenantId, versionId)`, durable playable state by `(tenantId, playableStateNamespaceId, playableStateScope)`, and runtime world instances by `(tenantId, gameInstanceId)` with references back to the active `versionId`. The physical runtime-instance key `{tenantId, gameInstanceId}` is replaceable and is not the persisted or validated playable-state binding; that exact namespace/scope binding is defined by [runtime-and-data.md](./runtime-and-data.md#replacement-instance-state-classification) and [ADR 0122](../../decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md).
