...
- Every admission, scheduler firing, retry/replay, durable work item, plugin trigger, and Game Session handoff carries the exact patch and pin epoch under which it was admitted. Automation rejects a mismatch at each local durable boundary and reports a bounded pin-state outcome rather than substituting a different ready artifact. Ordinary non-script gameplay is outside this admission failure boundary.
