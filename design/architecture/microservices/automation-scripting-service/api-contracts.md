...
- `ListScriptHandoffEvents(tenantId, gameInstanceId?, scriptPatchVersion?, workItemId?, handoffOutcome?, changedAfterMs?, changedBeforeMs?, limit?)` – returns durable per-command handoff history, including `automationDispatchId`, target entity, resolved `playableStateScope`, rendered emitted command text, Game Session command id, and handoff outcome/reason for one emitted gameplay command attempt.
