...
- `GetAutomationPinConvergence` is an observed, freshness-bounded projection of Game Session's exact `{scriptPatchVersion, scriptPinEpoch}` for an instance. It is useful for admission and diagnostics, but it cannot commit a pin, mint rollout history, or authorize work when the authoritative read is unavailable. A bounded refresh is permitted; stale/local state and an operator override are not.
...
For instance-scoped gameplay/runtime ingress, Automation obtains a bounded-fresh observation of the exact Game Session `(scriptPatchVersion, scriptPinEpoch)` tuple, refreshing its local projection from the authoritative read when required. That observation supplies both fields; Automation persists them with the normalized Trigger Identity and event-scope claim in one local durable transaction before evaluation. This preserves ADR 0108's bounded-fresh projection model and does not introduce a globally atomic refresh-and-admit RPC. Same-version work from a different epoch is a distinct claim and an older epoch cannot be treated as current.
