...
- Producer services own the versioned event semantics and schema manifests. Automation mechanically validates and deterministically materializes those manifests into the one complete catalogue used by ingress and read APIs; manually copied producer definitions are not a second authority.
- Each accepted catalogue exposes an immutable revision identity and canonical digest over its validated manifest inputs and ordered entries. Failed or partial materialization retains the prior complete accepted catalogue rather than exposing a mixed revision.
...
- `GetScriptEventDefinition(eventType, eventSchemaVersion)` and `ListScriptEventDefinitions(ownerService?)` – expose the canonical event catalogue used by ingress admission. These reads include catalogue revision/digest identity, allowed producers, required Trigger Identity fields, snapshot authority, consistency class, quota class, replay semantics, allowed binding scopes, dry-run support, and deprecation status. Game Design consumes these reads without authoring a competing registry.
...
- Read authoritative current pin and bounded rollout history directly from Game Session. Join that evidence with `GetAutomationPinConvergence` when operator tooling needs Automation's observed-pin freshness.
- Treat the Automation pin projection as participant convergence state only. Work-item transitions and projection refresh must not author `PINNED`, `ROLLED_BACK`, or `REPINNED` history.
