...
On the public Automation & Scripting API, `workItemId` and `workItemIds` are aliases for the canonical persisted `outboxWorkItemId` identifiers. The aliases carry the same identifier values and do not define a second work-item identity. `automationWorkItemId` remains the current Game Session wire name for that same parent identifier at the handoff boundary; it is not a replacement public API name or a per-command identity.
