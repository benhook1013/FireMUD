...
- Each requested ID returns one canonical per-row outcome such as `retried evaluation`, `resumed dispatch`, `already recovered`, `not found/not owned`, `stage evidence unavailable`, or a specific fence mismatch; eligible rows advance independently and an ineligible row remains `DEAD_LETTERED`.
