...
- Dead-letter recovery accepts bounded explicit work-item identifiers and returns a per-row deterministic result. Evaluation-stage rows may re-enter the DSL only with the original work-item/scriptEventId, immutable graph, frozen payload/manifest, and exact fence; rows with committed evaluated output resume the stored child ledger without re-entering the DSL or generic admission, and recovery creates no new execution or dispatch identity. Incomplete stage/fence evidence leaves the row dead-lettered. The current aggregate requeue behavior remains an implementation/proof gap.
- Interval schedules use cancel-and-recreate unless both sides explicitly declare a compatible stable continuity key; `pluginId` is the stable plugin owner while `pluginVersionId` and `scriptPatchVersion` remain provenance and execution-fence metadata. One-shot timer transfer is not implied by this rule.
...
- `scriptPinEpoch` for instance-scoped gameplay/runtime triggers, paired with `scriptPatchVersion`; tenant-readiness `onLoad` uses its existing pre-instance-pin identity and omits the epoch.
...
- `ScriptPatchPinChanged` – consumed only as a non-authoritative notification to refresh Automation's observed exact-pin/convergence projection. Game Session's direct current-pin and bounded rollout-history reads remain authoritative; Automation never derives rollout history locally.
- `GetScriptPatchInstanceRolloutStatus(tenantId, gameInstanceId, scriptPatchVersion)` and `ListScriptPatchInstanceRollouts(...)` – read APIs for Automation's bounded observed-pin/convergence projection and correlation; they do not represent committed rollout history and must be reconciled against Game Session owner reads.
...
- `CancelPendingWorkItemsForPatch(tenantId, scriptPatchVersion, scriptPinEpoch, gameInstanceId?, regionId?, controlPlaneRequestId, actorPrincipal, reason)` – cancels both durable layers for a script patch and requires/enforces the displaced exact `{scriptPatchVersion, scriptPinEpoch}` tuple so rollback/drain workflows can prove displaced work will not be evaluated or handed off later. `PENDING_EVALUATION` transitions to terminal `CANCELED` without running the DSL with `finalStage=ADMISSION`, `finalOutcome=canceled`; target `EXECUTING` is fenced and its descriptor-commit marker is inspected. If the descriptor commit is present, cancellation never resumes or re-dispatches a committed child: `PENDING` and `INDEXED` children compare-and-set to `CANCELED` with durable `cancelReason`, `finalStage=WORK_ITEM_PERSIST`, and `finalOutcome=canceled`; `HANDOFF_IN_FLIGHT` fences further retry and reconciles the durable downstream outcome (remaining active/unresolved when that outcome is ambiguous); and `HANDED_OFF`, `CANCELED`, or `DEAD_LETTERED` children retain their outcome and no-op. Explicit cancellation of an uncommitted `EXECUTING` row transitions it to terminal `CANCELED` with `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and bounded cancellation metadata, and it is never replay-eligible. Only the distinct expired-stale recovery-owner path may transition an uncommitted `EXECUTING` row to `DEAD_LETTERED` with `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and `finalReason=stale_execution_fenced`; the cancellation API itself does not dead-letter stale displaced rows. Only eligible `DEAD_LETTERED` rows enter later recovery: an evaluation-stage row with `finalReason=stale_execution_fenced` may retry when its frozen evidence and all current fences match, while `CANCELED` rows are terminal and are not recovery candidates. All transitions use the canonical stage-aware mapping in the normative contract tables; evaluation-stage recovery may invoke the DSL only with its frozen evidence and original identity, while post-evaluation recovery does not re-enter the DSL.
- `ListScriptDeadLetters(tenantId, gameInstanceId?, scriptPatchVersion?, limit?)` – returns bounded, newest-first `DEAD_LETTERED` work-item rows with trigger identity, resolved `playableStateScope`, exact script patch/epoch tuple, event identity, reason, and timestamps so operators can inspect failed scripting work without raw table access.
- `ReplayDeadLetteredWorkItems(tenantId, gameInstanceId?, regionId?, workItemIds?, scriptPatchVersion?, createdAfterMs?, createdBeforeMs?, limit?, controlPlaneRequestId, actorPrincipal, reason)` – **Current implementation:** accepts only bounded explicit parent `workItemIds` selecting dead-letter rows, requeues eligible parent rows as `PENDING_EVALUATION`, and returns aggregate counts; it does not prove stage-specific recovery. **Target-state only:** descriptor references and filter-based selection remain deferred; evaluation-stage recovery retries with the original work-item and `scriptEventId`, frozen payload/manifest, exact compiled graph, and exact admitted tuple, while post-evaluation recovery resumes stored evaluated output and unfinished child dispatches without re-entering the DSL or generic admission. Recovery never creates a new execution or dispatch identity. The exact current `{scriptPatchVersion, scriptPinEpoch}` tuple, plugin identity/version, runtime region/epoch, and routing bundle including playable-state scope must match; the same patch under a new epoch is ineligible. Same-request retries return persisted per-row outcomes under the canonical [operator replay contract](../../system-architecture-scripting-runtime-execution.md#operator-replay-of-dead_lettered_work).
Replay claim and scope rules for `ReplayDeadLetteredWorkItems`:

- Reusing a `controlPlaneRequestId` with a different canonical request fingerprint returns an idempotency conflict and cannot alter an existing result. An exact retry of the same `controlPlaneRequestId` and canonical request fingerprint returns the identical stored per-row outcomes, including `rejected`, without repeating evaluation or dispatch or relabeling the result; `already_recovered` is emitted only for a new recovery request that finds the row was already recovered by an earlier successful request.
- The initial mutation accepts only bounded explicit work-item IDs; descriptor references and descriptor filters remain deferred. Each requested ID is checked and claimed at its owning durable work-item boundary, and recovery preserves the original work-item and `scriptEventId` rather than minting a new execution or dispatch identity.
- The `DEAD_LETTERED` recovery claim/attempt is persisted atomically with the status claim and records the expected exact `(scriptPatchVersion, scriptPinEpoch)`, runtime scope, applicable plugin binding tuple, and `controlPlaneRequestId`. Recovery revalidates that exact fence and binding immediately before evaluation and again before dispatch. An expired claim is reclaimed under the same attempt identity/record (with a new owner fence when needed) and never creates a duplicate recovery attempt or audit record.
...
- Each requested ID returns exactly one canonical per-row `outcome`: `retried_evaluation`, `resumed_dispatch`, `already_recovered`, or `rejected`. Only `outcome=rejected` carries `rejectionReason`, using bounded values such as `not_found_or_not_owned`, `stage_evidence_unavailable`, `script_pin_epoch_mismatch`, `plugin_binding_mismatch`, or `runtime_scope_mismatch`; eligible rows advance independently and an ineligible row remains `DEAD_LETTERED`. `recoveryStage` is optional/nullable and is `null` when the row is `not_found_or_not_owned` or `stage_evidence_unavailable` because no trustworthy stage is available.
- Replay eligibility requires the row's stored exact `{scriptPatchVersion, scriptPinEpoch}` tuple, plugin identity/version when applicable, runtime region/epoch, and routing scope to match the current authoritative evidence; the same patch under a new epoch is ineligible. A mismatch or unavailable authority/evidence leaves the row `DEAD_LETTERED`, returns a deterministic rejection, and creates no new execution or dispatch identity.
...
- `GetAutomationPinConvergence(tenantId, gameInstanceId)` – read-only operator surface for the latest exact pin observation (`observedPinnedScriptPatchVersion`, `observedScriptPinEpoch`, `lastObservedControlPlaneRequestId`, `observedAt`) used as input by admission and scheduler logic, plus freshness flags (`projectionAsOfMs`, `projectionLagMs`, `isProjectionStale`). It does not return an admission decision or handler outcome and never replaces the Game Session owner read.
...
- Read-model ownership for Automation status is a bounded, idempotent projection keyed by `(tenantId, gameInstanceId)`; `controlPlaneRequestId` is retained only as last-applied/idempotency evidence. The projection remains replaceable, non-authoritative, and never derives rollout history locally.
