...
The current `ReplayDeadLetteredWorkItems` implementation permits failed, non-superseded tenant-readiness `onLoad` rows through its `eligibleForOnLoadReplay` check; other eligible parent rows are requeued as `PENDING_EVALUATION`. It returns aggregate counts but does not prove stage-specific frozen-input retry or stored-output continuation; stale or superseded readiness work remains ineligible.
...
- Procedural population is not a live NPC/loot handoff: the current population hook calls bounded, seeded `PveEncounterService.generateEvent(...)` for each generated room and discards the returned event. It does not spawn or persist NPCs/loot or emit tick-driven commands. Full World-owned generation and tick handoff remain target work; see [Automation and Scheduler Runtime tracker](../../../project-management/implementation-tracking/automation-and-scheduler-runtime.md#active-gaps). Scripts do not persist world topology or directly mutate World Management instance rows.
