...
The current Automation pin projection and wire contract now retain the observed exact `(scriptPatchVersion, scriptPinEpoch)` execution fence plus `lastObservedControlPlaneRequestId` as separate owner evidence; admitted work stores that originating owner request as `scriptPinControlPlaneRequestId`. Instance admission remains non-admitted and retryable with `pin_state_unavailable` when required pin authority is unavailable. Scheduling authority-unavailability paths retain durable, non-admitted authority-fenced state for retry without recording a terminal `finalStage` or `finalOutcome`; a proven absent or mismatched fence or owner record is rejected. Operator dead-letter replay records the non-terminal audit marker `REPLAY` / `requeued` when it moves an eligible parent row back to `PENDING_EVALUATION`; it still does not prove a successful gameplay effect. Local Game Session admission, staging/replay, and final-effect fencing for the local script-pin tuple are implemented; complete source/target remote tuple proof and stage-aware recovery remain implementation/proof gaps.
