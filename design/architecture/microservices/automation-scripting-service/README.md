...
Random encounters and simple environmental hazards are generated by the service's
`PveEncounterService`. Encounters are seeded so results can be reproduced during
testing. Initial support provides a handful of forest and cave events chosen at
random when the Game Session Service requests a PvE interaction.
