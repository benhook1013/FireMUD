...
- **Target state:** Before constructing output, the runtime incrementally meters command count, per-entity count, serialized bytes, and data-dependent caps from the artifact-pinned cost metadata. Persistence is atomic per resolved handler: a cap or serialization failure discards all staged output and cannot leave a partial work item or handoff. See [ADR 0088](../../decisions/adr-0088-static-and-incremental-script-output-bounds.md) for the owner contract.
