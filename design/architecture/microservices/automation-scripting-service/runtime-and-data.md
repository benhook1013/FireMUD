...
- Runtime execution is instance-aware even when patch readiness is tenant-scoped: a tenant-level `READY` patch is only eligible for pinning, while admission, timer scheduling, and scoped Automation admission pause are evaluated per `<tenantId, gameInstanceId>`, and plugin activation is evaluated per `<tenantId, gameInstanceId, pluginId>`. Game Session remains the sole authority for the exact `{scriptPatchVersion, scriptPinEpoch}` and committed rollout history.
- Patch reload now also persists a first durable `script_schedule_definitions` catalog for scheduler-owned handlers such as `onInterval` / `onTimerExpire`. Its current live uniqueness key is `(tenantId, scriptPatchVersion, pluginId, pluginVersionId, scheduleDefinitionId)` with cadence/unit metadata and normalized schedule hash; because core scripts use empty plugin fields, their `scheduleDefinitionId` values are currently patch-wide unique. Observed Game Session pins create durable `script_schedule_instances` rows for the matching `(tenantId, gameInstanceId)` scope, retaining the exact pin tuple, schedule identity, binding target, and mutable due-point state. `ObserveRuntimeTickProgress` updates the runtime region and due-point fields for tick-aligned schedules, including the next future due tick; wall-clock schedules currently expose `nextDueAt`, which maps to target durable `dueAt` plus projected `nextRunAt`, not to independent deadlines. The target scheduler identity is scoped by stable owner kind/id as defined by [Scripting Scheduler and Timer Lifecycle](../../system-architecture-scripting-scheduler-and-timers.md#stable-schedule-instance-row-identity), while live region ownership remains a transitional non-empty scope marker and core-script owner identity remains narrower than that target. Richer recovery guarantees and their proof remain implementation gaps.
...
- Runtime scopes track the exact patch and pin epoch observed from Game Session for `<tenantId, gameInstanceId>` plus the owner-committed `controlPlaneRequestId` that produced that observation, alongside instance-scoped admission state such as `reloadState` and a scoped Automation admission pause. The Automation projection persists and retains that request ID through reload/convergence and relevant handoff/recovery evidence, exposing it as `lastObservedControlPlaneRequestId`; it remains projection evidence, not pin or rollout-history authority.
- When Game Session later commits a tenant-`READY` patch for a specific runtime scope, leaders set runtime-scope `pendingPatchVersion`/`pendingPinEpoch` and the committed pin-mutation `controlPlaneRequestId`, with `reloadState=RELOADING` while retaining the captured tuple and request identity of already-admitted work. New script/plugin triggers pause for that scope; ordinary Game Session gameplay ticks continue. In-flight evaluation may finish computation against its immutable captured graph, but every persistence, handoff, and final gameplay boundary enforces the captured patch and epoch; reload/convergence and recovery evidence retain the owner request ID rather than a locally invented correlation.
- Instance reload does not rerun tenant `onLoad`. Leaders load the already validated exact artifact for the newly observed tuple, reconcile version-scoped timers and other derived scheduler state, then atomically publish the observed projection and clear `reloadState=IDLE` only after evidence is complete.
...
**Target-state recovery evidence:** Dead-letter rows retain the immutable failure stage, original Trigger Identity, exact patch/epoch, immutable `lane` and bounded `cost_class`, owner-committed `controlPlaneRequestId`, plugin provenance, frozen manifest, immutable evaluated output, and a complete descriptor/child-dispatch ledger keyed by the full Command-Handoff Identity. Per-child recovery state records each child status, attempt/claim information, and durable downstream acknowledgement or outcome. The evaluated output and complete ledger are replay input; an evaluated-output or command digest is integrity evidence only and cannot substitute for the retained output or descriptor identity. `outboxWorkItemId`, the parent Trigger Identity, and `scriptEventId` remain correlation-only identifiers and are not child identity. The frozen manifest retains `priorityTag`, `quotaClass`, `lane`, `cost_class`, `componentCostRegistryDigest`, and `artifactRuntimeCapDigest` where applicable; evaluation-stage and post-evaluation recovery reuse that evidence verbatim without recomputing mutable registry, cap, or scheduler-policy values. Evaluation-stage recovery may retry the original graph; post-evaluation recovery only resumes unfinished children. Missing or contradictory evidence, including missing or changed lane/cost values at handoff, replay, or recovery, remains dead-lettered and fails closed. Schedule reconciliation defaults to cancel-and-recreate, with continuity only when the authored stable identity and compatibility declaration explicitly match.
