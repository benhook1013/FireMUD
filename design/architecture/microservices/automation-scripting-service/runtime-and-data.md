...
- A dry-run command-plan preview may use live gameplay facts only when the caller is authorized and one explicit `readSnapshotToken` or equivalent epoch fence governs every such read. Without that fence it uses declared fixtures, never an unfenced live read. Its result identifies the exact handler artifact/version, fixture or event inputs, and snapshot identity used.
...
- Dry-run command-plan previews branch before gameplay work-item persistence. After applying the same evaluator, component-policy, sandbox, loop, and output limits, they may persist only their isolated audit and bounded inspectable result; they never create a gameplay outbox item, `automation:queue:*` pointer, domain handoff, external side effect, or simulated gameplay state.
...
| `automation:test:capacity:<tenantId>:tenant`, `automation:test:capacity:<tenantId>:lease:<previewExecutionId>`, `automation:test:capacity:cluster*` | Cache/Rate-Limit | Reset-tolerant tenant-local and cluster-wide preview capacity counters and leases. Leases have bounded TTLs and are released on normal completion. These counters limit preview traffic but do not alone prove live-capacity protection; the execution substrate must also reserve a hard live worker share or provide an equivalent admission partition proved under preview saturation. |
...
- This section applies to live command-producing work. Dry-run command-plan previews never enter the gameplay work-item outbox or `automation:queue:*` projection; their permitted audit/result persistence remains in the isolated preview boundary.
