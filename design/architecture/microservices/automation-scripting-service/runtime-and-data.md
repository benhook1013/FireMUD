...
- Admitted script work items are persisted durably in a PostgreSQL-backed outbox in `PENDING_EVALUATION`, keyed by the full Trigger Identity plus sequencing fields as needed, including `playableStateScope` and `regionEpoch`, and the resolved binding `priorityTag`/`quotaClass` used for the canonical charge lifecycle. Target-state pre-DSL records also retain the sealed immutable handler input manifest and artifact-cost digests; the current outbox does not yet provide that complete manifest. Gameplay/runtime replay and handoff must preserve identity and manifest fields rather than inferring scope, epoch, or newer input versions from payload internals.
...
- Runtime scopes track only the exact patch and pin epoch observed from Game Session for `<tenantId, gameInstanceId>` plus instance-scoped admission state such as `reloadState` and a scoped Automation admission pause. The projection is not active-version or rollout-history authority.
- When Game Session later commits a tenant-`READY` patch for a specific runtime scope, leaders set runtime-scope `pendingPatchVersion`/`pendingPinEpoch` and `reloadState=RELOADING` while retaining the captured tuple of already-admitted work. New script/plugin triggers pause for that scope; ordinary Game Session gameplay ticks continue. In-flight evaluation may finish computation against its immutable captured graph, but every persistence, handoff, and final gameplay boundary enforces the captured patch and epoch.
