...
- **Current behavior:** The current service still invokes the relevant sandboxed DSL handler synchronously after admission and does not yet seal the complete manifest or persist pre-DSL `PENDING_EVALUATION` work; this is an implementation gap, not a change to the target lifecycle.
