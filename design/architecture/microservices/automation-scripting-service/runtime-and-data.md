...
- If tenant readiness fails, the patch remains `FAILED` for that tenant and never becomes eligible for pinning. If instance reload of an already-`READY` patch fails or reconciliation evidence is stale, the service marks the runtime scope `reloadState=FAILED`, discards partially loaded derived state, and fails closed: no new trigger or timer work is admitted until the current pin, schedule ownership, scope, epoch, and due state are durably reconciled. Recovery uses one idempotent recovery identity to transition `FAILED -> RELOADING`, and only a successful atomic reconciliation reaches `IDLE`; the prior observed patch remains diagnostic state only and is not a "last known good" execution fallback.
