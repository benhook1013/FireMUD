...
- Patch reload now also persists a first durable `script_schedule_definitions` catalog for scheduler-owned handlers such as `onInterval` / `onTimerExpire`. Its current live uniqueness key is `(tenantId, scriptPatchVersion, pluginId, pluginVersionId, scheduleDefinitionId)` with cadence/unit metadata and normalized schedule hash; because core scripts use empty plugin fields, their `scheduleDefinitionId` values are currently patch-wide unique. The target scheduler identity is scoped by stable owner kind/id as defined by [Scripting Scheduler and Timer Lifecycle](../../system-architecture-scripting-scheduler-and-timers.md#stable-schedule-instance-row-identity), so core-script owner scoping remains an implementation gap. That is patch-scoped schedule metadata only; it is not yet the later instance/region timer-runtime table.
