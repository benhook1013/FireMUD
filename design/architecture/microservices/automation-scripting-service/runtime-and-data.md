...
- For gameplay-originated events, ingress and durable runtime state now also persist the resolved `playableStateScope` alongside `tenantId`, `gameInstanceId`, `regionId`, `regionEpoch`, and `entityId`, so shared-state and isolated-state realms remain distinct through work-item identity, timer/schedule materialization, replay checks, dead letters, and operator reads even when they share the same tenant. `regionEpoch` is an explicit Trigger Identity dimension and replay fence, not metadata that may be refreshed from the current region later. A retry is the same trigger only when it carries the same epoch; before admission, replay, or handoff, the service compares the supplied epoch with the authoritative current runtime epoch. A stale or mismatched event is rejected or made a fenced no-op under its original identity, recorded with a bounded `runtime_scope_changed` reason where an admitted run is canceled, and is never evaluated or reminted under the newer epoch. A producer must emit a new identity against the current epoch when the event is still valid.
