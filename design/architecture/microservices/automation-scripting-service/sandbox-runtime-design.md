...
2. **Execution sandbox**
   ...
   - In the target-state execution model, the scheduler must consider eligible handlers in canonical order, admit the ordered prefix whose immutable artifact-pinned estimated millisecond costs fit the aggregate `AUTOMATION_TICK_BUDGET_MS` envelope, defer the remainder, and acquire the separately fenced execution-capacity lease defined by the [quota lifecycle contract](../../system-architecture-scripting-quotas-and-operations.md#budget-accounting-rules) before submitting a run to the executor. The lease is a bounded concurrency/occupancy fence, not the millisecond budget mechanism; actual runtime is calibration telemetry only and does not create a same-tick refund. Current live enforcement remains the aggregate per-tenant, priority-tier reservation described below.
   ...
   - `AUTOMATION_TICK_BUDGET_MS` is a scheduler capacity envelope, not a per-run timeout. The separately fenced execution-capacity lease is a bounded concurrency/occupancy fence, not a millisecond debit; the scheduler envelope remains separate from that lease, and its artifact-pinned estimate is defined by ADR 0088. The executor separately enforces each run's wall-clock deadline and iteration limit; a run timing out does not create another capacity lease, and actual runtime does not reopen or refund the same tick.
...
2. **Sandbox setup**
   ...
   - The capacity-lease record stores the lease fence, scope, and expiry before execution starts. Completion or recovery reports the lease outcome through the quota owner and resumes from durable lease state; an interrupted recovery must not admit a second execution-start marker.
...
- **Tick alignment**
  ...
  - In the target-state model, scheduler admission applies the separate aggregate `AUTOMATION_TICK_BUDGET_MS` envelope using artifact-pinned estimated costs and deterministic ordered-prefix selection, while the quota owner's fenced capacity leases bound concurrent occupancy. Lease acquisition must not be inferred to measure or settle milliseconds. Sandbox-enforced per-run timeouts remain a separate runaway-execution guard and do not by themselves enforce the aggregate window budget; actual runtime is calibration telemetry only and does not create a same-tick refund.
...
- `AUTOMATION_TICK_BUDGET_MS` – **target-state** aggregate scheduler-admission envelope for script work selected inside a single automation tick, using artifact-pinned estimated costs and deterministic ordered-prefix selection. It is separate from the fenced occupancy lease required before execution, is not a per-run timeout, and is not the current live per-trigger or per-region enforcement bucket.
