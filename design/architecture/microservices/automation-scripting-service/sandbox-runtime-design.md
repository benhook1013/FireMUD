...
| Failure-rate circuit breaker integration | Use live-traffic qualifying handler failures to transition the Automation-owned exact-tuple core-script `breakerState` or a plugin's separate `breakerState` to `DISABLED_DUE_TO_ERRORS`. For plugins, the exact breaker identity is `(tenantId, gameInstanceId, pluginId, pluginVersionId)`; `pluginActivationEpoch` is separate lifecycle-fence evidence and is not part of, or a replacement for, the breaker aggregate identity, with mandatory dry-run/test isolation. A `runtimeStatus=DISABLED_DUE_TO_ERRORS` value, if exposed, is only a read-only effective-admission projection for the queried exact tuple, never persisted breaker authority or transition history. |
...
1. **Trigger admission**
   ...
   - At the scheduler's event/handler admission boundary, after raw event ingress, the service performs registry checks and, once a live handler is resolved, applies pre-materialization handler-scoped quota and capacity gates. This is the handler-materialization boundary: each resolved handler is an independent dedupe/audit unit keyed by its full applicable Trigger Identity, and the event-scope ingress response remains only the event-scope admission result; per-handler dispositions are recorded asynchronously in `script_event_audit` rather than collapsed into an aggregate ingress outcome. An exact retry reuses that handler identity and stored audit result. A denied handler receives its typed admission outcome (`finalStage=ADMISSION`, `finalOutcome=quota_denied`) without a durable work item, queue projection, lease, or execution marker. These gates do not acquire a lease or perform the later execution-scheduler estimate reservation. An ordinary post-executor-claim `ScriptTenantBudgetService` denial is a separate live-only boundary: the durable work item has already been created and claimed, so the service terminally cancels it before DSL evaluation and records `finalStage=ADMISSION` / `finalOutcome=tenant_budget_exceeded`; it does not retroactively become a pre-materialization denial. Materialized dry-run/test handlers skip the live quota and tenant-budget services and use the isolated `ScriptDryRunCapacityService` boundary described below. If authoritative resource/capacity exhaustion prevents producing the applicable decision, return non-OK `RESOURCE_EXHAUSTED`; if route, charge, worker, or dependency infrastructure is unavailable and cannot produce it, return non-OK `UNAVAILABLE`. Pre-resolution dry-run/test ingress denials instead use the event-scope `admissionOutcome`/`admissionReason` and `script_event_ingress_audit` contract and create no handler-scoped `script_event_audit` row.
   - For each admitted live handler, the service seals the handler input manifest and durably creates and queues `PENDING_EVALUATION` work. Queued work holds no capacity or execution marker; the later execution scheduler considers queued work for its canonical estimate reservation and leaves unselected work durably queued under the same identity. An ADR 0114 preview handler takes the isolated preview branch instead: it creates no live work item or live queue pointer and retains its result/provenance only in the isolated preview result/audit surface. The current legacy materialized dry-run path follows the existing work-item and `script_event_audit` path and does not establish the target preview contract. It never invokes `ScriptTenantBudgetService`: after handler materialization it reserves isolated tenant/cluster capacity through `ScriptDryRunCapacityService`; a denial records the handler-scoped `finalStage=ADMISSION`, `finalOutcome=quota_denied`, and `finalReason=dry_run_capacity_exhausted`, and emits only the test-capacity/test-run metrics, not live budget or catch-up candidate metrics.
...
2. **Sandbox setup and executor acceptance**
   - The scheduler allocates a **sandbox context** from the sealed handler manifest. The following locally useful categories are not an exhaustive schema; the [canonical read-consistency contract](../../system-architecture-scripting-dsl-reference-and-lifecycle.md#read-consistency-contract) defines every required field and evidence:
     - For instance-bound gameplay/runtime runs, the complete applicable owner/runtime scope (`tenantId`, `gameInstanceId`, `playableStateNamespaceId`, `playableStateScope`, `regionId`, `regionEpoch`, and `entityId` when applicable)
...
5. **Outcome recording**
   ...
   - Pre-admission quota denials (`finalStage=ADMISSION`, `finalOutcome=quota_denied`) are handled by `ScriptQuotaService` before sandbox work begins and do **not** contribute to sandbox failure metrics. Live sandbox errors (`finalStage=DSL_EVAL`, `finalOutcome=sandbox_error`) contribute to live sandbox-failure metrics; only handler-attributable deterministic evaluation, sandbox-limit, and authored-output failures from live traffic are eligible for the live failure-rate circuit breaker. Quota/capacity denial, infrastructure or owner unavailability, rollback/version fencing, expected gameplay precondition rejection, dry-run/test traffic, operator cancellation, and player-controlled invalid input are excluded from breaker sampling under [ADR 0166](../../decisions/adr-0166-attributable-script-breakers-and-tenant-first-fairness.md). The exact-tuple aggregate owns breaker state/history/reset. For plugins, that aggregate is keyed by `(tenantId, gameInstanceId, pluginId, pluginVersionId)`; the captured `pluginActivationEpoch` remains separate lifecycle-fence evidence and does not change breaker identity. Preview/test sandbox errors affect only isolated test metrics and breakers (for example `automation_script_test_sandbox_failures_total`) and never increment live-traffic error counters or advance the live breaker. See `design/architecture/system-architecture-scripting-observability-contract.md` for the authoritative metric families and label sets.
...
Repeated CPU budget violations by live handlers contribute to the live failure-rate circuit breaker. Once the rolling-window eligible classified sample count is greater than or equal to its configured minimum and qualifying handler-attributable failures divided by eligible classified samples are greater than or equal to its configured failure-rate threshold, the exact core-script breaker aggregate transitions to `breakerState=DISABLED_DUE_TO_ERRORS`, while a plugin transitions its separate `breakerState=DISABLED_DUE_TO_ERRORS` keyed by `(tenantId, gameInstanceId, pluginId, pluginVersionId)`; `pluginActivationEpoch` remains separate lifecycle-fence evidence and is not folded into that breaker identity. New live handlers are denied with `finalOutcome=disabled_due_to_errors` and `finalReason=failure_rate_breaker`. If surfaced, `runtimeStatus=DISABLED_DUE_TO_ERRORS` is only a read-only effective-admission projection for the queried exact tuple and is not persisted breaker authority or transition history. Preview/test failures never contribute to that live breaker; they remain within the isolated test-only metrics and breaker namespace.
...
- **Quotas first, then sandbox**
  - For live runs, `ScriptQuotaService` and the live `ScriptTenantBudgetService` decide whether a run is allowed to start; the latter is only consulted at the ordinary post-executor-claim boundary.
  - Materialized dry-run/test handlers skip both live services and use `ScriptDryRunCapacityService` with isolated test-only outcomes and metrics.
...
- For target-state core-script breaker incidents, query the exact tuple through `GetScriptBreakerStatus`; if a legacy/read-model `runtimeStatus=DISABLED_DUE_TO_ERRORS` is shown, treat it only as the read-only effective-admission projection for that queried tuple and not as breaker history or reset authority. Recovery through a new version requires publishing the immutable script version and then explicitly selecting/repinning the affected instance to it, thereby advancing the exact execution tuple; any newly selected exact core tuple (including a repin or rollback that advances `scriptPinEpoch` while retaining the same patch) has a distinct breaker aggregate that starts `CLOSED`. The alternative is invoking the exact audited `ResetScriptBreaker` operation for the same `(tenantId, gameInstanceId, scriptPatchVersion, scriptPinEpoch, scriptId)` tuple while the authoritative state is `DISABLED_DUE_TO_ERRORS`. The reset must compare the expected current breaker revision, validate digest-bound remediation evidence, and store one idempotent result bound to the request; do not re-enable by changing the projection. Decide whether to:
  - Adjust quotas or budgets
  - Fix the script definition
  - Publish and explicitly select/repin a new immutable version, or execute the audited exact-tuple reset after remediation
