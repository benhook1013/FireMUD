...
For the [ADR 0114](../../decisions/adr-0114-command-plan-preview-dry-run-isolation.md) command-plan preview, this lifecycle uses the same evaluator and sandbox guards with an explicit authorized fenced snapshot or fixture bundle. The preview returns an ordered bounded command plan and audit/result evidence, but does not create live gameplay work, enqueue a handoff, or perform external side effects. Preview identity and capacity remain isolated from live execution; the physical reservation mechanism and its proof are implementation gaps.

When `isDryRun=true` on the ADR 0114 preview surface, the preview branch is terminally isolated from the live work lifecycle: it skips durable live `PENDING_EVALUATION` work, live queue pointers, executor acceptance, capacity leases for live execution, `EXECUTING`, live persistence, and tick handoff. It retains only isolated preview/audit evidence and returns `finalStage=DRY_RUN_RESULT` (for example `finalOutcome=dry_run_success`) with the bounded command plan and its exact provenance/fixture-or-fenced-input evidence. The existing `TriggerScriptEvent` path is not this preview surface until its proto/runtime contract is separately upgraded and proved.
...
2. **Sandbox setup and executor acceptance**
   - The scheduler allocates a **sandbox context** from the sealed handler manifest. The following locally useful categories are not an exhaustive schema; the [canonical read-consistency contract](../../system-architecture-scripting-dsl-reference-and-lifecycle.md#read-consistency-contract) defines every required field and evidence:
     ...
     - Plugin provenance (`pluginId`, `pluginVersionId`, `bindingId`, and captured `pluginActivationEpoch`) when applicable
...
5. **Outcome recording**
   - The engine records a **stage-aware outcome** for the run in `script_event_audit`:
     - `finalStage` (`ADMISSION`, `DSL_EVAL`, `WORK_ITEM_PERSIST`, `TICK_HANDOFF`, `DRY_RUN_RESULT`)
