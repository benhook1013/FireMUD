...
| Failure-rate circuit breaker integration | Use live-traffic qualifying handler failures to transition the Automation-owned exact-tuple core-script `breakerState` or a plugin's separate `breakerState` to `DISABLED_DUE_TO_ERRORS` for the exact version/runtime scope, with mandatory dry-run/test isolation. A `runtimeStatus=DISABLED_DUE_TO_ERRORS` value, if exposed, is only a read-only effective-admission projection for the queried exact tuple, never persisted breaker authority or transition history. |
