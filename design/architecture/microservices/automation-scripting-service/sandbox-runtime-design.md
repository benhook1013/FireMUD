...
2. **Execution sandbox**
   ...
   - `AUTOMATION_TICK_BUDGET_MS` is a scheduler capacity envelope, not a per-run timeout. The executor separately enforces each run's wall-clock deadline and iteration limit; a run timing out does not create another capacity lease, and a lease does not imply that one run may consume the whole window.
