...
- `AUTOMATION_TICK_BUDGET_MS` – **target-state** cumulative scheduler-admission estimate reservation for script work selected inside a single automation tick, using artifact-pinned costs and deterministic ordered-prefix selection. It is separate from the fenced occupancy lease required before execution, is not actual execution time or a per-run timeout, and is not the current live per-trigger or per-region enforcement bucket.
