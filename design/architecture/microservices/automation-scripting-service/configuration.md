...
Script-transition configuration is local policy, not pin authority. In particular, `SCRIPT_PIN_PROJECTION_STALE_THRESHOLD_MS` triggers a bounded authoritative Game Session refresh when the projection is absent or stale; new script/plugin admission rejects with `pin_state_unavailable` only when that refresh fails, and the threshold must never be widened into a degraded stale-pin override.
