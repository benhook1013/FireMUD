...
These knobs are the authoritative settings referenced by the scripting architecture docs. Each row distinguishes a live default from a required target-only setting:
...
- **Target state only:** an owner-gated queue rebuild may define bounded cadence and scan limits through `SCRIPT_OUTBOX_QUEUE_REBUILD_INTERVAL_SECONDS` and `SCRIPT_OUTBOX_QUEUE_REBUILD_BATCH_SIZE`, and atomically enforce positive pointer TTL/cap settings through `SCRIPT_OUTBOX_QUEUE_POINTER_TTL_SECONDS` and `SCRIPT_OUTBOX_QUEUE_MAX_POINTERS_PER_KEY`; no current queue-rebuild execution or configuration is bound until that owner/serialization gate exists, and the live RPUSH-only path does not enforce those pointer bounds; and
...
The formation REST controllers are removed from the Automation service-local HTTP port and authoritative OpenAPI. Formation operations remain available only through the authenticated internal gRPC control-plane path; configuration does not establish Entity-owned NPC namespace or ownership proof. See the [API Contracts implementation status](./api-contracts.md#implementation-status) for the canonical boundary.

Current live bindings in the service are narrower than the full target-state scripting design:

- the live runtime binds live per-script quota, priority-tagged live tenant-budget, dry-run quota/capacity, output-budget, pin-projection freshness, ingress-claim recovery, signer/component-policy reconciliation cadence, outbox retention, and dead-letter size/age knobs listed below;
...
| `SCRIPT_OUTBOX_QUEUE_REBUILD_INTERVAL_SECONDS` | Target-state-only cadence for an owner-gated bounded rebuild; no current scheduled rebuild is bound | `60` | Internal implementation detail |
| `SCRIPT_OUTBOX_QUEUE_REBUILD_BATCH_SIZE` | Target-state-only maximum durable work items inspected per owner-gated rebuild sweep | `200` | Internal implementation detail |
| `SCRIPT_OUTBOX_QUEUE_POINTER_TTL_SECONDS` | Target liveness TTL for each disposable `automation:queue:*` pointer list; the live RPUSH-only path currently ignores this setting | Required; no default | Target-only, not currently consumed; each environment must provide a validated positive value before adoption |
| `SCRIPT_OUTBOX_QUEUE_MAX_POINTERS_PER_KEY` | Target maximum pointers retained in one disposable `automation:queue:*` list; the live RPUSH-only path currently ignores this setting | Required; no default | Target-only, not currently consumed; each environment must provide a validated positive value before adoption |
