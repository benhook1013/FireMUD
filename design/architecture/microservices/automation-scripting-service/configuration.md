...
- target queue pointer retention must follow `SCRIPT_OUTBOX_QUEUE_POINTER_TTL_SECONDS` and `SCRIPT_OUTBOX_QUEUE_MAX_POINTERS_PER_KEY`, applied atomically on enqueue/rebuild; the current RPUSH-only implementation has not adopted these settings and remains retention/memory-pressure drift; and
