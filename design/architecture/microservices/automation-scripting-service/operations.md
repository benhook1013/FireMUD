...
For instance-scoped script admission, a fresh observed Game Session pin/epoch projection may be used during a temporary owner-read outage. If the projection is absent or stale, the required bounded owner refresh must succeed; an absent/stale projection or failed refresh fails closed, while stale observations remain operator diagnostics only. A mismatched exact tuple also fails closed. The service may continue ordinary non-script runtime work while scoped script admission is unavailable. Cleanup of displaced work is bounded and asynchronous; cleanup completion is not the epoch-fence correctness barrier. Operators must use owner pin/history reads together with the Automation convergence projection and must not repair or purge rows by direct SQL.
