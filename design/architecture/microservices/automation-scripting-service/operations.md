...
For instance-scoped script admission, Automation must first use a fresh authoritative Game Session pin/epoch read through `GetPinnedScriptPatchVersion`. A successful semantic `UNPINNED` owner result remains the explicit no-script-configured case; an unavailable, stale, or mismatched authoritative read fails closed. The local observed-pin/convergence projection is diagnostic-only for this admission decision and provides no stale grace. The service may continue ordinary non-script runtime work while scoped script admission is unavailable. Cleanup of displaced work is bounded and asynchronous; cleanup completion is not the epoch-fence correctness barrier. Operators must use owner pin/history reads together with the Automation convergence projection and must not repair or purge rows by direct SQL.
