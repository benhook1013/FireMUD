...
- `PRIVATE`, blocked relationships, missing, unknown, legacy, or malformed policy, and policy-service unavailability expose no online/offline state, last-seen timestamp, character, location, activity, disconnect disposition, or policy label. For an otherwise authorized friend-snapshot request, the friendship position remains in the snapshot but uses the same redacted/no-presence-entry shape; a policy failure never rejects the page solely for that subject or exposes why it was redacted.
...
- `HIDDEN_STAFF` and all other unknown or legacy visibility values are treated as `PRIVATE` per subject: the subject remains in the authorized snapshot, but gameplay fields are redacted without rejecting the page. `HIDDEN_STAFF` is not part of the player-facing policy vocabulary or selectable by account holders. Global or support roles do not automatically hide a gameplay actor, and support diagnostics remain separate from player presence.
