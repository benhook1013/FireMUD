...
- `PRIVATE` is an internal evaluation result only. `PRIVATE`, denied or blocked relationships, missing, unknown, legacy, or malformed policy, and policy-service unavailability expose no online/offline state, last-seen timestamp, character, location, activity, disconnect disposition, policy label, or policy value. For an otherwise authorized friend-snapshot request, the friendship position remains in the snapshot but serializes solely as the existing redacted/no-presence-entry shape; a policy failure never rejects the page solely for that subject or exposes why it was redacted.
