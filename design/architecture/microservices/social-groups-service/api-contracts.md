...
- `PRIVATE`, blocked relationships, missing policy, malformed policy, and policy-service unavailability expose no online/offline state, last-seen timestamp, character, location, activity, disconnect disposition, or policy label. For an otherwise authorized friend-snapshot request, the friendship position remains in the snapshot but uses the same redacted/no-presence-entry shape; policy failure never rejects only that subject or exposes why it was redacted.
...
- `HIDDEN_STAFF` remains represented in `FriendPresenceDto` while gameplay fields are redacted. It is not selectable by account holders, and unknown or legacy visibility values are unsupported and fail closed; the service layer may reject them rather than converting them to `PRIVATE` before projection. Global or support roles do not automatically hide a gameplay actor, and support diagnostics remain separate from player presence.
- Request-level authorization is evaluated before a friend snapshot is created. Invalid caller identity, an unauthorized requested surface, caller/snapshot mismatch, or failure to establish the caller's authoritative accepted-friend snapshot rejects the whole request with no entries or continuation token. Once that request-level authorization succeeds, a per-subject block, denial, or privacy-policy failure is not a request authorization failure: the complete page preserves deterministic snapshot ordering and applies the same redacted/no-presence-entry shape for that subject.
- Friend lists larger than one raw-presence or policy batch use an opaque, short-lived snapshot established from the authorized friend set and its bounded presence/policy read epoch. The snapshot binds the viewer, subject set, filters, page size, and deterministic friend-ordinal/account-ID order; continuation tokens must use that same snapshot and must not re-query, reorder, duplicate, or skip subjects. Every Account and Game Session chunk is at most 100 account IDs. An expired snapshot returns a page-expired error with no entries or continuation token. A missing, malformed, denied, or unavailable Account policy result, including policy-read transport failure, yields a complete page with `PRIVATE` redaction for every affected subject and never a mixed success/failure page. Game Session raw-presence transport or chunk-shape failure, Account failure while establishing request-level authorization or the accepted-friend snapshot, and any snapshot-integrity failure reject the whole page with no partial entries or continuation token; the client must restart from a new snapshot.
