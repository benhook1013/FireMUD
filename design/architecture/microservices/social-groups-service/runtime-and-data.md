...
- Target `SendMail` additionally requires method-specific caller/workload authorization, verified sender and origin, and a stable request identity bound to a canonical request digest before durable acceptance. The current path accepts a caller-supplied sender through broad account scope and inserts mail non-idempotently; this is an implementation gap, not an authorization contract.
...
- Voice chat is an optional feature built on top of a lightweight WebRTC gateway; the gateway establishes peer-to-peer connections or relays media when direct communication is not possible. Target token issuance binds the subject to the authenticated caller and applicable channel/voice entitlement, while current implementation issues tokens only. Voice activity and finite per-type moderation evidence, retention, access, and proof remain target-only; no operator-impersonation issuance contract is authorized.
