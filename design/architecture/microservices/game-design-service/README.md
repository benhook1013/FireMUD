...
- For each `<tenantId, scriptPatchVersion>`, the Automation & Scripting Service tracks a tenant readiness lifecycle (`PENDING_VALIDATION`, `ONLOAD_RUNNING`, `READY`, `FAILED`, and terminal `SUPERSEDED`) as described in `design/architecture/system-architecture-scripting-dsl-reference-and-lifecycle.md#script-patch-lifecycle`. A terminal `SUPERSEDED` patch is neither pending nor ready and is ineligible for new tenant readiness, new instance pinning, or new trigger admission; an existing instance pinned to its exact tuple may continue until an explicit Game Session repin or rollback, or an epoch/fence rejection.
