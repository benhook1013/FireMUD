...
- `game_assets` table stores asset metadata and, in the current first slice, the uploaded binary bytes for files such as icons or sound effects in `data`; later private export creates version-scoped object-store copies. The target metadata-only model retains an equivalent immutable repair source before removing those bytes from PostgreSQL.
