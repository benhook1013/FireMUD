...
- When `PublishScriptPatchVersion` is called, the Game Design Service:
  ...
  - **Target state:** Notifies Automation & Scripting of the published patch so it can ingest the compiled definitions and bindings for the target `<tenantId, scriptPatchVersion>` and start or reuse the durable Temporal `script-patch-readiness` workflow.
