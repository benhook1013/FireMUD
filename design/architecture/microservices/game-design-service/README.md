...
Game Design records only the local tenant binding and opaque Account evidence reference/generation for a committed covered operation. Missing, stale, mismatched, ambiguous, or unavailable party, acceptance, or actor evidence fails closed; exact committed retries replay their stored result, while uncertain or changed-payload retries are re-evaluated or rejected. Game Design does not own a terms/party catalog, infer signer or materiality, or accept on behalf of a party.
