...
Current wire/storage evidence now carries `scriptPatchVersion`, `scriptPinEpoch`, and the owner-grounded request ID through live trigger/work-item, schedule/timer, projection, and replay surfaces. The deliberately extracted remote handoff/final-effect and complete history surfaces remain narrower; a response-side Game Design publication link containing `baseVersionId` is not an authoritative pin binding, and the target base-version/patch pair and its exact comparison remain unimplemented and unproved.
