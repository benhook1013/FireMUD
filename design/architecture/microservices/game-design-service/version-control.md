...
External Git synchronization and canonical multi-branch merge semantics are not implemented. Current authoring uses Game Design-owned revision APIs with optimistic validation and ordered mutation calls, but `SaveRevision` does not provide deterministic replay: it lacks durable request/revision deduplication and can leave a World mutation committed without its corresponding Game Design revision when the later local save fails. ADR 0129's durable multi-owner coordinator, owner-local compare-and-swap application, synchronized commit fence, and proof that partially applied owner rows remain invisible to ordinary Draft reads are also not implemented. Current release-bundle schema and plugin publication/activation code incorrectly reuse Automation & Scripting's aggregate participant `contentDigest` where the dedicated Game Logic-owned `abilitySchemaDigest` is required; exact ability-schema compatibility therefore remains an implementation and proof gap. Plugin publication currently uses signed-only intake; [ADR 0111](../../decisions/adr-0111-unified-dsl-with-distinct-embedded-script-and-plugin-lifecycles.md)'s approved unsigned provenance remains target-only. The plugin publication path now rejects terminal-state reuse: an uploaded `SUPERSEDED` or `REVOKED_DESIGN` row cannot be republished under the same `pluginVersionId`; target rollback still uses a new immutable plugin version and leaves the prior terminal row unchanged, while new-version rollback proof remains absent. Duplicate signed plugin `assetId` values are likewise not rejected before export and can produce ambiguous distribution-manifest keys; target intake rejects them deterministically before publication/export. The live World Management design handoff adds a separate lifecycle race: it checks Game Design's authoritative remote version state (`GetVersionState(...).versionState=DRAFT`) before its own transaction, but has no durable owner-local publication freeze or exact Game Design version-state epoch/fence in the mutation request. That remote state check is not proof that a WMS mutation cannot commit after publication; current status and evidence remain in [World Management Runtime and Data](../world-management-service/runtime-and-data.md#implementation-status).

Migration V28 corrects the historical V13 default by retaining every affected row while moving only non-script-only rows whose exact V13 marker remains (`version_state=PUBLISHED`, `version_state_epoch=1`) and whose exact `(tenant_id, version_id)` `published_release_bundle` is absent to `FAILED`, incrementing the epoch and refreshing `updated_at`. Bundled full versions and script-only publication rows are untouched, and the marker predicate makes the correction idempotent. Focused source-shape proof is in [V28__quarantine_legacy_unattested_full_versionsTest](../../../../services/game-design-service/src/test/java/unit/db/migration/V28__quarantine_legacy_unattested_full_versionsTest.java); PostgreSQL retained-row execution remains required. This data correction is separate from the still-open `CompareAndSetVersionState` transition gap: the live path still accepts any non-null lifecycle state after its in-memory epoch check and does not enforce release-bundle/artifact or retirement guards.

V26's participant-scope migration is deliberately coordinated/recreate and roll-forward-only. A pre-V26 writer cannot supply `base_version_id` to the recorded script-patch participant row, and a pre-V26 reader cannot distinguish the base-scoped identities that V26 permits; mixed generations would therefore either fail writes or misread retained evidence. The Kubernetes base and hosted Helm manifests use `Recreate` for Game Design so those generations do not overlap, and a pre-V26 binary must not be rolled back after V26 migrates the database. Deployments outside those manifests must enforce the same stop-before-start boundary.
...
- **Target state:** When a version is published, the service coordinates the durable Temporal `publish` workflow that validates and finalizes the existing Draft data in each domain service and transitions the version to Published as described in [Versioning & Runtime Configuration](../../system-architecture-versioning-runtime.md). Full-version publication is not satisfied by a short synchronous `common-saga`; that substrate is limited to caller-owned retry/compensation operations that do not make a release publish-complete. No separate design database is copied into domain services at publish time. **Current implementation:** public full publication now denies when the Temporal orchestrator is absent, rather than using the synchronous fallback. The former Automation admin-guard denial is closed by the distinct Game Design workload peer and exact publication-read guards, but the Temporal-enabled path is still not proved end to end: Automation now refuses both unsupported digest scopes instead of returning tenant-wide or unbound content, and live mounted-certificate, activity, and multi-version proof remain absent.
...
`GetDraftDesignDigest` is a read-only owner-to-owner publication read across World Management, Entity Management, Game Logic, and Automation & Scripting. Every receiver uses the shared [Security Architecture](../../system-architecture-security.md) and [Authentication & Authorization](../../system-architecture-authentication.md) workload and method-allowlist rules: only the authenticated Game Design workload identity may call the publication method, and other internal workloads plus user/admin JWTs are denied. The one narrow exception is the dedicated `game-design-baseline-migrator` certificate identity, which may call only Entity Management's digest-read method during the protected historical-baseline migration; it has no access to the other three publication reads or a public mutation route. The invocation binds exact `{tenantId, scope}` (`versionId` for full publish or the complete `{baseVersionId, scriptPatchVersion}` pair for script patch), stable `publishRequestId`, derived workflow identity, and canonical `requestDigest`; missing, changed, or omitted binding evidence fails closed before a digest is returned. This context is owner-authentication and replay evidence, not an end-user role and not part of the content digest. Temporal activities never propagate the human operator JWT; operator authority ends when Game Design starts or attaches to the workflow. Each response's tenant and complete scope tuple must be exact-compared by Game Design before recording or gating the attestation; Game Design remains publication coordinator and each participant remains owner of its digest.

The publication read uses its own `publicationDigestRequest/v1` SHA-256 contract, not ADR 0047's operator-mutation digest. Let `segment(bytes)` be ADR 0047's unsigned canonical ASCII byte count, `:`, then those exact bytes; let `field(name, value) = segment(utf8(name)) + segment(utf8(value))`. The preimage is `segment(utf8("publicationDigestRequest/v1"))` followed, in this fixed order, by fields `tenantId`, `scopeKind`, `versionId`, `baseVersionId`, `scriptPatchVersion`, `publishRequestId`, and `derivedWorkflowIdentity`. `scopeKind` is exactly `FULL_VERSION` or `SCRIPT_PATCH`. A full request has a canonical positive-decimal `versionId` and empty `baseVersionId`/`scriptPatchVersion`; a patch has an empty `versionId`, canonical positive-decimal `baseVersionId`, and nonempty `scriptPatchVersion`. Empty inactive fields are framed, not omitted. Text fields are nonempty Unicode NFC without malformed scalar values or surrounding whitespace, except for the explicitly empty inactive scope fields; tenant and publish-request IDs also reject `:` so the existing workflow-ID derivation cannot alias two tuples. The digest is the lowercase 64-hex-character SHA-256 of the preimage. The derived workflow identity is `publish:<tenantId>:publish-request:<publishRequestId>` for full publish and `publish-script-patch:<tenantId>:publish-request:<publishRequestId>` for script patch; receivers recompute and compare it before comparing the digest. Neither notes nor a digest of content is an input: notes belong to Game Design's separate control-plane content digest. The script-patch command must carry and retain a stable caller-supplied `publishRequestId`, rather than generating one during publication. Shared golden vectors cover both scope branches, non-ASCII and delimiter-bearing patch values, and changed/omitted-field rejection; producers and all four receivers use the same grammar rather than local string joins.

The shared golden vectors are `FULL_VERSION` `{tenantId=tenant-alpha, versionId=42, publishRequestId=request-7, derivedWorkflowIdentity=publish:tenant-alpha:publish-request:request-7}` → `1e1b26b00b2340b9002ca6241b85b8eb7a2160922c4c9a3483f65701374f1a2a`, and `SCRIPT_PATCH` `{tenantId=ténant, baseVersionId=7, scriptPatchVersion=patch:é:1, publishRequestId=req-é, derivedWorkflowIdentity=publish-script-patch:ténant:publish-request:req-é}` → `718889dbc0c6891443d725a0443ad030d7fc18f1eb8028d65e47ea21bea2e0ed`. Tests also assert the exact framed preimage bytes for the full vector and reject altered or omitted binding evidence.

**Implementation status:** The four `GetDraftDesignDigest` receivers authenticate a distinct Game Design publication workload certificate, enforce an exact method allowlist, and reject other workloads and user/admin JWT substitution before content read. The shared request contract carries and validates tenant, complete typed scope, stable publish request, derived workflow identity, and the framed request digest; Game Design exact-compares each response's tenant and full scope before gating it. Script-patch participant evidence and the protected Game Design publication read now retain the exact base-version/patch pair. Automation's former full digest input selected tenant-wide rows and its patch digest omitted the base; the public digest RPC now returns `UNSUPPORTED_SCOPE` for both until exact mapping and a versioned base-bound digest are implemented. Game Design still lacks an exact version-to-template/asset mapping for its control-plane content digest. Local negative tests do not replace live mounted-certificate, Temporal activity, or multi-version owner proof.
...
**Current implementation status:** Entity's digest producer and Game Design's publish gate use schema v2, while retained schema-v1 Entity full-version baselines are not automatically rewritten by an ordinary publish. A protected, one-shot Game Design maintenance operation enumerates those old baselines, obtains exact-scope v2 evidence from Entity, and conditionally replaces one complete expected-old tuple together with an immutable before/after audit in a single database transaction. It changes the digest, schema, and applied-commit evidence while retaining the live row's original and last publish-workflow provenance; the separate audit records the migration operation and time. It reports success only after exact baseline and audit readback. This reader/reconciler understands both stored baseline schemas; it never compares v1 content with v2 content for publication, and the v2-only gate remains fail closed. Deployment requires the controlled publication window and exact operator preflight in [Game Design operations](./operations.md). No live retained-row migration or no-data proof is implied by code or test fixtures. Entity's current `version:<versionId>` applied-commit token remains synthetic and does not establish an applied-revision ledger.
