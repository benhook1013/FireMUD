...
- Target contract: `enabledPluginVersions[] { pluginId, pluginVersionId }` is the complete deterministically ordered immutable requested plugin selection for launch and rollback, not proof that Automation has activated runtime state. Before persistence, Game Design sorts tuples ascending by `pluginId` and then `pluginVersionId`, comparing the canonical UTF-8 byte representation of each already validated opaque identifier with no locale collation, case folding, or alternate normalization. The selection must contain at most one entry per `pluginId`; a duplicate `pluginId` is a deterministic validation failure because the scoped Automation registry has one active version per plugin. Every selection must have publication status `PUBLISHED`, prove exact compatibility with the resolved base `versionId` and its `abilitySchemaDigest`, and have complete current policy evidence with no `BLOCKED` component/capability decision (a report-only decision may proceed). For signed provenance, that evidence includes a trusted, non-revoked signer; for operator-permitted unsigned provenance under ADR 0111, it instead includes an immutable package digest, complete validation, explicit scoped approval, platform acceptance attestation, and hosted-policy permission. These are design-time eligibility predicates, not plugin runtime readiness. The persisted order is returned unchanged for retries with the same `controlPlaneRequestId`, and rollback selects that recorded set rather than re-resolving a mutable alias. A new rollback request must revalidate every recorded selection as still `PUBLISHED` and activation-eligible; a `SUPERSEDED` or otherwise ineligible selection fails deterministically and requires a new control-plane request carrying an explicitly selected newly published replacement. An exact retry of an already-bound request returns its stored result without re-resolving or revalidating against newer publication state. After launch, Automation performs the separate activation step and validates each selected exact `(pluginId, pluginVersionId, bindingId)` tuple against its runtime-state/activation fence.
