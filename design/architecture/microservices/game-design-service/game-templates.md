...
- A new launch or rollback request's explicitly present `requestedEnabledPluginVersions[]` is the complete desired plugin selection, not a replacement delta or partial merge; the transport must distinguish omission from an explicitly empty set. For rollback, the request still carries `rollbackTargetLaunchDescriptorId` to bind the recorded target and audit history, but the explicit set creates a new descriptor selection and is not described as reusing the recorded set. When the field is omitted, rollback reuses the exact selection from that named descriptor. The canonical sorting, one-entry-per-`pluginId`, publication, ability-schema compatibility, and release-attestation rules above apply before computing the request's idempotency digest, and that digest includes the recorded target descriptor identity plus the complete canonicalized selection or its explicit reuse mode. A new request replacing an ineligible recorded plugin must therefore supply the full desired set; an exact retry returns the stored result and selection without re-resolution.
