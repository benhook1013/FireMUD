...
Game Design records only the local committed evidence binding tuple `{tenantId, creatorPartyId, acceptanceEvidenceId, generation}` for a covered operation, where `generation` is Account's material-acceptance generation; it does not copy the Account terms/party catalog. Missing, stale, mismatched, ambiguous, or unavailable party, acceptance, or actor evidence fails closed before a new side effect. Exact committed replay uses its stored outcome; an uncertain request requires a fresh currentness check, and changed normalized payload reuse conflicts. Game Design does not infer signer authority or materiality.
