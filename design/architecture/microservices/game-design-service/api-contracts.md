...
- `PublishVersion(PublishVersionRequest) returns (PublishVersionResponse)` – publishes a frozen version. When Temporal is enabled, the durable publish / release-attestation work runs through the Game Design `publish` workflow family while preserving the same synchronous API contract.
