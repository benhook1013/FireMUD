...
Game Design records only the tenant and opaque Account evidence binding for a committed covered operation. Missing, stale, mismatched, ambiguous, or unavailable party, acceptance, or actor evidence fails closed before a new side effect. Exact committed replay uses its stored outcome; an uncertain request requires a fresh currentness check, and changed normalized payload reuse conflicts. Game Design does not copy the Account terms/party catalog or infer signer authority.
