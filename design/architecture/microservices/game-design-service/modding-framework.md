...
Before evaluation and handoff, a plugin handler must also prove `breakerState=CLOSED` for the exact active `pluginVersionId` and runtime scope. A disabled breaker is recorded as the bounded handler admission outcome above; it cannot be inferred from or folded into the lifecycle `pluginState`.
