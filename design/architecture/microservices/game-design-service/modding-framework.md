...
- Each plugin instance is represented as a script-like runtime object in the Automation & Scripting Service, typically tagged with `scriptType=PLUGIN` and annotated with `pluginId` and `pluginVersionId`. It participates in:
  ...
  - Cluster-level automation ceilings and the aggregate automation tick budget, as described in `design/architecture/system-architecture-scripting-quotas-and-operations.md`. Scheduler admission reserves and settles this capacity before and after execution; it is distinct from the sandbox's per-run wall-clock timeout.
...
- All plugin triggers are at-most-once per full applicable Trigger Identity; the scheduler never re-runs the plugin’s DSL graph for the same trigger, even if downstream services retry idempotent operations. `scriptEventId` is one field of that identity, not the complete key. Each handler invocation has one `script_event_audit` row; command attempts and outcomes are child handoff records keyed by the command's persisted `automationDispatchId`.
...
- Plugin logic must therefore be designed to tolerate missed or delayed events (for example, by recomputing from current world state rather than assuming every interval has executed) and to keep each emitted command's effects idempotent with respect to the full Trigger Identity, the region-scoped tick timeline (for example `(regionEpoch, tickId)`), the per-command `automationDispatchId`, and the target aggregate identity. `automationDispatchId` is created once when the logical command is persisted and reused across retries; it is not regenerated per attempt. `scriptEventId` alone is insufficient for fan-out commands; follow the same rules described in `design/architecture/system-architecture-scripting-dsl-reference-and-lifecycle.md`.
...
- The timer-reconciliation operation identity includes `<tenantId, gameInstanceId, playableStateScope, pluginId, displacedPluginVersionId, replacementPluginVersionId?, bindingId, scheduleDefinitionId>` plus the same `controlPlaneRequestId` used by the surrounding control-plane operation. Each schedule is a separate durable child reconciliation identified by that full tuple; one parent control-plane request must not collapse distinct schedules into one child result. Each owner row retains its own `pluginVersionId`; the operation-level displaced and optional replacement owner versions must not be inferred from whichever row happens to be read first.
- For rollback or repin to a replacement `pluginVersionId`, any timer or interval owned by the displaced version must be reconciled by creating or confirming the replacement owner entry before retiring the displaced row, as one atomic durable result or a resumable idempotent operation. The operation must persist and reuse the same `controlPlaneRequestId` across replacement creation or confirmation and displaced-row retirement, while each schedule child retains its own `scheduleDefinitionId` and full reconciliation identity; it must not generate a new request identity per timer or phase. No normal scheduling or trigger admission resumes for the plugin until replacement ownership, queued-work cancellation, and durable timer reconciliation are complete.
- For disablement or revocation without a replacement version, the control plane must atomically fence admission, cancel pending work, and tombstone the displaced timer owner without creating a replacement entry, as one durable result or a resumable idempotent operation. It must persist and reuse the same `controlPlaneRequestId` across fencing, cancellation, and tombstoning, while each schedule child remains keyed by `<tenantId, gameInstanceId, playableStateScope, pluginId, displacedPluginVersionId, replacementPluginVersionId=null, bindingId, scheduleDefinitionId>`; no normal scheduling or trigger admission resumes for the disabled or revoked plugin until queued-work cancellation and durable timer tombstoning/reconciliation are complete.
...
- If a newer plugin version preserves the same schedule, the scheduler may carry it forward only when the old and new definitions share the same stable `scheduleDefinitionId` and canonical `scheduleSemanticsHash`; reconciliation must create or confirm the new `pluginVersionId` entry before retiring the old entry and must explicitly rewrite ownership without reusing old trigger claims.
...
`scheduleDefinitionId` may be reused across patch or plugin versions only when the definitions have the same canonical normalized schedule-semantics digest. Game Design must persist and publish that digest as the scheduler-facing `scheduleSemanticsHash`. Any change to cadence, trigger conditions, target binding, scope, handler identity, or other scheduling semantics requires a new `scheduleDefinitionId`; a reused ID with a different digest must fail validation and publication. Matching IDs alone are not evidence that due state or a schedule row may be carried forward.
