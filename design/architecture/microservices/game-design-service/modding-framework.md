...
The current intake also accepts duplicate signed `assetId` values in `assetRefs[]`; publication export writes each reference and can emit duplicate keys in `plugin-distribution-manifest.json`. The target contract rejects duplicate asset identities deterministically before publication persistence or export. `VersionServiceImpl` now rejects an uploaded `SUPERSEDED` or `REVOKED_DESIGN` row before returning it to `PUBLISHED`; those remain terminal design-time states, and rollback requires a newly uploaded and published `pluginVersionId` rather than reactivating the old row. Focused proof covers terminal-state rejection; new-version rollback proof remains absent.
...
- **Signature envelope (required)**:
  ...
  - **Target complete-signature policy:** Multiple signatures may be present; Game Design persists the complete canonically ordered verified signature set bound to the bundle digest and must not select or persist one preferred signer. Before allowlist or revocation checks, every envelope entry must be well formed, unique by `signerKeyId`, and cryptographically verified against `bundleDigest`; any malformed, duplicate, or unverifiable entry rejects the whole bundle. Verified entries are ordered ascending by the canonical UTF-8 byte representation of each validated opaque `signerKeyId`, with no locale collation, case normalization, or alternate normalization. Verification succeeds only when at least one signature is by an allowlisted signer and no signature is by an explicitly revoked signer. The current signed-intake v1 path instead accepts the first valid allowlisted signer in envelope order and persists only that signer, as stated in the implementation status above.
- **Verification points**:
...
- **Target-state revocation**:
  - Operators can revoke a signer by removing its `signerKeyId` from the allowlist and adding it to a revocation list.
  - When a signer is revoked, subsequent loads/activations of any bundle whose complete signature set contains that key must fail. For every already-enabled affected plugin, Automation must first durably create the request-bound admission barrier, which immediately blocks new trigger admission (including pre-handler rejection recorded in `script_event_ingress_audit` without creating a handler row) and remains effective across restart. Only after the required Game Session fence acknowledgement for the non-executable target may Automation commit `pluginState=DISABLED`, mandatory `statusReason=signer_revoked`, the activation/lifecycle fence advancement, and the corresponding authoritative history; a lost or failed acknowledgement leaves the barrier pending and the plugin fail closed rather than publishing a completed disable history state.
...
- **Target state:** Automation & Scripting must refresh signer policy on a bounded cadence (for example every 60 seconds) and must disable affected plugins within a fixed operator SLO (for example “revocation disables affected plugins within 5 minutes”). The current Automation runtime includes a scheduled plugin-policy reconciliation sweep over enabled plugin runtime states; it disables active plugins when Game Design publication metadata reports signer revocation, blocked/missing component policy, unavailable policy metadata, or a no-longer-published plugin version, but this current path does not establish complete-set revocation. That blocked/missing component-policy behavior is a current fail-closed implementation seam, not target authority for routine reclassification: routine `UNSAFE` remains new-use-blocked while preserving READY/pinned behavior, and component runtime containment or disablement requires the explicit audited emergency-revocation path.
...
**Target state:** Logging & Admin must also surface signer-policy propagation status and revocation application events (for example `SignerPolicyVersionObserved` and `SignerRevocationApplied`) so operators can prove enforcement timing across services.
...
Live plugin executions (`isDryRun=false`) appear in `script_event_audit` with the same identifiers as regular live scripts, plus plugin-specific metadata, and contribute to the corresponding live automation metrics, enabling operators to monitor plugin behavior without a separate observability pipeline. The current legacy materialized dry-run/test handler path records its terminal `DSL_EVAL` result in `script_event_audit` as `finalOutcome=dry_run_completed` with `finalReason=dry_run_no_handoff`; that outcome does not imply an alternative terminal result or gameplay handoff. Target ADR 0114 command-plan previews use the isolated preview result/audit surface or return the result directly and never write a handler audit row. Preview/test executions use test-only metric families and do not increment live metric families.
...
- Each plugin trigger has one logical evaluation per full applicable Trigger Identity. Before `EVALUATED_COMMITTED`, infrastructure recovery may re-enter the DSL only under that same identity and must converge on the same durable work item and command-child identities; after that boundary, recovery replays durable descriptors without re-entering the graph. `scriptEventId` is one field of Trigger Identity, not the complete key. Each resolved live handler invocation has one `script_event_audit` row, and the current legacy materialized dry-run/test handler path likewise records one handler row for its terminal result. Target ADR 0114 command-plan previews use the isolated preview result/audit surface or return the result directly and do not create a handler audit row; command attempts and outcomes are child handoff records keyed by the complete Command-Handoff Identity.
...
- Routine deprecated/`UNSAFE` classification for plugins blocks new publication, tenant-readiness, and runtime activation of affected versions until their graphs are migrated and republished; it does not mutate an already-`READY` artifact or existing pinned behavior. Only an explicit audited emergency revocation may block already-`READY` or pinned behavior.
...
- Each plugin version is identified by `pluginVersionId`. A registry in the Automation & Scripting Service tracks, per `<tenantId, gameInstanceId, pluginId>`:
  ...
  - `breakerState` – independent execution-admission state for the exact active `pluginVersionId` and runtime scope: `CLOSED` or `DISABLED_DUE_TO_ERRORS`.
  - `breakerReason` – absent when closed and otherwise the bounded value `failure_rate_breaker`.
...
- Enabling a newly activated plugin version reports `pluginState=ENABLED` and `breakerState=CLOSED` for a `<tenantId, gameInstanceId, pluginId, pluginVersionId>` only after the idempotent Game Session install and durable acknowledgement carry the exact target epoch/state/lifecycle revision; final execution then requires the exact current lifecycle revision. A failure-rate trip changes only `breakerState`, blocks newly resolved handlers with `finalOutcome=disabled_due_to_errors` and `finalReason=failure_rate_breaker`, and does not mutate `pluginState`, `pluginActivationEpoch`, or `lifecycleRevision`; breaker trips and audited resets independently change admission state and require no Game Session final-execution fence or acknowledgement. A newly activated version starts closed automatically; disabling and re-enabling or otherwise reactivating the same version never clears a tripped breaker unless the explicit audited reset contract has run, and lifecycle reactivation itself is not a reset. The canonical [ADR 0119 lifecycle fence](../../decisions/adr-0119-epoch-fenced-per-instance-plugin-activation.md) owns the full ordering and projection rules.
- Disabling a plugin can follow two modes:
  - **Hard disable** – [ADR 0119](../../decisions/adr-0119-epoch-fenced-per-instance-plugin-activation.md) defines the saga: initiation atomically persists a request-digest-bound pending transition/admission barrier with one reserved target epoch and `targetLifecycleRevision`, durably blocks new admission (including after restart), and leaves the current tuple unchanged. Game Session idempotently installs the target epoch/state/lifecycle revision; lost or failed acknowledgement leaves the barrier pending and the system fails closed, and an exact retry resumes that same install command. Only after durable acknowledgement does one Automation transaction commit the current epoch, lifecycle revision, and `pluginState=DISABLED` as non-executable; exact retries after completion return the stored result, while never-active and already-`DISABLED` requests are no-ops only under the ADR's completed-fence conditions. New triggers are rejected before handler resolution (`admissionOutcome=TRIGGER_ADMISSION_OUTCOME_VERSION_UNAVAILABLE`, `admissionReason=plugin_disabled`) and recorded in `script_event_ingress_audit` without creating a handler row; already-admitted work proceeds only while its exact fences pass, and cleanup remains asynchronous.
  - **Disable after drain** – reserves a same-epoch `targetLifecycleRevision = current lifecycleRevision + 1` for `DRAINING` without making it current, installs that exact target through Game Session, and waits for durable acknowledgement before committing the Automation owner state/history/revision. Once `DRAINING` is committed, only already-admitted work whose winning admission/fence CAS durably committed the immediately preceding `ENABLED` revision before the durable Automation-owned `DRAINING` admission barrier was created may complete within the bounded drain policy; capture or observation alone is insufficient. At bounded completion or forced timeout, it independently reserves `targetPluginActivationEpoch = current pluginActivationEpoch + 1` and `targetLifecycleRevision = current lifecycleRevision + 1` from the committed owner row for `DISABLED`, installs and durably acknowledges that exact target through Game Session, then commits the Automation owner state/history/epoch/revision. New triggers are rejected once draining begins and cleanup remains asynchronous. See [ADR 0119](../../decisions/adr-0119-epoch-fenced-per-instance-plugin-activation.md) for the single canonical drain exception rather than repeating its full fence contract here.
...
Plugin triggers share the same Trigger Identity and `scriptEventId` lifecycle as regular scripts. Pre-handler admission outcomes are recorded in `script_event_ingress_audit` and create no handler row. Each resolved live handler invocation has one `script_event_audit` row with the required Trigger Identity fields (including `tenantId`, `gameInstanceId`, and for gameplay/runtime triggers `playableStateNamespaceId` and `regionEpoch`) plus separately persisted exact-validated `playableStateScope` policy/routing/authorization/migration-fence evidence, `pluginId` / `pluginVersionId`, and stage-aware outcome fields (`finalStage`, `finalOutcome`, `finalReason`) so operators can correlate plugin behavior with publish and enable/disable operations and still distinguish “DSL evaluated” from “accepted into tick queues”. The current legacy materialized dry-run/test handler path also records one handler audit row for its terminal result. Target ADR 0114 command-plan previews instead use the isolated preview result/audit surface or direct authorized result path and never create a handler row. Per-command acceptance, retry, and downstream outcomes belong in handoff child records keyed by the complete Command-Handoff Identity (source/target runtime scope plus `automationDispatchId` and `commandOrdinal`), not by dispatch ID alone and not in extra handler audit rows. The parent Trigger Identity and `outboxWorkItemId` remain correlation-only.

At handler admission, before evaluation or handoff, a plugin handler must prove and capture `breakerState=CLOSED` for the exact active `pluginVersionId` and runtime scope. That captured admission decision remains valid through evaluation and handoff; a later breaker trip blocks only newly resolved handlers and does not retroactively cancel accepted work. A disabled breaker is recorded as the bounded handler admission outcome above; it cannot be inferred from or folded into the lifecycle `pluginState`.
...
- At most one binding in the complete resolved handler set for the full applicable [Trigger Identity scope](../../system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields) may have `requiresExclusiveEvent=true`. For instance-bound events, that scope includes `tenantId`, `gameInstanceId`, `playableStateNamespaceId`, `regionId`, `regionEpoch`, concrete target identity, `eventType`, and `eventSchemaVersion` as applicable. `playableStateScope` remains separately persisted and exact-validated as policy/routing/authorization/migration-fence evidence, not an identity or exclusivity-uniqueness dimension. Tenant-readiness `onLoad` uses the explicit pre-instance exception: it is tenant-scoped and omits instance, playable-state, region, and entity fields. An authorized exclusive binding is selected before fan-out and becomes the sole handler; no core-script or plugin sibling runs before or after it, and failure does not cause sibling fallback.
...
- Routine component-policy reclassification is migration/new-use blocking: it prevents new publication, activation, or other newly admitted use of a now-unsafe component but does not implicitly disable an already `READY` or pinned plugin or invalidate already-admitted work. Only an explicit audited emergency revocation may contain affected active work. Component-policy enforcement for a trigger is handler-scoped after binding resolution: the resolved handler row records `finalStage=ADMISSION`, `finalOutcome=plugin_component_blocked`, and its bounded `finalReason` in `script_event_audit`. It does not add a new event-scope ingress outcome or create an ingress-only rejection row. The handler audit and bounded metrics let operators see that policy enforcement, not quotas, prevented execution.
...
- **Enforcing phase** – once violations are understood and unacceptable plugins have been migrated or disabled:
  - Enforcement is enabled for the policy version; after a handler resolves, a violation must set `policyViolations[].decision=BLOCKED` and record `finalStage=ADMISSION`, `finalOutcome=plugin_component_blocked`, and a `finalReason` identifying the blocked component/policy decision in that handler's `script_event_audit` row. This handler-scoped outcome is not an event-scope ingress `admissionOutcome`.
...
- In report-only mode, a plugin trigger that references a newly discouraged `world.admin.teleport` component may still execute and, when every required dispatch is accepted, finish with `finalOutcome=handoff_accepted`, while `policyViolations[]` records that component with `decision=REPORT_ONLY` and `automation_plugin_policy_violations_total` increments for the same component and policy version.
  - In enforcing mode for that same policy version, the resolved handler stops at `finalStage=ADMISSION` with `finalOutcome=plugin_component_blocked` and a `finalReason` identifying the blocked `world.admin.teleport` component or policy decision in `script_event_audit`. `policyViolations[]` then records `decision=BLOCKED`, and operators should see the same component reflected in `automation_plugin_policy_violations_total`.
...
Operationally, Logging & Admin acts as the operator-facing orchestration and audit surface for plugin lifecycle management. The authoritative runtime control plane remains the Automation & Scripting APIs that own `activeVersionId`, `pendingVersionId`, `pluginState`, separate `breakerState`/`breakerReason`, `pluginActivationEpoch`, and `lifecycleRevision`; a pending transition also owns its `targetLifecycleRevision`. Logging & Admin coordinates those APIs rather than owning a competing runtime registry contract. The [Scripting Control Plane API](../../system-architecture-scripting-control-plane-api.md) and [ADR 0119](../../decisions/adr-0119-epoch-fenced-per-instance-plugin-activation.md) own the exact install/acknowledgement and final-fence rules.
...
Live plugin executions (`isDryRun=false`) participate in the same observability pipeline as live core scripts and use shared identifiers and live metric families. The current legacy materialized dry-run/test handler path records a terminal `finalStage=DSL_EVAL`, `finalOutcome=dry_run_completed`, and `finalReason=dry_run_no_handoff` in `script_event_audit`; target ADR 0114 command-plan previews remain isolated to the preview result/audit surface or direct authorized result path. Preview/test executions use the corresponding test-only metric families and do not increment live families:
