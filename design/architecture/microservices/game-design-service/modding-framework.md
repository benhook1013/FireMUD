...
- Before plugin work is evaluated and again before handoff, Automation revalidates the captured exact Game Session `(scriptPatchVersion, scriptPinEpoch)` tuple against current authoritative state for the same `(tenantId, gameInstanceId)` runtime scope, together with the exact `(pluginId, pluginVersionId, bindingId)` and applicable activation/runtime fence. Already-terminal work is unaffected; no new admission or final side effect proceeds without fresh signer-policy and Game Session authority. A stale epoch fails closed with `script_pin_epoch_mismatch`; temporary authority unavailability keeps unresolved work fenced and retryable (`authority_unavailable`), while a proven plugin tuple/binding mismatch or revocation is durably terminal/ineligible through the canonical stage-aware audit or command-handoff disposition. See [Scripting Runtime Execution](../../system-architecture-scripting-runtime-execution.md#version-fencing-and-rollback-safety) and the [Command-Handoff Identity](../../system-architecture-scripting-normative-contract-tables.md#command-handoff-identity-target-state) owner contracts.
...
- `PUBLISHED` – the plugin version is accepted into immutable design-time history after its signed and validated evidence is recorded, and is available as an input to a scoped runtime activation request; publication alone is not runtime eligibility.
