...
- Before plugin work is evaluated and again before handoff, Automation revalidates the exact `(pluginId, pluginVersionId, bindingId)` and applicable activation/runtime fence; temporary authority unavailability remains retryable (`authority_unavailable`), while a proven tuple/binding mismatch or revocation is durably terminal/ineligible through the canonical stage-aware audit or command-handoff disposition. See [Scripting Runtime Execution](../../system-architecture-scripting-runtime-execution.md#version-fencing-and-rollback-safety) and the [Command-Handoff Identity](../../system-architecture-scripting-normative-contract-tables.md#command-handoff-identity-target-state) owner contracts.
