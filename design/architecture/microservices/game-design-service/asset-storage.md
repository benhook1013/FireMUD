...
The record is written only by Game Design's authorized control-plane operation `AbandonFailedVersionAssets(tenantId, versionId, expectedArtifactStateEpoch, expectedVersionStateEpoch, abandonmentRequestId, abandonmentRequestDigest)` and is read through `GetFailedVersionAssetAbandonment(tenantId, versionId)`. A retry with the same request identity and digest returns the same record; reuse with a different digest is rejected. The write must require a caller-tenant version authority and artifact both in `FAILED`, matching expected epochs, complete frozen candidate proof, no unresolved publish write, and no caller-tenant release bundle or other release reachability. The operation records abandonment evidence but does not itself tombstone the artifact.
...
- `AbandonFailedVersionAssets(tenantId, versionId, expectedArtifactStateEpoch, expectedVersionStateEpoch, abandonmentRequestId, abandonmentRequestDigest)` and `GetFailedVersionAssetAbandonment(tenantId, versionId)` – target authorized, idempotent owner-local recording/read of the failed-candidate abandonment proof described above. These operations and their persistence are not live in the current first slice.
