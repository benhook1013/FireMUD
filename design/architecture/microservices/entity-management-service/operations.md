...
This service participates in tick processing by acquiring Redis locks before mutating entity state. The `TickLockService` uses the `tick:{tenantRegionTag}:lock:<entityId>` key described in the [Redis Architecture](../../system-architecture-redis.md) document so that lock keys share a hash tag with tick queues and pending state. Entity Management consumes effective budgets and TTLs from shared tick/Redis settings; it does not define a private derivation. The formulas in [Tick Concepts & Invariants](../../system-architecture-tick-concepts-and-invariants.md#tick-budget-ttls-and-region-health-conceptual) and [ADR 0073](../../decisions/adr-0073-evidence-calibrated-tick-budgets-and-lock-ttls.md) are shared bootstrap defaults only:

- The Game Session Service exposes `game.tick-interval-ms` as the primary pacing knob.
- The shared bootstrap derives a soft budget and TTLs using formulas such as `tick_budget_ms = tick_interval_ms * 0.8` and `lock_ttl_ms = clamp(tick_budget_ms * 8, 500, 5_000)`; production values are resolved from measured execution, dependency latency, runtime pauses, and recovery evidence.
