...
- **Trade between two entities** – when a tick performs a trade between `fromEntityId` and `toEntityId`:
  - the canonical identity source is the admitted Game Session command identity: use `(automationDispatchId, commandOrdinal)` for an automation fan-out command, otherwise use the admitted `commandId`; `scriptEventId`, participant/item fields, and mutable payload text are never fallback identities. Missing required identity is a validation failure, not permission to synthesize one from the participants;
  - the handler serializes that identity and the stable trade discriminator in a fixed versioned order using canonical UTF-8 length-prefixed fields, for example `trade:v1|<len(identity)>:<identity>|<len(fromEntityId)>:<fromEntityId>|<len(toEntityId)>:<toEntityId>|<len(itemId)>:<itemId>|<len(quantity)>:<quantity>`; the same bytes must be used on retry, while two admitted trades remain distinct even when participants and item match in one tick;
