...
- Target responses carry `tenantId`, `playableStateNamespaceId`, `gameInstanceId`, and `roomInstanceId` (a `RoomInstanceRef`) so consumers can unambiguously scope the entity list to a durable playable-state namespace and its current running instance. `gameInstanceId` remains a replaceable runtime fence; the current adapter response still lacks the namespace field and does not prove this target boundary.
...
- `CreateCharacter(CreateCharacterRequest) returns (CreateCharacterResponse)` – Entity-owned allocation and persistence of a primary controllable actor after the caller has passed Account membership/grant/realm admission. The target request carries the resolved `{accountId, tenantId, worldSlug, realmSlug}`, stable `playableStateNamespaceId`, derived `playableStateScope`, active `gameInstanceId`, exact published entry-policy descriptor identity/version, and proof of the catalog/admission-pointer snapshot. Entity allocates `characterId`, validates the namespace/scope and active-instance fence, persists the `{accountId, tenantId, playableStateNamespaceId, characterId}` association, and never derives identity from account/name/hash or delegates character ownership to Account. Durable persistence/keying uses `{tenantId, playableStateNamespaceId, playableStateScope, characterId}`; a replaceable runtime identity is never the durable character identity. Entity Management does not expose a direct player REST route. The current request path does not yet carry and prove this complete target boundary, so it remains an implementation/proof gap rather than a claim about live enforcement.
- `UpdateEntity(UpdateEntityRequest) returns (UpdateEntityResponse)` – updates mutable character state for the requested `{tenantId, playableStateNamespaceId, gameInstanceId, playableStateScope, characterId}` target. Target state requires a fresh, server-resolved active-instance authorization fence for that exact namespace/scope and instance, checked atomically immediately before mutation; a caller-asserted instance, stale snapshot, or global `characterId` is not a fence. The service must reject missing or mismatched namespace/scope or inactive-instance evidence instead of updating by global character id alone. The current request path does not yet carry and prove the complete fresh fence, so it remains an implementation/proof gap and must not be treated as target enforcement.
...
- `ListCharactersByAccount` – returns only persisted actors valid for the trusted account selector and resolved `{tenantId, playableStateNamespaceId, playableStateScope}` target. Entity validates the account-to-character association before returning any roster or visibility result; an account identifier is ownership evidence, not controller identity. The response includes exactly one published v1 entry policy (`PLAYER_CREATED`, `PRESEEDED_ONLY`, or `AUTO_PROVISIONED`) plus its exact descriptor/template identity and version, and echoes the resolved namespace/scope. `gameInstanceId` is a fresh active-runtime fence, not durable lookup identity. Zero, one, and many results are explicit: policy determines the zero-result next action, one may be selected automatically, and many require explicit selection. A roster is discovery evidence, not admission authority.
...
- `ListRoomEntities(ListRoomEntitiesRequest) returns (ListRoomEntitiesResponse)` – returns players, NPCs, and visible items present in a room, scoped by `RoomInstanceRef` and the resolved `playableStateNamespaceId`; PLAYER projections/lookups carry that namespace alongside the current `gameInstanceId` runtime fence.
