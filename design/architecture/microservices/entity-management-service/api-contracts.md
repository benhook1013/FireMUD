...
- `CreateCharacter(CreateCharacterRequest) returns (CreateCharacterResponse)` – idempotently creates a realm-valid controllable actor under a published `PLAYER_CREATED` entry policy and its exact versioned creation descriptor.
...
Published realm entry policy has exactly three v1 modes: `PLAYER_CREATED`, `PRESEEDED_ONLY`, and `AUTO_PROVISIONED`. Creation, entry resolution, and listing validate the exact tenant, realm runtime, `playableStateNamespaceId`, published version, policy, and authenticated account association. After explicit Account-owned join or equivalent realm entry, `AUTO_PROVISIONED` resolution idempotently returns or provisions the one actor allowed by its published template; replays cannot create a second actor. Selection is required only when listing returns more than one valid actor; exactly one may be selected automatically. Zero actors under `PRESEEDED_ONLY` is a hard entry denial, while zero under `PLAYER_CREATED` requests the versioned descriptor. Missing descriptor data blocks only `PLAYER_CREATED` and never triggers a generic platform RPG form.

Entity Management allocates every canonical `characterId`; callers must not supply or derive it from account identity, username, display name, or a hash. Race, class, base stats, ship, nation, and equivalent fields are authored component data rather than required platform-wide creation fields. Isolated copy materialization allocates a new fork-local ID and may record `sourceCharacterId` as non-authoritative provenance.
