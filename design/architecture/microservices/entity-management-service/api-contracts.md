...
`ApplyActorCondition` is the first mutation contract for the same actor-state table. It validates the concrete mTLS caller and method allowlist, `PlayerExecutionContext`, and playable-state scope before creating an `actor_active_conditions` row with `conditionKey`, `stackCount`, `sourceType`, `sourceId`, optional `expiresAt`, and optional `effectPayloadJson`. The current request has no dedicated `effectId`; Game Session writes its operation-unique durable effect id into required `sourceId`, and Entity Management uses `{tenantId, sourceId}` as the unique replay key under operation name `ApplyActorCondition`. A completed duplicate returns the original serialized `ApplyActorConditionResponse` without another condition mutation; an in-progress duplicate returns conflict, reuse by another operation returns invalid argument, and an unreadable stored response fails internally. The current guard does not compare the complete condition payload for same-operation reuse, and persisting the operation id as source provenance conflates two identities. The target contract therefore adds a dedicated `effectId`, retains `{tenantId, effectId}` replay semantics, and restores `sourceId` to authored/source provenance before additional producers may reuse one source across applications. This is intended for gameplay-owned actions such as the first transient `blocking` state as well as later spells, consumables, and scripted effects. Reads ignore expired rows and the scheduled expiry sweep removes elapsed rows; callers must still treat the mutation response as the authoritative applied state for that request.
