...
- `ListCharactersByAccount` – returns only persisted actors valid for the account and resolved `{tenantId, playableStateNamespaceId, playableStateScope}` target. The response includes exactly one published v1 entry policy (`PLAYER_CREATED`, `PRESEEDED_ONLY`, or `AUTO_PROVISIONED`) plus its exact descriptor/template identity and version, and echoes the resolved namespace/scope. `gameInstanceId` is a fresh active-runtime fence, not durable lookup identity. Zero, one, and many results are explicit: policy determines the zero-result next action, one may be selected automatically, and many require explicit selection. A roster is discovery evidence, not admission authority.
- `AutoProvisionCharacter` (target-state) – idempotently provisions at most one actor from the published realm template under `{accountId, playableStateNamespaceId}` after explicit entry. Exact retries and concurrent duplicate requests return the same persisted actor; a changed descriptor/template version or namespace is rejected. The current surface and proof are not implemented.
