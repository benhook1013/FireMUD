...
- `CreateCharacter(CreateCharacterRequest) returns (CreateCharacterResponse)` – internal character-persistence authority invoked by the Account-owned bootstrap character-creation facade after Account has authorized the caller and realm target. The target request carries the resolved `{accountId, tenantId, worldSlug, realmSlug}`, stable `playableStateNamespaceId`, derived `playableStateScope`, active `gameInstanceId`, and Account-verified proof of the exact catalog/admission-pointer snapshot for that target, including its exact `catalogRevision`/`pointerVersion`. Account applies the canonical `stateScope`-to-`playableStateScope` projection before delegation; Entity validates that the received namespace/scope and active-instance fence equal the Account-attested snapshot, rejects missing, stale, or mismatched proof (including caller-selected namespace/scope), and does not resolve a second scope. Durable character persistence/keying uses `{tenantId, playableStateNamespaceId, playableStateScope, characterId}`; a replaceable runtime identity is never the durable character identity. Entity Management does not expose a direct player REST route. The current request path does not yet carry and prove this complete target boundary, so it remains an implementation/proof gap rather than a claim about live enforcement.
...
- `ListCharactersByAccount` – returns all characters owned by an account for the requested `{tenantId, playableStateNamespaceId, gameInstanceId, playableStateScope}` gameplay target. Each returned `Character` echoes the resolved scope/namespace so later consumers do not infer realm policy or durable identity from the request or hidden storage-key conventions.
