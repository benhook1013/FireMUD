...
- Target responses carry flattened `tenantId`, `gameInstanceId`, and scalar `roomInstanceId` fields; together they form the `RoomInstanceRef` so consumers can unambiguously scope the entity list to a durable playable-state namespace and its current running instance. They also carry `playableStateNamespaceId`; `gameInstanceId` remains a replaceable runtime fence, and the current adapter response still lacks the namespace field and does not prove this target boundary.
