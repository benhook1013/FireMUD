...
The repository also currently exposes and advertises HTTP inventory and equipment mutation routes whose controllers call Entity mutation services directly, without Game Session durable effect, tick, or barrier admission. These routes are current implementation drift and are not production-safe gameplay mutation paths; their use for any non-gameplay purpose requires explicit isolation under a separately approved owner/admin contract. This status note does not authorize a direct gameplay bypass or change the gRPC target contract.
