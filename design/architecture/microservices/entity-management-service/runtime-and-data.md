...
- The current actor-condition expiry job runs every 30 seconds by default and invokes a global, unbounded delete for rows whose `expires_at` has elapsed, without tenant/namespace/instance predicates or a batch bound. Expiry is not a disposal decision: for durable `S2` condition state, terminality, retention eligibility, cross-service holds/blockers, and safe cleanup proof are absent, as are bounded catch-up and oldest-row health proofs. The current path therefore does not establish that an expired row is disposable; retention classification remains open under [ADR 0163](../../decisions/adr-0163-service-owned-retention-classes-with-cross-service-safety.md).
