...
- The legacy crafting REST adapter is runtime-reachable but now returns `CRAFTING_UNAVAILABLE` before reading or mutating recipes; it is not a supported or advertised boundary. The crafting paths and schemas are omitted from the ping-only authoritative OpenAPI, whose root `security: []` applies only to the health contract. The deployed interceptor requires any `AUTHENTICATED` bearer because this non-public path is not in the public-route list, but neither layer provides tenant binding. The dormant `CraftingServiceImpl` trusts the DTO's `tenantId`, `resultItemId`, and ingredient item IDs; `CraftingRecipeRepository.findWithIngredientsById` and the update branch use recipe ID alone, while ingredient loading also uses recipe ID alone. The tenant-qualified list methods first select IDs but then re-enter that unqualified lookup, so they are not evidence of an end-to-end tenant predicate. Focused controller denial tests prove no service dispatch, not caller-tenant authorization, cross-tenant recipe isolation, or same-tenant result/ingredient ownership needed to re-enable crafting.
...
- The current actor-condition expiry job runs every 30 seconds by default and invokes a global, unbounded delete for rows whose `expires_at` has elapsed, without tenant/namespace/instance predicates or a batch bound. Reads filter expired rows before evaluating state, so expired gameplay effects are not restored. The deletion targets only `actor_active_conditions`; it does not delete the separate `entity_mutation_effects` receipt that `ApplyActorCondition` checks before mutation. No concrete duplicate-application or S2-loss claim is made from this path. The target still requires explicit S1/S2 classification, a replay/restore horizon, dependent-reference/hold checks, a receipt-retention guarantee, a safe watermark, and typed target, request-digest, and concurrency proof under [ADR 0163](../../decisions/adr-0163-service-owned-retention-classes-with-cross-service-safety.md); expiry is not itself disposal eligibility.
...
- `EntityDraftDesignDigestServiceImpl` now emits `digestSchemaVersion=2` and hashes normalized item `equipmentSlotGroupKey`, matching `EquipmentServiceImpl` slot compatibility admission. Focused tests cover equivalent optional-key spellings, distinct slot-group constraints, and tenant/version-scoped reads; Game Design's publish gate requires Entity v2 and rejects v1. Existing v1 participant baselines remain invalid: affected `(tenantId, versionId)` evidence must be replayed or recomputed, re-recorded with its applied commit/digest/schema, and read back before publish. No live baseline migration or cross-service publish proof is claimed by this slice.
...
- Spatial containment mutations that participate in cross-service effects are the exception: before Entity Management acknowledges a spatial mutation's persisted `EffectId` (the root `EffectId` for a root effect or the persisted child `EffectId` for a generated child effect) back to Game Session, it must durably flush the effect’s idempotency guard plus the affected containment/container rows for that effect within the same local transaction. A participant acknowledgement must never be emitted for Redis-only staged state.
- Target-state cross-service participant guards use the persisted mutation `EffectId` (the root `EffectId` for a root mutation or the persisted child `EffectId` for a generated child mutation), typed operation, target aggregate, and immutable request digest; any enclosing `rootEffectId` is lineage and reconciliation context only. Matching retries return the durable result, while an operation, target, or digest mismatch fails closed. This target guard is not the current domain-local replay table described above. The pending [ADR 0183 proposal](../../decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is non-authoritative.
...
- **Target classification:** `inventory` and `character_equipment` rows are `S2` template-bound survivor state only within the resolved `(tenantId, playableStateNamespaceId)` identity and immutable scope evidence. The current schema has no namespace column for these families, so the live tenant-wide `hasS2Rows` count cannot establish that rows belong to the requested namespace/source instance and is non-authoritative cutover evidence; a namespace-bound implementation must first prove the rows' scope. Under the target contract, if either family has rows in that resolved namespace and no approved `remapSetId` was frozen by launch resolution, validation returns `result=INCOMPATIBLE`, `hasS2Rows=true`, `remapSetRequired=true`, and `ENTITY_REMAP_REQUIRED`.
