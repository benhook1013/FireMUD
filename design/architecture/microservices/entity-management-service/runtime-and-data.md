...
- The first shared effect-evaluation seam is an in-process Entity Management service that evaluates typed resource modifiers and granted states deterministically. Active condition payloads and equipped item-template payloads are now wired through it for actor-state reads, and an internal condition/action-state mutation service can create active condition rows and expire elapsed rows. Player command wiring and scheduled expiry execution remain future work.
