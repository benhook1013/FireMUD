...
- Target-state cross-service participant guards use a structured root `EffectId`, typed operation, target aggregate, and immutable request digest; matching retries return the durable result, while an operation, target, or digest mismatch fails closed. This target guard is not the current domain-local replay table described above. The pending [ADR 0182 proposal](../../decisions/adr-0182-deterministic-effect-id-allocation-and-replay-binding.md) is non-authoritative.
