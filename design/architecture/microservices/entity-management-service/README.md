...
Equipment templates now use game-authored equipment schema data rather than a platform-global slot enum. The runtime model stores versioned slot definitions, optional slot-group compatibility keys, body-layout slot membership, and each character's `bodyLayoutKey`. For a game with equipment capability, publication and runtime validation fail closed when schema or layout data is missing or unknown; binding validates slot existence, item slot-group compatibility, body-layout membership, and occupancy before changing equipment. Existing direct-string bootstrap behavior is legacy implementation residue, not a target fallback. Version cutover validates surviving equipped state against the exact target schema and blocks unresolved renamed, removed, split, merged, or newly incompatible definitions pending explicit mapping or resolution. See [ADR 0127](../../decisions/adr-0127-game-authored-equipment-layouts-with-fail-closed-publication.md) for the owning contract.
