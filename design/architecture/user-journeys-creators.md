...
5. **Emergency Override** – A `platformAdmin` emergency launch enters the Logging & Admin validation, immutable audit/action-family, canonical `mutationDigest/v1`, and durable-intent boundary before any Account-owned launch mutation is invoked. Account reauthorizes the actor and target against current authority and returns a durable idempotent outcome; Game Session proceeds only from that outcome. Creators do not depend on operators for routine tenant launches.
