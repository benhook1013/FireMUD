...
2. **Launch the Production Realm** – A `tenantAdmin` submits the launch through Gateway to Logging & Admin. Logging & Admin reauthorizes the exact actor and tenant, commits the immutable audit/action family, canonical mutation digest, caller-stable operation identity, and durable intent, then invokes [Game Session Service](./microservices/game-session-service/README.md) through the internal owner-mutation boundary. Direct Game Session launch is internal orchestration only, not an external tenant-admin route. The [World Creation Workflow](./microservices/world-management-service/world-creation-workflow.md) describes how initial world state is seeded from the published world data when a brand new world is created.
