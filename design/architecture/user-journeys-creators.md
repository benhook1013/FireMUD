...
5. **Emergency Override** – A `platformAdmin` emergency launch enters the Logging & Admin validation, immutable audit/action-family, canonical `mutationDigest/v1`, and durable-intent boundary before any launch mutation is forwarded. Logging & Admin obtains an Account-issued human operator authorization reference whose current generation and `privileged_control` assurance cover the exact actor and target. The owner redeems that reference directly with Account, bound to the exact action schema, scope, request ID, and canonical digest, before committing the launch; forwarded assertions are not authority. Account returns the durable idempotent owner outcome, and Game Session proceeds only from that outcome. Creators do not depend on operators for routine tenant launches.
