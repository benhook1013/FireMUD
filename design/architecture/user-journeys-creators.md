...
3. **Invite Testers** – Access is explicit. The fork uses the same platform accounts as production, but only authorized testers, creators, and operators see it in `REALMS <world>`. In v1, `tenantAdmin` manages these explicit access grants for the fork, while `platformAdmin` remains break-glass override only.
   ...
   - Every tester grant has an explicit expiry no later than the fork expiry. Its effective expiry is the earlier of the grant and fork expiries, and extending the fork does not silently extend tester access; grant extensions are explicit tenant-admin actions.
   - Account Service owns monotonic grant revisions and revocation tombstones. Replayed or stale grant commands cannot reverse a newer expiry, extension, or revocation.
   - Revoking or expiring a grant removes current authority without deleting the fork. It immediately hides the realm from that account, denies connect-token issuance and `PLAY`, fences new command admission, and terminates the affected gameplay binding through the bounded revocation contract. Work already durably admitted before the fence may finish once.
   - A friendly scheduled ending is a separate lifecycle sequence: close admission, perform the bounded realm drain, then revoke or expire the grants. Revocation itself is not a drain request.
