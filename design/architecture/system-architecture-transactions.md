...
Script evaluation and pin transitions are not a new transaction substrate. The scripting contracts own exact-pin admission, stage-aware recovery, and DSL lifecycle; this document owns the transaction consequence that script-emitted gameplay effects still use the ordinary tick effect identity, owner-local transaction, outbox, or durable intent boundary selected for that effect. **Target state:** a rollback epoch fence rejects stale effects without converting ordinary gameplay into a routine global transaction pause. The current handoff does not carry, persist, or enforce `scriptPinEpoch` end to end, so it cannot yet reject same-version work from an older epoch; this remains an implementation gap rather than a weakened target invariant. See [Scripting Contracts](./system-architecture-scripting-contracts.md) and [Runtime Execution](./system-architecture-scripting-runtime-execution.md).
...
- **Target-state identity contract:** Game Session assigns one stable root `EffectId` for the admitted logical effect. It preserves the intended pre/post state, required participant set, and status. A participant guard's uniqueness identity has exactly these inputs: the root `EffectId`, typed operation, and target aggregate. The guard row binds the immutable request digest and stores the outcome; outcome/status, evidence, and reconciliation fields are mutable guard-row state protected by compare-and-set (CAS), not uniqueness-identity inputs. A matching identity with a changed operation, target, or request digest fails closed. Ordinary retries and replay keep the root `EffectId`; deterministic child effects are used for derived reactions.
- **Post-abandon re-drive:** A fresh root is not an ordinary retry or replay. It is permitted only after the original effect is conclusively terminal `ABANDONED` and its source claim is terminalized, and only when recovery allocates a later `(regionEpoch, tickId)` coordinate, a fresh root `EffectId` and participant projections, and a new retry/source identity with durable lineage to the original identities. Until that allocation commits, the work remains recovery work and is not returned to an ordinary queue.
- **Command/effect boundary:** An accepted command lost before durable staging terminalizes only at command scope as `executionOutcome = LOST_BEFORE_STAGING` and `gameplayResult = NOT_APPLIED`; no effect ledger row is invented. Once an effect is durably claimed or staged, physical execution is at least once and its immutable identity/digest guard permits at most one logical mutation. Staged effects terminalize as `APPLIED` or `ABANDONED` only when authoritative evidence permits; inconclusive work remains `SCHEDULED`/reconciliation-required, explicitly including timeout, retry exhaustion, or missing coordination. `REPLAY_NOOP` is an `APPLIED` reason, never a third status. Command `SUCCESS`, `PARTIAL`, `FAILED`, `TIMEOUT`, and `NOT_APPLIED` remain derived player-result vocabulary under ADR 0053. See [ADR 0069](./decisions/adr-0069-at-least-once-effect-execution-with-one-logical-terminal-outcome.md).
- **Single propagation contract:** The coordinator sends the root `EffectId`, typed operation, target, immutable request digest, and required-participant context. Participants validate them and atomically commit their guard with effect-visible rows before acknowledging. They do not generate a fresh identity or bind a guard to mutable payload.
- **Current live implementation gap:** The live Game Session proto does not yet carry `commandOrdinal` or the full Trigger Identity for this handoff. `TickStagingService` currently derives `effectKey` from `commandId`, falling back to a hash of command text plus the staging slot when no command id is available. That current fallback is not the target-state automation fan-out identity and must not be documented as though `(automationDispatchId, commandOrdinal)` were already enforced live.
- The canonical `effectKey` serialization is versioned, deterministic, and shared by all participants: before encoding, validate every identity-bearing and typed field against its declared type and domain, then serialize only canonical representations of the stable identity, per-effect discriminator or operation identity, and other fields in a fixed order with canonical UTF-8 length-prefixed values. Numeric values use canonical base-10 decimal representations where their type permits them; a participant must not independently stringify, normalize differently, or reorder the fields. The exact serialized bytes are reused unchanged on retry.
...
- **Generated effect chains:** Generated children carry immutable parent/root identity, `depth = parent.depth + 1`, a digest-covered deterministic child ordinal, and deterministic root-chain count/cost/per-target accounting. Only a child exceeding a bound is suppressed; committed parents and earlier children remain authoritative. Required/optional authored classification derives truthful `SUCCESS`, `PARTIAL`, or `FAILED` outcomes, and suppression evidence uses bounded metric dimensions with raw identities retained for audit. See [ADR 0075](./decisions/adr-0075-depth-cost-and-count-bounds-for-generated-effect-chains.md).
...
- **Target-state shared idempotency helper contract (not yet fully implemented or proven):** To keep this contract consistent across services, tick-driven handlers use a shared idempotency helper from `common-data-runtime` (for example an `IdempotentEffectExecutor`) instead of ad-hoc “check or insert” patterns. The helper:
  - **Target-state request shape:** Accepts a structured `ParticipantGuardRequest` containing the root `EffectId` (`rootEffectId`), typed operation, target aggregate type, target aggregate ID, and immutable request digest (`requestDigest`), plus callbacks for “apply-if-first” and “handle-replay”.
  - **Target-state full-field replay validation:** Treats an existing guard as a valid replay only when all of those request fields match the persisted guard identity and digest; a changed operation, target aggregate, or digest fails closed and must not invoke the replay callback.
...
The following is the target deterministic replay contract for the `(versionId, scriptPatchVersion, scriptPinEpoch)` tuple that was in effect when a given effect was applied. Current gameplay-command rows and selected-work manifest items preserve `scriptPatchVersion`, but the live `EnqueueAutomationCommandIfAbsentRequest` carries no `scriptPinEpoch`, and the live `tick_batch` and effect-ledger schema does not yet durably seal the complete tuple; the selected-work manifest also does not yet preserve `versionId`. Until that schema and replay path converge, the fail-closed tuple contract below remains an implementation gap rather than a claimed live guarantee. Same-version old-epoch work therefore cannot be rejected by the current handoff boundary today.

- The Game Session Service records the `versionId`, exact `scriptPatchVersion`, and `scriptPinEpoch` for each `gameInstanceId` and seals the tuple into durable tick-batch/effect manifest or effect-ledger state alongside `EffectId`. Logs and optional audit records may project that context but are not its replay authority.
- Tick handlers and script runners must treat the sealed `(versionId, scriptPatchVersion, scriptPinEpoch)` tuple as immutable effect execution context: replays and retries load the same durable tuple that was recorded for the original effect, even if the instance later moves to a different patch or advances the epoch. Missing or mismatched durable execution context fails closed rather than falling back to the instance's current tuple.
