...
- **Target-state identity and guard contract:** The deterministic command-plan allocation above is the canonical command-root contract. Each participant guard is uniquely derived from exactly the persisted root `EffectId`, typed operation, and target aggregate; its row binds the immutable request digest and stores mutable outcome/evidence state under CAS. Matching replay returns the stored result, while an operation, target, or digest mismatch fails closed. Generated children retain the enclosing root and their owner-defined lineage identity.
