...
The structured participant-guard request, deterministic command-plan root identity, shared `IdempotentEffectExecutor`, standardized tick-effect outcome metric, and replay-verification helper described below are target-state contracts. The current replay tables and helpers remain narrower and domain-local; complete propagation and validation of the plan's persisted `planOrdinal`/root-or-child `EffectId`, typed operation, target aggregate, owner-resolved S1/S2-versus-explicit-S3 guard partition, applicable `playableStateNamespaceId` and separately persisted `playableStateScope` evidence, separately validated active-instance fence, immutable request digest, complete participant set, and shared helper/metric behavior are not yet fully implemented or proven. Unknown or unclassified guard families must be rejected before any participant-guard allocation or admission; no implicit third partition exists. The current local instance-bound scripting handoff now carries, persists, and enforces the exact `scriptPinEpoch` tuple with its owner request evidence, including same-version old-epoch rejection. Automation also commits each deterministic child `HANDOFF_IN_FLIGHT` intent before invoking Game Session outside a local transaction, then records a definitive response in a separate local transaction; ambiguous post-dispatch failures remain recoverable intent rather than being reported as a clean rejection. It does not yet implement the durable evaluated-descriptor and reconciliation owner needed to recover every ambiguous handoff. Complete source/target remote tuple binding and full command identity also remain implementation gaps rather than weakened target invariants. The target contract remains authoritative: matching requests may replay safely, while an operation, target, namespace, scope, active-instance evidence, or digest mismatch fails closed.

Lifecycle and authoring workflows follow the same local-transaction boundary. World Management commits its lifecycle row/epoch with database compare-and-set; Temporal coordinates retries and waits but is not a transaction or authority. Current instance termination performs Entity cleanup outside the World database transaction and commits the World-owned row cleanup plus final `TERMINATED` state in one local transaction after that acknowledgement. This closes partial World-local cleanup on failure but does not implement the target frozen multi-owner cleanup snapshot, acknowledgement set, or complete recovery proof. Replacement cutover follows the canonical [PREPARING-to-ACTIVE proof sequence](./system-architecture-versioning-runtime.md#realm-routing-contract-for-player-addressable-realms) and [Game Session cutover boundary](./microservices/game-session-service/api-contracts.md#world-lifecycle-and-admission-boundary): World owns the lifecycle CAS and one-shot cutover hold, while the Game Session owner-local cutover transaction validates the hold id/fence, expected pointer version, and exact source and target `ACTIVE` proofs before its pointer CAS. World finalizes the hold only after authoritative Game Session readback proves the local pointer/audit/prepared-execution/source-cleanup/drain-fence transaction; no global transaction spans the services. A multi-owner Draft commit carries the complete [ADR 0129](./decisions/adr-0129-durable-fenced-multi-owner-draft-commits.md) binding, including target `tenantId`/`versionId`, request or proposal identity, exact `baseCommitId`, canonical revision order, proposed input/digest, and the complete affected `(owner, aggregateId, scopeId, epoch)` set. It performs compare-and-set in each owner database, records durable per-owner outcomes, and publishes through a synchronized read fence; it never uses a global epoch, silent merge, or distributed transaction. These are control-plane workflows and do not add a global ACID path to ordinary gameplay.
...
Tick execution is replayable: retries, failover, and Redis AOF replay can cause the same logical effect to be attempted more than once. For gameplay commands this is expected and safe only because tick-invoked domain mutations are required to be idempotent with respect to the stable persisted mutation `EffectId`—the root `EffectId` for a root mutation or the generated child's persisted `EffectId` for a child mutation—with any enclosing `rootEffectId` retained only as lineage and reconciliation context, as defined by the identifier glossary.

- **Target-state identity and guard contract:** The deterministic command-plan allocation above is the canonical command-root contract. Participant-guard durability follows the owning runtime-family classification: S1/S2 owner families use `(tenantId, playableStateNamespaceId)` plus the owning identity/effect (`EffectId`, typed operation, and target aggregate), while only an explicitly owner-classified S3 family uses `(tenantId, gameInstanceId)` as its durable partition. For S1/S2, the active `gameInstanceId` remains separately exact-validated authorization/fence evidence and does not create a second guard identity; unknown or unclassified families are rejected before any participant-guard allocation or admission and are never inferred to be S3. Root effects use the root `EffectId`, while generated child mutations use their deterministic persisted child `EffectId` and retain the enclosing root as lineage and reconciliation context. The guard row binds the immutable request digest and stores mutable outcome/evidence state under CAS. Matching replay returns the stored result, while a tenant, namespace, owning effect, operation, target, scope, active-instance evidence, or digest mismatch fails closed. Work explicitly outside playable-state scope must use an accepted owner-specific idempotency/partition contract; it cannot omit namespace/scope and proceed under this gameplay participant-guard contract. Exact child allocation remains outside this accepted guard decision and is described only by non-authoritative [ADR 0183](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md).
...
- **Command/effect boundary:** An `ACCEPTED_VOLATILE` command lost before durable staging may terminalize only at command scope as `executionOutcome = LOST_BEFORE_STAGING` and `gameplayResult = NOT_APPLIED` while its ingress status is `RECEIVED` or `ENQUEUED` and no `tickBatchId` survives; an `ACCEPTED_DURABLE` command follows its feature-specific durable replay/re-drive contract instead. No effect ledger row is invented for either command unless an effect was durably claimed or staged. A durable pre-staging source/effect claim establishes recoverability but does not itself make physical execution at least once; only a staged or otherwise dispatchable effect enters physical execution, after which its immutable identity/digest guard permits at most one logical mutation. Staged effects terminalize as `APPLIED` or `ABANDONED` only when authoritative evidence permits; inconclusive work remains `SCHEDULED`/reconciliation-required, explicitly including timeout, retry exhaustion, or missing coordination. `REPLAY_NOOP` is an `APPLIED` reason, never a third status. Command `SUCCESS`, `PARTIAL`, `FAILED`, `TIMEOUT`, and `NOT_APPLIED` remain derived player-result vocabulary under ADR 0053. See [ADR 0069](./decisions/adr-0069-at-least-once-effect-execution-with-one-logical-terminal-outcome.md).
- **Single propagation contract:** The coordinator sends the mutation's persisted root-or-child `EffectId`, typed operation, target, immutable request digest, conditional owner-family partition, separately validated active-instance authorization/fence evidence, and required-participant context; generated children also carry their enclosing root/parent lineage binding. Game Session retains the command root's associated `planOrdinal` in its plan/ledger binding. Participants validate the complete binding and atomically commit their guard with effect-visible rows before acknowledging. They do not generate a fresh identity or bind a guard to mutable payload.
- **Current live implementation gap:** The live Game Session proto does not yet carry `commandOrdinal`, the complete applicable Command-Handoff Identity, or the deterministic plan manifest/opaque root allocation binding for this handoff. `TickStagingService` derives the live `effectKey` from `commandId`; it retains a deterministic command-text-plus-slot fallback helper for selection metadata, but durable tick batching rejects any queued entry without a `commandId` before staging or effect persistence. That fallback is therefore not a live staging, replay, or recovery identity, and it is not the target-state automation handoff or command-plan identity; the complete Command-Handoff Identity and root allocation binding are not yet enforced live.
...
- **Target-state shared idempotency helper contract (not yet fully implemented or proven):** To keep this contract consistent across services, tick-driven handlers use a shared idempotency helper from `common-data-runtime` (for example an `IdempotentEffectExecutor`) instead of ad-hoc “check or insert” patterns. The helper:
  - **Target-state request shape:** Accepts a structured `ParticipantGuardRequest` containing the owner-resolved runtime-family classification and conditional durable partition—S1/S2 uses `(tenantId, playableStateNamespaceId)` plus the owning identity/effect, while only an explicitly owner-classified S3 uses `(tenantId, gameInstanceId)`—and separately carries the active `gameInstanceId` authorization/fence evidence for S1/S2. The request also contains mutation guard `EffectId` (`effectId`), typed operation, target aggregate type, target aggregate ID, applicable `playableStateNamespaceId` evidence, separately persisted server-derived `playableStateScope` evidence, and immutable request digest (`requestDigest`), plus callbacks for “apply-if-first” and “handle-replay”. A generated child request also carries its enclosing `rootEffectId` and parent/child lineage binding; for S1/S2, namespace is the uniqueness partition while scope and active-instance evidence are exact-validated sealed-context fences, and lineage fields do not replace the conditional partition, `effectId`, operation, and exact target in guard uniqueness. Unknown or unclassified families, or a request with no accepted owner-specific partition, are rejected before any participant-guard allocation or admission; they are never inferred to be S3.
  - **Target-state full-field replay validation:** Treats an existing guard as a valid replay only when all of those request fields match the persisted guard identity, conditional allocation partition, namespace evidence, separately persisted scope evidence, separately validated active-instance authorization/fence evidence, and digest. A changed tenant, namespace, owning effect, operation, target aggregate, scope, active-instance evidence, or digest fails closed and must not invoke the replay callback; an S3 family additionally exact-compares its durable `gameInstanceId` partition. A work item explicitly outside playable-state scope must use an accepted owner-specific idempotency/partition contract; it cannot omit namespace/scope and proceed under this gameplay participant-guard contract. No family may omit its declared partition.
...
- **Replay verification transaction (target state):** Replay evidence is not an audit-only side effect and is not proved by a root effect or one ledger row in isolation. The verifier reconstructs the complete expected concrete participant-projection set from the sealed required-participant and manifest context. It validates every expected participant guard and corresponding durable domain evidence against the declared S1/S2 or explicit S3 partition, namespace/owning identity, and separately validated active-instance fence, rejects missing, extra, partial, or conflicting projections, and CASes all required rows transactionally within their owning durable boundaries before the fenced Game Session reconciliation transaction records the result. A stale status, fence/context, missing evidence, incomplete set, set mismatch, or concurrent winner fails closed and rolls back/rejects or retries; external logs and audit streams are projections and cannot authorize a replay transition.
...
`DROP` and `PICKUP` keep the holder mutation in Entity Management, but their World fact read and Entity commit form one Game Session-controlled critical section. Game Session acquires an actor lock keyed by the complete gameplay scope, including `regionId`, and `actorEntityId`, captures the lock's opaque Redis token separately from the current `(regionId, regionEpoch, executorFence)` durable-authority tuple, and, before World validation, durably records one in-flight spatial critical-section barrier keyed by the existing scoped root identity `(tenantId, gameInstanceId, EffectId)` and bound to `actorEntityId`, `regionId`, the exact `playableStateNamespaceId`, the separately exact-bound `playableStateScope`, `RoomInstanceRef`, `regionEpoch`, `executorFence`, and the canonical immutable `requestDigest`. The namespace and scope are barrier evidence, not additional uniqueness dimensions; barrier uniqueness remains `(tenantId, gameInstanceId, EffectId)`. The barrier is coordination/evidence state, not a second effect identity. `regionId` comes from Game Session's durable region authority and is propagated unchanged through World validation and any Game Logic re-resolution under the same root `EffectId` and unchanged `requestDigest`. Game Session retains the actor-lock token/lease and durable fence through the Entity-local transaction while asking World to validate the `TargetingFactSnapshot` location/version evidence. World returns an attestation bound to that same root `EffectId`, actor, `regionId`, exact `playableStateNamespaceId`, exact `playableStateScope`, `RoomInstanceRef`, `regionEpoch`, `executorFence`, and exact unchanged `requestDigest`.

Game Session renews the actor-lock lease with that same opaque Redis lock token through Entity's commit acknowledgement and passes the attestation unchanged to Entity; the durable `executorFence` is validated separately at the barrier and Entity commit gates. Entity verifies the attestation's exact binding, including `regionId`, exact `playableStateNamespaceId`, and exact `playableStateScope`, against the request, participant guard, and canonical unchanged `requestDigest` for the same root `EffectId` in its local transaction before changing the holder. A missing, mismatched, or stale attestation (including a changed region, namespace, scope, epoch, or fence) commits no Entity mutation; the Game Session orchestrator invokes Game Logic to re-resolve the targeting facts under that same root `EffectId`, preserving the unchanged `requestDigest`, namespace, scope, and `regionId`. A changed request is rejected under the participant guard rather than reusing the root. Barrier creation, renewal, replay, terminalization, and reconciliation are durable, fence-checked transitions; each preserves and exact-compares the namespace and scope evidence, and Redis lock expiry cannot clear or terminalize the barrier. A lock-renewal failure or fence change fences the old orchestrator and leaves the durable barrier `RECONCILIATION_REQUIRED`. Handoff/recovery must drain or reconcile that barrier from durable guard/holder evidence, including the exact namespace and scope; a new owner may reconcile the exact root `EffectId`, but MOVE admission cannot authorize a conflicting actor move merely because the Redis lock expired, and must wait for terminal barrier evidence. Every `MOVE` for the actor must acquire the same actor lock and pass the barrier gate, so it cannot interleave between World validation and Entity commit. After Entity commits, the acknowledgement is recorded, and the barrier and lock are released, a later valid `MOVE` is permitted. This is serialization, durable barrier evidence, and fencing across local transactions, not a distributed World/Entity transaction.

Any handoff or recovery path that performs maintenance mutation on this barrier must first carry the unchanged ADR-0085 external maintenance handoff: exact `operationId`, external maintenance fence/lock binding, classifier, immutable evidence digest, and complete affected scope. The only alternative is an explicitly owner-recorded non-conflicting routine-replay or reconciliation exemption with its classification and evidence. The actor Redis lock and durable `executorFence` remain separate execution aids and never substitute for either authorization; a missing or mismatched handoff or exemption leaves the barrier `RECONCILIATION_REQUIRED` and fails closed. These controls do not alter the accepted `(tenantId, gameInstanceId, EffectId)` identity or its exact request/namespace/scope evidence.

Focused proof must exercise stale location/version evidence, root/actor/room/namespace/scope/epoch/fence or request-digest mismatches, Entity rejection before holder mutation, actor-lock lease expiry while the barrier is in flight, an owner crash after World validation and before Entity acknowledgement, lock-renewal failure, executor-fence change fencing the old orchestrator, handoff/recovery reconciliation of the exact root, an old Entity commit racing a new MOVE, and a valid `MOVE` after terminal barrier evidence. For `PICKUP`, a previously moved item is replay only when the stored participant guard matches the same root `EffectId`, immutable request digest, and exact destination container for that actor; another holder or a different destination is a conflict/stale/reconciliation outcome, never a generic replay.
...
**Current implementation status:** this reconciliation backlog, its typed `EffectReconciliationAuthority`, its durable backlog/authority schema, and the list/retry/acknowledgement control-plane interfaces below are target-only and unimplemented. Current Game Session persists `tick_batch` and `tick_effect` rows, but it has no implemented `effect_reconciliation_backlog` schema or API. Existing domain participant guards and remote-follow-up reconciliation do not establish this cross-service backlog surface. This paragraph records current status only; the owner and contract below remain normative target state.
...
- Minimum persisted fields:
  - `effectType` (the typed operation/effect identity, not only a display category)
  - `targetAggregateId` (the exact target aggregate identity)
  - `targetScope` (the complete target scope, including the applicable namespace, region/epoch, and other scope-fence evidence; `RoomInstanceRef` or region scope is not sufficient by itself)
  - `requestDigest` (the immutable digest of the normalized request and operation inputs)
  - `sealedManifestRef` and `sealedManifestDigest` for the immutable participant/operation manifest, or an equivalent immutable reference whose read is mandatory before action
  - `ownerFence`, `executorFence`, and applicable pin/version and namespace/scope evidence, when those guards apply to the effect
  ...
  - `backlogRevision` (a monotonic compare-and-set revision for control-plane actions)
  - `status` (`PENDING`, `CONVERGED`, `DEAD_LETTER`)
  - `lastErrorCode` / `lastErrorMessage`
- Every immutable guard field is binding evidence written with the initial backlog row; it is not an additional backlog uniqueness or partition dimension, and retries and acknowledgements must not rewrite it. A compact reference to the sealed effect ledger is acceptable only when every retry and acknowledgement atomically resolves that reference and exact-compares the complete guard before action. A missing, unreadable, stale, or mismatched guard is an identity conflict/reconciliation-required outcome and fails closed; it is never treated as a fresh effect or a permissive wildcard.
...
- Inserts and status transitions must remain idempotent on `(tenantId, gameInstanceId, effectId)` so duplicate scheduling does not create duplicate backlog rows; guard fields such as namespace, scope, operation, target, and fence are exact-compared conflict/reconciliation evidence rather than new uniqueness dimensions. Every control-plane mutation also compares `backlogRevision` and the expected status and advances the revision atomically with its result.
...
- `EffectParticipantAcknowledgement/v1` is the typed target acknowledgement exchanged by each mutation owner and Game Session. It carries the participant key and owner, a stable acknowledgement identity, the complete immutable effect guard (or mandatory `sealedManifestRef` plus `sealedManifestDigest`), exact effect request digest, target aggregate, complete target scope, applicable owner/executor/pin/version fence, owner revision, durable `applied` or `no-op` result/read reference, acknowledgement status, and evidence digest. An exact identity/guard retry replays the stored acknowledgement; changed identity, guard, target, scope, digest, fence, or owner revision fails closed. Only exact durable `applied`/`no-op` acknowledgements for every member of the sealed participant manifest may advance the backlog to `CONVERGED`; pending, final-failure, stale, missing, or ambiguous evidence cannot do so. World and Entity expose this participant contract from their owner boundaries; Game Session remains the cross-service backlog authority.

Retry and dead-letter policy:

- Default retry strategy is bounded exponential backoff with jitter and no mutation of `EffectId`.
- A backlog row remains `PENDING` until either all required participants acknowledge applied/no-op for the same `EffectId`, allowing reconciliation to transition it to `CONVERGED`, or retry exhaustion or explicit audited operator action transitions the backlog row to `DEAD_LETTER` without terminalizing the underlying effect.
- `DEAD_LETTER` is a backlog retry-control status, not an effect-ledger outcome; no destructive compensation is issued from this path. The underlying effect ledger remains governed by the `APPLIED`/evidence-qualified `ABANDONED` contract and remains nonterminal when execution is inconclusive.
- Dead-letter backlog rows remain replayable via explicit operator/API actions; an authorized replay transitions the backlog row back to `PENDING` under the existing audit/recovery contract and preserves the original `EffectId`. Acknowledgement is separate attention/audit evidence: it leaves retry-control status `DEAD_LETTER`, never terminalizes or compensates the underlying effect, and records immutable acknowledgement time, actor, reason, action identity, and result. A later authorized retry CASes `DEAD_LETTER` to `PENDING` while preserving that acknowledgement history.
...
- `RetryEffectReconciliation(tenantId, gameInstanceId, effectId, actionRequestId, actionDigest, expectedBacklogRevision, expectedStatus, authority: EffectReconciliationAuthority)` for explicit replay from `DEAD_LETTER` or stuck `PENDING`.
- `AcknowledgeEffectDeadLetter(tenantId, gameInstanceId, effectId, reason, actionRequestId, actionDigest, expectedBacklogRevision, expectedStatus, authority: EffectReconciliationAuthority)` for audited operator decisions. It records the immutable acknowledgement evidence while leaving the row in `DEAD_LETTER` and the underlying effect outcome unchanged.
- Logging & Admin dashboards should consume these APIs; operators should not mutate backlog tables directly.
- `actionRequestId` is stable for one requested control-plane action. Before any effect execution or external/authority call, the owner must persist a unique, digest-bound action-intent row keyed by the stable action-family request identity `(tenantId, gameInstanceId, effectId, actionRequestId)` with status `IN_PROGRESS`; `actionType` is an immutable tuple member, not a deduplication partition, and the existing identity must be loaded and compared before selecting the retry or acknowledgement branch. No action may execute without that intent. Reuse of the same request identity with a different action type or any other complete-tuple/digest difference fails closed as an identity conflict; it must not create a second intent row. `actionDigest` remains the API field name where useful and is the `mutationDigest/v1` value for the `effect-reconciliation-action/v1` action-family schema pair; it is not a competing `actionDigest/v1` digest owner. This family uses ADR 0047's exact seven top-level fields only: `scope` is the exact affected tenant/game/runtime scope; `target` is the effect identity plus exact target aggregate; `expectedVersion` is the expected backlog status and revision; `mutation` is the closed retry-or-ack action and its typed action inputs; and `auditReason` is the typed reason with explicit presence/value. `actionRequestId`, the opaque Account authorization reference, `EffectReconciliationAuthority` branch/reference/evidence, handoff/member evidence, actor/workload evidence, and reservation/fence fields remain excluded from the digest exactly as ADR 0047 requires and are separately exact-compared in the complete post-authorization/execution tuple. Absent and explicit `null` are distinct canonical values. The action-family mapping requires shared cross-language golden vectors, including absent/null cases, and explicit schema-version migration rules before any version change.
- Every `IN_PROGRESS` intent also records the current recovery owner/claim identity, a monotonic attempt fence, lease/deadline evidence, and the durable action identity used downstream. A successor may take over only through an owner-local CAS after lease expiry; stale owners and stale attempt fences are rejected before any backlog mutation or external call. The downstream owner or external operation must receive the stable action identity plus the current attempt fence, or an equivalent durable idempotency binding. For an owner-local backlog mutation, the action-intent row, backlog status/revision CAS, and final action result/outcome commit atomically in one owner-local transaction, so a successful local CAS cannot exist without the result persistence. Each intent has at most one terminal action result; a response is only a readback of that durable result. For an external or cross-service consequence, the action remains `IN_PROGRESS` until readback reconciliation against the exact effect guard, handoff and sealed member-manifest evidence, current backlog status/revision, and current attempt fence determines whether the consequence is applied, already applied, or still inconclusive. An ambiguous action must not be blindly reissued or assigned a new identity; the same `actionRequestId`/digest resumes reconciliation, while a changed digest fails closed as an identity conflict. Concurrent retry and acknowledgement actions serialize through the backlog CAS, rather than last-writer-wins behavior.
- The durable action ledger/result stores the digest, actor/time, expected state, CAS result, recovery claim/attempt fence, and returned outcome. Owner-local proof covers crash before the intent or before the atomic transaction commits, plus response loss after the committed result (which replays that result). Separately, external or cross-service proof covers a successful downstream CAS/consequence before local result persistence or a lost external response; recovery must reconcile the still-`IN_PROGRESS` intent from exact evidence. Lease expiry, takeover, stale-owner rejection, and all ambiguous cases must preserve the same action identity and either replay the committed result, reconcile the consequence, or remain safely inconclusive without a second action identity.

`EffectReconciliationAuthority` is a typed union/reference carried by both mutating requests:

- `ExternalMaintenanceHandoff` is the unchanged ADR-0085 handoff: exact `operationId`, external maintenance fence/lock binding, classifier, immutable evidence digest, and complete affected scope. The handoff also contains either a sealed exact-effect manifest or an immutable selection predicate plus a frozen owner-resolved member manifest and digest. For each action, the owner atomically resolves the handoff and compares its operation/fence/classifier/evidence digest/affected scope and member membership with the selected effect's immutable guard and current backlog revision/status. A handoff authorizes only its bounded sealed member set; it is not globally single-use, and each member action is independently idempotent through `actionRequestId`/`actionDigest`. Scope widening, stale revision or fence, absent membership, or any mismatch fails closed. The handoff evidence digest does not replace the effect's request digest.
- `OwnerRoutineReplayExemption` is an owner-issued, durably recorded non-conflicting exemption. It carries the stable owner-recorded `exemptionId` (and the existing owner authorization/audit record link where applicable), the exact effect identity, exact target, complete affected scope, classification, and immutable evidence digest. The owner persists this record before the request and atomically reads it back and exact-compares those fields with `(tenantId, gameInstanceId, effectId)`, the target, and the requested scope before allowing replay or reconciliation. A caller cannot self-label a request as routine or supply an unverified exemption.

For spatial or ambient maintenance, recovery, or reset mutations invoked through either API, exactly one authority branch must resolve and match before mutation. The selected authority must also pass the common atomic effect-guard and backlog revision/status comparison; an authority that is valid for another effect, target, scope, or revision cannot authorize this action. Ordinary owner-local replay/reconciliation that satisfies the owner exemption may preserve the original `EffectId`; missing, ambiguous, or contradictory handoff/exemption evidence fails closed. Existing operator authorization issuance and redemption policy remains owned by its canonical Account/Logging & Admin contract; this carrier links to that owner evidence rather than redefining it here.
...
A concrete adopter must satisfy [Mandatory Workflow Adopter Classification](#mandatory-workflow-adopter-classification) in its canonical owning-service architecture document or an accepted ADR before it exposes the library's accompanying Flyway migrations at `classpath:db/migration/saga`; module or convention inclusion alone does not establish Saga adoption. See [ADR 0080](./decisions/adr-0080-service-owned-schemas-with-adopter-local-shared-migrations.md). The migrations persist `saga_instance` and `saga_step` in each adopter's own schema. Short-saga examples and consequences are documented in the [Game Session Service runtime and data](./microservices/game-session-service/runtime-and-data.md) and the Logging & Admin Service README; the [World Creation Workflow](./microservices/world-management-service/world-creation-workflow.md) is the Temporal `world-lifecycle` adopter, not a Saga.
