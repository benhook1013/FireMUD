...
- **Target-state identity contract:** The Game Session Service computes one stable shared logical operation identity for the admitted gameplay operation and deterministically derives one target-specific `EffectId` from the region-scoped tick context (`tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `tickId`, `effectKey`) plus each target aggregate identity (`targetAggregateType`, `targetAggregateId`). Within one tenant/game/scope/region/epoch/tick, `effectKey` must be unique for each admitted logical command or operation; it must incorporate a stable admitted command identity or explicit operation identity rather than only an effect type, participant tuple, or mutable payload. For automation fan-out, the identity is `(automationDispatchId, commandOrdinal)`; otherwise it is the admitted `commandId`. If a non-automation command can emit sibling effects, `commandId` must be combined with a deterministic per-effect discriminator or operation identity so each sibling receives a distinct `effectKey` and target-specific `EffectId`. Retries reuse the same logical operation identity and each target projection; distinct admitted operations do not collide.
- **Single propagation contract:** The coordinator derives the shared logical operation identity once, then derives the target-specific opaque serialized `EffectId` projection for each participant from that participant's canonical target aggregate identity. It sends each participant its projection together with the shared operation identity and complete structured tuple. Participants validate the tuple and expected target projection; they do not append an unregistered domain scope, substitute a mutable payload hash, or generate a fresh participant ID. A physical opaque `effectId` column is valid only when its target-specific canonical tuple projection and shared operation identity are retained or deterministically recoverable.
...
- A multi-service operation has one shared logical operation identity, not one shared targetless `EffectId`. It deterministically projects one target-specific `EffectId` per participant aggregate by appending that aggregate's canonical type and id. Each owning service persists its exact target projection, guard outcome, and any outbox/reconciliation reference; retries reuse that projection unchanged. The shared operation identity and expected participant set drive reconciliation, while no targetless `EffectId` or one target's terminal outcome is reused across participant aggregates.
- Owning services must implement durable idempotency guards (unique constraints, monotonic updates, transactional outbox) keyed by the target-specific `EffectId` projection so duplicate attempts become OK/no-op outcomes rather than double-applying side effects. The guard must persist the target projection and its terminal outcome, not only the shared logical operation identity.
- For gameplay-visible mutations, target-specific `EffectId`-backed guard rows are the default idempotency boundary. Simpler `last_tick_id` watermark patterns are allowed only for aggregates explicitly documented as receiving at most one logical mutation per tick.
