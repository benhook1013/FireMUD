...
- **Target-state identity contract:** Game Session assigns the opaque root `EffectId` for each command-root logical operation in the supplied semantic plan order. The durable gameplay command row already freezes/binds its request fingerprint and resolved runtime scope. The root allocation row is unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`, also binds the ordered plan manifest digest, and validates that frozen command/runtime binding. Reuse requires the same command row identity, frozen command/runtime binding, `planOrdinal`, and manifest digest; any mismatch fails closed. For Automation, the complete applicable Command-Handoff Identity is the admission/deduplication identity and must exact-map to one durable target `commandId`; after that mapping, allocation uses the target command row's same canonical key, not the handoff tuple itself. Trigger Identity and `scriptEventId` remain handler/correlation identities. One logical operation has one root. `planOrdinal` applies only to command-root work; passive, inbound/remote, timer, retry, and already-generated effects retain owner/source-specific root identities unless they materialize as an admitted command. Random staging, mutable payload/text, source ordering, participant tuples, and child ordinals cannot substitute for the durable command row identity. It preserves the intended pre/post state, required participant set, and status. A participant guard's uniqueness identity has exactly these inputs: the root `EffectId`, typed operation, and target aggregate. The guard row binds the immutable request digest and stores the outcome; outcome/status, evidence, and reconciliation fields are mutable guard-row state protected by compare-and-set (CAS), not uniqueness-identity inputs. A matching identity with a changed operation, target, or digest fails closed. Ordinary retries and replay keep the persisted plan ordinal and root `EffectId`; generated child effects retain the enclosing root and their owner-defined lineage identity.
...
- **Single propagation contract:** The coordinator sends the persisted root `EffectId`, typed operation, target, immutable request digest, and required-participant context; Game Session retains the associated `planOrdinal` in its plan/ledger binding. Participants validate the root contract and atomically commit their guard with effect-visible rows before acknowledging. They do not generate a fresh identity or bind a guard to mutable payload.
