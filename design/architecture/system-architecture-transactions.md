...
- Inserts and status transitions must be idempotent on `(tenantId, gameInstanceId, effectId)` so duplicate scheduling does not create duplicate backlog rows. Every control-plane mutation also compares `backlogRevision` and the expected status and advances the revision atomically with its result.
