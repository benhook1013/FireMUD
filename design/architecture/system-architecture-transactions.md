...
- **Target-state identity contract:** The Game Session Service computes and propagates a stable `EffectId` derived from the admitted gameplay-state and region-scoped tick context (`tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `tickId`, `effectKey`) plus the required target aggregate identity (`targetAggregateType`, `targetAggregateId`). Within one tenant/game/scope/region/epoch/tick, `effectKey` must be unique for each admitted logical command or operation; it must incorporate a stable admitted command identity or explicit operation identity rather than only an effect type, participant tuple, or mutable payload. For automation fan-out, the identity is `(automationDispatchId, commandOrdinal)`; otherwise it is the admitted `commandId`. If a non-automation command can emit sibling effects, `commandId` must be combined with a deterministic per-effect discriminator or operation identity so each sibling receives a distinct `effectKey` and terminal outcome. Retries reuse the same identity and distinct admitted operations do not collide.
- **Single propagation contract:** The coordinator derives the canonical identity once and sends the same opaque serialized `effectId` plus the complete structured tuple to every participant. Participants validate the tuple and project it into their own durable guard; they do not append an unregistered domain scope, substitute a mutable payload hash, or generate a fresh participant ID. A physical opaque `effectId` column is valid only when its canonical tuple projection is retained or deterministically recoverable.
...
- A multi-service operation has one logical operation identity but projects one target-specific `EffectId` per participant aggregate by appending that aggregate's canonical type and id. Each owning service guards its own projection; the shared operation identity and expected participant set drive reconciliation, while no targetless `EffectId` is reused across participant aggregates.
