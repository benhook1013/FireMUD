...
- For plugin-only rollback or update, durable effect staging and replay must also preserve the exact `(pluginId, pluginVersionId, bindingId)` and captured Automation-owned `pluginActivationGeneration` runtime-state fence through final effect/replay, rejecting displaced work. This generation is monotonic: each committed invalidating plugin transition advances it exactly once, while `DRAINING` does not; the captured generation is revalidated at evaluation, persistence, handoff, staged/final effect, retry, replay, and recovery boundaries under the [plugin version-fencing contract](./system-architecture-scripting-contracts.md#8-plugin-version-fencing-and-control-plane-scope). A plugin-only change does not advance `scriptPinEpoch` unless the script pin changes in the same operation.
