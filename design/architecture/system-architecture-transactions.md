...
- **Target-state identity contract:** Game Session assigns one stable root `EffectId` for the admitted logical effect. It preserves the intended pre/post state, required participant set, and status. A participant derives its durable guard identity from that root identity, typed operation, and target aggregate; the guard binds the immutable request digest and stored outcome. Same guard identity with a changed operation, target, or digest fails closed. Retries keep the root `EffectId`; deterministic child effects are used for derived reactions.
