...
Terminal-parent cleanup therefore detaches the nullable `script_event_audit.work_item_id` reference before deleting the parent, preserving the audit row for its independent retention horizon; handoff children that cannot outlive the parent are disposed in the same transaction.
