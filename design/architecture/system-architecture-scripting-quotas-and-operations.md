...
- Event-scope admission does not charge live per-script quota or execution capacity. After handler resolution, Automation persists or reuses the durable pre-DSL trigger-record row keyed by the full applicable Trigger Identity (including each `onLoad` handler) and records an exactly-once admission marker on that row.
- Execution capacity is a separate, fenced, reclaimable lease. A handler waiting in `queue_until_free` holds no capacity; the execution-start marker is recorded exactly once only when a valid lease is acquired. Duplicate deliveries and recovery attempts reuse that same durable pre-DSL trigger-record row and its markers.
...
- **Live per-script quota:** Current ingress acquires `ScriptQuotaService` for `STANDARD_RUNTIME` resolved handlers before durable `script_work_items` materialization. A denial writes a handler-scoped audit row with `finalStage=ADMISSION`, `finalOutcome=quota_denied`, and `finalReason=script_quota_denied`; no outbox work item is created. The target full-Trigger-Identity admission marker on the durable pre-DSL trigger-record row is not yet implemented. The current physical table name does not make this pre-evaluation record a post-DSL script work item.
...
Timer and interval limits are evaluated against the canonical runtime-and-mode scope tuple `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, isDryRun>`. A per-tenant or per-game-instance timer limit must not substitute for that tuple and accidentally couple unrelated instances, playable-state namespaces, regions, or live versus dry-run/test capacity; any broader aggregate ceiling is an additional explicitly named safety limit. `playableStateScope` is separately persisted and exact-validated as policy/routing/authorization/migration-fence evidence used for handler/work fencing; it remains outside timer/schedule uniqueness and does not replace the scheduler's runtime-and-mode tuple for these timer-capacity limits.
...
- **One pre-DSL trigger-record row:** For every resolved handler, including `onLoad`, Automation creates or reuses the existing durable pre-DSL trigger-record row keyed by the full applicable Trigger Identity. This is distinct from the post-DSL script work item/descriptor even where the current physical table retains the legacy `script_work_items` name. The row has separate exactly-once `admissionCharged` and `executionStarted` markers. A duplicate or recovery attempt with the same identity reads and reuses those markers on that same row rather than charging again; this is not a separate charge record or schema.
...
- **Execution-start marker and lease:** Before DSL evaluation, the worker acquires a separately fenced, reclaimable tenant/cluster capacity lease, then in one Automation-owned durable executor-acceptance transaction revalidates the current fence, durably accepts/claims the run for the executor on that same durable pre-DSL trigger-record row, persists the exactly-once execution-start marker, and transitions that row to `EXECUTING`. Evaluation begins only after that commit. If the executor cannot accept the run, the transition does not commit, the lease is released or reclaimed, and no execution charge is recorded. A crash after commit recovers from the executor claim on that same durable row and may reacquire a lease without creating another marker; a lease is not a quota refund.
...
- `S2` is accepted under `concurrencyPolicy=queue_until_free`. Its durable pre-DSL trigger-record row records the admission marker, but it holds no execution capacity until a fenced lease is acquired and the execution-start marker is committed on that same row.
- `S3` is admitted directly to execution. Its durable pre-DSL trigger-record row records the admission marker and then the exactly-once execution-start marker under a valid capacity lease.
...
- **Core identifiers**
  ...
  - `playableStateNamespaceId` – durable admitted gameplay-state namespace identity for gameplay/runtime triggers; absent for tenant-readiness `onLoad`.
  - `playableStateScope` – separately persisted and exact-validated routing, authorization, and migration-fence evidence for gameplay/runtime triggers; it is not namespace identity or a join key and is absent for tenant-readiness `onLoad`.
...
- `playableStateNamespaceId` (gameplay/runtime only) – identifies the durable admitted gameplay-state namespace and participates in complete Trigger Identity correlation. Tenant-readiness `onLoad` intentionally omits it.
- `playableStateScope` (gameplay/runtime only) – is separately retained and exact-validated routing, authorization, and migration-fence evidence. It is not namespace identity or a correlation join key and is omitted for tenant-readiness `onLoad`. See the [normative Trigger Identity table](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields).
...
1. Start from a player-visible issue or a game tick log that includes `tenantId`, `gameInstanceId`, `playableStateNamespaceId` and separately validated `playableStateScope` when gameplay/runtime-scoped, `regionId`, `regionEpoch`, `entityId`, and `tickId`.
2. Join the tick log, `script_event_audit`, and logs/traces by the complete applicable Trigger Identity: `tenantId`, `gameInstanceId`, `playableStateNamespaceId` when gameplay/runtime-scoped, `regionId`, `regionEpoch`, `entityId` when applicable, `scriptId`, `eventType`, `eventSchemaVersion`, `scriptPatchVersion`, `scriptPinEpoch` when applicable, `scriptEventId`, `isDryRun`, and plugin/scheduler fields when applicable. Exact-validate `playableStateScope` alongside that join as separate routing, authorization, and migration-fence evidence; do not use it as namespace identity or a join key. `scriptEventId` and `correlationId` are scoped filters that narrow a search; neither is a standalone execution identity and neither may be used as the join key without the remaining applicable fields. Do not rely on `scriptEventId` as a metric label; use metrics to understand aggregate rates by bounded `scope` / `script_kind` / `event_class` dimensions and use audit/log/trace queries for per-trigger correlation.
...
- **Timer and scheduling knobs** – influence `onInterval` / `onTimerExpire` behavior, including cadence and maximum timers evaluated per canonical runtime-and-mode tuple `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, isDryRun>`, plus any backoff or delay settings applied when regions are degraded.
