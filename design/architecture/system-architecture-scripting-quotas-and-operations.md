...
These controls work alongside the **failure-rate circuit breaker**, whose authoritative persisted state, transition history, and validated reset are owned by the exact Automation breaker aggregate for one immutable script/plugin version and runtime activation scope. It can automatically place that exact aggregate into `DISABLED_DUE_TO_ERRORS` when, within the configured rolling window, eligible classified samples are greater than or equal to the configured minimum and qualifying handler-attributable failures divided by eligible classified samples are greater than or equal to the configured failure-rate threshold, with all settings within platform hard bounds. For core scripts, any `runtimeStatus=DISABLED_DUE_TO_ERRORS` wording is only a read-only effective-admission projection for the queried `(tenantId, gameInstanceId, scriptPatchVersion, scriptPinEpoch, scriptId)` tuple; it is not persisted breaker authority or transition history.

Only handler-attributable deterministic evaluation, sandbox-limit, and authored-output failures count. Quota/capacity denial, infrastructure or owner unavailability, rollback/version fencing, expected gameplay precondition rejection, dry-run/test traffic, operator cancellation, and player-controlled invalid input do not. A trip blocks new admission but does not cancel accepted work or reverse effects. Recovery requires a new version or explicit audited reset after validation; emergency component revocation remains a separate immediate fence. The classified samples and policy version are retained as durable audit evidence by the breaker owner.

For plugin handlers, the trip changes only the separate `breakerState` for the exact `(tenantId, gameInstanceId, pluginId, pluginVersionId)` identity, where `(tenantId, gameInstanceId)` is the runtime activation scope; persisted breaker state and read-model lookup use this complete tuple. It does not change lifecycle `pluginState`, `pluginActivationEpoch`, or `lifecycleRevision`. A newly resolved handler is denied with `finalStage=ADMISSION`, `finalOutcome=disabled_due_to_errors`, and bounded `finalReason=failure_rate_breaker`; a new version starts `CLOSED`, and an audited reset is independent of lifecycle mutation.
...
Timer and interval limits are evaluated against the canonical mode-specific per-namespace runtime scope tuple `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, isDryRun>`. A per-tenant or per-game-instance timer limit must not substitute for that tuple and accidentally couple unrelated playable-state namespaces, regions, or live/test modes; any separately named broader aggregate ceiling (for example, a cluster-wide safety limit) is an additional safety limit. `playableStateScope` is server-derived and must be exact-validated against the authoritative namespace/routing proof, then retained unchanged as separately persisted policy/routing/authorization/migration-fence evidence on event-scope ingress, durable schedules, due-candidate records, firing claims, candidate-audit records, mode-specific resume-window records, handler/work items, and per-command handoff records. This persistence list is non-exhaustive; the complete timer identity and recovery projections are owned by [Scripting Scheduler and Timer Lifecycle](./system-architecture-scripting-scheduler-and-timers.md#target-state-design) and the [normative timer semantics matrix](./system-architecture-scripting-normative-contract-tables.md#table-3-timer-semantics-matrix). Logs, audit rows, and traces may carry bounded correlation copies for diagnosis, but those copies do not become authority and must not be used to reconstruct or override the retained evidence. Scope evidence does not replace the scheduler's runtime scope tuple or become a Trigger Identity, uniqueness, or deduplication input; see the [normative identity tables](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields).
...
- **Per-tenant**:
  ...
  - For target pre-handler/catch-up candidate ceiling accounting, each tenant's `high`, `normal`, and `background` ceiling is an independent capacity. When an applicable tenant ceiling is exhausted, every candidate governed by that ceiling is denied regardless of priority; priority orders candidates only while capacity remains and never bypasses an exhausted ceiling. Such candidate-level denials use the Table 4 pre-evaluation skip consequence with reason `tenant_budget_exceeded`, while other tenants and independently capacitated tiers continue to make progress. A live `ScriptTenantBudgetService` denial occurs after the executor's durable work-item claim but before the separate executor-acceptance boundary and DSL evaluation. It terminally cancels the claimed work item, records the handler-scoped audit outcome, increments `automation_script_triggers_total`, and increments `automation_script_tenant_budget_denied_total`; the already-committed per-script admission marker and its charge record are preserved. Because executor acceptance did not commit, no fenced execution-capacity lease, execution-start marker, or execution-start charge is recorded. It does not increment `automation_script_skips_total`. Catch-up candidate ceiling skips do not increment the handler tenant-budget decision metric.
...
Tenant-scoped script-side keys remain scoped by `tenantId`, and scheduler ownership must remain explicit enough that each tenant’s automation workload can be reasoned about and tuned independently while still sharing the same infrastructure. The capacity counter/lease pairs use matching Redis Cluster hash tags: per-tenant dry-run and readiness keys use `{tenantId}`; tenantless dry-run cluster keys use fixed `{automation-test-capacity}`; and tenantless readiness cluster keys use fixed `{automation-readiness-capacity}`. Only the cluster capacity counters are intentionally tenantless owner-scoped exceptions; their leases retain tenant/work identity after the fixed slot tag and remain bounded-TTL operational state, never tenant identity or authority. The per-tenant and cluster lease keys therefore remain atomically releasable by their Lua scripts without cross-slot failures, while lease identity still prevents release or reclaim across tenants or work items. Operator-facing metrics, however, must use the bounded `scope`, category, family, or tier labels defined in canonical Table 4 rather than raw tenant/runtime identifiers. Do not infer a separate canonical `script-leader:*` prefix unless the Redis coordination docs explicitly add one.
...
- **Core identifiers**
  ...
  - `scriptPinControlPlaneRequestId` – mandatory nonblank owner evidence paired with `scriptPatchVersion` and positive `scriptPinEpoch` for instance-scoped gameplay/runtime and scheduler triggers. It must be retained and exact-compared through quota admission, durable work, retry, replay, and recovery, but is excluded from the trigger, candidate, handler, and work-item claim/uniqueness keys governed here and is not execution identity. It is absent for tenant-readiness `onLoad` and explicitly unpinned epoch-zero records.
...
Current live cleanup deletes retention-eligible `HANDED_OFF` and `CANCELED` work-item parents only after detaching the nullable `script_event_audit.work_item_id` reference and deleting handoff children that cannot outlive the parent in the same transaction; the audit remains available for its independent retention horizon. Automatic age- and row-cap cleanup of `DEAD_LETTERED` parents fails closed and deletes nothing because the live schema cannot yet prove recovery-aware whole-bundle eligibility. Target cleanup retains every `EVALUATED_COMMITTED` parent and corresponding audit marker, atomically purges only an eligible descriptor/output/child evidence bundle, and persists `descriptorEvidencePurgedAt` on that retained parent so recovery readback rejects it as non-recoverable. The separate target path for a terminal pre-DSL `DEAD_LETTERED` row may purge retention-eligible trigger/failure payload material without that marker only after its recovery window expires; retained purge audit and idempotency evidence then make it permanently non-recoverable.
...
- `playableStateNamespaceId` (gameplay/runtime only) – identifies the durable playable-state namespace in applicable Trigger Identity and deduplication. `playableStateScope` is separately persisted and exact-validated policy/routing/authorization/migration-fence evidence; retain it in applicable audit/log/trace correlation, not as an identity or metric label. See the [normative Trigger Identity table](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields); tenant-readiness `onLoad` intentionally omits both fields.
...
1. Start from a player-visible issue or a game tick log that includes `tenantId`, `gameInstanceId`, `playableStateNamespaceId` when gameplay/runtime-scoped, `regionId`, `regionEpoch`, `entityId`, and `tickId`; retain `playableStateScope` separately as exact-validated policy/routing/authorization/migration-fence evidence.
2. Join the tick log, `script_event_audit`, and logs/traces by the complete applicable Trigger Identity: `tenantId`, `gameInstanceId`, `playableStateNamespaceId` when gameplay/runtime-scoped, `regionId`, `regionEpoch`, `entityId` when applicable, `scriptId`, `eventType`, `eventSchemaVersion`, `scriptPatchVersion`, `scriptPinEpoch` when applicable, `scriptEventId`, `isDryRun`, and plugin/scheduler fields when applicable. Retain `playableStateScope` as separately persisted, exact-validated policy/routing/authorization/migration-fence evidence for filtering/readback, not as identity or a join-key dimension. `scriptEventId` and `correlationId` are scoped filters that narrow a search; neither is a standalone execution identity and neither may be used as the join key without the remaining applicable fields. Do not rely on `scriptEventId` as a metric label; use metrics to understand aggregate rates by bounded `scope` / `script_kind` / `event_class` dimensions and use audit/log/trace queries for per-trigger correlation.
...
- Dry-run/test executions must also have isolated execution capacity:
  ...
  - A capacity counter or lease key-shape change is not safe as an ordinary rolling deployment. Until deployment automation enforces this gate, operators must stop every old Automation writer, wait for the full ten-minute capacity-lease TTL, verify that no old writer remains, and only then start binaries that use the new key shape. Mixed-version writers must never reserve or release against separate key families.
...
The failure-rate circuit breaker counts only handler-attributable deterministic evaluation failures, sandbox-limit failures, and authored-output validation failures for live scripts and plugins when deciding to transition the exact core-script breaker aggregate into `breakerState=DISABLED_DUE_TO_ERRORS` or a plugin's separate `breakerState=DISABLED_DUE_TO_ERRORS`. It excludes quota/capacity denial, infrastructure or owner unavailability, rollback/version fencing, expected gameplay precondition rejection, dry-run/test traffic, operator cancellation, and player-controlled invalid input. These excluded outcomes do not, by themselves, trigger disables, although they should still be visible in metrics and dashboards. For core scripts, any `runtimeStatus=DISABLED_DUE_TO_ERRORS` value is only a read-only effective-admission projection for the queried exact tuple and must not be persisted as breaker state or used as transition history/reset authority. For plugins, this breaker state is keyed to the exact active version/runtime scope and must not be represented by `pluginState`.
...
- **Timer and scheduling knobs** – influence `onInterval` / `onTimerExpire` behavior, including cadence and maximum timers evaluated per canonical mode-specific per-namespace runtime scope tuple `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, isDryRun>`, plus any separately named broader aggregate ceilings and backoff or delay settings applied when regions are degraded.
