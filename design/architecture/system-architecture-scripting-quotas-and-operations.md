...
Terminal-parent cleanup therefore atomically marks every retained `script_event_audit` row terminal and non-recoverable, detaches its nullable `work_item_id` reference, disposes handoff children that cannot outlive the parent, and only then deletes the parent in that same transaction. Recovery readback treats a retained audit whose parent reference is absent as terminal, non-recoverable evidence; it must never infer recovery eligibility from that detached row. The audit remains available for its independent retention horizon.
