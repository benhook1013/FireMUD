...
- `S2` is accepted under `concurrencyPolicy=queue_until_free`. Its durable pre-DSL trigger-record/work-item row records the admission marker, but it holds no execution capacity until a fenced lease is acquired and the execution-start marker is committed on that same row.
- `S3` is admitted directly to execution. Its durable pre-DSL trigger-record/work-item row records the admission marker and then the exactly-once execution-start marker under a valid capacity lease.
