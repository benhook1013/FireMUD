...
Tenant-scoped script-side keys remain scoped by `tenantId`, and scheduler ownership must remain explicit enough that each tenant’s automation workload can be reasoned about and tuned independently while still sharing the same infrastructure. The intentionally tenantless `automation:test:capacity:cluster*` and `automation:readiness:capacity:cluster*` keys are a narrow owner-scoped exception: they are bounded-TTL operational capacity state, never tenant identity or authority, and do not remove tenant scoping from their corresponding per-tenant keys. Operator-facing metrics, however, must use the bounded `scope`, category, family, or tier labels defined in canonical Table 4 rather than raw tenant/runtime identifiers. Do not infer a separate canonical `script-leader:*` prefix unless the Redis coordination docs explicitly add one.
