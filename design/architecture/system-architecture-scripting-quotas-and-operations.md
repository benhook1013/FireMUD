...
Tenant-scoped script-side keys remain scoped by `tenantId`, and scheduler ownership must remain explicit enough that each tenant’s automation workload can be reasoned about and tuned independently while still sharing the same infrastructure. Only the cluster capacity counters `automation:test:capacity:cluster` and `automation:readiness:capacity:cluster` are intentionally tenantless owner-scoped exceptions; they are bounded-TTL operational counters, never tenant identity or authority. Their lease keys remain tenant/work scoped: `automation:test:capacity:cluster:lease:<tenantId>:<workItemId>` and `automation:readiness:capacity:cluster:lease:<tenantId>:<workItemId>`. The per-tenant lease keys are likewise scoped as `automation:test:capacity:<tenantId>:lease:<workItemId>` and `automation:readiness:capacity:<tenantId>:lease:<workItemId>`, so a lease cannot be released or reclaimed across tenants or work items. Operator-facing metrics, however, must use the bounded `scope`, category, family, or tier labels defined in canonical Table 4 rather than raw tenant/runtime identifiers. Do not infer a separate canonical `script-leader:*` prefix unless the Redis coordination docs explicitly add one.
