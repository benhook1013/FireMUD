...
- **Per-tenant**:
  ...
  - For target pre-handler/catch-up candidate ceiling accounting, when a tenant exhausts its budget for a tier, lower-priority work for that tenant is skipped with the Table 4 candidate-level pre-evaluation skip consequence using reason `tenant_budget_exceeded`, while other tenants continue to make progress. An ordinary current post-executor-claim `ScriptTenantBudgetService` denial is a handler outcome in `automation_script_triggers_total` and increments `automation_script_tenant_budget_denied_total`; it does not increment `automation_script_skips_total`. Catch-up candidate ceiling skips do not increment the handler tenant-budget decision metric.
