...
- `automation_script_work_item_outcomes_total{stage, outcome, dryRun, priorityTag}` – counts terminal durable work-item execution outcomes using the same `finalStage` / `finalOutcome` vocabulary written to `script_event_audit`, so quota, output-budget, dry-run, and handoff outcomes can be compared without parsing audit rows.
