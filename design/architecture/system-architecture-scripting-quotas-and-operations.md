...
The failure-rate circuit breaker primarily considers **sandbox_error**, **`validation_error` from live handler/runtime validation**, and other qualifying deterministic handler failures when deciding to transition the exact core-script breaker aggregate into `breakerState=DISABLED_DUE_TO_ERRORS` or a plugin's separate `breakerState=DISABLED_DUE_TO_ERRORS`. Quota denials and purely infrastructure-level errors do not, by themselves, trigger disables, although they should still be visible in metrics and dashboards. For core scripts, any `runtimeStatus=DISABLED_DUE_TO_ERRORS` value is only a read-only effective-admission projection for the queried exact tuple and must not be persisted as breaker state or used as transition history/reset authority. For plugins, this breaker state is keyed to the exact active version/runtime scope and must not be represented by `pluginState`.
