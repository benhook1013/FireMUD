...
Timer and interval limits are evaluated against the canonical runtime scope tuple `<tenantId, gameInstanceId, playableStateNamespaceId, regionId>`. A per-tenant or per-game-instance timer limit must not substitute for that tuple and accidentally couple unrelated instances, playable-state namespaces, or regions; any broader aggregate ceiling is an additional explicitly named safety limit. `playableStateScope` is separately persisted and exact-validated as policy/routing/authorization/migration-fence evidence used for handler/work fencing; it remains outside timer/schedule uniqueness and does not replace the scheduler's runtime scope tuple for these timer-capacity limits.
