...
This section records current behavior only. The current Automation claim boundary is `PENDING_EVALUATION` -> `EVALUATING`; it has no recovery owner, evaluation lease expiry/fencing generation, or descriptor-commit marker/status, and its recovery behavior is unimplemented and unverified. The current replay mutation still accepts optional `gameInstanceId`/`regionId` scope fields and empty-ID selection, selects parent `workItemIds`, requeues eligible `script_work_items` rows as `PENDING_EVALUATION`, and returns aggregate counts instead of target per-row recovery results; it does not select independent command descriptors or prove post-evaluation recovery. Current `CancelPendingWorkItemsForPluginVersion` accepts only tenant/plugin-version/instance/region plus request/audit fields and returns aggregate `canceledCount`; it does not accept the target activation/script tuples or explicit work-item IDs, return per-parent results, or reconcile evaluated descriptors and children, so the richer cancellation contract below is target-only. `GetAutomationPinConvergence` currently serves the durable `ScriptPatchPinProjection`/`script_patch_pin_projections` view with the observed patch/epoch tuple and last observed owner request identity; it does not expose `observedConvergenceAttemptGeneration`, so attempt-bound convergence proof remains unavailable. Current `GetAutomationDrainStatus` selects its counted set by mode: `NORMAL` includes only rows whose `workItem.admissionEpoch` equals the current `admissionEpoch`; `PAUSED_FOR_ROLLBACK` includes rows whose `workItem.admissionEpoch <= 0` or `workItem.admissionEpoch < current admissionEpoch`, excluding positive current-epoch rows. Within that set `activeExecutionCount` counts `EVALUATING` and `HANDOFF_IN_FLIGHT`, including unresolved stale `EVALUATING`, and `pendingCancelableWorkItemCount` counts `PENDING_EVALUATION`. The current query does not filter or group by `scriptPinEpoch`, so a response can aggregate mixed pin epochs and cannot prove cleanup for one exact epoch. The target mapping instead uses current-epoch pre-DSL and evaluated-descriptor counts. Current terminal-row cleanup is retention-based; the target terminal-evidence purge rule is not current trigger-state recovery and does not establish that the current cleanup preserves an `EVALUATED_COMMITTED` marker, audit, or replay causation. The emergency component-revocation workflow below is also target-only, unimplemented, and unproved; current runtime checks do not establish authoritative affected-scope discovery, durable containment convergence, or the complete pre-effect revocation fence. These implementation facts are a cleanup-projection gap; they do not relax the target exact-artifact convergence and schedule-reconciliation gates for Automation admission.
...
- The counted set is mode-dependent: `NORMAL` includes only rows whose `workItem.admissionEpoch` equals the current `admissionEpoch`; `PAUSED_FOR_ROLLBACK` includes legacy rows with `workItem.admissionEpoch <= 0` and rows from an earlier epoch (`workItem.admissionEpoch < current admissionEpoch`). Active and pending counts retain their existing status classifications after this epoch filter.
...
- Read-only.
- Reports the latest pin observation used by admission and scheduler logic. Instance-scoped exact-tuple admission/replay requires a positive `scriptPinEpoch`, a nonblank `scriptPinControlPlaneRequestId`, and an exact match of `(scriptPatchVersion, scriptPinEpoch, scriptPinControlPlaneRequestId)` against the returned `(observedPinnedScriptPatchVersion, observedScriptPinEpoch, lastObservedControlPlaneRequestId)` observation; missing, stale, or mismatched evidence fails closed. For promotion or rollback convergence, the observation is an acknowledgment only when the expected `(scriptPatchVersion, scriptPinEpoch, convergenceAttemptGeneration, controlPlaneRequestId)` tuple is present, `isProjectionStale=false`, and `projectionLagMs` is inside the configured freshness bound; a missing attempt generation separately prevents convergence proof. The freshness bound is the configured `SCRIPT_PIN_PROJECTION_STALE_THRESHOLD_MS` value from [Automation & Scripting Service Configuration](./microservices/automation-scripting-service/configuration.md).
...
- If an authoritative refresh returns a partial or otherwise incoherent owner tuple and no stored projection is available, the live endpoint returns application error `INVALID_RUNTIME_PIN_TUPLE`. Clients keep exact-tuple admission and convergence fail-closed rather than treating the response as `UNPINNED`; operators repair the Game Session owner record or read path and then retry.
...
- Idempotent.
- Under the shared queue mutation/tick lease, terminal-marks matching durable Game Session command rows before post-commit Redis queue/pending cleanup. `executionOutcome = LOST_BEFORE_STAGING` applies only to commands acknowledged `ACCEPTED_VOLATILE` whose ingress status is still `RECEIVED` or `ENQUEUED` and that have no surviving batch; `ACCEPTED_DURABLE` pre-batch work cannot use this outcome and remains under its durable retry/recovery contract. An explicitly retryable command with a durable prior tick-effect binding uses `executionOutcome = ABANDONED`. Both use `gameplayResult = NOT_APPLIED`, `failureCode = ROLLBACK_PURGED`, and the validated nonblank ingress `reason` as `failureMessage`.
...
The initiating mutation follows the shared control-plane idempotency, authorization, and audit contract without prescribing a concrete API name: its `controlPlaneRequestId` is durably bound to a canonical digest of the operation kind, exact revoked `componentId`, requested security-policy scope, authenticated `actor`, and nonblank `reason`. The stored result binds the accepted component-revocation fence and exact affected-scope discovery/progress evidence. An exact retry returns that deterministic result or resumes the same bounded containment workflow; reuse with a changed digest conflicts before mutation. Durable audit retains the component, requested and discovered scope evidence, actor, reason, and terminal or still-contained outcome.

Before discovery begins, the accepted revocation fence is installed as an admission fence: it blocks new triggers and `PENDING_EVALUATION` creation for every scope not yet proven unaffected. The fence remains until authoritative discovery completes and the normal pause or safe-target rollback/disable convergence completes.
...
2. Discover affected published patches, active pins, and current Automation scopes from authoritative dependency and runtime indexes; do not rely on tenants to self-identify exposure. Stale, contradictory, or unavailable required indexes make the discovery proof incomplete and fail closed: retain the emergency component fence, treat every Automation scope that the incomplete evidence cannot prove unaffected as potentially affected and keep it contained or paused, do not resume affected Automation, and retry the same audited workflow until the authoritative indexes converge on one coherent discovery result. A scope leaves containment only after coherent authoritative evidence proves it unaffected or the safe-target workflow below converges.
...
4. Where an exact safe target exists, prepare it and drive the normal explicit disable or fenced rollback, including pin/epoch convergence and displaced-work cancellation and purge. Staged or recovered work remains subject to the component-revocation fence at every applicable persistence, handoff, staged/final-effect, retry, replay, and recovery boundary, including the immediate pre-gameplay-effects check; those gates are owned by [ADR 0116](./decisions/adr-0116-routine-component-migration-and-explicit-emergency-revocation.md) and the [cross-service version-fencing contract](./system-architecture-scripting-contracts.md#3-version-fencing-rollback-safety), not redefined by this orchestration workflow.
