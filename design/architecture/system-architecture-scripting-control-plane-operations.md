...
Ineligible `DEAD_LETTERED` rows remain `DEAD_LETTERED` and produce no recovery claim/attempt record. A proven `script_pin_epoch_mismatch` or `plugin_activation_generation_mismatch` remains permanently ineligible under every later request; exact-request retries return the stored rejection, and a new request cannot reinterpret the old epoch or generation as eligible. A rejection caused only by temporarily unavailable authoritative evidence may be retried by a later request after that evidence becomes available. Non-`DEAD_LETTERED` rows without immutable prior successful-recovery evidence remain unchanged and return `work_item_not_dead_lettered`; missing or contradictory stage evidence remains dead-lettered.
