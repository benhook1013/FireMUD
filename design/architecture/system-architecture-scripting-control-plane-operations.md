...
- Before recovery claim acquisition for plugin-backed work, resolve the current binding and `pluginActivationEpoch` for that same tenant, instance, and plugin, then compare the stored immutable `(pluginId, pluginVersionId, bindingId)` tuple and captured activation epoch to that current evidence.
...
- Plugin work items whose immutable `(pluginId, pluginVersionId, bindingId)` tuple or captured `pluginActivationEpoch` does not match the currently active binding/epoch for the same scoped `<tenantId, gameInstanceId, pluginId>` must be rejected from replay. An epoch mismatch is permanently ineligible for that work item because the Automation-owned epoch is monotonic.
...
- [Scripting & Automation: Control Plane Events](./system-architecture-scripting-control-plane-events.md) defines advisory notification families and the separately named durable event flows emitted by the workflow APIs in this document.
