...
- Before recovery claim acquisition for plugin-backed work, resolve the current binding and `pluginActivationGeneration` for that same tenant, instance, and plugin, then compare the stored immutable `(pluginId, pluginVersionId, bindingId)` tuple and captured generation to that current evidence.
