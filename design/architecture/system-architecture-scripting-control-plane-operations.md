...
All plugin lifecycle calls use the Automation-owned single pending-transition slot for `(tenantId, gameInstanceId, pluginId)`. The captured current tuple includes `(pluginVersionId, pluginActivationEpoch, pluginState, lifecycleRevision)`. Initiation CASes that tuple only when no transition is pending; every state-changing transition reserves `targetLifecycleRevision = current lifecycleRevision + 1`, and an epoch-advancing transition also reserves `targetPluginActivationEpoch = current + 1`. An exact request resumes its unchanged reserved tuple, while a different activation, switch, drain, disable, revocation, reactivation, or policy mutation fails closed as `transition_in_progress`. Completion CAS requires the captured tuple, pending request identity/digest, target epoch, and target lifecycle revision to remain unchanged, and commits the owner state only after the corresponding Game Session install is durably acknowledged, so stale workers cannot overwrite newer state. Security, component, and signer-policy fences remain independent immediate fail-closed checks.

For same-epoch `DRAINING`, recovery may use only the immediately preceding `ENABLED` lifecycle revision, and only when that work's winning admission/fence compare-and-set committed before the durable `DRAINING` admission barrier was created. A later or arbitrary lower revision is not admitted by the predecessor exception.
