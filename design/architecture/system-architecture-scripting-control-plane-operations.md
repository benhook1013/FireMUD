...
Ineligible rows remain `DEAD_LETTERED` and produce no recovery claim/attempt record. Their rejected result is not a success claim, so a later request may retry after authoritative evidence or the exact pin/epoch becomes eligible. Missing or contradictory stage evidence remains dead-lettered.
