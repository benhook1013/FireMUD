...
7. **Validation and Errors** – World Management validates generation requests, validates generator outputs, and guarantees **no partial persistence** for the affected template or instance scope.
   ...
   - World Management must enforce a uniqueness constraint on `(typedTarget, targetScopeKey, generationRequestId)`, where `typedTarget` contains exactly one complete design or runtime target identity and an `S1`/`S2` `targetScopeKey` contains tenant, namespace, and owning-domain identity but not `playableStateScope`; scope remains a separately validated binding predicate, never a uniqueness dimension. Duplicate requests therefore converge to one run without crossing namespaces.
   - World Management must enforce single-writer semantics per typed target and target scope through the scope epoch/fence or equivalent storage-level compare-and-set, together with request uniqueness, so concurrent runs, retries, and recovery cannot cross targets or both commit. For `S1`/`S2` output, that serialization is keyed by the durable output identity `(tenantId, playableStateNamespaceId, owningDomainIdentity)`; `gameInstanceId` remains authorization evidence only.
