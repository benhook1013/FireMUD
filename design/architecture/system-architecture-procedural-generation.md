...
- **Runtime/instance generation** – invoked from the Temporal world-lifecycle workflow or tick-driven commands for a concrete `(tenantId, gameInstanceId)` target. The request target is instance-scoped, but persistence follows the owner's replacement classification: `S1` and `S2` durable output identity is `(tenantId, playableStateNamespaceId, owning-domain identity)`, where the owning-domain identity is defined by the domain that owns the generated family. The immutable server-derived `playableStateScope` is persisted and exact-validated separately as binding, routing, policy, and authorization evidence; it is never a durable key or uniqueness dimension. `S3` disposable output is keyed by `(tenantId, gameInstanceId)` only. These flows never modify template tables for Published versions.

Runtime generation is a concrete instance target, not a durable playable-state identity. Any generated family that survives replacement must use the stable `playableStateNamespaceId` plus active-instance authorization under [ADR 0122](./decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md). `S1` output and retained references survive unchanged only when their owning identity contract explicitly makes them namespace-stable. An instance-keyed reference is not `S1`: when it must survive through an explicit owner-validated mapping it is `S2`, and when its owner explicitly declares it disposable it is `S3`. For `S1`/`S2`, the durable identity remains `(tenantId, playableStateNamespaceId, owning-domain identity)`; `playableStateScope` remains separately persisted and exact-validated evidence, not part of that identity or its uniqueness. `S2` mapping contents and successful owner application are required before cutover; `S3` remains instance-scoped and may be cleaned up with the old instance and regenerated for the new one. Unknown, unowned, or unclassified generated families block replacement rather than defaulting to `S3`, and a generation request or echoed mapping identifier does not authorize a namespace transition or prove mapping application.
...
Callers do not supply a free `generationMode` as an authority selector. World Management derives the mode from the authenticated ingress and target, rejects cross-namespace combinations, validates generator output, and persists it. Design ingress may write only Draft template rows; Published rows are immutable. Runtime ingress may write generated durable output only according to the owner's classification: `S1`/`S2` output uses `(tenantId, playableStateNamespaceId, owning-domain identity)` with active `gameInstanceId` authorization, while the separately persisted and exact-validated `playableStateScope` is not a durable key or uniqueness dimension. Declared `S3` output uses instance rows keyed by `(tenantId, gameInstanceId)` only. Instance-scoped request and generation-provenance records may retain the concrete `gameInstanceId` without making the generated durable output instance-owned. See [ADR 0100](decisions/adr-0100-separate-generation-ingress-with-one-world-owned-engine.md).

Runtime generation reads the authoritative `(tenantId, gameInstanceId)` lifecycle row, state, and epoch from World Management. For already-admitted dynamic generation, it resolves the catalog selectors `worldSlug`/`realmSlug` to the durable `realmId` and consumes Game Session's canonical `GetAdmissionPointer(tenantId, realmId)` plus the exact referenced realm-catalog snapshot; the pointer response carries those catalog selectors and the snapshot supplies `playableStateNamespaceId` and resolves `stateScope` to `playableStateScope` under the [Multi-Tenancy realm catalog and admission-pointer contract](./system-architecture-multi-tenancy.md#realm-catalog-and-admission-pointer-contract). Pre-admission `PREPARING` world-lifecycle work binds the same server-resolved catalog snapshot but is authorized by the exact World lifecycle proof, not by inventing an `OPEN` pointer. Before output binding or finalization, World Management exact-compares tenant, instance, namespace, scope, lifecycle state/epoch and, when admitted, the `OPEN` target plus exact `pointerVersion` and `catalogRevision`; missing, stale, ambiguous, mismatched, replacement, or replay evidence fails closed. Focused proof must cover namespace, scope, lifecycle epoch, stale pointer/catalog revision, replacement, and replay mismatches. Select and report that proof using the shared [Validation and Runtime Proof workflow](../developer-workflows/validation-and-runtime-proof.md), so target-only generation contracts are not presented as implemented runtime evidence.
...
- When invoked from the world-lifecycle workflow or tick-driven commands for **runtime instances**, `S1`/`S2` results are finalized as normalized durable rows or immutable topology artifacts keyed by `(tenantId, playableStateNamespaceId, owning-domain identity)` and authorized for the active `gameInstanceId`; their separately persisted `playableStateScope` is exact-validated as binding, routing, policy, and authorization evidence, never as a durable key or uniqueness dimension. Declared `S3` results are finalized as instance rows or artifacts keyed by `(tenantId, gameInstanceId)` and refer back to the chosen `versionId`. Instance-scoped request/provenance records may use `gameInstanceId` for retry and audit evidence, but do not change the output classification; template rows remain unchanged.
...
- `generationRequestId` is the stable business identity for one admitted request and its output-deduplication/replay boundary; retries reuse it. `generationRunId` is the concrete generation execution/provenance identity scoped to the complete typed target (including `gameInstanceId` for a runtime target), and private staging is keyed by that run. A retry maps the same request and durable target binding to the existing run/output; an intentional regeneration receives a new request identity and run.
...
- for a runtime target, an immutable output binding containing `{tenantId, playableStateNamespaceId, owningDomainIdentity, playableStateScope, outputClassification, gameInstanceId, lifecycleState, lifecycleEpoch}` for `S1`/`S2` (with `owningDomainIdentity` required and part of the typed binding) and the corresponding instance binding for `S3`; when the runtime has already been admitted, it also carries the complete immutable admission-pointer proof `{tenantId, realmId, worldSlug, realmSlug, state=OPEN(gameInstanceId), pointerVersion, catalogRevision}`. `worldSlug`/`realmSlug` are catalog selectors resolved to `realmId` before the pointer lookup, while the pointer response carries them from the referenced catalog snapshot. For `S1`/`S2`, durable output identity is `{tenantId, playableStateNamespaceId, owningDomainIdentity}`, while `playableStateScope` is separately persisted and exact-validated binding evidence rather than a key or uniqueness dimension. `gameInstanceId` is only the concrete runtime identity, while the exact World lifecycle proof authorizes activation-time work in `PREPARING` and the complete pointer proof additionally authorizes already-admitted dynamic work
...
After canonical output is generated or private staging completes, exactly one compare-and-set transition requires the exact `PENDING` state and expected `outputStateRevision`, then binds `RECORDED` or `STAGED`, the output identity, canonical topology `outputDigest`, exact output row count, and canonical serialized-byte count while incrementing `outputStateRevision` exactly once. Every successful output-state transition increments this durable revision exactly once. The expected prior state/revision and resulting bound identity participate in the retry and finalize guard. Retry, replay, and recovery carry and compare that same persisted `outputStateRevision`. Runtime finalize, replay, and recovery compare the same applicable persisted fields exactly: tenant, namespace, `owningDomainIdentity` for `S1`/`S2`, separately persisted scope, classification, concrete `gameInstanceId`, `lifecycleState`, `lifecycleEpoch`, and, when admission already occurred, the complete `{tenantId, realmId, worldSlug, realmSlug, state=OPEN(gameInstanceId), pointerVersion, catalogRevision}` pointer proof, where the slugs are catalog selectors resolved to `realmId` before lookup. Any mismatch fails closed. An exact replay may return the already-bound identity, but a stale or conflicting concurrent/replayed transition fails closed. Seed, generator type, and configuration remain separate provenance and cannot substitute for that binding. S1/S2 output remains keyed by tenant, namespace, and `owningDomainIdentity`; `playableStateScope` is not a durable key or uniqueness dimension. Only declared S3 output is instance-owned.
...
If a future generated location family is classified as `S1` or `S2`, its durable location identity must instead be keyed by `(tenantId, playableStateNamespaceId, owning-domain identity)`, and readers must perform an explicit namespace-to-active-instance resolution before obtaining an instance-scoped `RoomInstanceRef`. The separately persisted `playableStateScope` must be exact-validated as binding, routing, policy, and authorization evidence, but cannot serve as a durable key or uniqueness dimension. An instance-keyed `roomInstanceId` cannot serve as that durable identity. This future rule does not reclassify the current World runtime families or add a mapping API; it preserves their existing `S3` instance scope.
...
3. **Seed & Metadata Persistence** – Every admitted generation keeps its authoritative typed request/output binding separate from provenance containing the seed, `generatorType`, and canonical configuration snapshot. A design-time request binds the complete version-scoped target. A runtime request binds `{tenantId, playableStateNamespaceId, owningDomainIdentity, playableStateScope, outputClassification, gameInstanceId, lifecycleState, lifecycleEpoch}` and, after admission, the complete immutable pointer proof `{tenantId, realmId, worldSlug, realmSlug, state=OPEN(gameInstanceId), pointerVersion, catalogRevision}`; `worldSlug`/`realmSlug` are catalog selectors resolved to `realmId` before pointer lookup. For `S1`/`S2`, durable output identity and serialization/CAS scope are `{tenantId, playableStateNamespaceId, owningDomainIdentity}`; `playableStateScope` is persisted and exact-validated separately and is not a durable key or uniqueness dimension. `gameInstanceId` is only concrete runtime identity and authorization evidence, never the durable identity or serialization key for shared output. Activation-time work in `PREPARING` carries the exact World lifecycle proof without a pointer, while already-admitted dynamic work also carries the complete pointer proof that authorizes it. Its provenance may retain the concrete `gameInstanceId` for retry and audit. Metadata required to interpret S1/S2 output is namespace-owned or durably linked to output keyed by the complete `{tenantId, playableStateNamespaceId, owningDomainIdentity}` tuple, while only declared S3 output uses `gameInstanceId` as durable output identity. The provenance record is not an alternate authority for generated topology.
...
7. **Validation and Errors** – World Management validates generation requests, validates generator outputs, and guarantees **no partial persistence** for the affected template or instance scope.
   ...
   - `generationRequestId` must be derived from business identity rather than transient execution identity (for example a hash of `tenantId`, target scope key, generation step name, canonicalized generator config, and the immutable admitted-generation revision or approval identity, or explicit regeneration sequence). For `S1`/`S2`, the target scope key uses `(tenantId, playableStateNamespaceId, owning-domain identity)` and excludes `playableStateScope`, which is separately persisted and exact-validated evidence. Retries through a new Temporal run or synchronous retry must reuse the same `generationRequestId`; an intentional regeneration with the same inputs must use a distinct sequence and request ID.
   - World Management must enforce request uniqueness by classification: `S1`/`S2` output uses exactly `(tenantId, playableStateNamespaceId, owningDomainIdentity, generationRequestId)`, declared `S3` output uses `(complete instance-typed target, generationRequestId)`, and design-time output uses `(complete version-typed target, generationRequestId)`. The `S1`/`S2` branch excludes both `gameInstanceId` and `playableStateScope`; they remain active-instance authorization and separately validated binding evidence, never durable identity or uniqueness dimensions. These branches must remain distinct so duplicate requests converge to one run without crossing namespaces or aliasing design/instance output.
   - World Management must enforce single-writer semantics per typed target and target scope through the scope epoch/fence or equivalent storage-level compare-and-set, together with the classification-specific request uniqueness above, so concurrent runs, retries, and recovery cannot cross targets or both commit. For `S1`/`S2` output, that serialization is keyed by the durable output identity `(tenantId, playableStateNamespaceId, owningDomainIdentity)`; `gameInstanceId` remains authorization evidence only.
   ...
   - Output above those limits, or output requiring chunked persistence, uses private staging keyed by `(tenantId, generationRunId)`. A short finalize transaction performs an exact compare-and-set of the bound `STAGED` state and expected `outputStateRevision` to `RECORDED`, increments that revision once, and atomically installs or selects the graph or immutable root chunk manifest as authoritative; an exact retry returns the recorded result, while a conflicting or stale revision fails closed. `STAGED` therefore remains private and never becomes reader-visible authority.
