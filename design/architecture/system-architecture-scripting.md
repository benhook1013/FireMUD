...
3. **`onLoad` initialization (Automation & Scripting Service, per tenant patch)**
   ...
   - **Current implementation gap:** the live readiness path still needs complete manifest-completion proof and monotonic publication-sequence fencing; these target rules do not authorize durable `onLoad` effects.
