...
The live Automation runtime provides durable work-item execution, per-script quota and aggregate tenant-tier accounting, best-effort queue coordination, and the current structured command-template evaluator with narrow output, target, and DSL-shape limits. Current queue reset/rebuild/resume is unavailable and must fail closed because the implementation lacks stale/owner/CAS reconciliation. The scheduled queue projection still performs an unsafe refresh while service readiness is accepting traffic, but it is not a safe reset or recovery operation; see [Scripting Runtime Execution: Current Queue-Rebuild Concurrency Gap](./system-architecture-scripting-runtime-execution.md#current-queue-rebuild-concurrency-gap). Atomic owner-reconciled recovery of safe pre-DSL `PENDING_EVALUATION` work is target-only. Broader sandbox resource guards and output metering, failure-rate breaker authority, tenant-first scheduling, the visual graph DSL, compiled-artifact execution, advanced NPC behavior modules, procedural population, and richer runtime seams described below are target-state architecture, not evidence of live end-to-end implementation. In particular, the current structured evaluator does not prove per-run CPU, wall-clock, or iteration guards; references to those guards below describe the target sandbox contract unless they are explicitly identified as current evaluator checks. Readiness proof is also partial: complete manifest-completion and monotonic publication-sequence fencing remain unproved, and stale `ONLOAD_RUNNING` executor work is not yet reclaimed or terminalized; the target recovery-owner path remains defined by the DSL lifecycle owner.

- **Implemented and in active use**
  - The current Automation & Scripting runtime, including per-script quota and aggregate tenant-tier counters, durable work-item execution, structured command-template evaluation with narrow output, target, and DSL-shape checks, and best-effort queue-pointer projection; current queue reset/rebuild/resume is unavailable and fail-closed, while atomic owner-reconciled pre-DSL `PENDING_EVALUATION` recovery remains target-only.
  - Hot reloading of scripts published by the Game Design Service for non-empty `affectedScripts` notifications, with version-aware script execution aligned with the versioning model in [Versioning & Runtime Configuration](./system-architecture-versioning-runtime.md#script-only-patch-versions). The current normal Game Design publication sends an empty list and therefore does not create readiness state or reload scripts; the target zero-handler path requires immutable manifest proof as described below.
...
| Automation queues & durable execution | Implemented / evolving | Instance-aware automation queue indexes (for example `automation:queue:{tenantInstanceTag}:<entityId>`), canonical `automation:quota:<tenantId>:<scriptId>` counters, and the durable work-item executor are implemented. The target owner-reconciled queue rebuild remains a gap: current rebuild unconditionally includes `EVALUATING` rows and lacks an atomic status/owner gate, so reset or resume is unavailable and must fail closed regardless of the observed status mix. Remaining `10.4` work also includes richer runtime output and any later queue-consumer evolution, not restoring a separate staging layer. |
...
3. **`onLoad` initialization (Automation & Scripting Service, per tenant patch)**
   ...
   - For an eligible non-empty notification, the readiness wait is durably hosted in the Temporal `script-patch-readiness` workflow family, which polls the canonical readiness projection until the patch reaches a terminal state and exposes workflow identity/status to operator control-plane reads. The current normal empty-`affectedScripts` path returns before creating the readiness row or starting that workflow; it is not a successful readiness transition. An empty handler set may bypass handler work and the synthetic workflow only when the immutable patch manifest proves zero handlers and a terminal `READY` pin-readiness record is created and exposed. If that proof is absent or cannot be verified for an identifiable tenant/patch notification, the readiness owner must instead create a durable `FAILED` record with bounded reason `zero_handler_manifest_unverifiable`, publish status/audit evidence, and prevent `READY`; it must not silently reject, retry the DSL, or replace the readiness identity. An unidentifiable malformed notification may fail transport validation without inventing a patch identity; the mandatory `onLoad` gate remains in force.
...
4. **Version pinning (Game Session Service)**
   ...
   - **Current implementation gap:** the live Automation-to-Game Session handoff carries and validates the exact `scriptPatchVersion`/`scriptPinEpoch` pair (and optional owner request identity), so same-version old-epoch work is rejected at that admission boundary. Complete command identity, source/target remote binding, and final-effect fence proof remain unimplemented/proof-incomplete.
...
3. **Durable parent work persistence and queue staging** – After binding and the applicable handler admission, Automation persists each admitted parent work item in the durable outbox with `PENDING_EVALUATION` and stages a pre-DSL queue pointer identified by `outboxWorkItemId`; current drain batches deduplicate repeated pointers, while target publication/rebuild must provide atomic uniqueness for that identity. Current queue semantics and the target post-DSL evaluated-descriptor/child boundary are owned by [Scripting Runtime Execution](./system-architecture-scripting-runtime-execution.md#current-implementation-status); the pointer is a best-effort derived coordination index, while current reset/rebuild is unavailable and must fail closed because the implementation lacks stale/owner/CAS reconciliation. Target owner-reconciled atomic pre-DSL `PENDING_EVALUATION` recovery remains unimplemented. Current `EVALUATING` pointers are inspection-only and cannot be picked up, reclaimed, or re-enter the DSL; owner-reconciled `EVALUATING` recovery is target-only. Loss/reset must still be observable. Region-scoped tick keys remain the responsibility of the Game Session Service.
