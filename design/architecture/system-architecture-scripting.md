...
3. **`onLoad` initialization (Automation & Scripting Service, per tenant patch)**
   ...
   - For an eligible non-empty notification, the readiness wait is durably hosted in the Temporal `script-patch-readiness` workflow family, which polls the canonical readiness projection until the patch reaches a terminal state and exposes workflow identity/status to operator control-plane reads. The current normal empty-`affectedScripts` path returns before creating the readiness row or starting that workflow; an empty handler set is valid only when the immutable patch manifest proves that zero handlers are expected.
