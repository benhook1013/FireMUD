...
3. **`onLoad` initialization (Automation & Scripting Service, per tenant patch)**
   ...
   - `ONLOAD_RUNNING` recovery is durable. After a worker or process crash, the readiness workflow resumes the unfinished persisted step for the same patch and step identity, or records terminal exhaustion if its retry budget/deadline has elapsed. It must not mint a new `scriptEventId` or step identity, and it must never retry the DSL merely because the process was restarted.
...
6. **Game tick execution** – The Game Session Service selects at most one root actor action per eligible entity from the actor-action lane; separately bounded passive, inbound, and actor-generated effect lanes may also process work without consuming that actor-action slot. All lanes apply the canonical lock, fairness, ordering, and replay rules.
