...
| Automation queues & durable execution | Implemented / evolving | Instance-aware automation queue indexes (for example `automation:queue:{tenantInstanceTag}:<entityId>`), canonical `automation:quota:<tenantId>:<scriptId>` counters, and the durable work-item executor are implemented. The target owner-reconciled queue rebuild remains a gap: the callable current rebuild unconditionally includes `EVALUATING` rows and lacks an atomic status/owner gate, so reset or resume is unavailable and must fail closed regardless of the observed status mix. No live scheduled rebuild invokes it. Remaining `10.4` work also includes richer runtime output and any later queue-consumer evolution, not restoring a separate staging layer. |
