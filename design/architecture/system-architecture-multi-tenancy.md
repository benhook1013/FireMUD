...
- **Game instance (`gameInstanceId`)** – A specific running instance of a tenant’s world, keyed as described in [Versioning & Runtime Configuration](./system-architecture-versioning-runtime.md#version-activation--rollback). In the initial deployment the platform runs at most one game instance per tenant, so some Redis keys and APIs use `tenantId` where `gameInstanceId` will be threaded in later; designs and key formats must leave room for this separation.
