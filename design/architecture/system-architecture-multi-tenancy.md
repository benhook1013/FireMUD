...
The realm-catalog, admission-pointer, realm-local character, and explicit first-join contracts below are normative target behavior. Current runtime proof is partial: `IssueConnectToken` and text `PLAY` still use `EnsurePublicProductionPlayerMembership` to create or return implicit public-production membership when it is absent, while the explicit first-join action and `membershipAuthorityGeneration` live reread at connect-token issuance are unimplemented/proof gaps. Pointer updates also still use a read-then-write check with separate pointer/audit/prepared-execution writes rather than an atomic pointer CAS and atomic persistence proof. The first-party public-production admission enablement gate therefore remains closed for claiming the target until explicit `JOIN`, the complete generation reread, the atomic pointer CAS, and atomic persistence of the pointer, append-only audit event, and any required prepared-execution state are implemented and proven. Private/playtest admission remains distinct: returning and grant-backed callers remain eligible when the current caller-bound membership and Account-owned grant checks pass; these callers never use `JOIN` or create membership. The generation and pointer proof gaps prevent claiming complete target proof, but do not make all private/playtest admission unavailable. These are implementation gaps only; they do not weaken tenant isolation, the distinct `tenantSlug` and tenant-scoped authored-world `worldSlug` selectors, or the required `GetAdmissionPointer(tenantId, worldSlug, realmSlug)` contract.
...
- Public-production `JOIN`/`Join & Play` is the only gameplay-adjacent operation that may create a durable `player` membership, and it may do so only when membership is missing. Existing members use their existing membership; character creation, connect-token issuance, and `PLAY` never create it implicitly.
- Private/playtest admission requires both an existing caller-bound membership and the current Account-owned access grant. The grant is checked independently, never substitutes for membership, and never auto-creates membership.
