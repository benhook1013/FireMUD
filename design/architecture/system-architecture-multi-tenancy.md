...
- **Game instance (`gameInstanceId`) / realm** – A specific running instance of a tenant’s world, keyed as described in [Versioning & Runtime Configuration](./system-architecture-versioning-runtime.md#version-activation--rollback). FireMUD treats player-addressable game instances as **realms** from a user-experience perspective. Persistence models, APIs, and key formats must include `gameInstanceId` explicitly rather than overloading `tenantId`.
  - Each tenant may expose multiple player-addressable realms.
  - One realm may be designated the tenant’s **default production realm** for ordinary/public admission.
  - Additional realms may be designated as **playtest forks** or other non-production realms with explicit access controls and visibility rules.
  - Realm selection is part of the canonical lobby/admission flow; clients must never be forced to infer or guess a target `gameInstanceId`.
...
- Gameplay clients select a realm using human-friendly world/realm selectors returned by the lobby flow. Public players may be routed to the tenant's default production realm, while authorized testers may also be shown additional playtest forks for the same tenant.
...
- Player-facing realms are a product-level concept, not just an operational detail. A tenant may run a default production realm plus additional explicitly addressed realms such as creator-managed forked playtest environments derived from snapshots of live state.
