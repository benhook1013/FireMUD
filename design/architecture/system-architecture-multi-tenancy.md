...
- Public-production `Join & Play` (text `JOIN`) is the only gameplay-adjacent operation that may change a durable `player` membership. Its exact lifecycle is missing -> `ACTIVE` and `INACTIVE` -> `ACTIVE`, each with a `membershipVersion` advance; `membershipAuthorityGeneration` advances independently only when `callerBoundAuthorityInvalidated=true`; `ACTIVE` returns the exact idempotent current snapshot and all other states reject. `JOIN_REQUIRED` covers missing or `INACTIVE` membership. Character creation, connect-token issuance, and `PLAY` never create or restore membership implicitly.
