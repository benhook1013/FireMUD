...
Scripting pins and rollout history are tenant/game-instance scoped: Game Session owns the exact `(scriptPatchVersion, scriptPinEpoch)` for each instance, and Automation's observed pin projection is keyed by `(tenantId, gameInstanceId)` and carries that exact observed version/epoch tuple. Tenant-scoped readiness only says a patch is eligible for an explicit instance pin; it never authorizes instance work. Only the authoritative exact tuple for the target instance can admit instance-bound work; a stale, local, missing, or mismatched pin cannot admit any such work, including for the same tenant and game instance. The exact identity and epoch rules belong to [Scripting Contracts](./system-architecture-scripting-contracts.md); this document owns the tenancy and routing consequence.
