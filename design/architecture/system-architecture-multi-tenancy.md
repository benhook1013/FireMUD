...
- **Game instance (`gameInstanceId`)** – A specific running instance of a tenant’s world, keyed as described in [Versioning & Runtime Configuration](./system-architecture-versioning-runtime.md#version-activation--rollback). Persistence models, APIs, and key formats must include `gameInstanceId` explicitly rather than overloading `tenantId`.
  ...
  - One visible realm may be flagged as the public-production realm in the separately revisioned Game Session catalog/policy record. In v1, that flagged realm is the only realm that may be publicly discoverable to authenticated accounts that do not already hold tenant membership. The admission pointer contains only `OPEN(gameInstanceId)` or `CLOSED` routing authority plus its route version; callers consume the catalog/policy flag rather than inferring public-production behavior from a slug.
