...
- **Session** – A transient **gameplay session** binding between a connected client and a character in a tenant, managed by the Game Session Service and stored in Redis. These gameplay session keys are always tenant-scoped and never change the global `accountId`. In the long-term design, sessions bind sockets to **character identities** within a tenant; the current implementation temporarily treats `playerId` as `accountId`, but Redis key formats and APIs are defined so they continue to work once character selection is introduced (see [Authentication & Authorization](./system-architecture-authentication.md#login-and-session-flow) and [Session and Identity Management](./system-architecture-authentication.md#session-and-identity-management)). Auth token sessions and control-plane UI sessions are described in the Authentication design and are distinct from gameplay sessions.
