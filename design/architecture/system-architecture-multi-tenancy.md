...
The realm-catalog, admission-pointer, realm-local character, explicit first-join, and quota-boundary contracts below are normative target behavior. Current runtime proof is partial: `IssueConnectToken` and text `PLAY` now require existing membership, never call `EnsurePublicProductionPlayerMembership`, and return `JOIN_REQUIRED` for eligible missing public-production membership; the legacy RPC/client/service seam remains present but has no current admission caller; the membership-generation live reread at connect-token issuance is not implemented; pointer updates still use a read-then-write check with separate pointer/audit/prepared-execution writes; scoped-role population and tenant switching are not fully implemented/proved; authoritative entitlement freshness plus complete owning-service quota enforcement remain incomplete; and broader authority-generation claims, durable projections, and enforcement remain incomplete across issuer, account, tenant, and membership scopes. Explicit `JOIN`/`Join & Play` remains unimplemented, so the first-join flow remains target-state only. The remaining gaps do not change the distinct `tenantSlug` and tenant-scoped authored-world `worldSlug` selectors, the exact-tenant quota binding, or the required `GetAdmissionPointer(tenantId, worldSlug, realmSlug)` contract.
...
Account Service owns the issuer, account, tenant, and caller-bound membership authority generations and their projections. Tenant generation applies by default. `platformAdmin` authorization in `tenant_regular` and `cross_tenant_data_bearing` requires an independently TOTP-backed `privileged_control` window and binds the exact current target-tenant generation as a freshness fence without acquiring membership. Only the explicit route-class allowlist is exempt from the target tenant generation: `billing_safe_tenant` still requires issuer/account and caller-bound membership generations plus a live `tenantAdmin` check; `cross_tenant_support_safe` has separate role branches, with a live global `support` branch that requires current issuer/account generations and global token scope but no elevation, and an explicitly allowed live global `platformAdmin` branch that requires those checks plus `privileged_control`; and `cross_tenant_billing_safe` still requires issuer/account generations, a live global `billingAdmin` or explicitly allowed `platformAdmin` role, global token scope, and `privileged_control`. Support does not require that elevation; the `platformAdmin` branch of `cross_tenant_support_safe` does. No route may use cached authorization, and a new route class does not inherit this allowlist.
