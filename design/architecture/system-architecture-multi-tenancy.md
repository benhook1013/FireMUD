...
The realm-catalog, admission-pointer, realm-local character, and explicit first-join contracts below are normative target behavior. Current runtime proof is partial: `IssueConnectToken` and text `PLAY` now reject absent public-production membership with `JOIN_REQUIRED`, but the explicit first-join action and `membershipAuthorityGeneration` live reread at connect-token issuance are both unimplemented/proof gaps, and pointer updates still use a read-then-write check with separate pointer/audit/prepared-execution writes. Affected first-party admission remains fail-closed and unavailable until both the explicit join action and generation reread are implemented and proven. These are implementation gaps only; they do not weaken tenant isolation, the distinct `tenantSlug` and tenant-scoped authored-world `worldSlug` selectors, or the required `GetAdmissionPointer(tenantId, worldSlug, realmSlug)` contract.
