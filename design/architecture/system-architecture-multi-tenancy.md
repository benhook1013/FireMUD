...
- **Gameplay session** – A transient session binding between a connected client and a character in a tenant, managed by the Game Session Service and stored in Redis using keys such as `session:game:<tenantId>:<sessionId>`. These gameplay session keys are always tenant-scoped (and, over time, instance-scoped) and never change the global `accountId`. In the long-term design, sessions bind sockets to **character identities** within a tenant; the current implementation temporarily treats `playerId` as `accountId`, but Redis key formats and APIs are defined so they continue to work once character selection is introduced (see [Authentication & Authorization](./system-architecture-authentication.md#login-and-session-flow) and [Session and Identity Management](./system-architecture-authentication.md#session-and-identity-management)).
