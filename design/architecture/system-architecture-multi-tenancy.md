...
- **Gameplay session** – A transient session binding between a connected client and a character in a tenant, managed by the Game Session Service and stored in Redis using keys such as `session:game:<tenantId>:<gameInstanceId>:<sessionId>`. These gameplay session keys are always tenant- and instance-scoped and never change the global `accountId`. Sessions bind sockets to character identities within a tenant, and any temporary `playerId` usage must map one-to-one to `characterId` while migration completes (see [Authentication & Authorization](./system-architecture-authentication.md#login-and-session-flow) and [Session and Identity Management](./system-architecture-authentication.md#session-and-identity-management)).
  - During the interim pre-character-selection phase, uniqueness for takeover/resume is enforced as `{tenantId, gameInstanceId, accountId}`. After character selection is fully enabled, canonical uniqueness is `{tenantId, gameInstanceId, characterId}`.
  - Active uniqueness semantics are controlled by a single platform `identity_mode` contract and must not be inferred independently by services.
  - `identity_mode` source of truth is the Account Service control-plane API/read model; other services consume but do not write this mode.
  - Cutover from interim to canonical uniqueness must follow the [Character-Selection Cutover Contract](./system-architecture-authentication.md#character-selection-cutover-contract-required).
