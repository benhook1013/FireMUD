...
The realm-catalog, admission-pointer, realm-local character, and explicit first-join contracts below are normative target behavior. Public-production membership follows one lifecycle: missing -> explicit `JOIN` creates `ACTIVE`; `INACTIVE` -> `JOIN` restores `ACTIVE` and advances `membershipAuthorityGeneration` and `membershipVersion`; `ACTIVE` -> `JOIN` returns the exact idempotent current snapshot; all other states reject. `JOIN_REQUIRED` covers missing or `INACTIVE` public-production membership, and character creation, `IssueConnectToken`, and text `PLAY` never create or restore membership. Current runtime proof is partial: `IssueConnectToken` and text `PLAY` still use `EnsurePublicProductionPlayerMembership` to create or return implicit public-production membership when it is absent, while the explicit first-join action and `membershipAuthorityGeneration` live reread at connect-token issuance are unimplemented/proof gaps; that behavior is implementation drift against the target contract. Pointer updates also still use a read-then-write check with separate pointer/audit/prepared-execution writes rather than an atomic pointer CAS and atomic persistence proof. The first-party public-production admission enablement gate therefore remains closed for claiming the target until explicit `JOIN`, the complete generation reread, the atomic pointer CAS, and atomic persistence of the pointer, append-only audit event, and any required prepared-execution state are implemented and proven. Private/playtest admission remains distinct: returning and grant-backed callers require existing `ACTIVE` membership and the exact current Account-owned grant; these callers never use `JOIN` or create or restore membership. The generation and pointer proof gaps prevent claiming complete target proof, but do not make all private/playtest admission unavailable. These are implementation gaps only; they do not weaken tenant isolation, the distinct `tenantSlug` and tenant-scoped authored-world `worldSlug` selectors, or the required `GetAdmissionPointer(tenantId, worldSlug, realmSlug)` contract.
...
- Public-production `JOIN`/`Join & Play` is the only gameplay-adjacent operation that may change a durable `player` membership. Its exact lifecycle is missing -> `ACTIVE`, `INACTIVE` -> `ACTIVE` with advances to `membershipAuthorityGeneration` and `membershipVersion`, and `ACTIVE` -> the exact idempotent current snapshot; all other states reject. `JOIN_REQUIRED` covers missing or `INACTIVE` membership. Character creation, connect-token issuance, and `PLAY` never create or restore membership implicitly.
