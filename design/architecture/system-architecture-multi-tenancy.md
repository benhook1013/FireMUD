...
The realm-catalog, admission-pointer, realm-local character, and explicit first-join contracts below are normative target behavior. Current runtime proof is partial: `IssueConnectToken` and text `PLAY` now reject absent public-production membership with `JOIN_REQUIRED`, but the explicit first-join action and `membershipAuthorityGeneration` live reread at connect-token issuance are unimplemented/proof gaps, and pointer updates still use a read-then-write check with separate pointer/audit/prepared-execution writes rather than an atomic pointer CAS and atomic persistence proof. The first-party admission enablement gate therefore remains closed until explicit `JOIN`, the complete generation reread, the atomic pointer CAS, and atomic persistence of the pointer, append-only audit event, and any required prepared-execution state are implemented and proven. Private/playtest admission remains distinct: it requires existing caller-bound membership plus the current Account-owned grant, never uses `JOIN` or creates membership, and remains unavailable until its own generation and the shared pointer CAS/persistence proofs are complete. These are implementation gaps only; they do not weaken tenant isolation, the distinct `tenantSlug` and tenant-scoped authored-world `worldSlug` selectors, or the required `GetAdmissionPointer(tenantId, worldSlug, realmSlug)` contract.
