...
Scripting pins and rollout history are tenant/game-instance scoped: Game Session owns the exact `(scriptPatchVersion, scriptPinEpoch)` for each instance, and Automation's observed pin projection is keyed by `(tenantId, gameInstanceId)` and carries that exact observed version/epoch tuple. Tenant-scoped readiness only says a patch is eligible for an explicit instance pin; it never authorizes instance work. A stale or local pin cannot admit work for another tenant or instance. The exact identity and epoch rules belong to [Scripting Contracts](./system-architecture-scripting-contracts.md); this document owns the tenancy and routing consequence.
