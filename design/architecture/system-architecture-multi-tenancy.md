...
- **Game instance (`gameInstanceId`)** – A specific running instance of a tenant’s world, keyed as described in [Versioning & Runtime Configuration](./system-architecture-versioning-runtime.md#version-activation--rollback). Persistence models, APIs, and key formats must include `gameInstanceId` explicitly rather than overloading `tenantId`.
  - The first implementation exposes exactly one player-addressable gameplay-admissible instance per tenant, using a stable value such as `"primary"`.
  - Additional running instances may exist for operational workflows, but they are not part of the player lobby contract and must not be implicitly selected by players.
...
- Gameplay admission is stricter: player-facing `WORLDS` / `CHARS` / `PLAY` selection uses caller-bound tenant membership and entitlement checks, and global roles alone do not grant gameplay admission.
...
- Gameplay clients select a world in the lobby flow, and the server resolves that selection to the tenant's single gameplay-admissible instance.
...
- Even though operations may run more than one instance per tenant, the player-facing contract for the first implementation remains one gameplay-admissible instance per tenant.
