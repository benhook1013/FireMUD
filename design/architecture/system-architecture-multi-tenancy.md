...
- Tenant roles, profiles, characters, purchases, subscriptions, entitlements, and grants are target-state tenant-scoped relationships or records. Gameplay state is tenant-owned but may be shared-state or realm-/instance-scoped according to realm policy, with isolated state keyed by `gameInstanceId`. Current scoped-role population and tenant-switching proof remain incomplete: Account authentication currently mints `globalRoles` without fully populating or proving `scopedRoles[tenantId]`. Leaving one game changes only that relationship under its retention rules and does not delete the account or unrelated relationships.
