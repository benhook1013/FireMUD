...
- **Character (`characterId`)** – A per-tenant gameplay identity controlled by a platform account within a specific tenant. Characters, inventories, and progress are always keyed by `tenantId` plus `characterId`.
