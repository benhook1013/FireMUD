...
- **Session** – A transient gameplay binding between a connected client and a character in a tenant, managed by the Game Session Service and stored in Redis. Session keys are always tenant-scoped and never change the global `accountId`.
