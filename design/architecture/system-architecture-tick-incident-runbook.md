...
3. **Apply targeted remediation**
   - For “applied but not marked” rows:
     ...
     - For a current-epoch row, run the service-owned verifier/reconcile path selected by the effect family using that exact ledger projection and its linked root `EffectId` as inputs, including `effectKey` and target aggregate identity, within the exact `<tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch>` scope. The owning domain must prove the effect is already reflected in authoritative state or in its durable participant guard, then let that path transition the ledger row to `APPLIED` with an audit/`tick_effect_outcome_total{outcome="replay_ok"}` record when the domain call is a replay no-op. Use the canonical [tick failure/operations replay-verification CAS](./system-architecture-tick-failures-and-operations.md#tick-effect-ledger-and-replay-guarantees): the same ledger transition CAS must match the current `executorFence` and sealed execution-context digest captured by the batch. A stale or missing `executorFence` or sealed execution-context digest rejects with zero affected rows and no ledger mutation. Replay no-op is an attempt outcome, not a third ledger status. Do not reconcile the same root effect identity across region epochs.
