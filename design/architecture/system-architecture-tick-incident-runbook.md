...
3. **Apply a region-scoped coordination reset (target-state branch only)**
   - **Current-live branch:**
     ...
     5. For any reset or recovery mutation, Automation must be contained before relying on Game Session tick/region containment; Game Session containment alone does not stop Automation. The instance-wide `PauseTicksForScope` emergency fence permitted in step 1 may be applied first when immediate tick containment is required, but it is only a containment fence and never recovery proof. The earlier claim that the live per-scope `SetAutomationAdmissionMode`/`GetAutomationDrainStatus` surfaces now provide durable exact-scope request-result acknowledgement and matching readback overstates the current Get response. `SetAutomationAdmissionMode` persists the successful exact-scope request result and resulting admission epoch. `GetAutomationDrainStatus` can read back the request ID, target mode, immutable fingerprint, outcome, and acknowledgement timestamp, but its `admissionEpoch` is mutable current state and the current response does not expose the acknowledged resulting epoch separately. The epoch match is therefore unavailable in current readback and must fail closed; do not describe these surfaces as a complete matching request-result acknowledgement. Any available identity evidence must still match the request and cover the complete affected-scope enumeration. Do not treat a successful RPC, mutable admission mode/epoch, fresh `observedAt`, or zero drain counts alone as containment proof; keep the fence in place and escalate when required evidence is unavailable, missing, stale, or contradictory. For the current fallback, first resolve the complete affected scope set from the authoritative durable PostgreSQL/runtime inventory, then use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration, and a distinct durable deployment/owner acknowledgement plus authoritative readback that no executor claims or unsafe `rebuildPendingWorkItemIndex` invocations continue before relying on Game Session containment for reset/recovery. The callable method has no live scheduled caller but remains unsafe; a per-scope admission pause or readiness observation alone is insufficient. Exact pin-epoch, process-cessation, and safe-rebuild proof remain unavailable, so reset/resume stays fail-closed. Preserve this containment and escalation path while recovery evidence is incomplete. See the [production recovery fallback](../operations/deployments/production/recovery/README.md#current-operator-fallback) for the canonical current-live procedure.
     6. Treat enumerated `regionId` values as exact affected-scope, blast-radius, and scope-mismatch evidence. The live per-scope Set/Get sequence must enumerate every regional and existing empty/unscoped scope, use the exact normalized scope and workflow request identity, and require durable request-result/fingerprint/acknowledgement readback before recovery proceeds for the available fields; omitted/blank `regionId` is only the exact empty/unscoped row, never a wildcard or aggregate. The current Get response does not separately expose the acknowledged resulting admission epoch, so epoch-match proof remains unavailable and this gap fails closed. Matching Set/Get evidence is required but is not complete recovery authorization: exact pin-epoch, process-cessation, and safe-rebuild proof remain unavailable, so do not use a region-qualified or instance-wide Automation row to authorize reset or resume; preserve the current deployment-wide containment fallback and escalate.
