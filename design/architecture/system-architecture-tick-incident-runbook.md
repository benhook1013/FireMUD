...
1. **Quiesce tick work for the region**
   - **Current-live branch:**
     1. Use only the deployed containment/fencing controls available at the supported `<tenantId, gameInstanceId>` boundary; the shipped `PauseTicksForScope`/`ResumeTicksForScope` control is currently an instance-wide administrative pause/resume, rejects `regionId`, and is not an exact regional or ADR-0048-complete control. After the broader-scope approval gate below, this instance-wide `PauseTicksForScope` may be applied as an initial emergency fence when immediate tick containment is required, but it is not exact regional containment, Automation containment, or recovery authorization. Do not infer or invoke an exact regional pause.
...
3. **Apply a region-scoped coordination reset (target-state branch only)**
   - **Current-live branch:**
     ...
     5. For any reset or recovery mutation, Automation must be contained before relying on Game Session tick/region containment; Game Session containment alone does not stop Automation. The instance-wide `PauseTicksForScope` emergency fence permitted in step 1 may be applied first when immediate tick containment is required, but it is only a containment fence and never recovery proof. The live per-scope `SetAutomationAdmissionMode`/`GetAutomationDrainStatus` surfaces now provide durable exact-scope request-result acknowledgement and matching readback. Any reset/recovery mutation must match the request ID, target mode, immutable fingerprint, admission epoch, and outcome, and must cover the complete affected-scope enumeration. Do not treat a successful RPC, admission mode/epoch, fresh `observedAt`, or zero drain counts alone as containment proof; keep the fence in place and escalate when the durable tuple is missing, stale, or contradictory. For the current fallback, first resolve the complete affected scope set from the authoritative durable PostgreSQL/runtime inventory, then use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration, and a distinct durable deployment/owner acknowledgement plus authoritative readback that no executor claims or unsafe `rebuildPendingWorkItemIndex` invocations continue before relying on Game Session containment for reset/recovery. The callable method has no live scheduled caller but remains unsafe; a per-scope admission pause or readiness observation alone is insufficient. Exact pin-epoch, process-cessation, and safe-rebuild proof remain unavailable, so reset/resume stays fail-closed. Preserve this containment and escalation path while recovery evidence is incomplete. See the [production recovery fallback](../operations/deployments/production/recovery/README.md#current-operator-fallback) for the canonical current-live procedure.
     6. Treat enumerated `regionId` values as exact affected-scope, blast-radius, and scope-mismatch evidence. The live per-scope Set/Get sequence must enumerate every regional and existing empty/unscoped scope, use the exact normalized scope and workflow request identity, and require durable request-result/fingerprint/acknowledgement readback before recovery proceeds; omitted/blank `regionId` is only the exact empty/unscoped row, never a wildcard or aggregate. Matching Set/Get evidence is required but is not complete recovery authorization: exact pin-epoch, process-cessation, and safe-rebuild proof remain unavailable, so do not use a region-qualified or instance-wide Automation row to authorize reset or resume; preserve the current deployment-wide containment fallback and escalate.
...
4. **Resume ticks and verify recovery**
   - **Target state only, once implemented and proven:** Resume tick scheduling for the region only after Game Session coordination cleanup and, **once the target Automation projections exist**, executable owner-reconciled Automation cleanup/rebuild, exact readback, the canonical post-reset smoke gate, and all applicable recovery release gates have completed. Target-state resume additionally requires Automation admission-mode readback to show the intended post-incident mode and a verified executor start/drain gate; Game Session resume is not sufficient evidence that Automation has resumed safely. The current-live instance/deployment remains paused and non-accepting while that Automation recovery is unavailable; the current operator fallback has no executable Automation cleanup/rebuild and must not claim this target gate is complete.
