...
3. **Apply a region-scoped coordination reset (target-state branch only)**
   - **Current-live branch:**
     ...
     5. For any reset or recovery mutation, Automation must be contained before relying on Game Session tick/region containment; Game Session containment alone does not stop Automation. The instance-wide `PauseTicksForScope` emergency fence permitted in step 1 may be applied first when immediate tick containment is required, but it is only a containment fence and never recovery proof. The live per-scope `SetAutomationAdmissionMode`/`GetAutomationDrainStatus` surfaces are not a recovery authorization because they do not yet provide a durable request-result acknowledgement or matching readback identity. Do not treat a successful RPC, admission mode/epoch, fresh `observedAt`, or zero drain counts as containment proof; keep the fence in place and escalate when per-scope state is missing or ambiguous. The durable Set result, immutable fingerprint, and matching Get/owner acknowledgement described by the [Scripting & Automation control-plane API owner contract](./system-architecture-scripting-control-plane-api.md#setautomationadmissionmode) are target-state evidence only until implemented and proven. For the current fallback, first resolve the complete affected scope set from the authoritative durable PostgreSQL/runtime inventory, then use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration, and a distinct durable deployment/owner acknowledgement plus authoritative readback that no executor claims or unsafe `rebuildPendingWorkItemIndex` invocations continue before relying on Game Session containment for reset/recovery. The method has no live scheduled caller but remains unsafe; a per-scope admission pause or readiness observation alone is insufficient. Preserve this containment and escalation path while per-scope recovery evidence is unavailable. See the [production recovery fallback](../operations/deployments/production/recovery/README.md#current-operator-fallback) for the canonical current-live procedure.
     6. Treat enumerated `regionId` values as exact affected-scope, blast-radius, and scope-mismatch evidence. A target-state per-scope Set/Get sequence must enumerate every regional and existing empty/unscoped scope, use the exact normalized scope and workflow request identity, and require durable request-result/fingerprint/acknowledgement readback before recovery proceeds; omitted/blank `regionId` is only the exact empty/unscoped row, never a wildcard or aggregate. Until that target evidence exists in the live owner, do not use a region-qualified or instance-wide Automation row as recovery proof; preserve the current deployment-wide containment fallback and escalate.
