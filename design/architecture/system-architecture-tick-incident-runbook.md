...
3. **Apply a region-scoped coordination reset (target-state branch only)**
   - **Current-live branch:**
     ...
     5. Production recovery ordering is strict: pause Automation admission and prove its drain before applying Game Session tick/region containment; Game Session containment alone does not stop Automation. First resolve the complete affected scope set from the authoritative durable PostgreSQL/runtime inventory, including every affected region and any existing empty/unscoped work. For each affected current-live `<tenantId, gameInstanceId[, regionId]>` scope, issue Automation's implemented, deployed `SetAutomationAdmissionMode(mode=PAUSED_FOR_ROLLBACK)` once with the workflow-scoped `controlPlaneRequestId`; each idempotency key is `(controlPlaneRequestId, normalized exact scope, explicit step/target mode)`, so the same workflow ID may span deliberate distinct scopes and the later `NORMAL` step. A supplied nonblank `regionId` addresses only that exact regional row; omitted/blank `regionId` selects only the exact empty/unscoped row and never regional rows (`region_id=""`), never an instance-wide aggregate or wildcard. Then read back `GetAutomationDrainStatus` once and require a durable successful Set result and matching `GetAutomationDrainStatus` readback for every exact scope that the deployed owner can address, including matching exact scope, request ID, target mode, fingerprint, admission epoch, outcome, and fresh `observedAt`; `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` remain diagnostic drain observations only. Before Game Session containment or reset proceeds, also require a durable owner/deployment drain acknowledgement bound to each exact scope, admission epoch, and control-plane request proving that no executor can claim new work and no scheduled rebuild invocation remains; the read-only drain counts and ordinary metrics/logs alone are not proof. Missing regional controls, incomplete inventory, unavailable result/readback, or contradictory evidence fails closed; do not infer coverage from aggregate counts, metrics, or logs. The current-live drain surface does not expose exact `scriptPinEpoch`; target-state recovery additionally requires exact script-pin-epoch equality and a pin-epoch-bound acknowledgement, but that equality is not a current-live prerequisite.
     6. Treat enumerated `regionId` values as exact affected-scope, blast-radius, and scope-mismatch evidence. Reuse the workflow-scoped `controlPlaneRequestId` across deliberate exact-scope regional calls; that request ID plus exact normalized scope and explicit target mode/step remains the idempotency identity. Region-qualified current-live Automation mutation and drain calls are supported for those regional boundaries; do not use the empty/unscoped row as a substitute for complete affected-scope enumeration or exact regional acknowledgement. Preserve broader containment and escalate until each affected exact scope has durable owner acknowledgement.
