...
For standalone issuer coverage, the durable operation record and every post-fence carrier persist `parentRecoveryOperationId=NOT_APPLICABLE`, `parentLifecycle=NOT_APPLICABLE`, `parentOperationFence=NOT_APPLICABLE`, and `parentCoverageFence=NOT_APPLICABLE`; the operation self-binds `issuerCoverageOperationId=operationId`, `issuerCoverageLifecycle=lifecycle`, and `issuerCoverageOperationFence=operationFence`, plus its distinct `coverageFence`. Linked-child coverage retains the exact parent tuple and remains nested under the parent maintenance lock; standalone evidence never fabricates a parent identity, lifecycle, or fence.
...
- The Account-validated `rebindHandle` must also bind and match the gameplay binding's exact scalar `membershipVersion` and complete stream-key-ordered `outboxCheckpoints` set. A handle with a missing, extra, reordered, stale, regressed, contradictory, or aggregate-only checkpoint, or a membership version that differs from the binding/Account lease, fails closed; rebind must not merge handle and binding values or retain only a latest checkpoint.
- Current Account authority for the exact account and tenant, including the applicable `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, `authorityTuple.membershipAuthorityGeneration`, and private/playtest realm-grant identity and `grantVersion` when applicable, plus current `membershipVersion`, public-production entitlement, revocation, authority-freshness lease, and the complete exact `outboxCheckpoints` set for every applicable authority stream. Missing, extra, stale, regressed, or aggregate-only checkpoint evidence fails closed. For an entitlement read that is unavailable, only the same-binding public-production continuity fallback below may apply; it never supplies current authority evidence.
...
For a destructive full-deployment or AOF reset, a node-level AOF reset must resolve its actual cluster blast radius as `cluster`, or provide a fresh signed attestation from a trusted deployment attestor independently proving physical dedication to the exact resolved region or tenant scope, or use an equivalent proven node-isolation protocol that independently proves no other scope can be affected. A whole-deployment `FLUSHALL` or AOF replacement is permitted only for resolved `cluster` scope or under that fresh dedication or equivalent proven node protocol; otherwise region/tenant scope is rejected before the external handoff. Logical scope labels and operator assertions are not proof. The canonical pre-wipe gates are named `scope_paused_and_locked`, `account_authority_token_cutover`, `replay_domain_quarantine_fence`, and `immutable_external_handoff_evidence`. These are internal evidence gates, not public commands; every gate must be durably bound to the same `operationId` and server-issued `maintenanceLockToken` before the external storage action occurs.
...
- `post_reset_replacement_verification` is a separate evidence group recorded after the replacement starts; the trusted deployment attestor generates and signs it from a fresh challenge bound to the same `operationId`, resolved scope, and replacement deployment identity, and it is never an operator assertion. The controller independently verifies endpoint identity, health, ACL/configuration, and empty keyspace before recovery continuation, rejecting replayed, expired, mismatched, or caller-supplied evidence.
