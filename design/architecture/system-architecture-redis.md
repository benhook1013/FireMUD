...
The canonical durable active-binding inventory is a Game Session-owned PostgreSQL `GameplayBindingInventory` (or an equivalent durable control-store record with the same transaction and snapshot guarantees). It contains one row for every `ACTIVE`, `PROVISIONAL`, or unresolved account/issuer repair binding, keyed by the exact `bindingRef`, and records the tenant/game/session identity, `bindingGeneration`, issuer and layout identity, lifecycle state, index-obligation state, every matching `IssuerPartitionReservation` identity/owner/transition/lifecycle/fence/revision, and durable inventory revision. A reservation-only pre-binding row is also included until it reaches terminal `RELEASED`. It is the authority for inventory completeness, recovery enumeration, and cutoff capacity coverage; it is not a substitute for Account authority, the `session:game:*` reconnect record, or the region-local `tick:{tenantRegionTag}:session-binding:<entityId>` command-admission record.
