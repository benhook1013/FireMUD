...
- **Coordination vs cache prefixes**
  ...
  - Coordination prefix ownership is normative: Account owns the explicit `session:auth:token:*` and `session:auth:generation:*` prefixes, including the exact-token registry and canonical authority-generation projections. Game Session owns the separately approved gameplay families under `session:game:*`, including `session:game:auth:issuer-generation:v1:<issuerId>`; it never owns the Account token or generation prefixes, and no wildcard `session:auth:*` grant is implied. Game Session also owns `tick:*`, `timer:*`, `retry:*`, and `tick-executor-lease:*`; it does not own or mutate the Automation & Scripting families `automation:tick:*`, `automation:timer:*`, or `script-scheduler:*`. Automation & Scripting owns those automation families, and non-owner participation is contract-bound through approved helpers. See `design/architecture/decisions/adr-0009-coordination-redis-ownership-boundary.md`.
