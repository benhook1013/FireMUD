...
| `automation:tick:{tenantInstanceScriptTag}:lock` / `queue` / `pending` | Per-script automation tick locks and staging (Automation & Scripting Service) scoped to `<tenantId> + <gameInstanceId> + <scriptId>`. |
