...
The rotation `installationIdentity` is not an additional `schemaVersion=2` authenticated gameplay field. It is candidate-installation metadata carried only by the lease-scoped acknowledgement and durable installation-attempt carriers, with the established shape `{rotationOperationId, leaseId, gameplayBindingId, installationFence}`; it is not retained as an undeclared field in the final admissible gameplay binding. The `INSTALLING` candidate remains non-admissible until the matching acknowledgement and second fenced CAS complete.
...
For whole-deployment `FLUSHALL`, use the complete [canonical pre-wipe gates and physical-dedication proof](./system-architecture-redis-ops-access.md#canonical-pre-wipe-gates) and independently evidence the blast radius as `cluster`. A node-local AOF reset or replacement is a separate destructive action and uses the same complete pre-wipe gates with a fresh canonical `physical-dedication-proof/v1` binding the exact operation, deployment, node set, and resolved scope. This Redis architecture hub does not restate that proof schema; its immutable handoff evidence must bind the exact resolved scope to the same operation and maintenance lock. A resolved scope value alone is insufficient and the external handoff is rejected when the applicable evidence is absent or mismatched.
