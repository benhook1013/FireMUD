...
Entitlement fallback is deliberately non-circular. New `PLAY`, a new binding, a target change, expansion, or any capacity-creating operation requires a fresh Account-authenticated `account-auth-evidence-bundle/v1`; if that bundle is unavailable, stale, contradictory, or gapped, the operation fails closed with `ENTITLEMENT_UNAVAILABLE` or the applicable stale-admission error. Only resume of the same still-resumable binding may use a previously accepted positive bundle retained in a protected binding-continuity record. The fallback is permitted only for an entitlement-read-only outage, while every other current Account predicate succeeds, the exact activation lease is committed, and a fresh Account-authenticated delegation JWT check succeeds; it never supplies authority, revocation, checkpoint, lease, or token evidence. That record must contain the exact target, `bundleVersion`, `snapshotIdentity`, `evaluationIdentity`, `evaluatedAt`, `entitlementVersion`, `tenantBillingSequence`, applicable `tenantBillingCutoff`, complete `authorityTuple`, and complete `outboxCheckpoints` from one Account evaluation. At `fallbackAt`, define `effectiveContinuityDeadline = min(continuityBindingExpiresAt, resumeDeadline)` and `entitlementFallbackDeadline = min(effectiveContinuityDeadline, fallbackAt + 5 minutes, fallbackAt + session_expiration_ms)`, where `session_expiration_ms = min(FIREMUD_AUTH_SESSION_EXPIRATION_MS, 300000)`. The fallback is usable only before that deadline and only for the unchanged binding; when Account is reachable for the entitlement read, the live bundle is required and must equal those stored identity/version/checkpoint fields. An unavailable non-entitlement predicate, activation lease, or delegation-JWT check never qualifies for fallback. A missing fallback record, known newer billing sequence or authority fence, expired grace window, target change, negative/revoked snapshot, or contradictory local evidence fails closed. The fallback never validates itself by treating its own version as current authority.
