...
- **Coordination timeline = epoch-scoped `(regionEpoch, tickId)`**
...
- The current target layout is one logical set per account, sharded by the complete untagged key. Each member is the owner-defined versioned, generation-safe `accountIndexMember`: it contains the complete tenant-qualified `bindingRef`, a positive `bindingGeneration`, and the `accountIndexFence` ordered by that generation. A bare `sessionId`, tenant-only member, member without fence evidence, or Redis-derived tenant association is invalid. The exact member and acknowledgement contract is owned by [Session Behavior](./system-architecture-session-behavior.md#global-account-active-binding-index); this hub does not define a second shape.
