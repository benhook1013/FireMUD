...
- **Bootstrap vs stream**
  ...
  - A single **per-region metadata key** captures the Redis-side view of this timeline for coordination scripts:
    - `tick:{tenantRegionTag}:meta` is a hash that stores at least:
      ...
      - `current_tick_terminal_at_ms` – persisted first-terminal timestamp marking when `current_tick_state` first entered `APPLIED` or `ABANDONED`; exact retries reuse the persisted value, while a conflicting caller value is a non-mutating idempotency conflict. It is used for observability and bounded cleanup only, never for correctness decisions inside Lua.
    ...
    - Schedulers and operators:
      ...
      - On a normal cold start with empty Coordination Redis and no active recovery, the next winning tick executor initializes `tick:{tenantRegionTag}:meta` during hot-path staging from PostgreSQL `RegionStatus` only after the caller explicitly proves that the exact environment and deployment are a documented non-reset profile; schedulers and operators do not treat missing `meta` as a manual pre‑seeding task. While reset recovery is active, missing metadata keeps the scope fenced and hot-path staging must not create or recreate it until the canonical recovery release in [Redis Reset & Recovery](./system-architecture-redis-reset-and-recovery.md).
      ...
      - Treat Redis `pending` contents as an implementation detail of the hot path, not as proof of durable convergence. The durable proof that a tick is safe to move past is explicit terminal `APPLIED` or `ABANDONED` evidence for every required participant in the PostgreSQL ledger or equivalent durable terminal records; reconciliation-backlog presence and `replay_ok` attempt outcomes are insufficient by themselves. An inconclusive old-epoch row remains non-terminal under its original root `EffectId` and blocks this proof. Only after that proof does the caller record `APPLIED` or `ABANDONED` in `tick:{tenantRegionTag}:meta`.
      - Recovery after tail loss or reset does **not** reconstruct old ticks by silently restaging them through normal hot-path scripts. Recovery completes or abandons older work from durable manifests, ledger rows, and reconciliation backlog state only where the evidence policy permits; inconclusive old-epoch work remains non-terminal reconciliation work under its original root `EffectId` before newer ticks may stage.
...
- The Account-validated `rebindHandle` must also bind and match the gameplay binding's exact one-tenant `membershipVersion` map `{tenantId: version}` and complete stream-key-ordered `outboxCheckpoints` set. The map comparison is exact over presence, one tenant key, and version value, and is independent from the `authorityTuple.membershipAuthorityGeneration` predicate. A scalarized map or checkpoint set, missing, extra-tenant, differently keyed, stale, regressed, contradictory, or aggregate-only membership value or checkpoint fails closed; scalar authentication fields retain their declared scalar types. Rebind must not merge handle and binding values or retain only a latest checkpoint. Account's `GetTenantMembershipForRuntime` response remains scalar and is not changed by this persisted/cross-service carrier requirement.
...
- **Coordination vs cache prefixes**
  - Coordination prefixes (for example `tick:*`, `timer:*`, `retry:*`, `session:game:*`, `session:auth:token:*`, `session:auth:generation:*`, `tick-executor-lease:*`) live **only** on Coordination Redis.
  - Cache and rate‑limit prefixes (for example `inventory:*`, `view:room-look:*`, `ratelimit:*`) live **only** on Cache/Rate‑Limit Redis.
  - Coordination prefix ownership is normative: Account owns the live legacy `session:auth:account:*` and `session:auth:tenant:*` projections and the target `session:auth:token:*` and `session:auth:generation:*` prefixes, including the exact-token registry and canonical authority-generation projections. Game Session owns the separately approved gameplay families under `session:game:*`, including `session:game:auth:issuer-generation:v1:<issuerId>`; it never owns the Account auth prefixes, and no wildcard `session:auth:*` grant is implied. Game Session also owns `tick:*`, `timer:*`, `retry:*`, and `tick-executor-lease:*`; it does not own or mutate the Automation & Scripting families `automation:tick:*`, `automation:timer:*`, or `script-scheduler:*`. Automation & Scripting owns those automation families, and non-owner participation is contract-bound through approved helpers. See `design/architecture/decisions/adr-0009-coordination-redis-ownership-boundary.md`.
