...
| `session:game:index:issuer:{issuerIndexPartitionTag}:<issuerId>:<partitionId>` | Bounded issuer partition hash of active/provisional binding references and captured issuer generations, maintained through the separate issuer repair/CAS protocol and swept across the finite configured partition range for issuer cutoffs. |
