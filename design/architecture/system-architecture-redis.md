...
- An exact `session:auth:token:<tokenHash>` registry record that is present, active, unrevoked, unexpired, and matches the session's exact `jti`, `tokenGeneration`, account identity, canonical token profile, audience, and registry `iat`/`exp` values, with the verified JWT's required `nbf` also valid. The registry is authoritative for `jti`, `tokenGeneration`, `iat`, and `exp`; the verified JWT is authoritative for its signed `nbf`, and neither source may substitute for or repair the other.
