...
The canonical member schema, repair obligations, account-wide child/standalone recovery carriers, acknowledgement lifecycle, durable inventory, and fail-closed coverage rules are defined in [Session Behavior](./system-architecture-session-behavior.md#global-account-active-binding-index). This Redis hub retains only the physical storage and atomicity consequences.

- `session:game:index:account:<accountId>` is the intentionally untagged global Redis set used by Game Session to project active bindings across tenants. It is not an authorization source and is never included in tenant-local Lua or session-only CAS.
