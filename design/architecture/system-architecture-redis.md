...
For a destructive full-deployment or AOF reset, use the complete [canonical pre-wipe gates and physical-dedication proof](./system-architecture-redis-ops-access.md#canonical-pre-wipe-gates). This Redis architecture hub does not restate that proof schema. A whole-deployment `FLUSHALL` or AOF replacement remains limited to a resolved `cluster` scope or an accepted canonical proof; otherwise the external handoff is rejected.
