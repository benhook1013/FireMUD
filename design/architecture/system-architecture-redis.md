...
- Index mutation is a separate fenced operation from the tenant-local binding CAS. Narrow recovery may repair only individually proven members and must preserve unrelated tenants; account-wide rebuild uses the complete durable inventory and exact full-key readback. Redis `SCAN`, key absence, or the projection alone cannot establish coverage.
...
5. Region reset reconciliation is also generation-based:
   - Region-scoped reset policy is defined by [Redis Reset & Recovery](./system-architecture-redis-reset-and-recovery.md): the operation must carry exactly one explicit `--preserve-sessions` or `--invalidate-sessions` option, and no default is inferred. Locally, clearing the affected `tick:{tenantRegionTag}:*` binding family requires retained `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` records to rebind before intake under `--preserve-sessions`, while `--invalidate-sessions` requires fresh admission; derived `session:game:index:*` projections reconcile and read back under either policy, and pre-auth transport context is invalidated or rebuilt.
...
For a destructive full-deployment or AOF reset, use the complete [canonical pre-wipe gates and physical-dedication proof](./system-architecture-redis-ops-access.md#canonical-pre-wipe-gates). This Redis architecture hub does not restate that proof schema; its immutable handoff evidence must bind the exact resolved scope to the same operation and maintenance lock. A whole-deployment `FLUSHALL` or AOF replacement is permitted only when the actual blast radius is independently evidenced as `cluster` or the operation carries a fresh canonical `physical-dedication-proof/v1`; a resolved scope value alone is insufficient and the external handoff is rejected otherwise.
