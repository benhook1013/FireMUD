...
For a destructive full-deployment or AOF reset, the action is allowed only at resolved `cluster` scope or when a fresh signed record from a trusted deployment attestor independently proves physical dedication to the exact resolved region or tenant scope. Logical scope labels and operator assertions are not proof. The canonical pre-wipe gates are named `scope_paused_and_locked`, `account_authority_token_cutover`, `replay_domain_quarantine_fence`, and `immutable_external_handoff_evidence`. These are internal evidence gates, not public commands; every gate must be durably bound to the same `operationId` and server-issued `maintenanceLockToken` before the external storage action occurs.
