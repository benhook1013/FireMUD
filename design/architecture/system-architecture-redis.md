...
For standalone issuer coverage, the durable operation record and every post-fence carrier, digest, readback, acknowledgement, retry, and completion record persist `parentRecoveryOperationId=NOT_APPLICABLE`, `parentLifecycle=NOT_APPLICABLE`, `parentOperationFence=NOT_APPLICABLE`, and `parentCoverageFence=NOT_APPLICABLE`; the operation self-binds `issuerCoverageOperationId=operationId`, `issuerCoverageLifecycle=lifecycle`, and `issuerCoverageOperationFence=operationFence`, plus its distinct `coverageFence`. Linked-child coverage retains the exact parent tuple through every request, digest, readback, acknowledgement, retry, and parent-consumption record and remains nested under the parent maintenance lock; standalone evidence never fabricates a parent identity, lifecycle, or fence.
