...
When account-wide coverage is required by preserved-session region- or tenant-scoped recovery, the parent links one singleton Game Session-owned account child operation through an independent `recovery-parent-edge/v1`, not by nesting the child under the parent maintenance lock. The singleton is keyed by account-coverage family, exact `accountId`, and compatible immutable scope/layout/snapshot identity. The child owns `accountCoverageOperationId`, lifecycle, `accountCoverageOperationFence`, `coverageFence`, complete-account `inventorySnapshotRevision`, and later `coverageGeneration`; once coverage-proven, its immutable result is globally shareable. The parent owns the deployment maintenance lock, while the linked child never acquires, owns, transfers, or refreshes that lock; a standalone account operation owns its own lock. The durable edge, every linked child request and digest, transition, exact readback, acknowledgement, parent-consumption record, and retry carry and exact-compare `parentRecoveryOperationId`, `parentLifecycle`, parent `operationFence`, distinct `parentCoverageFence`, child identity/lifecycle/fence, resolved `accountId`, and link revision. One controller mutates the singleton; an overlapping compatible parent waits for or attaches to the same child, a parent-fence change invalidates only its edge, and an incompatible tuple fails closed rather than creating another child. Each edge consumes the immutable child result exactly once, exact retries replay that edge, and the child has no global consumed state. The child enumerates the complete all-tenant `GameplayBindingInventory` snapshot for that account, repairs the untagged `session:game:index:account:<accountId>` key, reads back the exact complete generation-qualified member set and readback evidence, and publishes acknowledgements only after every inventory row, transition, and obligation is covered under that tuple and a later `coverageGeneration` is recorded. The parent validates the same identities, lifecycles, both operation fences, both coverage fences, snapshot, readback, acknowledgements, and later generation, then consumes only the complete child result after the child reaches its coverage-proven lifecycle. An unavailable, stale, partial, contradictory, duplicate, or narrow-scope result fails closed; a parent operation cannot infer account-wide coverage from a local snapshot, affected-member readback, empty key, aggregate count, or `SCAN`.

A standalone account-wide operation uses the same recovery carrier without a parent edge: `parentRecoveryOperationId`, `parentLifecycle`, `parentOperationFence`, and `parentCoverageFence` are the exact typed `NOT_APPLICABLE` arm; `accountCoverageOperationId=operationId`, `accountCoverageLifecycle=lifecycle`, and `accountCoverageOperationFence=operationFence` self-bind the operation; and a distinct `coverageFence`, complete all-tenant `inventorySnapshotRevision`, later `coverageGeneration`, and exact `accountId` bind the finite coverage result. It owns its own maintenance lock, enumerates every tenant in the account inventory, repairs and reads back the complete untagged account key, publishes its own acknowledgements, and has no parent-consumption path. The standalone account carrier is the same shape as standalone issuer coverage; only the self-bound global coverage identity changes.
...
For standalone issuer coverage, the durable operation record and every post-fence carrier, digest, readback, acknowledgement, retry, and completion record persist `parentRecoveryOperationId=NOT_APPLICABLE`, `parentLifecycle=NOT_APPLICABLE`, `parentOperationFence=NOT_APPLICABLE`, and `parentCoverageFence=NOT_APPLICABLE`; the operation self-binds `issuerCoverageOperationId=operationId`, `issuerCoverageLifecycle=lifecycle`, and `issuerCoverageOperationFence=operationFence`, plus its distinct `coverageFence`, complete issuer `inventorySnapshotRevision`, and later `coverageGeneration`. Linked-child coverage retains the exact parent tuple through every request, digest, readback, acknowledgement, retry, and parent-consumption record, but the linked child is a globally shareable immutable coverage operation and does not nest under the parent maintenance lock; standalone evidence never fabricates a parent identity, lifecycle, or fence.
