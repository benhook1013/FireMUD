...
- **Bootstrap vs stream**
  - The authoritative **baseline** for `(region_epoch, tickId)` comes from Game Session’s control/status surface (for example a `GetRegionTickStatus` API) backed by a PostgreSQL `RegionStatus`-style table; a reset durably establishes the new epoch baseline consistently in that PostgreSQL state and Redis `tick:{tenantRegionTag}:meta` before normal progress. New consumers and operational tooling must obtain their initial view of the timeline from the owner surface rather than inferring it from Redis keys.
