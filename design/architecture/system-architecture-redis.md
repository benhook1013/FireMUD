...
- A complete target `schemaVersion=2` authenticated gameplay session payload, including `issuanceFence`, `rebindHandle`, `continuityBindingExpiresAt`, `membershipVersion`, and `authorityTuple.membershipAuthorityGeneration`; a duplicate top-level `membershipAuthorityGeneration` is rejected, while a `schemaVersion=1` or incomplete record is storage-only and cannot be rebound.
