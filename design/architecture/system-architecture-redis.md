...
5. Region reset reconciliation is also generation-based:
   ...
   - Until the rebind succeeds, each attempted gameplay command in that entity/region must be durably terminal with `executionOutcome = REGION_REBIND_REQUIRED` and a non-applied gameplay result (the shared default is `NOT_APPLIED`), or the client must re-`PLAY`; it must not fall back to advisory session fields as gameplay authority.
