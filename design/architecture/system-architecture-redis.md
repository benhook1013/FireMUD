...
5. Region reset reconciliation is also generation-based:
   - A region-scoped coordination reset clears the region-local binding keys for the affected `tick:{tenantRegionTag}:*` family and must carry exactly one explicit session policy, `--preserve-sessions` or `--invalidate-sessions`; there is no implicit default. `--preserve-sessions` retains only `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` records for bounded rebind. `session:game:index:*` projections are derived and may be dropped under either policy, but must reconcile and read back before release; pre-auth transport context is invalidated or rebuilt. `--invalidate-sessions` makes the canonical records non-resumable and requires fresh admission.
