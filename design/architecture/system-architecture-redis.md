...
For a destructive full-deployment or AOF reset, use the complete [canonical pre-wipe gates and physical-dedication proof](./system-architecture-redis-ops-access.md#canonical-pre-wipe-gates). This Redis architecture hub does not restate that proof schema; its immutable handoff evidence must bind the exact resolved scope to the same operation and maintenance lock. A whole-deployment `FLUSHALL` or AOF replacement is permitted only when the actual blast radius is independently evidenced as `cluster`, or under that exact fresh proof or equivalent independently verified node-isolation protocol; a resolved scope value alone is insufficient and the external handoff is rejected otherwise.
