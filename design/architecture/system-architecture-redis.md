...
- The current layout is one logical set per account, sharded by the complete untagged key. Members are versioned, generation-qualified `accountIndexMember` values; a bare `sessionId`, tenant-only member, or member without fence evidence is invalid.
