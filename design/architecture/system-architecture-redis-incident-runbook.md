...
- When disk pressure, corruption, or replay issues are detected:
  ...
  - Follow only the documented AOF recovery paths:
    ...
    - The scoped reset/full-wipe flow in `system-architecture-redis-operations.md` is target state only until the durable recovery controller, Account projection-repair workflow, immutable evidence gates, and bounded maintenance tooling are implemented and proven. Its future node-level destructive action must resolve its actual cluster blast radius as `cluster`, or provide a fresh signed attestation from a trusted deployment attestor independently proving physical dedication to the exact resolved region or tenant scope, or use an equivalent proven node-isolation protocol that independently proves no other scope can be affected. Whole-deployment `FLUSHALL` or AOF replacement is allowed only for resolved `cluster` scope or under that fresh dedication or equivalent proven node protocol; region- or tenant-scoped requests are rejected before external handoff when the required proof is absent, and logical scope labels and operator assertions are not proof. The action requires `scope_paused_and_locked`, `account_authority_token_cutover`, `replay_domain_quarantine_fence`, and immutable pre-wipe `immutable_external_handoff_evidence` for the same operation and server-issued maintenance lock, followed by `post_reset_replacement_verification` generated and signed by the trusted deployment attestor from a fresh replacement-bound challenge. The attestor evidence must include the exact `coord_ops_ro/v2` and `coord_redis_config/v1` digests plus negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`. The controller must independently verify endpoint identity, ACL/configuration, empty keyspace, and deployment health before continuation or protected admission.
