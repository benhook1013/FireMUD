...
- When disk pressure, corruption, or replay issues are detected:
  ...
  - Follow only the documented AOF recovery paths:
    ...
    - Discard coordination state and run the scoped reset/full-wipe flow from `system-architecture-redis-operations.md`. A full deployment/AOF wipe requires the protected Account authority/token cutover and replay-domain quarantine/fence to be established with immutable evidence first; earlier reset phases may perform only scope-safe cleanup.
...
3. **Act**
   1. If the chosen mode is `replay_first`:
      ...
      - If the replay budget/status gate passes and the operation recorded `--preserve-sessions`, validate every preserved binding in the durable affected-session inventory before continuation. Each binding must prove its exact active and unrevoked token-registry record, authenticated account identity, current authority-freshness lease and committed checkpoint, complete applicable issuer/account/tenant/membership/grant generation tuple, binding generation, and continuity with the unchanged recovery/region epoch. Missing, stale, ambiguous, or mismatched evidence blocks preserved-session reopen; the operation must complete the documented audited session-invalidation path and satisfy `--invalidate-sessions` recovery proof rather than reopening under the preserve policy.
