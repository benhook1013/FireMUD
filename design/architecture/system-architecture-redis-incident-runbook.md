...
3. **Act**
   1. If the chosen mode is `replay_first`:
      ...
      - If the replay budget/status gate passes and a region- or tenant-scoped operation recorded `--preserve-sessions`, validate every preserved binding in the durable affected-session inventory against the complete canonical preserved-session rebind predicate: a complete target `schemaVersion=2` session payload with `rebindHandleEnvelope`, valid `continuityBindingExpiresAt` / `resumeDeadline`, and `membershipVersion`; an exact active, unrevoked, unexpired token-registry record addressed by `authTokenHash`; Account validation of the opaque rebind envelope proving its bound token hash, signed `jti` and `nbf`, token lineage, account identity, canonical token profile, and audience match the registry record and gameplay binding exactly; exact registry `tokenGeneration`, `iat`, and `exp`; current Account identity, entitlement, revocation, authority-freshness lease, and committed checkpoint; exact applicable `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable; and the expected `binding_generation`, current recovery/region epoch, and lease fence. `session:game:*` and pre-auth transport context are not authority substitutes. Any missing, stale, ambiguous, expired, or mismatched token/profile/audience/time evidence blocks preserved-session rebind and reopen.
