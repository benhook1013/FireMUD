...
- When disk pressure, corruption, or replay issues are detected:
  ...
  - Follow only the documented AOF recovery paths:
    ...
    - The scoped reset/full-wipe flow in `system-architecture-redis-operations.md` is target state only until the durable recovery controller, Account projection-repair workflow, immutable evidence gates, and bounded maintenance tooling are implemented and proven. Its future whole-deployment `FLUSHALL` or AOF replacement is allowed only for resolved `cluster` scope or when a fresh signed record from a trusted deployment attestor independently proves physical dedication to the exact resolved region or tenant scope; logical scope labels and operator assertions are not proof. The action requires `scope_paused_and_locked`, `account_authority_token_cutover`, `replay_domain_quarantine_fence`, and immutable pre-wipe `immutable_external_handoff_evidence` for the same operation and server-issued maintenance lock, followed by `post_reset_replacement_verification` generated and signed by the trusted deployment attestor from a fresh replacement-bound challenge. The controller must independently verify endpoint identity, ACL/configuration, empty keyspace, and deployment health before continuation or protected admission.
