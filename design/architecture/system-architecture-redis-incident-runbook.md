...
- When disk pressure, corruption, or replay issues are detected:
  - Any target-state reset gate requires both the exact `coord_ops_ro/v2` ACL digest and the exact `coord_redis_config/v1` observed configuration digest for the target deployment generation, plus the canonical positive and negative probes. Negative probes include `FLUSHALL`, `FLUSHDB`, and `SCRIPT FLUSH` in addition to writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, and telemetry; database- or server-wide probes run only in the isolated disposable target, while key-scoped probes may use the protected probe namespace, and all include cleanup/readback of key, TTL, coordination, and script-cache sentinels.
