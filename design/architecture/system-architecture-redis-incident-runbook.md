...
- **Renewal linearization** – Account publishes a renewal with the exact binding identity, current applicable authority-generation/checkpoint tuple, and a monotonic lease fence. Game Session accepts it only at one atomic compare-and-replace linearization point that requires the authority tuple to match the Account-authoritative snapshot and the presented lease fence to be strictly newer than the binding's stored fence. That compare-and-replace replaces the fence, generation/checkpoint tuple, and `authorityLeaseExpiresAt` together; no stale or racing renewal may replace any one of those fields independently or extend the deadline before acceptance succeeds. Ordinary gameplay commands, socket activity, reconnect attempts, local heartbeats, and retries cannot renew or recreate the lease.
- **Concurrent revocation and invalidation ordering** – Account and Game Session must prove the renewal ordering against concurrent revocation and other renewals. If an Account generation/revocation advance linearizes before Game Session's compare-and-replace, the renewal's authority tuple no longer matches and is rejected before replacing the generation/checkpoint/deadline. If the renewal compare-and-replace linearizes first, the later Account generation/revocation advance invalidates that accepted binding through the normal generation mismatch. A stale or racing renewal with an equal/older lease fence, stale authority snapshot, gap, revocation, or advance is rejected before mutation; no caller may continue on stale authority.
...
3. **Act**
   1. If the chosen mode is `replay_first`:
      - Target state only, not a current operator instruction: once the orchestration is implemented and proven, the future workflow would run `coordination-maintenance recover --mode replay-first ... <session-policy-option>` for the affected scope without bumping `region_epoch`, selecting exactly one of `--preserve-sessions` or `--invalidate-sessions`. The high-level workflow owns ledger and command convergence; those phases are not separate public operator verbs, the session policy is always explicit, and the acquired lock/operation records dedicated `compatibilityClass=replay-first` derived from `--mode`.
      ...
      - If the replay budget/status gate passes and the operation recorded `--preserve-sessions`, validate every preserved binding in the durable affected-session inventory against the complete canonical preserved-session rebind predicate: a complete target `schemaVersion=2` session payload with `rebindHandleEnvelope`, valid `continuityBindingExpiresAt` / `resumeDeadline`, and `membershipVersion`; an exact active, unrevoked, unexpired token-registry record matching the exact token identity, account identity, canonical token profile, audience, `jti`, `tokenGeneration`, and every registry time field (`iat` and `exp`) exactly, with the verified JWT's required `nbf` also valid; current Account identity, entitlement, revocation, authority-freshness lease, and committed checkpoint; exact applicable `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable; and the expected `binding_generation`, current recovery/region epoch, and lease fence. `session:game:*` and pre-auth transport context are not authority substitutes. Any missing, stale, ambiguous, expired, or mismatched token/profile/audience/time evidence blocks preserved-session rebind and reopen.
