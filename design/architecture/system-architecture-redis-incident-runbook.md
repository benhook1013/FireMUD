...
- **Tick executors**
  ...
  - If `pending` is missing (for example due to AOF tail loss), treat this as “coordination state may have been partially lost” rather than silently skipping work:
    - Advance to the next `tickId` only after running the tick effect ledger replay controller / reconcile tooling for the affected scope so any lingering `SCHEDULED` effects converge to `APPLIED` or `ABANDONED` with an explicit tail-loss reason where the evidence policy permits. Inconclusive old-epoch effects remain non-terminal reconciliation work under their original root `EffectId`, block reset-scope convergence/reopening, and prevent current-epoch executors from replaying that `EffectId` or advancing `regionEpoch` until recovery requirements and ledger/command convergence are satisfied.
