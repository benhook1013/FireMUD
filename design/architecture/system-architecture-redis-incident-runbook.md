...
3. **Act**
   1. If the chosen mode is `replay_first`:
      - `replay_first` is a recovery decision, not a maintenance CLI operation. Run `coordination-maintenance pause --operation cleanup ...` using the canonical compatibility class, capture the returned `maintenanceLockToken`, and keep the affected scope in `PAUSED` while replay converges; do not run `reset` unless escalation selects `reset_first`.
      ...
      - If the replay budget/status gate passes, run `coordination-maintenance resume ... --maintenance-lock-token <token>` to release the same maintenance lock without bumping `region_epoch`.
      ...
      - Worked example:
        ...
        6. If pending age and stalled signals recover within one emitted budget window, the operator runs `coordination-maintenance resume --scope region --tenant 7b3b074e-d597-4e9b-b96f-4f5946d26120 --game-instance 9a2bb6d1-74c7-4f81-a9e8-418e65f6ad78 --region R7 --maintenance-lock-token <token>` and the region stays on epoch `13`; otherwise the operator escalates to `reset_first` and releases the lock through the failure path.
