...
- The full `coordination-maintenance` replay/reset orchestration referenced below is not yet shipped as a complete repo-local CLI/control-plane surface; where the runbook names those verbs, read them as target-state recovery tooling rather than a claim that every step is already available today. Initial `recover` issues the server-side maintenance lock; every subsequent control below presents that lock through `--maintenance-lock-token-file <permissioned-token-file>` (or the documented protected FD form), never as a command-line token value.
...
- **Generations and invalidation** – A mismatch, gap, revocation, or advance in any applicable issuer, account, tenant, membership, grant, or lease-fence value is evaluated against the binding's stored fence and invalidates the binding. The sole exception is the binding's own Account-published renewal: Game Session atomically compare-and-replaces the stored fence and associated generation/checkpoint tuple with that renewal, so the renewal cannot self-invalidate. Any mismatch or advance outside that atomic replacement stops the binding rather than continuing on stale authority.
...
3. **Run the canonical session cleanup workflow**
   - Use the session schema/TTL cleanup flow described in [Session Schema Cleanup and Large Keyspaces](./system-architecture-redis-operations.md#session-schema-cleanup-and-large-keyspaces):
     - Run the single supported `coordination-maintenance recover --mode session-schema-cleanup --scope tenant --tenant <tenantId> --invalidate-sessions [--dry-run]`, using `--dry-run` first if the blast radius is uncertain. `session-schema-cleanup` requires the explicit `--invalidate-sessions` policy in the initial supported contract: it cannot preserve or infer gameplay sessions because unsupported or corrupted schema cannot be safely rebound. The operation owns the maintenance lock and invokes session cleanup as an internal recovery phase, not a separately supported public verb. A dry run performs discovery and validation only, then transitions directly to terminal phase `finalized` with terminal operation status `SUCCEEDED`; it never enters `AWAITING_RESUME`, calls `continueRecovery`/public `resume`, or uses `release-lock` to finish. A mutating operation records its bounded cursor/continuation state durably and may be retried through the canonical continuation controls with its server-issued lock rather than a caller-created resume token.
     ...
     - For a mutating cleanup, after the cleanup and its evidence are complete, run `coordination-maintenance continue-recovery --operation-id <operationId> --expected-phase ready_to_reopen --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>` for the same durable operation. It may only reconcile `ready_to_reopen` into `AWAITING_RESUME`; it does not authorize release. Then run the external public `coordination-maintenance resume --operation-id <operationId> --expected-phase awaiting_resume --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>` gate, which validates the operation-owned scope, server-issued lock, immutable evidence, and any applicable Account projection or replay-domain evidence and records `RESUME_AUTHORIZED`. Only after that authorization may the recover operation run its separate internal success-release phase. After an operator abort or failed mutating workflow, use only `coordination-maintenance release-lock --operation-id <operationId> --scope tenant --tenant <tenantId> --maintenance-lock-token-file <permissioned-token-file> --reason <reason> --evidence-ref <evidenceRef>` against the matching active workflow.
4. **Verify recovery**
...
3. **Act**
   1. If the chosen mode is `replay_first`:
      - Run `coordination-maintenance recover --mode replay-first ... (--preserve-sessions|--invalidate-sessions)` for the affected scope without bumping `region_epoch`. The high-level workflow owns ledger and command convergence; those phases are not separate public operator verbs, the session policy is always explicit, and the acquired lock/operation records dedicated `compatibilityClass=replay-first` derived from `--mode`.
      ...
      - If the replay budget/status gate passes and the operation recorded `--preserve-sessions`, validate every preserved binding in the durable affected-session inventory against the complete canonical preserved-session rebind predicate: a complete target `schemaVersion=2` session payload with `rebindHandleEnvelope`, valid `continuityBindingExpiresAt` / `resumeDeadline`, and `membershipVersion`; an exact active, unrevoked, unexpired token-registry record matching token and account identity; current Account identity, entitlement, revocation, authority-freshness lease, and committed checkpoint; exact applicable `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, caller-bound `membershipAuthorityGeneration`, and private-realm `grantVersion` when applicable; and the expected `binding_generation`, current recovery/region epoch, and lease fence. `session:game:*` and pre-auth transport context are not authority substitutes. Missing, stale, ambiguous, or mismatched evidence blocks preserved-session reopen.
      - A failed preserved-session predicate never implicitly changes the policy. The operation remains paused and fenced under the same `operationId` and `maintenanceLockToken`; the operator must either perform an explicit audited preserve-to-invalidate transition under that same lock, recording actor, reason, and immutable evidence before invalidation, or complete audited `release-lock` abandonment and start an explicit new recover operation with `--invalidate-sessions`. The invalidation proof must then complete before continuation; rebind failure alone is not proof.
      - After the applicable preserved-session or invalidation proof passes, run `coordination-maintenance continue-recovery --operation-id <operationId> --expected-phase ready_to_reopen --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>` for the exact operation. It may only reconcile into `AWAITING_RESUME`. Then run the external public `coordination-maintenance resume --operation-id <operationId> --expected-phase awaiting_resume --maintenance-lock-token-file <permissioned-token-file> --evidence-ref <evidenceRef>` gate for the operation's recorded scope, with the required replay-domain and Account authority/projection evidence; it records `RESUME_AUTHORIZED` but does not release the active recovery lock. Only afterward may the recover operation perform its separate internal success-release phase without bumping `regionEpoch`.
      ...
      - Worked example:
        ...
        2. Operator runs `coordination-maintenance recover --mode replay-first --scope region --tenant T1 --game-instance G3 --region R7 --preserve-sessions`.
