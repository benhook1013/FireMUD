...
3. **Act**
   1. If the chosen mode is `replay_first`:
      ...
      - Escalate to `reset_first` immediately if replay cannot make bounded progress, if inconsistent-state signals appear, or if the region transitions to `STALLED`. Escalation atomically upgrades the existing maintenance lock's compatibility class from `cleanup` to `reset` under the same `maintenanceLockToken`, retains the original recovery audit lineage, and records that transition before bumping `regionEpoch`. Do not release and reacquire the lock between recovery modes; if the tooling cannot perform that atomic upgrade, it must leave the scope paused for an explicit operator failure path rather than starting a second recovery lock.
      - Worked example:
