...
3. **Run the canonical session cleanup workflow**
   - Use the session schema/TTL cleanup flow described in [Session Schema Cleanup and Large Keyspaces](./system-architecture-redis-operations.md#session-schema-cleanup-and-large-keyspaces):
     ...
     - Call the public `coordination-maintenance resume --operation-id <operationId> --expected-phase AWAITING_RESUME --scope tenant --tenant <tenantId> --maintenance-lock-token <token> --evidence-ref <evidenceRef>` control for the matching workflow, then let the recover operation perform its internal success release. After an operator abort or failed workflow, use only `coordination-maintenance release-lock --operation-id <operationId> --scope tenant --tenant <tenantId> --maintenance-lock-token <token> --reason <reason> --evidence-ref <evidenceRef>` against the matching active workflow.
4. **Verify recovery**
...
3. **Act**
   1. If the chosen mode is `replay_first`:
      ...
      - If the replay budget/status gate passes, invoke the public `resume` control for the exact operation and scope, then let the recover operation perform its internal success-release phase without bumping `regionEpoch`; `resume` records `RESUME_AUTHORIZED` but does not release the active recovery lock.
