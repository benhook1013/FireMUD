...
3. **Act**
   1. If the chosen mode is `replay-first`:
      ...
      - If the replay budget/status gate passes and a region- or tenant-scoped operation recorded `--preserve-sessions`, use the [canonical active-binding recovery evidence contract](./system-architecture-redis-ops-access.md#canonical-active-binding-recovery-evidence-contract), not Redis-derived session or index state. Every preserved binding must be enumerated from the immutable Game Session-owned `GameplayBindingInventory` snapshot identified by the exact `inventorySnapshotRevision`, with the operation admission/creation fence and later `coverageGeneration` proving complete coverage. A region- or tenant-scoped operation may acknowledge binding-local recovery only; it cannot publish or acknowledge issuer-wide coverage or an issuer-partition acknowledgement, even when its local evidence is complete. Issuer-wide coverage requires a separate issuer-wide inventory and admission `coverageFence` defined by the canonical ops-access contract; until that proof exists, issuer coverage remains unacknowledged and affected issuer admission remains fenced. Every per-partition acknowledgement must match the complete canonical issuer-wide recovery tuple: `operationId`, maintenance `operationFence`, `issuerId`, issuer-wide `inventorySnapshotRevision`, distinct `coverageFence`, later `coverageGeneration`, pinned active layout `{issuerIndexLayoutVersion, issuerIndexPartitionCount, issuerIndexPartitionCapacity, partitionId}`, exact `issuer/<issuerId>` cutoff including issuer generation and `issuanceFence`, complete applicable `outboxCheckpoints`, exact reservation identities with owner, transition, lifecycle, fence, and revision, and `{partitionId, reservationCount, partitionCapacity, aggregateReservationCount, aggregateCapacity}`. Stale, partial, unavailable, conflicting, or Redis-derived evidence blocks preserved-session rebind and reopen. The binding itself must also satisfy the complete [canonical preserved-session rebind predicate](./system-architecture-redis.md#session-and-region-binding-contract), including the target `schemaVersion=2` payload's exact `issuanceFence`, `authorityTuple.membershipAuthorityGeneration`, complete `outboxCheckpoints` set, and every token-registry, identity, authority, expiry, generation, epoch, and lease-fence predicate defined there. `session:game:*` and pre-auth transport context are not authority substitutes.
