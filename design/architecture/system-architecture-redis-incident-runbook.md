...
3. **Act**
   1. If the chosen mode is `replay_first`:
      ...
      - Escalate to `reset_first` immediately if replay cannot make bounded progress, if inconsistent-state signals appear, or if the region transitions to `STALLED`. Escalation stays inside the same paused workflow and carries the same `maintenanceLockToken` into `reset`, reconciliation, smoke-check, and either `resume` or the failure path; do not release and reacquire the lock between recovery modes.
      - Worked example:
