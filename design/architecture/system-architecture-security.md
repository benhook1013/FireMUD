...
FireMUD has one application-facing contract for exportable secrets: Kubernetes workloads consume narrowly scoped Kubernetes Secrets through fixed mounted-file or bounded `secretKeyRef` interfaces, while local tooling consumes equivalent read-only files outside the repository. FireMUD does not bundle or require Vault or another external secret-manager service; operators may populate canonical Kubernetes Secret names from external infrastructure without creating a second application mode. In the target JWT resource mode, the reserved `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap have no external-population path: the materialization controller and Account are their sole writers, respectively. The checked-in runtime's current Secret-backed JWKS mode is described in the implementation notes below and is not target readiness. Target-state JWT custody delegates private-key operations to a non-exportable signer in every environment while Account remains the sole issuer, promotion, JWKS-publication, and pruning authority. The controlled interim fallback uses a materialization-controller-written Secret consumed only by the Account application workload. Rotation automation receives no private material and cannot mutate signing state or public JWKS. High-impact credentials require explicit rotation SLAs, age/missed-rotation alerts, incident runbooks, and measurable compliance evidence.
...
- The target Account-issued asymmetric/JWKS boundary, phased rotation workflow, dedicated rotation-job automation, key-overlap/pruning operations, projected-volume reload proof, deployment-wide validator convergence, and player-facing readiness gate below remain target/operational design rather than completed live capability. The current runtime reads a Secret-backed `jwt-jwks` file and still permits the documented classpath fallback drift; current deployment preflight also validates the legacy Secret path/mount contract and rejects target ConfigMap mode. That current behavior is implementation drift rather than enforcement of the target. This document does not change runtime, preflight, or manifest behavior.
...
- Target-state signing keys remain in **non-exportable signer custody** under the phased protocol in [ADR 0014](./decisions/adr-0014-phased-jwt-signing-key-rotation-and-readiness.md). Account Service owns key-generation requests, validation, promotion, JWKS publication, and public/private pruning; the signer performs only private-key operations Account delegates. Until signer delegation is implemented, `jwt-signing-keys` is a Kubernetes Secret projected read-only only into Account: the materialization controller is its sole API reader/writer and a narrowly scoped infrastructure private-material custodian for generation and Secret CAS, while Account has no Secret API authority and is the only application workload that mounts or uses the bundle. Rotation automation may neither read nor update it.
...
- Each workload receives only the credentials required by its exact identity and function. In particular, Account is the only application workload that mounts or uses the Account JWT private key, and every mTLS workload has a distinct private identity.
- The Account service account has no Kubernetes API authority for the pre-created `jwt-signing-keys` Secret and has name-scoped `get`, `update`, and `patch` authority only for the `jwt-jwks` ConfigMap. The materialization controller alone has name-scoped `get`, `update`, and `patch` authority for `jwt-signing-keys`; this makes it an additional infrastructure custodian even though its implementation uses `get` only to obtain the current `resourceVersion` for CAS. It may generate, materialize, and prune only the requested private-material slots, after authenticating the Account operation, and must return the observed/resulting resource version, generation, `kid`, non-secret digest, and pruning outcome for Account reconciliation. It may not sign tokens, select or promote a signer, publish JWKS, expose key material, or use the key for any other purpose. Neither actor has list, create, or delete authority for those pre-created resources; the rotation Job has no read/write authority over the private Secret and no write authority over public JWKS.
...
- The target JWKS endpoint serves Account-published public material from the fixed-name, mutable `jwt-jwks` ConfigMap. Its contents change only through Account-owned `resourceVersion` compare-and-set, and the Account workload observes the resulting generation through its read-only projected mount before signer promotion. This ConfigMap and projected-mount path is the single canonical player-facing target; the current Secret-backed JWKS preflight/runtime mode is not equivalent. The non-exportable signer performs only delegated private-key operations. Account requests or reconciles private-material pruning through the signer/controller, proves validator visibility, and commits the lifecycle result; rotation automation only observes the result and never reads or updates `jwt-signing-keys` or writes `jwt-jwks`.

### JWT Key & JWKS Rotation Workflow

JWT rotation is coordinated inside the Kubernetes cluster using one Account-owned JWT rotation control/status interface. Account Service owns private-key generation requests, validation, promotion, JWKS publication, and private/public pruning; dedicated Jobs observe that interface, publication/convergence, and evidence without requesting or mutating rotation state. A non-exportable signer may perform only the private-key operations Account delegates. [ADR 0014](./decisions/adr-0014-phased-jwt-signing-key-rotation-and-readiness.md) defines the accepted rotation and readiness boundary. The goals are:

- Keep target-state signing keys non-exportable and never in the repository; restrict interim Secret custody to the materialization controller and Account, and application consumption to Account Service.
...
- A `jwt-signing-keys` Secret (per environment) stores a materialization-controller-written, Account-consumed versioned signing bundle only until non-exportable signer delegation is implemented:
...
Rotation evidence is collected by a Kubernetes `CronJob` template (for example `jwt-rotation`) and a dedicated service account (for example `sa-jwt-rotation`) with narrow authority to observe Account-owned rotation transitions and status, run validator-convergence probes, and write the dedicated `jwt-rotation-status` evidence resource. It has no authority to request a transition, no write access to `jwt-jwks`, no read or update access to `jwt-signing-keys`, and no `patch` authority on the Account Service Deployment. The materialization controller alone has name-scoped `get`, `update`, and `patch` authority over the signing Secret, with `get` used only to read the current `resourceVersion` for authenticated CAS; Account has no Secret API authority, consumes the bundle through its read-only projected mount, and has update/patch authority over the public JWKS ConfigMap. Both use operation-bound `resourceVersion` evidence and neither may list, create, or delete the pre-created resources. The single Account JWT rotation control/status interface carries the Account-owned requested transition, current phase/generation/publication status, and convergence-observation correlation; `jwt-rotation-status` carries the automation's immutable evidence record rather than a second rotation state machine. A planned rotation is an ordered state machine rather than two resource updates treated as atomic:
...
2. Account advances its accepted lifecycle transition by publishing the resulting public JWK alongside every still-valid old public key, carrying a common generation and pending-key phase across rotation resources. Rotation automation only observes that publication, probes validator convergence, and records operation-bound evidence.
...
- Production: the `jwt-rotation` CronJob is defined with `spec.suspend: true`. An explicit operator runbook requests the Account-owned transition through its control interface and creates a one-off Job from this template to observe convergence and retain evidence.
...
- Through the single Account JWT rotation control/status interface, Account publishes **only uncompromised public keys**. Do not retain a compromised key in any overlap or rollback slot during compromise response; automation observes the Account-published result rather than requesting or writing `jwt-jwks`.
...
Mounted file paths and a served JWKS document do not establish JWT readiness. Before any player-facing environment is described as promotable or traffic-open, startup and deployment gates must prove Account-only application signing, bounded materialization-controller custody, no private-key distribution to validators or rotation automation, `kid`/JWKS validation with HMAC fallback disabled, a successful planned-rotation drill through prune, a successful compromise hard-cutover drill, and retained validator-convergence evidence. Until those conditions are implemented and proved, the current shared-HMAC topology is non-production implementation debt rather than an alternate supported design.
