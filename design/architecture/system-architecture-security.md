...
Scripting uses one DSL/compiler/sandbox security boundary for embedded scripts and linked plugins. Provenance or plugin origin does not grant trust: plugin admission remains bound to its immutable artifact/version and the exact Game Session pin/epoch execution fence. The [DSL lifecycle reference](./system-architecture-scripting-dsl-reference-and-lifecycle.md) owns artifact/lifecycle semantics; this document owns only security, workload identity, and sandbox consequences.
