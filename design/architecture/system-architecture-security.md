...
- **Revocation/incident response**: if an operator credential is suspected compromised, quarantine/close the affected operator ingress while an emergency no-overlap `ROTATION` publishes a replacement-only active binding; the old leaf is rejected on publication/readback and is never a fallback. A terminal `REVOCATION` instead has empty active and overlap sets and accepts no replacement; later reissuance requires a separately authorized binding/issuance path. Tighten NetworkPolicy allowlists during response and retain the generation, exact leaf identities, and readback results. The [canonical target operator credential binding record](./system-architecture-deploy-preflight-policy.md#canonical-target-operator-credential-binding-record) owns the lifecycle state and evidence. **Current implementation/proof gap:** that record, endpoint-side generation/leaf-identity allowlist validation, and accepted/rejected readback evidence are not yet live in the repository, so the runbook remains target-state guidance.
