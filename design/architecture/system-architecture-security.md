...
FireMUD has one application-facing contract for exportable secrets: Kubernetes workloads consume narrowly scoped Kubernetes Secrets through fixed mounted-file or bounded `secretKeyRef` interfaces, while local tooling consumes equivalent read-only files outside the repository. FireMUD does not bundle or require Vault or another external secret-manager service; external infrastructure may populate canonical Kubernetes Secret names for current legacy wiring and other credentials. In the accepted player-facing custody modes, the reserved `jwt-signing-keys` Secret and its materialization-controller write/prune path are interim-only, while the public `jwt-jwks` ConfigMap is Account-owned in both `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK` and target non-exportable-signer mode and is written only through Account's documented name-scoped resource-version CAS contract. This does not create a second application mode. The target JWT custody mode is non-exportable: no application workload mounts or receives private signing material, Account delegates private-key operations to the signer, and Account remains the sole issuer, lifecycle, promotion, and public-JWKS authority under [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative). The fixed `jwt-signing-keys` Secret, its materialization-controller write/prune path, `FIREMUD_AUTH_JWT_SECRET_PATH`, and Account-mounted private material belong only to the interim mounted fallback. The checked-in runtime's current Secret-backed JWKS mode is described in the implementation status below and is not target readiness. The local/dev HMAC compatibility profile is outside the asymmetric signer-custody contract and is never player-facing or shared. Rotation automation is observation-only: it may read public JWKS and bounded validator metadata for convergence evidence, but has no private material, no read authority for the private signing Secret, no JWKS write authority, and no signer authority, including signer selection or promotion. High-impact credentials require explicit rotation SLAs, age/missed-rotation alerts, incident runbooks, and measurable compliance evidence.
...
- JWT profiles, issuance, registry, generation, signing, and JWKS authority are defined in [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). In target non-exportable-signer mode, no application workload consumes private signing material: Account delegates private-key operations to the signer, validators consume Account-owned public JWKS, and observation-only rotation automation (excluding the interim materialization controller) receives no private material or signer authority. In the interim mounted fallback, only Account consumes the private signing material; validators still consume public JWKS. HMAC compatibility profiles do not acquire this private-key custody rule.
...
- A **JWKS endpoint** exposes public keys for internal services. In current legacy mode, the configured path resolves to the externally provisioned `jwt-jwks` `Secret`; in `INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK` and target non-exportable signer mode, it resolves to the Account-owned `jwt-jwks` `ConfigMap` through `/var/run/secrets/firemud/jwks/jwks.json` selected by `FIREMUD_AUTH_JWKS_PATH`. JWKS never contains the private key, and classpath fallback is local/test only.
...
- JWT/JWKS Kubernetes permissions are mode-specific:
  - **Current legacy Secret-backed mode:** the checked-in `jwt-signing-keys` and public `jwt-jwks` resources are pre-created `Secret` objects supplied by external environment setup. The current baseline also mounts `jwt-signing-keys` beyond Account; that is implementation drift, not interim custody. The current `jwt-jwks` Secret has no Account-owned writer contract; current mounts and `PREFLIGHT-JWKS-001` are legacy wiring evidence only and do not imply interim or target permissions.
  - **`INTERIM_ACCOUNT_ONLY_MOUNTED_FALLBACK`:** the fixed `jwt-signing-keys` `Secret` is written by the materialization controller through name-scoped `get`, `update`, and `patch` authority; Account has no Kubernetes API authority for that private Secret and is the only application workload that mounts or uses it. The fixed public `jwt-jwks` `ConfigMap` is written only by Account through name-scoped `get`, `update`, and `patch` resource-version CAS authority. Neither actor has list, create, or delete authority for those pre-created resources.
  ...
  The materialization controller is an additional interim private-material custodian, not an issuer, signer selector, promoter, JWKS publisher, or pruning authority. It may execute only an Account-authorized private-slot pruning operation during reconciliation and returns only the authenticated CAS/pruning evidence required by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative). Rotation Jobs have no read/write authority over private signing material and no write authority over the Account-owned public `jwt-jwks` `ConfigMap`.
...
- External secret infrastructure may populate canonical Kubernetes objects or mounted paths other than the interim fallback's `jwt-signing-keys` Secret and the Account-owned `jwt-jwks` ConfigMap, whose contents may be written only through their documented materialization-controller and Account CAS contracts. FireMUD neither deploys external secret infrastructure nor makes workload readiness depend on its API. Synchronization should preserve already-materialized healthy workload secrets during an upstream outage.
...
In the current legacy mode, environment setup may bootstrap the pre-created reserved JWT objects described above, but it may not continue populating either object as a second writer. A custody-mode transition must fail closed if the documented one-writer boundaries cannot be established.
...
- JWT publication, generation, pruning, and validator convergence follow [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). Dedicated rotation Jobs are observation-only and exclude the interim materialization controller: Jobs may read public JWKS and bounded validator metadata solely for convergence evidence, but must not read private material or the `jwt-signing-keys` Secret, write the Account-owned public `jwt-jwks` `ConfigMap`, mutate signing state, or select or promote a signer. The controller retains only its bounded Account-authorized `jwt-signing-keys` Secret CAS/pruning execution role described above.
...
Rotation jobs and evidence must be observation-only, excluding the interim materialization controller's bounded Account-authorized `jwt-signing-keys` Secret CAS/pruning execution path described above. Jobs may read public JWKS and bounded validator metadata for convergence evidence, but must not read private signing Secrets or other private material, mutate signing state, write the Account-owned public `jwt-jwks` `ConfigMap`, or select or promote a signer. The exact lifecycle, convergence, overlap, pruning, and readiness evidence are owned by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md) and the environment-specific deployment docs.
...
Mounted file paths and a served JWKS document do not establish JWT readiness. In target non-exportable-signer mode, readiness requires signer health, Account-owned public-JWKS convergence, and proof that no private key is mounted or distributed. In the interim mounted fallback, readiness also requires private/public projection proof. The owner defines the signing, registry, generation, rotation, and validator-convergence proof; Security retains the local gate that validators receive no private key, HMAC fallback is disabled in player-facing environments, and uncertain trust convergence keeps traffic closed. Current shared-HMAC and fallback behavior remains implementation debt, not an alternate supported design.
...
| JWT Signer Custody | Target for asymmetric profiles: Account delegates private-key operations to non-exportable signer custody; interim asymmetric fallback: the materialization controller generates/CAS-writes `jwt-signing-keys` and executes private-slot pruning only when authorized by Account during reconciliation, Account is the only application workload that mounts or uses it, validators receive Account-published JWKS, and rotation Jobs receive no private material. HMAC compatibility remains local/dev only |
