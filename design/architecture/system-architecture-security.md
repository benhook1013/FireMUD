...
- JWT profiles, issuance, registry, generation, signing, and JWKS authority are defined in [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). Security's local custody consequence is that only Account consumes private signing material; validators consume public JWKS; and observation-only rotation automation may read public JWKS and bounded validator metadata but receives no private material or signer authority.
...
- During the interim fallback, Account may reload signing material when mounted files change; malformed or mismatched material is rejected and fails closed, quarantining player-facing issuance and protected traffic. A retained rollback signer is rollback material only and must not serve player traffic until the canonical rotation and trust-convergence proof passes. The file watcher is implementation plumbing, not a second rotation contract.
...
JWT rotation follows the owner contract and [ADR 0014](./decisions/adr-0014-phased-jwt-signing-key-rotation-and-readiness.md). Security must preserve non-exportable or Account-only private-key custody, fail closed during trust-boundary uncertainty, and retain operational evidence for player-facing readiness.

The local delivery boundary is one read-only private-key mount for Account through `FIREMUD_AUTH_JWT_SECRET_PATH`, public JWKS consumption by validators, and no private-key distribution to validators or rotation automation. Detailed mount/resource behavior remains in [Environment and Secrets Overview](./infrastructure/environment-and-secrets-overview.md#authentication--jwt).
