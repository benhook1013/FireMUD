...
- JWT publication, generation, pruning, and validator convergence follow [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). Dedicated Jobs are observation-only; they must not read or update `jwt-signing-keys` or write `jwt-jwks`.
...
Rotation jobs and evidence must be observation-only and must not mutate signing state, private signing Secrets, or public JWKS. The exact lifecycle, convergence, overlap, pruning, and readiness evidence are owned by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md) and the environment-specific deployment docs.
