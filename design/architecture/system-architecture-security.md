...
- A **JWKS endpoint** exposes public keys for internal services to validate tokens. The Account Service serves these keys at `/.well-known/jwks.json`. In player-facing environments, the fixed Account-owned `jwt-jwks` ConfigMap is mounted read-only at `/var/run/secrets/firemud/jwks`, and `FIREMUD_AUTH_JWKS_PATH` points to `/var/run/secrets/firemud/jwks/jwks.json`. Account alone updates that public resource through Kubernetes `resourceVersion` CAS. JWKS must never contain the Account private key. Classpath fallback is local/test only.
...
- The Account service account has name-scoped `get`, `update`, and `patch` authority only for the pre-created `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap. It has no list, create, or delete authority for those resource kinds; the rotation Job has no read/write authority over the private Secret and no write authority over public JWKS.
...
- External secret infrastructure may populate canonical Kubernetes objects or mounted paths other than the Account-owned `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap, whose contents may be written only through the Account-owned CAS contract. FireMUD neither deploys external secret infrastructure nor makes workload readiness depend on its API. Synchronization should preserve already-materialized healthy workload secrets during an upstream outage.
...
- The JWKS endpoint serves Account-published public material from the fixed-name `jwt-jwks` ConfigMap, whose contents change only through Account-owned resource-version compare-and-set, or from an equivalent integrity-controlled public artifact mounted read-only into Account. Account validates and CAS-updates the artifact through narrowly scoped authority, then observes the projected generation before signer promotion. The non-exportable signer performs only delegated private-key operations. Rotation automation requests publication or pruning through the Account control/status interface, proves validator visibility, and observes the result; it never reads or updates `jwt-signing-keys` or writes `jwt-jwks`.
...
Rotation is coordinated by a Kubernetes `CronJob` template (for example `jwt-rotation`) and a dedicated service account (for example `sa-jwt-rotation`) with narrow authority to request Account-owned rotation transitions and status, run validator-convergence probes, and write the dedicated `jwt-rotation-status` evidence resource. It has no write access to `jwt-jwks`, no read or update access to `jwt-signing-keys`, and no `patch` authority on the Account Service Deployment. Account's service account alone has name-scoped `get`, `update`, and `patch` authority over the two pre-created rotation resources and uses `resourceVersion` CAS; it cannot list, create, or delete resources of those kinds. The single Account JWT rotation control/status interface carries the requested transition, current Account-owned phase/generation/publication status, and convergence-observation correlation; `jwt-rotation-status` carries the automation's immutable evidence record rather than a second rotation state machine. A planned rotation is an ordered state machine rather than two resource updates treated as atomic:
...
- Treat compromise of the environment-wide Account signing key as global for that issuer. Atomically advance the issuer authority generation with the compromise event, invalidate the affected `session:auth:token:<tokenHash>` issued-token records, and require reauthentication; tenant-selective cleanup is not sufficient containment.
