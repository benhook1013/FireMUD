...
- A **JWKS endpoint** exposes public keys for internal services to validate tokens. The Account Service serves these keys at `/.well-known/jwks.json`. In player-facing environments, an immutable ConfigMap or equivalent integrity-controlled public artifact is mounted read-only at `/var/run/secrets/firemud/jwks`, and `FIREMUD_AUTH_JWKS_PATH` points to `/var/run/secrets/firemud/jwks/jwks.json`. JWKS must never contain the Account private key. Classpath fallback is local/test only.
...
- The JWKS endpoint serves Account-published public material from an immutable `jwt-jwks` ConfigMap or equivalent integrity-controlled public artifact. Rotation automation requests Account-owned publication or pruning through the single Account JWT rotation control/status interface, proves validator visibility, and observes the result. Account validates signer/public-key correspondence, promotes the signer, and remains the only authority that mutates public JWKS; the non-exportable signer performs only delegated private-key operations. Automation records evidence without reading or updating `jwt-signing-keys` or writing `jwt-jwks`.
