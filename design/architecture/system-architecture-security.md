...
- The target Account-issued asymmetric/JWKS boundary, phased rotation workflow, dedicated rotation-job automation, key-overlap/pruning operations, projected-volume reload proof, deployment-wide validator convergence, and player-facing readiness gate below remain target/operational design rather than completed live capability. The current player-facing Kustomize runtime reads a legacy shared-HMAC signing Secret and public `jwt-jwks` ConfigMap file while still permitting the documented classpath fallback drift; current deployment preflight requires the explicit ConfigMap binding, non-empty data, and Account path/mount as a diagnostic-only wiring check. Neither result proves target ConfigMap publication authority or validator convergence. This document does not change runtime, preflight, or manifest behavior.
...
- **Checked-in current deployment mode: legacy Secret-backed signing plus public ConfigMap JWKS wiring.** The current player-facing Kustomize runtime uses `jwt-signing-keys` as a Secret and `jwt-jwks` as a ConfigMap, with the shared-HMAC and classpath-fallback drift described above. The executable's `PREFLIGHT-JWT-001` checks legacy private path/mount wiring, while `PREFLIGHT-JWKS-001` checks the explicit diagnostic ConfigMap binding and public path/mount shape. These diagnostic results are not custody proof or player-facing readiness evidence.
...
- A **JWKS endpoint** exposes public keys for internal services. In the checked-in current deployment mode, the configured path resolves to the externally provisioned public `jwt-jwks` `ConfigMap`. In the interim mounted fallback, it resolves to the separate interim Account-owned `jwt-jwks` `ConfigMap` projection; in target non-exportable signer custody, it resolves to the separate target Account-owned `jwt-jwks` `ConfigMap` projection. Both accepted projections use `/var/run/secrets/firemud/jwks/jwks.json` selected by `FIREMUD_AUTH_JWKS_PATH`; the current ConfigMap is diagnostic-only and externally provisioned. JWKS never contains the private key, and classpath fallback is local/test only.
...
- JWT/JWKS Kubernetes permissions are mode-specific:
  - **Current legacy Secret-backed mode:** the checked-in `jwt-signing-keys` Secret and public `jwt-jwks` ConfigMap are pre-created resources supplied by external environment setup. The current baseline also mounts `jwt-signing-keys` beyond Account; that is implementation drift, not interim custody. The current `jwt-jwks` ConfigMap has no Account-owned writer contract; current mounts and `PREFLIGHT-JWKS-001` are legacy wiring evidence only and do not imply interim or target permissions.
