...
- Internal service-to-service **gRPC** traffic is authenticated using **mTLS** with cert-manager-issued certificates. The documented Gateway-to-backend HTTP/WebSocket hops remain explicit in-cluster exceptions and retain their existing Gateway trust, header-canonicalization, and NetworkPolicy controls. Current implementation drift is that Gateway REST listens with the default plaintext HTTP configuration and forwards credential-bearing requests (including `Authorization`) over in-cluster `http://` routes; this is not target HTTPS/mTLS or traffic-open/promotion proof. The target Gateway management plane uses HTTPS with its dedicated operator mTLS binding, while any remaining in-cluster HTTP exception must stay explicitly bounded and non-authorizing by itself.
...
- Among application microservices, the **Spring Cloud Gateway** and **TCP Proxy Service** reside in the **network DMZ** and serve as the only ingress points for client traffic. Separately, the target public site router/static frontend host serves frontend documents and `/frontend-assets/**`, while the published `/assets/**` family is a distinct read-only asset-delivery boundary (Gateway- or CDN-fronted); these delivery surfaces are not additional application-microservice ingress paths and remain governed by their own contracts.
...
- **Credential/login abuse controls (Account Service)**:
  ...
  - Reserves `security_locked` for a policy-driven compromise assessment only when Account has evaluated verified or high-confidence evidence after sufficient identity proof, or for a separately audited operator break-glass action; either basis triggers revocation and recovery, while failed-attempt thresholds remain temporary throttles. Public failure behavior avoids account enumeration.
...
- Product admin functionality (creator/moderator APIs exposed via Spring Cloud Gateway) uses Account-issued exact-profile JWTs as the authentication input; the consuming owner enforces the route's exact profile/issuer/audience, matching issued-token registry/freshness, live role/scope/generation, and any other route-specific predicates.
...
- Operator control-plane diagnostics are treated separately from product admin APIs: they are **internal-only** and require mutual TLS (mTLS) client certificates, with reachability restricted by `ClusterIP` Services, private ingress, and `NetworkPolicy` allowlists. Generic Gateway route-mutation components and endpoints are dev/test-only and absent or disabled in player-facing environments; production route changes use the separately accepted declarative deployment workflow, so the production operator surface remains diagnostics-only.
  - **Certificate trust root**: operator clients for gRPC management endpoints must resolve the CA bundle and client identity from the selected `operator-credential-binding/v1` record according to its binding type: a Secret-backed binding reads its named dedicated operator Secret; a cert-manager binding resolves the named Certificate/issuer binding and its declared Secret output; and a workload-identity binding resolves the platform-projected client identity and trust bundle from that binding. Missing, unknown, or mismatched source type fails closed. The service `FIREMUD_GRPC_*` paths remain internal workload gRPC TLS identity and peer-trust configuration, not operator-tool inputs. Gateway HTTP management tooling uses its dedicated listener trust bundle `FIREMUD_GATEWAY_HTTP_CA_CERT_PATH` and client identity `FIREMUD_GATEWAY_HTTP_CLIENT_CERT_CHAIN_PATH` / `FIREMUD_GATEWAY_HTTP_CLIENT_PRIVATE_KEY_PATH`; those paths are not the Gateway service's `FIREMUD_GRPC_*` workload identity.
  - **Client certificate profile**: operator certificates must include the `clientAuth` extended key usage and the required operator URI SAN/profile. A Secret-backed or cert-manager binding should use a dedicated Kubernetes Secret that is not mounted by normal workloads; a workload-identity binding uses the provider's projected identity instead.
  - **Distribution and access**: Kubernetes RBAC and Secret scoping must restrict which service accounts can read/mount a Secret-backed or cert-manager output Secret; workload-identity provider policy must restrict which approved operator tool or workload can obtain the projected identity. NetworkPolicies restrict which pods can reach the management endpoints so that holding a valid certificate alone is not sufficient outside the approved operator surface.

### Operator Client Certificate Lifecycle

Operator control-plane access relies on mTLS client certificates. To keep this access auditable and revocable, each target operator endpoint validates the CA chain and network placement plus the current environment-owned bounded operator credential generation/allowlist. The accepted leaf identity must exact-match the generation's fingerprint and required URI SAN/profile; a valid certificate from the CA alone is insufficient. The binding representation and environment selection remain owned by the canonical [target operator credential binding record](./system-architecture-deploy-preflight-policy.md#canonical-target-operator-credential-binding-record), selected through the [deployment preflight operator-credential rules](./system-architecture-deploy-preflight-policy.md#operator-credential-representation-rule), not by a second endpoint-local configuration.

- **Issuance**: A `SECRET_BACKED` operator binding resolves the selected existing operator-managed Secret and its file-backed client material without invoking cert-manager; a `CERT_MANAGER` binding issues the client certificate through the named issuer/profile into its dedicated output Secret, with `clientAuth` EKU; and a `WORKLOAD_IDENTITY` binding obtains the provider-projected client identity and trust bundle without a substitute certificate or Secret. Operator certificates are not reused as ordinary workload identities. Initial issuance creates generation 1 with `operationKind=INITIAL_ISSUANCE`, the exact active leaf, URI SAN/profile, endpoint/workload audience, and no overlap; immutable readback must prove new acceptance and no previous leaf, as defined by the canonical [operator credential binding record](./system-architecture-deploy-preflight-policy.md#canonical-target-operator-credential-binding-record).
- **Storage**: a Secret-backed or cert-manager binding stores the operator client certificate and private key in its named dedicated Kubernetes Secret, readable/mountable only by the minimal approved operator-facing tools; a workload-identity binding uses provider-projected material and does not require a substitute Secret.
- **Rotation**: operator certificates are rotated on a fixed cadence (or immediately after personnel/device changes). A routine rotation may use the bounded overlap represented by the canonical `operator-credential-binding/v1` record, in which the current and replacement exact leaf identities are accepted only before the record's `overlapExpiresAt` deadline. Endpoint validators reject the old overlap identity at or after that deadline, and the owner must atomically advance the environment binding generation and remove the old fingerprint/SAN identity no later than expiry. Completion requires readback proving the replacement is accepted and the old identity is rejected; updating the Secret alone is not revocation. `INITIAL_ISSUANCE` and `REVOCATION` retain an empty overlap set and no overlap deadline.
- **Revocation/incident response**: if an operator credential is suspected compromised, rotate immediately with no post-incident overlap: atomically advance the binding generation and remove the compromised fingerprint/SAN identity, then verify old rejection and replacement acceptance. Tighten NetworkPolicy allowlists during response, and retain the generation, exact leaf identities, and both readback results in the operator/recovery evidence. **Current implementation/proof gap:** the canonical binding record, endpoint-side generation/leaf-identity allowlist validation, and accepted/rejected readback evidence are not yet live in the repository, so the runbook remains target-state guidance.
