...
- The Account service account has name-scoped `get` authority only for the pre-created `jwt-signing-keys` Secret and name-scoped `get`, `update`, and `patch` authority for the `jwt-jwks` ConfigMap. The materialization controller alone has name-scoped `update` and `patch` authority for `jwt-signing-keys`. Neither actor has list, create, or delete authority for those pre-created resources; the rotation Job has no read/write authority over the private Secret and no write authority over public JWKS.
