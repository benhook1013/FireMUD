...
- Treat compromise of the environment-wide Account signing key as global for that issuer. Atomically advance the issuer authority generation with the compromise event and globally invalidate every `session:auth:token:<tokenHash>` issued-token record whose issuer generation is prior to the new generation, regardless of account, tenant, or token profile. Require reauthentication; tenant-selective cleanup is not sufficient containment.
