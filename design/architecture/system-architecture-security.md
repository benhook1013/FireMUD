...
- Signing keys are stored as **Kubernetes Secrets** under the phased protocol in [ADR 0014](./decisions/adr-0014-phased-jwt-signing-key-rotation-and-readiness.md). Account Service or a non-exportable signer owns private-key generation, validation, promotion, and private rollback pruning. Rotation automation handles public JWKS metadata, validator convergence, public pruning, and evidence, and must never read or update `jwt-signing-keys`. This storage baseline remains subject to the separate secret-storage decision.
...
- JWT public JWKS metadata is coordinated by dedicated Kubernetes Jobs, while Account Service or a non-exportable signer owns private-key generation, validation, promotion, and private rollback pruning. The rotation Job/CronJob must never read or update `jwt-signing-keys`. See [JWT Key & JWKS Rotation Workflow](#jwt-key--jwks-rotation-workflow) and [Environment Variables & Secrets Management](./infrastructure/environment-and-secrets.md) for how Account and validators consume these resources.
...
- The JWKS endpoint serves a key file that is mounted into the Account Service pod (from a `jwt-jwks` Secret in player-facing environments). Rotation automation prepublishes public keys and proves validator visibility; Account or the non-exportable signer validates and promotes the matching private signer. Automation then prunes public keys and records evidence without reading or updating `jwt-signing-keys`.

### JWT Key & JWKS Rotation Workflow

JWT public JWKS metadata and rotation evidence are coordinated inside the Kubernetes cluster using dedicated Jobs, while Account Service or a non-exportable signer owns private-key generation, validation, promotion, and private rollback pruning. [ADR 0014](./decisions/adr-0014-phased-jwt-signing-key-rotation-and-readiness.md) defines the accepted rotation and readiness boundary. The goals are:
...
- A `jwt-signing-keys` Secret (per environment) stores an Account-only versioned signing bundle:
...
Rotation is coordinated by a Kubernetes `CronJob` template (for example `jwt-rotation`) and a dedicated service account (for example `sa-jwt-rotation`) with narrow RBAC for public `jwt-jwks` metadata, validator-convergence probes, public-key pruning, evidence, and optional `patch` on the Account Service Deployment for rollout annotations. The service account and its Job/CronJob must have no read or update access to `jwt-signing-keys`; private operations remain inside Account Service or a non-exportable signer. A planned rotation is an ordered state machine rather than two resource updates treated as atomic:

1. Account Service or a non-exportable signer generates a new asymmetric keypair and unique `kid` without changing the active signer. The private key never enters rotation automation.
2. Rotation automation prepublishes the resulting public JWK alongside every still-valid old public key, carrying a common generation and pending-key phase across rotation resources.
3. Wait at least the configured validator JWKS maximum cache age and actively prove that every required validator can resolve the pending `kid`.
4. Account Service or a non-exportable signer validates the private key, `kid`, and matching published JWK, then promotes the new Account signer. Signer promotion is the commit point; a malformed bundle or mismatch leaves the old signer active and fails readiness.
...
6. Rotation automation prunes the retired public key and proves active-key acceptance and retired/expired-key rejection. Account Service or the non-exportable signer separately prunes any rollback private key. Automation records immutable rotation evidence and never reads or updates `jwt-signing-keys`.
...
- Restore mode must quarantine JWT issuance and JWT-protected traffic, publish only fresh uncompromised keys in JWKS, invalidate environment-wide issuer authority, and require validator-convergence evidence before traffic reopen.
...
- Immediately run compromise-mode key rotation through Account Service or a non-exportable signer to generate and promote a new private signing generation. Rotation automation must not read or update `jwt-signing-keys`.
- Publish `jwt-jwks` with **only uncompromised public keys**. Do not retain a compromised key in any overlap or rollback slot during compromise response.
