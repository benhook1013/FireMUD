...
- Account Service remains the sole JWKS publication authority. Dedicated Kubernetes Jobs request Account-owned public-key transitions, prove validator convergence, and record evidence; the rotation Job/CronJob must never read or update `jwt-signing-keys` or publish `jwt-jwks`. See [JWT Key & JWKS Rotation Workflow](#jwt-key--jwks-rotation-workflow) and [Environment Variables & Secrets Management](./infrastructure/environment-and-secrets.md) for how Account and validators consume these resources.
...
- The JWKS endpoint serves a key file that is mounted into the Account Service pod (from a `jwt-jwks` Secret in player-facing environments). Rotation automation requests Account-owned publication of public keys, proves validator visibility, and observes the Account-published result; Account or the non-exportable signer validates and promotes the matching private signer and owns public-key pruning. Automation records evidence without reading or updating `jwt-signing-keys`.
...
Rotation is coordinated by a Kubernetes `CronJob` template (for example `jwt-rotation`) and a dedicated service account (for example `sa-jwt-rotation`) with narrow authority to invoke Account-owned rotation operations, run validator-convergence probes, and write the dedicated `jwt-rotation-status` status/evidence resource. It has no write access to `jwt-jwks`, no read or update access to `jwt-signing-keys`, and no `patch` authority on the Account Service Deployment; rollout/restart remains under Account or normal deployment control. The status/evidence resource carries the phase, generation identifiers, convergence results, and immutable evidence reference instead of deployment annotations. A planned rotation is an ordered state machine rather than two resource updates treated as atomic:
