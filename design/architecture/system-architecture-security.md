...
- JWT profiles, issuance, registry, generation, signing, and JWKS authority are defined in [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). For asymmetric profiles, Security's local custody consequence is that only Account consumes private signing material; validators consume public JWKS; and observation-only rotation automation may read public JWKS and bounded validator metadata but receives no private material or signer authority. HMAC compatibility profiles do not acquire this private-key custody rule.
...
- Each workload receives only the credentials required by its exact identity and function. In particular, for asymmetric JWT profiles Account is the only application workload that mounts or uses the Account JWT private key, and every mTLS workload has a distinct private identity.
...
The local delivery boundary for asymmetric profiles is one read-only private-key mount for Account through `FIREMUD_AUTH_JWT_SECRET_PATH`, public JWKS consumption by validators, and no private-key distribution to validators or rotation automation. Detailed mount/resource behavior remains in [Environment and Secrets Overview](./infrastructure/environment-and-secrets-overview.md#authentication--jwt).
