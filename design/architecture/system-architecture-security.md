...
FireMUD has one application-facing contract for exportable secrets: Kubernetes workloads consume narrowly scoped Kubernetes Secrets through fixed mounted-file or bounded `secretKeyRef` interfaces, while local tooling consumes equivalent read-only files outside the repository. FireMUD does not bundle or require Vault or another external secret-manager service; operators may populate canonical Kubernetes Secret names from external infrastructure without creating a second application mode. In the target JWT resource mode, the reserved `jwt-signing-keys` Secret and `jwt-jwks` ConfigMap have no external-population path: the materialization controller and Account are their sole writers, respectively. The checked-in runtime's current Secret-backed JWKS mode is described in the implementation status below and is not target readiness. For asymmetric JWT profiles, [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative) assigns Account ownership of desired key-ring state and pruning decisions while Account remains the sole issuer, promotion, and JWKS-publication authority. The local/dev HMAC compatibility profile is outside that asymmetric signer-custody contract and is never player-facing or shared. The controlled interim asymmetric fallback uses a materialization-controller-written Secret consumed only by the Account application workload. Rotation automation is observation-only: it may read public JWKS and bounded validator metadata for convergence evidence, but has no private material, no read authority for the private signing Secret, no JWKS write authority, and no signer authority, including signer selection or promotion. High-impact credentials require explicit rotation SLAs, age/missed-rotation alerts, incident runbooks, and measurable compliance evidence.
...
- For the interim mounted fallback, the Account service account has no Kubernetes API authority for the pre-created `jwt-signing-keys` Secret and has name-scoped `get`, `update`, and `patch` authority only for the `jwt-jwks` ConfigMap. The materialization controller alone has name-scoped `get`, `update`, and `patch` authority for `jwt-signing-keys`; this makes it an additional infrastructure custodian even though its implementation uses `get` only to obtain the current `resourceVersion` for CAS. It may execute only an Account-authorized private-slot pruning operation during reconciliation; it may not independently decide that pruning is due, choose private slots, or mutate desired key-ring state. It returns only the authenticated CAS/pruning execution evidence required by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#signing-key-rotation-contract-normative) for Account reconciliation; that canonical contract owns the exact request, authority, and evidence contract. It may not sign tokens, select or promote a signer, publish JWKS, expose key material, or use the key for any other purpose. Neither actor has list, create, or delete authority for those pre-created resources; the rotation Job has no read/write authority over the private Secret and no write authority over public JWKS.
...
- JWT publication, generation, pruning, and validator convergence follow [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). Dedicated Jobs are observation-only: they may read public JWKS and bounded validator metadata solely for convergence evidence, but must not read private material or the `jwt-signing-keys` Secret, write `jwt-jwks`, mutate signing state, or select or promote a signer.
...
Rotation jobs and evidence must be observation-only. Jobs may read public JWKS and bounded validator metadata for convergence evidence, but must not read private signing Secrets or other private material, mutate signing state, write public JWKS, or select or promote a signer. The exact lifecycle, convergence, overlap, pruning, and readiness evidence are owned by [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md) and the environment-specific deployment docs.
