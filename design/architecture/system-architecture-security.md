...
Browser-authenticated state-changing requests that rely on cookies require `Secure`, `HttpOnly`, `SameSite=Strict` cookies, a valid anti-CSRF token in the designated request header, and an `Origin` that exactly matches the configured first-party allowlist. Missing, malformed, mismatched, or unallowlisted `Origin` or CSRF proof fails closed; `SameSite`, `Referer`, user-agent classification, or a caller-supplied header alone is not sufficient. This policy includes `POST /ws/game/connect-token/revoke`, the exact Gateway route for revoking the HttpOnly gameplay connect token, and browser gameplay WebSocket admission. It does not weaken the separate authenticated transport boundaries for non-browser clients or Telnet.
