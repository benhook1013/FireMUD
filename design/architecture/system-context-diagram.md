...
For canonical room-state reads, Game Session mints the read fence used to join World Management occupancy with Entity Management containment/presentation. World and Entity must either satisfy that fence or reject the read; mixed-tick best-effort room assembly is not allowed in the high-level architecture contract.
