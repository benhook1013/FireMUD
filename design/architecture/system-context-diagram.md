...
- External admin/creator UIs call services through Gateway, not Logging & Admin directly, so Gateway classifies routes, checks required `Authorization` header presence, and applies routing, coarse route protections, and rate limiting. Consuming services validate JWTs and enforce fine-grained authorization, while infrastructure-management traffic uses the separate Gateway-management authentication contract.
...
- Canonical room-state reads: Game Session allocates the causal floor from durable region commit authority when it invokes `ResolveLook`, and Game Logic propagates that floor unchanged to World Management and Entity Management. Each participant returns a scoped `servedThroughTickId` plus an opaque component version; same-scope/epoch responses with `servedThroughTickId >=` the requested `committedTickId` are accepted, while missing, behind-floor, or mixed-scope/epoch responses are rejected or retried. Opaque component versions are not directly compared; Game Logic owns composition and Game Session renders/caches the resulting transcript. The current request/proto paths remain floor-free and expose only scope markers.
