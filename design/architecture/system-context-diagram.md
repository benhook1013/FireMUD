...
- Canonical room-state reads: World Management supplies the causal floor used to align room occupancy with Entity Management containment/presentation. Game Logic owns composition for the same room scope and epoch, accepts participant versions at or beyond the floor (including bounded newer skew), and rejects or retries missing, behind-floor, or mixed-scope/epoch responses; Game Session initiates the read and renders/caches the resulting transcript.
