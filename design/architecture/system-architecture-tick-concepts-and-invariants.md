...
- **Target-state automation identity:** Script-generated child dispatch persists the complete Command-Handoff Identity separately from the effect ledger/storage projection. Its child record retains the complete applicable parent Trigger Identity for correlation and carries the command-level `(automationDispatchId, commandOrdinal)` discriminator in its handoff identity; `scriptEventId` alone cannot distinguish fan-out commands. `effectKey` may encode that pair only as deterministic descriptor/correlation metadata and scheduler tie-break metadata. It never replaces the root `EffectId`, typed operation, exact target aggregate, immutable request digest, sealed participant/manifest context, or digest-conflict enforcement.
