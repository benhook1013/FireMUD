...
The exact `scriptPinEpoch` propagation and final same-version old-epoch rejection described above are now enforced for the live instance-bound Automation handoff and final Game Session execution. Complete source/target remote tuple binding and broader target command identity remain target-state gaps; the target invariant remains required at those boundaries.
...
The identifier glossary owns the stable root `EffectId` for a tick-driven mutation. A root mutation uses the root `EffectId`; a generated child retains its persisted deterministic child `EffectId` as `effectId` and the enclosing root as `rootEffectId` lineage and reconciliation context. A concrete ledger/guard/storage projection may include the namespace-complete tuple `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectKey, targetAggregateType, targetAggregateId)` plus `effectId` and `rootEffectId` for scope, ordering, target lookup, and lineage, but that tuple is a storage projection rather than the mutation identity. `playableStateScope` is separately persisted and exact-validated evidence, not a replacement for `playableStateNamespaceId`; `regionEpoch` and `tickId` are ordering/timeline metadata, not effect-guard uniqueness dimensions. The ADR-level `targetAggregateIdentity` is represented by the target aggregate type and ID (or an equivalent explicitly proven collision-safe encoding). Participant guard logical uniqueness uses the owner-declared runtime-family partition plus the persisted mutation `EffectId`—the root `EffectId` for root operations and the child `EffectId` for generated children—typed operation, and target aggregate: S1/S2 uses `(tenantId, playableStateNamespaceId)`, while only an explicitly owner-classified S3 family uses `(tenantId, gameInstanceId)`; unknown or unclassified families fail closed. Additional physical partitioning used only for storage placement cannot alter this logical identity or deduplication. The immutable request digest is bound to the logical identity, while durable outcome/evidence state is mutable guard-row state protected by CAS. A bare tick tuple or bare `effectKey` is not sufficient for that guard contract. Ordinary retries and replay preserve the persisted mutation identity, original participant ledger projection, enclosing root lineage, and collision-safe participant-guard identity; retry eligibility/attempt metadata is separate and must not mutate the original `regionEpoch`, `tickId`, `effectKey`, or effect/typed-operation/target projection. A future linked-attempt or supersession model requires an explicit contract. A fresh root is allowed only for a **post-abandon re-drive** after conclusive terminal `ABANDONED` plus source-claim terminalization, later coordinate allocation, and durable lineage. The deterministic allocator and versioned fan-out binding in the pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) remain conditional and non-authoritative; they do not change this guard contract.
...
- The live gameplay-command staging path requires a present, nonblank durable `commandId` before Redis enqueue or durable batch staging. Malformed pre-existing entries fail closed before staging and require owner reconciliation. No-ID behavior is not valid for tick work; any explicitly non-tick/control-plane path, if one exists, must define its own identity contract.
...
- **Current-live fallback:** The Game Session handoff does not yet carry `commandOrdinal` or the full Trigger Identity. `TickStagingService` currently derives `effectKey` from `commandId`, falling back to a hash of command text plus the staging slot when no command id is available. This fallback is selection metadata only: durable tick batching rejects any queued entry without a `commandId` before staging or effect persistence. It is not the target-state automation identity and must not be documented as though `(automationDispatchId, commandOrdinal)` were enforced live.
...
- For the complete expected concrete participant-projection set for each logical mutation, including each namespace-complete `(effect_id, root_effect_id, typed_operation, tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, region_epoch, tickId, effectKey, targetAggregateType, targetAggregateId)` projection keyed by its persisted mutation `EffectId`—the root `EffectId` for a root mutation or the child `EffectId` for a generated child—with any enclosing root retained separately as lineage and reconciliation context, every expected projection must eventually have exactly one **terminal** outcome in PostgreSQL (`APPLIED` or `ABANDONED`) when the existing evidence policy permits terminalization. `playableStateScope` is exact-validated evidence, and `region_epoch`/`tickId` are ordering metadata; neither is a participant guard uniqueness dimension. Reconciliation must reject missing, extra, partial, or conflicting projections rather than treating one mutation row or root row as sufficient, even if:
...
- Any work that cannot be safely replayed after Redis loss or tick re-execution must be:
  ...
  - Intentionally marked `ABANDONED` in the tick effect ledger with a precise reason only when durable evidence proves it was not applied and the existing policy permits terminalization (for example, reset scopes as described in the failures and operations doc). Inconclusive old-epoch work remains non-terminal reconciliation-required under its original persisted mutation `EffectId`, with any enclosing `rootEffectId` retained separately as lineage and reconciliation context; reset scope alone does not authorize bulk terminalization.
...
Generated effects are bounded by a shared bootstrap depth ceiling (`8`), deterministic root-chain count and cost budgets, and a deterministic per-target cap. Platform hard ceilings bound every setting; operators and authored features may lower them but may not raise them. Each child records immutable parent/root identity, `depth = parent.depth + 1`, and a digest-covered deterministic child ordinal. The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is not current target state.
...
- During a solo-budget tick, health and alerting use the solo-derived metric-family signature (non-executable notation) (`tick_execution_time_ms_*{scope_class,tick_mode="solo"} / on (scope_class,tick_mode) label_replace(solo_lock_ttl_ms{scope_class},"tick_mode","solo","scope_class",".*")`) for that tick rather than the normal `tick_lock_ttl_ms` denominator. Normal samples use `tick_mode="normal"` and `label_replace(tick_lock_ttl_ms{scope_class},"tick_mode","normal","scope_class",".*")`; `tick_mode` is a closed bounded metric label, never a command or runtime identity. The explicit mode match preserves normal and solo series as distinct outputs.

These mode-aware metric families and formulas are target-only and currently unavailable from the live Game Session producer. Brace-form references in this section are metric-family signatures (non-executable notation) unless they include explicit bounded selectors; they must not be copied as queries with unbounded label matching. If any required series is absent or stale, treat the metric and any derived ratio as unknown/unavailable; current decisions use authoritative runtime-health/control-plane evidence through `GetRuntimeOwnershipStatus` with `ObserveRuntimeTickProgress`. Target-state deployments use `RegionStatus` through `GetRegionTickStatus` once that surface is available.
...
Runtime health pressure is also detected via metric-family signatures (non-executable notation) such as `tick_execution_time_ms_p95{scope_class,tick_mode="normal"} / on (scope_class,tick_mode) label_replace(tick_lock_ttl_ms{scope_class},"tick_mode","normal","scope_class",".*")` or `tick_execution_time_ms_p99{scope_class,tick_mode="normal"} / on (scope_class,tick_mode) label_replace(tick_lock_ttl_ms{scope_class},"tick_mode","normal","scope_class",".*")` for normal ticks; an admitted solo-budget tick uses `tick_execution_time_ms_p95{scope_class,tick_mode="solo"} / on (scope_class,tick_mode) label_replace(solo_lock_ttl_ms{scope_class},"tick_mode","solo","scope_class",".*")` or `tick_execution_time_ms_p99{scope_class,tick_mode="solo"} / on (scope_class,tick_mode) label_replace(solo_lock_ttl_ms{scope_class},"tick_mode","solo","scope_class",".*")`. These mode-aware ratios are actionable only when the target metric capability is advertised and proven and every required mode-specific series is present and fresh; otherwise the ratios are unknown/unavailable and current decisions must use authoritative runtime-health/control-plane evidence. When available, they remain class-level Prometheus rollups for detection and escalation, not per-region status or actionability:
