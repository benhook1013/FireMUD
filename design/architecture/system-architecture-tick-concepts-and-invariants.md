...
- **Current-live automation identity:** The Game Session handoff does not yet carry `commandOrdinal` or the full Trigger Identity. Live gameplay staging requires a present, nonblank durable `commandId`, and `TickStagingService` derives `effectKey` as `command:<commandId>`. The target-state exact pin tuple and `automationDispatchId` plus `commandOrdinal` effect identity are implementation gaps, not current behavior.
