...
These formulas are safe bootstrap defaults, not permanent production evidence. Game Session owns one shared resolver used by every consumer; individual services must not define alternate derivations. The resolver accepts deployment-specific bounds and evidence, validates positive values and declared relationships, and does not impose universal numeric minimums or maximums. `tick_interval_ms` is gameplay cadence, configured only at its declared game/operator levels within caps and fixed for one live epoch. `tick_budget_ms` and `lock_ttl_ms` are operator safety settings whose production values are calibrated from p95/p99 execution, participant RPC latency/error, GC and scheduler pauses, cleanup lag, representative backlog, recovery objectives, and fault tests. Lower-level overrides are accepted only when that individual key declares eligibility and still satisfy the applicable deployment policy.
