...
- A fresh Coordination Redis keyspace reset, including a destructive full-deployment/AOF reset that produces an empty keyspace, is permitted only after the protected Account authority/token cutover and replay-domain quarantine/fence have completed and their immutable pre-wipe evidence has been recorded. The pre-wipe external evidence is limited to the canonical immutable handoff fields: old deployment identity, intended replacement identity, authorized operator, action, timestamp/time, tooling digest, authorization, exact resolved scope, and old-deployment fencing, all bound to the same durable `operationId` and maintenance-lock digest or opaque reference; post-reset replacement facts are not required or available at this gate. It is the external AOF/deployment-reset phase in the canonical sequence, never an early pause/reset step; the external handoff is part of the same durable operation, is not authorized by an empty keyspace, and does not add another public continuation phase.
- After the replacement starts, the operator records separate immutable `post_reset_replacement_verification` containing replacement endpoint identity, ACL/configuration, empty-keyspace proof, and health after startup. The controller validates both evidence groups against the same `operationId`, exact resolved scope, and maintenance-lock digest or opaque reference derived from the same server-issued `maintenanceLockToken` before the internal recovery worker proceeds to `ready_to_reopen`; it validates them again as part of the public `resume(... expectedPhase=awaiting_resume ...)` gate. The internal post-reset smoke-check phase proves the new epoch can acquire leases, stage work, converge, and clean up correctly, proves replay-domain quarantine/fencing and durable consume acknowledgement where the reset affects replay admission, and for tenant/cluster scopes samples the required representative regions only after the Account projection and exact-token gates pass. Protected admission remains closed through those projection, token, replacement-verification, and smoke gates; the operation then atomically enters `ready_to_reopen` without dropping the maintenance lock or traffic fence. Its evidence must include the completed Account projection rebuild and any required session-schema-cleanup completion evidence. Only the public `continueRecovery(... expectedPhase=ready_to_reopen ...)` transition may advance it to `AWAITING_RESUME`.
...
- If the workflow aborts before entering `PARTIAL_RELEASE_RECONCILING`, operators may use the audited `coordination-maintenance release-lock ...` control rather than inventing an alternate unlock sequence. Once partial-release reconciliation begins, `release-lock` is prohibited; remediation continues through the same durable operation until containment and every release postcondition have durable successful observation evidence.
...
2. Before destructive reset, the operation establishes the protected Account authority/token cutover and replay-domain quarantine/fence, records their immutable evidence, and verifies that the cutovers are bound to the same operation and scope. It also records one immutable pre-wipe handoff record containing the old deployment identity, intended replacement identity, authorized operator, action, timestamp/time, tooling digest, authorization, exact resolved scope, and old-deployment fencing, with every field bound to the same durable `operationId` and maintenance-lock digest or opaque reference; the plaintext token is never recorded. An AOF wipe must not occur before `scope_paused_and_locked` and these same-operation/same-lock fences and evidence exist.
...
4. The replacement starts with the required empty keyspace and protected credentials/ACLs. After startup, the operator records a separate immutable `post_reset_replacement_verification` containing independent verification of endpoint identity, ACL/configuration, empty-keyspace state, and deployment health after replacement starts. Redis key absence alone is not the pause or ownership proof.
5. The operator binds both evidence groups to the same durable `operationId`, exact resolved scope, and maintenance-lock digest or opaque reference derived from the same server-issued `maintenanceLockToken`. The controller validates `scope_paused_and_locked`, the protected-domain cutover evidence, the immutable pre-wipe authorization/fencing evidence, and the post-reset replacement verification before its internal recovery worker resumes rebuild phases; it never reconstructs the operation or releases the fence from the new empty keyspace. This handoff is durable operation evidence, not an additional public `continueRecovery` phase.
...
6. Perform the storage-level reset only in the external AOF/deployment reset handoff after `scope_paused_and_locked`, the protected-domain cutovers, replay fencing, and immutable pre-wipe authorization/fencing evidence are established for the same operation and lock, by stopping Redis, deleting or recreating the AOF volume, and restarting Redis with the desired AOF configuration. Record post-reset replacement verification only after the replacement starts. The earlier internal reset phase may perform only scope-safe cleanup; a full-AOF deletion must never precede the protected-domain cutovers or be treated as their authorization boundary.
