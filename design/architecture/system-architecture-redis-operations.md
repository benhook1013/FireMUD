...
| --- | --- | --- | --- | --- |
| Decrease JWT/session TTL without reset | Lower `FIREMUD_AUTH_JWT_EXPIRATION_MS` and/or `FIREMUD_AUTH_SESSION_SAFETY_MARGIN_MS`, roll out services. | New `session:game:<tenantId>:<sessionId>` and `session:auth:<tenantId>:<tokenHash>` keys get shorter TTLs; existing keys keep their original TTL until they expire naturally. | Existing sessions continue until their original TTLs; new logins and reconnects enforce the tighter lifetime immediately. | Not required. Operators may optionally run per‑tenant session cleanup (delete `session:game:<tenantId>:*`) to accelerate convergence if memory is tight. |
