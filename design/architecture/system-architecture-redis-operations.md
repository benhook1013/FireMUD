...
- Coverage carriers use `coverageGeneration=NOT_YET_ISSUED` from child creation through pre-coverage readback and assign a concrete generation only in the atomic coverage-proven transition. A linked child result is parent-independent; the parent tuple remains only on the independent parent edge and the parent's exact consumption record. A flow that acquired account-wide fencing retains `historicalAccountAdmissionFence` after releasing the live fence and never changes that arm to `NOT_APPLICABLE`; only flows that never acquired account-wide fencing carry `accountAdmissionFence=NOT_APPLICABLE`, `historicalAccountAdmissionFence=NOT_APPLICABLE`, and `NO_ACTIVE_ACCOUNT_WIDE_FLOW`.
