...
- **reset-sensitive**
  - gameplay session prefixes such as `session:game:*`, plus the Account-owned auth prefixes `session:auth:token:*` and `session:auth:generation:*`, and the Account-owned `session:connect-token:*` issuance-result projections
