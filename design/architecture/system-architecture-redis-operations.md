...
4. The target starts with the required empty keyspace and protected credentials/ACLs. Only the trusted deployment attestor, through the authenticated deployment-control channel, atomically consumes the single-use challenge and emits signed `post_reset_replacement_verification/v1`; no controller, operator, or other caller may consume it or emit a substitute record. `SAME_DEPLOYMENT` requires the same immutable deployment identity plus a new boot/startup and storage/AOF generation and independent `postWipeEmptyStartupAttestation/v1`; `REPLACEMENT_DEPLOYMENT` requires a distinct target identity plus exact target deployment generation, node set, Redis build, Lua registry, and startup evidence. Both require the exact `coord_ops_ro/v2` ACL/config digest and negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, and `CONFIG`. Redis key absence or an operator observation alone is not the pause, ownership, or empty-startup proof.
5. The recovery controller never consumes the challenge. It independently verifies the attestor-produced evidence, binds both evidence groups to the same durable `operationId`, exact resolved scope, deployment boundary, operation fence, and maintenance-lock digest or opaque reference derived from the same server-issued `maintenanceLockToken`, and durably records that verification before the operation reaches `ready_to_reopen`; public `resume(... expectedPhase=awaiting_resume ...)` verifies and durably records the same evidence again as the release gate without consuming the challenge or re-emitting the attestation. It never reconstructs the operation or releases the fence from the new empty keyspace. This handoff is durable operation evidence, not an additional public `continueRecovery` phase.
...
7. after startup, only the trusted deployment attestor, through the authenticated deployment-control channel, atomically consumes the challenge and emits mode-specific `post_reset_replacement_verification/v1`: `SAME_DEPLOYMENT` requires a new boot/storage generation plus independent `postWipeEmptyStartupAttestation/v1`, while `REPLACEMENT_DEPLOYMENT` requires a distinct exact target deployment identity and generation. Both require the exact `coord_ops_ro/v2` ACL/config digest and negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, and `CONFIG`. The recovery controller independently verifies and durably records that attestor evidence before the operation reaches `ready_to_reopen`, and verifies and durably records it again at public `resume(... expectedPhase=awaiting_resume ...)`; it never consumes the challenge or re-emits the attestation. Rebuild coordination state from PostgreSQL plus fresh activity, then validate normalization, shard locality, Lua registry compatibility, and migration evidence before calling `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with canonical `expectedPhase=ready_to_reopen`, after which the internal release phase is required
