...
- Account-wide recovery must acquire the account-scoped admission/creation fence `accountAdmissionFence` before capturing `inventorySnapshotRevision`, reject or queue new bindings for that account while the fence is held, and retain it through exact full-key readback and durable `coverageGeneration`. Every account binding creation/reconciliation CAS, retry, acknowledgement, parent-consumption record, and standalone account-wide flow must carry and validate that same fence; any missing, stale, or mismatched fence fails closed. The fence is distinct from the deployment maintenance lock, parent/child operation fences, and `coverageFence`.
- Coverage carriers use `coverageGeneration=NOT_YET_ISSUED` from child creation through pre-coverage readback and assign a concrete generation only in the atomic coverage-proven transition. A linked child result is parent-independent; the parent tuple remains only on the independent parent edge and the parent's exact consumption record. `accountAdmissionFence` is concrete only for an active account-wide child or standalone account-wide flow, is retained through coverage, and is atomically released with the coverage-proven transition; all other flows carry `accountAdmissionFence=NOT_APPLICABLE` and `NO_ACTIVE_ACCOUNT_WIDE_FLOW`.
...
4. The target starts with the required empty keyspace and protected credentials/ACLs. Only the trusted deployment attestor, through the authenticated deployment-control channel, atomically consumes the single-use challenge and emits signed `post_reset_replacement_verification/v1`; no controller, operator, or other caller may consume it or emit a substitute record. `SAME_DEPLOYMENT`, including a fresh logical-database reset on the same deployment, requires the same immutable deployment identity plus both a new boot/startup generation and a new storage/AOF generation and independent `postWipeEmptyStartupAttestation/v1`; no lesser same-deployment reset path is valid. `REPLACEMENT_DEPLOYMENT` requires a distinct target identity plus exact target deployment generation, node set, Redis build, Lua registry, and startup evidence. Both require the exact `coord_ops_ro/v2` ACL/config digest and negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`. Redis key absence or an operator observation alone is not the pause, ownership, or empty-startup proof.
...
7. after startup, only the trusted deployment attestor, through the authenticated deployment-control channel, atomically consumes the challenge and emits mode-specific `post_reset_replacement_verification/v1`: `SAME_DEPLOYMENT`, including a fresh logical-database reset on the same deployment, requires a new boot/startup generation and a new storage/AOF generation plus independent `postWipeEmptyStartupAttestation/v1`, while `REPLACEMENT_DEPLOYMENT` requires a distinct exact target deployment identity and generation. No lesser same-deployment reset path is valid. Both require the exact `coord_ops_ro/v2` ACL/config digest and negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`. The recovery controller independently verifies and durably records that attestor evidence before the operation reaches `ready_to_reopen`, and at public `resume(... expectedPhase=awaiting_resume ...)` re-verifies the evidence and active maintenance-lock state before durably recording `RESUME_AUTHORIZED`; it never consumes the challenge or re-emits the attestation. Rebuild coordination state from PostgreSQL plus fresh activity, then validate normalization, shard locality, Lua registry compatibility, and migration evidence before calling `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with canonical `expectedPhase=ready_to_reopen`, after which the internal release phase is required
