...
- Before destructive handoff or startup, the controller creates `replacementVerificationChallenge/v1` with an independent nonce, issue/expiry times, single-use state, source/target deployment identities, mode, operation tuple, and resolved scope. After startup, only the trusted deployment attestor atomically consumes that challenge and emits signed `post_reset_replacement_verification/v1`. In `SAME_DEPLOYMENT`, including a fresh logical-database reset on the same deployment, source and target have the same immutable deployment identity but the target must have both a new boot/startup generation and a new storage/AOF generation, and the record must include independently verifiable `postWipeEmptyStartupAttestation/v1`; no lesser same-deployment reset path is valid. In `REPLACEMENT_DEPLOYMENT`, source and target identities differ and the attestor must prove the exact target identity, deployment generation, node set, Redis build, Lua registry, and startup state plus the distinct signed `replacementEmptyStartupAttestation/v1` bound to the same challenge, operation/fence, target identity, target deployment generation, and resolved scope. Both modes must independently verify the exact `coord_ops_ro/v2` and `coord_redis_config/v1` digests, successful positive probes for every allowlisted command (`GET`, `MGET`, `HGET`, `HMGET`, `HGETALL`, `SMEMBERS`, `SCARD`, `ZRANGE`, `ZRANGEBYSCORE`, `ZCARD`, `XRANGE`, `XLEN`, `SSCAN`, `HSCAN`, `ZSCAN`, `TYPE`, `TTL`, `PTTL`, `EXISTS`, `SLOWLOG|LEN`, `CLIENT|ID`, `MEMORY|USAGE`, and `CLUSTER|INFO`), and negative probes proving `coord_ops_ro` cannot run database-wide `SCAN`, write, mutate expiry, run `EVAL`/`EVALSHA`, run `SCRIPT LOAD`, execute `CONFIG`, or expose `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, or `CLUSTER NODES`. The controller rejects missing, expired, replayed, caller-supplied, or mismatched challenge/evidence and validates both evidence groups against the same operation, scope, fence, and maintenance-lock reference before the internal worker reaches `ready_to_reopen`; it validates them again, including active maintenance-lock state, at public `resume(... expectedPhase=awaiting_resume ...)`. The internal smoke-check phase proves the new epoch can acquire leases, stage work, converge, and clean up correctly, proves replay-domain quarantine/fencing and durable consume acknowledgement where applicable, and samples representative regions only after the Account projection and exact-token gates pass. Protected admission remains closed through these gates; only `continueRecovery(... expectedPhase=ready_to_reopen ...)` may advance to `AWAITING_RESUME`.
...
The post-reset negative probe set includes `FLUSHALL`, `FLUSHDB`, and `SCRIPT FLUSH` in addition to the existing write, expiry, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, and telemetry probes. Database- or server-wide probes use the isolated disposable target; only key-scoped probes may use the protected probe namespace. All probes use the cleanup/readback contract owned by that canonical gate.
...
This handoff uses the canonical post-reset gate in [Coordination Redis Ops Access & Tooling](./system-architecture-redis-ops-access.md#post-reset-replacement-verification-gate): both exact `coord_ops_ro/v2` ACL and `coord_redis_config/v1` configuration evidence are required, with positive probes for every allowed command and negative probes for database-wide `SCAN`, writes, mutating expiry, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`. `SAME_DEPLOYMENT` requires independent `postWipeEmptyStartupAttestation/v1`; `REPLACEMENT_DEPLOYMENT` requires a distinct trusted signed `replacementEmptyStartupAttestation/v1` bound to the same operation/challenge, exact target deployment generation, and exact target deployment identity.
...
3. The authorized operator performs the AOF reset or replacement only after the pre-wipe authorization/fencing evidence has been accepted. That evidence cannot include endpoint, ACL/configuration, empty-keyspace, or health facts that do not exist until replacement startup.
4. The target starts with the required empty keyspace and protected credentials/ACLs. Only the trusted deployment attestor, through the authenticated deployment-control channel, atomically consumes the single-use challenge and emits signed `post_reset_replacement_verification/v1`; no controller, operator, or other caller may consume it or emit a substitute record. `SAME_DEPLOYMENT`, including a fresh logical-database reset on the same deployment, requires the same immutable deployment identity plus both a new boot/startup generation and a new storage/AOF generation and independent `postWipeEmptyStartupAttestation/v1`; no lesser same-deployment reset path is valid. `REPLACEMENT_DEPLOYMENT` requires a distinct target identity plus exact target deployment generation, node set, Redis build, Lua registry, startup evidence, and distinct signed `replacementEmptyStartupAttestation/v1` bound to the same challenge, operation/fence, target identity, target deployment generation, and resolved scope. Both require the exact `coord_ops_ro/v2` and `coord_redis_config/v1` digests, successful positive probes for every allowlisted command (`GET`, `MGET`, `HGET`, `HMGET`, `HGETALL`, `SMEMBERS`, `SCARD`, `ZRANGE`, `ZRANGEBYSCORE`, `ZCARD`, `XRANGE`, `XLEN`, `SSCAN`, `HSCAN`, `ZSCAN`, `TYPE`, `TTL`, `PTTL`, `EXISTS`, `SLOWLOG|LEN`, `CLIENT|ID`, `MEMORY|USAGE`, and `CLUSTER|INFO`), and negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`. Redis key absence or an operator observation alone is not the pause, ownership, or empty-startup proof.
...
The migration reset gate requires both independent expected values: the exact `coord_ops_ro/v2` ACL digest and the exact `coord_redis_config/v1` observed target-configuration digest bound to the target deployment generation. It also requires the existing positive probes for every allowlisted command and the existing negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, telemetry, and the destructive `FLUSHALL`, `FLUSHDB`, and `SCRIPT FLUSH` probes with their isolated-target cleanup/readback evidence. A combined or caller-asserted ACL/config value cannot satisfy this gate.
...
7. after startup, only the trusted deployment attestor, through the authenticated deployment-control channel, atomically consumes the challenge and emits mode-specific `post_reset_replacement_verification/v1`: `SAME_DEPLOYMENT`, including a fresh logical-database reset on the same deployment, requires a new boot/startup generation and a new storage/AOF generation plus independent `postWipeEmptyStartupAttestation/v1`, while `REPLACEMENT_DEPLOYMENT` requires a distinct exact target deployment identity and generation plus the distinct signed `replacementEmptyStartupAttestation/v1` bound to the same challenge, operation/fence, target identity, target deployment generation, and resolved scope. No lesser same-deployment reset path is valid. Both require the exact `coord_ops_ro/v2` ACL/config digest, successful positive probes for every allowlisted command (`GET`, `MGET`, `HGET`, `HMGET`, `HGETALL`, `SMEMBERS`, `SCARD`, `ZRANGE`, `ZRANGEBYSCORE`, `ZCARD`, `XRANGE`, `XLEN`, `SSCAN`, `HSCAN`, `ZSCAN`, `TYPE`, `TTL`, `PTTL`, `EXISTS`, `SLOWLOG|LEN`, `CLIENT|ID`, `MEMORY|USAGE`, and `CLUSTER|INFO`), and negative probes for database-wide `SCAN`, writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, `CONFIG`, `CLIENT LIST`, `SLOWLOG GET`, `INFO`, `MEMORY STATS`, `LATENCY LATEST`, `LATENCY DOCTOR`, and `CLUSTER NODES`. The recovery controller independently verifies and durably records that attestor evidence, including the replacement empty-startup attestation when applicable, before the operation reaches `ready_to_reopen`, and at public `resume(... expectedPhase=awaiting_resume ...)` re-verifies the evidence and active maintenance-lock state before durably recording `RESUME_AUTHORIZED`; it never consumes the challenge or re-emits the attestation. Rebuild coordination state from PostgreSQL plus fresh activity, then validate normalization, shard locality, Lua registry compatibility, and migration evidence before calling `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with canonical `expectedPhase=ready_to_reopen`, after which the internal release phase is required
