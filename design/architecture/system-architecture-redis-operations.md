...
- Before destructive handoff or startup, the controller creates `replacementVerificationChallenge/v1` with an independent nonce, issue/expiry times, single-use state, source/target deployment identities, mode, operation tuple, and resolved scope. After startup, only the trusted deployment attestor atomically consumes that challenge and emits signed `post_reset_replacement_verification/v1`. In `SAME_DEPLOYMENT`, source and target have the same immutable deployment identity but a new boot/startup and storage/AOF generation, and the record must include independently verifiable `postWipeEmptyStartupAttestation/v1`; in `REPLACEMENT_DEPLOYMENT`, source and target identities differ and the attestor must prove the exact target identity, deployment generation, node set, Redis build, Lua registry, and startup state. Both modes must independently verify the exact `coord_ops_ro/v2` ACL/config digest and negative probes proving `coord_ops_ro` cannot write, mutate expiry, run `EVAL`/`EVALSHA`, run `SCRIPT LOAD`, or execute `CONFIG`. The controller rejects missing, expired, replayed, caller-supplied, or mismatched challenge/evidence and validates both evidence groups against the same operation, scope, fence, and maintenance-lock reference before the internal worker reaches `ready_to_reopen`; it validates them again at public `resume(... expectedPhase=awaiting_resume ...)`. The internal smoke-check phase proves the new epoch can acquire leases, stage work, converge, and clean up correctly, proves replay-domain quarantine/fencing and durable consume acknowledgement where applicable, and samples representative regions only after the Account projection and exact-token gates pass. Protected admission remains closed through these gates; only `continueRecovery(... expectedPhase=ready_to_reopen ...)` may advance to `AWAITING_RESUME`.
...
4. The target starts with the required empty keyspace and protected credentials/ACLs. Only the trusted deployment attestor consumes the single-use challenge and records signed `post_reset_replacement_verification/v1`. `SAME_DEPLOYMENT` requires the same immutable deployment identity plus a new boot/startup and storage/AOF generation and independent `postWipeEmptyStartupAttestation/v1`; `REPLACEMENT_DEPLOYMENT` requires a distinct target identity plus exact target deployment generation, node set, Redis build, Lua registry, and startup evidence. Both require the exact `coord_ops_ro/v2` ACL/config digest and negative probes for writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, and `CONFIG`. Redis key absence or an operator observation alone is not the pause, ownership, or empty-startup proof.
5. The controller atomically consumes the challenge once, binds both evidence groups to the same durable `operationId`, exact resolved scope, deployment boundary, operation fence, and maintenance-lock digest or opaque reference derived from the same server-issued `maintenanceLockToken`, and validates them before its internal recovery worker resumes rebuild phases. It never reconstructs the operation or releases the fence from the new empty keyspace. This handoff is durable operation evidence, not an additional public `continueRecovery` phase.
6. A missing, stale, mismatched, or ambiguous evidence group leaves the operation paused and gameplay admission closed. Internal retries use the same operation-owned state and do not repeat the AOF reset or begin rebuild concurrently. The controller must validate both evidence groups again before public `resume` can record `RESUME_AUTHORIZED`; the only public `continueRecovery` invocation remains the later `expectedPhase=ready_to_reopen` transition into `AWAITING_RESUME`, and the separate internal release is still required before reopening.
...
6. only after validating that bound pre-wipe evidence, any required `physical-dedication-proof/v1`, and the single-use `replacementVerificationChallenge/v1`, start a fresh Coordination Redis deployment or logical database with an empty keyspace as the operation's recorded external-infrastructure step
7. after startup, only the trusted deployment attestor consumes the challenge and records mode-specific `post_reset_replacement_verification/v1`: `SAME_DEPLOYMENT` requires a new boot/storage generation plus independent `postWipeEmptyStartupAttestation/v1`, while `REPLACEMENT_DEPLOYMENT` requires a distinct exact target deployment identity and generation. Both require the exact `coord_ops_ro/v2` ACL/config digest and negative probes for writes, expiry mutation, `EVAL`/`EVALSHA`, `SCRIPT LOAD`, and `CONFIG`. Rebuild coordination state from PostgreSQL plus fresh activity, then validate normalization, shard locality, Lua registry compatibility, and migration evidence before calling `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with canonical `expectedPhase=ready_to_reopen`; the controller must validate both external evidence groups before continuation and public `resume(... expectedPhase=awaiting_resume ...)`, after which the internal release phase is required
