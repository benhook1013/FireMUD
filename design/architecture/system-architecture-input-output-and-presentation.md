...
- Prompt output now has a first baseline pipeline plus a narrow per-session prompt-throttling window, and first-party web command responses plus fresh reconnect prompt delivery can now consume structured prompt payloads directly. Richer burst-end scheduling and canonical buffered prompt/status replay are still future work.
