...
- Communication actor and recipient responses now flow through the same late renderer from metadata-only Game Logic delivery views, but first-party web still shares the generic text payload path and reconnect/screen-buffer boundaries still store rendered transcript text.
