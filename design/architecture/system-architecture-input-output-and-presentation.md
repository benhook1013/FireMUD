...
- the durable resume transcript is keyed by the admitted tenant, playable-state namespace, and character identity, so runtime instance replacement does not fragment context and an isolated playtest remains separate from production;
...
- after current authentication and authorization establish access to the exact tenant, playable-state namespace, and character, FireMUD replays retained context in ordering-token order, then emits a fresh authoritative `LOOK` and one current prompt;
- explicit gameplay `LOGOUT`, character ownership transfer, or loss of replay authorization clears the context or suppresses its private replay;
...
This replay is semantic recent context, not a transport delivery acknowledgement. It may repeat output the client already displayed and may omit output that was not replay eligible, no longer retained, or unavailable. Client and player-facing UX must call it recent context rather than missed messages and must not imply exact or exactly-once delivery. FireMUD does not retain arbitrary raw bytes, frames, unsent transport buffers, command input, or prompts as part of this context.
...
When a byte bound is exceeded, FireMUD evicts complete oldest retained entries. The soft ceiling preserves the configured message and line floors only where they fit beneath the hard ceiling. The hard ceiling bounds the complete persisted context without exception. When one complete current entry alone exceeds it, FireMUD omits that entry from reconnect retention or stores a bounded omission marker whose complete persisted size fits beneath the ceiling; it does not retain a partial or silently truncated semantic entry. Inactivity expiry removes the whole retained context. A normal multiplayer game may keep a small recent window and expire it after a period such as sixty inactive days; a persistent RPG may retain a larger short window without inactivity expiry. Neither case implies an unbounded archive of all gameplay output.

The byte bound is deterministic: each retained entry costs the UTF-8 byte length of its complete scope-bound persisted transcript envelope, not only its rendered text. The compact outer JSON envelope has lexically ordered members for `briefRenderPolicy`, `characterId`, `gameInstanceId`, `occurredAt`, `orderingToken`, `outputKind`, `payload`, `payloadType`, `playableStateNamespaceId`, `renderedText`, `replayPolicy`, and `tenantId`; every optional outer member is present and absent values are JSON `null`. `playableStateNamespaceId` is the authority and retention scope, while `gameInstanceId` is retained only as emission provenance. Outer-envelope strings are normalized to Unicode NFC, timestamps are RFC 3339 UTC with fixed millisecond precision, numbers use their shortest normalized JSON form, and no insignificant whitespace is emitted. The `payload` member preserves the canonical structured-output JSON emitted by the live output encoder. This accounts for entry metadata, structured payload, and the derived rendered-text compatibility projection exactly once in both the durable source of truth and Redis hot cache. FireMUD never separately adds the same payload or rendered text again for transport projections. A retained entry or omission marker that cannot fit under the hard byte bound is omitted.
...
- Reconnection uses bounded durable semantic recent context followed by a fresh `LOOK` and one prompt; it does not acknowledge delivery or replay arbitrary missed transport bytes.
