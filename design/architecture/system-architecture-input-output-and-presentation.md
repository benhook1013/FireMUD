...
- The canonical resume-transcript model is locked at the architecture layer: replay-eligible structured entries are retained durably as one bounded per-character resume context, while rendered plain text remains a derived cache/compatibility surface rather than transcript source truth. Durable persistence is planned implementation work; current runtime replay remains the hot reconnect buffer described below.
