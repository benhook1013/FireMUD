...
2. Run compromise-mode rotation.
   ...
   - Account promotes the new signing generation, publishes its JWKS, and performs the sole issuer-generation advance for this operation exactly once. Account must reconcile the controller's authenticated CAS result and Account-owned public-JWKS status before declaring compromise rotation complete. Do not overlap, retain, or roll back to a compromised key.
...
3. Verify and record the environment-wide issuer authority committed in Step 2.
   ...
   The corresponding issuer-generation projection is applied set-if-greater; missing, stale, or uncertain projection evidence fails closed. Account must record an authority-projection freshness fence that identifies the same committed issuer generation, its source transaction or outbox/event version, the observed projection generation, and the projection status before recovery can advance. Reauthentication remains mandatory after the authority transition.
