...
2. Run compromise-mode rotation.
   - Have Account Service initiate one authenticated, operation-bound transition for a new asymmetric signing generation. In target custody it requests the non-exportable signer; in the interim fallback it requests the materialization controller to generate, CAS-write, or prune only the named private-Secret slots. The signer/controller performs only the private-key operation and returns operation-bound generation, `kid`, resource-version, non-secret digest, and pruning evidence. Account validates and reconciles that result, promotes the generation, publishes its JWKS, advances issuer authority, and owns the lifecycle decision. Account remains authoritative for validation, promotion, issuer authority, JWKS publication, and public/private pruning policy. Rotation automation is observation-only and never receives private material. The private key must never enter rotation automation.
