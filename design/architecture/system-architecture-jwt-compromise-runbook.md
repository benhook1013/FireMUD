...
- Durable compromise-rotation operation ID, immutable request digest, expected and committed signer generations, uniqueness/CAS evidence, JWKS publication evidence, and proof that the issuer generation advanced exactly once; a same-operation retry must return this evidence without another advance, while a conflicting operation or digest must leave state unchanged.
