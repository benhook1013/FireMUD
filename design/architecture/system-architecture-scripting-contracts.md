...
- Automation owns a monotonic `pluginActivationGeneration` per `(tenantId, gameInstanceId, pluginId)`. It advances exactly once for each committed transition that invalidates current plugin work, including disable, revocation or policy-driven disable, and materially changed activation or binding; a later same-version enable or activation uses a newer generation. Failed operations, no-ops, and exact retries of the same committed control-plane request reuse the stored result and do not advance it. This is Automation runtime-state evidence, not a Game Session-owned plugin epoch.
...
- Script work items and tick commands produced by plugins must carry `pluginId`, `pluginVersionId`, and `bindingId` in addition to `scriptPatchVersion` and the captured `pluginActivationGeneration` whenever the command came from a binding-scoped plugin handler. Admitted plugin work preserves and revalidates that generation through evaluation, persistence, handoff, staged/final effect execution, retry, replay, and recovery.
  ...
  - If a command’s embedded `pluginVersionId` does not match the instance’s currently active plugin version for that `pluginId`, or its captured `pluginActivationGeneration` does not match the current Automation-owned generation evidence for that scope, Game Session must not execute it. Game Session consumes the applicable current generation evidence and does not become the plugin lifecycle authority.
...
- In that state, new Automation admission remains fail-closed for the affected instance until the named same-workflow timeout recovery path (`ROLLBACK_CONVERGENCE_TIMEOUT -> CONVERGING` with the same `controlPlaneRequestId`) in the [Scripting & Automation: Rollout and Rollback](./system-architecture-scripting-rollout-and-rollback.md#rollback-orchestration-state-machine-required), explicit repair, or another repin. Ordinary player-command admission and gameplay ticks continue when their own dependencies and fences are healthy; a full tick pause is exceptional and requires a named unfenced effect or migration boundary.
...
- Recovery revalidates those fences immediately before evaluation for the evaluation-stage path and immediately before each post-evaluation dispatch for the stored-output path. For plugin work, it must revalidate the captured `pluginActivationGeneration` against the current Automation-owned generation as well as fresh current activation/lifecycle, component-policy, capability-grant, and signer/publication evidence. Missing, stale, or temporarily unavailable evidence fails closed with established bounded reasons such as `authority_unavailable`, `component_policy_unavailable`, or `signer_policy_unavailable`; a captured-generation mismatch returns `plugin_activation_generation_mismatch`; proven lifecycle, publication, component/capability, signer, tuple, or scope mismatches use `plugin_disabled`, `plugin_version_not_published`, `plugin_component_policy_blocked`, `signer_revoked`, `plugin_binding_mismatch`, `script_pin_epoch_mismatch`, or `runtime_scope_mismatch` as applicable.
- `ReplayDeadLetteredWorkItems` accepts bounded explicit parent `workItemIds[]` only; descriptor references and filters remain deferred mutation selectors. Each row returns exactly one `outcome`: `retried_evaluation`, `resumed_dispatch`, `already_recovered`, or `rejected`, together with its resolved `failureGeneration` when the parent is owned. Only a `rejected` row carries `rejectionReason`, using bounded values such as `not_found_or_not_owned`, `stage_evidence_unavailable`, `work_item_not_dead_lettered`, `script_pin_epoch_mismatch`, `plugin_binding_mismatch`, or `runtime_scope_mismatch`. `already_recovered` is permitted only when immutable prior successful-recovery evidence binds the same current generation; an older generation's success neither satisfies nor blocks a new current-generation request. An exact retry with the same `controlPlaneRequestId` and canonical request fingerprint returns its stored result and generation without reevaluating current row status. Eligible rows progress independently; ineligible `DEAD_LETTERED` rows remain unchanged. A stable `controlPlaneRequestId`, actor, and reason make duplicate recovery requests idempotent.
