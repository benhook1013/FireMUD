...
The `room:*` Class A cache contract below is target-state only. It is not a current correctness path until World Management has an opaque room component version and a proven invalidation path. Namespace-backed `inventory:*` and explicitly instance-scoped `inventory:room-ground:*` cache adoption is likewise target-state; current readers remain authoritative until Entity Management's owner-version/event validation is proven. Current readers must not substitute `worldSnapshotId` or `roomDynamicVersion` for that component version; they must use authoritative reads when the target validation contract is unavailable. A cache-embedded version, TTL, or invalidation event alone is not currentness proof. The current Game Session `ratelimit:<sessionId>` key is likewise a legacy implementation: `SessionRateLimiterImpl` writes it through the generic Game Session Redis client with a one-second window/TTL, while its Cache/Rate-Limit endpoint and ACL binding remain unproved. It must be registered as current legacy state, its writer stopped, and its maximum one-second TTL allowed to elapse with an empty-key readback before unversioned target rate-limit helpers consume the shared family; an adopter that cannot prove that drain must use a distinct versioned prefix. The current Game Session `lookcache:<tenantId>:<legacySessionOrInstanceId>` writer is also legacy implementation drift: it is disposable and non-authoritative, but its incomplete scope and ten-minute TTL cannot satisfy target causal room-read semantics. Current composition remains authoritative/uncached until the target `view:room-look:*` contract is proven. The future reader, invalidation, and version-advance rules below remain normative.

The current Automation implementation also emits `automation:test:quota:*`, `automation:test:capacity:*`, and `automation:readiness:capacity:*` families. They are named in the catalog below as Cache/Rate-Limit, Class B, reset-tolerant projections. Current readiness and dry-run capacity counter/lease reservations pass two keys with one shared tenant or fixed-family hash tag to each Lua invocation, so each pair occupies one Redis Cluster slot and is not inherently `CROSSSLOT`. Central ownership registration, canonical key encoding, durable charge/admission, reset, retention, and idempotent lease-accounting proof remain incomplete; the same-slot shape alone does not establish a supported clustered workload. The current Game Session `IpConnectionLimiter` has a distinct live limitation: registration passes `ipconn:<address>` and `sessionip:<sessionId>` to one Lua invocation without a shared hash slot, so Redis Cluster returns `CROSSSLOT`; this is in addition to the raw-address/privacy drift. The target must follow ADR 0087's one-mutation/one-opaque-subject-bucket boundary, with session lifecycle correlation kept outside any exact cross-key atomicity requirement; no final reverse-index shape is assumed here. Destructive resets additionally require the external, generation-bound [Cache deployment maintenance gate](./system-architecture-redis-operations.md#external-cache-deployment-maintenance-gate), not a fence key stored in the cache being reset.
...
| `lookcache:<tenantId>:<legacySessionOrInstanceId>` | Cache (current legacy only; target role unapproved) | Legacy disposable projection (not target Class B) | Reset-tolerant for disposable loss | Current Game Session writer only. The ten-minute payload is non-authoritative, but the legacy session/game-instance scope lacks viewer, policy, and causal read-fence identity; a clustered cache reset may delete and verify this family only while the external, generation-bound Cache deployment maintenance gate is held through deletion and verification. Remove/drain this writer or migrate to the exact `view:room-look:*` contract before target helpers consume it; otherwise continue authoritative uncached composition. This prefix is not target Class B authority. |
| `ipconn:<address>` and `sessionip:<sessionId>` | Cache / Rate-Limit (current legacy implementation drift) | Legacy heuristic (not target Class B authority) | Reset-tolerant heuristic loss | Game Session owns the current paired raw-address counter and session reverse-map. Entries use the current connection TTL (`GAME_SESSION_CONN_TTL_SEC`, default 3600 seconds); a reset may temporarily relax or tighten abuse limits and can leave the pair incomplete, but must not be treated as durable authority. Current registration passes both untagged keys to one Lua invocation, so it fails with `CROSSSLOT` on Redis Cluster; this is in addition to the raw-address/privacy defect covered by ADR 0087. The target must use one mutation on one opaque subject bucket and keep session lifecycle correlation outside exact cross-key atomicity; no final reverse-index shape is assumed here. |
| `screenbuffer:<tenantId>:<gameInstanceId>:<characterId>` | Cache | TTL-only (Class B) | Reset-tolerant | Current Game Session disposable hot-cache prefix for durable semantic reconnect context; PostgreSQL `resume_transcript_entry` rows remain authoritative. This prefix is distinct from `view:room-look:*` and never serves as semantic authority. Current keying is `gameInstanceId`-based; target reconnect scope is `{tenantId, playableStateNamespaceId, characterId}` under ADR 0134, so namespace-key migration and complete reconnect-envelope/generation proof remain implementation gaps. The row records prefix semantics, not current endpoint/role wiring. |
| `chat:say:<tenantId>:<senderAccountId>`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:<characterId>`, `chat:tell:<tenantId>:<conversationId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, `chat:account:<tenantId>:<accountId>` | Cache | TTL-only (Class B) | Reset-tolerant | Social & Groups short-lived chat projections; current `SAY` is sender-account scoped. Any future gameplay-SAY projection must publish its exact recipient/view scope and original-view contract rather than infer it from a bare character ID. See the [canonical `chat:whisper` Class-B contract](#canonical-social--groups-chatwhisper-class-b-contract). |
| `chat:whisper:<tenantId>:<accountId>` | Cache | TTL-only (Class B) | Reset-tolerant | Legacy `ChatService` account-keyed whisper implementation-drift prefix (`chat:whisper:%d:%d`); it does not provide the target playable-state/character scope and remains a reset-tolerant disposable projection during migration. |
| `automation:queue:{tenantInstanceTag}:*` | Cache / Rate-Limit | TTL-only (target Class B; current reset/rebuild unavailable) | **Current queue reset/recovery is unavailable and must fail closed**; the implementation's rebuild query selects only `PENDING_EVALUATION`, but it lacks owner/CAS reconciliation and cannot establish a safe disposition for existing `EVALUATING` rows, so this projection must not be flushed, rebuilt, or resumed as a supported current operation. Once the owner-reconciled target path is implemented and proven, it may be reset and rebuilt from durable work. | Automation & Scripting derived work-item pointers; PostgreSQL remains authoritative. See the [Automation & Scripting runtime/data owner contract](./microservices/automation-scripting-service/runtime-and-data.md#redis-roles-and-prefixes) and [API implementation status](./microservices/automation-scripting-service/api-contracts.md#implementation-status). The current queue implementation has no per-list TTL/trim or declared key-count/payload-size envelope; those are adoption/proof gaps, not implied bounds. |
| `automation:quota:<tenantId>:*`, `automation:tenant-budget:<tenantId>:tier:<tier>`, `automation:test:quota:<tenantId>:script:<scriptId>:tenant`, `automation:test:quota:<tenantId>:script:<scriptId>:principal:<principalKey>`, `automation:test:capacity:{<tenant-id>}:*`, `automation:test:capacity:{automation-test-capacity}:cluster*`, `automation:readiness:capacity:{<tenant-id>}:*`, `automation:readiness:capacity:{automation-readiness-capacity}:cluster*` | Cache / Rate-Limit | TTL-only (Class B) | Reset-tolerant | Automation & Scripting per-script quota and tenant-budget counters, one-minute dry-run/test tenant/principal quota counters, exact tenantless cluster capacity counters paired with tenant/work-scoped leases, and bounded tenant/cluster `PUBLISH_READINESS` capacity reservations. Capacity counter/lease pairs share the documented hash tag (`{<tenant-id>}` for tenant-scoped pairs and fixed family tags for cluster pairs) so their Lua operations remain in one Redis Cluster slot. The owner settings and capacity bounds are defined in the [Automation service configuration](./microservices/automation-scripting-service/configuration.md#service-specific-variables); the implementation uses one-minute counter windows and ten-minute capacity-lease TTLs. The counters/leases are disposable Cache/Rate-Limit projections, not Coordination Redis state, and are not rebuilt from Redis after reset. The catalog currently has no per-prefix key-count or serialized-payload envelope for these families; do not infer one from the concurrency limits until the owner declares and proves it. Durable trigger/work-item/readiness state and execution correctness remain in PostgreSQL and the owning service. |
...
| `ratelimit:<sessionId>` | Cache / Rate-Limit (current legacy; generic-client role unproved) | Legacy heuristic (not target Class B authority) | Reset-tolerant heuristic loss | Game Session's live `SessionRateLimiterImpl` command limiter. The current generic client may land on the wrong Redis role; the one-second counter/TTL is disposable and loss may relax or tighten heuristic limits. Cache-only writer registration and ACL proof do not make the legacy arity compatible with target helpers. Before any unversioned target `ratelimit:*` helper is enabled, stop the legacy writer, allow the maximum one-second TTL to elapse, and prove empty-key readback; an adopter that cannot prove that drain must use a distinct versioned prefix. |

The current `automation:queue:{tenantInstanceTag}:*` implementation is not yet TTL-only: its enqueue and rebuild paths append to Redis lists without `EXPIRE`, `LTRIM`, or another bound. Target adoption requires configuration-backed `SCRIPT_OUTBOX_QUEUE_POINTER_TTL_SECONDS` and `SCRIPT_OUTBOX_QUEUE_MAX_POINTERS_PER_KEY` (exact values remain environment/service settings), applied atomically with every enqueue and rebuild. Truncation or expiry is acceptable only as pointer loss: Automation must re-drive from eligible durable PostgreSQL work-item rows, and reset/rebuild release requires every eligible durable identity to be rematerialized or durably settled. That release also requires the external, generation-bound Cache deployment maintenance gate to remain held through deletion, post-reset verification, and the authorized rebuild/readback; because the current gate and safe rebuild path are unavailable, queue reset/rebuild must fail closed. Until that policy and focused retention, memory-pressure, and rebuild proof exist, this is implementation/proof drift rather than a proven TTL-only cache. Separately, the live `tenantInstanceTag` builder concatenates raw nonblank IDs and has no proven delimiter/brace rejection or collision-safe encoding; it remains implementation drift until the shared encoded tuple or canonical UUID-boundary contract is enforced.

### Canonical Social & Groups `chat:whisper` Class-B Contract

The target gameplay `WHISPER` projection uses the exact recipient scope `{tenantId, playableStateNamespaceId, characterId}` and the key `chat:whisper:<tenantId>:<playableStateNamespaceId>:<characterId>`. It is distinct from account-scoped direct messaging under `chat:account:<tenantId>:<accountId>`. This is a Social & Groups Class-B, TTL-only cache for a short-lived projection. Its authority and owner-specific settings are defined by [ADR 0149](./decisions/adr-0149-communication-type-specific-history-and-retention.md), [Social & Groups Runtime and Data](./microservices/social-groups-service/runtime-and-data.md#redis-role-and-prefixes), and [Social & Groups Configuration](./microservices/social-groups-service/configuration.md#service-specific-variables); the applicable reset behavior is recorded in the [Redis reset policy matrix](./system-architecture-redis-reset-and-recovery.md#reset-policy-matrix-prefix-summary).

The live Social request/DTO/entity/schema and `ChatService` key remain account-keyed as `chat:whisper:<tenantId>:<accountId>`. That is implementation drift and must not serve player-history reads or cache refills that could mix characters or playable-state namespaces belonging to one account. The cross-service proto/DTO/entity/schema/key migration and original-view proof remain tracked gaps; this catalog does not claim that migration is implemented.

- The current service defaults remain a `7200`-second TTL through `FIREMUD_CHAT_WHISPERS_TTL_SECONDS` and a maximum of `50` messages through `FIREMUD_CHAT_WHISPERS_MAX_MESSAGES`. These are implementation settings; the target scope does not change the budgets.
- Redis is a rebuildable, non-authoritative projection. Cache loss, expiry, reset, miss, or unavailable Redis may fall back to authoritative Social/PostgreSQL history or rebuild the window from persisted history only when that communication type promises durable player history; otherwise there is no recovered window. Cache refill never supplies safety or idempotency data and must not change retention, erasure, or recipient disclosure.
- Metrics follow the `chat:*` family contract: `cache.chat_hits_total` and `cache.chat_misses_total` with chat-type labels, plus key-count gauges where available.
- The target reset matrix entry `chat:whisper:<tenantId>:<playableStateNamespaceId>:*` is reset-tolerant: dropping the buffer may create gaps or a non-contiguous transient Redis projection for the affected namespace. Reads for a communication type that promises durable player history use or fall back to authoritative Social/PostgreSQL history, so reset/TTL loss does not create a durable-history entitlement gap. The live account-keyed prefix remains an implementation migration gap.
- Current focused proof is limited to `ChatServiceImplTest.whisperCachesSeparatelyFromTell` in [ChatServiceImplTest](../../services/social-groups-service/src/test/java/unit/net/firedevops/firemud/socialgroups/service/impl/ChatServiceImplTest.java), which checks the live account-keyed write, TTL assignment, and separation from `TELL`. No regression currently proves actual TTL expiry, target-scope recomputation, Redis reset loss, or authoritative-history fallback. Those missing signals, together with the scope migration above, keep the target prefix from being implementation-ready.
- Environment-specific adoption remains an explicit gap: no `dev_local`, `hobby_self_hosted`, or `production_clustered` override values or environment-specific reset evidence are currently declared. Do not infer or invent those values; this prefix must remain an adoption gap until the owning service configuration, observability references, and reset evidence define them.
...
| Short-lived chat buffers | `chat:say:<tenantId>:<senderAccountId>`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:<characterId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, etc. | TTL-only | Rolling windows of recent messages with fixed-size buffers; current `SAY` uses a sender-account projection, while any future gameplay-SAY recipient/view projection must publish its exact scope and original-view contract. Target gameplay `chat:whisper` follows the [canonical Class-B contract](#canonical-social--groups-chatwhisper-class-b-contract), while the live Social request/DTO/entity/schema/key remains account-keyed as the documented migration gap. |
...
- `automation:queue:*` / `automation:quota:*` / `automation:tenant-budget:*` / `automation:test:quota:*` / `automation:test:capacity:*` / `automation:readiness:capacity:*` - queue, quota, and bounded-capacity counters or leases; service metrics should make best-effort loss, expiry, and any durable re-admission/reconciliation visible across tenant and cluster variants.
