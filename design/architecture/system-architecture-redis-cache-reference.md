...
| `view:room-look:<tenantId>:<gameInstanceId>:<roomInstanceId>` | Cache | TTL-only (Class B) | Reset-tolerant | Game Session rendered room views for `LOOK`. This is part of the built-in gameplay view-cache subsystem for canonical platform read/redraw commands. Game Session is the sole writer and direct Redis reader for this prefix; Game Logic must not read it directly and must consume authoritative LOOK results over gRPC instead. |
...
- `automation:queue:*` / `automation:quota:*` - queue and quota counters or equivalent service metrics that make best-effort loss and rebuild visible.
