...
| `automation:queue:{tenantInstanceTag}:*`, `automation:quota:<tenantId>:*`, `automation:tenant-budget:<tenantId>:tier:<tier>`, `automation:test:quota:<tenantId>:script:<scriptId>:*`, `automation:test:capacity:<tenantId>:*`, `automation:test:capacity:cluster*`, `automation:readiness:capacity:<tenantId>:*`, `automation:readiness:capacity:cluster*` | Cache / Rate-Limit | TTL-only (Class B) | Reset-tolerant | Current/legacy Automation & Scripting queued work-item, live/test quota, dry-run capacity, and readiness-capacity key shapes. Durable triggers/effect tables in PostgreSQL, not Redis, guarantee eventual execution and quota correctness. The current multi-key capacity reservations are not cluster-safe; no canonical target prefix is registered until the owner selects and proves a shared hash-tag or single-key design. |
