...
The current Automation implementation also emits `automation:test:quota:*` and `automation:readiness:capacity:*` families. They are named in the catalog below as Cache/Rate-Limit, Class B, reset-tolerant projections, but the listed untagged shapes describe current/legacy implementation rather than a canonical cluster-safe target. Readiness and dry-run capacity counter/lease reservations currently pass two untagged keys to one Lua invocation, so a Redis Cluster deployment can reject them with `CROSSSLOT`. No canonical target family is registered until the owner chooses and proves either one shared hash-tag shape per atomic key set or a single-key reservation; this catalog does not invent that unresolved target. The current Game Session `IpConnectionLimiter` has the same current limitation: registration passes `ipconn:<address>` and `sessionip:<sessionId>` to one Lua invocation without a shared hash slot, so Redis Cluster returns `CROSSSLOT`; this is in addition to the raw-address/privacy drift. The target must follow ADR 0087's one-mutation/one-opaque-subject-bucket boundary, with session lifecycle correlation kept outside any exact cross-key atomicity requirement; no final reverse-index shape is assumed here. Destructive resets additionally require the external, generation-bound [Cache deployment maintenance gate](./system-architecture-redis-operations.md#external-cache-deployment-maintenance-gate), not a fence key stored in the cache being reset.
