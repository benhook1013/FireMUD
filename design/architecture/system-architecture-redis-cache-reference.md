...
| `automation:queue:{tenantInstanceTag}:*`, `automation:quota:<tenantId>:*` | Cache / Rate-Limit | TTL-only (Class B) | Reset-tolerant | Automation & Scripting queued work items and quota counters. Durable triggers/effect tables in PostgreSQL, not Redis, guarantee eventual execution and quota correctness. |
