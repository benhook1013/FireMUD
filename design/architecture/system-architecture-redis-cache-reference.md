...
| `chat:say:<tenantId>:<characterId>`, `chat:whisper:<tenantId>:<accountId>`, `chat:tell:<tenantId>:<conversationId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, `chat:account:<tenantId>:<accountId>` | Cache | TTL-only (Class B) | Reset-tolerant | Social & Groups short-lived chat history buffers; see the [canonical `chat:whisper` Class-B contract](#canonical-social--groups-chatwhisper-class-b-contract). |
...
`chat:whisper:<tenantId>:<accountId>` is a Social & Groups Class-B, TTL-only cache for a short-lived history projection. Its authority and owner-specific settings are defined by [ADR 0149](./decisions/adr-0149-communication-type-specific-history-and-retention.md), [Social & Groups Runtime and Data](./microservices/social-groups-service/runtime-and-data.md#redis-role-and-prefixes), and [Social & Groups Configuration](./microservices/social-groups-service/configuration.md#service-specific-variables); the applicable reset behavior is recorded in the [Redis reset policy matrix](./system-architecture-redis-reset-and-recovery.md#reset-policy-matrix-prefix-summary).

- The current service defaults are a `7200`-second TTL through `FIREMUD_CHAT_WHISPERS_TTL_SECONDS` and a maximum of `50` messages through `FIREMUD_CHAT_WHISPERS_MAX_MESSAGES`. The message cap is the bounded history-window size budget.
...
- The reset matrix entry `chat:whisper:<tenantId>:*` is reset-tolerant: dropping the buffer may create gaps or a non-contiguous recent window, while authoritative persisted state remains intact.
...
| Short-lived chat buffers | `chat:say:<tenantId>:<characterId>`, `chat:whisper:<tenantId>:<accountId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, etc. | TTL-only | Rolling windows of recent messages with fixed-size buffers; `chat:whisper` follows the [canonical Class-B contract](#canonical-social--groups-chatwhisper-class-b-contract). |
