...
| `view:room-look:<tenantId>:<gameInstanceId>:<roomInstanceId>:<sessionId>:<viewerContextHash>:<policyContextHash>` | Cache | TTL-only (Class B) | Reset-tolerant | Current Game Session-owned disposable presentation/redraw helper bound to the exact room, viewer/session, and policy context. It is never semantic reconnect context, frame/output replay, a transcript archive, or a delivery ledger; it is not authoritative for fresh `LOOK` and is not part of the canonical target-state read model. |
