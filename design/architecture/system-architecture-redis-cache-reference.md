...
| `chat:say:<tenantId>:<characterId>`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:<characterId>`, `chat:tell:<tenantId>:<conversationId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, `chat:account:<tenantId>:<accountId>` | Cache | TTL-only (Class B) | Reset-tolerant | Social & Groups short-lived chat projections; see the [canonical `chat:whisper` Class-B contract](#canonical-social--groups-chatwhisper-class-b-contract). |
...
- The target reset matrix entry `chat:whisper:<tenantId>:<playableStateNamespaceId>:*` is reset-tolerant: dropping the buffer may create gaps or a non-contiguous recent window for the affected namespace, while authoritative persisted state remains intact. The live account-keyed prefix remains an implementation migration gap.
...
| Short-lived chat buffers | `chat:say:<tenantId>:<characterId>`, `chat:whisper:<tenantId>:<playableStateNamespaceId>:<characterId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, etc. | TTL-only | Rolling windows of recent messages with fixed-size buffers; target gameplay `chat:whisper` follows the [canonical Class-B contract](#canonical-social--groups-chatwhisper-class-b-contract), while the live Social request/DTO/entity/schema/key remains account-keyed as the documented migration gap. |
