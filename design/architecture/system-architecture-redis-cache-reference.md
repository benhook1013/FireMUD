...
| `chat:say:<tenantId>:<characterId>`, `chat:whisper:<tenantId>:<accountId>`, `chat:tell:<tenantId>:<conversationId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, `chat:account:<tenantId>:<accountId>` | Cache | TTL-only (Class B) | Reset-tolerant | Social & Groups short-lived chat history buffers. |
...
| Short-lived chat buffers | `chat:say:<tenantId>:<characterId>`, `chat:whisper:<tenantId>:<accountId>`, `chat:guild:<tenantId>:<guildId>`, `chat:city:<tenantId>:<cityId>`, etc. | TTL-only | Rolling windows of recent messages with fixed-size buffers. |
