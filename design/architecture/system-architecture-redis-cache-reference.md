...
| `view:room-look:<tenantId>:<gameInstanceId>:<sessionId>:<viewerContextHash>:<policyContextHash>` | Cache | TTL-only (Class B) | Reset-tolerant | Reconnect-adjacent rendered-room snapshot helper bound to the exact viewer/session and policy context. This prefix is not authoritative for fresh `LOOK`, is not part of the canonical target-state read model, and remains implementation debt pending fuller cleanup. |
...
| Room LOOK views | `view:room-look:<tenantId>:<gameInstanceId>:<sessionId>:<viewerContextHash>:<policyContextHash>` | TTL-only | Recomputed on demand from `ResolveLook`, cached for a short TTL, and primarily used for reconnect/UI redraw rather than as the canonical answer for fresh gameplay reads. The exact viewer/session and policy context binding prevents cross-viewer or cross-policy reuse. |
