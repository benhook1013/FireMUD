...
| `automation:queue:{tenantInstanceTag}:*`, `automation:quota:<tenantId>:*`, `automation:tenant-budget:<tenantId>:tier:<tier>`, `automation:test:quota:<tenantId>:script:<scriptId>:tenant`, `automation:test:quota:<tenantId>:script:<scriptId>:principal:<principalKey>`, `automation:test:capacity:<tenantId>:*`, `automation:test:capacity:cluster*`, `automation:readiness:capacity:<tenantId>:*`, `automation:readiness:capacity:cluster*` | Cache / Rate-Limit | TTL-only (Class B) | Reset-tolerant | Automation & Scripting queued work-item indexes, live per-script quota and tenant-budget counters, one-minute dry-run/test tenant/principal quota counters, tenant/cluster dry-run capacity leases, and bounded tenant/cluster `PUBLISH_READINESS` capacity reservations. All are disposable, owner-accessed Cache/Rate-Limit projections; their counters/leases are not Coordination Redis state and are not rebuilt from Redis after reset. Durable trigger/work-item/readiness state and execution correctness remain in PostgreSQL and the owning service. |
