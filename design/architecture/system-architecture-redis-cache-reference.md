...
The current Automation implementation also emits `automation:test:quota:*` and `automation:readiness:capacity:*` families. They are now named in the catalog below as Cache/Rate-Limit, Class B, reset-tolerant projections; this records their intended disposable role and does not claim that ACL, reset, metrics, or cluster proof is complete. Readiness and dry-run capacity counter/lease reservations currently pass two untagged keys to one Lua invocation, so a Redis Cluster deployment can reject them with `CROSSSLOT`; convergence requires a shared canonical hash tag for each atomic key set (or a single-key reservation) and focused proof before the target adoption is complete.
...
| `automation:queue:{tenantInstanceTag}:*`, `automation:quota:<tenantId>:*`, `automation:tenant-budget:<tenantId>:tier:<tier>`, `automation:test:quota:<tenantId>:script:<scriptId>:*`, `automation:test:capacity:<tenantId>:*`, `automation:test:capacity:cluster*`, `automation:readiness:capacity:<tenantId>:*`, `automation:readiness:capacity:cluster*` | Cache / Rate-Limit | TTL-only (Class B) | Reset-tolerant | Automation & Scripting queued work items, live and test quota counters, tenant/cluster dry-run capacity leases, and readiness capacity counters/leases. Durable triggers/effect tables in PostgreSQL, not Redis, guarantee eventual execution and quota correctness. Current readiness/dry-run reservation scripts still require same-slot key convergence and focused proof. |
