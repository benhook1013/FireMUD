...
- the live Game Session automation handoff carries `scriptPatchVersion`, positive `scriptPinEpoch`, and `scriptPinControlPlaneRequestId` on `EnqueueAutomationCommandIfAbsentRequest`; Game Session compares the tuple with authoritative owner state and rejects incomplete or stale evidence before staging or duplicate replay. Exact tuple checks also remain at the live instance-bound Automation ingress, timer, and work-item admission boundaries. The handoff still lacks `commandOrdinal` and the full Trigger Identity required for target-state per-command diagnostics. Live gameplay staging requires a present, nonblank durable `commandId` before Redis enqueue or durable batch staging, and `TickStagingService` derives `effectKey` as `command:<commandId>`; complete source/target binding and remote final-effect fence proof remain implementation gaps.
- Any future region-scoped/cross-source ordering allocator, including its scope and reset sequence behavior, remains conditional and non-authoritative pending [ADR 0183](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md); it cannot change the accepted current `enqueueSeq` or replay/effect identity rules. Timer/retry/remote-follow-up source-claim breadth and the cross-region result-return contract described below are also target-state follow-through rather than fully shipped behavior.
- the target deterministic command-plan root contract is not fully live: the owning command/action contract for the frozen typed command or `ResolvedEffectPlan` must supply semantic root order, with built-ins declaring equivalent stable order; ambiguous or duplicate order fails before admission. Before allocation, the durable gameplay command row must freeze/bind its request fingerprint, resolved runtime scope, and applicable `playableStateNamespaceId`. Game Session must persist the ordered plan manifest/digest and an opaque root allocation row unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`; the row binds the manifest digest and validates the frozen command/runtime/namespace binding, and retries reuse it only when all of those values match, otherwise failing closed. For Automation, the complete applicable Command-Handoff Identity must exact-map at admission to one durable target `commandId`; allocation then uses that target command row's canonical key. The current `tickBatchId`/`effectKey` derivation is not evidence of that contract.
...
Before staging, each admitted command is materialized as a deterministic plan: the ordered set of logical root effects for that command in the frozen typed command or `ResolvedEffectPlan` semantic order supplied by its owning command/action contract. Built-in commands declare an equivalent stable order; ambiguous or duplicate order fails before admission. The durable gameplay command row freezes/binds its request fingerprint and resolved runtime scope before Game Session persists the ordered plan manifest/digest and owns an opaque local durable allocation row unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`. The row binds the manifest digest and validates that frozen command/runtime binding; retries reuse it only when the command row identity, frozen binding, ordinal, and digest match, otherwise failing closed. One logical operation has one root `EffectId` with participant guards beneath it. A single-root plan uses `planOrdinal=0`, a multi-root plan uses stable zero-based ordinals, and a zero-effect plan allocates no ordinal or root. For Automation, the complete applicable Command-Handoff Identity exact-maps at admission to one durable target `commandId`, after which allocation uses that target command row's canonical key. This command-root contract does not synthesize plans for passive, inbound/remote, timer, retry, or already-generated effects that retain owner/source-specific identities unless they materialize as an admitted command. A shared root for a conserved multi-participant operation does not replace ADR 0053 atomicity, co-location, or reservation semantics.
...
- **Target state only; current live absent:** Consumer checkpoint and offset state is physically stored under region-scoped Redis keys but has one explicit authority per consumer: because checkpoints are per consumer, the target event-offset record `tick-events-offset:{tenantRegionTag}:{consumerId}` owns `streamOffset` and stores `{tenantId, gameInstanceId, regionId, consumerId, region_epoch, latestTickId, streamOffset}`. Its stable logical identity is `(tenantId, gameInstanceId, regionId, consumerId)`; `region_epoch`, `latestTickId`, and `streamOffset` are mutable timeline state, not identity fields. Durable schedule identity remains separately due-point-aware. Consumers must compare the event-offset record's stored epoch with the authoritative epoch before using its `streamOffset` or `latestTickId`; an epoch mismatch is a reset boundary, so the old record is discarded and rebuilt from the active canonical status/progress adapter before resuming. Current-live consumers do not persist or read this target offset record; they use `ObserveRuntimeTickProgress` and durable owner state instead.
- The logical API/contract field `regionEpoch` is serialized as the Redis offset field `region_epoch` in this target record (and `latestTickId`/`streamOffset` retain their serialized spellings). Producers, readers, and validators must apply this explicit `regionEpoch` -> `region_epoch` mapping rather than treating the two names as competing fields.

Consumers that persist offsets must key stable consumer identity by `(tenantId, gameInstanceId, regionId, consumerId)` and store `regionEpoch` and `latestTickId` as mutable timeline state. For durable schedules, `scheduleInstanceId` is the immutable physical row-generation identity allocated under the materializing operation; it is not the stable logical continuity identity and is not sufficient for trigger uniqueness. The scheduler-owned stable continuity key is `{stableOwnerKind, stableOwnerId, scheduleDefinitionId, playableStateNamespaceId, targetScopeType, targetScopeId}` for each `isDryRun` mode; compatible transitions may retain the same row generation, while reset or incompatible replacement can allocate a fresh `scheduleInstanceId` under that continuity model. Mutable runtime-region metadata (`runtimeRegionId`, `runtimeRegionEpoch`), due point, and exact pin/provenance remain schedule state or evidence rather than continuity identity. Trigger-instance de-duplication and candidate-audit identity instead use the complete event-scope due-candidate/firing identity owned by [Scripting Scheduler and Timers](./system-architecture-scripting-scheduler-and-timers.md#target-state-design) and [Normative Table 1](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields), including applicable runtime region/epoch, event/schema, exact patch/pin, dry-run, trigger mode, target selector, exactly one tagged due point (`dueTickId` or `dueAt`), catch-up `resumeWindowId`, and schedule-owned script/plugin fields. A wall-clock source may carry a separately assigned admission `due_tick_id` for tick ordering, but it does not become or alias the source `dueAt`; tick IDs and epoch-millisecond values are never compared. This document does not redefine that schema or mutate the schedule itself. Any observed epoch jump is a reset boundary: state derived from the old epoch is discarded or reconciled from PostgreSQL before resuming.
...
Target-state only (currently unavailable): once implemented, the Game Session Service will expose a **tick event stream** in addition to the gRPC heartbeat for schedulers and observers. The current live deployment has no `tick-events:*` producer, stream-offset store, or consumer lease; current Automation integration uses `ObserveRuntimeTickProgress` and durable PostgreSQL schedules. The contract below defines the future stream and its reset-tolerant behavior and is not a current runtime surface:
...
- The event stream is a **best-effort coordination structure**, not a durable log of record:
  ...
  - Once implemented, it is classified as **reset-tolerant** in the Redis reset policy matrix: region/tenant/cluster resets may drop both the event stream and any stored offsets without violating correctness.
...
This event stream is an observer/wakeup hint used for reconnection timer replay hints and other out-of-band reporting. “Every N ticks” scheduling correctness comes from the active deployment's committed status/progress timeline (target-state heartbeat/RegionStatus) plus durable PostgreSQL schedules; tick events may reduce latency by prompting quicker work discovery, but missing or duplicated events must not silently drop correctness-bearing one-shot intent or eligible durable work. Policy-governed recurring and advisory missed occurrences may be skipped as specified by the scheduler recovery policy.

Durable automation schedules and trigger-instance de-duplication remain PostgreSQL/domain-owned. In the target state, Account owns plan-driven quota values through tenant entitlement and plan-limit authority; the current Automation implementation instead uses service configuration for its limits. Live per-script and tenant-tier quota reservations/counters are Redis-backed enforcement state (for example, `automation:tenant-budget:<tenantId>:tier:<tier>`). Redis quota counters are reset-tolerant and a reset may temporarily relax fairness; it does not change durable schedule or work truth. The current `tick-events:{tenantRegionTag}` stream and its offsets are separate best-effort coordination hints only; losing or resetting those hints must not silently drop correctness-bearing one-shot intent or eligible durable work or create duplicate durable claims, but may affect discovery timing and policy-governed missed recurring/advisory firings, including declared `SKIP_MISSED`, cap-excluded, or terminal capacity-denial outcomes. The `script-scheduler:{tenantRegionTag}:lastTickId` projection is target-only and unimplemented, with no current reset operation; once delivered, it will be a similarly reset-tolerant discovery hint rather than an authority for automation work.
...
- In target-state deployments, once implemented, maintain per-region scheduler state in Redis as a derived discovery hint (for example `script-scheduler:{tenantRegionTag}:lastTickId`). Its value stores `{regionEpoch, latestTickId}` and never owns or persists `streamOffset`; the canonical per-consumer event-offset record remains `tick-events-offset:{tenantRegionTag}:{consumerId}`. The target scheduler must reject the hint when its epoch differs from the authoritative current epoch, discard it on reset, and rebuild it from the current canonical status/progress observation. This Redis projection is currently unimplemented and has no current reset operation; current live Automation instances must use `ObserveRuntimeTickProgress` and durable PostgreSQL schedules without reading or writing it. If the event stream is truncated, only each affected consumer's canonical event-offset record is invalidated for resumption; a target scheduler still validates or rebuilds its separate hint from canonical progress rather than using the hint as a stream offset. The event-offset identity is `(tenantId, gameInstanceId, regionId, consumerId)`, while `regionEpoch`, `tickId`, and the stream event offset are never key dimensions.
- After a due candidate passes the applicable schedule-admission checks, claim or insert a durable PostgreSQL trigger-instance/outbox row keyed by an **instance-aware** uniqueness projection before enqueueing any `onInterval` or other tick-derived trigger so duplicate heartbeat consumers or failover cannot create duplicate logical gameplay actions. Discovery of a due candidate alone does not create a firing claim or `scriptEventId`; a terminal/permanent or declared pre-admission skip is recorded as an immutable event-scope candidate audit under a deterministic `scheduleCandidateId` derived from the complete canonical event-scope due-candidate fields, while only owner-defined transient retryable reload, reconciliation, or authority-fence failures retain the original exact-tuple due candidate as fenced, retry-eligible nonterminal state with no firing claim or `scriptEventId` until settlement. The schedule's canonical natural-key fields participate as defined by the scheduler owner; a surrogate `scheduleInstanceId` is only physical row-generation metadata and is not an identity input or substitute.
  - The complete trigger-instance due-candidate/firing/candidate-audit identity and its field-presence rules are owned by [Scripting Scheduler and Timers](./system-architecture-scripting-scheduler-and-timers.md#target-state-design) and [Normative Table 1](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields): applicable runtime region/epoch, event/schema, exact patch/pin, dry-run, trigger mode, target selector, exactly one tagged due point (`dueTickId` or `dueAt`), catch-up `resumeWindowId`, and schedule-owned script/plugin fields. `automationDispatchId` plus `commandOrdinal` remains the per-command child identity and is not a replacement for the parent Trigger Identity.

Target-state only: Before schedule admission resumes under a new `scriptPinEpoch`, an atomic durable or resumable-idempotent reconciliation must fence pending occurrences/claims from the old `scriptPinEpoch` and make old-epoch enqueue attempts fail. Reconciliation separates scheduler candidate-audit state from linked durable tick/effect/source-claim state: when durable evidence proves the linked effect was applied, its original durable identity remains terminal `APPLIED`; when evidence proves no firing/effect was accepted or applied, the linked effect/source claim may terminalize `ABANDONED` with its owner-defined explicit reset/reconciliation reason, while an applicable scheduler candidate is recorded only with its owner-defined canceled/permanent-skip outcome and bounded reason, never `APPLIED` or `ABANDONED`. An ambiguous or unreadable claim remains nonterminal/reconciliation-required under its original identity until reconciled. The new epoch cannot enqueue the same schedule occurrence unless it is explicitly represented by a distinct durable logical occurrence/due identity; candidate-audit identity and outcome remain immutable. Detailed transition and recovery semantics remain with [Scripting Scheduler and Timers](./system-architecture-scripting-scheduler-and-timers.md), [Transaction Strategies](./system-architecture-transactions.md#tick-effects-are-at-least-once-idempotency-is-mandatory), and the [Inconclusive Old-Epoch Reconciliation Policy](./system-architecture-tick-failures-and-operations.md#inconclusive-old-epoch-reconciliation-policy). See [current scripting runtime status](./system-architecture-scripting-runtime-execution.md#current-implementation-status) and the [Game Session implementation status](../project-management/implementation-tracking/game-session-runtime-and-tick-coordination.md#script-pin-epoch-and-rollout-history-reconciliation).
...
- **Target-state follow** via streaming heartbeats:
...
Tick execution never depends on external buses. In target-state deployments, external services consume the heartbeat stream and/or tick event stream only; current-live Automation uses the unary `ObserveRuntimeTickProgress` progress adapter. See `system-architecture-tick-concepts-and-invariants.md`, `system-architecture-scripting-dsl-reference-and-lifecycle.md`, and `system-architecture-scripting-runtime-execution.md` for details.
...
World Management owns region topology (layout and `<regionId>` assignments scoped by `{tenantId, gameInstanceId}`) and may, over time, support “drain and split” or “merge” flows. These flows and the protocol below are target-state tooling, not current operator capabilities: the live durable ownership boundary remains instance-scoped, and no shipped split/merge cutover or region-scoped reset/rebuild controller currently exists.
...
### Topology Changes (Split/Merge) Protocol (Target-State Required Invariants)

Region split/merge operations interact directly with tick idempotency, Redis key ownership, and cross-region follow-ups. Once the target topology cutover and recovery tooling is implemented and proven, topology changes must follow a single, explicit protocol:
...
4. **Reset or migrate coordination state using shared key builders**
   - Target-state first implementation resets/rebuilds coordination state from durable PostgreSQL state after the epoch bump instead of moving live Redis keys; this is unavailable until the reset/rebuild controller and its focused proof exist.
...
6. **Resume (target-state release)**
   - Re-enable tick scheduling and command intake only after the target mapping, durable reconciliation, coordination reset/rebuild, and owner-defined health/release evidence are complete.
...
### Current Topology Cutover Fallback

The current runtime has no supported split/merge operation, region-partitioned durable ownership boundary, or topology reset/rebuild/resume controller. Do not move or rename live Redis queues, hand-edit coordination keys, flush/rebuild a topology scope, or invoke the target protocol. For an actual topology or reset incident, use only the shipped instance-scoped `PauseTicksForScope` / `GetRuntimeOwnershipStatus` boundary and read-only inspection, keep the affected instance paused/fenced or stopped, and escalate through [Redis Reset & Recovery](./system-architecture-redis-reset-and-recovery.md#current-operator-fallback). The current instance-scoped resume endpoint is an ordinary pause control, not evidence that a mapping-changing cutover or reset/rebuild completed; it must not be used to release this fallback.
...
- Commit: call into domain services, which apply changes under the root/participant guard contract; the region-scoped tick timeline `(regionEpoch, tickId)` supplies ordering and fencing context across replays and resets.
...
- Domain-level idempotency rules use root/participant guards and namespace-complete effect projections for logical identity; `(regionEpoch, tickId)` remains ordering and fencing context rather than a universal mutation token.

Crash recovery re-invokes a domain handler for a staged effect only when durable evidence proves that the `SCHEDULED` effect was unapplied and replay is safe under its owner contract; a missing response or ambiguous guard leaves the effect `SCHEDULED` and reconciliation-required. Replays must not double-apply logical effects. Redis loss is evaluated against the measured `redis_unreplicated_write_window_slo_ms`; `ticks_exposed` is diagnostic only. The namespace-complete storage projection tuple `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectKey, targetAggregateType, targetAggregateId)` identifies the ledger/storage view; durable projections also retain the persisted root-or-child `EffectId`, enclosing `rootEffectId` lineage, typed operation, immutable request digest, and required command/source bindings where allocated. `playableStateScope` is separately persisted and exact-validated evidence, while `regionEpoch` and `tickId` provide ordering/timeline context rather than guard uniqueness. Participant-guard logical uniqueness is the owner-declared runtime-family partition plus the persisted root-or-child `EffectId`, typed operation, and exact target aggregate: root operations use the root `EffectId`, while generated children use their persisted child `EffectId` and retain `rootEffectId` only as enclosing lineage and reconciliation context. S1/S2 uses `(tenantId, playableStateNamespaceId)` and only an explicitly owner-classified S3 family uses `(tenantId, gameInstanceId)`; the partition must be sealed and exact-validated and cannot create a second guard or fork the same logical mutation. For S1/S2, `gameInstanceId` is separately exact-validated active-runtime authorization/fence evidence; for all families, `playableStateScope`, timeline, digest, and lineage remain immutable bound projection evidence rather than additional uniqueness dimensions. Game Session command-root allocation remains the opaque durable command-row identity `(tenantId, gameInstanceId, commandId, planOrdinal)`; passive, inbound/remote, timer, retry, and already-generated effects retain owner/source-specific identities. The combination of the coordination timeline, tick effect ledger, and per-service idempotency guards applies the ADR 0058 class-specific outcomes: each projection converges to a single terminal outcome (`APPLIED` or `ABANDONED`) only when the evidence policy permits terminalization; an inconclusive old-epoch projection remains reconciliation-required and non-terminal under its original mutation `EffectId`, with enclosing `rootEffectId` retained as lineage and reconciliation context. See [ADR 0058](./decisions/adr-0058-class-specific-redis-loss-outcomes.md) and [Tick Failure and Operations](./system-architecture-tick-failures-and-operations.md#inconclusive-old-epoch-reconciliation-policy).

---

## Timers and Time Scaling

Every scheduler/timer source carries exactly one tagged, persisted source due point: `dueTickId` for tick-based scheduling or immutable `dueAt` for wall-clock scheduling; it carries neither both nor neither as its source due point. Tick-based queued commands, retries, remote follow-ups, and tick timers use their persisted `dueTickId` directly. A wall-clock timer remains eligible from its immutable `dueAt` (`now >= dueAt`). Once eligible, one durable firing-claim/admission transaction reads the authoritative target timeline and assigns/persists exactly one target `regionEpoch` and `due_tick_id` ordering coordinate (normally the next committable tick), together with the source claim. Retries and replays reuse those persisted values and never recompute them from cadence or wall time. An epoch change follows old-epoch reconciliation and new admission; it never rebinds the old firing. The within-entity tuple uses the persisted admission `due_tick_id` for wall-clock timers and the persisted `dueTickId` for tick timers. Wall-clock timestamps and tick IDs are different domains and must not be compared numerically or aliased.

Timer projections are stored and scheduled via Redis timer keys and remain aligned with the tick heartbeat and tick cadence. Time-scaling rules may speed up or slow down perceived time while preserving ordering, but a later cadence or time-scale change never rewrites an existing due point. Wall-clock eligibility uses caller-supplied `now_ms` in Lua (never Redis `TIME`); tick eligibility uses the committed region timeline.
...
Domain services use the region-scoped tick timeline `(regionEpoch, tickId)` for ordering and fencing, not as the canonical idempotency token for a logical effect. The default mutation boundary is the mutation's stable persisted `EffectId` plus a participant guard containing the typed operation, exact target aggregate, and immutable request digest. Root effects use the root `EffectId`; generated child mutations use their deterministic persisted child `EffectId` and retain the enclosing root as lineage. A narrow last-tick watermark is permitted only for an explicitly documented aggregate proven to receive at most one logical mutation per tick timeline; it must not replace the effect/participant guard for general command effects. Exact allocator format and allocation-boundary details remain pending and non-authoritative in [ADR 0183](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md).

- Single-aggregate updates may use an epoch-aware “last applied tick” pattern (for example storing and comparing `(last_region_epoch, last_tick_id)` for the aggregate) only when the owning contract proves that at most one logical update for that aggregate can occur in each tick. Without that one-update-per-aggregate-per-tick invariant, each logical effect requires a participant guard derived from the stable root `EffectId`, typed operation, and target aggregate; a root-only ledger or guard is insufficient. The timeline tuple is ordering/fence context and is not sufficient idempotency identity.
- Multi-aggregate operations use explicit `tick_effect_guard` storage projections that retain the namespace-complete fields (for example `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectKey, targetAggregateType, targetAggregateId)`) and link each row to the enclosing root lineage plus the mutation's persisted root-or-child `EffectId` and derived typed operation/target guard identity. The guard uniqueness identity follows the owner-declared runtime family: S1/S2 uses the `(tenantId, playableStateNamespaceId)` partition plus `EffectId`, typed operation, and exact target aggregate, while only an explicit S3 family uses `(tenantId, gameInstanceId)` plus those identity fields. For S1/S2, `gameInstanceId` is separately exact-validated active-runtime authorization/fence evidence; `playableStateScope`, timeline, digest, and lineage remain immutable evidence, not uniqueness dimensions. Unknown or unclassified families fail closed and are never inferred to be S3.

### Tick Effect Identity and Idempotency Contract

Effect identity and idempotency rules are defined jointly by:

- The `(regionEpoch, tickId)` carried on tick-driven calls as ordering/fencing context.
- **Target-state plan-root identity:** Game Session supplies tick execution with the persisted deterministic command-plan root binding owned by [Transaction Strategies](./system-architecture-transactions.md#command-atomicity-and-outcome-aggregation). That frozen binding includes the applicable `playableStateNamespaceId` as exact runtime evidence and provides the stable root `EffectId`; participant guards derive from that root or a deterministic persisted child, typed operation, exact target aggregate, and immutable request digest. Tick scheduling and `(regionEpoch, tickId)` fence that work but never allocate, replace, or reinterpret its `EffectId`/`planOrdinal`; owner/source-specific non-command roots remain distinct. Script-generated commands retain `(automationDispatchId, commandOrdinal)` in their admitted operation identity, so `scriptEventId` alone cannot distinguish fan-out commands. The current live path is documented above and remains a tracked implementation gap.

The root identity and generated-child identity distinction is canonical in `system-architecture-transactions.md`; ledger records retain the enclosing root lineage, while participant guards use the mutation's persisted root-or-child `EffectId` with its deterministic operation/target projection rather than ad-hoc keys. The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is non-authoritative.
