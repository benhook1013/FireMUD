...
The phases are ordered as follows: (1) due start-of-tick passive effects, (2) one selected actor action for each still-eligible entity, and (3) effects generated by those selected actor actions. Passive or inbound work that becomes due after start selection is deferred to a later tick; phase 3 does not drain it. A start-of-tick effect may make an entity ineligible to act. An actor-generated effect does not retrospectively remove an action that already executed, and neither lane may recursively grant another root actor action in the same tick.

Every work source declares its phase, lane, and cost class before admission. Within each phase, the persisted manifest retains its phase, lane, and cost class alongside stable ordering inputs such as policy-defined priority, due point, enqueue sequence, source kind, stable source-item/effect identity, and entity identity where applicable; comparisons are phase-local or explicitly phase-aware. Accepted TICK-12 FIFO applies within the same per-entity contention-retry/source queue. Cross-source relative ordering and the tie-break tuple—including how queue-local FIFO interacts with priority/due ordering—remain deferred to TICK-09 and must not be implemented from this parcel. Each lane has separate per-entity count and cost caps, region-wide work budgets, bounded carry-over/backoff, and pressure metrics. Overflow is deferred without changing its persisted ordering inputs or retry/action/effect identity; a new source cannot silently introduce a custom scheduler.
...
- Drains actor-generated effects and other admitted passive/inbound work under their separate budgets.
...
See `system-architecture-tick-execution-flows.md` for the detailed algorithm, including lane classification, retry identity, persisted ordering inputs (with exact relative ordering and tie-break tuple deferred to TICK-09), and bounded carry-over.
...
Crash recovery replays staged ticks safely by re-invoking domain handlers; replays must not double-apply logical effects. Even when Redis loses or replays up to a few ticks within the tail-loss envelope, the combination of the coordination timeline, the tick effect ledger, and per-service idempotency guards ensures that each explicit ledger/guard storage projection `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, effectKey, targetAggregateType, targetAggregateId)`, linked to one stable root `EffectId`, converges to a single terminal outcome (`APPLIED` or `ABANDONED`) when the evidence policy permits terminalization; an inconclusive old-epoch projection remains reconciliation-required and non-terminal under its original root `EffectId`. See [Tick Failure and Operations](./system-architecture-tick-failures-and-operations.md#inconclusive-old-epoch-reconciliation-policy) for the detailed policy.
