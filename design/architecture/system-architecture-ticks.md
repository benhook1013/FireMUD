...
Durable automation schedules, quotas, and trigger-instance de-duplication live in PostgreSQL (see the scripting DSL and Automation & Scripting service docs). The current `tick-events:{tenantRegionTag}` stream and its offsets are best-effort coordination hints only; losing or resetting them must not change which automation jobs are eventually executed, only when they are next discovered. The `script-scheduler:{tenantRegionTag}:lastTickId` projection is target-only and unimplemented, with no current reset operation; once delivered, it will be a similarly reset-tolerant discovery hint rather than an authority for automation work.
