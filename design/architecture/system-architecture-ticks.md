...
- Domain-level idempotency rules use root/participant guards; `(regionEpoch, tickId)` remains ordering and fencing context rather than a universal mutation token.

Crash recovery replays staged ticks safely by re-invoking domain handlers; replays must not double-apply logical effects. Redis loss is evaluated against the measured `redis_unreplicated_write_window_slo_ms`; `ticks_exposed` is diagnostic only. The collision-safe ledger/storage projection is `(tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, effect_id, root_effect_id, typed_operation, effect_key, target_aggregate_type, target_aggregate_id)`, where `effect_id` is the persisted root-or-child mutation identity and `root_effect_id` is the enclosing root retained for lineage and reconciliation; `effect_key` is descriptor/query metadata only, not a ledger identity or replay key. `playable_state_namespace_id` is immutable bound runtime evidence and must exact-match the sealed execution context and request digest where applicable; it does not extend the runtime-instance tick timeline, batch-coordinate uniqueness, or scheduler partition. Participant guard uniqueness remains separately `(effect_id, typed_operation, target_aggregate_type, target_aggregate_id)` as owned by Transaction Strategies; tenant/game, namespace, and scope are exact-bound evidence, and any mismatch fails closed rather than creating a second guard. For command-root work, the persisted `planOrdinal` and opaque root allocation binding are retained alongside that storage view; `tickBatchId`, source ordering, `effectKey`, and participant tuples remain distinct metadata/guard projections and cannot substitute for the durable command row identity. Passive, inbound/remote, timer, retry, and already-generated effects retain their owner/source-specific identities. The immutable request digest is bound to and compared by that participant guard. The combination of the coordination timeline, tick effect ledger, and per-service idempotency guards applies the ADR 0058 class-specific outcomes: each projection converges to a single terminal outcome (`APPLIED` or `ABANDONED`) only when the evidence policy permits terminalization; an inconclusive old-epoch projection remains reconciliation-required and non-terminal under its original identity. See [ADR 0058](./decisions/adr-0058-class-specific-redis-loss-outcomes.md) and [Tick Failure and Operations](./system-architecture-tick-failures-and-operations.md#inconclusive-old-epoch-reconciliation-policy).
...
- **Target-state plan-root identity:** Game Session supplies tick execution with the persisted deterministic command-plan root binding owned by [Transaction Strategies](./system-architecture-transactions.md#command-atomicity-and-outcome-aggregation). That frozen binding includes the applicable `playableStateNamespaceId` as exact runtime evidence. Tick scheduling and `(regionEpoch, tickId)` fence that work but never allocate, replace, or reinterpret its `EffectId`/`planOrdinal`; owner/source-specific non-command roots remain distinct. The current live path is documented above and remains a tracked implementation gap.
