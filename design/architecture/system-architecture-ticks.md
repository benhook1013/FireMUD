...
Before schedule admission resumes under a new `scriptPinEpoch`, an atomic durable or resumable-idempotent reconciliation must fence or terminalize pending occurrences/claims from the old `scriptPinEpoch` and make old-epoch enqueue attempts fail. The new epoch cannot enqueue the same schedule occurrence unless it is explicitly represented by a distinct durable logical occurrence/due identity; candidate-audit identity and outcome remain immutable. Detailed transition and recovery semantics remain with [Scripting Scheduler and Timers](./system-architecture-scripting-scheduler-and-timers.md).
