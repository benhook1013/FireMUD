...
- the live Game Session automation handoff carries only `scriptPatchVersion`: `EnqueueAutomationCommandIfAbsentRequest` has no `scriptPinEpoch` or owner request identity, so this downstream boundary cannot reject same-version work from an older epoch. Exact tuple checks remain at the live instance-bound Automation ingress, timer, and work-item admission boundaries, while this handoff still lacks `commandOrdinal` and the full Trigger Identity required for target-state per-command diagnostics. Live gameplay staging requires a present, nonblank durable `commandId` before Redis enqueue or durable batch staging, and `TickStagingService` derives `effectKey` as `command:<commandId>`; the target-state complete handoff identity, source/target binding, and final-effect fence proof remain implementation gaps.
