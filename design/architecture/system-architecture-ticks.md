...
Every work source declares its lane and cost class before admission. Within each phase, persisted ordering uses a deterministic tuple containing priority, due point, enqueue sequence, source kind, stable source-item/effect identity, and entity identity; comparisons are phase-local or explicitly phase-aware. Each lane has separate per-entity count and cost caps, region-wide work budgets, bounded carry-over/backoff, and pressure metrics. Overflow is deferred without changing its order or retry/action/effect identity; a new source cannot silently introduce a custom scheduler.
...
See `system-architecture-tick-execution-flows.md` for the detailed algorithm, including lane classification, retry identity, deterministic ordering, and bounded carry-over.
