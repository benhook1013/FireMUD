...
Every work source declares its lane and cost class before admission. Within each phase, persisted ordering uses a deterministic tuple containing priority, due point, enqueue sequence, source kind, entity identity, and the original action or effect identity. Each lane has separate per-entity count and cost caps, region-wide work budgets, bounded carry-over/backoff, and pressure metrics. Overflow is deferred without changing its order or retry/action/effect identity; a new source cannot silently introduce a custom scheduler.
