...
Durable automation schedules and trigger-instance de-duplication remain PostgreSQL/domain-owned. In the target state, Account owns plan-driven quota values through tenant entitlement and plan-limit authority; the current Automation implementation instead uses service configuration for its limits. Live per-script and tenant-tier quota reservations/counters are Redis-backed enforcement state (for example, `automation:tenant-budget:<tenantId>:tier:<tier>`). Redis quota counters are reset-tolerant and a reset may temporarily relax fairness; it does not change durable schedule or work truth. The target `tick-events:{tenantRegionTag}` stream and its offsets are separate best-effort coordination hints only; once implemented, losing or resetting those hints may delay next discovery but must not change which automation jobs are eventually executed or create duplicate durable claims. The current live path has no such stream or offsets and uses `ObserveRuntimeTickProgress`. The `script-scheduler:{tenantRegionTag}:lastTickId` projection is target-only and unimplemented, with no current reset operation; once delivered, it will be a similarly reset-tolerant discovery hint rather than an authority for automation work.
...
- The `(regionEpoch, tickId)` carried on tick-driven calls as timeline ordering/fence context, not as the standalone logical identity of an effect.
