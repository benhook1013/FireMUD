...
- Consumers (for example, schedulers or reconnection logic) typically:
  ...
  - Persist the canonical event-offset record `{tenantId, gameInstanceId, regionId, regionEpoch, latestTickId, streamOffset}` in **Coordination Redis** under `tick-events-offset:{tenantRegionTag}`. This record is the sole owner of `streamOffset`; scheduler hints must not copy or compete with it. Its logical scope is `(tenantId, gameInstanceId, regionId, regionEpoch)`, so consumers can resume from the last observed stream entry only after the stored epoch matches the authoritative epoch. If the record is missing, epoch-mismatched, or the stream has been truncated/reset, consumers discard it and bootstrap from the active deployment's canonical status/progress adapter (`GetRuntimeOwnershipStatus` plus `ObserveRuntimeTickProgress` currently, `GetRegionTickStatus` plus `StreamTickHeartbeats` at target state), then establish a new offset rather than assuming the stream is a complete history.
...
- Maintain per-region scheduler state in Redis (for example `script-scheduler:{tenantRegionTag}:lastTickId`) only as a derived discovery hint. Its value stores `{regionEpoch, latestTickId}` and never owns or persists `streamOffset`; the canonical event-offset record remains `tick-events-offset:{tenantRegionTag}`. The scheduler must reject the hint when its epoch differs from the authoritative current epoch, discard it on reset, and rebuild it from the current canonical status/progress observation. If the event stream is truncated, only the canonical event-offset record is invalidated for resumption; the scheduler still validates or rebuilds its separate hint from canonical progress rather than using the hint as a stream offset. Its logical scope is `(tenantId, gameInstanceId, regionId, regionEpoch)`, while `tickId` and the event stream offset are never key dimensions.
...
- After a due candidate passes admission and before enqueueing its trigger, the scheduler must claim or insert a durable trigger-instance row keyed by an **instance-aware** uniqueness projection. Duplicate schedulers that race on the same heartbeat boundary must observe the same durable row and must not enqueue a second logical trigger.
  - The pre-claim uniqueness projection must include the stable scheduler Trigger Identity fields `tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `entityId`, `scriptId`, `eventType`, `eventSchemaVersion`, `scriptPatchVersion`, `isDryRun`, `scheduleDefinitionId`, the persisted tagged due point, and `triggerMode`; plugin identity means all three `pluginId`, `pluginVersionId`, and `bindingId`, even when `scheduleId` is globally unique. It must not include generated `scriptEventId`; the winning claim derives/allocates that immutable event identity only after the stable claim succeeds while retaining those plugin binding-owner fields, and all contenders reuse the winner's result.
- On startup or after a reset:
  ...
  - If either the scheduler hint or the canonical event-offset record has a different `regionEpoch` than the current status, the scheduler rejects the stale value, records `checkpoint_region_epoch_mismatch` in scheduler audit, increments `automation_script_timer_runtime_fence_dropped_total{scope, script_category, eventType, reason="checkpoint_region_epoch_mismatch"}`, discards the stale hint and event offset, and rebuilds the hint and event-offset record from authoritative current-epoch status before reconciling durable due points under the bounded catch-up policy.
  - A reset that bumps `regionEpoch` closes the old-epoch durable schedule rows and creates or confirms current-epoch rows for every still-enabled schedule before timer admission resumes. Close-and-recreate is one atomic durable result or a resumable idempotent operation keyed by the reset's control-plane request identity; retries confirm the new row before tombstoning the old row, so a reset cannot strand a logical schedule or leave both epochs admitting work.
  - Epoch rebind preserves due-point semantics rather than copying an old timeline coordinate. An absolute `dueAt` remains the same durable instant. A tick-aligned schedule derives a fresh current-epoch `dueTickId` from its persisted cadence and the normative resume rule against the captured new-epoch progress; it must not reuse the old numeric `dueTickId`. Old-epoch firing claims and `scriptEventId` values remain immutable history and are never attached to the current-epoch row.
...
- After recovery, the scheduler:
  ...
  - Uses `script-scheduler:{tenantRegionTag}:lastTickId` only as a derived discovery hint; losing the hint or losing/truncating the canonical `tick-events-offset:{tenantRegionTag}` record affects when work is next discovered, not which durably-configured schedules eventually execute. The scheduler must rebuild the affected state from the active canonical status/progress source, and durable trigger-instance uniqueness remains the de-duplication boundary.
...
- **Target-state effect identity:** A stable, structured effect identity is derived deterministically from the command payload and tick context, including at minimum `tenantId`, `gameInstanceId`, resolved `playableStateScope`, `regionId`, `regionEpoch`, `tickId`, `effectKey`, `targetAggregateType`, and `targetAggregateId`. For script-generated commands, `effectKey` must incorporate the command-level `(automationDispatchId, commandOrdinal)`; `scriptEventId` alone cannot distinguish fan-out commands. The current live path is documented above and remains a tracked implementation gap.
