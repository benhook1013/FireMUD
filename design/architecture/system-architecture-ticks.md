...
- the live `tick_batch` / `tick_effect` substrate is real, and the current gameplay-command manifest now carries current-boundary `enqueueSeq`, `sourceType`, `dueTickId`, and explicit claimed-source state plus digest-checked replay reuse. The live `enqueueSeq` comes from one database-wide sequence; the target allocator is cross-source and monotonic per entity only within the complete `(tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch)` batch scope, with `playableStateScope` separately persisted and exact-validated as fence evidence. An epoch reset starts a new sequence domain and may reuse a numeric value, while old-epoch ordering identities remain immutable and are never rebound. Timer/retry/remote-follow-up source-claim breadth and the cross-region result-return contract described below are also target-state follow-through rather than fully shipped behavior.
- the live Game Session automation handoff does not yet carry `scriptPinEpoch`, `commandOrdinal`, or the complete applicable Command-Handoff Identity required for target-state per-command diagnostics. `TickStagingService` currently derives `effectKey` as `command:<commandId>` and falls back to `command-text:<hash>:slot:<index>` when no command id is available; the target-state exact pin tuple and complete command-handoff identity are therefore implementation gaps, not current behavior.
