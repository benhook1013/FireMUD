...
`(regionEpoch, tickId)` is ordering and fencing context, not a universal idempotency token for tick-driven mutations. The default mutation boundary is the stable root `EffectId` plus a participant guard containing the typed operation, exact target aggregate, and immutable request digest. A narrow last-tick watermark is permitted only for an explicitly documented aggregate proven to receive at most one logical mutation per tick timeline; it must not replace the root/participant guard for general command effects.
...
- **Target-state plan-root identity:** Game Session materializes the deterministic root-effect plan for each admitted command in the frozen typed command or `ResolvedEffectPlan` semantic order supplied by its owning command/action contract; built-in commands declare an equivalent stable order, and ambiguous or duplicate order fails before admission. The durable gameplay command row freezes/binds its request fingerprint and resolved runtime scope before allocation. Game Session persists the ordered plan manifest/digest and owns an opaque root allocation row unique on `(tenantId, gameInstanceId, commandId, planOrdinal)` in the local durable plan/staging transaction; the row binds the manifest digest and validates the frozen command/runtime binding. Reuse requires the same command row identity, frozen binding, ordinal, and manifest digest, otherwise failing closed. A single-root plan uses `planOrdinal=0`, a multi-root plan uses stable `0..n-1`, and a zero-effect plan allocates neither. Ordinary player/built-in plans use their complete canonical admitted command/runtime identity. For Automation, the complete applicable Command-Handoff Identity is the admission/deduplication identity and must exact-map to one durable target `commandId`; after that mapping, allocation uses the target command row's same canonical key. Passive, inbound/remote, timer, retry, and already-generated effects retain their owner/source-specific root identities and do not synthesize a command plan unless they materialize as an admitted command. Participant guard identities derive from the persisted root effect, typed operation, and target aggregate; they bind the immutable request digest. `tickBatchId`, mutable command text, `effectKey`, `sourceOrdinal`/`enqueueSeq`, participant tuples, Trigger Identity/`scriptEventId`, and generated child ordinals cannot substitute for the durable command row identity or allocation binding. Ordinary retry, replay, and reconciliation reuse the persisted plan/root binding. The current live path is documented above and remains a tracked implementation gap.
