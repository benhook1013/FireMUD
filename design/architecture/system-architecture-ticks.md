...
- **Target-state plan-root identity:** Game Session supplies tick execution with the persisted deterministic command-plan root binding owned by [Transaction Strategies](./system-architecture-transactions.md#command-atomicity-and-outcome-aggregation). Tick scheduling and `(regionEpoch, tickId)` fence that work but never allocate, replace, or reinterpret its `EffectId`/`planOrdinal`; owner/source-specific non-command roots remain distinct. The current live path is documented above and remains a tracked implementation gap.
