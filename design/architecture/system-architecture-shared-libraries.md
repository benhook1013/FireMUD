...
- **Database Connectors** – `DatabaseAutoConfiguration` with `PostgresProperties` and `RedisProperties` reduces boilerplate setup. The current `RedisProperties` and connection factory expose one generic `firemud.redis` host/port, so Redis-backed services do not yet choose independent role-specific clients and prefixes. The target role-qualified binding is owned by [Redis Architecture](./system-architecture-redis.md) and the [Environment & Secrets Catalog](./infrastructure/environment-and-secrets-catalog.md):
  - Coordination clients (ticks, locks, timers, sessions) must bind to `FIREMUD_REDIS_COORD_HOST` / `FIREMUD_REDIS_COORD_PORT`.
  - Cache/rate-limit clients (for example Spring Cloud Gateway) must bind to `FIREMUD_REDIS_CACHE_HOST` / `FIREMUD_REDIS_CACHE_PORT`.
