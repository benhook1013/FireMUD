...
- **Test and lint hooks** – Shared test fixtures validate that:
  ...
  - Automation scripts are declared as **single-hash-slot** scripts (hash-tagged on `{tenantInstanceScriptTag}`) and are forbidden from mixing `automation:tick:{tenantInstanceScriptTag}:*` keys with `tick:{tenantRegionTag}:*` keys in a single invocation; tests assert hash-slot alignment for automation keys when they are multi-key.
