...
Redis helpers follow the owner-local execution and aggregated-contract boundary in [ADR 0171](decisions/adr-0171-owner-local-redis-execution-with-aggregated-contracts.md).
...
The shared module owns this reusable table contract, but each adopter deliberately includes it and owns application into its own schema, Flyway history, deployment, and failure recovery under [ADR 0080](decisions/adr-0080-service-owned-schemas-with-adopter-local-shared-migrations.md). Shared migration versions cannot collide with adopter-local versions, and neither the library nor an adopter may use the shared artifact to alter another service's schema.
