...
- `requiredTriggerIdentityFields`
  ...
  - For gameplay-originated events whose producer already resolved shared-versus-isolated realm state, the event envelope must carry `playableStateScope` as separately persisted and exact-validated policy/routing/authorization/migration-fence evidence. After handler resolution, the full Trigger Identity must include `playableStateNamespaceId`; the namespace identity prevents distinct playable-state namespaces from collapsing, while `playableStateScope` remains excluded from Trigger, schedule, and handoff uniqueness.
