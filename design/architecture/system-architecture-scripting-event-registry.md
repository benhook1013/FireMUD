...
- `requiredTriggerIdentityFields`
  ...
  - Scheduler/timer due candidates are another deliberate pre-handler exception: because Automation already materialized a durable schedule with its owner identity, the scheduler firing identity and deterministic event-scope `scriptEventId` include the schedule-owned `scriptId` and any schedule-owned `pluginId`, `pluginVersionId`, and `bindingId`; plugin-owned candidates also include the captured `pluginActivationEpoch`, and `CATCH_UP` candidates include `resumeWindowId`. The captured `lifecycleRevision` travels alongside as required non-identity fence evidence and is not included in candidate, firing-claim, or `scriptEventId` identity. For gameplay/runtime and scheduler candidates, the paired nonblank `scriptPinControlPlaneRequestId` is retained and exact-compared as owner/fence evidence, but is excluded from candidate, firing-claim, work-item, and derived event identity. The scheduler must not fabricate a field absent from that schedule. This scheduler-owned exception does not authorize ordinary producer ingress to supply handler-only identity.
  ...
  - **Target-state exact-fence requirement:** For gameplay/runtime and scheduler events, this set must include the exact Game Session `scriptPatchVersion` plus `scriptPinEpoch` and the paired owner request evidence; a version-only or partial tuple is incomplete and cannot be admitted after a repin.
- `snapshotAuthority`
