...
The target exact-fence and materialized-catalogue requirements below are not fully implemented. At the live instance-bound ingress, the current request carries and validates the exact `scriptPatchVersion`/positive `scriptPinEpoch` pair together with its paired nonblank `scriptPinControlPlaneRequestId`, so same-version work from an older epoch or owner request is rejected at that boundary. For plugin-owned scheduler candidates, the target registry rule additionally carries the captured `(pluginActivationEpoch, lifecycleRevision)` pair as lifecycle-fence evidence; `pluginActivationEpoch` participates in the scheduler candidate/firing identity and derived event ID, while `lifecycleRevision` remains paired execution evidence and neither field is part of handler Trigger Identity. The current registry/runtime does not yet persist and propagate that pair through every scheduler, retry, recovery, and final-effect path, so this plugin activation-epoch rule is target-state and an implementation gap, not a live capability. The current static registry still lacks an accepted materialized catalogue revision/digest and the applicable immutable schema identity/digest evidence required for its mutable `payloadSchemaRef` anchors. Registry-classified reload policy, transient attempt state, producer-specific durable retry, complete source/target command identity, and policy-specific proof under [ADR 0173](./decisions/adr-0173-registry-classified-reload-admission-policy.md) are also unimplemented. See the [runtime execution implementation status](./system-architecture-scripting-runtime-execution.md#current-implementation-status); these gaps do not weaken the target registry contract.
...
- `requiredTriggerIdentityFields`
  ...
  - Scheduler/timer due candidates are another deliberate pre-handler exception: because Automation already materialized a durable schedule with its owner identity, the scheduler firing identity and deterministic event-scope `scriptEventId` include the schedule-owned `scriptId`, any schedule-owned `pluginId`, `pluginVersionId`, and `bindingId`, and the exact Game Session `scriptPatchVersion` and positive `scriptPinEpoch`. For plugin-owned schedules, the target candidate/firing identity additionally carries the captured `pluginActivationEpoch`; the paired `lifecycleRevision` travels with it as execution-fence evidence, and both are revalidated without becoming handler Trigger Identity fields. The paired nonblank `scriptPinControlPlaneRequestId` remains mandatory owner and fence evidence, is exact-compared for candidate/firing claims, and is propagated to durable rows and audits, but is excluded from candidate, firing, work-item claim, and uniqueness keys as well as from the derived `scriptEventId` and logical Trigger Identity. The scheduler must not fabricate a field absent from that schedule. This scheduler-owned exception does not authorize ordinary producer ingress to supply handler-only identity. **Target-state only:** the current registry/runtime path does not yet persist and propagate the plugin lifecycle pair through every required boundary, so current plugin ingress must not be described as proving this fence.
  - For gameplay-originated events, the producer must resolve shared-versus-isolated realm state before making the gameplay/runtime event-scope claim or attempting durable admission. This set must then include the resolved `playableStateScope` so durable ingress/work-item identity, timer follow-up work, and operator read models do not collapse distinct playable-state namespaces that happen to share the same tenant and instance identifiers; missing or unspecified scope fails closed.
  - **Target-state exact-fence requirement:** For gameplay/runtime and scheduler events, durable admission evidence must include the exact Game Session `scriptPatchVersion`, `scriptPinEpoch`, and paired owner `scriptPinControlPlaneRequestId`; a version-only or partial tuple is incomplete and cannot be admitted after a repin. The owner request ID is evidence/correlation, not a third logical Trigger Identity component.
- `snapshotAuthority`
