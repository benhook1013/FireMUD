...
- `requiredTriggerIdentityFields`
  ...
  - **Target-state exact-fence requirement:** For gameplay/runtime and scheduler events, this set must include the exact Game Session `scriptPatchVersion`, `scriptPinEpoch`, and paired owner `scriptPinControlPlaneRequestId`; a version-only or partial tuple is incomplete and cannot be admitted after a repin.
- `snapshotAuthority`
