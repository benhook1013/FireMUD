...
- `requiredTriggerIdentityFields`
  ...
  - For gameplay/runtime and scheduler events, this set must include the exact Game Session `scriptPatchVersion` plus `scriptPinEpoch`; a version-only event is incomplete and cannot be admitted after a repin.
- `snapshotAuthority`
