...
- `requiredTriggerIdentityFields`
  ...
  - For gameplay-originated events whose producer already resolved shared-versus-isolated realm state, the event envelope must carry `playableStateNamespaceId` before handler resolution and include it in the pre-resolution event-scope key so distinct namespaces cannot share one ingress claim. The envelope also carries `playableStateScope` as separately persisted and exact-validated policy/routing/authorization/migration-fence evidence. After handler resolution, the full Trigger Identity retains that same `playableStateNamespaceId`, while `playableStateScope` remains excluded from Trigger, schedule, and handoff uniqueness.
...
- `quotaClass`
  - The canonical budget-policy class used at handler admission and then persisted onto durable pre-DSL trigger records and evaluated script work items so execution-time tenant-budget handling reads the same classification instead of re-inferring from `eventType`.
...
When a resolved handler is plugin-owned, Automation ingress must only admit that handler when the same plugin version is currently active for the explicit lifecycle-owner scope `{tenantId, gameInstanceId, pluginId}`. For gameplay/runtime handlers, `playableStateNamespaceId` remains part of the complete Trigger Identity retained in durable handler audit and deduplication records. `playableStateScope`, `regionId`, and `regionEpoch` remain separately retained runtime scope or fence evidence; none of those fields replaces or widens the plugin lifecycle-owner lookup. Plugin ownership on resolved-handler audit and durable-execution records must come from the resolved owner truth rather than from optional producer-supplied plugin identity on the ingress request; the resolved owner, not producer ingress, determines audit and durable ownership.
