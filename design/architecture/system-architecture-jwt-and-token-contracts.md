...
| `authorityTuple.tenantAuthorityGeneration` | Required map; Empty when no allowed route requires target-tenant authority | Empty map | Absent | Exactly one key for every tenant-bound delegation; Empty map for an explicitly non-tenant profile | Bounded positive-generation map keyed by exact tenant UUIDs; for `control-ui`, its keys are exactly the non-empty `scopedRoles` tenant keys whose token-allowed routes require target-tenant authority, and a tenant-bound private delegation has one key for the exact delegated `tenantId`, never an account, grant, token, or wildcard scope |
| `authorityTuple.membershipAuthorityGeneration` | Required map; Empty when no allowed route requires caller-membership authority | Empty map | Absent | Exactly one key for every tenant-bound delegation; Empty map for an explicitly non-tenant profile | Bounded map keyed by the profile's independently applicable exact membership scope. For `control-ui`, its keys are exactly the caller-membership tenants required by the token's allowed route classifications, including `billing_safe_tenant`; equality with `tenantAuthorityGeneration` or every `scopedRoles` key is not implied. The value is the Account-owned `{accountId, tenantId}` membership authority generation, not a tenant-wide or account-wide substitute |
| `membershipVersion` | Required map; keys exactly match the applicable membership-authority map | Empty map | Absent | Exactly one key for every tenant-bound delegation; Empty map for an explicitly non-tenant profile | Separate Account-owned membership projection/version map. For `control-ui`, its keys exactly match `authorityTuple.membershipAuthorityGeneration`, including an applicable `billing_safe_tenant` key; for other profiles, they are determined by the profile and route contract. It is compared independently from `authorityTuple.membershipAuthorityGeneration`; neither field substitutes for the other |
| `authorityTuple.privateRealmGrantVersions` | Empty list | Empty list | Absent | Exact applicable entries or empty list | `control-ui` and `player-bootstrap` do not carry gameplay-realm grants. A private player delegation carries exact `{tenantId, worldSlug, realmSlug, grantVersion}` entries when applicable |
| `authorityTuple.accountSecurityCutoff` | Present when applicable | Present when applicable | Absent | Present when applicable | Optional object field; absent only when not applicable, never an empty-object or collection-omission shorthand; exact Account cutoff checkpoint; never a replacement authority |
| `authorityTuple.tenantBillingCutoff` | Present when applicable | Present when applicable | Absent | Present when applicable | Optional object field; absent only when not applicable, never an empty-object or collection-omission shorthand; exact tenant-ID map with billing checkpoint values |
