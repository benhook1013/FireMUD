...
| `authorityTuple.tenantAuthorityGeneration` | Required when `scopedRoles` is non-empty; otherwise Empty map | Empty map | Absent | Exactly one key for every tenant-bound delegation; Empty map for an explicitly non-tenant profile | Bounded positive-generation map keyed by exact tenant UUIDs; a tenant-bound private delegation has no end-user `scopedRoles`, so its sole entry is keyed by the exact delegated `tenantId` and never by account, grant, token, or wildcard scope |
