...
- A registry-backed JWT must be cryptographically valid (signature, required claims `iss`, `sub`, `jti`, `accountId`, `aud`, `iat`, `nbf`, `exp`, `tokenGeneration`, `authorityTuple`, `issuanceFence`, and expected token profile/type and audience) and must have one matching `session:auth:token:<tokenHash>` record in Coordination Redis whose account, issuer, profile/type, exact audience, `jti`, `tokenGeneration`, complete `authorityTuple`, `issuanceFence`, and time fields agree with the verified claims. Only an `active` registry record authorizes a fresh protected request. `pending`, `retiring`, `revoked`, absent, expired, malformed, and unsupported-version records deny fresh authorization; `retiring` may only finish work admitted while the record was still `active` or accept the exact retirement acknowledgement defined by ADR 0035.
- The matching registry snapshot must compare every applicable field in `authorityTuple`, both cutoff objects, `issuanceFence`, and separate `membershipVersion` data with current Account-owned state and fail closed on any mismatch. `membershipVersion` and `membershipAuthorityGeneration` are independent predicates. `iat` remains required for chronology, bounded clock-skew handling, and audit, but is not an authorization or revocation authority and cannot replace generation, cutoff, grant-version, or fence comparison.
...
- The authority outbox stream key is exactly `account:auth-authority:v1:<scopeId>`, where `scopeId` is `issuer/<issuerId>`, `account/<accountId>`, `tenant/<tenantId>`, `membership/<accountId>/<tenantId>`, or `grant/<accountId>/<tenantId>/<worldSlug>/<realmSlug>`. `outboxSequence` starts at `1` independently for each exact stream key; `tenantBillingSequence` remains the separate tenant billing sequence. Consumers keep a watermark per exact stream key: a duplicate at or below the watermark is a no-op only when its event identity/digest matches, the next sequence is contiguous, and a higher sequence is a gap only within that same stream. An unrelated scope never creates a gap. A same-key gap or conflicting duplicate stops affected admission/validation until Account reconciliation proves checkpoint coverage.
...
| `authorityTuple.tenantAuthorityGeneration` | Required when `scopedRoles` is non-empty; otherwise Empty map | Empty map | Absent | Exactly one key for every tenant-bound delegation; Empty map only for an explicitly non-tenant profile | Bounded positive-generation map keyed by exact tenant UUIDs; a tenant-bound private delegation has no end-user `scopedRoles`, so its sole entry is keyed by the exact delegated `tenantId` and never by account, grant, token, or wildcard scope |
| `authorityTuple.membershipAuthorityGeneration` | Required when `scopedRoles` is non-empty; otherwise Empty map | Empty map | Absent | Exactly one key for every tenant-bound delegation; Empty map only for an explicitly non-tenant profile | Same exact tenant keys as `authorityTuple.tenantAuthorityGeneration`; the value is the Account-owned `{accountId, tenantId}` membership authority generation, not a tenant-wide or account-wide substitute |
...
`Required` means the claim must be present with a valid type. Positive-value constraints apply only where a generation, cutoff sequence, fence, or other numeric claim is specified to be positive; string claims must be non-empty and satisfy their exact issuer, subject, profile, or audience contract, and time claims must be valid UTC epoch-second values. `Empty map` means the claim must be present as `{}`; `Empty list` means it must be present as `[]`; `Absent` means the claim must not be serialized for that profile. For `control-ui`, `scopedRoles` is required even when empty, and both `authorityTuple.tenantAuthorityGeneration` and `authorityTuple.membershipAuthorityGeneration` are required as `{}` when it is empty and otherwise have exactly the same non-empty canonical tenant UUID keys. Omission of `scopedRoles`, `authorityTuple`, or either required generation map is rejected. `player-bootstrap` is tenant-free, so its authority maps and `scopedRoles` are empty and its private-realm grant list is empty. A tenant-bound private player-delegation profile has no end-user role scope or `scopedRoles` key requirement, but it must carry both authority maps with exactly one shared, non-empty key derived from the exact delegated gameplay binding `{accountId, tenantId}`; the receiver rejects any extra, missing, empty, wildcard, bare realm, account, or differently keyed entry. Only an explicitly non-tenant private-delegation profile may carry empty maps and lists.
