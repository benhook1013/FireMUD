...
| `membershipVersion` | Required map; keys exactly match the applicable membership-authority map | Empty map | Exactly one selected-tenant key | Exactly one key for every tenant-bound delegation; Empty map for an explicitly non-tenant profile | Separate Account-owned membership projection/version map compared independently from membership authority generation |
| `authorityTuple.privateRealmGrantVersions` | Empty list | Empty list | Exact applicable selected-realm entry or empty list | Exact applicable entries or empty list | Public-production gameplay-connect uses `[]`; a selected private/playtest target carries its exact current grant entry |
