...
- The hardening workflow observes one Account-owned restore-cutover operation and records separate evidence for Account authority/delegation invalidation and Game Session gameplay-binding/session invalidation. For that `restoreCutoverOperationId`, `sa-jwt-rotation` is the sole validator/Game Session convergence reconciler; the recovery orchestrator only observes and records evidence, and neither workload performs those owner mutations.
