...
Before any `StartRestoreCutover` operation exists, Account must resolve the configured environment custody backend and validate that the requested custody mode is available and matches that backend. A missing, unavailable, or mismatched backend/mode fails closed before an operation record, signer request, materialization request, or other cutover side effect is created; only the validated request may be created or reused.
