...
The durable `PARTIAL_RELEASE_RECONCILING` record contains one entry for every applicable release effect. Each entry records a stable `releaseEffectId`, closed `releaseEffectType`, target identity, apply outcome/evidence, readback outcome/evidence, ambiguity state, and containment or re-fence action/evidence. The closed type set covers at least workload admission, ingress exposure, traffic fence, quarantine state, and maintenance lock; an operation must declare any additional effect type before release starts. Retries use the same recovery `operationId` and per-effect identities, never a new release operation. Every unapplied effect remains fenced, and the controller cannot leave this phase until every applied or ambiguous effect has durable effective containment/re-fence evidence and the complete declared inventory is reconciled. An effect that is merely applied, observed, or reported successful does not satisfy this exit condition.
