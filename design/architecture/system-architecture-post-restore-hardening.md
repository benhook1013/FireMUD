...
- `sa-jwt-rotation` observes the persisted Account-owned restore-cutover request/status, including Account's phase/generation/publication status, runs validator-convergence probes, and reconciles validator and Game Session convergence; it does not reconcile or mutate the Account request/state machine. It has no write access to `jwt-jwks`, no read or update access to `jwt-signing-keys`, and no `patch` authority on the Account Service Deployment. In the interim mounted fallback, the materialization controller alone writes the private signing Secret; Account Service remains the only actor that advances issuer authority, invalidates restored Account authority and `game-session-account-delegation` lineages, publishes public JWKS, and commits the requested transition. Target non-exportable-signer mode has no application private-key Secret or mount; its recovery proof observes Account's authenticated signer operation and public-JWKS convergence instead. Game Session separately invalidates gameplay bindings and gameplay session state from the durable cutoff event.
...
- `sa-jwt-rotation` observes the persisted Account-owned restore-cutover request/status, runs validator-convergence probes, and reconciles validator and Game Session convergence; it does not reconcile or mutate the Account request/state machine. It may not create or request a transition. It has no write access to `jwt-jwks`, no read or update access to `jwt-signing-keys`, and no `patch` authority on the Account Service Deployment.
...
- The hardening workflow observes one Account-owned restore-cutover operation and separately reconciles Account authority/delegation invalidation and Game Session gameplay-binding/session invalidation. Neither the orchestrator nor the JWT rotation Job performs those owner mutations.
