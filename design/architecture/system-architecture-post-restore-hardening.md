...
The restore mode, reopen, and fixed erasure-replay boundary are canonical in [Backup & Disaster Recovery](./system-architecture-backup-recovery.md). Post-restore hardening retains the local gate that must be evidenced before that owner-defined boundary can pass:
...
The restore controller's reopen sequence is canonical in [Backup & Disaster Recovery](./system-architecture-backup-recovery.md). Runbooks must still treat `post-restore-secret-hardening` as mandatory and retain these local hardening consequences:
