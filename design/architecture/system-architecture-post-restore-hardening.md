...
- reconcile the committed result of that same Account-owned restore-cutover operation, including its invalidation of restored Account and gameplay session authority, before normal work or player traffic can resume
...
6. Run `post-restore-secret-hardening` in the target namespace and wait for success. The orchestrator invokes or observes the separate delegated `jwt-rotation` Job using the already-persisted restore-cutover request. `jwt-rotation` submits that request to Account, then only retries, observes, and reconciles the same operation while Account publishes the new signing generation, invalidates restored Account and gameplay session authority, and the Job proves validator convergence; neither recovery workload performs those Account-owned mutations itself.
