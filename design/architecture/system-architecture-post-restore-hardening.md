...
- after the Account cutover, rebuild and verify the issuer, account, tenant, and membership generation projections plus the issued-token projections from Account durable authority. Record per-scope generations and non-secret, generation-bound issued-token projection evidence using identifiers, bounded counts, and hashes/digests; never record bearer-token values or raw credentials. Complete this rebuild before restore-safe smoke or any recovery continuation can pass; missing, stale, malformed, or mismatched projection state keeps quarantine closed
...
- rebuild coordination state only from restored durable authority after authoritative, complete, reachable participant and external-effect inventories have recorded a qualifying disposition for every declared and enabled entry: `converged`, `terminalized`, or `invalidated`. `fenced_disabled_backlog_retained` remains representable for a disabled participant, but is non-qualifying and keeps quarantine closed until the backlog is resolved or the participant reaches a qualifying disposition. Missing, unknown, unreachable, or unsafe entries also keep quarantine closed.
- record the backup artifact, snapshot-bound `artifactErasureHighWater`, immutable `initialCatchupHighWater`, immutable final-cutover `restoreHighWater`, and ordered evidence for both fixed replay intervals. `erasureOverlayReconciliation` remains bound to the complete interval `(artifactErasureHighWater, restoreHighWater]`; its `sequenceDispositions[]` field covers exactly the final interval `(initialCatchupHighWater, restoreHighWater]` and must persist exactly one canonical owner disposition for every sequence in that interval. Aggregate counts or a replay-through high-water are insufficient. Also record restore/recovery tool digests, recovery-contract fingerprint, participant inventory, convergence results, confidentiality proof, and controlled-reopen evidence in the durable recovery controller; emit checked-in projections only after finalization

The recovery artifact ranges are intentionally distinct: `erasureOverlayReconciliation` covers the complete overlay `(artifactErasureHighWater, restoreHighWater]`, while `sequenceDispositions[]` covers only the final cutover interval `(initialCatchupHighWater, restoreHighWater]`, with one canonical owner disposition for each sequence there.
...
5. Resolve authoritative, complete, reachable validator, durable-participant, and external-effect inventories and run the complete enabled reconciliation set; unknown, missing, unreachable, or unsafe outcomes keep quarantine closed. A participant may be represented as `fenced_disabled_backlog_retained` while disabled, but that non-qualifying, quarantine-blocking state cannot satisfy recovery readiness or authorize reopen.
6. Read the immutable artifact `artifactErasureHighWater` and capture immutable `initialCatchupHighWater` once. First verify and idempotently replay exactly every independently retained journal sequence in the initial interval `(artifactErasureHighWater, initialCatchupHighWater]`, recording its ordered, contiguous, complete, and gap-free evidence. Only after that interval is complete, perform one bounded final cutover that fences or serializes new erasure assignment, captures immutable `restoreHighWater` once, and replays exactly the final interval `(initialCatchupHighWater, restoreHighWater]`. Keep `erasureOverlayReconciliation` bound to the complete overlay interval `(artifactErasureHighWater, restoreHighWater]`, while its `sequenceDispositions[]` covers exactly the final interval `(initialCatchupHighWater, restoreHighWater]` with exactly one canonical owner disposition for every sequence, not only aggregate counts. Prove both intervals are contiguous, complete, gap-free, and free of duplicate application before atomically installing `restoreHighWater` as the restored consumer cursor through one transactional or compare-and-set handoff to online erasure processing. Retain the assignment fence or serialization until the handoff has one durably observed committed cursor state; reconcile an ambiguous result under that fence, and keep the restore quarantined if one committed state cannot be established. The restore overlay ends at that fixed `restoreHighWater`; deletions accepted later use the normal online consumer/reconciliation path and are not retroactively added to the restore evidence. Never advance either boundary from a moving current high-water. A missing sequence, integrity failure, unavailable owner, ambiguous boundary, or incomplete convergence keeps quarantine closed.
...
15. Call the public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` safety gate with persisted-form `expectedPhase=awaiting_resume`. The controller must validate the exact durable operation, expected phase, its recorded recovery scope, maintenance-lock token, replay/Account evidence, and immutable evidence binding; `evidenceRef` must resolve to complete, immutable pre-release evidence whose operation, scope, required gate results, and evidence digests match the durable controller state. Any missing, stale, partial, ambiguous, or mismatched binding/evidence fails closed without mutation. A successful call durably records `RESUME_AUTHORIZED` while retaining the maintenance lock and traffic fence; it does not release them or expose traffic.
16. Only after `RESUME_AUTHORIZED` is durably recorded may the internal release phase proceed for that same operation. It idempotently reconciles `releasing -> finalized` and applies every operation-owned release effect in controlled order. Release effects include workload admission, ingress exposure, traffic fences, quarantine state, maintenance-lock release, and any other non-workload effect declared by the operation. The maintenance lock remains owned throughout `releasing` and `PARTIAL_RELEASE_RECONCILING`; its idempotent release is the final external release effect, after every other effect is applied, read back, and staged behind the still-closed traffic fence. Before invoking that effect, the controller durably records `FINALIZATION_PENDING` evidence with the operation identity, current scope fencing epoch, lock identity, and complete prior-effect readbacks. It then compare-and-matches the same operation, token digest, and fencing epoch when releasing the lock, reads back the exact release outcome, and only afterward compare-and-sets its own durable operation to `finalized`. A new operation cannot acquire the scope merely because the external lock is absent: acquisition also requires the previous controller operation to be durably terminal and a strictly newer scope fencing epoch. A crash or ambiguity between external lock release and durable finalization therefore remains fail-closed in `PARTIAL_RELEASE_RECONCILING`; retry observes or reconciles the same release identity and epoch rather than repeating it or admitting a new operation. Player traffic remains blocked by the traffic fence until finalization is durably observed, after which `status=SUCCEEDED` may be recorded and the finalized gate may expose the already-verified release state. An ambiguous or failed final lock release remains owned or fail-closed, enters or stays in `PARTIAL_RELEASE_RECONCILING`, and cannot expose traffic. During `releasing`, external ingress remains closed and every workload remains technically fenced from accepting or creating normal side effects until its applicable release effects are applied and observed. If a later release effect times out, fails readiness, or has an ambiguous outcome after an earlier effect may have applied, the controller durably enters `PARTIAL_RELEASE_RECONCILING`, keeps every unapplied effect fenced, and immediately reverses, re-fences, or otherwise contains every applied or ambiguous effect before another release attempt. Traffic and normal side effects remain closed until that containment is observed. Reconciliation uses the same operation and per-effect identities, may safely observe an already-contained or already-applied effect, and must not duplicate or skip a release. The controller may return to `releasing` only after the complete partial-release inventory is durable and every applied or ambiguous effect has durable effective containment/re-fence evidence; observing an apply alone is insufficient. No rollback promise beyond this explicit re-fencing/containment contract is assumed. Export the immutable checked-in recovery and traffic-open evidence projections only after every release effect is durably and observably complete and the controller has `phase=finalized` and `status=SUCCEEDED`.

The durable `PARTIAL_RELEASE_RECONCILING` record contains one entry for every applicable release effect. Each entry records a stable `releaseEffectId`, versioned `releaseEffectType`, target identity, apply outcome/evidence, readback outcome/evidence, ambiguity state, and containment or re-fence action/evidence. Effect types come from a controller-owned versioned registry whose schema version and digest are bound into the operation before release starts. The registry includes workload admission, ingress exposure, traffic fence, quarantine state, and maintenance lock; extensions require a registered schema and deterministic apply, readback, and containment semantics before an operation may declare them. Unknown, unregistered, schema-mismatched, or undeclared effect types fail closed. Retries use the same recovery `operationId` and per-effect identities, never a new release operation. Every unapplied effect remains fenced, and the controller cannot leave this phase until every applied or ambiguous effect has durable effective containment/re-fence evidence and the complete declared inventory is reconciled. An effect that is merely applied, observed, or reported successful does not satisfy this exit condition.

For hobby/self-hosted environments that do not use the Kubernetes Job template directly, operators must run equivalent one-shot restore-hardening automation that performs the same control groups, calls `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with `expectedPhase=ready_to_reopen`, calls public `resume(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` with persisted-form `expectedPhase=awaiting_resume`, and only then allows the internal release phase to reach `phase=finalized` with `status=SUCCEEDED`. Its immutable checked-in recovery projection is exported only after that successful terminal state.
