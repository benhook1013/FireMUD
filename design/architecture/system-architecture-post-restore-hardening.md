...
For ordinary replacement hardening, aggregate `jwtHardening.oldOrRestoredKeyIds` must be a non-empty unique list of every old or restored key that the replacement path removes, and `jwtHardening.replacementEvidence` must contain one record per listed key. Each record contains exactly `oldKid`, `candidateKid`, `oldKidRejected`, `candidateKidAccepted`, and `validatorEvidenceRef`; the IDs are distinct and non-empty, both booleans are `true`, each reference exactly matches the corresponding aggregate `validatorConvergenceEvidence`, the candidate is present in `resultingKeyIds`, and every recorded old ID is absent. The two sets must match exactly; a missing record or an unlisted old key fails closed. This per-key proof applies to `rotated`, `reissued`, and applicable `rebound` dispositions only. It is prohibited for compromise-classified and `verified_not_restored` branches. For a compromise-classified hard cutover, both `validatorConvergenceEvidence` and aggregate `jwtHardening` evidence must instead carry the exact `compromisedKid`, `candidateKid`, `compromisedPublicKeyFingerprint`, and `candidatePublicKeyFingerprint` pairings from the Account-owned operation; fingerprints are lowercase `sha256:<64 hex>`. Each validator must verify the exact `kid`-to-public-key-fingerprint mapping: every token using the compromised `kid` or compromised fingerprint is rejected, and the distinct candidate `kid` is accepted only with the exact candidate fingerprint. A swapped, duplicate, missing, or mismatched `kid`/fingerprint pair, acceptance of any compromised identity, or rejection of the exact candidate pair fails closed; `verified_not_restored` records the unchanged current key set instead and must not fabricate a replacement pair.
...
- the constrained job must authenticate with its provisioned workload identity to the controller/proxy. The proxy must validate the stable operation identity, target cluster/database, exact explicitly allowed role, and canonical request digest against the durable rotation operation before authorizing the mutation; network reachability alone, an unbound or stale operation, a stale authority generation, or any target/role/digest mismatch is rejected before password mutation
...
The refresh must not add `postgres-admin-credentials` as a second application credential class. When rotation Jobs are enabled, that Secret is transient target-cluster authority for the operation-bound `ALTER ROLE` action, not the application credential lineage retained by the compliance record. Its authority evidence is therefore validated separately: it must identify the target cluster/database, exact allowlisted role, controller/proxy and privileged-role identities, binding and authority generation, Secret reference, and immutable issuance artifact, all bound to the durable rotation operation and recorded before password mutation. The authority issuance/validation timestamp must be at or before recovery finalization; it is not copied into `secretComplianceRefresh.freshness` and does not reset the `postgres-application-credentials` age. Missing, stale, conflicting, or snapshot-derived admin authority fails closed.

The recovery handoff intentionally uses recovery-record keys. Its fixed projector mapping is `backup-storage` -> secret-compliance `backup-object-store-credentials`; when external asset storage is enabled, `asset-storage` -> secret-compliance `asset-store-credentials`; when outbound communications are enabled, `outbound-comms` -> secret-compliance `outbound-communications-credentials`; and `operator-credentials` -> secret-compliance `operator-credentials` for the external binding mapped from `operator-leaf`. The recovery `credentialClasses` and `freshness` keys must not be replaced with the other compliance-record keys. The checked-in secret-compliance validator requires the compliance-record names, while the recovery validator retains the recovery names. Disabled asset and outbound integrations have no projected compliance class or freshness entry.

Each `secretComplianceRefresh.freshness` entry is keyed exactly by its refreshed `credentialClasses` and carries `lineage`, `field`, `value`, `previousField`, and `previousValue`. A new lineage/first issuance uses `lineage=new`, either `rotated` or `reissued`, `field=lastProvisionedAt`, and no previous field/value. An existing-lineage `rotated` or `reissued` replacement uses `lineage=existing`, `field=lastRotationAt`, and an advanced timestamp over the recorded previous value. A binding-only `rebound` or `verified_not_restored` refresh uses `lineage=existing` and preserves the exact previous field/value; its separate operation identity and evidence do not reset credential age. During pre-release hardening, the owner validates every selected and previous freshness timestamp against the controller-issued, persisted `controllerContinuationAt` for the same operation and continuation attempt, not against `finalizedAt`, which does not yet exist as a release fact. The hardening owner transports this value unchanged in the immutable handoff; it does not supply, choose, refresh, or infer it. The controller accepts the handoff only when `controllerContinuationAt` exact-matches its persisted operation/continuation binding and is not future-dated relative to the controller's trusted validation time. Only after the controller completes the guarded release and reaches `finalized` may the post-finalization projection validator compare the retained freshness timestamps with the finalized recovery record's `finalizedAt`; that projection check is not a substitute for the pre-release handoff check. Missing class entries, an incompatible disposition/lineage/field combination, a future freshness timestamp, or a changed preserved timestamp fails closed. The referenced evidence payload must include an immutable artifact identifier, exact target-environment binding evidence, and material-lineage evidence, and must be linked from the durable recovery-controller state under `secretComplianceRefresh`; the checked-in projection mirrors this after finalization.

The secret-compliance publisher is the environment/secrets owner’s authenticated provisioning, rotation, or rebinding controller, using a dedicated publisher identity authorized only for the target environment’s compliance path and its content-addressed evidence objects. A Git author, branch, commit timestamp, file path, or successful local process is not a trusted publisher and cannot authorize a recovery or release. Before release, the hardening workflow hands the durable recovery controller one complete immutable `secretComplianceRefresh` handoff containing the stable restore operation identity, controller-issued `controllerContinuationAt` and its continuation binding, credential event kind and operation identity, target environment, exact credential-class keys, per-class freshness lineage/field/value and previous field/value, evidence references and digests, and canonical parent-record digest. A `rebound` or `verified_not_restored` handoff also binds the retained material-lineage identity and exact preserved freshness pair; a rebound additionally binds the exact new target binding. The owner transports the controller-issued continuation value unchanged, validates the handoff, and publishes the evidence artifact before the handoff is accepted; the handoff is committed by one compare-and-set/atomic operation, and a retry may reuse it only when every operation, continuation, environment, key, and digest field exact-matches. A partial artifact, missing digest, conflicting retry, or ambiguous publisher result keeps quarantine closed.
...
Recommended flow (target-state/design-only; not executable):

The following sequence is illustrative target-state guidance only. Do not execute it or substitute manual steps; the prerequisites and the explicit target-only block above remain mandatory until the workload and target-bound controller/proxy exist with the required proof.
