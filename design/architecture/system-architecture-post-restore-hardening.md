...
16. Only after `RESUME_AUTHORIZED` is durably recorded may the internal release phase proceed for that same operation. It idempotently reconciles `releasing -> finalized` and moves already-started restore-safe workloads toward normal operation in controlled order. During `releasing`, external ingress remains closed and every workload remains technically fenced from accepting or creating normal side effects until its release step is applied and observed. The controller reaches `finalized` only after the complete release is observed, and only then may player traffic be exposed. If a later release step times out, fails readiness, or has an ambiguous outcome after an earlier workload was released, the controller durably enters `PARTIAL_RELEASE_RECONCILING`, keeps all unreleased workloads fenced, and immediately re-fences or otherwise contains every already-released workload before another release attempt. Traffic and normal side effects remain closed until that containment is observed. Reconciliation uses the same operation and per-workload release identities, may safely observe an already-contained or already-released effect, and must not duplicate or skip a release. The controller may return to `releasing` only after the complete partial-release inventory is durable and every previously released workload has durable effective containment/re-fence evidence; observing a release alone is insufficient. No rollback promise beyond this explicit re-fencing/containment contract is assumed. Export the immutable checked-in recovery and traffic-open evidence projections only after every release step is durably and observably complete and the controller is `finalized`.

The durable `PARTIAL_RELEASE_RECONCILING` record contains one entry for every workload release identity, its applied/observed/ambiguous result, and the re-fence or containment action and evidence. Retries use the same recovery `operationId` and per-workload identities, never a new release operation; all unreleased workloads remain fenced, and the controller cannot leave this phase until every previously released workload has durable effective containment/re-fence evidence and the complete inventory is reconciled. A release that is merely applied, observed, or reported successful does not satisfy this exit condition.
