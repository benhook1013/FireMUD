...
- `DRAINING` is required. Rollback must not resume admission or ticks until a fresh authoritative `GetAutomationDrainStatus` response, taken after final reconciliation, cancellation, and purge, reports a complete two-layer drain for the current rollback-scope `admissionEpoch`: no active pre-DSL evaluation lease or descriptor handoff, and no pending pre-DSL trigger or evaluated descriptor that can refill the scope. A cached, stale, or earlier-epoch response is unsatisfied evidence. Current projections expose this through `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0`, but the target count covers the state mapping in [Scripting Runtime Execution](./system-architecture-scripting-runtime-execution.md#current-state-mapping-drain-and-rebuild-rules).
