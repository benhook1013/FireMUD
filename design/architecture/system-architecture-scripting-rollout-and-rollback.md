...
15. Operators monitor `script_event_audit` and automation metrics defined by the [Scripting & Automation Observability Contract](./system-architecture-scripting-observability-contract.md); per-event correlation uses `scriptEventId` in audit, logs, and traces, not metric labels.

If promotion fails or is aborted after `PauseTicks`, it must not automatically clear either fence. The operator may issue an explicit idempotent `ResumeTicks` only after the durable promotion state is safe to resume; otherwise ticks and admission remain paused for operator action. The admission mode returns to `NORMAL` only after that successful resume.
...
9. Wait for `GetAutomationDrainStatus` to report the canonical two-layer drain complete for the rollback scope under the current `admissionEpoch`; current projections expose `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0`, but these fields must include every applicable pre-DSL and evaluated-descriptor state after convergence.
...
- Step 8: Poll `GetAutomationDrainStatus(11111111-1111-4111-8111-111111111111, 44444444-4444-4444-8444-444444444444)` until the canonical two-layer drain is complete. An expired `EVALUATING` lease must first be terminalized by the Automation recovery owner with audited `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and `finalReason=stale_execution_fenced`, unless recovery finds the committed descriptor set and resumes from those descriptors.
...
- `DRAINING` is required. Rollback must not resume admission or ticks until the current rollback-scope `admissionEpoch` has a complete two-layer drain according to `GetAutomationDrainStatus`: no active pre-DSL evaluation lease or descriptor handoff, and no pending pre-DSL trigger or evaluated descriptor that can refill the scope. Current projections expose this through `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0`, but the target count covers the state mapping in [Scripting Runtime Execution](./system-architecture-scripting-runtime-execution.md#current-state-mapping-drain-and-rebuild-rules).
- A stale `EVALUATING` row is not inferred complete. After its lease expires, Automation's recovery owner must CAS-transition it to terminal `DEAD_LETTERED` with audited `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and `finalReason=stale_execution_fenced` without re-entering the DSL; if the evaluated descriptor commit is already present, recovery proceeds from those persisted descriptors. Drain remains closed until one of those two outcomes is durably visible.
