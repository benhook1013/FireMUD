...
- Step 8: Poll `GetAutomationDrainStatus(11111111-1111-4111-8111-111111111111, 44444444-4444-4444-8444-444444444444)` until the canonical two-layer drain in [Rollback Orchestration State Machine](#rollback-orchestration-state-machine-required) is complete. For this example, the response must be fresh, post-cleanup, scoped to the current rollback `admissionEpoch`, and report both counts as zero; the current live projection remains fail-closed for unresolved `EVALUATING` and handoff-capable `PENDING_EVALUATION`.
...
- Explicit operator recovery of `ROLLBACK_CONVERGENCE_TIMEOUT` is an idempotent retry of the same Game Session workflow record with the same `controlPlaneRequestId`; it transitions that record back to `CONVERGING`, not to a new workflow or state machine. Recovery resumes unfinished durable `CONVERGING`/`DRAINING` work from its recorded state without creating duplicate completed work, and repeated recovery requests return the current state. It must not clear either fence. If fresh convergence proof remains unavailable, the same record returns to `ROLLBACK_CONVERGENCE_TIMEOUT` and retains the timeout state and both fences.
...
- An operator may explicitly retry the same durable workflow idempotently. The retry first returns the existing record to `CONVERGING` and resumes unfinished durable work using the persisted absolute deadline. Before `ResumeTicks`, it must obtain fresh matching Game Session and Automation acknowledgments for the [Pin Convergence Acknowledgment Predicate](#pin-convergence-acknowledgment-predicate) and a fresh post-cleanup `GetAutomationDrainStatus` response with the current `admissionEpoch` and both counts zero. Only that fresh proof permits the existing `ResumeTicks` step, followed by the existing `NORMAL` admission transition; otherwise the record returns to `ROLLBACK_CONVERGENCE_TIMEOUT` with admission and ticks still fenced.
