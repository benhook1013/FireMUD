...
- A stale `EVALUATING` row is not inferred complete. After its lease expires, Automation's recovery owner must CAS-transition it to the terminal audited outcome-unknown/dead-letter state without re-entering the DSL; if the evaluated descriptor commit is already present, recovery proceeds from those persisted descriptors. Drain remains closed until one of those two outcomes is durably visible.
