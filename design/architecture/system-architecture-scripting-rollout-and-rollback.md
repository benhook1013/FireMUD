...
The rollout and rollback state machine below is target-state canonical. The live Game Session handoff still lacks complete per-command Trigger Identity propagation; the detailed current fallback is recorded in [Command Identity and Live Handoff Boundary](#command-identity-and-live-handoff-boundary). `targetEntityIds[]` multi-target fan-out is target-state only because the existing owners do not define persisted deterministic per-target identity, scope validation, and deduplication at the live boundary. Automation claims work through `PENDING_EVALUATION` to `EVALUATING`; `GetAutomationDrainStatus` counts `EVALUATING` and `HANDOFF_IN_FLIGHT` in `activeExecutionCount`, including unresolved stale `EVALUATING`, while `pendingCancelableWorkItemCount` covers every current handoff-capable `PENDING_EVALUATION` row because that is the current cancelable set. No accepted owner contract defines stale-`EVALUATING` lease recovery, so rollback must keep admission and ticks paused until `activeExecutionCount=0` and no handoff-capable `PENDING_EVALUATION` row remains rather than infer a reclaim. Metric names, labels, audit fields, and handoff diagnostics come from the [Scripting & Automation Observability Contract](./system-architecture-scripting-observability-contract.md).
...
8. Wait for pin-convergence acknowledgments from both Automation & Scripting and Game Session for the requested `controlPlaneRequestId`. `GetAutomationPinConvergence` is an acknowledgment only when the expected patch and request ID are observed with `isProjectionStale=false` and `projectionLagMs` inside the configured freshness bound.
9. Wait for `GetAutomationDrainStatus` to report `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` for the promotion scope under the current `admissionEpoch`; the current pending count covers every handoff-capable `PENDING_EVALUATION` row, and unresolved stale `EVALUATING` rows remain in `activeExecutionCount` and therefore block the drain.
...
8. Wait for pin-convergence acknowledgments from both Automation & Scripting and Game Session for the new pin (`controlPlaneRequestId` must match). `GetAutomationPinConvergence` is an acknowledgment only when the expected patch and request ID are observed with `isProjectionStale=false` and `projectionLagMs` inside the configured freshness bound.
9. Wait for `GetAutomationDrainStatus` to report `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` for the rollback scope under the current `admissionEpoch`; the current pending count covers every handoff-capable `PENDING_EVALUATION` row, and unresolved stale `EVALUATING` rows remain in `activeExecutionCount` and therefore block the drain.
...
- Step 7: Poll `GetAutomationPinConvergence(11111111-1111-4111-8111-111111111111, 44444444-4444-4444-8444-444444444444)` and `GetGameSessionPinConvergence(11111111-1111-4111-8111-111111111111, 44444444-4444-4444-8444-444444444444)` until both report `observedPinnedScriptPatchVersion=P21` and `lastObservedControlPlaneRequestId=RB-42`, and the Automation response reports `isProjectionStale=false` with `projectionLagMs` inside the configured freshness bound.
- Step 8: Poll `GetAutomationDrainStatus(11111111-1111-4111-8111-111111111111, 44444444-4444-4444-8444-444444444444)` until `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0`; unresolved stale `EVALUATING` work is included in the active count and keeps this gate closed.
...
- `DRAINING` is required. Rollback must not resume admission or ticks until the current rollback-scope `admissionEpoch` has `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` according to `GetAutomationDrainStatus`; the current pending count covers every handoff-capable `PENDING_EVALUATION` row, unresolved stale `EVALUATING` rows remain in `activeExecutionCount`, and no reclaim is inferred.
- An unresolved stale `EVALUATING` row has no established terminal or reclaim path. If it remains after pin convergence, the workflow stays in `DRAINING` with admission and ticks paused and requires operator escalation; the established `ROLLBACK_CONVERGENCE_TIMEOUT` outcome applies only when the convergence wait times out and is not a stale-work reclaim.
...
- If timeout is reached before Game Session reports the expected pin/request ID and `GetAutomationPinConvergence` reports the expected pin/request ID with `isProjectionStale=false` and `projectionLagMs` inside the configured freshness bound, the rollback enters terminal state `ROLLBACK_CONVERGENCE_TIMEOUT`.
