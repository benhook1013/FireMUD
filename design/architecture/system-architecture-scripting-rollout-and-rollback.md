...
9. Wait for `GetAutomationDrainStatus` to report `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` for the promotion scope under the current `admissionEpoch`; unresolved stale `EVALUATING` rows remain in `activeExecutionCount` and therefore block the drain.
...
9. Wait for `GetAutomationDrainStatus` to report `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` for the rollback scope under the current `admissionEpoch`; unresolved stale `EVALUATING` rows remain in `activeExecutionCount` and therefore block the drain.
...
- `DRAINING` is required. Rollback must not resume admission or ticks until the current rollback-scope `admissionEpoch` has `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` according to `GetAutomationDrainStatus`; unresolved stale `EVALUATING` rows remain in `activeExecutionCount`, and no reclaim is inferred.
