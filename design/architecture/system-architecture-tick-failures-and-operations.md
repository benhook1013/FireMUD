...
- The live rollback purge path is weaker than that target boundary: `TickQueueControlService.purgeCandidate` checks only `GameplayCommandRepository.hasDurableTickEffect(commandId)` before selecting `ABANDONED` versus `LOST_BEFORE_STAGING`. That non-atomic existence check does not prove the absence of a surviving batch, effect, or source claim, and it can race concurrent staging, drain, or effect ownership. The target atomic owner-defined CAS/no-batch proof remains unimplemented and unproved; this is a current implementation/proof gap, not a relaxation of the target `ACCEPTED_VOLATILE`/`RECEIVED`/`ENQUEUED` eligibility contract.
- The target deterministic command-plan root contract is also not live: the owning command/action contract for the frozen typed command or `ResolvedEffectPlan` must supply semantic root order, with built-ins declaring equivalent stable order and ambiguous or duplicate order rejected before admission. Before allocation, the durable gameplay command row must freeze/bind its request fingerprint and resolved runtime scope. Game Session must persist the ordered plan manifest/digest and allocate an opaque root `EffectId` row unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`; the row binds the manifest digest and validates the frozen command/runtime binding, and reuse requires all of those values to match or fails closed. For Automation, the complete applicable Command-Handoff Identity is the admission/deduplication identity and must exact-map to one durable target `commandId`; allocation then uses the target command row's canonical key. The current `tickBatchId`/`effectKey` fallback does not prove that contract, and no global allocator is implied. The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is not current target state.
...
- Root/participant idempotency guards, with a narrow last-tick watermark exception only for explicitly proven at-most-once aggregates; the region-scoped tick timeline `(regionEpoch, tickId)` remains ordering and fencing context.
...
- **Domain Idempotency Rules (Timeline Context + Owner Guards)** – how mutation-identity/participant guards enforce logical effect idempotency while `(regionEpoch, tickId)` provides ordering and fencing context.
...
Under ADR 0085, each coordination-maintenance recovery, takeover, replay, reset, cleanup, or topology-changing operation is admitted and serialized by one external durable maintenance operation/fence and the evidence-derived `replay_first`/`reset_first` classifier owned by [Redis Reset & Recovery](./system-architecture-redis-reset-and-recovery.md). Before any mutation or reconciliation, the durable handoff must validate, persist, and pass the exact external maintenance `operationId`, fence/lock binding, classifier decision, durable-evidence digest, and complete affected scope (keys, durable work, producers, and consumers). A missing, mismatched, or incomplete handoff or scope fails closed and escalates; Redis-local leases and executor fences remain separate execution aids, not substitutes for that maintenance fence. An ordinary idempotent crash replay may run outside this maintenance operation only when the owner contract explicitly classifies it as non-conflicting routine replay and records that classification and evidence; no path may infer that exemption from Redis residue alone.
...
- Before replay, cleanup, or takeover adoption, the successor revalidates the same in-memory Redis lease token it acquired and the expected `regionEpoch`, then separately verifies the current durable executor fence under the owner contract. When the owner classifies the path as coordination maintenance under ADR 0085, it also validates the exact external maintenance handoff (`operationId`, fence/lock binding, classifier decision, evidence digest, and complete affected scope) and persists/passes that handoff unchanged with recovery evidence before any mutation; an ordinary crash replay may omit that maintenance operation only under the explicit owner classification described above. A missing or mismatched token, epoch, fence, handoff field, or scope pauses and enters fenced reconciliation, affects no state, and never persists the raw token. Lane identity is immutable during recovery: actor-action retries remain actor actions, passive/effect retries remain passive effects, and replay cannot mint a new root actor action for the same tick.
- A recovery skip after the ADR 0085 handoff is permitted only for an explicitly disposable ADR 0058 hint, or after ADR 0058 reconciliation durably proves an empty cadence with no unresolved authoritative work. A missing Redis entry, empty hint scan, timeout, or maintenance handoff alone never proves that a tick or authoritative work item may be skipped; otherwise recovery must replay or reconcile the durable work and retain any inconclusive item as non-terminal/reconciliation-required.
...
- Domain services use the mutation-identity/participant-guard boundary to ensure that replays do not double-apply logical effects; `(regionEpoch, tickId)` remains ordering and fencing context even when Redis state is partially lost.
...
- Every tick effect that has been durably claimed or staged for execution (for example, rows associated with a tick batch, replay-eligible retry work, or durable follow-up records) is mirrored into a Game Session–owned ledger table (for example `tick_effects`) with columns such as:
  ...
  - `tick_batch_id`
  - `effect_id` (persisted mutation identity used by the participant guard: the root `EffectId` for a root mutation or the deterministic child `EffectId` for a generated child)
  - `root_effect_id` (enclosing stable logical root identity retained on every ledger projection for lineage and reconciliation; equal to `effect_id` for a root mutation)
  - `plan_ordinal` (stable zero-based root position in the command's deterministic plan; absent for a zero-effect command because no effect row exists)
  - `typed_operation` (canonical operation identity retained alongside the mutation `effect_id` and enclosing root on every participant projection)
  ...
  - `request_digest` (immutable request digest bound to the mutation guard and compared on replay; not part of uniqueness)
...
- The ledger retains the enclosing root's `plan_ordinal`/`root_effect_id` binding as command-plan identity and reconciliation metadata. A root mutation uses that root as its participant-guard `effect_id`; a generated child uses its persisted deterministic child `EffectId` as `effect_id` while retaining the enclosing root for lineage and reconciliation. A shared root for a conserved multi-participant operation does not replace ADR 0053 atomicity, co-location, or reservation semantics. For command-root work, the root is allocated by Game Session's local durable plan/staging transaction from the opaque binding unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`; the durable command row's frozen request/runtime binding, applicable `playableStateNamespaceId`, and ordered plan manifest digest are validated on reuse, and a mismatch fails closed. Passive, inbound/remote, timer, retry, and already-generated effects retain their owner/source-specific root identities unless they materialize as an admitted command. Random batch, mutable command, effect-key, source-ordering, participant, trigger, and generated-child identities cannot substitute for that durable command row identity or allocation binding; child-ID persistence uses its owning deterministic child contract and does not create another command-root allocator.
- Target-state requires one durable tick-batch record per `(tenantId, gameInstanceId, regionId, regionEpoch, tickId)`, owned by Game Session, and stores at minimum:
  ...
  - exactly one immutable sealed execution-context binding: `sealed_execution_context_digest`, or a durable `sealed_execution_context_ref` whose referenced record contains that digest. The binding covers the batch coordinates, executor fence, applicable `playableStateNamespaceId`, selected-work manifest/digest, effect identities, and pinned execution inputs/version/script patch; replay must use the sealed context rather than reconstructing mutable context.
...
- For the complete expected concrete participant-projection set linked to an enclosing root `EffectId` and participant guard contract, every expected `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, target_aggregate_type, target_aggregate_id)` projection must eventually have **exactly one evidence-qualified terminal state**, and only when the applicable recovery/evidence policy permits terminalization. Explicitly inconclusive or ambiguous projections remain non-terminal/reconciliation-required (for example, `SCHEDULED`) and continue to block convergence; they cannot be recorded as `APPLIED` or `ABANDONED`. `effect_id` is the mutation's persisted root-or-child `EffectId`; `root_effect_id` remains enclosing lineage and reconciliation context. `typed_operation` is part of the participant projection alongside the mutation ID and exact target aggregate; guard uniqueness uses the owner-declared runtime-family partition: `playable_state_namespace_id` participates for S1/S2, while only explicit S3 uses `game_instance_id`; for S1/S2, `game_instance_id` is separately exact-validated active-runtime authorization/fence evidence. `playable_state_scope` and the remaining timeline/evidence fields are immutable bound evidence, and `effect_key` remains descriptor metadata for correlation and lookup, not a substitute for that identity. Replay/recovery must reconcile the whole expected set and fail closed for missing, extra, partial, or conflicting projections; one root-effect or one ledger-row result is insufficient:
...
- **Game Session Service**
  ...
  - Defines the concrete ledger projection of the mutation's persisted root-or-child `EffectId` and its typed participant-guard contract, while retaining the enclosing root `EffectId` as lineage and reconciliation context. It is responsible for converging `(effectId, rootEffectId, typedOperation, tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, targetAggregateType, targetAggregateId)`—with `effectKey` retained as descriptor metadata and linked to that mutation, root, operation, and guard evidence—to `APPLIED` or `ABANDONED` when the recovery policy permits terminalization.
- **Domain services (Entity Management, World Management, etc.)**
  ...
  - Use operation-level guards by default; a per-aggregate `last_tick_id` watermark is allowed only with an explicit proof of at-most-one logical mutation per tick timeline. Do not introduce additional “mini-ledgers” for tick effects.

New designs must not create ad-hoc ledger tables for tick effects in other services; they should either extend the Game Session–owned ledger/follow-up schema or add domain-local guard tables that project the existing mutation `EffectId` and retain its enclosing root lineage.
...
- When reprocessing a tick, the executor loads ledger rows for that `<tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId>` with `status = SCHEDULED` and:
  ...
  - Marks effects `ABANDONED` with a precise reason only when replay proves no required mutation succeeded and the effect is no longer valid (expired session, entity gone, descheduled tick, and so on). Inconclusive work remains non-terminal reconciliation work under its original mutation `EffectId`, with the enclosing root retained as lineage, and is escalated for reconciliation.
  - When idempotency evidence may prove that an effect was already applied, replay verification reconciles the complete expected concrete participant-projection set and binds the evidence to the `SCHEDULED -> APPLIED` transitions in fenced/CAS transactions (conceptual shape):
    ...
    -- effect: mutation EffectId (root or persisted child) + typed operation
    -- + exact target aggregate; enclosing root is lineage/reconciliation context,
...
- Recovery rules for Redis/SQL mismatch are explicit:
  - durable tick-batch + `SCHEDULED` ledger rows exist, Redis `pending` missing:
    ...
    - First implementation treats normal tick Lua scripts as hot-path guards only. Recovery drives effects directly to `APPLIED`/`ABANDONED` only where the evidence policy permits, leaves any inconclusive work non-terminal under its original mutation `EffectId` and enclosing-root lineage, escalates it for reconciliation, reconciles any surviving source claims/entries against the manifest, and then clears stale coordination residue.
  - Redis `pending` exists without a durable tick-batch:
    - treat the Redis entry as orphaned coordination state; clear it only after the exact ADR 0085 handoff has been validated and durably persisted, then passed unchanged to the cleanup operation (`operationId`, fence/lock binding, classifier decision, evidence digest, and complete affected scope), or after the owner has durably recorded this specific non-conflicting routine replay/cleanup classification and evidence. The cleanup must validate a nonblank operation identity, the current fence/lock, classifier, evidence digest, and complete scope immediately before deletion; it must not manufacture or reuse an operation record from Redis residue. Missing, stale, mismatched, or incomplete handoff/classification fails closed and escalates; alert and do not commit work from the orphaned entry.
...
The canonical root and plan allocation are owned by [Transaction Strategies](./system-architecture-transactions.md#command-atomicity-and-outcome-aggregation). This operations document owns their failure consequences: the ledger retains the persisted mutation `EffectId`, enclosing root lineage, `plan_ordinal`, timeline, typed operation, exact target, digest, participant projection, and replay lineage; ordinary retry and uncertain replay preserve them. A root mutation uses its root `EffectId`; a generated child uses its persisted deterministic child `EffectId` while retaining the enclosing root as lineage and reconciliation context. Retry eligibility is mutable operational metadata and never rewrites that binding. A post-abandon command re-drive requires conclusive terminal evidence, a new durable `commandId`, and explicit lineage to the abandoned command/effect; reusing the old command id returns its terminal result. Non-command sources follow their owner-specific linked re-drive identity. The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is not current target state.
...
- The tick effect ledger persists a concrete, namespace-complete projection of the persisted mutation `EffectId`, enclosing root lineage, and tick coordinates for scope, ordering, evidence, and target lookup:
  - The storage projection includes at minimum `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, target_aggregate_type, target_aggregate_id)`. `playable_state_scope` and the tick/region coordinates are persisted and exact-validated evidence or ordering metadata, not uniqueness dimensions. The ledger's logical uniqueness key is exactly `(effect_id, typed_operation, target_aggregate_type, target_aggregate_id)`, where `effect_id` is the root `EffectId` for a root mutation and the persisted child `EffectId` for a generated child; `root_effect_id` is retained as enclosing-root lineage and reconciliation context, not as a substitute for `effect_id`. An owner-local physical partition, if used, is storage placement only and cannot alter logical uniqueness or deduplication. The physical schema must enforce or project that exact logical identity without promoting `playable_state_scope` or tick coordinates into guard uniqueness. `effect_key` may be stored in the projection for stable descriptor/correlation metadata, but it is not the mutation identity or sole collision protection. Mutable `status`, `reason`, `outcome`, evidence/reconciliation fields, and timestamps remain outside the uniqueness key. The physical schema may encode target identity separately, but it must retain an explicit collision-safe projection containing `effect_id`, `root_effect_id`, `typed_operation`, and the exact target aggregate.
  - Additional columns such as `command_id`, `automation_dispatch_id`, or `command_ordinal` may exist for queryability and scripting correlation, but they do not replace the persisted mutation `EffectId` or participant guard identity.
  - On reuse of the persisted mutation identity, the writer exact-compares all immutable scope, lineage, timeline, descriptor, namespace, and request-digest bindings, including the stored projection fields, and fails closed on any conflict; it must not insert another row for a conflicting reuse. For a root mutation, `effect_id` is the root `EffectId`; for a generated child, the persisted child `EffectId` remains the distinct mutation identity while `root_effect_id` retains the enclosing-root lineage used for reconciliation.
- For the Game Session command-root and tick-ledger contract in this document, the command-root allocation identity remains the opaque `(tenantId, gameInstanceId, commandId, planOrdinal)` binding and its collision-safe runtime allocation partition remains `(tenantId, gameInstanceId)`; that command allocation scope is distinct from owner-local participant-guard uniqueness. Every ledger row retains the namespace-complete tick/recovery projection above even when the owner-declared mutation identity or allocation partition uses a narrower uniqueness key. `effect_key` remains descriptor metadata, and mutable status, reason, outcome, evidence, and timestamps remain outside identity protection.
- Guard tables such as `tick_effect_guard` implement the deterministic participant projection for multi-effect operations:
  - Their uniqueness identity follows the owner-declared runtime family: S1/S2 uses the `(tenantId, playableStateNamespaceId)` partition plus the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate; only an explicitly owner-classified S3 family uses `(tenantId, gameInstanceId)` plus those identity fields. For S1/S2, `gameInstanceId` is separately exact-validated active-runtime authorization/fence evidence. The immutable request digest and applicable namespace binding are exact-compared on every replay; `playableStateScope`, active-instance evidence, timeline, and lineage remain immutable evidence, while durable outcome/reconciliation fields are mutable row state protected by CAS and are not uniqueness inputs. Unknown or unclassified families fail closed before guard admission and are never inferred to be S3.
  - The concrete tick/ledger projection columns remain storage scope/order/lookup metadata, not an alternative guard identity. Handlers must not invent alternative idempotency keys for tick-driven effects or use enclosing-root lineage, child ordinal, `effect_key`, outcome, or evidence state as guard identity. Work explicitly outside playable-state scope must use an accepted owner-specific idempotency/partition contract; it cannot omit namespace/scope and proceed under this gameplay participant-guard contract.
...
- Dashboards and alerts should show bounded scope-class rollups where `tick_effects_pending_total{scope_class=~"^(region|game_instance|tenant|cluster)$"} > 0` and `increase(tick_effects_replay_batches_total{scope_class=~"^(region|game_instance|tenant|cluster)$"}[5m]) == 0` (or the equivalent `rate(...[5m]) == 0`) over the explicit five-minute window; exact affected regions require authoritative runtime-health/control-plane lookup.
...
- A background “ledger replay controller” in Game Session:
  ...
  - Marks rows `ABANDONED` with a precise reason only when authoritative owner/domain evidence proves the effect was not applied and an applicable feature rule makes it no longer safe or meaningful (for example, an entity was removed or a session expired, or a reset/epoch change invalidated the effect after that evidence); a reset or epoch change alone never proves non-application. Inconclusive work remains non-terminal under the original mutation `EffectId`, with the enclosing root retained as lineage, is escalated for reconciliation, and follows the applicable evidence policy.
...
- The controller also runs on service startup for each complete recovery scope to converge any lingering `SCHEDULED` rows before normal tick processing resumes; any inconclusive row instead remains reconciliation-required, is escalated, and cannot be hidden by normal startup replay. Before startup replay mutates or reconciles a row, the owner must explicitly classify the path as non-conflicting routine idempotent replay and record that classification/evidence, or pass the exact ADR 0085 maintenance handoff; an absent or incomplete classification/handoff fails closed.
- For incident handling, the same replay logic is exposed via coordination tooling (for example, an admin CLI or maintenance API) so operators can explicitly drive convergence for a selected complete `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch)` after authoritative enumeration and exact validation. Incomplete or caller-supplied namespace/scope evidence must fail closed before any replay or maintenance mutation when guided by runbooks in the Redis operations docs. The incident path is a coordination-maintenance operation and must pass the exact ADR 0085 handoff before replay, reconciliation, or cleanup mutation; Redis leases and executor fences do not substitute for it.
...
The same policy applies whenever a region-, tenant-, or cluster-scoped reset or epoch fence leaves an old-epoch effect in `SCHEDULED` and normal domain inspection cannot yet prove whether the effect was applied. Recovery must use a bounded, out-of-band attestation under the original mutation `EffectId`, retaining its enclosing root as lineage and reconciliation context; reset scope alone is never evidence for terminalization. The attestation:

- retains the original concrete ledger/guard projection `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectId, rootEffectId, typedOperation, requestDigest, effectKey, targetAggregateType, targetAggregateId)`, with the persisted mutation `effectId` as the root-or-child collision identity and `rootEffectId` retained only as enclosing lineage/reconciliation context; the owner-declared runtime-family partition plus `effectId`, `typedOperation`, and exact target aggregate form the collision-safe participant identity (S1/S2 uses `tenantId + playableStateNamespaceId`; only explicit S3 uses `tenantId + gameInstanceId`). The attestation independently exact-compares the partition, typed operation, exact target aggregate, immutable request digest, and every other projection field against both the durable ledger row and authoritative domain guard/state; any missing, extra, partial, or conflicting field fails closed. It also retains the exact external maintenance `operationId`, fence/lock binding, classifier decision, evidence digest, and complete affected scope that authorized the reset; `playableStateScope` and active-instance evidence remain immutable bound evidence;
...
- permits exactly these outcomes: ledger `status=APPLIED` when durable domain state or the authoritative guard proves the effect occurred; ledger `status=ABANDONED` with an explicit reset/reconciliation reason only when durable state proves it was unapplied and cannot be safely re-driven; or retained ledger `status=SCHEDULED` with explicit `reconciliation_required` metadata/reason while the evidence remains inconclusive.
...
Current-epoch executors must never re-drive the old mutation `EffectId`. Any feature that needs to carry work across the epoch boundary requires separately designed maintenance or saga/outbox tooling. A new current-epoch identity is a **post-abandon re-drive** only: the old effect and source claim must first be terminal `ABANDONED`, then the tooling allocates a later coordinate, fresh root and retry/source identity, and durable lineage. Ordinary retry/replay preserves the old mutation identity and its enclosing root lineage, and inconclusive work cannot be re-driven.
...
- **Primary crash, AOF fully up to date**
  ...
  - Action: new executor replays any surviving `pending` entries; ticks complete or are retried automatically. This ordinary replay may omit a maintenance operation only when the owner explicitly classifies it as non-conflicting routine idempotent replay and records the classification/evidence; a recovery skip remains limited to an explicitly disposable ADR 0058 hint or an ADR 0058 reconciliation that durably proves an empty cadence with no unresolved authoritative work. Otherwise the exact ADR 0085 handoff must be validated and persisted before replay or reconciliation, and inconclusive work remains non-terminal.
...
- **GC pause > `lock_ttl_ms` but < `lease_ttl_ms`**
  ...
  - PostgreSQL: any effects applied before the pause remain consistent; replays of the same concrete ledger/guard projection `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectId, rootEffectId, typedOperation, requestDigest, effectKey, targetAggregateType, targetAggregateId)` retain the original mutation `EffectId` and enclosing root lineage and are treated as no-ops by idempotent handlers. Guard uniqueness follows the owner-declared runtime family (`tenantId + playableStateNamespaceId` for S1/S2, `tenantId + gameInstanceId` only for explicit S3, plus the mutation identity/operation/exact target); `playableStateScope`, active-instance evidence, and `requestDigest` are immutable bound evidence.
...
- **Redis coordination cluster outage**
  ...
  - Action: Game Session halts ticks/commands for affected regions, following the Redis outage policy; after Redis recovers, the owner recovery subsystem or operators may choose retry or repair for missing ticks after validating, persisting, and passing the exact ADR 0085 handoff (`operationId`, fence/lock binding, classifier decision, evidence digest, and complete affected scope) unchanged. A skip is permitted only for an explicitly disposable ADR 0058 hint or after ADR 0058 reconciliation durably proves an empty cadence with no unresolved authoritative work; otherwise recovery replays or reconciles durable work, retaining inconclusive items as non-terminal. A non-conflicting routine replay may omit that maintenance operation only when the owner explicitly records the specific classification and evidence; missing or incomplete handoff/classification fails closed and escalates.
...
- An automated recovery worker:
  - Re-reads the current control-plane/runtime-health authority for the complete candidate scope and, before any recovery or cleanup mutation, revalidates the same in-memory Redis lease token it acquired and the expected candidate `regionEpoch`, then separately verifies the current durable `executorFence`. It must also pass the exact ADR 0085 maintenance handoff, or have the owner explicitly classify and record this specific path as non-conflicting routine idempotent replay; that classification cannot be inferred from Redis residue. A missing, stale, or mismatched token, epoch, authority record, handoff, or classification fails closed; the worker must not persist the raw token, clear pending state, or stage/commit work under a different owner or epoch.
  - Uses the same idempotent handlers and ledger/guard patterns as normal ticks to drive any effects associated with the stuck tick to terminal `APPLIED` or `ABANDONED` outcomes only when the existing evidence policy permits; inconclusive effects remain non-terminal reconciliation work and are escalated. It reconciles a higher-level command status only through the canonical command-outcome contract and supported durable `executionOutcome` values, using its monotonic status version/CAS and preserving `failureCode`/`failureMessage`. Tick-local labels such as `FAILED` or `SKIPPED` are not additional canonical `GetGameplayCommandStatus` values: they must be mapped to a supported command outcome only when the complete effect evidence and command-family policy justify that mapping; otherwise the command remains pending/reconciliation-required. The status record is authoritative, while events/metrics remain projections.
  - Immediately before clearing, revalidates the selected branch: the exact ADR 0085 handoff (`operationId`, external maintenance fence/lock binding, evidence-derived classifier decision, durable-evidence digest, and complete affected scope), or the owner-recorded non-conflicting routine-replay classification/evidence with its applicable fence, epoch, and scope proof. For a maintenance handoff, every operation/fence/classifier/digest/scope field must exact-match; for routine replay, missing or mismatched classification, non-conflict evidence, fence, epoch, or scope fails closed. Any stale or regressed evidence leaves `pending` and retry metadata untouched and escalates for fenced reconciliation; only then does a dedicated idempotent helper clear `tick:{tenantRegionTag}:pending` and associated retry metadata.
...
Domain services use the **region-scoped tick timeline** `(regionEpoch, tickId)` as ordering and fence context, not as the canonical idempotency token for tick-driven effects. The canonical participant guard logical uniqueness is the owner-declared runtime-family partition plus `(effectId, typedOperation, exact target aggregate)`, where `effectId` is the persisted root `EffectId` for a root operation and the persisted child `EffectId` for a generated child; S1/S2 uses `(tenantId, playableStateNamespaceId)`, while only an explicitly owner-classified S3 family uses `(tenantId, gameInstanceId)`, and unknown or unclassified families fail closed. `rootEffectId` is retained only as enclosing-root lineage and reconciliation context. The immutable request digest is bound to that identity. Additional physical partitioning used only for storage placement cannot alter this logical identity or deduplication. A narrow last-tick watermark is permitted only for an explicitly documented aggregate proven to receive at most one logical mutation per tick timeline; it must not replace the effect/participant guard for general command effects. Any applicable namespace binding is validated separately from timeline ordering and fence evidence, and a guard must not substitute the timeline tuple for the root/operation/aggregate identity. Exact allocator details remain pending and non-authoritative under the conditional [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md); they do not change this accepted guard contract.

`tickId` is monotonic only **within a given `regionEpoch`** and may restart at `0` after a region-scoped or tenant-scoped reset that bumps `regionEpoch`. Any ordering or watermark scheme that keys only on `tickId` (without `regionEpoch`) is therefore unsafe across resets; the timeline pair still does not replace the effect/participant guard.
...
- **Operation-level effect guard**
  ...
  - Operations that may touch multiple aggregates or legitimately apply multiple distinct effects to the same aggregate in a single tick (for example trades, combat damage, healing, AoE damage, room occupancy changes, drops/pickups, or multi-target buffs) use a small guard table such as `tick_effect_guard` with a concrete participant projection storing:
    - The uniqueness identity follows the owner-declared runtime family: S1/S2 uses the `(tenantId, playableStateNamespaceId)` partition, while only an explicitly owner-classified S3 family uses `(tenantId, gameInstanceId)`; each partition also includes the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate. A root mutation uses the root `EffectId`; a generated child uses its persisted child `EffectId` while retaining the enclosing root as lineage and reconciliation context. For S1/S2, `gameInstanceId` is separately exact-validated active-runtime authorization/fence evidence. The immutable request digest and applicable namespace binding are bound evidence exact-compared on replay; `playableStateScope`, active-instance evidence, timeline, and lineage remain immutable projection evidence, and durable outcome/evidence is mutable guard-row state advanced by CAS. The concrete fields below are retained as the participant projection for scope, ordering, and target lookup and must not be mistaken for an alternative uniqueness identity. Unknown or unclassified families fail closed before guard admission and are never inferred to be S3.
    - `effect_id` (the persisted mutation `EffectId` used by guard uniqueness)
    - `root_effect_id` (enclosing root lineage and reconciliation context)
    - `typed_operation`
    - `tenant_id`
    - `game_instance_id`
    ...
    - `request_digest` – immutable request digest bound to the participant guard and compared on replay, not a uniqueness input.
    - `target_aggregate_type`
    - `target_aggregate_id`
  - Inside the same transaction as the domain update, the handler attempts to insert a participant guard whose collision-safe uniqueness identity follows the owner-declared runtime family: `(tenantId, playableStateNamespaceId, mutation EffectId, typed operation, target aggregate type, target aggregate ID)` for S1/S2, or `(tenantId, gameInstanceId, mutation EffectId, typed operation, target aggregate type, target aggregate ID)` only for explicit S3. `tenantId` and `playableStateNamespaceId` partition namespace-backed S1/S2 guards, while `tenantId` and `gameInstanceId` partition explicitly disposable S3 guards; for S1/S2, `gameInstanceId` is separately exact-validated active-runtime authorization/fence evidence. `playableStateScope`, timeline, digest, and lineage fields are immutable bound evidence, not additional uniqueness inputs. The guard binds the immutable request digest and stores the namespace-complete concrete projection `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, effect_key, target_aggregate_type, target_aggregate_id)` as scope/order/ledger metadata. For a generated child, `root_effect_id` is retained as enclosing lineage/reconciliation context and is not substituted for `effect_id`. Unknown or unclassified families, or work explicitly outside playable-state scope without an accepted owner-specific partition, are rejected before this insert.
  ...
  - If the uniqueness identity conflicts, the handler compares the stored owner-declared runtime-family partition, typed operation, exact target aggregate, immutable request digest, applicable `playableStateNamespaceId` binding, separately bound `playableStateScope` evidence, and separately validated active-instance authorization/fence evidence. An exact existing guard replay returns its stored outcome without reapplying domain state; any changed family/partition, operation, target, digest, namespace, scope, or active-instance evidence fails closed. It must also verify the complete expected participant-guard set and corresponding authoritative state. Only a complete, consistent set is a replay/no-op; a partial conflict is incomplete and must reconcile the original mutation `EffectId` and enclosing root lineage or fail closed.
  - A replay attempted later in physical time with the same mutation `EffectId`, typed operation, and target aggregate reuses the same guard only when it carries and exactly matches the original sealed runtime-family partition and namespace-complete timeline projection, including the bound namespace where applicable, separately bound `playableStateScope`, active-instance authorization/fence evidence, `region_epoch`, and `tick_id`. A request with changed coordinates or a changed active-instance fence is conflicting reuse and fails closed; neither changed coordinates nor `effect_key`, enclosing-root lineage, or mutable outcome/evidence state can create a second participant guard or overwrite the original projection.
...
- **Trade between two entities (operation-level effect guard)**
  ...
  - In one transaction it:
    - Attempts to insert one guard per affected inventory aggregate with the namespace-backed S1/S2 inventory uniqueness identity `(tenantId, playableStateNamespaceId, mutation EffectId=rootEffectId, typed operation=TradeItem, targetAggregateType=INVENTORY, targetAggregateId)`. Inventory is durable player value and therefore remains namespace-stable, with any S2 version mapping validated and applied by its owner; it is not an S3 disposable family. The guard binds and exact-compares `requestDigest`, separately validates active `gameInstanceId` authorization/fence evidence, and retains `playableStateScope`, `regionId`, `regionEpoch`, `tickId`, and `effectKey` as exact-bound projection/ledger evidence, not additional uniqueness dimensions.
    - If any uniqueness identity conflicts, it re-reads the complete expected guard set and the authoritative state of both source and target inventories. Only a complete guard set whose mutation/root effect, typed operation, target aggregates, immutable `requestDigest`, and inventory state all match the committed transfer is a replay/no-op; a partial conflict, missing guard, or mismatch must reconcile the original mutation/root effect or fail closed. `effectKey` is never used to admit a replay.
...
- Every tick-driven write path must use the operation-level guard pattern by default; the per-aggregate `last_tick_id` pattern is allowed only with an explicit proof of at-most-one logical mutation per tick timeline.
- The default review assumption is that a gameplay-visible mutation requires an effect guard unless the design explicitly proves “at most one logical effect per aggregate per tick”; that proof must not be inferred from `(regionEpoch, tickId)` alone.
...
Tick-driven domain calls use the mutation's persisted root-or-child `EffectId` plus a shared participant guard contract for retries:

- For command-root work, Game Session assigns the opaque root `EffectId` from the durable command row's `(tenantId, gameInstanceId, commandId, planOrdinal)` allocation binding, which also validates the frozen request fingerprint/runtime scope, applicable `playableStateNamespaceId`, and ordered plan manifest digest. Passive, inbound/remote, timer, retry, and already-generated work retain their owner/source-specific root identity unless they materialize as an admitted command. A generated child uses its persisted deterministic child `EffectId` under the owning child contract and does not create another command-plan root or allocator. A concrete ledger/guard/storage projection must carry the namespace-complete applicable mutation identity and tick context `(effectId, rootEffectId, typedOperation, tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectKey, targetAggregateType, targetAggregateId)` for scope, ordering, and exact target lookup; generated children retain `rootEffectId` as lineage and reconciliation context. The command-root/tick-ledger allocation binding remains distinct from participant-guard uniqueness: each participant derives its guard identity from the owner-declared runtime-family partition—`(tenantId, playableStateNamespaceId)` for S1/S2 or `(tenantId, gameInstanceId)` only for explicit S3—plus the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate, binding the immutable request digest and applicable namespace evidence. For S1/S2, `gameInstanceId` is separately exact-validated active-runtime authorization/fence evidence; durable outcome/evidence state is mutable guard-row state protected by CAS. Unknown or unclassified families, and work explicitly outside playable-state scope without an accepted owner-specific partition, fail closed before guard admission. Additional operation-specific distinctions belong in that typed operation/target/request-digest contract; they must not create a competing root identity. The exact allocator and versioned fan-out binding remain conditional on the non-authoritative [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md).
- Game Session passes the mutation's persisted root-or-child `EffectId` plus the structured operation, target, immutable request digest, and any concrete ledger projection fields to each tick-invoked handler. Generated children also carry their enclosing root/parent lineage binding. Handlers validate and project that contract into their own guards; they must not generate fresh random IDs, re-derive from mutable payload, or silently drop required scope or target fields.
...
- **Duplicate handling** – repeated calls with the same effect identity must return OK / “already applied” semantics (for example, no new HP or inventory changes) instead of logical errors that drive infinite retries.
  - **Already-in-desired-state handling** – if the target state already reflects the intended outcome, the endpoint first resolves the participant guard in the owner transaction. An exact existing guard with the same owner-declared runtime-family partition (S1/S2 `(tenantId, playableStateNamespaceId)` or explicit S3 `(tenantId, gameInstanceId)`), mutation `EffectId` (root or persisted child), typed operation, exact target aggregate, immutable `requestDigest`, separately persisted `playableStateScope`, and separately exact-validated active-instance fence evidence returns its stored outcome; unknown or unclassified families, or work explicitly outside playable-state scope without an accepted owner-specific partition, are rejected before guard admission. When no matching guard exists, the owner records a new durable guarded no-op under the same conditional partition and exact evidence before returning OK. A generated child also retains its enclosing root as lineage and reconciliation context. Mutable state already matching the requested value is not replay evidence by itself, and a conflicting or ambiguous guard fails closed.
...
- **Tenant-scoped reset**
  - Timeline impact:
    ...
    - Cross-region flows (for example follow-ups) resume only under the new epochs; stale follow-ups from previous epochs are reconciled under the [Inconclusive Old-Epoch Reconciliation Policy](#inconclusive-old-epoch-reconciliation-policy), never ignored, and remain `APPLIED`, evidence-qualified `ABANDONED`, or fenced non-terminal `SCHEDULED` according to that policy.
...
- For each complete expected concrete participant-projection set linked to an enclosing root `EffectId`, every expected `(effectId, rootEffectId, typedOperation, tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, targetAggregateType, targetAggregateId)` projection must eventually have exactly one terminal ledger state (`APPLIED` or `ABANDONED`) when the evidence policy permits terminalization. `effectId` is the mutation's persisted root-or-child `EffectId`; `rootEffectId` remains enclosing lineage and reconciliation context. `typedOperation` is part of the expected set alongside the mutation ID and exact target aggregate; guard uniqueness follows the owner-declared runtime-family partition (`tenantId + playableStateNamespaceId` for S1/S2, `tenantId + gameInstanceId` only for explicit S3), while `playableStateScope`, active-instance evidence, and `effectKey` remain immutable projection/descriptor evidence rather than additional uniqueness dimensions. Reconciliation rejects missing, extra, partial, or conflicting projections; any inconclusive row remains reconciliation-required and non-terminal, with old-epoch rows additionally following the [Inconclusive Old-Epoch Reconciliation Policy](#inconclusive-old-epoch-reconciliation-policy), including across resets.
- Reset tooling and Game Session control flows must ensure that no tick remains forever “half-applied” in the ledger (for example, perpetually `SCHEDULED` with no chance of replay), by running a per-effect tick-effect-ledger reconcile for the relevant `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch)` combinations as part of the reset flow. The reconcile must inspect durable domain state before terminalizing each row and may abandon only effects confirmed unapplied; inconclusive effects remain `SCHEDULED`/reconciliation-required, and reset reconciliation must neither force nor require their terminalization without that evidence.
...
- Each leg of a cross-region flow (origin-region effects, target-region effects, and any results relayed back) uses its own persisted mutation `EffectId` and converges independently to `APPLIED` or `ABANDONED` in the tick effect ledger only when the existing evidence policy permits terminalization; inconclusive work remains non-terminal reconciliation work under its original mutation `EffectId`, with its enclosing root retained as lineage, and is escalated, with old-epoch work additionally following the applicable reset policy.
...
When origin reconciliation accepts a target result that drives a dependent child effect (for example, remote lifesteal damage driving a local heal), the acceptance transition is crash-safe: it cannot become durable without a deterministic child-effect intent. The accepted result and child intent commit in one owner-local transaction when possible, or a resumable durable acceptance intent records both before acceptance is acknowledged. Acceptance replay reuses the child identity and exact accepted-result digest; missing, extra, partial, or conflicting result/intent projections fail closed. This is a durability consequence of the existing coordinator lifecycle, not a new outcome. Any future opaque allocation shape remains conditional on the non-authoritative [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md).
...
- When a target region’s `regionEpoch` is bumped (via region/tenant/cluster reset):
  ...
  - Target-region tick executors reject old-epoch work as non-executable based on epoch/tick guards in their coordination scripts; they only stage and apply effects for the current epoch. That rejection is not a terminal outcome or a disposable “ignored” observation: the durable follow-up/ledger identity remains enumerated and must reconcile to ledger `status=APPLIED`, evidence-qualified ledger `status=ABANDONED`, or retained ledger `status=SCHEDULED` with explicit `reconciliation_required` metadata/reason and its scope fenced. An ambiguous or missing execution response therefore cannot strand the follow-up behind an `ignored` label or make it eligible in the new epoch.
- Origin regions:
...
- **Identity and scoping**
  - Each follow-up is tied to a specific target region timeline and effect identity, including at minimum:
    ...
    - An immutable `request_digest` and sealed required-participant manifest (or immutable manifest reference plus digest) are persisted as binding/comparison evidence. Target admission and replay compare the exact operation, target, namespace, separately persisted `playable_state_scope`, digest, and manifest binding; any missing, conflicting, or mismatched value fails closed.
...
- **Topology changes**
  ...
  - If region split/merge changes which region owns the target entity, topology-change tooling must either:
    ...
    - Mark it `ABANDONED` with a topology-change reason only when the existing evidence policy proves replaying it under the new mapping is not valid and no required mutation succeeded; otherwise retain the old identity as non-terminal reconciliation work under its original mutation `EffectId`, with its enclosing root retained as lineage and reconciliation context.
...
- Repeat the same `effectKey` against a different `targetAggregateType` or `targetAggregateId` and assert that the distinct target is not incorrectly deduplicated. For script-generated fan-out effects, vary `commandOrdinal` under one `automationDispatchId` and assert distinct logical identities, then replay the same dispatch/ordinal pair and assert stable deduplication; also vary, as applicable, the persisted root-or-child `EffectId`, enclosing root lineage, typed operation, and target aggregate. `effectKey` is projection/storage metadata, never guard uniqueness, and a request-digest mismatch for the same guard or dispatch/ordinal identity must fail closed.
- Exercise deterministic command-plan admission with zero, one, and multiple roots: assert that semantic order comes from the frozen typed command/action contract or `ResolvedEffectPlan`, built-ins declare equivalent stable order, ambiguous/duplicate order fails before admission, and the ordered plan manifest/digest plus opaque `(tenantId, gameInstanceId, commandId, planOrdinal)` allocation binding persist in the local plan/staging transaction and reuse only when the command row identity, frozen request/runtime/namespace binding, ordinal, and manifest digest match. Assert no root allocation for zero effects, and that random batch IDs, mutable text, effect keys, source ordering, participant tuples, Trigger Identity/`scriptEventId`, and generated child ordinals cannot substitute for the durable command row identity. Exercise Automation handoff admission exact-mapping to one durable target `commandId`, and exercise passive, inbound/remote, timer, retry, and already-generated work through their existing owner/source-specific root identities rather than synthesizing a command plan unless materialized as an admitted command.
...
- **What is the idempotency key?**
  - Which frozen typed command/action contract or `ResolvedEffectPlan` supplies semantic order for the command's logical root effects, and how does it reject ambiguous/duplicate order before admission, persist the ordered manifest/digest, and allocate an opaque binding unique on `(tenantId, gameInstanceId, commandId, planOrdinal)` only after the durable command row freezes/binds its request fingerprint, resolved runtime scope, and applicable `playableStateNamespaceId`? Reuse must validate the same command row identity, frozen binding, ordinal, and manifest digest; any mismatch fails closed. A zero-effect command must allocate neither. Source ordering remains separate, and non-command-root work retains its owner/source-specific identity.
  - For single-aggregate updates: has the owner proved that the aggregate receives at most one logical mutation per tick and declared its runtime family? An explicit S3 watermark uses the complete `(tenant_id, game_instance_id, playable_state_namespace_id, region_id, target_aggregate_type, aggregate_id)` key plus separately exact-validated `playable_state_scope` evidence and `(last_region_epoch, last_tick_id)` (or equivalent) fields. An S1/S2 watermark instead uses its owner-defined namespace partition and exact-validates the active `game_instance_id` only as separate authorization/fence evidence; it must not copy the S3 instance-keyed identity. Any aggregate capable of multiple effects in one tick uses the participant/effect guard contract below instead.
  - For multi-aggregate or multi-effect operations: what participant guard identity combines the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate, and how is the immutable request digest bound and checked? A generated child retains its enclosing root as lineage and reconciliation context; `effect_key` is a concrete storage projection/descriptor, not a replacement for that guard identity.
