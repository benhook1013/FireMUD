...
The canonical root and plan allocation are owned by [Transaction Strategies](./system-architecture-transactions.md#command-atomicity-and-outcome-aggregation). This operations document owns their failure consequences: the ledger retains the persisted root, `plan_ordinal`, timeline, typed operation, exact target, digest, participant projection, and replay lineage; ordinary retry and uncertain replay preserve them. Retry eligibility is mutable operational metadata and never rewrites that binding. A post-abandon command re-drive requires conclusive terminal evidence, a new durable `commandId`, and explicit lineage to the abandoned command/effect; reusing the old command id returns its terminal result. Non-command sources follow their owner-specific linked re-drive identity.
