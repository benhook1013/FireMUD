...
- **Region-scoped reset**
  - Timeline impact:
    ...
    - Tenant-scoped coordination such as `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` and current `sessionctx:*` context follows the explicitly recorded region reset policy in [Redis Reset & Recovery](./system-architecture-redis-reset-and-recovery.md); no session-retention default is inferred. Account-issued token registry records and issuer-generation projections under `session:auth:token:*` and `session:auth:generation:*` are not deleted by a region-only prefix scan. If the recorded policy preserves gameplay sessions, they must still pass auth/revocation validation and any required re-authentication before rebind.
...
- **Tenant-scoped reset**
  - Timeline impact:
    ...
    - Session and authentication keys follow the canonical reset-policy matrix in [Redis Reset & Recovery](./system-architecture-redis-reset-and-recovery.md): a tenant reset does not perform account-wide token invalidation, while gameplay/session-context records are retained or invalidated only under the explicit recorded policy. Preserved sessions must rebind after auth validation.
  - Ledger behavior:
