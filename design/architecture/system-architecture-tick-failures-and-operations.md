...
- **Operation-level effect guard**
  ...
  - Operations that may touch multiple aggregates or legitimately apply multiple distinct effects to the same aggregate in a single tick (for example trades, combat damage, healing, AoE damage, room occupancy changes, drops/pickups, or multi-target buffs) use a small guard table such as `tick_effect_guard` with a concrete participant projection storing:
    ...
    - `target_aggregate_id`
  - Inside the same transaction as the domain update, the handler attempts to insert a participant guard whose collision-safe uniqueness identity is exactly `(tenantId, playableStateNamespaceId, mutation EffectId, typed operation, target aggregate type, target aggregate ID)`. `tenantId` and `playableStateNamespaceId` partition the owner-local EffectId allocation boundary; `gameInstanceId`, `playableStateScope`, and the remaining runtime coordinates are immutable bound evidence, not additional uniqueness inputs. The guard binds the immutable request digest and stores the concrete projection `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, effect_key, target_aggregate_type, target_aggregate_id)` as scope/order/ledger metadata. For a generated child, `root_effect_id` is retained as enclosing lineage/reconciliation context and is not substituted for `effect_id`.
