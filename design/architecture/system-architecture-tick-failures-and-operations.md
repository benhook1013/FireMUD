...
- The tick effect ledger persists a concrete, namespace-complete projection of the root `EffectId` and its tick coordinates for scope, ordering, evidence, and target lookup:
  ...
  - On reuse of the root identity, the writer exact-compares all immutable scope, lineage, timeline, descriptor, namespace, and request-digest bindings, including the stored projection fields, and fails closed on any conflict; it must not insert another row for a conflicting reuse. For a root mutation, `effect_id` is the root `EffectId`; for a generated child, the persisted child `EffectId` remains distinct mutation identity while `root_effect_id` retains the enclosing-root lineage used for reconciliation.
