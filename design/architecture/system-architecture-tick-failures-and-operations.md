...
- The tick effect ledger’s primary or unique key retains a concrete projection of the persisted mutation `EffectId`, its enclosing root lineage, and its tick coordinates:
  - At minimum `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_scope, region_id, region_epoch, tick_id, target_aggregate_type, target_aggregate_id)`; `effect_id` is the root `EffectId` for a root mutation and the persisted child `EffectId` for a generated child, while `root_effect_id` remains lineage and reconciliation context. This collision-safe storage projection includes the mutation identity, typed operation, and exact target aggregate alongside the concrete scope and supplies gameplay scope, ordering, and target lookup. `effect_key` may be stored in the projection for stable descriptor/correlation metadata, but it is not an identity or sole collision protection. Mutable `status`, `reason`, `outcome`, evidence/reconciliation fields, and timestamps remain outside the uniqueness key. The physical schema may encode target identity separately, but it must retain an explicit collision-safe projection containing `effect_id`, `root_effect_id`, `typed_operation`, and the exact target aggregate.
