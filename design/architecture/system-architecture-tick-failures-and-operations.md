...
- **Domain Idempotency with Tick Ordering and Fencing Context** – how mutation-identity/participant guards enforce mutation idempotency while `(regionEpoch, tickId)` provides ordering and fencing context.
...
- For the complete expected concrete participant-projection set linked to an enclosing root `EffectId` and participant guard contract, every expected `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, target_aggregate_type, target_aggregate_id)` projection must have **exactly one terminal state once the applicable recovery/evidence policy permits terminalization**; explicitly inconclusive projections remain non-terminal reconciliation-required states (for example, `SCHEDULED`) and continue to block convergence. `effect_id` is the mutation's persisted root-or-child `EffectId`; `root_effect_id` remains enclosing lineage and reconciliation context. `typed_operation` is part of the participant projection alongside the mutation ID and exact target aggregate; `playable_state_namespace_id` is immutable bound runtime evidence, not a replacement guard identity. `effect_key` remains descriptor metadata for correlation and lookup, not a substitute for that identity. Replay/recovery must reconcile the whole expected set and fail closed for missing, extra, partial, or conflicting projections; one root-effect or one ledger-row result is insufficient:
...
- The tick effect ledger’s logical identity is the persisted mutation `EffectId`, typed operation, and exact target aggregate within one explicitly declared allocation partition. For the Game Session command-root and generated-child contract in this document, that collision-safe partition is `(tenantId, gameInstanceId)`; additional physical partition columns do not become logical uniqueness inputs unless an owning feature contract declares a different allocation boundary:
  - `effect_id` is the root `EffectId` for a root mutation and the persisted child `EffectId` for a generated child. `playable_state_namespace_id`, `root_effect_id`, `region_epoch`, `tick_id`, `effect_key`, and other lineage/timeline fields remain stored immutable projections for reconciliation, ordering, correlation, and lookup; they are not uniqueness inputs. The physical schema may encode target identity separately, but its primary or unique constraint must retain an explicit collision-safe projection of `effect_id`, `typed_operation`, the exact target aggregate, and only the concrete scope needed by the feature’s identity boundary. Mutable `status`, `reason`, `outcome`, evidence/reconciliation fields, and timestamps remain outside the uniqueness key.
  - On reuse of that identity, the writer exact-compares all immutable scope, lineage, timeline, descriptor, namespace, and request-digest bindings (including the stored projection fields) and fails closed on any conflict; it must not insert another row for a conflicting reuse. This preserves queryability of the immutable projections without allowing them to create a second mutation guard.
  - Additional columns such as `command_id`, `automation_dispatch_id`, or `command_ordinal` may exist for queryability and scripting correlation, but they do not replace the persisted mutation `EffectId`, enclosing root lineage, or participant guard identity.
- Guard tables such as `tick_effect_guard` implement the deterministic participant projection for multi-effect operations:
  - Their uniqueness identity binds the required `(tenantId, gameInstanceId)` allocation partition, the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate. A root mutation uses the root `EffectId`; a generated child uses its persisted child `EffectId` while retaining the enclosing root as lineage and reconciliation context. The immutable request digest and applicable `playableStateNamespaceId` binding are exact-compared on every replay; durable outcome/evidence/reconciliation fields are mutable row state protected by CAS and are not uniqueness inputs.
  - The concrete tick/ledger projection columns remain storage scope/order/lookup metadata, not an alternative guard identity. Handlers must not invent alternative idempotency keys for tick-driven effects or use enclosing-root lineage, child ordinal, `effect_key`, outcome, or evidence state as guard identity.
...
- Candidate stuck ticks are enqueued into a `tick_recovery` queue or table with metadata such as `<tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, executorFence, firstSeenAt, lastRetryAt>`. Metrics and Redis contents identify candidates only; they do not establish the authoritative region, epoch, or owner.
...
## Domain Idempotency with Tick Ordering and Fencing Context

`(regionEpoch, tickId)` is ordering and fencing context, not a universal idempotency token for tick-driven mutations. The default mutation boundary is the mutation's stable persisted `EffectId` plus a participant guard containing the typed operation, exact target aggregate, and immutable request digest. Root mutations use the root `EffectId`; generated child mutations use their deterministic persisted child `EffectId` and retain the enclosing root as lineage and reconciliation context. A narrow last-tick watermark is permitted only for an explicitly documented aggregate proven to receive at most one logical mutation per tick timeline; it must not replace the effect/participant guard for general command effects.
...
- **Per-aggregate last-tick state**
  ...
  - A shadow tick-state record such as `entity_tick_state` is keyed by the complete `(tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, target_aggregate_type, aggregate_id)` identity, not by the aggregate identifier alone.
  ...
  - When applying a tick effect:
    - The handler resolves and reads the shadow tick-state row using the complete `(tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, target_aggregate_type, aggregate_id)` key; an aggregate-only, aggregate-type-free, or namespace-free lookup is invalid.
...
- **Operation-level effect guard**
  ...
  - Operations that may touch multiple aggregates or legitimately apply multiple distinct effects to the same aggregate in a single tick (for example trades, combat damage, healing, AoE damage, room occupancy changes, drops/pickups, or multi-target buffs) use a small guard table such as `tick_effect_guard` with a concrete participant projection storing:
    - The uniqueness identity is the required `(tenantId, gameInstanceId)` allocation partition, the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate. A root mutation uses the root `EffectId`; a generated child uses its persisted child `EffectId` while retaining the enclosing root as lineage and reconciliation context. The immutable request digest and applicable `playableStateNamespaceId` are bound evidence exact-compared on replay; durable outcome/evidence is mutable guard-row state advanced by CAS. The concrete fields below are retained as the participant projection for scope, ordering, and target lookup and must not be mistaken for an alternative uniqueness identity.
    ...
    - `target_aggregate_id`
  - Inside the same transaction as the domain update, the handler attempts to insert a participant guard whose collision-safe uniqueness identity is exactly `(tenantId, gameInstanceId, mutation EffectId, typed operation, target aggregate type, target aggregate ID)`. `tenantId` and `gameInstanceId` partition the Game Session root/child allocation boundary so reuse in another allocation scope cannot be mistaken for replay. `playableStateNamespaceId`, `playableStateScope`, and the remaining runtime coordinates are immutable bound evidence, not additional uniqueness inputs. The guard binds the immutable request digest and stores the concrete projection `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, region_epoch, tick_id, effect_key, target_aggregate_type, target_aggregate_id)` as scope/order/ledger metadata. For a generated child, `root_effect_id` is retained as enclosing lineage/reconciliation context and is not substituted for `effect_id`.
    ...
    - If the uniqueness identity conflicts, the handler compares the stored typed operation, exact target aggregate, immutable request digest, and applicable `playableStateNamespaceId` binding. An exact existing guard replay returns its stored outcome without reapplying domain state; any changed operation, target, digest, or namespace fails closed. It must also verify the complete expected participant-guard set and corresponding authoritative state. Only a complete, consistent set is a replay/no-op; a partial conflict is incomplete and must reconcile the original mutation `EffectId` and enclosing root lineage or fail closed.
  - A replay attempted later in physical time with the same mutation `EffectId`, typed operation, and target aggregate reuses the same guard only when it carries and exact-matches the original sealed timeline projection, including the bound namespace, `region_epoch`, and `tick_id`. A request with changed coordinates is conflicting reuse and fails closed; neither changed coordinates nor `effect_key`, enclosing-root lineage, or mutable outcome/evidence state can create a second participant guard or overwrite the original projection.
...
- **Once-per-tick aggregate watermark (per-aggregate last-tick state)**
  ...
  - It reads the shadow tick state for the complete `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, targetAggregateType, targetAggregateId)` key and applies the update only when `(last_region_epoch, last_tick_id) < (regionEpoch, tickId)`.
...
- **Trade between two entities (operation-level effect guard)**
  ...
  - In one transaction it:
    - Attempts to insert one guard per affected inventory aggregate with uniqueness identity `(mutation EffectId=rootEffectId, typed operation=TradeItem, targetAggregateType=INVENTORY, targetAggregateId)`, binding and comparing `requestDigest` and retaining `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectKey)` as projection/ledger metadata.
...
- **Identity and scoping**
  - Each follow-up is tied to a specific target region timeline and effect identity, including at minimum:
    ...
    - `tenant_id`, `target_game_instance_id`, target `playable_state_namespace_id`, resolved `playable_state_scope`, `target_region_id`, `target_region_epoch`
    ...
    - An immutable `request_digest` and sealed required-participant manifest (or immutable manifest reference plus digest) are persisted as binding/comparison evidence. Target admission and replay compare the exact operation, target, namespace, digest, and manifest binding; any missing, conflicting, or mismatched value fails closed.
...
- **What is the idempotency key?**
  ...
  - For single-aggregate updates: which complete `(tenant_id, game_instance_id, playable_state_namespace_id, playable_state_scope, region_id, target_aggregate_type, aggregate_id)` key plus `(last_region_epoch, last_tick_id)` (or equivalent) fields and table enforce “at most one update per tick timeline” for that aggregate?
