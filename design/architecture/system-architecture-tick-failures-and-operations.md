...
- **Redis coordination cluster outage**
  ...
  - Action: Game Session halts ticks/commands for affected regions, following the Redis outage policy; after Redis recovers, the owner recovery subsystem or operators may choose skip, retry, or repair for missing ticks only after validating, persisting, and passing the exact ADR 0085 handoff (`operationId`, fence/lock binding, classifier decision, evidence digest, and complete affected scope) unchanged. A non-conflicting routine replay may omit that maintenance operation only when the owner explicitly records the specific classification and evidence; missing or incomplete handoff/classification fails closed and escalates.
