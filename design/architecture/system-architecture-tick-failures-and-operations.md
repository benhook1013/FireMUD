...
- retains the original concrete ledger/guard projection `(tenantId, gameInstanceId, playableStateNamespaceId, playableStateScope, regionId, regionEpoch, tickId, effectId, rootEffectId, typedOperation, requestDigest, effectKey, targetAggregateType, targetAggregateId)`, with `rootEffectId` and `typedOperation` part of the collision-safe participant identity, and exact-compares that projection against both the durable ledger row and the authoritative domain guard/state; any missing, extra, partial, or conflicting field fails closed. It also retains the mutation `EffectId`, enclosing root lineage, and the exact external maintenance `operationId`, fence/lock binding, classifier decision, evidence digest, and complete affected scope that authorized the reset; guard uniqueness follows the owner-declared runtime family (`tenantId + playableStateNamespaceId` for S1/S2, `tenantId + gameInstanceId` only for explicit S3, plus the mutation identity/operation/exact target), while `playableStateScope`, active-instance evidence, and `requestDigest` are immutable bound evidence;
