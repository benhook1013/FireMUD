...
- `durable_committed`:
  - Ledger rows for `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId)` are terminal (`APPLIED` or `ABANDONED`) where the existing evidence policy permits terminalization; any inconclusive row remains non-terminal reconciliation work under its original mutation `EffectId`, with its enclosing root retained as lineage, and
...
- The tick effect ledger’s primary or unique identity is the persisted mutation `EffectId`, typed operation, and exact target aggregate, plus only the storage scope required to partition that identity boundary (for example, the applicable tenant, game-instance, playable-state, or region scope):
  - `effect_id` is the root `EffectId` for a root mutation and the persisted child `EffectId` for a generated child. `root_effect_id`, `region_epoch`, `tick_id`, `effect_key`, and other lineage/timeline fields remain stored immutable projections for reconciliation, ordering, correlation, and lookup; they are not uniqueness inputs. The physical schema may encode target identity separately, but its primary or unique constraint must retain an explicit collision-safe projection of `effect_id`, `typed_operation`, the exact target aggregate, and only the concrete scope needed by the feature’s identity boundary. Mutable `status`, `reason`, `outcome`, evidence/reconciliation fields, and timestamps remain outside the uniqueness key.
  - On reuse of that identity, the writer exact-compares all immutable scope, lineage, timeline, descriptor, and request-digest bindings (including the stored projection fields) and fails closed on any conflict; it must not insert another row for a conflicting reuse. This preserves queryability of the immutable projections without allowing them to create a second mutation guard.
...
- Guard tables such as `tick_effect_guard` implement the deterministic participant projection for multi-effect operations:
  - Their uniqueness identity binds the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate. A root mutation uses the root `EffectId`; a generated child uses its persisted child `EffectId` while retaining the enclosing root as lineage and reconciliation context. The immutable request digest is bound and compared on every replay; durable outcome/evidence/reconciliation fields are mutable row state protected by CAS and are not uniqueness inputs.
...
- A background “ledger replay controller” in Game Session:
  ...
  - Marks rows `ABANDONED` with a precise reason only when replay evidence proves the effect was not applied and is no longer safe or meaningful (for example, entities removed, sessions expired, or region/tenant/cluster resets that bumped `regionEpoch`); inconclusive work remains non-terminal under the original mutation `EffectId`, with the enclosing root retained as lineage, is escalated for reconciliation, and follows the applicable evidence policy.
...
- retains the original concrete ledger/guard projection `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, effectId, rootEffectId, effectKey, targetAggregateType, targetAggregateId)`, its mutation `EffectId`, enclosing root lineage, and the maintenance/recovery fence that authorized the reset;
...
- **Operation-level effect guard**
  ...
  - Operations that may touch multiple aggregates or legitimately apply multiple distinct effects to the same aggregate in a single tick (for example trades, combat damage, healing, AoE damage, room occupancy changes, drops/pickups, or multi-target buffs) use a small guard table such as `tick_effect_guard` with a concrete participant projection storing:
    - The uniqueness identity is the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate. A root mutation uses the root `EffectId`; a generated child uses its persisted child `EffectId` while retaining the enclosing root as lineage and reconciliation context. The immutable request digest is bound to that identity and checked on replay; durable outcome/evidence is mutable guard-row state advanced by CAS. The concrete fields below are retained as the participant projection for scope, ordering, and target lookup and must not be mistaken for an alternative uniqueness identity.
    ...
    - `target_aggregate_id`
  - Inside the same transaction as the domain update, the handler attempts to insert a participant guard whose uniqueness identity is exactly `(mutation EffectId, typed operation, target aggregate type, target aggregate ID)`, binding the immutable request digest and storing the concrete projection `(effect_id, root_effect_id, typed_operation, tenant_id, game_instance_id, playable_state_scope, region_id, region_epoch, tick_id, effect_key, target_aggregate_type, target_aggregate_id)` as scope/order/ledger metadata. For a generated child, `root_effect_id` is retained as enclosing lineage/reconciliation context and is not substituted for `effect_id`.
  ...
  - A replay at a later coordinate that retains the same mutation `EffectId`, typed operation, and target aggregate therefore reuses/conflicts with the same guard; `region_epoch`, `tick_id`, `effect_key`, enclosing-root lineage, and mutable outcome/evidence state cannot create a second participant guard.
...
- **Trade between two entities (operation-level effect guard)**
  - The `TradeItem` endpoint explicitly receives `rootEffectId` (the mutation `EffectId` for this root operation) and immutable `requestDigest` in addition to `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, fromEntityId, toEntityId, itemId)`. It creates one participant guard/ledger projection linked to that mutation/root effect per affected inventory aggregate.
  ...
  - In one transaction it:
    - Attempts to insert one guard per affected inventory aggregate with uniqueness identity `(mutation EffectId=rootEffectId, typed operation=TradeItem, targetAggregateType=INVENTORY, targetAggregateId)`, binding and comparing `requestDigest` and retaining `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, effectKey)` as projection/ledger metadata.
...
- For command-root work, Game Session assigns the opaque root `EffectId` from the durable command row's `(tenantId, gameInstanceId, commandId, planOrdinal)` allocation binding, which also validates the frozen request fingerprint/runtime scope and ordered plan manifest digest. Passive, inbound/remote, timer, retry, and already-generated work retain their owner/source-specific root identity unless they materialize as an admitted command. A generated child uses its persisted deterministic child `EffectId` under the owning child contract and does not create another command-plan root or allocator. A concrete ledger/guard/storage projection may carry the applicable mutation identity and tick context `(effectId, rootEffectId, typedOperation, tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, effectKey, targetAggregateType, targetAggregateId)` for scope, ordering, and exact target lookup; generated children retain `rootEffectId` as lineage and reconciliation context. Each participant derives its guard uniqueness identity from the mutation's persisted root-or-child `EffectId`, typed operation, and exact target aggregate, binding the immutable request digest; durable outcome/evidence state is mutable guard-row state protected by CAS. Additional operation-specific distinctions belong in that typed operation/target/request-digest contract; they must not create a competing root identity.
...
- For each complete expected concrete participant-projection set linked to an enclosing root `EffectId`, every expected `(effectId, rootEffectId, typedOperation, tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, targetAggregateType, targetAggregateId)` projection must eventually have exactly one terminal ledger state (`APPLIED` or `ABANDONED`) when the evidence policy permits terminalization. `effectId` is the mutation's persisted root-or-child `EffectId`; `rootEffectId` remains enclosing lineage and reconciliation context. `typedOperation` is part of the expected set alongside the mutation ID and exact target aggregate; `effectKey` remains descriptor metadata for correlation and lookup. Reconciliation rejects missing, extra, partial, or conflicting projections; any inconclusive row remains reconciliation-required and non-terminal, with old-epoch rows additionally following the [Inconclusive Old-Epoch Reconciliation Policy](#inconclusive-old-epoch-reconciliation-policy), including across resets.
...
- **What is the idempotency key?**
  - Which frozen typed command/action contract or `ResolvedEffectPlan` supplies semantic order for the command's logical root effects, and how does it reject ambiguous/duplicate order before admission, persist the ordered manifest/digest, and allocate an opaque binding unique on `(tenantId, gameInstanceId, commandId, planOrdinal)` only after the durable command row freezes/binds its request fingerprint and resolved runtime scope? Reuse must validate the same command row identity, frozen binding, ordinal, and manifest digest; any mismatch fails closed. A zero-effect command must allocate neither. Source ordering remains separate, and non-command-root work retains its owner/source-specific identity.
