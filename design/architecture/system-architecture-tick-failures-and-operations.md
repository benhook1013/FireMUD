...
- retains the original concrete ledger/guard projection `(tenantId, gameInstanceId, playableStateScope, regionId, regionEpoch, tickId, effectId, rootEffectId, typedOperation, requestDigest, effectKey, targetAggregateType, targetAggregateId)`, its mutation `EffectId`, enclosing root lineage, and the maintenance/recovery fence that authorized the reset; `requestDigest` is immutable bound evidence, not a uniqueness input;
