...
- **Domain Idempotency with Tick Ordering and Fencing Context** – how root/participant guards enforce mutation idempotency while `(regionEpoch, tickId)` provides ordering and fencing context.
...
- Domain services use the root/participant guard boundary to ensure that replays do not double-apply logical effects; `(regionEpoch, tickId)` remains ordering and fencing context even when Redis state is partially lost.
...
- The ledger retains the root's `plan_ordinal`/`root_effect_id` binding as command-plan identity metadata. One logical operation has one root and participant guards remain beneath it; a shared root for a conserved multi-participant operation does not replace ADR 0053 atomicity, co-location, or reservation semantics. For command-root work, the root is allocated by Game Session's local durable plan/staging transaction from the opaque binding unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`; the durable command row's frozen request/runtime binding and the ordered plan manifest digest are validated on reuse, and a mismatch fails closed. Passive, inbound/remote, timer, retry, and already-generated effects retain their owner/source-specific root identities unless they materialize as an admitted command. Random batch, mutable command, effect-key, source-ordering, participant, trigger, and generated-child identities cannot substitute for that durable command row identity or allocation binding.
...
The canonical root `EffectId` described in the identifier glossary and `system-architecture-transactions.md` ties together one logical command-root effect in the command's deterministic plan, its Game Session participant status, and replay lineage. Game Session persists the stable `plan_ordinal` alongside the root in the local plan/staging transaction; a single-root plan uses `0`, a multi-root plan uses stable `0..n-1`, and a zero-effect plan allocates neither. Plan order comes from the frozen typed command or `ResolvedEffectPlan` semantic order supplied by the owning command/action contract; built-ins declare an equivalent stable order, and ambiguous or duplicate order fails before admission. The durable gameplay command row freezes/binds its request fingerprint and resolved runtime scope before allocation. The opaque allocation row is unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`, binds the ordered plan manifest digest, and validates that frozen command/runtime binding; reuse requires the same command row identity, frozen binding, ordinal, and manifest digest, otherwise failing closed. Ordinary player/built-in plans use their complete canonical admitted command/runtime identity. For Automation, the complete applicable Command-Handoff Identity is the admission/deduplication identity and must exact-map to one durable target `commandId`; after that mapping, allocation uses the target command row's same canonical key, not the handoff tuple itself. Trigger Identity and `scriptEventId` remain handler/correlation identities. Participant guard uniqueness is deterministically derived from exactly the root effect, typed operation, and target aggregate; the immutable request digest is bound to the guard identity, while durable outcome/evidence state is mutable guard-row state protected by CAS. The root identity, original participant ledger projection, and collision-safe guard identity are preserved for every ordinary retry or uncertain replay. `tickBatchId`, mutable command text, `effectKey`, `sourceOrdinal`/`enqueueSeq`, participant tuples, and generated child ordinals cannot substitute for the durable command row identity or allocation binding. Passive, inbound/remote, timer, retry, and already-generated effects retain their owner/source-specific root identities unless they materialize as an admitted command. Retry eligibility/attempt metadata (such as retry count, next eligible tick, and source-claim state) is separate and must not rewrite the original `regionEpoch`, `tickId`, `effectKey`, or root/typed-operation/target projection. A future linked-attempt or supersession model requires an explicit contract; only a **post-abandon re-drive**—after the original effect is conclusively terminal `ABANDONED` and its source claim is terminalized—may receive a fresh logical root under the applicable owner contract, a later tick coordinate, a new retry/source identity, and durable lineage to the original identities.
