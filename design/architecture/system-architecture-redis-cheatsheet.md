...
| `session:auth:revoked_after:account:<accountId>`, `session:auth:revoked_after:tenant:<tenantId>`, `session:auth:revoked_after:membership:<accountId>:<tenantId>` | **Coordination Redis** | Account Service | Revocation watermark keys used to invalidate older JWTs without scanning token records. They are Account-owned security state and must not be written by downstream services. |
