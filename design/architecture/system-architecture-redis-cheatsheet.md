...
The Account issuer, account, tenant, and membership generation projections and issued-token projections shown below are required target state; these equivalent Redis projections are not currently implemented. Reset recovery must rebuild and verify these projections from Account durable authority before post-reset smoke gates and public `resume` authorization, and protected traffic remains closed when the projections are missing, stale, malformed, or mismatched. The `sessionctx:*` row describes current implementation-local bootstrap/session-context keys, not gameplay authority.
...
| `session:auth:generation:issuer:<issuerId>`, `session:auth:generation:account:<accountId>`, `session:auth:generation:tenant:<tenantId>`, `session:auth:generation:membership:<accountId>:<tenantId>` | **Approved global/account exception: Account durable authority with required Coordination Redis projection** | Account Service | Account advances one positive monotonic `generation` per scope transactionally in its durable store and projects it with idempotent set-if-greater semantics. Account is the sole writer and downstream services are read-only. Consumers fail closed when the projection is unavailable, stale, malformed, or mismatched; reset recovery rebuilds it from Account authority and verifies every affected scope before reopening protected traffic. Region- and tenant-scoped resets preserve it; a cluster reset may discard it only after the Account repair/reset cutover and must rebuild and verify it before reopening. |
