...
| Account-owned issuer/account/tenant/membership authority-generation state | **Coordination Redis or Account durable authority store** | Account Service | Monotonic authority generations used to invalidate earlier JWT authority without scanning token records or comparing timestamps. Downstream services must not write this state. |
