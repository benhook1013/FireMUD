...
| `gateway:connect-token:jti:<jti>` and the shared replay-readiness generation | **Coordination Redis** | Spring Cloud Gateway | Security-critical, non-evicting single-use connect-token state. Consumption requires the configured durability acknowledgement. Any reset, failover, or continuity uncertainty keeps new gameplay handshakes closed for the maximum token lifetime plus skew before a new readiness generation may admit tokens. |
