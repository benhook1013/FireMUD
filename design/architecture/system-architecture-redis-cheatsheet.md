...
| `session:auth:token:<tokenHash>` | **Coordination Redis** | Account Service | One versioned server-side issued-token record per revocable Browser, player-bootstrap, or private Service JWT as described in [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). Ownership is split from `session:game:*`; reset invalidation forces reauthentication and token re-issuance. |
| `session:auth:generation:issuer:<issuerId>`, `session:auth:generation:account:<accountId>`, `session:auth:generation:tenant:<tenantId>`, `session:auth:generation:membership:<accountId>:<tenantId>` | **Coordination Redis** | Account Service | Current positive monotonic auth-generation projections used to invalidate older JWT authority without clocks or token scans. They are Account-owned security state, project set-if-greater, and must not be written by downstream services. |
