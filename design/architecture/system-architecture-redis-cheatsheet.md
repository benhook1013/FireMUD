...
| Account-owned issuer/account/tenant/membership authority-generation state (target projection keys: `account:authority-generation:issuer`, `account:authority-generation:account:<accountId>`, `account:authority-generation:tenant:<tenantId>`, `account:authority-generation:membership:<accountId>:<tenantId>`) | **Account durable authority store; optional Coordination Redis projection** | Account Service | **Target only; no equivalent projection is currently implemented.** Account advances one positive monotonic `generation` per scope transactionally in its durable store. If projected, each key contains the current generation for its named scope; Account is the sole writer, downstream services are read-only, and reset recovery rebuilds the projection from Account authority. Region- and tenant-scoped resets preserve it; a cluster reset may discard it only after the Account repair/reset cutover and must rebuild and verify it before reopening protected traffic. |
