...
| `session:auth:token:<tokenHash>` | **Coordination Redis** | Account Service | One versioned server-side issued-token record per revocable `control-ui`, player-bootstrap, or receiver-specific private player-delegation JWT as described in [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md). Ownership is split from `session:game:*`. Region- and tenant-scoped resets preserve these records; a cluster-scoped reset drops them and forces reauthentication and token re-issuance. |
| Account-owned issuer/account/tenant/membership authority-generation state | **Account durable authority store; optional Coordination Redis projection** | Account Service | Account advances monotonic generations transactionally in its durable store. Redis may hold a fail-closed read projection only; downstream services never write it, and reset recovery rebuilds it from Account authority. |
