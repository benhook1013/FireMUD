...
| `session:auth:revoked_after:account:<accountId>`, `session:auth:revoked_after:tenant:<tenantId>`, `session:auth:revoked_after:membership:<accountId>:<tenantId>` | **Coordination Redis** | Account Service | Revocation watermark keys used to invalidate older JWTs without scanning allowlist entries. They are Account-owned security state and must not be written by downstream services. |
| `remote:{tenantInstanceTag}:<entityId>` | **Coordination Redis** | Game Session Service / Automation & Scripting | Best-effort, TTL-bounded hint marker for remote participation scoped to `<tenantId, gameInstanceId>` (typically “this entity has remote follow-ups due”). Default `remote_hint_ttl_ms = 60_000`. Region-scoped resets leave instance-scoped hints intact; tenant/cluster resets resolve every game instance from the durable scope inventory and enumerate `remote:{tenantInstanceTag}:*` per instance before removal. No tenant-only remote key family exists. |
