...
| `session:game:index:character:<tenantId>:<gameInstanceId>:<characterId>`, `session:game:index:account-tenant:<accountId>:<tenantId>`, `session:game:index:tenant:<tenantId>` | **Coordination Redis** | Game Session Service | Secondary gameplay-session indexes over the canonical `session:game:*` records. Reset and rebuild behavior follows the owning session keys; these indexes are not separate authority. |
