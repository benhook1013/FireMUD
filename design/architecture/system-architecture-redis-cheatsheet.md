...
The Account issuer, account, tenant, and membership generation projections and issued-token projections shown below are required target state; these equivalent Redis projections are not currently implemented. Reset recovery must rebuild and verify these projections from Account durable authority before smoke or `resume`, and protected traffic remains closed when the projections are missing, stale, malformed, or mismatched. The `sessionctx:*` row describes current implementation-local bootstrap/session-context keys, not gameplay authority.
