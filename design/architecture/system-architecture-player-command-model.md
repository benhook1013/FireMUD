...
Game Design persists each command definition as a typed `COMMAND_DEFINITION` revision payload with one stable logical command identity. The payload contains command aliases, stage, category, tags, history policy, capability requirements, help metadata, and its typed execution-effect declarations. Platform declarations set `historyRecordable` explicitly; an omitted authored/plugin policy normalizes to non-recordable, and publication materializes that explicit effective value in the immutable artifact. Revisions are version-scoped DML inputs; publish validates them, freezes the command-definition artifact and digest for the version, and rejects ambiguous aliases, invalid history policy, invalid effect payloads, or unsupported effect schemas.
