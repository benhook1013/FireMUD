...
`EVALUATED_COMMITTED` is the canonical target trigger state reached by the atomic descriptor commit. One transaction persists the immutable evaluated descriptor rows, parent outbox evidence, terminal evaluation outcome, and audit update, and compare-and-set transitions the pre-DSL trigger from `EXECUTING` to `EVALUATED_COMMITTED`. Recovery uses the committed descriptors and retained outcome as its only replay input and never re-enters the DSL. The trigger state itself contributes zero to `activeExecutionCount`; any unresolved child descriptors contribute through their descriptor status.
...
- The target lifecycle for the **pre-DSL trigger record** is `PENDING_EVALUATION` -> `EXECUTING` -> `EVALUATED_COMMITTED`, with explicit terminal cancellation/dead-letter transitions only where the cancellation and stale-recovery rules below permit them. The record owns the complete applicable Trigger Identity, event payload, pinned script/configuration and artifact references, the immutable handler-scoped input manifest, admission epoch, evaluation lease, fencing generation, and, for command-capable gameplay/runtime evaluations only, one opaque `automationDispatchId` reservation for its eventual dispatch group, plus the durable descriptor-commit marker/status. It contains no evaluated gameplay command descriptors. The manifest is the deterministic read input; an owner-specific `read_snapshot_token`, when applicable, is one recorded manifest entry rather than universal authority.
...
- A minimal identity checksum (for example `gameInstanceId`, exact `scriptPatchVersion` plus `scriptPinEpoch`, optional `pluginVersionId`) so rebuild/drain logic can detect version-fence mismatches early without reading full payloads.
...
| `EVALUATING` (current live) | Current pre-DSL trigger state claimed before DSL evaluation. | The live projection counts it as active, including unresolved stale rows; current recovery does not infer reclaim, terminalization, or safe resumption from lease age. |
...
| `DEAD_LETTERED` | Terminal non-progressing trigger/descriptor. | Terminal for drain and rebuild; no automatic DSL replay. Controlled recovery follows the [operator replay contract](#operator-replay-of-dead_lettered-work), preserves the original stage identity, and does not create a new dispatch identity. |
...
**Target-state `GetAutomationDrainStatus` mapping:** for the current scope and `admissionEpoch`, `activeExecutionCount` is exactly the sum of `EXECUTING` pre-DSL triggers and `HANDOFF_IN_FLIGHT` evaluated descriptors; an `EVALUATED_COMMITTED` trigger itself contributes zero. `pendingCancelableWorkItemCount` is exactly the sum of `PENDING_EVALUATION` pre-DSL triggers and `PENDING`/`INDEXED` evaluated descriptors, including descriptors committed by an `EVALUATED_COMMITTED` trigger. Terminal states, another scope or epoch, and derived queue pointers do not contribute. These counts expose asynchronous cleanup progress only; they do not gate Automation resumption or ordinary gameplay ticks, and no cleanup path enters or re-enters the DSL.

`PENDING` and `INDEXED` are target descriptor states; the current implementation does not persist them as separate statuses or persist `EVALUATED_COMMITTED`. **Current projection mapping:** the live projection counts `EVALUATING`, including unresolved stale rows, and `HANDOFF_IN_FLIGHT` as active, and counts every handoff-capable `PENDING_EVALUATION` as pending. Until the implementation converges, drain and rebuild projections must report that gap rather than presenting `PENDING_EVALUATION` or `EVALUATING` as evaluated command work; unresolved counts remain diagnostic cleanup backlog and do not gate ordinary ticks or target Automation resumption.

### Operator Replay of DEAD_LETTERED Work

Recovery selection, eligibility, stage mapping, idempotency, and the atomic recovery claim/attempt owner are defined by [Control Plane Operations](./system-architecture-scripting-control-plane-operations.md#replaydeadletteredworkitems); that owner persists and revalidates the expected exact fence/binding and `controlPlaneRequestId`, including same-attempt claim reclaim. This runtime section records only local consequences: command-capable gameplay/runtime evaluation-stage recovery may invoke the DSL evaluator again using the original work-item and `scriptEventId`, original persisted payload, frozen manifest/graph evidence, exact admitted immutable graph and artifact references, and recorded digests, while remaining outside the normal admission path and reusing the pre-DSL-reserved `automationDispatchId` when no descriptor was committed; it must never refresh to the latest/default payload, manifest, graph, or artifact. Post-evaluation recovery reads the immutable evaluated output and complete child-dispatch ledger keyed by the full Command-Handoff Identity, preserving original child identities, ordinals, downstream acknowledgements/outcomes, and per-child recovery state, and never invokes the DSL. Evaluated-output and command digests are integrity evidence only, not substitutes for retained output or identity; `outboxWorkItemId`, the parent Trigger Identity, and `scriptEventId` remain correlation-only. Neither stage reopens the original audit row.

**Target-state only:** The control-plane recovery mutation accepts bounded explicit `workItemIds[]` only; descriptor references and filter selectors remain deferred until preview plus stable per-row proof. A `DEAD_LETTERED` evaluation-stage trigger with no committed descriptor may retry when its original work-item, `scriptEventId`, frozen manifest/graph evidence, and exact admitted immutable graph are present; a missing or contradictory stage record remains ineligible. The current mutation selects parent `workItemIds` and requeues eligible `script_work_items` rows, so it does not yet prove independent descriptor recovery. Command-capable gameplay/runtime evaluation-stage recovery may invoke the DSL evaluator again under the original identity and evidence, but remains outside the normal admission path, must reuse the pre-DSL-reserved `automationDispatchId`, and must converge on the original work item and child identities. Post-evaluation recovery must resume only the stored output/child ledger and never invoke the DSL. Recovery must never move a dead-lettered item into a fresh admission path, create a new dispatch identity, regenerate output after the evaluation boundary, or append new history to the original audit row. Before recovery, the service must revalidate the current exact `(scriptPatchVersion, scriptPinEpoch)` tuple and, for plugin work, the immutable `(pluginId, pluginVersionId, bindingId)` tuple against the currently active binding for the same scoped `<tenantId, gameInstanceId, pluginId>`. If the authoritative tuple or binding read is temporarily unavailable, return bounded `authority_unavailable`, leave the already-`DEAD_LETTERED` item unchanged with no recovery record, and permit a later retry under the original identity. A proven tuple/binding mismatch or revocation remains terminal/ineligible, leaves the item `DEAD_LETTERED`, and produces no recovery record.

Target-state recovery is idempotent by stable `controlPlaneRequestId` for each original work item. Retrying the same operator request returns its stored per-work-item recovery result unchanged and creates no second recovery attempt or audit history; only a new request with a different `controlPlaneRequestId` that encounters a prior successful recovery returns `outcome=already_recovered`. Missing or contradictory stage evidence remains dead-lettered.
...
Timer admission, due-point identity, catch-up, reload, and failure outcomes follow the [normative timer semantics](./system-architecture-scripting-normative-contract-tables.md#table-3-timer-semantics-matrix). Before `EVALUATED_COMMITTED`, an admitted timer evaluation may retry only under the same full Trigger Identity, must reuse the pre-DSL-reserved `automationDispatchId`, and must converge on the same work-item and child identities with durable uniqueness enforced on the complete Command-Handoff Identity before any child emission or commit. After that commit boundary, recovery replays durable descriptors and never re-enters the DSL; independently idempotent downstream operations may still retry. Recurring/advisory firings remain bounded scheduling signals rather than guaranteed cadence ledgers, while correctness-bearing one-shot timers converge to one execution or an explicit terminal outcome.
