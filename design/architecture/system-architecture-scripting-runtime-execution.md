...
- Pre-handler rollback pause is an ingress-audit backpressure outcome only. Post-resolution handler backpressure such as `skipped_reloading` and `rollback_paused` is not treated as final for low-rate external events.
