...
Recovery selection, eligibility, stage mapping, and idempotency are owned by [Control Plane Operations](./system-architecture-scripting-control-plane-operations.md#replaydeadletteredworkitems). This runtime section records only local consequences: command-capable gameplay/runtime evaluation-stage recovery may invoke the DSL evaluator again using the original work-item and `scriptEventId`, frozen manifest/graph evidence, and exact admitted immutable graph, while remaining outside the normal admission path and reusing the pre-DSL-reserved `automationDispatchId` when no descriptor was committed; post-evaluation recovery resumes the stored child-dispatch ledger with its original child identities, ordinals, payload digests, and acknowledgements and never invokes the DSL. Neither stage reopens the original audit row.
