...
Target-state recovery is idempotent by stable `controlPlaneRequestId` for each original work item. Retrying the same operator request returns the existing recovery result with `outcome=already_recovered` and creates no second recovery attempt or audit history; missing or contradictory stage evidence remains dead-lettered.
