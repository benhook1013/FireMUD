...
The current evaluator accepts exactly one emitted command per work item. A result with more than one command is rejected before persistence or handoff with `finalStage=DSL_EVAL`, `finalOutcome=sandbox_error`, and bounded `finalReason=command_count_exceeded`. This is a current implementation limitation only; the target multi-command handoff contract remains canonical in [Scripting & Automation: Cross-Service Contracts](./system-architecture-scripting-contracts.md#2-script-work-item-vs-tick-command-boundary), including deterministic `(automationDispatchId, commandOrdinal)` identity.
...
- `HANDOFF_IN_FLIGHT` rows are active evaluated descriptors during queue rebuild and recovery. Recovery must load the committed descriptor and retry or confirm handoff by `(automationDispatchId, commandOrdinal)` without re-entering the DSL; if the durable handoff-lease evidence is insufficient, retain the row active/unresolved rather than rebuilding it as fresh DSL work.
- `CancelPendingWorkItemsForPatch` and `CancelPendingWorkItemsForPluginVersion` must make durable transitions in both layers: pre-DSL triggers follow the fencing-aware cancellation contract above, while evaluated descriptors in `PENDING` or `INDEXED` transition to `workItemStatus=CANCELED` with durable `cancelReason`. Cancellation remains authoritative if Redis is reset and is reflected in the corresponding stage-aware `script_event_audit` outcome.
...
| `HANDOFF_IN_FLIGHT` | Evaluated descriptor currently being handed to Game Session. | Count as active; rebuild/recovery retries or confirms the committed descriptor by `(automationDispatchId, commandOrdinal)`, never by re-entering the DSL. If handoff-lease evidence is insufficient, keep it active/unresolved until a durable recovery decision exists. |
...
- **Target state only:** A stale `ONLOAD_RUNNING` execution is fenced by its readiness publication/generation and compare-and-set transitioned by Automation's recovery owner to a terminal audited `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and `finalReason=stale_execution_fenced`. It blocks `READY`; the same canonical readiness identity is never re-entered. A replacement readiness identity is permitted only after the stale execution's fencing and terminal audit are durable, and must come from a distinct accepted publication identity rather than an implicit retry of the old identity.
