...
The accepted claim boundary is the atomic PostgreSQL transition from `PENDING_EVALUATION` to `EVALUATING`; the existing Game Session lease rules govern region-owned tick execution, not Automation work-item claims. No accepted Automation owner contract defines a stale-`EVALUATING` timeout, reclaim transition, or recovery owner. This document therefore makes no automatic stale-`EVALUATING` recovery claim: such rows are an unresolved current limitation to report, not to reclassify or replay by inference.
