...
Runtime metering is incremental and happens before constructing each generated command or data-dependent collection. The evaluator reserves bounded accounting state and checks, before construction, the cumulative command count, per-entity count, serialized bytes, payload/collection bounds, and every data-dependent cap declared by the pinned metadata. A run that would exceed any cap stops with the stage-correct `DSL_EVAL` non-success outcome and bounded reason; it must not construct an unbounded intermediate result and discover the violation only at serialization or handoff.
