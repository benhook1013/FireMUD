...
Historical Game Design migration status is also unresolved: `V5__update_version_table.sql` dropped the legacy `version.revision_ids` relationship, and `V17__revision_version_scope.sql` later assigned each tenant's revisions to its latest version. The current lineage therefore cannot prove immutable revision/version history, and no recovery is implied for already-dropped arrays; retained-data disposition or a verified no-data boundary and future migration/proof remediation remain required.
