...
- **Eligible non-expanding rollback/recovery** is limited to restarting, recovering, or rolling back already-entitled capacity without increasing capacity or quota consumption. Its authoritative entitlement source is Account's `GetTenantEntitlementsForRuntime(tenantId, requestId)` result and the Account-owned durable commitment/evidence that it returns or can reconcile; local caches, Game Session state, and copied service projections are not authority. Account evidence must be available at recovery time and identify the same target, exact `tenantAuthorityGeneration`, `entitlementVersion`, `tenantBillingSequence`, source transaction/outbox checkpoint, and freshness/evaluation data needed to prove that no newer billing or entitlement decision is being bypassed. It may use an otherwise authoritative positive last-known-good snapshot only when `(localNow - evaluatedAt + maxClockSkew) < 300000` ms, `evaluatedAt` is not future-dated beyond `maxClockSkew`, current Account evidence for the same target has exactly the snapshot's `tenantAuthorityGeneration` and `entitlementVersion`, no hard denial or revocation is observed, the billing sequence has not advanced beyond the snapshot, and no sequence gap exists. If Account evidence or the Account-owned commitment is unavailable, timed out, malformed, stale, regressed, or cannot prove the exact sequence relationship, recovery fails closed. A reconnect that creates a new binding or changes realm target is strict new admission, not non-expanding recovery. This exception only supplies eligible input to the Account-owned commitment; it does not authorize a local activation or routing write.
...
Immediately before the final activation or admission-pointer commit, Game Session must call the Account-owned `CommitTenantCapacityAdmission` contract with `tenantId`, stable `requestId`/`admissionId` admission identity, the expected complete applicable `authorityTuple` including the exact tenant generation and applicable `tenantBillingCutoff`, separate expected `entitlementVersion`, exact expected billing `{outboxStreamKey, outboxSequence, tenantBillingSequence}` evidence, the operation's bounded `capacityDelta`, and a versioned `mutationDigest` over that complete payload. The separate billing evidence must equal the applicable cutoff entry and its covered checkpoint; no flat `tenantBillingSequence` tuple member is invented. Capacity-creating activation uses its positive bounded delta and may reserve/increment usage only after the final Account fence succeeds. Non-expanding recovery must call the same contract with `capacityDelta=0`; it performs no usage increment or quota consumption, but still creates or replays the exact durable commitment/evidence for the unchanged entitlement state. An exact identity-and-payload retry replays; any changed tuple, entitlement, billing evidence, delta, or digest is an idempotency conflict and cannot mutate usage. Game Session must bind that Account fence and evidence identity, exact `entitlementVersion`, and committed billing checkpoint into its own fenced activation or routing CAS; a local re-read, conditional read, or recheck is not a strict capacity guarantee. A stale, replaced, or concurrently invalidated Account commitment cannot authorize the state change. This Account commitment and final CAS are in addition to version-state, release-attestation, compatibility, lifecycle-fence, and routing-pointer proof; last-known-good never substitutes for those proofs. Same-binding player resumption additionally requires Account to validate current lifecycle, security, membership, and revocation authority and commit the ADR 0030 resume lease. If any required evidence is unavailable or unsafe, the realm remains closed or the previous pointer remains the sole admissible target. The target request/replay contract is now explicit, but the proto RPC, caller integration, durable Account authority/usage ledger, and reservation lifecycle remain unimplemented.
