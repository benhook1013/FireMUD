...
- Rolling back a script-only patch affects only automation behavior for the current `version_id`; it does not change templates or abilities. Game Session explicitly repins the exact target `(scriptPatchVersion, scriptPinEpoch)` and advances `scriptPinEpoch`; routine epoch-fenced rollback keeps ordinary gameplay ticks running while Automation admission is fenced. Rolling back a plugin-only version instead uses the Automation plugin control-plane's exact `(pluginId, pluginVersionId, bindingId)` and runtime-state fence for the affected instance; it does not advance `scriptPinEpoch` unless the script pin changes in the same operation.
