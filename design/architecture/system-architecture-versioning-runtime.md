...
Before any operation that changes whether a tenant is actively serving gameplay through one of its player-addressable realms, Game Session must evaluate one operation-specific entitlement gate rather than a gameplay-availability flag alone. The operation is classified before the read:

- **Capacity-creating activation/cutover** includes starting or creating an additional instance, scale-out, paid-feature activation, creating a playtest fork, or a replacement cutover that prepares new capacity. These operations require a fresh Account snapshot from `GetTenantEntitlementsForRuntime(tenantId, requestId)` with `evaluatedAt <= localNow + maxClockSkew` and satisfying `(localNow - evaluatedAt + maxClockSkew) < 15000` ms, an operation-specific flag that permits the requested lifecycle change, and enough plan-derived quota for the requested instance count and configuration. General gameplay availability during `grace` is not sufficient.
- **Eligible non-expanding rollback/recovery** is limited to restarting, recovering, or rolling back already-entitled capacity without increasing capacity or quota consumption. Its authoritative entitlement source is Account's complete authenticated `GetTenantEntitlementsForRuntime(tenantId, requestId)` evidence bundle and the Account-owned durable commitment/evidence that it returns or can reconcile; local caches, Game Session state, and copied service projections are not authority. Account evidence must be available at recovery time and identify the same target, with exact equality for the bundle version, snapshot identity, evaluation identity, authority tuple, and billing cutoff/checkpoints, plus the entitlement fields and freshness data needed to prove that no newer billing or entitlement decision is being bypassed. It may use an otherwise authoritative positive last-known-good bundle only when `(localNow - evaluatedAt + maxClockSkew) < 300000` ms, `evaluatedAt` is not future-dated beyond `maxClockSkew`, current Account evidence for the same target exactly matches the bundle's version, snapshot identity, evaluation identity, authority, and entitlement identities, and the bundle is bound to a prior accepted capacity-creating `CommitTenantCapacityAdmission` identity and `mutationDigest` for the same target. The recovery operation's separate `capacityDelta=0` commitment proves the current non-expanding recovery attempt; it is not the prior capacity-creation proof. No hard denial or revocation may be observed, the billing sequence must not have advanced beyond the snapshot, and no sequence gap may exist. If Account evidence or the Account-owned commitment is unavailable, timed out, malformed, stale, regressed, or cannot prove these exact identities, digest binding, or sequence relationship, recovery fails closed. A reconnect that creates a new binding or changes realm target is strict new admission, not non-expanding recovery. This exception only supplies eligible input to the Account-owned commitment; it does not authorize a local activation or routing write.
- **Non-serving planned closure/termination** includes an intentional realm closure without replacement, completion of a replacement source drain, and stopping or terminating an instance after admission has been closed. This class does not serve gameplay or create capacity, so it must not be blocked by `gameplayAvailable`, serving operation flags, serving quotas, or the presence of a positive serving-entitlement snapshot. It remains Account-authorized: the shutdown decision must carry current Account-owned tenant lifecycle, authority, and revocation evidence or an Account-redeemed operator authorization reference, and local caches or Game Session state are never sufficient. The class is bound to the ordered shutdown contract above: Game Session closes admission and marks the runtime non-admissible, World Management owns the lifecycle fence and `InstanceTermination`, and completion is reported only after terminal cleanup. It cannot reopen a realm, admit gameplay, create capacity, or bypass the owner shutdown workflow.

For capacity-creating activation/cutover, the complete entitlement predicate requires a finite, contract-valid snapshot containing `subscriptionStatus`, `gameplayAvailable`, the applicable operation flags such as `allowPublicJoin`, `allowNewGameplayBindings`, and `allowNewInstanceStarts`, the applicable quotas, `evaluatedAt`, `tenantAuthorityGeneration`, `entitlementVersion`, and `tenantBillingSequence`; current tenant lifecycle/billing state compatible with the operation; the operation-specific activation or scale permission; requested capacity and quota within plan limits; and a sequence-safe snapshot with no gap, regression, or newer billing evidence. `gameplayAvailable` means already-authorized gameplay may continue under the snapshot; it is necessary but never sufficient for a new join, binding, or instance-start commitment, which must also satisfy its corresponding operation-specific flag.

Eligible non-expanding rollback/recovery instead requires the applicable authoritative current or permitted last-known-good snapshot, lifecycle and billing compatibility with recovery, the freshness and sequence-safety evidence defined above, and durable proof that the exact target capacity is already entitled and that `capacityDelta=0` does not expand usage. It does not require capacity-creating flags such as `allowNewInstanceStarts`, scale permission, or requested-capacity/quota validation; requiring those creation-only gates would incorrectly turn recovery of already-entitled capacity into a new-capacity operation. It still fails closed when the authoritative snapshot, commitment, existing-capacity proof, lifecycle compatibility, freshness, or sequence relationship is missing, malformed, stale, future-dated beyond skew, unavailable, regressed, or ambiguous. A fresh or eligible last-known-good snapshot is only input to the Account-owned strict commitment and cannot be replaced by a caller-local read or recheck. The non-serving planned closure/termination class uses its separate Account-authorized shutdown predicate instead of either serving-entitlement predicate.

Immediately before the final activation or admission-pointer commit, Game Session must call the Account-owned `CommitTenantCapacityAdmission` contract with stable outer `(tenantId, requestId, admissionId)`, a fresh per-attempt `admissionAttemptId`, the exact complete Account-authenticated evidence bundle returned by evaluation, the operation's bounded `capacityDelta`, and a versioned `mutationDigest` over that attempt identity and complete payload. This digest uses exactly [ADR 0047's `mutationDigest/v1` byte grammar](./decisions/adr-0047-logging-admin-as-external-operator-write-ingress.md#canonical-external-ingress): SHA-256 over the published capacity-admission action-family schema pair and its fixed-order typed fields, with explicit absent/null encoding, canonical decimal and Unicode rules, bounded raw and normalized inputs, and shared cross-language golden vectors. The planned wire schema gives `capacityDelta` explicit presence; `CommitTenantCapacityAdmission` must reject an absent `capacityDelta` before commitment or usage mutation. Capacity-creating activation requires an explicitly present positive bounded delta, while non-expanding recovery requires an explicitly present zero delta; a digest-valid request with no bound capacity change is still invalid. `present(value=0)` is distinct from an absent field and must never be defaulted into the other. Both the present-zero and absent encodings require cross-language golden vectors. The capacity schema maps the complete outer `(tenantId, requestId, admissionId)` identity and per-attempt `admissionAttemptId`, Account evidence bundle, and `capacityDelta` into the grammar's scope, target, expected-version, mutation, and audit-reason positions. The bundle includes its version, exact snapshot identity, exact evaluation identity, all entitlement fields, the complete applicable `authorityTuple`, exact applicable `tenantBillingCutoff`, and complete `outboxCheckpoints[]`; the cutoff stream and sequence must equal its covered checkpoint, and no flat `tenantBillingSequence` tuple member is invented. Each accepted commitment binds that exact evidence identity and payload to the stable outer request tuple, per-attempt identity, and digest; a cached bundle from another snapshot or evaluation, or a recomputed digest over changed evidence, is not equivalent. Capacity-creating activation uses its positive bounded delta and may reserve/increment usage only after the final Account fence succeeds. Non-expanding recovery must call the same contract with `capacityDelta=0`; planned non-serving closure/termination never calls this capacity commitment and instead follows its separate Account-authorized ordered shutdown predicate. An exact retry of the same attempt replays; any changed bundle field, cutoff/checkpoint, delta, wire presence, attempt identity, or digest is an idempotency conflict before usage mutation. Game Session binds the Account fence and evidence identity into its own fenced activation or routing CAS; a local re-read, conditional read, or recheck is not a strict capacity guarantee. A stale, replaced, or concurrently invalidated Account commitment cannot authorize the state change. This Account commitment and final CAS are in addition to version-state, release-attestation, compatibility, lifecycle-fence, and routing-pointer proof; last-known-good never substitutes for those proofs. Same-binding player resumption additionally requires Account to validate current lifecycle, security, membership, and revocation authority and commit the ADR 0030 resume lease. If any required evidence is unavailable or unsafe, the realm remains closed or the previous pointer remains the sole admissible target.

When ADR 0048 permits a fresh-owner retry, it starts a new `admissionAttemptId` only from the original terminal `OWNER_EXECUTION/NOT_EXECUTED_AFTER_AUTHORIZATION` result and durable no-commit proof. `FENCE_REJECTED`, `ACCOUNT_AUTHORIZATION/AUTHORIZATION_PENDING`, `OWNER_EXECUTION/OWNER_EXECUTION_PENDING`, `COMMITTED`, `FAILED`, or any ambiguous result is not a rearm prerequisite; it remains read-only and non-replayable. Account performs the rearm as one atomic CAS over the existing outer request record and reservation: it compares the stable `(tenantId, requestId, admissionId)`, prior attempt identity and terminal evidence, current reservation state, current reservation fence, exact authority tuple, and prior digest; then it advances the reservation's monotonic fence, binds a fresh `admissionAttemptId`, fresh attempt payload and `mutationDigest`, and the current owner/fencing evidence in the same commit. The reservation identity, bounded capacity hold, and usage accounting remain unchanged; no second reservation, replacement hold, usage increment, or old-attempt replay is created. A stale recovery worker or owner cannot perform this CAS. The stable outer tuple remains the logical request and idempotency key, while the new attempt identity and digest are included in the Account commitment.

For a positive `capacityDelta`, the durable Account result includes the exact reservation identity, expiry, state, and payload digest. Game Session must report or reconcile the final activation/routing CAS outcome against that reservation before it can become committed usage. The final Game Session activation/routing CAS must compare the reservation identity and current reservation fence and reject an expired, released, or fenced reservation, including any stale reservation claim fence, before changing activation or routing state. A timeout, response loss, takeover, or ambiguous CAS result triggers bounded read-only reconciliation using the original outer `requestId`/`admissionId`, attempt identity, exact payload, and prior reservation fence; it never creates a replacement reservation. A bounded Account-owned reservation-recovery worker claims the reservation with a durable CAS and a bounded lease, reads the Game Session activation/routing result and effect evidence, and is the only component allowed to resolve the ambiguous reservation. If positive runtime-commit evidence exists, it finalizes the existing reservation and usage exactly once. If durable no-commit evidence exists and the terminal `OWNER_EXECUTION/NOT_EXECUTED_AFTER_AUTHORIZATION` prerequisite is present, the worker must atomically advance the reservation's monotonic fence, bind the fresh attempt identity and digest from the rearm CAS above, and preserve the existing reservation identity and capacity hold for the current owner; it must not release the reservation before that rearm commit. If no valid terminal rearm prerequisite exists, the worker must durably advance the reservation's monotonic fence before releasing it; the release CAS and `RELEASED` state must be rejected for an expired or superseded fence. If evidence remains ambiguous, recovery leaves admission closed and the reservation fenced, records the bounded retry/deadline, and does not release, finalize, rearm, or start another attempt. Account commitments therefore cannot remain reserved indefinitely or diverge silently from the runtime pointer. Identical retries of the same attempt replay the stored reservation, finalization, release, or rearm result; a changed authority tuple, `capacityDelta`, mutation digest, attempt identity, target, or other bound payload field is an `IDEMPOTENCY_CONFLICT` before usage or reservation mutation. A stale owner, stale reservation fence, or superseded recovery worker returns `FENCE_REJECTED` only with durable no-accept/no-commit proof and cannot rearm. A `capacityDelta=0` commitment uses the same outer identity, attempt, expiry, reconciliation, rearm, and replay rules but reserves or releases no usage.

For the target recovery contract, a stale owner or fence returns canonical `FENCE_REJECTED` with reason `STALE_OWNER` or `STALE_FENCE`, carrying the original outer request identity, attempt identity, exact digest, observed owner/fence, and durable proof that no Account commitment or Game Session mutation accepted the request. `FENCE_REJECTED` is non-rearmable and permits only read-only reconciliation. If Account accepted the capacity commitment before the Game Session owner or fence became stale, the operation remains `OWNER_EXECUTION/OWNER_EXECUTION_PENDING` and indeterminate under its original outer request tuple, attempt identity, exact payload, reservation identity, and digest; ownership loss cannot rewrite that accepted commitment as `FENCE_REJECTED` or allocate a replacement reservation. Read-only reconciliation must finalize the original commitment from positive runtime-commit proof or release it only from durable no-commit proof after the reservation fence advance. Retry against the current owner with the same external request tuple is allowed only after reconciliation durably transitions the original operation to terminal `OWNER_EXECUTION/NOT_EXECUTED_AFTER_AUTHORIZATION` under ADR 0048 and the Account recovery worker atomically advances the existing reservation fence and binds a fresh owner attempt identity and digest. That retry creates a new Account commitment attempt bound to the fresh attempt identity and digest, preserves the existing reservation, and never replays the old terminal result or allocates a duplicate reservation. Exact retries of the same attempt replay the stored result; changed outer identity, authority tuple, reservation identity, payload, attempt identity, or digest returns `IDEMPOTENCY_CONFLICT`; a stale owner, stale reservation fence, or superseded worker returns `FENCE_REJECTED` with no-accept/no-commit proof and cannot rearm. Timeouts, missing or inconsistent evidence, response loss after acceptance, and all other outcomes are ambiguous and require read-only reconciliation of the original request before any replay. A `capacityDelta=0` retry still preserves the outer request tuple and existing reservation semantics and must use the fresh attempt identity and matching attempt digest.
...
- All player and control-plane flows must resolve the referenced catalog/policy revision together with the routing pointer, and must fail closed on a missing, stale, mismatched, or ambiguous pair:
  ...
  - `CHARS` uses that policy decision and its declared membership, conditional realm-grant, and runtime-entitlement checks before resolving caller-valid characters for the selected realm and routed instance; unlike `PLAY`, it is not the admission-pointer and gameplay-binding operation.
