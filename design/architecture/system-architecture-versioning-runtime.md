...
- **Eligible non-expanding rollback/recovery** is limited to restarting, recovering, or rolling back already-entitled capacity without increasing capacity or quota consumption. Its authoritative entitlement source is Account's complete authenticated `GetTenantEntitlementsForRuntime(tenantId, requestId)` evidence bundle and the Account-owned durable commitment/evidence that it returns or can reconcile; local caches, Game Session state, and copied service projections are not authority. Account evidence must be available at recovery time and identify the same target, exact bundle version and snapshot/evaluation identities, complete authority tuple and billing cutoff/checkpoints, entitlement fields, and freshness data needed to prove that no newer billing or entitlement decision is being bypassed. It may use an otherwise authoritative positive last-known-good bundle only when `(localNow - evaluatedAt + maxClockSkew) < 300000` ms, `evaluatedAt` is not future-dated beyond `maxClockSkew`, current Account evidence for the same target exactly matches the bundle's authority and entitlement identities, no hard denial or revocation is observed, the billing sequence has not advanced beyond the snapshot, and no sequence gap exists. If Account evidence or the Account-owned commitment is unavailable, timed out, malformed, stale, regressed, or cannot prove the exact sequence relationship, recovery fails closed. A reconnect that creates a new binding or changes realm target is strict new admission, not non-expanding recovery. This exception only supplies eligible input to the Account-owned commitment; it does not authorize a local activation or routing write.
