...
6. **Target-only bounded source drain:** in the same cutover commit, persist the source instance, a unique `sourceDrainId`, an initially inactive source command-fence identity, and absolute `sourceDrainDeadlineAt` resolved from `firemud.game-session.cutover-drain.duration-ms`. Keep the old instance closed to new/reconnected bindings while already connected source sessions finish before that deadline. At the deadline, the drain reconciler activates the persisted fence (immediately for zero duration), command handling rejects further source commands, World Management acquires the lifecycle fence and confirms that rejection, then Game Session unconditionally makes one bounded update-notice attempt and closes the remaining source sockets before the standard `InstanceTermination` workflow. The notice attempt cannot hold sockets open indefinitely.
...
- In that target contract, Game Session may complete a drain early after the source session index is empty. Deadline enforcement follows the durable reconciliation contract below; in particular, World Management's lifecycle fence and command-rejection confirmation precede the bounded notice and unconditional source-socket close.
...
Each external effect uses a durable replay identity derived from `{sourceDrainId, sourceGameInstanceId, effectType}`. The effect types are source command-fence activation and rejection, lifecycle fencing, bounded notice delivery, socket closure, and `InstanceTermination`. The persisted source command-fence identity is initially inactive; the drain reconciler activates enforcement at `sourceDrainDeadlineAt`, immediately for zero duration, and command handling rejects source commands after activation. Game Session persists each effect's pending, applied, and confirmed/read-back state. A crash after an external effect but before local acknowledgement therefore retries with the same identity and reconciles authoritative readback rather than emitting an untracked duplicate.
...
2. Activate the persisted source command-fence identity at the deadline (immediately for zero duration), confirm that command handling rejects further source commands, and request World Management to acquire the source lifecycle fence and transition the source to `TERMINATING`; lifecycle and command-rejection readback must complete before notice.
