...
For capacity-creating activation/cutover and eligible non-expanding rollback/recovery, the complete entitlement predicate requires a finite, contract-valid snapshot containing `subscriptionStatus`, `gameplayAvailable`, the applicable operation flags such as `allowPublicJoin`, `allowNewGameplayBindings`, and `allowNewInstanceStarts`, the applicable quotas, `evaluatedAt`, `tenantAuthorityGeneration`, `entitlementVersion`, and `tenantBillingSequence`; current tenant lifecycle/billing state compatible with the operation; the operation-specific activation/scale permission; requested capacity and quota within plan limits; and a sequence-safe snapshot with no gap, regression, or newer billing evidence. `gameplayAvailable` means already-authorized gameplay may continue under the snapshot; it is necessary but never sufficient for a new join, binding, or instance-start commitment, which must also satisfy its corresponding operation-specific flag. A stale, malformed, future-dated beyond-skew, unavailable, or unsafe snapshot fails closed with `AUTH_UNAVAILABLE`. A fresh or eligible last-known-good snapshot is only input to the Account-owned strict commitment and cannot be replaced by a caller-local read or recheck. The non-serving planned closure/termination class uses its separate Account-authorized shutdown predicate instead of this serving-entitlement predicate.
