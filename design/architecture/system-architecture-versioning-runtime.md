...
- **Capacity-creating activation/cutover** includes starting or creating an additional instance, scale-out, paid-feature activation, creating a playtest fork, or a replacement cutover that prepares new capacity. These operations require a fresh Account snapshot from `GetTenantEntitlementsForRuntime(tenantId, requestId)` satisfying `(localNow - evaluatedAt + maxClockSkew) < 15000` ms, an operation-specific flag that permits the requested lifecycle change, and enough plan-derived quota for the requested instance count and configuration. General gameplay availability during `grace` is not sufficient.
