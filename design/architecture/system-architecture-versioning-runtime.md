...
- **Eligible non-expanding rollback/recovery** is limited to restarting, recovering, or rolling back already-entitled capacity without increasing capacity or quota consumption. It may use an otherwise authoritative positive last-known-good snapshot only when `(localNow - evaluatedAt + maxClockSkew) < 300000` ms, `evaluatedAt` is not future-dated beyond `maxClockSkew`, current authoritative entitlement evidence for the same target has exactly the snapshot's `entitlementVersion`, no hard denial or revocation is observed, the billing sequence has not advanced beyond the snapshot, and no sequence gap exists. A reconnect that creates a new binding or changes realm target is strict new admission, not non-expanding recovery.

For both classes, the complete entitlement predicate requires a finite, contract-valid snapshot containing `subscriptionStatus`, `gameplayAvailable`, the applicable operation flags such as `allowPublicJoin`, `allowNewGameplayBindings`, and `allowNewInstanceStarts`, the applicable quotas, `evaluatedAt`, `entitlementVersion`, and `tenantBillingSequence`; current tenant lifecycle/billing state compatible with the operation; the operation-specific activation/scale permission; requested capacity and quota within plan limits; and a sequence-safe snapshot with no gap, regression, or newer billing evidence. `gameplayAvailable` means already-authorized gameplay may continue under the snapshot; it is necessary but never sufficient for a new join, binding, or instance-start commitment, which must also satisfy its corresponding operation-specific flag. A stale, malformed, future-dated beyond-skew, unavailable, or unsafe snapshot fails closed with the applicable entitlement-unavailable outcome. When the non-expanding exception does not apply, Game Session must obtain the required fresh result from `GetTenantEntitlementsForRuntime(tenantId, requestId)` rather than reuse local state.

Immediately before the final activation or admission-pointer commit, Game Session must re-evaluate the same class-specific freshness, lifecycle, operation-flag, quota, `entitlementVersion`, and billing-sequence predicate and atomically commit the exact entitlement evidence identity, exact `entitlementVersion`, and evaluated sequence with the activation or pointer CAS. The precommit `entitlementVersion` must equal the version accepted at eligibility; a stale, replaced, or concurrently invalidated result cannot authorize the state change. This final precommit predicate is in addition to version-state, release-attestation, compatibility, lifecycle-fence, and routing-pointer proof; last-known-good never substitutes for those proofs. Same-binding player resumption additionally requires Account to validate current lifecycle, security, membership, and revocation authority and commit the ADR 0030 resume lease. If any required evidence is unavailable or unsafe, the realm remains closed or the previous pointer remains the sole admissible target.
