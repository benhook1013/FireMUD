...
Every freshness gate resolves its applicable finite limit `L` and `maxClockSkew` from the canonical Account policy defined by [ADR 0028](./decisions/adr-0028-differentiated-entitlement-freshness.md); runtime code must not make a fixed `15000` or `300000` millisecond value authoritative. Before evaluating evidence, the effective policy must prove that `maxClockSkew` is finite, nonnegative, and strictly less than every applicable finite freshness or continuity limit. For an Account-issued `evaluatedAt`, the shared predicate is the conjunction `evaluatedAt <= localNow + maxClockSkew` and `localNow < evaluatedAt + L - maxClockSkew`. The age form `max(0, localNow - evaluatedAt) + maxClockSkew < L` is equivalent only after that future-timestamp guard; when the guard fails, `conservativeAge` is invalid and the evidence is rejected. Evidence within the skew is evaluated by both checks and never widens the window, evidence beyond the skew is invalid even if the age expression alone would pass, and the exact deadline is stale. The same predicate is used for current/fresh and permitted last-known-good/continuity evidence, with focused vectors for within-skew, beyond-skew, and exact-deadline timestamps.
