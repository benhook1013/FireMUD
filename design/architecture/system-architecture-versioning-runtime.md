...
Every freshness gate resolves its applicable finite limit `L` and `maxClockSkew` from the canonical Account policy defined by [ADR 0028](./decisions/adr-0028-differentiated-entitlement-freshness.md); runtime code must not make a fixed `15000` or `300000` millisecond value authoritative. Before evaluating evidence, the effective policy must prove that `maxClockSkew` is finite, nonnegative, and strictly less than every applicable finite freshness or continuity limit. For an Account-issued `evaluatedAt`, the shared predicate is `evaluatedAt <= localNow + maxClockSkew` and `localNow < evaluatedAt + L - maxClockSkew`, equivalently `max(0, localNow - evaluatedAt) + maxClockSkew < L`. Evidence future-dated beyond the skew is invalid, evidence within the skew never widens the window, and the exact deadline is stale. The same predicate is used for current/fresh and permitted last-known-good/continuity evidence.

The serving operation is classified before the read:

- **Capacity-creating activation/cutover** includes starting or creating an additional instance, scale-out, paid-feature activation, creating a playtest fork, or a replacement cutover that adds net capacity. These operations require a fresh Account snapshot from `GetTenantEntitlementsForRuntime(tenantId, requestId)` that satisfies the shared predicate with the effective recent-entitlement limit `L`, an operation-specific flag that permits the requested lifecycle change, and enough plan-derived quota for the requested instance count and configuration. General gameplay availability during `grace` is not sufficient.
- **Eligible non-expanding rollback/recovery** is limited to restarting, recovering, or rolling back already-entitled capacity, or an entitlement-preserving one-for-one replacement cutover, without increasing capacity or quota consumption. Its authoritative entitlement source is Account's complete authenticated `GetTenantEntitlementsForRuntime(tenantId, requestId)` evidence bundle and the Account-owned durable commitment/evidence that it returns or can reconcile; local caches, Game Session state, and copied service projections are not authority. Account evidence must be available at recovery time and identify the same target, with exact equality for the bundle version, snapshot identity, evaluation identity, authority tuple, and billing cutoff/checkpoints, plus the entitlement fields and freshness data needed to prove that no newer billing or entitlement decision is being bypassed. The current/fresh branch uses the effective recent-entitlement limit `L`; a permitted authoritative positive last-known-good branch uses the effective continuity limit `L` from ADR 0028 and the same shared predicate. Current Account evidence for the same target must exactly match the bundle's version, snapshot identity, evaluation identity, authority, and entitlement identities, and the bundle must be bound to a prior accepted capacity-creating `CommitTenantCapacityAdmission` identity and `mutationDigest` for the same target. The recovery or replacement operation's separate `capacityDelta=0` commitment proves the current non-expanding attempt; it is not the prior capacity-creation proof. No hard denial or revocation may be observed, the billing sequence must not have advanced beyond the snapshot, and no sequence gap may exist. If Account evidence or the Account-owned commitment is unavailable, timed out, malformed, stale, regressed, future-dated beyond skew, or cannot prove these exact identities, digest binding, or sequence relationship, recovery fails closed. A reconnect that creates a new binding or changes realm target is strict new admission, not non-expanding recovery. This exception only supplies eligible input to the Account-owned commitment; it does not authorize a local activation or routing write.
- **Non-serving planned closure/termination** includes an intentional realm closure without replacement, completion of a replacement source drain, and stopping or terminating an instance after admission has been closed. This class does not serve gameplay or create capacity and does not use either serving-entitlement predicate. Its separate Account-authorized `shutdownAllowed` predicate requires current Account-owned tenant lifecycle, authority, and revocation evidence for the exact target and shutdown request, or an Account-redeemed operator authorization reference; local caches and Game Session state are never sufficient. `shutdownAllowed` is not blocked by `gameplayAvailable`, serving operation flags, serving quotas, or positive serving-entitlement availability. The class is bound to the ordered shutdown contract above: Game Session closes admission and marks the runtime non-admissible, World Management owns the lifecycle fence and `InstanceTermination`, and completion is reported only after terminal cleanup. It cannot reopen a realm, admit gameplay, create capacity, or bypass the owner shutdown workflow.

For capacity-creating activation/cutover, the complete entitlement predicate requires a finite, contract-valid snapshot containing `subscriptionStatus`, `gameplayAvailable`, the applicable operation flags such as `allowPublicJoin`, `allowNewGameplayBindings`, and `allowNewInstanceStarts`, the applicable quotas, `evaluatedAt`, `tenantAuthorityGeneration`, `entitlementVersion`, and `tenantBillingSequence`; current tenant lifecycle/billing state compatible with the operation; the operation-specific activation or scale permission; requested capacity and quota within plan limits; and a sequence-safe snapshot with no gap, regression, or newer billing evidence. Its `evaluatedAt` must satisfy the effective recent-entitlement limit from ADR 0028 through the shared predicate. `gameplayAvailable` means already-authorized gameplay may continue under the snapshot; it is necessary but never sufficient for a new join, binding, or instance-start commitment, which must also satisfy its corresponding operation-specific flag.

Eligible non-expanding rollback/recovery instead requires the applicable authoritative current or permitted last-known-good snapshot, lifecycle and billing compatibility with recovery, the freshness and sequence-safety evidence defined above, and durable proof that the exact target capacity is already entitled and that `capacityDelta=0` does not expand usage. It does not require capacity-creating flags such as `allowNewInstanceStarts`, scale permission, or requested-capacity/quota validation; requiring those creation-only gates would incorrectly turn recovery of already-entitled capacity into a new-capacity operation. It still fails closed when the authoritative snapshot, commitment, existing-capacity proof, lifecycle compatibility, freshness, or sequence relationship is missing, malformed, stale, future-dated beyond skew, unavailable, regressed, or ambiguous. A fresh or eligible last-known-good snapshot is only input to the Account-owned strict commitment and cannot be replaced by a caller-local read or recheck. The non-serving planned closure/termination class uses only its separate Account-authorized shutdown predicate, not either serving-entitlement predicate.
