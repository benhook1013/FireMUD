...
- **Eligible non-expanding rollback/recovery** is limited to restarting, recovering, or rolling back already-entitled capacity without increasing capacity or quota consumption. It may use an otherwise authoritative positive last-known-good snapshot only when `(localNow - evaluatedAt + maxClockSkew) < 300000` ms, `evaluatedAt` is not future-dated beyond `maxClockSkew`, no hard denial or revocation is observed, the billing sequence has not advanced beyond the snapshot, and no sequence gap exists. A reconnect that creates a new binding or changes realm target is strict new admission, not non-expanding recovery.
