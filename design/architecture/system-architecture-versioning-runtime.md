...
The versioned publish and replacement-cutover substrate is partially implemented, including durable cutover preparation and execution records. The current admission-pointer writer performs a read-then-write version check and persists pointer, audit, and prepared-upgrade execution changes through separate repository calls; it does not yet prove the target single-transaction compare-and-set boundary described below. Current cutover implementation drift is also explicit: it clears active bindings rather than preserving them through the target bounded source-drain sequence of a persisted deadline, World Management lifecycle fence with confirmed command rejection, one bounded notice attempt after that fence, unconditional socket closure, and terminal `InstanceTermination` workflow. Notice failure never keeps source sockets open or delays termination. Current Game Session pin reads and the live `EnqueueAutomationCommandIfAbsent` handoff now carry the script patch, positive epoch, and owner request evidence; Game Session rejects incomplete or stale tuples at handoff, while current Automation instance-bound ingress, timer, and work-item admission paths also reject same-version work from an older epoch. Append-only request-idempotent rollout history, complete source/target command identity, and remote final-effect fencing remain implementation/proof gaps; Automation rollout rows/events remain synthetic, non-authoritative projections. The target sections that follow are normative design, not proof that the current runtime already satisfies those effects.
...
The versioning launch boundary also has an identity-representation drift that blocks launch-readiness claims. The accepted [ADR 0020](./decisions/adr-0020-scoped-domain-and-operational-identifiers.md) target remains opaque scoped UUID logical identities, including `tenantId` and `versionId`; it is not changed by the current implementation. Game Design currently persists `tenant_id` as `VARCHAR(36)` while its launch/published-release protobufs expose numeric `version_id`; Game Session persists `tenant_id` and `version_id` as `BIGINT`/`Long` and stringifies the numeric tenant when calling Game Design. See [Game Design V1 schema](../../services/game-design-service/src/main/resources/db/migration/V1__init.sql), [Game Session baseline schema](../../services/game-session-service/src/main/resources/db/migration/V1__baseline.sql), [Game Design protobuf](../../protos/game-design/v1/game_design_service.proto), and [Game Session GameDesignClient](../../services/game-session-service/src/main/java/net/firedevops/firemud/gamesession/client/GameDesignClient.java). This is current cross-boundary implementation drift and a deployment/launch blocker, not an alternate identity contract. Migration must converge persistence, protobufs, adapters, and retained-data mapping together, with exact tenant/version scope and positive cross-service readback before this boundary can be claimed proven.
...
- Ensure canonical Game Design launch-descriptor resolution rejects a retired version, so operators cannot accidentally start new instances against it. Retirement does not revoke historical attestation or reachability evidence: `GetPublishedReleaseBundle(tenantId, versionId)` remains queryable for the immutable release/asset attestation and lifecycle checks; separately authorized asset purge may remove eligible bytes while retaining required terminal metadata. The legacy untenant-qualified `game_manifest` table is unsupported implementation drift and is not launch or retirement authority; any retained rows require the disposition and migration proof recorded by the Game Session runtime contract.
...
- The launch orchestrator must treat `versionStateEpoch` as part of preflight proof, not informational metadata. If attestation verification or downstream activation sees a different epoch before instance creation, launch fails closed before any persistent instance row or `PREPARING` world state is created. If epoch or release proof drifts after `PREPARING` exists but before admission, the target workflow fails closed, invokes fenced `FailPreparedWorldInstance`, records terminal `FAILED_PRE_ACTIVATION`, and runs the canonical World Management Class A pre-activation compensation/cleanup described in [World Creation Workflow](./microservices/world-management-service/world-creation-workflow.md). That durable lifecycle is coordinated by Temporal where restart-safe continuation is required; it is not a generic Saga rollback. Current implementation status: Game Session's `failPreparedWorldInstance` path currently stops at the failed-preparation state and does not yet initiate the durable World termination and owner-cleanup flow. This pre-activation outcome is distinct from a failure after activation may have been issued or committed: current Game Session retains both the runtime projection and local instance row as `STARTING`, and keeps them quarantined pending lifecycle reconciliation rather than calling stale `failPrepared` or deleting either record. See [Game Session Runtime and Data](./microservices/game-session-service/runtime-and-data.md#implementation-status). A fresh launch after the `FAILED_PRE_ACTIVATION` outcome requires a new `controlPlaneRequestId` and new `gameInstanceId`; an activation-ambiguous `STARTING` row remains quarantined pending reconciliation rather than being reused. Replaying the old `controlPlaneRequestId` retains its deterministic result and original instance mapping.
...
- When starting a game, it resolves a Game Design launch descriptor for the selected tenant/template. That descriptor binds the base `versionId` to the immutable published release bundle, canonical manifest and required asset-byte proofs, plus any compatible script/plugin selections; Game Session stores the resolved runtime version in `game_instances` only after the complete preflight succeeds.
- The legacy untenant-qualified `game_manifest` table is unsupported implementation drift, not a list of launchable versions and not retirement authority. Current Game Session code still retains that schema/repository surface, and active `GameManifestRepository` writers, including conditional smoke seeding, still insert/update it; those writers must be retired or disabled before retained-row inventory, disposition, or migration. Launch must not read it or treat its rows as proof; tenant-qualified replacement proof and Game Design owner-readback remain implementation gaps.
...
- Publish/finalization and activation-prep use **Class A** pre-activation compensation semantics (compensation is allowed before activation). Publish/finalization's restart-safe durable continuation uses the owning Game Design Temporal workflow. Activation-prep continuation that must survive a restart or wait for external work uses the owning service's Temporal workflow, with the World database row/epoch authoritative for World lifecycle work. Reserve a short synchronous `common-saga` for work that is caller-retryable and does not need durable workflow execution.
