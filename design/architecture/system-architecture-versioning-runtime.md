...
- Durable fork-local playable state is keyed by `(tenantId, playableStateNamespaceId, playableStateScope)` and is evaluated with the active `gameInstanceId`, runtime version, and routing/admission context.
...
- This section owns the complete playtest-state contract for [ADR 0137 (TENANT-03)](./decisions/adr-0137-isolated-playtest-state-modes-and-reset.md). A new lifecycle explicitly selects exactly one initialization mode: `fresh` (empty isolated namespace plus only minimum build-required state), `seeded` (an explicit versioned seed/sample definition), or `snapshot` (an explicit source realm and scope). Snapshot scope is exactly `whole-realm` or `selected-roster`; selected-roster copies the complete dependency closure required for the selected accounts/characters, not only character rows. Every owner must classify its state as preserved, mapped, or recreated/discarded and contribute a manifest bound to the same source tenant, realm, source `gameInstanceId`, source namespace, runtime version, optional patch, scope, counts/completeness evidence, and digest or immutable snapshot identity. Any missing, wrong-source/build, duplicate, stale, unclassified, or disagreeing manifest rejects preparation. Game Session publishes or admits the target only after all manifests and target materialization are complete under one accepted boundary; partial preparation remains non-admissible and is cleaned up or retried.
- Reset is generation replacement: it prepares a fresh namespace using the selected mode/scope, advances the playtest-state generation, validates the same owner-manifest and isolation rules, and atomically moves the realm pointer. Failure before pointer movement leaves the old generation authoritative; old-generation work is fenced after movement and retired through the bounded lifecycle/retention workflow. Production credentials, sessions, token replay state, billing/entitlement authority, source moderation/audit history, and source incident records are never cloned as playtest authority. External effects are suppressed or redirected to explicit test sinks.
