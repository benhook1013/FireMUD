...
- Ensure the canonical Game Design launch descriptor and its immutable release/asset attestation no longer resolve a retired version, so operators cannot accidentally start new instances against it. The legacy untenant-qualified `game_manifest` table is unsupported implementation drift and is not launch or retirement authority; any retained rows require the disposition and migration proof recorded by the Game Session runtime contract.
