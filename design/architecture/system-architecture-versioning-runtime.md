...
- Publish/finalization and activation-prep use **Class A** pre-activation compensation semantics (compensation is allowed before activation). The workflow substrate is chosen by the owning flow: restart-safe World lifecycle and cleanup use Temporal with the World database row/epoch as authority; a short synchronous `common-saga` is only suitable where caller-owned retry/compensation is sufficient.
