...
- Reset is generation replacement: it retains the `playtestLifecycleId`, prepares a fresh namespace using the selected mode/scope, advances the playtest-state generation, validates the same owner-manifest and isolation rules, and atomically moves the realm pointer with the old-generation write fence in the same admission-pointer CAS boundary. Owner outcomes aggregate to `APPLIED`, `NOT_APPLIED`, or unknown; mixed, unknown, or contradictory evidence is `RECONCILIATION_REQUIRED` with admission closed, and a pointer CAS failure after any `APPLIED` outcome requires authoritative ADR 0122 reconciliation before the old generation can reopen. Failure before pointer movement leaves the old generation authoritative; no old-generation write may commit between pointer movement and fencing. Old-generation work is retired through the bounded lifecycle/retention workflow. Production credentials, sessions, token replay state, billing/entitlement authority, source moderation/audit history, and source incident records are never cloned as playtest authority. External effects are suppressed or redirected to explicit test sinks.
