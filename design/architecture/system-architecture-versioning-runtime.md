...
- All player and control-plane flows must resolve the referenced catalog/policy revision together with the routing pointer, and must fail closed on a missing, stale, mismatched, or ambiguous pair:
  ...
  - `CHARS` uses that policy decision and requires caller-bound membership with exact `membershipLifecycleState=ACTIVE`; for non-public realms it additionally requires the current exact Account-owned realm-access grant for `{accountId, tenantId, worldSlug, realmSlug}`, plus runtime-entitlement checks, before resolving caller-valid characters for the selected realm and routed instance. Unlike `PLAY`, it is not the admission-pointer and gameplay-binding operation.
