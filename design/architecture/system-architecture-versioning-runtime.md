...
- Rolling back a script-only or plugin-only patch affects only automation behavior for the current `version_id`; it does not change templates or abilities. Game Session explicitly repins the exact target patch and advances `scriptPinEpoch`; routine epoch-fenced rollback keeps ordinary gameplay ticks running while Automation admission is fenced.
