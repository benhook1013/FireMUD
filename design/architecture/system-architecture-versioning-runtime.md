...
Immediately before the final activation or admission-pointer commit, Game Session must call the Account-owned `CommitTenantCapacityAdmission` contract with `tenantId`, stable `requestId`/`admissionId` admission identity, the expected canonical authority tuple containing the exact `tenantAuthorityGeneration`, `tenantBillingSequence`, and `entitlementVersion`, the operation's bounded `capacityDelta`, and versioned `mutationDigest`. Capacity-creating activation uses its positive bounded delta and may reserve/increment usage only after the final Account fence succeeds. Non-expanding recovery must call the same contract with `capacityDelta=0`; it performs no usage increment or quota consumption, but still creates or replays the exact durable commitment/evidence for the unchanged entitlement tuple. An exact identity-and-payload retry replays; a changed authority tuple, delta, or digest is an idempotency conflict and cannot mutate usage. Game Session must bind that Account fence and evidence identity, exact `entitlementVersion`, and committed sequence into its own fenced activation or routing CAS; a local re-read, conditional read, or recheck is not a strict capacity guarantee. A stale, replaced, or concurrently invalidated Account commitment cannot authorize the state change. This Account commitment and final CAS are in addition to version-state, release-attestation, compatibility, lifecycle-fence, and routing-pointer proof; last-known-good never substitutes for those proofs. Same-binding player resumption additionally requires Account to validate current lifecycle, security, membership, and revocation authority and commit the ADR 0030 resume lease. If any required evidence is unavailable or unsafe, the realm remains closed or the previous pointer remains the sole admissible target. The target request/replay contract is now explicit, but the proto RPC, caller integration, durable Account authority/usage ledger, and reservation lifecycle remain unimplemented.
