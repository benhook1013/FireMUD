...
Account owners request portable account data through the [Account Service](./microservices/account-service/README.md). Under [ADR 0050](./decisions/adr-0050-versioned-export-retention-and-erasure-policy.md), the target account-owner export is a durable asynchronous full-subject lifecycle: `POST /accounts/{accountId}/exports` initiates or replays the job, `GET /accounts/{accountId}/exports/{exportId}` returns its status/manifest, and `GET /accounts/{accountId}/exports/{exportId}/content` downloads completed content. The routes are bound to the global account owner and do not accept a tenant selector. During `deactivated_pending_delete`, this same full-subject lifecycle is the explicitly allowlisted `pending_deletion_scoped` export route; it is distinct from ordinary account-session access. The current implementation remains drift: `GET /accounts/{accountId}/export` returns only Account and profile rows and is not proof of a complete cross-service export. Account freezes the required-owner set and export policy version, records the request, gathers one idempotent contribution from every required owning service, and exposes status/progress plus an explicitly nonterminal versioned manifest while owner fan-in is partial or has retryable, failed, or omitted owner outcomes. A contribution is explicitly `COMPLETE`, `REDACTED`, `OMITTED`, `SEPARATELY_RETAINED`, `RETRYABLE_FAILURE`, `UNAVAILABLE`, or `FAILED_TERMINAL`; an unavailable owner after bounded retry is a terminal export failure, not an implicit omission. The export job is `COMPLETE` only when every required owner contributes successfully, `PARTIAL` only when all required owners are terminal but the manifest explicitly contains policy-allowed redacted, omitted, or separately retained categories, and `FAILED` when a required owner is unavailable or terminally failed. `RETRYABLE_FAILURE` leaves the job nonterminal and resumes the same export generation. The status manifest remains available for terminal partial or failed outcomes and names each owner and category; content download is available only for `COMPLETE`. No result is presented as a complete export when required owner data is missing, unavailable, or failed, and the contract is not a promise to return every account-related record, secret, retained evidence, or provider object.
