...
This is the target onboarding flow for discovering and entering a realm. In the current runtime, existing members may still enter through direct `LOGIN` plus `PLAY` because explicit `JOIN`/`Join & Play` remains unimplemented; that current path does not create membership. First-party WebSocket connect tokens and any future hidden Telnet smart-client metadata may narrow the target realm, but they never replace the authenticated lobby contract.
