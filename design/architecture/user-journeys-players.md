...
Account owners request portable account data through the [Account Service](./microservices/account-service/README.md). Under [ADR 0050](./decisions/adr-0050-versioned-export-retention-and-erasure-policy.md), the target account-owner export is a durable asynchronous full-subject lifecycle: `POST /accounts/{accountId}/exports` initiates or replays the job, `GET /accounts/{accountId}/exports/{exportId}` returns its status/manifest, and `GET /accounts/{accountId}/exports/{exportId}/content` downloads completed content. The routes are bound to the global account owner and do not accept a tenant selector. During `deactivated_pending_delete`, this same full-subject lifecycle is the explicitly allowlisted `pending_deletion_scoped` export route; it is distinct from ordinary account-session access. The current implementation remains drift: `GET /accounts/{accountId}/export` returns only Account and profile rows and is not proof of a complete cross-service export.

The canonical export generation is keyed by the account subject, one high-entropy request identity, and the export policy version. Account durably creates one generation for that tuple before owner fan-in; a retry with the same tuple and request digest returns the same `exportId`, manifest version, and owner work items, while a changed digest conflicts instead of creating a second snapshot. The generation moves through `REQUESTED` and `COLLECTING` to `COMPLETE`, policy-allowed `PARTIAL`, or terminal `FAILED`. Account creates exactly one durable work item per required owner per generation. Retries reuse that `{owner, generation}` identity and update the same item idempotently through zero or more nonterminal attempts to exactly one terminal result: `COMPLETE`, `REDACTED`, `OMITTED`, `SEPARATELY_RETAINED`, `UNAVAILABLE`, or `FAILED_TERMINAL`; `RETRYABLE_FAILURE` remains nonterminal and resumes the same item. Content is available only for `COMPLETE`, while status and the versioned manifest remain available for every terminal result. No result is presented as complete when required owner data is missing, unavailable, or failed, and the contract is not a promise to return every account-related record, secret, retained evidence, or provider object.

Tenant-admin exports are separate tenant-controlled contracts, not account-owner exports. The regular operational export is classified `tenant_regular`, requires live caller membership and `tenant_generation` freshness for the exact target tenant, and is unavailable while billing-blocked. The recovery export uses the canonical tenant-admin route `GET /tenant-admin/tenants/{tenantId}/export`, is classified `billing_safe_tenant`, and is eligible for the canonical billing-blocked tenant states (`grace`, `suspended`, or `canceled`): validation still requires fresh issuer and account authority, live caller-bound membership authority, the exact target tenant, and a live `tenantAdmin` role for that tenant. The route deliberately omits only the target-tenant generation under the closed billing-safe exception; issuer/account freshness and caller-bound membership authority remain required, and no global-role or cached-authorization bypass is implied. Tenant-admin results contain only tenant-owned exportable records and minimum stable subject references; they never expose global credentials, email, external identities, security state, or unrelated account data from other tenants.
...
Account deletion is a separate confirmed operation. The retention-aware erasure and finite-retention guarantees in this section are target/partial rather than a claim that the full cross-service workflow is shipped. It revokes active sessions and is recorded by the [Logging & Admin Service](./microservices/logging-admin-service/README.md) for audit purposes. Deletion eligibility is blocked while the account owns an authoritative subscription in `pending`, `provisioning`, `trialing`, `active`, `past_due`, `grace`, or `suspended`; while it is the old or proposed owner in a nonterminal `subscription_transfer`; while any customer, subscription, transfer, cancellation/end, webhook, outbox, or provider-reconciliation work remains unresolved; or while `BILLING_RECONCILIATION_TERMINAL_FAILURE` or another terminal reconciliation failure lacks explicit operator resolution proving terminal provider cancellation or completed audited ownership transfer. Billing reconciliation is bounded and records an explicit terminal outcome for each affected tenant/provider operation, but a terminal failure is not proof of terminal provider cancellation or completed audited ownership transfer: it remains a durable deletion blocker, leaves the account in `deactivated_pending_delete` or returns the deletion request with the blocker, and requires explicit recovery before deletion can proceed. Deletion eligibility clears only after a terminal provider cancellation or completed audited ownership transfer; provider absence or reconciliation alone cannot clear it. Reconciliation is not an independent clearing outcome; it merely confirms that every affected provider, webhook, outbox, cancellation, transfer, and reconciliation item is terminal after one of those eligibility-clearing outcomes is recorded. The creator must first transfer billing ownership or cancel the subscription terminally and allow all related provider and durable-work items to reconcile so payment instruments, invoices, and tenant hosting responsibility are not orphaned. Direct identity is erased after the cancellation cutoff, while shared or legally/policy-required evidence may remain only in the minimized form and finite duration declared by the canonical retention registry. A durably scheduled step is intermediate pending evidence only and must not be reported as terminal deletion.

When billing reconciliation cannot complete, the deletion workflow persists one retry substate, `BILLING_RECONCILIATION_RETRY_REQUIRED`, while the account remains in `deactivated_pending_delete`. A retry uses the same deletion workflow identity and request digest; another failure remains in that substate with updated diagnostics, while success records the verified provider cancellation or audited ownership transfer and returns the workflow to ordinary erasure processing. Entering the retry substate never clears the billing blocker, restores normal access, or permits terminal deletion.
