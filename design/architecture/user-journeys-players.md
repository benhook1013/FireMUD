...
The text-to-Account translation is one canonical operation across clients. `JOIN <world>` resolves the exact lobby selector to `worldSlug` and the configured default public-production `realmSlug`; it uses the corresponding opaque `connectScopeId` and caller-generated `requestId` when calling the current compatibility seam for `JoinPublicProductionMembership`. Ambiguous or stale world/realm selectors are rejected before the call and require fresh discovery; a stale scope fails closed rather than being silently rebound to a newer target. Account binds `requestId` to the caller, scope, and resolved slugs so an exact retry replays the same membership result or deterministic failure, while a changed selector conflicts. Explicit text `JOIN` and first-party `Join & Play` remain unimplemented; these mappings describe the target seam, not current behavior.
