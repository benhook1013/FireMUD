...
The target journey below requires an explicit `JOIN` / `Join & Play`, then realm-scoped `CHARS`, before character creation, connect-token issuance, or `PLAY` for first-time public-production entry; later admission surfaces never create or restore membership. The public-production membership lifecycle is exact: missing -> `JOIN` creates `ACTIVE`; `INACTIVE` -> `JOIN` restores `ACTIVE` and advances `membershipAuthorityGeneration` and `membershipVersion`; `ACTIVE` -> exact idempotent current snapshot; all other states reject. `JOIN_REQUIRED` covers missing or `INACTIVE` membership. Returning `ACTIVE` members and grant-backed non-public players preserve their existing membership/grant-backed discovery flow and skip only the public-production join action. The canonical join operation is `JoinPublicProductionMembership`, exposed as `POST /auth/bootstrap/join` and translated from text `JOIN <world>`. The differently named current `EnsurePublicProductionPlayerMembership` proto seam is implementation drift that must be replaced, not a compatibility operation. The target `JOIN_REQUIRED` outcome remains canonical, but current Account `EnsurePublicProductionPlayerMembership` is still the implicit membership writer used by connect-token issuance and text `PLAY` and can create missing public-production membership until convergence; that behavior is drift against the target contract. The explicit join action and realm-scoped character gate are not yet implemented across all clients, and credential-bearing text clients with existing active membership and a selected character can still enter through direct `LOGIN` plus `PLAY` without the target discovery/`CHARS` sequence. That is tracked implementation drift, not target behavior.
...
For the target public-production flow, the explicit join action changes the player's Account-owned membership transactionally before character creation, connect-token issuance, or gameplay binding. Missing membership becomes `ACTIVE`; `INACTIVE` becomes `ACTIVE` with advances to `membershipAuthorityGeneration` and `membershipVersion`; `ACTIVE` returns the exact idempotent current snapshot; all other states reject. A successful join remains the intentional durable relationship even if later connection or `PLAY` fails; a failed join transaction creates or changes nothing. For `POST /auth/connect-token`, public-production targets evaluate public-joining policy and may return `PUBLIC_PRODUCTION_ADMISSION_DENIED` or `JOIN_REQUIRED` after entitlement and routing checks. Private/playtest targets never evaluate public-joining policy: they require existing `ACTIVE` membership plus the exact current realm-access grant and return `NON_PUBLIC_ENROLLMENT_REQUIRED`, not `JOIN_REQUIRED`, for missing or `INACTIVE` membership. Both target modes preserve lifecycle, membership-generation, membership-version, entitlement, and routing checks. Character creation, connect-token issuance, and `PLAY` never create or restore membership.

The text-to-Account translation is one canonical operation across clients. `JOIN <world>` resolves the exact lobby selector to `worldSlug` and the configured default public-production `realmSlug`; it uses the corresponding verified opaque `connectScopeId` and caller-generated `requestId` when calling `JoinPublicProductionMembership`. Account binds `requestId` to the canonical operation `JoinPublicProductionMembership`, the authenticated `accountId`/caller, the verified `connectScopeId`, and the resolved `{tenantId, worldSlug, realmSlug, gameInstanceId, pointerVersion}`. Ambiguous or stale world/realm selectors are rejected before the call and require fresh discovery; a stale scope fails closed rather than being silently rebound to a newer target. An exact retry with the same binding replays the stored membership result or deterministic failure, while a changed selector or digest conflicts and requires a new `requestId`. Explicit text `JOIN` and first-party `Join & Play` remain unimplemented; the differently named current proto seam is implementation drift pending direct replacement, and these mappings describe the target contract rather than current behavior.
...
If a tenant is temporarily unavailable because billing or entitlements block gameplay, the player sees a clear tenant-scoped error before `PLAY` succeeds. The cross-surface `PLAY` routing, scope, and billing outcomes described here include `REALM_UNAVAILABLE` for a complete `CLOSED` realm, `ADMISSION_POINTER_UNAVAILABLE` for missing or unusable routing/pointer evidence, `CONNECT_SCOPE_MISMATCH` for a verified scope conflict, and `TENANT_BILLING_BLOCKED` for authoritative billing denial. These outcomes preserve the authenticated lobby/session state and create no gameplay binding; pointer failure takes precedence over scope evaluation, scope mismatch requires a fresh discovery snapshot, and billing denial is evaluated only after routing and scope are valid. If a creator or operator cuts a realm over to a replacement instance, reconnect follows the same lobby and admission flow and lands on the currently routable realm target.
