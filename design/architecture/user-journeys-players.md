...
Players request portable account data within a defined account export scope through the [Account Service](./microservices/account-service/README.md). Under [ADR 0050](./decisions/adr-0050-versioned-export-retention-and-erasure-policy.md), export initiation is durable and asynchronous: Account freezes the required-owner set and export policy version, records the request, gathers one idempotent contribution from every required owning service, and exposes the manifest only after every required owner reaches a terminal contribution state. A contribution is explicitly `COMPLETE`, `REDACTED`, `OMITTED`, `SEPARATELY_RETAINED`, `RETRYABLE_FAILURE`, `UNAVAILABLE`, or `FAILED_TERMINAL`; an unavailable owner after bounded retry is a terminal export failure, not an implicit omission. The export job is `COMPLETE` only when every required owner contributes successfully, `PARTIAL` only when all required owners are terminal but the manifest explicitly contains policy-allowed redacted, omitted, or separately retained categories, and `FAILED` when a required owner is unavailable or terminally failed. `RETRYABLE_FAILURE` leaves the job nonterminal and resumes the same export generation. The status manifest is available for terminal partial or failed outcomes and names each owner and category; content download is available only for `COMPLETE`. No result is presented as a complete export when required owner data is missing, unavailable, or failed, and the contract is not a promise to return every account-related record, secret, retained evidence, or provider object. Tenant admins have two separate tenant-wide export routes. The regular operational export is classified `tenant_regular`, requires live caller membership and `tenant_generation` freshness for the exact target tenant, and is unavailable while billing-blocked. The recovery export is broadly eligible for the canonical billing-blocked tenant states (`grace`, `suspended`, or `canceled`) and is classified `billing_safe_tenant`: validation still requires fresh issuer and account authority, live caller-bound membership authority, the exact target tenant, and a live `tenantAdmin` role for that tenant. The route deliberately omits only the target-tenant generation under the closed billing-safe exception; issuer/account freshness and caller-bound membership authority remain required, and no global-role or cached-authorization bypass is implied. Both contain only tenant-owned exportable recovery data and minimum stable subject references; neither exposes global credentials, email, external identities, security state, or unrelated account data from other games.
