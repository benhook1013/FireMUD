...
- Automation Scripting hosts the `script-patch-readiness` workflow family for eligible non-empty `NotifyScriptVersionUpdate` notifications and durable `onLoad`/readiness progression. The current normal Game Design integration sends an empty `affectedScripts` list, so Automation returns before creating a readiness row and the conditional workflow start is skipped; that publication has no readiness candidate or workflow.
