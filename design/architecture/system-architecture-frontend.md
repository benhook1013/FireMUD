...
- **Admin and creator UIs**  
  ...
  - A durable logout `PENDING` state rejects racing issuance, refresh, rebind, and installation for the fenced token lineage. It also rejects reconciliation that could recreate that identity, but permits the matching Account lifecycle reconciliation when the caller presents the opaque `logoutHandle` bound to the exact request, digest, token identity, and fence. The handle never authenticates a request, restores authority, or returns a replacement credential.
