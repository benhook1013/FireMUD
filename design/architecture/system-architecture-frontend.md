...
- `JOIN_REQUIRED` – Preserve in-memory authentication, present the `Join & Play` action for the selected public-production realm, and block character creation, connect-token issuance, and `PLAY` retry until the explicit join succeeds. Do not retry those operations automatically or create membership implicitly; private/playtest targets require existing membership plus the current grant and do not use public join as a fallback.
