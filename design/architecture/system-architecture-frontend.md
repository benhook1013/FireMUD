...
- `CONNECT_TOKEN_REJECTED` with `reason=unsupported_carrier_or_route`: use the supported carrier/route without refreshing the token. With `reason=invalid_token_content`, request a fresh token and retry with bounded backoff, restarting the first-party bootstrap flow if rejection repeats. Do not infer the reason from HTTP status alone.
