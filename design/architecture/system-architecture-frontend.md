...
- **Player UI (gameplay)**  
  ...
  - The first-join, returning-player, reconnect, resume, grant-backed private/playtest, and related gameplay-admission descriptions in this section are target-state contracts, not confirmed current supported behavior. Current text `PLAY` and connect-token issuance require existing `membershipLifecycleState=ACTIVE` membership and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The obsolete implicit membership-writer surface has been removed; explicit `JOIN` / `Join & Play` and the required `membershipAuthorityGeneration` plus `membershipVersion` reread at connect-token issuance remain unimplemented/proof gaps. The frontend must apply the unimplemented join action and proof gaps as an unavailable gate to first public-production entry with missing or `INACTIVE` membership. Returning active members and private/playtest callers with existing membership plus the current grant remain eligible when their own checks pass, including exact membership-generation and independent membership-version rereads; any path lacking that proof remains unavailable for its own authority gap. The target descriptions remain the behavior to implement.
