...
- **Admin and creator UIs**  
  ...
  - **Target-only logout:** The target flow clears the in-memory auth state and calls Account Service `POST /auth/logout` to revoke only the presented browser/device token; account-wide “logout all devices” uses `POST /auth/logout-all` and terminates the account's control-plane, bootstrap, and active gameplay bindings across tenants. These Account endpoints are unavailable in the current runtime. The current fallback only clears local reconnect state and relies on bounded credential expiry; it does not perform Account revocation. Closing a browser tab does not revoke auth token sessions.
