...
- **Player UI (gameplay)**  
  ...
  - The first-join, returning-player, reconnect, resume, grant-backed private/playtest, and related gameplay-admission descriptions in this section are target-state contracts, not confirmed current supported behavior. Text `PLAY` and connect-token issuance now reject missing public-production membership with `JOIN_REQUIRED` without invoking the membership writer. Explicit `JOIN` / `Join & Play` and the required `membershipAuthorityGeneration` reread at connect-token issuance are both unimplemented/proof gaps. The frontend must apply those gaps as an unavailable gate only to first public-production entry without membership. Returning active members and private/playtest callers with existing membership plus the current grant remain eligible when their own checks pass, including exact membership-generation and independent membership-version rereads; any path lacking that proof remains unavailable for its own authority gap. The target descriptions remain the behavior to implement.
  ...
  - Tenant membership, non-public realm-access-grant checks, and runtime entitlement checks occur after account bootstrap. The `player-bootstrap` token is still a tenant-free, non-gameplay bootstrap credential: it authorizes caller-bound discovery and the explicit join/connect-token APIs, but it does not itself carry or grant gameplay scope. The bounded `gameplay-connect` token is the only edge-admission credential carrying the selected target snapshot. In target behavior, a first public-production join presents an explicit `Join & Play` action that may create the durable Account-owned membership only when it is missing, before character creation and `POST /auth/connect-token`; connect-token issuance never creates membership implicitly. Current connect-token issuance now requires existing membership and returns `JOIN_REQUIRED` when it is absent without invoking the membership writer; text `PLAY` has the same behavior. Explicit `JOIN` / `Join & Play` and the membership-authority-generation reread at issuance are both unimplemented/proof gaps. The frontend must mark only the first-public-production-entry flow unavailable/unsafe rather than treating any implicit creation or stale-generation path as a fallback; existing-member and grant-backed flows remain separately gated by their own current checks. The selected target must come from canonical discovery and may be the public production realm or an explicitly authorized alternate realm such as a playtest fork.
  ...
  - Returning players with an existing Account-owned membership use browser bootstrap, bootstrap discovery, and `POST /auth/connect-token` before taking the normal `PLAY <world> [realm] [character]` path; they skip only the membership-creation action because their membership already exists. Target behavior requires an account with no membership for the selected game, including an otherwise returning account whose membership is absent, to take the explicit `Join & Play` path before character creation/connect, and forbids a connect-token or `PLAY` path from creating that membership implicitly. Text `PLAY` and connect-token issuance now enforce their missing-membership side of this invariant without invoking the membership writer, but the explicit join action and membership-authority-generation reread remain unimplemented; the tracker therefore still records the first public-production path as unavailable drift. The resulting membership powers subsequent “my games” and return flows. Game Session resolves world/realm/character identifiers, rechecks membership, routing, and entitlements, returns bundle metadata, and binds the socket in Redis.
