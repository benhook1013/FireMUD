...
Explicit `JOIN`/`Join & Play` and removal of implicit membership creation are target behavior, not implemented in the current runtime. Current connect-token and text `PLAY` paths may still create membership implicitly; that behavior is a live security-contract violation and known drift, and affected admission must fail closed rather than treating the implicit creation as a valid fallback. The target flow below must not be read as runtime completion. Gateway `POST /ws/game/connect-token/revoke` and the dependent immediate edge-revocation step described below are also target-only and are not implemented in the current runtime. The supported current logout fallback is Account `POST /auth/logout` plus clearing the client cookie/reconnect state and relying on the bounded connect-token lifetime; it must not be described as immediate Gateway revocation.
...
- **Player UI (gameplay)**  
  ...
  - First-party clients must first call exactly `POST /auth/player-bootstrap` with player credentials to establish short-lived account identity for gameplay bootstrap only, then use bootstrap-authenticated HTTP discovery endpoints to choose a caller-visible world/realm target, explicitly call `POST /auth/bootstrap/join` when public-production membership is required, use bootstrap-authenticated character discovery/creation to select a character, and call `POST /auth/connect-token` with the discovery-provided `connectScopeId`. Browser and mobile-browser clients receive the short-lived `Firemud-Connect-Token` HttpOnly cookie used by the `/ws/game/**` handshake; first-party native-mobile clients using a cookie jar remain cookie-only. Explicitly classified non-first-party/public native-mobile clients store the token in OS secure storage and present it through `X-Firemud-Connect-Token`. The client then opens `/ws/game/**` and completes gameplay authentication by issuing `LOGIN` over the WebSocket channel using the already-verified bootstrap/connect context rather than replaying username/password/OTP from the browser, as described in [Authentication & Authorization](./system-architecture-authentication.md#login-and-session-flow).
  ...
  - Explicit player logout must revoke the current `player-bootstrap` token with Account Service `POST /auth/logout`, clear local reconnect/client state, and use the supported bounded connect-token lifetime fallback because Gateway `POST /ws/game/connect-token/revoke` is not implemented yet. The browser cannot directly clear the HttpOnly gameplay cookie; when Gateway revoke is unavailable it relies on the bounded cookie/token lifetime, while replay/deny enforcement remains the responsibility of the Gateway revoke path. The target Gateway revoke operation is an HTTP operation, not a WebSocket upgrade: it would read the HttpOnly cookie server-side, record a bounded deny marker for its `jti` or equivalent edge revocation fence checked before token consumption, and return the clearing `Set-Cookie` response where that response surface supports it. The browser must satisfy the fail-closed `Origin` and anti-CSRF policy defined by ADR 0021; the React client never reads the token. The client clears active player state immediately even if Account is unavailable. The shared frontend auth/session controller is the sole owner of one dedicated, memory-only pending-revocation slot and never persists, logs, or uses its credential for gameplay/bootstrap calls.
  - The target canonical player logout order is: stop reconnect attempts, capture the current `player-bootstrap` token into the pending-revocation slot, send gameplay `LOGOUT` and close the gameplay socket, call the Gateway revoke route when it exists or otherwise clear the client cookie/reconnect state and rely on the bounded token lifetime, call `POST /auth/logout` with the captured token when one exists, immediately clear remembered world/character selection and active in-memory player state, and render the logged-out state. The target Gateway revoke call has its own two-second per-attempt timeout, at most two retries, 250 ms then 500 ms backoff, and five-second total deadline; this dedicated logout controller budget is separate from the generic `AUTH_UNAVAILABLE` retry policy below. Account revocation retains its existing policy of at most five attempts, a five-second timeout per attempt capped by a 30-second overall deadline, and full-jitter exponential backoff starting at 250 ms and capped at four seconds. The controller erases the raw credential at the earliest of Account confirmation, token expiry, the 30-second deadline, or process termination. Confirmation enters terminal local state `LOGGED_OUT_REVOKED`; retry exhaustion or token expiry enters `LOGGED_OUT_REVOCATION_UNCONFIRMED`, remains locally logged out, and requires a fresh `POST /auth/player-bootstrap` before any later gameplay flow. Process termination destroys the memory-only slot; a restarted process likewise begins logged out with revocation unconfirmed and must restart bootstrap rather than claim server revocation succeeded. Token expiry or local clearing is not reported as confirmed server revocation, and raw credential retention never extends its signed lifetime.
  - Logout invalidates the local bootstrap/connect-token request epoch and aborts every in-flight bootstrap, discovery, join, character, and connect-token request owned by that epoch. A late response is ignored and must not set a cookie, restore selection, open a socket, or overwrite logged-out state. The server-side logout fence is authoritative as well: Account rejects a bootstrap or connect-token issuance that races a committed logout for the same account/token lineage, and Gateway rejects a connect token whose deny/revocation fence wins before consumption.
  ...
  - Returning players with an existing Account-owned membership use browser bootstrap, bootstrap discovery, and `POST /auth/connect-token` before taking the normal `PLAY <world> [realm] [character]` path; they skip only the membership-creation action because their membership already exists. An account that has no membership for the selected game, including an otherwise returning account whose membership is absent, must take the explicit `Join & Play` path before character creation/connect. No connect-token or `PLAY` path may create that missing membership implicitly. The resulting membership powers subsequent “my games” and return flows. Game Session resolves world/realm/character identifiers, rechecks membership, routing, and entitlements, returns bundle metadata, and binds the socket in Redis.
