...
- **Player UI (gameplay)**  
  ...
  - The first-join, returning-player, reconnect, resume, grant-backed private/playtest, and related gameplay-admission descriptions in this section are target-state contracts, not confirmed current supported behavior. Current text `PLAY` and connect-token issuance require existing `membershipLifecycleState=ACTIVE` membership, never invoke Account's `EnsurePublicProductionPlayerMembership`, and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The old RPC/client/service seam remains present as unused implementation drift with no current admission caller; explicit `JOIN` / `Join & Play` and the required `membershipAuthorityGeneration` plus `membershipVersion` reread at connect-token issuance remain unimplemented/proof gaps. The frontend must apply the unimplemented join action and proof gaps as an unavailable gate to first public-production entry with missing or `INACTIVE` membership, while treating the unused writer seam as drift rather than as a callable fallback. Returning active members and private/playtest callers with existing membership plus the current grant remain eligible when their own checks pass, including exact membership-generation and independent membership-version rereads; any path lacking that proof remains unavailable for its own authority gap. The target descriptions remain the behavior to implement.
  ...
  - Returning players with an existing Account-owned membership use browser bootstrap, bootstrap discovery, and `POST /auth/connect-token` before taking the normal `PLAY <world> [realm] [character]` path; they skip the membership-creation action after a fresh authoritative check confirms `ACTIVE` membership and its exact authority/version evidence. A valid current character is additionally required only before connect-token issuance or direct `PLAY`; otherwise the player proceeds through realm-scoped character discovery or creation. Target behavior requires an account with missing or `INACTIVE` membership for the selected game, including an otherwise returning account whose membership is no longer active, to take the explicit `Join & Play` path before character creation/connect, and forbids a connect-token or `PLAY` path from creating or restoring that membership implicitly. Current text `PLAY` and connect-token issuance require existing `membershipLifecycleState=ACTIVE` membership, never invoke `EnsurePublicProductionPlayerMembership`, and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The old RPC/client/service seam remains present as unused implementation drift with no current admission caller; explicit `Join & Play` remains unimplemented, as does the required membership-authority-generation and independent membership-version reread proof at issuance. The resulting membership powers subsequent “my games” and return flows. Game Session resolves world/realm/character identifiers, rechecks membership, routing, and entitlements, returns bundle metadata, and binds the socket in Redis.
