...
- **Player UI (gameplay)**  
  ...
  - For both returning-member and grant-backed private/playtest flows, `POST /auth/connect-token` must independently reread the exact current Account-owned one-tenant `membershipVersion` map for the selected `{accountId, tenantId}` and the current `membershipAuthorityGeneration` from the same authoritative membership snapshot. It must compare both values independently with the selected admission baseline, bind the exact `membershipVersion` map and generation into the connect-token issuance/binding evidence, and fail closed when either reread is missing, stale, mismatched, or assembled from another snapshot; a current grant never substitutes for the membership-version reread.
