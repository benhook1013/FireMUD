...
- **Player UI (gameplay)**  
  ...
  - The first-join, returning-player, reconnect, resume, grant-backed private/playtest, and related gameplay-admission descriptions in this section are target-state contracts, not confirmed current supported behavior. Text `PLAY` and connect-token issuance now reject missing public-production membership with `JOIN_REQUIRED` without invoking the membership writer. Explicit `JOIN` / `Join & Play` and the required `membershipAuthorityGeneration` reread at connect-token issuance are both unimplemented/proof gaps. Because both are required for a complete first-party path, the frontend must mark first-join, returning-member, reconnect/resume, and grant-backed private/playtest connect-token flows unavailable/unsafe rather than expose them as supported until both are implemented and proven. The target descriptions remain the behavior to implement.
