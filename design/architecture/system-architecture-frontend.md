...
- `JOIN_REQUIRED` – Preserve in-memory authentication when public-production membership is missing or `INACTIVE`, mark the affected admission unavailable, and do not expose an actionable `Join & Play` retry while explicit `JOIN` and the required `membershipAuthorityGeneration` reread are unimplemented. Keep character discovery/creation, connect-token issuance, and `PLAY` blocked; do not retry those operations automatically or create membership implicitly. An explicit `JOIN`/`Join & Play` creates missing membership or restores `INACTIVE`; private/playtest targets with missing or non-`ACTIVE` membership use `NON_PUBLIC_ENROLLMENT_REQUIRED`, remain separately gated by the current grant, and never use public `JOIN` as a fallback.
