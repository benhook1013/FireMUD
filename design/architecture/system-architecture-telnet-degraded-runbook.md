...
- For bridge-level shutdown attribution, reason in the bounded structured classes `planned_drain`, `upstream_logout`, and `unattributed_failure` rather than treating every non-`backend_unavailable` close as the same kind of event:
  - `planned_drain` means the authenticated bridge delivered `1012/service_restart`.
  - `upstream_logout` means the authenticated bridge delivered terminal `1000/logout`; controller takeover instead has top-level `1000/session_replaced`.
...
- If the authenticated Proxy → Gateway bridge drops without a valid top-level close such as `logout`, `session_replaced`, or `service_restart`, treat the event as `bridge_shutdown_class=unattributed_failure`.
