...
- **WebSocket path checks when Web clients are degraded**
  - When Web clients (browser or other WebSocket-based tools) experience frequent disconnects or stalled gameplay while Telnet remains healthy:
    ...
    - Check Gateway metrics for elevated connection churn or WebSocket send failures on the gameplay route, and correlate with Game Session metrics to ensure backend pods are healthy. In particular, distinguish:
      ...
      - **Reroute / shard handoff** (close-reason metrics dominated by `1013` / `reroute`), which indicates lease moves or planned shard drains rather than an outage. Elevated `reroute` volume is expected during scaling, shard rebalancing, or region-lease transfers; treat it as an incident only if it persists unexpectedly or is accompanied by `backend_unavailable` spikes.
...
- Treat `reroute` closes as normal session handoff (lease moves, shard drains). They should be fast-retriable (small randomized delay) and are not, by themselves, indicators of core gameplay outages.
