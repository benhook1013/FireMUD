...
- Also check the installed backup fallback conditions (`backup_pipeline_recent_backup_slo_breached`, `backup_pipeline_recent_verification_slo_breached`, `backup_pipeline_recent_restore_drill_slo_breached`, `backup_artifact_lineage_invalid`, `backup_artifact_restore_unreadable`, `recovery_participant_convergence_blocked`, `recovery_environment_convergence_blocked`, `recovery_participant_convergence_coverage_missing`, and `recovery_participant_convergence_source_missing`). Treat the cumulative `recovery_participant_convergence_total` counter as historical evidence only; use the current participant-state and coverage recordings to determine whether a blocker remains active.
...
- `RecoveryParticipantConvergenceCoverageMissing` is driven by `recovery_participant_convergence_coverage_missing{environment,participant}`. That recording compares the current `recovery_participant_convergence_coverage` projection against authoritative `recovery_required_participant_inventory{environment,participant}` and requires `recovery_required_participant_inventory_complete{environment}=1`. The controller keeps the completeness marker at `0` while atomically refreshing an environment's inventory and asserts `1` only after the complete set is visible. Participant-specific series identify the missing participant; environment-level inventory-completeness failures use the reserved `participant="__environment__"` value. Missing coverage blocks readiness for that environment even when a stale or independent participant-state series remains.
- `RecoveryParticipantConvergenceMetricsAbsent` separately consumes `recovery_participant_convergence_source_missing{source_family}` when an entire required inventory source disappears. It identifies a global monitoring gap, not an environment's durable convergence state. Do not infer readiness from the missing telemetry or use Prometheus as readiness evidence until the source is restored; the durable recovery controller and retained evidence remain authoritative.
