...
- Also check the installed backup fallback conditions (`backup_pipeline_recent_backup_slo_breached`, `backup_pipeline_recent_verification_slo_breached`, `backup_pipeline_recent_restore_drill_slo_breached`, `backup_artifact_lineage_invalid`, `backup_artifact_restore_unreadable`, `recovery_participant_convergence_blocked`, `recovery_environment_convergence_blocked`, and `recovery_participant_convergence_coverage_missing`). Treat the cumulative `recovery_participant_convergence_total` counter as historical evidence only; use the current participant-state and coverage recordings to determine whether a blocker remains active.
...
- `RecoveryParticipantConvergenceMetricsAbsent` is driven by `recovery_participant_convergence_coverage_missing`. That recording compares current participant-state coverage against authoritative `recovery_required_participant_inventory{environment,participant}` and requires `recovery_required_participant_inventory_complete{environment}=1`. The controller keeps the completeness marker at `0` while atomically refreshing an environment's inventory and asserts `1` only after the complete set is visible. Missing participant state, an incomplete projection, or either absent inventory family fails closed. Keep recovery quarantined until coverage is restored and the durable recovery controller proves readiness.
...
- Query Prometheus directly for a small set of critical “is it healthy?” checks:
  ...
  - backup fallback conditions (`backup_pipeline_recent_backup_slo_breached`, `backup_pipeline_recent_verification_slo_breached`, `backup_pipeline_recent_restore_drill_slo_breached`, `backup_artifact_lineage_invalid`, `backup_artifact_restore_unreadable`, `recovery_participant_convergence_blocked`, `recovery_environment_convergence_blocked`, `recovery_participant_convergence_coverage_missing`),
