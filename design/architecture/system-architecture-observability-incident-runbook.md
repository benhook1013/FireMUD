...
- Check the installed backup fallback conditions (`backup_pipeline_recent_backup_slo_breached`, `backup_pipeline_recent_verification_slo_breached`, `backup_pipeline_recent_restore_drill_slo_breached`, `backup_artifact_lineage_invalid`, and `backup_artifact_restore_unreadable`). The recovery-controller recordings (`recovery_participant_convergence_blocked`, `recovery_environment_convergence_blocked`, `recovery_participant_convergence_coverage_missing`, and `recovery_participant_convergence_source_missing`) are target-state monitoring contracts until the durable recovery controller and exporter are implemented and proved; they are not an installed operator fallback. When available in that target state, treat the cumulative `recovery_participant_convergence_total` counter as historical evidence only and use the current participant-state and coverage recordings to identify active blockers.
- Explicitly check the five backup source-absence alerts: `BackupLastSuccessMetricsAbsent`, `BackupVerificationLastSuccessMetricsAbsent`, `BackupRestoreDrillLastSuccessMetricsAbsent`, `BackupArtifactLineageMetricsAbsent`, and `BackupArtifactRestoreReadabilityMetricsAbsent`. Treat them as readiness-blocking for the evidence they protect. Also check the environment/participant-scoped `RecoveryParticipantConvergenceCoverageMissing` alert and the global `RecoveryParticipantConvergenceMetricsAbsent` monitoring-health alert.
- In the target state, `RecoveryParticipantConvergenceCoverageMissing` is driven by `recovery_participant_convergence_coverage_missing{environment,participant}`. That recording compares the current `recovery_participant_convergence_coverage` projection against authoritative `recovery_required_participant_inventory{environment,participant}` and requires `recovery_required_participant_inventory_complete{environment}=1`. The controller keeps the completeness marker at `0` while atomically refreshing an environment's inventory and asserts `1` only after the complete set is visible. Participant-specific series identify the missing participant; environment-level inventory-completeness failures use the reserved `participant="__environment__"` value. Missing coverage blocks readiness for that environment even when a stale or independent participant-state series remains.
- In the target state, `RecoveryParticipantConvergenceMetricsAbsent` separately consumes `recovery_participant_convergence_source_missing{source_family}` when an entire required source disappears. It identifies a global monitoring gap, not an environment's durable convergence state. Before that monitoring is implemented and proved, or whenever telemetry or controller proof is absent, readiness remains blocked. Prometheus never becomes recovery authority; the durable recovery controller and retained evidence remain authoritative.
...
- Query Prometheus directly for a small set of critical “is it healthy?” checks:
  ...
  - backup fallback conditions (`backup_pipeline_recent_backup_slo_breached`, `backup_pipeline_recent_verification_slo_breached`, `backup_pipeline_recent_restore_drill_slo_breached`, `backup_artifact_lineage_invalid`, `backup_artifact_restore_unreadable`, `recovery_participant_convergence_blocked`, `recovery_environment_convergence_blocked`, `recovery_participant_convergence_coverage_missing`, `recovery_participant_convergence_source_missing`),
  - the five backup source-absence alerts (`BackupLastSuccessMetricsAbsent`, `BackupVerificationLastSuccessMetricsAbsent`, `BackupRestoreDrillLastSuccessMetricsAbsent`, `BackupArtifactLineageMetricsAbsent`, `BackupArtifactRestoreReadabilityMetricsAbsent`), which block readiness for the evidence they protect,
  - `RecoveryParticipantConvergenceCoverageMissing`, which blocks the affected environment, and `RecoveryParticipantConvergenceMetricsAbsent`, which marks a global monitoring gap without replacing or mutating durable controller state,
