...
- Explicitly check all six backup/recovery source-absence alerts: `BackupLastSuccessMetricsAbsent`, `BackupVerificationLastSuccessMetricsAbsent`, `BackupRestoreDrillLastSuccessMetricsAbsent`, `BackupArtifactLineageMetricsAbsent`, `BackupArtifactRestoreReadabilityMetricsAbsent`, and `RecoveryParticipantConvergenceMetricsAbsent`, plus the environment-scoped `RecoveryParticipantConvergenceCoverageMissing` alert. Treat any active one as readiness-blocking; do not infer backup or recovery readiness while a required source or participant coverage signal is missing.
- `RecoveryParticipantConvergenceCoverageMissing` is driven by `recovery_participant_convergence_coverage_missing{environment,participant?}`. That recording compares current participant-state coverage against authoritative `recovery_required_participant_inventory{environment,participant}` and requires `recovery_required_participant_inventory_complete{environment}=1`. The controller keeps the completeness marker at `0` while atomically refreshing an environment's inventory and asserts `1` only after the complete set is visible. `RecoveryParticipantConvergenceMetricsAbsent` separately consumes `recovery_participant_convergence_source_missing{source_family}` when an entire required inventory source disappears. Keep recovery quarantined until both source health and environment coverage are restored and the durable recovery controller proves readiness.
