...
- If Prometheus is reachable, query it directly for a small set of critical “is it healthy?” diagnostics. Each result is a bounded, freshness-labelled snapshot rather than routed alert state; stale values become `unknown` and do not authorize readiness or recovery:
  ...
  - `WebSocketEntryPathBlackboxMetricsAbsent` and `TelnetEntryPathBlackboxMetricsAbsent` when the required-profile Prometheus mirror has no series for an exposed path; these are P1 unknown/degraded evidence signals, not observed path-outage results. Restore the external monitor or mirror and verify the authoritative off-cluster evidence before treating the path as healthy,
