...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect flow is owned by [Authentication](./system-architecture-authentication.md#login-and-session-flow) and [Session Behavior](./system-architecture-session-behavior.md#session-and-identity-management). Direct text/Telnet starts with fresh `WORLDS` discovery before credential-bearing `LOGIN`, while first-party WebSocket starts with `POST /auth/player-bootstrap` and `/auth/bootstrap/*` discovery. Current first-party connect-token issuance and text `PLAY` admit only fresh `ACTIVE` membership: eligible missing or `INACTIVE` public-production membership fails closed with `JOIN_REQUIRED`, while missing or non-`ACTIVE` membership for a non-public target remains `WORLD_ACCESS_DENIED`. Target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` is not current behavior. Explicit `JOIN`/`Join & Play` and the required connect-token membership-authority/version rereads remain unimplemented/proof gaps. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
...
Private/playtest reconnect admission follows the [Authentication contract](./system-architecture-authentication.md#login-and-session-flow). Current missing or non-`ACTIVE` membership remains `WORLD_ACCESS_DENIED`; current text `PLAY` checks the current realm grant, while current connect-token grant validation remains incomplete. Target admission requires existing caller-bound `ACTIVE` membership, the current Account-owned realm grant, fresh entitlement, exact routing, current `membershipAuthorityGeneration`, and independent current `membershipVersion` evidence. The target-only membership outcome is `NON_PUBLIC_ENROLLMENT_REQUIRED`, distinct from public `JOIN_REQUIRED`; target character discovery or allowed realm-local character creation follows only after `ACTIVE` membership and the current realm grant, with an invalid or missing grant returning `REALM_ACCESS_DENIED`, while unavailable grant or other required authority returns `AUTH_UNAVAILABLE`.
...
Every reconnect path that would handle `JOIN_REQUIRED` or invoke public `JOIN`/`Join & Play` follows the [Authentication membership repair contract](./system-architecture-authentication.md#login-and-session-flow). In the target, credential-bearing text clients perform fresh `WORLDS` discovery before `LOGIN`, authenticated `REALMS` discovery before the conditional in-band `JOIN`, and the Account-owned `ACTIVE` membership prerequisite before `CHARS` or character creation; Game Session carries only the current text-flow scope from that `REALMS` exchange into the live transport, while a missing, expired, or mismatched scope blocks `JOIN` and returns the client to fresh `REALMS`. A denied policy preserves authentication and membership, creates no binding, and stops before those later steps or `PLAY`. Target first-party WebSocket clients complete HTTP `Join & Play` before character repair, connect-token issuance, socket admission, bare `LOGIN`, and `PLAY`; a denied policy returns `PUBLIC_PRODUCTION_ADMISSION_DENIED`, preserves bootstrap authentication and membership state, and stops before those later steps. Current runtime does not implement explicit `JOIN`/`Join & Play`: text clients instead receive non-actionable `JOIN_REQUIRED` without mutation, while first-party browser/mobile bootstrap/connect-token surfaces remain partial and the complete flow is target-only. Only a permitted target repair continues after Account returns `ACTIVE`.
...
- Telnet clients reconnect by establishing a new TCP connection to the TCP Proxy Service and performing fresh authoritative `WORLDS`, credential-bearing `LOGIN`, and authenticated `REALMS` discovery. In the target they take the conditional `JOIN` step when public membership repair is required, then perform `CHARS` or allowed character creation after the Account-owned `ACTIVE` membership prerequisite and issue `PLAY` before gameplay commands; current text clients stop with non-actionable `JOIN_REQUIRED` and no mutation when that repair is required. Direct text reconnect obtains a new transport-local scope from `REALMS`; it has no browser/WebSocket connect-token snapshot to reuse and must not select or attest a target from stale prior fields. If smart-client attach hints return later, they should be re-sent as hidden MCP metadata on the new connection.
...
For clarity, Telnet clients do not receive hidden recovery after their TCP socket or TCP Proxy-to-Gateway edge bridge is lost. Even if the proxy-wide admission circuit later returns to closed after such an edge outage, the affected Telnet client must reconnect on a fresh TCP socket and repeat the applicable `WORLDS`/credential-bearing `LOGIN`/authenticated `REALMS`/conditional `JOIN`/`CHARS` or character-creation/`PLAY` flow. This does not prevent Gateway from retaining a still-established proxy bridge while it performs ADR 0013's bounded rebind of its separate Game Session upstream.
