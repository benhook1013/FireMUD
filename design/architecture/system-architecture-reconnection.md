...
Text clients may use the `LOGIN` -> `PLAY` shortcut only after the current routing, membership, character, entitlement, and grant checks defined by [Authentication](./system-architecture-authentication.md#in-band-play-admission-boundary) pass for the same target. First-party WebSocket clients rerun bootstrap/discovery and obtain a fresh connect token before the new handshake. The current implementation caveat remains that first-party connect-token issuance and text `PLAY` require existing `ACTIVE` membership and return `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership; explicit `JOIN`/`Join & Play` remains unimplemented. Gameplay commands are not admitted before `PLAY`, and Telnet attach hints remain advisory hidden MCP metadata only.
...
Every reconnect path that would handle `JOIN_REQUIRED` or invoke public `JOIN`/`Join & Play` follows the [Authentication membership repair contract](./system-architecture-authentication.md#login-and-session-flow). The client-visible consequence is that a denied policy returns `PUBLIC_PRODUCTION_ADMISSION_DENIED` and stops before `CHARS`, character creation, connect-token issuance, `LOGIN`, or `PLAY`; only a permitted repair continues after Account returns `ACTIVE`. The current direct text `LOGIN` -> `PLAY` path remains implementation drift.
...
The current Game Session continuity setting remains an implementation caveat: the target cap is five minutes, while current code defaults to one hour and does not enforce that cap. This does not change the owner-defined token or session rules.
