...
- a bounded durable resume transcript keyed by the admitted tenant/game, game-instance, and character identity replays its retained narrative/system entries after successful `LOGIN` + `PLAY` for an eligible resume or fresh non-logout admission; Redis may cache that context but is not authoritative. Explicit gameplay `LOGOUT` terminates the binding and suppresses private replay, so later `LOGIN` + `PLAY` does not replay that terminated binding's context;
