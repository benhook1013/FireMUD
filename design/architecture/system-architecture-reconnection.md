...
Reauthentication is required when the client-to-Gateway edge connection disconnects, or when server-side auth state expires or is revoked. A retained-edge upstream rebind, including a qualifying hidden Game Session restart, preserves the existing session and does not repeat `LOGIN` or `PLAY`.
