...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect binding flow is `LOGIN` then `PLAY` for confirmed existing members; a first public-production entry must complete explicit `JOIN`/`Join & Play` first and receives `JOIN_REQUIRED` when required durable membership is absent. Text `PLAY` now enforces that missing-membership result, but connect-token issuance may still create membership implicitly; that is confirmed implementation drift, not target behavior. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
...
The target admission contract above is distinct from confirmed current implementation drift: connect-token issuance may still create public-production membership implicitly, although text `PLAY` now returns `JOIN_REQUIRED` when it observes missing public-production membership. Reconnect documentation must not treat the remaining implicit writer as the target, and a client must not infer current membership or target identity from a stale Redis binding, cached tenant/realm fields, or an old `connectScopeId` after `CONNECT_SCOPE_MISMATCH`.
