...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect binding flow is `LOGIN` then `PLAY` only when confirmed existing `ACTIVE` membership and an already selected current character are both present. Otherwise the client completes lobby and character selection. A first public-production entry with missing or `INACTIVE` membership must evaluate fresh selected-target catalog/pointer evidence together with the Account-owned `allowPublicJoin` entitlement before explicit `JOIN`/`Join & Play` repair; denial returns `PUBLIC_PRODUCTION_ADMISSION_DENIED` without changing membership, while permission allows `JOIN_REQUIRED` and the repair operation to continue. The canonical membership lifecycle is missing -> `JOIN` creates `ACTIVE` and `INACTIVE` -> `JOIN` restores `ACTIVE`, with either transition advancing `membershipVersion`; `membershipAuthorityGeneration` advances independently only when `callerBoundAuthorityInvalidated=true`; `ACTIVE` returns the exact idempotent current snapshot and all other states reject. Character selection, connect-token issuance, and `PLAY` never create or restore membership. Current first-party connect-token issuance and text `PLAY` require existing `membershipLifecycleState=ACTIVE` membership and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The obsolete implicit membership-writer surface has been removed; explicit `JOIN`/`Join & Play` and the required connect-token membership-authority/version rereads remain unimplemented/proof gaps. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
...
- **Credential-bearing text lobby flow** – Telnet and other non-WebSocket text clients open a fresh TCP connection, send `LOGIN <email> <secret>` / `LOGON <email> <secret>` or the applicable email-code `LOGIN` / `LOGON` forms, then perform `WORLDS`, `REALMS <world>`, selected-target-policy-gated `JOIN`/`JOIN_REQUIRED` handling, realm-scoped `CHARS` or character creation, and `PLAY`. This flow does not use HTTP bootstrap, browser discovery snapshots, or connect tokens.
  - **First-party WebSocket flow** – The client reruns HTTP `player-bootstrap` and discovery, must evaluate fresh selected-target catalog/pointer evidence and the Account-owned `allowPublicJoin` entitlement before completing `Join & Play` when public-production membership is missing or `INACTIVE`, performs realm-scoped character selection or creation, requests a fresh HTTP connect token, opens `/ws/game/**`, sends bare `LOGIN` using the verified bootstrap/connect context, and completes `PLAY`. A denied policy returns `PUBLIC_PRODUCTION_ADMISSION_DENIED`, leaves membership unchanged, and stops before character repair or token issuance. A returning player may skip `Join & Play` after fresh Account evidence confirms `membershipLifecycleState=ACTIVE`, the exact current `membershipAuthorityGeneration`, and independently current `membershipVersion` for the unexpired discovery snapshot. A valid current character is additionally required before connect-token issuance or `PLAY`; otherwise the player proceeds through realm-scoped discovery or creation. Bare `LOGIN` never prompts for or replays credentials.

Text clients may use the `LOGIN` -> `PLAY` shortcut only after fresh server-resolved routing and target scope identify the same `{tenantId, gameInstanceId}`, the selected pointer and catalog revision remain consistent, Account confirms `membershipLifecycleState=ACTIVE` with the exact current `membershipAuthorityGeneration` and independently current `membershipVersion`, and valid current entitlement evidence is available. Private or playtest targets additionally require the current Account-owned grant for the exact target. Any missing, stale, revoked, regressed, or mismatched routing, entitlement, membership, generation, version, or grant evidence denies the shortcut rather than treating the caller as an existing member; the client must already have an explicitly selected current character for that same target. First-party WebSocket clients may skip `Join & Play` only under the fresh Account evidence and unexpired-snapshot conditions above; otherwise they must evaluate the selected target's public-production policy and complete the applicable join, lobby, and character selection steps. In the target flow, a first public-production entry with missing or `INACTIVE` membership uses explicit `JOIN`/`Join & Play` before character selection, and returns `JOIN_REQUIRED` only when fresh Account entitlement carries `allowPublicJoin=true` and fresh selected-target catalog/pointer evidence identifies the public-production target; a denied policy returns `PUBLIC_PRODUCTION_ADMISSION_DENIED` and cannot create or restore membership. `JOIN` applies the canonical missing/`INACTIVE`/`ACTIVE` lifecycle above. Character creation, connect-token issuance, and `PLAY` never create or restore membership. In the current flow, first-party connect-token issuance and text `PLAY` require existing `membershipLifecycleState=ACTIVE` membership and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The obsolete implicit membership-writer surface has been removed; explicit `JOIN`/`Join & Play` remains unimplemented. A grant-backed path may proceed without public `JOIN` only when its required caller-bound `ACTIVE` membership and Account-owned grant checks pass; the grant never creates or substitutes membership. First-party WebSocket reconnects must also acquire a fresh connect token before the `/ws/game/**` handshake. Gameplay commands are not admitted before `PLAY` except in explicitly documented dev/test bypass modes. If Telnet smart-client attach hints return later, they should ride hidden MCP metadata on the new TCP connection and remain advisory only.
...
The target admission contract above is distinct from current implementation: explicit `JOIN`/`Join & Play` remains unimplemented, while current first-party connect-token issuance and text `PLAY` require existing `membershipLifecycleState=ACTIVE` membership and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The obsolete implicit membership-writer surface has been removed, and the required exact membership-authority/version rereads remain proof gaps. A client must not infer current membership or target identity from a stale Redis binding, cached tenant/realm fields, or an old `connectScopeId` after `CONNECT_SCOPE_MISMATCH`.

### Public membership repair during reconnect (normative)

Every reconnect path that would handle `JOIN_REQUIRED` or invoke public `JOIN`/`Join & Play` must evaluate fresh selected-target catalog/pointer evidence together with the Account-owned authoritative `allowPublicJoin` entitlement before treating membership repair as available. The [Account membership contract](./microservices/account-service/api-contracts.md#grpc-apis) owns the atomic policy and membership decision, while the [protocol-bridging repair gate](./system-architecture-protocol-bridging.md#public-membership-repair-gate-normative) owns the shared client sequencing. A denied policy returns `PUBLIC_PRODUCTION_ADMISSION_DENIED`, cannot create or restore membership, and stops the flow before `CHARS`, character creation, connect-token issuance, `LOGIN`, or `PLAY`. Only a permitted policy allows missing membership to be created or `INACTIVE` membership to be restored; after the successful `ACTIVE` result, reconnect may continue to character repair, token issuance, `LOGIN`, or `PLAY`. The current direct text `LOGIN` -> `PLAY` compatibility path remains implementation drift and does not change this target ordering.
...
- **Physical Redis deletion** – The Redis key TTL is storage cleanup for the binding. Expiration processing, failover, or AOF replay can leave a key present after logical expiry, while cleanup can remove it earlier. Every gameplay-binding TTL refresh atomically preserves the earliest of the existing physical expiry, the requested refresh deadline, and `continuityBindingExpiresAt`; it cannot recreate a missing or expired binding. Key presence is never permission to resume, and refreshing the physical TTL must never rewrite `continuityBindingExpiresAt` or extend an earlier cleanup deadline.
...
Resume is authorized from current identity and current `ACTIVE` membership/entitlement authority, not from the previous backend token alone. After a fresh successful `LOGIN`, Game Session must rebind any resumed gameplay session to a fresh backend token and reject resume if current membership authority for the tenant has been removed or is not `ACTIVE`. For a retained-edge upstream rebind or a qualifying Game Session restart, the durable protected single-use rebind handle in the gameplay binding is the restart authority; `authTokenGeneration`, a token hash, or workload identity alone cannot recreate backend-token authority.
...
- **Retained-edge rebind** – Gateway rebinds the stable edge transport to the replacement Game Session instance. This is not a new public admission: the original first-party connect token need not remain valid for the internal rebind, and a successful rebind requires no new `LOGIN` or `PLAY`. Game Session still applies the canonical session continuity, authority, and fencing checks.
...
`/ws/game/**` is the only gameplay route. Before classification, Gateway strips any client-supplied `X-Firemud-Connection-Mode`, connect-context, or other gateway-owned authentication marker. Non-proxy clients with missing, invalid, expired, replayed, scope-mismatched, or replay-protection-unavailable connect-token state are rejected before upgrade with HTTP `403` and a bounded `CONNECT_*` handshake class. Trust-boundary denials (for example internal-listener/mTLS policy mismatch) are rejected with HTTP `403` and handshake class `POLICY_DENY`. After the appropriate origin and token/trust checks succeed, Gateway itself emits the trusted `X-Firemud-Connection-Mode: first_party_web` marker for the current protected-cookie path, including supported native or other non-browser first-party clients using a protected cookie jar; a dedicated public non-browser header carrier remains target-only until separately registered and proven. Gateway emits `X-Firemud-Connection-Mode: trusted_tcp_proxy` for an authenticated TCP Proxy bridge. Game Session accepts these values only as gateway-issued markers on the authenticated Gateway-to-Game-Session hop, never as client claims; positive mode selection remains required rather than inferring trust from header absence.
