...
- **Replacement-state continuity** – [ADR 0122](./decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md) adds a durable `playableStateNamespaceId` to the replacement/reconnect context without making `gameInstanceId` durable identity. The current uniqueness key and implementation remain `{tenantId, gameInstanceId, characterId}`; target reconnect must authorize the active instance for the resolved namespace, reject stale/old-instance bindings, and preserve durable character state across replacement. [ADR 0123](./decisions/adr-0123-database-authoritative-temporal-coordinated-world-lifecycle.md) keeps lifecycle state/epoch authoritative in the owner database while Temporal coordinates cleanup; existing restart/reconnect proof does not establish these replacement guarantees.
...
At most one active gameplay binding is supported per current uniqueness key `{tenantId, gameInstanceId, characterId}` at any point in time. For replacement, that binding is additionally admitted only when its `gameInstanceId` is the active fenced runtime for the resolved `playableStateNamespaceId`; an old instance is not a second durable-state authority while it drains.
