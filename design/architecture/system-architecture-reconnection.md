...
- then FireMUD obtains a fresh authoritative `LOOK` and emits exactly one current prompt. This reconstruction defines current state; retained context is narrative orientation only.
...
- reconnect should restore semantic context first, then obtain a fresh authoritative `LOOK`, then emit exactly one fresh current prompt;
