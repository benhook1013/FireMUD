...
- **Telnet and WebSocket parity** – Both transports share the same gameplay authentication and lobby-binding contract (`LOGIN` then `PLAY`) after an actual edge reconnect. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
...
`session_expiration_ms` is the independent gameplay-continuity policy supplied by `FIREMUD_AUTH_SESSION_EXPIRATION_MS`. Its effective value is capped at `300000` ms (five minutes), regardless of JWT lifetime or issued-token cleanup margin. It is not a JWT validity period or an uninterrupted-play cutoff: every server-side JWT remains valid only through its own `exp`, while healthy active sessions rotate backend tokens. Rotation cannot extend the continuity anchor or resume deadline. Transcript retention under `firemud.reconnection.buffer.*` is independent and never extends resume eligibility. Current Game Session code still defaults this setting to one hour and does not enforce the target cap; that is implementation drift.
...
| Active-socket authentication invalidation | Game Session terminates the active binding, records the disconnection episode, and requires fresh `LOGIN` and `PLAY`; re-admission may consume that episode only if current authority still permits gameplay |
