...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect binding flow is `LOGIN` then `PLAY` for confirmed existing members; a first public-production entry must complete explicit `JOIN`/`Join & Play` first and receives `JOIN_REQUIRED` when required durable membership is absent. The current connect-token/`PLAY` admission path may still create membership implicitly where the existing implementation permits it; that is confirmed implementation drift, not target behavior. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
...
After `LOGIN` succeeds, clients must re-establish gameplay scope by selecting a world, optional realm, and character via the lobby commands (`WORLDS`, `REALMS <world>`, `CHARS <world> [realm]`, and `PLAY <world> [realm] [character]`) as defined in [Tenant Selection for Gameplay](./system-architecture-authentication.md#tenant-selection-for-gameplay-lobby-selection). For a confirmed existing durable member, reconnect uses `LOGIN` -> `PLAY`; a first public-production entry uses explicit `JOIN`/`Join & Play` before `PLAY`, and missing required membership returns `JOIN_REQUIRED`. A grant-backed path may proceed without public `JOIN` only when its required durable membership already exists; the grant never creates or substitutes membership. First-party WebSocket reconnects must also acquire a fresh connect token before the `/ws/game/**` handshake. Gameplay commands are not admitted before `PLAY` except in explicitly documented dev/test bypass modes. If Telnet smart-client attach hints return later, they should ride hidden MCP metadata on the new TCP connection and remain advisory only.

The target admission contract above is distinct from confirmed current implementation drift: some current connect-token and `PLAY` paths may still create public-production membership implicitly. Reconnect documentation must not treat that behavior as the target, and a client must not infer current membership or target identity from a stale Redis binding, cached tenant/realm fields, or an old `connectScopeId` after `CONNECT_SCOPE_MISMATCH`.
...
The preceding table applies only before the server returns `101 Switching Protocols`. Once `/ws/game/**` has upgraded, a scope mismatch is a Game Session protocol outcome rather than an HTTP handshake response. If `PLAY` or the signed connect-context admission check finds that the requested target does not match the verified connect-token scope, Game Session emits `ERROR CONNECT_SCOPE_MISMATCH` on the established protocol and closes the socket with the existing `1008/policy_violation` close category. The client must discard the stale bootstrap/discovery state, complete HTTP `Join & Play` if membership is absent, obtain a fresh connect token, and open a new socket; it must not retry `PLAY` on the rejected socket or treat the outcome as an HTTP `403`.
