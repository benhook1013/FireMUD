...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect flow is owned by [Authentication](./system-architecture-authentication.md#login-and-session-flow) and [Session Behavior](./system-architecture-session-behavior.md#session-and-identity-management). Direct text/Telnet starts with fresh `WORLDS` discovery before credential-bearing `LOGIN`, while first-party WebSocket starts with `POST /auth/player-bootstrap` and `/auth/bootstrap/*` discovery. Current Game Session membership, grant, and entitlement gRPC calls fail with `FAILED_PRECONDITION` before data access, so valid-route direct-text `PLAY` returns `AUTH_UNAVAILABLE` before membership-derived `JOIN_REQUIRED` or `WORLD_ACCESS_DENIED`; `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before calling Entity. Membership outcomes in downstream code paths are conditional behavior exercised only when Account responses are successfully mocked, not currently reachable end-to-end behavior. The Account REST connect-token issuance path is separate and retains its own current behavior; it is not covered by this Game Session gRPC failure. Target lifecycle, generation, and independent membership-version checks remain unproved. The Account JOIN RPC, browser bootstrap join facade, and direct-text REALMS-scope/JOIN dispatch exist locally; PostgreSQL execution, direct text-to-Logging end-to-end proof, unknown-outcome recovery, and the required connect-token membership-authority/version rereads remain unproved. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`. See [Authentication Implementation Status](./system-architecture-authentication.md#implementation-status) for the canonical admission outcome.
- **Runtime authority checks** – The `PLAY` implementation attempts runtime membership and tenant-entitlement checks before fresh entry or resume/takeover, but the current Game Session Account gRPC calls fail with `FAILED_PRECONDITION` before data access. The runtime therefore returns `AUTH_UNAVAILABLE`; it does not currently complete those checks or prove authorization from Redis gameplay identity.
...
> Spring Cloud Gateway restarts affect only Telnet clients whose bridge is attached to the restarting Gateway instance. Planned drains map to Telnet `service_restart`; every other valid authenticated Gateway top-level close is preserved unchanged through the Telnet bridge. Absent or invalid bridge attribution, or missing or invalid authenticated Gateway top-level close metadata, falls back to observation-specific `backend_unavailable`.
...
Every reconnect path that would handle `JOIN_REQUIRED` or invoke public `JOIN`/`Join & Play` follows the [Authentication membership repair contract](./system-architecture-authentication.md#login-and-session-flow), including its [direct-text REALMS-to-JOIN scope](./system-architecture-authentication.md#direct-text-realms-to-join-scope-normative) procedure. This document retains only the transport consequence: fresh discovery precedes the authenticated text flow, and `CHARS` or character creation is conditional on no valid selected character being resolved. The local Account JOIN RPC, browser bootstrap join facade, and direct-text dispatch exist, but full text and first-party browser/mobile flow and proof remain incomplete; current valid-route text `PLAY` returns `AUTH_UNAVAILABLE` before membership evaluation, so the downstream `JOIN_REQUIRED` outcome is not currently reachable end-to-end.
...
Gameplay command delivery is layered as defined by [ADR 0062](./decisions/adr-0062-layered-gameplay-command-delivery-semantics.md): Telnet and plain-text WebSocket clients remain fire-and-forget at the edge and do not attach identities, while trusted Game Session acceptance assigns or preserves a durable `commandId` and status before acknowledgement. Accepted ordinary commands are tracked but not automatically replayable; explicitly replayable classes require their own safe-intake contract. Internal retries preserve the accepted identity and owner guards, and reconnect never replays raw outbound frames. A close class never proves whether admitted work committed; a client or tool holding `{tenantId, gameInstanceId, commandId}` reconciles it through the authoritative command-status surface.
...
| Spring Cloud Gateway restart | Only sockets attached to the restarting Gateway instance disconnect. Unaffected sockets on other healthy Gateway instances remain up and new handshakes continue. Connections bound to the specific Gateway instance being drained or crashing are visible failures. Planned drains emit `1012/service_restart` for affected WebSocket sessions and Telnet `service_restart`; graceful unplanned failures may emit `1011/internal_error`. A WebSocket hard crash that drops transport without a close frame uses the `internal_error` retry fallback. Telnet preserves every valid authenticated upstream top-level class unchanged, while abrupt bridge loss without valid authenticated upstream close metadata uses the observation-specific `backend_unavailable` fallback. Exact translation and precedence remain owned by the [Gateway matrix](./system-architecture-gateway.md#canonical-close-translation-matrix). Affected clients obtain a fresh connect token where applicable before reopening `/ws/game/**`. |
...
- **Retry caps**
  ...
  - Client policy handling is split into explicit classes:
    ...
    - `policy_violation_non_retriable` (for example sustained abuse or malformed protocol outcomes) – every top-level `1008/policy_violation` close maps to this client class by default: stop auto-retry or switch to very long backoff and surface corrective action to the user. Diagnostic subreasons, including `edge_backpressure`, never make that top-level class retriable and do not change lifecycle or retry semantics. The sole documented exception is when the established protocol first emits `ERROR CONNECT_SCOPE_MISMATCH` and then closes `1008/policy_violation`; recovery is retriable, `CONNECT_SCOPE_MISMATCH` is not a close subreason, and the client must discard the stale scope, rerun discovery/token issuance, and open a new socket. Follow [Post-upgrade Game Session admission failures](#post-upgrade-game-session-admission-failures) for that flow.
- **Scope of reconnection**
- Telnet clients reconnect by establishing a new TCP connection to the TCP Proxy Service and following the fresh-discovery and authenticated text procedure in [Authentication](./system-architecture-authentication.md#login-and-session-flow). Valid-route text `PLAY` currently returns `AUTH_UNAVAILABLE` at the Account gRPC boundary before membership evaluation; the downstream public-production `JOIN_REQUIRED` branch is conditional behavior exercised only with successful mocked Account responses. Local direct-text REALMS-scope/JOIN dispatch and Account RPC exist, but PostgreSQL, unknown-outcome recovery, lifecycle/generation rereads, and direct text-to-Logging end-to-end proof remain open. Direct text reconnect obtains a new transport-local scope from authenticated `REALMS`; it has no browser/WebSocket connect-token snapshot to reuse and must not select or attest a target from stale prior fields. `CHARS` or allowed character creation is conditional on no valid selected character being resolved. If smart-client attach hints return later, they should be re-sent as hidden MCP metadata on the new connection.
- Web clients reconnect by rerunning bootstrap discovery and, in the target when required, `Join & Play` plus realm-scoped `CHARS`/allowed character creation before obtaining a fresh connect token; they then open a new WebSocket to `/ws/game/**`, issue bare `LOGIN`, and issue `PLAY`. The local browser bootstrap join facade exists, while full first-party browser/mobile flow and proof remain incomplete. The target `Join & Play` and any required character repair therefore precede connect-token issuance and bare `LOGIN`. The token-backed WebSocket flow uses the fresh, short-lived discovery snapshot only for its selected edge target and discards it on expiry or scope mismatch. A valid snapshot is not a substitute for current `ACTIVE` membership or a valid character, and an invalid character must be repaired before token issuance rather than discovered after the socket opens. Web clients must not assume that any prior MCP or hidden smart-client attach metadata has survived, as described in [Mud Client Protocol (MCP) Support](./system-architecture-mud-client-protocol.md#reconnection--session-recovery).
...
For clarity, Telnet clients do not receive hidden recovery after their TCP socket or TCP Proxy-to-Gateway edge bridge is lost. Even if the proxy-wide admission circuit later returns to closed after such an edge outage, the affected Telnet client must reconnect on a fresh TCP socket and follow the applicable Authentication-owned `WORLDS`/credential-bearing `LOGIN`/authenticated `REALMS`/conditional `JOIN`/conditional `CHARS` or character-creation/`PLAY` flow; current valid-route text `PLAY` returns `AUTH_UNAVAILABLE` at the Account gRPC boundary before membership evaluation. The downstream `JOIN_REQUIRED` branch is conditional behavior exercised only with successful mocked Account responses. The local direct-text dispatch and Account JOIN RPC exist, but end-to-end proof remains open. This does not prevent Gateway from retaining a still-established proxy bridge while it performs ADR 0013's bounded rebind of its separate Game Session upstream.

Clients must also treat pre-disconnect input and output as non-resumable transport state. FireMUD does not replay client commands, raw transport bytes, prior WebSocket frames, unsent Telnet output, or partially delivered prompts onto a newly opened connection. A command durably admitted before edge loss remains tracked under its existing server-side command/effect identity and requires ordinary reconciliation; admission alone guarantees neither execution nor commit, and the client never recreates the input. Instead, after fresh `LOGIN` and `PLAY`, reconnect restores player-visible context in three distinct steps:

The close reason never establishes that command outcome. The current gameplay-command status surfaces are restricted to authorized operator or internal-service clients; an authorized client holding `{tenantId, gameInstanceId, commandId}` may reconcile that tuple, but ordinary players cannot use knowledge of `commandId` to query it. A future caller-bound first-party status route may expose the same authoritative record only after it binds the requested tuple to the immutable ingress account, character, or session identity required by the owning [command-outcome contract](./system-architecture-tick-execution-flows.md#command-outcome-status-surface-required). Until that route exists, ordinary browser and classic line clients receive later state reconstruction, must not treat any retryable close as proof that the last command failed, and must not recreate the input automatically.

- current implementation persists ordered `resume_transcript_entry` rows and can best-effort freshly render available retained context after a binding-checked eligible reconnect. The Unit 1B local projection no longer treats a first-party connect token by itself as replay authority after fresh `PLAY`, but it does not durably fence a terminated logout episode or prove post-logout race/retry safety. Target replay eligibility requires the exact current `resumeContextGeneration` and a non-terminated semantic-context episode; under that target, explicit gameplay `LOGOUT` durably terminates the episode before acknowledgement and prevents later replay of its private context. [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#canonical-resume-context-model) records the entry, cache, presentation, and current proof consequences;
...
The durable semantic recent-context, binding logout behavior, retention bounds, and Game Session persistence model are canonical in [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#canonical-resume-context-model) and [Session Behavior](./system-architecture-session-behavior.md#gameplay-logout-and-resume-transcript). This document retains the client-visible consequence: every first-party `LOGIN` + `PLAY` success obtains a fresh authoritative `LOOK`, regardless of whether bounded reconnect context is available; when retained context exists, it is rendered before that fresh `LOOK` and the owner-defined prompt result. Raw client input, prior bytes, frames, and unsent transport output are never replayed. The current implementation and integration proof do not yet establish this no-buffer first-party guarantee across the real admission path.
...
- `close_due_to_valid_upstream` – preserve every valid authenticated Gateway top-level close as the same Telnet token, including planned `service_restart`; bounded subreason context remains diagnostic only. A preserved top-level `policy_violation` maps to the client class `policy_violation_non_retriable` unless the sole established-protocol `CONNECT_SCOPE_MISMATCH` exception applies.
- `close_due_to_unattributed_loss` – absent or invalid authenticated bridge attribution, or missing or invalid authenticated Gateway top-level close metadata, closes the Telnet connection immediately with the observation-specific `backend_unavailable` fallback. The proxy does not keep the client TCP socket open while attempting a hidden gameplay-bridge reattach.
- `close_due_to_edge_backpressure` – if buffered lines exceed `TCP_PROXY_GATEWAY_MAX_BUFFERED_LINES` while upstream is still reachable, close that Telnet connection with top-level `policy_violation` and emit `edge_backpressure` as diagnostic context only; clients map it to `policy_violation_non_retriable`, and the diagnostic subreason never makes it retriable.
