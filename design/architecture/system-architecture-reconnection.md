...
- **Retry caps**
  ...
  - Client policy handling is split into explicit classes:
    ...
    - `policy_violation_non_retriable` (for example sustained abuse, malformed protocol outcomes, or `subreason=edge_backpressure`) – stop auto-retry or switch to very long backoff and surface corrective action to the user. Retry behavior keys only on the top-level `policy_violation` class; any subreason is diagnostic and never changes lifecycle or retry semantics.
...
- a bounded durable semantic recent-context window keyed by `{tenantId, playableStateNamespaceId, characterId}` is freshly rendered from retained structured entries after current authorization permits reconnect. It may repeat output the client already displayed, may omit output that was not retained or available, and is not a delivery acknowledgement, exact missed-message list, command history, or transcript archive. Redis may cache that context but is not authoritative. Explicit gameplay `LOGOUT` immediately suppresses private replay for the terminated binding; physical deletion may complete asynchronously, and later `LOGIN` + `PLAY` does not replay that terminated binding's context;
...
- then FireMUD obtains a fresh authoritative `LOOK` and emits exactly one reconnect prompt only when both effective `firemud.presentation.prompt.enabled` and `firemud.presentation.prompt.emit-after-reconnect-restore` are enabled; if either is disabled, it emits zero reconnect prompts. When enabled, duplicate-prevention still ensures one prompt. This reconstruction defines current state; retained context is narrative orientation only. See [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#prompt-behavior) for the owning precedence rule.
