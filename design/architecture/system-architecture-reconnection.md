...
The target admission contract above is distinct from confirmed current implementation drift: connect-token issuance and text `PLAY` now return `JOIN_REQUIRED` when required public-production membership is absent, but the explicit first-join action remains unimplemented. A client must not infer current membership or target identity from a stale Redis binding, cached tenant/realm fields, or an old `connectScopeId` after `CONNECT_SCOPE_MISMATCH`.
