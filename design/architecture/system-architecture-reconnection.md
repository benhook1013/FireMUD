...
Session entries in Redis expire after the absolute derived `session_expiration_ms` window (`FIREMUD_AUTH_JWT_EXPIRATION_MS + FIREMUD_AUTH_SESSION_SAFETY_MARGIN_MS`) documented in [Environment and Secrets](./infrastructure/environment-and-secrets.md#authentication). Disconnected resume is allowed only within the shorter effective `firemud.reconnection.policy.resume-window-ms` window and while that absolute session lifetime remains valid. Transcript retention under `firemud.reconnection.buffer.*` is independent and never extends resume eligibility.
