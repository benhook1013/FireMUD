...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect binding flow is `LOGIN` then `PLAY` only when confirmed existing `ACTIVE` membership and an already selected current character are both present. Otherwise the client completes lobby and character selection. A first public-production entry with missing or `INACTIVE` membership must evaluate fresh selected-target catalog/pointer evidence together with the Account-owned `allowPublicJoin` entitlement before explicit `JOIN`/`Join & Play` repair; denial returns `PUBLIC_PRODUCTION_ADMISSION_DENIED` without changing membership, while permission allows `JOIN_REQUIRED` and the repair operation to continue. The canonical membership lifecycle is missing -> `JOIN` creates `ACTIVE` and `INACTIVE` -> `JOIN` restores `ACTIVE`, with either transition advancing `membershipVersion`; `membershipAuthorityGeneration` advances independently only when `callerBoundAuthorityInvalidated=true`; `ACTIVE` returns the exact idempotent current snapshot and all other states reject. Character selection, connect-token issuance, and `PLAY` never create or restore membership. `JOIN_REQUIRED` is target-only until explicit join converges: the current first-party connect-token and text `PLAY` paths can still create or return implicit public-production membership through `EnsurePublicProductionPlayerMembership` when membership is absent, which is implementation drift. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
...
The target admission contract above is distinct from current implementation: `JOIN_REQUIRED` is target-only. Until explicit `JOIN` converges, Account's `EnsurePublicProductionPlayerMembership` operation remains the current implicit membership writer used by first-party connect-token issuance and text `PLAY` when public-production membership is absent; that behavior violates the target lifecycle, which removes the writer from both paths and returns `JOIN_REQUIRED` instead. A client must not infer current membership or target identity from a stale Redis binding, cached tenant/realm fields, or an old `connectScopeId` after `CONNECT_SCOPE_MISMATCH`.
