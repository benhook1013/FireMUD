...
- **Telnet and WebSocket parity** – Both transports share the same gameplay authentication and lobby-binding contract (`LOGIN`, optional first-time public-production `JOIN`, then `PLAY`) after an actual edge reconnect. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
