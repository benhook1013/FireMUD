...
- current implementation persists ordered `resume_transcript_entry` rows and can freshly render available retained context after current authorization permits reconnect. The target durable semantic recent-context window is namespace-scoped to `{tenantId, playableStateNamespaceId, characterId}`, uses complete-entry byte bounds with bounded oversize omission/marker handling, and suppresses private replay after explicit gameplay `LOGOUT`; these guarantees remain target-state, with current gaps recorded in [Input, Output, and Presentation Implemented Status](./system-architecture-input-output-and-presentation.md#implemented-status). The context may repeat output the client already displayed, may omit output that was not retained or available, and is not a delivery acknowledgement, exact missed-message list, command history, or transcript archive. Redis may cache that context but is not authoritative. Physical deletion may complete asynchronously, and later `LOGIN` + `PLAY` must not replay a terminated binding's context once the target logout suppression is implemented;
- new semantic recent-context entries retain structured player-output metadata alongside rendered protocol text when the output came from the structured `PlayerOutput` path, so first-party clients can receive typed context entries while classic clients continue to receive text;
- then current reconnect flow obtains a fresh authoritative `LOOK`; the target restoration contract emits exactly one reconnect prompt only when both effective `firemud.presentation.prompt.enabled` and `firemud.presentation.prompt.emit-after-reconnect-restore` settings are enabled, and zero when either is disabled. Prompt restoration and its duplicate-prevention proof remain target-state/current-gap details under [Input, Output, and Presentation Implemented Status](./system-architecture-input-output-and-presentation.md#implemented-status), and the effective settings caveat remains authoritative. This reconstruction defines current state; retained context is narrative orientation only. See [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#prompt-behavior) for the owning precedence rule.
...
- reconnect should restore semantic context first, then obtain a fresh authoritative `LOOK`, then emit exactly one fresh current prompt when both effective reconnect-prompt settings are enabled, or zero reconnect prompts when either is disabled;
...
Service-local typed properties provide operator defaults. `common-platform-core` owns the shared effective persisted-override resolver: it merges tenant overrides and then game-instance overrides into one bounded layer for ordinary domains. `reconnection.buffer` is the stable-namespace exception: its persisted overrides resolve tenant before `{tenantId, playableStateNamespaceId}` values so a replacement runtime inherits the bounds without copying them. Game Session consumes that merged layer and applies service-local operator defaults only as its local fallback; it does not resolve tenant/game override precedence. Because the soft and hard byte ceilings form one effective invariant while those operator defaults remain service-local, a tenant override that changes either ceiling must persist both values; a namespace override may set one ceiling only when the tenant layer supplies the complete pair. Tenant put and delete operations validate existing namespace reconnection overrides against the prospective tenant layer before changing it. The current resolver remains game-instance keyed and does not yet prove this stable-namespace path. Operator caps and presets remain future work. Prompt exclusion from semantic recent-context restoration remains a canonical reconnect/output rule rather than a separately surfaced toggle; prompt emission precedence is owned by [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#prompt-behavior).

### Canonical durable resume-context policy

The durable semantic recent-context, binding logout behavior, retention bounds, and Game Session persistence model are canonical in [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#canonical-resume-context-model) and [Session Behavior](./system-architecture-session-behavior.md#gameplay-logout-and-resume-transcript). This document retains the client-visible consequence: a new transport receives bounded context freshly rendered from retained structured entries, then a fresh authoritative `LOOK` and exactly one current prompt when both effective reconnect-prompt settings are enabled, or zero reconnect prompts when either is disabled, after the owner permits an eligible reconnect. Raw client input, prior bytes, frames, and unsent transport output are never replayed, and `LOGOUT` remains excluded from private context replay.
