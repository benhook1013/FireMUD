...
- **Telnet and WebSocket parity** – The target shared gameplay authentication and reconnect binding flow is `LOGIN` then `PLAY` for confirmed existing members; a first public-production entry must complete explicit `JOIN`/`Join & Play` first and receives `JOIN_REQUIRED` when required durable membership is absent. `JOIN_REQUIRED` is target-only until explicit join converges: the current first-party connect-token and text `PLAY` paths can create or return implicit public-production membership through `EnsurePublicProductionPlayerMembership` when membership is absent. First-party WebSocket clients must obtain a fresh connect token before opening `/ws/game/**`.
