...
- Emits a `NotifyDisconnect` event to the Game Session Service over an internal-only gRPC event sink as a best-effort, at-least-once hint when Telnet sockets close. Game Session must treat this stream as idempotent and advisory only, never as the sole source of truth for session liveness, and it keys consumption by `{proxyConnectionId, disconnectSequence}` so late or duplicate events are safe to ignore. The proxy retries delivery only for transport-level gRPC failures and only for a short, bounded window after socket close, governed by the `TCP_PROXY_NOTIFY_DISCONNECT_MAX_RETRY_MS` setting described in the TCP Proxy design’s **Service Interactions** section; logical contract-level failures reported via `NotifyDisconnectResponse.error` are logged and counted but not retried. Downstream liveness detection must not depend on eventual delivery. For the full canonical `NotifyDisconnect` semantics, including envelope context and retry behaviour, see the TCP Proxy Service design’s **Service Interactions** section.
