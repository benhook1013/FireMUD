...
> Spring Cloud Gateway restarts disconnect Telnet clients under the canonical two-path rule: planned drains map to `service_restart` when the deterministic bridge-drain close is received, while abrupt or unattributed bridge loss retains the immediate `backend_unavailable` fallback.
...
| Spring Cloud Gateway restart | Fleet-level behavior should stay largely invisible: unaffected sockets on other healthy Gateway instances remain up and new handshakes continue. Connections bound to the specific Gateway instance being drained or crashing are visible failures. Planned drains emit `1012/service_restart` for affected WebSocket sessions and Telnet `service_restart` through a clean bridge close. Graceful unplanned failures may emit `1011/internal_error`, while hard crashes may drop transport without a close frame; WebSocket clients treat missing close metadata as the `internal_error` retry class and must fetch a fresh connect token before reopening `/ws/game/**`. Abrupt unattributed bridge loss remains `backend_unavailable` for Telnet. |
...
- a bounded durable resume transcript keyed by the admitted tenant, playable-state namespace, and character identity replays retained semantic narrative/system context after successful `LOGIN` + `PLAY`; `gameInstanceId` may remain entry provenance but is not the durable scope, and Redis may cache the context but is not authoritative;
...
Every admitted `{tenantId, playableStateNamespaceId, characterId}` scope retains one durable, bounded resume transcript. This is short rolling semantic context, not an archive, delivery acknowledgement, or player-selected setting. The effective policy resolves from platform defaults with an optional tenant/game override:
...
New entries evict complete oldest retained entries when a byte bound is exceeded. The soft ceiling preserves configured message and line floors only where they fit beneath the absolute hard ceiling. Byte accounting uses the deterministic canonical structured-entry envelope defined in [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#resume-transcript-bounds), including structured replay metadata and rendered compatibility text exactly once. A single entry larger than the hard bound is omitted from reconnect retention or represented by a bounded omission marker; it never bypasses the ceiling. Inactivity expiry removes the whole context. After authorized `LOGIN` + `PLAY`, FireMUD replays retained semantic entries in ordering-token order before fresh `LOOK` and one current prompt. Explicit logout, character ownership transfer, or lost replay authorization clears or suppresses private context. A persistent RPG may use no inactivity expiry while still retaining only its configured recent screen window.
...
- `close_due_to_terminal_logout` – if the proxy receives `1000/logout` on the authenticated bridge path, it closes the Telnet session as terminal `logout`.
- `close_due_to_session_replaced` – if the proxy receives `1000/session_replaced`, it closes only the displaced Telnet transport as `session_replaced`.
- `close_due_to_service_restart` – if the proxy receives `1012/service_restart`, it closes the Telnet transport with the retryable `service_restart` reason.
