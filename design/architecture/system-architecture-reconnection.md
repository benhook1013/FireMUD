...
- current implementation persists ordered `resume_transcript_entry` rows and can best-effort freshly render available retained context after a binding-checked eligible reconnect. Target replay eligibility requires the exact current `resumeContextGeneration` and a non-terminated semantic-context episode; explicit gameplay `LOGOUT` durably terminates that episode before acknowledgement, so later admission cannot replay its private context. Current first-party post-logout suppression remains incomplete. [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#canonical-resume-context-model) records the entry, cache, presentation, and current proof consequences;
