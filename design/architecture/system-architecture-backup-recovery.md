...
6. Capture immutable `initialCatchupHighWater`, then capture immutable final-cutover `restoreHighWater`, complete gap-free erasure replay from `artifactErasureHighWater`, and complete offline participant convergence and epoch/fence reset. Reconcile Account's durable restore-cutover evidence for session and authority invalidation before coordination initialization; recovery does not perform that invalidation itself.
