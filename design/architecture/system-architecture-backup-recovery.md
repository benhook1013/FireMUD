...
The public recovery-control verb is `continueRecovery(operationId, expectedPhase, evidenceRef)`. It validates the immutable evidence named by `evidenceRef`, then continues one durable recovery operation through an atomic compare-and-set or transactional transition from `expectedPhase`. The controller records exactly one idempotent result per `operationId`; concurrent and retried calls return that result and cannot apply quarantine release twice. Its internal durable `pause/lock` phase holds quarantine and prevents conflicting work; recovery does not expose separate public `pause`, `resume`, `lock`, or `release-lock` verbs.
