...
- for an actual destructive PostgreSQL rewind into the authoritative player-facing target, before applying the operation the recovery controller must issue and durably bind `restoreAttemptId`, exact `targetBoundaryFingerprint`, selected `recoveryPointId`, controller-owned UTC `recoveryPointDecisionAt`, and computed non-negative integer-ceiling `effectiveDataLossWindow` to the recovery operation, target environment, and candidate artifact, plus exactly one immutable authorization branch from the existing recovery-point contract. The operator branch explicitly requires `recoveryPointApprovedBy`, `recoveryPointApprovalRef`, and `recoveryPointApprovalDigest`; the approval reference and digest must resolve to one immutable closed `recovery-point-approval/v1` record whose `recoveryPointId`, `recoveryPointDecisionAt`, and `effectiveDataLossWindow` exact-match the controller binding and approval branch, while the automatic-DR branch requires `automaticDrPolicyRef`. The complete controller binding therefore includes the five recovery tuple fields plus the selected branch fields; missing, duplicate, mutable, stale, or mismatched authorization fails closed. The branches are mutually exclusive, and the automatic-DR branch remains unavailable under the current baseline policy. The decision timestamp is established only after the old-authority fence and write cutoff are active and final candidate selection is durably fixed. `recoveryPointId` must resolve to exactly one immutable `verified-restorable-point/v1` record in the target environment's canonical `design/operations/deployments/<target-environment>/verified-restorable-points/` namespace; the concrete namespaces are `design/operations/deployments/production/verified-restorable-points/` and `design/operations/deployments/hobby-self-hosted/verified-restorable-points/` for their respective environments. The resolved record's `environment` must exact-match the target environment. The durable controller tuple must retain that record's exact `recordDigest`, nested `backupArtifact.artifactDigest`, and computed non-negative integer-ceiling `effectiveDataLossWindow` from the record's authoritative `backupArtifact.snapshotAt` to `recoveryPointDecisionAt`; the artifact digest must exact-match the candidate artifact digest and immutable artifact identity. Immediately before the destructive apply, the controller must reread the immutable record bytes, reject schema or duplicate-member changes, recompute and exact-match `recordDigest`, revalidate the environment/database/lineage/restore-tool bindings and point freshness, recompute the elapsed-seconds ceiling from the same `recoveryPointDecisionAt`, and exact-match both `recoveryPointDecisionAt` and `effectiveDataLossWindow` plus both digests and the artifact identity to the already-issued tuple. A mutable alias, object-existence check, caller-supplied digest, or a record that resolves to zero or multiple points is not sufficient; later no-write apply delay does not alter the authorized state-loss window. Before any artifact decompression or `psql` invocation for this actual rewind, the controller must verify the complete controller-issued binding and available target-bound authority. Non-authorizing per-artifact readability checks and isolated production-equivalent drills retain their existing artifact/tooling and isolated-boundary gates and do not require this actual-loss authorization. A future physical path must apply the same actual-rewind gate before a base backup or WAL replay. An unknown or ambiguous result keeps quarantine closed; matching the tuple is necessary but never sufficient. The controller must durably read back its controller-owned restore-state record and classify the effect as not started, completed, failed before apply, a specific resumable checkpoint, or unknown/partially applied. For the logical `psql` path and any future physical base-backup or WAL-replay path, continuation is allowed only from a controller-recorded resumable checkpoint or after controller-owned target reinitialization proves a clean target for a new attempt; an unknown or partially applied effect without one of those proofs has no retry authority. If the controller or target-bound authority/readback is unavailable, stale, contradictory, or ambiguous, fail closed without applying or retrying the destructive operation;
...
The destructive recovery-point choice requires operator authorization by default. Under the owner contract in [Backup Recovery Evidence and Compliance](./system-architecture-backup-recovery-evidence-and-compliance.md#canonical-recovery-record), the controller first fixes `recoveryPointDecisionAt` after old-authority fencing, the write cutoff, and final candidate selection; `effectiveDataLossWindow` is then the non-negative integer ceiling of elapsed seconds from the selected point's authoritative `backupArtifact.snapshotAt` to that decision timestamp. The operator's immutable approval record exact-binds `recoveryPointId`, `recoveryPointDecisionAt`, and `effectiveDataLossWindow`; a separately configured automatic-DR policy may pre-authorize the same exact point/decision/window only with strict old-authority fencing and candidate-selection proof. Later no-write apply delay does not change the authorized state-loss window, and this automatic baseline remains disabled.

### Logical Backup Scale and PITR Trigger

Online logical backups remain the initial hosted, hobby, and small-deployment baseline. Hobby/self-hosted operators may select a slower configured cadence, but a compliant online logical-backup posture still requires the `verifiedRestorablePointRef`, `verifiedRestorablePointDigest`, `verifiedRestorablePointSnapshotAt`, `verifiedRestorablePointNamespace`, and `verifiedRestorablePointEligibility` fields defined in [Backup Recovery Evidence and Compliance](./system-architecture-backup-recovery-evidence-and-compliance.md#hobby-backup-compliance-evidence). Those fields map to one immutable `verified-restorable-point/v1` record in the hobby namespace; the ref, digest, and snapshot timestamp must exact-match the record, while eligibility is recomputed against the current event's trusted `assessedAt` by the consumer. The selected `backupArtifact.snapshotAt` must be no older than 24 hours at that assessment, must not be future-dated, and must be the unique greatest eligible snapshot among the complete hobby point set; a slower schedule never waives that freshness gate and a missing, stale, ambiguous, or mismatched point makes the posture non-compliant or `recovery-unverified`. `lastSuccessfulBackupAt` is pipeline metadata and cannot substitute for the selected point snapshot. Operator status and recovery evidence must show the configured policy, effective RPO, and age of the selected verified restorable point.
...
3. For the actual destructive rewind into the authoritative target, obtain the controller-issued `restoreAttemptId`, exact `targetBoundaryFingerprint`, selected `recoveryPointId`, `recoveryPointDecisionAt`, and `effectiveDataLossWindow`, verify the target-bound authority and the single immutable operator-approval or automatic-DR authorization branch, including all branch fields, exact-bound to that controller tuple, and only then decompress the selected `.sql.gz` artifact and restore its plain SQL into the target PostgreSQL pod with `psql`. Non-authorizing per-artifact readability checks and isolated production-equivalent drills retain their existing gates and do not require actual-loss authorization. After an unknown or ambiguous result, keep quarantine closed; tuple or authorization matching alone does not authorize retry. Require controller-owned restore-state readback showing a known resumable checkpoint, or controller-owned target reinitialization proving a clean target before starting a new attempt; unavailable or contradictory controller authority or readback fails closed.
