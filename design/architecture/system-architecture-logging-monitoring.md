...
- **TCP Proxy metrics** capture Telnet DMZ behavior:
  ...
  - Safety and abuse meters such as `tcpproxy.telnet.discarded` (malformed Telnet negotiation, buffer overflows, or repeated malformed `SESSION` envelopes) and `tcpproxy.tls.misconfig` (TLS startup errors). These allow operators to see when legacy Telnet clients or misconfigurations are causing noisy connections at the DMZ boundary.
  ...
  Operators should create Grafana panels and Alertmanager rules that highlight sustained non-zero `tcpproxy.connections.limit.exceeded` and sharp increases in `tcpproxy.telnet.discarded`, since both typically indicate abusive clients, mis-tuned caps, or buggy Telnet tools.
