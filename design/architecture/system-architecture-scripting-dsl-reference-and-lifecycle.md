...
- **Automation queue** – an instance-aware, per-entity Redis queue (`automation:queue:{tenantInstanceTag}:<entityId>`) that holds derived indexes/pointers after durable pre-DSL trigger admission. It is reset-tolerant and rebuildable from the durable outbox; it must not be treated as an authoritative log of pending work or as the post-DSL script work-item collection.
