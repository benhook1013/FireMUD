...
- A script patch may be pinned for a game only after Automation has loaded and validated that exact patch for the tenant and marked it `READY` as part of the readiness lifecycle. An eligible non-empty notification reaches `READY` through the readiness workflow; a verified zero-onLoad-handler patch reaches terminal `READY` without a workflow or synthetic handler work, with its pinned manifest reference/digest and zero declared and terminal handler counts recorded and exposed. Workflow metadata is therefore optional and absent for the verified zero-onLoad-handler path; missing or unverifiable manifest proof cannot establish `READY`.
