...
- **Automation queue** – an instance-aware, per-entity Redis queue (`automation:queue:{tenantInstanceTag}:<entityId>`) populated only with a derived pointer when a durable pre-DSL trigger record commits in `PENDING_EVALUATION`. It is reset-tolerant and rebuildable only from eligible durable pre-DSL outbox records; it never contains post-DSL descriptors or script work items and is not an authoritative log of pending work.
...
| Runtime admission, schedule, work, and effect fencing | `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch>`; `playableStateScope` is separate exact-validated fence evidence | Automation and domain runtime owners | Admission, due-point, work-item, command, and effect fences | Resolved trigger scope, region ownership, epoch changes, reload/rollback barriers |
