...
Ingress admission and handler execution are intentionally distinct. The first accepted parent event resolves and sorts the complete binding-level candidate set, assigns every candidate its stable `handlerSequence`, and freezes that ordered manifest plus the applicable activation fences. Automation then derives the executable view from the frozen order. When several core candidates share one `scriptId`, the earliest candidate sequence is the script's sole execution position; that executable entry retains an ordered member list containing every coalesced candidate's distinct binding/scope identity and original `handlerSequence`, and each later candidate is marked as coalesced to that execution position rather than being omitted. Those retained later sequences are resolution and audit evidence, not additional effect-order positions. Plugin handlers are never coalesced and remain distinct by stable `bindingId` and captured `pluginActivationEpoch`.
