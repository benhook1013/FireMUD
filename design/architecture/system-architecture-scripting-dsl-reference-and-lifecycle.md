...
- **Automation execution loop** – the durable executor path inside the Automation & Scripting Service. **Target state:** it claims evaluated **script work items** from the outbox, evaluates no DSL again, and hands resulting commands to the Game Session Service so Game Session can enqueue **tick commands** into per-entity tick queues for later execution by game ticks. **Current implementation:** the readiness-guarded `ScriptOutboxQueueRebuildJob` scans a bounded durable batch of `PENDING_EVALUATION` and `EVALUATING` rows and republishes missing pre-DSL pointers; the executor drains and deletes those pointer queues, then its durable claim filters to `PENDING_EVALUATION`, so `EVALUATING` pointers are drained but never picked up, reclaimed, or re-enter the DSL. Because there is no stale-owner/CAS recovery, queue reset/rebuild/resume remains unavailable as an operational recovery path and must fail closed. Atomic owner-reconciled recovery of safe post-DSL `PENDING` work is target-only.
...
| 3 | **Script work item** | The target-state post-DSL, per-trigger evaluated command descriptor/outbox contains one immutable descriptor per emitted command, the applicable Trigger Identity defined in the [normative contract tables](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields), and version metadata. The pre-DSL trigger state is a separate durable role; current implementation boundaries are recorded by the runtime owner. | **Target state only:** evaluated work is indexed via `automation:queue:{tenantInstanceTag}:<entityId>` when it targets an entity and is later claimed by Automation's durable executor for Game Session handoff. **Current implementation:** that queue is only a derived pre-DSL parent-work pointer projection for `PENDING_EVALUATION`/`EVALUATING`, not an evaluated-command descriptor index. |
