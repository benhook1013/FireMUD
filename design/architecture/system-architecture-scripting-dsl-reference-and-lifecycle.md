...
This document is target-state canonical for DSL shape and lifecycle semantics. For each resolved handler admitted to durable execution after handler-scoped gates, one durable parent work item starts at `PENDING_EVALUATION`, transitions to `EXECUTING`, and atomically reaches `EVALUATED_COMMITTED` while gaining immutable evaluated command descriptors/child-dispatch rows; descriptor rows then have independent handoff statuses. A handler denied before work-item materialization keeps its handler audit outcome but creates no parent work item. Executor acceptance, the execution-start charge marker, and the fenced capacity lease commit before DSL evaluation, and recovery of a committed parent replays its descriptors without re-entering the DSL. This target flow is distinct from the current pre-DSL queue path: the live Automation claim boundary is `PENDING_EVALUATION` -> `EVALUATING`, and `automation:queue` contains only derived pre-DSL parent-work pointers; the target evaluated descriptor/child projection and lease/fencing-generation recovery remain implementation gaps. Current runtime boundaries are recorded in [Scripting Runtime Execution](./system-architecture-scripting-runtime-execution.md#current-implementation-status): bounded local multi-command staging is live, including local expansion of `targetEntityIds[]` into individually attempted single-command, single-target handoff calls. The live handoff request itself carries only one command and target and lacks complete durable child/target identity, scope validation, and end-to-end dedup/progress proof, so target-state multi-target fan-out remains incomplete. Durable parent-event identity, canonical request-digest conflict handling, and atomic freezing of the complete ordered handler manifest under [ADR 0172](./decisions/adr-0172-parent-event-and-frozen-handler-execution-identity.md) are also unimplemented. Canonical scripting audit and handoff diagnostics are defined by the [Scripting & Automation Observability Contract](./system-architecture-scripting-observability-contract.md); metric families, labels, and increment units are solely owned by [normative Table 4](./system-architecture-scripting-normative-contract-tables.md#table-4-metrics-label-matrix).
...
- **`{tenantRegionTag}` hash tag** – an opaque placeholder produced by shared Redis key builders from the complete `<tenantId, gameInstanceId, regionId>` scope documented in [Redis Architecture](./system-architecture-redis.md#key-naming-and-shard-discipline). It exists for shard locality across region-scoped keys; it is not a replacement for those identity fields and does not define a competing schedule or tenant key family. `playableStateScope` is intentionally not part of this existing tag topology; durable Trigger, scheduler, and command-handoff identities use `playableStateNamespaceId`, while scope remains immutable policy/routing/authorization/fence evidence and must not be inferred from the tag. Downstream mutation guards exact-validate applicable namespace/scope evidence separately from their own effect-guard uniqueness.
- **`{tenantInstanceTag}` hash tag** – an opaque placeholder produced by shared Redis key builders from `<tenantId, gameInstanceId>`. It scopes instance-wide automation projections: `automation:queue:{tenantInstanceTag}:<entityId>` is a Cache/Rate-Limit disposable dispatch-pointer index rebuilt from eligible durable pre-DSL records, while `remote:{tenantInstanceTag}:<entityId>` is a separate Coordination Redis wake-up hint. The tag must not be substituted for the region-scoped `{tenantRegionTag}` where a region timeline or lease fence applies.
...
- **Automation execution loop** – the durable executor path inside the Automation & Scripting Service. **Target state:** it claims committed descriptor children under a durable parent work item, evaluates no DSL again, and hands resulting commands to the Game Session Service so Game Session can enqueue **tick commands** into per-entity tick queues for later execution by game ticks. **Current implementation:** the scheduled queue-rebuild hook/configuration has been removed/disabled; the executor drains derived pointers and its durable claim filters to `PENDING_EVALUATION`, so `EVALUATING` pointers are not picked up, reclaimed, or re-entered into the DSL. Because there is no stale-owner/CAS recovery, queue reset/rebuild/resume remains unavailable and must fail closed; see [Scripting Runtime Execution: Current Queue-Rebuild Concurrency Gap](./system-architecture-scripting-runtime-execution.md#current-queue-rebuild-concurrency-gap). Atomic owner-reconciled recovery of safe post-DSL `PENDING` work is target-only.
- **Automation queue** – an instance-aware, per-entity Redis queue (`automation:queue:{tenantInstanceTag}:<entityId>`) that currently holds **derived pre-DSL parent-work indexes/pointers** for `PENDING_EVALUATION` and `EVALUATING` work, not an authoritative log of pending work. `PENDING_EVALUATION` pointers can be claimed for evaluation; `EVALUATING` pointers are inspection-only and cannot be picked up, reclaimed, or re-enter the DSL. The former rebuild hook/configuration is removed/disabled; queue reset/rebuild/resume remains unavailable and must fail closed. See [Scripting Runtime Execution: Current Queue-Rebuild Concurrency Gap](./system-architecture-scripting-runtime-execution.md#current-queue-rebuild-concurrency-gap). Target post-DSL descriptor recovery uses separate `PENDING`/`INDEXED` states and atomic owner-reconciled recovery of safe work; it is not implemented.
...
- **`runtimeStatus`** – the administrative lifecycle/readiness state of a script as seen by the scheduler (for example, `ENABLED`, `DISABLE_AFTER_DRAIN`, or `DISABLED`). `runtimeStatus` controls whether new triggers are accepted, drained, or skipped and is updated by hot reload flows and administrative actions. If `DISABLED_DUE_TO_ERRORS` is exposed for a core script, it is only a read-only effective-admission projection for the queried exact `(tenantId, gameInstanceId, scriptPatchVersion, scriptPinEpoch, scriptId)` breaker tuple; the persisted breaker state/history/reset owner is the exact-tuple `breakerState` aggregate.

### One DSL, Distinct Artifact and Lifecycle Roles

FireMUD has one component-based DSL, compiler, validator, sandbox, and execution runtime for game-authored automation. An embedded script is a game-owned DSL graph or handler entrypoint materialized in the Game Design revision model and released with an immutable game version or script-only patch; it follows that version's publication, tenant-readiness, pinning, and rollback lifecycle. A linked plugin is an immutable independently versioned bundle containing graphs in the same DSL, bindings, bounded configuration, and optional plugin-owned assets; it retains stable `pluginId` and exact `pluginVersionId` provenance and has its own instance-scoped enable, drain, disable, update, and rollback lifecycle. Plugin activation and updates are not folded into the Game Session script-patch pin tuple: each activation/update resolves a `PUBLISHED`, compatibility- and policy-admitted exact plugin/binding set, and runtime work carries the current Game Session `(scriptPatchVersion, scriptPinEpoch)` plus exact `(pluginId, pluginVersionId, bindingId)` provenance and the captured Automation-owned `(pluginActivationEpoch, lifecycleRevision)` pair as separate runtime-fence evidence. Neither lifecycle value is stable plugin identity, handler Trigger Identity, Command-Handoff Identity, stable schedule-row identity, continuity identity, or ordering identity. The scheduler may retain `pluginActivationEpoch` as a tagged candidate/firing-claim field where the owner table requires it; that tagged fence behavior does not make it a stable schedule identity, and `lifecycleRevision` remains fence evidence beside it rather than an identity field. Distribution source, signatures, marketplace status, or local-file provenance do not create a second language or a weaker sandbox.
...
| 2 | **DSL run** | Execution of a script handler in the sandboxed DSL for a single trigger. It evaluates the frozen inputs within the durable parent work item and produces domain commands, not direct state changes. | In-memory execution in the Automation & Scripting Service; results are committed to that parent and, at `EVALUATED_COMMITTED`, to immutable command descriptors. |
| 3 | **Script work item** | For each admitted resolved handler, one durable parent work item owns the applicable Trigger Identity and pre-DSL lifecycle; when it reaches `EVALUATED_COMMITTED`, it gains immutable evaluated command descriptor/child-dispatch rows, one per emitted command, with version metadata. A handler denied before work-item materialization has no parent work item. The descriptor rows have independent handoff statuses while the parent marker remains the recovery boundary. | **Target state only:** committed descriptor children are indexed via `automation:queue:{tenantInstanceTag}:<entityId>` when they target an entity and are later claimed by Automation's durable executor for Game Session handoff. **Current implementation:** that queue is only a derived pre-DSL parent-work pointer projection for `PENDING_EVALUATION`/`EVALUATING`, not an evaluated-command descriptor index. |
...
Triggers lead to durable parent work items for admitted handlers; their DSL runs execute within those parents and, after `EVALUATED_COMMITTED`, gain immutable descriptor children plus queue-pointer projection entries. Automation's execution loop turns those committed descriptors into tick commands for the Game Session Service.

This is an overview rather than an exhaustive lifecycle. **Target state only:** an identifiable readiness notification whose immutable manifest proves zero `onLoad` handlers takes a direct terminal readiness-record path with `status=READY` and readiness-level evidence; one whose zero-handler claim lacks verifiable immutable proof attempts a direct terminal readiness-record path with `status=FAILED` and `statusReason=zero_handler_manifest_unverifiable`, subject to the readiness terminalization CAS. If that CAS loses because the same readiness identity is already terminal (`READY`, `FAILED`, or `SUPERSEDED`), the later proof result is a no-op and cannot relabel the prior terminal outcome, including after a newer publish supersedes the candidate. Neither path creates synthetic `onLoad` handler work or a handler audit row. **Current implementation:** the notification wire has no immutable manifest-proof or handler-count fields; an empty `affectedScripts` notification returns without creating readiness, while the current projection's integer count path records `READY` with `statusReason=no_scripts_in_patch` and does not expose proof or declared/terminal counts.
...
- **Interaction with reloads and recovery**
  - The Automation & Scripting Service treats `onLoad` as **at-most-once per canonical readiness Trigger Identity**, even across process restarts and readiness-worker leadership changes. The readiness owner persists one readiness record per `<tenantId, scriptPatchVersion>` separately from the per-handler audit records; completed or terminalized load state is tracked in persistent metadata so that simply restarting a readiness worker instance does not re-fire a completed execution.
...
- The readiness worker persists one readiness record per `<tenantId, scriptPatchVersion>` with readiness-level status/outcome fields, separately from handler execution/audit records. Its deterministic handler ID generation, retry restriction, and absent runtime fields are defined in the [normative Trigger Identity table](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields). Publication/readiness generation is a separate fencing value, not an execution identity; after stale terminalization, only a newly republished immutable `scriptPatchVersion` may create a new onLoad identity.
- Each resolved `onLoad` run is recorded in `script_event_audit` with `eventType=onLoad`, `tenantId`, `scriptId`, and the target `scriptPatchVersion`; the [normative audit table](./system-architecture-scripting-normative-contract-tables.md#table-2-script_event_audit-stages-and-outcomes) defines its handler-scoped stage-aware outcome. Successful readiness updates the separate readiness record (`status=READY`, readiness-level outcome `readiness_success`) rather than a handler `finalOutcome` or tick handoff.
...
- If patch `P21` contains a patrol interval with stable key `{SCRIPT, patrol, patrol.main.v1, playableStateNamespaceId, ENTITY, guard-9}` and patch `P22` declares compatible continuity for that same key, the scheduler may preserve the row only after validating the prior `(scriptPatchVersion, scriptPinEpoch)` tuple and the exact target tuple, `source.isDryRun == target.isDryRun`, and typed cadence, interval-kind, target-binding, and playable-scope checks. It rewrites ownership, persists the target patch provenance and exact epoch, and applies the normative resume calculation before admission resumes; continuity does not require source and target tuples to be equal, and `scheduleSemanticsHash` is diagnostic evidence only.
- If either side omits continuity, explicitly resets it, changes the key/cadence/target, or authoritative reconciliation proves the target schedule absent, the scheduler atomically retires/tombstones P21, terminalizes any proven-displaced candidate under its original identity, and creates no P22 row, due state, firing claim, or `scriptEventId`. If target-definition or other required authority is unavailable, contradictory, or ambiguous, it retains P21 and its candidates/claims fenced and nonterminal; it does not tombstone or create replacement state until reconciliation. When a validated target definition exists on the reset/incompatible path, it creates P22 with fresh due state. A distinct combat-alert timer uses a new key and fresh state.
...
This ordering is stable across deployments so that the same admitted binding set produces the same command sequence for a given event. Each admitted handler, including a selected exclusive handler, receives a durable `handlerSequence` or equivalent stable ordinal before handler audit, durable work, generated-command, handoff, or application records are written; a resolved sibling excluded by exclusive selection is not admitted and receives no sequence. Those records preserve the admitted sequence plus handler-local command order; queue position, timestamps, row IDs, worker claims, and retry timing are not semantic ordering authority.

### Canonical Stable Handler-Order Identity

The DSL lifecycle owns one ordering-only projection, `canonicalStableHandlerOrderIdentity`, for a resolved handler. It is deliberately not the complete Trigger Identity defined in the [normative contract tables](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields) and must not be used as a substitute for trigger deduplication, execution fencing, or audit identity. The projection is materialized only after the complete handler set is resolved and uses tagged handler kinds and canonical normalized values:

- A core handler uses `("SCRIPT", canonical(scriptId), "CORE_BINDING_SCOPE", canonical(scopeKind), canonicalSelector?, canonicalAuthoredBindingIdentity?)`. `scopeKind` is required. `canonicalSelector` is the typed, canonical selector for the applicable authored scope when one exists, and `canonicalAuthoredBindingIdentity` is the stable authored binding identity when the authoring model provides one; at least one of those two trailing fields is required, and both are retained when applicable. The authored binding/scope model must make this trailing projection unique for each distinct binding: if two bindings share a scope kind and selector, their stable authored binding identity is required to disambiguate them. This makes `scriptId` plus the applicable authored binding/scope identity distinguish separate core bindings and scopes. A core binding that cannot produce that identity is invalid and fails closed rather than falling back to `scriptId`.
- A plugin handler uses `("PLUGIN", canonical(pluginId), canonical(pluginVersionId), canonical(bindingId))`. `bindingId` is the immutable manifest binding identity within that `pluginVersionId`; the captured `(pluginActivationEpoch, lifecycleRevision)` pair is runtime-fence evidence and is not part of this ordering identity.

Before comparison, Automation normalizes every resolved handler to the finite kind `SCRIPT` or `PLUGIN` and derives the handler-kind rank. The comparator is field-wise and lexicographic from left to right over `(orderIndex, normalized handler-kind rank, canonicalStableHandlerOrderIdentity)`. `orderIndex` is compared as an integer; the normalized rank uses the default `SCRIPT`-before-`PLUGIN` order or the explicitly configured operator policy rank. Identity tags use their fixed canonical enum order; canonical textual values use their normalized, locale-independent value, and typed selectors/identities use their canonical field order. At every optional tuple field, presence is compared before the value, with absent sorting before present; an absent field is never replaced by a sentinel. The comparator never consults arrival, queue, database-row, worker, timestamp, or retry order.

Automation rejects duplicate identical normalized total keys `(orderIndex, normalized handler-kind rank, canonicalStableHandlerOrderIdentity)` in one complete resolution before assigning any `handlerSequence`. No later tie-breaker may be invented.

Failures are isolated per non-exclusive executable handler. If one executable handler fails through quota denial, sandbox exception, compilation error, infrastructure failure, or another terminal outcome, Automation records that outcome and continues sibling processing. Later handler commands become eligible for ordered application after the earlier executable position reaches a terminal outcome; failure or an empty command set does not cancel siblings. Quota checks (`ScriptQuotaService`) remain per executable handler, including for an exclusive handler: each distinct plugin handler is checked separately, while coalesced core members receive one check at their elected executable position against the complete combined manifest cost rather than one charge per member.

Ingress admission and handler execution are intentionally distinct. The first accepted parent event resolves the complete matching binding set only far enough to apply the exclusive-binding rule above. Automation then forms the admitted set: either the one authorized exclusive binding or, when none is selected, the complete non-exclusive set. Only that admitted set is sorted, assigned stable `handlerSequence` values, and frozen with its applicable activation fences. A sibling excluded by exclusive selection never receives a sequence, manifest entry, work item, or executable position, and retry cannot restore or reorder it. Automation derives the executable view from this frozen admitted order. When several admitted core candidates share one `scriptId`, the earliest candidate sequence is the script's sole execution position; that executable entry retains an ordered member list containing every coalesced candidate's distinct binding/scope identity and original `handlerSequence`, and each later candidate is marked as coalesced to that execution position rather than being omitted. Those retained later sequences are resolution and audit evidence, not additional effect-order positions. Plugin handlers are never coalesced and remain distinct by stable `bindingId` and captured `pluginActivationEpoch`.

Retries reuse the exact frozen candidate order, coalescing membership, executable positions, and fences; they never resolve the current binding set, omit a previously resolved binding, or reorder the manifest. Changed-input reuse of the parent identity is an idempotency conflict. Endpoint ownership, handler identity, retry, and audit rules are defined in [ADR 0172](./decisions/adr-0172-parent-event-and-frozen-handler-execution-identity.md) and the [normative contract tables](./system-architecture-scripting-normative-contract-tables.md#table-1a-event-ingress-scripteventid-ownership-matrix). Locally, `admitted=true` means only that the request was accepted for handler resolution; per-handler outcomes remain asynchronous in `script_event_audit` and related status/event surfaces.

A coalesced core script receives one execution manifest assembled from the retained member slices in ascending original `handlerSequence`. Each slice remains keyed by its complete canonical binding/scope identity; distinct scope-qualified inputs remain distinct ordered entries rather than one scope silently winning. Byte-identical owner evidence for the same scope-qualified input may be stored once with all referring members, while contradictory values, versions, or digests for the same scope-qualified input fail admission before evaluation. The DSL receives that ordered, scope-tagged collection as one immutable input and executes once. Quota and sandbox admission are evaluated once for the executable core handler, but their input-size and cost calculation includes the complete combined manifest; duplicate binding resolution therefore neither receives a second execution allowance nor hides aggregate cost. Commands emitted by that execution carry the elected earliest `handlerSequence` plus handler-local command order. Later member sequences record a terminal coalesced outcome and emit no commands, so ordered application can advance without treating them as missing work.
...
- At most one binding in the complete resolved set for the full applicable event/Trigger scope may set `requiresExclusiveEvent=true`. For instance-bound events, that uniqueness scope includes `tenantId`, `gameInstanceId`, `playableStateNamespaceId`, `regionId`, `regionEpoch`, the concrete target identity, `eventType`, and `eventSchemaVersion` as applicable, plus the derived `sourceService` for custom or service-specific events; the server-derived `playableStateScope` is retained and exact-validated as immutable policy/routing/authorization/fence evidence, not as a uniqueness discriminator. Tenant-readiness `onLoad` uses its pre-instance-pin scope and intentionally omits instance, playable-state namespace/scope, region, and entity fields. Game Design validates publish-time-known conflicts and Automation revalidates the complete base-script plus active-plugin set at activation. Multiple or unauthorized exclusive claims fail closed; runtime does not select one by sort order.
...
- **Game Design Service**
  ...
  - Drives a **publication and readiness handoff** when a script version is published:
    ...
    - For an eligible non-empty handler notification, Automation & Scripting starts or reuses the durable Temporal `script-patch-readiness` workflow, which tracks validation and `onLoad` processing to terminal readiness without conflating publication with runtime admission. **Target state only:** when the immutable patch manifest verifiably declares zero `onLoad` handlers, Automation instead creates and exposes a terminal `READY` record carrying the pinned manifest reference/digest and zero declared and terminal handler counts, without a Temporal workflow or synthetic handler work. For an identifiable notification whose zero-handler claim lacks verifiable immutable proof, the readiness owner attempts to create and expose a durable `FAILED` record with a bounded `zero_handler_manifest_unverifiable` reason and readiness status/audit evidence, subject to the first-terminal readiness CAS; an already terminal `READY`, `FAILED`, or `SUPERSEDED` identity remains unchanged, including when a newer publish has superseded it. It does not silently reject or retry the DSL. **Current implementation:** the notification has no manifest-proof/count fields; an empty `affectedScripts` input creates no readiness, and the integer count path uses `READY`/`no_scripts_in_patch` without proof fields.
...
- **Automation & Scripting Service**
  - Owns the **compiled graph schema and runtime registry**: it stores compiled DSL graphs, per-tenant script metadata, and administrative runtime flags (`runtimeStatus`, quotas, priorities) in its own database. Core-script failure-rate breaker state/history/reset are owned separately by the exact-tuple breaker aggregate; a `DISABLED_DUE_TO_ERRORS` runtime projection is read-only.
...
- A script patch may be pinned for a game only after Automation has loaded and validated that exact patch for the tenant and marked it `READY` as part of the readiness lifecycle. An eligible non-empty notification reaches `READY` through the readiness workflow. **Target state only:** a verified zero-onLoad-handler patch reaches terminal `READY` without a workflow or synthetic handler work, with its pinned manifest reference/digest and zero declared and terminal handler counts recorded and exposed; missing or unverifiable manifest proof cannot establish `READY`. **Current implementation:** the wire cannot carry or expose that proof; empty `affectedScripts` creates no readiness, while the integer count path can record `READY`/`no_scripts_in_patch` without proof. Workflow metadata is therefore optional only in the target proof-based zero-handler path.
...
- Admission, timer scheduling, and runtime work/effect fencing must carry the complete runtime identity `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch>`, even if implementations batch internal work by tenant. `playableStateScope` remains separately exact-validated immutable policy/routing/authorization/fence evidence rather than a durable identity field. A non-region tenant/control-plane event must use an existing non-gameplay branch; it must not fabricate sentinel region fields or use a gameplay/runtime Trigger Identity. Pin observation and pin convergence are instance-wide control-plane state carrying the exact `<scriptPatchVersion, scriptPinEpoch>` tuple. Pin-transition admission pause for promotion and rollback is separate instance-wide control-plane state keyed by `<tenantId, gameInstanceId, controlPlaneRequestId>` with the existing admission epoch as state/fence; the exact pin tuple is convergence and execution-fence evidence, not pause identity. Neither control-plane state splits by `playableStateScope`. Plugin activation/configuration remains materialized per `<tenantId, gameInstanceId, pluginId>`; plugin trigger identity and plugin-owned derived schedule/work state carry `playableStateNamespaceId` as durable identity plus the effective `playableStateScope` as immutable fence evidence. Region and epoch fields remain required for gameplay/runtime timelines.
...
| Runtime pin observation | `<tenantId, gameInstanceId>`; current observed value/fence evidence is the exact `(scriptPatchVersion, scriptPinEpoch)` tuple | Game Session pin plus Automation observation | Observed exact tuple, `reloadState=RELOADING`, observed new tuple, `reloadState=FAILED`, scoped admission/convergence checkpoints | `SetPinnedScriptPatchVersion`, `RollbackScriptPatchVersion`, pin-change events, reload reconciliation, promotion and rollback pin-transition orchestration |
| Runtime admission, schedule, work, and effect fencing | Complete `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch>` runtime identity; `playableStateScope` is separate exact-validated fence evidence | Automation and domain runtime owners | Admission, due-point, work-item, command, and effect fences | Resolved trigger scope, region ownership, epoch changes, reload/rollback barriers |
| Plugin activation | `<tenantId, gameInstanceId, pluginId>` | Automation & Scripting runtime registry | Active, pending, draining, disabled plugin version state | Operator activation/disable/drain actions; plugin trigger-derived state retains `pluginVersionId`, `bindingId`, `playableStateNamespaceId`, and immutable `playableStateScope` fence evidence |
...
- `PENDING_VALIDATION` – the Game Design Service has published a script-only patch version and the Automation & Scripting Service has accepted an eligible non-empty handler notification with its compiled graphs and bindings, but `onLoad` initialization has not yet completed for the tenant.
- `ONLOAD_RUNNING` – `onLoad` handlers for an eligible non-empty patch notification are executing for the tenant. These at-most-once DSL executions are keyed by the full applicable onLoad identity, `<tenantId, scriptId, eventSchemaVersion, scriptPatchVersion, eventType=onLoad, scriptEventId, isDryRun=false>`; publication/readiness generation fences stale execution but is not an additional execution identity; only independently idempotent external infrastructure steps may be retried.
- `READY` – all declared `onLoad` handlers for the patch have completed successfully for the tenant, or a verifiable immutable patch manifest declares zero handlers and the readiness owner has recorded the pinned manifest reference/digest with zero declared and terminal handler counts. The readiness record uses status `READY` and readiness-level outcome `readiness_success` for the successful non-empty case; the zero-handler case records its manifest proof and zero counts without synthetic handler success. The patch is eligible for instance-scoped exact pinning by games in that tenant; `READY` does not create or mutate a tenant-wide pin, require all instances to switch, or replace an instance's observed tuple. Game Session may explicitly pin it as the `scriptPatchVersion` component for an individual instance, paired with a new `scriptPinEpoch`.
- `FAILED` – one or more `onLoad` handlers for the patch have failed for the tenant with a logical or sandbox error, an independently idempotent external infrastructure step has exhausted its bounded retries, or an identifiable zero-handler notification lacks verifiable immutable-manifest proof. The readiness owner durably records status `FAILED` with bounded `statusReason=zero_handler_manifest_unverifiable` (or the applicable handler/infrastructure reason) and emits the normal readiness status/audit evidence. The at-most-once DSL evaluation is not retried, and missing proof does not create synthetic handler work, a handler audit row, or a replacement readiness identity. The previous instance-observed pin remains in use for running games, and the failed patch is not eligible to be pinned.
...
Typical transitions for an eligible non-empty handler notification are:
...
4. `PENDING_VALIDATION|ONLOAD_RUNNING → SUPERSEDED` when a newer publish is accepted for the same tenant before the older patch reaches a terminal readiness state. `SUPERSEDED` is terminal and must be emitted before the newer patch begins readiness work. A patch already terminalized `FAILED` for unverifiable zero-handler proof remains visible as `FAILED`; a newer patch proceeds under its own readiness identity and does not relabel or retry the failed candidate.

A verifiable immutable manifest declaring zero `onLoad` handlers bypasses these `PENDING_VALIDATION` and `ONLOAD_RUNNING` transitions and the synthetic readiness workflow/handler work: the readiness owner creates and exposes the terminal readiness record with `status=READY`, the pinned manifest reference/digest, and zero declared and terminal handler counts. Missing or unverifiable proof for an identifiable tenant/patch notification uses the same first-terminal readiness CAS as every other terminalization: it writes readiness `status=FAILED` with bounded `statusReason=zero_handler_manifest_unverifiable` only while the row is non-terminal and its expected publication/readiness generation and immutable manifest identity still match. If a newer publish first wins that CAS to `SUPERSEDED`, the later proof failure is a no-op; no path relabels a terminal row or retries the failed candidate in place. Only a newer accepted patch uses a new readiness identity. An unidentifiable malformed notification may fail transport validation without inventing a readiness identity.

Every readiness terminalization uses one durable compare-and-set rule for the same `<tenantId, scriptPatchVersion>` readiness identity: `READY`, `FAILED`, or `SUPERSEDED` may be written only when the current row is still in a non-terminal state and its expected publication/readiness generation and immutable manifest identity match. The first committed terminal transition wins; later handler completion, proof failure, supersession, retries, or stale workflow callbacks must be no-ops that cannot overwrite the terminal status, counts, reason, manifest reference/digest, or audit evidence. A newer publish owns a distinct readiness identity (and may supersede the older row only by winning that row's same CAS), so no path can relabel a committed `READY` or `FAILED` record or reopen it.

Per-instance rollout state consists of Game Session's committed exact pin tuple `(scriptPatchVersion, scriptPinEpoch)` and its append-only history of successful and unsuccessful `SET`, `ROLLBACK`, and `REPIN` attempts keyed by `controlPlaneRequestId`; each history entry records previous and resulting exact pin tuples as nullable pairs, with each pair either fully present or fully absent (semantic `UNPINNED` only for a never-pinned instance, never a sentinel). An accepted deterministic validation/preparation failure records equal tuples without mutating the pin or advancing the epoch: a first-pin failure records absent -> absent, while a failure against an existing pin preserves that present tuple on both sides. The `SetPinnedScriptPatchVersion` and `RollbackScriptPatchVersion` APIs create the authoritative state/history attempts; `ScriptPatchPinChanged` is only a notification for successful committed tuple changes. An instance rollback does not imply tenant patch state transition away from `READY`.
...
- If the supplied `scriptPatchVersion` is `READY` for the tenant, the service compares the supplied exact `(scriptPatchVersion, scriptPinEpoch)` tuple to a fresh authoritative Game Session tuple for `<tenantId, gameInstanceId>`. Admission proceeds only when both fields match Game Session authority.
- If the instance's promotion or rollback owner workflow is in terminal `PIN_CONVERGENCE_TIMEOUT`, pre-handler admission fails closed with `admissionOutcome=TRIGGER_ADMISSION_OUTCOME_VERSION_UNAVAILABLE`, `admissionReason=pin_convergence_timeout`, and no `retryAfterMs`; no handler-scoped final outcome is created for this terminal gate.
...
For authoritative gameplay outputs and commands, the immutable deterministic input is the complete applicable Trigger Identity, the bounded event payload, the pinned script/artifact references, the durable handler-scoped input manifest, and the applicable tick context. The manifest is sealed for each executable resolved handler, including `onLoad`; a coalesced core handler's one manifest contains the ordered scope-tagged member slices defined above. It is reused byte-for-byte by retries and recovery. Logs and metrics are excluded from replay determinism and remain non-authoritative side channels. Command-level handoff identity and fan-out rules follow the [cross-service scripting contracts](./system-architecture-scripting-contracts.md#2-script-work-item-vs-tick-command-boundary).

### Read Consistency Contract

Determinism depends on one immutable **handler input manifest**, not on a universal snapshot token. The manifest is a durable, bounded, digest-bound record created for each executable resolved handler at admission. For a coalesced core handler, it also records the elected execution sequence and every ordered member's original sequence plus canonical binding/scope identity:
...
- `onLoad` receives a tenant-readiness manifest containing only configuration/runtime metadata and recomputable-cache versions, never mutable gameplay state. The target command-plan preview uses one caller-supplied authorized fenced snapshot or epoch for live gameplay facts, or declared fixtures that fully supply the simulated inputs. When live facts are requested, it rejects missing, unauthorized, stale, or substituted fences and never silently selects owner versions. The current legacy materialized dry-run path may server-select owner versions and record them in its returned/audited manifest; that behavior does not satisfy the ADR 0114 target preview contract.
...
handlerInputManifest/v1 {
  ...
  coalescedCoreMembers?: [{ handlerSequence, canonicalBindingScopeIdentity }],
  artifacts: [{ artifactId, artifactVersion, artifactDigest, componentCostRegistryDigest, artifactRuntimeCapDigest }],
  inputs: [{ bindingScopeIdentity, ownerService, schemaVersion, boundedValue, ownerVersion, ownerReadToken? }],
