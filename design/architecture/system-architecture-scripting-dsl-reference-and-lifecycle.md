...
- **Automation execution loop** – the durable executor path inside the Automation & Scripting Service. **Target state:** it claims committed descriptor children under a durable parent work item, evaluates no DSL again, and hands resulting commands to the Game Session Service so Game Session can enqueue **tick commands** into per-entity tick queues for later execution by game ticks. **Current implementation:** the readiness-guarded `ScriptOutboxQueueRebuildJob` scans a bounded durable batch of `PENDING_EVALUATION` and `EVALUATING` rows and republishes missing pre-DSL pointers; the executor drains and deletes those pointer queues, then its durable claim filters to `PENDING_EVALUATION`, so `EVALUATING` pointers are drained but never picked up, reclaimed, or re-enter the DSL. Because there is no stale-owner/CAS recovery, queue reset/rebuild/resume remains unavailable as an operational recovery path and must fail closed. Atomic owner-reconciled recovery of safe post-DSL `PENDING` work is target-only.
- **Automation queue** – an instance-aware, per-entity Redis queue (`automation:queue:{tenantInstanceTag}:<entityId>`) that currently holds **derived pre-DSL parent-work indexes/pointers** for `PENDING_EVALUATION` and `EVALUATING` work, not an authoritative log of pending work. `PENDING_EVALUATION` pointers can be claimed for evaluation; `EVALUATING` pointers are drained for inspection but filtered out by the durable claim and cannot be picked up, reclaimed, or re-enter the DSL. The guarded rebuild job is not stale-owner recovery, so queue reset/rebuild/resume remains unavailable as an operational recovery path and must fail closed. Target post-DSL descriptor recovery uses separate `PENDING`/`INDEXED` states and atomic owner-reconciled recovery of safe work; it is not implemented.
