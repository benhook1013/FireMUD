...
- At most one binding in the complete resolved set for the full applicable event/Trigger scope may set `requiresExclusiveEvent=true`. For instance-bound events, that scope includes `tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, the concrete target identity, `eventType`, and `eventSchemaVersion` as applicable, plus the derived `sourceService` for custom or service-specific events; tenant-readiness `onLoad` uses its pre-instance-pin scope and intentionally omits instance, playable-state, region, and entity fields. Game Design validates publish-time-known conflicts and Automation revalidates the complete base-script plus active-plugin set at activation. Multiple or unauthorized exclusive claims fail closed; runtime does not select one by sort order.
