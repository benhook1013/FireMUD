...
The comparator is field-wise and lexicographic from left to right over `(orderIndex, handlerType, canonicalStableHandlerOrderIdentity)`. `orderIndex` is compared as an integer. `handlerType` and all identity tags use their fixed canonical enum order (or the explicitly configured operator policy rank); canonical textual values use their normalized, locale-independent value, and typed selectors/identities use their canonical field order. At every optional tuple field, presence is compared before the value, with absent sorting before present; an absent field is never replaced by a sentinel. The comparator never consults arrival, queue, database-row, worker, timestamp, or retry order.

Automation rejects duplicate identical total keys `(orderIndex, handlerType, canonicalStableHandlerOrderIdentity)` in one complete resolution before assigning any `handlerSequence`. No later tie-breaker may be invented.
