...
- **Interaction with reloads and recovery**
  ...
  - A stale `ONLOAD_RUNNING` execution is fenced by its publication/readiness generation and terminalized by Automation's recovery owner as an audited `outcome_unknown` result. It blocks `READY`; the same execution is never re-entered, and a new publication or readiness generation is required.
...
For authoritative gameplay outputs and commands, the immutable deterministic input is the complete applicable Trigger Identity, the event payload, the pinned script graph/configuration, the authoritative `read_snapshot_token`, and the applicable tick context. The token is captured at admission and reused byte-for-byte; it is not silently derived from `scriptEventId`, a later tick, or a fresher read. Together these inputs fully determine authoritative gameplay behavior and emitted commands. Non-replayable logs and metrics are excluded from this determinism guarantee and may remain non-replayable as side channels. Command-level handoff identity and fan-out rules follow the [cross-service scripting contracts](./system-architecture-scripting-contracts.md#2-script-work-item-vs-tick-command-boundary).
