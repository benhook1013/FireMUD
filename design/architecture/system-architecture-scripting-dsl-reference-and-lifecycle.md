...
This document is target-state canonical for DSL shape and lifecycle semantics. The target pre-DSL trigger lifecycle is `PENDING_EVALUATION` -> `EXECUTING` -> `EVALUATED_COMMITTED`; executor acceptance, the execution-start charge marker, and the fenced capacity lease commit before DSL evaluation. Current runtime boundaries are recorded in [Scripting Runtime Execution](./system-architecture-scripting-runtime-execution.md#current-implementation-status): bounded local multi-command staging is live, including local expansion of `targetEntityIds[]` into individually attempted single-command, single-target handoff calls. The live handoff request itself carries only one command and target and lacks complete durable child/target identity, scope validation, and end-to-end dedup/progress proof, so target-state multi-target fan-out remains incomplete. The current Automation claim boundary is `PENDING_EVALUATION` to `EVALUATING`; lease/fencing-generation recovery and the separate evaluated descriptor/outbox boundary remain implementation gaps, while their target behavior is owned by the runtime document. Durable parent-event identity, canonical request-digest conflict handling, and atomic freezing of the complete ordered handler manifest under [ADR 0172](./decisions/adr-0172-parent-event-and-frozen-handler-execution-identity.md) are also unimplemented. Canonical scripting audit and handoff diagnostics are defined by the [Scripting & Automation Observability Contract](./system-architecture-scripting-observability-contract.md); metric families, labels, and increment units are solely owned by [normative Table 4](./system-architecture-scripting-normative-contract-tables.md#table-4-metrics-label-matrix).
...
- **Automation execution loop** – the durable executor path inside the Automation & Scripting Service. It claims persisted pre-DSL trigger records from the outbox, evaluates the current command-emission format, durably commits any post-DSL script work-item descriptors, and hands resulting commands to the Game Session Service so Game Session can enqueue **tick commands** into per-entity tick queues for later execution by game ticks. `automation:queue:{tenantInstanceTag}:<entityId>` remains a best-effort derived pointer index for visibility, not an authoritative execution log; current queue reset/rebuild/resume is unavailable and must fail closed, while atomic owner-reconciled recovery of safe `PENDING` work is target-only.
- **Automation queue** – an instance-aware, per-entity Redis queue (`automation:queue:{tenantInstanceTag}:<entityId>`) that currently holds **derived pre-DSL parent-work indexes/pointers** for `PENDING_EVALUATION` and `EVALUATING` work, not an authoritative log of pending work. `PENDING_EVALUATION` pointers may be picked up for evaluation; `EVALUATING` pointers are inspection-only and cannot be picked up, reclaimed, or re-enter the DSL. Current reset/rebuild/resume is unavailable and must fail closed. Target post-DSL descriptor recovery uses separate `PENDING`/`INDEXED` states and atomic owner-reconciled recovery of safe work; it is not implemented.
...
- **Game Design Service**
  ...
  - Drives a **publication and readiness handoff** when a script version is published:
    ...
    - For an eligible non-empty handler notification, Automation & Scripting starts or reuses the durable Temporal `script-patch-readiness` workflow, which tracks validation and `onLoad` processing to terminal readiness without conflating publication with runtime admission. When the immutable patch manifest verifiably declares zero `onLoad` handlers, Automation instead creates and exposes a terminal `READY` record carrying the pinned manifest reference/digest and zero declared and terminal handler counts, without a Temporal workflow or synthetic handler work. For an identifiable notification whose zero-handler claim lacks verifiable immutable proof, the readiness owner creates and exposes a durable `FAILED` record with a bounded `zero_handler_manifest_unverifiable` reason and readiness status/audit evidence; it does not silently reject or retry the DSL.
...
- A script patch may be pinned for a game only after Automation has loaded and validated that exact patch for the tenant and marked it `READY` as part of the readiness lifecycle. An eligible non-empty notification reaches `READY` through the readiness workflow; a manifest-proven zero-handler patch reaches terminal `READY` without a workflow or synthetic handler work, with its pinned manifest reference/digest and zero declared and terminal handler counts recorded and exposed. Workflow metadata is therefore optional and absent for the zero-handler path; missing or unverifiable manifest proof cannot establish `READY`.
