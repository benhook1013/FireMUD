...
- A plugin handler uses `("PLUGIN", canonical(pluginId), canonical(pluginVersionId), canonical(bindingId))`. `bindingId` is the immutable manifest binding identity within that `pluginVersionId`; `pluginActivationEpoch` is runtime-fence evidence and is not part of this ordering identity.
