...
- If the supplied `scriptPatchVersion` is `READY` for the tenant, or is tenant-`SUPERSEDED` but remains the existing exact Game Session pin with its immutable artifact and applicable local runtime fence intact, the service compares the supplied exact `(scriptPatchVersion, scriptPinEpoch)` tuple to a fresh-enough observed Game Session tuple for `<tenantId, gameInstanceId>`. Admission proceeds only when both fields match Game Session authority. The existing-pin exception permits continued execution only; it does not make a `SUPERSEDED` patch eligible for a new pin or pin progression.
...
- If the patch is unknown or in a non-ready state (`PENDING_VALIDATION`, `ONLOAD_RUNNING`, `FAILED`, or `SUPERSEDED`) and is not the exact `SUPERSEDED` Game Session pin covered by the exception above, the trigger is rejected at admission with `version_unavailable` (or a specific bounded reason such as `onload_failed`). Pre-resolution rejections are recorded in ingress audit; handler-scoped rejections use `script_event_audit.finalStage=ADMISSION`. A drop metric like `automation_script_triggers_dropped_total{reason="version_unavailable"}` is incremented.
