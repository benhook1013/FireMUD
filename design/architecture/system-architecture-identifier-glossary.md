...
- `scriptPatchVersion` – identifies an immutable embedded-script patch independently of instance selection. Game Session selects that patch for an instance by pairing it with the Game Session-owned `scriptPinEpoch`; the pair, not the version alone, is authoritative for instance-bound execution. Tenant-readiness `onLoad` may use a declared candidate patch in its pre-instance-pin readiness identity.
- `scriptPinEpoch` – identifies the Game Session pin-selection epoch for one `(tenantId, gameInstanceId)`, paired with `scriptPatchVersion`. It advances on every pin change, including repin/rollback to the same version; a version-only match is not an execution fence.
