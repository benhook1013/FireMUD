...
- Intentional legacy numeric-as-string contracts are narrow, explicit migration exceptions and do not relax the UUID rule for UUID-governed identifiers. The Logging & Admin game-session-pin reads are one such exception: REST path values and the internal Game Session pin-read request fields plus convergence response fields use positive decimal text, while the convergence DTO's current JSON-number representation remains an unsafe implementation gap. That convergence representation must migrate to decimal strings for JavaScript-safe exactness; see [Logging & Admin API Contracts](./microservices/logging-admin-service/api-contracts.md). This is tracked migration status, not canonical permission to expose other UUID-governed identifiers as decimal strings.
