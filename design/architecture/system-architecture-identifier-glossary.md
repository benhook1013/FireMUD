...
- `playtestLifecycleId` – the immutable Game Session/coordinator-owned identity for one playtest lifecycle in the exact tenant/world/realm scope. It is allocated fresh for every new lifecycle and is never reused; a reset within that lifecycle retains the lifecycle ID while replacing the `playableStateNamespaceId` and advancing to a fresh playtest-state generation. It is distinct from the durable state namespace, the monotonic generation, the replaceable runtime `gameInstanceId`, and the Account-owned per-grant `grantRevision`; it is carried as lifecycle and authority evidence, not used as an Entity durable key.
