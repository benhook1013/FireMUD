...
- `characterId` – identifies a character owned within a tenant. Target gameplay-session uniqueness uses `{tenantId, playableStateNamespaceId, playableStateScope, characterId}`; `gameInstanceId` is active-runtime fence/value evidence. Explicitly instance-local state may use `gameInstanceId` only under its owning service's declared scope and lifecycle contract.
