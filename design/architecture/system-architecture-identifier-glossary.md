...
- `playableStateNamespaceId` – identifies the durable player/entity state namespace that survives replacement. Allocation and uniqueness are tenant-scoped; realm policy selects which tenant-scoped namespace record applies (shared, isolated, or playtest) but is not an additional allocation-uniqueness dimension. The server-derived `playableStateScope` is a separate policy classification. The namespace remains stable while a logical playable lifecycle continues and is distinct from the replaceable `gameInstanceId`. A shared production realm uses one tenant namespace, an isolated realm retains its own stable namespace, and each new playtest lifecycle receives a new namespace.
