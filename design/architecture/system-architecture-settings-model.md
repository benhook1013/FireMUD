...
- Game Session exposes the current effective result at `/actuator/settings/effective`, including resolved `presentation`, `prompts`, `reconnection`, `movement`, and `worldTopology`, plus normalized subgroup views for the live room-view/transcript seams (`transcriptRendering`, `reconnectionPolicy`, and `reconnectBuffer`) and the current scoped `communication` override layer it sees for the same session or synthesized scope.
...
- Game Session exposes the resolved `presentation`, `prompts`, `reconnection`, `movement`, and `worldTopology` result through `/actuator/settings/effective`, and also includes the current scoped `communication` override view for the same session or synthesized scope, while Game Logic exposes the fully merged effective `communication` result through `/actuator/settings/effective/communication`;
