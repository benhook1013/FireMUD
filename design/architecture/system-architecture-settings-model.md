...
- `reconnection.buffer`
  ...
  - The current `gameInstanceId`-based resolver merges `softMaxBytes` and `hardMaxBytes` independently at each persisted tenant/game-instance layer and rejects that layer only when the resulting effective pair has `hardMaxBytes < softMaxBytes`; a partial persisted override can therefore inherit the omitted ceiling when the resulting pair is valid. It does not require a layer changing either ceiling to carry the complete pair atomically. The target `{tenantId, playableStateNamespaceId}` namespace override remains target-only until the resolver/envelope migrates; its invariant must follow that namespace across runtime replacement rather than being keyed by `gameInstanceId`. Because operator defaults remain service-local, a tenant override changing either ceiling must persist the complete pair. A namespace child may omit one ceiling only when the tenant parent supplies the complete pair, inherits the omitted member from that parent, and validates the resulting pair before persistence. Prospective tenant mutation validates existing namespace children against the prospective parent.
