...
- Current `SettingsAuthorityService` validation is domain-specific only for `reconnection`, `commandHistory`, and `commandCapabilities`. Other accepted domain payloads can persist without schema-bound validation, and consumers may normalize invalid values to defaults; this is current implementation status, not a change to the target rule that invalid writes are rejected.
...
- `reconnection.buffer`
  - tenant/game override inputs are surfaced today over service-local operator defaults, but they are only partial configuration inputs; they do not prove strict complete-envelope live retention or resource-bound enforcement. Retention never grants resume or replay authority.
  - The current `gameInstanceId`-based resolver merges `softMaxBytes` and `hardMaxBytes` independently at each persisted tenant/game-instance layer and rejects that layer only when the resulting effective pair has `hardMaxBytes < softMaxBytes`; a partial tenant override can therefore inherit the omitted global peer when the resulting pair is valid. It does not require a current layer changing either ceiling to carry the complete pair atomically. The target `{tenantId, playableStateNamespaceId}` namespace override and its complete-pair mutation rule remain target-only until the resolver/envelope migrates; that invariant must follow the namespace across runtime replacement rather than being keyed by `gameInstanceId`. In the target model, a namespace child may omit one ceiling only when the tenant parent supplies the complete pair, inherits the omitted member from that parent, and validates the resulting pair before persistence. Prospective tenant mutation validates existing namespace children against the prospective parent.
...
- Current `common-platform-core` resolution uses one merged persisted override layer per `{tenantId, optional gameInstanceId}` for every surfaced domain, including `reconnection.buffer`, applying tenant overrides before optional game-instance overrides;
- Target-only `reconnection.buffer` resolution migrates the child scope to `{tenantId, playableStateNamespaceId}` so runtime replacement inherits its bounds. Until the resolver and envelope implement that scope, callers must not present, infer, or substitute the unavailable stable-namespace result as current behavior;
