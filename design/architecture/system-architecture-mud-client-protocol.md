...
Classic-client semantic extension support is experimental, disabled, unadvertised, and unimplemented. MCP 2.1, `mcp-negotiate`, `mcp-cord`, GMCP, Telnet option packages, and FireMUD-specific package names are research candidates rather than supported contracts. Existing marker recognition, a server greeting, configuration, or parser tests do not establish interoperability or an advertised compatibility promise. The current TCP Proxy still recognizes MCP-looking markers and, when `TCP_PROXY_MCP_ENABLED` is enabled, emits an `#$#mcp version:2.1` greeting; this is implementation drift, not supported MCP. Supported deployments must keep that path disabled, unadvertised, and fail closed until an accepted Game Session-owned adapter has end-to-end proof.
...
- A new Telnet connection performs fresh anonymous `WORLDS` discovery before the applicable credential-bearing `LOGIN`, authenticated realm/character steps, conditional public `JOIN`, `PLAY`, and fresh state output. The TCP Proxy does not reuse browser bootstrap state or prior transport-local scope.
...
Current implementation evidence is insufficient for any classic-client extension support claim. TCP Proxy may contain disabled-by-default marker recognition or a `TCP_PROXY_MCP_ENABLED` greeting seam, but that is an implementation gap against this ownership boundary, not compliant negotiation or package proof. The seam must remain disabled, unadvertised, and fail closed until a Game Session-owned adapter is accepted and proven. There is no accepted current-client compatibility matrix or end-to-end proof for a stable extension/version pair.
