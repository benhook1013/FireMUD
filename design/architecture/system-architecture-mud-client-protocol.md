...
- **Direct-text/Telnet clients:** after a new connection, complete fresh public `WORLDS` discovery, credential-bearing `LOGIN`, and authenticated `REALMS`. For a public-production target, `JOIN` is conditional: it is permitted only when the current policy permits public joining and membership is missing or `INACTIVE`; otherwise the join attempt returns `PUBLIC_PRODUCTION_ADMISSION_DENIED` and must not mutate membership. Private/playtest targets do not use `JOIN`; they require existing `ACTIVE` membership and the current realm grant. Continue with realm-scoped `CHARS` or allowed character creation when no valid character is already selected, then `PLAY` and a fresh `LOOK`. Current text runtime may stop with `JOIN_REQUIRED` while explicit join remains unavailable, but it must not create membership implicitly.
- **First-party `/ws/game/**` clients:** complete the first-party connect-token bootstrap and owner-defined discovery. `Join & Play` is permitted only for a public-production target whose current policy permits public joining; otherwise it returns `PUBLIC_PRODUCTION_ADMISSION_DENIED` without mutating membership. Private/playtest targets require existing `ACTIVE` membership and the current realm grant and do not use `Join & Play`. Complete any required character prerequisites, open a new `/ws/game/**` socket, then issue bare `LOGIN` followed by `PLAY`; clients must not replay credential-bearing text `LOGIN` after bootstrap has been re-established.
