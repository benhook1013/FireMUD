...
1. The Account Service emits a membership-change event containing `accountId`, `tenantId`, `membershipVersion`, complete applicable `authorityTuple`, positive `issuanceFence`, `outboxStreamKey`, `outboxSequence`, the changed role set, and whether gameplay admission remains allowed.
...
- Consumers maintain an additional outbox watermark per exact `outboxStreamKey`. A duplicate at or below that watermark is a no-op only when its event identity/digest matches; the next sequence is contiguous; a higher sequence is a gap only within that same stream; and an unrelated issuer, account, tenant, membership, or grant stream never creates a gap. A same-key gap or conflicting duplicate stops affected admission/reconnect and requires Account reconciliation of the exact checkpoint before processing resumes.
...
- Current entitlement authority is fresh for a new binding. Only the entitlement-freshness predicate for resume of the same still-resumable binding may use an eligible positive last-known-good entitlement snapshot no older than five minutes when refresh is unavailable; the snapshot must be authoritative for the same target and authority tuple. Issuer, account, tenant, membership, private-realm grant, cutoff, issuance-fence, revocation, and both independent membership predicates must remain current and may never be supplied by this entitlement cache. New joins, fresh bindings, expansion, target changes, and any uncertain, missing, stale, negative, revoked, or gapped authority fail closed.

For every new binding or reconnect/resume, Account issues an exact-binding admission lease only after validating current lifecycle, billing, `authorityTuple`, `issuanceFence`, separate `membershipVersion`, token-identity fence, and security authority; grace continuity uses the exact-resume-episode `resumeActivationLease` specialization. Each lease has a unique lease ID, a monotonic fence, the immutable `authorityTuple`, `issuanceFence`, exact `outboxStreamKey`/`outboxSequence` checkpoint, and a bounded absolute expiry no longer than the admission SLA; it cannot be renewed to extend an admission attempt. Game Session's binding CAS includes the complete tuple, `issuanceFence`, separate membership version, lease fence, and cutoff checkpoint and may publish only a provisional record. For grace resume that record is `RESUME_PENDING`; it is never admissible, resumable, or takeover-eligible until Account durably finalizes the exact lease, binding, authority tuple, fence, and resume-episode identity as `COMMITTED`. Account finalization, abort, and reconciliation are idempotent by lease ID and request ID. If an issuer, account, tenant, membership, grant, billing, token-identity, or security cutoff fences the lease before finalization, the local CAS or late finalization fails closed and the provisional record is removed or remains blocked. If finalization commits first, the binding was admitted under the then-current authority and a later cutoff follows the bounded active-revocation path. If the binding CAS or response is lost, reconciliation removes or quarantines orphan bindings, indexes, and any token issued for the attempt; it must not infer admission from local presence. Missing, expired, fenced, ambiguous, or uncommitted lease state never admits a binding. This is an ordered idempotent cross-service protocol, not a cross-store atomic transaction.

Resume validation must not depend on the previous private player-delegation token remaining valid. After a fresh successful `LOGIN`, Game Session must request and obtain a fresh `game-session-account-delegation` token issued by Account; Game Session never mints this token and Account is the sole JWT issuer. Only after the exact activation lease is `COMMITTED` may Game Session atomically replace stored `authTokenHash`, `authTokenIssuedAt`, `authTokenExpiresAt`, `tokenGeneration`, `issuanceFence`, and `rebindHandle`, replace the complete `authorityTuple`, update separate `membershipVersion`, consume the current disconnection episode, and transition the binding from `RESUME_PENDING` to admitted. A binding CAS that loses the lease fence or otherwise fails must not leave the replacement token usable; Game Session must invoke Account's idempotent retire/abort path for the orphan token before reporting resume failure. The new token's expiration, complete authority tuple, issuance fence, and replacement rebind handle must be persisted before resumed backend calls use the replacement. Resume is rejected for any failed validation above, including subject mismatch, stale or lost gameplay membership or grant, expired or non-resumable gameplay state, an expired resume window, a changed uniqueness key, or revoked issuer, account, tenant, cutoff, or membership authority state. The fresh token's validity remains bounded by its own `exp`; obtaining it does not extend `continuityBindingExpiresAt` or the current episode's `resumeDeadline`.
