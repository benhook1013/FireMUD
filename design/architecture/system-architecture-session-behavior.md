...
Rebind-handle consumption uses a crash-safe two-party protocol. Before calling Account, Game Session durably records a `PENDING` rebind attempt containing a stable `rebindRequestId`, canonical request digest, gameplay-binding identity, current envelope identity, and token-identity fence. Account uses one durable atomic transaction or equivalent compare-and-set to consume the single-use handle, record that exact request tuple, and commit a bounded encrypted response escrow for the authorized Game Session workload containing the replacement backend token and next rebind handle until installation acknowledgement or expiry. Handle consumption cannot commit unless the matching escrow is durable. The escrow is the only permitted durable raw-token representation: it is encrypted with Account-owned application encryption, readable only by the exact authorized Game Session workload and request/binding tuple, never logged, traced, metered, or exported, expires at the bounded acknowledgement/retry deadline, and is deleted after installation acknowledgement, terminal abort, or expiry. Before installation acknowledgement, an exact retry returns the escrowed recorded response. After acknowledgement, replay returns only stored completed-installation metadata and never the raw token; reuse of the request ID with a different digest or binding always fails closed.

Game Session does not claim a transaction across its durable attempt store, Coordination Redis binding, and Account. It uses an ordered fenced installation state machine instead. The binding CAS first writes a non-admissible `INSTALLING` candidate containing the replacement token identity, generation, membership metadata, token-identity and installation-lease fences, next envelope, `rebindRequestId`, and request digest. After exact readback, Game Session durably advances the matching attempt from `PENDING` to `BINDING_INSTALLED`; a crash in either direction is reconciled by comparing the attempt and binding identities and retrying the same Account escrow result before acknowledgement. Game Session then sends the idempotent installation acknowledgement. Only after Account records that exact installation as committed may a second fenced binding CAS mark the candidate `INSTALLED` and admissible for new backend calls. A stale fence, mismatched tuple, expired escrow or lease, ambiguous acknowledgement, or unrecoverable write leaves the candidate non-admissible and invokes Account's idempotent abort/retire path. A crash before acknowledgement resumes with the same request identity without consuming another handle or minting another replacement. After acknowledgement, a restarted owner that lost the process-local JWT treats replay as metadata-only success and uses the persisted next `rebindHandleEnvelope` with a new request identity to establish a fresh replacement; it never asks Account to replay the retired raw token. Game Session never durably stores the raw delegation JWT. Expired, unavailable, or ambiguous escrow without a recorded installation acknowledgement fails closed and requires fresh login or a new authorized recovery path; identity fields alone never substitute for the handle.
...
For every new binding or reconnect/resume, Account issues an exact-binding admission lease only after validating current lifecycle, billing, membership, grant, and security authority; grace continuity uses the exact-resume-episode `resumeActivationLease` specialization. Each lease has a unique lease ID, a monotonic fence, an immutable authority tuple/cutoff checkpoint, and a bounded absolute expiry no longer than the admission SLA; it cannot be renewed to extend an admission attempt. Game Session's binding CAS includes the lease fence and cutoff checkpoint and may publish only a provisional record. For grace resume that record is `RESUME_PENDING`; it is never admissible, resumable, or takeover-eligible until Account durably finalizes the exact lease, binding, and resume-episode identity as `COMMITTED`. Account finalization, abort, and reconciliation are idempotent by lease ID and request ID. If a membership, grant, billing, or security cutoff fences the lease before finalization, the local CAS or late finalization fails closed and the provisional record is removed or remains blocked. If finalization commits first, the binding was admitted under the then-current authority and a later cutoff follows the bounded active-revocation path. If the binding CAS or response is lost, reconciliation removes or quarantines orphan bindings, indexes, and any token issued for the attempt; it must not infer admission from local presence. Missing, expired, fenced, ambiguous, or uncommitted lease state never admits a binding. This is an ordered idempotent cross-service protocol, not a cross-store atomic transaction.
