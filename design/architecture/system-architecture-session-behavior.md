...
Target control-plane logout for admin and creator UIs is defined as explicit Account Service APIs, with the gameplay-connect edge step ordered before the Account logout step when a connected gameplay credential is being ended:
...
For per-token logout, a gameplay-connected client first asks Gateway to atomically record the dedicated deny marker for the known gameplay-connect `jti` and clear the connect-token cookie. This is edge-only revocation, not Account logout or mutation of the ordinary token registry. The client then calls `POST /auth/logout` with the current JWT in the `Authorization` header and JSON body `{ "requestId": "<high-entropy caller-generated id>" }`. Account derives the authenticated account, exact token hash/`jti`, and token profile from the validated bearer token; the client does not send `accountId`, `tokenHash`, or `requestDigest`. Account computes the immutable versioned request digest from `TOKEN_LOGOUT`, the authenticated account, exact token profile, and presented token hash. A retry after an ambiguous response reuses the same bearer token and `requestId`; Account requires the recomputed digest, profile, token identity, and account to match the stored operation exactly. Reusing the request ID with changed meaning conflicts without mutation, while a genuinely new logout attempt uses a new request ID. The Account Service then:

- Computes the `tokenHash` from the presented JWT and, in one Account transaction, commits a durable `PENDING` operation keyed by `(accountId, requestId)` plus a new exact-token `PENDING_LOGOUT` fence version bound to the immutable request digest and exact token identity before touching the ordinary `session:auth:token:<tokenHash>` registry. The `(accountId, requestId)` key is unique; a retry with the same digest/profile/token replays the stored outcome, while reuse with a different digest, profile, token, account, or operation conflicts and cannot mutate Redis.
