...
| Game Session Service | `PLAY` | `gameplay_admission` | **Target:** requires an existing caller-bound membership plus any non-public realm grant, current entitlements, and the current admissible realm pointer; missing public-game membership returns `JOIN_REQUIRED`, and `PLAY` never creates it. **Current drift:** connect-token issuance and text `PLAY` may still invoke the membership writer implicitly. First-party `/ws/game/**` also enforces connect-context scope, and unavailable/ambiguous routing returns `ADMISSION_POINTER_UNAVAILABLE`; a deliberately closed realm returns `REALM_UNAVAILABLE`. |
