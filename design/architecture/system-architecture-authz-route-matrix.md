...
| Game Session Service | `POST /sessions/{sessionId}/stop` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, and `privileged_control_when_global_role` assurance; canonical external ingress is Logging & Admin |
| Game Session Service | `POST /sessions/{sessionId}/restart` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, and `privileged_control_when_global_role` assurance; canonical external ingress is Logging & Admin |
| Game Session Service | `POST /sessions/{sessionId}/refresh-roles` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, and `privileged_control_when_global_role` assurance; canonical external ingress is Logging & Admin |
