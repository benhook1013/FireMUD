...
| `public_production_onboarding` | No JWT for in-band gameplay commands; otherwise the exact route-declared profile (`player-bootstrap` for Account bootstrap writes) | No tenant authority generation before join; membership authority generation applies after join | **Target:** discovery and explicit open-enrollment join for the default public production realm. Brand-new authenticated accounts may discover it before membership exists, but `JOIN`/`Join & Play` creates the durable Account-owned membership before character creation, connect-token issuance, or `PLAY`. The Gateway WebSocket's one-use `gameplay-connect` profile belongs to `gameplay_admission`, not onboarding. **Current drift:** connect-token issuance and text `PLAY` can still create public-production membership implicitly and must converge on `JOIN_REQUIRED`. Grant-backed private/playtest realms validate their grant and any separately required existing membership, skip `JOIN`, and never use this class to create membership. |
| `gameplay_admission` | No JWT for in-band `PLAY`; WebSocket upgrade uses the exact one-use `gameplay-connect` profile or the separately classified trusted-proxy mTLS path | Target-tenant authority generation and membership authority generation: Yes through the exact-binding admission lease; tenant billing cutoff evidence remains operation-specific | Authenticated transition into gameplay after discovery/onboarding. It requires an existing admissible membership, applicable realm grant, current entitlement and routing evidence, and exact selected-target binding. It never creates membership and is distinct from public-production onboarding. |
...
| `billing_safe_tenant` | One matching token record for the exact profile declared by the route | Tenant authority generation: No; membership authority generation: Yes; independent `membershipVersion`: Yes | Must remain reachable during `suspended`/`canceled`, but must fail immediately after caller-bound membership/role revocation. `membershipVersion` is an independent freshness input and cannot be inferred from the authority generation |
...
| Game Session Service | `CHARS` | `public_production_onboarding` | Requires an existing caller-bound `ACTIVE` membership plus any non-public realm grant and current entitlements. It resolves characters for the selected catalog realm but does not apply `PLAY`'s `admission_pointer` live-admission check; character discovery and creation do not require an existing character. Missing or `INACTIVE` public-game membership returns `JOIN_REQUIRED`; missing or non-`ACTIVE` private/playtest membership returns `NON_PUBLIC_ENROLLMENT_REQUIRED`, and character discovery is not membership repair. |
| Game Session Service | `PLAY` | `gameplay_admission` | **Target:** requires an existing caller-bound membership plus any non-public realm grant, current entitlements, and the current admissible realm pointer; `PUBLIC_PRODUCTION_ADMISSION_DENIED` takes precedence when public-production policy or selected-target admission fails, and only an admissible target with missing public-game membership returns `JOIN_REQUIRED`; `PLAY` never creates membership. **Current drift:** connect-token issuance and text `PLAY` may still invoke the membership writer implicitly. First-party `/ws/game/**` also enforces connect-context scope, and unavailable/ambiguous routing returns `ADMISSION_POINTER_UNAVAILABLE`; a deliberately closed realm returns `REALM_UNAVAILABLE`. |
