...
| Account Service | `EnterPrivilegedControlWindow` | `account_scoped` | Exact `control-ui` profile plus recent ordinary reauthentication and independent TOTP; this route creates the bounded role-scoped window and does not require an existing elevation window |
...
| Account Service | `GET /tenant-admin/tenants/{tenantId}/export` (`ExportTenantData`) | `tenant_regular` | Live `tenantAdmin` membership and target tenant-generation freshness for the path tenant; tenant-wide tenant-owned export only, with no account subject selector. |
