...
| Logging & Admin | `POST /feature-flags/toggle`, `POST /moderation/actions`, `POST /tick-remediation/pause`, `POST /tick-remediation/resume` | `tenant_regular` operator ingress | Exact `control-ui` profile and current operator role. Tenant-role branches require live membership and membership-generation freshness; the global `platformAdmin` branch uses target-tenant generation without target membership. The moderation owner-enforcement call remains coverage drift. |
| Logging & Admin | `POST /admission-pointers`, `POST /admission-pointers/cutover`, `POST /admission-pointers/version-upgrades` | `tenant_regular` operator ingress | Exact `control-ui` profile and current operator role. Tenant-role branches require live membership and membership-generation freshness; the global `platformAdmin` branch uses target-tenant generation without target membership. |
| Game Session Service | `ToggleFeatureFlag`, `PauseTicksForScope`, `ResumeTicksForScope`, `SetAdmissionPointer`, `PrepareVersionUpgrade`, `ExecutePreparedVersionCutover` | `internal_workload` mirrored owner RPCs | Exact Logging & Admin mTLS workload identity plus an Account-redeemed bounded operator authorization reference; no end-user JWT. Quota override and owner-side moderation enforcement remain coverage drift, not invented routes. |
| Game Session Service | `StartSession` / `RestartSession` / `StopSession` | `internal_workload` operator RPCs | Exact Logging & Admin mTLS workload identity plus an Account-redeemed bounded operator authorization reference; no end-user JWT |
| Game Session Service | `RefreshRoles` | `internal_workload` | Exact allowlisted workload identity plus current session and Account role state; no delegated operator authorization reference |
| Game Session Service | `POST /sessions` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, and `privileged_control_when_global_role` assurance; tenant billing and membership authority-generation applicability are explicit in the YAML entry; canonical external ingress is Logging & Admin |
| Game Session Service | `POST /sessions/{sessionId}/stop` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, `privileged_control_when_global_role` assurance, and explicit tenant plus membership generation applicability; canonical external ingress is Logging & Admin |
| Game Session Service | `POST /sessions/{sessionId}/restart` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, `privileged_control_when_global_role` assurance, and explicit tenant plus membership generation applicability; canonical external ingress is Logging & Admin |
| Game Session Service | `POST /sessions/{sessionId}/refresh-roles` | `tenant_regular` | Exact `control-ui` profile, `tenantAdmin` or `platformAdmin`, `privileged_control_when_global_role` assurance, and explicit tenant plus membership generation applicability; canonical external ingress is Logging & Admin |
...
| Account Service | `GetProfile` / `UpdateProfile` (`/tenants/{tenantId}/profiles/{accountId}`) | `tenant_regular` | Exact `control-ui` profile and live tenant membership. `player`, `moderator`, and `designer` are self-only and bind `accountId` to the caller; `tenantAdmin` may target another profile only within the caller's tenant membership scope. No `platformAdmin` override exists unless a route entry explicitly declares one |
...
| Account Service | `LinkExternalAccount` (`POST /accounts/{accountId}/external`) | `account_scoped` | Target-only and not currently routable; caller account subject plus recent reauthentication, verified provider authorization, and no tenant-role or `platformAdmin` override. The current tenant-scoped caller-asserted scaffold is implementation drift |
