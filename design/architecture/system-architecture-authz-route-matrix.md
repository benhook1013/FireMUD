...
| Account Service | `AuthLogout` / `AuthLogoutAll` | `account_scoped` | Accepts `control-ui` (`audience: control-ui`) or `player-bootstrap` (`audience: player-bootstrap`) with registry-backed authenticated account scope. Bounded retries may return no-op success only for an absent exact token record (`AuthLogout`) or durable proof that a prior logout-all superseded the token (`AuthLogoutAll`); the retry exception grants no reusable authorization context |
