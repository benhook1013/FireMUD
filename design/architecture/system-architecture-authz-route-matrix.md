...
| `gameplay_admission` | No JWT for in-band `PLAY`; WebSocket upgrade uses the exact one-use `gameplay-connect` profile or the separately classified trusted-proxy mTLS path | Membership authority generation: Yes for selected-target admission; tenant billing authority generation remains operation-specific | Authenticated transition into gameplay after discovery/onboarding. It requires an existing admissible membership, applicable realm grant, current entitlement and routing evidence, and exact selected-target binding. It never creates membership and is distinct from public-production onboarding. |
