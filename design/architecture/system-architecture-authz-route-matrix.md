...
- `mutation_contract` such as `shared_instrument_ack_required`; when the owning payment-instrument route is not yet routable, the entry must mark this as target-only rather than claiming an implemented acknowledgement path
...
| Account Service | `GetProfile` / `UpdateProfile` (`/tenants/{tenantId}/profiles/{accountId}`) | `tenant_regular` | Exact `control-ui` profile, live tenant membership, and one of `player`, `moderator`, `designer`, or `tenantAdmin` in `scopedRoles[tenantId]`; `accountId` is bound to `caller_account_id`; no `platformAdmin` override |
...
| Account Service | `BillingArtifactsTenant` | `billing_safe_tenant` | Target-only, not currently routable; `tenantAdmin` (tenant-scoped), with `shared_instrument_ack_required` when a mutation affects an account-wide payment instrument |
