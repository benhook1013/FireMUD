...
| Game Session Service | `CHARS` | `public_production_onboarding` | Requires an existing caller-bound `ACTIVE` membership plus any non-public realm grant and current entitlements. It resolves characters for the selected catalog realm but does not apply `PLAY`'s `admission_pointer` live-admission check. Missing or `INACTIVE` public-game membership returns `JOIN_REQUIRED`; missing or non-`ACTIVE` private/playtest membership returns `NON_PUBLIC_ENROLLMENT_REQUIRED`, and character discovery is not membership repair. |
