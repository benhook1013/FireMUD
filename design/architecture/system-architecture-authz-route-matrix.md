...
| Game Session Service | `REALMS` | `public_production_onboarding` | Conditional branch: public production remains pre-membership discovery; private/playtest realms require caller-bound `membershipLifecycleState=ACTIVE` plus a current Account-owned realm-access grant and never use public discovery to repair enrollment |
...
| Spring Cloud Gateway | `/ws/game/**` (connect-token bootstrap) | `gameplay_admission` | Applies to the current public non-proxy WebSocket bootstrap for every first-party protected-cookie client, including browser/mobile-browser and native or other nonbrowser Account clients using a protected cookie jar. Its applicability is `connection_mode: first_party_web` plus `operation: websocket_upgrade`; tokenless public WebSocket support does not exist. The dedicated `X-Firemud-Connect-Token` non-browser route is target-only/unavailable until separately registered and proven, so it is not current `first_party_web` support. Gateway consumes exactly one `gameplay-connect` token with audience `gameplay-connect`, strips its carrier, atomically enforces single use using the dedicated replay record/fence, matches the signed connect scope to the request, and emits the signed connect context for Game Session. Non-`101` outcomes use the bounded `CONNECT_TOKEN_MISSING`, `CONNECT_TOKEN_EXPIRED`, `CONNECT_TOKEN_REPLAYED`, `CONNECT_SCOPE_MISMATCH`, `CONNECT_REPLAY_PROTECTION_UNAVAILABLE`, `CONNECT_TOKEN_REJECTED`, `POLICY_DENY`, `POLICY_PRESSURE`, `BACKEND_UNAVAILABLE`, `PROTOCOL_MISMATCH`, or `INTERNAL_ERROR` handshake classes. |
