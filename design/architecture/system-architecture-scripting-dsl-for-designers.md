...
- **Where to look when debugging**
  ...
  - For **safe test runs**, the ADR 0114 command-plan preview endpoint is the target-state path for the Game Design / Logging & Admin UIs, but it is currently unimplemented and unavailable; do not imply that users can call a target endpoint today. Once implemented, it will be a separate non-committing path that exercises the same sandbox and validation logic without live work or tick-queue handoff. The current `TriggerScriptEvent(isDryRun=true)` path is a legacy materialized dry run: it retains the shared sandbox and validation limits, authorization, separate live/test Trigger Identity namespace, and dedicated rate and budget controls, but it does not prove the target preview's caller-supplied fenced snapshot or fixture provenance, isolated preview result/audit and capacity guarantees, exact command-plan response, or zero-live-work boundary. Test tools are **privileged** and subject to their own rate limits and budgets so they cannot overload the scripting cluster; avoid running unbounded batches of previews against production tenants.
