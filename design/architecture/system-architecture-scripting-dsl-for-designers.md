...
- **Where to look when debugging**
  ...
  - When working with support or operators, you can use `scriptEventId` as a “debug ticket” for a single trigger, but never use it alone. The ticket must carry the complete applicable [Trigger Identity](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields): `tenantId`, `gameInstanceId`, `scriptId`, `eventType`, `eventSchemaVersion`, `scriptPatchVersion`, `scriptEventId`, and `isDryRun`; include `playableStateScope`, `regionId`, `regionEpoch`, and `entityId` for gameplay/runtime triggers; `pluginId`, `pluginVersionId`, and `bindingId` for resolved plugin handlers; and `triggerMode`, `scheduleDefinitionId`, `targetScopeType`/`targetScopeId`, plus exactly one applicable due point (`dueTickId` or `dueAt`) for scheduler triggers. Include command-level `outboxWorkItemId`, `automationDispatchId`, and `commandOrdinal` when a handoff child is in scope; those command-level fields are target-state until the Game Session handoff carries them end to end.
