...
- Browser clients cannot read failed WebSocket upgrade headers. Before the socket opens, they therefore apply Gateway's conservative generic recovery rule with a single fresh-token retry budget per failed-upgrade episode: treat the connect token as potentially consumed, discard the failed discovery/connect-token bundle, rerun bootstrap discovery, obtain one fresh token, and retry once with bounded backoff. If that retry fails, the budget is exhausted; stop automatic recovery and surface a recovery/configuration action rather than refreshing or fast-looping. Detailed bounded classes remain available to capable non-browser callers and operator telemetry rather than becoming a second browser status API.
...
For target browser onboarding and reconnect, the realm-scoped character gate remains in the sequence: first-time public entry, or a returning flow whose selected character is missing, invalid, or no longer visible for the resolved realm, must complete `CHARS`/allowed character creation after target-only `ACTIVE` membership and before `POST /auth/connect-token` and WebSocket `PLAY`. Current runtime has no lifecycle-state predicate: its current admission fields are `membershipExists` and `gameplayAdmissionAllowed`. `CHARS` and character creation require the applicable membership/grant and entitlement checks but do not require an existing character; a character becomes mandatory only before connect-token issuance and `PLAY`. **Target-only returning shortcut:** the shortcut is valid only after a fresh same-target entitlement check and the applicable current Account-owned realm-grant check have passed before `POST /auth/connect-token` issuance, with target `ACTIVE` membership, an unexpired complete discovery snapshot, and a valid current character all matching the selected target. Current runtime validates pointer/visibility for this flow but not the private/playtest realm grant, so the current shortcut and any resulting token are not authorization proof. `POST /auth/connect-token` may be the final target gate for those checks, but it must obtain a fresh Account snapshot and validate them before issuing a token; it must not rely on cached discovery or token-carried grant evidence. If the exact current private/playtest realm grant is unavailable, target private/playtest onboarding fails closed. A connect token carries a short-lived discovery snapshot for edge admission; it does not bypass `CHARS` or turn stale character evidence into gameplay authority.
