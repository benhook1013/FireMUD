...
- Disconnect handling is **layered**: the proxy cleans up Telnet sessions; Spring Cloud Gateway retains the client or proxy-facing gameplay WebSocket while performing ADR 0013's bounded rebind of a rebindable Game Session upstream, then closes with `1013/backend_unavailable` if recovery exhausts its window; and Game Session revalidates continuation authorization, binding, revocation, and target authority under the [Authentication](./system-architecture-authentication.md#login-and-session-flow) and [Gateway architecture](./system-architecture-gateway.md#tenant-aware-edge-connect-token-gameplay-handshake) contracts and, after current `LOGIN` identity checks and `PLAY` binding checks, currently presents/restores available retained semantic context on a best-effort basis in canonical order, then performs a fresh authoritative `LOOK`. Structured status/MCP state and any applicable prompt are additional projections, not alternatives to that `LOOK`. Empty or expired retained context produces no context output. The target namespace-scoped context, complete-envelope byte bounds and oversize omission handling, and post-logout replay suppression remain the shared [Input, Output, and Presentation](./system-architecture-input-output-and-presentation.md#implemented-status) contract's target guarantees; its Implemented Status records the current gaps. Target authorization of retained context is reserved until post-logout replay suppression is implemented and proved. Shared state presence never authorizes continuation. A retained-edge upstream rebind is not an actual edge disconnect and does not require `LOGIN`, `JOIN`, or `PLAY`. After an actual edge disconnect, clients follow [Reconnection Strategy](./system-architecture-reconnection.md#client-reconnection-behaviour) and the Authentication-owned admission sequence; this bridge does not duplicate JOIN or membership ordering. The Telnet path does not support hidden reattachment after its distinct Proxy -> Gateway gameplay WebSocket is lost: the proxy closes that Telnet connection rather than silently opening a fresh edge bridge behind the same client TCP socket.
...
- After a successful reconnect and `PLAY`, Game Session currently presents/restores an available bounded per-player retained-context window on a binding-checked, best-effort basis in canonical order, followed by a mandatory fresh authoritative `LOOK` derived from current state for the new transport. An applicable prompt, structured status, or newly generated MCP state is an additional projection, not an alternative to `LOOK`. Empty or expired context produces no context output. The target namespace scope, complete-envelope byte bound and oversize omission handling, and post-logout suppression are recorded with their current gaps in [Input, Output, and Presentation Implemented Status](./system-architecture-input-output-and-presentation.md#implemented-status).
