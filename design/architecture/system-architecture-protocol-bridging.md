...
This design is the **canonical specification** for gameplay command flows through the edge: it defines ordering and delivery guarantees, backpressure and slow-client behaviour, Telnet and WebSocket reconnection and buffering rules, and the Telnet disconnect reason taxonomy. Service-specific designs such as the TCP Proxy Service README describe implementation details and configuration but must remain consistent with the invariants in this document. The target first public-production text flow is `LOGIN` -> conditional `JOIN` -> `PLAY`; returning members skip `JOIN`. Any current direct `LOGIN` -> `PLAY` path that creates membership implicitly is implementation drift.
...
- All clients still require in-band `LOGIN`, conditional `JOIN` for first public-production membership, and `PLAY` before gameplay commands. Returning members skip `JOIN`; the current direct `LOGIN` -> `PLAY` implicit-membership path is implementation drift.
...
- **Edge termination plus internal PROXY mode** – The public Telnet edge terminates client TLS and forwards plaintext Telnet plus a PROXY header over an authenticated, cryptographically protected edge-to-TCP Proxy channel (mTLS or equivalent) to the internal-only `TCP_PROXY_PROXY_PROTOCOL_PORT`. `TCP_PROXY_TLS_ENABLED=false` for this mode; TCP Proxy does not terminate client TLS on that listener, and raw and PROXY-protocol ports are not public.
...
- In both modes, prove Proxy -> Gateway uses the internal `wss://` mTLS listener and that the bridged client reaches the same `LOGIN -> conditional JOIN -> PLAY -> LOOK` flow as a WebSocket client; returning members skip `JOIN`.
...
- **Telnet → Web takeover**
  ...
  - Later, a Web client connects via WebSocket to `/ws/game/**` and successfully issues `LOGIN` plus the conditional `JOIN` step when membership is absent, then `PLAY` for the same character.
...
- **Web → Telnet takeover**
  - A Web client connects via `/ws/game/**`, issues `LOGIN`, takes the conditional `JOIN` step when membership is absent, and enters gameplay with `PLAY` for a character.
