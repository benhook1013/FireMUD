...
This design is the **canonical specification** for gameplay command flows through the edge: it defines ordering and delivery guarantees, backpressure and slow-client behaviour, Telnet and WebSocket reconnection and buffering rules, and the Telnet disconnect reason taxonomy. Service-specific designs such as the TCP Proxy Service README describe implementation details and configuration but must remain consistent with the invariants in this document. **Target flow:** first public-production credential-bearing text entry is canonical Account email `LOGIN <email> <secret>` (or the supported email-code challenge) -> conditional explicit in-band `JOIN` -> realm-scoped `CHARS`/character creation -> `PLAY`; `LOGIN` establishes only socket/account authentication and `PLAY` performs gameplay binding. Only that public-production join may create missing or restore `INACTIVE` durable player membership. Returning members may use the abbreviated selected-character path and skip `JOIN` only with an unexpired complete discovery snapshot, current `ACTIVE` membership, and a valid current character. Browser and other first-party WebSocket clients use HTTP `Join & Play` before connect-token issuance, then complete bare in-band `LOGIN` -> `PLAY` without an in-band `JOIN`. Grant-backed private/playtest entry requires existing `membershipLifecycleState=ACTIVE` membership and the current grant, skips `JOIN`, and never auto-creates membership.

## Implementation Status

The target flow and delivery contracts below remain normative. Explicit `JOIN`/`Join & Play` and the required exact `membershipAuthorityGeneration`, `membershipLifecycleState`, and `membershipVersion` reread at connect-token issuance are both unimplemented/proof gaps in the current runtime. The target `JOIN_REQUIRED` outcome remains canonical, but current connect-token issuance and text `PLAY` may still invoke Account's `EnsurePublicProductionPlayerMembership` and implicitly create missing public-production membership until convergence. Returning active members and grant-backed private/playtest flows are not disabled by the missing public join action, but remain available only when their own current membership, grant, entitlement, routing, and membership-generation/version checks pass. This implementation drift does not relax the at-most-once delivery, authentication, admission, reconnect, or close-taxonomy requirements.
...
- Game Session restart should ideally be absorbed behind the established edge connection using shared state and backend rebind, as described in [Reconnection Strategy](./system-architecture-reconnection.md). Spring Cloud Gateway remains the edge socket owner, so a client whose live WebSocket was terminated on the specific Gateway process that restarted or crashed may use the reconnect shortcut only while the complete discovery snapshot is present and unexpired, and after fresh authority confirms current `ACTIVE` membership plus a valid current character for the selected target. If fresh membership, registry, grant, or other required non-entitlement authority is unavailable, the result is `AUTH_UNAVAILABLE`; preserve authentication and retry that authority before join or character repair. If the authority checks pass but either membership or character check fails, it must pass fresh Account entitlement `allowPublicJoin` plus fresh selected-target catalog/pointer/public-production evidence before completing HTTP `Join & Play` for missing or `INACTIVE` public-production membership, or rerun current character discovery/creation when the character is invalid, then acquire a fresh connect token and re-run `LOGIN` and `PLAY`. Unaffected sockets on other healthy Gateway instances should remain up, and the gateway fleet should continue accepting new handshakes through those healthy instances.
...
- Non-proxy gameplay clients must present a valid connect token; an absent `Firemud-Connect-Token` cookie is `CONNECT_TOKEN_MISSING`, while a present but invalid value uses the applicable expired, replayed, or rejected class; all return HTTP `403`.
...
- All gameplay sessions require in-band `LOGIN` and `PLAY` before gameplay commands. `LOGIN` uses Account's canonical email login for credential-bearing text clients or the verified bootstrap/connect context for first-party WebSocket clients; it establishes only socket/account authentication. `PLAY` performs gameplay admission and binding. For a public-production target, missing or `INACTIVE` membership is canonically `JOIN_REQUIRED` only after fresh Account entitlement says `allowPublicJoin=true` and fresh selected-target catalog/pointer/public-production evidence passes: credential-bearing text clients use the conditional in-band `JOIN`, while browser and other first-party WebSocket clients complete the equivalent HTTP `Join & Play` operation before connect-token issuance and do not send `JOIN` in-band. An entitlement outage is `ENTITLEMENT_UNAVAILABLE`, and a policy or target denial is `PUBLIC_PRODUCTION_ADMISSION_DENIED`. Returning `ACTIVE` members skip the join operation after current membership authority/version checks; a valid current character is additionally required only for the abbreviated direct-`PLAY` path, otherwise they proceed through `CHARS` or creation. Private/playtest clients with missing or non-`ACTIVE` membership receive `NON_PUBLIC_ENROLLMENT_REQUIRED`, must already have `membershipLifecycleState=ACTIVE` membership and the applicable current grant, and never use public `JOIN`; neither the grant nor connect-token/`PLAY` admission creates membership. Current text `PLAY` and connect-token issuance may still invoke `EnsurePublicProductionPlayerMembership`; that is implementation drift, not the target flow.
...
Every target connect-token issuance, including the first issuance immediately after explicit join, and every target `LOGIN` -> `PLAY` path performs the full canonical admission check: caller-bound membership must have exact `membershipLifecycleState=ACTIVE`, current `membershipAuthorityGeneration`, and independent `membershipVersion` evidence from the same fresh authoritative snapshot; current entitlement and routing must pass; and private/playtest targets additionally require the current exact realm grant plus existing membership. A grant never substitutes for membership, and token-backed `PLAY` must match the unchanged issuance baseline where one exists.

### Public membership repair gate (normative)

For every public-production membership repair requested through `JOIN`, `Join & Play`, or `JOIN_REQUIRED`, fresh Account entitlement with `allowPublicJoin=true` and fresh selected-target catalog/pointer/public-production evidence are separate gates evaluated before the repair is presented, invoked, or treated as eligible to continue. The [Account membership contract](./microservices/account-service/api-contracts.md#grpc-apis) owns the entitlement field and atomic membership decision; Game Session owns the selected-target routing evidence. An entitlement outage is `ENTITLEMENT_UNAVAILABLE`; a definitive Account policy denial is `PUBLIC_PRODUCTION_ADMISSION_DENIED`; missing or mismatched selected-target routing uses `ADMISSION_POINTER_UNAVAILABLE`, `REALM_UNAVAILABLE`, or `CONNECT_SCOPE_MISMATCH` as applicable. The operation preserves the existing membership state and cannot create or restore membership. The client must stop before `CHARS`, character creation, connect-token issuance, `LOGIN`, or `PLAY`. Only after both gates permit joining may the explicit operation create missing membership or restore `INACTIVE` membership, and only after that successful `ACTIVE` result may the flow continue to character repair, token issuance, `LOGIN`, or `PLAY`. Existing `ACTIVE` members use the separate current-membership admission path and do not require membership repair.
...
- `JOIN_REQUIRED` - The current public-production membership predicate is false because durable membership is missing or `INACTIVE`, fresh Account entitlement says `allowPublicJoin=true`, and fresh selected-target catalog/pointer evidence identifies the visible public-production target. Preserve authentication, present explicit `JOIN` or `Join & Play` as the restorable onboarding action, and block character creation, connect-token issuance, and `PLAY` retry until it succeeds. `JOIN` creates missing membership or restores `INACTIVE`; private/playtest membership or grant failure is not a public join fallback. For missing or `INACTIVE` public membership, pointer/routing failure returns `ADMISSION_POINTER_UNAVAILABLE` or `REALM_UNAVAILABLE`, scope failure returns `CONNECT_SCOPE_MISMATCH`, entitlement outage returns `ENTITLEMENT_UNAVAILABLE`, authoritative billing denial returns `TENANT_BILLING_BLOCKED`, and authoritative nonbilling denial, including `allowPublicJoin=false`, returns `PUBLIC_PRODUCTION_ADMISSION_DENIED`; stale or last-known-good entitlement cannot produce `JOIN_REQUIRED` or authorize join. Only when those earlier checks pass with fresh entitlement and fresh selected-target evidence is the result `JOIN_REQUIRED`.
...
- Normal clients may optionally issue `WORLDS` for discovery. `REALMS <world>` uses one `public_production_onboarding` route class whether discovery precedes membership or follows tenant/world resolution: public-production discovery may be pre-membership, while non-public results require exact membership with `membershipLifecycleState=ACTIVE` plus the current Account grant. Both stages use the shared catalog/pointer pair; unusable pointer evidence is `ADMISSION_POINTER_UNAVAILABLE` and a complete `CLOSED` realm is `REALM_UNAVAILABLE`. **Target first-entry sequence:** `LOGIN` -> conditional `JOIN` -> realm-scoped `CHARS`/character creation -> `PLAY`; missing or `INACTIVE` public-production membership is `JOIN_REQUIRED` only after fresh Account entitlement says `allowPublicJoin=true` and fresh selected-target catalog/pointer/public-production evidence permits repair, while a returning member may skip `JOIN`/`CHARS` only when current authority confirms `ACTIVE` membership and a valid current character for the selected target. Explicit `JOIN`/`Join & Play` is not implemented in the current runtime. **Current runtime:** credential-bearing text clients may still use direct `LOGIN` -> `PLAY`, and current `PLAY` or connect-token issuance may invoke Account's `EnsurePublicProductionPlayerMembership` and implicitly create missing public-production membership; this is implementation drift, not the target admission contract. Private/playtest clients never use public `JOIN`: missing or non-`ACTIVE` membership is `NON_PUBLIC_ENROLLMENT_REQUIRED`, and eligible clients require existing `membershipLifecycleState=ACTIVE` membership plus the applicable grant; no later step may create membership. Typed `SESSION` lines are not part of the Telnet contract. If smart-client attach hints return later, they travel as hidden MCP metadata and remain advisory transport context only.
- Direct text/Telnet reconnect is a fresh discovery path: after a new TCP/Telnet connection, the client or text adapter must perform current `WORLDS`/`REALMS` discovery as applicable, resolve a new Account-bound `connectScopeId`, validate the current catalog/pointer pair, and establish fresh non-entitlement authority before verifying current `ACTIVE` membership plus a valid current character. An unavailable membership, registry, grant, or other required authority is `AUTH_UNAVAILABLE`; preserve authentication and retry it before join or character repair. If that authority is available but public membership is missing or `INACTIVE`, fresh Account entitlement `allowPublicJoin=true` and fresh selected-target catalog/pointer/public-production evidence must permit repair before the client receives or takes the conditional `JOIN` step; an entitlement outage returns `ENTITLEMENT_UNAVAILABLE`, a denied policy/target returns `PUBLIC_PRODUCTION_ADMISSION_DENIED`, and an allowed gate produces `JOIN_REQUIRED`; if the current character is invalid, it must rerun character discovery/creation. It must not use a prior token-backed snapshot to select or attest the target. Only stale prior token-backed snapshot fields and token-backed `CONNECT_SCOPE_MISMATCH` state are inapplicable and discarded for this direct text flow; token-backed browser/WebSocket reconnect retains its existing fresh-token, new-socket, and full-snapshot-discard rules.

Private/playtest reconnect has a separate outcome: if current authoritative membership is missing or not `ACTIVE`, the reconnect returns `NON_PUBLIC_ENROLLMENT_REQUIRED`. It never offers public `JOIN`, uses public membership repair, or treats `CHARS`/character discovery as membership repair. Only after existing membership is confirmed `ACTIVE` and the current realm grant is valid may character discovery or allowed realm-local character creation repair a missing/invalid character; a missing or invalid grant remains `REALM_ACCESS_DENIED`.
...
`JOIN <world>` is a Telnet selector, not an authority-bearing tenant or route input. The Game Session text-protocol adapter keeps the client world/realm selectors local, resolves the public-production choice server-side, and obtains the Account-bound `{connectScopeId, requestId}` for the authenticated operation. It passes only `{connectScopeId, requestId}` plus the trusted authenticated caller context to `JoinPublicProductionMembership`; it never passes client-supplied or adapter-resolved `tenantId`, world, realm, route, `gameInstanceId`, `catalogRevision`, `pointerVersion`, or other route authority as operation inputs. Before any public membership mutation or `JOIN_REQUIRED` repair, fresh Account entitlement must say `allowPublicJoin=true` and fresh selected-target catalog/pointer/public-production evidence must pass; entitlement outage is `ENTITLEMENT_UNAVAILABLE`, a definitive policy denial is `PUBLIC_PRODUCTION_ADMISSION_DENIED`, and routing failures retain their routing codes. Account resolves the target from `connectScopeId`, binds the exact `{catalogRevision, pointerVersion}` pair into the request/target digest and membership commit, and revalidates that pair at the membership commit gate. A missing relationship creates `ACTIVE` and an `INACTIVE` relationship restores `ACTIVE`, with exactly one transition audit/outbox event for either transition; an `ACTIVE` retry returns the exact snapshot without an event. Private/playtest realms do not use this operation and require existing membership plus their current grant.
...
- In both modes, prove Proxy -> Gateway uses the internal `wss://` mTLS listener and that the bridged Telnet client reaches the target credential-bearing `LOGIN -> conditional JOIN -> realm-scoped CHARS/character creation -> PLAY -> LOOK` flow. Existing members may use the current abbreviated `LOGIN -> PLAY -> LOOK` path only after current `ACTIVE` membership and a valid current character are verified. Native browser/WebSocket clients instead complete HTTP `Join & Play` when needed, then use `LOGIN -> PLAY -> LOOK`; returning members skip the join operation only under the same current membership and character checks.
...
- Those hints must never bypass `LOGIN` + conditional `JOIN` + the realm-scoped character gate + `PLAY` or retroactively alter an already-established gameplay binding.
...
- **Client expectations**
- When WebSocket connections close due to slow-client behavior, abuse, network issues, or backend unavailability, browser and other first-party WebSocket clients may use the reconnect shortcut only while the complete discovery snapshot is present and unexpired, and after current `ACTIVE` membership plus a valid current character are confirmed. Otherwise they must complete HTTP `Join & Play` if public-production membership is missing or `INACTIVE`, or rerun character discovery/creation when the character is invalid, then fetch a fresh connect token, reconnect, re-`LOGIN`, and re-`PLAY`. Credential-bearing text clients take the conditional in-band `JOIN` step under the same checks. Game Session’s Redis-backed state determines whether gameplay resumes or starts fresh, per [Reconnection Strategy](./system-architecture-reconnection.md).
