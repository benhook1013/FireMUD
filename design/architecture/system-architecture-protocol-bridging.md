...
Game Session translates admission outcomes into the shared `ERROR <CODE> <message>` form on the established gameplay protocol. These three `PLAY` outcomes are canonical and preserve the authenticated session unless their own contract says otherwise:
...
- `AUTH_UNAVAILABLE` - A required current non-entitlement authority dependency, including membership authority or its generation, is unreachable or timed out. Preserve authentication, create no gameplay binding, and retry the affected admission with bounded backoff; cached authority cannot authorize it.
...
- Normal clients may optionally issue `WORLDS` for discovery. **Target first-entry sequence:** `LOGIN` -> conditional `JOIN` -> realm-scoped `CHARS`/character creation -> `PLAY`; a returning member with an already selected character may use the abbreviated `LOGIN` -> `PLAY` path and skip `JOIN`/`CHARS`. Explicit `JOIN`/`Join & Play` is not implemented in the current runtime, so current clients with existing membership may still use direct `LOGIN` -> `PLAY`; this implementation drift does not change the target sequence. Private/playtest clients never use public `JOIN`: they require existing membership and the applicable grant, and no later step may create membership. Typed `SESSION` lines are not part of the Telnet contract. If smart-client attach hints return later, they travel as hidden MCP metadata and remain advisory transport context only.
