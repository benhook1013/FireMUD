...
Precedence is deterministic: classify an incomplete or unavailable pointer as `ADMISSION_POINTER_UNAVAILABLE` and a complete `CLOSED` pointer as `REALM_UNAVAILABLE`; only after routing is valid evaluate `CONNECT_SCOPE_MISMATCH`; then evaluate membership/grant and entitlement, with authoritative billing denial taking precedence over a normal entitlement denial. Non-routing authority outages use `AUTH_UNAVAILABLE`; entitlement authority outages use `ENTITLEMENT_UNAVAILABLE`.

- `JOIN_REQUIRED` - The current public-production membership predicate is false because durable membership is missing. Preserve authentication, require explicit `JOIN` or `Join & Play`, and block character creation, connect-token issuance, and `PLAY` retry until join succeeds. Private/playtest membership or grant failure is not a public join fallback. An entitlement outage uses `ENTITLEMENT_UNAVAILABLE`, and an authoritative entitlement denial uses `TENANT_BILLING_BLOCKED`.
