...
- **Telnet → Web takeover**
  - A Telnet client connects via the TCP Proxy, performs fresh `WORLDS`, sends credential-bearing `LOGIN <email> <secret>` / the applicable email-code form, performs authenticated `REALMS <world>`, takes the target-only `JOIN` step only for first public-production entry, conditionally completes realm-scoped `CHARS`/character creation when no valid selected character exists, and enters gameplay with `PLAY <world> [realm] [character]` for the selected character.
...
- **Web → Telnet takeover**
  ...
  - A Telnet client later connects through the TCP Proxy, performs fresh `WORLDS`, sends credential-bearing `LOGIN <email> <secret>` / the applicable email-code form, performs authenticated `REALMS <world>`, takes the target-only `JOIN` step only for first public-production entry, conditionally completes realm-scoped `CHARS`/character creation when no valid selected character exists, and enters gameplay with `PLAY <world> [realm] [character]` for the same selected character.
