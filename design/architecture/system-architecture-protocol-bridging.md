...
- After a successful reconnect and `PLAY`, Game Session currently presents/restores an available bounded per-player retained-context window on a binding-checked, best-effort basis in canonical order, followed by a mandatory fresh authoritative `LOOK` derived from current state for the new transport. Current post-`LOGOUT` suppression remains a gap; target restoration is unavailable after durable termination or newer binding-generation evidence, and complete restoration is not claimed until focused `LOGOUT` -> `LOGIN` -> `PLAY` no-replay proof exists. See [Input, Output, and Presentation Implemented Status](./system-architecture-input-output-and-presentation.md#implemented-status) for the restoration status and target gaps. An applicable prompt, structured status, or newly generated MCP state is an additional projection, not an alternative to `LOOK`. Empty or expired context produces no context output.
