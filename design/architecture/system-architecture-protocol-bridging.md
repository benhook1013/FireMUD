...
- **Client expectations**
  - When WebSocket connections close due to slow-client behavior, abuse, network issues, or backend unavailability, clients must reconnect, re-`LOGIN`, and re-`PLAY`. Game Session’s Redis-backed state determines whether gameplay resumes or starts fresh, per [Reconnection Strategy](./system-architecture-reconnection.md).
