...
- After a successful reconnect and `PLAY`, Game Session presents/restores an available, authorized bounded per-player retained-context window in canonical order, followed by a mandatory fresh authoritative `LOOK` derived from current state for the new transport. An applicable prompt, structured status, or newly generated MCP state is an additional projection, not an alternative to `LOOK`. Empty or expired context, or context omitted under CMD-04 bounds, produces no context output.
