...
The target flow and delivery contracts below remain normative. Explicit `JOIN`/`Join & Play` and the required exact `membershipAuthorityGeneration`, `membershipLifecycleState`, and `membershipVersion` reread at connect-token issuance remain unimplemented/proof gaps in the current runtime. Current connect-token issuance and text `PLAY` consume the Account membership response fields `membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt`, and proceed only when the two admission fields are true; an eligible public-production request with `membershipExists=false` may fail closed with non-actionable `JOIN_REQUIRED`. The obsolete implicit membership-writer surface has been removed. Current non-public missing or non-admitting membership remains `WORLD_ACCESS_DENIED`; target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` is defined below. Current private/playtest text `PLAY` also requires the current realm grant, while connect-token grant validation remains implementation drift. Returning members may proceed through the current existing-membership path, but that behavior does not prove the target generation/version reread at connect-token issuance; the target requires lifecycle state, authority generation, and independent membership version from one fresh atomic membership snapshot. Every target admission uses the applicable tenant runtime entitlement; private/playtest uses its exact Account-owned grant rather than the public-production enrollment predicate `allowPublicJoin`. This implementation drift does not relax the at-most-once delivery, authentication, admission, reconnect, or close-taxonomy requirements.
...
Direct text/Telnet clients must follow the fresh-discovery and authenticated `REALMS`-to-`JOIN` procedure in [Authentication](./system-architecture-authentication.md#direct-text-realms-to-join-scope-normative). The bridge carries the resulting text flow and retains only the current transport-local scope; it does not create or restore membership. Reachable invalid or stale pointer evidence is `ADMISSION_POINTER_UNAVAILABLE`; an unreachable or timed-out Account or routing authority is `AUTH_UNAVAILABLE`; a complete `CLOSED` realm remains `REALM_UNAVAILABLE`.
