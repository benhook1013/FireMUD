...
This design is the canonical specification for protocol-specific gameplay carriage through the edge: it defines ordering and delivery guarantees, backpressure and slow-client behaviour, Telnet and WebSocket reconnection and buffering rules, and the Telnet disconnect reason taxonomy. [Gateway architecture](./system-architecture-gateway.md) owns route lifecycle, gameplay sharding, lease-aware selection, and canonical close translation; [Authentication](./system-architecture-authentication.md#login-and-session-flow) owns the login, membership, grant, and gameplay-admission rules. Service-specific designs retain their transport configuration and apply those owning contracts. Direct-text ordering and JOIN scope handling follow [Authentication](./system-architecture-authentication.md#direct-text-realms-to-join-scope-normative); this bridge forwards the resulting protocol sequence without duplicating the membership contract. First-party WebSocket onboarding remains separate from this text carriage path and uses its owner-defined bootstrap/connect-token sequence; grant-backed private/playtest traffic is forwarded without creating membership.

## Implementation Status

The target flow and delivery contracts below remain normative. The full target `WORLDS` -> `LOGIN` -> `REALMS` -> conditional `JOIN` -> conditional `CHARS`/character creation -> `PLAY` bridge sequence is target-only; current smoke proof is limited to the abbreviated existing-member `LOGIN` -> `PLAY` -> `LOOK` path and does not prove `WORLDS`, `REALMS`, or `JOIN`. Explicit `JOIN`/`Join & Play` and the required exact `membershipAuthorityGeneration`, `membershipLifecycleState`, and `membershipVersion` reread at connect-token issuance remain unimplemented/proof gaps in the current runtime. Current connect-token issuance and text `PLAY` consume the Account membership response fields `membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt`, and proceed only when the two admission fields are true; an eligible public-production request with `membershipExists=false` may fail closed with non-actionable `JOIN_REQUIRED`. The live Account adapter has no `membershipLifecycleState`: `membershipExists=false` is the current missing-membership case, while `membershipExists=true` with `gameplayAdmissionAllowed=false` is existing non-admitting membership and cannot be classified as `INACTIVE`. The obsolete implicit membership-writer surface has been removed. Current non-public missing or non-admitting membership remains `WORLD_ACCESS_DENIED`; target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` is defined below. Current private/playtest text `PLAY` also requires the current realm grant, while current connect-token issuance validates pointer/visibility but not the private/playtest grant; target-only connect-token grant validation remains implementation drift. A connect token or returning shortcut must not be treated as authorization proof. Returning members may proceed through the current existing-membership path, but that behavior does not prove the target generation/version reread at connect-token issuance; the target requires lifecycle state, authority generation, and independent membership version from one fresh atomic membership snapshot. Every target fresh admission requires applicable entitlement; public production requires a fresh positive entitlement result, while private/playtest additionally requires existing caller-bound `ACTIVE` membership and its exact Account-owned realm grant rather than the public-production enrollment predicate `allowPublicJoin`. The only last-known-good exception is unchanged public-production binding continuity during an entitlement-only outage after current membership, routing, public-policy, and other resume predicates pass; outside that exact continuity case, entitlement unavailability remains `ENTITLEMENT_UNAVAILABLE`. This implementation drift does not relax the at-most-once delivery, authentication, admission, reconnect, or close-taxonomy requirements.
...
- The supported `first_party_web` variant requires the protected `Firemud-Connect-Token` cookie; `CONNECT_TOKEN_MISSING` means no connect-token carrier is supplied, not merely that the cookie is absent. A target-only `non_first_party_public` variant may use `X-Firemud-Connect-Token` only after its dedicated route registration and issuance, replay, signed-context, response, and carrier proof; a header-only request on an unsupported or unregistered route is `CONNECT_TOKEN_REJECTED`, not `CONNECT_TOKEN_MISSING`. The header is not a fallback for the cookie route, and a present but invalid value uses the applicable expired, replayed, or rejected class. These handshake outcomes return HTTP `403`. TCP Proxy bridge traffic is admitted only through the authenticated internal mTLS and header-trust branch and consumes no gameplay connect token.
- Browser clients cannot read failed WebSocket upgrade headers. Before the socket opens, they follow the canonical [browser failed-upgrade recovery rule](./system-architecture-reconnection.md#browser-failed-upgrade-recovery): exactly one fresh discovery/token retry per failed-upgrade episode with bounded backoff, then no automatic refresh or reconnect and recovery guidance. Detailed bounded classes remain available to capable non-browser callers and operator telemetry rather than becoming a second browser status API.
- All gameplay sessions require in-band `LOGIN` and `PLAY` before gameplay commands. `LOGIN` establishes socket/account authentication and `PLAY` performs gameplay admission and binding under [Authentication](./system-architecture-authentication.md#login-and-session-flow). The current implementation caveat remains: explicit join is unimplemented, current text `PLAY` and connect-token issuance consume `membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt`, and proceed only when the two admission fields are true; an eligible public-production request with `membershipExists=false` may return non-actionable `JOIN_REQUIRED` without creating membership.

For target browser onboarding and reconnect, the realm-scoped character gate remains in the sequence: first-time public entry, or a returning flow whose selected character is missing, invalid, or no longer visible for the resolved realm, must complete `CHARS`/allowed character creation after target-only `ACTIVE` membership and before `POST /auth/connect-token` and WebSocket `PLAY`. Returning players use the canonical [first-party reconnect-shortcut gate](./system-architecture-reconnection.md#canonical-first-party-reconnect-shortcut-gate); the bridge must preserve its fresh same-target entitlement, current authority, independent `membershipAuthorityGeneration` and `membershipVersion`, and exact current Account-owned realm-grant requirements, along with the discovery-snapshot and valid-character prerequisites defined there. The bridge forwards that owner-defined order and does not treat discovery or token evidence as authorization proof. Current runtime has no lifecycle-state predicate: its current admission fields are `membershipExists` and `gameplayAdmissionAllowed`; `CHARS` and character creation require the applicable membership/grant and entitlement checks but do not require an existing character, while a character becomes mandatory before connect-token issuance and `PLAY`. Current runtime validates pointer/visibility for this flow but not the private/playtest realm grant, so current shortcut and token behavior do not prove authorization. A connect token carries a short-lived discovery snapshot for edge admission; it does not bypass `CHARS` or turn stale character evidence into gameplay authority.
...
For public-production repair, clients follow the canonical [Authentication membership and admission flow](./system-architecture-authentication.md#login-and-session-flow). The local bridge consequence is transport-only: fresh text discovery precedes credential-bearing `LOGIN` and authenticated `REALMS`, while first-party clients complete the owner-defined `Join & Play` gate before connect-token issuance, socket open, bare `LOGIN`, or `PLAY`. A denied repair preserves authentication and membership state, creates no binding, and stops before character discovery or creation, token issuance, socket admission, and gameplay login. The wire outcomes remain those defined by Authentication; existing `ACTIVE` members do not use repair.
...
The precedence and error rows below are the normative target inventory. Current runtime mapping is narrower: an eligible public-production response with `membershipExists=false` may return non-actionable `JOIN_REQUIRED` while explicit `JOIN` is unimplemented, and current non-public missing or non-admitting membership remains `WORLD_ACCESS_DENIED`; the target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` row must not be reported as current behavior.

Admission precedence is deterministic and evaluated in stages: first classify current routing/pointer/catalog evidence (`ADMISSION_POINTER_UNAVAILABLE` for reachable incomplete, malformed, ambiguous, stale, or otherwise invalid evidence; `AUTH_UNAVAILABLE` for an unreachable or timed-out Account or routing authority; `REALM_UNAVAILABLE` for a complete `CLOSED` pointer); then evaluate verified connect scope (`CONNECT_SCOPE_MISMATCH`). Every admission then evaluates the applicable fresh tenant runtime entitlement. Public production additionally evaluates its enrollment policy, while private/playtest requires existing caller-bound `ACTIVE` membership and the exact current Account-owned realm grant without consuming `allowPublicJoin`. The remaining membership and authority outcomes follow [Authentication](./system-architecture-authentication.md#in-band-play-admission-boundary). A routing or pointer failure masks later results; a scope mismatch masks later results; and an entitlement outage or authoritative public denial masks public `JOIN_REQUIRED`. The local bridge does not create or restore membership.

- `ADMISSION_POINTER_UNAVAILABLE` - Reachable catalog/pointer evidence is missing, malformed, ambiguous, stale, or otherwise contract-invalid. Preserve authentication, create no binding, rerun discovery or reconciliation, and retry only with fresh routing evidence. An unreachable or timed-out Account or routing authority is `AUTH_UNAVAILABLE`, not this code.
...
- `ENTITLEMENT_UNAVAILABLE` - Fresh entitlement authority cannot be established for `PLAY` or the join/admission decision. Preserve authentication, create no gameplay binding or new membership, and retry with bounded backoff; last-known-good entitlement may supply input only for unchanged public-production binding continuity during an entitlement-only outage after the other resume predicates pass. It never authorizes fresh admission, private/playtest admission, `JoinPublicProductionMembership`, membership creation/restoration, or an expanded binding. This is distinct from `AUTH_UNAVAILABLE`, which covers unavailable membership, grant, registry, or other required non-routing, non-entitlement authority.
...
- `JOIN_REQUIRED` - Current public-production membership is missing or `INACTIVE` in the target-only Authentication flow. Preserve authentication and follow [Authentication](./system-architecture-authentication.md#login-and-session-flow) for the explicit `JOIN` or `Join & Play` action; current text reconnect instead stops with non-actionable `JOIN_REQUIRED` and performs no membership mutation because explicit `JOIN` is unavailable. Private/playtest membership or grant failure is not a public join fallback.
...
- `AUTH_UNAVAILABLE` - A required current Account or routing authority, including membership, grant, registry, or realm-routing authority, is unreachable or times out. Preserve authentication, create no gameplay binding, and retry the affected admission with bounded backoff; unavailable authority cannot be interpreted as missing/`INACTIVE` membership and cached authority cannot authorize it. Reachable stale, regressed, malformed, revoked, or mismatched evidence is the applicable denial or revocation outcome; reachable invalid pointer evidence is `ADMISSION_POINTER_UNAVAILABLE`, and entitlement authority failures use `ENTITLEMENT_UNAVAILABLE` instead.
...
Direct text/Telnet clients must follow the fresh-discovery and authenticated `REALMS`-to-`JOIN` procedure in [Authentication](./system-architecture-authentication.md#direct-text-realms-to-join-scope-normative). The bridge carries the resulting text flow and does not create or restore membership; Game Session retains pending JOIN transport state under the contract below. Reachable invalid or stale pointer evidence is `ADMISSION_POINTER_UNAVAILABLE`; an unreachable or timed-out Account or routing authority is `AUTH_UNAVAILABLE`; a complete `CLOSED` realm remains `REALM_UNAVAILABLE`.
Direct text/Telnet reconnect is a fresh transport and discovery path described in [Reconnection Strategy](./system-architecture-reconnection.md#client-reconnection-behaviour). The proxy does not reuse a browser/WebSocket bootstrap snapshot or prior transport-local scope. Current proxy routing still bootstraps a hidden default and does not enforce `WORLDS` as a prerequisite; explicit-join and membership-reread gaps remain those recorded in [Authentication](./system-architecture-authentication.md#implementation-status).

Target private/playtest reconnect does not use public `JOIN`; missing or non-`ACTIVE` membership is `NON_PUBLIC_ENROLLMENT_REQUIRED`, and a missing or invalid grant is `REALM_ACCESS_DENIED`. Current private/playtest missing or non-admitting membership remains `WORLD_ACCESS_DENIED`; current text `PLAY` requires the current realm grant, while connect-token grant validation remains implementation drift.

A retained-edge upstream rebind is not a reconnect from the client's perspective. Gateway and the proxy may rebind the established edge to a replacement Game Session owner without repeating `LOGIN`, `JOIN`, or `PLAY`; unchanged public-production binding continuity may use bounded last-known-good entitlement only during an entitlement-only outage after current Account membership, routing, public policy, revocation, and resume-lease checks pass. This exception does not change the strict fresh-entitlement rules for a fresh gameplay binding/new admission, private/playtest admission, or a client-facing edge disconnect; an exact same-binding reconnect on a new transport does not receive this retained-edge exception and must use fresh admission rules.

### Telnet `JOIN` Resolution Contract

`JOIN <world>` is a Telnet selector, not an authority-bearing tenant or route input. The text adapter/Game Session path retains the complete fresh `WORLDS -> LOGIN -> REALMS` evidence for the current transport: a tenant-scoped `WORLDS` snapshot, the authenticated `REALMS` result and opaque scope, and the exact `catalogRevision`/`pointerVersion` pair for the selected target. Game Session owns and generates the high-entropy join-attempt request ID; it retains that ID with the opaque scope and response-local selector, carries it through every retry and the terminal outcome, and retains the operation state until terminal cleanup or scope expiry. The same request ID is never replaced during a retry, and a new ID is generated only for a new attempt after terminal outcome, scope expiry, or fresh `REALMS`. The client supplies only the response-local selector or index. If a selector is stale or ambiguous, or the retained tenant-scoped snapshot, authenticated scope, or revision pair is missing, expired, or no longer current, the adapter rejects `JOIN` before calling Account and requires fresh `WORLDS`/`REALMS` resolution; it must not re-resolve the stale selector or fall back to client-supplied target fields. Account-owned membership mutation and the exact scope/request binding are defined in [Authentication](./system-architecture-authentication.md#direct-text-realms-to-join-scope-normative); Account binds and replays the Game Session-owned request ID but does not generate it. Private/playtest realms do not use public `JOIN`; after the common runtime-entitlement gate they require existing caller-bound `ACTIVE` membership plus their current grant rather than `allowPublicJoin`.
...
- In both modes, prove Proxy -> Gateway uses the internal `wss://` mTLS listener and that the bridged Telnet client reaches the Authentication-owned target `WORLDS -> LOGIN -> authenticated REALMS -> conditional JOIN -> conditional realm-scoped CHARS/character creation when no valid selected character exists -> PLAY -> LOOK` flow.
...
- Gateway closes the authenticated internal bridge with `1012/service_restart;subreason=gateway_restart`.
- TCP Proxy classifies that machine-parseable bridge close as `bridge_shutdown_class=planned_drain`.
- The Telnet client receives a final `service_restart` disconnect rather than `logout` or `backend_unavailable`.

Authenticated upstream lifecycle translation:

- TCP Proxy preserves every valid authenticated Gateway top-level close as the same Telnet token: `logout`, `session_replaced`, `service_restart`, `idle_timeout`, `policy_violation`, `internal_error`, or `backend_unavailable`.
- `bridge_shutdown_class` is bridge-only operational metadata, never lifecycle authority: clean `1012/service_restart` is `planned_drain`; every other valid authenticated top-level close is `valid_upstream_close`; and only absent, invalid, or otherwise unattributable bridge close metadata is `unattributed_failure`. The top-level reason remains authoritative, and only absent or invalid close metadata uses observation-specific `backend_unavailable` for the Telnet client. Any subreason is optional diagnostic context only. See [Gateway Architecture](./system-architecture-gateway.md) and [TCP Proxy Service Operations](./microservices/tcp-proxy-service/operations.md).

Unattributed bridge-loss example affecting one established Telnet bridge:

- The authenticated internal bridge drops without a valid top-level close (for example abrupt transport reset or crash).
...
- **At-most-once edge delivery with bounded loss before acceptance** – The edge protocol path for gameplay commands is **at most once**: once a command on a given connection is dropped by any edge layer (for example due to buffer ceilings or abuse limits), that layer does not retry or replay it. Potential loss is limited to commands still resident before trusted Game Session acceptance; there is no implicit replay across disconnects. At trusted acceptance, Game Session assigns or preserves a stable `commandId` and durably records the command lifecycle before acknowledging it. Ordinary interactive commands remain tracked but non-replayable: internal loss before staging terminalizes as `NOT_APPLIED` rather than silently disappearing or executing later. Internal retries reuse the same identity and owner guards; see [ADR 0062](./decisions/adr-0062-layered-gameplay-command-delivery-semantics.md), [ADR 0016](./decisions/adr-0016-canonical-gameplay-command-status-lifecycle.md), and [Transactions & Idempotency](./system-architecture-transactions.md).
...
- **No implicit replay on reconnect** – Neither Spring Cloud Gateway nor the TCP Proxy Service replays gameplay commands, raw frames/bytes, the previous transport stream, unsent output, or MCP messages across reconnects. After an actual client-facing edge disconnect, or after authentication invalidation has terminated the active binding, direct text/Telnet clients follow [Reconnection Strategy](./system-architecture-reconnection.md#client-reconnection-behaviour) and the Authentication-owned fresh-discovery and admission procedure, obtaining a new transport-local scope from authenticated `REALMS` rather than reusing a browser/WebSocket snapshot. Target-specific entitlement, membership, and grant predicates remain owned by Authentication; this transport never substitutes public `allowPublicJoin` for a private/playtest grant. Reachable invalid pointer evidence and unreachable authority retain their distinct error classes. Both client classes must re-run MCP negotiation for that new connection if they use MCP. A retained-edge Game Session upstream rebind is not a client reconnect and is exempt from repeating `LOGIN`, `JOIN`, and `PLAY`; successful bounded hidden rebind is invisible to the client, while failure closes with `backend_unavailable`. After fresh admission, Game Session may currently present bounded semantic recent context on a binding-checked, best-effort basis before the fresh authoritative `LOOK` and reconnect prompt governed by [ADR 0133](./decisions/adr-0133-fresh-edge-reconnect-without-client-input-replay.md) and [ADR 0134](./decisions/adr-0134-bounded-durable-semantic-reconnect-context.md); this current restoration can still replay retained context after deliberate `LOGOUT` because suppression is unimplemented, and retained context is not authorization. The target permits context only for an eligible, non-terminated binding, with fresh authoritative `LOOK` remaining mandatory; this is not transport replay. Clients then rely on Game Session and Redis to resume or start fresh according to [Reconnection Strategy](./system-architecture-reconnection.md). A versioned capable-client envelope may supply a `commandId` and query canonical status, but base Telnet and text WebSocket clients do not manage command identities and a reused identity never implies automatic stale-intent replay.
...
- Automatic durable replay is limited to explicitly classified features with safe expiry and context preconditions; internal effect identifiers and owner guards protect retries without becoming part of the base text protocol.
...
- `logout` – terminal gameplay logout or forced session termination; maps to WebSocket `1000/logout`. When the authenticated upstream bridge closes with that top-level reason, the TCP Proxy preserves Telnet `logout`.
- `session_replaced` – the transport displaced by a successful controller takeover; maps to WebSocket `1000/session_replaced`. The replacement remains authoritative and this token is not terminal logout.
- `service_restart` – planned Gateway drain or restart; maps to WebSocket `1012/service_restart` and is retryable maintenance, not logout.
...
The authoritative cross-layer translation table and precedence rules for WebSocket and Telnet close outcomes live in [Gateway Architecture](./system-architecture-gateway.md#canonical-close-translation-matrix). This document defines only the TCP Proxy’s local token translation and must remain consistent with that table; it does not add a second external taxonomy. There is no client-visible `reroute` token: lease movement and shard ownership remain internal to Game Session.
...
- Client authors choose reconnect behavior from the top-level reason alone: terminal `logout` is non-retriable, `session_replaced` is a displaced-transport outcome, `service_restart`, `idle_timeout`, `internal_error`, and `backend_unavailable` are retryable according to [Reconnection Strategy](./system-architecture-reconnection.md#client-reconnection-behaviour), and `policy_violation` is non-retriable or uses much longer backoff. Optional subreason, including `edge_backpressure`, remains diagnostic and never changes that lifecycle or retry classification.
...
Telnet disconnect messages and structured logs may preserve the same bounded subreason context used by the WebSocket side (`user_logout`, `takeover`, `gateway_restart`, `admin_termination`, `edge_backpressure`, `none`) for operations correlation. Subreason is never lifecycle authority: clients select behavior from the top-level token alone. For Telnet disconnects caused by edge backpressure, `subreason=edge_backpressure` remains useful diagnostic context and does not turn a policy close into a backend outage.

Concrete takeover example:

- A Telnet client is already in gameplay.
- A second client successfully takes over the same `{tenantId, playableStateNamespaceId, characterId}` controller key. `playableStateScope` is server-derived binding evidence, and the active `gameInstanceId` is runtime-fence/value evidence rather than durable character identity.
- Game Session closes the authenticated upstream gameplay bridge with `1000/session_replaced`.
- TCP Proxy preserves that as Telnet `session_replaced`; it does not translate the event into terminal `logout` or `backend_unavailable`.

A close token reports connection/session lifecycle, not whether an in-flight gameplay command committed. A capable client or operator with a known `commandId` uses Game Session’s authoritative command-status surface; classic Telnet and text clients must not infer command success or failure from any close token.
...
The following target-state examples assume the public-production joining policy permits repair. The underlying authentication and gameplay services enforce a **single active gameplay binding per `{tenantId, playableStateNamespaceId, characterId}`**; `playableStateScope` is server-derived binding evidence, and `gameInstanceId` is active-runtime fence/value evidence. The current implementation remains on `{tenantId, gameInstanceId, characterId}` as documented implementation drift. These target rules are described in [Authentication & Authorization](./system-architecture-authentication.md#multi-client-behavior-and-session-takeover) and [Reconnection Strategy](./system-architecture-reconnection.md#resume-vs-reload-scenarios). From the networking and protocol edge, this manifests as follows:

For a replacement realm, the bridge carries only the player-facing world/realm/character selectors. Game Session resolves the stable `playableStateNamespaceId`, resolved `playableStateScope`, and active `gameInstanceId` server-side; a stale source-instance binding is rejected or drained under the lifecycle contract, and a client must not infer a new target from a cached runtime ID or an echoed remap identifier. [ADR 0122](./decisions/adr-0122-stable-playable-state-namespaces-for-runtime-replacement.md) and [ADR 0123](./decisions/adr-0123-database-authoritative-temporal-coordinated-world-lifecycle.md) define this as a local admission/reconnect consequence rather than a new transport authority.

- **Telnet → Web takeover**
  - A Telnet client connects via the TCP Proxy, performs fresh `WORLDS`, sends credential-bearing `LOGIN <email> <secret>` / the applicable email-code form, performs authenticated `REALMS <world>`, takes the target-only `JOIN` step when public-production membership is missing or `INACTIVE`, conditionally completes realm-scoped `CHARS`/character creation when no valid selected character exists, and enters gameplay with `PLAY <world> [realm] [character]` for the selected character.
  ...
  - Game Session treats the Web client as the new active binding, terminates or demotes the Telnet connection according to takeover rules, and closes the Telnet path with `session_replaced` (mapped to WebSocket `1000/session_replaced` in the Gateway-owned taxonomy above). No ordering guarantees are provided between the last Telnet commands and the first WebSocket commands; only per-connection FIFO holds on each individual connection.
...
- **Web → Telnet takeover**
  ...
  - A Telnet client later connects through the TCP Proxy, performs fresh `WORLDS`, sends credential-bearing `LOGIN <email> <secret>` / the applicable email-code form, performs authenticated `REALMS <world>`, takes the target-only `JOIN` step when public-production membership is missing or `INACTIVE`, conditionally completes realm-scoped `CHARS`/character creation when no valid selected character exists, and enters gameplay with `PLAY <world> [realm] [character]` for the same selected character.
  - Game Session treats the Telnet client as the new active binding; the displaced WebSocket session is closed with `1000/session_replaced` and the Telnet connection becomes authoritative. Again, cross-connection ordering is not defined: only per-connection FIFO is guaranteed, and clients must not attempt to sequence commands across the old and new transports.
...
- **Client expectations**
- When WebSocket connections close due to slow-client behavior, abuse, network issues, or backend unavailability, only first-party token-backed WebSocket clients may use the canonical [first-party reconnect-shortcut gate](./system-architecture-reconnection.md#canonical-first-party-reconnect-shortcut-gate). The bridge forwards fresh discovery, token issuance, socket reconnect, bare `LOGIN`, and `PLAY` in that owner-defined order; it does not authorize the shortcut. Current browser/mobile clients have only partial bootstrap/connect-token surfaces and explicit `Join & Play` remains unavailable, so an eligible missing public-production membership (`membershipExists=false`) remains non-actionable `JOIN_REQUIRED` with no mutation; an existing non-admitting response cannot be classified as `INACTIVE` by the live adapter. In the target, where public joining policy permits repair, those clients complete HTTP `Join & Play` for missing or `INACTIVE` membership before any fresh connect-token issuance, socket reconnect, bare `LOGIN`, or `PLAY`, or rerun character discovery/creation when the character is invalid before those later steps. Credential-bearing text clients follow [Reconnection Strategy](./system-architecture-reconnection.md#client-reconnection-behaviour) and the Authentication-owned discovery/JOIN ordering; current text clients stop with the same non-actionable `JOIN_REQUIRED` fallback when repair is required. Game Session’s Redis-backed state determines whether gameplay resumes or starts fresh, per [Reconnection Strategy](./system-architecture-reconnection.md).
...
- Retained-edge upstream rebind is not an actual client-facing disconnect: Spring Cloud Gateway retains the client or proxy-facing gameplay WebSocket while performing ADR 0013's bounded rebind of a rebindable Game Session upstream, then closes with `1013/backend_unavailable` if recovery exhausts its window. Game Session revalidates the existing continuation binding, authorization, revocation, and target authority under the [Authentication](./system-architecture-authentication.md#login-and-session-flow) and [Gateway architecture](./system-architecture-gateway.md#tenant-aware-edge-connect-token-gameplay-handshake) contracts; it does not require fresh `LOGIN`, `JOIN`, or `PLAY`, and this hidden rebind does not trigger retained-context restoration or a fresh `LOOK`.
- After an actual client-facing disconnect, the proxy cleans up Telnet sessions and clients follow [Reconnection Strategy](./system-architecture-reconnection.md#client-reconnection-behaviour) and the Authentication-owned fresh admission sequence; this bridge does not duplicate JOIN or membership ordering. Only after fresh admission completes, including current `LOGIN` identity checks and `PLAY` binding checks, does Game Session currently present/restore available retained semantic context on a binding-checked, best-effort basis in canonical order and perform a mandatory fresh authoritative `LOOK`; current post-`LOGOUT` suppression is not enforced, so this is not a complete restoration claim. Target restoration is unavailable after durable termination of the private context episode or evidence of a newer `resumeContextGeneration`, and complete restoration requires that suppression plus focused `LOGOUT` -> `LOGIN` -> `PLAY` no-replay proof. See [ADR 0134](./decisions/adr-0134-bounded-durable-semantic-reconnect-context.md) and [Input, Output, and Presentation Implemented Status](./system-architecture-input-output-and-presentation.md#implemented-status) for the target contract, restoration status, and gaps. Structured status/MCP state and any applicable prompt are additional projections, not alternatives to that `LOOK`. The Telnet path does not support hidden reattachment after its distinct Proxy -> Gateway gameplay WebSocket is lost: the proxy closes that Telnet connection rather than silently opening a fresh edge bridge behind the same client TCP socket.
...
- After a successful reconnect and `PLAY`, Game Session currently presents/restores an available bounded retained-context window using its available binding checks on a best-effort basis in canonical order, followed by a mandatory fresh authoritative `LOOK` derived from current state for the new transport. The current generation-qualified proof is incomplete. Target replay eligibility is restricted to the exact private semantic-context episode `{tenantId, playableStateNamespaceId, characterId, resumeContextGeneration}`. `bindingGeneration` remains the independent controller-ownership fence: an authorized reconnect, takeover, or runtime replacement carries the same `resumeContextGeneration` even when `bindingGeneration` advances. Durable termination for explicit logout, ownership transfer, or lost replay authority prevents restoration under that generation; a later authorized episode uses a newer generation. See [ADR 0134](./decisions/adr-0134-bounded-durable-semantic-reconnect-context.md) and [Input, Output, and Presentation Implemented Status](./system-architecture-input-output-and-presentation.md#implemented-status) for the target contract, restoration status, and gaps. An applicable prompt, structured status, or newly generated MCP state is an additional projection, not an alternative to `LOOK`; no transport backlog or exact-delivery replay is permitted. Empty or expired context produces no context output.
