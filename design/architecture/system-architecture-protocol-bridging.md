...
- **Explicit drop conditions (edge layers)** – Commands or lines may be dropped under clearly defined conditions, including:
  - TCP Proxy upstream-bridge failures: if the TCP Proxy cannot establish or maintain its WebSocket bridge to Spring Cloud Gateway, it closes the Telnet connection with a clear, player-visible message and a Telnet disconnect reason of `backend_unavailable` rather than attempting to keep a seemingly-healthy Telnet socket open while the upstream gameplay path is broken. Clients reconnect and re-`LOGIN` as described in [Reconnection Strategy](./system-architecture-reconnection.md).
