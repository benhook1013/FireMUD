...
- Those hints must never bypass fresh `WORLDS` discovery, `LOGIN`, conditional `JOIN`, the realm-scoped character gate, or `PLAY`, and must never retroactively alter an already-established gameplay binding.
