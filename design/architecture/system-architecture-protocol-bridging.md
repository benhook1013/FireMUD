...
- Browser clients cannot read failed WebSocket upgrade headers. Before the socket opens, they therefore apply Gateway's conservative generic recovery rule: treat the connect token as potentially consumed, discard the failed discovery/connect-token bundle, rerun bootstrap discovery, obtain a fresh token, and retry with bounded backoff. Detailed bounded classes remain available to capable non-browser callers and operator telemetry rather than becoming a second browser status API.
