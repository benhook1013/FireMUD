...
The target flow and delivery contracts below remain normative. Explicit `JOIN`/`Join & Play` and the required exact `membershipAuthorityGeneration`, `membershipLifecycleState`, and `membershipVersion` reread at connect-token issuance remain unimplemented/proof gaps in the current runtime. Current connect-token issuance and text `PLAY` consume the Account membership response fields `membershipExists`, `gameplayAdmissionAllowed`, `membershipVersion`, and `evaluatedAt`, and proceed only when the two admission fields are true; an eligible public-production request with `membershipExists=false` may fail closed with non-actionable `JOIN_REQUIRED`. The live Account adapter has no `membershipLifecycleState`: `membershipExists=false` is the current missing-membership case, while `membershipExists=true` with `gameplayAdmissionAllowed=false` is existing non-admitting membership and cannot be classified as `INACTIVE`. The obsolete implicit membership-writer surface has been removed. Current non-public missing or non-admitting membership remains `WORLD_ACCESS_DENIED`; target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` is defined below. Current private/playtest text `PLAY` also requires the current realm grant, while current connect-token issuance validates pointer/visibility but not the private/playtest grant; target-only connect-token grant validation remains implementation drift. A connect token or returning shortcut must not be treated as authorization proof. Returning members may proceed through the current existing-membership path, but that behavior does not prove the target generation/version reread at connect-token issuance; the target requires lifecycle state, authority generation, and independent membership version from one fresh atomic membership snapshot. Every target admission uses the applicable tenant runtime entitlement; private/playtest uses its exact Account-owned grant rather than the public-production enrollment predicate `allowPublicJoin`. Fresh entitlement is mandatory for a new Telnet admission or fresh gameplay binding. A retained-edge continuation may use bounded last-known-good entitlement only for the exact same still-resumable, non-expanding binding under ADR 0028, while current membership, grant, revocation, and resume-lease checks remain mandatory. This implementation drift does not relax the at-most-once delivery, authentication, admission, reconnect, or close-taxonomy requirements.
...
A retained-edge upstream rebind is not a reconnect from the client's perspective. Gateway and the proxy may rebind the established edge to a replacement Game Session owner without repeating `LOGIN`, `JOIN`, or `PLAY`; this exact same-binding, non-expanding continuation may use bounded last-known-good entitlement only under ADR 0028 and with current Account membership, grant, revocation, and resume-lease checks. This exception does not change the strict fresh-entitlement rules for a fresh gameplay binding/new admission or a client-facing edge disconnect; an exact same-binding reconnect on a new transport remains governed by the separate ADR 0028 resume rule.
