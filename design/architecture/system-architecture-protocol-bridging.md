...
- Planned `service_restart` is attributed as `bridge_shutdown_class=planned_drain`; every other valid authenticated upstream close uses the existing `upstream_logout` attribution. Any subreason is optional diagnostic context only.
