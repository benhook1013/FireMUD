...
- All gameplay sessions require in-band `LOGIN` and `PLAY` before gameplay commands. Credential-bearing text clients use conditional in-band `JOIN` for first public-production membership; that operation may create membership only when missing. Browser and other first-party WebSocket clients complete the equivalent HTTP `Join & Play` operation before connect-token issuance and do not send `JOIN` in-band. Returning members skip the join operation. Private/playtest clients must already have both membership and the applicable current grant; neither the grant nor connect-token/`PLAY` admission creates membership.
...
Game Session translates admission outcomes into the shared `ERROR <CODE> <message>` form on the established gameplay protocol. The routing, scope, and tenant-billing subset comprises the first four outcomes below; together with the authority and membership outcomes that follow, this remains a canonical admission inventory rather than an inventory of every character or protocol-stage error. All preserve the authenticated pre-`PLAY` session and create no gameplay binding:
