...
- Normal clients may optionally issue `WORLDS` for discovery. `REALMS <world>` uses one `public_production_onboarding` route class whether discovery precedes membership or follows tenant/world resolution: public-production discovery may be pre-membership, while non-public results require exact membership with `membershipLifecycleState=ACTIVE` plus the current Account grant. Both stages use the shared catalog/pointer pair; unusable pointer evidence is `ADMISSION_POINTER_UNAVAILABLE` and a complete `CLOSED` realm is `REALM_UNAVAILABLE`. **Target first-entry sequence:** `LOGIN` -> conditional `JOIN` -> realm-scoped `CHARS`/character creation -> `PLAY`; a returning member with an already selected character may use the abbreviated `LOGIN` -> `PLAY` path and skip `JOIN`/`CHARS`. Explicit `JOIN`/`Join & Play` is not implemented in the current runtime, so only credential-bearing text clients with existing membership and a selected character may still use direct `LOGIN` -> `PLAY`; this implementation drift does not change the target sequence. Private/playtest clients never use public `JOIN`: they require existing membership and the applicable grant, and no later step may create membership. Typed `SESSION` lines are not part of the Telnet contract. If smart-client attach hints return later, they travel as hidden MCP metadata and remain advisory transport context only.
