...
- All gameplay sessions require in-band `LOGIN` and `PLAY` before gameplay commands. For a public-production target, missing or `INACTIVE` membership is canonically `JOIN_REQUIRED` only after the selected target's `allowPublicJoin` policy permits repair: credential-bearing text clients use the conditional in-band `JOIN`, while browser and other first-party WebSocket clients complete the equivalent HTTP `Join & Play` operation before connect-token issuance and do not send `JOIN` in-band. Returning `ACTIVE` members skip the join operation after current membership authority/version checks; a valid current character is additionally required only for the abbreviated direct-`PLAY` path, otherwise they proceed through `CHARS` or creation. Private/playtest clients with missing or non-`ACTIVE` membership receive `NON_PUBLIC_ENROLLMENT_REQUIRED`, must already have `membershipLifecycleState=ACTIVE` membership and the applicable current grant, and never use public `JOIN`; neither the grant nor connect-token/`PLAY` admission creates membership.
...
For every public-production membership repair requested through `JOIN`, `Join & Play`, or `JOIN_REQUIRED`, the selected target's authoritative `allowPublicJoin` policy is evaluated before the repair is presented, invoked, or treated as eligible to continue. The [Account membership contract](./microservices/account-service/api-contracts.md#grpc-apis) owns that atomic policy and membership decision. If the policy denies joining, the result is `PUBLIC_PRODUCTION_ADMISSION_DENIED`; the operation preserves the existing membership state and cannot create or restore membership. The client must stop before `CHARS`, character creation, connect-token issuance, `LOGIN`, or `PLAY`. Only after the policy permits joining may the explicit operation create missing membership or restore `INACTIVE` membership, and only after that successful `ACTIVE` result may the flow continue to character repair, token issuance, `LOGIN`, or `PLAY`. Existing `ACTIVE` members use the separate current-membership admission path and do not require membership repair.
...
- `ENTITLEMENT_UNAVAILABLE` - Fresh entitlement authority cannot be established for `PLAY` or the join/admission decision. Preserve authentication, create no gameplay binding or new membership, and retry with bounded backoff; stale entitlement is allowed only under the exact bounded same-binding continuity contract. This is distinct from `AUTH_UNAVAILABLE`, which covers unavailable membership, grant, registry, or other required non-routing, non-entitlement authority.
