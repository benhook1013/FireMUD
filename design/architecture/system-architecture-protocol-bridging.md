...
- After a successful reconnect and `PLAY`, Game Session presents/restores an available, authorized bounded per-player retained-context window in canonical order, followed by fresh state reconstruction output derived from current authoritative state (including a fresh `LOOK` and prompt policy, or a status snapshot or newly generated MCP state) for the new transport. Empty or expired context, or context omitted under CMD-04 bounds, produces no context output.
