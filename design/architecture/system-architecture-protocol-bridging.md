...
Where transport/framework constraints allow, Telnet disconnect messages and structured logs should preserve the same bounded subreason context used by the WebSocket side (`user_logout`, `takeover`, `gateway_restart`, `admin_termination`, `edge_backpressure`, `none`) so deploy drains and edge pressure can be distinguished from true outages without introducing a separate Telnet-only taxonomy.
