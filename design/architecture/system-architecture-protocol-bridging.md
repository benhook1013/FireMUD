...
- A second client successfully takes over the same `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` binding, or an admin ends that session cleanly. The active `gameInstanceId` is runtime-fence/value evidence, not durable character identity.
