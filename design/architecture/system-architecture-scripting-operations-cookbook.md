...
In all of these cases, `script_event_audit` is the primary source of truth for Automation-owned handler lifecycle, while `ListScriptHandoffEvents` is the source for per-command handoff and later execution dispositions. Combined operator views must show the handler fields together with `commandHandoffDispositions[]` keyed by `automationDispatchId`; metrics from the parent doc’s glossary indicate whether the problem is localized to a script/plugin, a tenant budget, or cluster capacity.
...
1. **Fence new evaluation**
   - Pause tick execution and set Automation & Scripting admission to rollback pause mode for the affected scope before repin, explicitly including timer admission, so new triggers do not refill queues during cleanup.
...
3. **Enforce execution-time version fencing**
   - Game Session must reject any queued tick commands whose embedded `scriptPatchVersion` does not match the instance’s currently pinned version, and record a per-command handoff result returned by `ListScriptHandoffEvents` with the full applicable Trigger Identity and command identity, including `tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `entityId`, `scriptId`, `scriptEventId`, `automationDispatchId`, and `scriptPatchVersion`, so operators can diagnose rollback impact.
4. **Resume in order**
   - Return Automation & Scripting admission to normal only after schedule reconciliation, cancel/purge, and rollback draining confirm that every stale old-version execution is terminal or fenced and that pre-pause executions and cancelable outbox work for the current rollback-scope `admissionEpoch` have quiesced, then resume ticks.
...
4. During target-version reconciliation for `tenantId=11111111-1111-4111-8111-111111111111`, `gameInstanceId=44444444-4444-4444-8444-444444444444`, and target `P21`, create or confirm the corresponding schedule entries across every region of the instance before retiring `P22`-owned entries. Replacement and retirement are one atomic durable result or a resumable idempotent operation keyed by `controlPlaneRequestId=RB-42`; preserve matching `scheduleDefinitionId` values only as logical schedule identities and create no firing claim or `scriptEventId`. Any `playableStateScope` change is also a schedule/runtime migration fence. This system-owned scheduler mutation records `requestedBy=operator:alice` and `executedBy=system:automation` as separate audit fields, with the same `reason="rollback RB-42"`; it must not overload `actor` with the executing principal.
...
7. Poll `GetAutomationDrainStatus(tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444)` until `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` for the current rollback-scope `admissionEpoch`. Expect a bounded number of old-epoch audit rows for executions admitted before pause and later fenced by the advanced `admissionEpoch`; these remain non-success outcomes, not silent loss.
8. Call `SetAutomationAdmissionMode(tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444, mode=NORMAL, controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` only after reconciliation, cancellation, purge, convergence, and drain checks pass.
9. Call `ResumeTicks(tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444, controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` last so gameplay resumes only after both runtime services agree on the rollback target and old-version work has quiesced.
...
3. Roll back the active script patch if necessary.
   ...
   - Repinning must also ensure rollback safety:
     ...
     - These drops must be visible through `ListScriptHandoffEvents` and the combined `commandHandoffDispositions[]` view alongside `script_event_audit` and operator dashboards so rollback impact is diagnosable per command.
