...
In all of these cases, `script_event_audit` is the primary source of truth for Automation-owned handler lifecycle. The complete per-command handoff diagnostic model in the remainder of this section is **target-state**: the live Game Session proto has `automationDispatchId`, command id/text, and selected provenance fields, but not `commandOrdinal` or the full Trigger Identity. Current live reads therefore expose narrower diagnostics. In the target state, `ListScriptHandoffEvents` is the source for per-command handoff and later execution dispositions, and combined operator views must show the handler fields together with `commandHandoffDispositions[]` keyed by the complete command identity; metrics from the parent doc’s glossary indicate whether the problem is localized to a script/plugin, a tenant budget, or cluster capacity.
