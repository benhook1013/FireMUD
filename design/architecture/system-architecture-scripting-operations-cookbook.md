...
3. Roll back the active script patch if necessary.
   ...
   - Repinning does not backfill skipped logical firings or rewrite existing automation queues; automation and tick processing continue from the current point in time under the older patch. The same Trigger Identity remains idempotent and replay-safe, while physical evaluation attempts may retry only before `EVALUATED_COMMITTED` under the lifecycle contract.
