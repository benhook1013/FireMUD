...
5. **Resume in order**
   ...
   - **Target state only:** Cancel `PENDING_EVALUATION` durably to terminal `CANCELED` with `finalStage=ADMISSION`, `finalOutcome=canceled`, and the bounded rollback/operator reason, without entering the DSL. For `EVALUATING`, first inspect the descriptor-commit marker: resume committed descriptors without DSL re-entry; transition an explicitly canceled uncommitted trigger to terminal `CANCELED` with `finalStage=DSL_EVAL`; transition an expired stale uncommitted trigger to terminal `DEAD_LETTERED` with `finalStage=DSL_EVAL`, `finalOutcome=canceled`, and `finalReason=stale_execution_fenced`. Under the current implementation, keep unresolved `EVALUATING` active and the rollback fences in place; do not infer terminal/fenced progress, reclaim the row, re-enter the DSL, or advance to `ResumeTicks` from lease expiry. If the canonical convergence predicate does not succeed by its deadline, follow the owner workflow's `ROLLBACK_CONVERGENCE_TIMEOUT` and retain the fences until the supported idempotent `ResumeTicks` action is safe.
...
7. Poll `GetAutomationDrainStatus(tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444)` until the canonical two-layer drain is complete. Expect a bounded number of old-epoch audit rows for executions admitted before pause and later fenced by the advanced `admissionEpoch`; these remain non-success outcomes, not silent loss. **Target state only:** cancel `PENDING_EVALUATION` without DSL entry; for `EVALUATING`, resume a committed descriptor without DSL re-entry, dead-letter an expired stale row only when no descriptor was committed, and apply the explicit-cancellation terminal mapping before advancing to `ResumeTicks`. Under the current implementation, unresolved `EVALUATING` remains counted as active and every handoff-capable `PENDING_EVALUATION` remains counted as pending; keep the fences in place and do not infer that the drain is complete from lease expiry.
...
- `ReplayDeadLetteredWorkItems(tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444, scriptPatchVersion=P21, controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` for bounded replay of recoverable target-version items. Plugin-backed candidates must match the immutable `(pluginId, pluginVersionId, bindingId)` stored on each dead-lettered work item against the active binding for the same scoped `<tenantId, gameInstanceId, pluginId>`; ingress audit is supplemental provenance, not the eligibility source.
