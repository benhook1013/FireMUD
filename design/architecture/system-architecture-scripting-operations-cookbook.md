...
3. Roll back the currently pinned script tuple if necessary.
   ...
   - Repinning must also ensure rollback safety:
     ...
     - **Target-state cleanup:** if plugin versions are also being rolled back, disabled, or revoked, pending work for displaced `(pluginId, pluginVersionId, bindingId)` or `pluginActivationGeneration` evidence is canceled before queue purge; this cleanup is likewise bounded and non-blocking and is not the correctness barrier.
     - **Target-state only:** before handoff, Automation validates and carries the exact `(pluginId, pluginVersionId, bindingId)`, captured `pluginActivationGeneration`, and current lifecycle/publication/policy evidence for plugin work. That independent fence survives evaluation, persistence, handoff, replay, final execution, and dead-letter recovery. Game Session enforces the exact script tuple and rejects stale applicable plugin tuple or generation evidence at execution time, including same-version work from an older script epoch or plugin activation generation. **Current live gap:** `EnqueueAutomationCommandIfAbsentRequest` carries `scriptPatchVersion` but lacks `scriptPinEpoch` and the complete plugin fence, so the current handoff cannot make those rejections; do not present them as shipped.
