...
3. **Enforce execution-time version fencing**
   - Game Session must reject any queued tick commands whose embedded `scriptPatchVersion` does not match the instance’s currently pinned version, and record the handoff disposition with the full applicable Trigger Identity and command identity, including `tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `entityId`, `scriptId`, `scriptEventId`, `automationDispatchId`, and `scriptPatchVersion`, so operators can diagnose rollback impact.
4. **Resume in order**
...
4. During target-version reconciliation, create or confirm the corresponding `P21` schedule entries before retiring `P22`-owned entries. Replacement and retirement are one atomic durable result or a resumable idempotent operation keyed by `controlPlaneRequestId`; preserve matching `scheduleDefinitionId` values only as logical schedule identities and create no firing claim or `scriptEventId`. Any `playableStateScope` change is also a schedule/runtime migration fence. This scheduler mutation records `actor=system:automation` and `reason="rollback RB-42"`.
5. Call `CancelPendingWorkItemsForPatch(tenantId=11111111-1111-4111-8111-111111111111, scriptPatchVersion=P22, gameInstanceId=44444444-4444-4444-8444-444444444444, controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")`, then call `PurgeQueuedTickCommandsForScriptPatch` for the same scope and operation identity.
6. Poll `GetAutomationPinConvergence` and `GetGameSessionPinConvergence` until both report `observedPinnedScriptPatchVersion=P21` and the latest observed `controlPlaneRequestId=RB-42`.
7. Poll `GetAutomationDrainStatus` until `activeExecutionCount=0` and `pendingCancelableWorkItemCount=0` for the paused scope. Expect a bounded number of old-epoch audit rows for executions admitted before pause and later fenced by the advanced `admissionEpoch`; these remain non-success outcomes, not silent loss.
8. Call `SetAutomationAdmissionMode(..., mode=NORMAL, controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` only after reconciliation, cancellation, purge, convergence, and drain checks pass.
9. Call `ResumeTicks(..., controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` last so gameplay resumes only after both runtime services agree on the rollback target and old-version work has quiesced.
...
- `ReplayDeadLetteredWorkItems(..., controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` for bounded replay of recoverable items.
- `PurgeOutboxWorkItems(..., controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` for auditable cleanup of terminally invalid/stale items.
