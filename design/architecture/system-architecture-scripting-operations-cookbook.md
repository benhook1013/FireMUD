...
In all of these cases, `script_event_audit` is the primary source of truth for Automation-owned handler lifecycle. **Current live diagnostics** use that handler audit, the current Automation `ListScriptHandoffEvents` rows with their narrower per-command/work-item fields, and the Game Session command/result/fence fields that are currently exposed. They do not yet provide one downstream execution-disposition view keyed by the complete applicable Trigger Identity because the live Game Session handoff does not carry `commandOrdinal` and all Trigger Identity fields end to end. **Target state** adds the complete `ListScriptHandoffEvents`/`commandHandoffDispositions[]` view keyed by `tenantId`, `gameInstanceId`, `playableStateScope`, region and epoch, handler identity, `automationDispatchId`, `commandOrdinal`, and the applicable target identity. Metrics from the parent doc’s glossary indicate whether the problem is localized to a script/plugin, a tenant budget, or cluster capacity.
...
3. **Enforce execution-time version fencing**
   - **Target-state diagnostic requirement:** Game Session must reject any queued tick commands whose embedded `scriptPatchVersion` does not match the instance’s currently pinned version, and record a per-command handoff result returned by `ListScriptHandoffEvents` with the full applicable Trigger Identity and command identity, including `tenantId`, `gameInstanceId`, `playableStateScope`, `regionId`, `regionEpoch`, `entityId`, `scriptId`, `scriptEventId`, `automationDispatchId`, `commandOrdinal`, and `scriptPatchVersion`, so operators can diagnose rollback impact. The live proto does not yet expose this complete handoff diagnostic payload.
4. **Resume in order**
...
3. Roll back the active script patch if necessary.
   ...
   - Repinning must also ensure rollback safety:
     ...
     - **Target state:** These drops must be visible through `ListScriptHandoffEvents` and the combined `commandHandoffDispositions[]` view alongside `script_event_audit` and operator dashboards so rollback impact is diagnosable per command. **Current live fallback:** correlate `script_event_audit`, the narrower Automation handoff rows, and the Game Session command/result/fence fields currently exposed; do not infer a complete Trigger Identity or downstream per-command disposition when the live handoff does not carry it.
