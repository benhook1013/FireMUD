...
3. Roll back the currently pinned script tuple if necessary.
   ...
   - Repinning must also ensure rollback safety:
     ...
     - **Target-state cleanup:** if plugin versions are also being rolled back, disabled, or revoked, pending work for displaced `pluginVersionId` values is canceled before queue purge; this cleanup is likewise bounded and non-blocking.
     - **Target-state only:** Game Session enforces an exact tuple fence at execution time and rejects any tick-queue entry whose embedded `(scriptPatchVersion, scriptPinEpoch)` does not match the currently pinned value, including same-version work from an older epoch. **Current live gap:** `EnqueueAutomationCommandIfAbsentRequest` carries `scriptPatchVersion` but lacks `scriptPinEpoch`, so the current handoff cannot reject same-version old-epoch work; do not present that rejection as shipped.
