...
- **Target-state only:** `ListOutboxWorkItems(tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444, scriptPatchVersion=P22, pluginVersionId=plugin-v22, workItemStatus=PENDING)` inspects evaluated descriptor/outbox work for the instance; omit `pluginVersionId` only when the operation is not plugin-owned. The current diagnosis remains the `PENDING_EVALUATION` and `EVALUATING` state distinction above, without treating either state as evaluated-command `PENDING`.
- `ReplayDeadLetteredWorkItems(tenantId=11111111-1111-4111-8111-111111111111, references=[{tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444, playableStateScope=isolated, regionId=R2, regionEpoch=14, automationDispatchId=work-9, commandOrdinal=0}], controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` for bounded replay of one recoverable command. Plugin-backed candidates must match the immutable `(pluginId, pluginVersionId, bindingId)` stored on the selected dead-lettered descriptor against the active binding for the same scoped `<tenantId, gameInstanceId, pluginId>`; ingress audit is supplemental provenance, not the eligibility source.
- `PurgeOutboxWorkItems(tenantId=11111111-1111-4111-8111-111111111111, references=[{tenantId=11111111-1111-4111-8111-111111111111, gameInstanceId=44444444-4444-4444-8444-444444444444, playableStateScope=isolated, regionId=R2, regionEpoch=14, automationDispatchId=work-9, commandOrdinal=0}], controlPlaneRequestId=RB-42, actor=operator:alice, reason="rollback RB-42")` for auditable cleanup of one terminally invalid or stale command descriptor.

For command-level diagnosis, use [Command Identity and Live Handoff Boundary](./system-architecture-scripting-rollout-and-rollback.md#command-identity-and-live-handoff-boundary). The target query is `ListScriptHandoffEvents`; the current live fallback is the correlation of `script_event_audit`, narrower Automation handoff/work-item rows, `outboxWorkItemId`, `automationDispatchId`, `gameSessionCommandId`, and the Game Session command/result/fence fields currently exposed. Do not infer complete Trigger Identity or downstream per-command disposition from that fallback.
