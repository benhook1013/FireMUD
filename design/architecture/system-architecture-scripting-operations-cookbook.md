...
Metric names, labels, and increment units in the examples below are illustrative query patterns only. The [normative Table 4](./system-architecture-scripting-normative-contract-tables.md#table-4-metrics-label-matrix) is the sole metric schema owner; this cookbook records only local operator consequences and diagnostic usage.
...
The current runtime does not prove target-state drain recovery from lease age: unresolved `EVALUATING` work remains active and handoff-capable `PENDING_EVALUATION` remains pending. Keep the rollback admission and tick fences in place until the owner workflow reports the required state; do not infer reclaim, terminalization, or safe resumption locally.
