...
3. Roll back the currently pinned script tuple if necessary.
   ...
   - Repinning must also ensure rollback safety:
     ...
     - **Target-state cleanup:** if plugin versions are also being rolled back, disabled, or revoked, pending work for displaced `(pluginId, pluginVersionId, bindingId)` or `pluginActivationEpoch` evidence is canceled before queue purge; this cleanup is likewise bounded and non-blocking and is not the correctness barrier.
