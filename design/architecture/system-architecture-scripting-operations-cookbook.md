...
- **Target-state only and capability-gated:** `PurgeOutboxWorkItems(tenantId=11111111-1111-4111-8111-111111111111, scriptPatchVersion=P22, scriptPinEpoch=41, workItemIds=[work-9], controlPlaneRequestId=PURGE-42, actor=operator:alice, reason="rollback RB-42")` is auditable cleanup only when each selected work item matches the displaced exact tuple, is terminal (`HANDED_OFF`, `CANCELED`, or `DEAD_LETTERED`), its parent trigger is `EVALUATED_COMMITTED`, and its configured retention horizon has elapsed. A same-version repin with a newer epoch does not by itself make terminal rows from the displaced tuple ineligible; the selector must remain the displaced exact tuple, so current-epoch rows cannot be selected. When terminal descriptor/evaluated-output evidence is purged under the retained parent, the same transaction persists durable `descriptorEvidencePurgedAt`; that marked parent is non-recoverable and non-replayable, and consumers reject it deterministically rather than treating missing children as recoverable. The purge preserves the trigger marker, corresponding `script_event_audit`, and recovery-causation evidence; it does not mutate active or nonterminal work.
