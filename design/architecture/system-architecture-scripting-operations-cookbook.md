...
3. Roll back the currently pinned script tuple if necessary.
   - If regressions are widespread or difficult to isolate, use the Game Session-owned workflow to explicitly repin the affected game instance to the exact target `(scriptPatchVersion, scriptPinEpoch)` selected by the operator. Concretely:
