...
This file is intentionally focused on **concepts and invariants**: roles, tail-loss envelopes, shard discipline, and high-level coordination guarantees. Detailed Lua authoring patterns and concrete runbooks live in:

- [Redis Lua Patterns](./system-architecture-redis-lua-patterns.md) – script categories, idempotency rules, `schemaVersion` handling.
- [Redis Operations & Migrations](./system-architecture-redis-operations.md) – AOF profiles, coordination resets, and migration flows.
- [Coordination Redis Ops Access & Tooling](./system-architecture-redis-ops-access.md) – ACLs, allowed commands, and ops personas.
- [Redis Cheat Sheet](./system-architecture-redis-cheatsheet.md) – quick routing by prefix and common tasks.
...
For a task-oriented entry point (for example “add a new coordination prefix” or “plan a coordination reset”), start with the [Redis Cheat Sheet](./system-architecture-redis-cheatsheet.md) and follow the linked sections from there.

### Topology Compatibility Overview

Redis features and assumptions in this document span both single-instance and clustered deployments. Use the table below as a quick compatibility guide; operational details are expanded in [Redis Operations & Migrations](./system-architecture-redis-operations.md).
...
In all cases, Coordination Redis and Cache/Rate-Limit Redis remain **separate deployments** (distinct processes/containers) even when they run on the same host.
...
To make misconfigurations easier to detect:
...
- Shared configuration helpers in `firemud-common` should perform a best-effort check (for example, comparing host/port pairs) and emit a clear log warning and health indicator when the two roles resolve to identical targets.
- Operational dashboards should include a simple “Redis role endpoints” panel so operators can verify at a glance that coordination and cache deployments are distinct for each environment.
...
### Redis Version Policy

FireMUD assumes a stable Redis major/minor line for Coordination Redis so AOF, replication, and Lua behavior remains predictable.

- Docker Compose pins Redis to `redis:7.4.3` (`docker/docker-compose.yml`).
- Kubernetes examples pin the Bitnami Redis chart to a specific chart version in Terraform (`k8s/terraform/main.tf`, `k8s/terraform-production/main.tf`); the pinned chart currently packages Redis `7.4.3`.

Upgrading Redis versions (or the Redis chart version) is treated as an operational change that should be validated with a restart/replay drill and basic tick/session smoke tests.
...
On restart, Redis replays AOF entries in order. Depending on Redis version and configuration, Lua-driven mutations may be represented in AOF and replication streams as either:

- The original `EVAL`/`EVALSHA` invocation (re-running the script), or
- The expanded sequence of mutating Redis commands executed by the script.

FireMUD does **not** rely on a specific representation for correctness. The coordination contract is instead:

- Restart and failover may cause a subset of recent coordination state to be lost within the tail-loss envelope, but replay of whatever survives must not violate the safety invariants (no double-apply, at-most-one active executor per region, and session binding monotonicity).
- Lua scripts must be deterministic and idempotent with respect to their declared `KEYS`, `ARGV`, and current Redis state so that either representation is safe to replay.

Tick and session scripts are therefore required to be idempotent with respect to their inputs and to:
...
Structured coordination payloads (pending ticks, timers, retry queues, and other versioned coordination values) expose a `schemaVersion` so Lua scripts can safely evolve. Best-effort hint markers such as `remote:<tenantId>:<entityId>` are intentionally not treated as versioned payloads.
...
> 🔗 `remote:<tenantId>:<entityId>` keys are best-effort hints for cross-region follow-ups. See [Cross-Region Command Execution and Result Relay](./system-architecture-ticks.md#cross-region-command-execution-and-result-relay)
> for details.
> 📌 For session-related keys and structure, see [Session Keys and Gameplay Binding](#session-keys-and-gameplay-binding)
> ⚠️ Tick regions are **always scoped to a single Redis shard** to preserve atomicity for multi-key Lua scripts. Player sessions are single-key and do not require shard co-location; session keys are intentionally allowed to distribute across the cluster to avoid hot shards.
