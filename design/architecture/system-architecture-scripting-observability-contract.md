...
The complete per-command handoff diagnostic model is **target-state**. The live `EnqueueAutomationCommandIfAbsent` contract currently carries the narrower scope, dispatch/work-item, script/plugin provenance, target-command, routing, and origin-source fields listed in [Scripting Runtime Execution](./system-architecture-scripting-runtime-execution.md#work-item-outbox-contract-normative), plus the returned Game Session `commandId`/admission outcome. It does not yet carry `scriptPinEpoch`, `commandOrdinal`, or the full applicable Trigger Identity, including fields such as `bindingId`, `eventType`, `eventSchemaVersion`, `scriptEventId`, `isDryRun`, and scheduler identity. Current live command status/readbacks therefore expose only that narrower fallback surface; the examples below must not be read as evidence that the full target-state handoff contract is already implemented. The legacy dry-run's internal `HANDED_OFF` work-item status is not its terminal audit stage and is not evidence of downstream handoff. The current onLoad executor records `completed_no_commands` for each resolved non-emitting handler; patch-level `readiness_success` is a target-state, unimplemented outcome. Current readiness evidence is the projection status (`READY`, `ONLOAD_RUNNING`, `FAILED`, with lifecycle statuses such as `ROLLED_BACK` or `SUPERSEDED` where applicable) plus `statusReason`; no current readiness-success outcome is emitted.

The ADR 0114 command-plan preview/result surface is target-only, unimplemented, and unproved; the current legacy materialized dry-run path remains a separate handler/audit behavior. Complete plugin-fence observability is also target-only: current audit, handoff, and readback surfaces do not yet prove propagation of the applicable component-revocation fence and Automation-owned `(pluginActivationEpoch, lifecycleRevision)` through every required execution, staged-effect, retry, replay, recovery, and final-effect boundary.
...
- Metric schema and exact vocabulary are owned by [Table 4](./system-architecture-scripting-normative-contract-tables.md#table-4-metrics-label-matrix). Every application family carries the bounded runtime identity of its emitting service in `service` plus only that table's family-specific finite labels; Automation-owned families use `service="automation-scripting-service"`, while the Game Session-owned `automation_pin_convergence_timeout_total` uses `service="game-session-service"`. Creator/producer strings and raw `tenantId`, `gameInstanceId`, `regionId`, `scriptId`, `pluginId`, `pluginVersionId`, and dispatch/event identities belong in audit rows, protected logs/traces, and control-plane queries rather than Prometheus labels. Unknown values collapse only to the `other` or `unknown` categories permitted by Table 4. The queue-delay family samples waiting as well as processed live work by Table 4's bounded `priority`, so prolonged background deferral remains measurable even when it has not yet produced a skip or terminal outcome.
...
- Event-scope ingress audit/logging records genuine pre-resolution domain decisions for the incoming event, such as reload backpressure, rollback pause, pin-state unavailability, signer-policy unavailability, or version unavailability. A typed event-scope rejection returns `admitted=false` with its `admissionOutcome` and `admissionReason`, records the same pair in `script_event_ingress_audit`, and has the corresponding Table 4 ingress-drop consequence using the bounded reason. Failures proven to occur before event-scope claim creation—including transport authentication/authorization, schema or request validation, deadline/cancellation, resource exhaustion, dependency unavailability, and internal failures that prevent producing an admission decision—use canonical non-OK gRPC status and do not require admission fields, an ingress-audit row, or an ingress-drop metric. Once a claim exists, failures preserve that claim and its single ingress-audit row under the claim's retry/recovery or terminalization lifecycle rather than becoming a no-audit outcome. Event-scope records use the uniqueness key and atomic first-claim/retry rules in [Table 1](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields), must not invent a synthetic `scriptId`, `bindingId`, or command identity, and must not create a handler-scoped `script_event_audit` row. The live implementation uses a bounded `claimStartedAt` lease and compare-and-set row-version fence to recover stale `IN_PROGRESS` claims under the same identity; stale-owner recovery remains visible through the durable reclaim reason.
- `script_event_audit` records resolved-handler/materialized-work lifecycles, including scheduler/timer-scoped and current legacy materialized dry-run/test executions after a concrete script or plugin handler identity exists. Tenant-readiness `onLoad` uses this surface only for a non-empty patch, with exactly one row per resolved `onLoad` handler/Trigger Identity; it never creates an aggregate patch-readiness or zero-handler handler row. Target ADR 0114 command-plan previews use the isolated preview result/audit surface instead. For event ingress, pre-handler dry-run rejection, pre-handler signer-policy unavailability, and rollback backpressure remain ingress-audit outcomes; scheduler due-candidate skips use the `scheduleCandidateId` candidate-audit surface. **Target state:** a live admitted event with zero handlers is metric-only. The current ingress path still persists an ingress-audit row with `admissionReason=admitted_no_handlers`; this is implementation drift and does not create a handler row. **Target state:** a tenant-readiness notification with an immutable manifest proving zero handlers records terminal `READY` readiness evidence, while an identifiable notification with missing or unverifiable zero-handler proof records terminal `FAILED` with bounded reason `zero_handler_manifest_unverifiable` and normal readiness status/audit evidence; neither readiness path fabricates a handler row or is represented as a live event metric.
...
Each observed resolved-handler/materialized-work trigger, including a scheduler/timer firing after candidate admission and handler resolution, a resolved tenant-readiness `onLoad` handler, or current legacy materialized dry-run/test execution must write (or update) a single handler-audit record keyed by the trigger identity described in [Table 1](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields). For a non-empty readiness patch this is one row per resolved `onLoad` handler, not one row for the patch or readiness aggregate. Target ADR 0114 command-plan previews instead retain their result and provenance in the isolated preview result/audit surface and do not create or update a `script_event_audit` row. For event ingress, pre-handler dry-run rejection, pre-handler signer-policy unavailability, and rollback backpressure remain ingress-audit outcomes; scheduler due-candidate skips use the `scheduleCandidateId` candidate-audit surface. **Target state:** a live admitted event with zero handlers remains metric-only; the current ingress drift is recorded in [Ingress Audit vs Handler Audit](#ingress-audit-vs-handler-audit). **Target state:** readiness notifications are different: an immutable zero-handler manifest records terminal `READY`, while missing or unverifiable zero-handler proof for an identifiable patch writes the durable readiness `FAILED` status/audit outcome, in both cases without fabricating a handler row.
...
- Once that event-scope claim exists, every failure after claim creation preserves the claim and its single ingress-audit row in `IN_PROGRESS` or another explicitly retryable state until recovery or terminalization; it must not silently erase the claim/audit pair. The no-claim/no-audit outcome is reserved for failures proven to occur before claim creation.
...
- Identity and versioning
  ...
  - `scriptPinControlPlaneRequestId` (retained immutable owner/audit evidence, required with a positive `scriptPinEpoch` for gameplay/runtime and scheduler triggers; absent for tenant-readiness `onLoad` and explicitly unpinned epoch-zero records)
  - `scriptEventId`
  - `isDryRun` (boolean)
...
- Outcomes (stage-aware)
  ...
  - For a plugin breaker admission denial, `finalReason=failure_rate_breaker` is canonical; `breakerState=DISABLED_DUE_TO_ERRORS`, bounded `breakerReason=failure_rate_breaker` (the same value, carried as runtime-state evidence rather than a competing audit reason), and the exact active `pluginVersionId`/runtime scope that was checked are retained
...
- `DRY_RUN_RESULT` is not a `script_event_audit` stage. It is target ADR 0114 preview-stage/outcome terminology for a non-committing command-plan preview completed after DSL evaluation; the preview returns the bounded would-be commands to the authorized caller or retains them on its isolated preview result/audit surface. Either path avoids persisting a live work item or handing off to tick queues. The current legacy materialized dry-run/test handler has not converged on this isolated preview surface: it records `finalStage=DSL_EVAL`, `finalOutcome=dry_run_completed`, and `finalReason=dry_run_no_handoff` in `script_event_audit`.

Required fields:

- `finalStage` must be one of the handler-audit stages above and must match the last stage attempted for the trigger. `DRY_RUN_RESULT` is excluded from `script_event_audit`; the current legacy materialized dry-run/test handler is an explicit implementation exception that records `DSL_EVAL`/`dry_run_completed`/`dry_run_no_handoff` and does not claim `WORK_ITEM_PERSIST` or `TICK_HANDOFF` as its terminal audit stage. Target ADR 0114 previews use the non-committing `DRY_RUN_RESULT` semantics only through the authorized result path defined by that ADR: direct return to the authorized caller or the isolated preview result/audit surface.
...
- Target-state tenant readiness is a separate patch-level outcome: successful completion of a non-empty patch uses readiness-owner/projection status `READY` with readiness-level outcome `readiness_success`; it is not a resolved handler's `finalOutcome`. Each resolved `onLoad` handler remains independently audited and, when it emits no commands, uses `finalStage=DSL_EVAL`, `finalOutcome=completed_no_commands` (or its actual handler failure outcome). Tooling must keep this per-handler outcome separate from the patch-level readiness result. A zero-handler readiness notification uses the terminal `READY` shortcut only with immutable manifest proof, or a durable readiness `FAILED` status/audit outcome with bounded reason `zero_handler_manifest_unverifiable` when that proof is missing or unverifiable; these readiness outcomes are not handler `finalOutcome` values.
- `finalOutcome=dry_run_success` must imply `finalStage=DRY_RUN_RESULT` and `isDryRun=true`. It means only that the target ADR 0114 non-committing preview completed and its inspectable would-be commands were returned directly to the authorized caller or retained on the isolated preview result/audit surface; it never creates a handler `script_event_audit` row. The legacy materialized dry-run path instead uses `DSL_EVAL`/`dry_run_completed`/`dry_run_no_handoff`.
...
- Event-scope admission, skip, and drop outcomes remain distinct from resolved handler outcomes. The zero-handler target/current boundary and its audit behavior are defined in [Ingress Audit vs Handler Audit](#ingress-audit-vs-handler-audit); Table 4 owns the bounded metric-only `outcome="admitted_no_handlers"` consequence, while this section does not restate implementation status.
- Each typed event-scope rejection has the Table 4 ingress-drop consequence with its bounded event-scope `admissionReason`, including `signer_policy_unavailable` when applicable. Transport/pre-domain failures use canonical non-OK gRPC and do not imply an ingress-audit row, admission fields, or a handler audit row/handler `finalOutcome`.
