...
- Tenant-readiness `onLoad` completion for a non-empty patch with resolved handlers must use `finalStage=DSL_EVAL` and `finalOutcome=readiness_success`; a valid live handler emitting no commands uses `finalOutcome=completed_no_commands`; neither is a live gameplay handoff signal. A zero-handler readiness notification uses the terminal `READY` shortcut only with immutable manifest proof, or a durable readiness `FAILED` status/audit outcome with bounded reason `zero_handler_manifest_unverifiable` when that proof is missing or unverifiable; these readiness outcomes are not handler `finalOutcome` values.
