...
- `script_event_audit` records resolved-handler/materialized-work lifecycles, including scheduler/timer-scoped, tenant-readiness `onLoad`, and current legacy materialized dry-run/test executions after a concrete script or plugin handler identity exists. Target ADR 0114 command-plan previews use the isolated preview result/audit surface instead. For event ingress, pre-handler dry-run rejection, pre-handler signer-policy unavailability, and rollback backpressure remain ingress-audit outcomes; scheduler due-candidate skips use the `scheduleCandidateId` candidate-audit surface. A live admitted event with zero handlers is metric-only. A tenant-readiness notification with an identifiable but missing or unverifiable zero-handler manifest is a separate readiness failure: the readiness owner durably records `FAILED` with bounded reason `zero_handler_manifest_unverifiable` and emits the normal readiness status/audit evidence; it is not silently rejected or represented as a live event metric.
...
Each observed resolved-handler/materialized-work trigger, including a scheduler/timer firing after candidate admission and handler resolution, tenant-readiness `onLoad` trigger, or current legacy materialized dry-run/test execution must write (or update) a single handler-audit record keyed by the trigger identity described in [Table 1](./system-architecture-scripting-normative-contract-tables.md#table-1-trigger-identity-required-fields). Target ADR 0114 command-plan previews instead retain their result and provenance in the isolated preview result/audit surface and do not create or update a `script_event_audit` row. For event ingress, pre-handler dry-run rejection, pre-handler signer-policy unavailability, and rollback backpressure remain ingress-audit outcomes; scheduler due-candidate skips use the `scheduleCandidateId` candidate-audit surface. A live admitted event with zero handlers remains metric-only. Readiness notifications are different: missing or unverifiable zero-handler proof for an identifiable patch writes the durable readiness `FAILED` status/audit outcome, without fabricating a handler row.
