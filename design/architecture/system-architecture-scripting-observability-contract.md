...
- Tenant-readiness `onLoad` completion must use `finalStage=DSL_EVAL` and `finalOutcome=readiness_success`; a valid live handler emitting no commands uses `finalOutcome=completed_no_commands`; neither is a live gameplay handoff signal. A readiness notification claiming zero handlers may use the terminal `READY` shortcut only with immutable manifest proof; missing or unverifiable proof is a durable readiness `FAILED` status/audit outcome with bounded reason `zero_handler_manifest_unverifiable`, not a handler `finalOutcome` or silent rejection.
