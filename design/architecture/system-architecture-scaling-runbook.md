...
- Tick duration vs budget (primary safety ratio):
  ...
  - Watch `tick_execution_time_ms_p95{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode=~"^(normal|solo)$"}` and `tick_execution_time_ms_p99{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode=~"^(normal|solo)$"}` (recording rules derived from `tick_execution_time_ms_bucket{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode=~"^(normal|solo)$",le=~".+"}`) relative to the mode-specific **lock TTL**: normal samples use `tick_execution_time_ms_p99{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode="normal"} / on (scope_class, tick_mode) label_replace(tick_lock_ttl_ms{scope_class=~"^(region|game_instance|tenant|cluster)$"}, "tick_mode", "normal", "scope_class", ".*")`, while solo-budget samples use `tick_execution_time_ms_p99{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode="solo"} / on (scope_class, tick_mode) label_replace(solo_lock_ttl_ms{scope_class=~"^(region|game_instance|tenant|cluster)$"}, "tick_mode", "solo", "scope_class", ".*")`. These are bounded class-level rollups, not selectors for an individual region.
  ...
  - The cadence input/query `tick_interval_ms{scope,scope_class}` is target-only until its deployment-configuration producer and bounded-label contract are implemented and proved. An absent or stale cadence series is `UNKNOWN`, not zero, and blocks a cadence-based capacity decision; continue using authoritative runtime-health/control-plane evidence. When the target input is fresh, for intuition you may also track the same explicit normal/solo branches against its deployment-scoped class-only maximum: normal uses `tick_execution_time_ms_p99{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode="normal"} / on (scope_class, tick_mode) label_replace(max by (scope_class) (tick_interval_ms{scope=~".+",scope_class=~"^(region|game_instance|tenant|cluster)$"}), "tick_mode", "normal", "scope_class", ".*")`, while solo uses `tick_execution_time_ms_p99{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode="solo"} / on (scope_class, tick_mode) label_replace(max by (scope_class) (tick_interval_ms{scope=~".+",scope_class=~"^(region|game_instance|tenant|cluster)$"}), "tick_mode", "solo", "scope_class", ".*")`. Do not directly join the raw `scope` label to target mode-aware families. Decisions should be grounded in the mode-specific TTL-based ratio because production `lock_ttl_ms` and `solo_lock_ttl_ms` are the shared resolver's evidence-calibrated settings. The interval-based relationship is a bootstrap default only, not a production TTL derivation.
...
- Keep `pod_tick_cost_ms` below the effective scheduling envelope implied by a fresh target `tick_interval_ms` input, lock TTL headroom, and observed Redis script latency. If that cadence input is absent or stale, this model is unknown and cannot authorize a capacity decision.
...
- Calibrate each term from load tests in the target profile (`dev_local`, `hobby_self_hosted`, `production_clustered`) and record:
  - `p99_region_tick_ms` from load-test measurements and exact per-region runtime-health evidence; the bounded `tick_execution_time_ms_p99{scope_class=~"^(region|game_instance|tenant|cluster)$",tick_mode=~"^(normal|solo)$"}` rollup is corroborating pressure only and cannot identify a region’s p99.
