...
- Tick duration vs budget (primary safety ratio):
  ...
  - For intuition, you may also track `tick_execution_time_ms_p99{scope_class,tick_mode} / on (scope_class) tick_interval_ms{scope_class}`, but decisions should be grounded in the mode-specific TTL-based ratio because production `lock_ttl_ms` and `solo_lock_ttl_ms` are the shared resolver's evidence-calibrated settings. The interval-based relationship is a bootstrap default only, not a production TTL derivation.
