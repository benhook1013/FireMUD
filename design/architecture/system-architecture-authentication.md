...
- **Tenant authority-generation scope** – `session:auth:generation:tenant:<tenantId>` applies to tenant-scoped regular and gameplay-affecting operations. A tenant-generation advance does not by itself revoke `billing_safe_tenant`, which survives through the Account-owned caller-bound `{accountId, tenantId}` membership authority generation and live `tenantAdmin` check, or cross-tenant support/billing-safe routes, which survive through current Account-owned issuer/account authority, global role, and global token scope. None of these routes may use cached authorization.
...
| Cross-tenant (support-safe) | One matching token record for the exact profile declared by the route | Require `support` or `platformAdmin` | Issuer + account | None | Tenant parameters are allowed only through the current Account-owned global `support` role and global token scope; responses must be limited to high-level, troubleshooting-safe data (for example derived entitlements and subscription status, not invoices/payment methods); log/audit the target tenant |
| Cross-tenant (billing-safe) | One matching token record for the exact profile declared by the route | Require `billingAdmin` or `platformAdmin` plus route-appropriate assurance | Issuer + account | None | Tenant parameters are allowed only through the current Account-owned global billing role and global token scope; cross-tenant use requires the bounded `privileged_control` window with independent TOTP; log/audit the target tenant |
