...
- Resolves `[character]` to a canonical `characterId` scoped to `{accountId, tenantId, playableStateNamespaceId, playableStateScope}` according to the selected realm's character policy, then binds the live session to target uniqueness `{tenantId, playableStateNamespaceId, characterId}` with active runtime/value evidence `{playableStateScope, gameInstanceId, sessionId, bindingGeneration}`.
