...
- `CONNECT_SCOPE_MISMATCH` means the verified target scope or its exact `catalogRevision`/`pointerVersion` pair differs from the requested connect scope: the server-resolved `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId}` plus applicable `playtestLifecycleId` and positive `playtestStateGeneration` for the requested stable world/realm does not match the verified first-party connect context or connect-token scope, or a valid scope changed its exact revision/generation tuple. It is an admission-scope/issuance drift and requires fresh bootstrap, token issuance, and connection establishment.
