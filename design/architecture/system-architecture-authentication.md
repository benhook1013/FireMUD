...
- **Membership behavior (current drift):** Email challenge and credential-bearing `LOGIN` / `LOGON` are the implemented raw-text authentication paths. The explicit join action and the required `membershipAuthorityGeneration` reread at connect-token issuance are unimplemented/proof gaps; current connect-token issuance and text `PLAY` may still invoke Account's `EnsurePublicProductionPlayerMembership`, contrary to the target lifecycle. The target Game Session admission, character-creation precondition, and `JOIN_REQUIRED` behavior for `PLAY` are therefore not current guarantees, and the complete first-party browser/mobile sequence remains target-only; individual bootstrap/connect-token endpoints are partial implementation surfaces. Returning active members and grant-backed private/playtest callers remain separately eligible only when their own current membership, grant, entitlement, routing, generation, and version checks pass.
...
[JOIN <world>]  # target-only; current runtime may still use the documented implicit membership-writer drift, so JOIN_REQUIRED is not a current guarantee
...
The current shipped `PLAY` path resolves the selected realm to its admissible `{tenantId, gameInstanceId}`, validates caller-bound gameplay access and entitlement state, resolves the character, and binds `{tenantId, gameInstanceId, characterId}`. Its current membership behavior remains the drift recorded in Implementation Status: text `PLAY` may still invoke `EnsurePublicProductionPlayerMembership`, which is not an allowed target behavior. The target Game Session membership gate, character-creation precondition, and `JOIN_REQUIRED` behavior must not be reported as current behavior merely because their failure codes are already reserved. It also does not claim that the live path already resolves or commits the target region, region epoch, lease owner, or lease fence. Those ownership and timeline checks are target-state responsibilities described above.
