...
- **Gameplay-connect replay-fence path** – ADR 0029 intentionally owns the dedicated one-use `gameplay-connect` issuance/replay record, carrier, `replayAdmissionFence`, quarantine, deny-marker, atomic exact-`jti` consume, and signed Gateway-to-Game Session context contract. The token is consumed only by Gateway and does not create or consult ADR 0035's Account issued-token registry, active token record, `tokenGeneration`, `authorityTuple`, `issuanceFence`, or `session:auth:token:<tokenHash>` record; Game Session receives only the signed connect context. ADR 0022 owns the service-authority partition between those paths.
