...
| `pending_deletion_scoped` | One Account-issued `pending-deletion-access` credential; no JWT | Dedicated pending-deletion validator and workflow state; no account, tenant, or membership generation | None | None | The credential is bound to the account and deletion workflow and authorizes only status, cancellation, export, or necessary billing settlement. Invalid, expired, or revoked state returns `AUTH_SESSION_REVOKED`; unavailable state returns `AUTH_UNAVAILABLE`; normal JWT/session/gameplay authority is never a fallback |
