...
Account remains the authority for ordinary token logout, while Gateway owns the edge-only revocation of a known gameplay-connect `jti`. The target order is Gateway first atomically recording the dedicated edge deny marker and clearing the connect-token cookie, then Account performing logout for the underlying `player-bootstrap` or control-plane token. Gateway's first step does not mutate the ordinary `session:auth:token:<tokenHash>` registry/session and is not full Account logout.

This target edge-before-Account ordering is not currently proven. The current fallback is Account logout plus local cleanup, with bounded expiry covering an already-issued gameplay-connect token. Local state, cookie clearing, and expiry are not proof of complete cross-service logout or successful Account issuance-fence advancement; account-wide logout remains subject to its separate Account authority-generation and downstream gameplay-revocation contract.
