...
- `WORLDS` – list worlds the authenticated account can enter (a numbered menu plus one stable, globally unique `worldSlug` for each entry).
...
- `REALMS <world>` – list the visible realms for the selected world (`<world>` is a response-local
  world index or the stable globally unique `worldSlug` returned by `WORLDS`). Responses include the
