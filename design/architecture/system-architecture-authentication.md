...
- **Ordinary gameplay authentication** – Gameplay `LOGIN` may use the account-selected `PASSWORD` and/or verified `EMAIL_OTP`. It does not perform active-gameplay reauthentication or elevate the gameplay session into account/control-plane authority; HTTPS step-up remains separate.
