...
- `REALMS <world>` – list the visible realms for the selected world (`<world>` is a response-local
  ...
  forks.
- `JOIN <world>` – explicitly create the caller's durable `player` membership only when it is missing, or return the existing membership for the world's public production realm. First-party clients expose the equivalent `Join & Play` action through Account bootstrap.
