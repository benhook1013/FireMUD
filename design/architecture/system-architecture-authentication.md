...
- Membership responses must include `membershipAuthorityGeneration`, `gameplayAdmissionAllowed`, `evaluatedAt`, and `membershipVersion`. Account must produce these fields, the caller-bound account/tenant identity, applicable authority tuple, and outbox checkpoint from one authoritative snapshot/transaction; `evaluatedAt` describes the freshness of that complete response, not a separately read field. Callers must reject a response that is missing any field, combines values from different checkpoints, or cannot prove this atomic freshness.
