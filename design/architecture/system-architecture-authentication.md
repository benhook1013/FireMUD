...
- During authenticated `REALMS` resolution, Account Service issues an opaque, short-lived `connectScopeId` for the selected realm. It is bound to the authenticated caller and the exact server-resolved `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion}` snapshot, including its expiry; a non-public snapshot additionally carries its exact `playtestLifecycleId` and positive `playtestStateGeneration`, while a public-production snapshot omits both fields. Ordinary-private and playtest grants use the same lifecycle-bound field. The scope record commits the normalized digest for that exact snapshot. It is not a client-selected target or a durable realm handle. The same snapshot is the only source from which Game Session derives and carries the namespace/scope/lifecycle identity through `JOIN`, `CHARS`, connect-token issuance, and `PLAY`; neither `realmId`, `playableStateNamespaceId`, `playableStateScope`, `playtestLifecycleId`, nor `playtestStateGeneration` is supplied by the client or accepted as join input.
...
- For first-party clients on `/ws/game/**`, `PLAY` accepts stable world/realm/character selection only. Game Session resolves the current admissible `{tenantId, realmId, playableStateNamespaceId, playableStateScope, gameInstanceId}` server-side and requires that resolved scope to match the connect-token context. Scope mismatch is rejected with canonical error `CONNECT_SCOPE_MISMATCH`; clients must request a fresh connect token for the intended stable realm target and reconnect.
...
Set-Cookie: Firemud-Connect-Token=<connectToken>; HttpOnly; Secure; SameSite=Strict; Path=/ws/game; Max-Age=30
-> { accountId, tenantId: "7b3b074e-d597-4e9b-b96f-4f5946d26120", realmId: "9b8bc865-65f6-4f76-ae06-8a8d56d2ad67", realmSlug: "playtest-docks", playtestLifecycleId: "2a9c4d8e-6f10-4b32-9c75-1d8e0a6b4f21", gameInstanceId: "ad63c32f-b076-48de-9434-87fb16b73c1d", expiresAt, issuedAt }
...
- `CHARS <world> [realm]` – list characters for the selected world and optional realm; it does not require a selected character. Game Session resolves `playableStateScope` and the stable `playableStateNamespaceId` from the exact server-side realm snapshot before the character query; callers cannot provide that scope, namespace, a storage key, or a join-derived substitute.
...
- Resolves the selected realm's gameplay-admissible instance and records that `gameInstanceId` in the gameplay binding. The selected-target catalog/policy revision and routing pointer remain the authoritative pair; Account entitlement `allowPublicJoin` does not replace either routing input.
  - Resolves the realm's stable `playableStateNamespaceId` and server-derived `playableStateScope` from its exact catalog/policy state. Durable character/progression/inventory state uses that namespace and scope, while disposable runtime state remains keyed to `gameInstanceId`; missing or contradictory namespace/scope evidence fails closed.
  - First-party `/ws/game/**` contract: if a validated connect token is present, resolved `tenantId`, `playableStateNamespaceId`, `playableStateScope`, and `gameInstanceId` must match token claims. On mismatch, reject admission with `CONNECT_SCOPE_MISMATCH` and do not bind session scope.
...
- Resolves `[character]` to a canonical Entity-owned persisted `characterId` within the resolved `{tenantId, playableStateNamespaceId, playableStateScope}` target according to the selected realm's published entry policy and its applicable versioned descriptor or template. Entity first validates the trusted `accountId`-to-character association and caller visibility for that exact target; Game Session creates no binding until both ownership and visibility evidence complete. Server-derived `playableStateScope` is binding/routing/policy/authorization evidence rather than controller identity. It then binds the live session to target uniqueness `{tenantId, playableStateNamespaceId, characterId}` with active runtime/value evidence `{playableStateScope, gameInstanceId, sessionId, bindingGeneration}`.
