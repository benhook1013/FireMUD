...
- **Authority-generation writer** – The Account Service owns durable issuer, account, tenant, and `{accountId, tenantId}` membership authority generations and is the sole writer of their `session:auth:generation:*` projections. Account commits the durable generation and monotonic outbox event together; the projection is an asynchronous outbox output, not part of that atomic durable transaction. Consumers fail closed while the projection or its freshness/source evidence is missing, stale, malformed, regressed, or ambiguous. Other services must publish billing/security events and must not write authority-generation state or projection keys directly.
- **Authority validation outcomes** – A registry, authority-generation source, or authority-projection freshness fence that cannot be reached is a retryable `AUTH_UNAVAILABLE` / HTTP 503 condition; it does not revoke the client's authentication and no cached authority may authorize the failed operation. Reachable missing, malformed, expired, revoked, stale, or mismatched registry or generation evidence is an authentication failure (`AUTH_SESSION_REVOKED` or the specific invalid-token outcome) and requires reauthentication.
...
- **JWT claim contract** – Services must validate a strict JWT claim profile (required claims and audience per token profile), not only signature plus ad-hoc fields. `control-ui` must include `scopedRoles` as `{}` when empty; omission is rejected, and its tenant/membership generation-map keys must exactly match the scoped-role tenant UUID keys.
...
2. **Check issued-token registry** – Compute `tokenHash` and require one matching `session:auth:token:<tokenHash>` record in Coordination Redis. Validate its account, profile, `jti`, per-lineage `tokenGeneration`, and time fields against the already verified token. Account-owned authority generations are a separate validation dimension; private-token refresh must compare-and-set the applicable durable authority generation so it cannot commit across logout-all or another security cutoff. Missing or mismatched state means the token is revoked or unregistered and returns the canonical “session revoked” error (`AUTH_SESSION_REVOKED` or equivalent). The only authorization exceptions are an explicitly matrix-classified public route whose contract requires no JWT and the exact matrix-declared `gameplay-connect` Gateway handshake. The gameplay-connect handshake is not validated by an issued-token registry lookup: ADR 0029 instead requires Account-signed claims, the current signed replay fence, quarantine cutoff, atomic exact-`jti` single-use replay consumption, Gateway deny-marker state, and bounded lifetime checks. Logout retries use only their separate durable intent/tombstone reconciliation path and create no authorization context. No other route or profile inherits these exceptions.
3. **Check Account authority generations** – Enforce bulk revocation without relying on wildcard deletes, key scans, or JWT timestamps. Every route that accepts an issued JWT applies the issuer and account authority-generation checks below, except an explicitly matrix-classified public no-JWT route and the exact `gameplay-connect` bounded single-use exception declared above and in the route matrix. `gameplay-connect` omits issuer/account generation claims and accepts the deliberate maximum 30-second lifetime plus at most five seconds of clock skew under ADR 0029's replay-fence, quarantine, deny-marker, and atomic-consume contract. Tenant and membership generations are additional, route-class-specific checks; they do not replace the universal issuer/account checks. The route table distinguishes those conditional tenant and membership checks from the universal checks:
...
| `player_bootstrap_tenant` | One matching `player-bootstrap` token record | Require the `player-bootstrap` token profile for the authenticated account | Issuer + account | Membership where declared | Used for caller-bound player-bootstrap routes targeting a tenant, including gameplay bootstrap and report-submission routes; `IssueConnectToken` must use current caller-bound membership authority generation plus live membership, entitlement, and admission-pointer checks |
