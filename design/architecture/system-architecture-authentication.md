...
- **Membership behavior (current drift):** Email challenge and credential-bearing `LOGIN` / `LOGON` are the implemented raw-text authentication paths. The explicit join action and the required `membershipAuthorityGeneration` reread at connect-token issuance remain unimplemented/proof gaps. Current connect-token issuance and text `PLAY` require fresh authoritative `membershipLifecycleState=ACTIVE` membership, return `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership, and return `AUTH_UNAVAILABLE` for unavailable or unsafe membership authority. This current `JOIN_REQUIRED` result is a fail-closed, non-actionable denial while explicit `JOIN`/`Join & Play` is unimplemented; it must not be presented as proof that the target repair flow is available. Current non-public missing or non-`ACTIVE` membership remains the existing `WORLD_ACCESS_DENIED` behavior; target-only `NON_PUBLIC_ENROLLMENT_REQUIRED` is defined separately. The obsolete writer seam has been removed, while the complete first-party browser/mobile sequence remains target-only and individual bootstrap/connect-token endpoints remain partial implementation surfaces. Returning members may proceed when current runtime membership and character checks pass, but current behavior does not prove the target exact generation/version reread at connect-token issuance; the target requires both independent values from one fresh membership snapshot. Current private/playtest text `PLAY` checks the current realm grant, while current connect-token grant validation remains incomplete. The target requires both paths to verify caller-bound `ACTIVE` membership, the current Account-owned grant where applicable, entitlement, routing, exact current `membershipAuthorityGeneration`, and independent current `membershipVersion` before admission; target missing or non-`ACTIVE` membership is `NON_PUBLIC_ENROLLMENT_REQUIRED`, target missing or invalid grant is `REALM_ACCESS_DENIED`, and unavailable authority is `AUTH_UNAVAILABLE`. Grant-backed private/playtest callers remain separately eligible only when their own current membership, grant, entitlement, routing, generation, and version checks pass.
...
Gateway strips the external carrier and attaches one signed, short-lived context. Gateway owns issuance, replay, signing-key rotation, and carrier policy; Game Session owns receiving-service validation, including the explicit audience/recipient binding, before applying gameplay admission.

Minimum context fields are `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, explicit `audience` and `recipient` values bound to the registered Game Session receiver, the exact bounded selected-target `authorityTuple`, separate exact `membershipVersion`, `connectTokenJti`, `replayAdmissionFence`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`.
...
- `REALMS`, `CHARS`, `POST /auth/connect-token`, `PLAY`, and reconnect consume the same catalog/pointer result. Missing, malformed, ambiguous, stale, or unavailable pointer evidence is `ADMISSION_POINTER_UNAVAILABLE`; a complete `CLOSED` pointer is `REALM_UNAVAILABLE`.
...
- `CONNECT_SCOPE_MISMATCH` means the verified target scope differs from the requested connect scope: the server-resolved `{tenantId, gameInstanceId}` for the requested stable world/realm does not match the verified first-party connect context or connect-token scope. It is an admission-scope/issuance drift and requires fresh bootstrap, token issuance, and connection establishment.
...
| JWT Usage | Raw Telnet gameplay command streams do not carry JWTs; browser/mobile gameplay uses `player-bootstrap` for bootstrap and an HttpOnly-cookie connect token for `/ws/game/**`; profile and registry authority is [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md), while public non-browser issuance/header support remains target-only |
...
| Gameplay Reauthentication | [Session Behavior](./system-architecture-session-behavior.md) owns disconnect, expiry, revocation, refresh, and resume; the client-visible command remains fresh `LOGIN` then `PLAY` when required |
...
| Multi-Client Behavior | One session per character; takeover and rebinding are defined in [Session Behavior](./system-architecture-session-behavior.md#multi-client-behavior-and-session-takeover) |
