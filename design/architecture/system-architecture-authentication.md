...
### Playtest Grant Admission Consequence

Account's explicit grant record, bounded expiry, revision/request replay, tombstone, tenant-admin authority, and audited platform-admin break-glass mutation path are canonical in [Account API Contracts](./microservices/account-service/api-contracts.md#account-owned-playtest-grant-contract). Authentication owns only the caller-bound consequence: private/playtest discovery, connect-token issuance, `PLAY`, reconnect, and continued gameplay require the current Account grant in addition to `ACTIVE` membership, entitlement, and exact routing evidence. Effective expiry or revocation is an immediate authority cutoff, not a grace period; Game Session consumes Account's bounded authority change and terminates affected bindings. A friendly scheduled ending uses the separate close-and-drain lifecycle before grant revocation. No JWT role, cached discovery result, or grant projection creates gameplay authority by itself.

For every private/playtest grant consequence, the exact Account target is `{accountId, tenantId, worldSlug, realmSlug, playtestLifecycleId}`; the immutable lifecycle ID is required on grant streams, lookups, mutations, discovery filters, connect-token issuance, `PLAY`, reconnect, and admission. Realm slugs are not tenant-global grant identity. The durable controller identity remains `{tenantId, playableStateNamespaceId, characterId}`. `accountId` is separate ownership evidence supplied by the trusted authenticated gameplay workload: Entity Management must receive and validate that account selector when resolving roster visibility or a character, and Game Session must not create or bind a controller until Entity confirms the persisted account-to-character association. Account ownership evidence authorizes the association check; it does not redefine `accountId` as controller identity, and `playableStateScope` remains server-derived routing/authorization context.
...
- **Current first-party endpoint and runtime boundary:** The current first-party browser/mobile runtime includes dedicated `POST /auth/player-bootstrap` and `POST /auth/connect-token` surfaces, Gateway rejection for missing, expired, replayed, or scope-mismatched connect tokens, and Game Session validation of the signed connect context before bare `LOGIN`, but these remain partial implementation surfaces rather than proof of the complete target flow. Current `PLAY` resolves the admissible `gameInstanceId` from realm routing and binds the legacy `{tenantId, gameInstanceId, characterId}` identity; that per-instance binding is implementation drift. Target `PLAY` binds the controller identity `{tenantId, playableStateNamespaceId, characterId}`, carries server-derived `playableStateScope`, and retains `{gameInstanceId, sessionId, bindingGeneration}` as runtime/value evidence. Current behavior does not claim authoritative `regionId`, `regionEpoch`, lease-owner, or lease-fence resolution. Public non-browser issuance and the dedicated handshake-header carrier remain unavailable.
- **Current realm-identity gap:** Current discovery, admission-pointer mutation/read models, gameplay-connect claims, and the Gateway signed context do not carry and prove durable `realmId` end to end; several runtime surfaces still key or resolve routes directly from `worldSlug`/`realmSlug`. The target resolves those selectors to `{tenantId, realmId}` before pointer lookup and carries `realmId` in the exact selected-target snapshot and signed context.
...
- Character selection and gameplay takeover target the one-controller identity `{tenantId, playableStateNamespaceId, characterId}` with an atomic monotonic `bindingGeneration`; server-derived `playableStateScope` remains binding context, and the current `{tenantId, gameInstanceId, characterId}` identity is implementation drift.
...
- **Current admission status:** The Account gRPC membership, grant, and entitlement reads used by Game Session currently fail with `FAILED_PRECONDITION` before data access, including calls from the Game Session workload. Consequently, a valid-route text `PLAY` reaching runtime admission returns `AUTH_UNAVAILABLE` before evaluating membership, a private/playtest grant, or the Entity roster; it does not currently reach a `JOIN_REQUIRED` or `WORLD_ACCESS_DENIED` membership result. Direct-text `CHARS` returns `CHARACTER_LIST_UNAVAILABLE` before calling Entity. These failures are current transport consequences, not evidence about stored membership, grants, or characters. The local Account explicit join operation (`JoinPublicProductionMembership`) and Game Session command dispatch exist, but successful Account authority reads, PostgreSQL execution, pending-operation recovery, lifecycle/authority-generation snapshot, and direct-text/first-party end-to-end proof remain incomplete. Current first-party connect-token issuance and other Account admission reads remain subject to their own current route behavior; target lifecycle, generation, and independent membership-version checks are not proved. Target-only `JOIN_REQUIRED` identifies missing or `INACTIVE` public-production membership after earlier checks pass, and target `PLAY` requires existing `ACTIVE` membership. The full target sequence and its ordering are defined here and in [Game Session Protocols](./microservices/game-session-service/protocols.md); this current failure does not change the target `JOIN` -> `CHARS`/character creation -> `PLAY` design. See [Account Runtime and Data](./microservices/account-service/runtime-and-data.md) for the Account-side call boundary.
- The current connect-token replay-cache behavior and persistence gaps are owned by [Account Runtime and Data](./microservices/account-service/runtime-and-data.md#connect-token-issuance-persistence-and-retention); a replay hit does not establish fresh current gameplay authority. The target requires applicable fresh tenant entitlement, membership lifecycle and authority generation, independent membership version, exact routing, and, for private/playtest targets, the current Account-owned realm grant. Private/playtest admission never consumes the public-enrollment `allowPublicJoin` predicate. Target missing or non-`ACTIVE` private/playtest membership is `NON_PUBLIC_ENROLLMENT_REQUIRED`, target missing or invalid grant is `REALM_ACCESS_DENIED`, and unavailable authority is `AUTH_UNAVAILABLE`.
...
- **Membership freshness evidence** – Any authoritative Account membership response/evidence used for billing-safe checks, gameplay admission, reconnect, or revocation must carry or authenticate the complete applicable `authorityTuple`, exact `membershipLifecycleState`, separate `membershipVersion`, `evaluatedAt`, and the matching `outboxCheckpoints` entry from one authoritative Account snapshot/transaction or equivalent Account-owned fence. The target response fields are `accountId`, `tenantId`, `membershipExists`, `membershipLifecycleState`, `roles[]`, `gameplayAdmissionAllowed`, `membershipVersion`, `membershipAuthorityGeneration`, `authorityTuple`, `evaluatedAt`, and `outboxCheckpoints[]`; `membershipAuthorityGeneration` is the tuple's membership member, and a separately carried copy must authenticate the same value. For a committed stream, the matching checkpoint is `{outboxStreamKey: account:auth-authority:v1:membership/<accountId>/<tenantId>, outboxSequence: <committed sequence>, sourceEventId, sourceEventDigest}`. Account may instead return sequence `0` only when the exact account/tenant membership stream has never committed and the same fenced snapshot proves no membership row together with the authenticated tuple, generation, and version. A sequence-zero checkpoint omits event ID and digest, grants no admission, and is followed by the first committed `JOIN` event at sequence `1`. A formerly joined or left stream never resets to zero. `evaluatedAt` describes that complete snapshot and is not restamped from a projection or cache. Missing fields, a checkpoint that does not cover the tuple, lifecycle state, or membership version, or values assembled from different checkpoints are invalid and fail closed.
- **Never-joined pair baseline** – For each verified account/tenant pair, Account owns a durable pair-authority record with a positive membership generation and independent positive membership version even while membership is absent. A sequence-zero response requires one Account-fenced snapshot that proves this exact record, the verified pair identity, no current membership, and no committed membership event or retained contradictory history; absence of a row alone is never a generation, version, or checkpoint. The first explicit `JOIN` commits event sequence `1` and advances `membershipVersion` from that recorded absence baseline. It advances `membershipAuthorityGeneration` independently only if the transition records `callerBoundAuthorityInvalidated=true`. Existing retained membership versions and positive event sequences are preserved rather than assigned a synthetic absence baseline or renumbered. Unmapped or contradictory pairs remain denied, and sequence zero never authorizes admission.
...
- **Membership event digest** – `account-auth-authority-event/v1` is the closed wire/preimage schema for a committed Account membership-change event; other event types, including a deletion/tombstone, require their own declared schema before emission. Its required top-level fields are `schemaVersion` (exactly `account-auth-authority-event/v1`), `eventType` (exactly `MEMBERSHIP_CHANGED`), `eventId`, `requestId`, `outboxStreamKey`, `outboxSequence`, `sourceScope`, `accountId`, `tenantId`, `membershipExists` (exactly `true`), `membershipLifecycleState` (`ACTIVE` or `INACTIVE`), `membershipVersion`, `membershipAuthorityGeneration`, `authorityTuple`, `issuanceFence`, `roles`, `gameplayAdmissionAllowed`, and `callerBoundAuthorityInvalidated`. The sole additional wire field is `eventDigest`; the digest preimage excludes **only** that field. No undeclared field may be emitted or accepted, and a changed declared field changes the digest. `accountId` and `tenantId` are canonical lowercase UUID strings; `sourceScope` and stream key encode those exact IDs. `gameplayAdmissionAllowed` is false for `INACTIVE`; a missing row is not an event. `roles` is a required, duplicate-free array of canonical role names sorted by Unicode code-point order; its empty value is `[]`, not omission. The complete applicable `authorityTuple` uses the field names and presence rules in [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md#canonical-authority-tuple): `issuerAuthGeneration`, `accountAuthorityGeneration`, `tenantAuthorityGeneration`, `membershipAuthorityGeneration`, and `privateRealmGrantVersions` are always present; `accountSecurityCutoff` and `tenantBillingCutoff` are omitted only when inapplicable. Both generation maps have exactly the event's `tenantId` key, and the membership-map value equals the separately carried `membershipAuthorityGeneration`; no unrelated tenant entry is applicable to this one-member event. The optional `tenantBillingCutoff` map, when present, likewise has exactly that tenant key, and every private-realm grant entry, when present, belongs to that tenant. Maps are keyed by exact canonical tenant UUID; private-realm grant entries contain all five canonical fields and are sorted lexicographically by `(tenantId, worldSlug, realmSlug, playtestLifecycleId)`, with duplicate identities rejected. All generations, versions, sequences, cutoffs, grant versions, and `issuanceFence` in this event are canonical unsigned decimal **strings** (`0` only where the owning field permits it; no sign, leading zero, exponent, whitespace, or JSON number), preserving values above JavaScript's exact-integer range. `null` never substitutes for omission or an empty list/map. The producer serializes this complete event without `eventDigest` as RFC 8785 canonical JSON UTF-8 and sets `eventDigest` to lowercase `sha256:` plus the SHA-256 hex of those bytes; receivers validate the closed shape, field types, scope coherence, canonical collection rules, and recomputed digest before applying or acknowledging it. This digest binds immutable source evidence; it does not substitute for Account authentication, source sequencing, or current-authority comparison.
...
- **Gameplay session identity key** – Target session uniqueness and takeover scope are keyed by `{tenantId, playableStateNamespaceId, characterId}` with atomic monotonic `bindingGeneration`; server-derived `playableStateScope` and the binding value/evidence `{sessionId, gameInstanceId, bindingGeneration}` remain separate context. The existing `{tenantId, gameInstanceId, characterId}` key is current implementation drift.
...
- **First-party bare `LOGIN` delegation** – A first-party WebSocket `LOGIN` without credentials may obtain the private `game-session-account-delegation` JWT only through an Account-owned, one-use, idempotent exchange. Account verifies its committed gameplay-connect issuance and bootstrap subject, the verified Gateway-signed connect context, the exact Game Session mTLS caller, the player/tenant/target binding, current authority, and the issuance fence before issuing. Gateway context or a connect-token `jti` alone is not Account authentication. Game Session never mints this JWT or returns it to a gameplay client; an exact retry recovers the same bounded exchange result, while changed binding or fence evidence fails closed.
...
- **Gameplay session ownership** – Game Session owns gameplay binding records, the atomic monotonic binding CAS, and bounded secondary indexes that map gameplay bindings by the target uniqueness key `{tenantId, playableStateNamespaceId, characterId}`, account/tenant scope, and tenant scope so takeover, reconnect, and revocation do not require scans. Server-derived `playableStateScope` and binding values/evidence retain `{sessionId, gameInstanceId, bindingGeneration}` as locally appropriate. Account owns the admission decision and exact-binding lease only; Account finalization commits lease/decision evidence and never creates, deletes, or mutates Game Session bindings or indexes. Orphan evidence from Account reconciliation becomes a durable cleanup request that Gateway and Game Session consume under their respective edge and gameplay fences.
...
Official-hosted creator-party and signer authority under [ADR 0180](./decisions/adr-0180-account-owned-hosted-terms-acceptance-gate.md) is Account-owned evidence, not a global or tenant role and must not be embedded as a broad JWT role. Account evaluates the party, authenticated signer authority, current terms acceptance, and requested legal action at the acceptance boundary. `tenantAdmin`, `designer`, organization administration, billing ownership, payment, or a company email domain do not establish signer authority. Game Design separately requires current tenant authorization and current party acceptance; neither substitutes for the other.
...
`<world>` is either the stable `tenantSlug/worldSlug` pair carried by the caller's exact `WORLDS` browse snapshot or an index bound to that same response snapshot and catalog revision. A bare `tenantSlug` is accepted only when the tenant exposes exactly one visible authored world, and a bare `worldSlug` is never resolved globally. `[realm]` is a tenant-local `realmSlug` or an index bound to the corresponding `REALMS` snapshot. World and realm selectors resolve server-side to `tenantId` and durable `realmId`; a later catalog reorder never changes the meaning of an unexpired index. Menu indices have bounded expiry, are response-local conveniences, and are never stored or forwarded as durable identity; an expired or invalidated index requires fresh discovery. `REALMS` is authenticated discovery after `LOGIN`; `JOIN` is conditional on the selected public-production membership state; `CHARS` or allowed character creation follows only after membership is `ACTIVE`.
...
- After the selector resolves to a canonical tenant, the same class performs exact tenant-scoped checks against that tenant's current catalog/pointer pair. The public-production realm still permits discovery without membership when its public visibility and entitlement checks pass. Every non-public realm requires both an existing caller-bound membership with exact `membershipLifecycleState=ACTIVE` for that tenant and the current Account-owned realm-access grant for `{accountId, tenantId, worldSlug, realmSlug, playtestLifecycleId}`; a grant never substitutes for membership. Hidden or unauthorized realms are omitted rather than disclosed.
...
- During authenticated `REALMS` resolution, Account Service issues an opaque, short-lived `connectScopeId` for the selected realm. It is bound to the authenticated caller and the exact server-resolved `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion, evaluatedAt, connectScopeExpiresAt}` snapshot; a non-public snapshot additionally carries its exact `playtestLifecycleId` and positive `playtestStateGeneration`, while a public-production snapshot omits both fields. Ordinary-private and playtest grants use the same lifecycle-bound field. The scope record commits the normalized `connectScopeSnapshotDigest` for that exact snapshot. It is not a client-selected target or a durable realm handle. The same snapshot is the only source from which Game Session derives and carries the namespace/scope/lifecycle identity through `JOIN`, `CHARS`, connect-token issuance, and `PLAY`; neither `realmId`, `playableStateNamespaceId`, `playableStateScope`, `playtestLifecycleId`, nor `playtestStateGeneration` is supplied by the client or accepted as join input.
- Game Session obtains that exact scope, including its immutable `evaluatedAt`, and retains it as transport-local session state alongside the response-local `REALMS` selector or index. The text client receives ordinary selectors and display data only; it never receives or supplies authority IDs, a scope, a tenant, a runtime target, a catalog revision, or a pointer version.
...
- Account computes `connectScopeSnapshotDigest` as the immutable snapshot-integrity digest for the scope record; it is distinct from the JOIN idempotency `requestDigest` and is never an idempotency key. The digest is `SHA-256` over the UTF-8 bytes of this exact newline-delimited `connectScopeSnapshotDigest/v1` preimage, including the final newline and preserving this field order:

  ```text
  scopeDigestVersion=v1
  accountId=<authenticated accountId>
  targetClass=<PUBLIC_PRODUCTION|NON_PUBLIC>
  ...
  evaluatedAt=<bound evaluatedAt>
  connectScopeExpiresAt=<bound connectScopeExpiresAt>
  [playtestLifecycleId=<bound playtestLifecycleId>]
  [playtestStateGeneration=<bound playtestStateGeneration>]
  ```
...
- `PLAY` binds the gameplay session to `{tenantId, playableStateNamespaceId, characterId}`, carries server-derived `playableStateScope`, and retains `{gameInstanceId, sessionId, bindingGeneration}` as runtime/value evidence.
...
- **Pre-admission checks:** Game Session server-resolves the target and validates the bound session, exact target `{tenantId, realmId, playableStateNamespaceId, characterId}` identity plus server-derived scope and runtime/value evidence `{playableStateScope, gameInstanceId, sessionId, bindingGeneration}`; a non-public target additionally requires exact `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. It also validates the current Account membership response (`membershipExists=true`, exact `membershipLifecycleState=ACTIVE`, `membershipAuthorityGeneration`, `membershipVersion`, `gameplayAdmissionAllowed`, complete `authorityTuple`, matching `outboxCheckpoints[]`, and atomic freshness), entitlements, realm/access grant, routing pointer, coordination ownership, and the Account-owned exact-binding admission lease. For connect-token-backed admission, the current state, generation, and version must exactly match the immutable issuance baseline; a boolean `membershipExists` or a token-carried selector alone is not sufficient. The Game Session binding CAS includes the unchanged baseline, lease fence, authority tuple, checkpoint, and target uniqueness identity and publishes no admissible binding until the ordered finalization protocol is complete. The current `{tenantId, gameInstanceId, characterId}` binding is implementation drift.
...
Minimum context fields are `accountId`, `tenantId`, `realmId`, `worldSlug`, `realmSlug`, `playableStateNamespaceId`, `playableStateScope`, `playtestLifecycleId` and positive `playtestStateGeneration` when the target is non-public (both absent for public production), `gameInstanceId`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `connectRequestId`, `audience: game-session`, `recipient: game-session-service`, the exact bounded selected-target `authorityTuple`, separate exact `membershipVersion`, `connectTokenJti`, `replayAdmissionFence`, `issuedAt`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`. The signed context's `issuedAt`-to-`expiresAt` lifetime is no longer than 30 seconds, subject only to the configured bounded clock-skew handling owned by Gateway. `connectRequestId` is the canonical signed-context name and carries the source `gameplay-connect` token's `requestId` (the connect-token issuance/request-operation identity); the context does not expose that source field under the generic name `requestId`.
...
- `CONNECT_SCOPE_MISMATCH` is computed by exact comparison of this verified context and server-resolved first-party scope `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion}`; non-public targets additionally compare `playtestLifecycleId` and positive `playtestStateGeneration`, while public-production targets omit both. It is never computed from raw `X-Tenant-Id` or `X-Game-Instance-Id` headers.
...
- For first-party clients on `/ws/game/**`, `PLAY` accepts stable world/realm/character selection only. Game Session resolves the current admissible `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion}` server-side and requires that resolved scope and the applicable non-public `playtestLifecycleId`/positive `playtestStateGeneration` pair (omitted for public production) match the connect-token context. Scope mismatch is rejected with canonical error `CONNECT_SCOPE_MISMATCH`; clients must request a fresh connect token for the intended stable realm target and reconnect.
...
-> [{
     ...
     realmId: "57c58f36-c5ea-4aa8-8ef7-91a45e407f01",
...
-> { accountId, tenantId: "7b3b074e-d597-4e9b-b96f-4f5946d26120", realmId: "57c58f36-c5ea-4aa8-8ef7-91a45e407f01", realmSlug: "production", gameInstanceId: "2f1c7ad0-8d5a-4a61-9d4b-6c93f11a2e01", expiresAt, issuedAt }
...
-> [{
     ...
     realmId: "ee7137a2-2410-4a27-8b56-4ff260f73b11",
...
Set-Cookie: Firemud-Connect-Token=<connectToken>; HttpOnly; Secure; SameSite=Strict; Path=/ws/game; Max-Age=30
-> { accountId, tenantId: "e14f2d0c-8b7a-4f26-9c51-6a3d7e8b2c40", realmId: "ee7137a2-2410-4a27-8b56-4ff260f73b11", realmSlug: "production", gameInstanceId: "7b63923a-43bd-45ab-8b39-80d95d74e2ce", expiresAt, issuedAt }
...
- The current realm-scoped backend creation substrate carries `{tenantId, accountId, name, gameInstanceId, playableStateScope}` into Entity Management. The target internal actor-entry boundary additionally carries the stable `playableStateNamespaceId` and Account-verified proof of the exact realm catalog/admission-pointer snapshot: resolved `{tenantId, realmId, worldSlug, realmSlug, gameInstanceId}`, exact `catalogRevision`/`pointerVersion`, the immutable `evaluatedAt`, and the snapshot's `playableStateScope` consequence; a non-public target also carries exact `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. Account applies the canonical `stateScope`-to-`playableStateScope` projection defined by [Multi-Tenancy](./system-architecture-multi-tenancy.md#realm-catalog-and-admission-pointer-contract) before delegation, and Entity validates the received namespace/scope against that exact attested snapshot and active-instance fence; neither creates a second snapshot authority or accepts a caller-selected namespace or scope. `gameInstanceId` remains only the active-runtime authorization fence, while the target one-controller identity is `{tenantId, playableStateNamespaceId, characterId}` and durable actor storage may retain the server-derived scope; missing or mismatched namespace, scope, target, or exact snapshot proof fails closed. Account creation/bootstrap does not provision a default actor. After explicit realm entry, Entity evaluates the published versioned realm-authored entry policy and generic descriptor/template defined by [ADR 0140](./decisions/adr-0140-realm-authored-controllable-actor-entry.md); this local handoff does not make Authentication the actor-row owner.
...
Set-Cookie: Firemud-Connect-Token=<connectToken>; HttpOnly; Secure; SameSite=Strict; Path=/ws/game; Max-Age=30
-> { accountId, tenantId: "7b3b074e-d597-4e9b-b96f-4f5946d26120", realmId: "9b8bc865-65f6-4f76-ae06-8a8d56d2ad67", realmSlug: "playtest-docks", playtestLifecycleId: "2a9c4d8e-6f10-4b32-9c75-1d8e0a6b4f21", playtestStateGeneration: 2, gameInstanceId: "ad63c32f-b076-48de-9434-87fb16b73c1d", expiresAt, issuedAt }
...
FireMUD uses a **single shared entrypoint** for many worlds (tenants). After public `WORLDS` discovery and successful `LOGIN`, clients complete a lobby selection step that binds the authenticated connection to a specific world (`tenantId`), resolved playable namespace/scope (`playableStateNamespaceId`, `playableStateScope`), gameplay-admissible instance (`gameInstanceId`), and gameplay identity (`characterId`) before gameplay commands are accepted. The one-controller binding key is `{tenantId, playableStateNamespaceId, characterId}`; `playableStateScope` is server-derived binding context and `gameInstanceId` remains runtime/value evidence.

Players must never be asked to type platform-scope identifiers such as `tenantId`, `gameInstanceId`, or `characterId` during lobby selection. Lobby flows accept the human-friendly `tenantSlug/worldSlug` pair, scoped realm slugs, menu indices, and character names or indices and resolve them server-side. Gameplay may separately expose stable numeric runtime-entity IDs when useful for distinguishing visible live instances; those IDs remain scoped selectors rather than authorization.
...
- `WORLDS` – list worlds discoverable to the authenticated account, including publicly onboardable worlds where membership is missing or inactive (a numbered menu plus the stable `tenantSlug/worldSlug` pair and the applicable availability status, such as playable, `JOIN_REQUIRED`, or unavailable, for each entry). Non-members can therefore select a discoverable world and proceed to `JOIN` rather than being omitted.
...
- `REALMS <world>` – list the visible realms for the selected world (`<world>` is a response-local
  world index or the stable `tenantSlug/worldSlug` pair returned by `WORLDS`). Responses include the
...
- `CHARS <world> [realm]` – list characters for the selected world and optional realm; it does not require a selected character. Game Session resolves the exact server-side `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion, evaluatedAt}` snapshot before the character query; a non-public target additionally carries `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. Callers cannot provide that scope, namespace, a storage key, or a join-derived substitute.
...
- `CHARS <world> [realm]` and `PLAY <world> [realm] [character]` become tenant/realm-scoped only after `<world>` and optional `[realm]` are resolved server-side to canonical `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion, evaluatedAt}`; a non-public target also carries `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. The namespace is stable playable-state identity, the scope is the realm policy projection, and the instance is the current runtime fence.
...
- Resolves the selected realm's gameplay-admissible instance and records that `gameInstanceId` in the gameplay binding. The selected-target catalog/policy revision and routing pointer remain the authoritative pair; Account entitlement `allowPublicJoin` does not replace either routing input.
  - Resolves the realm's stable `realmId`, `playableStateNamespaceId`, and server-derived `playableStateScope` from its exact catalog/policy state. A non-public target carries exact `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. Durable character/progression/inventory state uses that namespace and scope, while disposable runtime state remains keyed to `gameInstanceId`; missing or contradictory namespace/scope evidence fails closed.
  - First-party `/ws/game/**` contract: if a validated connect token is present, resolved `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion}` must match token claims; non-public targets additionally require exact `playtestLifecycleId` and positive `playtestStateGeneration`, while public-production targets omit both. On mismatch, reject admission with `CONNECT_SCOPE_MISMATCH` and do not bind session scope.
...
- **Target membership-writer contract:** Membership lifecycle writer authority remains Account Service. The accepted canonical API is `JoinPublicProductionMembership`; the obsolete differently named proto/client/service seam has been removed rather than retained as a compatibility operation. The text-protocol adapter passes only the Account-bound `{connectScopeId, requestId}` plus trusted caller context to `JoinPublicProductionMembership`; world and realm remain adapter-local selectors, and the adapter never passes client- or adapter-supplied `tenantId`, route, `gameInstanceId`, `catalogRevision`, `pointerVersion`, or route authority. Account resolves and revalidates the complete scope `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion, evaluatedAt, connectScopeExpiresAt}` and `connectScopeSnapshotDigest` from `connectScopeId` at the commit gate; a non-public target additionally carries exact `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. It does not accept those fields as independently authoritative player inputs. The operation is explicit `JOIN`/`Join & Play`; character creation, connect-token issuance, and `PLAY` never create or restore membership. Current writer and issuance gaps are recorded in [Implementation Status](#implementation-status).
- Performs an authoritative internal membership read for `{accountId, tenantId}`, requires `membershipExists=true` and exact `membershipLifecycleState=ACTIVE`, and obtains both `membershipVersion` and `membershipAuthorityGeneration`. For connect-token-backed admission, these three values must exactly equal the issuance baseline captured from one Account snapshot; the baseline is carried unchanged through the Account lease, Game Session binding CAS, and Account finalization. The membership response must also assert `gameplayAdmissionAllowed=true`; gameplay admission must not source membership state, either membership field, or gameplay authority from JWT claims or local caches. Before the binding can become admissible, Account must issue an exact-binding admission lease containing a unique lease ID, monotonic lease fence, immutable authority tuple/cutoff checkpoint, the unchanged membership lifecycle/generation/version baseline, and a bounded absolute expiry no longer than the admission SLA; the lease cannot be renewed to extend an admission attempt. Game Session's binding CAS includes that fence and may publish only a provisional record. Account finalization, abort, and reconciliation are idempotent by lease ID and request ID and must reject any recomputed or changed baseline. A binding CAS failure, lease expiry, authority cutoff, membership lifecycle/generation/version change, or ambiguous outcome invokes the idempotent Account abort/reconcile path; when a token identity or matching issued-token record exists, Account must retire or revoke that exact orphan token through the same idempotent path, while no-token outcomes are recorded without fabricating an identity. Account emits durable orphan binding evidence and a cleanup request, Gateway performs fenced edge cleanup, and Game Session performs fenced binding/index cleanup. A provisional record is never admissible, resumable, or takeover-eligible; only a durably finalized `COMMITTED` lease can admit gameplay.
- Resolves `[character]` to a canonical Entity-owned persisted `characterId` within the resolved first-party scope `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion, evaluatedAt}` according to the selected realm's published entry policy and its applicable versioned descriptor or template. A non-public target additionally carries exact `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. Entity first validates the trusted `accountId`-to-character association and caller visibility for that exact target; Game Session creates no binding until both ownership and visibility evidence complete. Server-derived `playableStateScope` is binding/routing/policy/authorization evidence rather than controller identity. It then binds the live session to target uniqueness `{tenantId, playableStateNamespaceId, characterId}` with active runtime/value evidence `{playableStateScope, gameInstanceId, sessionId, bindingGeneration}`.
  - A zero-roster result branches on exactly one published policy: `PLAYER_CREATED` returns creation-required and uses the explicit creation flow; `AUTO_PROVISIONED` invokes Entity's idempotent provisioning and re-reads the resulting actor; `PRESEEDED_ONLY` returns the pre-seeded-only denial. No policy permits a synthetic ID or binding from an empty roster.
...
- **Target authoritative admission:** before committing gameplay binding, Game Session applies the current region/lease ownership and exact-binding authority contract. Gateway owns edge route lifecycle and connect-token scope; Game Session remains the receiving service for binding and ownership validation. Missing or ambiguous ownership returns `OWNERSHIP_UNAVAILABLE`, an unavailable dependency returns `AUTH_UNAVAILABLE`, stale ownership or timeline returns `STALE_TIMELINE`, and a verified first-party scope `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId, catalogRevision, pointerVersion}` differing from the requested scope returns `CONNECT_SCOPE_MISMATCH`; non-public targets additionally compare `playtestLifecycleId` and positive `playtestStateGeneration`, while public production omits both. `PLAY` must not bind from cached ownership, raw transport headers, or a stale discovery result.
...
`CONNECT_SCOPE_MISMATCH` and `STALE_TIMELINE` are intentionally disjoint, and the former has two distinct causes under one wire code:

- `CONNECT_SCOPE_MISMATCH` (resolved-target mismatch) means the server-resolved `{tenantId, realmId, worldSlug, realmSlug, playableStateNamespaceId, playableStateScope, gameInstanceId}` for the requested stable world/realm, plus the applicable `playtestLifecycleId` and positive `playtestStateGeneration`, does not match the verified first-party connect context or connect-token scope.
- `CONNECT_SCOPE_MISMATCH` (issued-snapshot drift) means the current catalog/pointer authority is complete and valid, but its exact issued-snapshot tuple differs from the verified scope: `catalogRevision` or `pointerVersion` changed, or a private/playtest `playtestLifecycleId`/positive `playtestStateGeneration` changed. This remains `CONNECT_SCOPE_MISMATCH` because a valid current target invalidates the uncommitted discovery/connect-token snapshot; it is not `ADMISSION_POINTER_UNAVAILABLE`, which is reserved for invalid current pointer authority. In either cause, preserve account authentication but discard the complete discovery snapshot and every derived connect-token field, then rerun bootstrap/discovery, any required `Join & Play` and character gate, token issuance, and connection establishment. No field from the invalidated bundle may be reused or used to infer a replacement target.
...
The target ownership checks may produce `OWNERSHIP_UNAVAILABLE` for a reachable but incomplete or ambiguous ownership response; an ownership or coordination dependency that cannot be read or times out returns `AUTH_UNAVAILABLE`. Precedence is current pointer validity first (`ADMISSION_POINTER_UNAVAILABLE` for reachable missing, malformed, ambiguous, stale, or unresolvable catalog/pointer evidence; `AUTH_UNAVAILABLE` for an unreachable or timed-out authority; `REALM_UNAVAILABLE` for a complete `CLOSED` pointer), then verified connect-scope comparison (`CONNECT_SCOPE_MISMATCH` for either cause above), then entitlement and remaining admission checks. A pointer or authority failure masks scope comparison; a verified scope mismatch masks later checks. The classifications must not relabel a verified connect-scope mismatch as a stale timeline or relabel a stale runtime fence as a connect-scope mismatch.
...
Reconnect sequencing, snapshot reuse, transport-local scope replacement, and resume/takeover behavior are owned by [Reconnection Strategy](./system-architecture-reconnection.md) and [Session Behavior](./system-architecture-session-behavior.md). Authentication's target-state consequence is that each reconnect re-establishes the exact transport authentication path and then uses fresh routing plus one fresh Account membership snapshot before admission; stale selectors, expired scopes, partial membership evidence, and client-supplied authority never authorize fallback. Current runtime deviations from the target flow are recorded in [Implementation Status](#implementation-status), including incomplete membership lifecycle/generation reread and end-to-end proof for the local explicit `JOIN` operation.

Logical character identity is the canonical `characterId` resolved under the caller's tenant and selected playable-state namespace; it is distinct from active gameplay-binding uniqueness. All Redis key formats and Game Session Service APIs must treat `characterId` as the abstract character identifier so sessions bind sockets to characters rather than raw accounts. Canonical target takeover and resume uniqueness is `{tenantId, playableStateNamespaceId, characterId}` with atomic monotonic `bindingGeneration`; its binding value/evidence retains server-derived `playableStateScope`, `{sessionId, gameInstanceId, bindingGeneration}`, with `gameInstanceId` as the active-runtime fence. The existing `{tenantId, gameInstanceId, characterId}` identity is current implementation drift.

Gameplay identity is single-mode and canonical in the target: uniqueness key `{tenantId, playableStateNamespaceId, characterId}` with atomic monotonic `bindingGeneration`; `playableStateScope` remains binding context. The current per-instance uniqueness key `{tenantId, gameInstanceId, characterId}` remains an implementation gap.
...
| Multi-Client Behavior | One gameplay session per target `{tenantId, playableStateNamespaceId, characterId}` with atomic monotonic `bindingGeneration`; current per-instance uniqueness is implementation drift. Takeover and rebinding are defined in [Session Behavior](./system-architecture-session-behavior.md#namespace-scoped-controller-transfer-session-02) |
