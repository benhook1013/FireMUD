...
The sequence above is the target flow. Current deviations, including the unavailable explicit `JOIN` step and its non-actionable `JOIN_REQUIRED` result, are recorded once in [Implementation Status](#implementation-status); target-flow sections do not redefine current runtime behavior.
...
`PLAY` has an explicit pre-admission versus post-admission failure boundary. Before Account finalizes the exact-binding admission lease and Game Session publishes an admissible binding, an unreachable or timed-out lease, binding/CAS fence, applicable authority or projection, routing authority, or required coordination-health dependency returns retryable `AUTH_UNAVAILABLE` / HTTP 503. No provisional binding becomes admissible, no cached authority or local binding substitutes for the dependency, and the client retains authentication state for bounded retry. Reachable missing, malformed, stale, ambiguous, or contradictory catalog/pointer evidence returns `ADMISSION_POINTER_UNAVAILABLE`, while a complete `CLOSED` pointer returns `REALM_UNAVAILABLE`. A reachable missing, malformed, stale, regressed, expired, mismatched, or revoked non-routing value remains the applicable denial or `AUTH_SESSION_REVOKED`; it is not reclassified as an outage.
...
| --- | --- | --- | --- |
| `/ws/game/**` handshake (`403`) | `CONNECT_TOKEN_MISSING` | No connect-token carrier is supplied. For the supported `first_party_web` route, this means no `Firemud-Connect-Token` cookie; a header-only request does not qualify when the `non_first_party_public` route is unsupported or unregistered. | Obtain a fresh connect token and open a new socket with bounded retry/backoff. This is a handshake classification, not a post-connect text-protocol `ERROR <CODE>` response. |
...
| `/ws/game/**` handshake (`403`) | `CONNECT_TOKEN_REJECTED` | A connect-token carrier is supplied but is not accepted for the applicable route variant, including header-only `X-Firemud-Connect-Token` on an unsupported or unregistered `non_first_party_public` route, or the token is malformed, signature-invalid, missing required claims, wrong-audience, or otherwise rejected outside the narrower classes above | Obtain a fresh connect token and open a new socket with bounded retry/backoff. |
...
| `PLAY` | `ADMISSION_POINTER_UNAVAILABLE` | A reachable catalog/pointer authority returns missing, malformed, ambiguous, stale, or contradictory pointer evidence | Preserve authenticated lobby state, create no gameplay binding, rerun discovery/reconciliation, and retry only with fresh pointer evidence; never substitute a cached target. An unreachable or timed-out Account/routing authority dependency is the separate retryable `AUTH_UNAVAILABLE` class. |
...
| JWT Usage | Current raw Telnet gameplay command streams do not carry JWTs. Target browser/mobile gameplay uses `player-bootstrap` for bootstrap and an HttpOnly-cookie connect token for `/ws/game/**`; current endpoint implementation remains partial as recorded in [Implementation Status](#implementation-status). Profile and registry authority is [JWT and Token Contracts](./system-architecture-jwt-and-token-contracts.md), while public non-browser issuance/header support remains target-only. |
