...
- When the selected public-production tenant has missing or `INACTIVE` caller-bound membership, `JOIN <world>` explicitly establishes `ACTIVE` membership after `LOGIN`; an `INACTIVE` membership is restored with advances to `membershipAuthorityGeneration` and `membershipVersion`. An `ACTIVE` membership returns the exact idempotent current snapshot. Any other lifecycle state rejects. A caller with existing `ACTIVE` membership omits `JOIN` and continues to character selection and `PLAY`. First-party browser/mobile clients use the equivalent Account bootstrap join endpoint.
...
- Connect-token issuance API: control-plane endpoint (for example `POST /auth/connect-token`) that produces one durable request outcome per request digest, and at most one short-lived token for a committed success. Its `gameplay-connect` issuance/replay record remains separate from the ordinary `session:auth:token:<tokenHash>` registry/session. For a committed success, that dedicated record is the durable selected-route authority carrier and binds `accountId`, `tenantId`, `gameInstanceId`, `membershipLifecycleState=ACTIVE`, the exact selected-tenant `membershipAuthorityGeneration` and `membershipVersion`, `jti`, and issuance timestamp. The tenant-free `player-bootstrap` JWT and its ordinary registry record retain an empty membership-version scope and never substitute for this record. Connect-token identities may appear only in access-controlled security audit evidence; raw tokens and stable token identifiers are forbidden in ordinary logs, metric labels, and trace attributes.
  ...
  - Before every issuance, including the first issuance immediately after explicit join, Account Service must resolve `connectScopeId` to the canonical `{tenantId, worldSlug, realmSlug, gameInstanceId, pointerVersion, catalogRevision}` tuple, perform the live runtime entitlement check for `tenantId`, and perform a live realm-routing read for the selected realm target via the Game Session control-plane API. Entitlement authority unavailability returns `ENTITLEMENT_UNAVAILABLE`; authoritative billing denial returns `TENANT_BILLING_BLOCKED`. Account then branches by the resolved target mode while preserving the same entitlement, lifecycle, generation, version, and routing checks:
    ...
    - For either mode, Account independently rereads and validates the current `membershipAuthorityGeneration` and `membershipVersion` as separate fields from one coherent Account snapshot on every issuance, binding both unchanged values into a successful `COMMITTED` issuance baseline. A valid tenant-free `player-bootstrap` token alone is insufficient; an unavailable membership or other non-entitlement authority is `AUTH_UNAVAILABLE`, not `JOIN_REQUIRED`.
...
- Required claims: `iss`, `aud`, `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, `membershipAuthorityGeneration`, `membershipVersion`, `iat`, `exp`, `jti`, `replayAdmissionFence`.
...
- Gateway-issued context fields (minimum): `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `membershipAuthorityGeneration`, `membershipVersion`, `connectTokenJti`, `replayAdmissionFence`, `verifiedAt`, `expiresAt`, `gatewayRequestId`.
