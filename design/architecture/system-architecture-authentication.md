...
- Character selection and gameplay takeover target the one-active-character identity `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with `bindingGeneration`; the current `{tenantId, gameInstanceId, characterId}` identity is implementation drift.
...
- **Gameplay session identity key** – Target session uniqueness and takeover scope are keyed by `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with `bindingGeneration`; the binding value/evidence retains `{sessionId, gameInstanceId, bindingGeneration}`. The existing `{tenantId, gameInstanceId, characterId}` key is current implementation drift.
...
- **Gameplay session ownership** – Game Session owns gameplay binding records, the binding CAS, and bounded secondary indexes that map gameplay bindings by the target uniqueness key `{tenantId, playableStateNamespaceId, playableStateScope, characterId}`, account/tenant scope, and tenant scope so takeover, reconnect, and revocation do not require scans. Binding values/evidence retain `{sessionId, gameInstanceId, bindingGeneration}` as locally appropriate. Account owns the admission decision and exact-binding lease only; Account finalization commits lease/decision evidence and never creates, deletes, or mutates Game Session bindings or indexes. Orphan evidence from Account reconciliation becomes a durable cleanup request that Gateway and Game Session consume under their respective edge and gameplay fences.
...
- `PLAY` binds the gameplay session to `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` and retains `{gameInstanceId, sessionId, bindingGeneration}` as runtime/value evidence.
...
- Resolves `[character]` to a canonical `characterId` scoped to `{accountId, tenantId, playableStateNamespaceId, playableStateScope}` according to the selected realm's character policy, then binds the live session to target uniqueness `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with active runtime/value evidence `{gameInstanceId, sessionId, bindingGeneration}`.
...
Logical character identity is the canonical `characterId` resolved under the caller's tenant and selected playable-state namespace/scope; it is distinct from active gameplay-binding uniqueness. All Redis key formats and Game Session Service APIs must treat `characterId` as the abstract character identifier so sessions bind sockets to characters rather than raw accounts. Canonical target takeover and resume uniqueness is `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with `bindingGeneration`; its binding value/evidence retains `{sessionId, gameInstanceId, bindingGeneration}`, with `gameInstanceId` as the active-runtime fence. The existing `{tenantId, gameInstanceId, characterId}` identity is current implementation drift.

Gameplay identity is single-mode and canonical in the target: uniqueness key `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with `bindingGeneration`. The current per-instance uniqueness key `{tenantId, gameInstanceId, characterId}` remains an implementation gap.
...
| Multi-Client Behavior | One gameplay session per target `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with `bindingGeneration`; current per-instance uniqueness is implementation drift. Takeover and rebinding are defined in [Session Behavior](./system-architecture-session-behavior.md#multi-client-behavior-and-session-takeover) |
