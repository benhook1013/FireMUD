...
The current raw-text email-code challenge is two commands: `LOGIN <email>` (or `LOGON <email>`) requests an Account-owned, server-side verified-email challenge bound to the normalized account/email and its configured expiry without authenticating the session, then `LOGIN <email> <code>` (or `LOGON <email> <code>`) submits that code. The separate immediate-auth form is `LOGIN <email> <secret>` (or `LOGON <email> <secret>`) with one secret. Account selects the `EMAIL_OTP` validator first when the supplied secret matches the live challenge; otherwise it verifies `PASSWORD` when that account mode is enabled. The client does not select the validator, and a code is not valid outside its server-side challenge and expiry.
...
- **Membership behavior (target):** Only an explicit public-production `JOIN` / `Join & Play` through `JoinPublicProductionMembership` may change the durable caller-bound `player` membership. The lifecycle is exact: missing -> `JOIN` creates `ACTIVE` and advances `membershipVersion`; `INACTIVE` -> `JOIN` restores `ACTIVE` and advances `membershipVersion`; `membershipAuthorityGeneration` advances independently only when the transition records `callerBoundAuthorityInvalidated=true`; `ACTIVE` -> `JOIN` returns the exact idempotent current snapshot; every other lifecycle state rejects. `JOIN_REQUIRED` covers missing or `INACTIVE` membership before a successful join. Game Session admission, connect-token issuance, character creation, and `PLAY` require existing `ACTIVE` membership and must never invoke `EnsurePublicProductionPlayerMembership` or create or restore membership.
- **Membership behavior (current drift):** Email challenge and credential-bearing `LOGIN` / `LOGON` are the implemented raw-text authentication paths. The explicit join action and the required `membershipAuthorityGeneration` reread at connect-token issuance remain unimplemented/proof gaps. Current connect-token issuance and text `PLAY` require existing `membershipLifecycleState=ACTIVE` membership, never invoke Account's `EnsurePublicProductionPlayerMembership`, and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The old RPC/client/service seam remains present as unused implementation drift with no current admission caller, while the complete first-party browser/mobile sequence remains target-only and individual bootstrap/connect-token endpoints remain partial implementation surfaces. Returning active members and grant-backed private/playtest callers remain separately eligible only when their own current membership, grant, entitlement, routing, generation, and version checks pass.
...
**Target gameplay membership contract:** Tenant-scoped operational/design/runtime actions may allow `platformAdmin` per the route-class matrix, but gameplay admission and gameplay switching must not. Anonymous `WORLDS` may expose only the bounded public-production catalog. Authenticated discovery may combine caller-bound membership with public-production visibility. Character access, character creation, connect-token issuance, and `PLAY` require an existing caller-bound `ACTIVE` membership plus any applicable non-public realm grant; global roles or public visibility alone must never grant gameplay access. The only membership-writing exception is explicit public-production `JOIN`/`Join & Play`: missing -> `ACTIVE` and `INACTIVE` -> `ACTIVE` each advance `membershipVersion`, while `membershipAuthorityGeneration` advances independently only when `callerBoundAuthorityInvalidated=true`; `ACTIVE` returns the exact idempotent current snapshot and all other states reject. `JOIN_REQUIRED` covers missing or `INACTIVE` public-production membership. Private/playtest admission requires existing `ACTIVE` membership and the current grant, and no grant, connect-token, character, or `PLAY` path may create or restore membership implicitly. `billingAdmin` and `support` must never gain gameplay/design authority by virtue of being global roles. Current implementation drift is recorded in Implementation Status and is not overridden by this target contract.
...
- **Gameplay-connect replay-fence path** – ADR 0029 intentionally owns the dedicated one-use `gameplay-connect` issuance/replay record, carrier, `replayAdmissionFence`, quarantine, deny-marker, atomic exact-`jti` consume, and signed Gateway-to-Game Session context contract. The token is consumed only by Gateway and does not create or consult ADR 0035's Account issued-token registry, active token record, `tokenGeneration`, `issuanceFence`, or `session:auth:token:<tokenHash>` record. Its bounded selected-target `authorityTuple` and separate `membershipVersion` are immutable issuance evidence carried through the signed connect context for later exact-binding admission, not registry keys or standalone authority. ADR 0022 owns the service-authority partition between those paths.

### Auth Middleware Algorithm (Normative)

Any HTTP/gRPC route that depends on identity, roles, or tenant scoping must be protected by the shared auth middleware (for example, `AuthTokenInterceptor` plus a `TenantAccessGuard`). The middleware dispatches by the route matrix's authentication path: the registry lookup and Account generation steps below are one sequence only for registry-backed JWT-presenting routes. Public no-JWT routes, trusted TCP Proxy's exact-mTLS gameplay branch, non-JWT `LOGIN`/`PLAY`, routine gameplay, bound-session reconnect/resume, and the dedicated `gameplay-connect` path use their own checks and must not be made to satisfy a synthetic JWT registry contract. Implementations must follow the same decision logic so authorization behavior does not drift across services:

1. **Validate the JWT on registry-backed JWT routes** – For a route that presents a registry-backed JWT, verify signature (JWKS), time-based claims (`exp`, `nbf`), and the expected token profile/audience (`aud`). Reject tokens with an unexpected profile (for example a `control-ui` JWT presented to a player-bootstrap endpoint). The dedicated `gameplay-connect` token is validated by Gateway under its replay-fence contract instead; public no-JWT, trusted TCP Proxy, non-JWT `LOGIN`/`PLAY`, routine gameplay, and bound-session paths do not enter this step.
2. **Check issued-token registry on registry-backed JWT routes** – Compute `tokenHash` and require one matching `session:auth:token:<tokenHash>` record in Coordination Redis. Validate its account, profile, `jti`, per-lineage `tokenGeneration`, complete applicable `authorityTuple`, positive `issuanceFence`, applicable separate `membershipVersion`, and time fields against the already verified token. Lease, gameplay binding, token-identity fence, and membership-version evidence is required only when the token profile or exact route declares those predicates; every declared predicate and its applicable Account evidence must match current state. Reachable missing, stale, malformed, expired, revoked, or mismatched state means the token is revoked or unregistered and returns the canonical “session revoked” error (`AUTH_SESSION_REVOKED` or equivalent); an unavailable or timed-out dependency returns retryable `AUTH_UNAVAILABLE` and never uses cached state. This step applies only to a route that presents an issued JWT. Matrix-classified public no-JWT routes, the `pending_deletion_scoped` workflow credential, trusted TCP Proxy, in-band `LOGIN`/`PLAY` and routine gameplay commands, and the exact `gameplay-connect` Gateway handshake use their separately defined paths above and never consult this registry. Fresh gameplay admission uses its bound-session contract. The intentionally separate ADR 0029 gameplay-connect path instead requires its dedicated issuance/replay record, complete signed claim shape including its bounded selected-target `authorityTuple` and separate `membershipVersion`, current signed `replayAdmissionFence`, quarantine cutoff, atomic exact-`jti` single-use replay consumption, Gateway deny-marker state, and bounded lifetime checks; it is exempt from the active registry, `tokenGeneration`, `issuanceFence`, and session-token record. ADR 0035 remains authoritative only for registry-backed JWT profiles. Logout retries use only their separate durable intent/tombstone reconciliation path and create no authorization context. No other registry-backed JWT route or profile may bypass the registry.
3. **Check Account authority generations** – Enforce bulk revocation without relying on wildcard deletes, key scans, or JWT timestamps. Every registry-backed route that presents an issued JWT applies one Account-owned composite comparison of every applicable `authorityTuple` field, `accountSecurityCutoff`, `tenantBillingCutoff`, `issuanceFence`, and separate `membershipVersion` evidence. The route matrix, not JWT presence alone, determines whether tenant authority, caller-membership authority, and `membershipVersion` are applicable. Public no-JWT routes, the `pending_deletion_scoped` workflow credential, trusted TCP Proxy, in-band `LOGIN`/`PLAY` and routine gameplay commands, and `gameplay-connect` are outside this registry-backed JWT generation-read step; each follows its separately defined authority contract. In-band routine gameplay adds no per-command Account or Redis reads. Fresh `PLAY`, reconnect, and resume still perform the bound-session admission checks and only the current-authority checks explicitly required by those contracts. `gameplay-connect` carries a bounded selected-target `authorityTuple` and separate `membershipVersion` from issuance but performs no ordinary Account evidence-bundle or issued-token-registry read at Gateway; it accepts the deliberate maximum 30-second handshake lifetime plus at most five seconds of clock skew under ADR 0029's dedicated issuance/replay record, replay-fence, quarantine, deny-marker, and atomic-consume contract, then passes the unchanged baseline into the later exact-binding admission checks. Tenant and membership fields are additional route-class-specific checks; they do not replace the universal issuer/account checks on registry-backed JWT-presenting routes. A refresh or competing revocation writer must use one serializable Account transaction or composite CAS/fence across every applicable field, not independent per-field updates. The route table distinguishes those conditional tenant and membership checks from the JWT path's universal checks:
   - Within the registry-backed JWT path, `authorityTuple.issuerAuthGeneration` applies to every registry-backed JWT-presenting protected route and must advance for Account signing-key compromise or player-facing post-restore trust reset; it does not replace rejection of the affected `kid`.
   - Within the registry-backed JWT path, `authorityTuple.accountAuthorityGeneration` applies to account-wide security cutoffs.
   - For registry-backed routes classified as tenant-scoped regular or gameplay-affecting, the exact requested-tenant entry in `authorityTuple.tenantAuthorityGeneration` applies.
   - For registry-backed routes, the exact caller-bound `{accountId, tenantId}` entry in `authorityTuple.membershipAuthorityGeneration` applies only to caller-membership-bound tenant-role branches, caller-membership-scoped lifecycle routes, `player_bootstrap_tenant` routes where declared, and public-production onboarding after membership is created. A `platformAdmin` branch is the exception: it uses the exact target-tenant generation for the requested tenant without caller membership or caller-membership generation; `membershipVersion` remains a separate field/predicate and is never substituted for either authority generation.
   - The closed `billing_safe_tenant` exception intentionally omits the target-tenant authority generation for its caller-membership-bound branch; its issuer/account, caller-bound membership-generation, live-role, and route-class checks still apply within the registry-backed JWT path. This is not a general target-tenant-generation bypass, and no other route class inherits it.
...
| `pending_deletion_scoped` | One Account-issued `pending-deletion-access` credential; no JWT | Dedicated pending-deletion validator and workflow state; no account, tenant, or membership generation | None | None | The credential is bound to the account and deletion workflow and authorizes only status, cancellation, export, or necessary billing settlement. All four route families use this same scoped credential path; invalid, expired, revoked, binding-mismatched, or wrong-state credential/workflow evidence has internal outcome `AUTH_SESSION_REVOKED`, while unavailable credential registry or deletion-workflow authority has internal outcome `AUTH_UNAVAILABLE`. The separate unauthenticated recovery-response exception applies only to the public pending-deletion recovery challenge routes, not to these scoped routes; normal JWT/session/gameplay authority is never a fallback |

- **Unavailable versus revoked** - An unreachable or timed-out registry, lease, binding, token-identity fence, authority-generation source, or Account evidence dependency returns retryable `AUTH_UNAVAILABLE` / HTTP 503 and does not tell clients to discard authentication. Reachable missing, deleted, expired, malformed, stale, regressed, revoked, or mismatched authority returns `AUTH_SESSION_REVOKED` or the specific invalid-token outcome and requires reauthentication. The exception is limited to the unauthenticated public pending-deletion recovery challenge routes: their `account_state_disclosure` is prohibited and their response profile is uniform, so state or credential rejection is folded into that uniform response rather than emitted as `AUTH_SESSION_REVOKED`; the four `pending_deletion_scoped` status, cancellation, export, and necessary billing-settlement route families are not covered by that exception. A genuine authority outage remains `AUTH_UNAVAILABLE`. During `AUTH_UNAVAILABLE`, the UI may retain in-memory auth state for retry, but no cached JWT role, membership, generation, projection, or allowlist result may authorize the failed operation.
...
- **Target first-party bootstrap ownership:** Account Service owns `POST /auth/player-bootstrap`, bootstrap discovery, bootstrap-authenticated character discovery and character creation, explicit `/auth/bootstrap/join`, `POST /auth/connect-token`, and membership lifecycle. Game Session exposes the equivalent text `JOIN` command and owns in-socket `LOGIN`/`PLAY`, but delegates membership mutation to `JoinPublicProductionMembership` and never invokes `EnsurePublicProductionPlayerMembership` during `PLAY`. This target ownership is not a current-completion statement; see the membership status above.
- **Target join contract:** `POST /auth/bootstrap/join` and the delegated `JoinPublicProductionMembership` operation accept the verified discovery `connectScopeId` plus `requestId`, not an independently authoritative tenant/world/realm tuple. Account binds the request to the canonical operation, authenticated `accountId`/caller, verified `connectScopeId`, and resolved `{tenantId, worldSlug, realmSlug, gameInstanceId, catalogRevision, pointerVersion}`, then rechecks that selector and exact catalog/pointer pair plus the digest at the membership commit gate. A create or `INACTIVE` restore emits one transition audit/outbox event; an exact `ACTIVE` retry replays the stored result without an event; a changed selector or digest conflicts.
- **Target bootstrap-discovery and mutation APIs:** authenticated first-party HTTP endpoints (for example `GET /auth/bootstrap/worlds`, `GET /auth/bootstrap/worlds/{world}/realms`, `GET /auth/bootstrap/worlds/{world}/realms/{realm}/characters`, `POST /auth/bootstrap/join`, and bootstrap-authenticated `POST /auth/bootstrap/worlds/{world}/realms/{realm}/characters`) that accept only the `player-bootstrap` token profile and return or mutate the canonical lobby data used to choose a target before socket open. Character creation is allowed only after the explicit join where public-production membership is required.
  ...
  - Minimum selector fields returned by discovery for an admissible realm target: `connectScopeId`, `tenantId`, `worldSlug`, `realmSlug`, `gameInstanceId`, `pointerVersion`, `catalogRevision`, `evaluatedAt`, and `connectScopeExpiresAt`.
...
- Connect-token issuance API: control-plane endpoint (for example `POST /auth/connect-token`) that produces one durable request outcome per request digest, and at most one short-lived token for a committed success. Its `gameplay-connect` issuance/replay record remains separate from the ordinary `session:auth:token:<tokenHash>` registry/session. For a committed success, that dedicated record is the durable selected-route authority carrier and binds `accountId`, the unchanged routing baseline `{connectScopeId, tenantId, worldSlug, realmSlug, gameInstanceId, catalogRevision, pointerVersion, evaluatedAt, connectScopeExpiresAt}`, `membershipLifecycleState=ACTIVE`, the complete bounded selected-target `authorityTuple`, separate exact `membershipVersion`, `jti`, and issuance timestamp. `catalogRevision` is the authoritative Game Session routing record's reference to the separately versioned catalog/policy snapshot; Account's fresh entitlement snapshot, including `allowPublicJoin`, is a separate authority. The same Account issuance evidence boundary also carries the exact selected-route `runtimeEntitlementAuthorityReference` and `admissionPointerAuthorityReference`; these are source/version/fence references to the owning inputs, not replacement authority fields. The tuple includes the exact selected-tenant target-authority and caller-membership generations plus any applicable selected-realm grant/cutoff evidence from the same issuance snapshot. The tenant-free `player-bootstrap` JWT and its ordinary registry record retain empty tenant and membership scope and never substitute for this record. Connect-token identities may appear only in access-controlled security audit evidence; raw tokens and stable token identifiers are forbidden in ordinary logs, metric labels, and trace attributes.
  - Minimum request fields: `connectScopeId`, `requestId`.
  - Response fields are non-secret metadata only: `expiresAt`, `accountId`, `tenantId`, `gameInstanceId`, `realmSlug`, and `issuedAt`. The connect token is set only as the `Firemud-Connect-Token` HttpOnly cookie. First-party native-mobile and other first-party non-browser clients use a protected cookie jar; this endpoint never returns a raw `connectToken` response-body field. Explicitly classified non-first-party/public WebSocket clients have a separate target-only issuance route, not this endpoint. That route is not yet named, registered, or implemented and remains unavailable; before use it must have a dedicated authorization-matrix entry plus the dedicated `gameplay-connect` issuance/replay record, `replayAdmissionFence`, quarantine/deny, signed-context, response, and carrier proof. The target route issues the same `aud=gameplay-connect` single-use token, returns it exactly once in a protected response body to the classified client for OS secure storage, and permits carriage only through `X-Firemud-Connect-Token`. No browser or first-party cookie-jar client may use it.
  - Before every issuance, including the first issuance immediately after explicit join, Account Service must resolve `connectScopeId` to the canonical `{tenantId, worldSlug, realmSlug, gameInstanceId, pointerVersion, catalogRevision}` tuple, where `catalogRevision` comes from the authoritative Game Session routing record, perform the live Account entitlement check for `tenantId`, and perform a live realm-routing/catalog read for the selected realm target via the Game Session control-plane API. Entitlement authority unavailability returns `ENTITLEMENT_UNAVAILABLE`; authoritative billing denial returns `TENANT_BILLING_BLOCKED`. Account then branches by the resolved target mode while preserving the separate entitlement, lifecycle, generation, version, catalog, and routing checks:
    - **Public production:** require fresh Account entitlement with `allowPublicJoin=true` and fresh selected-target catalog/pointer evidence identifying the visible public-production realm. An Account policy denial returns `PUBLIC_PRODUCTION_ADMISSION_DENIED`; only after both gates pass may missing or `INACTIVE` membership return `JOIN_REQUIRED`. `JOIN_REQUIRED` is not a membership-absence shortcut that takes precedence over entitlement or public-admission policy. Connect-token issuance never creates or restores membership.
    - **Private/playtest:** require existing caller-bound `membershipLifecycleState=ACTIVE` and the exact current Account-owned realm-access grant. This branch never evaluates public-joining policy and never returns `JOIN_REQUIRED` for missing or `INACTIVE` membership; use `NON_PUBLIC_ENROLLMENT_REQUIRED` for that predicate. A grant never substitutes for membership.
    - For either mode, Account reads and validates the complete bounded selected-target `authorityTuple` plus separate exact `membershipVersion` from one coherent Account snapshot on every issuance, including the exact current target-tenant and caller-membership generations and any applicable selected-realm grant/cutoff evidence. It binds the unchanged routing baseline, membership baseline, and exact `runtimeEntitlementAuthorityReference` and `admissionPointerAuthorityReference` into the same Account issuance evidence boundary and carries the bounded tuple plus separate membership version into the signed token and Gateway context for a successful `COMMITTED` issuance. A valid tenant-free `player-bootstrap` token alone is insufficient; an unavailable membership or other non-entitlement authority is `AUTH_UNAVAILABLE`, not `JOIN_REQUIRED`.
  ...
  - `requestId` is the idempotency key and `requestDigest` is the immutable digest for connect-token issuance. The digest covers the canonical operation, caller account, `connectScopeId`, and every request field and resolved target field that participates in the decision. Account durably stores one request outcome keyed by the caller-bound request identity and digest before returning a terminal result. `COMMITTED` stores the one token payload and immutable membership issuance baseline; deterministic failures such as `JOIN_REQUIRED`, `REALM_UNAVAILABLE`, `CONNECT_SCOPE_MISMATCH`, `TENANT_BILLING_BLOCKED`, and other reachable validation denials store the stable error and terminal evidence without a token or issuance baseline. Retrying the exact `(accountId, requestId, requestDigest)` returns that stored success or failure and never re-evaluates a terminal failure or mints a replacement token. A changed digest is an idempotency conflict; a caller intentionally starting a new attempt after rediscovery must use a new `requestId`.
...
- Required claims: `iss`, `aud`, `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, the bounded selected-target `authorityTuple` including the exact target-tenant and membership-authority generations, separate exact `membershipVersion`, `iat`, `exp`, `jti`, and `replayAdmissionFence`.
...
- Ordinary JWT registry middleware explicitly excludes the `gameplay-connect` audience/profile and must not require an ordinary issued-token registry lookup for this route. Dedicated gameplay-connect admission uses its own Account issuance/replay record, signed `replayAdmissionFence`, and Gateway consume marker; ordinary JWT registry middleware remains authoritative for the profiles and routes that declare it.
...
- Gateway-issued context fields (minimum): `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, the exact bounded selected-target `authorityTuple`, separate exact `membershipVersion`, `connectTokenJti`, `replayAdmissionFence`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`.
...
- Replay guard: Gateway owns replay protection for `connectTokenJti` at handshake time using the shared replay cache. Game Session does not implement a second replay authority for that token identifier; it treats `connectTokenJti` inside the signed context as access-controlled audit correlation only and never emits it as a metric label, ordinary log field, or trace attribute.
...
5. Success responses cause the Game Session Service to establish or refresh the authenticated socket/session account context only; they do not select a tenant or character and do not create or refresh the gameplay session binding. Account creates the one issued-token registry record for the returned `game-session-account-delegation` token, and Game Session uses that token only for Account-bound backend calls under its exact profile and audience. Token `jti` and connect-token identity evidence remain internal registry, replay, and audit evidence; login/session responses and protocol carriers never send `jti` to clients. **Target-only orphan-token retirement (not current):** if the Game Session authenticated-context CAS fails after token issuance, Game Session must call Account's idempotent retire/abort operation for that exact request and token identity; Account removes or revokes the orphan registry record, and an orphaned token is never accepted merely because it was cryptographically valid. Game Session emits `OK LOGIN Logged in` (or equivalent account-confirming text) on the wire only after the authenticated account context binds successfully. `PLAY` performs gameplay admission, binding, and finalization. Error responses are translated to the shared `ERROR <CODE> <message>` format so protocol clients see consistent codes regardless of transport.
...
- `REALMS <world>` – list the visible realms for the selected world (`<world>` is a response-local
  ...
  forks.
- **Target `JOIN <world>`** – treat `<world>` as an adapter-local selector and obtain only the Account-bound `{connectScopeId, requestId}` for the authenticated operation. `JoinPublicProductionMembership` receives those two inputs plus the trusted caller context; Account binds `requestId` to the canonical operation, authenticated `accountId`/caller, verified `connectScopeId`, and resolved `{tenantId, worldSlug, realmSlug, gameInstanceId, pointerVersion}`. It applies the exact lifecycle: missing -> create `ACTIVE` and advance `membershipVersion`; `INACTIVE` -> restore `ACTIVE` and advance `membershipVersion`; either transition advances `membershipAuthorityGeneration` only when `callerBoundAuthorityInvalidated=true`; `ACTIVE` -> return the exact idempotent current snapshot; all other states reject. An exact retry replays the stored result, while a changed selector or digest conflicts. Raw client or adapter tenant, route, world, realm, or scope values are never operation authority. First-party clients expose the equivalent `Join & Play` action through Account bootstrap. This is target behavior; the current explicit join operation is not implemented.
...
- After the canonical routing and connect-scope gates, consults the runtime entitlement contract `GetTenantEntitlementsForRuntime(tenantId, requestId)` to confirm that the tenant is currently available for gameplay (for example, subscription state is not `suspended` or `canceled` and hard quotas are not violated).
...
- **Target membership-writer contract:** Membership lifecycle writer authority remains Account Service. The accepted canonical API is `JoinPublicProductionMembership`; the differently named current proto/client/service seam `EnsurePublicProductionPlayerMembership` remains present as unused implementation drift with no current admission caller, not as a retained compatibility operation. It must be replaced after the Account proto/service, authenticated caller, Gateway/auth routing and allowlists, configuration, tests, and generated references converge on the canonical name. The text-protocol adapter passes only the Account-bound `{connectScopeId, requestId}` plus trusted caller context to `JoinPublicProductionMembership`; world and realm remain adapter-local selectors, and the adapter never passes client- or adapter-supplied `tenantId`, route, `gameInstanceId`, `catalogRevision`, `pointerVersion`, or route authority. Account resolves and revalidates the tenant, world, realm, game instance, and exact catalog/pointer pair from `connectScopeId` at the commit gate rather than accepting those fields as independently authoritative player inputs. The operation is explicit `JOIN`/`Join & Play`; character creation, connect-token issuance, and `PLAY` never create or restore membership. Current text `PLAY` and connect-token issuance require existing `membershipLifecycleState=ACTIVE` membership, never invoke `EnsurePublicProductionPlayerMembership`, and fail closed with `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership; explicit `JOIN`/`Join & Play` remains unimplemented, and the target character-creation membership precondition plus required issuance proof remain the implementation/proof gaps recorded above.
...
The current shipped `PLAY` path resolves the selected realm to its admissible `{tenantId, gameInstanceId}`, validates caller-bound gameplay access and entitlement state, resolves the character, and binds `{tenantId, gameInstanceId, characterId}`. Its current membership behavior now requires existing `membershipLifecycleState=ACTIVE` membership, never invokes `EnsurePublicProductionPlayerMembership`, and returns `JOIN_REQUIRED` for eligible missing or `INACTIVE` public-production membership. The old writer seam remains unused implementation drift with no current admission caller; explicit `JOIN`/`Join & Play` and the target character-creation membership precondition remain unimplemented/proof gaps. It also does not claim that the live path already resolves or commits the target region, region epoch, lease owner, or lease fence. Those ownership and timeline checks are target-state responsibilities described above.
...
Clients re-authenticate **only after disconnecting** (TCP or WebSocket loss) or when server-side auth state has expired or been revoked. Direct text/Telnet reconnect resends the applicable credential-bearing `LOGIN` flow and first establishes fresh non-entitlement authority. If membership, registry, grant, or another required authority is unavailable, the result is `AUTH_UNAVAILABLE`; preserve authentication and retry that authority before evaluating missing/`INACTIVE` membership, offering `JOIN`, or repairing a character. Only after those checks pass does reconnect take the conditional `JOIN` step when required and complete `PLAY`; it has no connect-token reconnect snapshot, so `connectScopeId`, `catalogRevision`, and `CONNECT_SCOPE_MISMATCH` do not apply to that reconnect mode. Token-backed `/ws/game/**` reconnect obtains a fresh connect token, opens a new socket, issues bare `LOGIN`, and completes lobby selection again (`PLAY <world> [realm] [character]`) under the same ordering. A first-party reconnect shortcut is permitted only while the complete token-backed discovery snapshot, including its selected character and `connectScopeExpiresAt`, is present and unexpired, and fresh Account evidence confirms `membershipLifecycleState=ACTIVE`, the exact `membershipAuthorityGeneration`, independent `membershipVersion`, and a valid current character for that target. Once any snapshot field is missing or `connectScopeExpiresAt` has passed, the client must rerun discovery rather than reuse cached selection. Only that token-backed reconnect uses the snapshot bundle containing the selected `worldSlug`/world/realm/character, `connectScopeId`, `tenantId`, `gameInstanceId`, `pointerVersion`, `catalogRevision`, `evaluatedAt`, and `connectScopeExpiresAt`; a `CONNECT_SCOPE_MISMATCH` discards the entire bundle and all derived connect-token metadata. If a resumable gameplay session exists for the selected `{tenantId, gameInstanceId, characterId}`, the Game Session Service resumes it; otherwise it creates a fresh gameplay session binding.
...
The canonical per-token and logout-all ordering, retry, token-fence, Gateway deny-marker, and reconciliation contract is defined once in [Session Behavior](./system-architecture-session-behavior.md#control-plane-logout). Authentication surfaces must follow that contract rather than restating or weakening it here. After the ordinary registry record is deleted, an exact per-token logout retry is lifecycle-only: Account recovers the result from the matching durable `COMMITTED` tombstone using the original request ID, digest, token identity, and opaque logout handle; registry absence alone never proves completion or authenticates the retry.
...
| Topic | Description |
| --- | --- |
| Auth Command | `LOGIN` (or `LOGON`) — prompt exchange is target-only; current clients use `LOGIN <email>` / `LOGON <email>` then `LOGIN <email> <code>` / `LOGON <email> <code>` for the email-code challenge, or immediate `LOGIN <email> <secret>` / `LOGON <email> <secret>` |
