...
- Resolves `[character]` to a canonical `characterId` scoped to `{accountId, tenantId, playableStateNamespaceId}` according to the selected realm's character policy, then binds the live session to the active `{tenantId, gameInstanceId}` runtime for that namespace.
