...
- **Target first-party bootstrap ownership:** Account Service owns `POST /auth/player-bootstrap`, bootstrap discovery, bootstrap-authenticated character discovery and character creation, explicit `/auth/bootstrap/join`, `POST /auth/connect-token`, and membership lifecycle. Game Session exposes the equivalent text `JOIN` command and owns in-socket `LOGIN`/`PLAY`, but delegates membership mutation to `JoinPublicProductionMembership` and never performs membership mutation during `PLAY`. This target ownership is not a current-completion statement; see the membership status above.
...
The current shipped `PLAY` path resolves the selected realm to its admissible `{tenantId, gameInstanceId}`, validates caller-bound gameplay access and entitlement state, resolves the character, and binds `{tenantId, gameInstanceId, characterId}`. Its current membership behavior requires a fresh authoritative membership snapshot, returns `JOIN_REQUIRED` for public-production membership that is missing or `INACTIVE`, and returns `AUTH_UNAVAILABLE` when that authority is unavailable or unsafe; it never creates or restores membership. The obsolete writer seam has been removed; explicit `JOIN`/`Join & Play` and the target character-creation membership precondition remain unimplemented/proof gaps. It also does not claim that the live path already resolves or commits the target region, region epoch, lease owner, or lease fence. Those ownership and timeline checks are target-state responsibilities described above.
