...
- Resolves `[character]` to a canonical Entity-owned persisted `characterId` within `{tenantId, playableStateNamespaceId}` according to the selected realm's published entry policy and its applicable versioned descriptor or template, then validates `accountId` separately as ownership evidence for the authoritative Entity association. Server-derived `playableStateScope` is binding/routing/policy/authorization evidence rather than controller identity. It then binds the live session to target uniqueness `{tenantId, playableStateNamespaceId, characterId}` with active runtime/value evidence `{playableStateScope, gameInstanceId, sessionId, bindingGeneration}`.
