...
- If the public realm is visible but the account's durable membership is missing or `INACTIVE`, returns `JOIN_REQUIRED` with `JOIN <world>`/`Join & Play` recovery guidance and does not create membership or other admission state. This classification occurs only after the canonical entitlement and public-policy gates. Explicit public-production `JOIN` establishes or restores `ACTIVE` membership as defined above; an `ACTIVE` membership is returned idempotently and all other states reject. Private/playtest admission requires existing `ACTIVE` membership plus its current grant and never invokes this writer.
