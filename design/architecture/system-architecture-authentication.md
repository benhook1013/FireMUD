...
Gateway strips the external carrier and attaches one signed, short-lived context. Gateway owns issuance, replay, signing-key rotation, and carrier policy; Game Session owns receiving-service validation before applying gameplay admission.

Minimum context fields are `accountId`, `tenantId`, `gameInstanceId`, `worldSlug`, `realmSlug`, `pointerVersion`, `catalogRevision`, `connectScopeId`, `requestId`, the exact bounded selected-target `authorityTuple`, separate exact `membershipVersion`, `connectTokenJti`, `replayAdmissionFence`, `verifiedAt`, `expiresAt`, and `gatewayRequestId`.

- Game Session validates the asymmetric signature and `kid` against Gateway verification keys, enforces the bounded expiry, and rejects missing, invalid, or expired context with `CONNECT_CONTEXT_INVALID` before `PLAY`.
