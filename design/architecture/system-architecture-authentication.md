...
- Telnet and other non-WebSocket text clients use `LOGIN <email>` to request a verified-email code or `LOGIN <email> <secret>` to authenticate immediately. They do not receive or transmit `control-ui`, `player-bootstrap`, private delegation, or gameplay-connect JWTs. Any richer multi-line prompt flow is target-only and is not part of the current client capability contract.
...
- Connect-token issuance API: control-plane endpoint (for example `POST /auth/connect-token`) that mints exactly one short-lived token per request, records a dedicated `gameplay-connect` issuance/replay record separate from the ordinary `session:auth:token:<tokenHash>` registry/session, and logs `accountId`, `tenantId`, `gameInstanceId`, `jti`, and issuance timestamp.
  ...
  - Response fields are non-secret metadata only: `expiresAt`, `accountId`, `tenantId`, `gameInstanceId`, `realmSlug`, `jti`, and `issuedAt`. The connect token is set only as the `Firemud-Connect-Token` HttpOnly cookie. First-party native-mobile and other first-party non-browser clients use a protected cookie jar; this endpoint never returns a raw `connectToken` response-body field. Explicitly classified non-first-party/public WebSocket clients use a separate target-only issuance route, not this endpoint. That route is not yet named or implemented and must be registered in the authorization matrix before use; it issues the same `aud=gameplay-connect` single-use token, returns that token exactly once in a protected response body to the classified client for OS secure storage, and permits carriage only through `X-Firemud-Connect-Token`. No browser or first-party client may use it.
...
- Telnet and other non-WebSocket text transports use `LOGIN <email>` for an email-code challenge or `LOGIN <email> <secret>` for immediate authentication. Any richer multi-line prompt flow is target-only and is not part of the current client capability contract. Third-party WebSocket clients must use the classified bootstrap/connect-token header path rather than introducing a credential-bearing public WebSocket exception.
...
- `REALMS <world>` – list the visible realms for the selected world (`<world>` is a response-local
  ...
  forks.
- `JOIN <world>` – treat `<world>` as a selector, resolve it server-side to the public-production `{worldSlug, realmSlug, connectScopeId, requestId}` join context, and explicitly create the caller's durable `player` membership only when it is missing, or return the existing membership for that realm. The adapter obtains the Account-bound `connectScopeId` and operation `requestId`; raw client tenant, route, realm, or scope values are never authority. First-party clients expose the equivalent `Join & Play` action through Account bootstrap.
...
- `REALMS <world>` and explicit `JOIN <world>` participate in `public_production_onboarding` when the selected realm is the default public production realm. Brand-new authenticated accounts may discover that realm without a membership, but `CHARS`, character creation, and connect-token issuance require durable membership plus any applicable realm grant. `PLAY` participates in `gameplay_admission` and applies one of its selected admission modes. A grant never substitutes for membership; grant-backed private/playtest entry requires existing membership plus the current grant, skips public `JOIN`, and never auto-creates membership.
...
- `AUTH_UNAVAILABLE` – a required current non-entitlement authority dependency is unreachable or timed out. Preserve authenticated state, create no gameplay binding, and retry the affected admission with bounded backoff; never authorize from cached authority.
...
| `PLAY` | `TENANT_BILLING_BLOCKED` | Entitlement state denies gameplay, including `suspended`/`canceled` or a `grace` state that denies the requested new commitment | Keep auth state, surface a billing-blocked state for that tenant, and disable the denied admission or commitment flow. |
