...
2. **Check issued-token registry** – Compute `tokenHash` and require one matching `session:auth:token:<tokenHash>` record in Coordination Redis. Validate its account, profile, `jti`, per-lineage `tokenGeneration`, complete `authorityTuple`, positive `issuanceFence`, and time fields against the already verified token. The record's tuple, lease, binding, token-identity fence, and Account evidence must all match the current applicable state. Reachable missing, stale, malformed, expired, revoked, or mismatched state means the token is revoked or unregistered and returns the canonical “session revoked” error (`AUTH_SESSION_REVOKED` or equivalent); an unavailable or timed-out dependency returns retryable `AUTH_UNAVAILABLE` and never uses cached state. The only authorization exceptions are an explicitly matrix-classified public route whose contract requires no JWT and the exact matrix-declared `gameplay-connect` Gateway handshake. The gameplay-connect handshake is not validated by an issued-token registry lookup: ADR 0029 instead requires Account-signed claims, the current signed replay fence, quarantine cutoff, atomic exact-`jti` single-use replay consumption, Gateway deny-marker state, and bounded lifetime checks. Logout retries use only their separate durable intent/tombstone reconciliation path and create no authorization context. No other route or profile inherits these exceptions.
3. **Check Account authority generations** – Enforce bulk revocation without relying on wildcard deletes, key scans, or JWT timestamps. Every route that accepts an issued JWT applies one Account-owned composite comparison of every applicable `authorityTuple` field, `accountSecurityCutoff`, `tenantBillingCutoff`, `issuanceFence`, and separate `membershipVersion` evidence, except an explicitly matrix-classified public no-JWT route and the exact `gameplay-connect` bounded single-use exception declared above and in the route matrix. `gameplay-connect` omits the tuple and issuance fence and accepts the deliberate maximum 30-second lifetime plus at most five seconds of clock skew under ADR 0029's replay-fence, quarantine, deny-marker, and atomic-consume contract. Tenant and membership fields are additional route-class-specific checks; they do not replace the universal issuer/account checks. A refresh or competing revocation writer must use one serializable Account transaction or composite CAS/fence across every applicable field, not independent per-field updates. The route table distinguishes those conditional tenant and membership checks from the universal checks:
