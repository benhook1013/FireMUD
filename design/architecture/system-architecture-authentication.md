...
- **Target authoritative admission:** before committing gameplay binding, Game Session applies the current region/lease ownership and exact-binding authority contract. Gateway owns edge route lifecycle and connect-token scope; Game Session remains the receiving service for binding and ownership validation. Missing or ambiguous ownership returns `OWNERSHIP_UNAVAILABLE`, an unavailable dependency returns `AUTH_UNAVAILABLE`, and a stale timeline or verified target returns `STALE_TIMELINE` or `CONNECT_SCOPE_MISMATCH`. `PLAY` must not bind from cached ownership, raw transport headers, or a stale discovery result.
