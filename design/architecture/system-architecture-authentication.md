...
- **Target behavior:** Only an explicit public-production `JOIN` / `Join & Play` may create the durable caller-bound `player` membership. Game Session admission, connect-token issuance, character creation, and `PLAY` must require that existing membership and must never create it implicitly.
