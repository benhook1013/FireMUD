...
- Resolves `[character]` to a canonical `characterId` within `{tenantId, playableStateNamespaceId}` according to the selected realm's character policy, then validates `accountId` separately as ownership evidence. Server-derived `playableStateScope` is binding/routing/policy/authorization evidence rather than controller identity. It then binds the live session to target uniqueness `{tenantId, playableStateNamespaceId, characterId}` with active runtime/value evidence `{playableStateScope, gameInstanceId, sessionId, bindingGeneration}`.
