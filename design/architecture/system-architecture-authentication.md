...
- Resolves the selected realm's gameplay-admissible instance and records that `gameInstanceId` in the gameplay binding. The selected-target catalog/policy revision and routing pointer remain the authoritative pair; Account entitlement `allowPublicJoin` does not replace either routing input.
  - Resolves the realm's stable `playableStateNamespaceId` from its catalog/policy state. Durable character/progression/inventory state uses that namespace, while disposable runtime state remains keyed to `gameInstanceId`; missing or contradictory namespace evidence fails closed.
...
Gameplay identity is canonicalized on `characterId` within a tenant. All Redis key formats and Game Session Service APIs must treat `characterId` as the abstract character identifier so sessions bind sockets to characters rather than raw accounts. Canonical target takeover and resume identity is `{tenantId, playableStateNamespaceId, playableStateScope, characterId}` with `bindingGeneration`; the binding value/evidence retains `{sessionId, gameInstanceId, bindingGeneration}`. The existing `{tenantId, gameInstanceId, characterId}` identity is current implementation drift.
