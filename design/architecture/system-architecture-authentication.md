...
`REALMS <world>` uses the existing `public_production_onboarding` route classification. Its public-onboarding semantics cover only discovery of the default public-production realm and the explicit first-join path; non-public realm discovery remains caller-bound and is governed by the membership/grant checks below, not by public onboarding. The classification is deterministic across the two points at which the command may run; the resolved world/tenant is an input to the checks, not a second route class:
