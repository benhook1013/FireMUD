...
Authorization: Bearer <bootstrapToken>
-> []  # abbreviated character list; the realm snapshot above carries the admission fields
...
| Claims | Profile-dependent: shared JWT claims include `iss`, `sub`, `jti`, `aud`, `iat`, `nbf`, and `exp`; registry-backed profiles must additionally carry `authorityTuple`, `membershipVersion`, and `issuanceFence`, plus applicable `accountId`, `tokenGeneration`, `globalRoles[]`, or `scopedRoles{}` according to the token contract |
