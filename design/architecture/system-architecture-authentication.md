...
- Bootstrap-discovery APIs: authenticated first-party HTTP endpoints (for example `GET /auth/bootstrap/worlds`, `GET /auth/bootstrap/worlds/{world}/realms`, `GET /auth/bootstrap/worlds/{world}/realms/{realm}/characters`) that accept only the `player-bootstrap` token profile and return the canonical lobby discovery data used to choose a target before socket open.
  ...
  - For non-public realms such as playtest forks, visibility is controlled by an explicit realm-access grant. The target minimum grant record is `{tenantId, worldSlug, realmSlug, accountId, state, grantRevision, grantedByAccountId, grantedAt, grantExpiresAt?, requestId}`. A playtest tester grant always has `grantExpiresAt` no later than the fork's required expiry; its effective expiry is `min(grantExpiresAt, forkExpiresAt)`. Extending a fork never extends a tester grant implicitly. The current first implementation centralizes Account Service grant authority and runtime reads, but expiry and tenant-admin management UX remain product/control-plane follow-through.
  ...
  - Required semantics for realm-access-grant reads/writes:
    - validate the exact tenant, world, realm, account, current fork lifecycle, and caller authority before mutation; `tenantAdmin` is routine authority only for its tenant and `platformAdmin` is an audited break-glass path
    - idempotent create, extend, and revoke commands by `requestId`, with one monotonic `grantRevision` per `{accountId, tenantId, worldSlug, realmSlug}` scope
    - retain a revisioned revocation tombstone rather than erasing ordering evidence; a stale grant retry cannot resurrect access, while a deliberate later re-grant must observe and advance the latest revision
    - expired grants are treated as non-existent for visibility and admission; playtest access ends at the earlier of the grant and fork expiries
    - successful create, extend, and revoke must be immediately visible to subsequent discovery/admission reads
  ...
  - Revocation or expiry hides the realm from the affected account, denies connect-token issuance and `PLAY`, fences new gameplay command admission, and causes Game Session to terminate every affected active binding through the bounded authority-revocation contract. Work durably admitted before the fence may finish idempotently once. A friendly scheduled ending instead closes admission and completes the bounded realm drain before grants are revoked or expire.
...
- Losing realm visibility or realm-grant authority must fail admission before gameplay binding; clients must not remain eligible for a non-public realm only because they still hold a stale discovery response. Loss of a required grant also terminates an existing affected binding under the bounded revocation contract; it is not deferred until reconnect.
