...
The normative contract below remains mTLS for internal gRPC outside intentionally relaxed local development. Current hosted Helm renders `pr-preview` with `FIREMUD_GRPC_PLAINTEXT=false` and the shared `firemud-grpc-tls` Secret, so hosted preview currently uses mTLS; plaintext is reserved for the bounded, documented Spring gRPC `1.0.x` migration policy exception and is not the current `pr-preview` transport. The remaining hardening path is CI/static checking that rejects legacy or ignored server-TLS property usage. Protobuf generation exists, but deliberate Buf compatibility baselines, release compatibility classification, transitional bridge proof, and durable late-event safety proof under [ADR 0174](./decisions/adr-0174-maturity-scoped-protobuf-compatibility.md) remain incomplete. These current caveats do not weaken the workload identity, exact method allowlist, or canonical Spring SSL-bundle requirements.

**Current transport implementation/proof gap:** common client/server wiring still exposes generic plaintext/TLS toggles without an enforced profile guard. Every workload outside an explicitly selected local/test profile or the documented hosted-preview migration exception must reject plaintext and disabled server TLS at startup; no generic toggle may silently enable either state. The bounded local/hosted-preview exception remains explicitly profile-scoped and documented. This is an implementation/proof gap, not a change to the target transport.
...
- Required labels are `service` (from `spring.application.name`) and a bounded `code` from the shared catalog or transport classification.
  - The `service` label may be attached explicitly per counter increment, or injected globally via a Micrometer `commonTags("service", spring.application.name)` configuration from the shared `CommonAutoConfiguration` in the applicable split `common-*` module.
...
The bounded Spring gRPC `1.0.x` migration-policy exception permits plaintext internal gRPC only in an explicitly documented preview migration window; it is not the current hosted `pr-preview` transport, which uses mTLS. Even when exercised, plaintext preview transport is never evidence for traffic-open or promotion readiness. The exception must be removed after the SSL-bundle migration and preview re-proof complete and does not change the canonical non-local target state above.
