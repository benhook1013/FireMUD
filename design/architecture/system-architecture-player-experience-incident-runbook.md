...
Use only the selected profile's advertised observability paths. The default indexed profile uses Grafana/Kibana/Jaeger, a compatible profile uses its documented mappings, and a reduced profile uses only its declared console/journal path; if none is declared, log retrieval is unavailable, without claiming indexed search. If any advertised backend is degraded, follow the degraded-observability procedures in `system-architecture-observability-incident-runbook.md` and the degraded-mode branches in each scenario below.
...
6. **Degraded-mode branch (if observability backends are unavailable)**
   ...
   - If the selected indexed query path is unavailable, or the profile omits indexed search: use the profile's declared console/journal path filtered by `service`, `traceId`, and `correlationId`; if none is declared, log retrieval is unavailable. Do not require gameplay identity fields for this pre-gameplay login path.
...
- Determine whether the latency is:
  ...
  - **Tick-bound** (mode-specific tick execution p99 approaching `tick_lock_ttl_ms` for normal samples or `solo_lock_ttl_ms` for solo-budget samples).
...
1. **Check tick health first**
   - Use the Tick Health & Ledger dashboard:
     ...
     - Only when a current Game Session producer contract and bounded `scope_class`/`tick_mode` label proof exist, inspect the mode-specific ratio `tick_execution_time_ms_p99{scope_class=~".+",tick_mode="normal"} / on (scope_class) tick_lock_ttl_ms{scope_class=~".+"}` or `tick_execution_time_ms_p99{scope_class=~".+",tick_mode="solo"} / on (scope_class) solo_lock_ttl_ms{scope_class=~".+"}` for affected approved scope buckets. Otherwise use authoritative Game Session/control-plane runtime-health evidence and structured logs; then resolve exact regions through those same owner reads.
