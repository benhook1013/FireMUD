...
Use only the selected profile's advertised observability paths with current retained asynchronous emitter-to-query proof for any indexed search. The default indexed profile uses Grafana/Kibana/Jaeger, a compatible profile uses its documented mappings, and a reduced profile uses only its declared console/journal path; an advertised indexed path without current passed proof is unavailable/degraded, so use the declared current fallback (including owner-permitted Logging & Admin/PostgreSQL or direct logs) and do not claim indexed search. If none is declared, log retrieval is unavailable. If any advertised backend is degraded, follow the degraded-observability procedures in `system-architecture-observability-incident-runbook.md` and the degraded-mode branches in each scenario below.
...
- If traces are too sparse:
  - If service-scoped escalation is advertised and proved, apply its supported sampler control through the authorized deployment-control owner with the exact affected service/workflow scope, incident identity, start time, volume budget, positive TTL/lease, and enforceable automatic expiry. A durable expiry/reconciliation path must remove the elevation even if the initiator disappears; at expiry it must retry a safe reload/rollback, use an emergency disable-to-baseline path when available if reload/rollback fails, record completion only after sampled volume returns to the measured pre-escalation baseline, and otherwise retain an explicit incomplete/degraded state. Environment variables are usable only where the environment declares them wired.
  - If the environment advertises and independently proves tenant/game-instance/region-scoped escalation for the affected workflow, use its supported collector tail-sampling control for the impacted runtime scope after resolving it through control-plane/runtime-health reads and logs. The escalation must carry the incident identity, exact scope, start time, positive TTL/lease, volume budget, and expiry under the authorized collector-control owner; durable expiry must remove it even if the initiator disappears. At expiry, retry safe collector reload/rollback; if either fails, do not leave the expired elevated policy active or retain it as terminal last-valid state: continue durable reconciliation, use an emergency disable-to-baseline path when available with safe reload/rollback and verification, record completion only after sampled volume returns to the measured pre-escalation baseline, and otherwise retain an explicit incomplete/degraded state.
- If the environment does not meet the advertised-and-proved [ADR 0167: Allowlisted Sensitive Trace Attributes](./decisions/adr-0167-allowlisted-sensitive-trace-attributes.md) capability for the required level, treat it as baseline or service-scoped-only as applicable and do not claim scoped escalation.
...
3. **Check backing services**
   - Confirm Postgres and Redis health via:
     ...
     - Redis coordination health and, when the profile advertises and proves a current measured producer, the canonical tail-loss budget/breach pair (`redis_unreplicated_write_window_ms{scope}` compared with `redis_unreplicated_write_window_slo_ms{scope}` or its proved breach series), plus tick health. If measured evidence is unavailable, stale, or unproved, classify tail-loss state as unknown and use the profile's declared reset-tolerance and latency/recovery fallback.
4. **Mitigate**
...
5. **Verify recovery**
   ...
   - Use the selected profile's supported operator-query path to spot-check representative logs by `service`, `traceId`, and `correlationId`, adding `tenantId` or `characterId` only when those fields are present, to confirm that errors have returned to normal levels. The default indexed profile may use the `player-incident-drilldown.json` Kibana saved search, and a compatible indexed profile may use its mapped equivalent, only when the profile has current passed retained asynchronous emitter-to-query proof; otherwise treat the indexed path as unavailable/degraded and use the declared current fallback, including owner-permitted Logging & Admin/PostgreSQL or direct logs. A reduced profile uses only its declared console/journal path; if none is declared, log retrieval is unavailable.
6. **Degraded-mode branch (if observability backends are unavailable)**
   ...
   - If the selected indexed query path is unavailable, lacks current passed retained asynchronous emitter-to-query proof, or the profile omits indexed search: use the profile's declared current fallback (console/journal filtered by `service`, `traceId`, and `correlationId`, or owner-permitted Logging & Admin/PostgreSQL/direct logs); if none is declared, log retrieval is unavailable. Do not require gameplay identity fields for this pre-gameplay login path.
...
- Determine whether the latency is:
  ...
  - **Tick-bound** only when the selected profile advertises the mode-aware tick metrics and the current producer has proved a complete, fresh p99/tick-TTL/solo-TTL series for every affected bounded `scope_class`/`tick_mode` class; then use mode-specific tick execution p99 approaching `tick_lock_ttl_ms` for normal samples or `solo_lock_ttl_ms` for solo-budget samples. If any affected class lacks a complete fresh series, classify from authoritative Game Session/control-plane runtime-health evidence and structured logs rather than p99/TTL metrics.
...
1. **Check tick health first**
   - Use the Tick Health & Ledger dashboard:
     - The canonical mode-aware execution/TTL/ratio families and this dashboard are target-only and currently unavailable until Game Session emits and proves the bounded `scope_class`/`tick_mode` labels. Treat absent or stale series as unknown and use authoritative runtime-health/control-plane reads and structured logs for current decisions.
     - Only when a current Game Session producer contract advertises the mode-aware capability and proves a complete, fresh p99/tick-TTL/solo-TTL series for every affected bounded `scope_class`/`tick_mode` class, inspect the mode-specific ratio `tick_execution_time_ms_p99{scope_class=~"region|game_instance|tenant|cluster",tick_mode="normal"} / on (scope_class,tick_mode) label_replace(tick_lock_ttl_ms{scope_class=~"region|game_instance|tenant|cluster"},"tick_mode","normal","scope_class",".*")` or `tick_execution_time_ms_p99{scope_class=~"region|game_instance|tenant|cluster",tick_mode="solo"} / on (scope_class,tick_mode) label_replace(solo_lock_ttl_ms{scope_class=~"region|game_instance|tenant|cluster"},"tick_mode","solo","scope_class",".*")` for affected approved scope buckets. If any affected class is missing, stale, or incomplete, use authoritative Game Session/control-plane runtime-health evidence and structured logs; then resolve exact regions through those same owner reads.
     - Inspect the exact canonical `tick_retry_queue_depth{scope_class}` and `tick_command_queue_depth{scope_class}` series only when the selected profile advertises them and the current producer proves complete, fresh samples with the bounded `scope_class` label. Absent, stale, unavailable, or unproved series are unknown; use authoritative Game Session/control-plane runtime-health evidence and structured logs instead.
...
2. **Check Redis coordination**
   - On the Redis & Coordination Health dashboard:
     - Inspect the canonical measured tail-loss budget/breach pair only when its current producer, freshness, and bounded deployment scope are proved; otherwise classify the tail-loss state as unknown and use the Redis incident runbook's reset-tolerance and latency/recovery fallback. Do not infer an exact gameplay scope from class-level Redis metrics.
...
5. **Verify recovery**
   ...
   - Use the selected profile's supported operator-query path to correlate any remaining slow commands by `service` and `traceId`, adding `correlationId` and the applicable `tenantId`, `gameInstanceId`, `regionId`, `characterId`, and `tickId` fields only when the affected records expose them. The default indexed profile may use the `player-incident-drilldown.json` and `tick-region-logs.json` Kibana saved searches, and a compatible indexed profile may use its mapped equivalents, only when the profile has current passed retained asynchronous emitter-to-query proof; otherwise treat the indexed path as unavailable/degraded and use the declared current fallback, including owner-permitted Logging & Admin/PostgreSQL or direct logs. A reduced profile uses only its declared console/journal path; if none is declared, log retrieval is unavailable. Resolve the exact `<tenantId, gameInstanceId, regionId>` runtime scope through Game Session/control-plane runtime-health reads and structured logs rather than inferring `gameInstanceId` from query filters.
6. **Degraded-mode branch (if observability backends are unavailable)**
   - If Grafana is down: run direct PromQL checks for command p99 latency, synthetic command-canary success/latency only when current, fresh, valid advertised evidence exists for the matching profile and exposed path, and the mode-specific tick safety ratio only when a current Game Session producer contract advertises the mode-aware capability and proves a complete, fresh, valid `tick_execution_time_ms_p99`/`tick_lock_ttl_ms`/`solo_lock_ttl_ms` series for every affected bounded `scope_class`/`tick_mode` class. If any affected class lacks that complete fresh proof, treat the ratio as unknown and use authoritative Game Session/control-plane runtime-health evidence and structured logs. Inspect `tick_retry_queue_depth{scope_class=~"region|game_instance|tenant|cluster"}` / `tick_command_queue_depth{scope_class=~"region|game_instance|tenant|cluster"}` only when the selected profile advertises these exact canonical series and the current producer proves complete, fresh samples; otherwise treat queue-depth evidence as unknown and use authoritative Game Session/control-plane runtime-health evidence and structured logs. Inspect the canonical measured tail-loss budget/breach pair only when its producer and freshness are proved; otherwise classify tail-loss state as unknown and use the declared reset-tolerance and latency/recovery fallback. Do not infer exact gameplay scope from any class-level metrics; resolve it through Game Session/control-plane health and structured logs.
   ...
   - If the selected indexed query path is unavailable, lacks current passed retained asynchronous emitter-to-query proof, or the profile omits indexed search: use the profile's declared current fallback (console/journal by `service` and `traceId`, or owner-permitted Logging & Admin/PostgreSQL/direct logs), adding `correlationId` and conditional gameplay identity fields (`tenantId`, `gameInstanceId`, `regionId`, `characterId`) only when present in the affected records; if none is declared, log retrieval is unavailable.
...
4. **Verify recovery**
   ...
   - Use the selected profile's supported operator-query path to validate that chat-related errors or delays have subsided by `service` and `traceId`, adding `correlationId` and conditional gameplay identity fields only when present in the affected records. The default indexed profile may use the `player-incident-drilldown.json` Kibana saved search, and a compatible indexed profile may use its mapped equivalent, only when the profile has current passed retained asynchronous emitter-to-query proof; otherwise treat the indexed path as unavailable/degraded and use the declared current fallback, including owner-permitted Logging & Admin/PostgreSQL or direct logs. A reduced profile uses only its declared console/journal path; if none is declared, log retrieval is unavailable.
5. **Degraded-mode branch (if observability backends are unavailable)**
   ...
   - If the selected indexed query path is unavailable, lacks current passed retained asynchronous emitter-to-query proof, or the profile omits indexed search: use the profile's declared current fallback (console/journal using `service` and `traceId`, or owner-permitted Logging & Admin/PostgreSQL/direct logs), adding `correlationId` and conditional `tenantId`/`gameInstanceId`/`regionId`/`characterId` fields when present; if none is declared, log retrieval is unavailable.
   - If Prometheus is down: use service health + queue/dependency indicators and, when available, the profile's declared console/journal path; if no path is declared, log evidence is unavailable. Reduce chat feature pressure (throttle or temporary feature disable) if needed.
...
2. **Mitigate**
   - For cap- or abuse-driven `policy_rejection` identified by its bounded diagnostic reason or protected logs:
     ...
     - Treat a Telnet cap increase as an exposure-increasing owner mutation, not an ad-hoc deployment edit. A supported typed cap-increase family must satisfy the canonical [owner-control availability contract](./microservices/logging-admin-service/api-contracts.md#availability-classes-by-endpoint-family): mandatory authentication and exact scope, action-family schema and shared digest vectors, Account authorization-reference issuance and owner redemption, durable intent/audit, authoritative TCP Proxy owner state and fences, stable request identity and idempotency, and durable acknowledgement. That control family is target-only until implemented and proved; if the control surface or any required authorization, scope, owner-state, fence, or acknowledgement evidence is unavailable, including during degraded operation, fail closed and do not raise the caps.
...
4. **Degraded-mode branch (if observability backends are unavailable)**
   ...
   - If the selected indexed query path is unavailable, lacks current passed retained asynchronous emitter-to-query proof, or the profile omits indexed search: use the profile's declared current fallback (console/journal preserving `service`, `traceId`, and `correlationId`, or owner-permitted Logging & Admin/PostgreSQL/direct logs), adding conditional gameplay identity fields only when present, to classify canonical outcomes (`server_failure`, `user_rejection`, `policy_rejection`, or `unknown`) and their bounded diagnostic reasons; if none is declared, log retrieval is unavailable. Use `unknown` when the available evidence is missing, ambiguous, or insufficient to distinguish the other outcomes; do not infer an outage class from an absent or incomplete record.
   - If Prometheus is down: follow [Direct External-Monitor Retrieval When Prometheus Is Unavailable](#direct-external-monitor-retrieval-when-prometheus-is-unavailable) for `independent-required` profiles, then use edge health, pod events, and the profile's declared console/journal evidence as supplementary classification and action evidence. If no path is declared, log evidence is unavailable. Omitted or otherwise non-required profiles use those strongest available local signals with the same stale/unknown handling and retain degraded detection.
