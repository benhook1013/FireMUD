...
- **Named operations**
  ...
  - **Per-tenant reset** – execute one tenant-scoped maintenance workflow over the complete durable affected-region snapshot, not a tenant-wide shortcut. Before any rebuild, rebind, smoke check, or resume, `pause` must drive every affected region to authoritative `PAUSED`; `reset` must then bump and persist each region's `region_epoch` and fence every pre-reset executor, lease, lock, session, and observer for every region. Only after every region has recorded old/new epoch evidence and stale-state rejection may the workflow clear and rebuild coordination state, invoke Automation & Scripting cleanup/rebuild for its timer and scheduler prefixes, and apply the explicit session policy. Tenant resets always invalidate `session:auth:*`; preserve `session:game:*` / current `sessionctx:*` context only when the operator explicitly chooses that option for the reset, and rebind preserved sessions only after all-region fencing, reconciliation, and rebuild complete. `RunPostResetSmokeCheck(scope)` must then pass before command intake, traffic, or tick scheduling resumes for the tenant.
