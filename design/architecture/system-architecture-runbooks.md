...
- **Named operations**
  ...
  - **Per-region reset** – for a single `<tenantId, gameInstanceId, regionId>`, execute these separately fenced steps before normal operation resumes:
    1. Acquire the deployment maintenance lock and record its `maintenanceLockToken`.
    2. Pause the affected control-plane scope and wait for the authoritative `PAUSED` state.
    3. Pass that same `maintenanceLockToken` to the reset operation, which verifies lock ownership and then bumps the authoritative PostgreSQL `region_epoch`. Lock acquisition, reaching `PAUSED`, and the epoch bump are ordered coordination steps; they must not be represented as one atomic lock-plus-epoch action.
    4. Reject surviving executors, region-local leases, locks, bindings, and observers from the prior epoch as stale, then have Game Session clear only its documented region-scoped coordination keys: `tick:{tenantRegionTag}:meta`, `tick:{tenantRegionTag}:pending`, `tick:{tenantRegionTag}:queue:<entityId>`, `tick:{tenantRegionTag}:lock:<entityId>`, `timer:{tenantRegionTag}`, `retry:{tenantRegionTag}`, and `tick-executor-lease:{tenantRegionTag}`. For session state, clear only the affected `tick:{tenantRegionTag}:session-binding:<entityId>` session-to-region bridge keys; preserve the broader gameplay session records and session-context records according to the canonical reset policy, and do not use a region-only scan to delete `session:game:*`, `sessionctx:*`, or `session:auth:*`. The reset-sensitive `session:auth:*` state still requires auth/revocation validation and any required re-authentication for preserved sessions before rebind. Also drop the reset-tolerant observer hints `tick-events-lease:{tenantRegionTag}`, `tick-events:{tenantRegionTag}`, and all per-consumer `tick-events-offset:{tenantRegionTag}:<consumerId>` keys.
    5. Reconcile old-epoch tick effects and accepted-but-unbound commands, initialize the new-epoch Redis metadata baseline, then invoke an Automation & Scripting-scoped cleanup/rebuild for `automation:timer:{tenantRegionTag}` and `script-scheduler:{tenantRegionTag}:lastTickId`. Automation & Scripting owns those prefixes and rebuilds them from durable schedules, trigger-instance rows, and the active status/progress adapter. Rebind preserved sessions through the bridge only after the service-owned cleanup and rebuild prerequisites pass. Observers reacquire leases and re-establish baselines from the same active adapter and durable domain state, so loss or duplication affects discovery latency but not correctness.
    6. Require `RunPostResetSmokeCheck(scope)` to prove that a sample new-epoch tick can stage, converge, and clear while normal traffic remains paused. Only after that gate may the canonical resume operation reopen command intake, player traffic, and normal tick scheduling, return the scope to `RUNNING`, and release the maintenance lock.
  - **Per-tenant reset** – execute one tenant-scoped maintenance workflow over the complete durable affected-region snapshot, not a tenant-wide shortcut. Before any rebuild, rebind, smoke check, or resume, `pause` must drive every affected region to authoritative `PAUSED`; `reset` must then bump and persist each region's `region_epoch` and fence every pre-reset executor, region-local lease, lock, binding, and observer for every region. Only after every region has recorded old/new epoch evidence and stale-state rejection may the workflow clear and rebuild coordination state, invoke Automation & Scripting cleanup/rebuild for its timer and scheduler prefixes, and apply the explicit session policy. Tenant resets preserve Account-owned `session:auth:token:<tokenHash>` records and authority-generation projections regardless of `--preserve-sessions`; preserve `session:game:*` / current `sessionctx:*` context only when the operator explicitly chooses that option for the reset, and rebind preserved sessions only after all-region fencing, reconciliation, and rebuild complete. `RunPostResetSmokeCheck(scope)` must then pass before command intake, traffic, or tick scheduling resumes for the tenant.
...
- **How to choose**
  ...
  - If multiple regions for the same `<tenantId, gameInstanceId>` are polluted or broken in similar ways → run a **per-tenant reset** only when the affected-instance blast radius requires it; enumerate the affected instances and regions before widening the scope.
