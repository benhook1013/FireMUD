...
- The scheduler and recovery tooling determine recency only from the authoritative monotonic ownership generation assigned by the serialized ownership record. They compare the current-live opaque `executorFence` only for exact equality with that generation's live token; a mismatch fails closed and never supports an older/newer ordering claim.
...
- On a non-reset cold start where Coordination Redis is empty:
  - In the current-live deployment, the next winning executor bootstraps owner, selected region, authoritative ownership generation, opaque `executorFence`, and the committed tick baseline from the serialized runtime ownership record surfaced through `GetRuntimeOwnershipStatus`, then observes `ObserveRuntimeTickProgress` to confirm the active `(tenantId, gameInstanceId, regionId, regionEpoch, tickId)` progress before deriving the next requested tick.
