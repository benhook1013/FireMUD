...
1. **Durable batch creation**
   ...
   - Target-state requires exactly one durable tick batch for a given `(tenantId, gameInstanceId, regionId, regionEpoch, tickId)`:
     ...
     - Target-state batch allocation is an atomic CAS-fenced durable operation. The creating transaction reads the authoritative current epoch, the authoritative monotonic `ownership_generation` assigned by the serialized ownership-acquisition record, and the current opaque `executorFence`, conditionally inserts or adopts the coordinate row only while the epoch and generation remain current and the opaque fence still matches exactly, writes the manifest, ledger allocation, and source claims, and commits before Redis staging. Fence bytes are never parsed or ordered. A `RegionStatus.executorFence` value is target-state only unless the live ownership surface explicitly projects it.
     - In that target state, if the epoch or authoritative ownership generation changes, the opaque fence no longer equals the current token, the mapping generation changes, or a competing owner wins, the transaction must allocate no new batch and fail closed. Redis `pending` cannot be used to repair an allocation that failed this durable CAS.
     - The current-live schema and `GetRuntimeOwnershipStatus` surface do not persist or expose `ownership_generation`. Current staging carries `regionEpoch` plus an opaque `executorFence` and may compare that fence only for exact equality. It cannot order owners or transfer a batch by inferred recency; an ownership mismatch fails closed and requires reconciliation. The generation-backed allocation, transfer, and recency rules below remain target-state until the serialized ownership record, status projection, and tick-batch schema persist the same monotonic generation.
     - Target-state winner rule:
       - The target-state winner is the transaction that successfully creates the tuple-unique `(tenantId, gameInstanceId, regionId, regionEpoch, tickId)` row while holding the currently valid Redis lease, the authoritative current ownership generation, and the opaque `executorFence` that exactly matches that generation's live token.
       - If a competing executor finds that the existing row carries the authoritative current `ownership_generation` and an `executorFence` exactly equal to the current live token, it must reuse that row as authoritative state for replay/continuation. The same generation with unequal fence bytes is an inconsistent authority projection, not evidence that either fence is newer; the affected region pauses for reconciliation.
       - If the existing batch's authoritative `ownership_generation` precedes the current generation, the current owner must not continue that batch on the hot path. It pauses the affected region before a dedicated fence-guarded recovery path may transfer a wholly unstarted batch in place to the current generation and exact current fence, preserving its unique tick coordinate, manifest, effect identities, and command bindings.
       - If transfer preconditions fail or any effect might have started, reconciliation must not bulk-abandon the batch. It determines each effect from durable domain state and authoritative effect guards: an effect becomes `APPLIED` only when that evidence proves it is reflected, becomes `ABANDONED` only when that evidence proves it was not applied, and otherwise remains `SCHEDULED`. `reconciliation_required`, its reason, the observed ownership generations/fences, and the last proof attempt are recovery metadata outside the closed effect status enum; they are not another effect state.
       ...
       - If the existing batch's authoritative `ownership_generation` follows the caller's generation, the authoritative generation proves that the caller is stale. The caller must stop without modifying the batch, replaying effects, or requeueing source work. It must never infer this ordering from `executorFence`, create a second batch for the same unique coordinate, or requeue work into that same coordinate.
   ...
   - Target-state worked allocation-race example:
     ...
     - `E1` inserts a batch row while holding Redis lease token `L9001`, authoritative `ownership_generation=104`, and opaque `executorFence=fence-a9c`. This example assumes the required ownership-generation and tuple-uniqueness migrations; the current-live schema cannot make either claim.
     - `E2` then reads the existing row.
       - If the authoritative status still names `ownership_generation=104` and its live token exactly equals `fence-a9c`, `E2` may continue/replay from that row.
       - If `E2` holds the authoritative later acquisition `ownership_generation=105` with opaque `executorFence=fence-q2m`, the generation establishes recency; no comparison between `fence-a9c` and `fence-q2m` is valid. `E2` pauses the region and runs the fenced recovery path. A wholly unstarted transferable batch changes ownership to generation `105` and exact fence `fence-q2m` in place while preserving tick `42`'s row, manifest, ledger identities, and bound command state. Otherwise reconciliation atomically converges proven effect, command, and source-claim outcomes, retains unresolved work outside ordinary retry/requeue, and assigns conclusively independent retry work a later coordinate and new identities.
       - If `E2` is acting under `ownership_generation=104` while the authoritative status and existing row carry generation `105`, the generation proves `E2` is stale. It must stop without modifying the batch, replaying effects, or requeueing source work, regardless of the opaque fence strings.
   ...
   - Target-state canonical fence/write sequence (normative):
     ...
     2. Game Session atomically advances the authoritative monotonic ownership generation and installs a fresh opaque current-live runtime ownership `executorFence` for that generation; `RegionStatus` is only the target-state durable projection when the live ownership surface is not available.
     3. The same ownership generation creates `tick_batch` and `SCHEDULED` ledger rows carrying that `ownership_generation` and exact `executorFence`.
...
- Target-state scheduler and recovery tooling determine recency only from the authoritative monotonic ownership generation assigned by the serialized ownership record, and compare the opaque `executorFence` only for exact equality with that generation's live token.
- Current-live tooling has no persisted ownership generation. It compares the current `regionEpoch` and opaque `executorFence` for exact equality only; a mismatch fails closed and never supports an older/newer ordering claim.
