...
2. Orders fairly:
   ...
   - Orders selections within each lane using a deterministic ordering function and enforces per-entity count/cost caps plus a region-wide work budget:
   ...
   - Each work item type must declare its lane and expose the same canonical ordering tuple within that lane so the sort is reproducible across retries and failover:
