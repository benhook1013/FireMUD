...
1. **Durable batch creation**
   ...
   - The selected-work manifest is the authoritative record of which source items were chosen for the tick before Redis staging. At minimum, each selected item records:
     - the batch scope `(tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch)`; `entity_enqueue_seq` values are allocated monotonically per entity within that complete scope. A `regionEpoch` reset starts a new entity-local sequence domain; old-epoch ordering identities remain immutable and are never rebound.
