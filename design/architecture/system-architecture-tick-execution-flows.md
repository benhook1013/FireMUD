...
3. **Region-Local Mutations**
   - Before target tick staging or any tick-domain ledger or domain write, Game Session materializes the command's deterministic plan: the ordered set of logical root effects admitted for that command in the frozen typed command or `ResolvedEffectPlan` semantic order supplied by its owning command/action contract. Built-in commands declare an equivalent stable order; ambiguous or duplicate order fails before admission. Each logical operation receives one root `EffectId` and a stable zero-based `planOrdinal` (`0` for a single root, `0..n-1` for multiple roots); a zero-effect plan receives neither. The durable gameplay command row already freezes/binds its request fingerprint and resolved runtime scope. Game Session persists the ordered plan manifest/digest and owns an opaque root allocation row unique on `(tenantId, gameInstanceId, commandId, planOrdinal)`; the row binds the manifest digest and validates that frozen command/runtime binding in the local durable plan/staging transaction. Reuse requires the same command row identity, frozen command/runtime binding, ordinal, and manifest digest, otherwise failing closed. For Automation, the complete applicable Command-Handoff Identity exact-maps at admission to one durable target `commandId`, after which allocation uses that target command row's canonical key. It reuses the allocation across retry, replay, and reconciliation.
   - A conserved multi-participant operation normally shares one root with participant guards beneath it, but the command still uses ADR 0053's required/optional classification and co-location or reservation/escrow semantics. A root is not a global transaction or participant-set identity. Before target tick staging or any tick-domain ledger or domain write, Game Session requires a present, nonblank root `EffectId`; a missing or blank identity is a non-mutating rejection. No-ID bypasses are limited to explicitly non-tick/control-plane paths. The current live command/text fallback remains an implementation gap and is not target behavior.
...
Every phase must use its phase-specific durable identity so replays after failure do not double-apply logical work. Command ingress uses `(tenantId, gameInstanceId, commandId)`; source claims and selection use the durable source identity (such as a timer member ID, retry member ID, or follow-up row ID) together with its bound target/timeline; command-root tick effects retain their persisted plan `planOrdinal`, opaque root `EffectId` allocation binding, and participant guards; passive, inbound/remote, timer, retry, and already-generated effects retain their owner/source-specific root identities unless they materialize as an admitted command; and cross-region completion uses the coordinator identity plus the target ledger/result identity. Ordinary retry/replay preserves each identity, including any plan/root binding and original participant ledger/guard projection. Root allocation uses the durable command row's `(tenantId, gameInstanceId, commandId, planOrdinal)` key plus its frozen command/runtime binding and ordered plan manifest digest; it does not use random `tickBatchId`, mutable command text, `effectKey`, `sourceOrdinal`/`enqueueSeq`, participant tuples, Trigger Identity/`scriptEventId`, or generated child ordinals as substitutes. Retry eligibility/attempt metadata is separate and must not rewrite the original `regionEpoch`, `tickId`, `effectKey`, or root/typed-operation/target projection; a future linked-attempt or supersession model requires an explicit contract. A fresh root is reserved for a **post-abandon re-drive**, which requires a conclusively terminal `ABANDONED` effect and terminalized source claim, a later coordinate, a fresh root and participant projections under the applicable owner contract, and durable lineage to the original identities. A command or source item may cross tick boundaries without changing its ingress or source identity.
