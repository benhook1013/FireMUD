...
- the live gameplay-command staging path still uses its current `commandId`/deterministic text-and-slot fallback when a complete authored handoff identity is unavailable. The target path is fail-closed: Game Session assigns the stable root `EffectId` before acknowledgement when possible and rejects before acknowledgement when assignment fails. If assignment fails after an `ACCEPTED_VOLATILE` acknowledgement, `LOST_BEFORE_STAGING` is limited to an eligible unbound `RECEIVED`/`ENQUEUED` command with no surviving batch; batch-bound or inconclusive work remains non-terminal/reconciliation-required and cannot become evidence-free `ABANDONED`. The target wording below is not a claim that the fallback has already been removed from code.
...
2. Selects and orders the lanes fairly:
   ...
   - Passive or inbound work becoming due after start selection is deferred to a later tick without changing its persisted ordering inputs or stable identity.
