...
- the live gameplay-command staging path requires a nonblank durable `commandId` before Redis enqueue and durable batch staging; malformed pre-existing queue entries fail closed before staging and require owner reconciliation. The target path is fail-closed: Game Session assigns the stable root `EffectId` before acknowledgement and rejects before acknowledgement when assignment fails. Target tick execution also rejects any missing or blank root `EffectId` before staging, ledger, or domain writes; a no-ID bypass is permitted only for an explicitly non-tick/control-plane path. The following `ACCEPTED_VOLATILE` clause is exceptional recovery for an acknowledgement that was already issued; it never authorizes late `EffectId` assignment. If assignment failure is discovered after that acknowledgement, `LOST_BEFORE_STAGING` is limited to an eligible unbound `RECEIVED`/`ENQUEUED` command with no surviving batch; batch-bound or inconclusive work remains non-terminal/reconciliation-required and cannot become evidence-free `ABANDONED`.
