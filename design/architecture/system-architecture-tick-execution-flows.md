   ...
   - For cross-region commands, the origin region:
     ...
     - If the command needs to wait for remote completion, the origin region creates or updates a separate durable cross-region coordinator record. This waiting state is **not** kept as a non-terminal row inside the origin tick batch.
       ...
       - Target completion is returned through one durable origin-owned result path, not an unspecified transient message:
         ...
         - origin-side processing claims and applies that result idempotently when advancing the coordinator lifecycle. Acceptance of a result that has a dependent child effect is crash-safe: the durable acceptance transition cannot commit without a deterministic child-effect intent. The result acceptance and child intent commit in one durable transaction where they share an owner, or a resumable durable acceptance intent records both before either is treated as accepted. Replays reuse the child identity and the exact accepted-result digest; missing, extra, partial, or conflicting intent/projection evidence fails closed. This is an acceptance/intent durability invariant, not another coordinator outcome. Exact opaque allocation remains non-authoritative in [ADR 0182](./decisions/adr-0182-deterministic-effect-id-allocation-and-replay-binding.md).
...
4. **Heal Leg (origin region)**
   - Only after existing coordinator reconciliation accepts the target result does region A record the actor-generated “apply lifesteal heal” effect for the caster. A rejected or abandoned result creates no heal effect; an accepted heal has a deterministic, stable idempotent child `EffectId` derived from the accepted result and is not a new root actor command. Acceptance replay reuses that child identity and the exact accepted-result digest; it cannot create a second heal intent or silently accept a conflicting result. The pending [ADR 0182 proposal](./decisions/adr-0182-deterministic-effect-id-allocation-and-replay-binding.md) is non-authoritative.
