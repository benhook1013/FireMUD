...
- the live batch/effect substrate exists with the current gameplay-command selected-work manifest on `tick_batch`, but timer/retry/remote-follow-up source-claim manifests, the target-state empty-cadence watermark path, cross-region result-return plumbing, and some richer command-status fields are still target-state follow-through. The current-live `ScheduleRequest`, `RemoteCommandCoordinator`, `RemoteFollowup`, and `RemoteFollowupResult` models/migrations omit `targetPlayableStateNamespaceId`, so the complete target-scope and request-digest equality contract below is not live or proven. The current-live runtime has no dedicated target-state empty-cadence status transition or `StreamTickHeartbeats` path.
...
3. **Region-Local Mutations**
   - Before target tick staging or any tick-domain ledger or domain write, Game Session materializes and persists the command plan and root allocation under the canonical [Transaction Strategies](./system-architecture-transactions.md#command-atomicity-and-outcome-aggregation) contract. The execution flow consumes that frozen plan in its supplied semantic order and reuses the stored binding across retry, replay, and reconciliation; it does not infer or reallocate roots from tick selection.
   - A conserved multi-participant operation normally shares one root with participant guards beneath it, but the command still uses ADR 0053's required/optional classification and co-location or reservation/escrow semantics. A root is not a global transaction or participant-set identity. For each root in a non-empty materialized plan, Game Session requires a present, nonblank root `EffectId` before target tick staging or any tick-domain ledger or domain write; a missing or blank identity is a non-mutating rejection. A zero-effect plan allocates no root, is not rejected for lacking one, and stages or writes no tick-domain state. No-ID bypasses are limited to explicitly non-tick/control-plane paths. The current live command/text fallback remains an implementation gap and is not target behavior.
   ...
   - Domain services apply changes under local transactions and the persisted-mutation-`EffectId` participant-guard contract owned by Transaction Strategies: a root mutation uses its root `EffectId`, while a generated child uses its persisted child `EffectId`; any enclosing `rootEffectId` is retained separately as lineage and reconciliation context.
4. **Cross-Region Effects (if any)**
   - For cross-region commands, the origin region:
     ...
     - If the command needs to wait for remote completion, the origin region creates or updates a separate durable cross-region coordinator record. This waiting state is **not** kept as a non-terminal row inside the origin tick batch.
       - The target `ScheduleRequest`, coordinator, follow-up, and origin-addressed result projections share the complete target scope `(targetGameInstanceId, targetPlayableStateNamespaceId, targetPlayableStateScope, targetRegionId, targetRegionEpoch, dueTickId)` and the immutable request digest covering that scope, exact target identity, operation, and sealed manifest binding. Each projection preserves these fields for exact replay/result equality checks; a missing, conflicting, or mismatched namespace, scope, target, or digest fails closed.
       - Minimum coordinator fields include durable `coordinatorId`, `tenantId`, `originCommandId`, `originGameInstanceId`, `originRegionId`, `originRegionEpoch`, `originTickId`, target scope (`targetGameInstanceId`, `targetPlayableStateNamespaceId`, `targetPlayableStateScope`, `targetRegionId`, `targetRegionEpoch`, `dueTickId`), exact target aggregate (`targetAggregateType`, `targetAggregateId`), the originating `originEffectId` and distinct target-leg `targetEffectId`, target-leg `typedOperation`, immutable `requestDigest`, and the sealed required-participant manifest (or immutable manifest reference plus digest). These identity/binding fields are compared on replay and a missing or conflicting value fails closed. The lifecycle is (`PENDING_REMOTE`, `REMOTE_APPLIED`, `REMOTE_ABANDONED`, `REMOTE_TIMEOUT_ABANDONED`, `LATE_RESULT_IGNORED`, `LATE_RESULT_RECONCILED`), with origin-timeline deadline (`originDeadlineRegionEpoch`, `originDeadlineTickId`; storage may use names such as `remote_deadline_region_epoch` / `remote_deadline_tick_id`), final player-facing/result-facing projection fields when applicable (`executionOutcome`, `gameplayResult`), and `updatedAt`.
       - Target completion is returned through one durable origin-owned result path, not an unspecified transient message:
         - the target leg writes an origin-addressed result or inbox row keyed to `coordinatorId` and repeats `originCommandId`, `originGameInstanceId`, origin region/epoch/tick, target game instance, `targetPlayableStateNamespaceId`, `targetPlayableStateScope`, target region/epoch and `dueTickId`, the distinct target `targetEffectId`, typed operation, exact target aggregate, and one immutable follow-up binding/digest reference that binds the request digest and sealed-manifest context,
         ...
         - origin-side processing claims and applies that result idempotently when advancing the coordinator lifecycle. Acceptance of a result that has a dependent child effect is crash-safe: the durable acceptance transition cannot commit without a deterministic child-effect intent. The result acceptance and child intent commit in one durable transaction where they share an owner, or a resumable durable acceptance intent records both before either is treated as accepted. Replays reuse the child identity and the exact accepted-result digest; missing, extra, partial, or conflicting intent/projection evidence fails closed. This is an acceptance/intent durability invariant, not another coordinator outcome. Exact opaque allocation remains non-authoritative in [ADR 0183](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md).
...
Every phase must use its phase-specific durable identity so replays after failure do not double-apply logical work. Command ingress uses `(tenantId, gameInstanceId, commandId)`; source claims use their durable owner identity and bound target/timeline; command-root effects use the persisted plan/root binding and participant guards owned by Transaction Strategies; inbound/remote, timer, retry, and generated work retain their owner/source roots unless materialized as admitted commands; and cross-region completion uses the coordinator plus target ledger/result identity. Ordinary retry/replay preserves those identities. Retry metadata never rewrites the original timeline or root/operation/target projection. A post-abandon command re-drive uses a new linked `commandId`; reusing the old id returns its terminal result. A command or source item may cross tick boundaries without changing its ingress or source identity.
...
- For any path that must assign a stable root `EffectId` at admission, the normal target contract assigns it before acknowledgement; assignment failure before acknowledgement is rejected. The `ACCEPTED_VOLATILE` rule that follows is exceptional recovery for an acknowledgement that was already issued and never authorizes late `EffectId` assignment: such a command may use `LOST_BEFORE_STAGING` only while it remains `RECEIVED`/`ENQUEUED` with no surviving `tickBatchId`; batch-bound or inconclusive work remains non-terminal/reconciliation-required.
...
2. Selects and orders the lanes fairly:
   ...
   - Within one entity's eligible queue, orders work by the persisted tuple `(priority, due_tick_id, entity_enqueue_seq, source_kind, commandId_or_effectKey)`. `entity_enqueue_seq` is monotonic within the entity's current `(tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch)` scope and assigned by the owning Game Session admission/scheduling path. Same-tick derived children use the persisted parent order plus deterministic child ordinals rather than completion timing.
...
3. Atomically binds selected work:
   - While the candidate locks are held, Game Session atomically creates the durable PostgreSQL `tick_batch`, its selected-work manifest, `SCHEDULED` ledger rows, and source claims in one transaction for the complete `(tenantId, gameInstanceId, regionId, regionEpoch, tickId)` runtime coordinate. The selected-work manifest exact-binds the active `playableStateNamespaceId` and separately persisted `playableStateScope` as immutable execution evidence; missing, stale, mixed, or mismatched namespace/scope evidence fails before batch admission rather than creating a second scheduler partition. Neither namespace nor scope changes tick-batch uniqueness or scheduler partitioning. Any selected command also moves to `BOUND_TO_BATCH` in that transaction.
...
- `entity_enqueue_seq`:
  - Strictly increasing across all candidate work sources for one entity within its current `<tenantId, gameInstanceId, playableStateNamespaceId, regionId, regionEpoch>` scope. The owning Game Session admission/scheduling path assigns and persists it; it is not a region-global, tenant-only, game-instance-only, or cross-region counter.
...
1. **Durable batch creation**
   ...
   - Target-state requires exactly one durable tick batch for a given `(tenantId, gameInstanceId, regionId, regionEpoch, tickId)`:
     ...
     - Target-state winner rule:
       ...
       - If the current owner encounters prior-fence work, it must not continue that batch on the hot path or transfer it based on inferred fence recency. It pauses the affected region and uses the fence-guarded recovery path while preserving the unique tick coordinate, manifest, effect identities, and command bindings where proof permits; inconclusive old-epoch work remains non-terminal reconciliation work under its original persisted mutation `EffectId`, with any enclosing `rootEffectId` retained separately as lineage and reconciliation context.
       ...
       - Independently retryable work becomes eligible for a **post-abandon re-drive** only after the original effect is conclusively `ABANDONED` and its command/source claim is terminalized in that same transaction. The recovery controller must then eventually assign that work a later `(regionEpoch, tickId)` coordinate and new `tick_batch_id`, a fresh root identity under the applicable owner contract with its ledger/participant-guard projections, and a new retry/source-claim identity before the region resumes; if it is re-admitted as a command, Game Session allocates a fresh command plan/root binding under the same contract. It records lineage to the original identities and never reuses or requeues into the old coordinate. Ordinary retry/replay keeps the original root `EffectId`. Until that fresh-root allocation succeeds, the work remains durable recovery work and cannot return to an ordinary queue.
     ...
     - an immutable collision-safe pending envelope for each staged member carrying the exact `tick_batch_id` plus the complete mutation `effect_id`/participant projection (with `root_effect_id` retained for lineage), or an equivalent exact ledger-row identity; `effect_key` remains descriptor metadata only and is not sufficient correlation or replay identity by itself
   - The selected-work manifest is the authoritative record of which source items were chosen for the tick before Redis staging. At minimum, each selected item records:
     - the batch runtime scope `(tenantId, gameInstanceId, regionId, regionEpoch)` plus exact bound `playableStateNamespaceId` and separately persisted `playableStateScope` execution evidence; both values are immutable and exact-compared on replay/readback. Missing, stale, mixed, or mismatched namespace/scope evidence fails closed. `entity_enqueue_seq` values are allocated monotonically per entity within that complete runtime-and-namespace context. Namespace and scope evidence do not extend tick-batch uniqueness or the scheduler timeline. A `regionEpoch` reset starts a new entity-local sequence domain; old-epoch ordering identities remain immutable and are never rebound.
...
3. **Domain application**
   - Domain services process staged effects under participant guards keyed by the persisted mutation `EffectId` (root ID for a root mutation, child ID for a generated child), with `rootEffectId` retained separately as lineage, and update authoritative PostgreSQL state in their own databases.
   - Game Session records the outcome of each effect in its tick effect ledger (`SCHEDULED` → `APPLIED` or `ABANDONED`) only when the durable domain evidence and existing recovery policy permit that transition; inconclusive old-epoch work remains non-terminal reconciliation work under its original mutation `EffectId`, with enclosing `rootEffectId` retained as lineage and reconciliation context. Domain services do not write to the Game Session ledger directly.
4. **Commit visibility**
   - For a non-empty batch, once all ledger rows that belong to that tick batch are terminal (`APPLIED` or `ABANDONED`) under the existing evidence policy, one fenced durable commit boundary transitions `tick_batch.status` to `COMMITTED` and advances the commit authority `RegionStatus.lastCommittedTickId` for the complete current scope under the same `executorFence`; only then may the target-state tick heartbeat be emitted. An inconclusive old-epoch row remains non-terminal and blocks this advancement. A truly empty boundary uses the separate fenced one-tick status transition and post-commit heartbeat branch above, with no batch ledger. The current-live deployment instead advances the committed tick fields on `RuntimeOwnershipStatus` under the current-live opaque fence; `ObserveRuntimeTickProgress` reports that progress, and neither `RegionStatus` nor `StreamTickHeartbeats` is implied to be live.
...
5. **Coordination cleanup**
   - A final commit/cleanup script clears the `pending` entry for the tick and releases any entity locks for that region only after exact equality between the pending envelope set and the durable batch/ledger expected set has been established. `expected_effect_count` and any digest are diagnostic hints and cannot authorize cleanup on their own.
...
- Recovery treats PostgreSQL as the source of truth for staging intent:
  - `tick_batch` exists, Redis `pending` missing:
    - Recovery replays directly from the durable batch manifest and `SCHEDULED` ledger rows to drive effects to terminal `APPLIED` or `ABANDONED` outcomes only when durable evidence permits; inconclusive old-epoch work remains non-terminal reconciliation work under its original mutation `EffectId`, with enclosing `rootEffectId` retained as lineage and reconciliation context. Missing Redis state is not treated as “no work existed”.
...
The **TickScheduler** in Game Session enforces a **single in-flight tick per runtime region** invariant and derives tick positions from durable state:
...
- The scheduler does not start a new tick for that `<tenantId, gameInstanceId, regionId>` until the previous tick is `coordination_cleared` as part of normal cleanup or explicitly handled during crash recovery. Every admitted work item must exact-match that active instance's bound `playableStateNamespaceId`; namespace is sealed execution evidence, not a second tick timeline or scheduler partition.
...
- Each cross-region mutation leg has its own persisted mutation `EffectId`: `originEffectId` for the origin leg and `targetEffectId` for the target leg; an enclosing command/root `EffectId`, when present, remains lineage and reconciliation context rather than collapsing the legs into one participant identity. The origin/target ledger and follow-up storage projections retain the specific scope/timeline on that region plus the applicable leg identity, typed operation, immutable request digest, sealed manifest binding, exact target `playableStateNamespaceId`, separately persisted and exact-validated target `playableStateScope` fence evidence, and exact target aggregate. The target leg is inbound/remote follow-up work: its deterministic `targetEffectId` is bound to the durable coordinator/follow-up identity under the remote-flow owner contract and is not a command-plan `planOrdinal` allocation unless the target explicitly materializes a new admitted command. `effectKey` remains descriptor/correlation metadata only, never uniqueness or guard authority. The payload also carries exact target identity plus required ownership, location, and aggregate-version preconditions; a matching target epoch alone is insufficient, and any identity, scope, operation, digest, manifest, or target conflict fails closed. The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is not current target state; its proposed child-manifest schema/serializer/digest binding is not an accepted requirement.
...
- When a region/tenant/cluster reset bumps `regionEpoch` for a region, any surviving `SCHEDULED` ledger rows or follow-ups from the old epoch are reconciled under the existing evidence policy: only conclusively unapplied work may converge to `ABANDONED`, while inconclusive work remains non-terminal under its original mutation `EffectId`, with any enclosing root retained as lineage; none is silently retried on the new epoch.
...
Some effects (for example, explosions, chained spells, scripted traps) spawn follow-up actions. Each root chain is bounded by the shared bootstrap depth ceiling (`8`), deterministic total count and cost budgets, and a deterministic per-target cap. A generated child records immutable parent/root identity, `depth = parent.depth + 1`, and an owner-defined deterministic child ordinal allocated and persisted by the owning contract for replay-stable reuse. Script-generated command children additionally retain the applicable `automationDispatchId` and `commandOrdinal`; the generic generated-child ordinal replaces neither script-handoff field, and exact retries reuse the same complete binding. When the chain is under an admitted command, it remains beneath that command's root; otherwise it retains its owner/source-specific root identity and does not synthesize a command plan. Admission suppresses only the child that would exceed a limit; committed parents and earlier children remain authoritative. Required/optional authored classification derives truthful `SUCCESS`, `PARTIAL`, or `FAILED` outcomes, and every suppression records durable evidence with bounded metric labels. Generated effects remain passive/effect work and cannot grant another root actor action in the same tick. See [ADR 0075](./decisions/adr-0075-depth-cost-and-count-bounds-for-generated-effect-chains.md). The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is non-authoritative.
...
3. **Damage Leg (target region)**
   ...
   - In the next tick for `<tenantId, gameInstanceId, regionB>`, the target region’s executor:
     ...
     - Acquires the target’s lock (`tick:{tenantRegionTag}:lock:<entityId>`) and applies damage via Entity Management under the target leg’s persisted mutation `EffectId` and its operation/target guard; any enclosing root remains lineage and reconciliation context.
...
4. **Heal Leg (origin region)**
   - Only after existing coordinator reconciliation accepts the target result does region A record the actor-generated “apply lifesteal heal” effect for the caster. A rejected or abandoned result creates no heal effect; an accepted heal has a deterministic, stable idempotent child `EffectId` derived from the accepted result and is not a new root actor command. Acceptance replay reuses that child identity and the exact accepted-result digest; it cannot create a second heal intent or silently accept a conflicting result. The pending [ADR 0183 proposal](./decisions/adr-0183-deterministic-effect-id-allocation-and-replay-binding.md) is non-authoritative.
...
- Each leg is idempotent through its persisted mutation `EffectId` and operation/target guard in the domain service; the enclosing root remains lineage and reconciliation context. The heal child is idempotent through the accepted-result digest and child identity; its acceptance/replay rule is a local durability consequence, not a new outcome state.
