...
| Gateway replay admission | `gateway:connect-token:{gatewayReplayDomain}:readiness`, `gateway:connect-token:{gatewayReplayDomain}:jti:<jti>`, and `gateway:connect-token:{gatewayReplayDomain}:deny:jti:<jti>` | The `gateway-connect-token-replay-consume/v1` descriptor is the only three-key descriptor in this category: it runs only on Coordination Redis and receives readiness/fence, replay, and deny keys in that order. The literal `{gatewayReplayDomain}` hash tag is required on all three keys so Redis Cluster executes the atomic operation in one slot; the readiness value carries `replayAdmissionFence`. The separate `gateway-connect-token-browser-revoke/v1` descriptor is a one-key deny-marker mutation using only `gateway:connect-token:{gatewayReplayDomain}:deny:jti:<jti>` with the same literal tag; it is not part of this three-key key order. The current generic Cache-bound writer's only untagged output is the replay marker; it has no deny-marker, readiness, or replay-fence writer. That old replay key is migration-only drift and is not a valid descriptor shape. |
...
- Reset and coordination-loss metadata:
  ...
  - Shard-locality metadata for multi-key scripts, including whether all `KEYS` must share the same `{tenantRegionTag}`, `{tenantInstanceTag}`, `{tenantGameplayTag}`, or fixed `{gatewayReplayDomain}` hash tag and slot.
...
- For scripts that declare shard-local multi-key behavior, CI verifies that:
  - All declared `KEYS` share the same hash tag (for example `{tenantRegionTag}` or the fixed `{gatewayReplayDomain}`), and
