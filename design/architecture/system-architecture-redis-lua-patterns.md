...
| Gateway replay admission | `gateway:connect-token:{gateway-connect-token-replay-v1}:readiness`, `gateway:connect-token:{gateway-connect-token-replay-v1}:jti:<jti>`, and `gateway:connect-token:{gateway-connect-token-replay-v1}:deny:jti:<jti>` | The `gateway-connect-token-replay-consume/v1` descriptor is the only three-key descriptor in this category: it runs only on Coordination Redis and receives readiness/fence, replay, and deny keys in that order. The literal `{gateway-connect-token-replay-v1}` hash tag is required on all three keys so Redis Cluster executes the atomic operation in one slot; the readiness value carries `replayAdmissionFence`. The separate `gateway-connect-token-browser-revoke/v1` descriptor is a one-key deny-marker mutation using only `gateway:connect-token:{gateway-connect-token-replay-v1}:deny:jti:<jti>` with the same literal tag; it is not part of this three-key key order. The current generic Cache-bound writer's only untagged output is the replay marker; it has no deny-marker, readiness, or replay-fence writer. That old replay key is migration-only drift and is not a valid descriptor shape. |
...
| Automation helpers (coordination role only, target-only once implemented) | `script-scheduler:{tenantRegionTag}:lastTickId` and similar | This is a reserved target pattern only: no current Automation-owned coordination helper or key is implemented. Any future helper must not touch Cache/Rate-Limit prefixes. |
...
Automation queue-pointer enqueue and rebuild are a separate Cache/Rate-Limit descriptor boundary, not additions to the Coordination categories above. An atomic operation over `automation:queue:{tenantInstanceTag}:*` must declare `redis_role=cache`, use only queue keys for one exact `{tenantInstanceTag}` scope, and be authorized by `cache_app` for normal projection or by the target-only `automation_queue_rebuild_maint` ACL principal during an externally fenced reset/rebuild. The descriptor must carry the operation/fence, deployment generation, and exact prefix/scope admission fields described in the [Cache/Rate-Limit reset runbook](./system-architecture-redis-operations.md#external-cache-deployment-maintenance-gate); it must reject Coordination keys and `coord_app`, and its outcome must remain a best-effort pointer projection rather than durable-work authority. If the atomic enqueue/rebuild implementation uses single-key Redis commands instead of Lua, the same `redis_role`, key, slot, and ACL boundary still applies. These descriptors must be represented separately from the Coordination script categories in the ADR 0176 registry.

The Gateway replay-consume descriptor is `gateway-connect-token-replay-consume/v1`: `redis_role=coordination`, `KEYS[1]=gateway:connect-token:{gateway-connect-token-replay-v1}:readiness`, `KEYS[2]=gateway:connect-token:{gateway-connect-token-replay-v1}:jti:<jti>`, and `KEYS[3]=gateway:connect-token:{gateway-connect-token-replay-v1}:deny:jti:<jti>`. The descriptor must require the literal `{gateway-connect-token-replay-v1}` tag on every key, reject the untagged current-drift replay marker, and declare the stable outcomes `1` (marker created), `0` (already replayed), and `-1` (browser denied), with no mutation for `0` or `-1`. Focused contract tests must prove the three keys resolve to one Redis Cluster slot, reject any missing or different tag, preserve readiness/fence and deny-marker race ordering, and keep the same `KEYS` order under RESP2 and RESP3. Migration tests separately inventory/read back the current untagged replay prefix under quarantine and prove that it cannot be accepted by the target descriptor.

Browser revocation has a separate descriptor, `gateway-connect-token-browser-revoke/v1`, because its deny-marker mutation and durability acknowledgement are not the three-key replay-consume script contract. It uses `redis_role=coordination`, `KEYS[1]=gateway:connect-token:{gateway-connect-token-replay-v1}:deny:jti:<jti>`, and the token's absolute expiry (including the configured clock-skew allowance) as the bounded marker TTL. The idempotent write returns `1` when it creates the marker and `0` when the marker already exists; both outcomes identify the committed deny state, while malformed or conflicting state is non-mutating and fails closed. Gateway must immediately issue `WAITAOF requiredLocalAofCount requiredReplicaAofCount firemud.gateway.replayConsumeAckTimeoutMs` on the same pinned connection, with the same exact two-integer threshold validation as replay consumption; only that acknowledgement yields `DURABLE_BROWSER_REVOKE_ACK`. This descriptor must require the literal `{gateway-connect-token-replay-v1}` tag and must not be folded into, or represented as, the replay-consume descriptor's three-key invocation. The current implementation provides neither this deny-marker write nor its `WAITAOF` proof: it is replay-only, untagged, and Cache-bound migration drift.
...
- **Pattern 3 – Immutable pending-envelope identity and replay (target state; no collision-prone staging)**
- Each target staged member in `pending` carries an immutable envelope whose logical identity is the exact `tick_batch_id` plus the collision-safe participant/target projection: `(tenant_id, game_instance_id, playableStateNamespaceId, regionId, regionEpoch, tickId, mutation EffectId (effect_id), typed_operation, target_aggregate_type, target_aggregate_id)`. `playableStateScope` remains immutable envelope and exact-comparison/readback evidence, but is not a pending-member uniqueness dimension. The envelope also carries the exact immutable request payload/digest needed to resolve one durable ledger row. The enclosing `root_effect_id` is retained as immutable lineage/conflict evidence but is excluded from member uniqueness, so it cannot collapse sibling mutations. Envelope construction must reject a missing, malformed, or ambiguous typed operation or exact target; `effect_key` remains descriptor metadata only and is never the pending member identity or replay authority. Physical encoding may use a hash, set/ZSET member, or another registered representation, but it must preserve this complete logical envelope unchanged. Current Game Session `gamesession:tick:pending:*` entries instead use the narrower command envelope documented by [Tick Execution Flows](./system-architecture-tick-execution-flows.md#implementation-notes); complete Pattern 3 producer, consumer, replay, and focused proof remain an implementation gap.
- Before adding or replaying a staged member, the script compares the complete envelope and bound request payload, including the mutation `effect_id`, typed operation, participant/target projection, and immutable request digest; `root_effect_id` remains lineage/conflict evidence and `effect_key` remains descriptor metadata. An exact full-envelope/request-payload match returns the prior stored staging/replay outcome without creating another member or changing its payload or TTL. Distinct envelopes may share an `effect_key` when their complete identities differ; an exact identity with conflicting immutable envelope or request-payload evidence, including a differing bound `root_effect_id`, is a conflict/fail-closed result and must not overwrite or merge the existing member. Replay uses the mutation `effect_id`/participant identity, never `effect_key` or the enclosing root alone.
...
- **Compatibility levels**
  - `compatible` – purely additive changes (for example, new return fields or additional observability) that preserve the accepted-input set, outcomes, fencing, idempotency, and mutation semantics. Any validation that can reject a previously valid caller or payload requires the versioned/reset path. These may be rolled out without coordination resets as long as callers tolerate older results.
...
- Reset and coordination-loss metadata:
  ...
  - Shard-locality metadata for multi-key scripts, including whether all `KEYS` must share the same `{tenantRegionTag}`, `{tenantInstanceTag}`, `{tenantGameplayTag}`, or fixed `{gateway-connect-token-replay-v1}` hash tag and slot.
...
- For scripts that declare shard-local multi-key behavior, CI verifies that:
  - All declared `KEYS` share the same hash tag (for example `{tenantRegionTag}` or the fixed `{gateway-connect-token-replay-v1}`), and
