...
- **`tickId` guard via meta** – when touching `tick:{tenantRegionTag}:pending` or other tick-scoped structures, use the canonical metadata key `tick:{tenantRegionTag}:meta` as the monotonic guard: read `current_tick_id` from that hash and verify it is ≤ the requested `tickId` before staging new work. Commit/rollback scripts abort if the guard indicates the requested `tickId` is out of order so only the intended tick makes progress. Any `tickId` fields embedded inside `pending` payloads are informational only and must not be used as guards.
...
- **Pattern 2 – Compare-and-set on `region_epoch` + `tickId` (monotonic guards)**
  - Scripts that touch `tick:{tenantRegionTag}:pending` treat both `region_epoch` and `tickId` as monotonic guards:
    ...
    - The script reads the current epoch/tick metadata from the canonical metadata key:
      ...
      - The script loads `region_epoch` (and, when needed, `current_tick_id`) from that hash.
    - Behavior:
      ...
      - If the epoch matches and there is an existing `current_tick_id` that is greater than the requested `tickId`, the script returns a replay/out-of-date result and does not modify state.
      - If the epoch matches and `current_tick_id` is equal to the requested `tickId`, the script proceeds but treats existing effect entries as already staged (see Pattern 3).
      - If the epoch matches and there is no `current_tick_id` or it is less than the requested `tickId`, the script updates `current_tick_id` in `tick:{tenantRegionTag}:meta` and stages new effects.
