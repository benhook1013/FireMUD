...
| Session CAS / bindings | `session:game:{tenantGameplayTag}:<gameInstanceId>:<sessionId>` plus `session:game:index:*:{tenantGameplayTag}:*`, and `session:auth:token:<tokenHash>` | Gameplay session CAS/update scripts may be shard-local multi-key scripts where all gameplay-session keys share `{tenantGameplayTag}`. Issued-token registry projection operations remain single-key. Durable issuer/account/tenant/membership authority generations advance transactionally in Account Service; any Redis projection is read-through state, not a Lua-owned authority mutation. Session scripts are never mixed with tick keys in the same invocation. |
