...
| Automation helpers (coordination role only, target-only once implemented) | `script-scheduler:{tenantRegionTag}:lastTickId` and similar | Future shard-local scripts may operate on per-region scheduler metadata; no current Automation helper or key is implemented, and any future helper must not touch Cache/Rate-Limit prefixes. |
