...
- `serviceDigests` – map of service name to an immutable OCI manifest or index reference (`image@sha256:...`). A direct single-architecture service records its manifest reference; an index-backed service records its OCI index reference, including when that index currently admits only one platform.
- `servicePlatformDigests` – map of service name to a map of canonical lowercase OCI platform keys (`os/architecture` or `os/architecture/variant`, for example `linux/amd64` or `linux/arm64/v8`) to exact child-manifest references (`image@sha256:...`). A multi-architecture or single-platform index-backed service's map keys must exactly equal the production-admitted platform set, with no missing or extra entries, and each index descriptor must prove the recorded platform-to-child binding; the child digest is descriptor-selected and must never repeat the index digest. A direct single-architecture manifest has exactly one admitted-platform entry whose child digest equals the `serviceDigests` manifest digest.
...
- `approvedBy` – identity of the human who authorized this exact immutable production plan. An automation identity may generate, record, transport, or execute that human-approved plan and its evidence, but it cannot independently populate or satisfy `approvedBy`; a change ticket or workflow identity is supplemental and not approval.
...
- `serviceDigests` must identify the exact staged application-service artifact lineage being promoted. This map is not a complete inventory of rendered workload images.
- The exact production plan and promotion gate must also bind every image-bearing container in the rendered staging/production workload set, including supporting images such as PostgreSQL, Velero, and Fluent Bit. Every such image must have immutable digest lineage unless a later accepted owner-bound exemption exists; matching `serviceDigests` alone does not prove that coverage. The current preflight image check enumerates only selected application images, so this complete non-service image lineage remains an implementation/proof gap.
...
- Every production overlay application-service digest must match the digest in `serviceDigests`, and its resolved platform-child map must match `servicePlatformDigests` exactly. This application-service comparison does not by itself cover supporting workload images; the complete rendered workload image set remains subject to the immutable-lineage rule above.
