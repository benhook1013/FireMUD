...
| `skipped_reloading` | `ADMISSION` | Explicit reload backpressure. For a low-rate event denied before handler resolution, the caller may retry the same event-scope ingress identity, including its event-scope `scriptEventId`, if policy allows; after handler resolution, a handler retry reuses that handler's full Trigger Identity and the same propagated `scriptEventId`. |
| `rollback_paused` | `ADMISSION` | Post-resolution handler rollback backpressure while control-plane rollback pause is active; a handler retry reuses its full Trigger Identity and the same propagated `scriptEventId`, if policy allows. For low-rate event ingress denied before handler resolution, retry the same event-scope ingress identity instead. Pre-handler pause uses the event-scope ingress pair `TRIGGER_ADMISSION_OUTCOME_BACKPRESSURE_ROLLBACK` / `rollback_pause`; timer candidates use the Table 3 candidate-audit outcome. |
