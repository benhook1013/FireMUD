...
The source scope fields are required for gameplay/runtime commands; the target scope fields are included exactly when the emitted command is routed to a distinct target runtime scope, and `targetPlayableStateScope` is included whenever that distinct target scope has its own shared/isolated playable-state namespace. Absent optional fields remain absent. `commandOrdinal` is zero-based: the first emitted command under a dispatch is `0`, followed by `1`, and so on. `automationDispatchId` is a dispatch-group identifier and is not assumed to be globally unique. `outboxWorkItemId` is parent correlation only. The complete identity above is the canonical identity for command-child handoff deduplication, execution-fence reporting, and effect idempotency; the `(automationDispatchId, commandOrdinal)` pair is only a suffix and is insufficient without the complete source/target scope. The parent Trigger Identity, including `scriptEventId`, is retained on each child for correlation and is not part of the command-child uniqueness key; `script_event_audit` remains a separate one-row-per-handler surface.
...
| Recurring/advisory normal operation | Evaluate once per due candidate under budgets and quotas; only an admitted candidate becomes a firing. | An admitted firing receives one `script_event_audit` row per handler-scoped Trigger Identity with stage-aware outcomes. |
| Recurring/advisory quota/budget denied | Skip the candidate; do not create a firing claim or replay it later. | Record an event-scope candidate audit keyed by deterministic `scheduleCandidateId`, with `finalStage=ADMISSION` and an explicit deny outcome/reason. |
...
| Recurring/durable-recurring leader failover or short downtime | Apply each complete stable schedule-instance identity's declared `SKIP_MISSED` or `COALESCE_ONE` policy. A coalescing schedule instance may admit at most one synthetic firing in the durable resume window; one deterministic fair `SCRIPT_TIMER_CATCH_UP_MAX_FIRINGS_PER_RESUME` cap applies across independent schedule instances, grouped by the complete identity defined in the scheduler lifecycle (including target scope and plugin-binding dimensions), not by `scheduleDefinitionId` alone. Excluded candidates are dropped and never deferred as backlog. | Catch-up firings use `triggerMode=CATCH_UP` and deterministic identity from the coalesced due point and resume window. Skipped or cap-excluded candidates emit operator-visible audit, metric, and bounded reason evidence. |
...
| Recurring/advisory timer preserved across reload/rollback | Apply the canonical resume formula only when the preserved timer's `runtimeRegionId`, runtime scope (`gameInstanceId` and `playableStateScope`), and `regionEpoch` match the resumed runtime. If any differs, fence the old due point and derive a new due point on the new timeline after the replacement/new-scope schedule entry is durable; do not reuse the stale `previousDueTickId`. For an unchanged runtime region, scope, and epoch, recalculate from `resumeTickId`, `previousDueTickId`, and cadence without replaying the paused window; if the cadence boundary is exactly `resumeTickId`, the firing is due immediately and must not be advanced by one interval. | The preserved firing cadence must remain derivable from durable schedule metadata and the documented resume rule; scope/epoch fences must retain the dropped old due-point evidence. |
...
For `intervalTicks > 0`, and only after the preserved timer's `runtimeRegionId`, runtime scope (`gameInstanceId` and `playableStateScope`), and `regionEpoch` match the resumed runtime, let `remainder = (resumeTickId - previousDueTickId) % intervalTicks`:
...
This formula advances to the first valid cadence boundary at or after resume without replaying the paused window. A modulo-zero result is the boundary case, not a reason to skip to the following interval. If runtime region, scope, or epoch changes, the old due point is fenced and this formula is not applied; the new schedule timeline derives its own due point.
