...
| Explicitly preserved interval across reload/rollback | Only a compatible source-and-target continuity declaration may preserve the logical interval. Recalculate the next due point from the canonical resume formula using `resumeTickId`, `previousDueTickId`, and cadence; do not replay the paused window unless the next valid cadence boundary lands exactly on resume. | Record the stable logical key, source/target versions, prior and resulting due points, and reconciliation outcome. Without valid opt-in, tombstone and recreate with fresh due state. |
