...
Before handler resolution, a scheduler/timer firing claim is keyed by the complete durable schedule-instance identity plus the applicable runtime scope, target selector, trigger mode, and tagged due point. It therefore includes schedule-owned `scriptId` and any schedule-owned `pluginId`, `pluginVersionId`, and `bindingId`; it must not fabricate fields absent from the durable schedule. This owner-known firing identity is distinct from the ordinary producer event-ingress key below and derives the one event-scope `scriptEventId` propagated to resolved handlers.
