...
| Recurring/advisory `PAUSED_FOR_ROLLBACK` | Do not admit new timer firings while rollback cleanup and repin complete; this Automation admission barrier does not pause ordinary gameplay ticks. | Record the non-admitted due candidate in the event-scope candidate-audit surface keyed by deterministic `scheduleCandidateId` with `finalStage=ADMISSION`, `finalOutcome=rollback_paused`, and `finalReason=rollback_pause`; this is not an ingress-audit record. Do not create a firing claim, handler Trigger Identity, or `scriptEventId`. |
