...
| Recurring/advisory normal operation | Admit at most one logical firing per due candidate under budgets and quotas. Before `EVALUATED_COMMITTED`, a physical evaluation attempt may retry only under the same Trigger Identity and must converge on that firing; after the boundary, recovery replays its durable descriptors without re-entering the DSL. | An admitted firing receives one `script_event_audit` row per handler-scoped Trigger Identity with stage-aware outcomes. |
