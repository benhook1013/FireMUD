...
| `automation_script_work_item_outcomes_total` | `stage`, `outcome`, `priorityTag`, `sourceService` | `scriptEventId` | Increments exactly once for each terminal durable non-dry-run work-item execution outcome (`isDryRun=false`) using the `script_event_audit` stage/outcome vocabulary. Dry-run/test executions never increment this family; use the dedicated test metric families instead. |
...
| `automation_script_errors_total` | `scope`, `script_category`, `reason`, optional `plugin_family` | `scriptEventId` | Counts higher-level classified script errors, including downstream failures. |
