...
| Recurring/advisory `PAUSED_FOR_ROLLBACK` | Do not admit new timer firings while the exact pin/artifact converges and schedules reconcile; displaced-work cleanup is asynchronous/diagnostic and does not gate resumption. This Automation admission barrier does not pause ordinary gameplay ticks. | Record the non-admitted due candidate in the event-scope candidate-audit surface keyed by deterministic `scheduleCandidateId` with `finalStage=ADMISSION`, `finalOutcome=rollback_paused`, and `finalReason=rollback_pause`; this is not an ingress-audit record. Do not create a firing claim, handler Trigger Identity, or `scriptEventId`. |
...
- By default, a version transition tombstones the displaced interval and creates a target interval with fresh due state. Carrying due state requires explicit compatible continuity declarations on both sides plus typed interval/cadence/target/playable-scope checks and the exact pin/epoch transition identity. A same-version repin is still a new epoch transition and must retire or rewrite old-epoch schedule rows, due candidates, firing claims, and current list/read state before admission resumes. `scheduleSemanticsHash` is diagnostic evidence only; equality cannot grant continuity. Replacement creation and retirement are one atomic durable result or a resumable, idempotent operation keyed by `controlPlaneRequestId`.
