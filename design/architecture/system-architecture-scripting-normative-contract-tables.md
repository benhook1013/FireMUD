...
| `automation_script_work_item_outcomes_total` | `stage`, `outcome`, `dryRun`, `priorityTag`, `sourceService` | `scriptEventId` | Increments once for each terminal durable work-item execution outcome using the `script_event_audit` stage/outcome vocabulary. |
