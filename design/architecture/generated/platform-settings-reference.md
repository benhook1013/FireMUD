...
| `firemud.presentation.prompt.coalesce-window-ms` | `prompts.coalescing` | Maximum prompt burst window, in milliseconds, used to suppress prompt spam while still retaining prompts for explicit boundary commands such as LOOK. | `150` | integer from 1 to 1000 ms; platform hard bound applies to operator defaults and tenant/game overrides | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `150` |
