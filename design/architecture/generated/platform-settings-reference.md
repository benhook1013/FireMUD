...
| `firemud.presentation.prompt.emit-after-reconnect-restore` | `prompts.transportPresentation` | Whether reconnect restore permits a fresh prompt after rendering bounded semantic reconnect context and a fresh LOOK; firemud.presentation.prompt.enabled must also be true. | `true` | `true`, `false` | `operator-only` | `tenant/game-configurable` | `no` | `no` | `true` |
