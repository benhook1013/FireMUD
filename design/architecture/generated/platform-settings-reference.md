...
| `firemud.presentation.prompt.emit-after-reconnect-restore` | `prompts.transportPresentation` | Whether reconnect restore appends a fresh prompt after rendering bounded semantic reconnect context and a fresh LOOK. | `true` | `true`, `false` | `operator-only` | `tenant/game-configurable` | `no` | `no` | `true` |
...
| `firemud.reconnection.buffer.ttl-ms` | `reconnection.buffer` | TTL, in milliseconds, for bounded semantic reconnect context. The current runtime retains each context under `{tenantId, gameInstanceId, characterId}`; the canonical target scope is `{tenantId, playableStateNamespaceId, characterId}` pending migration. Retention does not grant resume or replay authority. | `1800000` | integer >= 0 ms; 0 retains bounded semantic reconnect context until size eviction; retention never grants resume or replay authority | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `1800000` |
| `firemud.reconnection.buffer.max-entries` | `reconnection.buffer` | Maximum complete entries retained in one durable bounded semantic reconnect context before oldest-entry eviction; retention does not grant resume or replay authority. | `256` | integer >= 1 complete persisted entries | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `256` |
...
| `firemud.reconnection.buffer.hard-max-bytes` | `reconnection.buffer` | Target guarantee: an absolute byte ceiling for each complete scope-bound persisted envelope; an oversized entry must be omitted or represented by a bounded marker and never partially truncated. Current runtime limitation: it may retain one oversized entry and lacks omission/marker enforcement and proof. | `65536` | integer >= 1 bytes; absolute bound for the complete scope-bound persisted envelope; oversized entries must be omitted or represented by a bounded marker and never partially truncated; effective hardMaxBytes >= softMaxBytes; current runtime oversized-single-entry enforcement remains an implementation/proof gap | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `65536` |
