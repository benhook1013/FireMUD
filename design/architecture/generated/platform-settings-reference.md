...
| `firemud.reconnection.buffer.ttl-ms` | `reconnection.buffer` | TTL, in milliseconds, for the bounded reconnect transcript screen buffer stored for each player binding. | `1800000` | integer >= 0 ms; 0 retains the bounded transcript until size eviction | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `1800000` |
| `firemud.reconnection.buffer.max-entries` | `reconnection.buffer` | Maximum complete entries retained in one durable reconnect transcript before oldest-entry eviction. | `256` | integer >= 1 complete entries | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `256` |
| `firemud.reconnection.buffer.min-messages` | `reconnection.buffer` | Minimum message floor retained for reconnect transcript replay before byte ceilings are applied. | `8` | integer >= 1 | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `8` |
| `firemud.reconnection.buffer.min-lines` | `reconnection.buffer` | Minimum line floor retained for reconnect transcript replay before byte ceilings are applied. | `24` | integer >= 1 | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `24` |
| `firemud.reconnection.buffer.soft-max-bytes` | `reconnection.buffer` | Soft byte ceiling for reconnect transcript retention before older entries are trimmed when message and line floors have been satisfied. | `16384` | integer >= 1 bytes; effective softMaxBytes <= hardMaxBytes | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `16384` |
| `firemud.reconnection.buffer.hard-max-bytes` | `reconnection.buffer` | Hard byte ceiling for reconnect transcript retention; oldest entries are trimmed to stay within this bound for multi-entry windows, while a complete single entry may exceed it. | `65536` | integer >= 1 bytes; effective hardMaxBytes >= softMaxBytes | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `65536` |
