...
| `firemud.reconnection.buffer.ttl-ms` | `reconnection.buffer` | TTL, in milliseconds, for bounded semantic reconnect context retained for each canonical playable-character scope `{tenantId, playableStateNamespaceId, characterId}`; retention does not grant resume or replay authority. | `1800000` | integer >= 0 ms; 0 retains bounded semantic reconnect context until size eviction; retention never grants resume or replay authority | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `1800000` |
...
| `firemud.reconnection.buffer.hard-max-bytes` | `reconnection.buffer` | Target hard byte ceiling for bounded semantic reconnect-context retention; it applies to each complete scope-bound persisted envelope and never permits an oversized entry to bypass the bound. Current runtime retains a single oversized entry, so omission/marker enforcement and proof remain implementation gaps. | `65536` | integer >= 1 bytes; absolute bound for the complete scope-bound persisted envelope; oversized entries must be omitted or represented by a bounded marker and never partially truncated; effective hardMaxBytes >= softMaxBytes; current runtime oversized-single-entry enforcement remains an implementation/proof gap | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `65536` |
