...
| `firemud.reconnection.buffer.max-entries` | `reconnection.buffer` | Maximum complete replay-eligible PlayerOutput entries retained in one durable bounded semantic reconnect context before oldest-entry eviction; prompts and non-replay status/notice/error output are outside the target context and byte accounting. Retention does not grant resume or replay authority. | `256` | integer >= 1 complete persisted entries | `tenant/game-configurable` | `tenant/game-configurable` | `no` | `yes` | `256` |
