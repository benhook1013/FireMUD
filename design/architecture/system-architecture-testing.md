...
Unless a more specific runbook defines another canonical evidence path for the same event, store first-live and reopen evidence under `design/operations/deployments/hobby-self-hosted/traffic-open/<deployment-ref>/<deployment-event-id>.json`. Restore-hardening verification evidence belongs under `design/operations/deployments/hobby-self-hosted/recovery/<recovery-ref>.json` and is linked from the event-bound traffic-open record; it is not an alternate traffic-open record.
