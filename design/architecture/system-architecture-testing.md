...
[Validation and Runtime Proof](../developer-workflows/validation-and-runtime-proof.md#runtime-and-smoke-changes) owns the maintained smoke invocations and evidence-reporting procedure. At this architecture boundary, every live hosted-assurance run whose authoritative `exposedPublicPlayerPaths` includes Telnet requires TLS and an explicit server hostname; a custom CA file is optional only when the certificate chains to the runner's system trust roots. Plaintext is reserved for simulation or separate non-public local smoke tooling, and the runner does not infer plaintext from a localhost target.
