...
### Smoke and reset isolation boundary

Smoke has two distinct modes. The baseline `LOGIN -> PLAY -> LOOK` journey is read-only with respect to ordinary playable state; its expected login, session, presence, and transport effects remain subject to the normal authentication, admission, and session contracts. An advertised player-flow canary remains subject to [ADR 0161](./decisions/adr-0161-profile-aware-isolated-synthetic-player-flow-canaries.md), including its Account-owned restricted synthetic identity and transport isolation, even though its representative command is read-only. Read-only smoke does not authorize a playable-state mutation, a broad reset, or a restore, and an existing smoke result cannot be reused as proof that a later mutating run is safe.

An unscoped playable-state mutation or any whole-stack reset is permitted only inside an explicitly disposable deployment boundary owned by that run. The boundary must be uniquely identified and either be a private per-run Docker Compose deployment or a disposable preview deployment bound to the exact head under test. A whole-stack reset means any reset that can rewrite or wipe state across the stack (including PostgreSQL, Redis, or equivalent shared supporting state); it is allowed only within that run-owned boundary and only while the run's target identity and cleanup outcome remain known. A stable shared Compose stack, shared `dev-demo-cluster`, hobby deployment, staging, or production deployment cannot be made disposable by an operator label or a smoke-script convention.

Mutating smoke on a persistent or shared boundary is a narrower exception and never authorizes a whole-stack reset. It requires an Account-owned restricted synthetic identity, a fresh server-authoritative `playableStateNamespaceId`, the applicable positive state generation and admission/reset fence, and a protected unique run identity. The identity, namespace, generation, and fence are acquired and exact-compared by the authoritative owners; callers must not invent, reuse, or infer them from `gameInstanceId`, a tenant, an account ID, a fixture name, or a local timestamp. When the run is a playtest lifecycle, the applicable `playtestLifecycleId` and `playtestStateGeneration` remain part of that exact tuple.

For the persistent/shared exception, only playable state owned by that namespace may be mutated. Global account identity and credentials, authentication sessions and authority generations, connect-token replay state, global or derived indexes, and irreversible external effects are excluded from the mutation boundary or redirected to explicit test sinks. A disposable run may clear its own run-owned stack during an authorized reset, but it still excludes or redirects irreversible external effects and must preserve the same identity and evidence rules. Synthetic status never bypasses authentication, authorization, abuse controls, moderation, security monitoring, or durable audit. A reset, mutation, or cleanup result that is unknown, partial, stale, contradictory, or unavailable keeps the affected scope fenced: it cannot reopen admission, release the run boundary, or be reused as a successful smoke result until the owning recovery/cleanup contract reconciles it.

Retain only a bounded, redacted run record under the existing smoke, preflight, and observability evidence and retention policies. It records the protected run identity, target deployment/boundary, namespace and scope, outcome/status, cleanup disposition, redacted command transcript, and references to the relevant logs and metrics. Credentials, tokens, secrets, and unredacted sensitive payloads are never retained, and this contract introduces neither a new storage path nor a new fixed TTL. Deleting or tearing down a uniquely run-owned disposable Compose or preview fixture is test-environment disposal, not an invocation of the target-only Coordination Redis destructive reset/maintenance contract; it grants no production/shared Redis reset semantics. Persistent/shared Coordination Redis remains on the current read-only/fenced fallback until the owning reset workflow exists. The detailed reset sequence and recovery fencing remain owned by [Redis Operations](./system-architecture-redis-operations.md#canonical-coordination-reset-sequence) and [Redis Reset and Recovery](./system-architecture-redis-reset-and-recovery.md#current-operator-fallback); this section defines the smoke boundary, not another reset command.

This is an accepted target contract, not current implementation evidence. The existing smoke, fresh-bootstrap, restart, and image-tag scripts, and the optional mutating command examples in the login smoke workflow, do not currently prove run-owned disposable-boundary ownership, Account-owned synthetic identity, fresh server-authoritative namespace/generation/fence, global-effect exclusion, or complete unknown/partial cleanup handling.
