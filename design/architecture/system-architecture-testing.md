...
For `hobby-self-hosted`, equivalent operator-run evidence is acceptable when nightly automation is not practical. External-authority shape and freshness requirements for the deadman and public-path evidence apply only to `independent-required`. If the independent path is omitted, retained preflight records the explicit degraded-detection status while local health and exposed-public-path structural checks remain applicable; `independent-omitted` must not synthesize external deadman or public-path authority. Target-state player-flow canaries remain conditional on the profile advertising that capability; until an Account-owned synthetic identity/isolation verifier is shipped, the current runner downgrades a requested advertised canary to omitted and the validator rejects retained evidence that claims `advertised`, while omission alone does not block player traffic.
Minimum acceptable operator-run evidence for an omitted profile is one retained preflight or smoke record naming the detection posture, one retained result for the locally available public-path checks and, once the advertised-canary verifier exists, player-flow checks, and one incident or deployment note recording who performed verification and when. A profile that claims independent detection must additionally retain current deadman, off-cluster public-path, and paging evidence; independent monitoring does not inherently require player-flow canaries.
...
Use `python3 dev-tools/observability/run-player-experience-smoke.py --external-authority-evidence <authority.json> --queryability-profile <deployment-profile> --queryability-freshness-budget-seconds <seconds> --evidence-out <evidence.json> [--telnet-transport tls --telnet-server-hostname <telnet-host> [--telnet-ca-file <ca.pem>]] [--metrics-out <mirrored.prom>] [--synthetic-identity-evidence <identity.json>]` to generate the journey and mirrored-signal part of hosted-assurance smoke evidence, incorporate actual results from the separately exercised canary alert path, then run `python3 dev-tools/observability/validate-player-experience-smoke-evidence.py <evidence.json>` before attaching the completed result to a traffic-open or recovery record. When Telnet is exposed for a live run, the TLS transport and server hostname flags shown above are required; the CA flag is optional when the certificate chains to the runner's system trust roots. `authority.json` must be the retained profile-aware result from the authoritative external monitor for the deadman and declared public paths, or an explicit `independent-omitted` record with its degraded-detection reason. An advertised player-flow canary additionally requires an implemented authoritative synthetic identity/isolation verifier; until that verifier exists, the runner downgrades the request to `playerFlowCanary=omitted` even when a local `identity.json` contains plausible assertions, and the validator rejects retained evidence that claims `advertised`. The queryability profile and positive freshness budget must be supplied by the selected deployment profile and must match the target environment's expected-bindings declaration; the runner does not invent defaults. The validator uses current UTC by default, or a trusted `--evaluation-time <RFC3339-UTC>` seam for deterministic contract tests. Only `--simulate` may synthesize that authority object. This hosted-assurance runner requires TLS for every live run whose authoritative `exposedPublicPlayerPaths` includes Telnet; plaintext is reserved for simulation or separate non-public local smoke tooling outside this runner. The runner does not infer plaintext from a localhost target.
...
The canonical player-flow baseline is `LOGIN` -> `PLAY` -> `LOOK`. It is read-only with respect to ordinary playable state, although login, session, presence, and transport effects still follow their normal contracts. An advertised player-flow canary remains subject to [ADR 0161](./decisions/adr-0161-profile-aware-isolated-synthetic-player-flow-canaries.md), including its Account-owned restricted synthetic identity and transport isolation, even when its representative command is read-only. The optional item/container/equipment extension is mutating. The current standalone WebSocket and Telnet clients default to the baseline; mutation requires `SMOKE_MUTATION_EXTENSION=true`, `SMOKE_MUTATION_BOUNDARY=run-owned-compose`, and the explicit per-run binding enforced by [`run-owned-compose.sh`](../../dev-tools/smoke/run-owned-compose.sh). Project-name shape alone must never authorize destructive or mutating smoke access.

[Validation and Runtime Proof](../developer-workflows/validation-and-runtime-proof.md#runtime-and-smoke-changes) owns validation-entrypoint selection and evidence reporting for this boundary.

The run-owned Compose contract is explicit and per invocation. In local mode, `FIREMUD_SMOKE_RUN_ID` is required and must match `^[a-z0-9][a-z0-9-]*$`, `COMPOSE_PROJECT_NAME` must equal `firemud-smoke-$FIREMUD_SMOKE_RUN_ID` exactly, and destructive or mutating access also requires a high-entropy `FIREMUD_SMOKE_OWNERSHIP_TOKEN`. The helper stores only that token's digest in an atomic user-private marker and holds a project lock. In GitHub Actions mode (`GITHUB_ACTIONS=true`), nonempty `GITHUB_RUN_ID`, `GITHUB_RUN_ATTEMPT`, and `GITHUB_JOB` are required; the invocation capability is derived from that exact run/attempt/job tuple and the project name must equal `smoke-full-$GITHUB_RUN_ID-$GITHUB_RUN_ATTEMPT` exactly. The helper fails closed when the binding or capability is missing, invalid, or mismatched.

Fresh-bootstrap and image-tag invocations may establish a new claim only when no project-labelled resources exist; an invocation retaining the exact matching capability may reuse its existing claim. Restart and mutation require the matching marker and project resources; unmarked, mismatched, malformed, symlinked, or colliding state fails closed and is never auto-reclaimed. Mutation additionally requires the canonical endpoint to map to exactly one running expected Compose service. Cleanup releases the marker only after all resources for the project are gone. This is an anti-accidental-mis-targeting and replay boundary, not authentication against a hostile Docker-daemon user. The fresh-bootstrap, restart-state, and image-tag wrappers execute both transports in baseline-only mode and reject mutation requests until independent transport identities and playable state exist. A persistent/shared environment may use the target restricted-synthetic identity, namespace, and fence verifier only after that verifier is implemented; it is not a current mutation path. A run-owned Compose project is the only currently supported mutation boundary. Its whole-stack teardown disposes that test deployment and is not authority to reset Coordination Redis; Coordination Redis reset remains governed by the canonical Redis operations owner and its quarantine/fence contract.

The future persistent/shared mutation path is narrower than run-owned Compose and never authorizes a whole-stack reset. A future disposable preview mutation/reset boundary must be uniquely owned by the exact run and head under test; stable/shared Compose, shared `dev-demo-cluster`, hobby, staging, and production deployments cannot become disposable by an operator label or smoke-script convention. It requires an Account-owned restricted synthetic identity, a fresh server-authoritative `playableStateNamespaceId`, the server-derived `playableStateScope`, applicable positive state generation and admission/reset fence, and protected unique run identity, all acquired and exact-compared by the authoritative owners immediately before any mutation. A missing, unavailable, stale, or mismatched namespace/scope binding fails closed; callers must not invent, reuse, or infer these bindings from `gameInstanceId`, tenant, account ID, fixture name, or local timestamp. Where the run is a playtest lifecycle, the applicable `playtestLifecycleId` and `playtestStateGeneration` are part of the exact tuple. Only playable state owned by that namespace and scope may be mutated. Global account identity and credentials, authentication sessions and authority generations, connect-token replay state, global or derived indexes, and irreversible external effects remain outside the boundary or are redirected to explicit test sinks. Run-owned disposable mutation likewise excludes or redirects irreversible external effects, and synthetic status never bypasses authentication, authorization, abuse controls, moderation, security monitoring, or durable audit.

Unknown, partial, stale, contradictory, or unavailable reset, mutation, or cleanup results keep the affected scope fenced: they cannot reopen admission, release the run boundary, or be reused as successful smoke evidence until the owning recovery or cleanup contract reconciles them. For unknown, partial, or unavailable smoke/reset failure evidence, the bounded, redacted run record retains the authoritative `playableStateNamespaceId` and `playableStateScope` when both owner values were obtained; when either owner value was not obtained, it retains a bounded `unavailable` reason naming the missing owner binding instead of a guessed or synthetic value, and the affected boundary remains fenced until owner recovery resolves that missing binding. Evidence remains under the existing smoke, preflight, and observability retention policies and records the protected run identity, target boundary, outcome, cleanup disposition, redacted transcript, and references to relevant logs and metrics. There is no new storage path or fixed TTL; credentials, tokens, secrets, and unredacted sensitive payloads are never retained.
...
In addition to functional, load, and security tests, FireMUD treats observability wiring as part of the system contract. The routed-alert and degraded-diagnostic boundary follows [ADR 0158](./decisions/adr-0158-simplified-observability-degradation-without-fallback-alert-authority.md), and profile-dependent external monitoring follows [ADR 0159](./decisions/adr-0159-profile-dependent-independent-deadman-and-public-path-monitoring.md). A capability advertised by the selected profile is a required proof obligation: missing, stale, unavailable, or unproved producer evidence is blocking/unknown and fails closed. Only a capability the selected profile omits or marks non-applicable may be recorded as unavailable without requiring its target series. A minimal set of checks should validate that critical metrics and alerts are present and correctly labeled:
...
- **Metric presence and labels**
  - After a small synthetic workload in CI (for example a short end-to-end smoke test that exercises login and a few commands), assert that:
    ...
    - Where the selected environment advertises and proves the target tick metric producer, at least one target tick-related metric such as `tick_execution_time_ms_bucket` or `tick_execution_time_ms_p95` appears with its documented bounded labels for a synthetic region. When the selected profile omits or marks that target capability non-applicable, the current live baseline may use `game_session_tick_duration_ms` or record the target family as explicitly unavailable; an advertised-but-unproved producer remains blocking/unknown and is not a passed presence assertion.
    ...
    - Where the selected environment advertises and proves the cadence producer and enables dynamic tail-loss or pause-budget rules, `tick_interval_ms` is exposed for that synthetic region so cadence-derived recordings can evaluate consistently. When the selected profile omits or marks the cadence capability non-applicable, record the cadence family as explicitly unavailable and do not require the series or its target-only recordings; an advertised-but-unproved producer remains blocking/unknown.
    - Where the selected environment advertises and proves the tick-effect outcome producer, `tick_effect_outcome_total` is emitted for at least one synthetic tick effect, with `outcome` values limited to the documented set (for example `first_apply`, `replay_ok`, `guard_error`). When the selected profile omits or marks that capability non-applicable, record it as explicitly unavailable and do not require the series; an advertised-but-unproved producer remains blocking/unknown.
    - If replay-controller instrumentation is advertised and proved, `tick_effects_replay_scan_lag_ms` and `tick_effects_replay_batches_total` appear for the synthetic region. If the selected profile omits or marks replay instrumentation non-applicable, record the replay family as explicitly unavailable rather than requiring target-only series; advertised-but-unproved instrumentation remains blocking/unknown.
    - Where the selected environment advertises and proves a current Redis coordination compatibility producer, expose a basic bounded diagnostic such as `redis_coordination_tail_loss_ms`; it is diagnostic only and never measured-SLO proof. If that producer is unavailable, stale, or unproved, record the compatibility diagnostic as blocking/unknown; only an omitted or explicitly non-applicable capability may be recorded as unavailable. Use the measured pair only when its own producer and proof are present.
    - Where the selected environment advertises and proves backup capability, backup metrics expose freshness, artifact-lineage validity, and restore-readability signals. If backup capability is omitted or non-applicable, record those signals as unavailable; advertised-but-unproved backup evidence remains blocking/unknown. Where the selected environment advertises and proves recovery-controller capability, recovery environments also expose bounded participant safe-disposition/convergence signals; omitted/non-applicable recovery capability is explicitly unavailable and advertised-but-unproved recovery evidence remains blocking/unknown.
...
- Verify the target-state canonical canary alert path can be exercised in non-production without using production paging destinations when the canonical alerts are installed/evaluated:
  - one failed login sample does not page P0; the canonical two-minute canary rule trips `PlayerFlowCanaryLoginFailed` with `severity="P1"`, while a separately proved sustained, fresh, confirmed complete-login policy may promote the incident to P0,
...
- These checks, including retained player-flow evidence, are required only for profiles advertising `playerFlowCanary=advertised` because live-traffic SLIs alone are not sufficient in low-traffic periods. Independent-monitoring profiles still retain their required deadman and public-path evidence even when they do not advertise player-flow canary metrics; independent monitoring alone does not require player-flow canaries.
...
- **Environment assurance (nightly or staging-gated, for profiles that claim it)**:
  ...
  - Prometheus rules conformance smoke: query the Prometheus rules API and, for every observability capability the selected profile advertises, require producer/bounded-label proof and verify that its required recording rules and Alertmanager evaluation inputs are loaded (tail-loss diagnostic, tick safety ratio recording, login success ratio recording, command p99 latency recording, entry-path availability recording, and chat delivery latency recording). Missing or unproved advertised capability evidence is blocking/unknown and fails closed; only omitted or non-applicable target producers may be recorded explicitly unavailable without requiring a recording rule. These rules are not a second routed-alert authority.
    - For an advertised and proved cadence producer, this includes the cadence-derived tail-loss compatibility pair (`redis_coordination_tail_loss_budget_ms`, `redis_coordination_tail_loss_slo_breached`) as diagnostic or Alertmanager-evaluation inputs, not measured-SLO proof. If the selected profile omits or marks cadence non-applicable, these recordings are target-only and unavailable. When implemented, the target measured pair (`redis_unreplicated_write_window_slo_ms`, `redis_unreplicated_write_window_slo_breached{scope}`) is checked separately, alongside both short-window and 1-day entry-path availability recordings when their producers are advertised and proved.
    - For any advertised and proved core-command latency capability, this includes preserving the bounded `service` and `command` labels on its recording rules so service-specific or single-command regressions continue to alert; an advertised-but-unproved capability remains blocking/unknown.
    - For an advertised and proved replay-controller producer, this includes the replay-convergence set (`tick_effects_pending_oldest_age_seconds`, `tick_effects_replay_convergence_budget_seconds`, `tick_effects_replay_slo_breached`, and `tick_effects_replay_starved`) so ledger backlog alerting does not drift into environment-specific guesswork. If the selected profile omits or marks replay non-applicable, these recordings are target-only and unavailable; advertised-but-unproved capability remains blocking/unknown.
    - The Alertmanager evaluation group (`firemud.alerts.observability`) is globally required so new platform-health rules cannot drift out of the shared ruleset silently. For an environment that advertises and proves backup capability, also require the backup diagnostic signals (`backup_pipeline_recent_backup_slo_breached`, `backup_pipeline_recent_verification_slo_breached`, `backup_pipeline_recent_restore_drill_slo_breached`, `backup_artifact_lineage_invalid`, `backup_artifact_restore_unreadable`). For an environment that advertises and proves recovery-controller capability, also require `recovery_participant_convergence_blocked`, `recovery_environment_convergence_blocked`, `recovery_participant_convergence_coverage_missing`, and `recovery_participant_convergence_source_missing`; omitted/non-applicable capability is explicitly unavailable, while advertised-but-unproved capability remains blocking/unknown.
    - For a profile advertising and proving the corresponding tick-state and remote-follow-up producers, this also includes the tick-state projections (`current_tick_state`, `current_tick_terminal_at_ms`) and the aggregate remote follow-up recordings (`remote_followups_due_total`, `remote_followups_drain_lag_ms`, `remote_followups_backlog_over_budget_total`) so the observability contract stays aligned with the Redis and scaling docs without drifting back into forbidden tenant/game-instance/region metric labels. If the selected profile omits or marks those target capabilities non-applicable, record those target families as explicitly unavailable rather than requiring rules that are not installed; advertised-but-unproved producers remain blocking/unknown.
