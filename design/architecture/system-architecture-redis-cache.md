...
The canonical cache policy in this file and the reference catalog in the companion document must stay aligned with the global reset policy matrix in `system-architecture-redis-reset-and-recovery.md`. Any destructive Cache/Rate-Limit reset also requires the external, generation-bound [Cache deployment maintenance gate](./system-architecture-redis-operations.md#external-cache-deployment-maintenance-gate); it is outside Cache/Rate-Limit Redis and must cover every registered writer through deletion, verification, and rebuild.
...
- Coordination workload isolation:
  ...
  - Coordination Redis must not host large, eviction-driven caches under any profile. Even in development and hobby/self-hosted profiles, caches and rate limits are pointed at the separate Cache/Rate-Limit deployment so eviction and OOM behavior cannot silently affect coordination keys. The only supported exception is one explicitly marked, one-shot ephemeral test/CI stack (never hosted `pr-preview` or another player-facing environment) whose tests are reset-tolerant, which surfaces the shared endpoint, and which passes the complete [canonical post-reset verification checklist](./system-architecture-redis-ops-access.md#canonical-post-reset-verification-checklist). That stack may run reset-tolerant tests but is outside and provides no role-isolation, replay, tail-loss, or coordination-SLO evidence. See `system-architecture-redis-usage-and-profiles.md` for environment profiles and mappings.
...
- The **Coordination Redis** deployment:
  ...
  - Does **not** store large cache payloads or unbounded aggregates; those belong in the Cache/Rate-Limit cluster or in PostgreSQL/object storage. There is no general Coordination Redis cache exception: only the explicitly labelled, one-shot ephemeral test/CI topology (never hosted `pr-preview`) may collapse the physical roles. It must be reset-tolerant, surface the shared endpoint, preserve logical prefix, ACL, reset, and memory-policy classification, and pass the complete [canonical post-reset verification checklist](./system-architecture-redis-ops-access.md#canonical-post-reset-verification-checklist). It yields no role-isolation, replay, tail-loss, or coordination-SLO evidence. No unregistered cache family may be placed on Coordination Redis.
...
- Only an explicitly marked, one-shot ephemeral CI/test stack other than hosted `pr-preview` may collapse roles into a single Redis instance. It must be labelled `ephemeral / single-Redis`, use reset-tolerant tests, not exercise coordination tail-loss or replay guarantees, surface the shared endpoint in configuration/metrics, and pass the complete [canonical post-reset verification checklist](./system-architecture-redis-ops-access.md#canonical-post-reset-verification-checklist). It provides no role-isolation, replay, tail-loss, or SLO evidence and must not be used for QA, staging, production, or any player-facing game instance.
