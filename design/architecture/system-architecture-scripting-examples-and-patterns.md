...
2. **`onEnterRegion` event is emitted**
   ...
   - Conceptually this is a unary `TriggerScriptEvent` call on the Automation & Scripting Service that carries:
     - `tenantId`, `gameInstanceId`, `playableStateNamespaceId`, and `regionId`; `playableStateScope` is carried separately as exact-validated policy/routing/authorization/migration-fence evidence, not as an identity field.
...
5. **Automation queue staging**
   - The committed pre-DSL trigger is projected into `automation:queue:{tenantInstanceTag}:<entityId>` only as a rebuildable pointer. Actions produced by the handler are converted into domain commands and persist only as durable evaluated work items in executor/outbox storage; they are never Redis queue-pointer payloads.
   - Firing admission links the durable firing claim, its committed pre-DSL trigger record and rebuildable queue-pointer projection, and due-point advancement in one atomic durable transition or a resumable idempotent recovery operation. Handler resolution and handler-scoped gates happen after that durable trigger admission: each resolved handler retains its audit row, while only admitted handlers create evaluated durable work items. A zero-handler firing creates no handler audit or evaluated work item, and a handler denied by quota or another handler-scoped gate creates no evaluated work item; neither case reclassifies the pre-DSL trigger pointer as post-DSL output. Firing-claim and queue-link recovery completes missing links from the existing claim rather than re-evaluating the handler.
   - Each work item carries the originating `scriptEventId`, `scriptId`, `gameInstanceId`, `playableStateNamespaceId`, exact `scriptPatchVersion`/`scriptPinEpoch`, and region context. It retains `playableStateScope` separately as exact-validated policy/routing/authorization/migration-fence evidence, excluded from Trigger Identity, Command-Handoff Identity, uniqueness, and deduplication keys. In the target handoff contract, each emitted child is keyed by the complete source runtime scope plus any distinct target runtime scope, the persisted `automationDispatchId`, and deterministic `commandOrdinal` from canonical output order; both child identities are reused for every handoff, retry, and downstream disposition. The full parent Trigger Identity (including `bindingId` where applicable), `outboxWorkItemId`, and `scriptEventId` remain correlation only, while bounded `workItemIds[]` are parent-row selectors rather than child identities. The target `ListScriptHandoffEvents` child records are keyed by the complete Command-Handoff Identity, with `(automationDispatchId, commandOrdinal)` retained only as its dispatch-group suffix, and retain the full parent Trigger Identity while the handler audit remains one row. See [Implementation Status](#implementation-status) for the current live handoff boundary.
...
7. **Execution, audit, and observability**
   ...
   - Metrics such as `automation_script_triggers_total`, `automation_script_skips_total`, `automation_script_triggers_dropped_total`, `script_quota_allowed_total`, `script_quota_denied_total`, and `automation_tick_events_enqueued_total` are updated throughout this flow; [normative Table 4](./system-architecture-scripting-normative-contract-tables.md#table-4-metrics-label-matrix) is the sole owner of their names, finite labels, and increment units.
   - An audit record is written to `script_event_audit` for each resolved handler Trigger Identity, with identifiers such as `scriptEventId`, `scriptId`, `tenantId`, `gameInstanceId`, `playableStateNamespaceId`, `regionId`, `regionEpoch`, and `tickId`, plus separately persisted `playableStateScope` policy/routing/authorization/migration-fence evidence and stage-aware outcome fields (`finalStage`, `finalOutcome`, `finalReason`) so operators can distinguish “DSL evaluated” from “accepted into tick queues”, enabling replay and troubleshooting as described in the same quotas and operations document.
...
4. **Sandbox execution and command enqueue**
   ...
   - Actions produced by the handler (for example, “move to the next patrol room,” “play an emote,” “schedule an `onTimerExpire` follow-up”) are converted into domain commands and persisted as a durable script work item in executor/outbox storage. The committed pre-DSL trigger's rebuildable `automation:queue:{tenantInstanceTag}:<entityId>` pointer is projected separately for the affected entity; the evaluated work item is not stored in that Redis index.
...
For carry-forward, match the stable schedule-instance identity and the stable logical continuity key `{stableOwnerKind, stableOwnerId, scheduleDefinitionId, playableStateNamespaceId, targetScopeType, targetScopeId}` within the applicable tenant/namespace scope, then apply the explicit typed compatibility checks. `scheduleSemanticsHash` and displaced/replacement patch or plugin versions are transition-mapping provenance and exact execution-fence evidence, not immutable continuity identity; matching only `scheduleDefinitionId` or semantic similarity is insufficient.
