...
- If patch `P11` and patch `P12` both define the NPC patrol timer as "run every 30 ticks while patrol is enabled" and both explicitly declare the same stable continuity key with compatible cadence, target, and playable scope, Automation preserves the existing stable row, rewrites its target patch/plugin provenance and exact pin/epoch fencing metadata in place, and applies the resume calculation without creating a replacement row or tombstone. The in-place update is atomic or resumable/idempotent; the stable row receives a firing claim and `scriptEventId` only after a due candidate passes admission, while the old owner history is not reused as a new row.
