...
The per-handler outcome mix is canonical, but the two-command child handoff shown below is **target-state only**. The current evaluator accepts exactly one emitted command and rejects larger fan-out before persistence or handoff; see [Scripting & Automation: Cross-Service Contracts](./system-architecture-scripting-contracts.md#implementation-status).
...
- **Target-state recovery:** Before `EVALUATED_COMMITTED`, a failed physical evaluation attempt may retry under the same full applicable Trigger Identity and must converge on the same work-item and child identities. After that descriptor-commit boundary, recovery replays the durable descriptors and does not re-enter the DSL; independently idempotent downstream operations may still retry. The current runtime has no separate descriptor layer or `EVALUATED_COMMITTED` marker, so unresolved `EVALUATING` work remains fail-closed and active rather than being reclaimed or replayed; see [Current Implementation Status](./system-architecture-scripting-runtime-execution.md#current-implementation-status).
