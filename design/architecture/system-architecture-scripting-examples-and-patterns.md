...
3. **Firing `onInterval` and enforcing budgets**
   - When an interval becomes due, the scheduler first applies event-scope candidate gates (admission, cadence, reload/version visibility, and applicable policy checks); it does not reserve aggregate `AUTOMATION_TICK_BUDGET_MS` capacity before firing or handler binding. Only after event-scope admission does it create a durable firing claim from the stable schedule-row identity plus the applicable runtime region and epoch, target selector identity (`entityId` only for an entity target scope), trigger mode, and tagged due point. It derives `scriptEventId` from that complete firing identity. After handlers are bound, the quota owner records each handler charge; queued work holds no capacity, and a separately fenced capacity lease with its execution-start marker is acquired immediately before DSL execution. The claim, work item, handler audit, queue projection, and canonical due-point advancement are committed together or recovered by one idempotent operation; retries do not re-run the DSL.
