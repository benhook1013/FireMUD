...
5. **Automation queue staging**
   ...
   - The firing admission links the durable firing claim, handler audit row, work item, queue-pointer projection, and due-point advancement in one atomic durable transition or a resumable idempotent recovery operation. Firing-claim and queue-link recovery completes missing links from the existing claim rather than re-evaluating the handler.
