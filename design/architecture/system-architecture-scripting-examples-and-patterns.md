...
3. **Firing `onInterval` and enforcing budgets**
   ...
   - If the script is outside its budgets or disabled, the due candidate is skipped and recorded in both metrics and the audit feed. The skip must atomically consume the candidate by advancing its canonical due point, or quarantine/tombstone the schedule when safe advancement is impossible; it creates no firing claim or `scriptEventId` and cannot be rediscovered on a later scan.
