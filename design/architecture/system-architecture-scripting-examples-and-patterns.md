...
5. **Automation queue staging**
   ...
   - Each work item carries the originating `scriptEventId`, `scriptId`, `gameInstanceId`, `playableStateScope`, version metadata, and region context. It receives one stable `automationDispatchId` when its emitted commands are first persisted, and every logical command receives a deterministic `commandOrdinal` under that dispatch. Both values are reused for every handoff, retry, and downstream disposition. Automation records each attempted command in a separate `ListScriptHandoffEvents` child keyed by `(automationDispatchId, commandOrdinal)`, while retaining one handler audit row. The complete command-level handoff identity remains target-state until Game Session carries `commandOrdinal` and the full Trigger Identity end to end.
...
- The first handler is admitted, evaluates successfully, and reaches `finalStage=TICK_HANDOFF`, `finalOutcome=success` after all of its commands are accepted; each logical command is identified separately by one persisted `automationDispatchId`, which is reused if that handoff is retried.
...
3. **Firing `onInterval` and enforcing budgets**
   - When an interval becomes due, the scheduler first evaluates the candidate through the applicable admission, quota, cadence, aggregate `AUTOMATION_TICK_BUDGET_MS` reservation, and budgeting layers. Only after the candidate passes admission does it create a durable firing claim from the stable schedule-row identity plus the applicable runtime region and epoch, target selector identity (`entityId` only for an entity target scope), trigger mode, and tagged due point. It derives `scriptEventId` from that complete firing identity. The claim, work item, handler audit, queue projection, and canonical due-point advancement are committed together or recovered by one idempotent operation; retries do not re-run the DSL.
   ...
   - If the script is outside its budgets or disabled, the due candidate is skipped and recorded in both metrics and an event-scope audit record keyed by a deterministic `scheduleCandidateId` derived from the stable schedule-row identity, applicable runtime region and epoch, and tagged due point. The skip must atomically consume the candidate by advancing its canonical due point, or quarantine/tombstone the schedule when safe advancement is impossible; it creates no firing claim, handler Trigger Identity, or `scriptEventId` and cannot be rediscovered on a later scan.
...
4. **Sandbox execution and command enqueue**
   ...
   - Each work item carries the full applicable Trigger Identity, including `scriptEventId`, `scriptId`, `gameInstanceId`, `playableStateScope`, version metadata, and the immutable trigger-created `regionId` and `regionEpoch`. If current entity state is needed to route the work at enqueue time, persist separate fields such as `enqueueRegionId` and `enqueueRegionEpoch`; never overwrite the original trigger fields. Deduplication, fencing, and replay continue to use the original `regionId` and `regionEpoch`. Each logical emitted command receives and persists its `automationDispatchId` once at work-item creation; retries reuse that same ID with the target aggregate identity for downstream effect idempotency.
...
- If patch `P11` and patch `P12` both define the NPC patrol timer as "run every 30 ticks while patrol is enabled" and Game Design emits the same `scheduleDefinitionId`, the same resolved `playableStateScope`, and the same canonical `scheduleSemanticsHash`, Automation & Scripting creates or confirms the `P12` version-owned timer row with the same logical `scheduleDefinitionId`, carries its due state forward through the canonical resume rule, and only then retires the `P11` row. Replacement and retirement are one atomic durable result or a resumable idempotent operation. The new row receives a firing claim and `scriptEventId` only after a due candidate passes admission; the old row and its trigger history are not reused.
- If patch `P12` replaces that patrol timer with a different logical schedule such as "run every 5 ticks while alerted" and the compiled `scheduleDefinitionId` changes, the scheduler creates the new timer before retiring the old row under the same atomic-or-resumable rule. Rollback to `P11` follows the same rule in reverse: carry due state only when `scheduleDefinitionId`, resolved `playableStateScope`, and `scheduleSemanticsHash` all match, and recreate timers with fresh due state when any identity component differs.
