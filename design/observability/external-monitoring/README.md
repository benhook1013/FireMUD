...
This is the target-state contract for hosted production profiles that claim externally verified availability or monitoring-resilient readiness. The repository provides a runtime smoke harness, retained-evidence validation, and a shared mirrored metric vocabulary. It does not ship the authoritative off-cluster monitor or pager, a deployment-owned expected-series inventory, or an Account-owned synthetic identity/isolation verifier. Until those deployment-owned gates exist, the checked-in independent-required published overlay installs only the profile-bound deadman rules; path-specific blackbox zero and absent-mirror alerts remain target-only so a non-exposed path cannot be misclassified. The runner also downgrades a requested advertised player-flow canary to `playerFlowCanary=omitted`, and the validator rejects retained evidence that claims `advertised`. Hobby, single-node, and small profiles may explicitly omit that infrastructure; preflight warns and records their degraded detection posture without blocking player traffic.
...
`prometheusMirrors` is declared as `published` or `omitted`, independently of `playerFlowCanary`, which is `advertised` or `omitted`. For `independent-required`, `prometheusMirrors=published` selects the `independent-required-prometheus-published` overlay and requires the `observability_deadman_stale{profile="independent-required"}` mirror; `prometheusMirrors=omitted` selects the corresponding overlay without that rule and requires the deadman mirror to be absent. Path-specific blackbox zero or absent-mirror alerts are applicable only when `prometheusMirrors=published` (or a documented equivalent mirror contract) and a deployment-owned expected-series configuration identifies the path as exposed; otherwise a non-exposed path is `not_applicable`, and no-series is not an applicable local alert. An `independent-omitted` profile never emits the required-profile deadman mirror or alert. The external deadman requirement remains authoritative and is not weakened by omitting its Prometheus convenience mirror. In the target state, an advertised player-flow canary runs and retains its complete login/command metric family, last-run timestamps, profile-derived freshness budget, and canonical alert-exercise records regardless of Prometheus mirror publication; an omitted canary retains none of those canary-specific records. Until the Account-owned synthetic identity/isolation verifier exists, the current runner downgrades a requested advertised canary to `omitted`, and the validator rejects retained evidence that claims `advertised`. For `independent-omitted`, `detectionBudgetSeconds` is canary timing only and is not external-monitoring authority. `externalAuthority.profile` is not a canary declaration. In particular, `independent-required` still requires the authoritative deadman and every exposed public-path result whether either optional capability is omitted.
...
- `--allow-failure-evidence` is an incident-evidence mode only. Pass it to the smoke runner when its retained authority source has a current red deadman or exposed-path outcome, and to the validator for the resulting artifact. In the **current** runner, a requested advertised canary is downgraded to `omitted`, so no canary-specific zero-valued metrics or `not_exercised` canary-alert records are emitted; fresh red deadman/public-path outcomes may still be retained with strict timestamps, reference provenance, chronology, and structural checks. The **target** advertised-canary implementation may retain zero-valued canary outcomes and explicitly `not_exercised` canary-alert records with complete coverage once its authoritative verifier and alert path exist. Red or unexercised incident records do not authorize readiness; red authority records may omit `pageEvidenceRef` when no page-delivery observation exists;
