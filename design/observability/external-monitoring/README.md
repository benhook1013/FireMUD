...
The independent monitor owns the `observability_deadman_stale` decision: it emits and updates that gauge only after applying the profile timing. The smoke runner never synthesizes or freezes the stale decision. Select `prometheusMirrors=published` only when the environment separately publishes this gauge and installs the matching overlay; the runner's `observability_deadman_heartbeat_timestamp_seconds` is a diagnostic timestamp mirror and does not derive the stale decision.
...
- `--allow-failure-evidence` is an incident-evidence mode only. It may retain fresh red external deadman/path outcomes and zero-valued mirrored canary/path outcomes (with complete record coverage), while keeping timestamps, reference provenance, and structural checks strict. Red incident records do not authorize readiness and may omit `pageEvidenceRef` when no page-delivery observation exists;
