...
- Current repository validation is static: `dev-tools/observability/validate-observability-contract.py` checks dashboard/snippet/saved-object/reference-rule consistency, and CI also runs the metric-cardinality guardrail. It does not prove that the authoritative external pager, mirrored canary/deadman metrics, Jaeger queries, or Kibana log-query path work in a live environment.
