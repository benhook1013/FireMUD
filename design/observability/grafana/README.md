...
- [redis-coordination-health.json](./redis-coordination-health.json) – Redis and coordination health dashboard, including the bounded compatibility tail-loss diagnostic (not measured-SLO authority; the dynamic-budget comparison is target-only and not currently displayed), AOF size, coordination memory and key counts, and per-prefix coordination metrics.
