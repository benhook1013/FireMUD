...
- [backups.json](./backups.json) – Backup pipeline dashboard with “time since last backup/verification”, restore-drill freshness, and maintenance/reset pause, alias-scope, and queue-growth signals.
...
- Entry-path availability panels should be computed from explicit attempts counters (for example `entrypath_connection_attempts_total{scope,path,outcome}` once the target-state SLI producers exist), not from a single failure-mode proxy like “connection limits exceeded”.
