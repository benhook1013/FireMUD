...
- Shared player-experience dashboards should also surface the mirrored synthetic canary signals `playerflow_canary_success{flow,path,target}` and `playerflow_canary_latency_ms{flow,path,target}` so low-traffic outages are visible even when live-traffic SLIs stay flat.
