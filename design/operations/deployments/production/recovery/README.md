...
- The per-scope `SetAutomationAdmissionMode` and `GetAutomationDrainStatus` surfaces now provide a durable exact-scope successful acknowledgement/readback. Any accepted operation must match its request ID, target mode, fingerprint, admission epoch, and outcome; do not treat a successful RPC response, admission mode/epoch, fresh `observedAt`, or zero drain counts alone as proof of recovery containment. Keep the fence in place and escalate when that durable tuple is missing, stale, or contradictory. See the [Scripting & Automation control-plane API owner contract](../../../../architecture/system-architecture-scripting-control-plane-api.md#setautomationadmissionmode) for the Set/Get identity and fingerprint semantics. For current recovery, use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration from the durable PostgreSQL/runtime inventory, and a distinct durable deployment/owner acknowledgement plus authoritative readback that no executor claims or unsafe `rebuildPendingWorkItemIndex` invocations continue. The method remains callable but has no live scheduled caller; a per-scope admission pause alone does not make an explicit rebuild invocation safe. The overall queue recovery/resume path still lacks exact pin-epoch, process-cessation, and rebuild proof, so these reads do not authorize resume or reopen.
