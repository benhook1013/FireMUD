...
- Before relying on Game Session containment, use the `SetAutomationAdmissionMode` and `GetAutomationDrainStatus` controls for each affected `{tenantId, gameInstanceId[, regionId]}` scope. When `regionId` is supplied, both controls address that exact regional admission/work-item boundary; omitted or blank `regionId` selects only the exact empty/unscoped row (`region_id=""`), never an instance-wide aggregate or wildcard. Enumerate the complete authoritative affected scope set from the durable PostgreSQL/runtime inventory, and require a successful `APPLIED` or `ALREADY_APPLIED` outcome plus matching request-result/readback identity for every exact scope. Require a durable deployment/owner acknowledgement that the admission barrier is in effect, plus verified cessation of Automation executor claims and scheduled `ScriptOutboxQueueRebuildJob` invocations. Matching scope, admission epoch/mode, fresh `observedAt`, `activeExecutionCount=0`, and `pendingCancelableWorkItemCount=0` are diagnostic drain evidence only and are not sufficient by themselves. Missing drain state, request-result/readback identity, or outcome may persist while recovery is incomplete; if any is missing or ambiguous, leave the fence in place and require owner acknowledgement. See the [Scripting & Automation control-plane API owner contract](../../../../architecture/system-architecture-scripting-control-plane-api.md#setautomationadmissionmode) for the canonical Set/Get identity and fingerprint semantics. If that per-scope control or readback is unavailable, use deployment-wide Automation containment only with explicit impact approval and authoritative readback that no executor claims or scheduled rebuild invocations continue.
