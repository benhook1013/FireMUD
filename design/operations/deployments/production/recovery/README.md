...
- Before relying on Game Session containment, use the instance-scoped `SetAutomationAdmissionMode` and `GetAutomationDrainStatus` controls for each affected `{tenantId, gameInstanceId}`. Any `regionId` values are blast-radius and scope-mismatch evidence only; current-live Automation controls remain instance-wide and do not accept a region-qualified mutation selector. Require a durable deployment/owner acknowledgement that the admission barrier is in effect, plus verified cessation of Automation executor claims and scheduled `ScriptOutboxQueueRebuildJob` invocations. Matching scope, admission epoch/mode, fresh `observedAt`, `activeExecutionCount=0`, and `pendingCancelableWorkItemCount=0` are diagnostic drain evidence only and are not sufficient by themselves. If that per-instance control or readback is unavailable, use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration, and authoritative readback that no executor claims or scheduled rebuild invocations continue.
