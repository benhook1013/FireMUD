...
- Before relying on Game Session containment, use the `SetAutomationAdmissionMode` and `GetAutomationDrainStatus` controls for each affected `{tenantId, gameInstanceId[, regionId]}` scope. When `regionId` is supplied, both controls address that exact regional admission/work-item boundary; omitted or blank `regionId` selects only the exact empty/unscoped row (`region_id=""`), never an instance-wide aggregate or wildcard. Require a durable deployment/owner acknowledgement that the admission barrier is in effect, plus verified cessation of Automation executor claims and scheduled `ScriptOutboxQueueRebuildJob` invocations. Matching scope, admission epoch/mode, fresh `observedAt`, `activeExecutionCount=0`, and `pendingCancelableWorkItemCount=0` are diagnostic drain evidence only and are not sufficient by themselves. If that per-scope control or readback is unavailable, use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration, and authoritative readback that no executor claims or scheduled rebuild invocations continue.
