...
- Before relying on Game Session containment, pause Automation for each affected `{tenantId, gameInstanceId[, regionId]}` scope with the shipped `SetAutomationAdmissionMode(mode=PAUSED_FOR_ROLLBACK)` operation and read back `GetAutomationDrainStatus`, requiring matching scope, admission epoch/mode, fresh `observedAt`, `activeExecutionCount=0`, and `pendingCancelableWorkItemCount=0`. If that per-scope control or readback is unavailable, use deployment-wide Automation containment only with explicit impact approval, complete affected-scope enumeration, and authoritative readback that no executor claims or scheduled rebuild invocations continue.
