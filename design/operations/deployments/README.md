...
- `<environment>/deployments/current.json`: mutable current-state index updated only after a successful apply and live-state verification. It points to one exact immutable deployment record by deployment ref, event ID, and record path; attestations never use it as a later “latest event” lookup.
- `staging/recovery/<recovery-ref>.json`: canonical finalized staging recovery projection; when production-origin data is restored, it references the separate immutable `recovery-sanitization-evidence/v1` artifact through `sanitizationEvidenceRef`.
- `production/attestations/<deployment-ref>.json`: production promotion attestation artifacts referenced by overlay PRs.
- `production/release-manifests/<deployment-ref>.json`: release digest manifests keyed by the stable production deployment ref; the final `releaseTag` is stored as a field when a Git tag exists.
...
- `production/traffic-open/<event>-<deployment-ref>/<deploymentEventId>.json`: post-finalization production first-live/reopen projection whose stored `deploymentEventId` and `preflightReportPath` match the canonical preflight event, the production backup binding, an `environment-wide-postgresql` artifact, and finalized `cold_start_restore` recovery-controller lineage. Tenant/region scope and standalone restore-drill timestamps are not traffic-open authority.
...
- `hobby-self-hosted/traffic-open/<deployment-ref>/<deploymentEventId>.json`: hobby traffic-open evidence whose stored `deploymentEventId` and `preflightReportPath` match the canonical hobby preflight event and backup-compliance record.
- `hobby-self-hosted/recovery/<recovery-ref>.json`: hobby restore-hardening evidence records.

Each `deployments/current.json` index uses `schemaVersion=deployment-current-index/v1` and records `environment`, `deploymentRef`, `deploymentEventId`, `deploymentRecordRef`, and `updatedAt`. The referenced immutable record must match the environment, ref, and event ID and carry successful apply and live-state verification. Updating this pointer changes current environment state only; it never retargets or invalidates an existing promotion attestation.
