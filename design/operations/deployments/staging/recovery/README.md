...
- Before relying on Game Session containment, pause Automation for each affected `{tenantId, gameInstanceId[, regionId]}` scope with the shipped `SetAutomationAdmissionMode(mode=PAUSED_FOR_ROLLBACK)` operation and read back `GetAutomationDrainStatus`, requiring the exact matching scope, current `admissionEpoch` and `admissionMode=PAUSED_FOR_ROLLBACK`, fresh `observedAt`, `activeExecutionCount=0`, and `pendingCancelableWorkItemCount=0`. The readback must also carry the complete durable successful Set acknowledgement tuple for that exact request: matching `controlPlaneRequestId`, immutable `requestFingerprint`, target `mode=PAUSED_FOR_ROLLBACK`, bounded successful `outcome` (`APPLIED` or `ALREADY_APPLIED`), and fresh `acknowledgedAt`; a missing/legacy acknowledgement, `NOT_FOUND`, or any other diagnostic/default result is not evidence. These zero-count and read-only drain observations, even with that acknowledgement tuple, are containment evidence only and do not by themselves authorize recovery, resume, or reopen: also require a durable exact-scope owner/deployment acknowledgement that the admission barrier is in effect and verified cessation of Automation executor claims and unsafe `rebuildPendingWorkItemIndex` invocations, following the [Automation drain contract](../../../../architecture/system-architecture-scripting-control-plane-operations.md#getautomationdrainstatus) and [canonical Redis incident fallback](../../../../architecture/system-architecture-redis-incident-runbook.md). The method remains callable but has no live scheduled caller. If the acknowledgement tuple, drain checks, owner/deployment acknowledgement, or cessation proof is unavailable, stale, or contradictory, retain the fence, use the linked broader-containment escalation with explicit impact approval, complete affected-scope enumeration, deployment-wide Automation containment, and authoritative no-claims/no-rebuild readback, or escalate without proceeding. The current-live [Game Session pin contract](../../../../architecture/microservices/game-session-service/api-contracts.md) and Automation drain response do not expose exact `scriptPinEpoch`, so this current fallback cannot establish the target exact-epoch resume proof; do not resume or reopen on current-live drain evidence alone.
- Use only the shipped `PauseTicksForScope` pause and `GetRuntimeOwnershipStatus` status surface for the supported `{tenantId, gameInstanceId}` boundary. `PauseTicksForScope` is instance-wide, rejects a `regionId` request, and therefore is not an exact regional pause or a complete [ADR 0048](../../../../architecture/decisions/adr-0048-durable-idempotent-operator-write-execution.md) owner-write/recovery flow. For any remote/shared Coordination Redis inspection, apply the [owner transport and credential gate](../../../../architecture/system-architecture-redis-ops-access.md#ops-user-vs-application-user), including authenticated TLS peer-certificate and endpoint-hostname/SAN verification; a missing or ambiguous gate leaves the fence in place and requires escalation. Follow the current failover/AOF procedures and escalation path in the [Redis incident runbook](../../../../architecture/system-architecture-redis-incident-runbook.md) and [Redis Operations](../../../../architecture/system-architecture-redis-operations.md); do not use target-state reset or reopen commands.
