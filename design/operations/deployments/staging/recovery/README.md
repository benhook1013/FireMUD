...
- Before relying on Game Session containment, obtain explicit impact approval for deployment-wide Automation containment, complete affected-scope enumeration from the durable PostgreSQL/runtime inventory, stop or scale the Automation deployment so all executor processes cease, and obtain a durable deployment/owner acknowledgement plus authoritative readback that no executor claims or unsafe `rebuildPendingWorkItemIndex` invocations continue. If any approval, complete inventory, containment, acknowledgement, or readback is unavailable, stale, or contradictory, retain the fence and escalate without proceeding. Per-scope `SetAutomationAdmissionMode`/`GetAutomationDrainStatus` pause-and-drain is target state only, not current recovery authorization; see the [Automation drain contract](../../../../architecture/system-architecture-scripting-control-plane-operations.md#getautomationdrainstatus) and [canonical Redis incident fallback](../../../../architecture/system-architecture-redis-incident-runbook.md). Only after the deployment-wide containment evidence is complete, use the shipped [Game Session pin contract](../../../../architecture/microservices/game-session-service/api-contracts.md) pause/status surface; do not resume or reopen on current-live Automation drain evidence alone.
