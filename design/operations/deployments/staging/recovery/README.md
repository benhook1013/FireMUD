...
- Before relying on Game Session containment, pause Automation for each affected `{tenantId, gameInstanceId[, regionId]}` scope with the shipped `SetAutomationAdmissionMode(mode=PAUSED_FOR_ROLLBACK)` operation and read back `GetAutomationDrainStatus`, requiring matching scope, admission epoch/mode, fresh `observedAt`, `activeExecutionCount=0`, and `pendingCancelableWorkItemCount=0`. These zero-count and read-only drain observations are diagnostic only: require a durable exact-scope owner/deployment acknowledgement that the admission barrier is in effect and verified cessation of Automation executor claims and unsafe `rebuildPendingWorkItemIndex` invocations, following the [Automation drain contract](../../../../architecture/system-architecture-scripting-control-plane-operations.md#getautomationdrainstatus) and [canonical Redis incident fallback](../../../../architecture/system-architecture-redis-incident-runbook.md). The method remains callable but has no live scheduled caller. If that acknowledgement or cessation proof is unavailable, stale, or contradictory, retain the fence, use the linked broader-containment escalation with explicit impact approval, complete affected-scope enumeration, deployment-wide Automation containment, and authoritative no-claims/no-rebuild readback, or escalate without proceeding. The current-live [Game Session pin contract](../../../../architecture/microservices/game-session-service/api-contracts.md) and Automation drain response do not expose exact `scriptPinEpoch`, so this fallback cannot establish target exact-epoch resume proof; do not resume or reopen on current-live drain evidence alone.
