...
- The public recovery gates are `continueRecovery(operationId, expectedPhase, maintenanceLockToken, evidenceRef)` followed by `resume(operationId, expectedPhase, scope, maintenanceLockToken, evidenceRef)`; pause/lock and success release remain internal durable phases, not public recovery verbs.
...
2. The authenticated public `resume(operationId, expectedPhase, scope, maintenanceLockToken, evidenceRef)` uses `expectedPhase=AWAITING_RESUME` and records `RESUME_AUTHORIZED` for the exact operation and scope; it does not release the lock or reopen traffic.
