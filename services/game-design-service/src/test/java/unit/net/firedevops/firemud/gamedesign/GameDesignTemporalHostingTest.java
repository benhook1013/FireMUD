...
class GameDesignTemporalHostingTest {
  private final ApplicationContextRunner contextRunner =
      new ApplicationContextRunner()
          ...
          .withPropertyValues(
              "spring.application.name=game-design-service",
  ...
  @Test
  void gameDesignModuleUsesSharedTemporalHostingPattern() {
    contextRunner.run(
        context -> {
          ...
          String taskQueue = taskQueues.forWorkflowFamily("publish");

          assertThat(taskQueue).isEqualTo("firemud:game-design-service:publish");
          assertThat(
                  FiremudWorkflowIds.workflowId("publish", "tenant-1", "publish-request", "req-1"))
              .isEqualTo("publish:tenant-1:publish-request:req-1");
  ...
  static class TestTemporalHostConfiguration {
    ...
    WorkerFactory workerFactory(WorkflowClient workflowClient, Worker worker) {
      ...
      when(workerFactory.newWorker("firemud:game-design-service:publish")).thenReturn(worker);
