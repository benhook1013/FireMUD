...
1. Start the service with `./gradlew :tcp-proxy-service:bootRun` (use `gradlew.bat` on Windows or launch the `dev` profile from your IDE); the task already defaults to `dev` when no profile is provided.
