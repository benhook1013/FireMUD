  ...
  void containsTheFinalAutomationSchemaAsDirectBaselineDdl() throws IOException {
    ...
    assertThat(ingress)
        .contains(
            ...
            "CONSTRAINT ck_script_event_ingress_audit_pin_tuple CHECK ( (script_pin_epoch IS NULL AND game_instance_id IS NULL AND NULLIF(BTRIM(script_pin_control_plane_request_id), '') IS NULL) OR (script_pin_epoch IS NOT NULL AND script_pin_epoch > 0 AND game_instance_id IS NOT NULL AND NULLIF(BTRIM(script_pin_control_plane_request_id), '') IS NOT NULL) )");
    ...
    assertThat(admissionRequestHistory)
        .contains(
            ...
            "CONSTRAINT ck_automation_admission_request_history_fingerprint CHECK ( NULLIF(BTRIM(control_plane_request_id), '') IS NOT NULL AND request_fingerprint ~ '^[0-9a-f]{64}$' )",
