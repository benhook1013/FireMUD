...
class V7__script_pin_epoch_runtime_consumersTest {
  ...
  void constrainsProjectionAndEventPinTuples() throws IOException {
    ...
    String projectionBlock = migration.substring(0, eventAlter);
    String projectionCheck = migration.substring(projectionConstraint, eventAlter);
    String eventCheck = migration.substring(eventAlter);
