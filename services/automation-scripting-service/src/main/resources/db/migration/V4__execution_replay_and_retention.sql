...
CREATE TABLE script_dead_letter_replay_requests (
    ...
    CONSTRAINT ck_script_dead_letter_replay_request_fingerprint CHECK (
        request_fingerprint ~ '^[0-9a-f]{64}$'
    ),
    CONSTRAINT ck_script_dead_letter_replay_request_status CHECK (
        status IN ('RUNNING', 'COMPLETED')
    ),
    CONSTRAINT ck_script_dead_letter_replay_request_counts CHECK (
        replayed_count >= 0 AND rejected_count >= 0
    )
);
...
CREATE TABLE script_dead_letter_replay_results (
    ...
    CONSTRAINT ck_script_dead_letter_replay_result_plugin_fence CHECK (
        (plugin_activation_epoch = 0 AND lifecycle_revision = 0)
        OR (plugin_activation_epoch > 0 AND lifecycle_revision > 0)
    ),
    CONSTRAINT ck_script_dead_letter_replay_result_nonnegative_evidence CHECK (
        script_pin_epoch >= 0 AND failure_generation >= 0
    )
);
