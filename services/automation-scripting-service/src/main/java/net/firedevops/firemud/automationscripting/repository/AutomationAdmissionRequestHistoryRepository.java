...
public class AutomationAdmissionRequestHistoryRepository {
  ...
  /** Inserts immutable result evidence, returning the existing winner on a concurrent retry. */
  public AutomationAdmissionRequestHistory insertOrGet(AutomationAdmissionRequestHistory entity) {
    ...
    return dsl.insertInto(AUTOMATION_ADMISSION_REQUEST_HISTORY)
        ...
        .doUpdate()
        // The conflict target is immutable. This no-op update makes PostgreSQL
        // return the winner atomically without replacing any recorded evidence.
        .set(AUTOMATION_ADMISSION_REQUEST_HISTORY.ID, AUTOMATION_ADMISSION_REQUEST_HISTORY.ID)
