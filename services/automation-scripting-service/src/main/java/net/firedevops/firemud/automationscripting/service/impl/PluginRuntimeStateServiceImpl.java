...
public class PluginRuntimeStateServiceImpl implements PluginRuntimeStateService {
  ...
  public boolean disable(PluginStateCommand command) {
    return transition(command, PluginState.PLUGIN_STATE_DISABLED, "operator_disable");
  }

  @Override
  @Transactional
  public boolean drain(PluginStateCommand command) {
    return transition(command, PluginState.PLUGIN_STATE_DRAINING, "operator_drain");
  }
  ...
  private boolean transition(
    ...
    if (targetState == PluginState.PLUGIN_STATE_DRAINING
        && (!PluginState.PLUGIN_STATE_ENABLED.name().equals(state.getPluginState())
            || state.getPluginActivationEpoch() <= 0
            || state.getLifecycleRevision() <= 0
            || previous.isBlank())) {
      recordFailedDrainRequest(state, requestId, requestFingerprint, now);
      return false;
    }
  ...
  private void recordFailedDrainRequest(
      PluginRuntimeState state, String requestId, String fingerprint, Instant createdAt) {
    if (requestHistoryRepository == null) {
      return;
    }
    PluginRuntimeRequestHistory history = new PluginRuntimeRequestHistory();
    ...
    history.setControlPlaneRequestId(requestId);
    history.setRequestFingerprint(fingerprint);
    history.setPreviousPluginVersionId(normalize(state.getActivePluginVersionId()));
    history.setActivePluginVersionId(normalize(state.getActivePluginVersionId()));
    history.setPluginActivationEpoch(state.getPluginActivationEpoch());
    history.setLifecycleRevision(state.getLifecycleRevision());
    history.setPluginState(state.getPluginState());
