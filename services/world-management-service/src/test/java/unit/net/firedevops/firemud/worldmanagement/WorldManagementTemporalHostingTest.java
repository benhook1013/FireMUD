...
class WorldManagementTemporalHostingTest {
  private final ApplicationContextRunner contextRunner =
      new ApplicationContextRunner()
          ...
          .withPropertyValues(
              "spring.application.name=world-management-service",
  ...
  @Test
  void worldManagementModuleUsesSharedTemporalHostingPattern() {
    contextRunner.run(
        context -> {
          ...
          String taskQueue = taskQueues.forWorkflowFamily("world-lifecycle");

          assertThat(taskQueue).isEqualTo("firemud:world-management-service:world-lifecycle");
          assertThat(
                  FiremudWorkflowIds.workflowId(
                      "world-lifecycle", "tenant-1", "world-instance", "create-request-1"))
              .isEqualTo("world-lifecycle:tenant-1:world-instance:create-request-1");
  ...
  static class TestTemporalHostConfiguration {
    ...
    WorkerFactory workerFactory(WorkflowClient workflowClient, Worker worker) {
      ...
      when(workerFactory.newWorker("firemud:world-management-service:world-lifecycle"))
          .thenReturn(worker);
