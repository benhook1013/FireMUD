...
class AccountPresenceVisibilityPolicyResolverTest {
  ...
  @Test
  void elevatedRoleUsesThePersistedAccountVisibilityPolicy() {
    ...
    assertEquals(
        ...
        resolver.resolve(22L, 123L, GameplayPresenceRole.GOD));
    verify(accountClient).getPresenceVisibilityPolicy(22L, 123L);
  }

  @Test
  void explicitSystemOwnedHiddenStaffPolicyRemainsVisibleToTheResolver() {
    AccountClient accountClient = mock(AccountClient.class);
    when(accountClient.getPresenceVisibilityPolicy(22L, 123L))
    ...
    assertEquals(
        AccountPresenceVisibilityPolicy.HIDDEN_STAFF,
        resolver.resolve(22L, 123L, GameplayPresenceRole.GOD));
  }
}
