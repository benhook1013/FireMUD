  ...
  void containsTheCompleteGameSessionBaselineContract() throws IOException {
    ...
    assertThat(scriptPinOperation)
        .contains(
            ...
            "outcome = 'COMMITTED' AND resulting_script_patch_version IS NOT NULL AND BTRIM(resulting_script_patch_version) <> '' AND resulting_script_pin_epoch IS NOT NULL AND resulting_script_pin_epoch > 0",
            "outcome = 'FAILED' AND ( (resulting_script_patch_version IS NULL AND resulting_script_pin_epoch IS NULL) OR (resulting_script_patch_version IS NOT NULL AND BTRIM(resulting_script_patch_version) <> '' AND resulting_script_pin_epoch IS NOT NULL AND resulting_script_pin_epoch > 0) ))",
